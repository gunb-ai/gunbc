...
#[cfg(not(feature = "text_lookup_work_counter"))]
pub fn take_text_lookup_chars_walked() -> u64 { 0 }
...
pub fn resolution_silent_pick_record_global_bare_lcp_pick(
    ...
    RESOLUTION_SILENT_PICK.with(|t| {
        t.borrow_mut().global_bare_lcp_picks.push(GlobalBareLcpPickSite {
            env_module_path,
            name,
            candidate_count,
            chosen_module_path,
...
pub fn resolution_silent_pick_record_global_bare_lcp_tie(
    ...
    RESOLUTION_SILENT_PICK.with(|t| {
        t.borrow_mut().global_bare_lcp_ties.push(GlobalBareLcpTieSite {
            env_module_path,
            name,
            candidate_count,
        });
...
pub fn resolution_silent_pick_record_fn_parent_first_hit(
    ...
    RESOLUTION_SILENT_PICK.with(|t| {
        t.borrow_mut().fn_parent_first_hits.push(FnParentFirstHitSite {
            env_module_path,
            name,
            parent_match_count,
...
impl<T: Clone> VecCompat<T> for Vec<T> {
    ...
    fn with_capacity(_capacity: usize) -> Self { Vec::new() }
}
...
impl V2Concat for String {
    fn v1_concat(mut self, other: String) -> String { self.push_str(&other); self }
}

impl<T: Clone> V2Concat for Vec<T> {
    fn v1_concat(mut self, other: Vec<T>) -> Vec<T> { self.extend(other); self }
}

pub fn concat<T: V2Concat>(a: T, b: T) -> T { a.v1_concat(b) }
...
pub fn count<T: Clone>(items: Rc<Vec<T>>) -> i64 { items.len() as i64 }
...
pub fn map_insert<K: std::cmp::Eq + std::hash::Hash + Clone, V: Clone>(mut map: HashMap<K, V>, key: K, value: V) -> HashMap<K, V> {
    map.insert(key, value); map
}

pub fn map_merge<K: std::cmp::Eq + std::hash::Hash + Clone, V: Clone>(mut base: HashMap<K, V>, overlay: HashMap<K, V>) -> HashMap<K, V> {
...
pub fn sorted_map_keys<K: Ord + Clone, V>(m: &HashMap<K, V>) -> Vec<K> { let mut keys = map_keys(m); keys.sort(); keys }

pub fn map_is_empty<K, V>(m: &HashMap<K, V>) -> bool { m.is_empty() }
...
pub fn rc_ptr_eq<T>(a: Rc<T>, b: Rc<T>) -> bool { Rc::ptr_eq(&a, &b) }
...
pub fn list_concat<T: Clone>(mut a: Vec<T>, b: Vec<T>) -> Vec<T> { a.extend(b); a }

pub fn list_push<T: Clone>(mut list: Vec<T>, item: T) -> Vec<T> { list.push_back(item); list }
...
impl<T: Clone> V2Concat for Rc<Vec<T>> {
    fn v1_concat(self, other: Rc<Vec<T>>) -> Rc<Vec<T>> { rc_list_concat(self, other) }
}
...
pub fn is_xid_continue(cp: i64) -> bool {
    char::from_u32(cp as u32).map(unicode_ident::is_xid_continue).unwrap_or(false)
}

pub fn is_emoji_ident(cp: i64) -> bool {
    use unicode_properties::emoji::UnicodeEmoji;
    use unicode_properties::emoji::EmojiStatus;
    char::from_u32(cp as u32).map(|c| matches!(c.emoji_status(), EmojiStatus::EmojiPresentation | EmojiStatus::EmojiPresentationAndModifierBase | EmojiStatus::EmojiPresentationAndEmojiComponent | EmojiStatus::EmojiPresentationAndModifierAndEmojiComponent) && !unicode_ident::is_xid_continue(c)).unwrap_or(false)
}
