...
#
# Phase 1 leaf-model verification host runner — python.dag R2a (int algebra inhabitance).
# Authority: src/v4/lens/leaf_model_verification.dag + python_r2a.dag claim wiring.
...
happy = extract("python_r2a_happy_fixture_source")
falsification = extract("python_r2a_falsification_fixture_source")
...
scratch="${RUNNER_TEMP:-/tmp}/v4-leaf-model-python-r2a-${run_suffix}"
rm -rf "$scratch"
mkdir -p "$scratch"
...
[[ "$falsification_status" -ne 0 ]] && grep -qE 'AttributeError.*log2_exact' <<<"$falsification_stderr" && falsification_pass=true

proven=false
[[ "$happy_pass" == true && "$falsification_pass" == true ]] && proven=true

export V4_PYTHON_R2A_HAPPY_STATUS="$happy_status"
export V4_PYTHON_R2A_FALSIFICATION_STATUS="$falsification_status"
export V4_PYTHON_R2A_HAPPY_PASS="$happy_pass"
export V4_PYTHON_R2A_FALSIFICATION_PASS="$falsification_pass"
export V4_PYTHON_R2A_PROVEN="$proven"
export V4_PYTHON_R2A_FALSIFICATION_STDERR="$falsification_stderr"
...
stderr = os.environ.get("V4_PYTHON_R2A_FALSIFICATION_STDERR", "")
print(
    json.dumps(
        {
            "schema": "scripts/v4-leaf-model-python-r2a-verify.sh::host_receipt_v1",
            "claim_id": "PythonR2aIntAlgebraOperations",
            "happy": {
                "python_exit": int(os.environ["V4_PYTHON_R2A_HAPPY_STATUS"]),
                "verdict": "Pass" if os.environ["V4_PYTHON_R2A_HAPPY_PASS"] == "true" else "Fail",
            },
            "falsification": {
                ...
                "verdict": "Pass"
                if os.environ["V4_PYTHON_R2A_FALSIFICATION_PASS"] == "true"
                else "Fail",
            },
            "proven": os.environ["V4_PYTHON_R2A_PROVEN"] == "true",
        },
        indent=2,
    )
)
...
if [[ "$proven" != true ]]; then
  echo "error: leaf-model python R2a verification failed" >&2
  exit 1
fi

echo "leaf-model python R2a verification PROVEN"
