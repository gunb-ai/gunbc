#!/usr/bin/env bash
# Must-pass affected-set → generated TestClaim CI gate.
#
# Each modeled row is a Bool witness run through `gunbc run --claim-run`.
# `--perturb-check` rewrites the witness body to `false` in a temp source-root
# and requires that row to fail.
...
gate_model="src/v4/test/claim/workflow/affected_testgen_ci_runner.dag"
...
expected_count="$(dag_string_data affected_testgen_claim_run_row_count)"
if [[ -z "$expected_count" ]]; then
  echo "error: missing affected_testgen_claim_run_row_count in $gate_model" >&2
  exit 2
fi
...
while IFS= read -r member; do
  ...
  if [[ -z "$row" ]]; then
    echo "error: list member $member missing AffectedTestgenClaimRunRow binding in $gate_model" >&2
    exit 2
  fi
...
if [[ "$row_count" -eq 0 ]]; then
  echo "error: affected_testgen_claim_run_rows has no members in $gate_model" >&2
  exit 2
fi

if [[ "$row_count" -ne "$expected_count" ]]; then
  echo "error: affected-testgen gate projected ${row_count} rows; modeled count is ${expected_count}" >&2
  exit 2
fi

echo "::notice title=affected-testgen::${row_count} discriminating witness(es) passed"
