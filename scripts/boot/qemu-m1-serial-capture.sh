...
# 0x00: cli
sector[0x00] = 0xFA
# 0x01: lgdt [0x7C4C]  — absolute physical address of gdt_desc (boot sector at 0x7C00 + offset 0x4C).
# SeaBIOS uses CS=0x0000:IP=0x7C00 with DS=0x0000, so [disp16] is a direct physical address.
# [0x004C] would hit the IVT (int 13h) and corrupt the GDT; must be [0x7C4C].
sector[0x01:0x06] = bytes([0x0F, 0x01, 0x16, 0x4C, 0x7C])
# 0x06: mov eax, cr0
