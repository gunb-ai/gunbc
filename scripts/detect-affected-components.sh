...
#   workflow_policy=true|false — true if GitHub Actions workflow definitions or the
...
if echo "$changed" | grep -qE '^\.github/workflows/|^scripts/check-workflow-path-regex-inventory\.sh|^scripts/workflow-path-regex-forbidden-substrings\.txt'; then
