...
# scripts/v4-mvp1*, scripts/v4-m1*, and scripts/v4-testclaim-* — v4 CI shell
# gates (MVP-1 e2e, M1 rust emit probe, T-22 TestClaim corpus bridge).
if echo "$changed" | grep -qE '^src/v4/|^fixtures/v4-mvp1/|^scripts/v4-mvp1|^scripts/v4-m1|^scripts/v4-testclaim-|^dsl/std/|^Cargo\.(toml|lock)$'; then
  v4_state="true"
