...
# `complexity.dag` no longer declares `CostLookup` — it imports
# `v3.std.lookup::Lookup` (authority in `src/v3/std/lookup.dag`, outside this
# script's counted surfaces). Tracked 2-variant debt: `cost` + `infer_helpers` only.
TRACKED_ROWS=(
