...
#   shard a, b -> primary src/v4 (all rows src/v4-rooted; gate-entry src/v4).
#   shard c    -> primary dsl    (all rows dsl-rooted). src/v4 is added as a SECOND, read-only
#                 resolve root solely so ci-claim-gate can resolve the gate-entry that holds the
#                 rows-fn (the runner lives under src/v4); no src/v4 row is perturbed in shard c,
...
# because the 20-row monolith full --perturb-check measured 1108s (~18.5m) > 13m. Three smaller
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    "")
      ...
      ;;
    *)
      echo "usage: $0 --shard a|b|c [--perturb-check]" >&2
      exit 2
      ;;
  esac
done

# Root-aligned: a/b are src/v4-only; c is dsl-primary with src/v4 added read-only for the
# gate-entry (see header). Primary root is listed FIRST (ci-claim-gate perturbs the primary).
case "$shard" in
  a|b) source_root_args=(--source-root src/v4) ;;
  c)   source_root_args=(--source-root dsl --source-root src/v4) ;;
  *)
    echo "error: --shard a|b|c is required" >&2
    exit 2
    ;;
esac
