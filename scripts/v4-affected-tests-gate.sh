...
# E-10 execution consumer: structural emit witness (claim-run) then ngspice -b oracle.
# Strict on GITHUB_ACTIONS (fail if ngspice missing); local dev skips when absent.
bash scripts/v4-spice-ngspice-op-gate.sh
