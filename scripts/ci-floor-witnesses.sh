...
run_ci_runner_function() {
  local fn="$1"
  local log="$2"
  set +e
  "$gunbc" run --source-root dsl --entry dsl/gunbc/tools/ci_runner.dag --function "$fn" >"$log" 2>&1
  local ec=$?
  cat "$log"
  return "$ec"
}
...
witness_red_gate3() {
  echo "::group::witness gate-3 RED (ci_runner-emitted run_logged_script — no bash mirror)"
  local tmp_log
  tmp_log="$(mktemp)"
  trap 'rm -f "$tmp_log"' RETURN
  echo "CHECK: exit-status propagation through ci_runner run_logged_script (witness_gate3_logged_script_exit_guard)"
  set +e
  run_ci_runner_function witness_gate3_logged_script_exit_guard "$tmp_log"
  local guard_ec=$?
  set -e
  echo "OBSERVED: witness_gate3_logged_script_exit_guard exited ${guard_ec} (expect non-zero)"
  if [[ "$guard_ec" -eq 0 ]]; then
    echo "error: ci_runner-emitted redirect swallowed gate-3 failure exit code" >&2
    return 1
  fi
  echo "exit-status propagation OK"
  echo "PLANTED: ci-claim-gate --perturb-check via witness_gate3_logged_script_perturb"
  set +e
  run_ci_runner_function witness_gate3_logged_script_perturb "$tmp_log"
  local ec=$?
  set -e
  echo "OBSERVED: witness_gate3_logged_script_perturb exited ${ec} (expect 0 when RED teeth bite)"
  if [[ "$ec" -ne 0 ]]; then
    echo "error: perturb-check failed through ci_runner run_logged_script entry" >&2
    return 1
  fi
  echo "DETECTED RED: perturb-check completed through emitted redirect — every row went RED under perturb and was caught"
