...
if has_unnamed.clone() {
    ...
    None => args.clone(),
    Some(sig) => {
                let arg_map = args.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, arg: Rc<Node>| {
                    let n = arg_name(arg.clone());
if (n.clone() != None) {
                        v2_rt::map_insert(acc.clone(), n.clone().unwrap(), arg.clone())
} else {
                        acc.clone()
}
});
let param_name_set = sig.params.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, param: Rc<Node>| v2_rt::map_insert(acc.clone(), param_node_name(param.clone()), true));
...
pub fn emit_node_type(n: Rc<Node>, target: RenderTarget) -> String {
    emit_node_type_rc(n.clone(), target.clone(), compile_error!("empty_map: value type unresolved"))
}
...
let has_map_template = (target_container_template(target.clone(), "map".to_string()) != None);
...
pub fn service_has_rest(fallback_transport: Rc<Node>, op_children: Rc<Vec<Rc<Node>>>) -> bool {
    {
        let from_fallback = is_rest_transport(fallback_transport.clone());
...
pub fn service_has_file(fallback_transport: Rc<Node>, op_children: Rc<Vec<Rc<Node>>>) -> bool {
    {
        let from_fallback = is_file_transport(fallback_transport.clone());
let from_ops = { let mut __found = false; for op in op_children.clone().iter().cloned() { if if (op.transport.clone() != None) {
