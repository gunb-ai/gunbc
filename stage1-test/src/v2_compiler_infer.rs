...
Rc::new(InferScopeComponents {
    ...
    svc_locals: v2_rt::map_insert(acc.svc_locals.clone(), root.clone(), nominal_type_binding(root.clone())),
})
}
} else {
...
pub fn collect_item_recursive_variant_fields(item: Rc<Node>, recursive_type_set: Rc<HashMap<String, bool>>) -> Rc<HashMap<String, Rc<Vec<Rc<RecursiveVariantFieldWitness>>>>> {
    ...
    Connective::Disj => if (v2_rt::map_get(&recursive_type_set, item.name.clone()) == None) {
        compile_error!("empty_map: value type unresolved")
} else {
        item.children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |variant_acc: _, variant_node: Rc<Node>| variant_node.children.clone().iter().cloned().fold(variant_acc.clone(), |field_acc: _, field_node: Rc<Node>| {
...
if (field_type_name.clone().as_str() == item.name.clone().as_str()) {
    ...
    _ => compile_error!("empty_map: value type unresolved"),
}
}
...
pub fn build_item_recursive_variant_fields(items: Rc<Vec<Rc<Node>>>, recursive_type_set: Rc<HashMap<String, bool>>) -> Rc<HashMap<String, Rc<Vec<Rc<RecursiveVariantFieldWitness>>>>> {
    items.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, item: Rc<Node>| merge_recursive_variant_fields(acc.clone(), collect_item_recursive_variant_fields(item.clone(), recursive_type_set.clone())))
}
...
pub fn build_params_scope(scope: Rc<InferScope>, params: Rc<Vec<Rc<Node>>>) -> Rc<InferScope> {
    {
        let new_locals = params.clone().iter().cloned().fold(scope.locals.clone(), |acc: _, p: Rc<Node>| v2_rt::map_insert(acc.clone(), param_node_name(p.clone()), Rc::new(TypeBinding {
...
let acc_under_resolved = match fold_acc.clone() {
    Some(acc_type) => (node_is_keyed_collection(acc_type.clone()) && ((acc_type.children.clone().len() as i64) < 2)),
    None => true,
};
...
let remaining_results = infer_method_args_with_fold(call_method_name.clone(), call_method_args.clone(), call_fold_info.clone(), call_fold_acc_type.clone(), elem_type.clone(), scope.clone());
...
let if_texpr = make_expr_node(Rc::new(ExprData::ExprIf), Rc::new(vec![cond_typed.clone(), then_typed.clone(), else_typed.clone()]), Some(Rc::new(InferredNode::Resolved {
    node: resolved_type.clone(),
})), span.clone());
Rc::new(InferResult {
    ...
    None => {
                let if_texpr2 = make_expr_node(Rc::new(ExprData::ExprIf), Rc::new(vec![cond_typed.clone(), then_typed.clone()]), Some(Rc::new(InferredNode::Resolved {
    node: unit_type(),
})), span.clone());
Rc::new(InferResult {
...
let val_result = infer_expr(val_expr.clone(), scope.clone(), None);
...
let body_result = infer_expr(body_expr.clone().unwrap(), extended.clone(), None);
let body_typed = body_result.typed.clone();
let body_diags = body_result.diagnostics.clone();
...
let elem_results = Rc::new({ let mut __result = Vec::new(); for e in elements.clone().iter().cloned() { __result.push(infer_expr(e.clone(), scope.clone(), None)); } __result });
...
let idx_texpr = make_expr_node(Rc::new(ExprData::ExprIndex), Rc::new(vec![base_typed.clone(), index_typed.clone()]), idx_inferred.clone(), span.clone());
Rc::new(InferResult {
...
let slc_texpr = make_expr_node(Rc::new(ExprData::ExprSlice), Rc::new(vec![base_typed.clone(), start_typed.clone(), end_typed.clone()]), slc_inferred.clone(), span.clone());
Rc::new(InferResult {
...
let is_some_ctor = ((type_name.clone().unwrap().as_str() == "Some".to_string().as_str()) && (local_variant_parent.clone().as_deref() == expected_optional_parent.clone().as_deref()));
...
let texpr = make_named_expr_node(rl_name.clone(), Rc::new(ExprData::ExprRecordLit {
...
let fn_scope = item.uses.clone().iter().cloned().fold(fn_scope.clone(), |s: _, u: Rc<Node>| extend_scope(s.clone(), resource_use_name(u.clone()), resource_use_resource(u.clone())));
...
let kernel_bindings_base = v2_rt::map_keys(&kernel_type_set()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), Rc::new(TypeBinding {
    name: name.clone(),
...
let kernel_bindings = v2_rt::map_insert(kernel_bindings_base.clone(), "Unit".to_string(), Rc::new(TypeBinding {
    name: "Unit".to_string(),
    resolved: Rc::new(Node {
    name: "Unit".to_string(),
    span: no_span(),
...
let std_types_parent_env = if ((module.module.clone().name.clone().as_str() != "std.types".to_string().as_str()) && (imports_std_types.clone() == false)) {
...
let std_import_bindings = std_types_parent_env.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, env: Rc<TypeEnv>| v2_rt::map_keys(&env.bindings.clone()).iter().cloned().fold(acc.clone(), |bacc: _, name: String| if is_bridge_placeholder(placeholder_names.clone(), name.clone()) {
            bacc.clone()
} else {
            match v2_rt::map_get(&env.bindings.clone(), name.clone()) {
    Some(binding) => v2_rt::map_insert(bacc.clone(), name.clone(), binding.clone()),
    None => bacc.clone(),
}
}));
let import_bindings = module.resolved_imports.clone().iter().cloned().fold(std_import_bindings.clone(), |acc: _, imp: Rc<ResolvedImport>| match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
    Some(typed_parent) => if (imp.module_path.clone().as_str() == "std.types".to_string().as_str()) {
            v2_rt::map_keys(&typed_parent.type_env.clone().bindings.clone()).iter().cloned().fold(acc.clone(), |bacc: _, name: String| if is_bridge_placeholder(placeholder_names.clone(), name.clone()) {
                bacc.clone()
} else {
    ...
    None => bacc.clone(),
            ...
            v2_rt::map_merge(acc.clone(), typed_parent.type_env.clone().bindings.clone())
},
    None => acc.clone(),
});
...
let import_recursive_set = parent_envs.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, env: Rc<TypeEnv>| v2_rt::map_merge(acc.clone(), env.recursive_type_set.clone()));
let import_recursive_variant_fields = parent_envs.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, env: Rc<TypeEnv>| merge_recursive_variant_fields(acc.clone(), env.recursive_variant_fields.clone()));
...
let local_bindings = module_items(module.module.clone()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, item: Rc<Node>| {
...
v2_rt::map_insert(acc.clone(), item.name.clone(), Rc::new(TypeBinding {
    ...
    resolved: bare_node.clone(),
                            ...
                            if ((((((item.properties.clone().len() as i64) > 0) && (item.connective.clone() == Connective::NoConnective)) && (item.transport.clone() == None)) && (item.inferred.clone() == None)) && ((item.params.clone().len() as i64) == 0)) {
                                v2_rt::map_insert(acc.clone(), item.name.clone(), nominal_type_binding(item.name.clone()))
} else {
                                acc.clone()
}
}
...
let param_bindings = module_items(module.module.clone()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, item: Rc<Node>| {
...
if (((item.params.clone().len() as i64) > 0) && is_type_decl.clone()) {
                {
                    let result = item.params.clone().iter().cloned().fold(acc.clone(), |pacc: _, p: Rc<Node>| v2_rt::map_insert(pacc.clone(), p.name.clone(), nominal_type_binding(p.name.clone())));
result.clone()
}
} else {
                acc.clone()
...
let local_name_set = v2_rt::map_keys(&local_bindings).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: compile_error!("unresolved CompilerError type reached emit")| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
let all_deps_map = v2_rt::map_values(&merged.bindings.clone()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, b: Rc<TypeBinding>| v2_rt::map_insert(acc.clone(), b.name.clone(), node_type_deps(b.resolved.clone())));
let cycle_set = detect_type_cycles_kahn(all_deps_map.clone(), merged.bindings.clone());
let cycle_map = cycle_set.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
let zero_span = make_span(0, 0);
let source_index = v2_rt::map_get(&source_indices, module.module.clone().span.clone().file.clone());
let kernel_bindings = v2_rt::map_keys(&kernel_type_set()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), Rc::new(TypeBinding {
    name: name.clone(),
    resolved: Rc::new(Node {
    name: name.clone(),
    span: no_span(),
...
pub fn build_module_context(contributions: Rc<Vec<Rc<ItemContribution>>>, parent_index: Rc<HashMap<String, Rc<TypedModule>>>, resolved_imports: Rc<Vec<Rc<ResolvedImport>>>, env: Rc<TypeEnv>, module_name: String) -> Rc<ModuleContext> {
    {
        let local = fold_module_contributions(contributions.clone(), Rc::new(vec![]), compile_error!("empty_map: value type unresolved"), compile_error!("empty_map: value type unresolved"), compile_error!("empty_map: value type unresolved"), compile_error!("empty_map: value type unresolved"), Rc::new(vec![]));
let imported_enum_names = resolved_imports.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, imp: Rc<ResolvedImport>| imp.specific_names.clone().iter().cloned().fold(acc.clone(), |inner: _, n: String| v2_rt::map_insert(inner.clone(), n.clone(), true)));
let variant_fold = v2_rt::map_values(&env.bindings.clone()).iter().cloned().fold(Rc::new(VariantFoldState {
...
if (curr_is_imported.clone() && prev_is_imported.clone()) {
    ...
    collision_errors: v2_rt::list_push(vacc.collision_errors.clone(), make_error_node(Rc::new(CompilerDiagnostic::VariantCollision {
    ...
    span: no_span(),
                            ...
                            if curr_is_imported.clone() {
                                Rc::new(VariantFoldState {
    locals: v2_rt::map_insert(vacc.locals.clone(), child.name.clone(), Rc::new(TypeBinding {
...
let merged_scope = merge_scope_from_imports(resolved_imports.clone(), parent_index.clone(), env.clone(), compile_error!("empty_map: value type unresolved"), local.svc_registry.clone(), local.svc_locals.clone());
...
pub fn typecheck_module(resolved: Rc<ResolvedModule>, parent_index: Rc<HashMap<String, Rc<TypedModule>>>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<TypecheckModuleResult> {
    {
        let env_result = build_type_env(resolved.clone(), parent_index.clone(), source_indices.clone());
...
pub fn resolve_env_bindings(env: Rc<TypeEnv>, module_name: String, local_names: Rc<HashMap<String, bool>>, deps_map: Rc<HashMap<String, Rc<Vec<String>>>>) -> Rc<EnvResolveResult> {
    {
        let remaining = Rc::new({ let mut __result = Vec::new(); for b in Rc::new({ let mut __result = Vec::new(); for b in v2_rt::map_values(&env.bindings.clone()).iter().cloned() { if emit_map_has(local_names.clone(), b.name.clone()) { __result.push(b); } } __result }).iter().cloned() { __result.push(b.name.clone()); } __result });
...
let remaining_set = remaining.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
let ready_set = ready.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
pub fn build_fielded_variants(modules: Rc<Vec<Rc<TypedModule>>>, type_summaries: Rc<HashMap<String, Rc<TypeSummary>>>) -> Rc<HashMap<String, bool>> {
    {
        let result = modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<TypedModule>| {
...
if has_fields.clone() {
                                v2_rt::map_insert(vacc.clone(), v2_rt::concat(v2_rt::concat(enum_name.clone(), "::".to_string()), variant.name.clone()), true)
} else {
                                vacc.clone()
}
})
...
pub fn build_value_contexts(modules: Rc<Vec<Rc<TypedModule>>>) -> Rc<HashMap<String, Rc<ValueContext>>> {
    modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<TypedModule>| m.items.clone().iter().cloned().fold(acc.clone(), |inner: _, item: Rc<Node>| {
...
v2_rt::map_insert(inner.clone(), item.name.clone(), Rc::new(ValueContext {
...
let all_recursive = modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<TypedModule>| v2_rt::map_merge(acc.clone(), m.type_env.clone().recursive_type_set.clone()));
...
pub fn typecheck(graph: Rc<ModuleGraph>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<TypedGraph> {
    typecheck_modules(graph.modules.clone(), Rc::new(vec![]), compile_error!("empty_map: value type unresolved"), compile_error!("empty_map: value type unresolved"), Rc::new(vec![]), source_indices.clone())
}
...
pub fn reconcile(graph: Rc<ModuleGraph>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<ResolvedGraph> {
    {
        let typed = typecheck(graph.clone(), source_indices.clone());
