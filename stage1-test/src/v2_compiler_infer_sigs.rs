...
let ready = Rc::new({ let mut __result = Vec::new(); for fn_name in remaining.clone().iter().cloned() { if {
            let local_callees = Rc::new({ let mut __result = Vec::new(); for c in Rc::new({ let mut __result = Vec::new(); for e in Rc::new({ let mut __result = Vec::new(); for e in call_edges.clone().iter().cloned() { if (e.caller.clone().as_str() == fn_name.clone().as_str()) { __result.push(e); } } __result }).iter().cloned() { __result.push(e.callee.clone()); } __result }).iter().cloned() { if emit_map_has(local_func_set.clone(), c.clone()) { __result.push(c); } } __result });
...
if ((ready.clone().len() as i64) == 0) {
    ...
    diagnostics: v2_rt::list_push(acc.diagnostics.clone(), make_error_node(Rc::new(CompilerDiagnostic::MissingAnnotation {
...
let ready_set = ready.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, fn_name: String| v2_rt::map_insert(acc.clone(), fn_name.clone(), true));
...
let local_func_set = local_func_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, fn_name: String| v2_rt::map_insert(acc.clone(), fn_name.clone(), true));
let call_edges = collect_func_call_edges(items.clone(), local_func_set.clone());
let parent_resolved = v2_rt::map_values(&declared_sigs).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, dsig: Rc<DeclaredFuncSig>| if emit_map_has(local_func_set.clone(), dsig.name.clone()) {
            acc.clone()
} else {
