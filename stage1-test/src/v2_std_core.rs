...
pub use crate::std_types::{FilePath, NonEmptyStr, SourceSpan, kernel_type_set, is_kernel_type, container_type_set, is_container_type};
...
pub fn resource_use_name_at(n: Rc<Node>, source_index: Option<Rc<NewlineIndex>>) -> String {
    authored_name_at(source_index.clone(), n.clone())
}
...
pub fn make_variant_node(name: String, fields: Rc<Vec<Rc<Node>>>, span: Rc<SourceSpan>) -> Rc<Node> {
    Rc::new(Node {
    name: name.clone(),
    span: span.clone(),
    ident_span: default_ident_span(name.clone(), span.clone()),
...
pub fn wrapper_child_roles() -> Rc<HashMap<String, Rc<Vec<Rc<ChildRole>>>>> {
    serde_json::from_value(serde_json::json!({"Arg": [{"name": "value", "accessor": "arg_value", "position": 0, "required": true}], "Arm": [{"name": "guard", "accessor": "arm_guard", "position": 0, "required": false}, {"name": "body", "accessor": "arm_body", "position": <<<UNSUPPORTED_MOCK_EXPR>>>, "required": true}], "FieldInit": [{"name": "value", "accessor": "field_init_node_value", "position": 0, "required": true}]}))
        .expect("valid data definition")
}

pub fn is_child_accessor_in_model(name: String) -> bool {
    ({ let mut __found = false; for roles in v2_rt::map_values(&expr_child_roles()).iter().cloned() { if { let mut __found = false; for r in roles.clone().iter().cloned() { if (r.accessor.clone().as_str() == name.clone().as_str()) { __found = true; break; } } __found } { __found = true; break; } } __found } || { let mut __found = false; for roles in v2_rt::map_values(&wrapper_child_roles()).iter().cloned() { if { let mut __found = false; for r in roles.clone().iter().cloned() { if (r.accessor.clone().as_str() == name.clone().as_str()) { __found = true; break; } } __found } { __found = true; break; } } __found })
}
...
pub fn expr_var_name_at(texpr: Rc<Node>, source_index: Option<Rc<NewlineIndex>>) -> String {
    authored_name_at(source_index.clone(), texpr.clone())
}
...
pub fn find_property(props: Rc<Vec<Rc<Node>>>, prop_name: String) -> Option<Rc<Node>> {
    match Rc::new({ let mut __result = Vec::new(); for p in props.clone().iter().cloned() { if (field_init_node_name(p.clone()).as_str() == prop_name.clone().as_str()) { __result.push(p); } } __result }).first().cloned() {
