...
pub fn variant_has_fields(emit_info: Rc<EmitGraphInfo>, enum_name: String, variant_name: String) -> bool {
    {
        let key = v2_rt::concat(v2_rt::concat(enum_name.clone(), "::".to_string()), variant_name.clone());
...
pub fn derive_variant_to_enum(type_summaries: Rc<HashMap<String, Rc<TypeSummary>>>) -> Rc<HashMap<String, String>> {
    v2_rt::map_values(&type_summaries).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, summary: Rc<TypeSummary>| match (*summary.repr.clone()).clone() {
    TypeRepr::EnumRepr { .. } => v2_rt::map_keys(&summary.variant_name_set.clone()).iter().cloned().fold(acc.clone(), |inner: _, vn: String| match v2_rt::map_get(&inner, vn.clone()) {
    Some(_) => inner.clone(),
...
pub fn is_known_variant(type_summaries: Rc<HashMap<String, Rc<TypeSummary>>>, name: String) -> bool {
    { let mut __found = false; for summary in v2_rt::map_values(&type_summaries).iter().cloned() { if match (*summary.repr.clone()).clone() {
...
children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, child: Rc<Node>| if (child.inferred.clone() == None) {
            acc.clone()
} else {
            {
...
let shared = Rc::new({ let mut __result = Vec::new(); for field_name in first_field_names.clone().iter().cloned() { if enum_field_present_in_all_variants(variants.clone(), field_name.clone()) { __result.push(field_name); } } __result });
let consistent = Rc::new({ let mut __result = Vec::new(); for field_name in shared.clone().iter().cloned() { if match find_first_enum_field_node(variants.clone(), field_name.clone()) {
...
consistent.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, field_name: String| match find_first_enum_field_node(variants.clone(), field_name.clone()) {
...
pub fn build_field_type_map(children: Rc<Vec<Rc<Node>>>) -> Rc<HashMap<String, String>> {
    children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, child: Rc<Node>| match child.inferred.clone().as_deref().cloned() {
...
Some(Rc::new(TypeSummary {
    ...
    variant_name_set: item.children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, child: Rc<Node>| v2_rt::map_insert(acc.clone(), child.name.clone(), true)),
    generic_param_names: gpn.clone(),
}))
}
}
