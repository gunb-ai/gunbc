...
let merged = extra.clone().iter().cloned().fold(info.service_names.clone(), |svc_list: _, svc: String| if { let mut __found = false; for s in svc_list.clone().iter().cloned() { if (s.clone().as_str() == svc.clone().as_str()) { __found = true; break; } } __found } {
                            svc_list.clone()
} else {
