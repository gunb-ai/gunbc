...
let left_second = match Rc::new(left.children.clone().iter().cloned().skip(1 as usize).collect::<Vec<_>>()).first().cloned() {
    Some(v) => v.clone(),
    None => left.clone(),
};
let right_second = match Rc::new(right.children.clone().iter().cloned().skip(1 as usize).collect::<Vec<_>>()).first().cloned() {
    Some(v) => v.clone(),
    None => right.clone(),
};
...
pub fn collect_named_templates(template: Rc<AlgebraTypeTemplate>, acc: Rc<HashMap<String, bool>>) -> Rc<HashMap<String, bool>> {
    ...
    AlgebraTypeTemplate::NamedTemplate { name: n, .. } => v2_rt::map_insert(acc.clone(), n.clone(), true),
...
pub fn collect_field_template_names(templates: Rc<Vec<Rc<AlgebraFieldTemplate>>>, acc: Rc<HashMap<String, bool>>) -> Rc<HashMap<String, bool>> {
    templates.clone().iter().cloned().fold(acc.clone(), |a: _, t: Rc<AlgebraFieldTemplate>| {
        let a2 = t.param_types.clone().iter().cloned().fold(a.clone(), |pa: _, pt: Rc<AlgebraTypeTemplate>| collect_named_templates(pt.clone(), pa.clone()));
...
pub fn bridge_placeholder_type_names() -> Rc<HashMap<String, bool>> {
    {
        let type_params = v2_rt::map_insert(v2_rt::map_insert(v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), "T".to_string(), true), "K".to_string(), true), "V".to_string(), true);
let n1 = collect_field_template_names(partial_function_templates(), compile_error!("empty_map: value type unresolved"));
...
let n5 = collect_field_template_names(boolean_algebra_templates(), n4.clone());
let n6 = collect_field_template_names(approximate_field_templates(), n5.clone());
let all_template_names = collect_field_template_names(ordered_ring_templates(), n6.clone());
let concrete_types = v2_rt::map_keys(&kernel_algebra_profile()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
let concrete_types = v2_rt::map_insert(v2_rt::map_insert(concrete_types.clone(), "Char".to_string(), true), "Ordering".to_string(), true);
v2_rt::map_keys(&all_template_names).iter().cloned().fold(type_params.clone(), |acc: _, name: String| match v2_rt::map_get(&concrete_types, name.clone()) {
    Some(_) => acc.clone(),
    None => v2_rt::map_insert(acc.clone(), name.clone(), true),
})
}
}
