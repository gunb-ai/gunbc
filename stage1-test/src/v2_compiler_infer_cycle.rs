...
pub fn compute_in_graph_deps(all_names: Rc<Vec<String>>, deps_map: Rc<HashMap<String, Rc<Vec<String>>>>, name_set: Rc<HashMap<String, bool>>) -> Rc<HashMap<String, Rc<Vec<String>>>> {
    {
        let result = all_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| match v2_rt::map_get(&deps_map, name.clone()) {
    Some(deps) => {
...
v2_rt::map_insert(acc.clone(), name.clone(), local.clone())
    ...
    None => v2_rt::map_insert(acc.clone(), name.clone(), Rc::new(vec![])),
});
result.clone()
}
}
...
pub fn build_reverse_adj(all_names: Rc<Vec<String>>, local_deps: Rc<HashMap<String, Rc<Vec<String>>>>) -> Rc<HashMap<String, Rc<Vec<String>>>> {
    all_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| match v2_rt::map_get(&local_deps, name.clone()) {
...
v2_rt::map_insert(inner_acc.clone(), dep.clone(), v2_rt::rc_list_push(existing.clone(), name.clone()))
}),
    None => acc.clone(),
})
}
...
pub fn build_in_degree(all_names: Rc<Vec<String>>, local_deps: Rc<HashMap<String, Rc<Vec<String>>>>) -> Rc<HashMap<String, i64>> {
    all_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| {
...
pub fn detect_type_cycles_kahn(deps_map: Rc<HashMap<String, Rc<Vec<String>>>>, bindings: Rc<HashMap<String, Rc<TypeBinding>>>) -> Rc<Vec<String>> {
    {
        let all_names = Rc::new({ let mut __result = Vec::new(); for b in v2_rt::map_values(&bindings).iter().cloned() { __result.push(b.name.clone()); } __result });
let name_set = all_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, n: String| v2_rt::map_insert(acc.clone(), n.clone(), true));
...
let sr_set = self_refs.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, n: String| v2_rt::map_insert(acc.clone(), n.clone(), true));
