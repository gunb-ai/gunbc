...
pub fn serialize_typed_module(module: Rc<TypedModule>) -> String {
    v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("{\"module\": ".to_string(), serialize_module(module.module.clone())), ", \"items\": ".to_string()), json_list(Rc::new({ let mut __result = Vec::new(); for item in module.items.clone().iter().cloned() { __result.push(serialize_node(item.clone())); } __result }))), ", \"item_registry_keys\": ".to_string()), json_list(Rc::new({ let mut __result = Vec::new(); for k in v2_rt::map_keys(&module.item_registry.clone()).iter().cloned() { __result.push(json_quote(k.clone())); } __result }))), "}".to_string())
}
...
let item_registry_json = Rc::new({ let mut __result = Vec::new(); for k in v2_rt::map_keys(&typed.item_registry.clone()).iter().cloned() { __result.push(json_quote(k.clone())); } __result }).join(&", ".to_string());
...
let source_indices = newline_indices.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, index: Rc<NewlineIndex>| v2_rt::map_insert(acc.clone(), index.file.clone(), index.clone()));
