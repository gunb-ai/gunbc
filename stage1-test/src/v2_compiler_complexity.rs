...
Rc::new(ParserProgressEnv {
    state_aliases: v2_rt::map_insert(env.state_aliases.clone(), name.clone(), state_progress.clone()),
    result_sources: v2_rt::map_insert(env.result_sources.clone(), name.clone(), result_source.clone()),
})
}
}
...
pub fn parser_member_is_always_advancing(entry: Rc<FuncEntry>, parser_name_set: Rc<HashMap<String, bool>>, proven: Rc<HashMap<String, bool>>) -> bool {
    ...
    Some(state_param) => {
        let edges = collect_parser_progress_edges(entry.name.clone(), entry.body.clone(), state_param.clone(), parser_name_set.clone(), empty_parser_progress_env(), proven.clone(), compile_error!("empty_map: value type unresolved"));
({ let mut __all = true; for edge in edges.clone().iter().cloned() { if !((edge.progress.clone() == ProgressKind::ProgressStrict)) { __all = false; break; } } __all } && (parser_success_progress(entry.body.clone(), state_param.clone(), empty_parser_progress_env(), proven.clone(), compile_error!("empty_map: value type unresolved")) == ProgressKind::ProgressStrict))
},
    None => false,
}
}
...
let __tco_2 = parser_name_set.clone();
let __tco_3 = reverse_graph.clone();
let __tco_4 = v2_rt::map_insert(proven.clone(), name.clone(), true);
queue = __tco_0;
func_index = __tco_1;
parser_name_set = __tco_2;
reverse_graph = __tco_3;
...
pub fn infer_parser_always_advancing_members(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>) -> Rc<HashMap<String, bool>> {
    {
        let parser_name_set = members.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
infer_parser_always_advancing_members_worklist(members.clone(), func_index.clone(), parser_name_set.clone(), reverse_graph.clone(), compile_error!("empty_map: value type unresolved"))
}
}

pub fn parser_function_names(func_index: Rc<HashMap<String, Rc<FuncEntry>>>) -> Rc<Vec<String>> {
    Rc::new({ let mut __result = Vec::new(); for name in v2_rt::map_keys(&func_index).iter().cloned() { if match v2_rt::map_get(&func_index, name.clone()) {
...
pub fn self_calls_have_strict_parser_progress(func_name: String, body: Rc<Node>, params: Rc<Vec<Rc<Node>>>, parser_always_advancing: Rc<HashMap<String, bool>>) -> bool {
    ...
    Some(state_param) => {
        let self_set = v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), func_name.clone(), true);
let edges = collect_parser_progress_edges(func_name.clone(), body.clone(), state_param.clone(), self_set.clone(), empty_parser_progress_env(), parser_always_advancing.clone(), compile_error!("empty_map: value type unresolved"));
...
pub fn build_call_graph_from_parser_edges(names: Rc<Vec<String>>, edges: Rc<Vec<Rc<ParserProgressEdge>>>) -> Rc<CallGraph> {
    {
        let initial_forward = names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), Rc::new(vec![])));
let initial_reverse = names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), Rc::new(vec![])));
...
Rc::new(CallGraphAcc {
    forward: v2_rt::map_insert(acc.forward.clone(), edge.caller.clone(), v2_rt::list_push(forward_neighbors.clone(), edge.callee.clone())),
    reverse: v2_rt::map_insert(acc.reverse.clone(), edge.callee.clone(), v2_rt::list_push(reverse_neighbors.clone(), edge.caller.clone())),
})
});
Rc::new(CallGraph {
...
pub fn classify_parser_scc_recursion_pattern(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>) -> Option<Rc<RecursionPattern>> {
    {
        let scc_name_set = members.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
let edges = Rc::new({ let mut __result = Vec::new(); for name in members.clone().iter().cloned() { __result.extend((*match v2_rt::map_get(&func_index, name.clone()) {
    ...
    Some(state_param) => collect_parser_progress_edges(name.clone(), entry.body.clone(), state_param.clone(), scc_name_set.clone(), empty_parser_progress_env(), parser_always_advancing.clone(), compile_error!("empty_map: value type unresolved")),
    None => Rc::new(vec![]),
},
    None => Rc::new(vec![]),
}).iter().cloned()); } __result });
...
let from_children = body.children.clone().iter().cloned().fold(0, |acc: _, child: Rc<Node>| (acc.clone() + count_self_calls(child.clone(), func_name.clone())));
...
let arg_calls = body.children.clone().iter().cloned().fold(0, |acc: _, child: Rc<Node>| {
...
let max_arm = match_arm_nodes(body.clone()).iter().cloned().fold(continue_calls.clone(), |acc: _, arm_node: Rc<Node>| {
...
let arg_calls = method_arg_nodes(body.clone()).iter().cloned().fold(0, |acc: _, arg_node: Rc<Node>| {
...
let from_children = body.children.clone().iter().cloned().fold(0, |acc: _, child: Rc<Node>| (acc.clone() + count_target_calls(child.clone(), target_set.clone())));
(own.clone() + from_children.clone())
}
    })
}
...
let arg_calls = body.children.clone().iter().cloned().fold(0, |acc: _, child: Rc<Node>| (acc.clone() + max_path_target_calls_with_cont(arg_value(child.clone()), target_set.clone(), 0)));
((own.clone() + arg_calls.clone()) + continue_calls.clone())
},
    ExprData::ExprMatch => {
...
pub fn build_scc_measure_params(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>) -> Rc<HashMap<String, Rc<HashMap<String, String>>>> {
    members.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| match v2_rt::map_get(&func_index, name.clone()) {
    Some(entry) => v2_rt::map_insert(acc.clone(), name.clone(), recursive_measure_param_names(entry.body.clone(), entry.params.clone())),
    None => acc.clone(),
})
}
...
if (set_has(target_set.clone(), callee.clone()) == false) {
    ...
    None => compile_error!("empty_map: value type unresolved"),
};
...
pub fn descending_name_set_without_key(names: Rc<HashMap<String, String>>, key: String) -> Rc<HashMap<String, String>> {
    v2_rt::map_keys(&names).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| if (name.clone().as_str() == key.clone().as_str()) {
        acc.clone()
} else {
...
let next_witnesses = match expr_descending_witness_source(let_value(body.clone()), value_witnesses.clone()) {
    Some(source_param) => v2_rt::map_insert(next_witnesses_base.clone(), let_binding_name(body.clone()), source_param.clone()),
...
match v2_rt::map_get(&param_set, name.clone()) {
    Some(_) => v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), name.clone(), name.clone()),
    None => compile_error!("empty_map: value type unresolved"),
}
},
    _ => expr.children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, child: Rc<Node>| v2_rt::map_merge(acc.clone(), condition_param_names(child.clone(), param_set.clone()))),
}
    })
}
...
pub fn recursive_measure_param_names(body: Rc<Node>, params: Rc<Vec<Rc<Node>>>) -> Rc<HashMap<String, String>> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        {
            let param_set = params.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, param: Rc<Node>| v2_rt::map_insert(acc.clone(), param_node_name(param.clone()), param_node_name(param.clone())));
match (*body.expr_data.clone()).clone() {
    ExprData::ExprIf => {
...
v2_rt::map_merge(v2_rt::map_merge(condition_param_names(if_condition(body.clone()), param_set.clone()), then_names.clone()), else_names.clone())
},
    ExprData::ExprMatch => {
                let scrut_names = condition_param_names(match_scrutinee(body.clone()), param_set.clone());
let arm_names = match_arm_nodes(body.clone()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, arm_node: Rc<Node>| v2_rt::map_merge(acc.clone(), recursive_measure_param_names(arm_body(arm_node.clone()), params.clone())));
v2_rt::map_merge(scrut_names.clone(), arm_names.clone())
},
    ExprData::ExprLet => match let_body(body.clone()) {
    Some(b) => recursive_measure_param_names(b.clone(), params.clone()),
    None => compile_error!("empty_map: value type unresolved"),
},
    ExprData::ExprBlock => body.children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, stmt: Rc<Node>| v2_rt::map_merge(acc.clone(), recursive_measure_param_names(stmt.clone(), params.clone()))),
    _ => body.children.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, child: Rc<Node>| v2_rt::map_merge(acc.clone(), recursive_measure_param_names(child.clone(), params.clone()))),
}
}
    })
}
...
let with_patterns = if scrut_is_descent.clone() {
                match_arm_nodes(body.clone()).iter().cloned().fold(vars.clone(), |acc: _, arm_node: Rc<Node>| match (*arm_pattern(arm_node.clone())).clone() {
    MatchPattern::VariantPattern { name: vname, field_bindings: bindings, .. } => if (vname.clone().as_str() == "Some".to_string().as_str()) {
                    bindings.clone().iter().cloned().fold(acc.clone(), |inner: _, fb: Rc<Node>| match (*field_binding_pattern(fb.clone())).clone() {
...
match_arm_nodes(body.clone()).iter().cloned().fold(with_patterns.clone(), |acc: _, arm_node: Rc<Node>| {
...
v2_rt::map_merge(acc.clone(), arm_result.clone())
})
},
    ExprData::ExprIf => {
...
v2_rt::map_merge(then_vars.clone(), else_vars.clone())
},
    ExprData::ExprBlock => body.children.clone().iter().cloned().fold(vars.clone(), |acc: _, stmt: Rc<Node>| collect_descent_vars(stmt.clone(), param_name.clone(), acc.clone(), check_child.clone(), check_list.clone())),
    _ => vars.clone(),
}
    })
}
...
pub fn all_self_calls_descend(body: Rc<Node>, func_name: String, param_name: String, check_child: bool, check_list: bool) -> bool {
    all_self_calls_descend_inc(body.clone(), func_name.clone(), param_name.clone(), compile_error!("empty_map: value type unresolved"), check_child.clone(), check_list.clone())
}
...
let arms_ok = { let mut __all = true; for arm_node in match_arm_nodes(body.clone()).iter().cloned() { if !({
    ...
    MatchPattern::VariantPattern { name: vname, field_bindings: bindings, .. } => if (vname.clone().as_str() == "Some".to_string().as_str()) {
                        bindings.clone().iter().cloned().fold(vars.clone(), |inner: _, fb: Rc<Node>| match (*field_binding_pattern(fb.clone())).clone() {
    MatchPattern::Bind { name: binding_name, .. } => v2_rt::map_insert(inner.clone(), binding_name.clone(), true),
    _ => inner.clone(),
})
} else {
    ...
    MatchPattern::Bind { name: binding_name, .. } => v2_rt::map_insert(vars.clone(), binding_name.clone(), true),
    _ => vars.clone(),
}
} else {
                    vars.clone()
...
((cond_ok.clone() && then_ok.clone()) && else_ok.clone())
    ...
    ok: true,
    vars: vars.clone(),
}), |acc: _, stmt: Rc<Node>| if (acc.ok.clone() == false) {
                acc.clone()
} else {
                {
...
pub fn try_dimension_for_param(body: Rc<Node>, func_name: String, param_name: String, check_child: bool, check_list: bool) -> Option<DescentEvidence> {
    collect_evidence_incremental(body.clone(), func_name.clone(), param_name.clone(), compile_error!("empty_map: value type unresolved"), check_child.clone(), check_list.clone())
}
...
let own_evidence = if (callee.clone().as_str() == func_name.clone().as_str()) {
                {
                    let arg_evidence = body.children.clone().iter().cloned().fold(DescentEvidence::DescentUnknown, |acc: _, arg_node: Rc<Node>| {
...
let local_func_set = names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
Rc::new(CallGraphAcc {
    forward: v2_rt::map_insert(acc.forward.clone(), edge.caller.clone(), v2_rt::concat(forward_neighbors.clone(), Rc::new(vec![edge.callee.clone()]))),
    reverse: v2_rt::map_insert(acc.reverse.clone(), edge.callee.clone(), v2_rt::concat(reverse_neighbors.clone(), Rc::new(vec![edge.caller.clone()]))),
})
});
Rc::new(CallGraph {
    forward: graph_acc.forward.clone(),
...
let own_edges = if set_has(target_set.clone(), callee.clone()) {
                {
                    let progress = body.children.clone().iter().cloned().fold(ProgressKind::ProgressUnknown, |acc: _, arg_node: Rc<Node>| {
                        let aname = arg_name(arg_node.clone());
if (aname.clone().as_deref() == Some(param_name.clone()).as_deref()) {
...
pub fn is_scc_container_child_descent(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>, scc_name_set: Rc<HashMap<String, bool>>) -> bool {
    ...
    Some(entry) => {
            let best_edges = entry.params.clone().iter().cloned().fold(Rc::new(vec![]), |best: _, p: Rc<Node>| {
                let pname = param_node_name(p.clone());
let descent_vars = collect_descent_vars(entry.body.clone(), pname.clone(), compile_error!("empty_map: value type unresolved"), true, true);
...
let own_edges = if set_has(target_set.clone(), callee.clone()) {
                {
                    let best_evidence = body.children.clone().iter().cloned().fold(Rc::new(vec![DescentEvidence::DescentUnknown, DescentEvidence::DescentUnknown]), |best: _, arg_node: Rc<Node>| {
...
pub fn collect_scc_proof_edges_for_dim(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>, scc_name_set: Rc<HashMap<String, bool>>, check_child: bool, check_list: bool) -> Rc<Vec<Rc<ProofEdge>>> {
    ...
    Some(entry) => {
        let best_edges = entry.params.clone().iter().cloned().fold(Rc::new(vec![]), |best: _, p: Rc<Node>| {
            let pname = param_node_name(p.clone());
let descent_vars = collect_descent_vars(entry.body.clone(), pname.clone(), compile_error!("empty_map: value type unresolved"), check_child.clone(), check_list.clone());
let param_edges = collect_scc_child_edges(entry.body.clone(), name.clone(), pname.clone(), descent_vars.clone(), scc_name_set.clone());
...
pub fn merge_edge_evidence(best_map: Rc<HashMap<String, DescentEvidence>>, pe: Rc<ParserProgressEdge>) -> Rc<HashMap<String, DescentEvidence>> {
    {
        let key = v2_rt::concat(pe.caller.clone(), v2_rt::concat("->".to_string(), pe.callee.clone()));
let existing = v2_rt::map_get(&best_map, key.clone());
let new_ev = progress_to_evidence(pe.progress.clone());
let best_ev = match existing.clone() {
    Some(prev) => match new_ev.clone() {
    DescentEvidence::Strict => DescentEvidence::Strict,
    DescentEvidence::NonIncreasing => match prev.clone() {
    DescentEvidence::DescentUnknown => DescentEvidence::NonIncreasing,
    _ => prev.clone(),
},
    DescentEvidence::DescentUnknown => prev.clone(),
},
    None => new_ev.clone(),
};
v2_rt::map_insert(best_map.clone(), key.clone(), best_ev.clone())
}
}
...
pub fn collect_scc_independent_dim_edges(members: Rc<Vec<String>>, func_index: Rc<HashMap<String, Rc<FuncEntry>>>, scc_name_set: Rc<HashMap<String, bool>>) -> Rc<Vec<Rc<ProofEdge>>> {
    Rc::new({ let mut __result = Vec::new(); for name in members.clone().iter().cloned() { __result.extend((*match v2_rt::map_get(&func_index, name.clone()) {
    Some(entry) => {
        let tree_edge_map = entry.params.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |best_map: _, p: Rc<Node>| {
            let pname = param_node_name(p.clone());
let tree_vars = collect_descent_vars(entry.body.clone(), pname.clone(), compile_error!("empty_map: value type unresolved"), true, false);
let param_edges = collect_scc_child_edges(entry.body.clone(), name.clone(), pname.clone(), tree_vars.clone(), scc_name_set.clone());
param_edges.clone().iter().cloned().fold(best_map.clone(), |bm: _, pe: Rc<ParserProgressEdge>| merge_edge_evidence(bm.clone(), pe.clone()))
});
let list_edge_map = entry.params.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |best_map: _, p: Rc<Node>| {
            let pname = param_node_name(p.clone());
let list_vars = collect_descent_vars(entry.body.clone(), pname.clone(), compile_error!("empty_map: value type unresolved"), false, true);
let param_edges = collect_scc_child_edges(entry.body.clone(), name.clone(), pname.clone(), list_vars.clone(), scc_name_set.clone());
param_edges.clone().iter().cloned().fold(best_map.clone(), |bm: _, pe: Rc<ParserProgressEdge>| merge_edge_evidence(bm.clone(), pe.clone()))
});
let all_keys = v2_rt::concat(v2_rt::map_keys(&tree_edge_map), v2_rt::map_keys(&list_edge_map));
let unique_keys = all_keys.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, k: compile_error!("unresolved CompilerError type reached emit")| v2_rt::map_insert(acc.clone(), k.clone(), true));
let edge_names = Rc::new({ let mut __result = Vec::new(); for p in entry.params.clone().iter().cloned() { __result.extend((*{
            let pname = param_node_name(p.clone());
let tvars = collect_descent_vars(entry.body.clone(), pname.clone(), compile_error!("empty_map: value type unresolved"), true, false);
collect_scc_child_edges(entry.body.clone(), name.clone(), pname.clone(), tvars.clone(), scc_name_set.clone())
}).iter().cloned()); } __result });
let name_map = edge_names.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, pe: Rc<ParserProgressEdge>| v2_rt::map_insert(acc.clone(), v2_rt::concat(pe.caller.clone(), v2_rt::concat("->".to_string(), pe.callee.clone())), pe.clone()));
Rc::new({ let mut __result = Vec::new(); for key in v2_rt::map_keys(&unique_keys).iter().cloned() { __result.push({
...
let caller_name = match v2_rt::map_get(&name_map, key.clone()) {
    Some(pe) => compile_error!("error type cascade"),
    None => name.clone(),
};
...
let lex_all_known = { let mut __all = true; for e in lex_edges.clone().iter().cloned() { if !({ let mut __found = false; for ev in e.evidence.clone().iter().cloned() { if match ev.clone() {
...
let member_set = component.members.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |inner: _, member: String| v2_rt::map_insert(inner.clone(), member.clone(), true));
...
let next_index = members.clone().iter().cloned().fold(acc.index.clone(), |inner: _, member: String| v2_rt::map_insert(inner.clone(), member.clone(), info.clone()));
Rc::new(SccBuildAcc {
...
pub fn collection_output(binder: String, size: Rc<SizeExpr>) -> Rc<HashMap<String, Rc<CostExpr>>> {
    {
        let result = v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), "result".to_string(), cost_loop(binder.clone(), size.clone(), Rc::new(CostExpr::CostConst {
    value: 1,
})));
result.clone()
}
...
pub fn scalar_output() -> Rc<HashMap<String, Rc<CostExpr>>> {
    {
        let result = compile_error!("empty_map: value type unresolved");
result.clone()
}
}
...
let os = if (pc.clone() == false) {
            ...
            {
                let r = v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), "result".to_string(), cost_loop(binder.clone(), size.clone(), Rc::new(CostExpr::CostConst {
    value: 1,
})));
r.clone()
}
...
let sort_os = v2_rt::map_insert(compile_error!("empty_map: value type unresolved"), "result".to_string(), cost_loop(binder.clone(), size.clone(), Rc::new(CostExpr::CostConst {
    value: 1,
})));
Rc::new(SummaryResult {
    summary: Rc::new(ComplexitySummary {
...
r.clone()
            ...
            {
                let r = compile_error!("empty_map: value type unresolved");
r.clone()
}
};
Rc::new(SummaryResult {
...
pub fn build_complexity_report(func_entries: Rc<Vec<Rc<FuncEntry>>>, recursion_ctx: Rc<RecursionContext>) -> Rc<ComplexityReport> {
    {
        let func_index = func_entries.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, entry: Rc<FuncEntry>| v2_rt::map_insert(acc.clone(), entry.name.clone(), entry.clone()));
