...
pub fn substitute_type_slots(n: Rc<Node>, slot_bindings: Rc<HashMap<String, Rc<Node>>>, decl_name: String) -> Rc<Node> {
...
if is_slot.clone() {
    ...
    None => n.clone(),
                ...
                {
                    let new_children = Rc::new({ let mut __result = Vec::new(); for child in n.children.clone().iter().cloned() { __result.push(if (child.name.clone().as_str() == decl_name.clone().as_str()) {
                        {
                            let substituted_args = Rc::new({ let mut __result = Vec::new(); for arg in child.children.clone().iter().cloned() { __result.push(substitute_type_slots(arg.clone(), slot_bindings.clone(), decl_name.clone())); } __result });
Rc::new(Node {
...
let slot_bindings = Rc::new(decl.params.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, pair: (i64, Rc<Node>)| {
...
if is_parameterized_alias.clone() {
                            {
                                let alias_target = match (*rt_node(decl.clone())).clone() {
    NodeType::Typed { node: target, .. } => substitute_type_slots(target.clone(), slot_bindings.clone(), type_name.clone()),
    NodeType::InferError { .. } => n.clone(),
    NodeType::InferVariable { .. } => n.clone(),
    NodeType::Untyped => n.clone(),
};
let target_result = resolve_node_bounded(alias_target.clone(), env.clone(), module_name.clone(), (depth.clone() + 1));
...
Rc::new(NodeResolveResult {
    ...
    diagnostics: v2_rt::concat(v2_rt::concat(arity_diags.clone(), arg_diags.clone()), target_result.diagnostics.clone()),
})
}
} else {
                            {
                                let substituted_children = Rc::new({ let mut __result = Vec::new(); for child in decl.children.clone().iter().cloned() { __result.push(substitute_type_slots(child.clone(), slot_bindings.clone(), type_name.clone())); } __result });
let is_recursive = is_recursive_type(env.clone(), type_name.clone());
...
Rc::new(NodeResolveResult {
    ...
    span: n.span.clone(),
    ident_span: n.ident_span.clone(),
    children: Rc::new(vec![key_resolved.clone(), val_resolved.clone()]),
    connective: n.connective.clone(),
    params: n.params.clone(),
    inferred: n.inferred.clone(),
    return_cardinality: n.return_cardinality.clone(),
...
Rc::new(FieldResult {
    field: make_field_node(field_node_name(field.clone()), type_resolved.clone(), field_node_cardinality(field.clone()), match default_resolved.clone() {
    Some(result) => Some(result.expr.clone()),
    None => None,
}, field_node_from_key(field.clone()), field.span.clone()),
...
Rc::new(ParamResult {
    param: make_param_node(param_node_name(param.clone()), type_resolved.clone(), match default_resolved.clone() {
    Some(result) => Some(result.expr.clone()),
    None => None,
}, param.span.clone()),
    diagnostics: v2_rt::concat(type_diags.clone(), default_diags.clone()),
...
Rc::new(MatchArmResolveResult {
    ...
    Some(result) => Some(result.expr.clone()),
    None => None,
}, body_expr.clone(), arm.span.clone()),
...
Rc::new(ExprResolveResult {
    ...
    semantics: s.clone(),
}), v2_rt::concat(Rc::new(vec![r.expr.clone()]), lam_param_nodes.clone()), texpr.inferred.clone(), texpr.span.clone()),
    diagnostics: r.diagnostics.clone(),
})
},
...
let env = tp_names.clone().iter().cloned().fold(env.clone(), |e: _, tp_name: String| Rc::new(TypeEnv {
    bindings: v2_rt::map_insert(e.bindings.clone(), tp_name.clone(), Rc::new(TypeBinding {
    name: tp_name.clone(),
    resolved: Rc::new(Node {
    name: tp_name.clone(),
    span: no_span(),
