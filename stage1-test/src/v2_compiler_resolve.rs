...
let module_index = modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<Node>| v2_rt::map_insert(acc.clone(), m.name.clone(), m.clone()));
let export_sets = modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<Node>| {
            let exported = get_exported_names(m.clone());
let exported_set = exported.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |inner_acc: _, name: String| v2_rt::map_insert(inner_acc.clone(), name.clone(), true));
...
let sorted_order_map = Rc::new(sorted_names.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, pair: (i64, String)| v2_rt::map_insert(acc.clone(), pair.1.clone(), pair.0.clone()));
...
v2_rt::concat(v2_rt::concat(v2_rt::concat(item_names.clone(), variant_names.clone()), imported_names.clone()), v2_rt::map_keys(&kernel_type_set()))
}
}
...
let adjacency = v2_rt::concat(explicit_edges.clone(), implicit_std_types_edges.clone()).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, edge: Rc<DepEdge>| adjacency_add_edge(acc.clone(), edge.from_module.clone(), edge.to_module.clone()));
let in_degree_map = modules.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, m: Rc<Node>| {
            let imports_std_types = { let mut __found = false; for imp in module_imports(m.clone()).iter().cloned() { if (imp.name.clone().as_str() == "std.types".to_string().as_str()) { __found = true; break; } } __found };
...
v2_rt::map_insert(acc.clone(), m.name.clone(), ((module_imports(m.clone()).len() as i64) + implicit_std_types_in_degree.clone()))
});
...
let result = kahn_drain(initial_queue.clone(), Rc::new(vec![]), in_degree_map.clone(), adjacency.clone(), module_count.clone());
if ((result.sorted.clone().len() as i64) == module_count.clone()) {
    ...
    cycle_error: None,
            ...
            {
                let sorted_set = result.sorted.clone().iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
...
let new_zero_set = Rc::new({ let mut __result = Vec::new(); for neighbor in Rc::new({ let mut __result = Vec::new(); for node in queue.clone().iter().cloned() { __result.extend((*match v2_rt::map_get(&adjacency, node.clone()) {
    ...
    Some(0) => true,
    _ => false,
} { __result.push(neighbor); } } __result }).iter().cloned().fold(compile_error!("fold empty_map: value type unresolved"), |acc: _, name: String| v2_rt::map_insert(acc.clone(), name.clone(), true));
let new_zero = Rc::new({ let mut __sorted: Vec<_> = (*v2_rt::map_keys(&new_zero_set)).clone(); __sorted.sort_by(|a: &_, b: &_| { let __ka = (|name: _| name.clone())(a.clone()); let __kb = (|name: _| name.clone())(b.clone()); __ka.partial_cmp(&__kb).unwrap_or(std::cmp::Ordering::Equal) }); __sorted });
{
