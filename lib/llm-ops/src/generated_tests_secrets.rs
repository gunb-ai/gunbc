...
// Content-Hash: eb011fab84e54cc89917ae93c9c07c343173edc62dc29e23779e5a347e5f5e1c
...
fn test_resource_api_openai_key_acquire() {
    if !guard_test("test_resource_api_openai_key_acquire", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let spec = mock_spec();
...
fn test_resource_api_openai_key_timeout() {
    if !guard_test("test_resource_api_openai_key_timeout", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let spec = mock_spec();
    let resource = spec.get_resource("api:openai_key").expect("resource should exist");
...
fn test_example_prepare_secret_auth_prepare_emits_rest_request() {
    if !guard_test("test_example_prepare_secret_auth_prepare_emits_rest_request", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
...
fn test_example_parse_secret_auth_parse_extracts_content_from_rest_response() {
    if !guard_test("test_example_parse_secret_auth_parse_extracts_content_from_rest_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
...
fn test_example_parse_secret_auth_parse_handles_skipped_transport_response() {
    if !guard_test("test_example_parse_secret_auth_parse_handles_skipped_transport_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
