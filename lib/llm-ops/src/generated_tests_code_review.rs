...
// Content-Hash: e8d18c0b9b83326cd84743af019b6e24ff93239b00ff99b4f4d4623647573cdd
...
fn test_optional_missing_prepare_system_prompt() {
    ...
    inputs.insert("messages".to_string(), Value::Str("Review this code".to_string()));
...
/// Node example: prepare - Code review prepare emits REST request
/// 
/// Tests that node 'prepare' produces expected outputs for given inputs.
#[test]
fn test_example_prepare_code_review_prepare_emits_rest_request() {
    if !guard_test("test_example_prepare_code_review_prepare_emits_rest_request", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
...
/// Node example: parse - Code review parse extracts content from REST response
/// 
/// Tests that node 'parse' produces expected outputs for given inputs.
#[test]
fn test_example_parse_code_review_parse_extracts_content_from_rest_response() {
    if !guard_test("test_example_parse_code_review_parse_extracts_content_from_rest_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
    inputs.insert("provider".to_string(), Value::Str("openai".to_string()));
    inputs.insert("response".to_string(), Value::Response(gunbc_ir::transport::TransportResponse::Rest(gunbc_ir::transport::RestResponse { status: 200, body: serde_json::json!({"choices":[{"finish_reason":"stop","index":0,"message":{"content":"Code looks good.","role":"assistant"}}],"id":"chatcmpl-mock-test","model":"gpt-4o","object":"chat.completion","usage":{"completion_tokens":20,"prompt_tokens":10,"total_tokens":30}}), headers: std::collections::HashMap::new() })));
    ...
    assert_eq!(*output_content, Value::Str("Code looks good.".to_string()), "expected exact value");
...
/// Node example: parse - Code review parse handles skipped transport response
/// 
/// Tests that node 'parse' produces expected outputs for given inputs.
#[test]
fn test_example_parse_code_review_parse_handles_skipped_transport_response() {
    if !guard_test("test_example_parse_code_review_parse_handles_skipped_transport_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
