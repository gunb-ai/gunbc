...
// Content-Hash: f6dceb7dbcc6356876caedcfbdb6e893684512bda7299a2e1ec4398916008c78
...
/// Optional input: prepare.system_prompt (cardinality: 0..1).
/// 
...
fn test_optional_missing_prepare_system_prompt() {
    ...
    let _outputs = gunbc_exec::execute_single_node(&dag, "prepare", inputs, gunbc_exec::ExecutionMode::Real).expect("optional input prepare.system_prompt missing should not error");
}
...
fn test_optional_wrong_type_prepare_system_prompt() {
    ...
    let result = gunbc_exec::execute_single_node(&dag, "prepare", inputs, gunbc_exec::ExecutionMode::Real);
    assert!(result.is_err(), "optional input prepare.system_prompt wrong type should error");
}

/// Optional input: prepare.temperature (cardinality: 0..1).
/// 
/// Proves: missing optional input does not crash.
#[test]
fn test_optional_missing_prepare_temperature() {
    ...
    inputs.insert("model".to_string(), Value::Str("gpt-4o".to_string()));
    inputs.insert("provider".to_string(), Value::Str("openai".to_string()));
    let _outputs = gunbc_exec::execute_single_node(&dag, "prepare", inputs, gunbc_exec::ExecutionMode::Real).expect("optional input prepare.temperature missing should not error");
}

/// Optional input: prepare.max_tokens (cardinality: 0..1).
/// 
/// Proves: missing optional input does not crash.
#[test]
fn test_optional_missing_prepare_max_tokens() {
    ...
    inputs.insert("model".to_string(), Value::Str("gpt-4o".to_string()));
    inputs.insert("provider".to_string(), Value::Str("openai".to_string()));
    let _outputs = gunbc_exec::execute_single_node(&dag, "prepare", inputs, gunbc_exec::ExecutionMode::Real).expect("optional input prepare.max_tokens missing should not error");
}
...
fn test_optional_wrong_type_prepare_max_tokens() {
    ...
    inputs.insert("provider".to_string(), Value::Str("openai".to_string()));
    let result = gunbc_exec::execute_single_node(&dag, "prepare", inputs, gunbc_exec::ExecutionMode::Real);
    assert!(result.is_err(), "optional input prepare.max_tokens wrong type should error");
}
...
fn test_example_prepare_openai_prepare_emits_rest_request_and_echoes_provider() {
    if !guard_test("test_example_prepare_openai_prepare_emits_rest_request_and_echoes_provider", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
...
fn test_example_resolve_auth_resolve_auth_maps_openai_provider_to_api_key_env_var() {
    if !guard_test("test_example_resolve_auth_resolve_auth_maps_openai_provider_to_api_key_env_var", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
...
/// Node example: parse - OpenAI parse extracts content, model, tokens from REST response
/// 
/// Tests that node 'parse' produces expected outputs for given inputs.
#[test]
fn test_example_parse_openai_parse_extracts_content_model_tokens_from_rest_response() {
    if !guard_test("test_example_parse_openai_parse_extracts_content_model_tokens_from_rest_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
    inputs.insert("provider".to_string(), Value::Str("openai".to_string()));
    inputs.insert("response".to_string(), Value::Response(gunbc_ir::transport::TransportResponse::Rest(gunbc_ir::transport::RestResponse { status: 200, body: serde_json::json!({"choices":[{"finish_reason":"stop","index":0,"message":{"content":"Test response content.","role":"assistant"}}],"id":"chatcmpl-mock-test","model":"gpt-4o","object":"chat.completion","usage":{"completion_tokens":20,"prompt_tokens":10,"total_tokens":30}}), headers: std::collections::HashMap::new() })));
    ...
    assert_eq!(*output_content, Value::Str("Test response content.".to_string()), "expected exact value");
...
fn test_example_parse_openai_parse_handles_skipped_transport_response() {
    if !guard_test("test_example_parse_openai_parse_handles_skipped_transport_response", TestClass::Hermetic, FermiCost::S, &["http"], &[]) {
    return ();
};
    let dag = crate :: graph :: build_chat_completion_graph();
    let mut inputs = std::collections::HashMap::new();
