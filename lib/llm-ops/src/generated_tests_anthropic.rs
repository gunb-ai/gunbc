...
fn test_example_resolve_auth_resolve_auth_maps_anthropic_provider_to_api_key_env_var() {
    ...
    assert_eq!(*output_header_name, Value::Str("x-api-key".to_string()), "expected exact value");
    // Check output port 'scheme'
    let output_scheme = outputs.get("scheme").expect("output port 'scheme' should exist");
    assert_eq!(*output_scheme, Value::Str("header".to_string()), "expected exact value");
