...
- **Wire.swift** is a mechanical mirror of `gunbc.auth.approval_device_wire` "The HTTP wire": an ordered emitter matching `serialize_json` byte for byte, strict readers (unknown/missing/empty member and unadmitted `kind` refuse at their path), path segments percent-encoded totally and injectively (only 0-9 A-Z a-z - _ ~ kept, "." and "%" included in what is encoded, upper-case hex) and transported verbatim. `ApproveTests` round-trips every request envelope in `vectors.json` through decoder → encoder and matches the bytes.
