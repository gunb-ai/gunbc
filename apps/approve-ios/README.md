...
5. Run `ApproveTests` (⌘U). The vector tests are the only equivalence evidence with the `.dag`
   folds, and they are executed nowhere else (see `gunbc.approve_ios_app`). `testStoredRequestReadMatchesEveryVector`
   needs the fixture's `stored_request` section (emitted by `stored_request_specimens`); a fixture
   without it fails that one test and no other.
...
- **Enrol**: type the one-time code printed by the enrol command the operator runs over SSH on srv1 (it is the challenge identifier; it is not shown on the dashboard) → refuse unless App Attest is
  ...
  and only an authenticated readback (active naming the expected id → Enrolled, revoked → terminal;
  any untyped refusal, 404 included, is NOT absence and leaves it standing) resolves it. Keys, attestation, code and transcript are retained throughout. The attest key id is
...
- **Push token**: registered on every enrolled launch; one serialized convergence loop forwards the
  ...
  still the desired one and the enrolment is still the one the update was made for; a failure whose
  token or enrolment has since changed continues at once, a failure for the exact current
  (enrolment, token) pair stops, and the next token delivery or enrolment adoption starts a fresh loop. `adopt()` (enrolment or readback) re-registers for APNs and runs the loop, so
