...
- [ ] **seed discovery invokes the `.dag` lens directly** *(dissolution trigger)* — floor discovery's layer-span lane routing (blocking floor vs non-blocking integration, #6054) currently runs a host-side seed mirror (`coarse_layer_of_module`/`witness_layer_boundary_span` in `cli_run.rs`) of the `.dag` authority `v2.lens.layer_span`, held in lockstep by a parity test. When a v1 seed host-effect can resolve+evaluate a `.dag` lens query, delete the Rust mirror and call `v2.lens.layer_span.keeps_on_floor` on the witness's host-gathered import closure — single authority (§3/§7). This is the concrete `dissolve-on:` target the seed carries.
