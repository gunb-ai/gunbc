...
- Single `src/v3/compiler/src/bin/regen_lens.rs` shim reads `src/v3/compiler/regen.dag`'s `LensRegistryEntry` records via `Dag::new()` and regenerates each declared `lens_<name>_generated.rs`. `read_registry` fails closed on duplicate `name` / duplicate `generated_file`.
- `sg6_hand_authored_census_test.rs` pins both the `src/bin/` census (`regen_lens.rs`, `regen_v3.rs`, `self_host_fixed_point.rs`) and the full `(name, lens_file, generated_file)` registry triple, plus uniqueness of `name` / `generated_file` and singleton resolution of `--lens <name>`.
...
**Deferral: SG-6 `regen_lens` CLI end-to-end smoke test (XS, follow-up).** PR #560's `sg6_hand_authored_census_test.rs` ratchets the driver's read-side invariants **structurally** — it walks the bootstrap Dag the same way `regen_lens::read_registry` does and asserts the registry's shape, triple pinning, uniqueness of `name` / `generated_file`, and singleton `--lens <name>` resolution. **No test actually invokes the `regen_lens` binary.** Argument parsing (`parse_args`), bootstrap-diagnostic exit codes, the unknown-selector stderr path, and the success-case write path are all only verified manually today (via synthetic-duplicate and unknown-selector runs during PR development). The negative smoke (`--lens <unknown>` → nonzero exit + structured stderr) is the cheapest first move and belongs here; a positive-path smoke needs either a `--dry-run` flag on the driver or a temp-output plumbing path so it does not race with migration-snapshot tests that read the committed generated files.
