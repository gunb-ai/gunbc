...
- **Substrate carrier port program (scopes the "substrate dissolution path" half of the honesty pass above)**: design doc at [docs/design-substrate-carrier-port-program.md](docs/design-substrate-carrier-port-program.md) (2026-04-22). Decomposes the `DescentEvidence` / `CallPattern` / `SubValueRelation` / `MethodSemantics` port into four ordered lanes, with per-carrier shape analysis, dependency ordering, and per-lane stop-signals. T/C/I are concrete ports; M is a design-decision lane gated on T/C/I evidence. Related: per-method metadata surface from `*_templates()` in `dsl/std/algebra.dag` (surfaced by Lane G / PR #654) — scoped in §6a of the design doc as a deferred follow-up, not part of the core four-carrier port. Unblocks `cost.dag` + `complexity.dag` PROXY → COMPLETE per the capability register, and opens P4 Decidability progress. Does NOT touch the emit-gap half (idempotency / parallelism); that stays in the receipt-closure wave. Owner: unassigned; lanes dispatchable independently in the order T → C → I → M.
  ...
  - **Lane E-I — port `SubValueRelation` + inductive fields + cost algebra (M)**: port Tier 1 (structural) + Tier 2 (lattice) + Tier 3 (`CostBound` + master theorem). Requires E-T, E-C. Pre-flight: verify v3 lowers `CostBound`'s self-referential-through-`List` shape. Acceptance: `cost.dag` / `complexity.dag` advance on the non-method-dispatch slice with cementing tests; full COMPLETE gated on E-M.
