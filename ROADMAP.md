...
### Lane 4 Stage 4a prep — handwritten parser snapshot harness (🟡 infrastructure landed)

**Purpose:** this is **prep only**, not the `parse.dag` cutover. It snapshots the incumbent handwritten `src/v3/compiler/src/parse.rs` output over the currently parseable corpus so Stage 4a has a ratchet once `.dag` authority exists.
