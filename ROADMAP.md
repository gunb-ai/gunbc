...
- [x] **inert-lens hygiene (executable backstop):** every `lens/*.dag` is wired (a discovered fail-closed witness) or **deleted** — an inert lens is a lie. This *runs* over the corpus. Gate: `lens_corpus_coverage_manifest.dag` — manifest length == registry length (anti-drift) + some-wired (anti-vacuity). Currently 4 InertScaffolded (Complexity, Cost: synthetic-only tests; EffectEnumeration, UnusedParameters: dark). Final all-wired gate is 🟡 SCAFFOLD, activates when the 4 scaffold entries are promoted (see "promote-or-delete" item above).
