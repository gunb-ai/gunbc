...
**Review arc.** R1 at `b42edc15` landed the `Bool + String?` → `CompositionVerdict` lift with `BrokenBy { first_breaker: OperationEffect }` wrapped in `ComposedEffect { operations, verdict }`. Codex review flagged the inner-variant hole (breaker payload admitted idempotent shapes); R2 partitioned `EffectShape` and narrowed the copied payload to `BreakingOperation`. ChatGPT review then flagged the outer-record hole (operations/verdict correlation was behavioral, not structural) and recommended returning `CompositionVerdict` directly; R3 is that fix. Stage 2b's first real consumer then graduated the copied `BreakingOperation` payload to `ElementRef<OperationEffect>` so the verdict points back into the caller-owned workflow evidence chain instead of copying a second breaker record. Each round pushed state-space soundness one layer deeper: outer sum → inner payload → record wrapper → provenance handle.

**Known trade-off, now explicitly tracked.** `ElementRef<OperationEffect>` closes the provenance hole (`BrokenBy` now points into the caller-owned workflow list), but it does **not** by itself prove the pointed operation is breaking. That subset fact still lives in the `compose_effects` / `first_breaker_ref` filter, so the remaining invariant is a Track 9 class constructor-validation asymmetry rather than a fully structural witness. This is acceptable at current Stage 2b scope because the first concrete consumer needed provenance more urgently than a new breaking-only handle kind, and the Rust mirror plus tests dereference through `operation_at` before rendering. **Tracked dissolution trigger:** the first consumer that must treat "breaking workflow element" as a standalone witness, rather than "workflow position plus re-check shape", should graduate this to a breaking-only handle or equivalent substrate proof. Until then, the verdict is honest about what it proves: in-bounds evidence-chain position, with breaking-subset validation still behavioral.
