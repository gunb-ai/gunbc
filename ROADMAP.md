...
- **Unit-mismatch enforcement for `Dimension<T, Unit>` consumers**: `src/v3/std/dimensions.dag` provides the `Dimension<Carrier>` framework (🟢 TERMINAL for structural surface), but no consumer lens rejects unit-mismatched operations like `Duration<Second> + Duration<Millisecond>` or `Money<USD> + Money<EUR>` without explicit conversion. Rust's `uom` crate provides the library-level runtime equivalent; the gunbc path is a substrate-level lens that uses the framework's unit phantom parameter to reject mismatches at compile time. **Dissolution trigger**: a lens authored in `.dag` that reads `Dimension<T, Unit>` at application sites and enforces unit matching via the already-typed phantom parameter. Adjacent to the v3 lens capability honesty pass (earlier in this section — Dimension wiring deferred for `cost.dag`); unit-mismatch is a separate consumer surface. Surfaced by `docs/thesis/compositional-modeling.md` Part 4. P1 Modeling Faithfulness. Owner: unassigned; S-M scope once the lens honesty pass lands.
