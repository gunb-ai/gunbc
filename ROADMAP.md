...
- Acceptance fixture: `src/v3/compiler/tests/lane2_stage_2d_symbolic_cost_test.rs` (13 tests). Covers Value/Transform/Branch/Loop lowering; zero-drop normalization; `Linear(v) * Linear(v) → Polynomial(v, 2)` nested-fold fingerprint; cross-variable product stays `Product`; dominance partial order (Unknown / Linear / Log / Polynomial degree ordering); `max_path` dominant-selection.
...
**Acceptance-test infra migration (documented side effect).** Pre-existing tests that queried `dag.nodes().iter().find_map(Behavior::as_transform)` etc. on the `compile_to_dag` output assumed the bootstrap Dag contributed no `Transform` / `Branch` / `Loop` / `Bind` nodes before the user's source lowered. Lane 2 Stage 2d invalidates that assumption — `src/v3/std/algebra.dag`'s lowered bodies contribute many nodes. Six tests (m0_acceptance's `test_let_binding_produces_dag_shape`; m1_substrate_test's `m17_operator_lowers_to_structural_transform_target` / `m17_comparison_operator_lowers_to_structural_transform_target` / `m17_user_function_call_lowers_to_callable_target` / `m18_r15_match_on_aliased_sum_type_compiles` / `m18_r13_mutual_recursion_poisons_callers` / `mutual_recursion_planner_ignores_callable_parameter_shadowing`; m1_3_emit_rust_test's three reflected-harness bind-count fixtures; emit_rust.rs lib test `render_field_project_constructs_owned_list_from_borrowed_nodes`) migrated to filter by `span.file` (or subtract a bootstrap baseline) so they pin the user-code count/shape rather than the global node list. This is a general test-authoring hygiene rule for every future std-module consumer to consider — a dedicated walker in Lane 1e would emit typed span-filtered iterators and dissolve the hand-written `.filter(|t| t.span.file == ...)` idiom.
