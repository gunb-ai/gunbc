...
Coercion is one mechanism, run both ways. **Ingestion is coercion and emission is coercion**: a structure-preserving search over declared inhabitants, performed by the compiler rather than any hand-written adapter ([`src/v4/std/coercion.dag`](src/v4/std/coercion.dag)). The whole language is **`.dag → IR → .dag`**, where a `.dag` can be anything modeled in the substrate — *including a language itself*. Ingesting a program coerces it from its language model into the canonical IR; emitting coerces the IR back out into a target's declared inhabitants. Same fold, opposite direction: the same coercion is the ingester and the emitter, with no separate engine beneath it. This is what makes omni-ingestion and omni-emission cheap — model N targets once and *derive* the N×M translations (**model local, derive global**), instead of authoring an adapter per pair.

Coercion is **total in both directions**:
...
Today emission-as-coercion is wired through the translate stage; the symmetric ingestion side is proven by source-authority round-trip claims (one module → IR → canonical `.dag` → IR′, normalized equality). Both are exercised as `TestClaim` data under [`src/v4/test/claim/`](src/v4/test/claim/), fail-closed refusals landing first as the safety floor. Full multi-language ingestion breadth and the refinement-aware rule that distinguishes faithful widening from lossy narrowing are in-flight / later-wave work.
