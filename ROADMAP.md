...
- [ ] **CI on compute fabric** — centralize host management under the fabric: one host `ResourceEnvelope` is the single authority, every knob **derived** from it and **generated** (gunbc owns unit shapes, ctrl owns srv1/srv2 instances), not the hand-set systemd+shell that drifts and contradicts today (the EAGAIN clone-reject flake, spawn_width OOM, and `MemoryMax` drift are all this one un-modeled-envelope root). **Chore list to model — derive, don't hand-set:** measured host facts (the fleet model is *wrong*: says 64c/256GB, real is 128c/128GB) · cgroup pids cap (`TasksMax`) · jobserver token count · `spawn_width` (memory-aware: cores ∧ mem — RUN phase; discovery shards run the prebuilt binary, pids-light) · per-build fan-out (`CARGO_BUILD_JOBS`×codegen-units×LLVM — BUILD phase, the pids burst: concurrent_builds × per-build-pids ≤ TasksMax) · `MemoryMax` (drifted srv1≠srv2) · watchdog / isolation / runner topology. This is the **parallelization-by-realization** arm of `std/realization.dag` (the twin of §2 caching). Full dimension map + the derive-from-one-envelope plan: [compute-envelope-model](docs/plans/compute-envelope-model.md).
