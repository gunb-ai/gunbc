...
- [ ] **compile-clean gate force-checks every fn body** (fail-closed) — unreached/uncalled fn bodies currently escape typecheck, so undefined symbols + body-level type errors in dead code pass the gate **green** (a §5 fail-open, execution-proven on `utf8_decode_bytes` in `secret_manager.dag`); the construction fix typechecks *total* over every declared body, no "unreached" escape. Load-bearing compiler semantics + a likely red wave → **measure-first** (report-mode blast radius), expose-then-triage, escalate before the enforce-flip. *Owned by §1 (quick-ant-298); instance fix (define the symbol) is `adhoc-8e5771e1-14a`*
