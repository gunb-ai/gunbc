...
- [ ] **ban source comments** (`.dag` + `.rs`) — comments are a parallel-ledger / dual-representation (§6 "mark on the carrier is the authority"); model the live-state survivors (dissolution triggers · `Anchor:` citations · gate-read markers), **delete the rest aggressively** (git is the backup). Inflates LOC, blocks review *(calm-seal-13: model-then-delete, pilot on CI-gate files → fan out)*
