...
- CI wall-time (~27 min → minutes)
  ...
  - P4 — affected-set floor skip kernel consumption (#5179) — `floor_witness_run_disposition`
    is policy authority; hand-Rust scaffold in `cli_run.rs` / `resolved_graph_cache.rs` thins to
    git/cache transport; dissolve-on `affected_set_reading` node-frontier replaces entry-closure
    git-diff intersection (interim gates: `floor_runner_host_kernel_skip_equivalent_holds`)
