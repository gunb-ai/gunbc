...
**Tracked follow-up — SG-0 ratchet split — resolved by PR #675.** The R1 split (T-PB-A owns non-test; T-PB-B owns test) is now fully structural. `EXPECTED_HAND_AUTHORED` in `sg0_census_test.rs` is split into `EXPECTED_HAND_AUTHORED_NON_TEST` (36 files, T-PB-A surface) and `EXPECTED_HAND_AUTHORED_TEST` (55 files, T-PB-B surface); two new tests (`sg0_partition_paths_are_structurally_correct`, `sg0_partition_sub_lists_are_disjoint`) enforce the partition as an invariant. `pb_hand_rust_at_shim_floor` (T-PB-A gate) now has a concrete set to scope against: `EXPECTED_HAND_AUTHORED_NON_TEST.len() ≤ 5`. `pb_rust_tests_outside_residual_zero` (T-PB-B gate) scopes to `EXPECTED_HAND_AUTHORED_TEST` minus the `TESTING.md §"Post-R2 shape"` residual. Both predicates remain `[ext]` pending T-TestGen schema extensions.
