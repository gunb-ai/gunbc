...
- **`tests/integration/common/mod.rs:319` narrow-scanner forces byte-constant workarounds in downstream tests** (T-Receipts, surfaced by T-Demo #686/#705). The scanner is "deliberately narrow" and does not model Rust character literals, so test authors touching files in its scan path cannot write `b'\\'` / `b'"'` and must substitute `DAG_ESCAPE_BYTE = 92` / `DAG_QUOTE_BYTE = 34` numeric constants instead (see the `t_demo_fixture_test` module in `src/v3/compiler/tests/integration.rs` prior to #705's bridge dissolution; the constants survived the dissolution because the scanner constraint remained). Dissolution: either widen the scanner to model Rust char literals, or replace its scan with a structural reader that doesn't need to exclude char-literal syntax. Small scope; cosmetic for now, but attracts bridge-shaped workarounds every time a new test lands under its scan path. Owner: unassigned; P2 Boundary Discipline / T-Receipts bundle.
