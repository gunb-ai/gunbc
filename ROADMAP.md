...
- [ ] **relational projection** *(first-class artifact — for the author and the pitch)* — render "arbitrary program DAG → numerical graph → kernel" as a triptych (your program · the elementwise-array subgraph lifted out as a dataflow/HLO-shaped graph · the layout/fusion plan + fidelity verdict). A **lens** over the `Node` tree + `RealizationPlan` (zero substrate edits), *derived* from the same plan the kernel lowers from (can't drift from what executes). Text/DOT projection first, HTML/React visual on top ("one more medium") — **converges with §7** as the demo-beside-the-TS-emit candidate. [plan](docs/plans/accelerator-demo-roundtrip.md)
