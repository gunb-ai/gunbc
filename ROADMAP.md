...
**🟡 Design partial (2026-04-18).** [phase1-lane3-consolidation-build-plan.md](docs/phase1-lane3-consolidation-build-plan.md) carries the design work landed 2026-04-18 in §§7–12: **walker contract** (§7), **spec reading protocol** (§8 — Q5 compliant, zero name-keyed lookups), **determinism plan** (§9 — invariant D-1, `BTreeMap` everywhere, structural grep gate), **migration plan** (§10 — prerequisite bridge **1e.0** Python schema migration + DB-2's six sub-stages 1e.1–1e.6 with definitions of done), **bootstrap-once + snapshot-ratchet** (§11 — mirrors PR #477 / #518 / #530 `lens_*_generated.rs` pattern), and **determinism test suite** (§12 — `tests/determinism_test.rs` per-fixture 5× re-run per DB-8). Pre-existing §§1–3 sketch the shape of the function inventory / spec-field gap list / bridge inventory but remain inventory-stub form with TODO markers; the actual separately-landing doc files (`docs/emit-functions-inventory.md`, `docs/spec-field-gaps.md`, `docs/emit-bridges.md`) are not yet authored. **Remaining for "design complete":** populate the three separate inventory docs + obtain P2-L1 sign-off (§Acceptance gates in the build plan is the authoritative list). **Post-review revisions (2026-04-18):** (i) added explicit 1e.0 sub-stage — Python still sits on private `Python*Realization` scaffolds with no `python_language: LanguageSpec`; 1e.1's full-authority-surface gate cannot hold until Python migrates onto the shared walker surface. (ii) §7 qualified: `src/v3/spec/<target>.dag` is the bootstrap-loaded fixture, NOT the canonical authority; the canonical home is `dsl/extdeps/languages/<target>/` per THESIS.md §"Bootstrap staging note". (iii) §8 strategy-cache wording firmed: `PatternStrategyVariants` and `CallableStrategyVariants` get named dissolution triggers tied to 1e.0 rather than "if/when" language. (iv) Status line walked back from "Design complete" to "Design partial" — the doc's own §Acceptance gates list five items, three of which (the per-file inventories + sign-off) remain open. **No new DB number allocated** — the plan extends DB-2 (walker API), DB-4 (clean-emission contract), and DB-8 (fixed-point ratchet) rather than introducing a novel structural decision. **Blocks Stage 1e dispatch until §Acceptance gates all hold.**
