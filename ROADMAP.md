...
- [ ] **ban source comments** (`.dag` + `.rs`) — comments are a parallel-ledger / dual-representation (§6 "mark on the carrier is the authority"); inflate LOC, block review. The fix is a **construction wall, not a cleanup**: (1) give the live-state survivors a *modeled grammar form* (dissolution triggers · `Anchor:` citations · gate-read markers are comment-encoded today); (2) migrate survivors → that form; (3) **delete the rest aggressively** (git is the backup); (4) **parser refuses free `//`** → comments unwritable by construction (§5). Step 4 lands LAST (load-bearing grammar — `02_parse`/`syntax`, model-before-implement) *(calm-seal-13: pilot CI-gate files → fan out → wall)*
...
- [ ] **invert hand-maintained artifacts** — emit each (ROADMAP flagship · doc indexes) from its `.dag` authority + drift-gate it (the ci.yml pattern) ([plan](docs/plans/invert-hand-maintained.md))
  - [ ] **PR→checkbox status** — box derived from an explicit *completes-iff-PR* binding (not mere mention), drift-gated; model + witness landed, host-fed gate next ([plan](docs/plans/invert-hand-maintained.md) §6.4)
