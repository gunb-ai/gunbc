...
- [ ] **resource budget tree** (§2 one-concept-every-scale, ns-memo→infra-deploy) — each level inherits a parent RAM budget as a transaction: static `Σ children ≤ parent` over authored budgets is a compile-time **wall** (over-commit unwritable by construction, §5); the runtime intent-vs-actual reconcile (capacity drop → evict by QoS / reschedule / loud-error on an unsatisfiable Guaranteed) is a fail-closed **handler**, *not* a wall (two verdicts, never conflated). Subsumes spawn-width (#5444) · cross-host placement (#5559) · compile-jobs (#5546) as consumer leaves — the uncapped-runner over-commit is the missing parent allocation. [plan](dsl/product/budget_tree.dag)
