...
- [ ] **emitted crate `cargo build`s green (Route-A last mile)** — execution-grounded (#5705, deep-wolf assembled the real 108-emitted-module + 22-periphery crate and cargo-built it): **73 cargo errors / 38 E0308**, where 38 is a **LOWER BOUND** (8 E0433 wiring failures mask further E0308 until resolved). 89% of E0308 reduces to **TWO emitter root causes**: Family A (25, =66% — map-literal `&str` keys emitted without `to_string` → `HashMap<&str,_>` vs declared `HashMap<String,_>`, ONE emitter fix) · Family B (10 — missing Box/Rc/Option auto-wrap) · Family C (3 — scalar string). **Pole: Family A → Family B.** §5 honesty: **unmask E0433 EARLY (concurrent with Family A) so the true E0308 ceiling surfaces — "done" is a re-measure-after-unmask showing no new family, not `38→0`** (no green against a masked lower bound). Separately owned: 12 E0560 measure-tower (just-wired) · 8 E0433 unwired-periphery (regen patch-list stale) → the regen-fixpoint lane. *(jolly-cat #5705; supersedes the stale ~12,600 / ~150-latent estimate, ~4x over)*
