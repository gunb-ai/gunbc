...
- [ ] **rust-gate coverage** (shared §1) — opt-level=3 restores Pop-A to per-PR (#5456); run-all-unless-`#[ignore]`d (#5427); the `.dag`→rust coverage wall is edge-(b) (rust-test↔consumed-`.dag` closure, fail-closed both directions) ([cause table](docs/plans/ci-selection-vs-scheduling.md), [edge-(b) keystone brief](docs/plans/edge-b-rust-dag-provenance-brief.md)) *(quick-ant-298)*
