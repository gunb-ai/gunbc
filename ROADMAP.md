...
- [ ] **emitted crate `cargo build`s green (Route-A last mile)** — execution-grounded, **re-measured after unmask** (#5714, deep-wolf, `--emit-fresh` full 108-emitted + 22-periphery closure): the earlier **38 E0308 was an E0433-masked LOWER BOUND** — true ceiling **647 E0308** (the §5-honesty call to re-measure-after-unmask was right, ~17x over the masked bound; *no* green was ever claimed against `38→0`). **Family A (24 map-literal `&str` keys) CLEARED → 0** by #5714 (type-DERIVED key coercion, 4 witnesses green, clean 4-file delta). **Pole MOVED → the first-Option family: 633/647 (98%)** — the `first` operator emits `.first().cloned()` (`Option<T>`) where the consumer expects the unwrapped `T`, an element-vs-Option mismatch; **PROVEN pre-existing, not a #5714 regression** (the identical `.first().cloned()` sits in the committed seed at `extdeps_version_semver.rs:81`, merely left unwired — the fuller closure only surfaces it). Fix = ONE type-derived emitter change at the pipe-first site: emit a **fail-closed unwrap** (a located abort on empty, NEVER `unwrap_or_default`) per the declared consumer element type, mirroring Family A. Tail: Family B Box/Rc/Option (~7) · E0560 measure-tower (12). **"done" = re-measure after the first-Option fix shows no new pole** (not `647→0` against one family). *(jolly-cat #5705/#5714; supersedes the stale 38-lower-bound and the earlier ~12,600 / ~150-latent estimate)*
