...
- **Class 5 Gap 3 — port-carried field values in `data` bodies (substrate capability gap)**: authoritative enumeration at `src/v3/DOWNSTREAM_REQUIREMENTS.md:239` (gap partially closed at M1(2.8) for literal-only fields via `ValueBody::Structural` / `SurfaceExpr::Record` / `lower_record_to_structural`; remaining open half is port-carried field values — nested records, list literals, declaration references, `Var` references, sum-variant literals). `compile_to_dag` rejects these shapes inside `data` bodies today. **Consumer impact surfaced 2026-04-24** by PR #693 escalation (sub-child `sharp-bear-829` under Surface Manager): blocks `sub_charclass_in_std_unicode` phase-2 — specifically the `tokenize.dag` retype of `StringEscapeSpec.suffix`, `LocalPunctSpec.pattern`, `OperatorSpec.symbol`, `dag_keyword_set` keys, etc. to `Char` / `List<Char>` + `CharClass` sum-variant literals. Adjacent future retype-to-structured-carrier lanes (syntax.dag keyword sets, other opaque-string fields) will hit the same wall. Dissolution: extend `compile_to_dag` to parse port-carried field values in data bodies. Alternative design call (out of scope here): shift the authority shape for these configs from `data` bodies to extractor-over-unparsed-source. Owner: unassigned; substrate-capability scope — routes through a self-hosting / substrate-capability lane, not through the consuming lanes. **Audit pattern:** future "this consumption gap has no substrate capability gap" claims should be verified by attempting the retype before the claim lands — the 2026-04-23 authoring of the Character-level row (this file, `:354`) made this claim and was factually wrong; retracted 2026-04-24 by the update to that row in this PR (see the `:354` Status block).
