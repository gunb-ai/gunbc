...
### Self-hosting Stage 4 / Phase 4a prep — handwritten parser snapshot harness (🟡 infrastructure landed)

**Purpose:** this is **prep only**, not the `parse.dag` cutover. It snapshots the incumbent handwritten `src/v3/compiler/src/parse.rs` output over the currently parseable corpus so Self-hosting Stage 4 / Phase 4a has a ratchet once `.dag` authority exists.

- **Harness location:** `src/v3/compiler/tests/integration.rs` module `parse_stage4_prep`.
- **Manifest:** `src/v3/compiler/tests/integration/parse_corpus_manifest.txt` records `(path, item count, debug-byte count, fnv1a64(pretty-Debug SurfaceModule))` for the current handwritten parser output.
- **Scope:** the seven bootstrap `dsl/std` files (`logic`, `bit`, `algebra`, `integer`, `float`, `types`, `string_type`), plus `src/v3/std/*.dag`, `src/v3/spec/*.dag`, `src/v3/compiler/*.dag`, and `tests/four_fixture_pressure/*.v3` that the handwritten parser accepts today.
- **Non-goals:** does **not** add `parse.dag`; does **not** generate a Rust projection; does **not** switch parser callers; does **not** retire `parse.rs`; does **not** by itself prove `.dag` parser equivalence.
- **Refresh command:** `cargo test -p v3-compiler refresh_handwritten_parse_snapshot_manifest -- --ignored`

**Cutover bar remains unchanged:** SG-2 closes only when `parse.dag` exists, callers switch to the generated/parser-projection authority, output parity is demonstrated against the incumbent `SurfaceModule` tree, and handwritten parser authority is removed or reduced to a generated-host boundary.
...
**Landed (PR #560, SG-6 regen-driver cutover / partial lane).**
- Four per-lens regen bins (`regen_lens_cost.rs`, `regen_lens_cost_symbolic.rs`, `regen_lens_structural_resolution.rs`, `regen_lens_unused_parameters.rs`) deleted.
- SG-4 prep's `regen_infer_helpers.rs` absorbed into the same registry per its own "SG-6 owns folding all of these" header — `infer_helpers` entry added to `regen.dag`, bin deleted, SG-4 freshness test's regen-command message repointed at `regen_lens -- --lens infer_helpers`.
- Single `src/v3/compiler/src/bin/regen_lens.rs` shim reads `src/v3/compiler/regen.dag`'s `LensRegistryEntry` records via `Dag::new()` and regenerates each declared `*_generated.rs`. `name` composes from `std.types::NonEmptyStr`; `lens_file` / `generated_file` from `std.types::FilePath`. `read_registry` fails closed on duplicate `name` / duplicate `generated_file`.
- `sg6_hand_authored_census_test.rs` pins: (a) the `src/bin/` census — both flat-file and directory-form Cargo autobin shapes; (b) the full `(name, lens_file, generated_file)` registry triple across all 6 lenses; (c) uniqueness of `name` / `generated_file`; (d) singleton resolution of `--lens <name>`; (e) **positive-path CLI smoke** via `sg6_regen_lens_cli_smoke_regenerates_named_entry_without_drift`, which spawns the bin through `CARGO_BIN_EXE_regen_lens`, asserts exit success, stdout target-path format, and byte-identity of the regenerated snapshot.

**Deferral: SG-6 registry consumer consolidation (S, follow-up).** `regen.dag` is now the primary authority for `(name, lens_file, generated_file)` triples, but two classes of downstream consumer still mirror those paths instead of resolving through the registry:
1. Every `src/v3/compiler/tests/integration/m2_lens_*_migration_test.rs` has a local `lens_path()` and a `checked_in_generated_module()` backed by `include_str!`.
2. `src/v3/compiler/src/lib.rs` (plus `lens_unused_parameters.rs`) embeds each generated file via `include_str!`.

The triple-ratchet in `sg6_hand_authored_census_test.rs` keeps them from diverging silently, but the duplication still exists.

Dissolution trigger: add a shared `tests/common` helper that walks `regen.dag` and returns a lens source path keyed by registry `name`; re-point every `m2_lens_*_migration_test.rs::lens_path()` at that helper. Once the source-side path is sourced exclusively from the registry, the `lens_file` column of the triple ratchet collapses into a dependency on the same helper (the SG-6 census then pins only `name` + `generated_file`, with `lens_file` re-derived at read time). `include_str!` on the generated output remains compile-time and stays hardcoded — that bridge dissolves only when/if the lib adopts a runtime-loaded generated-file surface.

**Deferral: SG-6 `regen_lens` CLI error-path smoke (XS, follow-up).** PR #560 landed the positive-path CLI smoke (see Landed, item e). The remaining gap is behavioral coverage of the error paths — `parse_args` on unknown flags, bootstrap-diagnostic exit codes, and the unknown-selector stderr path — none of which are exercised by any test today. The cheapest shape is a negative-path smoke that spawns `regen_lens -- --lens <unknown>` through `CARGO_BIN_EXE_regen_lens`, asserts exit != 0, and greps stderr for the registry-name enumeration the driver already prints. Landing this closes the last CLI-surface gap; nothing structural in the driver needs to change for it.

**Deferral: SG-6 remaining lane surfaces (XL aggregate, follow-up PRs).**
- `src/v3/compiler/src/pipeline_authority.rs` (276 lines): the text-scanning body parser for `pipeline.dag`'s `fn compile { ... }` body should dissolve into a declared `data pipeline_compile_order: List<DeclarationRef>` record consumed structurally (same pattern as `LensRegistryEntry`).
- `src/v3/compiler/src/lens_testgen.rs` (792 lines): port to a `.dag` lens under `src/v3/lenses/testgen.dag`, emitted into `src/v3/compiler/src/lens_testgen_generated.rs`; register with `regen.dag` (or its widened successor). This is the largest single chunk and satisfies SG-6's "adding a generated test no longer requires hand-editing Rust drivers" acceptance axis.
- `src/v3/compiler/src/bootstrap.rs` (468 lines): narrow to a host shim; `materialize_pipeline_realizations` becomes `.dag`-authored where possible.
