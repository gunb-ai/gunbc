...
- [ ] **floor runs the right things** — **cadence-decoupling = two orthogonal axes** (shared with §0): SELECTION (which tests a change could affect — §4 affected-set, sound/fail-closed, by *what changed*) vs SCHEDULING (when/where/how-wide — §1, by *cost + hermeticity*). **Cost informs scheduling, never selection** — relegating an *affected* test to nightly because it's slow drops it from the gate (§1 safety, caught "at interest"); this is why the cost-routed nightly lane was rejected. [selection-vs-scheduling](docs/plans/ci-selection-vs-scheduling.md). **Root-caused 2026-06-21 (proud-deer-709 cause-table, [expensive-test-cause-table.md](docs/plans/expensive-test-cause-table.md)): the "expensive" tests were NOT intrinsic seed cost — they were DEBUG-BUILD AMPLIFICATION** (the per-PR rust gate ran `cargo test` in *debug*, so the unoptimized v1-compiler seed ran 100–800× slower; the release CLI does the same whole-tree index + resolve + typecheck in ~0.1s). Two populations: **Pop-A** (~21 in-process pipeline tests, 28–118s in debug) and **Pop-B** (~16 subprocess/network wet-captures):
  ...
  - [ ] **per-PR gate = the sound baseline, shrunk to the minimal sound set.** Step 1 = **run-all-unless-`#[ignore]`d-with-a-written-reason** (**#5427**, fierce-hawk-540) — a *superset* of the affected set, already fail-closed on selection (never skips an affected test); completeness lens = "no affected test unrun; no un-run test without a written reason" (legit §6 residue: rustc `#[ignore]` is unstructurable). Step 2 = **shrink to the affected set** via the existing `v2.lens.affected_set` (compile-time, structural over `Node`/`DependencyView`, git-diff edit locus, fail-closed, CI-enrolled) — an *affordability* refinement *on top of* #5427's soundness, **not** a correction of it. Membership stays bounded by **affectedness, not cost**; expensive-but-affected runs per-PR, cost absorbed by parallelize + cache (the §2 realization arm).
  - [ ] **nightly = (a) full-corpus selector-backstop + (b) non-hermetic residue** — *not* "where expensive tests go" (the cost-routed lane was rejected: #5447 stood down, operator 2026-06-21). (a) run everything on a slow cadence to audit that affected-set selection missed no dep edge — fail-closes the *selector itself*, the insurance that lets the Step-2 shrink be safe; (b) live-network / real-external-service captures that can't gate deterministically regardless of cost. The "expensive" category drains to ~this residue as #5456 (opt-level) + #5450 (build-once) land, so lane machinery is **residue-gated, not built ahead** (§6). *Owned by §1 (quick-ant-298). ⚠ scheduled-workflow CI-gen is load-bearing — escalate before editing `gunbc ci`/`ci_spec`.*
...
- [ ] **CI on compute fabric** — centralize *all* host management/modeling under the fabric: the host's true `ResourceEnvelope` is the **single authority** and **every** resource/parallelism knob is **derived** from it and **generated** (gunbc owns the unit *shapes*, ctrl owns the srv1/srv2 *instances* — the units are emitted, not hand-typed). Today these knobs are independent hand-set systemd+shell that drift and contradict, so the host oversubscribes itself — this is *one* un-modeled-envelope problem wearing many hats (the EAGAIN clone-reject flake, the spawn_width OOM, the `MemoryMax` drift are all the same root). **The dimensions that must be modeled/calculated, not forgotten** (enumerated *now* precisely because there are too many to hold in our heads):
  - [ ] **ground the envelope in measured truth first** — the fleet model is currently *wrong* (says 64c/256GB Altra; measured ground truth is **128c/128GB** on both hosts). Every derived limit inherits this lie until fixed.
  - [ ] **cgroup pids cap** (`TasksMax` on `actions-runner@*.service` + the container template) — too low (4096) vs the build's thread fan-out ⇒ `clone()` rejected ⇒ EAGAIN false-failure; host has ~1M-thread headroom unused.
  - [ ] **jobserver token count** (`/var/lib/ctrl/jobserver` — the real cross-process parallelism authority, ~120) — must be *derived from* the envelope, not a magic constant the pids cap then contradicts.
  - [ ] **floor `spawn_width`** — must be **memory-aware (cores ∧ mem)**, not just `min(shard_count, hardware_threads)` (`ResourceEnvelope.memory` modeled but unwired → deterministic OOM as corpus grows). **NOT pids-bound:** discovery shards run the prebuilt binary (pids-light), so width-up is pids-safe on its own; the pids discipline lives on per-build fan-out × concurrent-builds (below), per #5462's RUN/BUILD phase-split.
  - [ ] **per-build fan-out** — `CARGO_BUILD_JOBS` × rustc codegen-units (debug default 256) × LLVM threads × sccache; the multiplicand that makes a single "job" burst past the pids cap.
  - [ ] **cgroup memory budget** (`MemoryMax`) — currently *drifted* (srv1 52% vs srv2 70%); must be one derived value, not per-host hand edits.
  - [ ] **watchdog / wall limits** (`WatchdogLimit.max_wall`), **isolation boundary** (`IsolationBoundary`), and **runner instance count / topology** (how many `actions-runner@srv1-NN` units, on which host) — also envelope-derived, also currently hand-set.

  Single carrier with §2 (caching) — this is the **parallelization-by-realization** arm of `std/realization.dag` (Schedule/Placement/Width), the structural twin of caching's Materialization arm; both derived from one `EffectShape`-independence authority. Naming the knobs here is the construction wall (§5): once each is *derived* from the envelope, "jobserver says 120 but pids caps 4096" becomes unwritable.
