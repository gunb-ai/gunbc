...
- [ ] **expressibility frontier** — partition each modeling discipline into wall / lens-residue / undecidable-review *before* gating (mis-pricing = the §5 "never"-trap or §6 coverage-by-illusion); the construction-justification rule above is one tactic of it. Anemic-modeling + complexity are the first two instances. → [expressibility-frontier](docs/plans/expressibility-frontier.md)
...
- [ ] **floor runs the right things** — cadence = **two axes**: SELECTION (§4 affected-set, by *what changed*, fail-closed) vs SCHEDULING (§1, by cost+hermeticity); **cost informs scheduling, never selection**. [selection-vs-scheduling](docs/plans/ci-selection-vs-scheduling.md) · debug-amplification root-cause + Pop-A/Pop-B in [cause-table](docs/plans/expensive-test-cause-table.md)
  - [x] **opt-level=3 restores Pop-A to per-PR** — #5456 merged (detail: [cause-table](docs/plans/expensive-test-cause-table.md))
  - [ ] **per-PR = #5427 run-all sound baseline, shrunk to the affected set** (`v2.lens.affected_set`; bounded by affectedness, not cost) — #5427
  - [ ] **nightly = full-corpus selector-backstop + non-hermetic residue** — not a slow-test dump; residue-gated (§6); #5447 stood down (⚠ CI-gen load-bearing) — §1 (quick-ant-298)
