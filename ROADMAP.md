...
- [ ] **`precompute_whole_tree_published_mock_keys` not guarded** — the mock-key precompute at `cli_run.rs:3509` runs unconditionally before the execution-skip gate; #5971 skips witnesses on a disjoint diff but the precompute still runs, keeping wall-clock and peak-RSS high even when all 59 execution witnesses skip. Guard with `!diff_touches_published_mock_closure && all_witnesses_will_skip` (conjunction required — see §7; mock-closure check alone is insufficient). Orthogonal to tidy-hawk-120 #5959 M1 resolve memo (M1 cuts resolve cost when corpus runs; this guard cuts precompute cost when all witnesses skip). [plan](docs/plans/affected-set-precompute-pruning.md) — ⏳ awaiting sign-off
