...
**◆ Milestones:** output gated ✓ (#5434) · coproduct-exhaustiveness structural ✓ (#5441) · cross-rep-equality ✓ (#5449) · oracle-method mapped ✓ (#5471) → **▸ NOW: wiring-liveness oracle + execution preflight** → anemia lens? *(parked, likely advisory)*
...
- [x] affected-set = the completeness half (#5430) — model the full repo-process universe — **frontier now has a paying consumer: the wiring preflight (below)**
  - [ ] *anemia lens?* (parked, DESIGN §2 leaf-side) — likely advisory, not a hard gate; decide whether to elevate
- [ ] **wiring-liveness oracle** — a declared input must influence its output; the cache-purity oracle read backwards (perturbation-response, one kernel two readings). Decidability split (§5): static dataflow reachability over the Node DAG is the **wall** (declared input with no path to an output = unwired, caught without running); perturbation/fuzz is the runtime **confirmation** for the opaque-seed residue (`resolve_auth` is Rust, invisible to `.dag` reflection today). → [design](docs/plans/wiring-liveness-preflight.md)
  - [ ] **pre-send fail-closed guard** (narrowest seam) — declared-auth-but-no-token ⇒ typed `AuthDeclaredButUnwired` refusal before send, not a silent unauthenticated request *(coordinate with the realization-fix lane)*
  - [ ] **wiring witness — opaque-realization generator** — perturbation witness over declared service input→request relations; RED when an input is dropped (the execution-grounded generalization of the auth case)
  - [ ] **wiring lens — static reachability** — over `.dag`-modeled input→output relations; grows to subsume the seed realizations as they self-host (§5/§7)
  - [ ] **execution preflight** — run the wiring lens over the `affected_set` `ReExecFrontier` (the isolated about-to-run slice) before the interpreter executes a section; `FailClosed` on a dead wire. Negligible compute (static over the frontier, no I/O) — the affected-set frontier's first paying consumer
