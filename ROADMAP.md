...
**Status — 2026-06-16 (wave reconciliation; unmerged work counts as *not done*).** Landed on main this wave: the `03_normalize → dispatch` body-lowering dissolution (#5019); the CI floor collapsed to a single modeled `.dag` binary (`dsl/gunbc/tools/ci_runner.dag`, #4960), with the per-gate wrapper scripts dissolved into the unified affected-tests gate (#5012, #5013) and the hand-maintained corpus/test-row enumerations replaced by reflection discovery (#5005). At product / `extdeps` scale: gunbhub M1/M2 (canonical-markup consumer + served-render surface, #4966, #4993); SPICE **SP-M1** — a composed RC circuit emitting an ngspice netlist that *simulates* (#4989, fixture #4967); and §10 OS slices (freestanding-ELF boot-0.5 #4935, exec-contract Slice 2 #4944).

**Not done — built and reviewed but closed unmerged, so absent from main.** **§4 (first runnable v4 program with I/O) did not land:** the effect-IO first-consumer (#5000) and the §4→TypeScript walkthrough/emit (#4980, #4988) were authored and approved but never merged, so §4 has no presence on main. Likewise not landed, and therefore still open design directions rather than completed work: the `time_affine` carrier (#4928); the §10 OCI container-runtime witness (#4971); the `compute_fabric` CI-runner-pool projection (#4976); the ProbeSelector↔ctrl consumer and its `affected_set` witness (#1625, #1671, #4981); the SPICE resistor-line emit/parse and the SPICE↔Modelica dual-emission (#4974, #4986); and the enforce-over-external-`.dag` host (#4951).
