...
- **v2 Rust emitter generic-bound fidelity blocks generic-helper dissolution**: the attempted Algebra Quartet total-order helper extraction surfaced a real authority/realization split: `.dag` declared unbounded `fn min_by<T>(...)`, while the v2 Rust emitter realized generic functions as `T: Clone`, silently inventing a stronger target-layer contract. The extraction was rolled back rather than normalize a hidden bound. Dissolution/prerequisite: generic function emission must preserve authority-declared bounds exactly (or the model must carry explicit bounds) before any future generic-helper lane lands. Applies to the deferred quartet remainder (`Encoding`, `SubValueRelation`) and likely any future generic `Lookup<T>` consolidation that depends on the same emission path. P1 Modeling Faithfulness / P2 Boundary Discipline receipt. Owner: unassigned; prerequisite lane before future generic-helper dissolution.
