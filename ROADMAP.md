...
### Post-merge debt (2026-04-22 deferred-from-wave)

Tracked debt not dispatched with the 2026-04-22 lane wave (Lane 1e P3.0, SG-3g-b, SG-2c-4, SG-4b-3, cascade cleanup, algebra inhabitance quartet). Opportunistic slots — no blocking dependencies, can be picked up whenever bandwidth frees.
...
- **"Return first match" cascade cleanup** (now dispatched in the 2026-04-22 wave; listed here only to cross-reference): `src/v3/compiler/src/dag.rs:1999` and `:2023`, and ROADMAP:169 row, all inherit the "return the first match or fail-closed on multiple" phrasing from #625. Correct dissolution target is fail-closed-only. Owner: 2026-04-22 wave (cascade cleanup micro-lane).
- **Complexity lens v2/v3 comparison + cementing test**: v3's `src/v3/lenses/complexity.dag` (162 lines, 🟢 TERMINAL) is wired into the registry as the "cost" entry but has no receipt against v2's 5488-line heuristic `src/v2/complexity.dag`, and no test cements v3's current output so regressions would pass silently. Thesis-receipt lane: ~13× LOC reduction with structurally stronger fail-closed guarantees, awaiting demonstration. Also rolls in the `lens_cost_entry` → `lens_complexity_entry` registry-name cleanup (Phase 1 cosmetic). Dispatch brief: `docs/briefs/complexity-v2-v3-comparison-receipt.md`. Owner: unassigned; queue for next wave after 2026-04-22's 5 lanes land.
