...
- **`pipeline_authority.rs` stage-order residual post-#637**: #637 (Pipeline Authority — structural-read cleanup) closed the **stage-bindings** portion — `PipelineStageBinding` records are now consumed structurally via `PIPELINE_STAGE_BINDING_TYPE` (see `src/v3/compiler/src/pipeline_authority.rs:9`). **Residual**: `pipeline_compile_order_names()` at `:225-275` still `include_str!`s `pipeline.dag`, slices the compile-fn body span, and iterates `body.lines()` for stage order. The stage-order fact lives in the `compile` fn's procedural body; reading it structurally requires substrate capability for ordered-body reflection that doesn't exist yet. This is a substrate-capability gap masquerading as partial-cleanup residual. Dissolution trigger: when `.dag` authority gains ordered-body reflection (related to SG-2c-proper emit-capability program), replace text-slicing with typed read. Owner: unassigned; blocked on substrate capability.
...
- **Compiler–`std/` consolidation program — specific migrations**: end-state defined in [docs/thesis/compiler-std-consolidation.md](docs/thesis/compiler-std-consolidation.md). Each of the migrations below is a standalone lane; together they collapse the compiler-specific type surface toward the positive definition (pipeline + regen + lens-specific return-type carriers + accessor). **Ratchet:** count of `type` declarations in `src/v3/compiler/*.dag` AND `src/v3/lenses/*.dag` that are NOT in the positive-def set AND NOT exempted → 0. Positive-def: pipeline/regen types + lens-API return carriers (`Origin`, `UnusedParameter`, `VariantPayloadShapeLookup` 3-variant, `TemplateArgumentBinding` 3-variant, etc.). Exempted: `parse_tables.dag` (4 types, pending SG-2c-proper per-row classification). In-ratchet: tokenize (6) + runtime_mirrors (14) + **strict 2-variant Missing|Found Lookup-pattern carriers across lenses (3 today: `CostLookup`, `SymbolicCostLookup`, `TemplateArgumentLookup`)**. Baseline: 23. See thesis doc table for per-file disposition.
...
- **Target emit-spec fabrication sentinels — `__EMIT_BUG_{0}__` divergence across 3 target specs**: `dsl/extdeps/languages/dag/emit.dag:54`, `go/emit.dag:107`, and `python/emit.dag:115` all declare `*_error_type_template: String = "__EMIT_BUG_{0}__"` — a sentinel-string fabrication pattern when emit encounters an error-reference in source. `dsl/extdeps/languages/rust/emit.dag:141` correctly uses `compile_error!(\"{0}\")` — Rust's fail-closed form catches the error at Rust-compile time. Target behavior already diverges between fail-closed and fabricated-output modes. P3 Fail-Closed / C-6 violation ("emit may not use `<error:unknown_*>` sentinels to preserve progress"). Dissolution: converge the three target emit specs on their language's native compile-time error form (`panic!()` at compile would be valid Rust but not the right shape; each target needs its fail-closed equivalent). Surfaced by 2026-04-22 exploratory analysis. Owner: unassigned; S scope — single-line change per target spec, plus verification that emit consumers expect the new template shape.

- **`algebra.dag` declaration-vs-template parallel authority**: `dsl/std/algebra.dag:176` declares `OrderedRing<T>` with 16 fields (add/sub/zero/negate/mul/div/one/compare/quotient/remainder/eq/ne/lt/le/gt/ge); `dsl/std/algebra.dag:447` declares `ordered_ring_templates()` returning 9 fields plus `clamp` not in the type declaration. Two authorities for the same algebra surface, already drifted. Same pattern for `PartialFunction<K,V>` declaration (`:315-325`) vs `partial_function_templates()` (`:553-569`). **Distinct** from the already-tracked "algebra.dag signature/comment reconciliation" debt (that's about comment prose vs declaration — documentation drift). This is **code-level parallel authority**: `*_templates()` functions are a second authority that emit code or dispatch on the same surface. Dissolution: `*_templates()` functions should be derived from the type declaration, not independently authored. P2 Boundary Discipline receipt. Surfaced by 2026-04-22 exploratory analysis. Owner: unassigned; M scope — requires deciding whether template tables are needed at all, or should be replaced by declaration-walking.

- **`extdeps/browser.dag` typed-carrier service-boundary collapse**: `dsl/extdeps/browser.dag:21-32` declares typed carriers (`BrowserConfig`, opaque `BrowserContext` / `Page` / `Element` handles, imports `Url`), but service ops use raw `String`: `Launch.input.headless: String = "false"`, `output.context_id: String`; `Goto.input.url: String`, `wait_until: String`, `output.final_url: String`; selector/query/evaluate outputs all remain `String` (`:42-47,56-60,75-103`). Same class as the LLM service flattening (tracked above) and the GitHub auth model bypass — typed carriers declared in the same file, then dropped at the operational boundary. P1 Modeling Faithfulness / M8 "dispatch structural, not string extraction". Dissolution: route service ops through the declared carriers. Could pilot the typed-service-boundary pattern that also unblocks LLM + GitHub dissolution. Surfaced by 2026-04-22 exploratory analysis. Owner: unassigned; M scope per service family.
