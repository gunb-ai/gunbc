...
if has_catch_all {
                    ...
                    {
                        let covered_set = arms.clone().iter().cloned().fold(Rc::new(HashMap::new()) /* BRIDGE: fold empty_map value type unresolved */, |acc: _, arm: Rc<Node>| match (*arm_pattern(arm.clone())).clone() {
