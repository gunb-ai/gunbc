...
echo "=== MUTATION 3: effect receipt length check dropped -> extra-effect witness must go FALSE (drop witness stays TRUE via containment join)"
python3 - <<PY
s=open("$SC").read()
...
open("$SC","w").write(s)
PY
run1 $SEM an_extra_unexpected_effect_is_a_host_realization_defect
