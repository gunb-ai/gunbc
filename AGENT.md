...
### Meta-Modeling for Build Artifacts (January 2026)

Build artifacts (Makefile, .gitignore, ci.yml) are generated from centralized sources of truth:

**Core Types:**
- `ToolRegistry` — Declarative list of tools with entrypoints, descriptions
- `BuildConfig` — Build system commands (cargo/buck2)
- `Renderable` trait — Standardized file generation with headers

**Key Files:**
- `lib/tools/makegen/src/registry.rs` — `ToolRegistry`, `BuildConfig`, `MetaTarget`
- `lib/tools/makegen/src/render.rs` — `MakefileRenderer` implements `Renderable`
- `lib/tools/makegen/src/gitignore.rs` — `GitignoreRenderer`, `IgnoreCategory`
- `core/ir/src/render.rs` — `Renderable` trait definition

**Renderable Pattern:**
```rust
pub trait Renderable {
    fn generator_name(&self) -> &str;       // e.g., "gunbc-makegen"
    fn regenerate_command(&self) -> &str;   // e.g., "make makegen"
    fn format_id(&self) -> &str;            // e.g., "makefile", "yaml"
    fn render_content(&self) -> String;     // Body without header
    
    fn render(&self) -> String {            // Full output with header
...
**Adding a New Tool to ToolRegistry:**

1. Edit `lib/tools/makegen/src/registry.rs`:
   ```rust
   pub fn default_registry() -> Self {
       ...
       registry.register(ToolInfo {
           crate_name: "gunbc-mytool",
           short_name: "mytool",
           description: "Does something useful",
           entrypoints: vec![
               EntrypointParam::new("input", "INPUT", "--input", "String"),
           ],
           extra_targets: vec![],
       });
...
2. The Makefile target is auto-generated with:
   - `make mytool` — runs the tool
   - `make mytool-dry` — dry run mode
   - Help text included automatically

### Language Module for Code Generation (January 2026)

Centralized language/format definitions in `core/ir/src/language/`:

**Purpose:** Eliminate hardcoded patterns for comments, naming conventions, and type mappings across code generators.

**Key Types:**
- `NamingCase` — Case conversion (SnakeCase, PascalCase, KebabCase, etc.)
- `CommentSyntax` — Comment syntax per format (`#`, `//`, `///`, `/* */`)
- `MakefileConfig`, `RustConfig`, `GitignoreConfig` — Format-specific settings

**Usage Examples:**

```rust
use gunbc_ir::language::{NamingCase, MAKEFILE, rust_type};
use gunbc_ir::language::traits::comment::{comment_syntax_for, generated_header};

// Naming conventions
let flag_name = NamingCase::KebabCase.apply("my_option");  // "my-option"
let type_name = NamingCase::PascalCase.apply("my_tool");   // "MyTool"

// Comment syntax
let syntax = comment_syntax_for("rust").unwrap();
assert_eq!(syntax.line_prefix, "//");

// Generated file headers
let header = generated_header("gunbc-mytool", "make mytool", "#");
// "# Generated by gunbc-mytool\n# DO NOT EDIT - regenerate with: make mytool"

// Type mapping
let rust = rust_type("Int");  // "i64"
let rust = rust_type("List<String>");  // "Vec<String>"

// Makefile indentation (tab is required)
assert_eq!(MAKEFILE.indent, "\t");
```

**Module Structure:**
```
core/ir/src/language/
├── mod.rs              # LanguageOp enum, NamingCase, build_languages_dag()
├── patterns/           # Regex, Glob, Variable syntax SubDags
├── traits/             # TypeSystemMapping, NamingConventions, CommentPrefix
├── categories/         # TuringComplete, ConfigFormat SubDags
└── languages/          # Rust, Makefile, Gitignore SubDags
```

**Integration with Renderable:**
The `Renderable` trait uses `format_id()` → `comment_syntax_for()` for automatic comment prefix selection.

**Related files:**
- `TODO/meta-modeling-consolidation.md` — Design document for consolidation
