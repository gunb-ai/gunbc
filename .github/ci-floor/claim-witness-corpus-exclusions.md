...
| `v4_lens_ci` node-frontier rows (`affected_set_ci_runner.dag`) | Already gated by `scripts/v4-affected-set-node-frontier-gate.sh` (`--green-only` in `v4_lens_ci`; perturb shards in `v4_lens_ci_perturb`) |
