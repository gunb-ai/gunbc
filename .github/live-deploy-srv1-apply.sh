...
# Mutation rides five sudo -n grants; tree sync AS service user via gunbc-tree-sync.service.
...
sudo -n /usr/bin/tailscale version >/dev/null || { echo "deploy_access: typed refusal before mutation — privileged commands not granted (sudo -n preflight miss)"; exit 1; }
