...
// Obligations: 14 obligations (3 discharged, 11 testable: A=5, B=4, C=2, D=0)
// Proven by construction: acyclicity, type compatibility, cardinality satisfaction.
// Content-Hash: a958d20d738a2328bde57dfec7f470f5dcb6c24d37c54075c65f9618c8fe47bd
...
fn test_transport_interception() {
    ...
    assert!(result.boundary_nodes.iter().any(|n| n == "execute_transport"), "transport executor 'execute_transport' should be in intercepted list");
}
...
fn test_scenario_execute_transport_fails() {
    ...
    mocks.set_value("execute_transport", "response", Value::Str("<TRANSPORT_FAILURE>".to_string()));
...
/// Window: load_registry -> render_makefile
#[test]
fn test_window_load_registry_through_render_makefile() {
    let dag = crate::build_makegen_graph().unwrap();
    ...
    let window = Window::from_nodes(&flat, vec!("load_registry", "render_makefile"));
    let mut mocks = mock_spec().to_boundary_mocks();
...
/// Window: render_makefile -> prepare_file_write
#[test]
fn test_window_render_makefile_through_prepare_file_write() {
    let dag = crate::build_makegen_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("render_makefile", "prepare_file_write"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: prepare_file_write -> execute_transport
#[test]
fn test_window_prepare_file_write_through_execute_transport() {
    let dag = crate::build_makegen_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("prepare_file_write", "execute_transport"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: load_registry -> prepare_file_write
#[test]
fn test_window_load_registry_through_prepare_file_write() {
    let dag = crate::build_makegen_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("load_registry", "render_makefile", "prepare_file_write"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: render_makefile -> execute_transport
#[test]
fn test_window_render_makefile_through_execute_transport() {
    let dag = crate::build_makegen_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("render_makefile", "prepare_file_write", "execute_transport"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: load_registry -> execute_transport
#[test]
fn test_window_load_registry_through_execute_transport() {
    let dag = crate::build_makegen_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("load_registry", "render_makefile", "prepare_file_write", "execute_transport"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
