...
/// Workspace-root globs for generated local state.
///
/// These directories are created by runtime workflow ledgers and ignored
/// end-to-end codegen compile tests. They are never source-of-truth.
const GENERATED_LOCAL_STATE_ROOT_GLOBS: &[&str] = &[
    "/.gunbc/",
    "/e2e_codegen_test*/",
];
...
pub fn derive_categories(config: &BuildConfig) -> Vec<Category> {
    ...
    categories.push(Category {
        name: "Generated local state".into(),
        source: Some("generated-state".into()),
        items: GENERATED_LOCAL_STATE_ROOT_GLOBS
            .iter()
            .map(|glob| (*glob).into())
            .collect(),
        rationale: Some(
            "Generated runtime/test scratch state rooted at workspace top-level".into(),
        ),
    });
    categories.push(Category {
...
mod tests {
    ...
    fn test_render_gitignore_has_patterns() {
        ...
        assert!(content.contains("/.gunbc/"));
        assert!(content.contains("/e2e_codegen_test*/"));
    }
