...
/// Generate CI workflow YAML files via DSL tool (CG-4).
///
/// Builds and executes the `tools/cigen.dag` DSL graph, which renders
/// GitHub Actions and GitLab CI YAML through pure DSL rendering functions.
/// The only Rust-side work is `discover_ci_config()` (extern func).
fn cmd_cigen(dry_run: bool) {
    println!("gunbc-codegen: cigen (DSL)");
    println!("  mode: {}", if dry_run { "dry-run" } else { "real" });
    println!();
    ...
    match gunbc_exec::execute_with_mode_and_inputs(&dag, mode, None) {
        Ok(log) => {
            // Report results from the execution log
            let github_written = log
                .get("tools.cigen::cigen")
                .and_then(|entry| entry.outputs.get("github_written"))
                .and_then(|v| v.as_bool());
            let gitlab_written = log
                .get("tools.cigen::cigen")
                .and_then(|entry| entry.outputs.get("gitlab_written"))
                .and_then(|v| v.as_bool());

            let github_status = match (dry_run, github_written) {
                (true, _) => "dry-run",
                (_, Some(true)) => "written",
                (_, Some(false)) => "unchanged",
                _ => "ok",
            };
            let gitlab_status = match (dry_run, gitlab_written) {
                (true, _) => "dry-run",
                (_, Some(true)) => "written",
                (_, Some(false)) => "unchanged",
                _ => "ok",
            ...
            println!("  [ci] .github/workflows/ci.yml ({})", github_status);
            println!("  [ci] .gitlab-ci.yml ({})", gitlab_status);
            println!();
...
mod tests {
    ...
    /// CI YAML generation is now DSL-driven (CG-4).
    /// Validation of the rendered templates happens via the DSL compiler
    /// (type checking) and the cigen DSL graph test in dsl_builder::tests.
    #[test]
    fn cigen_dsl_graph_builds_successfully() {
        let dag = gunbc_dag::dsl_builder::build_dsl_graph_for_entrypoint(
            "tools/cigen.dag",
            Some("cigen"),
        )
        .expect("cigen DSL graph should compile and resolve");
