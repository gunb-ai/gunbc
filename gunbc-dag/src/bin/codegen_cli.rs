...
fn cmd_cigen(dry_run: bool) {
    ...
    println!("gunbc-codegen: cigen (DSL tool)");
    println!("  mode: {}", if dry_run { "dry-run" } else { "real" });
    println!();

    let dag = match build_dsl_graph_for_entrypoint("tools/cigen.dag", Some("cigen")) {
        Ok(dag) => dag,
        Err(e) => {
            eprintln!("  [cigen] DSL graph build failed: {e}");
            std::process::exit(1);
        }
    };

    let mode = if dry_run {
        ExecutionMode::DryRun(BoundaryMocks::default())
    } else {
        ExecutionMode::Real
    };
    execute_and_display(&dag, mode, false, None, None);
}
