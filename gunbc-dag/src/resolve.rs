...
impl Executable for ServiceGcpStsExchangePrepareOp {
    fn execute(&self, inputs: HashMap<String, Value>) -> Result<HashMap<String, Value>, ExecError> {
        ...
        OutputMap::new()
            .request(
                "request",
                TransportRequest::Rest(RestRequest::post(
    ...
    fn execute(&self, inputs: HashMap<String, Value>) -> Result<HashMap<String, Value>, ExecError> {
        match inputs.get("response") {
            Some(Value::Response(TransportResponse::Rest(rest))) => {
                ...
                let payload = Value::List(bytes.into_iter().map(|byte| Value::Int(byte as i64)).collect());
                OutputMap::new().value("payload", payload).str("name", name).ok()
            }
            Some(Value::Skipped) | None => OutputMap::new()
...
mod tests {
    ...
    fn resolve_literal_source_op_emits_constant_output() {
        ...
        let outputs = result.execute(HashMap::new()).expect("literal source executes");
        assert_eq!(
    ...
    fn resolve_invalid_literal_source_fails_at_execution() {
        ...
        let result = resolve_node(&node).expect("invalid literal source should resolve to error op");
        let err = result
            .execute(HashMap::new())
