...
/// Window: prepare_scan_workspace -> execute_scan_workspace
#[test]
fn test_window_prepare_scan_workspace_through_execute_scan_workspace() {
    let dag = crate::build_bootstrap_graph().unwrap();
    ...
    let window = Window::from_nodes(&flat, vec!("prepare_scan_workspace", "execute_scan_workspace"));
    let mut mocks = mock_spec().to_boundary_mocks();
...
/// Window: prepare_scan_workspace -> parse_scan_result
#[test]
fn test_window_prepare_scan_workspace_through_parse_scan_result() {
    let dag = crate::build_bootstrap_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("prepare_scan_workspace", "execute_scan_workspace", "parse_scan_result"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: prepare_scan_workspace -> generate_gitignore
#[test]
fn test_window_prepare_scan_workspace_through_generate_gitignore() {
    let dag = crate::build_bootstrap_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("prepare_scan_workspace", "execute_scan_workspace", "parse_scan_result", "generate_gitignore"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
fn test_window_parse_scan_result_through_prepare_gitignore_write() {
    ...
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("parse_scan_result", "generate_gitignore", "generate_makefile", "prepare_gitignore_write"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
/// Window: prepare_scan_workspace -> generate_makefile
#[test]
fn test_window_prepare_scan_workspace_through_generate_makefile() {
    let dag = crate::build_bootstrap_graph().unwrap();
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("prepare_scan_workspace", "execute_scan_workspace", "parse_scan_result", "generate_gitignore", "generate_makefile"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
fn test_window_execute_scan_workspace_through_prepare_gitignore_write() {
    ...
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("execute_scan_workspace", "parse_scan_result", "generate_gitignore", "generate_makefile", "prepare_gitignore_write"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
...
fn test_window_parse_scan_result_through_prepare_makefile_write() {
    ...
    let flat = lower(&dag).expect("lower should succeed");
    let baseline = execute_with_mode(&dag, ExecutionMode::DryRun(mock_spec().to_boundary_mocks())).expect("baseline DryRun should succeed");
    let window = Window::from_nodes(&flat, vec!("parse_scan_result", "generate_gitignore", "generate_makefile", "prepare_gitignore_write", "prepare_makefile_write"));
    let mut mocks = mock_spec().to_boundary_mocks();
    apply_window_inputs(&flat, &window, &baseline, &mut mocks).expect("window inputs should be derivable from baseline");
    let window_dag = window_subdag(&flat, &window);
    let log = execute_with_mode(&window_dag, ExecutionMode::DryRun(mocks)).expect("window execution should succeed");
