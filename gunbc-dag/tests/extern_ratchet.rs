...
/// Increased from 2 → 3: added `discover_ci_config` for CG-3 (CI YAML generation DSL migration).
const EXTERN_FUNC_DECL_BASELINE: usize = 3;

/// Current baseline: number of extern implementations in `all_extern_symbols()`.
/// Increased from 6 → 7: added `discover_ci_config` for CG-3 (CI YAML generation DSL migration).
