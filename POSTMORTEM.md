...
**Design decision — interpreter-backed fallback nodes are banned (Invariant 7).** The TBD is resolved: interpreter-backed nodes (`ExprCompute`, `PipeOp`, `ForOp`) must not exist in the lowered DAG. Every expression either lowers to structural nodes or the compilation fails. See `DESIGN_INTERP_FALLBACK.md` for the full catalog of 18 violation sites, their root causes, and the dependency order for elimination.
...
The compiler was intended to have a strict zero-fallback policy: every compilation path either succeeds fully or fails with a clear error (now codified as README Invariants 7 and 8). An audit found ~70 sites across 7 layers where the compiler silently degrades instead of failing. This is the root cause of most items in this document. The interpreter-fallback subset (Layer 2/3, ~18 sites) is cataloged in `DESIGN_INTERP_FALLBACK.md`.
