...
| `src/v3/compiler/tests/integration/r3_substrate_gap_file_ingestion_test.rs` | **Plan:** `docs/r3-program-plan.md` §1.8 ledger row **#62** `substrate_gap_file_ingestion_closed` (external UTF-8 ingested without `include_str!`; see `docs/r3-structure.md` substrate-gap-class bullet for the same gate). **ROADMAP:** `ROADMAP.md` § **Forward-Tracked Lane: T-Workflow-As-Data** (closure predicate for file-ingestion / timing substrate work). **Dissolution:** remove when workflow-as-data carriers own timing/JSON ingestion structurally and compile-time `read_utf8_file` lowering retires to that substrate (no `lower.rs` special case). **Interim ratchet:** `dsl/std/render.dag` (bootstrap excerpt) declares `read_utf8_file`; `lower.rs` expands only calls resolved to that declaration id; `r3_substrate_gap_file_ingestion.dag` + this integration file pin `Compiles` + `..` rejection. |
