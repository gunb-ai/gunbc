...
### Problem shape: Path-exponential traversal of the shared graph (DB-15)

The substrate `Node` graph is a **shared DAG by design** — single-authority means a concept (a type, an algebra carrier, a target primitive) is asserted **once** and referenced from many sites (one `i32` model node, referenced three times by a two-argument `i32` signature and from every `i32`-typed value). A traversal written as naive tree-recursion — a catamorphism that recurses into every edge target with no dedup — re-processes each shared node **once per path** that reaches it. Root-to-leaf path count grows exponentially with diamond-sharing while the distinct-node count stays linear, so a structurally tiny graph folds in exponential time. The traversal **terminates** (the graph is finite and acyclic): this is a **redundant-work / performance defect, not a boundedness (P4) violation** — but it is intractable in practice, and it re-derives a once-asserted node per reference (the "assert once" sustainability rule violated at the traversal level).

**Solution shape:** A substrate traversal processes each **distinct node at most once** — sharing-aware folding: memoize the fold result by node identity / content-hash (hash-consing). Visit-once is simultaneously (a) the performance fix, `O(paths)` → `O(nodes + edges)`; (b) the bounded-forward-execution "never revisiting" traversal; and (c) a by-construction termination witness — the monotone unvisited-set is the descent measure (`std/cardinality.dag` `DescentEvidence`), so it subsumes the explicit-fuel bound **for the node-fold family only**. It does **not** cover recursion that is not a structural node-fold (inference fixpoints, worklist passes) — those still carry their own bound; do not over-read "memoization gives termination for free." A naive path-recursive fold over the shared graph is forbidden; consume the memoized / visit-once traversal primitive.

**Receipt:** `fold_node` (`std/node.dag`) shipped as a non-memoized catamorphism and was marked "DONE" by typecheck — the no-consumer disease (consumer invariant E-10, PR #4405) landing at the foundation, the point furthest from any consumer. The first real consumer to fold a real shared `Node` — `emit` on `fn add` — hung >600s (counting one tiny function's nodes alone ran >75s while unrelated witnesses ran in 60ms), because the `i32` inhabitant the signature references 3× is a deep shared sub-DAG and the fold walked paths, not nodes. Dissolution: memoize the fold by content-hash (`fold_node_content_hash` is half the machinery); the executed `fn add` emit witness is the consumer that proves it. Detail: `docs/v4-compiler-migration.md` Part 0 / S0 (PR #4405).
...
- **DB-15: Substrate Traversals Are Sharing-Aware (Visit Each Distinct Node Once)** — a fold/walk over the shared-DAG `Node` substrate processes each distinct node at most once (memoize by content-hash); a naive path-recursive catamorphism is a forbidden redundant-work defect (path-exponential on a node-linear structure). Visit-once is the node-fold family's structural termination witness; it does not retire bounds for non-fold recursion.
