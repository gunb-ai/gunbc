...
- **(e) Drains and authority.** Any `read` on a child stream for **presource management** (e.g. **nonblocking drain** so a pipe cannot fill and stall) must not **erase** byte ranges that a **different** step later treats as **evidence** for a distinct outcome (e.g. setup-failure vs exit mismatch on the *same* fd in quick succession). Either those bytes are **captured** into the same **bounded** budget the downstream scan uses, or the later step is documented as not depending on that stream; **no split-brain** where the drain is discard-only and a later string scan is still claimed as the authority. (Meta-review, PR #792, 2026-04-25, codex `b558fc53` — same *authority-boundary* class as “drain to fix P4” vs “stderr scan for P3/DB-1.”)

**Receipt / audit hook:** T-PB-B and PB-Runtime work streams consume this boundary through `src/v3/compiler/src/test_runner.rs` (`evaluate_execute_command_exit_code` and shared parsers). Rounds of review in PR #792 (2026-04-25) converged the same *class* of find — fail-closed holes at the host / typed-channel / **shared-fd** border — with per-instance fixes each tagged (TODO, dissolution, `cfg!(test)` triggers). The structural move is: **one invariant, one audit of the function against (a)–(e)**, then ship or **refactor into a typed decision/result model** (small explicit policy table) if the audit is not obvious, instead of an unbounded sequence of ad hoc patches at the same seam. (Normal per-commit review is **negative marginal value** until that audit lands — meta `b558fc53`.)
