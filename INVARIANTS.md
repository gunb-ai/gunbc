...
| `src/v3/compiler/tests/integration/cementing/cementing_provenance_origin_integration_test.rs` | **ROADMAP:** `ROADMAP.md` — Post-merge debt bullet **v3 lens capability honesty pass** (cementing-test discipline per `regen.dag`; see `docs/v3-lens-capability-register.md`). **Dual receipt:** `tests/dag/t_r3_gate_87_cementing_regen_provenance.dag` carries PB-B-1 `TestClaim` obligations for the literal/sum glue programs (narrow Int projections for variant shape). **Dissolution:** remove when `Origin` expected values (including carried `NodeId` equality vs producer authority) are authorable in `.dag` verification data so the harness alone carries payload-level behavior without this Rust mirror. **Interim ratchet:** `provenance_origin_of_cements_complete_row_via_compile_to_dag_fixture` pins the full `origin_of` ↔ `Behavior` mirror per `src/v3/lenses/provenance.dag`. |
