...
| `src/v3/compiler/tests/integration/r3_gate_87_lens_cementing_regen_receipts_test.rs` | **ROADMAP:** `ROADMAP.md` → `### Nine lanes` → **T-PB-B** (`pb_rust_tests_outside_residual_zero`). **Plan:** `docs/r3-program-plan.md` §1.8 gate **#87** `lens_cementing_test_discipline_complete`. **Dissolution:** retire when each `regen.dag` lens obligation is carried only by `.dag` gate-87 harness claims through `t_pb_b_1_dag_runner_test` / `cementing_dispatch` without paired Rust `compile_to_dag` on lens sources or host seam checks in this module. **Interim ratchet:** `r3_gate_87_regen_lens_registry_names_match_fixture_inventory` + per-lens `#[test]` receipts stay aligned to `R3_GATE_87_CEMENTING_REGEN_SUITES` and `tests/dag/t_r3_gate_87_cementing_regen_<name>.dag`; new `LensRegistryEntry` rows extend those artifacts in the **same PR** as the registry edit (INVARIANTS P2 single authority). |
