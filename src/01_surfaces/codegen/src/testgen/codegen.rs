...
pub struct TestGenerator<'a, T> {
    ...
    /// Failure-variant MockSpecs for error-path test generation (RV-4).
...
impl<'a, T: Clone + 'static> TestGenerator<'a, T> {
    ...
    /// Set failure-variant MockSpecs for error-path test generation (RV-4).
    ///
    /// Each `FailureVariant` contains a tag (e.g., "shell_exit_1") and a MockSpec
    /// with one transport boundary replaced by a failure response. Generated tests
    /// verify that DryRun execution completes without panicking under each
    /// failure scenario.
    pub fn with_failure_variants(mut self, variants: Vec<FailureVariant>) -> Self {
        self.failure_variants = variants;
        self
    }
    ...
    fn generate_test_file(
        ...
        // RV-4: Failure variant tests need execute_dag + ExecuteConfig + ExecutionMode.
        if self.config.failure_variant_tests && !self.failure_variants.is_empty() {
            file.imports.push(Import {
                path: vec!["gunbc_exec".to_string()],
                items: vec![
                    "ExecuteConfig".to_string(),
        ...
        if !invalids.is_empty() {
            ...
            for (i, obligation) in invalids.iter().enumerate() {
                ...
                tests.push(TestFn {
                    ...
                    attributes: vec!["#[should_panic]".to_string()],
        ...
        // RV-4: Failure variant tests (error-path coverage per transport boundary)
        if self.config.failure_variant_tests {
            if let Some(section) = self.build_failure_variant_section(graph_builder_fn) {
                file.sections.push(section);
            }
        }
                    ...
                    body: vec![
                        ...
                        Stmt::Item(Item::Raw(match kind {
                            gunbc_ir::coerce::CoercionKind::WrapScalar => format!(
                                "assert!(matches!(received, Value::List(_)), \"WrapScalar should deliver list to {}.{}; got {{:?}}\", received);",
                                to_node.0, to_port.0
                            ),
    ...
    fn build_optional_input_tests(
        ...
        for obligation in &optional_obligations {
            ...
            if matches!(
                node.kind,
                NodeKind::TransportPrepare | NodeKind::TransportParse
            ) {
                continue;
            }
...
impl<T: Clone + 'static> TestGenerator<'_, T> {
    ...
    /// Build the failure-variant test section (RV-4).
    ///
    /// Generates one test per failure variant tag. Each test builds the DAG,
    /// reconstructs the success-path MockSpec via `mock_spec()`, derives
    /// failure variants via `auto_mock_failure_variants()`, selects the
    /// variant matching the tag, and runs DryRun. The test verifies that
    /// execution completes without panicking -- the DAG should handle
    /// individual transport failures gracefully.
    fn build_failure_variant_section(&self, graph_builder_fn: &str) -> Option<TestSection> {
        if self.failure_variants.is_empty() {
            return None;
        }
        // Without mock_spec_fn, we can't generate runtime mock_spec() calls.
        // Failure variant tests need the base mock_spec to exist at test-time.
        self.mock_spec_fn.as_ref()?;

        let mut tests = Vec::new();

        for variant in &self.failure_variants {
            let safe_tag = NamingCase::SnakeCase.apply(&variant.tag);
            let test_name = format!("test_failure_{safe_tag}");
            let tag_literal = variant.tag.clone();
            ...
            let body = vec![
                ...
                Stmt::let_bind(
                    "variants",
                    Expr::call(
                        "gunbc_test::auto_mock_failure_variants",
                        vec![Expr::var("spec").ref_of()],
                    ),
                ),
                Stmt::let_bind(
                    "variant",
                    Expr::raw(format!(
                        "variants.iter().find(|v| v.tag == \"{tag_literal}\").expect(\"failure variant '{tag_literal}' should exist\")"
                    )),
                ),
                Stmt::let_bind(
                    "mocks",
            ...
            tests.push(TestFn {
                name: test_name,
                doc: vec![
                    format!("Failure variant: {}.", variant.tag),
                    String::new(),
                    ...
                    "One transport boundary is replaced with a failure response while all"
                        .to_string(),
                    "other boundaries keep their success values.".to_string(),
                ],
                attributes: vec![],
                body,
            });
        ...
        Some(TestSection {
            title: "RV-4: Failure Variant Tests".to_string(),
            notes: vec![
                "Each test replaces one transport boundary mock with a failure response"
                    .to_string(),
                "(e.g., shell exit 1, REST 401/500, file not found) and verifies that".to_string(),
                "DryRun execution completes without panicking.".to_string(),
            ],
            tests,
        })
    }
...
fn render_output_matcher_check(matcher: &OutputMatcher, var_name: &str) -> Vec<Stmt> {
    ...
    match matcher {
        ...
        OutputMatcher::NonEmpty => vec![Stmt::Item(Item::Raw(format!(
            "assert!(!{output_var}.is_empty() || matches!(*{output_var}, Value::Skipped | Value::Unit), \"expected non-empty value, Skipped, or Unit\");"
        )))],
        OutputMatcher::IsBool => vec![Stmt::Item(Item::Raw(format!(
            "assert!({output_var}.as_bool().is_some() || matches!(*{output_var}, Value::Skipped), \"expected Bool or Skipped for {output_var}\");"
        )))],
        ...
        OutputMatcher::IsResponse => vec![Stmt::Item(Item::Raw(format!(
            "assert!({output_var}.as_response().is_some() || matches!(*{output_var}, Value::Skipped), \"expected Response or Skipped for {output_var}\");"
        )))],
...
/// Check if a value is a placeholder witness like `<TypeName>`.
...
fn witness_value_for_count(
    ...
    let nonzero = witnesses
        ...
        .map(|w| w.value.clone())
        // Skip placeholder witnesses like `<CommentSyntax>` — these are
        // generated by scalar_witness_for_base for unknown base types
        // (product types) and should not be used as concrete mock values.
        .filter(|v| !is_placeholder_witness(v));
...
mod tests {
    ...
    fn test_coercion_coverage_asserts_target_input_shape() {
        ...
        assert!(
            code.contains("WrapScalar should deliver list to sink.in_items"),
    ...
    #[test]
    fn test_failure_variant_section_generates_tests() {
        use gunbc_ir::Node;
        use gunbc_test::{BoundaryMock, FailureVariant};

        // Build a simple DAG with a transport-like boundary node
        let mut dag: Dag<()> = Dag::new();
        dag.add_node(Node::opaque(
            "execute_shell",
            vec![],
        ...
        dag.add_node(Node::opaque(
            "parse",
            vec![port("response", "TransportResponse")],
            vec![port("stdout", "String")],
            (),
        ));
        dag.add_edge(edge("execute_shell", "response", "parse", "response"));
        ...
        let spec = MockSpec::new("test")
            .boundary(
                ...
                Value::Response(gunbc_ir::transport::TransportResponse::Shell(
                    gunbc_ir::transport::ShellResponse::ok("ok".to_string()),
                )),
            )
            .boundary("parse", "stdout", Value::Str("mock".to_string()))
            .skip_node_example("execute_shell")
            .skip_node_example("parse");

        let failure_variants = vec![FailureVariant {
            tag: "execute_shell__shell_exit_1".to_string(),
            spec: MockSpec {
                name: "test".to_string(),
                boundary_mocks: vec![BoundaryMock {
                    node: "execute_shell".to_string(),
                    port: "response".to_string(),
                    value: Value::Response(gunbc_ir::transport::TransportResponse::Shell(
        ...
        let code = generator.generate_test_module("test_module", "build_graph()");

        // Should contain the failure variant section header
        assert!(
            code.contains("RV-4: Failure Variant Tests"),
            "should contain RV-4 section header in generated code:\n{}",
            &code[..code.len().min(2000)]
        );

        // Should contain at least one failure test function
        assert!(
            code.contains("test_failure_"),
            "should contain at least one failure variant test function"
        );

        // Should call auto_mock_failure_variants at runtime
        assert!(
            code.contains("auto_mock_failure_variants"),
            "should reference auto_mock_failure_variants"
        );

        // Should use execute_dag
        assert!(
            code.contains("execute_dag"),
            "should use execute_dag for DryRun execution"
        );
    }

    #[test]
    fn test_failure_variant_section_empty_when_no_variants() {
        use gunbc_ir::Node;

        // Build a DAG with no transport boundaries
        let mut dag: Dag<()> = Dag::new();
        dag.add_node(Node::opaque(
            "pure_node",
            vec![port("input", "String")],
        ...
        let spec = MockSpec::new("test")
            .boundary("pure_node", "output", Value::Str("mock".to_string()))
            .skip_node_example("pure_node");
        ...
        // Should NOT contain failure variant section
        assert!(
            !code.contains("RV-4: Failure Variant Tests"),
            "should not contain RV-4 section when no failure variants"
        );
    }
}
