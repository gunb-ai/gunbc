...
mod tests {
    ...
    fn emit_exec_runtime_uses_configured_dependency_paths() {
        ...
        let config = EmitConfig {
            dependency_paths: DependencyPaths {
                gunbc_ir: "../../../src/0_foundation/ir".to_string(),
                gunbc_exec: "../../../src/9_execute/exec".to_string(),
                gunbc_lib_transport: "../../../src/8_materialize/transport".to_string(),
            },
        };
        ...
        assert!(
            cargo_toml
                .content
                .contains("gunbc-ir = { path = \"../../../src/0_foundation/ir\" }"),
            "Cargo.toml should use the configured gunbc-ir path"
        );
        assert!(
            cargo_toml
                .content
                .contains("gunbc-exec = { path = \"../../../src/9_execute/exec\" }"),
            "Cargo.toml should use the configured gunbc-exec path"
        );
        assert!(
            cargo_toml.content.contains(
                "gunbc-lib-transport = { path = \"../../../src/8_materialize/transport\" }"
            ),
