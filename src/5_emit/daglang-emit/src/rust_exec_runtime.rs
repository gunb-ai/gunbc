...
impl DependencyPaths {
    pub fn conventional_defaults() -> Self {
        Self {
            ...
            gunbc_exec: "../../7_execute/exec".to_string(),
            gunbc_lib_transport: "../../6_materialize/transport".to_string(),
        }
    }
}
    ...
    fn emit_exec_runtime_uses_configured_dependency_paths() {
        ...
        let config = EmitConfig {
            dependency_paths: DependencyPaths {
                ...
                gunbc_exec: "../../../src/7_execute/exec".to_string(),
                gunbc_lib_transport: "../../../src/6_materialize/transport".to_string(),
            },
        };
        ...
        assert!(
            cargo_toml
                .content
                .contains("gunbc-exec = { path = \"../../../src/7_execute/exec\" }"),
            "Cargo.toml should use the configured gunbc-exec path"
        );
        assert!(
            cargo_toml.content.contains(
                "gunbc-lib-transport = { path = \"../../../src/6_materialize/transport\" }"
            ),
