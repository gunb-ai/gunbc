//! Typed design prompt/review transforms for workflow runtime.
    ...
    fn prompt_includes_context_and_acceptance_tests() {
        let prompt = build_design_prompt(&DesignRequest {
            ...
            acceptance_tests: vec![
                "cargo test -q -p gunbc-tests".to_string(),
                "cargo run -q -p gunbc-codegen --bin gunbc-workflow -- --help".to_string(),
            ],
        });
        assert!(prompt.contains("Title: Improve intake idempotency"));
        assert!(prompt.contains("Context:\nCurrent flow duplicates issue comments"));
        assert!(
            prompt.contains("Acceptance tests:\n- cargo test -q -p gunbc-tests")
        );
    }
