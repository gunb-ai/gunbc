...
fn resolve_fixture() -> InterpContext {
    ...
    let sources =
        resolve_imports_transitively_with_source_roots(FIXTURE, &entry_content, &v2_source_roots());
    ...
    InterpContext::new(graph, resolved.source_indices.clone(), ExecutionMode::Wet)
}
