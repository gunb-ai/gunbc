...
impl ServiceOperationSpec {
    ...
    /// Returns mock response entries for test generation (RT-1).
    ///
    /// Only REST operations can carry mock responses. Returns an empty
    /// slice for all other transport types.
    pub fn mock_responses(&self) -> &[MockResponseEntry] {
        match self {
            Self::Rest(spec) => &spec.mock_responses,
            _ => &[],
        }
    }
}
...
pub struct RestOperationSpec {
    ...
    /// Mock response entries for test generation (RT-1).
    /// Compiled from `mock_response { STATUS => { body } }` blocks.
    /// When present, testgen uses these for realistic mock data instead of
...
/// Mock response entry for test generation (RT-1).
///
/// Provides realistic mock response bodies for each status code,
/// declared via `mock_response { STATUS => { body } }` blocks in `.dag` files.
/// Consumed by testgen and auto_mock to generate typed test data instead of
/// relying on generic provider-level synthesis.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
pub struct MockResponseEntry {
    /// HTTP status code for this mock response.
    pub status: u16,
    /// Mock response body as a serialized JSON string.
    /// Stored as a string to satisfy `Eq`/`Ord` requirements of `RestOperationSpec`.
    /// Use `serde_json::from_str(&entry.body_json)` to deserialize at consumption time.
