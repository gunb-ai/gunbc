...
// PRODUCTION NEXT_RUNG_TRIGGER_CITATIONS SUPPRESSION SEAM — same shape as
// `corpus_findings_is_wired_to_the_production_suppression_roster`, for one OutsideModeledGuarantee
// stamp site. A fixture tree cannot drive the PRODUCTION inverse arm (rows whose citers are
// absent from the fixture look spent — the reason the arm takes a parameter roster);
// the discriminating RED that NextRungTriggerCitationResolved fires is
// `a_next_rung_trigger_that_resolves_is_the_stamp_firing_not_a_lost_control` above. This proves
// the PRODUCTION trigger roster is in the corpus_findings suppression set at our site identity.
#[test]
