...
// ── `0 -> 1` HAS A THIRD STATE: THE ONE A GENERATED MODULE INHABITS ──
//
// gunbc#10822 made the failure-mode roster DERIVED — its content is produced by reading the row
// directory — so it has no author for `authored_here` to consult, and every row file added after
// that classified as a coincidence in the pool. The wall then refused the append the generator
// exists to perform: measured across five lanes and seven changes, every change adding a row
// MODULE refused while every change editing an existing row passed.
//
// THE PAIR BELOW IS THE DISCRIMINATING ONE, and the second arm is why this is a classifier and
// not an exemption: both fixtures are a derived roster gaining a binding it did not have. They
// differ in ONE fact — whether the newly bound module is a member of the generator's DECLARED
// INPUT directory. Admitting on "the module is generated" would pass both.

// THE ROSTER SPELLS THE ROW NAME ON BOTH SIDES, which is what the observed delta requires: the
// wall compares only rows present on both sides, and the real report reads `base {} -> head {..}`
// — the spelling denoting NOTHING at base and the row module at head. A fixture where the
// spelling appears only at head produces no delta at all, and asserting over an empty delta list
// is a pass that measured nothing.
const ROSTER: &str = "module gunbc.recurring_failure_mode.roster\n\nimport gunbc.recurring_failure_mode.beta_row { beta_row }\n\ndata recurring_failure_mode_roster: String = beta_row\n";
const ALPHA_ROW: &str =
    "module gunbc.recurring_failure_mode.alpha_row\n\ndata alpha_row: String = \"a\"\n";
const BETA_ROW_ABSENT: &str =
    "module gunbc.recurring_failure_mode.beta_row\n\ndata unrelated_name: String = \"b\"\n";
const BETA_ROW: &str =
    "module gunbc.recurring_failure_mode.beta_row\n\ndata beta_row: String = \"b\"\n";
const OUTSIDE_ROSTER: &str = "module gunbc.recurring_failure_mode.roster\n\nimport gunbc.elsewhere { beta_row }\n\ndata recurring_failure_mode_roster: String = beta_row\n";
const OUTSIDE_ABSENT: &str = "module gunbc.elsewhere\n\ndata unrelated_name: String = \"x\"\n";
const OUTSIDE_SUPPLIES: &str = "module gunbc.elsewhere\n\ndata beta_row: String = \"x\"\n";

#[test]
fn a_derived_roster_binding_to_a_new_row_module_is_the_generators_declared_input() {
    let report = compare(
        "derived_input",
        &[
            ("roster.dag", ROSTER),
            ("alpha.dag", ALPHA_ROW),
            ("beta.dag", BETA_ROW_ABSENT),
        ],
        &[
            ("roster.dag", ROSTER),
            ("alpha.dag", ALPHA_ROW),
    ...
    assert!(
        dispositions_for(&report, "beta_row")
            .contains(&NamespaceDeltaDisposition::DerivedGeneratorInputResolution),
        "a derived roster binding a module of its generator's declared input directory must be \
         DerivedGeneratorInputResolution, got: {:?}",
        report.deltas
    );
    // AND IT MUST ADJUDICATE. The classification is only the finding if the wall then admits it:
    // this is the arm that unblocks every change adding a row file.
    assert!(
        report_unadjudicated(&report).is_empty(),
        "the generator's own declared input must not need an authored admission, got: {:?}",
        report_unadjudicated(&report)
    );
}

#[test]
fn a_derived_roster_binding_outside_its_generators_inputs_still_refuses() {
    // THE POSITIVE CONTROL, AND THE REASON THIS IS NOT A CATEGORY EXEMPTION. Same derived module,
    // same `0 -> 1`, same absence of an author — but the module that began supplying the name is
    // `gunbc.elsewhere`, which the roster's generator never reads. Nothing explains this binding,
    // so it stays a coincidence and stays refused. If this arm ever greens, the wall has been
    // widened rather than sharpened.
    let report = compare(
        "derived_outside",
        &[
            ("roster.dag", OUTSIDE_ROSTER),
            ("alpha.dag", ALPHA_ROW),
            ("outside.dag", OUTSIDE_ABSENT),
        ],
        &[
            ("roster.dag", OUTSIDE_ROSTER),
            ("alpha.dag", ALPHA_ROW),
            ("outside.dag", OUTSIDE_SUPPLIES),
        ],
    );
    assert!(
        dispositions_for(&report, "beta_row")
            .contains(&NamespaceDeltaDisposition::NewPoolCoincidenceResolution),
        "a derived module gaining a binding its generator's inputs do NOT explain must stay \
         NewPoolCoincidenceResolution, got: {:?}",
        report.deltas
    );
    assert!(
        !report_unadjudicated(&report).is_empty(),
        "an unexplained binding on a derived module must remain unadjudicated, got: {:?}",
        report.deltas
    );
}
