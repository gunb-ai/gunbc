...
fn the_base_roster_names_the_base_rows_and_not_a_row_the_change_adds() {
    ...
    let base_paths: std::collections::BTreeSet<String> = [
        ...
        // A nested path is not a direct child and is not a row.
        "dag/gunbc/recurring_failure_mode/nested/deep_row.dag",
        // An unrelated tree entry must not leak in.
    ...
    let body = v1_compiler::cli_run::namespace_wave_admission::base_derived_roster_body(
        &base_paths,
...
fn an_empty_base_row_set_renders_a_present_empty_roster_not_an_absent_one() {
    // ABSENCE AND EMPTINESS ARE DIFFERENT ANSWERS. A base tree with no rows yet must still yield a
    // roster module that PARSES and denotes an empty membership -- returning nothing here would
    // make the base module vanish, and a vanished base module is not a comparable side.
    let body = v1_compiler::cli_run::namespace_wave_admission::base_derived_roster_body(
