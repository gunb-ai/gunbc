...
fn emit_tsv(path: &Path, rows: &[ProbeReceiptRow]) -> Result<(), String> {
    ...
    writeln!(
        out,
        "module\tsource_commit\tsource_tree\tsurvey_executable\tsource_roots_digest\tprobe_policy_revision\tself_emit_ready\tblocker_class\tlocated_stage\tlocated_reason\tprobe_error"
    )
