...
//! Runs the same path as batch-1 dag_compile_clean_gate (--target dag, corpus
//! global_bare overlay) but emits ALL hard diagnostics aggregated by class and
//! (class, name) — not the truncated first-20 window.
//!
//! Do NOT invoke from cargo tests (whole-tree resolve OOM risk in test harness).
...
fn main() -> ExitCode {
    ...
    // Residue probes: fold + type-mismatch pair (quiet-gull-833 lane).
