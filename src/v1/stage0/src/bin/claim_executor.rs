...
use v1_compiler::cli_run::{
    ...
    ClaimOutcome, DiscoveryCorpusOptions, DiscoverySummary, DiscoveryWitnessOutcome,
    FloorDiscoveryConsumerRole, FloorPhase, HistogramData, PhaseProfile, ResolutionDivergencePhase,
    ResolutionDivergencePhaseState, TimingPercentiles, WitnessRowCost,
...
fn run_walk(
    ...
    let resolve_receipt_ok =
        !emit_ordinary_floor_receipts || write_resolve_receipt(&batch_records, floor_finalization);
...
fn run() -> Result<ExitCode, ExitCode> {
    ...
    // claim scope, one cheap mutable frame per claim, one linear fold. It takes no plan, so it
    // short-circuits before the plan-arg requirement — there is no schedule to resolve, no batch
    // to assign, no worker to spawn and no selection to compute, and the absence of those flags
    // is the point rather than an omission. `run_required_floor` itself refuses when planned,
    // executed and terminal identity counts disagree, so a silently short roster cannot report
    // as a pass.
