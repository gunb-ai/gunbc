...
use v1_compiler::cli_run::{
    compile_clean_shard_module_in_process, compute_histogram_data, compute_witness_timing_rows,
    make_eval_context, resolve_entry_graph, run_claim, run_discovery_corpus_with_options,
    run_value, set_phase, top_n_slowest_witnesses, ClaimOutcome, DiscoveryCorpusOptions,
    DiscoverySummary, FloorPhase, HistogramData, PhaseProfile, TimingPercentiles,
...
const COMPILE_CLEAN_SHARD_TRANSPORT: &str = "dag/tools/dag_compile_clean_shard_transport.dag";
...
fn run_compile_clean_shard_batch_node(
    ...
    for shard in shards {
        ...
        handles.push(thread::spawn(move || {
            ...
            for path in shard {
                match compile_clean_shard_module_in_process(&roots, &path) {
                    Ok(true) => {}
...
mod tests {
    ...
    fn compile_clean_shard_batch() -> Runnable {
        Runnable::CompileCleanShardBatch {
            entry_paths: vec!["dag/std/logic.dag".to_string()],
            spawn_width_cap: 4,
        }
    }
    ...
    #[test]
    fn compile_clean_shard_batch_is_isolated_unit() {
        let batch = vec![compile_clean_shard_batch(), single("gate.dag", "g1")];
        let units = group_batch_units(&batch);
