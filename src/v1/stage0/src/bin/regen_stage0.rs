...
mod tests {
    ...
    #[test]
    fn ensure_splices_missing_seed_retained_pub_mods() {
        let dir = temp_test_dir("lib_rs_splice");
        fs::write(
            dir.join("lib.rs"),
            "// Generated by v1 compiler -- do not edit.\n\npub mod v2_compiler_body_producer;\npub mod wt_common;\n\npub struct X;\n",
        )
        .expect("write lib.rs");
        ensure_hand_maintained_pub_mods_in_lib_rs(&dir).expect("splice lib.rs");
        let out = fs::read_to_string(dir.join("lib.rs")).expect("read lib.rs");
        assert!(
            out.contains("pub mod v2_compiler_compile;"),
            "expected seed-retained pub mod; got:\n{out}"
        );
        assert!(
            out.contains("pub mod v2_compiler_normalize;"),
            "expected normalize pub mod; got:\n{out}"
        );
