...
fn auth_declared_no_source_fails_closed_pre_send(module_index: &HashMap<String, std::path::PathBuf>) {
...
fn auth_input_empty_fails_closed_pre_send(module_index: &HashMap<String, std::path::PathBuf>) {
...
fn no_auth_declared_does_not_fire_guard(module_index: &HashMap<String, std::path::PathBuf>) {
...
fn dual_declare_both_empty_fails_closed(module_index: &HashMap<String, std::path::PathBuf>) {
    std::env::remove_var("TEST_AUTH_GUARD_DUAL_FALLBACK_VAR");
    let resolved = resolve(module_index, SERVICE_DUAL_DECLARE);
    let graph = resolved.graph.as_ref().expect("graph");
    let ctx = v1_interpreter::InterpContext::new(
...
fn auth_resolution_enum_is_pub_and_discriminable(_module_index: &HashMap<String, std::path::PathBuf>) {
...
fn main() -> ExitCode {
    ...
    let tests: Vec<(&str, fn(&HashMap<String, std::path::PathBuf>))> = vec![
        (
