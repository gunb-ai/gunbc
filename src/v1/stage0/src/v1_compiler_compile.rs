...
pub struct FrontendResult {
    ...
    pub annotations: Arc<SourceAnnotationGraph>,
}
...
pub struct FrontendAccum {
    ...
    pub module_inputs: Arc<Vec<Arc<ModuleOccurrenceInput>>>,
    pub newline_indices: Arc<Vec<Arc<NewlineIndex>>>,
    pub intern_table: Arc<InternTable>,
    pub occurrence_allocator: OccurrenceIdAllocator,
    pub annotations: Arc<SourceAnnotationGraph>,
    pub annotation_diagnostics: Arc<Vec<Arc<ErrorNode>>>,
}
...
pub struct FrontendPrepared {
    ...
    pub annotations: Arc<Vec<Arc<UnboundAnnotationCapture>>>,
    pub source_length: i64,
    pub newline_index: Arc<NewlineIndex>,
}

pub fn merge_occurrence_transports(
    transports: Arc<Vec<Arc<OccurrenceTransport>>>,
) -> Arc<OccurrenceTransport> {
...
pub fn resolve_frontend_occurrence_transport(
    module_inputs: Arc<Vec<Arc<ModuleOccurrenceInput>>>,
    source_indices: Arc<HashMap<String, Arc<NewlineIndex>>>,
    occurrence_transport: Arc<OccurrenceTransport>,
) -> Arc<FrontendOccurrenceResolution> {
    match occurrence_transport_refusal(occurrence_transport.clone()) {
        ...
        None => {
            ...
            Arc::new(FrontendOccurrenceResolution {
...
pub fn front_end_sources(sources: Arc<Vec<Arc<SourceFile>>>) -> Arc<FrontendResult> {
    {
        ...
        let occurrence_transport = merge_occurrence_transports(Arc::new({
            let mut __result = Vec::new();
...
pub fn parse_census_fill_sources(sources: Arc<Vec<Arc<SourceFile>>>) -> Arc<CensusFillParse> {
    {
        ...
        Arc::new(CensusFillParse {
            ...
            occurrence_transport: merge_occurrence_transports(Arc::new({
                let mut __result = Vec::new();
                for input in module_inputs.clone().iter().cloned() {
                    __result.push(module_occurrence_input_transport(input.clone()));
                }
