...
use crate::std_occurrence_identity::OccurrenceCategory::{LexicalValueOccurrence, TypeOccurrence};
...
pub use crate::std_occurrence_identity::{
    ...
    OccurrenceId, OccurrenceIdAllocator, OccurrenceIndex, OccurrenceIndexEntry,
...
pub fn reference_derived_dependency_binding_references(
    ...
    references.clone().iter().cloned().fold(
        Rc::new(vec![]),
        |acc: Rc<Vec<Rc<ReferenceOccurrence>>>, reference: Rc<ReferenceOccurrence>| match reference
            .category
            .clone()
        {
            OccurrenceCategory::TypeOccurrence => acc.clone(),
            _ => v1_rt::concat(acc.clone(), Rc::new(vec![reference.clone()])),
        },
    )
}
...
pub fn remap_occurrence_id(
    ...
    match v1_rt::map_get(&state.remap.clone(), id.value.clone()) {
        ...
        None => {
            let alloc_result = alloc_occurrence_id(state.allocator.clone());
            Rc::new(OccurrenceIdRemapResult {
                id: alloc_result.id.clone(),
                state: Rc::new(OccurrenceIdRemapState {
                    allocator: alloc_result.alloc.clone(),
                    remap: v1_rt::rc_map_insert(
                        state.remap.clone(),
                        id.value.clone(),
                        alloc_result.id.clone(),
                    ),
                }),
            })
        }
...
pub fn remap_containment_path(
    ...
    {
        let ancestor_build = path.ancestors.clone().iter().cloned().fold(
            ...
            |acc: Rc<OccurrenceAncestorRemapBuild>, ancestor: OccurrenceId| {
                let acc = v1_rt::take_owned(acc);
                {
                    ...
                    Rc::new(OccurrenceAncestorRemapBuild {
                        ancestors: v1_rt::concat(acc.ancestors, Rc::new(vec![remapped.id.clone()])),
        ...
        let terminal_remapped =
            remap_occurrence_id(path.terminal.clone(), ancestor_build.state.clone());
        Rc::new(OccurrenceContainmentRemapResult {
            path: Rc::new(OccurrenceContainmentPath {
                ancestors: ancestor_build.ancestors.clone(),
...
pub fn rekey_occurrence_transport(
...
) -> Rc<OccurrenceTransportRekeyResult> {
    {
        let index_build = transport
            ...
            .fold(
                ...
                |acc: Rc<OccurrenceIndexRekeyBuild>, entry: Rc<OccurrenceIndexEntry>| {
                    let acc = v1_rt::take_owned(acc);
                    {
                        let occurrence_remapped = remap_occurrence_id(
                            entry.projection.clone().occurrence.clone(),
                            acc.state,
                        );
                        let containment_remapped = remap_containment_path(
                            entry.containment.clone(),
                            occurrence_remapped.state.clone(),
                        );
                        Rc::new(OccurrenceIndexRekeyBuild {
                            entries: v1_rt::concat(
                                Rc::new(vec![Rc::new(OccurrenceIndexEntry {
                                    projection: Rc::new(OccurrenceProjection {
                                        occurrence: occurrence_remapped.id.clone(),
                                        authored_name: entry
                                            .projection
                                            .clone()
                                            .authored_name
                                    ...
                                    containment: containment_remapped.path.clone(),
                                })]),
                                acc.entries,
                            ),
                            state: containment_remapped.state.clone(),
                        })
                    }
                },
            );
        let declaration_build = transport.declarations.clone().iter().cloned().fold(
        ...
        let reference_build = transport.references.clone().iter().cloned().fold(
            Rc::new(OccurrenceReferenceRekeyBuild {
                references: Rc::new(vec![]),
                state: declaration_build.state.clone(),
            }),
            |acc: Rc<OccurrenceReferenceRekeyBuild>, reference: Rc<ReferenceOccurrence>| {
                let acc = v1_rt::take_owned(acc);
                {
                    let occurrence_remapped =
                        remap_occurrence_id(reference.occurrence.clone(), acc.state);
                    let containment_remapped = remap_containment_path(
        ...
        Rc::new(OccurrenceTransportRekeyResult {
            transport: Rc::new(OccurrenceTransport {
                index: Rc::new(OccurrenceIndex {
                    entries: v1_rt::reverse(index_build.entries.clone()),
                }),
                declarations: v1_rt::reverse(declaration_build.declarations.clone()),
                references: v1_rt::reverse(reference_build.references.clone()),
            }),
            state: reference_build.state.clone(),
        })
    }
}
...
pub fn rekey_occurrence_binding_inputs(
    ...
    {
        let module_path_build = inputs.module_paths.clone().iter().cloned().fold(
            ...
            |acc: Rc<OccurrenceModulePathRekeyBuild>, row: Rc<OccurrenceModulePathRow>| {
                let acc = v1_rt::take_owned(acc);
                {
                    let remapped = remap_occurrence_id(row.occurrence.clone(), acc.state);
                    Rc::new(OccurrenceModulePathRekeyBuild {
                        module_paths: v1_rt::concat(
                            Rc::new(vec![Rc::new(OccurrenceModulePathRow {
                                occurrence: remapped.id.clone(),
                                module_path: row.module_path.clone(),
                            })]),
                            acc.module_paths,
                        ),
        ...
        let exposure_build = inputs.exposure_rows.clone().iter().cloned().fold(
            Rc::new(OccurrenceExposureRekeyBuild {
                exposure_rows: Rc::new(vec![]),
                state: module_path_build.state.clone(),
            }),
            |acc: Rc<OccurrenceExposureRekeyBuild>, row: Rc<DeclarationExposureRow>| {
                let remapped = remap_occurrence_id(row.occurrence.clone(), acc.state.clone());
                match (*row.exposure.clone()).clone() {
                    DeclarationExposure::LexicalExposure {
                        exposing_scope: scope,
                        ..
                    } => {
                        let scope_remapped =
                            remap_containment_path(scope.clone(), remapped.state.clone());
                        Rc::new(OccurrenceExposureRekeyBuild {
                            exposure_rows: v1_rt::concat(
                                Rc::new(vec![Rc::new(DeclarationExposureRow {
                                    occurrence: remapped.id.clone(),
                                    exposure: Rc::new(DeclarationExposure::LexicalExposure {
                                        exposing_scope: scope_remapped.path.clone(),
                                    }),
                                })]),
                                acc.exposure_rows.clone(),
                            ),
                            state: scope_remapped.state.clone(),
                        })
                    }
        ...
        let order_build = inputs.authored_order_rows.clone().iter().cloned().fold(
            Rc::new(OccurrenceOrderRekeyBuild {
                authored_order_rows: Rc::new(vec![]),
                state: exposure_build.state.clone(),
            }),
        ...
        Rc::new(OccurrenceBindingInputsRekeyResult {
            inputs: Rc::new(OccurrenceBindingCandidateInputs {
                module_paths: v1_rt::reverse(module_path_build.module_paths.clone()),
                exposure_rows: v1_rt::reverse(exposure_build.exposure_rows.clone()),
                authored_order_rows: v1_rt::reverse(order_build.authored_order_rows.clone()),
            }),
            state: order_build.state.clone(),
        })
    }
}
...
pub fn cross_file_binding_closure_row(
    ...
    module_path: String,
    file_path: String,
    exposure_rows: Rc<Vec<Rc<DeclarationExposureRow>>>,
    module_paths: Rc<Vec<Rc<OccurrenceModulePathRow>>>,
    authored_order_rows: Rc<Vec<Rc<AuthoredOrderRow>>>,
) -> Rc<CrossFileBindingClosureRow> {
    Rc::new(CrossFileBindingClosureRow {
        transport: transport.clone(),
        inputs: Rc::new(OccurrenceBindingCandidateInputs {
            module_paths: module_paths.clone(),
            exposure_rows: exposure_rows.clone(),
            authored_order_rows: authored_order_rows.clone(),
        }),
