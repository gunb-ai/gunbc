...
pub use crate::std_occurrence_identity::{
    DeclarationOccurrence, OccurrenceContainmentPath, OccurrenceId, OccurrenceIndexEntry,
...
pub fn authored_order_note() -> String {
    thread_local! {
        static CACHED: String = {
            "AuthoredOrderRow carries the authored token ordinal for one occurrence — the source-order axis, distinct from OccurrenceId identity. Module-sibling gating compares ordinals only; never OccurrenceId.value as a proxy for authored order.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}
    ...
    declarations.clone().iter().cloned().fold(
        ...
        |acc: Rc<HashMap<String, Rc<Vec<Rc<DeclarationOccurrence>>>>>,
         declaration: Rc<DeclarationOccurrence>| match v1_rt::map_get(
            ...
            None => acc.clone(),
            Some(entry) => declarations_by_name_insert(
                acc.clone(),
...
pub fn occurrence_candidate_index_build(
    ...
    None => Rc::new(OccurrenceCandidateIndexBuild::OccurrenceCandidateIndexReady {
    index: Rc::new(OccurrenceCandidateIndex {
    entries_by_id: validated.entries_by_id.clone(),
    declarations_by_name: declarations_by_name_build(validated.entries_by_id.clone(), validated.declarations.clone()),
...
pub fn occurrence_safe_collector_dedup_receipt() -> String {
    thread_local! {
        static CACHED: String = {
            "SCAFFOLD: production occurrence collector must deduplicate repeated discovery of the same identity before candidate binding. Dissolves when a live collector path executes the receipt and Section13RepeatedDiscoverySameIdentityCollapse moves to Executing.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}
...
pub fn section13_population_law_roster() -> Rc<Vec<Rc<Section13PopulationLawRosterRow>>> {
    thread_local! {
        ...
        receipt: section13_executing_receipt("dag/test/claim/occurrence_binding_candidates_witness_test.dag".to_string(), "module_scope_sibling_is_visible_holds".to_string()),
    }),
    }), Rc::new(Section13PopulationLawRosterRow {
        law: Section13PopulationLawId::Section13SiblingArmExclusion,
        evidence: Rc::new(Section13PopulationLawEvidence::Section13PopulationLawExecuting {
        receipt: section13_executing_receipt("dag/test/claim/occurrence_binding_candidates_witness_test.dag".to_string(), "sibling_match_arm_not_visible_holds".to_string()),
    }),
    }), Rc::new(Section13PopulationLawRosterRow {
        law: Section13PopulationLawId::Section13LexicalEnclosure,
        evidence: Rc::new(Section13PopulationLawEvidence::Section13PopulationLawExecuting {
        receipt: section13_executing_receipt("dag/test/claim/occurrence_binding_candidates_witness_test.dag".to_string(), "nested_lexical_only_inside_scope_holds".to_string()),
    }),
    }), Rc::new(Section13PopulationLawRosterRow {
        law: Section13PopulationLawId::Section13SourceOrderGate,
        evidence: Rc::new(Section13PopulationLawEvidence::Section13PopulationLawExecuting {
        receipt: section13_executing_receipt("dag/test/claim/occurrence_binding_candidates_witness_test.dag".to_string(), "declaration_below_occurrence_not_visible_holds".to_string()),
    }),
    }), Rc::new(Section13PopulationLawRosterRow {
        law: Section13PopulationLawId::Section13DistinctIdentityAmbiguity,
        evidence: Rc::new(Section13PopulationLawEvidence::Section13PopulationLawExecuting {
        receipt: section13_executing_receipt("dag/test/claim/occurrence_binding_candidates_witness_test.dag".to_string(), "same_text_distinct_identities_is_ambiguous_holds".to_string()),
    }),
    }), Rc::new(Section13PopulationLawRosterRow {
...
pub fn section13_population_law_roster_denominator_holds() -> bool {
    {
        let rows = section13_population_law_roster();
        (((((rows.clone().len() as i64) == 6) && section13_roster_covers_all_laws(rows.clone())) && !section13_roster_has_duplicate_law(rows.clone())) && rows.clone().iter().cloned().fold(true, |ok: bool, row: Rc<Section13PopulationLawRosterRow>| (ok && match (*row.evidence.clone()).clone() {
    Section13PopulationLawEvidence::Section13PopulationLawExecuting { receipt: _, .. } => true,
    Section13PopulationLawEvidence::Section13PopulationLawDeferredProductionOccurrenceCollectorMissing { dissolve_on: Some(_), .. } => true,
    _ => false,
})))
    }
}
