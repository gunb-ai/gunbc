                ...
                CallSemantics::ResolvedDirectCallSemantics {
                    ...
                    |acc: Rc<HashMap<String, Rc<DagCollectSlot>>>,
                     application: Rc<ResolvedCallFormal>| {
                        ...
                        let acc = dag_collect_product_application(
                            application.formal.clone().product_application.clone(),
                            acc.clone(),
                            collision_errors.clone(),
                        );
                        dag_collect_product_application(
                            application.produced_product_application.clone(),
                            acc.clone(),
                            collision_errors.clone(),
                        )
                    },
...
pub fn dag_collect_product_application(
    value: Option<Rc<TypeApplication<Rc<Node>>>>,
    slots: Rc<HashMap<String, Rc<DagCollectSlot>>>,
    collision_errors: Rc<Vec<Rc<ErrorNode>>>,
) -> Rc<HashMap<String, Rc<DagCollectSlot>>> {
    match value.clone() {
        Some(application) => application.arguments.clone().iter().cloned().fold(
            slots.clone(),
            |acc: Rc<HashMap<String, Rc<DagCollectSlot>>>, arg: Rc<Node>| {
                dag_collect_insert_slots(arg.clone(), acc, collision_errors.clone())
            },
        ),
        std::option::Option::None => slots.clone(),
    }
