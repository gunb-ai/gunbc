...
pub fn dag_node_collection_anchor(mut node: Rc<Node>) -> Rc<Node> {
    loop {
        if (node.authored_occurrence.clone() != None) {
            break node.clone();
        } else {
...
pub fn dag_authored_occurrence_key(occurrence: Rc<AuthoredOccurrence>) -> String {
    match (*occurrence.clone()).clone() {
        AuthoredOccurrence::AuthoredDeclarationOccurrence {
            occurrence: declaration,
            ..
        } => v1_rt::concat(
            "occ:declaration:".to_string(),
            (declaration.identity.clone().value.clone()).to_string(),
        ),
        AuthoredOccurrence::AuthoredReferenceOccurrence {
            occurrence: reference,
            ..
        } => v1_rt::concat(
            "occ:reference:".to_string(),
...
pub fn dag_collect_match_pattern(
    ...
    match pattern.clone().as_deref().cloned() {
        ...
        Some(MatchPattern::Bind {
            ...
            ..
        }) => dag_collect_insert_slots(declaration.clone(), slots, collision_errors.clone()),
        Some(MatchPattern::LitPattern { value: _, .. }) => slots,
        Some(MatchPattern::Wildcard) => slots,
        Some(MatchPattern::VariantPattern {
            field_bindings: bindings,
            ..
        }) => dag_collect_nodes_list(bindings.clone(), slots, collision_errors.clone()),
    }
}
...
pub fn dag_collect_insert_slots(
...
) -> Rc<HashMap<String, Rc<DagCollectSlot>>> {
    {
        ...
        match v1_rt::map_get(&slots, key.clone()) {
            Some(_) => slots.clone(),
            None => {
                let fp = if (anchor.authored_occurrence.clone() != None) {
