...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct PositiveMeasureCount {
    pub predecessor: Nat,
}
...
pub fn positive_measure_count_value(count: Rc<PositiveMeasureCount>) -> i64 {
    (count.predecessor.clone() + 1)
}

pub type PositiveCelsiusDelta = Rc<Measure<(), (), Rc<PositiveMeasureCount>>>;
...
pub fn positive_celsius_delta_count(delta: PositiveCelsiusDelta) -> i64 {
    positive_measure_count_value(measure_count(delta.clone()))
}
...
pub fn positive_millisecond_count(m: PositiveMillisecond) -> i64 {
    positive_measure_count_value(measure_count(m.clone()))
}
