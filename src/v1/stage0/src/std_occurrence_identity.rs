...
pub fn occurrence_identity_receipt_enrollment_debts(
    ...
    thread_local! {
        static CACHED: Rc<Vec<Rc<OccurrenceIdentityReceiptEnrollmentDebt>>> = {
            serde_json::from_value(serde_json::json!([{"id": "namespace-parser-occurrence-receipts-per-pr-enrollment", "receipt_functions": ["w_rebuilt_reference_identity_preservation_executes_holds", "w_structurally_equal_distinct_occurrences_execute_holds", "w_pattern_declaration_reachability_executes_holds", "w_same_spelling_parser_declaration_isolation_executes_holds", "pipeline_occurrence_sidecar_serde_round_trip"], "owner": "witness-scheduling-source-root-surface", "cause": "The four DAG receipts and the generated Rust rebuild assertion are green by execution and discriminating under planted defects, but none is executed per PR. The DAG receipts live in the offline docs/probes root and import the production parser from src/v1, while every witness execution batch resolves against witness_layer_roots = [dag, src/v2] and ScheduleWitnessEntry carries no per-entry source roots. The Rust test crate containing pipeline_occurrence_sidecar_serde_round_trip is cargo-checked but its tests are not executed. A compile-only Cargo harness is not execution, and a second workflow-local claim_batch step would fork the floor plan's scheduling authority.", "dissolve_on": "DISSOLVE-ON: ScheduleWitnessEntry admits authoritative per-entry source roots and the four DAG functions enroll with [dag, src/v1, docs/probes], or the single witness_layer_roots authority deliberately widens to cover their entry and imports; and pipeline_occurrence_sidecar_serde_round_trip executes per PR or its rebuild assertion migrates into an enrolled witness. Prove both enrollment surfaces and delete this row in the same PR."}]))
                .expect("valid data definition")
        };
    }
