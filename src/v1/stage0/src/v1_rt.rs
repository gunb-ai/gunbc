...
/// Row-5 discriminator + pool-present-unreachable census (swift-fox-347 /
/// import-strip §14). Records whether `symbol_index_lookup` was consulted on a
/// qualified type-name path and whether it hit or missed, plus every use-site
/// type ref that would resolve only via pool coincidence (global_bare /
/// symbol_index) rather than import/local reachability.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct SymbolIndexLookupSite {
    pub env_module_path: String,
...
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct TypeRefPoolPresentUnreachableSite {
...
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct TypeRefFailOpenTelemetry {
    pub symbol_index_lookups: std::vec::Vec<SymbolIndexLookupSite>,
    pub pool_present_unreachable: std::vec::Vec<TypeRefPoolPresentUnreachableSite>,
}

thread_local! {
    static TYPE_REF_FAIL_OPEN_ENABLED: Cell<bool> = const { Cell::new(false) };
    static TYPE_REF_FAIL_OPEN: RefCell<TypeRefFailOpenTelemetry> =
        RefCell::new(TypeRefFailOpenTelemetry::default());
}

pub fn type_ref_fail_open_telemetry_enable() {
    TYPE_REF_FAIL_OPEN.with(|t| *t.borrow_mut() = TypeRefFailOpenTelemetry::default());
    TYPE_REF_FAIL_OPEN_ENABLED.with(|e| e.set(true));
}

pub fn type_ref_fail_open_telemetry_disable() -> TypeRefFailOpenTelemetry {
    TYPE_REF_FAIL_OPEN_ENABLED.with(|e| e.set(false));
    TYPE_REF_FAIL_OPEN.with(|t| std::mem::take(&mut *t.borrow_mut()))
}
...
pub fn type_ref_fail_open_record_pool_present_unreachable(
    module_name: String,
    type_name: String,
    span_file: String,
    span_start: i64,
    ...
    TYPE_REF_FAIL_OPEN.with(|t| {
        t.borrow_mut()
            ...
            .push(TypeRefPoolPresentUnreachableSite {
                module_name,
                type_name,
                span_file,
                span_start,
...
pub fn type_ref_fail_open_drain_population() -> std::vec::Vec<TypeRefPoolPresentUnreachableSite> {
    TYPE_REF_FAIL_OPEN.with(|t| std::mem::take(&mut t.borrow_mut().pool_present_unreachable))
}
