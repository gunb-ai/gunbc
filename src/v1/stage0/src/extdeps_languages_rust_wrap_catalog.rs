...
pub fn rust_sg_rc_wrap_catalog_realization_fork_note() -> String {
    thread_local! {
        static CACHED: String = {
            "Residual §3 fork (non-blocking): stage0 Rust realization still stores carrier as String and compares via rust_sg_rc_wrap_carrier_from_type_name — the .dag row authority is OwnershipWrapCarrier; dissolve-on: structural enum emission in stage0 when the emitter can inhabit closed carriers without string-key compare.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}
