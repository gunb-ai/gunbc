        ...
        |acc: Rc<HashMap<String, Rc<TypeBinding>>>, imp: Rc<ResolvedImport>| match v1_rt::map_get(
            ...
            Some(typed_parent) => {
                ...
                selected.clone().iter().cloned().fold(
                    acc.clone(),
                    |bacc: Rc<HashMap<String, Rc<TypeBinding>>>, name: String| match v1_rt::map_get(
                        &export_surface.str_bindings.clone(),
                        name.clone(),
                    ) {
                        Some(binding) => {
                            v1_rt::rc_map_insert(bacc.clone(), name.clone(), binding.clone())
                        }
                        None => bacc.clone(),
                    },
                )
