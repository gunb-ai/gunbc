...
use self::DeclarationCallLookup::*;
...
pub use crate::std_occurrence_binding::{
    BindingOccurrence, ContainmentPath, OccurrenceBindingResult,
};
...
pub use crate::v1_compiler_infer_env::{
    ambiguous_binding_candidate_labels, bare_name_miss_diagnostic, binding_declares_name,
    declaration_binding_result, empty_symbol_index, empty_type_env_cache,
    ...
    symbol_index_insert, symbol_index_insert_decl, symbol_index_insert_declaration_path,
    symbol_index_insert_module_root, symbol_index_insert_service, symbol_index_lookup,
    type_binding_from_declaration, type_binding_from_declaration_path,
};
pub use crate::v1_compiler_infer_env::{
...
pub use crate::v1_compiler_infer_lookup::{
    ...
    resolved_func_sig_from_declaration_path, set_element_type_in_env,
};
...
pub struct InferScope {
    ...
    pub occurrence_ancestors: Rc<Vec<Rc<Node>>>,
}

pub fn scope_with_occurrence_parent(scope: Rc<InferScope>, parent: Rc<Node>) -> Rc<InferScope> {
    Rc::new(InferScope {
        ...
        occurrence_ancestors: v1_rt::concat(
            scope.occurrence_ancestors.clone(),
...
pub fn unbound_value_refusal(
    ...
    Rc::new(InferResult {
        typed: semantic_expr_error_node(
            v1_rt::concat(
                v1_rt::concat("unbound value '".to_string(), name.clone()),
                "'".to_string(),
            ),
            span.clone(),
        ),
...
pub fn infer_bound_declaration_value(
    texpr: Rc<Node>,
    name: String,
...
) -> Rc<InferResult> {
    {
        ...
        match resolved_func_sig_from_declaration_path(path.clone(), scope.type_env.clone()) {
            Some(fsig) => {
            ...
            None => match type_binding_from_declaration_path(
                path.clone(),
                scope.type_env.clone().source_indices.clone(),
            ) {
                Some(binding) => {
                    let binding_kind = if (type_binding_from_declaration(
                        path.terminal.clone(),
                        scope.type_env.clone().source_indices.clone(),
                    ) == None)
                    {
                        match v1_rt::reverse(path.ancestors.clone()).first().cloned() {
                            Some(owner) => Rc::new(VarBindingKind::VariantValueBinding {
                ...
                None => unbound_value_refusal(name.clone(), span.clone(), scope.clone()),
            },
        }
    }
}
...
pub fn build_params_scope(scope: Rc<InferScope>, params: Rc<Vec<Rc<Node>>>) -> Rc<InferScope> {
    {
        ...
        Rc::new(InferScope {
            ...
            occurrence_ancestors: scope.occurrence_ancestors.clone(),
        })
    }
}
...
pub fn infer_expr(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let occurrence = Rc::new(BindingOccurrence {
            containment: Rc::new(ContainmentPath {
                ancestors: scope.occurrence_ancestors.clone(),
                terminal: texpr.clone(),
                _phantom: std::marker::PhantomData,
            }),
            _phantom: std::marker::PhantomData,
        });
        let scope = scope_with_occurrence_parent(scope.clone(), texpr.clone());
        match (*texpr.expr_data.clone()).clone() {
            ...
            ExprData::ExprVar {
                ...
                match v1_rt::map_get(&scope.locals.clone(), name.clone()) {
                    ...
                    None => match (*declaration_binding_result(
                        scope.type_env.clone(),
                        occurrence.clone(),
                        name.clone(),
                    ))
                    .clone()
                    {
                        OccurrenceBindingResult::OccurrenceBound {
                            binding: accepted, ..
                        } => infer_bound_declaration_value(
                            texpr.clone(),
                            name.clone(),
                            accepted.candidate.clone().containment.clone(),
                            scope.clone(),
                        ),
                        OccurrenceBindingResult::OccurrenceUnbound { occurrence: _, .. } => {
                            unbound_value_refusal(name.clone(), span.clone(), scope.clone())
                        }
                        OccurrenceBindingResult::OccurrenceAmbiguous { candidates, .. } => {
                            ambiguous_reference_refusal(
                                name.clone(),
                                ambiguous_binding_candidate_labels(
                                    candidates.clone(),
                                    scope.type_env.clone().source_indices.clone(),
                                ),
                                span.clone(),
            ...
            ExprData::ExprCall { .. } => {
                ...
                let call_declaration =
                    declaration_call_lookup(scope.clone(), occurrence.clone(), func_name.clone());
                match (*call_declaration.clone()).clone() {
                    DeclarationCallLookup::DeclarationCallAmbiguous {
                    ...
                    _ => {
                        let sig = match (*call_declaration.clone()).clone() {
                            DeclarationCallLookup::DeclarationCallSig {
                                sig: resolved_sig, ..
                            } => Some(resolved_sig.clone()),
                            _ => None,
                        };
                        let bound_call_path = match (*call_declaration.clone()).clone() {
                            DeclarationCallLookup::DeclarationCallSig { path, .. } => {
                                Some(path.clone())
                            }
                            DeclarationCallLookup::DeclarationCallNonCallable {
                                path: path,
                                ..
                            } => Some(path.clone()),
                            _ => None,
...
match bare_s.clone() {
    ...
    diagnostics: arg_diags.clone(),
                                                        ...
                                                        match bound_call_path.clone() {
    Some(declaration_path) => match type_binding_from_declaration_path(declaration_path.clone(), scope.type_env.clone().source_indices.clone()) {
    Some(binding) => Rc::new(InferResult {
    typed: make_named_expr_node(func_name.clone(), Rc::new(ExprData::ExprCall {
    call_semantics: Some(CallSemantics::PlainCallSemantics),
    descent_evidence: None,
}), typed_arg_nodes.clone(), Some(Rc::new(InferredNode::Resolved {
    ...
    None => Rc::new(InferResult {
    typed: semantic_expr_error_node(v1_rt::concat(v1_rt::concat("declaration '".to_string(), func_name.clone()), "' is not callable".to_string()), span.clone()),
    diagnostics: v1_rt::concat(arg_diags.clone(), Rc::new(vec![make_error_node(Rc::new(CompilerDiagnostic::UnresolvedType {
    ...
    None => Rc::new(InferResult {
    typed: semantic_expr_error_node(v1_rt::concat(v1_rt::concat("function '".to_string(), func_name.clone()), "' not found in scope".to_string()), span.clone()),
    diagnostics: v1_rt::concat(arg_diags.clone(), Rc::new(vec![make_error_node(Rc::new(CompilerDiagnostic::UnresolvedType {
    name: func_name.clone(),
    span: span.clone(),
}), scope.module_name.clone())])),
            ...
            ExprData::ExprLambda => {
                ...
                let body_scope = Rc::new(InferScope {
                    ...
                    lambda_param_provenance: v1_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
                    occurrence_ancestors: lam_scope.occurrence_ancestors.clone(),
                });
                let body_result =
...
pub fn infer_item(item: Rc<Node>, scope: Rc<InferScope>) -> Rc<TypedItemResult> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let scope = scope_with_occurrence_parent(scope.clone(), item.clone());
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum DeclarationCallLookup {
    DeclarationCallSig {
        path: Rc<ContainmentPath<Rc<Node>>>,
        sig: Rc<ResolvedFuncSig>,
    },
    DeclarationCallNonCallable {
        path: Rc<ContainmentPath<Rc<Node>>>,
    },
    DeclarationCallUnbound,
    DeclarationCallAmbiguous {
        candidates: Rc<Vec<String>>,
    },
}

pub fn declaration_call_lookup(
    scope: Rc<InferScope>,
    occurrence: Rc<BindingOccurrence<Rc<Node>>>,
    func_name: String,
) -> Rc<DeclarationCallLookup> {
    if ((v1_rt::map_get(&scope.body_locals.clone(), func_name.clone()) != None)
        || (infer_builtin_call_type(func_name.clone()) != None))
    {
        Rc::new(DeclarationCallLookup::DeclarationCallUnbound)
    } else {
        match (*declaration_binding_result(
            scope.type_env.clone(),
            occurrence.clone(),
            func_name.clone(),
        ))
        .clone()
        {
            OccurrenceBindingResult::OccurrenceBound {
                binding: accepted, ..
            } => match resolved_func_sig_from_declaration_path(
                accepted.candidate.clone().containment.clone(),
                scope.type_env.clone(),
            ) {
                Some(sig) => Rc::new(DeclarationCallLookup::DeclarationCallSig {
                    path: accepted.candidate.clone().containment.clone(),
                    sig: sig.clone(),
                }),
                None => Rc::new(DeclarationCallLookup::DeclarationCallNonCallable {
                    path: accepted.candidate.clone().containment.clone(),
                }),
            },
            OccurrenceBindingResult::OccurrenceUnbound { occurrence: _, .. } => {
            ...
            OccurrenceBindingResult::OccurrenceAmbiguous { candidates, .. } => {
                Rc::new(DeclarationCallLookup::DeclarationCallAmbiguous {
                    candidates: ambiguous_binding_candidate_labels(
                        candidates.clone(),
                        scope.type_env.clone().source_indices.clone(),
                    ),
                })
...
pub fn local_binding_for_item(
    ...
    if is_namespace_alias_item(item.clone(), source_indices.clone()) {
        ...
        type_binding_from_declaration(item.clone(), source_indices.clone())
    }
}
...
pub fn symbol_index_insert_item(
    ...
    {
        let is_namespace_alias = is_namespace_alias_item(item.clone(), source_indices.clone());
        let projected = local_binding_for_item(item.clone(), source_indices.clone());
        let with_decl = if is_namespace_alias.clone() {
            match namespace_alias_target_path(item.clone(), source_indices.clone()) {
                None => index.clone(),
                Some(target_path) => {
        ...
        let declaration_name = authored_name_at(source_indices.clone(), item.clone());
        let with_path = if (is_namespace_alias.clone() || (projected.clone() != None)) {
            symbol_index_insert_declaration_path(
                with_decl.clone(),
                module_path.clone(),
                declaration_name.clone(),
                Rc::new(ContainmentPath {
                    ancestors: Rc::new(vec![module.clone()]),
                    terminal: item.clone(),
                    _phantom: std::marker::PhantomData,
                }),
            )
                ...
                |acc: Rc<SymbolIndex>, item: Rc<Node>| match item.connective.clone() {
                    Connective::Disj => item.children.clone().iter().cloned().fold(
                        ...
                        |a2: Rc<SymbolIndex>, child: Rc<Node>| {
                            ...
                            let with_variant_path = symbol_index_insert_declaration_path(
                                a2,
                                module_path.clone(),
                                vname.clone(),
                                Rc::new(ContainmentPath {
                                    ancestors: Rc::new(vec![module.clone(), item.clone()]),
...
) -> Rc<SymbolIndex> {
    {
        ...
        module_nodes.clone().iter().cloned().fold(
            ...
            |index: Rc<SymbolIndex>, module_node: Rc<Node>| {
                ...
                let with_root = symbol_index_insert_module_root(
                    index,
                    module_path.clone(),
                    module_node.clone(),
                );
...
pub fn symbol_index_with_bare_fill(
...
) -> Rc<SymbolIndex> {
    {
        ...
        let declaration_bare2 = bare_keys.clone().iter().cloned().fold(
            closure.declaration_bare.clone(),
            |acc: Rc<HashMap<String, Rc<Vec<Rc<ContainmentPath<Rc<Node>>>>>>>, k: String| {
                match v1_rt::map_get(&closure.global_bare.clone(), k.clone()) {
                    Some(_) => acc.clone(),
                    None => match v1_rt::map_get(&tree.declaration_bare.clone(), k.clone()) {
                        Some(paths) => v1_rt::rc_map_insert(acc.clone(), k.clone(), paths.clone()),
                        None => acc.clone(),
                    },
                }
            },
        );
        let module_roots2 = Rc::new(v1_rt::map_keys(&tree.module_roots.clone()))
            .iter()
            .cloned()
            .fold(
                closure.module_roots.clone(),
                |acc: Rc<HashMap<String, Rc<Node>>>, k: String| match v1_rt::map_get(
                    &acc,
                    k.clone(),
                ) {
                    Some(_) => acc.clone(),
                    None => match v1_rt::map_get(&tree.module_roots.clone(), k.clone()) {
                        Some(root) => v1_rt::rc_map_insert(acc.clone(), k.clone(), root.clone()),
                        None => acc.clone(),
                    },
                },
            );
        Rc::new(SymbolIndex {
            ...
            global_bare: global2.clone(),
            services: services2.clone(),
            declaration_entries: closure.declaration_entries.clone(),
            declaration_bare: declaration_bare2.clone(),
            module_roots: module_roots2.clone(),
        })
    }
}
            ...
            |index: Rc<SymbolIndex>, module_node: Rc<Node>| {
                ...
                let with_items = items.clone().iter().cloned().fold(
                    ...
                    |acc: Rc<SymbolIndex>, item: Rc<Node>| match local_binding_for_item(
                        ...
                        None => acc.clone(),
                        Some(binding) => symbol_index_insert_declaration_path(
                            symbol_index_insert(
                                acc.clone(),
                                v1_rt::concat(
                                    v1_rt::concat(module_path.clone(), ".".to_string()),
                            ...
                            binding.name.clone(),
                            Rc::new(ContainmentPath {
                                ancestors: Rc::new(vec![module_node.clone()]),
                                terminal: item.clone(),
                                _phantom: std::marker::PhantomData,
                            }),
                        ),
                    ...
                    |acc: Rc<SymbolIndex>, item: Rc<Node>| match item.connective.clone() {
                        Connective::Disj => item.children.clone().iter().cloned().fold(
                            ...
                            |a2: Rc<SymbolIndex>, child: Rc<Node>| {
                                let vname = authored_name_at(source_indices.clone(), child.clone());
                                match v1_rt::map_get(&counts, vname.clone()) {
                                    Some(1) => symbol_index_insert_declaration_path(
                                        symbol_index_insert(
                                            a2.clone(),
                                            v1_rt::concat(
                                                v1_rt::concat(module_path.clone(), ".".to_string()),
...
pub fn symbol_index_with_qualified_fill(
...
) -> Rc<SymbolIndex> {
    {
        ...
        let declaration_entries2 = Rc::new(v1_rt::map_keys(&fill.declaration_entries.clone()))
            .iter()
            .cloned()
            .fold(
                closure.declaration_entries.clone(),
                |acc: Rc<HashMap<String, Rc<Vec<Rc<ContainmentPath<Rc<Node>>>>>>>, k: String| {
                    match v1_rt::map_get(&acc, k.clone()) {
                        Some(_) => acc.clone(),
                        None => {
                            match v1_rt::map_get(&fill.declaration_entries.clone(), k.clone()) {
                                Some(paths) => {
                                    v1_rt::rc_map_insert(acc.clone(), k.clone(), paths.clone())
                                }
                                None => acc.clone(),
                            }
                        }
        ...
        let module_roots2 = Rc::new(v1_rt::map_keys(&fill.module_roots.clone()))
            .iter()
            .cloned()
            .fold(
                closure.module_roots.clone(),
                |acc: Rc<HashMap<String, Rc<Node>>>, k: String| match v1_rt::map_get(
                    ...
                    Some(_) => acc.clone(),
                    None => match v1_rt::map_get(&fill.module_roots.clone(), k.clone()) {
                        Some(root) => v1_rt::rc_map_insert(acc.clone(), k.clone(), root.clone()),
                        None => acc.clone(),
                    },
                },
        ...
        Rc::new(SymbolIndex {
            ...
            declaration_entries: declaration_entries2.clone(),
            declaration_bare: closure.declaration_bare.clone(),
            module_roots: module_roots2.clone(),
        })
    }
}
...
pub fn typecheck_module(
...
) -> Rc<TypecheckModuleResult> {
    {
        ...
        let infer_scope = Rc::new(InferScope {
            ...
            lambda_param_provenance: v1_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
            occurrence_ancestors: Rc::new(vec![resolved.module.clone()]),
        });
