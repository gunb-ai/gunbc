...
use crate::v1_std_core::CompilerDiagnostic::{InternalError, TypeMismatch, VariantCollision};
        ...
        if (type_name.clone() == None) {
        ...
        } else {
            {
                ...
                let type_diags = match effective_lookup.clone() {
                    ...
                    None => Rc::new(vec![inference_error(
                        v1_rt::concat(
                            v1_rt::concat("type '".to_string(), type_name.clone().unwrap()),
...
pub fn build_per_field_for_let(
    ...
    match (*val.expr_data.clone()).clone() {
        ExprData::ExprCall { .. } => {
            ...
            match v1_rt::map_get(&ctx.func_sigs.clone(), callee) {
                Some(sig) => {
...
pub fn classify_let_value(val: Rc<Node>, ctx: Rc<DescentContext>) -> Option<Rc<SubValueRelation>> {
    match (*val.expr_data.clone()).clone() {
        ...
        ExprData::ExprCall { .. } => {
            let callee =
                expr_call_func_at(val.clone(), ctx.type_env.clone().source_indices.clone());
            let from_provenance = match v1_rt::map_get(&ctx.func_sigs.clone(), callee.clone()) {
                    ...
                    Some(first_arg) => {
                        ...
                        match (*inner_rel.clone()).clone() {
                            ...
                            SubValueRelation::PreservedValue => {
                                ...
                                let from_provenance =
                                    match v1_rt::map_get(&ctx.func_sigs.clone(), callee.clone()) {
                                        Some(sig) => {
...
pub fn infer_output_provenance(
    ...
    func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>,
...
classify_body_provenance(arm_body(arm.clone()), param_names.clone(), param_types.clone(), type_env.clone(), func_sigs.clone(), arm_let_prov.clone())
});
                    }
                    __result
                });
...
pub fn method_call_args_by_name(
...
) -> Rc<HashMap<String, Rc<Node>>> {
    {
        ...
        let base = match v1_rt::map_get(&func_sigs, mname) {
...
pub fn compose_callee_provenance(
    ...
    match v1_rt::map_get(&func_sigs, callee) {
        Some(sig) => match sig.output_provenance.clone().first().cloned() {
            ...
            if ((item.body.clone() != None) && ((item.params.clone().len() as i64) > 0)) {
                {
                    ...
                    let annotated_body = if is_recursive.clone() {
                        annotate_descent_evidence(
                            ...
                            scope.func_env.clone().signatures.clone(),
                        )
                    } else {
                        body_typed.clone()
                    };
                    ...
                    if (item.is_self_recursive.clone() && (item.body.clone() != None)) {
                        Rc::new(Node {
                            ...
                            body: Some(annotate_descent_evidence(
                                ...
                                env.clone(),
                                data_locals.clone(),
                                updated_func_env.signatures.clone(),
                            )),
                            connective: item.connective.clone(),
