        ...
        match (*texpr.expr_data.clone()).clone() {
            ...
            ExprData::ExprCall { .. } => {
                    ...
                    && (sig.clone() == None))
                {
                    ...
                    {
                        let value_params = sig_params.clone().filter(|p| {
                            !param_is_generic_decl(
                                p.clone(),
                                scope.type_env.clone().source_indices.clone(),
                            )
                        });
                        ...
                        .fold(
                            ...
                            |st: Rc<ArgGenericFoldState>, pair: (i64, Rc<Node>)| {
                                let a = pair.1.clone();
                                let formal_lookup =
                                    nth_value_param(value_params.clone(), pair.0.clone());
                ...
                if (sig.clone() != None) {
                    {
                        ...
                        let value_params_for_check = sig_params.clone().filter(|p| {
                            !param_is_generic_decl(
                                p.clone(),
                                scope.type_env.clone().source_indices.clone(),
                            )
...
pub fn nth_value_param_note() -> String {
    thread_local! {
        static CACHED: String = {
            "Sig-typed Optional carrier for the callable-arg formal lookup: the match scrutinee must carry a RESOLVED Optional type for the emitter to lower Present/Absent to Some/None — a bare let-in-lambda pipeline (skip |> first) leaves the binding unstamped and the emitter falls back to a raw struct-variant pattern that is not legal Rust over Option.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}

pub fn nth_value_param(params: Rc<Vec<Rc<Node>>>, index: i64) -> Option<Rc<Node>> {
    params.clone().get(index.clone() as usize).cloned()
}
