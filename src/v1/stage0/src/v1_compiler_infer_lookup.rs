...
pub fn lookup_func_sig(
    ...
    match (*lookup_resolved_sig(func_env.clone(), name.clone())).clone() {
        FuncSigLookup::FuncSigResolved { sig: sig, .. } => {
            Arc::new(FuncSigLookup::FuncSigResolved { sig: sig.clone() })
        }
        FuncSigLookup::FuncSigAmbiguous {
            candidates: cands, ..
        } => Arc::new(FuncSigLookup::FuncSigAmbiguous {
            candidates: cands.clone(),
        }),
        FuncSigLookup::FuncSigUnresolved => {
            match func_sig_from_global_bare(type_env.clone(), name.clone()) {
                Some(sig) => Arc::new(FuncSigLookup::FuncSigResolved { sig: sig.clone() }),
                None => Arc::new(FuncSigLookup::FuncSigUnresolved),
            }
        }
    }
}

pub fn func_sig_if_resolved(lookup: Arc<FuncSigLookup>) -> Option<Arc<ResolvedFuncSig>> {
...
pub fn func_decl_owner_module(func_env: Arc<ResolvedFuncEnv>, name: String) -> Option<String> {
        ...
        if is_product.clone() {
            {
                ...
                Arc::new(StructuralMethodLookup {
                    ...
                    kernel_diagnostics: Arc::new(vec![]),
                })
            }
        } else {
            {
