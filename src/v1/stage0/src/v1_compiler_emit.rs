...
pub fn collect_type_names_from_node(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let self_name = if (n.ident_span.clone() != None) {
            vec![authored_name_at(source_indices.clone(), n.clone())]
        } else {
            vec![]
        };
                    ...
                    for child in expr.children.clone().iter().cloned() {
                        __result.push(match (*child.expr_data.clone()).clone() {
                            ...
                            _ => StringPart::Interpolation {
                                expr: Box::new(arg_value(child.clone())),
                            },
                        });
                    }
                    __result
...
pub fn named_type_vars_in_inferred(inferred: Option<InferredNode>) -> Rc<Vec<String>> {
    match inferred.clone().as_deref().cloned() {
        Some(InferredNode::TypeVariable { id: var_id, .. }) => vec![var_id.clone()],
                        ...
                        if (has_container_template.clone() && (param_count.clone() == 1)) {
                            ...
                            if (param_count.clone() > 0) {
                                ...
                                if (tn.clone() == tuple_type_name()) {
                                    render_tuple_parts(vec![], target.clone())
                                } else {
                                    coerce_primitive_type(target.clone(), tn.clone())
                                }
                            }
...
pub fn service_field_decls(fs: ServiceFieldSet, t: Rc<ServiceFieldTemplates>) -> Rc<Vec<String>> {
    {
        ...
        let result = if fs.has_rest.clone() {
            v1_rt::concat(result.clone(), vec![t.rest_decl.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_auth.clone() {
            v1_rt::concat(result.clone(), vec![t.auth_decl.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_shell.clone() {
            v1_rt::concat(result.clone(), vec![t.shell_decl.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_file.clone() {
            v1_rt::concat(result.clone(), vec![t.file_decl.clone()])
        } else {
            result.clone()
        };
        result.clone()
...
pub fn service_field_ctors(fs: ServiceFieldSet, t: Rc<ServiceFieldTemplates>) -> Rc<Vec<String>> {
    {
        let result = vec![];
        let result = if fs.has_rest.clone() {
            v1_rt::concat(result.clone(), vec![t.rest_ctor.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_auth.clone() {
            v1_rt::concat(result.clone(), vec![t.auth_ctor.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_shell.clone() {
            v1_rt::concat(result.clone(), vec![t.shell_ctor.clone()])
        } else {
            result.clone()
        };
        let result = if fs.has_file.clone() {
            v1_rt::concat(result.clone(), vec![t.file_ctor.clone()])
        } else {
            result.clone()
        };
        result.clone()
...
pub fn tco_reassign_core(
...
) -> Rc<Vec<String>> {
    {
        ...
        v1_rt::concat(
            ...
            vec![v1_rt::concat(line_prefix.clone(), continue_str.clone())],
        )
    }
}
            ...
            for arm in arms.clone().iter().cloned() {
                __result.push({
                    ...
                    let tco_body = emit_unified_tco_expr(
                        TcoFrame {
                            expr: Box::new(arm_body(arm.clone())),
                            scope: scope.clone(),
...
) -> String {
    {
        ...
        match emit_algebra_method_template(
            ...
            None => {
                ...
                let all_args = v1_rt::concat(vec![recv_str.clone()], arg_strs.clone());
                v1_rt::concat(
                    v1_rt::concat(
                        v1_rt::concat(
                            v1_rt::concat(
