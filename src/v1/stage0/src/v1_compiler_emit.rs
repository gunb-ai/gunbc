...
pub use crate::v1_compiler_languages::{
    BlockSyntax, CharSanitization, EmojiCharEscape, EscapePair, ExpressionSemantics, IfValueForm,
    ImportRule, ImportTrigger, InterpStyle, LanguageSpec, MatchValueForm, NamingCase,
    RecordLitSyntax, ReservedWordStrategy, ServiceFieldTemplates, StringInterpSyntax, TcoSyntax,
    TestConventions, TestNameStyle, VariantPatternSyntax, VisibilitySpec,
};
use crate::v1_rt;
...
pub fn apply_char_sanitization(name: String, rule: Rc<CharSanitization>) -> String {
    match (*rule).clone() {
        CharSanitization::NoCharSanitization => name,
        CharSanitization::EmojiEscape { rule: r, .. } => {
            escape_emoji_codepoints(name, r.prefix.clone(), r.suffix.clone())
        }
    }
}
...
pub fn emit_ident(name: String, target: RenderTarget) -> String {
    {
        let spec = language_spec(target);
        let sanitized = apply_char_sanitization(name, spec.char_sanitization.clone());
        let converted = match spec.naming_case.clone() {
            NamingCase::SnakeCase => to_snake(sanitized),
            NamingCase::CamelCase => to_camel(sanitized),
            NamingCase::PascalCase => capitalize_first(to_camel(sanitized)),
            NamingCase::AsAuthored => sanitized,
        };
        let is_reserved = {
            let mut __found = false;
...
pub fn go_variant_case_type(name: String, parent_enum: String) -> String {
    match parent_enum {
