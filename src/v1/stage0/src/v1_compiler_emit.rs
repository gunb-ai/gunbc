...
use self::BoundOperation::*;
use self::EmitterOutcome::*;
use self::ExprCategory::*;
use self::FileResultChannel::*;
use self::FileResultChannelResult::*;
use self::FileResultFieldResult::*;
use self::FileResultFieldsResult::*;
use self::FileVerb::*;
use self::FileVerbResult::*;
...
pub use crate::v1_std_core::{
    ...
    expr_literal_string_optional, expr_method_call_semantics, expr_method_name_at,
    expr_var_name_at, field_access_base, field_access_field_at, field_binding_name_at,
    field_binding_pattern, field_init_node_name_at, field_init_node_value,
    field_init_operation_modifier, find_child_named, find_property_string, foreach_body,
...
#[serde(tag = "_variant")]
pub enum FileResultChannel {
    FileChanSuccess,
    FileChanBytesWritten,
    FileChanPath,
    FileChanError,
    FileChanContent,
}
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct FileResultField {
    pub channel: FileResultChannel,
    pub optional: bool,
}
...
pub enum TransportBindingRefusal {
    UnrecognizedTransportShape,
    FileTransportMissingPath,
    FileTransportUnknownVerb { verb: String },
    FileTransportWriteMissingContent,
    FileTransportUnknownResultChannel { channel: String },
    FileTransportUndeclaredResultChannel,
}
...
pub enum BoundOperation {
    ...
    BindingRefused {
        cause: Rc<TransportBindingRefusal>,
    },
}
...
pub fn transport_binding_refusal_reason(cause: Rc<TransportBindingRefusal>) -> String {
    match (*cause.clone()).clone() {
    TransportBindingRefusal::UnrecognizedTransportShape => "the transport declaration is nonempty but is not a rest, shell or file transport".to_string(),
    TransportBindingRefusal::FileTransportMissingPath => "the file transport declares no `path:` template, so no operation could name a file".to_string(),
    TransportBindingRefusal::FileTransportUnknownVerb { verb: v, .. } => v1_rt::concat(v1_rt::concat("the file transport declares verb \"".to_string(), v.clone()), "\", which is not one of read, write, write_owner_only, delete, list".to_string()),
    TransportBindingRefusal::FileTransportWriteMissingContent => "the file transport declares a write verb but the operation declares no `content` input to write".to_string(),
    TransportBindingRefusal::FileTransportUnknownResultChannel { channel: c, .. } => v1_rt::concat(v1_rt::concat("an output field reads from \"".to_string(), c.clone()), "\", which the file realization does not produce".to_string()),
    TransportBindingRefusal::FileTransportUndeclaredResultChannel => "an output field declares no `from` key, so no file result could be projected into it".to_string(),
}
}

pub fn file_transport_path_template(
    t: Rc<Node>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Option<Rc<Node>> {
    match transport_base_path(t.clone(), source_indices.clone()) {
        Some(p) => match expr_literal_string_optional(p.clone()) {
            Some(ref __some_val) if __some_val == "" => None,
            _ => Some(p.clone()),
        },
        None => None,
    }
...
pub fn operation_has_param(
...
) -> bool {
    {
        ...
        for p in op_node.params.clone().iter().cloned() {
            if (param_node_name_at(p.clone(), source_indices.clone()) == name.clone()) {
                __found = true;
                break;
            }
        }
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum FileVerbResult {
    FileVerbBound { verb: FileVerb },
    FileVerbUnbound { cause: Rc<TransportBindingRefusal> },
}
...
pub fn bind_file_verb(
    ...
    match find_property_string(
        ...
        Some(v) => {
            if (v.clone() == "read".to_string()) {
                Rc::new(FileVerbResult::FileVerbBound {
                    verb: FileVerb::FileRead,
                })
            } else {
                if (v.clone() == "write".to_string()) {
                    Rc::new(FileVerbResult::FileVerbBound {
                        verb: FileVerb::FileWrite,
                    })
                } else {
                    if (v.clone() == "write_owner_only".to_string()) {
                        Rc::new(FileVerbResult::FileVerbBound {
                            verb: FileVerb::FileWriteOwnerOnly,
                        })
                    } else {
                        if (v.clone() == "delete".to_string()) {
                            Rc::new(FileVerbResult::FileVerbBound {
                                verb: FileVerb::FileDelete,
                            })
                        } else {
                            if (v.clone() == "list".to_string()) {
                                Rc::new(FileVerbResult::FileVerbBound {
                                    verb: FileVerb::FileList,
                                })
                            } else {
                                Rc::new(FileVerbResult::FileVerbUnbound {
                                    cause: Rc::new(
                                        TransportBindingRefusal::FileTransportUnknownVerb {
                                            verb: v.clone(),
                                        },
                                    ),
                                })
...
pub fn file_verb_writes_content(v: FileVerb) -> bool {
    match v.clone() {
        FileVerb::FileWrite => true,
        FileVerb::FileWriteOwnerOnly => true,
        FileVerb::FileRead => false,
        FileVerb::FileDelete => false,
        FileVerb::FileList => false,
    }
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum FileResultChannelResult {
    FileChanBound { channel: FileResultChannel },
    FileChanUnbound { cause: Rc<TransportBindingRefusal> },
}

pub fn bind_file_result_channel(key: String) -> Rc<FileResultChannelResult> {
    if (key.clone() == "write_success".to_string()) {
        Rc::new(FileResultChannelResult::FileChanBound {
            channel: FileResultChannel::FileChanSuccess,
        })
    } else {
        if (key.clone() == "bytes_written".to_string()) {
            Rc::new(FileResultChannelResult::FileChanBound {
                channel: FileResultChannel::FileChanBytesWritten,
            })
        } else {
            if (key.clone() == "path".to_string()) {
                Rc::new(FileResultChannelResult::FileChanBound {
                    channel: FileResultChannel::FileChanPath,
                })
            } else {
                if (key.clone() == "error".to_string()) {
                    Rc::new(FileResultChannelResult::FileChanBound {
                        channel: FileResultChannel::FileChanError,
                    })
                } else {
                    if (key.clone() == "content".to_string()) {
                        Rc::new(FileResultChannelResult::FileChanBound {
                            channel: FileResultChannel::FileChanContent,
                        })
                    } else {
                        Rc::new(FileResultChannelResult::FileChanUnbound {
                            cause: Rc::new(
                                TransportBindingRefusal::FileTransportUnknownResultChannel {
                                    channel: key.clone(),
                                },
                            ),
                        })
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum FileResultFieldsResult {
    FileFieldsBound {
        fields: Rc<Vec<Rc<FileResultField>>>,
        single_field: bool,
    },
    FileFieldsUnbound {
        cause: Rc<TransportBindingRefusal>,
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum FileResultFieldResult {
    FileFieldBound { field: Rc<FileResultField> },
    FileFieldUnbound { cause: Rc<TransportBindingRefusal> },
}

pub fn bind_file_result_field(
    ch: Rc<Node>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Rc<FileResultFieldResult> {
    match child_from_key(ch.clone(), source_indices.clone()) {
        None => Rc::new(FileResultFieldResult::FileFieldUnbound {
            cause: Rc::new(TransportBindingRefusal::FileTransportUndeclaredResultChannel),
        }),
        Some(key) => match (*bind_file_result_channel(key.clone())).clone() {
            FileResultChannelResult::FileChanUnbound { cause: cz, .. } => {
                Rc::new(FileResultFieldResult::FileFieldUnbound { cause: cz.clone() })
            }
            FileResultChannelResult::FileChanBound { channel: c, .. } => {
                Rc::new(FileResultFieldResult::FileFieldBound {
                    field: Rc::new(FileResultField {
                        channel: c.clone(),
...
pub fn bind_file_result_fields(
    op_node: Rc<Node>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Rc<FileResultFieldsResult> {
    {
        ...
        if ((children.clone().len() as i64) == 0) {
            Rc::new(FileResultFieldsResult::FileFieldsUnbound {
                cause: Rc::new(TransportBindingRefusal::FileTransportUndeclaredResultChannel),
            })
        } else {
            {
                let bound = Rc::new({
                    ...
                    for ch in children.clone().iter().cloned() {
                        __result.push(bind_file_result_field(ch.clone(), source_indices.clone()));
                    }
                    __result
                });
                let refusals = Rc::new({
                    let mut __result = Vec::new();
                    for b in bound.clone().iter().cloned() {
                        __result.extend(
                            (*match (*b.clone()).clone() {
                                FileResultFieldResult::FileFieldUnbound { cause: cz, .. } => {
                                    Rc::new(vec![cz.clone()])
                                }
                                FileResultFieldResult::FileFieldBound { field: _, .. } => {
                                    Rc::new(vec![])
                                }
                            })
                            .iter()
                ...
                match refusals.clone().first().cloned() {
                    Some(cz) => {
                        Rc::new(FileResultFieldsResult::FileFieldsUnbound { cause: cz.clone() })
                    }
                    None => Rc::new(FileResultFieldsResult::FileFieldsBound {
                        fields: Rc::new({
                            let mut __result = Vec::new();
                            for b in bound.clone().iter().cloned() {
                                __result.extend(
                                    (*match (*b.clone()).clone() {
                                        FileResultFieldResult::FileFieldBound {
                                            field: f, ..
                                        } => Rc::new(vec![f.clone()]),
                                        FileResultFieldResult::FileFieldUnbound {
                                            cause: _,
                                            ..
                                        } => Rc::new(vec![]),
                                    })
                        ...
                        single_field: ((children.clone().len() as i64) == 1),
                    }),
                }
            }
        }
...
pub fn bind_file_operation(
    ...
    match file_transport_path_template(t.clone(), source_indices.clone()) {
        None => Rc::new(BoundOperation::BindingRefused {
            cause: Rc::new(TransportBindingRefusal::FileTransportMissingPath),
        }),
        Some(path_tpl) => {
            match (*bind_file_verb(t.clone(), op_node.clone(), source_indices.clone())).clone() {
                FileVerbResult::FileVerbUnbound { cause: cz, .. } => {
                    Rc::new(BoundOperation::BindingRefused { cause: cz.clone() })
                }
                FileVerbResult::FileVerbBound { verb: v, .. } => {
                    let has_content = operation_has_param(
                        op_node.clone(),
                        "content".to_string(),
                        source_indices.clone(),
                    );
                    if (file_verb_writes_content(v.clone()) && !has_content.clone()) {
                        Rc::new(BoundOperation::BindingRefused {
                            cause: Rc::new(
                                TransportBindingRefusal::FileTransportWriteMissingContent,
                            ),
                        })
                    } else {
                        match (*bind_file_result_fields(op_node.clone(), source_indices.clone()))
                            .clone()
                        {
                            FileResultFieldsResult::FileFieldsUnbound { cause: cz, .. } => {
                                Rc::new(BoundOperation::BindingRefused { cause: cz.clone() })
                            }
                            FileResultFieldsResult::FileFieldsBound {
                                fields: fs,
                                single_field: sf,
                                ..
                            } => Rc::new(BoundOperation::FileBound {
                                verb: v.clone(),
                                path_template: path_tpl.clone(),
                                content_param: if has_content.clone() {
                                    Some("content".to_string())
                                } else {
                                    None
                                },
                                result_fields: fs.clone(),
                                single_field: sf.clone(),
                            }),
                        }
                    }
                }
...
pub fn transport_is_canonically_empty(t: Rc<Node>) -> bool {
...
pub fn bind_operation_transport(
    ...
    if is_rest_transport(t.clone(), source_indices.clone()) {
        Rc::new(BoundOperation::RestBound {
            transport: t.clone(),
        })
    } else {
        if is_shell_transport(t.clone()) {
            ...
            if is_file_transport(t.clone(), source_indices.clone()) {
                ...
                if transport_is_canonically_empty(t.clone()) {
                    Rc::new(BoundOperation::LocalBound)
                } else {
                    Rc::new(BoundOperation::BindingRefused {
                        cause: Rc::new(TransportBindingRefusal::UnrecognizedTransportShape),
                    })
                }
            }
        }
...
pub fn emit_unified_transport_dispatch(
    ...
    match (*bound.clone()).clone() {
        ...
        BoundOperation::LocalBound => render_local(op_name.clone(), depth.clone()),
        ...
        BoundOperation::BindingRefused { cause: cz, .. } => emit_error_expr(
            v1_rt::concat(
                ...
                transport_binding_refusal_reason(cz.clone()),
            ),
            target.clone(),
        ),
    }
...
pub fn emit_unified_operation_method(
...
) -> String {
    {
        ...
        let bound = bind_operation_transport(eff_transport.clone(), op_node.clone(), si.clone());
...
pub fn unmodeled_file_transport_missing_fact() -> String {
    thread_local! {
        static CACHED: String = {
            "this target has no file-transport renderer; the operation binds target-neutrally but only the Rust backend consumes a FileBound today, and a target without one refuses rather than falling back to the per-target renderers deleted in #8858".to_string()
        };
    }
...
pub fn target_renders_file_transport(target: RenderTarget) -> bool {
    match target.clone() {
        RenderTarget::Rust => true,
                ...
                __result.extend(
                    (*{
                        ...
                        if !is_file_transport_after_dispatch_precedence(t.clone(), si.clone()) {
                            ...
                            {
                                let missing = if !target_renders_file_transport(target.clone()) {
                                    Some(unmodeled_file_transport_missing_fact())
                                } else {
                                    match (*bind_operation_transport(
                                        t.clone(),
                                        op_node.clone(),
                                        si.clone(),
                                    ))
                                    .clone()
                                    {
                                        BoundOperation::BindingRefused { cause: cz, .. } => {
                                            Some(transport_binding_refusal_reason(cz.clone()))
                                        }
                                        BoundOperation::RestBound { transport: _, .. } => None,
                                        BoundOperation::ShellBound { transport: _, .. } => None,
                                        BoundOperation::LocalBound => None,
                                        BoundOperation::FileBound {
                                            path_template: __pt,
                                            content_param: __cp,
                                            result_fields: __rf,
                                ...
                                match missing.clone() {
                                    None => Rc::new(vec![]),
                                    Some(fact) => Rc::new(vec![make_error_node(
...
pub struct FileList;
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct FileChanSuccess;
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct FileChanBytesWritten;
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
