...
// --- Complexity bad-shape R1: accumulator-in-copied-port (fixture-only census) ---
//
// R1 flags a fold accumulator passed to a copy port: `map_merge(acc, …)` (base cloned)
// or `list_append(left: acc, …)` / `list_append(acc, …)` (left copied). Host-fed over
// synthetic fixtures only until whole-corpus `decl_facts(roots)` reflection lands (#5966).
// DISSOLUTION: fold into a pure `.dag` Node-tree reader at gunbc#5364; widen scan to
// witness_layer_roots once the enforcement-intent gate enrolls the object rule.

const COMPLEXITY_BAD_SHAPE_R1_FIXTURE_ROOT: &str =
    "src/v2/lens/fixture/complexity_bad_shape_r1";

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord)]
pub struct ComplexityBadShapeR1Finding {
    pub site: String,
    pub rule: String,
    pub acc_name: String,
}

fn r1_fixture_dag_files() -> Vec<(String, String)> {
    let dir = workspace_root().join(COMPLEXITY_BAD_SHAPE_R1_FIXTURE_ROOT);
    let mut paths = Vec::new();
    collect_dag_files_tolerant(&dir, &mut paths);
    let mut out = Vec::new();
    for p in paths {
        let rel = repo_rel(&p);
        if let Ok(content) = std::fs::read_to_string(&p) {
...
fn r1_callback_first_param(header: &str) -> Option<String> {
    let s = header.trim();
    let inner = if let Some(rest) = s.strip_prefix("fn(") {
        rest.split(')').next()?
    ...
    if acc.is_empty() || !nfr_is_ident(acc) {
        return None;
    }
    Some(acc.to_string())
}

fn r1_preceded_by_fold(text: &str, pos: usize) -> bool {
    let window_start = pos.saturating_sub(400);
    let window = &text[window_start..pos];
    window.contains("fold_list(") || window.contains("fold(")
}

fn r1_extract_callback_body(rest: &str) -> Option<(String, String)> {
    let rest = rest.trim_start();
    if let Some(after_fn) = rest.strip_prefix("fn(") {
        let close = after_fn.find(')')?;
        let acc = r1_callback_first_param(&format!("fn({}", &after_fn[..close]))?;
        let after_paren = &after_fn[close + 1..].trim_start();
        if let Some(after_brace) = after_paren.strip_prefix('{') {
            let close_brace = nfr_matching_brace(after_brace.as_bytes(), 0)?;
            return Some((acc, after_brace[..close_brace].to_string()));
        }
        return None;
    }
    if rest.starts_with('(') {
        let close = rest.find(')')?;
        let acc = r1_callback_first_param(&rest[..=close])?;
        let after_paren = rest[close + 1..].trim_start();
        if let Some(expr) = after_paren.strip_prefix("=>") {
            return Some((acc, expr.trim().to_string()));
        }
    }
    None
}

fn r1_fold_callbacks(body: &str) -> Vec<(String, String)> {
    let mut out = Vec::new();
    for anchor in ["cons:", "f:"] {
        let mut search_from = 0usize;
        while let Some(rel) = body[search_from..].find(anchor) {
            let abs = search_from + rel;
            if r1_preceded_by_fold(body, abs) {
                let after = abs + anchor.len();
                if let Some(cb) = r1_extract_callback_body(&body[after..]) {
                    out.push(cb);
                }
            }
...
fn r1_ws_stripped_contains(haystack: &str, needle: &str) -> bool {
    let hs: String = haystack.chars().filter(|c| !c.is_whitespace()).collect();
    let nd: String = needle.chars().filter(|c| !c.is_whitespace()).collect();
    hs.contains(&nd)
}

fn r1_acc_in_map_merge_copied_port(text: &str, acc: &str) -> bool {
    r1_ws_stripped_contains(text, &format!("map_merge({acc},"))
}

fn r1_acc_in_list_append_copied_port(text: &str, acc: &str) -> bool {
    if r1_ws_stripped_contains(text, &format!("list_append(left:{acc},")) {
        return true;
    }
    let mut search = 0usize;
    while let Some(rel) = text[search..].find("list_append(") {
        let abs = search + rel + "list_append(".len();
        let tail = text[abs..].trim_start();
        if tail.starts_with(acc)
            && tail
                .chars()
                .nth(acc.len())
                .is_none_or(|c| c == ',' || c == ')')
...
fn r1_findings_in_fn(rel: &str, sig: &NfrFnSig) -> Vec<ComplexityBadShapeR1Finding> {
    let mut out = Vec::new();
    for (acc, cb_body) in r1_fold_callbacks(&sig.body) {
        if r1_acc_in_map_merge_copied_port(&cb_body, &acc) {
            out.push(ComplexityBadShapeR1Finding {
                site: format!("{}::{}", rel, sig.name),
                rule: "map_merge_overlay".to_string(),
                acc_name: acc.clone(),
            });
        } else if r1_acc_in_list_append_copied_port(&cb_body, &acc) {
            out.push(ComplexityBadShapeR1Finding {
                site: format!("{}::{}", rel, sig.name),
                rule: "list_append_left".to_string(),
                acc_name: acc.clone(),
            });
        }
    }
...
fn r1_findings_in_files(files: &[(String, String)]) -> Vec<ComplexityBadShapeR1Finding> {
    let mut out = Vec::new();
    for (rel, content) in files {
        let src = nfr_strip_comments(content);
        for sig in nfr_parse_fns(&src) {
            out.extend(r1_findings_in_fn(rel, &sig));
        }
    }
    out.sort();
    out.dedup();
...
struct ComplexityBadShapeR1Report {
    findings: Vec<ComplexityBadShapeR1Finding>,
}

fn r1_build_fixture_report() -> &'static ComplexityBadShapeR1Report {
    static REPORT: std::sync::OnceLock<ComplexityBadShapeR1Report> = std::sync::OnceLock::new();
    REPORT.get_or_init(|| ComplexityBadShapeR1Report {
        findings: r1_findings_in_files(&r1_fixture_dag_files()),
    })
}

pub fn complexity_bad_shape_r1_fixture_count() -> i64 {
    r1_build_fixture_report().findings.len() as i64
}

pub fn complexity_bad_shape_r1_fixture_site_fired(site: &str) -> bool {
    r1_build_fixture_report()
        .findings
        .iter()
        .any(|f| f.site == site)
}

pub fn complexity_bad_shape_r1_fixture_red_control_fires() -> bool {
    [
        "src/v2/lens/fixture/complexity_bad_shape_r1/bad_map_merge_acc.dag::merge_bad",
        "src/v2/lens/fixture/complexity_bad_shape_r1/bad_list_append_acc.dag::append_bad",
        "src/v2/lens/fixture/complexity_bad_shape_r1/merge_envs_negative.dag::merge_envs_like",
    ]
    .into_iter()
    .all(complexity_bad_shape_r1_fixture_site_fired)
}

pub fn complexity_bad_shape_r1_fixture_green_control_holds() -> bool {
    [
        "src/v2/lens/fixture/complexity_bad_shape_r1/good_overlay_port.dag::merge_good",
        "src/v2/lens/fixture/complexity_bad_shape_r1/good_cons_prepend.dag::prepend_good",
    ]
    .into_iter()
    .all(|site| !complexity_bad_shape_r1_fixture_site_fired(site))
}

#[cfg(test)]
mod complexity_bad_shape_r1_tests {
    use super::*;
    ...
    #[test]
    fn red_control_map_merge_acc_in_copied_port() {
        let f = files(&[(
            "bad.dag",
            "module bad\nfn merge_bad(items: List<Int>) -> Map<String, Int> {\n  fold(items, init: empty_map(), f: (acc, item) =>\n    map_merge(acc, item)\n  )\n}\n",
        )]);
        let findings = r1_findings_in_files(&f);
        assert!(
            findings.iter().any(|x| x.rule == "map_merge_overlay"),
            "expected map_merge overlay finding; got {findings:?}"
        );
    }

    #[test]
    fn red_control_list_append_left_acc() {
        let f = files(&[(
            "bad.dag",
            "module bad\nfn append_bad(xs: List<Int>) -> List<Int> {\n  fold_list(xs: xs, empty: Empty, cons: fn(acc, x) {\n    list_append(left: acc, right: Cons { head: x, tail: Empty })\n  })\n}\n",
        )]);
        let findings = r1_findings_in_files(&f);
        assert!(
            findings.iter().any(|x| x.rule == "list_append_left"),
            "expected list_append left finding; got {findings:?}"
        );
    }

    #[test]
    fn green_control_map_merge_acc_in_overlay_port() {
        let f = files(&[(
            "good.dag",
            "module good\nfn merge_good(items: List<Int>) -> Map<String, Int> {\n  fold(items, init: empty_map(), f: (acc, item) =>\n    map_merge(item, acc)\n  )\n}\n",
        )]);
        let findings = r1_findings_in_files(&f);
        assert!(
            findings.is_empty(),
            "acc in overlay port must not fire R1; got {findings:?}"
        );
    }

    #[test]
    fn green_control_cons_prepend_is_not_r1() {
        let f = files(&[(
            "good.dag",
            "module good\nfn prepend_good(xs: List<Int>) -> List<Int> {\n  fold_list(xs: xs, empty: Empty, cons: fn(acc, x) {\n    Cons { head: x, tail: acc }\n  })\n}\n",
        )]);
        let findings = r1_findings_in_files(&f);
        assert!(findings.is_empty(), "Cons prepend must not fire R1; got {findings:?}");
    }

    #[test]
    fn merge_envs_negative_pattern_fires() {
        let f = files(&[(
            "merge.dag",
            "module merge\nfn merge_envs_like(envs: List<Map<String, Int>>) -> Map<String, Int> {\n  fold(envs, init: empty_map(), f: (acc, env) =>\n    map_merge(acc, flatten_visible_bindings(env: env))\n  )\n}\n",
        )]);
        let findings = r1_findings_in_files(&f);
        assert!(
            findings.iter().any(|x| x.site.ends_with("::merge_envs_like")),
            "merge_envs-shaped negative must fire; got {findings:?}"
        );
    }

    #[test]
    fn synthetic_fixtures_red_and_green_controls() {
        assert!(complexity_bad_shape_r1_fixture_red_control_fires());
        assert!(complexity_bad_shape_r1_fixture_green_control_holds());
    }
}
