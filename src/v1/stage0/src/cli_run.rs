...
mod global_bare_wiring_oracle_tests {
    ...
    fn receive_path_is_total_not_gated_by_contribution() {
        ...
        // Encoded structurally: `global_bare_for_decl_file` no longer exists, and the
        // reconcile-loop `typecheck_module` call passes `global_bare.clone()` directly.
        // If a receive-path gate is reintroduced, this comment and the call site diverge
        // and the histogram RED control (#6707) reappears.
    }
...
// census overlay arm, `global_bare_for_decl_file`, and the census call in
