...
pub struct CacheWalkReport {
    ...
    // typed_module_cache: TypeEnv.bindings
    pub total_bindings: usize,
    pub distinct_binding_ptrs: usize,
    // typed_module_cache: TypeEnv.source_indices
    pub total_source_index_entries: usize,
    pub distinct_source_index_ptrs: usize,
    // typed_module_cache: TypeEnv.inductive_fields (map entry count)
    pub total_inductive_field_entries: usize,
    pub distinct_inductive_field_vec_ptrs: usize,
    // typed_module_cache: TypeEnv.intern_table
    pub distinct_intern_table_ptrs: usize,
    // typed_module_cache: func_env.signatures
    pub total_func_sigs: usize,
    pub distinct_func_sig_ptrs: usize,
    // typed_module_cache: typed items (Rc<Node> in items vec)
    pub total_typed_items: usize,
    pub distinct_typed_item_node_ptrs: usize,
    // parse_cache: parsed module nodes (Rc<Node>)
    pub total_parse_module_nodes: usize,
    pub distinct_parse_module_node_ptrs: usize,
}
...
impl CacheWalkReport {
    pub fn format(&self) -> String {
        ...
        format!(
            "=== MultiEntryIndex cache walk ===\n\
             typed_module_cache: {} modules\n\
             parse_cache:        {} files\n\
             \n\
             TypeEnv.bindings:\n\
               total entries:    {}\n\
               distinct Rc<>:    {} (sharing {}, ideal 1×)\n\
             TypeEnv.source_indices entries:\n\
               total:            {}\n\
               distinct Rc<NewlineIndex>: {} (sharing {})\n\
             TypeEnv.inductive_fields entries:\n\
               total:            {}\n\
               distinct Rc<Vec<>>: {} (sharing {})\n\
             TypeEnv.intern_table:\n\
               distinct Rc<>:    {} (sharing {})\n\
             func_env.signatures:\n\
               total:            {}\n\
               distinct Rc<>:    {} (sharing {})\n\
             typed items (Rc<Node> in items):\n\
               total:            {}\n\
               distinct Rc<>:    {} (sharing {})\n\
             parse module nodes:\n\
               total:            {}\n\
...
impl MultiEntryIndex {
    ...
    pub fn measure_caches(&self) -> CacheWalkReport {
        ...
        let typed_cache = self.typed_module_cache.borrow();
        ...
        for tc_result in typed_cache.values() {
            let tm = &tc_result.typed;
            let env = &tm.type_env;

            for binding in env.bindings.values() {
                total_bindings += 1;
                binding_ptrs.insert(Rc::as_ptr(binding));
            }
            for si in env.source_indices.values() {
                total_si_entries += 1;
                si_ptrs.insert(Rc::as_ptr(si));
            }
            for iv in env.inductive_fields.values() {
                total_if_entries += 1;
                if_vec_ptrs.insert(Rc::as_ptr(iv) as usize);
            }
            intern_ptrs.insert(Rc::as_ptr(&env.intern_table));
            for sig in tm.func_env.signatures.values() {
                total_func_sigs += 1;
                sig_ptrs.insert(Rc::as_ptr(sig));
            }
            for item in tm.items.iter() {
                total_typed_items += 1;
                typed_item_ptrs.insert(Rc::as_ptr(item));
            }
        }

        let mut total_parse_nodes = 0usize;
        let mut parse_node_ptrs: HashSet<*const Node> = HashSet::new();
        for (parse_result, _) in parse_cache_ref.values() {
            if let Some(module) = &parse_result.module {
                total_parse_nodes += 1;
                parse_node_ptrs.insert(Rc::as_ptr(module));
            }
        }

        CacheWalkReport {
            typed_module_count: typed_cache.len(),
            parse_cache_entry_count: parse_cache_ref.len(),
            total_bindings,
            distinct_binding_ptrs: binding_ptrs.len(),
            total_source_index_entries: total_si_entries,
            distinct_source_index_ptrs: si_ptrs.len(),
            total_inductive_field_entries: total_if_entries,
            distinct_inductive_field_vec_ptrs: if_vec_ptrs.len(),
            distinct_intern_table_ptrs: intern_ptrs.len(),
            total_func_sigs,
            distinct_func_sig_ptrs: sig_ptrs.len(),
            total_typed_items,
            distinct_typed_item_node_ptrs: typed_item_ptrs.len(),
            total_parse_module_nodes: total_parse_nodes,
            distinct_parse_module_node_ptrs: parse_node_ptrs.len(),
        }
    }
}
