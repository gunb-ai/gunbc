...
pub enum FuncSigLookup {
    FuncSigResolved { sig: Arc<ResolvedFuncSig> },
    FuncSigUnresolved,
    FuncSigAmbiguous { candidates: Arc<Vec<String>> },
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct ChainSigScan {
    pub first_sig: Option<Arc<ResolvedFuncSig>>,
    pub owners: Arc<Vec<String>>,
}
...
) -> Arc<FuncSigLookup> {
    {
        ...
        match (*module_path_owner_binding_decide(scan.owners.clone())).clone() {
            ModulePathBindingProjection::ModulePathBindingMiss => {
                Arc::new(FuncSigLookup::FuncSigUnresolved)
            }
...
pub fn lookup_resolved_sig(env: Arc<ResolvedFuncEnv>, name: String) -> Arc<FuncSigLookup> {
            ...
            } else {
                {
                    let first_hit = if v1_rt::resolution_silent_pick_is_enabled() {
                        ...
                        env.parents.clone().iter().cloned().fold(
                            none_resolved_sig(),
                            |acc: Option<Arc<ResolvedFuncSig>>, p: Arc<ResolvedFuncEnv>| match acc
                                .clone()
                            {
