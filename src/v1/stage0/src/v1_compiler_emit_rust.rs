...
//
// HAND-SYNCED MIRROR (class-3 splice) -- this committed seed is a HAND-SYNCED
// MIRROR of the .dag authority, NOT a regen fixpoint. This file is one of three
// (with v1_compiler_infer.rs, v1_compiler_infer_emit_info.rs) spliced from a
// faithful regen of the class-3 .dag onto the committed seed, with two local
// drifts hand-resolved (cargo header inlined below so it does not import the
// deliberately-unwired extdeps_cargo_version orphan; policy ref/value alignment
// at wire_value_serialize.rs). The other modules stay byte-identical to committed.
//
...
pub fn render_rust_applied_type(
...
) -> String {
    {
        ...
        let base = match rust_seed_host_container_base(base_name.clone(), corpus_repr.clone()) {
...
pub fn emit_cargo_toml(crate_name: String, has_services: bool) -> Rc<TextFile> {
    {
        // HAND-SYNCED MIRROR drift fix: inline the [package] header instead of
        // importing render_cargo_package_header_prefix from the deliberately-
        // unwired extdeps_cargo_version orphan (byte-identical to committed emit).
