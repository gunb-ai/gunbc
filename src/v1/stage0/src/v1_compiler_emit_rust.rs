...
//
// HAND-SYNCED MIRROR -- this committed seed is a HAND-SYNCED MIRROR of the .dag
// authority, NOT a regen fixpoint. It is regenerated from the .dag emitter and then
// has two local drifts hand-resolved (cargo header inlined below so it does not import
// the deliberately-unwired extdeps_cargo_version orphan; policy ref/value alignment at
// wire_value_serialize.rs). The other modules stay byte-identical to committed.
//
// THIS PR (faithful-carrier String element): the faithful String carrier renders its
// DECLARED element Char (String=FreeMonoid<Char>, std/string_type.dag) uniformly at leaf
// and applied, single application -- correcting an earlier Nat bake that double-applied as
// FreeMonoid<Nat><Char>. Faithful-only (corpus_repr_is_faithful guarded); the HOST seed
// emit is byte-identical (proven). See rust_named_type_base / rust_applied_type_base /
// render_rust_text_carrier and 05_emit_rust.dag.
//
// LANE (deferred): regen-fixpoint emitter-self-host (~150-gap) -- wire-all-emitted-
//   modules so a clean regen reproduces this committed seed bit-identically (the
//   §7 reproduce-fixpoint that licenses deleting src/v1).
// SCOPE: a faithful full regen wires the deliberately-unwired orphan std modules
//   (std_measure / std_algebra / std_realization_schedule / std_machine_constraints
//   / std_integer / extdeps_version_semver / extdeps_cargo_version) and surfaces
//   ~150 latent emitter-completeness gaps (std numeric/measure-tower generic
//   emission). They stay UNWIRED here exactly as on main.
// PROVENANCE (#5514 verbatim deferral): "regen-write still emits a non-building
//   seed from ~150 emitter-completeness gaps, the clean green regen fixpoint is
//   deferred, converges with the Route-A emitter self-host".
...
// INVARIANCE CAVEAT (#5481, quick-seal, verbatim): class-3 host-vs-faithful SELECTION is
...
//   OWED by the deferred faithful target, where the selection actually fires. (THIS PR
//   begins delivering that owed proof: the faithful target's String element is now
//   FreeMonoid<Char> by execution -- census + render-witness, see this file's header.)
