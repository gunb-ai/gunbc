...
pub fn rust_corpus_includes_v1_compiler(
...
) -> bool {
    {
        ...
        for k in Rc::new(v1_rt::map_keys(&source_indices)).iter().cloned() {
...
pub fn rust_seed_host_numeric_alias(
    name: String,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Option<String> {
...
pub fn render_rust_applied_type(
...
) -> String {
    {
        ...
        let base = rust_named_type_base(base_name.clone(), source_indices.clone());
                ...
                {
                    {
                        ...
                        let base = if (def_mod.clone() != local_mod) {
                            v1_rt::concat(
                                v1_rt::concat(
...
pub fn emit_fn_def(
...
) -> String {
    {
        ...
        match rust_host_string_seam_fn_emit(name.clone()) {
            Some(seam_fn) => {
                if rust_emit_faithful_text_carrier(si.clone()) {
                    seam_fn.clone()
                } else {
                    "".to_string()
                }
...
pub fn emit_typed_record_lit(
    ...
    Some(ft) => match rust_zero_value(ft.clone(), si.clone()) {
        ...
        if ((exit_props.clone().len() as i64) == 0) {
        ...
        } else {
            {
                ...
                let default_arm = if has_nonzero {
                    ...
                    v1_rt::concat(v1_rt::concat("\n    _ => { let stderr = String::from_utf8_lossy(&output.stderr).to_string(); Err(".to_string(), emit_rust_host_to_dag_string_via_seam("stderr".to_string(), source_indices.clone())), ") },".to_string())
                };
                v1_rt::concat(
                    v1_rt::concat(
                        v1_rt::concat(
...
pub fn emit_exit_arm(
...
) -> String {
    {
        ...
        if is_success {
            ...
            v1_rt::concat(v1_rt::concat(v1_rt::concat(v1_rt::concat("    ".to_string(), pattern), " => { let stderr = String::from_utf8_lossy(&output.stderr).to_string(); Err(".to_string()), emit_rust_host_to_dag_string_via_seam("stderr".to_string(), source_indices.clone())), ") },".to_string())
        }
    }
}
