    ...
    fn method_existence_wall_witness() {
        ...
        let result = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let green_missing: Vec<_> = green_result.diagnostics.iter()
                    .filter(|d| matches!(*d.diagnostic, crate::v1_std_core::CompilerDiagnostic::MethodNotFound { .. }))
                    .collect();
                assert!(
                    green_missing.is_empty(),
                    "legitimate methods must not reach the wall, got: {:?}",
                    green_missing
                );
            })
            .expect("failed to spawn thread")
    ...
    fn declared_type_conformance_witness() {
        ...
        let result = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let green_mismatches: Vec<_> = green_result.diagnostics.iter()
                    .filter(|d| matches!(*d.diagnostic, crate::v1_std_core::CompilerDiagnostic::TypeMismatch { .. }))
                    .collect();
                assert!(
                    green_mismatches.is_empty(),
                    "conforming declarations must not red, got: {:?}",
                    green_mismatches
                );
            })
            .expect("failed to spawn thread")
