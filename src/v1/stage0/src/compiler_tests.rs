...
mod compiler_tests {
    ...
    #[test]
    fn probe_emit_clone_bounds() {
        use crate::cli_run::compile_dag_rust_emit_check;
        use crate::v1_compiler_artifact::RenderTarget;
        use crate::v1_compiler_compile;
        use std::rc::Rc;

        let alpha = "module fn_clone_bound_witness.alpha_phantom\n\ntype Holder<T> {\n  tag: Int\n}\n\nfn inspect<N>(h: Holder<N>) -> String {\n  \"holder\"\n}\n";
        let bounded = "module fn_clone_bound_witness.alpha_bounded_phantom\n\ntype BoundedPhantom<T: Clone> {\n  tag: Int\n}\n\nfn inspect<N>(h: BoundedPhantom<N>) -> String {\n  \"bounded\"\n}\n";

        for (label, src, path) in [
            ("alpha", alpha, "src/fn_clone_bound_witness_alpha_phantom.rs"),
            (
                "bounded",
                bounded,
            ...
            let result =
                v1_compiler_compile::compile_sources(sources, RenderTarget::Rust);
            let hard = result
                .diagnostics
                .iter()
        ...
        let a = compile_dag_rust_emit_check(
            alpha,
            "src/fn_clone_bound_witness_alpha_phantom.rs",
            &["fn inspect<N>".to_string()],
            &["fn inspect<N: Clone>".to_string()],
        );
        let b = compile_dag_rust_emit_check(
            bounded,
            "src/fn_clone_bound_witness_alpha_bounded_phantom.rs",
            &["fn inspect<N: Clone>".to_string()],
            &["fn inspect<N>".to_string()],
        );
        eprintln!("alpha_ok={a} bounded_ok={b}");
        assert!(a && b, "alpha_ok={a} bounded_ok={b}");
    }
}
