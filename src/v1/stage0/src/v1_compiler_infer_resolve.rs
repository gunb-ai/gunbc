...
pub fn lookup_unit_variant_phantom_type(
    ...
    if ((matches.clone().len() as i64) == 1) {
        match matches.clone().first().cloned().as_deref().cloned() {
...
pub fn resolve_field(
    ...
    let type_diags = v1_rt::concat(
        ...
        missing_generic_args_diagnostics(authored_type.clone(), env.clone(), module_name.clone()),
    );
    ...
    let default_diags = match default_resolved.clone().as_deref().cloned() {
    ...
    Rc::new(FieldResolveResult {
        field: make_field_node(
            ...
            match default_resolved.clone().as_deref().cloned() {
...
pub fn resolve_param(
    ...
    let default_expr = match default_resolved.clone().as_deref().cloned() {
        Some(result) => Some(result.expr.clone()),
        None => None,
    };
...
pub fn resolve_match_arm(
    ...
    let guard_result = match arm_g.clone().as_deref().cloned() {
    ...
    let guard_diags = match guard_result.clone().as_deref().cloned() {
        Some(result) => result.diagnostics.clone(),
        None => Rc::new(vec![]),
    };
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*texpr.expr_data.clone()).clone() {
            ...
            ExprData::ExprFieldAccess { summary: _, .. } => {
                let r = match texpr.children.clone().first().cloned().as_deref().cloned() {
                    Some(base) => {
            ...
            ExprData::ExprIf => {
                ...
                let cr = match ch.clone().first().cloned().as_deref().cloned() {
                ...
                let tr = match ch.clone().get(1 as usize).cloned().as_deref().cloned() {
                ...
                let er = match ch.clone().get(2 as usize).cloned().as_deref().cloned() {
                ...
                let resolved_children = match er.clone().as_deref().cloned() {
                ...
                Rc::new(ExprResolveResult {
                    ...
                    diagnostics: v1_rt::concat(
                        ...
                        match er.clone().as_deref().cloned() {
            ...
            ExprData::ExprLet => {
                let ch = texpr.children.clone();
                let vr = match ch.clone().first().cloned().as_deref().cloned() {
                ...
                let br = match ch.clone().get(1 as usize).cloned().as_deref().cloned() {
                ...
                let resolved_children = match br.clone().as_deref().cloned() {
                ...
                Rc::new(ExprResolveResult {
                    ...
                    diagnostics: v1_rt::concat(
                        v1_rt::concat(
                            ...
                            match br.clone().as_deref().cloned() {
                                Some(r) => r.diagnostics.clone(),
                                None => Rc::new(vec![]),
                            },
                        ),
            ...
            ExprData::ExprBinOp {
                ...
                let ch = texpr.children.clone();
                let lr = match ch.clone().first().cloned().as_deref().cloned() {
                ...
                let rr = match ch.clone().get(1 as usize).cloned().as_deref().cloned() {
            ...
            ExprData::ExprCast => {
                let ch = texpr.children.clone();
                let r = match ch.clone().first().cloned().as_deref().cloned() {
                    Some(inner) => {
            ...
            ExprData::ExprIndex => {
                let ch = texpr.children.clone();
                let br = match ch.clone().first().cloned().as_deref().cloned() {
                    Some(base) => {
                        resolve_expr_types(base.clone(), env.clone(), module_name.clone())
                    }
                    None => Rc::new(ExprResolveResult {
                ...
                let ir = match ch.clone().get(1 as usize).cloned().as_deref().cloned() {
                    Some(index) => {
            ...
            ExprData::ExprSlice => {
                ...
                let sr = match ch.clone().get(1 as usize).cloned().as_deref().cloned() {
                    Some(start) => {
