...
pub fn type_ref_generic_use_site_follow_up_note() -> String {
    thread_local! {
        static CACHED: String = {
            "BOUNDED RESIDUAL (not N1 close-now): is_user_generic_use_site skips type_ref_has_binding_authority — known fail-open for Generic<T> pool hits. Authority for owner/lane/trigger/rung: gunbc.type_ref_binding_authority_debt type_ref_generic_use_site_residual_* (owner quiet-hawk-219 namespace migration; lane N2 type-position containment; dissolve-on N2 body swap of type_ref_has_binding_authority; Scaffold type_ref_generic_use_site_residual_scaffold). Mid-PR arm widen forbidden: f531ba548d freeze was leaf-only; closing generics here forces option-(2) full re-census (quiet-hawk-219). reviews 47291/47314/47317: decline apply-now; 47317 alternative (bounded disposition) = this residual. Class-B EXPLICITATION vs PADDING: type_ref_import_add_discriminator_note. PROGRAM FINDING: freeze under-covers narrow contexts — type_ref_debt_universe_openness_note.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}
