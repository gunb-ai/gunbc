...
pub fn type_ref_generic_use_site_follow_up_note() -> String {
    thread_local! {
        static CACHED: String = {
            "DECLARED FOLLOW-UP (not N1): review 47291 correctly named that is_user_generic_use_site skips the binding-authority gate. Closing that arm widens the UnresolvedType population past the f531ba548d freeze census and was stripped from #7622 per quiet-hawk charter (decline mid-PR scope growth). Land as its own tranche: census under the widened arm → execution-decided classification → freeze (or wait for N2 containment binding to close those sites by construction). Do NOT re-admit via PADDING import-adds (N5/N7); Class-B EXPLICITATION (definer import of a referenced symbol) is a different shape — see gunbc.type_ref_binding_authority_debt type_ref_import_add_discriminator_note + type_ref_class_b_explicitation_count. PROGRAM FINDING (905307fb / calm-badger): the freeze census population is RESOLVE-CONTEXT-DEPENDENT — sites fire under narrow discovery that whole-tree resolve hides by pool coincidence — so the frozen roster under-covers narrow contexts by construction; N2's containment binding dissolves that context-dependence (evidence for N2 sequencing; universe openness declared on type_ref_debt_universe_openness_note).".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}
