...
impl<M: Clone> CostAxis<M> {
    pub fn value(&self) -> M {
        match self {
            CostAxis::CostAxisMeasured { value: __val, .. } => __val.clone(),
            CostAxis::CostAxisPredicted { value: __val, .. } => __val.clone(),
            CostAxis::CostAxisUnestablished => panic!("no value on unit variant"),
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum CostTimeAxis<S> {
    CostTimeMeasured { value: Rc<Measure<Time, S, i64>> },
    CostTimePredicted { value: Rc<Measure<Time, S, i64>> },
    CostTimeUnestablished,
}
impl<S: Clone> CostTimeAxis<S> {
    pub fn value(&self) -> Rc<Measure<Time, S, i64>> {
        match self {
            CostTimeAxis::CostTimeMeasured { value: __val, .. } => __val.clone(),
            CostTimeAxis::CostTimePredicted { value: __val, .. } => __val.clone(),
            CostTimeAxis::CostTimeUnestablished => panic!("no value on unit variant"),
        }
    }
}

pub fn cost_time_axis_fork_disposition() -> Rc<Disposition> {
    thread_local! {
        ...
        module_path: "std.realization_schedule".to_string(),
        decl_name: "CostTimeAxis".to_string(),
...
pub struct CostAccount<S> {
    pub time: Rc<CostTimeAxis<S>>,
    pub space: Rc<CostAxis<ByteSize>>,
    pub power: Rc<CostAxis<Watt>>,
    pub _phantom: std::marker::PhantomData<S>,
}

pub fn cost_account_predicted_zero<S>() -> Rc<CostAccount<S>> {
    Rc::new(CostAccount {
        time: Rc::new(CostTimeAxis::CostTimePredicted {
            value: crate::std_measure::time_measure(0),
        }),
        space: Rc::new(CostAxis::CostAxisPredicted {
            value: crate::std_measure::byte_size(0),
        }),
        power: Rc::new(CostAxis::CostAxisPredicted {
            value: crate::std_measure::watt(0),
        }),
...
pub fn cost_account_measured<S>(time: Rc<Measure<Time, S, i64>>) -> Rc<CostAccount<S>> {
    Rc::new(CostAccount {
        time: Rc::new(CostTimeAxis::CostTimeMeasured {
            value: time.clone(),
        }),
        space: Rc::new(CostAxis::CostAxisUnestablished),
        power: Rc::new(CostAxis::CostAxisUnestablished),
        _phantom: std::marker::PhantomData,
    })
}

pub fn cost_time_axis_established<S: Clone>(
    axis: Rc<CostTimeAxis<S>>,
) -> Option<Rc<Measure<Time, S, i64>>> {
    match (*axis.clone()).clone() {
        CostTimeAxis::CostTimeMeasured { value: t, .. } => Some(t.clone()),
        CostTimeAxis::CostTimePredicted { value: t, .. } => Some(t.clone()),
        CostTimeAxis::CostTimeUnestablished => std::option::Option::None,
    }
}

pub fn cost_account_time_count<S: Clone>(account: Rc<CostAccount<S>>) -> Option<Nat> {
    match cost_time_axis_established(account.time.clone()) {
