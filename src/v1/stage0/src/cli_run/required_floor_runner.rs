...
/// The module the producer itself composes for "which `test` decls does this content declare".
/// Named once so the enumeration over the removed sources and the producer's own fold cannot
/// drift onto two spellings of one authority.
const FLOOR_NAMING_HYGIENE_MODULE: &str = "v2.workflow.floor_naming_hygiene";
...
pub fn run_required_floor(
    ...
    //
    // Preparation carries the removed SOURCES and the disposition naming why each was removed;
    // WHAT THEY DECLARE IS ASKED OF THE `.dag` AUTHORITY, HERE, not answered in Rust. The
    // admitted population's roster comes from `v2.workflow.floor_discovery_producer`
    // (`discover_floor_rows_for_source`) and this one from
    // `v2.workflow.floor_naming_hygiene` (`floor_discovery_scan_test_decl_names`) — the same
    // enumeration the producer itself composes, asked at the level that answers the question this
    // population poses. A Rust rescan of these bytes would be the second scanner gunbc#9685 just
    // deleted, arriving through a side door.
    //
    // IT IS THE ENUMERATION AND DELIBERATELY NOT THE PLACEMENT POLICY. The producer's per-file
    // fold also refuses a misplaced `test` decl, a barren sidecar and a misplaced wire contract;
    // those refusals are judgements about hygiene, and applying them to sources the required gate
    // does not cover would widen this lane's subject from "which identities exist" to "is the
    // whole corpus well-placed" — a different change, with a different blast radius, made
    // silently as a side effect of counting. Stated rather than left implicit: the placement
    // rules are enforced over the PREPARED subject only, and this population's hygiene is
    // unjudged. Its next-rung trigger is the gate closure reaching these modules, which is the
    // same trigger the §4b rung drop already carries — not a second obligation.
    //
    // `sites_offered` keeps its old meaning — what the discovery authority offered over the
    // prepared subject — and is reported beside `declared` rather than replaced by it, because a
    // reader comparing runs needs the subject-relative and the tree-relative number to stay
    // distinguishable.
    if !prepared.undiscovered_sources.is_empty() {
        floor_seam("undiscovered-enumeration");
        let undiscovered_started = std::time::Instant::now();
        let hygiene_frame = floor_authority_frame(&prepared, FLOOR_NAMING_HYGIENE_MODULE)?;
        for (disposition, view) in &prepared.undiscovered_sources {
            let value = v1_interpreter::run_in_context_with_args(
                &hygiene_frame,
                "v2.workflow.floor_naming_hygiene.floor_discovery_scan_test_decl_names",
                &[(
                    Some("content".to_string()),
                    str_value(view.source.content.clone()),
                )],
                false,
            )
            .map_err(|e| {
                format!(
                    "REQUIRED-FLOOR REFUSAL cause=FloorDiscoveryAuthorityUnevaluable source={} — \
                     floor_discovery_scan_test_decl_names: {e}",
                    view.source.path
                )
            })?;
            let names = floor_decode_list(&hygiene_frame, Some(&value)).map_err(|why| {
                format!(
                    "REQUIRED-FLOOR REFUSAL cause=FloorDiscoveryAuthorityUnevaluable source={} — \
                     floor_discovery_scan_test_decl_names decode: {why}",
                    view.source.path
                )
            })?;
            for name in names {
                let v1_interpreter::Value::Str(function) = name else {
                    return Err(format!(
                        "REQUIRED-FLOOR REFUSAL cause=FloorDiscoveryAuthorityUnevaluable \
                         source={} — floor_discovery_scan_test_decl_names returned {}, not a \
                         String row",
                        view.source.path,
                        floor_value_shape(Some(name))
                    ));
                };
                let identity = format!("{}.{}", view.module_path, function);
                if !declared_identity_set.insert(identity.clone()) {
                    return Err(format!(
                        "REQUIRED-FLOOR REFUSAL cause=DuplicateWitnessIdentity identity={identity} \
                         — an identity preparation removed was ALSO discovered in the prepared \
                         subject, so it would carry two dispositions; the prepared subject and \
                         its complement must be disjoint"
                    ));
                }
                disposition_rows.push(RequiredFloorDispositionRow {
                    identity,
        ...
        eprintln!(
            "[floor-phase] phase=undiscovered-enumeration state=completed wall_ms={} \
             authority={} sources={} identities={}",
            undiscovered_started.elapsed().as_millis(),
            FLOOR_NAMING_HYGIENE_MODULE,
            prepared.undiscovered_sources.len(),
            declared_identity_set.len() - sites_offered
        );
    }
