...
pub fn diagnostic_to_message(d: Rc<CompilerDiagnostic>) -> String {
    ...
    CompilerDiagnostic::RecordLitInstantiationUndetermined { declared: d, cause: c, .. } => v1_rt::concat(v1_rt::concat(v1_rt::concat("cannot select the field template for this record literal: the position is rendered '".to_string(), d.clone()), v1_rt::concat("'. The fields are NOT judged here, because judging them would mean comparing values against a template that was never established -- so this says nothing about whether the fields are correct. It does NOT clear the literal: a constructor belonging to a different type than the position expects arrives here too, and at a generic position no other check judges it. Cause: ".to_string(), c.clone())), "".to_string()),
...
pub fn diagnostic_disposition(d: Rc<CompilerDiagnostic>) -> Rc<DiagnosticDisposition> {
    ...
    CompilerDiagnostic::RecordLitInstantiationUndetermined { .. } => Rc::new(DiagnosticDisposition {
    severity: DiagnosticSeverity::SeverityError,
    gate: Rc::new(DiagnosticGateDisposition::GateBlocking),
}),
