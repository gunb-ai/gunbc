// PROBE: required-regen must produce the candidate before refusing this drift.
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum AdmitCallersEntry {
    AdmitCallersEntryCoords { coords: Rc<DeclRefCoords> },
    AdmitCallersEntryUninterpretable { span: Rc<SourceSpan> },
}
...
pub fn fn_admit_callers(
    ...
    .cloned()
    {
        Some(p) => Some(admit_callers_entries_from_list(
...
pub fn admit_callers_entry_uninterpretable_spans(
    entries: Rc<Vec<Rc<AdmitCallersEntry>>>,
) -> Rc<Vec<Rc<SourceSpan>>> {
    Rc::new({
        let mut __result = Vec::new();
        for e in entries.iter().cloned() {
            __result.extend(
                (*match (*e.clone()).clone() {
                    AdmitCallersEntry::AdmitCallersEntryCoords { coords: _, .. } => Rc::new(vec![]),
                    AdmitCallersEntry::AdmitCallersEntryUninterpretable { span: s, .. } => {
                        Rc::new(vec![s.clone()])
                    }
                })
                .iter()
...
pub fn admit_callers_entry_coords(
    entries: Rc<Vec<Rc<AdmitCallersEntry>>>,
    ...
    Rc::new({
        ...
        for e in entries.iter().cloned() {
            __result.extend(
                (*match (*e.clone()).clone() {
                    AdmitCallersEntry::AdmitCallersEntryCoords { coords: c, .. } => {
                        Rc::new(vec![c.clone()])
                    }
                    AdmitCallersEntry::AdmitCallersEntryUninterpretable { span: _, .. } => {
                        Rc::new(vec![])
                    }
                })
                .iter()
