...
pub use crate::v1_std_core::{
    ...
    is_kernel_type, kernel_span, leaf_node_with_span, make_error_node, make_expr_error_node,
    make_expr_node, make_param_node, no_span, none_type, param_node_type_expr,
    qualified_last_segment, string_type, type_name_compatible, unit_type, variant_node_fields,
...
#[serde(tag = "_variant")]
pub enum TypeNodeChild {
    TypeArgumentChild { node: Rc<Node> },
    RecordFieldChild { node: Rc<Node> },
    VariantFieldChild { node: Rc<Node> },
}
impl TypeNodeChild {
    pub fn node(&self) -> Rc<Node> {
        match self {
            TypeNodeChild::TypeArgumentChild { node: __val, .. } => __val.clone(),
            TypeNodeChild::RecordFieldChild { node: __val, .. } => __val.clone(),
            TypeNodeChild::VariantFieldChild { node: __val, .. } => __val.clone(),
        }
    }
}

pub fn type_node_children(n: Rc<Node>) -> Rc<Vec<Rc<TypeNodeChild>>> {
    match n.connective.clone() {
        Connective::NoConnective => Rc::new({
            let mut __result = Vec::new();
            for child in n.children.clone().iter().cloned() {
                __result.push(Rc::new(TypeNodeChild::TypeArgumentChild {
                    node: child.clone(),
                }));
            }
            __result
        }),
        Connective::Conj => Rc::new({
            let mut __result = Vec::new();
            for child in n.children.clone().iter().cloned() {
                __result.push(Rc::new(TypeNodeChild::RecordFieldChild {
                    node: child.clone(),
                }));
            }
            __result
        }),
        Connective::Disj => Rc::new({
            let mut __result = Vec::new();
            for variant in n.children.clone().iter().cloned() {
                __result.extend(
                    (*Rc::new({
                        let mut __result = Vec::new();
                        for field in crate::v1_std_core::variant_node_fields(variant.clone())
                            .iter()
                            .cloned()
                        {
                            __result.push(Rc::new(TypeNodeChild::VariantFieldChild {
                                node: field.clone(),
                            }));
                        }
                        __result
        ...
        Connective::Arrow => Rc::new(vec![]),
    }
}

pub fn type_node_child_type(child: Rc<TypeNodeChild>) -> Rc<Node> {
    match (*child.clone()).clone() {
        TypeNodeChild::TypeArgumentChild { node: argument, .. } => {
            type_argument_node(argument.clone())
        }
        TypeNodeChild::RecordFieldChild { node: field, .. } => child_type_node(field.clone()),
        TypeNodeChild::VariantFieldChild { node: field, .. } => child_type_node(field.clone()),
    }
}
                ...
                .clone()
                {
                    ...
                    ContainerSpellingVerdict::ContainerSpellingDeclared {
                        ...
                        if ((n.children.clone().len() as i64) < expected.clone()) {
                            ...
                            type_node_children(n.clone()).iter().cloned().fold(
                                Rc::new(TypeResolutionVerdict::FullyResolved),
                                |acc: Rc<TypeResolutionVerdict>, ch: Rc<TypeNodeChild>| {
                                    combine_resolution_verdicts(
                                        acc,
                                        type_resolution_verdict(
                                            type_node_child_type(ch.clone()),
                                            source_indices.clone(),
                                        ),
                                    )
                                },
