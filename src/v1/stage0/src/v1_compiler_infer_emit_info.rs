...
pub fn empty_emit_graph_info() -> Rc<EmitGraphInfo> {
    Rc::new(EmitGraphInfo {
        type_summaries: panic!("call target identity was not established before Rust emission")(),
        type_decl_items: panic!("call target identity was not established before Rust emission")(),
        fn_decl_items: panic!("call target identity was not established before Rust emission")(),
        ...
        ownership_index: panic!("call target identity was not established before Rust emission")(),
        movable: v1_rt::rc_empty_set::<String>(),
        variant_to_enum: panic!("call target identity was not established before Rust emission")(),
        owned_bindings: v1_rt::rc_empty_set::<String>(),
        read_only_params_index: panic!(
            "call target identity was not established before Rust emission"
        )(),
        read_only_params: v1_rt::rc_empty_set::<String>(),
        clone_bounded_type_params: panic!(
            "call target identity was not established before Rust emission"
        )(),
        map_key_required_type_names: v1_rt::rc_empty_set::<String>(),
        clone_impl_required_type_params: panic!(
            "call target identity was not established before Rust emission"
        )(),
...
pub fn build_field_type_map(
    ...
    children.iter().cloned().fold(
        Rc::new(FieldTypeMapBuild {
            field_types: panic!("call target identity was not established before Rust emission")(),
...
pub fn add_emit_item_summary(
    ...
    TypeRepr::EnumRepr { unit_only: _, .. } => item.children.clone().iter().cloned().fold(state.type_summaries.clone(), |acc: Rc<HashMap<String, Rc<TypeSummary>>>, variant: Rc<Node>| if ((variant.children.clone().len() as i64) > 0) {
                {
...
let vname = crate::v1_std_core::authored_name_at(source_indices.clone(), variant.clone());
let qualified_vname = variant_summary_key(summary.name.clone(), vname.clone());
let variant_field_types = build_field_type_map(variant.children.clone(), source_indices.clone());
v1_rt::rc_map_insert(acc.clone(), qualified_vname.clone(), Rc::new(TypeSummary {
    ...
    field_import_surface_names: variant_field_types.import_surface_names.clone(),
    variant_name_set: panic!("call target identity was not established before Rust emission")(),
