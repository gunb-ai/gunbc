            ...
            for ch in peeled.children.clone().iter().cloned() {
                __result.extend(
                    (*collect_type_node_import_surface_occurrences(
                        if children_are_applied_arguments { ch.clone() } else { crate::v1_compiler_infer_types::child_type_node(ch.clone()) },
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "_variant")]
pub enum DataVariantWireSpelling {
    DataVariantInternalTagged { tag_field: String, tag: String },
    DataVariantBareString { tag: String },
    DataVariantUntagged,
    DataVariantSpellingRefused { reason: String },
}
...
pub fn emit_info_with_fn_type_context(
    ...
    Rc::new(EmitGraphInfo {
        ...
        data_variant_wire_spellings: emit_info.data_variant_wire_spellings.clone(),
