...
///
/// EMPTY BECAUSE ITS WAVE LANDED, WHICH IS THE CONTRACT WORKING RATHER THAN A ROSTER GOING
/// MISSING. It held 53 exact admissions for #9400's owner-qualified call-target cut. Once
/// #9436 merged, every branch with a current merge base carried that cut on BOTH sides, so the
/// admitted deltas no longer occur, so all 53 rows matched nothing and reported
/// `stale_admissions`. The row above already said what to do about that -- "this temporary
/// transition roster must shrink with its subject" -- and this is that shrink.
///
/// Measured before pruning: 53 rows authored, 53 reported stale, 0 unadjudicated deltas. Not a
/// subset, so no row was still carrying a live admission; the numbering runs 01-54 with 19
/// already pruned earlier by the same rule.
///
/// WHAT THIS DOES NOT FIX, AND IT IS THE MORE IMPORTANT HALF. Staleness is computed only inside
/// `WaveAdmissionOutcome::Adjudicated`. On `main` the baseline resolves to the head, the outcome
/// is `NoSubject`, and NO `WaveAdmissionReport` is built -- so a spent roster is structurally
/// invisible on the one branch everyone reads as the health signal, and its cost lands instead
/// on whoever opens the next unrelated PR. That is how these 53 came to block other lanes rather
/// than the wave's own author. The forcing function fires, but on the wrong party, and nothing
/// will surface the NEXT post-wave roster either. Reported as a gap in the instrument; the
/// repair is not attempted here because where that check belongs is a design question, and
/// making a wall fire on `main` is not a change to smuggle in beside an unblocking prune.
///
/// A row that no longer matches is itself a finding (`stale_admissions`), so this roster must
/// keep shrinking with its subject; no disposition-wide predicate is ever admitted.
