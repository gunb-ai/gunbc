...
///
/// TWENTY-FIRST DISSOLUTION (2026-09-03). The 17 `call-reachability grounding gunbc#10156` rows are
/// removed by their own trigger, which fired: #10156 merged, so the base binds every one of those
/// spellings to v2.std.fn_index and no run after it can produce those deltas. Their shared label
/// constant went with them. Run 33786062832 reported each of the 17 as `CONSUMED ADMISSION ...
/// already satisfied at the base — consumed by its own merge; deletion is owed on the roster's next
/// touch`, and the merge that brought them onto this branch is that touch.
///
/// THE SAME MOTION HAPPENED ONE RUN EARLIER WITH 47 gunbc#10106 ROWS, AND THAT REPETITION IS THE
/// POINT. Both sets were unioned in from main under the rule "keep the rows whose transitions are
/// still open"; both were already closed. The rule was right and the INPUT to it was guessed, which
/// is the whole error. A branch that merges main is downstream of main's own sweep, so main's rows
/// have almost always been consumed by the time they arrive here — and consumption is not
/// inspectable by reading the diff, only by running the gate.
///
/// SO THE OPERATIONAL FORM OF THE RULE IS: on a conflict in this roster, keep the rows THIS BRANCH
/// authored and whose transitions are open, take none of the incoming ones on faith, and let the
/// required run name any that are genuinely still open. The discriminator is the RECEIPT, never
/// which branch authored the row and never which side git happened to cut the hunk on.
///
...
/// 33786062832 reported it ADMITTED-BY this row, which is the positive control for everything above:
/// the roster is not merely emptier, it is still adjudicating the one delta this branch produces.
///
