...
// TENTH MERGE, AND THE FIRST WHERE BOTH SIDES CARRIED LARGE LIVE ROSTERS -- 40 SCM rows here, 181
// v2-native-route rows on main, which git split into 40 conflict hunks. Union, with MAIN'S FILE AS
// THE STRUCTURAL BASE so nothing outside the roster can be dropped: only the roster body is
// replaced, by main's rows plus this branch's. `git merge-file --union` was tried first and is
// rejected -- it preserves every row and produces a file that does not PARSE, because it duplicates
// the array declaration at each side's hunk boundary. Completeness of the rows is not the only
// obligation the resolution owes.
