...
/// ONE FURTHER ROW STANDS (gunbc#10218), AND IT IS NOT PART OF THE SUBJECT ABOVE. It adjudicates an
/// occurrence-binding relocation whose transition is open: `physical_asset_identity_eq` was authored
/// privately inside product.printed_chassis.manufacturing_manifest and now resolves in
...
/// stopped being tolerable when product.inventory needed the same comparison: inventory is a generic
/// authority, so importing a specific product's helper to obtain an equality would invert the
/// layering. The spelling is unchanged on both sides and only its TARGET moved, which is exactly
/// TargetChanged.
///
/// IT IS HERE BY UNION AND NOT BY TAKING A SIDE. This roster has now produced a merge conflict on
/// this branch three times, and twice the reflex answer was wrong in the same direction: main's rows
/// were taken whole, and 47 of them were already CONSUMED by their own subject's merge, which the
/// required run then refused. The discriminator is the RECEIPT, never which branch authored the row
/// -- union what is still open, delete what has landed, and read the gate rather than guessing.
///
