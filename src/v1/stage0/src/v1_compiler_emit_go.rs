...
pub fn go_package_name(module_name: String) -> String {
    {
        ...
        match parts.last().cloned() {
            Some(last_part) => Rc::new({
                let mut __result = Vec::new();
                for c in Rc::new(last_part.chars().map(|c| c as i64).collect::<Vec<_>>())
                    .iter()
                    .cloned()
                {
...
pub fn emit_go_struct_field_from_child(child: Rc<Node>, env: Rc<TypeEnv>) -> String {
    {
        ...
        v1_rt::concat(
            v1_rt::concat(
                v1_rt::concat(
                    v1_rt::concat("\t".to_string(), go_export_ident(child_text)),
                    " ".to_string(),
                ),
                ty,
            ),
...
pub fn emit_go_func_def(
...
) -> String {
    {
        ...
        let body_str =
            emit_unified_typed_func_body(body, RenderTarget::Go, registry.clone(), body_scope, 1);
        v1_rt::concat(
            v1_rt::concat(
                v1_rt::concat(
                    v1_rt::concat(
...
pub fn emit_go_shell_call(
...
) -> String {
    {
        ...
        let run_lines = v1_rt::concat(
            v1_rt::concat(
                v1_rt::concat(prefix, "output, err := cmd.Output()\n".to_string()),
...
pub fn go_export_ident(name: String) -> String {
    match (*language_spec(RenderTarget::Go).visibility.clone()).clone() {
        VisibilitySpec::CaseVisibility {
            ...
            let result = apply_naming_case(name, ec);
            if {
                let mut __found = false;
                ...
                v1_rt::concat(result, go_reserved_escape_suffix())
            } else {
                result
            }
        }
