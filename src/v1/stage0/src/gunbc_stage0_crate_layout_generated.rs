...
pub fn generated_stage0_filenames() -> Rc<Vec<String>> {
    thread_local! {
        static CACHED: Rc<Vec<String>> = {
            Rc::new(vec!["v1_interpreter.rs".to_string(), "cli_run.rs".to_string(), "coproduct_reflection.rs".to_string(), "resolved_graph_cache.rs".to_string(), "shared_typecheck_store.rs".to_string(), "recorded_fixture.rs".to_string(), "main.rs".to_string(), "phase_profile.rs".to_string(), "memory_governor.rs".to_string(), "usv_pilot_v2_std_algebra.rs".to_string(), "usv_pilot_v2_std_collection.rs".to_string(), "usv_pilot_v2_std_node.rs".to_string(), "v2_compiler_use_site_verdict.rs".to_string()])
        };
    }
    CACHED.with(|c: &Rc<Vec<String>>| c.clone())
}
