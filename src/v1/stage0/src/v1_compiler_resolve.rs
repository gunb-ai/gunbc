...
pub fn resolve_modules(
    ...
    let topo_diags = match topo_result.cycle_error.clone().as_deref().cloned() {
...
pub fn resolve_import(
    ...
    match target.clone().as_deref().cloned() {
        None => {
        ...
        let batch_result = queue.clone().iter().cloned().fold(
            ...
            |state: Rc<KahnDrainState>, node: String| {
                let state = v1_rt::take_owned(state);
                {
                    ...
                    let neighbors =
                        match v1_rt::map_get(&adjacency, node.clone()).as_deref().cloned() {
                            Some(ns) => ns.clone(),
                            None => Rc::new(vec![]),
                        };
