...
pub fn global_bare_fallback_invariant() -> String {
    thread_local! {
        static CACHED: String = {
            "Corpus-wide bare-name census (namespace-resolution-design.md §8 PR-4): keyed on the bare declared name, one authority on SymbolIndex.global_bare built once by build_symbol_index_census over graph.modules before any module typechecks (order-independent — mirrors v2's symbol_index_global_bare, docs/plans/namespace-resolution-design.md §7.5). Tracking is decl-only via symbol_index_insert_decl / local_binding_for_item (type/fn/data names) PLUS corpus-globally-unique Disj variant aliases (module-local unique AND corpus-wide unique — ambiguous homonyms stay qualified-path only in SymbolIndex.entries, fail-closed on bare lookup). lookup_binding_by_name consults it only after str_bindings/ancestry_str_bindings/intern+bindings all miss, and only a GlobalBareUniqueBinding resolves; GlobalBareAmbiguousBinding stays Absent (fail-closed, never guesses — §5). This is what unblocks the import strip: a bare reference that used to resolve via a deleted import chain still resolves here iff its name is globally unique in the corpus.".to_string()
        };
    }
