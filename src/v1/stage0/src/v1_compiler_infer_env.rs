...
pub fn global_bare_chain_candidates(
    env_module_path: String,
    candidates: Arc<Vec<Arc<GlobalBareCandidate>>>,
) -> Arc<Vec<Arc<GlobalBareCandidate>>> {
    {
        let env_segs = module_path_segments(env_module_path.clone());
        Arc::new({
...
pub fn global_bare_unique_chain_candidate(
...
) -> Option<Arc<GlobalBareCandidate>> {
    {
        ...
        match (*module_path_owner_binding_decide(Arc::new({
            let mut __result = Vec::new();
        ...
        .clone()
        {
            ModulePathBindingProjection::ModulePathBindingHit { owner: owner, .. } => Arc::new({
                let mut __result = Vec::new();
                for c in chain.clone().iter().cloned() {
...
pub fn type_ref_measure_binding_authority(env: Arc<TypeEnv>, name: String) -> bool {
