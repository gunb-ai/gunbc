...
pub fn decl_file_carries_three_states_note() -> String {
    thread_local! {
        static CACHED: String = {
            "THE IDENTITY KEY HAS THREE INHABITANTS AND ITS TYPE EXPRESSES TWO. decl_identity_file projects a declaration's ident_span FILE, and a kernel-minted node's span file is the synthetic string <kernel:NAME> (v1.std.core kernel_span). So a decl_file argument is one of THREE things -- a real declaring module path, the synthetic kernel identity, or the empty string meaning unknown -- while the gates that consume it discriminate only empty from non-empty. TWO SEPARATE FACTS, and conflating them understates the first: (1) THE MISREAD IS LIVE IN PRODUCTION. lookup_checkpoint and type_realization_decision both branch on decl_file == \"\" and treat every non-empty value as a KNOWN declaration, so <kernel:String> proceeds to the spelling-keyed arm as though a module had been named. Those gates are reached with exactly that value through type_reference_decl_file, which projects the resolved node's identity: v1.compiler.emit coerce_primitive_type call sites and v1.compiler.emit_rust rust_named_type_base and rust_applied_type_base all pass it. decl_file_declares_structurally then compares the synthetic identity against structural_declaration_modules_for with contains, which no <kernel:NAME> can ever match, so the structural gate is unreachable for a kernel-resolved reference BY CONSTRUCTION rather than by decision. (2) THE DISAGREEMENT IS OBSERVABLE ONLY IN AN INSTRUMENT, because nothing in production compares the two identity sources: type_reference_decl_file reads the INFERENCE side alone, and v1.tests.claim.carrier_realization_census is the only reader that also asks the type environment -- where the two answer <kernel:std.algebra.FreeMonoid> and dag/std/algebra.dag for one reference. Do not read (2) as bounding (1) to an instrument. THE THIRD STATE IS ALREADY LOAD-BEARING AND IS HANDLED AD HOC, which is the strongest evidence it deserves a constructor: numeric_realization_declaring_modules carries the literal \"<kernel:\" as a row in a roster of MODULE PATHS, matched by contains, and is_kernel_minted_file is a substring predicate over the same spelling -- while v1.compiler.env resolved_node_is_kernel_identity_for_name owns that fact by EXACT EQUALITY and its own note (gunbc#9850) says a substring or prefix test is a second, weaker authority for it. NO WRONG ANSWER IS CLAIMED AND NONE WAS MEASURED: for the kernel names reaching these gates today the spelling-keyed answer is the correct one, which is exactly why the conflation is invisible. It is filed so it ranks, not because an output is wrong. Measured on a fixture module importing v2.std.text String and v2.std.node Node, read through the census named above: the bare String reference carries <kernel:String> and realizes as the host spelling, against a positive control in the same fixture where a v2.std.node Node reference -- bare and qualified alike -- carries src/v2/std/node.dag. The instrument is named rather than its rows transcribed (DESIGN section 6). RUNG: mitigatable -- the invalid state is writable and nothing refuses it. CEILING: structurally impossible, because the distinction is decidable and already has an authority. NEXT-RUNG TRIGGER, naming the CAPABILITY rather than an artifact: the identity key becomes a typed carrier whose kernel-identity inhabitant is a distinct constructor from a declaring module, so no consumer can read a synthetic identity as a declaration without matching on it -- at which point this note dissolves, the \"<kernel:\" roster row deletes, and is_kernel_minted_file's substring test is replaced by the match.".to_string()
        };
    }
