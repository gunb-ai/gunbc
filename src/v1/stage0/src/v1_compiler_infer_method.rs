...
pub fn builtin_container_return_under_resolution_note() -> String {
    thread_local! {
        static CACHED: String = {
            "Measured frontier (NOT fixed in this PR): map_insert/map_merge/with, set_insert/set_union, list_push return type-variable Map/List/Set containers (builtin_function_registry seeds map_of_type_variables / list_of_type_variable) that inference does not concretize from receiver actuals — fan_in under-resolution main carries today. TYPE_REF_POOL_COINCIDENCE_MASK default OFF hides pool-coincidence type-ref refusal during whole-tree compile; mask ON additionally surfaces this class on container-receiver paths. Horizontal fix (receiver_self_container_concrete_return in infer) was analyzed but never committed to git — zero history for that symbol. Dissolution: builtin_container_return_under_resolution_dissolution_trigger.".to_string()
        };
    }
    CACHED.with(|c: &String| c.clone())
}

pub fn builtin_container_return_under_resolution_dissolution_trigger() -> Rc<Disposition> {
    thread_local! {
        ...
        module_path: "v1.compiler.infer_method".to_string(),
        decl_name: "builtin_function_registry".to_string(),
...
pub fn builtin_function_registry() -> Rc<HashMap<String, Rc<Node>>> {
    {
        ...
        let m = v1_rt::rc_map_insert(
            m.clone(),
            "type_ref_pool_coincidence_mask_is_enabled".to_string(),
            bool_type(),
        );
        let m = v1_rt::rc_map_insert(
            m.clone(),
        ...
        let m = v1_rt::rc_map_insert(
            m.clone(),
            "compile_dag_dag_fixture_witness_check".to_string(),
            bool_type(),
        );
        let m = v1_rt::rc_map_insert(
            m.clone(),
            "hand_import_gate_passes".to_string(),
            bool_type(),
        );
        let m = v1_rt::rc_map_insert(
            m.clone(),
