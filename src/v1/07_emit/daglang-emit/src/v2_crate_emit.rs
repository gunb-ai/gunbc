...
pub fn assemble_v2_crate(modules: &[(&str, &SourceFile)]) -> Vec<GeneratedFile> {
    ...
    let global_data_map_names: HashSet<String> = modules
        ...
        .collect();
    let global_data_string_list_names: HashSet<String> = modules
        .iter()
        .flat_map(|(_, sf)| {
            sf.items.iter().filter_map(|item| match &item.node {
                Item::DataDef(dd) => {
                    if matches!(&dd.ty, daglang_syntax::ast::TypeExpr::Generic(name, args) if name == "List" && args.len() == 1 && matches!(&args[0], daglang_syntax::ast::TypeExpr::Named(n) if n == "String"))
                    {
                        Some(dd.name.clone())
                    } else {
                        None
...
fn emit_module(
    ...
    // Collect data names that are List<String> (emitted as &[&str], need Rc wrapping)
    let mut data_string_list_names: HashSet<String> = global_data_string_list_names.clone();
    data_string_list_names.extend(items.iter().filter_map(|item| match &item.node {
        Item::DataDef(dd) => {
            if matches!(&dd.ty, daglang_syntax::ast::TypeExpr::Generic(name, args) if name == "List" && args.len() == 1 && matches!(&args[0], daglang_syntax::ast::TypeExpr::Named(n) if n == "String")) {
                Some(dd.name.clone())
            } else {
