...
pub fn testgen_target(args: TokenStream, input: TokenStream) -> TokenStream {
        ...
        match arg {
                ...
                match ident.as_deref() {
                    ...
                    Some("output") => {
                        if let Lit::Str(s) = nv.lit {
                            ...
                            return syn::Error::new_spanned(nv, "output must be a string literal")
                                .to_compile_error()
                                .into();
                        }
                    }
                    Some("module") => {
                        if let Lit::Str(s) = nv.lit {
                            ...
                            return syn::Error::new_spanned(nv, "module must be a string literal")
                                .to_compile_error()
                                .into();
                        }
                    }
                    ...
                    Some("class") => {
                        if let Lit::Str(s) = nv.lit {
                            ...
                            return syn::Error::new_spanned(nv, "class must be a string literal")
                                .to_compile_error()
                                .into();
                        }
                    }
                    Some("fermi") => {
                        if let Lit::Str(s) = nv.lit {
                            ...
                            return syn::Error::new_spanned(nv, "fermi must be a string literal")
                                .to_compile_error()
                                .into();
                        }
                    }
                    Some("live_class") => {
                        if let Lit::Str(s) = nv.lit {
                            live_test_class = Some(s);
                        } else {
                            return syn::Error::new_spanned(
                                nv,
                                "live_class must be a string literal",
                            )
                            .to_compile_error()
                            .into();
                        }
                    ...
                    Some("live_fermi") => {
                        if let Lit::Str(s) = nv.lit {
                            live_fermi_cost = Some(s);
                        } else {
                            return syn::Error::new_spanned(
                                nv,
                                "live_fermi must be a string literal",
                            )
                            .to_compile_error()
                            .into();
                        }
                    ...
                    Some("tool") => {
                        if let Lit::Str(s) = nv.lit {
                            ...
                            return syn::Error::new_spanned(nv, "tool must be a string literal")
                                .to_compile_error()
                                .into();
                        }
                    }
                    _ => {
                        return syn::Error::new_spanned(nv, "unknown testgen_target argument")
                            .to_compile_error()
                            .into();
                    }
                }
            ...
            NestedMeta::Meta(Meta::List(list)) => {
                ...
                match ident.as_deref() {
                    ...
                    Some("signature") => match syn::parse2::<Expr>(list.nested.to_token_stream()) {
                        Ok(expr) => signature = Some(expr),
                        Err(e) => return e.to_compile_error().into(),
                    },
                    Some("requires") => {
                    ...
                    _ => {
                        return syn::Error::new_spanned(
                            list,
                            "unknown testgen_target list argument",
                        )
                        .to_compile_error()
                        .into();
                    }
            ...
            _ => {
                return syn::Error::new_spanned(arg, "unsupported testgen_target argument")
                    .to_compile_error()
                    .into();
            }
        }
    ...
    let output = match output {
        ...
        None => {
            return syn::Error::new_spanned(
                &input_fn.sig.ident,
                "testgen_target requires output = \"...\"",
            )
            .to_compile_error()
            .into()
        }
    ...
    let module = match module {
        ...
        None => {
            return syn::Error::new_spanned(
                &input_fn.sig.ident,
                "testgen_target requires module = \"...\"",
            )
            .to_compile_error()
            .into()
        }
    ...
    let builder = match builder {
        ...
        None => {
            return syn::Error::new_spanned(
                &input_fn.sig.ident,
                "testgen_target requires builder(...) or builder = \"...\"",
            )
            .to_compile_error()
            .into()
        }
    ...
    let fermi_tokens = if let Some(cost) = fermi_cost {
        match cost.value().to_uppercase().as_str() {
            ...
            _ => {
                return syn::Error::new_spanned(cost, "fermi must be one of: XS, S, M, L, XL")
                    .to_compile_error()
                    .into();
            }
        }
    ...
    let live_fermi_tokens = if let Some(cost) = live_fermi_cost {
        match cost.value().to_uppercase().as_str() {
            ...
            "XL" => quote!(Some(gunbc_test::FermiCost::XL)),
            _ => {
                return syn::Error::new_spanned(cost, "live_fermi must be one of: XS, S, M, L, XL")
                    .to_compile_error()
                    .into();
            }
        }
            ...
            NestedMeta::Meta(Meta::NameValue(nv)) => {
                ...
                match ident.as_deref() {
                    ...
                    _ => {
                        return syn::Error::new_spanned(
                            nv,
                            "unknown resource_test_target argument",
                        )
                        .to_compile_error()
                        .into();
                    }
            ...
            NestedMeta::Meta(Meta::List(list)) => {
                ...
                match ident.as_deref() {
                    ...
                    _ => {
                        return syn::Error::new_spanned(
                            list,
                            "unknown resource_test_target list argument",
                        )
                        .to_compile_error()
                        .into();
                    }
                ...
                if let Some(ident) = path.get_ident() {
                    match ident.to_string().as_str() {
                        ...
                        _ => {
                            return syn::Error::new_spanned(
                                path,
                                "unknown resource_test_target flag",
                            )
                            .to_compile_error()
                            .into();
                        }
