...
//! 2. If stale, build a freshness sub-DAG (codegen → testgen → clippy → test)
...
/// sequential execution: codegen-dag → testgen → clippy → test-compile.
///
...
mod tests {
    ...
    fn no_overlap_for_generation_steps() {
        ...
        let gen_steps = vec![step("codegen"), step("testgen"), step("bootstrap")];
