...
/// Returns true when the selected success-port value is not explicitly
/// `Bool(true)`.
///
/// Generated tool DAGs emit the user-facing result from the root
/// `module::func` node, but internal helper nodes may also reuse generic
/// field names like `success` (for example transport parse nodes using
/// `SuccessStdoutStderr`). Prefer the last root-style match and fall back to
/// the last generic match only when no root node emitted the port.
fn success_port_failed(log: &crate::ExecutionLog, success_port: Option<&str>) -> bool {
    ...
    for entry in log.entries.iter().rev() {
        let Some(value) = entry.outputs.get(port) else {
            continue;
        };

        if entry.node_id.contains("::") {
...
mod tests {
    ...
    #[test]
    fn success_port_failed_prefers_root_node_over_internal_success_outputs() {
        let log = crate::ExecutionLog {
            entries: vec![
                crate::LogEntry {
                    node_id: "parse_transport_extdeps_gunbc_shell_Codegen_Run".to_string(),
                    inputs: None,
                    outputs: HashMap::from([("success".to_string(), Value::Bool(false))]),
                    was_intercepted: false,
                    coercions_applied: vec![],
                },
                crate::LogEntry {
                    node_id: "tools.codegen::codegen".to_string(),
                    inputs: None,
                    outputs: HashMap::from([("success".to_string(), Value::Bool(true))]),
                    was_intercepted: false,
                    coercions_applied: vec![],
                },
            ],
        ...
        assert!(!success_port_failed(&log, Some("success")));
    }

    #[test]
    fn success_port_failed_uses_last_non_root_match_as_fallback() {
        let log = crate::ExecutionLog {
            entries: vec![
                crate::LogEntry {
                    node_id: "prepare_transport_extdeps_gunbc_shell_Codegen_Run".to_string(),
                    inputs: None,
                    outputs: HashMap::from([("success".to_string(), Value::Bool(true))]),
                    was_intercepted: false,
                    coercions_applied: vec![],
