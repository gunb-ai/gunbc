...
pub fn parse(argv: &[String], schema: &[CliParam]) -> Result<ParseResult, ParseError> {
    ...
    // Build port-name lookup for positional KEY=VALUE parsing (infra-style tools).
    // Keys are UPPER_SNAKE, lower_snake, and original port_name for flexible matching.
    let mut port_name_map: HashMap<String, usize> = HashMap::new();
    for (idx, param) in schema.iter().enumerate() {
        ...
        // Positional KEY=VALUE: match on port_name, UPPER(port_name), and kebab-case
        port_name_map.insert(param.port_name.clone(), idx);
        port_name_map.insert(param.port_name.to_uppercase(), idx);
        port_name_map.insert(param.flag_name(), idx);
        port_name_map.insert(param.flag_name().to_uppercase(), idx);
    }
        ...
        match arg.as_str() {
            ...
            _ => {
                ...
                if let Some(&idx) = flag_map.get(flag) {
                    ...
                    // Positional KEY=VALUE arg (infra-style): match key to port name.
                    if let Some(&idx) = port_name_map.get(key) {
                        ...
                        if param.type_id == ParamType::Bool {
                            ...
                            if let Some((inner_k, inner_v)) = val.split_once('=') {
                                if !inner_k.trim().is_empty() {
                                    maps.get_mut(&idx)
                                        .unwrap()
                                        .insert(inner_k.to_string(), inner_v.to_string());
                                }
                            }
                        } else if param.is_repeatable() {
                            lists.get_mut(&idx).unwrap().push(val.to_string());
                        } else {
                            scalars.insert(idx, Some(val.to_string()));
                        }
                    }
                    // Unrecognized positional KEY=VALUE args are silently ignored
                    // to allow forward-compatibility with new params.
                }
            }
        }
        i += 1;
...
mod tests {
    ...
    // ========================================================================
    // Positional KEY=VALUE parsing (infra-style tools, CP-61)
    // ========================================================================

    #[test]
    fn test_positional_key_value_string_param() {
        let schema = vec![CliParam::new("environment", ParamType::Str)];
        let result = parse(&argv(&["prog", "ENVIRONMENT=prod"]), &schema).unwrap();
        assert_eq!(
            result.values["environment"],
            Value::Str("prod".into()),
            "UPPER_SNAKE positional KEY=VALUE should match port_name"
        );
    }

    #[test]
    fn test_positional_key_value_snake_case() {
        let schema = vec![CliParam::new("repo_path", ParamType::Str)];
        let result = parse(&argv(&["prog", "repo_path=my-repo"]), &schema).unwrap();
        assert_eq!(
            result.values["repo_path"],
            Value::Str("my-repo".into()),
            "snake_case positional KEY=VALUE should match port_name"
        );
    }

    #[test]
    fn test_positional_key_value_bool_param() {
        let schema = vec![CliParam::new("execute", ParamType::Bool)];
        let result = parse(&argv(&["prog", "EXECUTE=true"]), &schema).unwrap();
        assert_eq!(
    ...
    #[test]
    fn test_positional_key_value_int_param() {
        let schema = vec![CliParam::new("count", ParamType::Int).default("0")];
        let result = parse(&argv(&["prog", "COUNT=42"]), &schema).unwrap();
        assert_eq!(
            result.values["count"],
            Value::Int(42),
            "positional KEY=VALUE should set Int param"
        );
    }

    #[test]
    fn test_positional_key_value_mixed_with_flags() {
        let schema = vec![
            CliParam::new("environment", ParamType::Str),
            CliParam::new("runtime", ParamType::Str).default("docker"),
        ];
        let result = parse(
            &argv(&["prog", "ENVIRONMENT=prod", "--runtime", "k8s"]),
            &schema,
        )
        .unwrap();
        assert_eq!(
            result.values["environment"],
            Value::Str("prod".into()),
            "positional KEY=VALUE and flag args should coexist"
        );
        assert_eq!(
            result.values["runtime"],
            Value::Str("k8s".into()),
            "flag should override default when both styles are used"
        );
    }

    #[test]
    fn test_positional_key_value_unknown_key_ignored() {
        let schema = vec![CliParam::new("environment", ParamType::Str)];
        let result = parse(&argv(&["prog", "UNKNOWN_KEY=value"]), &schema).unwrap();
        assert!(
            !result.values.contains_key("UNKNOWN_KEY"),
            "unrecognized positional KEY=VALUE should be silently ignored"
        );
    }

    #[test]
    fn test_positional_key_value_multiple_params() {
        let schema = vec![
            CliParam::new("environment", ParamType::Str),
            CliParam::new("runtime", ParamType::Str),
            CliParam::new("execute", ParamType::Bool),
        ];
        let result = parse(
            &argv(&["prog", "ENVIRONMENT=staging", "RUNTIME=k8s", "EXECUTE=true"]),
            &schema,
        )
        .unwrap();
        assert_eq!(result.values["environment"], Value::Str("staging".into()));
        assert_eq!(result.values["runtime"], Value::Str("k8s".into()));
        assert_eq!(result.values["execute"], Value::Bool(true));
    }

    #[test]
    fn test_positional_key_value_with_dry_run() {
        let schema = vec![CliParam::new("environment", ParamType::Str)];
        let result = parse(&argv(&["prog", "-n", "ENVIRONMENT=prod"]), &schema).unwrap();
        assert!(result.dry_run);
        assert_eq!(result.values["environment"], Value::Str("prod".into()));
    }
}
