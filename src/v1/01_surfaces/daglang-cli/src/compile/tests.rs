...
#[test]
fn check_target_file_gunbc_auth_patterns_requires_remaining_provider_bindings() {
    let root = unique_temp_root("gunbc_auth_patterns_remaining_provider_bindings");
    let write_source = |relative: &str, content: &str| {
        let path = root.join(relative);
        std::fs::create_dir_all(path.parent().expect("fixture file should have parent"))
            .expect("failed to create fixture parent directory");
...
"#,
    ...
    write_source(
        "extdeps/cloud/gcp/gcp.dag",
        r#"module extdeps.cloud.gcp.gcp
service shell.OAuth2 {
  operation RefreshToken(refresh_token: String) -> { access_token: String }
}
service shell.GCloud {
  operation AuthPrintAccessToken() -> { access_token: String }
}
"#,
    );
    write_source(
        "extdeps/shell.dag",
        r#"module extdeps.shell
service shell.Env {
...
func chain(access_token: String, project_id: String) -> { secret: String } {
  impersonated = gcp.IAM.GenerateAccessToken(access_token: access_token, target_sa: "sa@example.com")
  secret = gcp.SecretManager.AccessVersion(access_token: impersonated.access_token, project_id: project_id, secret: "api-key")
  return { secret: secret.payload }
}

func local() -> { token: String } {
  env_url = shell.Env.Get(name: "CREDENTIALS_URL")
  refreshed = shell.OAuth2.RefreshToken(refresh_token: env_url.value)
  fallback = shell.GCloud.AuthPrintAccessToken()
...
"#,
    ...
    let output = check_from_context(&context)
        .expect("check should succeed with explicit provider bindings");
    assert_eq!(
        output.parsed_files, 5,
        "expected target file plus four imported service modules"
    );
...
"#,
    ...
    let error =
        check_from_context(&context).expect_err("check should fail without provider bindings");
    assert_typecheck_stage_error(&error);
    assert!(error.contains("unresolved service call"));
    for service_call in [
