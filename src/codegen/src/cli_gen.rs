...
fn build_cli_imports(tool: &ToolMeta, custom_import: Option<&str>, step_mode: bool) -> Vec<Item> {
    ...
    let tool_import = match custom_import {
        ...
        _ if tool.graph_builder_call.contains("::") || tool.graph_builder_call.contains('(') => {
            None
        }
        _ => Some(format!("use {}::build_{}_graph;", crate_module, tool.tool_name)),
    };
    // Raw because: tool imports vary per binary and can't be expressed as a fixed Use node.
    if let Some(tool_import) = tool_import {
...
///
/// `graph_builder_args` is treated as the complete argument list.
fn generate_graph_builder_call(tool: &ToolMeta, _has_profiles: bool) -> String {
    let f = &tool.graph_builder_call;
    let args = tool.graph_builder_args.to_string();
...
fn generate_arg_parsing_with_mode(entrypoints: &[CliEntrypoint], enable_mode: bool) -> String {
...
fn build_subcmd_run_fn(tool: &ToolMeta, subcmd: &crate::registry::SubcommandDef) -> FnDef {
    ...
    let args = subcmd.graph_builder_args.to_string();
...
mod tests {
    ...
    fn test_generate_step_mode_cli() {
        let tool = ToolMeta {
            ...
            graph_builder_args: "\"tools/build.dag\", gunbc_resolve::BuildOpts { entry_func: Some(\"build_all\"), profile: None }".into(),
            returns_result: true,
    ...
    fn test_generate_cli_with_result_builder() {
        ...
        assert!(
            !code.contains("use gunbc_build_all::build_build-all_graph;"),
            "fully-qualified DSL graph builders should not inject crate-local imports"
        );
    }
    ...
    fn test_generate_cli_with_mode_flag() {
        let tool = ToolMeta {
            ...
            graph_builder_call: DSL_GRAPH_BUILDER_ADAPTER.into(),
            graph_builder_args: "\"tools/deps.dag\", gunbc_resolve::BuildOpts { entry_func: Some(\"deps\"), profile: None }".into(),
            returns_result: false,
            success_port: None,
            enable_step_mode: false,
            mock_spec_call: Some("mock()".into()),
