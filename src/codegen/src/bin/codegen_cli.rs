...
//! # Architecture Note
//!
//! This binary lives in core/codegen and discovers tools via structural
...
fn codegen_bin_dir() -> PathBuf {
    workspace_layout_or_none()
        ...
        .unwrap_or_else(|| {
            // DSL-derived relative path -- no hardcoded fallback.
            // WorkspaceLayout loads from dsl/config/codegen_paths.dag;
            // this branch only fires when cargo metadata is unavailable.
            PathBuf::from(gunbc_ir::workspace_layout::codegen_paths_rel().bin_dir)
        })
}
...
fn codegen_lib_dir() -> PathBuf {
    workspace_layout_or_none()
        ...
        .unwrap_or_else(|| {
            // DSL-derived relative path -- no hardcoded fallback.
            PathBuf::from(gunbc_ir::workspace_layout::codegen_paths_rel().lib_dir)
        })
}
