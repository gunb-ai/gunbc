...
- **IRT-4 (result caching — this task + T-24).** A TestClaim's result is cached keyed by the `content_hash` of its `input` subgraph; unaffected TestClaims' results are REUSED, not recomputed — the concrete form of "caching is the dual of the affected set" (modeling decision above).
