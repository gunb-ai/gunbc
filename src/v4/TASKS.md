...
**Phase-1.5 scaffolding — forward dissolution (INVARIANTS P2 / illegal states unrepresentable)**:
- **`test/claim/manual/t19_manual_anchor_manifest.dag` — `Int` enum surrogates:** `ManualTestgenAnchor` uses `Int` for `tier` / `layer` / `testgen_category` / `assert_kind` only because M1(2.8) still treats many shared `data` record bodies as opaque when lowered in isolation; the manifest header maps codes to the closed classifiers in **`src/v4/std/verification.dag`** (`TestgenTier`, `TestgenLayer`, **`AssertKind`**) and the five scheduling arms named in the **`lens/testgen.dag`** header (`testgen_category` axis — not dsl/v3 Assert* / `TestPredicate` surfaces). Row (12) `behavior_value` is **not** connective-only: it forward-binds to the **unit** **`BehaviorValueSubject`** arm inside **`TypeConstructionSubject`** (semantically **`std/node.dag` `Behavior::Value`** only — no `behavior: Behavior` field that could admit other L1 variants; P2). Same `testgen_category=0` as connective anchors — internal sum, not a sixth top-level category. **Retirement trigger (same PR band as the unblock, not optional polish):** once (a) M2 loads `v4.std.*` peers in one compilation unit with `lens/testgen.dag`, and (b) record + sum literals in `data` bodies validate together without the opaque-body trap, replace those four fields with `TestgenTier`, `TestgenLayer`, the closed five-fold testgen discriminant (sum or enum ratified with `testgen.dag`), and `AssertKind` — **do not** extend the integer coding as the long-term surface. **`slug: String` → `Symbol` name-refs** in the same band per the manifest header (no `Sym` mint at authoring time today). **Post-enum hygiene (optional, non-blocking):** the twelve `data` rows are near-duplicates (slug + code tuple); after Int→enum lands, consider one `Set<ManualTestgenAnchor>`-valued fact or a projection from the ratified testgen-category sum so rows cannot drift independently.
- **`lens/testgen.dag` — comment-only `TestgenConcept` / subject carriers / five `fn … -> Set<TestClaim>` folds:** intentional until M2 cross-file module loading lands; the in-file Practice-4 dissolution ledger is binding at uncomment time. **Loader acceptance:** the first change set that satisfies M2 cross-file loading for this module **must** materialize those declarations as live `.dag` (uncomment or equivalent lowering in the same sequencing step) — comment-as-substrate is a **gate**, not permission to drift if the loader slips.
