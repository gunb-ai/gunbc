...
- **Optional carriers (P1 nominal split — Practices 2 & 8):** **`ReactOptRef`** = optional **ref-handle** slots (`element.ref`, hook ref operands). **`ReactOptKey`** = optional **`key`** on createElement-returned element objects (not ref semantics). **`ReactOptValue`** = optional **non-ref** hook operands (e.g. `useDeferredValue` `initialValue`, `useActionState` `permalink`, lazy `init`, optional `getServerSnapshot`, `useDebugValue` `format`, `useOptimistic` `reducer`). Each is its own sum with distinct variant/field names over **`ReactCrossDeclRef`** edges so unrelated react.dev facts are not compressed into one “optional ref” placeholder (`src/v4/DECISIONS.md` §T-4.7 `react-p4-opt-key` / `react-p4-opt-value` / `react-p4-optref`).
- **ReactNode (Practice‑4 🟡):** T‑4.7 **partial** `createElement` **children** slice in `react.dag` (`Element` | `Text` only). Full react.dev / JSX child lattice (arrays, portals, conditional `null`, …) is **Owned ELSEWHERE** — promotion / dissolution discipline is recorded here, in `docs/design-r4-phase-1-5-hookkind-custom-react-substrate-canvas.md`, and in **`src/v4/DECISIONS.md` §T-4.7** (`react-p4-node` + siblings — Practice 9: no multi-line Practice prose in `react.dag`; in-file **`// 🟢|🟡 coproduct dissolution — DECISIONS.md#…`** one-liners per T‑4 operator directive / **`docs/modeling-discipline.md` Practice 9** (emoji tags + `DECISIONS.md` cites). **Dissolution** = operator-ratified PR that extends or retires this sum — not silent “terminal” closure.
