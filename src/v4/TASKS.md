...
```
  ...
  T-21  lens/affected_set.dag — **incremental re-test selection** over
                T-4 conformance claims (IRT-2; see T-21 detail). Low-
                dependency (needs only T-1, T-2, T-3) but a hard T-4
                prerequisite for the per-language conformance deliverable.
...
**T-4 conformance consumer (operator-ratified 2026-05-25):** T-21 owns **incremental re-test selection** (IRT-2) over T-4's `LanguageBehaviorEquivalence` corpus — `test_claim_evaluation_touches_rerun_frontier` and `ci_select_from_affected_set` select which conformance claims re-run on change. T-21 does not emit claims; T-19 owns emission (see T-19 detail).
