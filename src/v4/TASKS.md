...
- **`t19_manual_anchor_manifest.dag` — `Int` surrogates:** M1(2.8) lowers many shared `data` bodies opaque in isolation, so `tier` / `layer` / `testgen_category` / `assert_kind` are `Int` today. **Row set = bijection to live `TestClaim` values in `connective_anchors.dag` ∪ `nat_law_anchors.dag` only** (no parallel forward grid). Header maps codes → **`src/v4/std/verification.dag`** (`TestgenTier`, `TestgenLayer`, `AssertKind`) + five arms in **`lens/testgen.dag`**; not dsl/v3 Assert* or v3 `TestPredicate`.
