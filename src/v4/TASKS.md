...
**Modeling decisions**:
- Per-language primitive **grounding** (per DECISIONS.md D2): alias-identity to the `std/` carrier (`type RustI32 = Int32`; `std::vector<T>` alias-identifies to `List<T>`), a grounding map (surface spelling), and operation-semantics carriers — NOT a re-declared algebra inhabitance (INVARIANTS P1:42)
