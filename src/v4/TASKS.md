...
2. **ParseTree carries a co-structural locus tree.** `ParseTree = Node` must be replaced by a type that carries locus alongside the node tree. Three alternatives are P2-invalid and explicitly rejected:
   ...
   **Preferred form** (P2 single authority, `Node` pivot A1 preserved, typed association):
   ...
   fn fold_parse_tree<R>(
     ...
     loci: LocusTree,
     init: R,
     f: fn(Option<Locus>, NodeKind, R, List<R>) -> R
   ) -> R
   ```
...
3. **Pipeline threading.** `03_normalize.dag` / `03_resolve.dag` / `04_infer.dag` must carry `(Node, LocusTree)` pairs through each stage using `fold_parse_tree` (step 2). The inferred carrier is `InferredFacts.locus: Option<Locus>` — **not** a mandatory `Locus` — because synthesized nodes have `locus: None` in the co-tree; mandatory `Locus` would force workers to fabricate a locus or silently drop a declared fact, both fail-open (INVARIANTS.md P3). `InferredFacts.locus: Some(Textual { ... })` for source-backed nodes; `None` for synthesized nodes. Diagnostic consumers that require a `Locus` must pattern-match on `Some` / `None` and emit a dedicated diagnostic for the `None` case rather than defaulting. Normalize/resolve transforms that produce a new `Node` from an existing one propagate the origin's `LocusTree` entry; synthesized nodes with no parse origin receive `LocusTree { locus: None, children: [...] }` (`locus: None` is the correct representation of "no source location" — `Locus` has no `Unavailable` variant; `Unavailable` is a `Correction` variant in `std/diagnostic.dag`).
