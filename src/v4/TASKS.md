...
- **ReactNode (Practice‑4 🟡):** T‑4.7 **partial** `createElement` **children** slice in `react.dag` (`Element` | `Text` only). Full react.dev / JSX child lattice (arrays, portals, conditional `null`, …) is **Owned ELSEWHERE** — promotion / dissolution discipline is recorded here, in `docs/design-r4-phase-1-5-hookkind-custom-react-substrate-canvas.md`, and in **`src/v4/DECISIONS.md` §T-4.7** (`react-p4-node` + siblings — Practice 9: no multi-line Practice prose in `react.dag`; in-file **`// 🟢|🟡 coproduct dissolution — DECISIONS.md#…`** one-liners per operator PR **#3234**). **Dissolution** = operator-ratified PR that extends or retires this sum — not silent “terminal” closure.
