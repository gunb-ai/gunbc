...
- **`t19_manual_anchor_manifest.dag` — `Int` surrogates:** M1(2.8) lowers many shared `data` bodies opaque in isolation, so `tier` / `layer` / `testgen_category` / `assert_kind` are `Int` today. Header maps codes → **`src/v4/std/verification.dag`** (`TestgenTier`, `TestgenLayer`, `AssertKind`) + five arms in **`lens/testgen.dag`**; not dsl/v3 Assert* or v3 `TestPredicate`.
- **Row (12) `behavior_value`:** `BehaviorValueSubject` inside `TypeConstructionSubject` = `std/node.dag` `Behavior::Value` only (no `behavior: Behavior` widening; P2). `testgen_category=0` with connective rows = internal sum, not a sixth top-level category.
- **Retirement (same band as M2 unblock):** when M2 loads `v4.std.*` with `lens/testgen.dag` and `data` literals validate without the opaque trap, replace the four `Int` fields with `TestgenTier`, `TestgenLayer`, ratified five-fold testgen discriminant, `AssertKind`; migrate `slug: String` → `Symbol` per manifest header. Optional later: collapse near-duplicate rows into one `Set<…>` fact.
- **`lens/testgen.dag` — comment-only carriers / five folds:** until M2 cross-file load; Practice-4 ledger binds uncomment. First M2 change set for this module must materialize live `.dag` — comment substrate is a gate, not drift permission.
