...
- **PTX path (operator-ratified / evidenced):** model **PTX** (the spec'd IR — clean, general, captures SIMT directly, parallel to `llvm_ir.dag`). The CUDA-C++-as-`cpp.dag` extension alternative is the **rejected** fork (entangled; the C++ surface is not where the stress is). **`ptx.dag`** records **T-4.14 PASS (IN-B)** (historical **L-3** B2-OMNI down-the-stack stake). **IN-B probe receipt** is **review-visible on this lane's merge PR** + **`docs/briefs/r4-program-dispatch-plan.md` §2** — *not* `src/v4/DECISIONS.md` (retired operator 2026-05-19). **`ptx.dag`** file header stays **domain-neutral** on the process axis (no process-axis status tag in the carrier). **Derived scheduling / completion state** for this slice is authoritative in **`docs/briefs/r4-program-dispatch-plan.md` §2** (Status column: **PASS (IN-B)**; **Blocked-on** `LANDED` — table-accuracy with main; *LANDED* ≠ every TASKS Scope:L tail dissolved — see that row's rework-obligation / keystone blast-radius notes in the dispatch plan).
