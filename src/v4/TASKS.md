...
2. **ParseTree carries a source map keyed by parse position.** `ParseTree = Node` must be replaced by a type that carries the locus association alongside the node tree. `Map<Node, Locus>` is **P2-invalid** — structurally equal nodes at distinct source positions would share one map entry (last-written wins), silently discarding prior location facts. Facts must flow forward; no occurrence may lose its file/span. Preferred form (P2 single authority, universal `Node` pivot A1 preserved):
   ```
   type ParseTree {
     root: Node
     source_map: Map<Int, Locus>
   }
   ```
   The map key must be an **occurrence-unique integer**, not `tok.start` alone. `tok.start` is insufficient: in `02_parse.dag`'s `Sequence` branch, both the `Sequence` result node and its left-child (`cap1`) begin at the same first unconsumed token — identical `tok.start`. Wrapper productions (`Capture`, `Labeled`) share the full span with their inner node. Any `tok.start`-keyed map would silently overwrite entries for nested nodes at the same start position, exactly the P2 fact-drop the task is meant to prevent. **`(start: Int, end: Int)` ByteRange pair is also insufficient**: `Capture` and `Labeled` wrappers produce nodes with the same span as their inner child.

   The correct key is a **monotonic parse-occurrence counter**: thread an `Int` allocation counter through all `parse_expr` / `parse_rule` invocations, bump it each time a node is constructed, and record the resulting index alongside the node's locus. The source map value is `Textual { file: tok.file, extent: ByteRange { start: tok.start, end: tok.end } }` where `tok.file`, `tok.start`, and `tok.end` come from the first and last tokens of the matched production. The `Node` pivot is untouched; occurrence identity lives in the map index, not in the node value. **Scope note:** threading the counter requires adding a counter parameter to `parse_expr` / `parse_rule` signatures — plan for this signature change within T-37.

3. **Pipeline threading.** `03_normalize.dag` / `03_resolve.dag` / `04_infer.dag` must accept and forward the `SourceMap` alongside their `Node` tree parameters. This is a seam-level change across T-8 and T-9 scope — it requires each stage's `Owns` I/O signature to carry the map through until `InferredFacts` stamps the locus as a declared fact on each inferred node (`InferredFacts.locus: Locus` populates from the source map's `Textual { file, extent }` entry rather than defaulting to a non-source locus). Normalize/resolve transforms that create new nodes assign them the locus of their origin node's start offset; synthesized nodes with no token origin have **no entry** in the source map (`Locus` has no `Unavailable` variant — `Unavailable` is a `Correction` variant in `std/diagnostic.dag`; absence from the map is the correct representation of "no source location").

4. **`lexeme` preservation.** `tok.lexeme` is the original source text of the matched token — needed for diagnostic message bodies that quote source. The source map's `ByteRange` carries enough information to slice `lexeme` from the original source buffer if the buffer is threaded. Decide in T-37 scope whether `lexeme` is threaded separately (as a `Map<Int, String>` companion to `source_map`, keyed by the same parse-occurrence counter index) or reconstructed from the span + the original `Source` string (no separate map, source buffer threaded instead). Single-authority: one of these paths, not both.
