...
2. **ParseTree carries a co-structural locus tree.** `ParseTree = Node` must be replaced by a type that carries locus alongside the node tree. Three alternatives are P2-invalid and explicitly rejected:
   ...
   **Preferred form** (P2 single authority, `Node` pivot A1 preserved, typed association):
   ...
   ```
   `LocusTree` is a co-structural parallel of the `Node` tree: same recursive shape, `children` list length and order match `Node.children` at every level. Consumers traverse `(Node, LocusTree)` in parallel using structural recursion — no external counter or implicit traversal convention. `locus: Some(Textual { file: tok.file, extent: ByteRange { start: tok.start, end: tok.end } })` for nodes with token origin; `locus: None` for synthesized nodes (no source location). The `Node` pivot is untouched; the co-tree is the boundary's fact-carry artifact. **Scope note:** `parse_expr`/`parse_rule` return `(Node, LocusTree)` pairs; callers compose the co-tree structurally as they assemble child results.

3. **Pipeline threading.** `03_normalize.dag` / `03_resolve.dag` / `04_infer.dag` must accept and forward the `LocusTree` alongside their `Node` tree parameters. This is a seam-level change across T-8 and T-9 scope — it requires each stage's `Owns` I/O signature to carry `(Node, LocusTree)` pairs through until `InferredFacts` stamps the locus as a declared fact on each inferred node (`InferredFacts.locus: Locus` populates from the co-tree's `Textual { file, extent }` entry at the matching position). Normalize/resolve transforms that produce a new `Node` from an existing one propagate the origin's `LocusTree` entry; synthesized nodes with no token origin receive `LocusTree { locus: None, children: [...] }` (`Locus` has no `Unavailable` variant — `Unavailable` is a `Correction` variant in `std/diagnostic.dag`; `locus: None` in the co-tree is the correct representation of "no source location").

4. **`lexeme` preservation.** `tok.lexeme` is the original source text of the matched token — needed for diagnostic message bodies that quote source. The source map's `ByteRange` carries enough information to slice `lexeme` from the original source buffer if the buffer is threaded. Decide in T-37 scope whether `lexeme` is threaded separately (as a `Map<ByteRange, String>` companion keyed by the `ByteRange` in each `LocusTree` entry's `Textual` locus) or reconstructed from the span + the original `Source` string (no separate map, source buffer threaded instead). Single-authority: one of these paths, not both.
