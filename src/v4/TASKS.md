...
### T-37 — parse-boundary token-fact carry-through: Locus::Textual population across `02_parse.dag`  [SCHEDULED]
...
**P2 violation — boundary drops a declared fact.** `std/diagnostic.dag` already declares `Locus = Textual { file: Symbol, extent: Extent } | NodeLocus | PortLocus` (marked `🟢 TERMINAL`) with `Extent = WholeFile | ByteRange { start: Int, end: Int }`. `Locus::Textual` is the existing single authority for file + byte-range source location. There is no path by which `Token.start` / `Token.end` / `Token.file` populate a `Textual` locus through the parse boundary. Every stage diagnostic after parse defaults to loci other than `Textual` even for errors whose source position is structurally knowable — a P2 fact-drop at the boundary.

**What must happen:**

1. **Use the existing authority — no new carrier.** `Locus::Textual { file: Symbol, extent: Extent }` in `std/diagnostic.dag` is already the single terminal authority for file+byte-range source location. T-37 does NOT introduce a new `SourceSpan` type. The dissolution target for `lens/application.dag`'s opaque `type SourceSpan` (line 64, "Mechanical port (interface-freeze pin §4): opaque until shared substrate graduates") is `Locus` from `v4.std.diagnostic` — replace the opaque with an import of `Locus` and use `Textual` at call sites. After T-37, `Locus::Textual` is the only source-location carrier; the opaque `type SourceSpan` is retired, not graduated to a new type.

2. **ParseTree carries a co-structural locus tree.** `ParseTree = Node` must be replaced by a type that carries locus alongside the node tree. Three alternatives are P2-invalid and explicitly rejected:
   - `Map<Node, Locus>` — structurally equal nodes at distinct source positions share one entry (last-written wins, facts drop).
   - `Map<Int, Locus>` keyed by `tok.start` — `Sequence` and its left child both begin at the same first unconsumed token; `Capture`/`Labeled` wrappers share the full span with their inner node. Any position-keyed flat map collides.
   - `Map<Int, Locus>` keyed by a monotonic parse-occurrence counter — leaves consumers holding a `Node` tree and a disconnected integer table; they cannot look up a node's locus without re-deriving an implicit traversal convention, violating P2's requirement for a typed query surface.

   **Preferred form** (P2 single authority, `Node` pivot A1 preserved, typed association):
   ```
   type LocusTree {
     locus: Option<Locus>
     children: List<LocusTree>
   }
   ...
   ```
   All pipeline consumers (normalize/resolve/infer) use `fold_parse_tree` — they do not manually zip `Node.children` and `LocusTree.children`. The fold is the single declared association surface; `Outcome<R>` is the return type so child-count shape mismatch produces a typed `Rejected` diagnostic rather than a panic or a silently-wrong result — P3 fail-closed through the standard `Outcome` error path. **Locus assignment:** source-backed leaf nodes carry `locus: Some(Textual { file: tok.file, extent: ByteRange { start: tok.start, end: tok.end } })`; composite nodes (e.g. `Sequence`) carry `locus: Some(Textual { file, extent: ByteRange { start: first_child_start, end: last_child_end } })` derived from child spans; synthesized nodes with no parse origin carry `locus: None`. The `Node` pivot is untouched; the co-tree is the boundary's fact-carry artifact. **Scope note:** `parse_expr`/`parse_rule` return `(Node, LocusTree)` pairs; callers compose the co-tree structurally as they assemble child results.
...
3. **Pipeline threading.** `03_normalize.dag` / `03_resolve.dag` / `04_infer.dag` must carry `ParseTree` (not bare `(Node, LocusTree)` pairs) through each stage and use `fold_parse_tree` (step 2) as the single access surface.
   ...
   **Critical — T-9 wave:** `InferredTree.facts: Map<Node, InferredFacts>` (current substrate, `04_infer.dag:94`) is keyed by structural `Node` value. Adding `InferredFacts.locus: Option<Locus>` while this map remains `Node`-keyed is **P2-invalid**: structurally equal nodes at distinct parse positions share one map entry, silently overwriting the locus of every duplicate — the same collapse as `Map<Node, Locus>`. The T-9 wave of T-37 **must** change `InferredTree.facts` from `Map<Node, InferredFacts>` to a co-structural carrier that preserves occurrence identity (e.g. parallel to `LocusTree`, or an `InferredTree` that mirrors the node-tree shape). This is a load-bearing signature change (`04_infer.dag` is an INVARIANTS.md-named stage); worker must escalate under explicit T-9/T-37 scope authorization before touching it.
   ...
   The inferred locus carrier is `InferredFacts.locus: Option<Locus>` — **not** mandatory `Locus` — because synthesized nodes have `locus: None` in the co-tree (mandatory `Locus` would force fabrication or silent drop, both fail-open against P3). `Some(Textual { ... })` for source-backed nodes; `None` for synthesized. Diagnostic consumers must pattern-match and emit a dedicated diagnostic on `None` rather than defaulting. Normalize/resolve produce new `ParseTree` values — they transform `(Node, LocusTree)` in tandem, not just thread the `Node`. Shape-changing rewrites (e.g. TypeSugar wrapper elimination) restructure the `LocusTree` in lockstep: when a wrapper node is removed, its `LocusTree` entry is dropped and the parent `LocusTree` node is updated to splice in the removed node's children directly, preserving structural co-correspondence. Synthesized nodes introduced by normalize/resolve receive `LocusTree { locus: None, children: [...] }` (`Locus` has no `Unavailable` variant; `Unavailable` is a `Correction` variant in `std/diagnostic.dag`).

4. **`lexeme` preservation.** `tok.lexeme` is the original source text of the matched token — needed for diagnostic message bodies that quote source. `Map<ByteRange, String>` as a lexeme companion is **invalid**: different source files can produce identical byte ranges, so a `ByteRange`-keyed map conflates tokens from separate files. The correct approach is **source-buffer reconstruction**: thread the original `Source` string alongside `ParseTree`; `lexeme` is sliced from the buffer using the `ByteRange` from the co-tree's `Textual` locus entry, with `file` as the disambiguation key when multi-file input is supported. No separate lexeme map. T-37 scope confirms this path.
...
**Scope:** L — `ParseTree` shape change + `lens/application.dag` opaque retirement + cross-cutting threading obligation across four pipeline stages. No new `std/` types. Dispatch in waves keyed to T-7, T-8, T-9 close-out trains.
