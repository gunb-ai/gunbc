...
1. **Source-span carrier in `std/` — P2 single-authority consolidation.** `lens/application.dag:64` already declares `type SourceSpan` as an **opaque scaffold** ("Mechanical port (interface-freeze pin §4): opaque until shared substrate graduates") consumed by `EnforcedApplication.span` and `IntrospectApplication.span`. This is the T-37 dissolution target — not a new name. T-37 owns the `SourceSpan` substrate graduation: define `SourceSpan { file: Symbol, range: ByteRange }` in `std/` (DFS per M9 — `std/diagnostic.dag` already owns `ByteRange`, `Locus`, and locus constructors; that is the concept-DAG home unless DFS finds a tighter ancestor), add a `SourceLocus { span: SourceSpan }` variant to `Locus` so downstream diagnostics have a concrete file+range arm, and replace `lens/application.dag`'s opaque `type SourceSpan` with an import from `v4.std.diagnostic` (or `v4.std.span` if that is the ratified home). After T-37, there is exactly one `SourceSpan` authority — a single-authority graduation, not a second carrier. This modeling decision is T-37's first deliverable; resolve before changing any pipeline file.
