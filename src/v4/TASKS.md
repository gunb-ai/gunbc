...
### T-4.9 / T-4.10 / T-4.11 — architecture stress probes (operator-ratified 2026-05-15)

Three **parallel** tasks (need only T-1 + the B2-OMNI `LanguageModel` contract; independent of each other and of T-4). Their value is that they are **maximally diverse on purpose** — each is a *falsification probe* for the just-ratified B2-OMNI O(N+M) claim. If adding one is genuinely O(1) (one declarative model, instantly cross-composing through the Node pivot), B2-OMNI is empirically validated. If any forces a core/pipeline change, B2-OMNI is leaking — surfaced now, before it is load-bearing. Long-held v2 intent, de-deferred per the frontload-the-hard-cases discipline.
