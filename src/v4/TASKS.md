...
- **T-25-core** — `src/v4/std/refinement.dag` (`Validation<B>`, `Refined<B>`, `refine`, `refined_base`).
  ...
  **Approach (operator-ratified 2026-05-25):** Strengthen the home type before reaching for a refinement. `posix.dag` ProcessId/ExitCode is the reference pattern: the home type (`ProcessId`, `ExitCode`) carries width + signedness facts from the POSIX spec; `Refined<B>` then narrows the valid range (0–65535 for ProcessId, 0–255 for ExitCode). Refinements are for externally-specified boundary values only — not for plugging weak home modeling. A refinement on an under-modeled home type does not fix the home; it adds a second layer of opacity over the gap.
