...
**Close-status (2026-05-28, post-#3783; T-37 probe `main@c65b9bdc5`):** predicates 1–3 **PARTIAL**; 4–5 **PAPER-ONLY**. **T-37 (crisp-raven-567):** exit 124 — full `src/v4` (261 modules) OOM/hang within 120s; v0.2.0 memoized DAG backend insufficient; empty `/tmp/v4-dag-out` (no `dag-artifact.json`); `scripts/v4-bootstrap-resolve-posture-gate.sh` **stays** until `--target dag` completes without SIGTERM. T-37 is a real v2 fix (span-based `dag_node_key` dedup in `src/v2/compile.dag` not enough); fix worker dispatched. Lane A: T-38 / T-20-fill still open. Side branch `T-4 → T-9`: T-29 / T-30 / T-25-core / T-21 landed; **P1-KEYSTONE** open (merge [#3752](https://github.com/gunb-ai/gunbc/pull/3752)); **T-19** landed [#3636](https://github.com/gunb-ai/gunbc/pull/3636) (TASKS sync: nimble-bee-438); **T-33** closeout [#3787](https://github.com/gunb-ai/gunbc/pull/3787).
