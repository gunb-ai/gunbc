...
**Modeling decisions**:
- Hook-as-substrate: **per-arm `ReactHookSite`** (design-r4 §4 Q2 / canvas:76) — each built-in hook is its own coproduct arm carrying only that call’s signature fields (`ReactHookInlineDependenciesArgument` only where react.dev admits **omit-vs-present** deps; **`UseMemo` / `UseCallback`** carry **required** `dependencies: List<ReactCrossDeclRef>` per pinned signature + design-r4 canvas `List<Reference>`; `ReactOptRef` / cleanup / refs elsewhere as admitted). Roster matches `react.dag`: **18** stable `use*` APIs under the react.dev **Hooks** index, plus the separate **`use(resource)`** API (listed outside Hooks with different rules; substrate arm `UseResource`) → **19** built-in arms, plus boundary arm **`CustomHook { implementation_ref: ReactCrossDeclRef }`** (= **20** `ReactHookSite` variants — opaque custom body via declaration edge; not `Node` embedded in the sum — see `ReactHookSite` in `react.dag` + `docs/design-r4-phase-1-5-hookkind-custom-react-substrate-canvas.md` for R4 Phase‑1.5 Custom posture).
...
- **ReactNode (Practice‑4 🟡):** T‑4.7 **partial** `createElement` **children** slice in `react.dag` (`Element` | `Text` only). Full react.dev / JSX child lattice (arrays, portals, conditional `null`, …) is **Owned ELSEWHERE** — promotion / dissolution discipline is recorded here and in `docs/design-r4-phase-1-5-hookkind-custom-react-substrate-canvas.md` (not as long-form Practice prose inside `react.dag`, per operator strict de-prose on comment volume). **Dissolution** = operator-ratified PR that extends or retires this sum — not silent “terminal” closure.
