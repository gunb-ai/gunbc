...
- **`t19_manual_anchor_manifest.dag` — slug join table (P2):** `ManualTestgenAnchor` rows carry **`slug: String` only** (no duplicated tier/layer/kind). Each live manual `TestClaim` in `connective_anchors.dag` ∪ `nat_law_anchors.dag` carries the same string on **`TestClaim.anchor_slug`** — mechanical join is `manifest.slug = claim.anchor_slug` (no declaration-name / file-order convention). **`TestClaim.classification`** and **`TestClaim.kind`** remain sole authority for tier×layer and assert form. Testgen scheduling arm (`type_construction` vs `algebra_law` …) is implied by slug prefix until M2 can read claims or carry a single non-duplicated discriminant if needed.
