...
- **#4 — T-16 SQL DDL — RESOLVED (operator 2026-05-17): fork (a).** SQL is
  ...
  **Single SQL authority — `sql.dag` is a PORT, not a second authority**
  (briansrls + codex blocking review, #3224, 2026-05-17). A v3 SQL authority
  already exists: `dsl/extdeps/sql/migration.dag` models migration-script
  shape + ordering constraints (`SqlMigrationOperationKind`,
  `SqlMigrationStep`/`Script`/`EmissionTarget`), and `dsl/extdeps/
  transports/sql.dag` carries the transport execution config. v4
  `extdeps/formats/sql.dag` **ports / consumes** that existing contract —
  reconciled into the v4 checked-Shape-B-format shape — it is **not** a
  freshly-authored parallel SQL model. Authoring a second generated-SQL
  authority beside `migration.dag` violates INVARIANTS P2 (single
  authority) and extdeps fidelity (the spec already has one modeled
  home). T-4.6's `sql.dag` brief states the port explicitly: every SQL
  fact the v3 file already models is carried forward, not re-invented;
  net-new modeling is only what the v3 contract did not cover, and the v3
  file is retired into the v4 one (no dual representation left standing).
