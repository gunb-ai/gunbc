...
2. **ParseTree carries a source map keyed by parse position.** `ParseTree = Node` must be replaced by a type that carries the locus association alongside the node tree. `Map<Node, Locus>` is **P2-invalid** — structurally equal nodes at distinct source positions would share one map entry (last-written wins), silently discarding prior location facts. Facts must flow forward; no occurrence may lose its file/span. Preferred form (P2 single authority, universal `Node` pivot A1 preserved):
   ...
   ```
   The map key is `tok.start: Int` — the byte offset of the first token of the matched production within the source file. Token start offsets are unique per occurrence within a file (`Token.start` is the authoritative position fact from `01_tokenize.dag`; two distinct parse occurrences cannot share the same start offset). The value is `Textual { file: tok.file, extent: ByteRange { start: tok.start, end: tok.end } }`. The `Node` pivot is untouched; the source map is the boundary's fact-carry artifact. **File-scoped uniqueness assumption:** `tok.start` is unique per file; multi-file source requires a compound key. Worker must confirm this is sufficient for the initial single-file case or escalate to a `{ file: Symbol, start: Int }` compound key before touching `02_parse.dag`.
