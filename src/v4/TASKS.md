...
- **Per-language conformance test (operator-ratified 2026-05-25):** Each language slice ships a **`LanguageBehaviorEquivalence` TestClaim corpus** (`lens/testgen.dag` emits; `test/claim/generated/language_behavior_equivalence.dag` is the receipt pattern) proving the fact-bundle round-trips against a frozen behavior snapshot — an **explicit T-4 deliverable**, not a post-close follow-on. **`[needs T-19, T-21]`** — testgen produces the corpus; affected-set selects which claims re-run on change (IRT-2 incremental re-test).
...
- **Lens verdict gate (operator-ratified 2026-05-25):** CI pass/fail for lens enforcement is **`run_required_lens_gates`** in `compiler/00_compile.dag` (`Outcome<List<LensGateWitness>>`), not compiler build exit code alone. `workflow/ci.dag` `LensCiCommand { required_lenses: … }` schedules the lens set; semantic verdict = lens-gate rejection diagnostics, structurally separate from "compile succeeded."
