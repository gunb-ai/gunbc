...
| **L-3** | **Down-the-stack** B2-OMNI probes added (operator-ratified 2026-05-15, parallel up front): T-4.12 `extdeps/languages/llvm_ir.dag` (SSA IR — generalize down the stack; F ~all-structural), T-4.13 `extdeps/languages/machine_code.dag` (bottom of stack; no cosmetics; disassembly = the extreme fail-closed/no-engine test), T-4.14 `extdeps/languages/ptx.dag` (CUDA SIMT — IN-B bet vs the 5 behaviors). Validates the model across the **full target spectrum** (source→IR→machine code) — concretizes C5-fidelity ("we don't necessarily output a binary; F is model+intent-relative"). **PROPOSED forks (confirm):** machine_code = ONE model parameterized by an `Isa` (recommended — parameterize not enumerate, matches O(N+M)) vs per-ISA files; CUDA = model **PTX** (spec'd IR, recommended) vs CUDA-C++ as a cpp.dag extension | `TASKS.md` T-4.12/4.13/4.14 + `extdeps/languages/{llvm_ir,machine_code,ptx}.dag` |
| **QRY-1** | "Language queries" ARE user-defined dimensions (THESIS §1.5/95-101) — a query IS a user-defined lens (`Node -> Set<Match>`/`Witness`), home = `lens/application.dag` (T-23). Named now because it was undertracked. Free-consequence properties asserted (were folklore): **language-agnostic by the B2-OMNI Node pivot** (one query, all ingested languages, O(N+M)); composes derived lens facts (> text/AST tools); introspective read by default + optional `Enforce` (uniform lens UX); a query RESULT is itself a `Node` (bounded kernel, no new carrier); fail-closed/no-engine. **NON-MOVE (binding):** NO `query.dag`/query subsystem — a query IS a lens; a query feature = parallel authority = STOP | `lens/application.dag` header + THESIS §1.5 |
