...
| `std/node.dag` | `reverse_folded_edges<A>` | 🟡 Yellow — Practice 10 substrate sequencing (derived-ops registry row 1 helper) | **Disposition 🟡 (modeling-discipline.md §10):** local list-reverse re-implements a derived list operation pending a shared std primitive. **Named missing primitive:** `reverse<T>(xs: List<T>) -> List<T>` (Freemonoid catamorphism; exact name ratified when authored) in **`std/collection.dag`**, owned by **`TASKS.md` T-3** list/cluster work — not in `node.dag` under T-1 to avoid a premature `node → collection` dependency before the T-3 wave lands. **Program track / dissolve-on-arrival:** substrate sequencing ledger **#3244**; on landing, `fold_node` **deletes** `reverse_folded_edges` and calls the shared `reverse` only. **Bootstrap receipt (STRUCTURE.md v2 gate):** `v2-compiler compile --source-root src/v4` rejects typed fold lambdas, `match` on `List<T>`, and bare `Empty`/`Cons` in this generic position; `fold_node` still owes source-order `List<FoldedEdge<A>>` to `reduce`. **Practice 9 in-file:** `std/node.dag` = one-line **`🟡 feature:reverse` `CP-3248-LIST-REV`** + one-line **`🟢` `CP-3243-FOLD-NODE`** tags only; prose in **CP-3243-FOLD-NODE** section below. |
...
## CP-3243-FOLD-NODE — `std/node.dag` catamorphism (Practice 9)

**Receipt (2026-05-18):** Rationale that would otherwise live in `std/node.dag` body comments is indexed here per `docs/modeling-discipline.md` Practice 9 (substrate files stay de-prosed; in-file = terse **DECISIONS.md** pointer only).

1. **Registry / anchor:** `docs/modeling-discipline.md` §10 derived-ops row 1 (structural catamorphism over modeled type shape); dissolution upstream **#3243** §1.1 / P5.
2. **API:** `fold_node` folds each `Edge.target` to `A`, then applies the supplied F-algebra on `(NodeKind, List<FoldedEdge<A>>)`; `FoldedEdge` carries `EdgeLabel` so the algebra does not lose named-vs-positional facts.
3. **Child order vs v2 bootstrap gate:** `reduce` receives children in **source** `List` order by left-`fold` over `n.children` into a reversed spine, then **`reverse_folded_edges`** (see **CP-3248-LIST-REV**). Motivation: `v2-compiler compile --source-root src/v4` rejects `match` on `List<T>`, typed inner-lambda formals on `fold` callbacks, and bare `Empty`/`Cons` in this generic position (`src/v4/STRUCTURE.md` bootstrap chain).
4. **Prereq-X1.b:** supply the algebra as a `fn` parameter; use `fold_node(..., algebra: alg.reduce)` to thread a `NodeCataAlgebra` field — do **not** write `alg.reduce(...)` with a parameter as the call head (call-on-field-through-parameter is not lowered in this slice).
