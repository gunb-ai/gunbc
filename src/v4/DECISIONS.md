...
**Practice-4 classification — 🟡 plan-bound scaffold.** `run_assert` executes `Equals` and `Compiles`, and models `RoundTrips` as its own named deferral. The remaining closed `AssertKind` variant is `Diagnostic`; Wave-0 defers it through `eval_rejected_assert_kind_deferred` because this slice does not yet model diagnostic-query semantics. The `.dag` import grammar has no alias/qualified-pattern syntax, so importing the `AssertKind.Diagnostic` variant would collide with the fail-closed `Diagnostic` carrier from `std/diagnostic.dag`; the fallback arm is therefore a named scaffold, not an open-ended semantic catch-all. **Dissolution trigger:** replace the fallback with the explicit `Diagnostic` assert-kind implementation when T-22/T-9 gives `TestClaim` a typed diagnostic evaluator path or the DSL gains alias/qualified pattern support. In-file tag: `// 🟡 assert-kind scaffold — DECISIONS.md T22-EVAL-ASSERT-DEFERRED`. | `src/v4/compiler/05_eval.dag` (`eval_rejected_assert_kind_deferred`, `run_assert`) |
