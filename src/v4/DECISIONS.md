...
| `lens/cost.dag` | `SymbolicCost` | Green coproduct (staged) | P9 (#3243 §1.1) seed of the single U1 kernel cost carrier (header U2): `Constant { units: Nat } | UnknownCost`. Green because the two arms are genuinely distinct terminal cost shapes — a fixed count of kernel cost units, and the lattice top for an unmodeled cost — not a hollow alias or a clone of a `std/` carrier. Staged: T-12 `[needs T-9]` expands the remaining lattice arms (`Linear`, `Polynomial`, `PolyLog`, `Exponential`, `Factorial`, `Log`, `Sum`/`Product`) and lands the lattice-axiom verification; this PR ships no `Node -> Witness<SymbolicCost>` read and no axiom obligations. Unblocks the `llvm_instruction_cost` 22-arm match-to-derive dissolution in `extdeps/languages/llvm_ir.dag` (Practice 10 row 7 — property projection); per-target realization cost is a model fact `extdeps/languages/*` declare as data and this lens consumes (U2). That consumer follow-up — hanging a per-instruction `SymbolicCost` field on `LlvmInstruction` and dissolving the match — stays downstream in the `extdeps/languages` lane once the carrier exists. |
