...
| `CppSignedIntegerSurfaceSpelling` / `CppUnsignedIntegerSurfaceSpelling` / `CppPlainCharSurfaceSpelling` / `CppWcharTSurfaceSpelling` / `CppFixedUnsignedCodeUnitIntegerSurfaceSpelling` / `Cpp*WidthSelection` / `CppWcharTSignedness` / `CppTwosComplement` / `CppScalar` | 🟢 terminal for the currently modeled C++ core integer fact-bundle shape; remaining per-spelling profile selection rows stay 🟡 under `feature:t4-cpp-scalar-ladder` | **T29-CPP-SCALAR-FORWARD.** This is the post-`#3338` forward reconciliation of `#3277`: preserve the operator-directed T-29 value (C++ integer widths come from the target ABI/data model) without restoring the stale `type CppBool = Bool` / spelling-map shape. `CppScalar` carries C++ scalar classifier facts through per-arm width-selection witnesses: spelling + `CppTargetProfile` + ABI-selected `CppIntegerWidth` travel together; plain-`char` carries `CppPlainCharSignedness`, and `wchar_t` now carries the separate implementation-defined `CppWcharTSignedness` fact instead of sharing a width-only code-unit carrier with fixed unsigned code-unit types. The bounded scaffold is the not-yet-generated **selection producer**: this PR declares the carrier surface only; `feature:t4-cpp-scalar-ladder` dissolves when the owning cpp-slice lands generated per-spelling target-profile selection rows that consume `CppTargetProfile` and choose the appropriate `CppIntegerWidth` and signedness facts for each implementation-defined spelling (INVARIANTS P5). `CppTargetProfile<abi_model,data_model>` remains the single authority for the target data model; `CppScalar` does **not** store a sibling `CppTargetDataModel` field. Signed scalar arms carry the fixed C++20+ two's-complement representation and the `CppRepresentableResultRequired` witness; unsigned scalar arms and fixed unsigned code-unit scalar arms carry the `CppUnsignedModuloArithmetic` witness. `BoolScalar` carries the bool classifier while the canonical-B decl-ref grounding remains `cpp_bool_grounding`. The shape is a fact bundle, not a hollow alias: no `type CppInt = Int`, no `type CppBool = Bool`, and no local `{spelling}` `GroundingMap`. Signed, unsigned, plain-`char`, `wchar_t`, and fixed unsigned code-unit integer scalar arms are split so unsigned spellings cannot inhabit the signed arm, implementation-dependent plain `char` signedness is carried, and implementation-defined `wchar_t` signedness is not collapsed into the fixed unsigned `char8_t` / `char16_t` / `char32_t` family; later per-spelling rows must select these facts from `CppTargetProfile` rather than guessing LP64/ILP32 constants in `cpp.dag`. Practice-4 dissolution for `CppWcharTSignedness`: fact-placement fails because `wchar_t` signedness is a scalar fact consumed with its width/profile selection; variant-is-data fails because signed and unsigned are externally named implementation alternatives; algebraic form fails because signedness affects integer operation/range semantics rather than only an algebra law; dimensional split fails because a target `wchar_t` has exactly one signedness; parameterized-family fails because signed/unsigned alternatives are closed spec facts, not a homogeneous payload set. |
