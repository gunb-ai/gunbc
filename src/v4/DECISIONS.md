...
| `std/cardinality.dag` | `Never`, `Unit` | Green opaque atoms | Canonical 0- and 1-inhabitance cardinality anchors: `Never` is the empty type (Rust `!` / never-type home; extdeps `NeverScalar` aliases here); `Unit` is the terminal singleton (Rust `()` / ML `unit`). Opaque atoms carry no fields; refinements and language fact-bundles cite these authorities instead of minting parallel empties/units. |
| `std/cardinality.dag` | `NonZeroNat` | Green proof record | Structural strictly-positive natural: `pred: Nat` denotes `Succ(pred)` — excludes `Zero` by construction (no separate ordering predicate). Use for positive counts/widths (e.g. “at least one”) without kernel-ambient `Int` width payloads. |
