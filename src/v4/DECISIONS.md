...
**PR #3272 extension:** P8's constant-expression carrier adds sites 27-28:
...
`ConstantSystemFunctionCall.arguments` remains a plain `List<ConstantExpression>`
because the system-function argument clause is optional; an empty list represents
the omitted clause and is not part of this non-empty deferral.

**Dissolution trigger:** `std/collection.dag` Wave-A2 `List<T> where non_empty` (28 total sites after PR #3272: merge-base sites 1-26 plus PR #3272 sites 27-28).
