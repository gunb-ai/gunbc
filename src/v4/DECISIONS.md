...
14. **`compiler/00_compile.dag` — `core` CP-1b closeout boundary:** T-8 now exposes the composite `core(parse_tree, lm) -> Outcome<CoreTree>` as **`resolve ∘ normalize`** over the shared `Node` pivot, with `CoreTree = ResolvedTree`. This is the handoff surface T-9 consumes; it does not flatten `ParseTree` / `NormalizedTree` / `ResolvedTree` aliases and does not add a second binding authority. **Compile anchor:** `test/claim/manual/resolve_compile_anchor.dag` imports `core` and calls it on the wave-1 empty-`Conj` path so the v2→v4 compile gate keeps the orchestration edge live until T-22 promotes runtime `TestClaim`s.
