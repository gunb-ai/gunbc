...
| **T-4.6-P4-SqlSchemaOperation** | `SqlSchemaOperation` | 🟡 | v4 grounded replacement for v3 `SqlMigrationOperationKind` + raw `statement: String` (the no-templating fix — T-16 emits DDL *through* this structured payload). `SqlRawStep { statement: String }` preserves the v3 `RawSqlStep` escape — **no `dialect` field** (single dialect authority = `SqlMigrationStep.dialect`, faithful to v3 where `RawSqlStep` was a bare nullary variant; see **T-4.6-SQL** "Single dialect authority"). Dissolve a raw step into a structured op when the first migration projection identifies the missing structural operation kind (carried forward from the v3 `RawSqlStep` trigger). Amend **T-4.6-SQL**. |
