...
| Carrier | Classification | Ledger result |
|---------|----------------|---------------|
| `IsaFamily` | 🟢 GREEN terminal | All five dissolution patterns fail: encoding/register/operand rules dispatch on family; `Symbol` would make the closed spec set stringly; not algebra; one ISA revision has one family; families are not one width-indexed carrier. |
...
| `InstructionWord` | 🟡 TRACKED scaffold | Fact placement fails; variant-is-data is partial because a bounded byte-count/Nat-indexed width carrier is not landed; algebra is N/A; dimensional test fails; parameterized-family dissolution is deferred until fixed-cardinality/Nat-indexed width carriers land in `std/machine.dag` or `std/cardinality.dag`. |
...
| `RegisterRef` | 🟢 GREEN terminal | All five dissolution patterns fail: operand legality/cost consumers need the register class fact to flow with each register reference; a bare `Symbol` loses class and a class/name product admits drift; not algebra; one register reference has one class namespace; register classes are heterogeneous. |
