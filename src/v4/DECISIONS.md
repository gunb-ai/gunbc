...
| `std/cardinality.dag` | `NatLeWitness`, `UpperBoundedNat` | Green proof bundle | Inductive witness for `value ≤ inclusive_max` on Peano `Nat`: `LeqZero { upper }` proves `Zero ≤ upper`; `LeqSucc { inner }` lifts `a ≤ b` to `Succ a ≤ Succ b`. `UpperBoundedNat` carries `{ inclusive_max, value, le }`; `nat_le_witness_well_formed` / `upper_bounded_nat_well_formed` validate the witness spine against the two `Nat` spines using total `nat_compare` from `std/nat.dag`. |
