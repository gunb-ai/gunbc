...
| Carrier | Classification | Ledger result |
|---------|----------------|---------------|
| `CppIntegerOverflowDisposition` | 🟢 GREEN terminal | Same dissolution posture as `GoIntegerOverflowDisposition` / Rust `OverflowDisposition<IRCarrier>` classifiers: `RepresentableResultRequired` vs `UnsignedModuloArithmetic` are two mutually exclusive implementation-defined overflow behaviors for C++ primitive arithmetic in this slice, not a bool proxy, not a width-indexed family, and not a consumer-local policy knob. Five-pattern receipt and merge-base recovery live under **Part 6 · CP-3229-GREEN-TERMINAL** (bulk sibling-language recovery); live file keeps the Practice-9 one-line 🟢 tag only. |
