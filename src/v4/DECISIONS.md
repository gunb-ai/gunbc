...
10. **`test/claim/manual/resolve_compile_anchor.dag` (Tier-1 compile anchor):** **Why:** codex #13724 — keep `resolve` / `dag_language_model_canonical_symbols` / wave-1 `DagLanguageModel` on the v2 `compile --source-root src/v4` graph so resolver/LM edges cannot silently rot pre-T-22. **What:** imports `resolve` + `dag_language_model_wave1_void` / `dag_c3_surface_sugar_service`; `anchor_resolve_wave1_service_atom_via_canonical_symbols` builds minimal `Conj` → `Atom(service)` and calls `resolve` (exercises CP-1b items 1–2 + `resolve_atom` canonical-set fallback, including **`language_identity == dag_language_model_surface_id`** per codex #13790). **Runtime:** `v2-compiler run` on v4 `TestClaim`s remains **deferred until T-22** (`test/v2_run_preflight/MOVE1_COVERAGE.txt`). **Bucket-B follow-up (CP-1b/T-8 close):** the Atom anchor is joined by `anchor_resolve_wave1_multi_edge_conj_via_canonical_symbols` (multi-positional-child `Conj` — `resolve_children_homogeneous_scope` coverage) and `anchor_resolve_wave1_bind_binder_use` (arity-3 `Bind` — `resolve_bind_node` / `resolve_bind_edges` coverage, item 3). **TestClaim promotion remains BLOCKED, not done:** `std/verification.dag`'s `AssertKind` is the closed set `Equals | Diagnostic | Compiles | RoundTrips` — there is no resolve-execution assert kind, and `verification.dag` forbids substrate extension (operator STOP). A `TestClaim` that runs `resolve` and asserts `Produced` is therefore gated on (a) a resolve-capable verification surface and (b) `v2 run` on v4 `TestClaim`s, still T-22-deferred. The compile-anchor `fn`s are the honest current-surface coverage; do **not** extend `AssertKind` to close this.
