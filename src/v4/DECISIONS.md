...
|----|------------------|-------------|--------------------------------|------------|
| **T-4.6-CSV** | RFC 4180 row/document carriers (`CsvRow` Peano depth = per-row field arity) + **`CsvRfc4180*` product witness records** (comma / DQUOTE / **CRLF record separator** / header / UTF-8 / `text/csv` facts — no new coproducts; witnesses state **normative CRLF** between records, not LF-as-record-boundary tolerance) | 🟡 | Cross-row/header-vs-body arity mesh is **operator-pending** until a ruling + substrate hook (TASKS.md T-4.6); until then validation may fail-closed without a claimed total mesh. | `extdeps/formats/csv.dag` |
...
| **T-4.6-P4-OpenApiHeaderDeferredKey** | `OpenApiHeaderDeferredKey` | 🟡 | Header Object deferred members (style / explode / **`deprecated`** / example / examples), matching Parameter-derived fixed fields OAS carries on Header (**T-4.6-OAS**); excludes query-only **`allowEmptyValue`** and excludes **`content`** (first-class on **`OpenApiHeaderPayloadSlot`**, XOR with **`schema`**). OAS Header churn on these deferrals ⇒ extend sum + row. |
...
| **T-4.6-P4-OpenApiHeaderPayloadSlot** | `OpenApiHeaderPayloadSlot` | 🟡 | Header Object **`schema` XOR `content`** (OAS 3.1 mutual exclusion); couples to `OpenApiSchema` vs `Map<String, OpenApiMediaType>` (**T-4.6-OAS**). |
