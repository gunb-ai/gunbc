...
| Carrier | Classification | Ledger result |
|---------|----------------|---------------|
| `CppBool` | 🟡 deferred → `feature:t4-cpp-scalar-ladder` | **A-vs-B = B (operator-ratified, ruled-B).** The prior `CppBool = Bool` + `cpp_bool_grounding { spelling: \"bool\" }` shape is **retired**: a bare D2 alias plus a `{spelling}` label asserts identity instead of demonstrating it — the hollow form the machine-readable-inhabitance bar forecloses (prose-only is "basically nil"). Under ruled-B a target type grounds by a fold-discharged structural coincidence, not an asserted alias/spelling-map (see `docs/modeling/grounding-worked-examples.md` §0 — the canonical single-valued exemplar landed on `rust.dag` as `RustScalar.BoolScalar`). `cpp.dag` carries **no** scalar sum yet (the C++ scalar ladder is the 🟡 `feature:t4-cpp-scalar-ladder` deferral above); bool therefore folds into that deferral and lands structurally as `CppScalar.BoolScalar` when the ladder arrives — an honest tracked dissolve, not a fake 🟢. `cpp.dag`'s per-file `GroundingMap` twin is retired with it (no spelling-only resolver rows post-D2-REV; cf. `machine_code.dag` / INVARIANTS §P2). |
