...
7. **`Scope` (`compiler/03_resolve.dag`):** **Classification — 🟢 GREEN terminal (CP-1b/T-8 close, bucket B).** `ScopeRoot { module: Namespace }` vs `ScopeFrame { locals, outer }` is the recursive lexical environment for K-1 use→def binding (B-4; Part 1 **B-4** row). **Practice-4 five-pattern ledger — all dissolution patterns fail:** (1) *fact placement* — the scope chain is read by `lookup_chain` / `scope_root_module` / `resolve_node` / `resolve_atom` / `resolve_bind_*` and is not relocatable to one consumer; (2) *variant-is-data* — the two arms carry heterogeneous payloads (`ScopeRoot` a module `Namespace`; `ScopeFrame` a locals `Map` plus a recursive `outer`), and a flat record would admit a frame with no enclosing scope; (3) *algebraic form* — a lexical environment is not an algebra inhabitance/law fact; (4) *dimensional* — root-vs-frame is not one coordinate of a product; (5) *parameterized family* — the two constructors are not one `F<X>` over identical payloads. It is **not** an `Outcome` clone — it carries no success/diagnostic axis. Terminal recursive carrier; no scaffold dissolution trigger remains. **Named consumers:** `lookup_chain`; `scope_root_module`; `resolve_node`; `resolve_atom`; `resolve_bind_*` family.
