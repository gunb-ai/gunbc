...
| `MachineScalar` | 🟢 GREEN terminal | Same dissolution posture as `RustScalar` `IntScalar` (`rust.dag`): kind×width bundles ISA scalar facts (**D2-REV**). Per-width `MachineI*` / `MachineU*` std aliases + spelling-only `GroundingMap` rows are **omitted** (reversed-D2 hollow). `AddressingMode` displacement slots use `MachineSignedDisplacement32` / `MachineSignedDisplacement64` — explicit `{ kind, width }` products citing SDM / ISA addressing facts; coincidence with std `Int32`/`Int64` is a separate proved obligation, not spelling. |
| `MachineSignedDisplacement32` | 🟢 GREEN terminal | D2-REV: two-field product (`MachineIntKind` × `MachineIntWidth`) models ISA addressing displacement facts for 32-bit-class encodings (SDM / ISA manuals); not a bare `Int32` alias; not one nullary coproduct arm. |
| `MachineSignedDisplacement64` | 🟢 GREEN terminal | D2-REV: same posture as `MachineSignedDisplacement32` for 64-bit absolute linear-address displacement forms. |
