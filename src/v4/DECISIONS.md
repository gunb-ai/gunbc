...
### PREFIX-T-23-registry — `lens/registry.dag` / `LensModulePathV0` (Lane A freeze-pin; 2026-05-18)
...
- **Dissolve-on-arrival:** delete or shrink the hand `compile_to_dag` smoke and this ledger bullet’s staging clauses when a generated `.dag` / emitted consumer is the sole reader of `LensRegistryEntryV0` / `LensModulePathV0` in the compiler substrate (same dissolution style as the T-30 `SourceSpecReadFact` bullet above).
