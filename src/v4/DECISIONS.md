...
| `extdeps/coordination.dag` | `CoordinationBoundary<T>` | Yellow exported-alias scaffold | Documented: `CoordinationBoundary<T> = Outcome<T>` exports the existing Diagnostic-backed fail-closed carrier for downstream coordination consumers, but no owned coordination field consumes the alias yet. Bounded use: consumers may use the alias only as the `Outcome<T>` authority from DECISIONS row I and must not introduce a parallel local result carrier; reviewers should treat it as an exported vocabulary hook, not evidence of an implemented boundary operation. Gate: `consumer:T-16-coordination-boundary-contracts` plus `feature:T-25-core-validation-boundary` (`src/v4/TASKS.md` T-16/T-25-core). Dissolve-on-arrival: when the T-16 coordination consumer lands boundary operations on the T-25-core validation substrate, wire this alias into the concrete boundary contract or delete it if no structural consumer remains. |
