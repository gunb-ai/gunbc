...
**Practice-4 classification — 🟡 plan-bound scaffold.** `t22_hash_claim`, `t22_hash_evaluator`, `t22_hash_resources`, and `t22_hash_extdeps` are symbolic `Hash` placeholders used only to instantiate Wave-0 `TestClaimRun` cache receipts while `std/node.dag` still declares `Hash` as opaque data and B1 `content_hash : Node -> Hash` is not executable in v4. The authoritative cache identity is the single carried `TestClaimCacheKey.subject` plus these placeholder hash slots; `TestClaimCacheReceipt.claim_input` is derived by `test_claim_cache_receipt` from that same subject so pass-receipt diagnostics cannot accept a caller-supplied locus. `TestClaimRun` carries the cache receipt and verdict, so the run cannot pair a verdict with a second, independently restated subject. No consumer may treat the placeholder symbols as content-derived digests. **Dissolution trigger:** replace these four data rows and `run_test_claim` hash parameters with derived B1 content hashes when the canonical `content_hash` fold over `Node` and TestClaim/context subgraphs lands. In-file tag: `// 🟡 cache-hash scaffold — DECISIONS.md T22-EVAL-CACHE-HASHES`. | `src/v4/compiler/05_eval.dag` (`TestClaimCacheKey.subject`, `TestClaimCacheReceipt.claim_input`, `t22_hash_*`) |
...
**Practice-4 classification — 🟡 plan-bound scaffold.** `TestClaimPassReceipt<T>` is the Wave-0 coverage gate over `TestClaimRun<T>` while the runner substrate is still data-modeled. `test_claim_ci_pass_receipt` produces a receipt only for `Pass`; `Fail` becomes `Rejected{eval_rejected_test_claim_not_passed}` at the `claim_input` locus carried by the run cache receipt, and `Deferred` propagates the runner diagnostic. The six `receipt_ci_*` rows are the negative-coverage receipts for LB-T22-3213, so an acceptance regression in `ci_pipeline_well_formed` is carried as a failed coverage outcome rather than as an inert `TestClaimRun` value. **Dissolution trigger:** replace this gate with the canonical TestClaim corpus runner once v4 has pass-only test execution receipts and CI can reject non-pass TestClaims as a first-class gate. | `src/v4/compiler/05_eval.dag` (`TestClaimPassReceipt`, `test_claim_ci_pass_receipt`, `receipt_ci_*`) |
