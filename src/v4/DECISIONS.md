...
| `extdeps/languages/typescript.dag` | `TS-SCALAR-PHASE3`: `TsNumberKind`, `TsBigIntKind`, `TsNumberWidth`, `TsBigIntWidth`, `TsScalar`, `TsEcma262PrimitiveOperationSemantics` | Green scalar fact-bundle coproduct / yellow alias + grounding deferrals | 2026-05-18 T-4 Phase-3 update: TypeScript's scalar tower now follows the `rust.dag` `RustScalar` shape — the outer coproduct selects the scalar family and numeric payloads carry kind × width facts (`number` = binary64 plus ToInt32/ToUint32 bitwise coercion widths; `bigint` = exact unbounded width). `TsNumberWidth`, `TsScalar`, and `TsEcma262PrimitiveOperationSemantics` are 🟢 terminal under this key: fact placement fails because each fact belongs to the TypeScript scalar model, variant-is-data would add looser name fields without more structure, algebraic form is deferred to std numeric coincidence rather than re-declared here, dimensional decomposition is already represented by kind × width payloads, and parameterized-family dissolution has no separately declared spec family to project over. `TsEcma262PrimitiveOperationSemantics` is terminal because resolver rows choose one ECMA primitive-semantics track among IEEE-754 `Number`, ToInt32 bitwise `Number`, ToUint32 bitwise `Number`, and exact `BigInt`; this is not Rust overflow policy and not a re-declared algebra. Per-primitive alias rows, including the previous `TsBoolean = Bool`, and grounding rows remain 🟡 held-provisional pending the operator A-vs-B ruling; no local `GroundingMap` or `ts_*_grounding` rows may be declared. Dissolve-on-arrival: when the operator A-vs-B decision lands, either model the ruled coincidence facts or the ruled alias/grounding shape in the next T-4 TypeScript follow-up; until then the per-primitive D2-fix shape is not modeled. Non-bool alias rows remain 🟡 tracked scaffolds: `TsNumber = Float64` waits on std/float `Float64` + `ApproximateField`, `TsBigInt = Int` waits on unbounded integer/BigInt alignment, `TsString = String` waits on UTF-16/std text refinement policy, and `symbol`/`null`/`undefined` wait on the LanguageModel/nominal-runtime substrate. D2b IEEE-754 `number` Arrow bodies remain deferred to the bundled T-4 grammar and std/float ApproximateField lane. |
