...
**Practice-4 classification — 🟡 plan-bound scaffold.** `t22_hash_claim`, `t22_hash_evaluator`, `t22_hash_resources`, and `t22_hash_extdeps` are symbolic `Hash` placeholders used only to instantiate Wave-0 `TestClaimRun` cache receipts while `std/node.dag` still declares `Hash` as opaque data and B1 `content_hash : Node -> Hash` is not executable in v4. The authoritative cache identity is the carried `TestClaimCacheKey` subject surface (`claim`, `context`, `claim_node`, `tree`, typed `input`, evaluator, resources, extdeps) plus these placeholder hash slots; no consumer may treat the placeholder symbols as content-derived digests. **Dissolution trigger:** replace these four data rows and `run_test_claim` hash parameters with derived B1 content hashes when the canonical `content_hash` fold over `Node` and TestClaim/context subgraphs lands. In-file tag: `// 🟡 cache-hash scaffold — DECISIONS.md T22-EVAL-CACHE-HASHES`. | `src/v4/compiler/05_eval.dag` (`t22_hash_*`) |
