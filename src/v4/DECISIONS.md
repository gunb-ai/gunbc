...
| **T-4.6-P4-SqlColumnConstraints** | `SqlColumnConstraints` (record, not a coproduct) | 🟡 | **P2 dissolution:** the prior `SqlColumnConstraint` sum + `List<SqlColumnConstraint>` admitted duplicate `NOT NULL`/`PRIMARY KEY`/`UNIQUE` (illegal). Dissolved to a record of independent presence facts `{ not_null: Bool, primary_key: Bool, unique: Bool }` — each column-constraint appears at most once, duplicates **structurally unrepresentable**. (Semantic implications like `PRIMARY KEY ⇒ NOT NULL` are not duplicate-illegal states; left as documented fact, not enforced here.) Constraint-grammar breadth (`CHECK`, `REFERENCES`, `COLLATE`, …) deferred — extend the record + amend **T-4.6-SQL** when a migration needs more. No coproduct ⇒ no in-file 🟢/🟡 tag (Practice 9). |
