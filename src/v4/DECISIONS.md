...
| `extdeps/languages/typescript.dag` | `TsNumberWidth`, `TsBigIntWidth`, `TsScalar`, `TsEcma262PrimitiveOperationSemantics` | Green scalar fact-bundle coproduct / yellow alias + grounding deferrals | 2026-05-18 T-4 Phase-3 update: TypeScript's scalar tower now follows the `rust.dag` `RustScalar` shape — the outer coproduct selects the scalar family and the payload carries the spec-read width coordinate (`number` = binary64 plus ToInt32/ToUint32 bitwise coercion widths; `bigint` = exact unbounded width). `TsEcma262PrimitiveOperationSemantics` is 🟢 terminal because resolver rows choose one ECMA primitive-semantics track among IEEE-754 `Number`, ToInt32 bitwise `Number`, ToUint32 bitwise `Number`, and exact `BigInt`; this is not Rust overflow policy and not a re-declared algebra. Per-primitive alias rows, including the previous `TsBoolean = Bool`, and grounding rows remain 🟡 held-provisional pending the operator A-vs-B ruling; no local `GroundingMap` or `ts_*_grounding` rows may be declared. Non-bool alias rows remain 🟡 tracked scaffolds: `TsNumber = Float64` waits on std/float `Float64` + `ApproximateField`, `TsBigInt = Int` waits on unbounded integer/BigInt alignment, `TsString = String` waits on UTF-16/std text refinement policy, and `symbol`/`null`/`undefined` wait on the LanguageModel/nominal-runtime substrate. D2b IEEE-754 `number` Arrow bodies remain deferred to the bundled T-4 grammar and std/float ApproximateField lane. |
