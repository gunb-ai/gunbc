...
| `std/network.dag` | `Rfc3986AbnfProduction`, `UriScheme`, `UriUserInfo`, `UriHost`, `UriPort`, `UriAuthority`, `UriPath`, `UriQuery`, `UriFragment`, `Url`, `NetworkAddress` | Green modeled facts | RFC 3986 §3 carriers model current-substrate facts now: `Url` is the component bundle; `UriAuthority` decomposes to userinfo/host/port; `UriPort` carries `Nat`; `UriScheme` carries a normalized spelling; every component carries its ABNF production data. |
