...
| `CppRepresentableResultRequired` / `CppUnsignedModuloArithmetic` | 🟢 terminal | **T29-CPP-INTEGER-OVERFLOW.** C++20 has exactly two modeled integer-overflow facts at this slice: unsigned arithmetic is modulo 2^N, while signed arithmetic must have a representable result (otherwise the expression is outside the modeled successful semantics / UB boundary). These are separate witness carriers rather than arms of a free coproduct so `CppScalar` cannot construct the invalid cross-pairing (`SignedIntegerScalar` with unsigned modulo, or `UnsignedIntegerScalar` with signed representability). This is not the 🟡 `feature:t4-cpp-scalar-ladder` scaffold: the ladder may add selection rows and non-integer scalars, but these two overflow facts are closed for the C++20 core integer semantics modeled here. |
| `CppSignedIntegerSurfaceSpelling` / `CppUnsignedIntegerSurfaceSpelling` / `CppPlainCharSurfaceSpelling` / `CppWcharTSurfaceSpelling` / `CppFixedUnsignedCodeUnitIntegerSurfaceSpelling` / `Cpp*WidthSelection` / `CppTwosComplement` / `CppScalar` | 🟢 terminal for the currently modeled C++ core integer fact-bundle shape; remaining per-spelling profile selection rows stay 🟡 under `feature:t4-cpp-scalar-ladder` | **T29-CPP-SCALAR-FORWARD.** This is the post-`#3338` forward reconciliation of `#3277`: preserve the operator-directed T-29 value (C++ integer widths come from the target ABI/data model) without restoring the stale `type CppBool = Bool` / spelling-map shape. `CppScalar` carries C++ scalar classifier facts through per-arm width-selection witnesses: spelling + `CppTargetProfile` travel together, but the selected width and implementation-defined signedness facts are **not stored as free sibling fields**. The width-selection records are profile-selection witnesses: a `ShortInt` / target-profile witness denotes the width selected by that profile for `short int`; a `wchar_t` / target-profile witness denotes the width and `CppWcharTSignedness` coordinate carried by that profile's `CppTargetDataModel` for `wchar_t`. The bounded scaffold is the not-yet-generated **selection producer**: this PR declares the carrier surface only; `feature:t4-cpp-scalar-ladder` dissolves when the owning cpp-slice lands generated per-spelling target-profile selection rows that consume `CppTargetProfile` and project the appropriate `CppIntegerWidth` and signedness facts for each implementation-defined spelling (INVARIANTS P5). `CppTargetProfile<abi_model,data_model>` remains the single authority for the target data model; `CppTargetDataModel<family,integer_widths,plain_char_signedness,wchar_t_signedness>` carries both implementation-defined character signedness coordinates, `CppScalar` does **not** store a sibling `CppTargetDataModel` field, and the selection carriers do not store parallel `width`, `plain_char_signedness`, or `wchar_t_signedness` fields that can disagree with the target profile. Signed scalar arms carry the fixed C++20+ two's-complement representation and the `CppRepresentableResultRequired` witness; unsigned scalar arms and fixed unsigned code-unit scalar arms carry the `CppUnsignedModuloArithmetic` witness. `BoolScalar` carries the bool classifier while the canonical-B decl-ref grounding remains `cpp_bool_grounding`. The shape is a fact bundle, not a hollow alias: no `type CppInt = Int`, no `type CppBool = Bool`, and no local `{spelling}` `GroundingMap`. Signed, unsigned, plain-`char`, `wchar_t`, and fixed unsigned code-unit integer scalar arms are split so unsigned spellings cannot inhabit the signed arm, implementation-defined plain `char` / `wchar_t` profile facts are selected by the target-profile witness rather than hand-paired, and fixed unsigned `char8_t` / `char16_t` / `char32_t` stay in their own unsigned family. |
