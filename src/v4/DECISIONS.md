...
| `extdeps/languages/python.dag` | (Practice-4 sum carriers + cost record) | Green coproduct family / records | Same as go row; merge-base had three 🟢 sum ledgers (see **Part 6 · CP-3229-GREEN-TERMINAL**). Post-#3338 de-prose keeps the canonical-B decl-ref surface, preserves the mandated upstream spec `Anchor:`, and drops only surplus rationale/body comments rather than adding false dependencies to satisfy Practice 9 ratio pressure. |
| `extdeps/languages/rust.dag` | (Practice-4 sum carriers + D2 resolver) | Green coproduct family / records | Same bulk **CP-3229-GREEN-TERMINAL** receipt; `PubInPath` semantic scaffold and `RustCost` raw-`Int` bridge remain producer obligations per Part 6 / substrate tables, not narration in the `.dag` body. `RustNever = Never` keeps the Rust never-type spec anchor in-file; coincidence evidence lives in **PR-3252-extdeps-deferrals** and `docs/modeling/grounding-worked-examples.md` §0. |
