...
**🟡 status after T-22 Wave-0.** The enforcement is structural and fail-closed, and the executable demonstration is now modeled in `compiler/05_eval.dag`: `run_ci_dangling_bootstrap_output`, `run_ci_duplicate_job_id`, `run_ci_duplicate_gate_id`, `run_ci_dangling_need`, `run_ci_dangling_gate_job`, and `run_ci_dependency_cycle` exercise `ci_pipeline_well_formed`'s rejection branches through `TestClaimRun<CiPipeline>`. The paired `receipt_ci_*` rows pass those runs through `test_claim_ci_pass_receipt`, so a non-`Pass` verdict becomes a modeled non-pass coverage verdict carrying the full `TestClaimRun` fact instead of a silent well-formed run value. **Remaining yellow scope:** the source-side `ci.dag` in-file tag and T-22 typed-input bridge remain plan-bound receipts until B1 `content_hash` facts replace the placeholder cache hashes and real Node projection / typed TestClaim input facts make the CI pipeline subject a first-class `TestClaim.input` subgraph. **Bilateral binding:** the same status is recorded in `src/v4/TASKS.md` T-22 scope text. In-file tag remains: `// 🟡 negative-coverage plan-bound (T-22) — DECISIONS.md LB-T22-3213`. | `src/v4/workflow/ci.dag` (`ci_pipeline_well_formed` rejection family) + `src/v4/compiler/05_eval.dag` (`run_ci_*`, `receipt_ci_*`, `TestClaimTypedInput`) |
