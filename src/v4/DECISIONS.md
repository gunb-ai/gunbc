...
| **LB-P10-3213-ALL** | `ci_all_job_ids_unique`, `ci_all_gate_ids_unique`, `ci_all_needs_resolve`, `ci_all_gate_jobs_resolve`, `ci_all_commands_authority_ok` (ci.dag) | 🟡 gated `feature:` | `all(p: fn(T) -> Bool, xs: List<T>) -> Bool` (universal). On arrival: the five predicates become `all(...)` compositions; delete the bespoke folds. **`ci_all_commands_authority_ok`** is the same **`fold` over `List<CiJob>`** / `all` shape as the first four; its per-job predicate **`ci_command_authority_ok`** is **also** **LB-T22-3213** (in-file tag on `ci_command_authority_ok` only) — **dual** dissolution obligation (list-op substrate **here** + T-22 executable negative-coverage plan **there**), not a sixth unrelated ledger row. |
