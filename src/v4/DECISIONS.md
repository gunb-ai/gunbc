...
| `extdeps/languages/cpp.dag` | (Practice-4 sum carriers + D2 resolver) | Green coproduct family / records | Same bulk **CP-3229-GREEN-TERMINAL** receipt as sibling `extdeps/languages/*` slices; `CppIntegerOverflowDisposition` 🟢 terminal per subsection **`### src/v4/extdeps/languages/cpp.dag`**; `cpp_bool_grounding` / scalar ladder per that subsection’s `CppBool` row. |
