...
| `extdeps/languages/typescript.dag` | `TS-SCALAR-PHASE3`: `TsScalar`, `TsEcma262PrimitiveOperationSemantics`, `TsScalarTowerSectionRefs` | Green scalar fact-bundle coproduct / yellow alias + grounding deferrals | 2026-05-18 T-4 Phase-3 staged update: TypeScript's scalar tower is the flat ECMA-262 primitive partition (`number`, `bigint`, `boolean`, `string`, `symbol`, `null`, `undefined`), not a Rust-style integer ladder. Version-pinned authorities are ECMA-262 16th edition / June 2025 (`https://262.ecma-international.org/16.0/`, especially §6.1.1-§6.1.6 language types) and TypeScript 5.9 (`https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-9.html`); `TsScalarTowerSectionRefs` is the typed boundary-index shape for those fixed section refs, with data rows deferred until record data bodies stop failing Class-5 gap #3. Forcing kind × width singleton products would over-generate and fabricated structure; the spec facts are one binary64 `number`, one arbitrary-precision `bigint`, and the named primitive families. `TsScalar` is 🟢 terminal under this key: fact placement fails because scalar-family membership belongs to the TypeScript scalar model, variant-is-data would replace the closed partition with looser labels, algebraic form is deferred to std numeric/text/bool coincidence rather than re-declared here, dimensional decomposition fails because TypeScript has no width variance inside `number` or `bigint`, and parameterized-family dissolution has no separately declared spec family to project over. `TsEcma262PrimitiveOperationSemantics` is terminal because resolver rows choose one ECMA primitive-semantics track among IEEE-754 `Number`, the combined ECMA bitwise-number coercion path that may require both ToInt32 and ToUint32 facts depending on the operator, and exact `BigInt`; this is not Rust overflow policy and not a re-declared algebra. Per-primitive alias rows, including the previous `TsBoolean = Bool`, and grounding rows remain 🟡 held-provisional pending the operator A-vs-B ruling; no local `GroundingMap` or `ts_*_grounding` rows may be declared. Dissolve-on-arrival: when the operator A-vs-B decision lands, either model the ruled coincidence facts or the ruled alias/grounding shape in the next T-4 TypeScript follow-up; until then the per-primitive D2-fix shape is not modeled. Non-bool alias rows remain 🟡 tracked scaffolds: `TsNumber = Float64` waits on std/float `Float64` + `ApproximateField`, `TsBigInt = Int` waits on unbounded integer/BigInt alignment, `TsString = String` waits on UTF-16/std text refinement policy, and `symbol`/`null`/`undefined` wait on the LanguageModel/nominal-runtime substrate. D2b IEEE-754 `number` Arrow bodies remain deferred to the bundled T-4 grammar and std/float ApproximateField lane. |
...
**Coproduct ledger (Practice 4):** New `N≥2` sum types in `compiler/02_parse.dag` / `compiler/03_normalize.dag` / `compiler/03_resolve.dag` carry the required in-file 🟡 tag; classification / dissolution pattern / named triggers are recorded here (not in the `.dag` body prose).
...
13. **`compiler/01_tokenize.dag` — `LexRules` expansion (🟡 gated; PR #3292):** **Classification — 🟡 YELLOW (interim alias).** Wave-1 E0 encodes `LexRules` as `Node` grammar-as-data (see CP-1b item **11**); this is an **under-modeled but justified deferral**, not a bare “tracked comment” in the `.dag` file. **Dissolution pattern — closed declared lex-rule carrier:** when lex rules grow beyond the void-book shape, `LexRules` becomes a **coproduct-first** closed lex-rule type (or mechanically equivalent grammar-data carrier) consumed by the generic tokenize walk — not an open-ended `Node` alias carrying informal conventions. **Named triggers / gates:** `feature:T-6` (`TASKS.md` — `compiler/01_tokenize.dag` realizes the generic lexical walk over declarative `LanguageModel.lex`); `feature:T-7` (`TASKS.md` — `compiler/02_parse.dag` realizes the paired parse walk; grammar-side companion to T-6). **Dissolve-on-arrival:** retire the interim `LexRules = Node` encoding when T-6/T-7 (plus `extdeps/languages/dag.dag` / L-5 grammar-data facts) supply the closed lex-rule discriminant the stage discriminates on; a **derived** `Node` projection remains **optional-only** if a ratified self-hosting milestone needs that view (never a second authority alongside the declared lex-rule carrier). **Practice 9 receipt:** forward rationale and process tracking for this gate live **only** here; `01_tokenize.dag` keeps the mandated header plus the one-line 🟡 tag on `LexRules` pointing at this item.
