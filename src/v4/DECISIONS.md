...
| `LeanScalar` | 🟢 GREEN terminal | Same dissolution posture as `RustScalar` `IntScalar` / `BoolScalar` (`rust.dag`): kind×width bundles fixed-precision facts read from the Lean 4.20 anchor; `BoolScalar` parallels the Rust scalar bundle while kernel `LeanBool = Bool` + `lean_bool_grounding` cites std coincidence for the kernel bool spelling only (**D2-REV**). Per-width std aliases + spelling-only `GroundingMap` rows for integers are **omitted** — they were the reversed-D2 hollow shape; unbounded / platform-width Lean numerics are deferred until modeled as explicit fact-bundles. |
...
| `MachineScalar` | 🟢 GREEN terminal | Same dissolution posture as `RustScalar` `IntScalar` (`rust.dag`): kind×width bundles ISA scalar facts (**D2-REV**). Per-width `MachineI*` / `MachineU*` std aliases + spelling-only `GroundingMap` rows are **omitted** (reversed-D2 hollow). `AddressingMode` displacement slots use `MachineSignedDisplacement32` / `MachineSignedDisplacement64` — explicit products over `MachineIntKind` plus **encoding-class-collapsed** width sums (`MachineDisplacement32WidthClass` / `MachineDisplacement64WidthClass`) so a 32-bit-class slot cannot carry `Bits64` at the type level (claude-opus api-review #14158 exploratory). |
| `MachineDisplacement32WidthClass` | 🟢 GREEN terminal | Collapses the displacement width axis to the 32-bit ISA encoding class only (`Bits32`); not a bare `Int32` alias; pairs with `MachineSignedDisplacement32`. |
| `MachineDisplacement64WidthClass` | 🟢 GREEN terminal | Collapses the width axis to the 64-bit absolute linear-address class (`Bits64`); pairs with `MachineSignedDisplacement64`. |
| `MachineSignedDisplacement32` | 🟢 GREEN terminal | D2-REV: `{ kind, width }` over `MachineDisplacement32WidthClass` — SDM / ISA 32-bit-class displacement facts without isomorphism to the 64-bit absolute carrier. |
| `MachineSignedDisplacement64` | 🟢 GREEN terminal | D2-REV: `{ kind, width }` over `MachineDisplacement64WidthClass` — 64-bit absolute displacement forms; not isomorphic to the 32-bit-class record. |
