...
| `LeanScalar` | 🟢 GREEN terminal | Same dissolution posture as `RustScalar` `IntScalar` / `BoolScalar` (`rust.dag`): kind×width bundles fixed-precision facts; `BoolScalar` parallels the Rust scalar bundle while kernel `LeanBool = Bool` + `lean_bool_grounding` cites std coincidence for the kernel bool spelling. Signed fixed-width `Int8`/`Int16`/`Int32`/`Int64`/`ISize` and unsigned `UInt*`/`USize` aliases + `lean_*_grounding` rows cite the std integer ladder per Lean 4.20 fixed-precision surface (same doc anchor as `lean.dag`). |
