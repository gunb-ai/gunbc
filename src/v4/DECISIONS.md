...
| `extdeps/languages/cpp.dag` | (Practice-4 sum carriers + D2 resolver) | Green coproduct family / records | Sibling `extdeps/languages/*` merge-base 🟢 bulk recovery: **Part 6 · CP-3229-GREEN-TERMINAL**. **`CppIntegerOverflowDisposition`** has its **own** terminal receipt (**Part 6 · CP-3229-CPP-INTEGER-OVERFLOW** + table under **`### src/v4/extdeps/languages/cpp.dag`** — not in the GREEN-TERMINAL bulk inventory). `cpp_bool_grounding` / scalar ladder per that subsection’s `CppBool` row. |
...
| Carrier | Classification | Ledger result |
|---------|----------------|---------------|
| `CppIntegerOverflowDisposition` | 🟢 GREEN terminal | Same dissolution posture as `GoIntegerOverflowDisposition` / Rust `OverflowDisposition<IRCarrier>` classifiers: **not** a lumped “implementation-defined overflow” pair — ISO C++ separates **defined** unsigned modular arithmetic from **signed overflow as undefined behavior** outside representable results. **`UnsignedModuloArithmetic`**: unsigned integral arithmetic is **well-defined modulo 2^N** for the type’s width (see `https://eel.is/c++draft/basic.fundamental`). **`RepresentableResultRequired`**: signed results are only in the modeled fragment **F** when the mathematical value is **representable** in the signed result type; **signed overflow is UB**, not a third resolver arm — consumers fail closed outside **F** (see `https://eel.is/c++draft/basic.types` for fundamental-type / value representation framing, and `https://eel.is/c++draft/expr.pre` for expression evaluation outside mathematically defined results). Not a bool proxy, not a width-indexed family, not a consumer-local policy knob. Live Practice-4 / Practice-9 tag cites **Part 6 · CP-3229-CPP-INTEGER-OVERFLOW** (dedicated subsection below; **not** the merge-base **CP-3229-GREEN-TERMINAL** bulk table — `cpp.dag` post-dates that inventory). |
