...
- **`rust.dag` / `RustNever`:** **A-vs-B = B (ruled-B).** **What is RETIRED:** *only* the D2a(2) `{spelling}` `GroundingMap` hollow form (an asserted spelling-map "grounding" the machine-readable-inhabitance bar forecloses). **`type RustNever = Never` is NOT retired — it is RETAINED and 🟢 TERMINAL.** It is the **direct primitive-identity** of Rust `!` to the std `Never` primitive: `!` is uninhabited, there is *nothing above the primitive to ground*, so the alias-identity `RustNever = Never` *is itself* the complete, machine-readable grounding (the blessed alias-identity form, cf. std `type List<T> = FreeMonoid<T>`; the degenerate base of the discipline). It is **terminal, not P2/E-6 staging and NOT consumer-gated** — there is no coincidence fold to discharge (nothing to compare beyond the identity), so it carries **no** `feature:canonical-b-grounding-consumer` scaffold (correctly — that scaffold is *only* for the consumer-gated bool decl-ref). **Contrast — the bool grounding IS the staging case:** `rust_bool_grounding: BooleanAlgebra<Bool> = bool_boolean_algebra` (decl-ref to the single std authority) is present + v2-parse-verified but **P2/E-6 STAGING** — not landed authority, no same-PR consumer, the canonical/coincidence fold *specified-not-realized*; it carries the E-6(b) bounded-exception scaffold + dissolve trigger at its declaration (`docs/modeling/grounding-worked-examples.md` §0). The two declarations are different categories and must not be conflated: bool = consumer-gated E-6(b) staging; `RustNever` = terminal direct primitive identity. Rust `!` / `bool`'s **classifier fact** (orthogonal to grounding) is `RustScalar.NeverScalar` / `RustScalar.BoolScalar` (surface enumeration). The per-file `{spelling}` `GroundingMap` twin is retired from `rust.dag` (cf. `machine_code.dag` post-D2-REV; INVARIANTS §P2). Numeric cost/width refinement (distinct from inhabitance `Never`/`Unit`) stays on its existing **T‑25 / nat / integer** triggers — unchanged by this row.
