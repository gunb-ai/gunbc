...
| `CppIntegerOverflowDisposition` | 🟢 terminal | **T29-CPP-INTEGER-OVERFLOW.** C++20 has exactly two modeled integer-overflow dispositions at this slice: unsigned arithmetic is modulo 2^N, while signed arithmetic must have a representable result (otherwise the expression is outside the modeled successful semantics / UB boundary). Practice-4 dissolution: fact-placement fails because overflow behavior is read uniformly by integer operation semantics/lowering; variant-is-data fails because modulo arithmetic and representable-result-required are distinct operation semantics, not payload values of one record; algebraic-form fails because unsigned modulo behavior relates to fixed-width arithmetic but signed UB/representability is a language operation boundary; dimensional split fails because each integer operation has one applicable disposition; parameterized-family fails because the two arms do not share a homogeneous payload. This is not the 🟡 `feature:t4-cpp-scalar-ladder` scaffold: the ladder may add selection rows and non-integer scalars, but these two overflow alternatives are closed for the C++20 core integer semantics modeled here. |
| `CppSignedIntegerSurfaceSpelling` / `CppUnsignedIntegerSurfaceSpelling` / `CppPlainCharSurfaceSpelling` / `CppCodeUnitIntegerSurfaceSpelling` / `Cpp*WidthSelection` / `CppTwosComplement` / `CppScalar` | 🟢 terminal for the currently modeled C++ core integer fact-bundle shape; remaining per-spelling profile selection rows stay 🟡 under `feature:t4-cpp-scalar-ladder` | **T29-CPP-SCALAR-FORWARD.** This is the post-`#3338` forward reconciliation of `#3277`: preserve the operator-directed T-29 value (C++ integer widths come from the target ABI/data model) without restoring the stale `type CppBool = Bool` / spelling-map shape. `CppScalar` carries C++ scalar classifier facts through per-arm width-selection witnesses: spelling + `CppTargetProfile` + ABI-selected `CppIntegerWidth` travel together, and plain-`char` additionally carries `CppPlainCharSignedness`. The bounded scaffold is the not-yet-generated **selection producer**: this PR declares the carrier surface only; `feature:t4-cpp-scalar-ladder` dissolves when the owning cpp-slice lands generated per-spelling target-profile selection rows that consume `CppTargetProfile` and choose the appropriate `CppIntegerWidth` for each spelling (INVARIANTS P5). `CppTargetProfile<abi_model,data_model>` remains the single authority for the target data model; `CppScalar` does **not** store a sibling `CppTargetDataModel` field. Signed scalar arms carry the fixed C++20+ two's-complement representation and signed representability disposition; unsigned scalar arms carry unsigned overflow/range disposition. `BoolScalar` carries the bool classifier while the canonical-B decl-ref grounding remains `cpp_bool_grounding`. The shape is a fact bundle, not a hollow alias: no `type CppInt = Int`, no `type CppBool = Bool`, and no local `{spelling}` `GroundingMap`. Signed, unsigned, plain-`char`, and code-unit integer scalar arms are split so unsigned spellings cannot inhabit the signed arm and implementation-dependent plain `char` signedness is a carried ABI fact rather than an invalid signed/unsigned constructor choice; later per-spelling rows must select a width from `CppTargetProfile` rather than guessing LP64/ILP32 constants in `cpp.dag`. |
