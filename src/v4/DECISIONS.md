...
| **T-4.6-P4-SqlDataType** | `SqlDataType` | 🟡 | ISO/IEC 9075 predefined-type slice expressible by the modeled DDL operations (numeric / character / boolean / temporal). NOT the full predefined-type grammar — a migration needing a type outside the slice ⇒ extend sum + amend **T-4.6-SQL**. |
...
| **T-4.6-P4-SqlTableConstraint** | `SqlTableConstraint` | 🟡 | Table-level `PRIMARY KEY` / `UNIQUE` / `FOREIGN KEY` over column lists; referential-action / check-constraint breadth deferred — extend sum + amend **T-4.6-SQL**. |
| **T-4.6-P4-SqlSchemaOperation** | `SqlSchemaOperation` | 🟡 | v4 grounded replacement for v3 `SqlMigrationOperationKind` + raw `statement: String` (the no-templating fix — T-16 emits DDL *through* this structured payload). `SqlRawStep` preserves the v3 `RawSqlStep` dialect-specific escape (carries `SqlDialect` + raw `String`) — dissolve a raw step into a structured op when the first migration projection identifies the missing structural operation kind (carried forward from the v3 `RawSqlStep` trigger). Amend **T-4.6-SQL**. |
