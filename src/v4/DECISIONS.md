...
| **T-4.6-P4-SqlColumnConstraints** | `SqlColumnConstraints` (record, not a coproduct) | 🟡 | **P2 dissolution:** the prior `SqlColumnConstraint` sum + `List<SqlColumnConstraint>` admitted duplicate constraints (illegal). Dissolved to a record of independent presence facts `{ not_null: Bool, unique: Bool }` — each appears at most once, duplicates **structurally unrepresentable**. **`primary_key` removed from the column record** (openai-pro 14721 single-authority): a column-inline `PRIMARY KEY` is *surface sugar* for the one table-level primary key; PK now has a **single authority** — `SqlTableDefinition.primary_key: SqlPrimaryKeySlot` (**T-4.6-P4-SqlPrimaryKeySlot**). A parser normalizes `col … PRIMARY KEY` into that slot (documented; the structured column/table model is net-new — no v3 fact dropped). Constraint-grammar breadth (`CHECK`, `REFERENCES`, `COLLATE`, …) deferred — extend the record + amend **T-4.6-SQL** when a migration needs more. No coproduct ⇒ no in-file 🟢/🟡 tag (Practice 9). |
