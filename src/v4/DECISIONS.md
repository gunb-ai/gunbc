...
| **T-4.6-P4-SqlColumnConstraint** | `SqlColumnConstraint` | 🟡 | Column-level constraint slice (`NOT NULL` / `PRIMARY KEY` / `UNIQUE`); table-level keys/FKs are **T-4.6-P4-SqlTableConstraint**. Constraint grammar breadth deferred — extend sum + amend **T-4.6-SQL** when a migration needs more. |
