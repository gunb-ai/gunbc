...
2. **`DagLanguageModel.canonical_symbols` / wave-1 void prelude (B2-OMNI; codex #13790; bucket C close):** The resolver **does not** infer canonical prelude from lex/grammar *shape*. `DagLanguageModel` carries an explicit `canonical_symbols: Set<Symbol>` field; `compiler/03_resolve.dag` `build_program_namespace` threads `lm.canonical_symbols` into `Namespace` unchanged—no `dag_language_model_is_wave1_void_shape` leg in the resolve consumer (INVARIANTS P1/P2; modeling-discipline Practices 3 & 5). **Authoring contract (`extdeps/languages/dag.dag`):** `dag_language_model_wave1_void()` wires the native `.dag` surface bundle: `language_identity: dag_language_model_surface_id` **and** `canonical_symbols: dag_language_model_wave1_void_canonical_symbols()` (the four `data dag_c3_surface_sugar_*` atoms). A void-shaped LM that must **not** inherit that slice supplies an **empty** `Set` (e.g. `dag_language_model_wave1_void_shape_empty_canonical_prelude_compile_receipt()` — non-native `language_identity` mint + `dag_language_model_empty_canonical_symbol_set()`). Per-language extractors (T-4 family) remain the forward path for non-`.dag` models. **Predicate disposition — 🟡 YELLOW:** `dag_language_model_is_wave1_void_shape` stays a bounded wave-1 structural query for other substrate consumers / forward T-6/T-7 dissolution; it is not the resolve prelude authority. **Consumer contract (`compiler/03_resolve.dag`, `resolve_atom`):** after binder lookup misses, the carried `Set` is consulted only as an opt-in self-canonical prelude shortcut; an **empty** `Set` leaves that branch inert, so unresolved `Atom` identities become `ResolveRejected` with `unbound_symbol_diagnostic`—not a silent success path (external api-review receipt, 2026-05-18).
...
10. **`test/claim/manual/resolve_compile_anchor.dag` (Tier-1 compile anchor):** **Why:** codex #13724 — keep `resolve` / `DagLanguageModel.canonical_symbols` / wave-1 `DagLanguageModel` on the v2 `compile --source-root src/v4` graph so resolver/LM edges cannot silently rot pre-T-22. **What:** imports `resolve` + `dag_language_model_wave1_void` / `dag_language_model_wave1_void_shape_empty_canonical_prelude_compile_receipt` / `dag_c3_surface_sugar_service`. **`anchor_resolve_wave1_service_atom_via_canonical_symbols`** — native `dag_language_model_wave1_void()` (non-empty carried prelude). **`anchor_resolve_wave1_service_atom_void_shape_empty_canonical_prelude_compile_only`** — same `Conj`→`Atom(service)` tree with `dag_language_model_wave1_void_shape_empty_canonical_prelude_compile_receipt()` (void shape, **empty** carried prelude; compile-only receipt that the graph stays typable without granting C3 atoms by shape). **Runtime:** `v2-compiler run` on v4 `TestClaim`s remains **deferred until T-22** (`test/v2_run_preflight/MOVE1_COVERAGE.txt`); follow-up — promote to a real `TestClaim` asserting `Produced` / `Rejected` on the two anchors, then add `Bind` / multi-edge cases.
