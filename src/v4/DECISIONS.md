...
**Practice-4 classification — 🟢 terminal.** `Verdict<T> = Pass | Fail { actual: Outcome<T> }` is the runner verdict carrier for TestClaim execution and structural negative checks. It is not a duplicate of `Outcome<T>`: `Outcome<T>` is the evaluated value-or-diagnostic result, while `Verdict<T>` is the assertion result over that outcome. **Patterns tried:** (1) fact-placement — verdict belongs in `compiler/05_eval.dag`, the TestClaim runner/eval owner; (2) variant-as-data — rejected because pass and fail are mutually exclusive assertion results, not simultaneous coordinates; (3) algebraic — no algebra inhabitance determines assertion success; (4) dimensional — no orthogonal axes remain after `actual` is carried only on failure; (5) parameterized family — applied as `Verdict<T>`, so TestClaim and CI negative checks share one carrier instead of parallel pass/fail sums. **No dissolution trigger:** this is the terminal generic verdict family unless a future runner substrate derives assertion verdicts directly from a more general proof carrier. In-file tag: `// 🟢 coproduct dissolution — DECISIONS.md T22-EVAL-VERDICT`. | `src/v4/compiler/05_eval.dag` (`Verdict<T>`) |
