...
**Practice-4 classification — 🟡 plan-bound scaffold.** `TestClaimTypedInput<T>` makes the Wave-0 split between the `TestClaim.input: Node` authority and typed evaluator payloads explicit instead of leaving adjacent `claim.input` / `evaluator_input` fields to drift. For `TestClaimEvalSubject<Node>`, `eval_test_claim_subject` derives both sides from `claim.input`; for the CI negative rows, `t22_ci_subject` records the symbol-node claim input together with the modeled `CiPipeline` value consumed by `ci_pipeline_well_formed`. This is not a terminal substitute for TestClaim-native typed input. **Dissolution trigger:** replace the bridge with real Node projection / typed TestClaim input facts when CI pipeline values can be represented as canonical `Node` subgraphs and B1 `content_hash` can key the whole TestClaim content directly. In-file tag: `// 🟡 typed-input bridge scaffold — DECISIONS.md T22-EVAL-TYPED-INPUT-BRIDGE`. | `src/v4/compiler/05_eval.dag` (`TestClaimTypedInput`, `t22_ci_subject`) |
