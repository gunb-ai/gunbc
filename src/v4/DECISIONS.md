...
| **#3153-subsumed** | `v4-affected-set-design` (#3153, no worker) is **superseded by this branch** (operator 2026-05-15): `docs/design-affected-set-lens.md` remains the prose design authority for the affected-set lens (design framing + 5 worked examples + dimension-aware/fail-closed exclusion discipline); `lens/affected_set.dag` owns only the terse machine-readable T-21 carrier/read surface under the strict de-prose rule. The branch-specific Ex.R1/Ex.R2/Ex.R3 labels are not a second prose authority; they are reconciled here to the now-*encoded* reciprocal contracts — Ex.R1→B-5 (`effect.dag`), Ex.R2→B-4 (`04_infer.dag`), Ex.R3→B-3 (the drafted #3157 B1-CANON point-3 correction). Dogfood-reality is tracked by the T-21 scaffold trigger rather than copied into the `.dag` file. IRT reconciliation (operator 2026-05-17): the decomposed `AffectedSetRead` model (`boundary_hash_receipt`, non-empty `changed_dimension_receipt`, `proven_transitive_changed_subgraph_frontier`, `propagation_receipt`) plus direct `post_edit_dag: fn(Dag, Edit) -> AffectedPostEditDagRead carrying `Outcome<Node>`` and top-level `affected_set(read_model, post_edit_dag, dag, diff)` must check unchanged/pure boundary receipts before post-edit application, prune only at unchanged/pure `content_hash` boundaries, seed every proven changed dimension for each edit, carry current-Dag state through the ordered `Diff` fold using the produced post-edit Dag or fail-closed diagnostic returned by the canonical `Outcome<Node>` only for expanding boundaries, fold an edit-aware proven transitive changed-subgraph frontier over the DAG, and make TestClaim result reuse the cache-dual keyed by the input subgraph `content_hash`; a whole-program traversal scaffold is rejected. Practice-4 coproduct classification: `ExclusionProof` is 🟡 scaffold; named trigger = promote or dissolve when the T-21 `affected_set` fold and `.dag` TestClaims prove root dimension-delta exclusion evidence cases. `PropagationExclusionProof` is 🟡 scaffold; named trigger = promote or dissolve when B-4/B-5 edge non-read evidence is substrate-produced for `DoesNotPropagate`; each proof preserves both the producer and source changed dimension so final exclusion evidence does not drop the axis carried by `EditFrontierEdge.changed_dimension`. `BoundaryExclusionProof` is 🟡 scaffold; named trigger = promote or dissolve when unchanged/pure boundary-prune evidence is folded into the final TestClaim exclusion receipt surface; it carries only proof evidence, with the ordered `BoundaryExclusion.edit` occurrence as the single path authority. `BoundaryExclusion` is 🟢 terminal: it pairs an ordered `Diff` edit occurrence with its boundary-prune proof in a list so repeated equal `Edit` values preserve occurrence evidence rather than collapsing through a map key. `HashReceipt` is 🟡 scaffold; named trigger = dissolve into B1 `content_hash` plus B-5 purity/effect receipts when those reads are substrate-produced. `BoundaryReceipt` is 🟡 scaffold; named trigger = dissolve when T-21 `.dag` TestClaims prove boundary prune/expand/unknown cases; `BoundaryUnknown` must probe `post_edit_dag` before whole-DAG fallback, preserving diagnostic evidence for invalid or non-resolving edits and using symbolic whole-DAG evidence only after post-edit application succeeds. `WholeDagFailClosedEvidence` is 🟡 scaffold; named trigger = dissolve when whole-DAG fail-closed reasons are all diagnostic-backed or terminal symbolic receipts. `ChangedDimensionReceipt` is 🟡 scaffold; named trigger = dissolve when non-empty per-edit dimension deltas are generated from D1 edits and B-5 effect dimensions, with empty/unknown deltas represented by the fail-closed unknown arm rather than an empty changed-dimension list. `FrontierReceipt` is 🟡 scaffold; named trigger = dissolve when the edit-aware transitive frontier read is substrate-produced with fail-closed unknown/diagnostic arms. `PropagationReceipt` is 🟡 scaffold; named trigger = dissolve when B-4/B-5 edge receipts are generated; `EditFrontierEdge.changed_dimension` carries the source changed dimension into propagation, while `FrontierEdge.consumer_dimension` is the consumer/key dimension used for the resulting `ExclusionKey`, so propagation cannot conflate or drop either axis. `ReExecFrontier` is 🟢 terminal: it is the IRT-1 whole-DAG-vs-changed-subgraph gate, with `WholeDagFailClosed` making conservative all-rerun structurally exclusive from changed-subgraph decisions and preserving either symbolic or diagnostic fail-closed evidence; its changed-subgraph arm carries ordered boundary-exclusion occurrences so unchanged/pure prune proofs are explicit without fabricating an `AffectedDimension` or leaving stale final path facts across an ordered `Diff`; inside an edit, `WholeDagFailClosed` is absorbing across dimension and edge folds so later unknowns cannot replace earlier diagnostic evidence. `AffectedFoldAcc` is 🟢 terminal internal fold state because it pairs the canonical `ReExecFrontier` with the current Dag required by D1 sequential rewrite; it carries no duplicate frontier taxonomy and stops semantic edit application once that frontier is `WholeDagFailClosed`. `FrontierDecision` is 🟡 scaffold: the rerun-vs-excluded coproduct is the per-`ExclusionKey` changed-dimension outcome authority and leaves node identity solely on the key path resolved against the input `Dag`; its `Excluded { first, rest }` field shape is a bounded T-25 cardinality bridge that keeps proofless skip unrepresentable until the canonical `List<T> where non_empty` / `NonEmptyList<T>` refinement is exported from `std/collection.dag`. Named trigger = T-25-core publishes the checked non-empty `List` refinement, then rewrite `Excluded` to carry that canonical refined list instead of this local head/rest bridge. When multiple changed producers hit the same key, `Rerun` absorbs `Excluded`, and repeated non-propagating producers aggregate all `PropagationExclusionProof`s so the final exclusion does not drop proof facts. #3153 carried no new file/type; close it as superseded | `docs/design-affected-set-lens.md` + `lens/affected_set.dag` carrier surface + `lens/affected_set_examples.dag` expected-frontier example rows |
...
| `extdeps/coordination.dag` | `NetworkAddress` | Green modeled fact (std import) | **`coordination.dag` consumes `v4.std.network.NetworkAddress`:** landed substrate is `NetworkAddress { authority: UriAuthority }` per RFC 3986 §3.2 (`std/network.dag`, T-26). This row supersedes the prior `NetworkAddress { identity: Symbol }` yellow scaffold / `feature:T-26-std-network-address` disposition. Bounded use: URI component spellings remain bridge carriers until the `std/network.dag` RFC 3986 validated-component refinement row (`feature:T-25-core`) lands — that gap is indexed on `std/network.dag`, not a parallel coordination-local address wrapper. |
...
5. **`SugarClassification` (`compiler/03_normalize.dag`):** **DISSOLVED 2026-05-19.** The local `Sugar | NotSugar` carrier was replaced by the existing optional carrier: `classify_sugar(n) -> SurfaceSugarKind?`. The remaining `SurfaceSugarKind` axis is item 4; absence is now the canonical `None` branch, not a second stage-local coproduct.

6. **`NormalizeChildrenResult` (`compiler/03_normalize.dag`):** **DISSOLVED 2026-05-19.** The local success-or-diagnostic carrier was replaced by the canonical `Outcome<List<Edge>>` from `std/diagnostic.dag`; `normalize_children`, `normalize_edge`, and `normalize_node` now share the same fail-closed carrier.

7. **`Scope` (`compiler/03_resolve.dag`) — DISSOLVED to a product (CP-1b/T-8 close, bucket B; PR #3363 operator + codex review).** The recursive two-arm carrier `ScopeRoot { module: Namespace } | ScopeFrame { locals: Map<Symbol, Symbol>, outer: Scope }` was the **hand-rolled cons-list encoding** of a non-empty lexical-frame stack. Per **P1 / M9** it dissolves to a product over the canonical sequence substrate: `Scope { module: Namespace, frames: List<Map<Symbol, Symbol>> }` — the `ScopeFrame` `outer:` recursion **is** `List` (`std/collection.dag` `List<T> = FreeMonoid<T>`), not a bespoke recursive sum, and the distinguished `ScopeRoot` payload is the always-present `module` field. **Frame ordering / lookup invariant:** `frames` is ordered outermost-first; `scope_push_frame` appends the new innermost frame; `lookup_chain` folds the frames left-to-right with each match overriding the accumulator (seeded with the module-bindings lookup), so the **last (innermost)** match wins and any frame match shadows the module bindings — exactly the prior `ScopeFrame`-first recursive-walk semantics. The earlier 🟢-terminal classification (claiming the Practice-4 *variant-is-data* pattern failed) was **wrong**: a `Base { x } | Cons { y, rest: Self }` carrier is precisely `{ x, rest: List<y> }`. **Named consumers:** `scope_from_namespace`; `scope_push_frame`; `scope_root_module`; `lookup_chain`; `resolve_node`; `resolve_bind_node`.

8. **`ResolveResult` (`compiler/03_resolve.dag`) — DISSOLVED (CP-1b/T-8 close, bucket B).** The bespoke `ResolvedNode | ResolveRejected` success-or-`Diagnostic` carrier is removed; resolve-stage single-node results now carry the canonical `Outcome<Node>` (`std/diagnostic.dag`) directly — `Produced { value }` / `Rejected { diagnostic }`. MODELING.md **M6** (one result pattern, not N result types): a structurally identical bespoke result record dissolves to the generic carrier rather than duplicating it. Fail-closed diagnostics unchanged (same `unbound_symbol_diagnostic` / `malformed_tree_diagnostic` on the `Rejected` branch). **Named consumers (now `Outcome<Node>`):** `resolve_node`; `resolve_atom`; `resolve_bind_node`; `resolve_bind_binder_target`; `resolve_children_homogeneous_scope`.

9. **`ResolveChildrenResult` (`compiler/03_resolve.dag`) — DISSOLVED (CP-1b/T-8 close, bucket B).** The bespoke `ResolvedChildren | ResolveChildrenRejected` carrier is removed; edge-list resolution now carries the canonical `Outcome<List<Edge>>` directly. **M6** dissolution as for item 8. The `EdgeResolveAcc` / `BindEdgeAcc` fold accumulators now carry `acc: Outcome<List<Edge>>` (they remain single-constructor records, not coproducts). The `empty_edge_list()` helper supplies an empty `List<Edge>` at a concrete return type — the generic `Outcome<T>` `Produced { value }` position does not pin a bare `[]` literal under the v2 bootstrap inferencer. **Named consumers:** `resolve_edges_first_outer_then_inner`; `resolve_bind_edges`.

10. **`test/claim/manual/resolve_compile_anchor.dag` (Tier-1 compile anchor):** **Why:** codex #13724 — keep `resolve` / `dag_language_model_canonical_symbols` / wave-1 `DagLanguageModel` on the v2 `compile --source-root src/v4` graph so resolver/LM edges cannot silently rot pre-T-22. **What:** imports `resolve` + `dag_language_model_wave1_void` / `dag_c3_surface_sugar_service`; `anchor_resolve_wave1_service_atom_via_canonical_symbols` builds minimal `Conj` → `Atom(service)` and calls `resolve` (exercises CP-1b items 1–2 + `resolve_atom` canonical-set fallback, including **`language_identity == dag_language_model_surface_id`** per codex #13790). **Runtime:** `v2-compiler run` on v4 `TestClaim`s remains **deferred until T-22** (`test/v2_run_preflight/MOVE1_COVERAGE.txt`). **Bucket-B follow-up (CP-1b/T-8 close):** the Atom anchor is joined by `anchor_resolve_wave1_multi_edge_conj_via_canonical_symbols` (multi-edge `Conj` — `resolve_children_homogeneous_scope` coverage) and `anchor_resolve_wave1_bind_binder_use` (arity-3 `Bind` — `resolve_bind_node` / `resolve_bind_edges` coverage, item 3). **Anchor-root well-formedness (codex review, PR #3363):** all three anchor roots are `Conj` records, and `Conj` carries the `LabeledEdges` discipline (`std/node.dag` `connective_edge_discipline`), so the program-module root edges are **`Named`** and name-distinct (`anchor_decl_first` / `anchor_decl_second`) — a positional-edge `Conj` root fails `node_well_formed` and `resolve` fail-closes before exercising the resolver path. The `Bind` node itself carries positional edges (a `ComputationNode`, no edge-discipline check). **TestClaim promotion remains BLOCKED, not done:** `std/verification.dag`'s `AssertKind` is the closed set `Equals | Diagnostic | Compiles | RoundTrips` — there is no resolve-execution assert kind, and `verification.dag` forbids substrate extension (operator STOP). A `TestClaim` that runs `resolve` and asserts `Produced` is therefore gated on (a) a resolve-capable verification surface and (b) `v2 run` on v4 `TestClaim`s, still T-22-deferred. The compile-anchor `fn`s are the honest current-surface coverage; do **not** extend `AssertKind` to close this.
...
| **T-4.6-SQL** | ISO/IEC 9075 SQL relational/DDL surface in `sql.dag` — Shape-B checked format. **PORT, single authority** (TASKS.md T-4.6 #4; briansrls + codex #3224): every fact the v3 split modelled is carried forward, not re-invented — `dsl/extdeps/sql/migration.dag` (`SqlMigrationOperationKind`/`SqlMigrationStep`/`SqlMigrationScript`/`SqlMigrationEmissionTarget`) and `dsl/extdeps/transports/sql.dag` (`SqlStatementKind`/`SqlParameterStyle`/`SqlTransactionMode`/`SqlTransportConfig`). Port map: v3 `SqlMigrationOperationKind` + raw `statement: String` ⇒ v4 **`SqlSchemaOperation`** (the no-templating fix — migrated DDL is a **grounded structured payload** T-16 emits *through*, not a string printout; the v3 `RawSqlStep` escape is preserved as **`SqlRawStep { statement: String }`**; the v3 `AlterTable`-class fact is preserved as **`SqlAlterTableRaw { table_name, alteration }`** for alter forms outside the structured arms — not collapsed into `SqlRawStep`, see **T-4.6-P4-SqlSchemaOperation**). **Single dialect authority:** v3 `SqlMigrationStep.dialect` was the *sole* dialect authority (v3 `RawSqlStep` was a bare nullary variant with no dialect) — ported faithfully: dialect lives **only** on `SqlMigrationStep.dialect`; `SqlRawStep` carries **no** `dialect` field (no second dialect authority on the operation payload). v3 transport `SqlStatementKind`/`SqlParameterStyle`/`SqlTransactionMode` ported byte-faithful (🟢 terminal, as in v3). v3 `dialect: String` ⇒ **`SqlDialect`** closed sum (Declared-normalized / Fail-closed per C5-fidelity, same as csv dialects — ISO baseline + the three the v3 spec headers cite: PostgreSQL / SQLite / MySQL). | 🟡 | (1) **WireFormat fact carried forward (facts-flow-forward, P3):** v3 `SqlTransportConfig.result_format: WireFormat` (`dsl/std/serialization.dag`: `WireFormat = Json | Text`) is ported as the closed sum **`SqlResultFormat = SqlResultFormatJson | SqlResultFormatText`** — the v3 alternatives are preserved, NOT collapsed to a marker (the earlier `SqlResultFormatSlot {}` dropped the fact and is removed). Same pattern as v3 `dialect: String` ⇒ `SqlDialect`. `src/v4/std/serialization.dag` is empty (no v4 `WireFormat` authority to defer to yet), so this is the SQL-transport-local projection of the result-format fact. **Dissolution trigger:** a v4 `std/serialization.dag` `WireFormat` substrate lands ⇒ fold `SqlResultFormat` into that single std authority (re-point, not a competing mint); amend this row. (2) `SqlDataType` / `SqlColumnConstraints` (the record — *not* the dissolved `SqlColumnConstraint` sum; see **T-4.6-P4-SqlColumnConstraints**) / `SqlTableConstraint` model the migration-expressible ISO 9075 slice (bounded by the DDL surface T-16 emits), not the full predefined-type/constraint grammar — extend the relevant carrier + amend this row when a migration operation needs a type/constraint outside the slice. (3) **Single-authority retirement obligation:** v3 `dsl/extdeps/sql/migration.dag` + `dsl/extdeps/transports/sql.dag` are **superseded / non-authoritative** (v3 frozen 2026-05-15); v4 `sql.dag` is the single SQL authority. Physical deletion of the frozen v3 `.dag` files + their v3 Rust test consumers (`extdeps_sql_transport_test.rs`, `sg0_census_test.rs`) is a **destructive frozen-tree op outside keystone-independent format modeling** — tracked here, executed under the v3-retirement / Rust→0 sweep, not in this PR. (4) **Numeric/character type-parameter positivity — STRUCTURAL (illegal-states-unrepresentable, P2):** `SqlTypeNumeric.precision` and `SqlTypeCharacter(Varying).length` are **`NonZeroNat`** (landed `std/cardinality.dag` carrier — a Nat ≥ 1 via `{ prev: Nat }`), so `NUMERIC(0, …)` / `CHAR(0)` / `VARCHAR(0)` are **structurally unrepresentable** per ISO/IEC 9075 (a precision-bearing numeric / a character-string type requires positive precision / length). No local refinement minted — the std carrier is reused. `SqlTypeNumeric.scale` stays `Nat` because ISO 9075 *permits* `scale = 0` (`NUMERIC(10,0)` is valid). **Residual bounded-P5 (genuinely T-25-core-gated):** the *cross-field* constraint `0 ≤ scale ≤ precision` and the implementation-defined *upper* maxima on precision/scale/length are relational/bounded refinements with **no landed substrate** (T-25-core refinement base + fail-closed validate, **not landed**); until then those are **parse/Diagnostic fail-closed**, not type-level. **Dissolution trigger:** T-25-core lands ⇒ encode `scale ≤ precision` + upper bounds as refinements; amend this row. (5) **Spec-non-empty DDL list scaffold (bounded P5):** ISO/IEC 9075 mandates ≥1 column on `SqlTableDefinition.columns`, `SqlCreateIndex.columns`, `SqlPrimaryKeyPresent.columns` (the single PK authority on `SqlTableDefinition.primary_key` — **not** a `SqlTableConstraint` variant; `SqlTablePrimaryKey` was removed per note (6)), and each of `SqlTableUnique`/`SqlTableForeignKey.columns` + `SqlTableForeignKey.referenced_columns`. Carried as **bare `List`** — **no per-file `NonEmptyList`/first+rest mint** (operator-flagged discipline: `NonEmptyList` is a deferred `std/collection.dag` Wave-A2 refinement, NEVER minted per-file; 5-lang precedent). 🟡 tracked-scaffold deferral: non-emptiness is **parse/Diagnostic fail-closed** until **dissolution trigger:** `std/collection.dag` Wave-A2 `List<T> where non_empty` refinement lands ⇒ migrate these slots to that carrier; amend this row. **Why deferral, not a structural fix:** re-verified — `std/collection.dag` exposes only a `non_empty<T>(xs) -> Bool` *predicate*, no landed `NonEmptyList`/first+rest *carrier*; the operator-flagged standing rule forbids minting one per-file (5-lang precedent), and the refinement form needs the unlanded T-25-core `where`-substrate — so the documented fail-closed deferral is the in-discipline disposition (claude + cursor api-reviews accepted this exact shape). **Same-class residual — `SqlTableForeignKey` column/referenced-column equal arity:** `|columns| = |referenced_columns|` is a cross-list relational constraint (same class as `scale ≤ precision`, note (4)); no landed substrate, **T-25-core-gated**, **parse/Diagnostic fail-closed** until then; dissolves with the same trigger. **Same-class residual — column-name / column-ref uniqueness (openai-pro 14793):** ISO 9075 forbids duplicate column *names* within `SqlTableDefinition.columns` (`SqlColumnDefinition.name`), and duplicate column *refs* within `SqlPrimaryKeyPresent.columns` / `SqlTableUnique.columns` / `SqlTableForeignKey.columns`+`referenced_columns` / `SqlCreateIndex.columns`. These stay **`List`** (not `Map`/`Set`) deliberately — column **order is a real SQL fact** (positional `INSERT`, `SELECT *` order, `ADD COLUMN` append) a `Map`/`Set` would drop (facts-flow-forward regression). Uniqueness-over-an-ordered-list is a `List<T> where unique[_by name]` **refinement** — same class as non-emptiness / FK arity: no landed carrier (per operator-flagged discipline, not minted per-file), **T-25-core-gated**, **parse/Diagnostic fail-closed** until then; dissolves with the same trigger. (6) **Single PK authority + identifier fact bundle (STRUCTURAL — openai-pro 14721):** `SqlIdentifier` is a `SqlRegularIdentifier`\|`SqlDelimitedIdentifier` fact bundle (ISO 9075 case-fold-vs-quote distinction), not a hollow `String` wrapper — within-class lexical validity is the only residual (T-25-core-gated, fail-closed). Primary key has **one authority** — `SqlTableDefinition.primary_key: SqlPrimaryKeySlot` (`Absent`\|`Present { columns }`) — `primary_key` removed from `SqlColumnConstraints`, `SqlTablePrimaryKey` removed from `SqlTableConstraint`; ≤1 PK/table + no column-vs-table disagreement is **structurally unrepresentable** (column-inline PK = surface sugar a parser folds into the slot; structured column/table model is net-new, no v3 fact dropped). `UNIQUE` likewise has a **single authority** (openai-pro 14780): `unique` removed from `SqlColumnConstraints`; the sole UNIQUE carrier is `SqlTableUnique` in `SqlTableConstraint` (column-inline `UNIQUE` = parser surface sugar; a one-column unique = `SqlTableUnique` with one column). `UNIQUE`/`FK` stay **repeatable** list entries (legitimately many per table — unlike ≤1-PK); `not_null` stays column-local (no table-level `NOT NULL` in SQL, no dual authority). See **T-4.6-P4-SqlIdentifier** / **-SqlPrimaryKeySlot** / **-SqlColumnConstraints** / **-SqlTableConstraint**. | `extdeps/formats/sql.dag` |
...
| **T-4.6-P4-SqlDialect** | `SqlDialect` | 🟡 | Declared-normalized / Fail-closed dialect set (ISO 9075 baseline + PostgreSQL / SQLite / MySQL — the dialects the v3 `migration.dag` / `transports/sql.dag` spec headers cite). Same C5-fidelity stance as csv dialects. A migration needing a dialect outside this set ⇒ extend sum + amend **T-4.6-SQL**. |
| **T-4.6-P4-SqlDataType** | `SqlDataType` | 🟡 | ISO/IEC 9075 predefined-type slice expressible by the modeled DDL operations (numeric / character / boolean / temporal). NOT the full predefined-type grammar — a migration needing a type outside the slice ⇒ extend sum + amend **T-4.6-SQL**. **`precision`/`length` = `NonZeroNat`** (landed `std/cardinality.dag`) so zero is **structurally unrepresentable** (P2); `scale` stays `Nat` (ISO 9075 allows `scale=0`). Residual cross-field `scale ≤ precision` + impl-defined upper maxima are T-25-core-gated bounded-P5, fail-closed Diagnostic until then — see **T-4.6-SQL** (4). |
| **T-4.6-P4-SqlColumnConstraints** | `SqlColumnConstraints` (record, not a coproduct) | 🟡 | **P2 dissolution:** the prior `SqlColumnConstraint` sum + `List<SqlColumnConstraint>` admitted duplicate constraints (illegal). Dissolved to a record of independent presence facts — now **`{ not_null: Bool }`** only. **`primary_key` removed** (openai-pro 14721) and **`unique` removed** (openai-pro 14780) for single-authority: column-inline `PRIMARY KEY` / `UNIQUE` are *surface sugar* for, respectively, the one table-level primary key (`SqlTableDefinition.primary_key: SqlPrimaryKeySlot`, **T-4.6-P4-SqlPrimaryKeySlot**) and a table-level `UNIQUE` constraint (`SqlTableUnique` in `SqlTableConstraint`, **T-4.6-P4-SqlTableConstraint** — the single UNIQUE authority; a one-column unique = `SqlTableUnique` with one column). A parser normalizes `col … PRIMARY KEY` / `col … UNIQUE` into those authorities (documented; the structured column/table model is net-new — no v3 fact dropped). `not_null` stays column-local — SQL has no table-level `NOT NULL`, so it is genuinely per-column with no dual authority. Constraint-grammar breadth (`CHECK`, `REFERENCES`, `COLLATE`, …) deferred — extend the record + amend **T-4.6-SQL** when a migration needs more. No coproduct ⇒ no in-file 🟢/🟡 tag (Practice 9). |
| **T-4.6-P4-SqlColumnDefault** | `SqlColumnDefault` | 🟡 | Column `DEFAULT` absent vs literal; structured default-expression grammar deferred (literal carried as `String` until a v4 SQL value-expression substrate exists) — amend **T-4.6-SQL** on dissolve. |
| **T-4.6-P4-SqlTableConstraint** | `SqlTableConstraint` | 🟡 | Table-level `UNIQUE` / `FOREIGN KEY` over column lists — **legitimately repeatable** (a table may have many `UNIQUE` constraints and many FKs), so `List<SqlTableConstraint>` is correct here. **`SqlTableUnique` is the single UNIQUE authority** (openai-pro 14780): `unique` was removed from `SqlColumnConstraints` — a one-column unique = `SqlTableUnique` with a single-element `columns`; column-inline `UNIQUE` is parser surface sugar folded here (mirrors the PK normalization). **`SqlTablePrimaryKey` variant removed** (openai-pro 14721): PK is **not** repeatable (ISO 9075: ≤1 PK per table) so it is **not** a list entry — moved to the single `SqlTableDefinition.primary_key: SqlPrimaryKeySlot` authority (**T-4.6-P4-SqlPrimaryKeySlot**). Referential-action / check-constraint breadth deferred — extend sum + amend **T-4.6-SQL**. Spec-non-empty `columns`/`referenced_columns` are bare `List` (bounded P5, no per-file mint) deferred to std/collection Wave-A2 `NonEmptyList` with fail-closed Diagnostic — see **T-4.6-SQL** (5). |
| **T-4.6-P4-SqlIdentifier** | `SqlIdentifier` | 🟡 | **Fact bundle (openai-pro 14721 — hollow-wrapper fix):** the prior single-field `{ name: String }` was a hollow `String` alias. Modeled as the real ISO/IEC 9075 lexical fact: `SqlRegularIdentifier` (case-folding, bare) **vs** `SqlDelimitedIdentifier` (case-preserving, quoted, may contain reserved words/specials) — a genuine semantic/emission distinction, not cosmetic. **Residual bounded-P5:** within-class lexical validity (regular charset/start-rule; ≤128-char standard limit; delimited escaping) is a string refinement with no landed substrate — **T-25-core-gated**, **parse/Diagnostic fail-closed** until then; dissolves with the T-25-core trigger. |
| **T-4.6-P4-SqlPrimaryKeySlot** | `SqlPrimaryKeySlot` | 🟡 | **Single primary-key authority (openai-pro 14721 / P2):** ISO 9075 — a table has **≤1** primary key. Modeled as one optional slot on `SqlTableDefinition` (`SqlPrimaryKeyAbsent` \| `SqlPrimaryKeyPresent { columns }`), **not** a repeatable `List` entry and **not** a per-column boolean — so "two PKs" / "column-vs-table PK disagreement" / "multiple table PK entries" are all **structurally unrepresentable**. Column-inline `PRIMARY KEY` is surface sugar a parser folds into this slot. Non-empty `columns` is the same bounded-P5 deferral as the other DDL lists — see **T-4.6-SQL** (5). |
| **T-4.6-P4-SqlSchemaOperation** | `SqlSchemaOperation` | 🟡 | v4 grounded replacement for v3 `SqlMigrationOperationKind` + raw `statement: String` (the no-templating fix — T-16 emits DDL *through* this structured payload). `SqlRawStep { statement: String }` preserves the v3 `RawSqlStep` escape — **no `dialect` field** (single dialect authority = `SqlMigrationStep.dialect`, faithful to v3 where `RawSqlStep` was a bare nullary variant; see **T-4.6-SQL** "Single dialect authority"). **`SqlAlterTableRaw { table_name, alteration: String }`** carries forward the v3 `SqlMigrationOperationKind.AlterTable` *class* fact (facts-flow-forward / P3 — codex 14738): an `ALTER TABLE` form not covered by the structured arms (`SqlAddColumn`/`SqlDropColumn`/`SqlAlterTableConstraint`) stays a distinct **ALTER-TABLE-class** operation on a **named table** — strictly more than v3 (which only tagged the kind, with the SQL in `statement: String`) — instead of collapsing into `SqlRawStep` (genuinely arbitrary SQL, no table identity). The unstructured `alteration` grammar (e.g. `ALTER COLUMN TYPE`, `RENAME`) is the 🟡 tracked-P5 residual: dissolve by adding structured alter arms as the first migration projection needs them. Dissolve a raw step / alter-raw into a structured op when the first migration projection identifies the missing structural operation kind (carried forward from the v3 `RawSqlStep` trigger). Amend **T-4.6-SQL**. |
| **T-4.6-P4-SqlStatementKind** | `SqlStatementKind` | 🟢 | Ported byte-faithful from v3 `transports/sql.dag` (🟢 terminal there): SELECT / mutation / DDL / transaction-control name externally observable transport behavior, not an emitter preference. |
| **T-4.6-P4-SqlParameterStyle** | `SqlParameterStyle` | 🟢 | Ported byte-faithful from v3 `transports/sql.dag` (🟢 terminal): externally specified bind-parameter syntaxes (SQLite / PostgreSQL / MySQL), not local aliases. |
| **T-4.6-P4-SqlTransactionMode** | `SqlTransactionMode` | 🟢 | Ported byte-faithful from v3 `transports/sql.dag` (🟢 terminal): irreducible transport-level transaction-scope ownership modes. |
| **T-4.6-P4-SqlResultFormat** | `SqlResultFormat` | 🟡 | Facts-flow-forward port of v3 `SqlTransportConfig.result_format: WireFormat` (`dsl/std/serialization.dag` `WireFormat = Json | Text`) — alternatives preserved, not dropped. SQL-transport-local projection while `src/v4/std/serialization.dag` is empty; dissolution trigger = v4 `std/serialization.dag` `WireFormat` lands ⇒ fold into that single std authority (re-point). See **T-4.6-SQL** (1). |
...
**`feature:` gate:** v4 coverage-lens registry projection for registered dissolution-lens declarations.

**Plan bind:** ROADMAP **T-PB-B** / `pb_rust_tests_outside_residual_zero` generated coverage path; this is the same retirement lane as the SG-0 host parse ratchet for PR #3318.
...
**🟡 status after T-22 Wave-0.** The enforcement is structural and fail-closed, and the executable demonstration is now modeled in `test/claim/workflow/pipeline_rejections.dag`: `run_dangling_bootstrap_output`, `run_duplicate_job_id`, `run_duplicate_gate_id`, `run_dangling_need`, `run_dangling_gate_job`, and `run_dependency_cycle` exercise `ci_pipeline_well_formed`'s rejection branches through `TestClaimRun<CiPipeline>`. The paired `gate_*` rows pass those runs through `workflow_pipeline_pass_gate`, so a non-`Pass` verdict becomes a modeled non-pass coverage verdict carrying the full `TestClaimRun` fact instead of a silent well-formed run value. **Remaining yellow scope:** the source-side `ci.dag` in-file tag and T-22 typed-input bridge remain plan-bound receipts until B1 `content_hash` facts replace the placeholder cache hashes and real Node projection / typed TestClaim input facts make the workflow pipeline subject a first-class `TestClaim.input` subgraph. **Bilateral binding:** the same status is recorded in `src/v4/TASKS.md` T-22 scope text. In-file tag remains: `// 🟡 negative-coverage plan-bound (T-22) — DECISIONS.md LB-T22-3213`. | `src/v4/workflow/ci.dag` (`ci_pipeline_well_formed` rejection family) + `src/v4/test/claim/workflow/pipeline_rejections.dag` (`run_*`, `gate_*`, `TestClaimTypedInput`) |

### T22-EVAL-VERDICT — `compiler/05_eval.dag` `Verdict<T>` coproduct ledger 🟢

**Practice-4 classification — 🟢 terminal.** `Verdict<T> = Pass | Fail { actual: Outcome<T> } | Deferred { actual: Outcome<T>, diagnostic: Diagnostic }` is the runner verdict carrier for TestClaim execution and structural negative checks. It is not a duplicate of `Outcome<T>`: `Outcome<T>` is the evaluated value-or-diagnostic result, while `Verdict<T>` is the assertion result over that outcome. `Deferred` preserves the true evaluated `actual` while carrying the runner diagnostic for assertion kinds that are not executable in Wave-0. **Patterns tried:** (1) fact-placement — verdict belongs in `compiler/05_eval.dag`, the TestClaim runner/eval owner; (2) variant-as-data — rejected because pass, failed assertion, and deferred assertion are mutually exclusive verdict states, not simultaneous coordinates; (3) algebraic — no algebra inhabitance determines assertion success; (4) dimensional — `Deferred` is not an independent axis because a deferred assertion cannot also pass or fail; (5) parameterized family — applied as `Verdict<T>`, so TestClaim and CI negative checks share one carrier instead of parallel verdict sums. **No dissolution trigger:** this is the terminal generic verdict family unless a future runner substrate derives assertion verdicts directly from a more general proof carrier. In-file tag: `// 🟢 coproduct dissolution — DECISIONS.md T22-EVAL-VERDICT`. | `src/v4/compiler/05_eval.dag` (`Verdict<T>`) |

### T22-EVAL-CACHE-HASHES — TestClaim placeholder cache hash facts 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `claim_hash_placeholder`, `evaluator_hash_placeholder`, `resources_hash_placeholder`, and `extdeps_hash_placeholder` are symbolic `Hash` placeholders used only to instantiate Wave-0 workflow-pipeline `TestClaimRun` cache receipts while `std/node.dag` still declares `Hash` as opaque data and B1 `content_hash : Node -> Hash` is not executable in v4. The authoritative cache identity is the single carried `TestClaimCacheKey.eval_subject` plus these placeholder hash slots; `TestClaimRun` carries the cache receipt and verdict, so the run cannot pair a verdict with a second, independently restated subject. No consumer may treat the placeholder symbols as content-derived digests. **Dissolution trigger:** replace these four data rows and `run_test_claim` hash parameters with derived B1 content hashes when the canonical `content_hash` fold over `Node` and TestClaim/context subgraphs lands. In-file tag: `// 🟡 cache-hash scaffold — DECISIONS.md T22-EVAL-CACHE-HASHES`. | `src/v4/compiler/05_eval.dag` (`TestClaimCacheKey.eval_subject`) + `src/v4/test/claim/workflow/pipeline_rejections.dag` (`*_hash_placeholder`) |

### T22-EVAL-ASSERT-DEFERRED — TestClaim diagnostic assert-kind scaffold 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `run_node_assert` and `run_pipeline_assert` execute `Equals` and `Compiles`. Wave-0 defers `RoundTrips` through `eval_rejected_roundtrip_deferred` until round-trip emit/lower/eval comparison facts are modeled, and defers `DiagnosticAssert` through `eval_rejected_assert_kind_deferred` until diagnostic-query semantics are modeled. The variant name intentionally avoids `Diagnostic` so the assertion runners can match exhaustively without shadowing the fail-closed `Diagnostic` carrier from `std/diagnostic.dag`. **Dissolution triggers:** replace the `RoundTrips` deferral when T-22/T-9 has an eval/emit round-trip witness over the same input subgraph; replace the `DiagnosticAssert` deferral when T-22/T-9 gives `TestClaim` a typed diagnostic evaluator path. | `src/v4/compiler/05_eval.dag` (`eval_rejected_roundtrip_deferred`, `eval_rejected_assert_kind_deferred`, `run_node_assert`) + `src/v4/test/claim/workflow/pipeline_rejections.dag` (`run_pipeline_assert`) |

### T22-EVAL-TYPED-INPUT-BRIDGE — typed TestClaim input bridge 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `TestClaimTypedInput<T>` makes the Wave-0 split between the `TestClaim.input: Node` authority and typed evaluator payload explicit without restating the claim-input node. For `TestClaimEvalSubject<Node>`, `eval_test_claim_subject` derives the typed payload from `claim.input`; for the workflow-pipeline rejection rows, `pipeline_rejection_subject` records only the modeled `CiPipeline` value consumed by `ci_pipeline_well_formed`, while diagnostics, cache subject identity, and claim contract read the single `subject.claim.input` authority. This is not a terminal substitute for TestClaim-native typed input. **Dissolution trigger:** replace the bridge with real Node projection / typed TestClaim input facts when CI pipeline values can be represented as canonical `Node` subgraphs and B1 `content_hash` can key the whole TestClaim content directly. In-file tag: `// 🟡 typed-input bridge scaffold — DECISIONS.md T22-EVAL-TYPED-INPUT-BRIDGE`. | `src/v4/compiler/05_eval.dag` (`TestClaimTypedInput`) + `src/v4/test/claim/workflow/pipeline_rejections.dag` (`pipeline_rejection_subject`) |

### T22-EVAL-PASS-RECEIPT — workflow TestClaim pass receipt gate 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `workflow_pipeline_pass_gate` is the Wave-0 coverage gate over `TestClaimRun<CiPipeline>` while the runner substrate is still data-modeled. It returns `Pass` only when the carried run verdict is `Pass`; `Fail` and `Deferred` remain non-pass verdicts whose `actual` carries the full `TestClaimRun<CiPipeline>`, preserving the original evaluated outcome without adding a second diagnostic-locus field. The six `gate_*` rows are the negative-coverage receipts for LB-T22-3213, so an acceptance regression in `ci_pipeline_well_formed` is carried as a failed coverage verdict rather than as an inert `TestClaimRun` value. **Dissolution trigger:** replace this gate with the canonical TestClaim corpus runner once v4 has pass-only test execution receipts and CI can reject non-pass TestClaims as a first-class gate. | `src/v4/test/claim/workflow/pipeline_rejections.dag` (`workflow_pipeline_pass_gate`, `gate_*`) |

### T22-EVAL-OUTCOME-EQ — TestClaim outcome equality scaffold 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `outcome_node_eq` is a local structural equality bridge for Wave-0 assertion execution over `Outcome<Node>` while v4 lacks a canonical derived equality/hash surface for modeled carriers. The helper compares full produced `Node` values and full rejected `Diagnostic` values; workflow-pipeline assertion only feeds it real rejected diagnostics, fails closed when a produced `CiPipeline` is observed where the claim expected rejection, and defers only produced-vs-produced pipeline comparison through `eval_rejected_pipeline_projection_deferred` rather than fabricating a comparable `Node`. **Dissolution trigger:** replace `outcome_node_eq` with the canonical derived equality/proof surface when B1 structural equality / `content_hash` facts are executable for `Outcome<Node>` and `Diagnostic`. In-file tag: `// 🟡 derived equality scaffold — DECISIONS.md T22-EVAL-OUTCOME-EQ`. | `src/v4/compiler/05_eval.dag` (`outcome_node_eq`) + `src/v4/test/claim/workflow/pipeline_rejections.dag` (`eval_rejected_pipeline_projection_deferred`, `run_pipeline_assert`) |

### T22-INFER-SYMBOLIC-COST-BRIDGE — `compiler/04_infer.dag` inferred cost coordinate 🟡

**Practice-4 classification — 🟡 plan-bound scaffold.** `InferredFacts.cost` now carries `lens/cost.dag`'s `SymbolicCost` instead of a raw `Node`, preserving U2's single cost authority at the infer→eval boundary. `InferredFacts.inhabits` carries `AlgebraRef { algebra, witness }` instead of a raw `Symbol`, preserving a typed boundary coordinate while the full algebra inhabitance authority is still pending. The v4 `SymbolicCost` carrier is intentionally minimal at T-22 scope (`ConstantCost { value }` plus conservative `UnknownCost { reason }`) because T-12 owns the total `Node -> Witness<SymbolicCost>` lattice/fold. `InferredTree.facts` is a `Map<Node, InferredFacts>`, so facts are coordinates for arbitrary nodes/subgraphs rather than a single root-level product. **Dissolution trigger:** replace the minimal cost carrier, typed algebra-reference bridge, and Wave-0 all-nodes scaffold lookup with the full T-12 SymbolicCost lattice, typed algebra inhabitance authority, and real per-node inferred-fact table when T-9/T-12 produce inferred-tree facts structurally. In-file tag: `// 🟡 coproduct dissolution — DECISIONS.md T22-INFER-SYMBOLIC-COST-BRIDGE`. | `src/v4/lens/cost.dag` (`SymbolicCost`), `src/v4/compiler/04_infer.dag` (`AlgebraRef`, `InferredFacts.cost`, `InferredFacts.inhabits`, `InferredTree.facts`) |
