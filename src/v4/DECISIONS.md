...
| **D3** | **ApproximateField<Float> witness + float primitive-op spine (v2 bridge + Tier-2 `Symbol` identities)** (operator-ratified 2026-05-16; T-3 hold-lift). `std/float.dag` ships `data float_approximate_field: ApproximateField<Float>` whose op fields **name-reference** primitive `fn` items on the THESIS:203 / **D2b** external-primitive path. **Executable IEEE finite `+`/`*`** are owned by **T-4 `extdeps/languages/*` Arrow bodies**, not Peano-on-exponent combinators in `std/`. **Bridge tension:** frozen **v2** parse requires **constructive `fn` bodies** at this site (**PARSE-1** bridge-era); `float_ieee754_primitive_add` / `mul` ship as **documented placeholders** until extdeps bodies replace them — not silent IEEE claims. **`one`** on `FloatBody.Finite` is **scaffold** until binary64 bias-1023 is authored via **`Word64` interchange + Grounding** (P2 single width authority). Tier-2 NaN fail-close uses intrinsic `data <id>: Symbol = <id>` for `Diagnostic.reason` / `PortLocus.port` (dimension-owned vocabulary; cross-ref **I** + `std/diagnostic.dag` header 42–52). **Encoded in:** `std/float.dag` + this row. Cross-ref **D2**, **PARSE-1**, **I**, INVARIANTS P1/P2/P3. | `std/float.dag` (`data float_approximate_field`, primitive `fn`s, Tier-2 `Symbol` data) + this row |
