# src/v4/archive — quarantine for consumer-less models
...
Rules:
- **Nothing in the active tree may import from `archive/`.** If active code needs something
  here, promote it out *with its consumer* in the same change.
- **No module-path collisions with the active tree.** Archived `.dag` use the
  `v4.archive.*` module namespace so the bootstrap/index never confuses them with live modules.
- **Archiving is git-recoverable and requires no reasoning.** Prefer it over auditing or
  porting consumer-less code (the trap).
- A review that finds a new model with no consumer **hard-blocks**: the author either wires a
  real consumer, moves the model here, or escalates to the operator to merge.
