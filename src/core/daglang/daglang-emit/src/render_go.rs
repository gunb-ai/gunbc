        ...
        Expr::Struct { name, fields, rest } => {
            if rest.is_some() {
                ...
                if field_strs.is_empty() {
                    format!("{}{{ /* ERROR: Go has no struct update syntax */ }}", name)
                } else {
                    format!(
                        "{}{{ {} /* ERROR: Go has no struct update syntax */ }}",
                        name,
                        field_strs.join(", "),
                    )
                }
...
mod tests {
    ...
    // -- RV-1: Go rejects struct update syntax --

    #[test]
    fn render_go_struct_update_emits_error_comment() {
        // Go has no struct update syntax. When `rest` is Some, the renderer
        // must emit an ERROR marker so the output is clearly invalid.
        ...
        assert!(
            ...
            "Go struct with rest must contain ERROR marker: got {rendered}"
        );
        assert!(
            rendered.contains("struct update syntax"),
            "error message should mention struct update syntax: got {rendered}"
        );
    }

    #[test]
    fn render_go_struct_update_empty_fields_emits_error() {
        // Even with empty fields, Go must reject struct update syntax.
        let expr = Expr::Struct {
            name: "Config".to_string(),
            fields: vec![],
            rest: Some(Box::new(Expr::var("defaults"))),
        ...
        assert!(
            rendered.contains("ERROR"),
            "Go struct with rest (empty fields) must contain ERROR marker: got {rendered}"
        );
        // Must not produce a spurious comma from empty fields join.
        assert!(
            !rendered.contains(", /*"),
            "must not contain comma before error comment: got {rendered}"
        );
    }
