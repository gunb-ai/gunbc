...
//! - In-package: `cargo run -p gunbc-codegen --bin gunbc-ci`
//!
...
pub struct CargoInvocation {
    ...
    pub binary: String,
    /// The package name, if different from the binary (e.g., "gunbc-codegen").
...
impl CargoInvocation {
    ...
    /// ```
    /// use gunbc_ir::CargoInvocation;
    /// let inv = CargoInvocation::composed("ci", "codegen");
    /// assert_eq!(inv.binary, "gunbc-ci");
    /// assert_eq!(inv.package, Some("gunbc-codegen".to_string()));
    /// assert_eq!(inv.command(), "cargo run -p gunbc-codegen --bin gunbc-ci");
    /// ```
...
mod tests {
    ...
    fn test_name_composition() {
        assert_eq!(name("ci"), "gunbc-ci");
        assert_eq!(name("gist"), "gunbc-gist");
        assert_eq!(name("codegen"), "gunbc-codegen");
    }
    ...
    fn test_composed() {
        ...
        assert_eq!(inv.binary, "gunbc-ci");
        assert_eq!(inv.package, Some("gunbc-codegen".to_string()));
        assert_eq!(inv.args(), "-p gunbc-codegen --bin gunbc-ci");
        assert_eq!(inv.command(), "cargo run -p gunbc-codegen --bin gunbc-ci");
    }
    ...
    fn test_binary_in_package() {
        let inv = CargoInvocation::in_package("gunbc-ci", "gunbc-codegen");
        assert_eq!(inv.args(), "-p gunbc-codegen --bin gunbc-ci");
        assert_eq!(inv.command(), "cargo run -p gunbc-codegen --bin gunbc-ci");
    }
    ...
    fn test_command_parts_composed() {
        ...
        assert_eq!(
            inv.command_parts(),
            vec!["cargo", "run", "-p", "gunbc-codegen", "--bin", "gunbc-ci"]
        );
    }
