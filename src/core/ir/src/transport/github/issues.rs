//! Provider-agnostic issue workflow adapter for GitHub issues.
//!
//! This module defines the canonical issue snapshot shape consumed by workflow
//! runtime code, plus capability-gate checks required for real-mode execution.

use serde::{Deserialize, Serialize};
use std::fmt;

/// Canonical issue lifecycle stages represented as GitHub labels.
...
/// Provider-agnostic issue snapshot consumed by workflow runtime operations.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Required issue-transport capabilities for workflow real-mode execution (IM12).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct IssueTransportCapabilities {
    pub issue_read: bool,
...
impl IssueTransportCapabilities {
...
/// Validate that provider capabilities satisfy workflow real-mode requirements.
///
...
pub fn ensure_issue_transport_capabilities(
    capabilities: IssueTransportCapabilities,
) -> Result<(), String> {
    ...
    Err(format!(
        "missing required issue transport capabilities: {}",
        missing.join(", ")
    ))
}
...
mod tests {
    ...
    fn capability_gate_fails_closed_when_required_capabilities_missing() {
        let err = ensure_issue_transport_capabilities(IssueTransportCapabilities {
            issue_read: true,
            issue_comment_upsert: false,
            issue_label_compare_and_set: false,
            managed_issue_search: true,
    ...
    fn capability_gate_passes_when_all_capabilities_present() {
        ensure_issue_transport_capabilities(IssueTransportCapabilities::github())
            .expect("full github capabilities should pass gate");
    }

    #[test]
    fn capability_gate_fails_closed_when_no_capabilities_present() {
        let err = ensure_issue_transport_capabilities(IssueTransportCapabilities::none())
