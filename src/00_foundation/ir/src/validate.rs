...
pub fn validate_subdag_interfaces<T>(dag: &Dag<T>) -> Vec<SubDagError> {
    ...
    validate_dag_recursive(dag, &mut errors, &registry, false);
    errors
}
...
fn validate_dag_recursive<T>(
    ...
    inside_loop_pattern: bool,
) {
    for node in &dag.nodes {
        if let NodeBody::SubDag(ref inner, ref kind) = node.body {
            let skip_interface_validation = inside_loop_pattern && node.id.0 == "body";
            if !skip_interface_validation {
                validate_single_subdag(node, inner, registry, errors);
            }
            // Recurse into the inner DAG
            let mut nested_errors = Vec::new();
            validate_dag_recursive(
                inner,
                &mut nested_errors,
                registry,
                matches!(kind, SubDagKind::Loop { .. }),
            );
