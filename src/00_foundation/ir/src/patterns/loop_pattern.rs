...
impl<T: Clone> LoopBuilder<T> {
        ...
        T: From<PatternOp>,
    {
        ...
        // Body subdag: processes each element. Only the collected `result`
        // should be visible at the loop pattern boundary; incidental inner
        // body boundaries (for example pattern helper outputs) must not leak.
        let mut body_node = Node::subdag("body", body_dag);
        if let Some(result_port) = body_node
            .outputs
            .iter()
            .find(|port| port.name.0 == "result")
            .cloned()
        {
            body_node.outputs = vec![result_port];
        }
        ...
        node.outputs = vec![Port::with_cardinality(
            self.output_port_name.as_str(),
            self.output_port_type.as_str(),
            self.input_cardinality,
        )];
...
mod tests {
    ...
    #[test]
    fn test_loop_outputs_do_not_leak_body_boundaries() {
        let mut body = Dag::new();
        body.add_node(Node::opaque(
            "transform",
            vec![Port::scalar("element", "String")],
            vec![
                Port::scalar("result", "String"),
        ...
        let node = LoopBuilder::new("test_loop").with_body(body).build();
