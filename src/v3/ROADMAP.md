...
**Follow-up — TargetCorrection target-native fix rendering (not blocking, first consumer gates graduation).** Lane 3 Stage 3b now ships DB-1's source-level `Correction` path only: `Diagnostic.fixes` carries `.dag` replacement text, and per-target `CorrectionStyle` shapes how that source snippet is rendered in diagnostics. Target-native fix syntax (IDE quick-fix payloads for emitted Rust/Go/Python, linter-specific rewrites, file-local comment syntax, etc.) remains intentionally deferred because DB-1 explicitly separates that domain into a future `TargetCorrection` carrier with target-file spans. **Dissolution trigger:** the first concrete IDE or linter integration consumer that needs target-native fix application. At that point, add the new carrier and keep the current source-level `Correction` path as the single authority for `.dag` edits rather than overloading it with target-language fields.
