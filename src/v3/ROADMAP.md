...
| 3a.3 `where` refinement (DB-11, M→L overrun; DB-16) | ✅ Shipped | PR #496, #515, #522, #524 | Predicate lowering, call-site flatten-and-subset discharge, arm-local narrowing, operator-operand refinement stripping, structural callable-predicate identity, and refined-generic substitution (PR #522). DB-11 out-of-fragment lowering-time rejection; additional lowering fail-closed gate for generic-dependent carriers still mentioned by PR #524 tests; DB-16 pipeline-stage invariant + FnExternalBody doc scoping (2a/2b, DB-14). See **Landing: 3a.3-full**, **Landing: DB-16 refined-generic substitution**, and **DB-16 (`FnExternalBody` reconciliation)** below. |
...
Acceptance: `src/v3/compiler/tests/m2_feature_parity_test.rs::test_3a3_*` — 16+ tests lock refined-parameter compile, literal-arg rejection, matching-refinement forwarding, distinct-refinement non-discharge, if-predicate narrowing (both unrefined and already-refined caller — the latter exercises composite-canonical conjunct matching), non-narrowing rejection, signatureless regression, `&&` / `||` parse + lower + Bool typing, non-Bool operand rejection, structural-callable-identity-across-sites, out-of-fragment predicate rejection at lowering, generic-dependent `where` rejection at lowering (`test_3a3_rejects_refinement_on_*`), substrate integrity (Behavior still 5 variants), and top-level `data` references inside predicates. Design: [design-m2-feature-parity.md §DB-11](../../docs/design-m2-feature-parity.md).

**DB-16 (`FnExternalBody` reconciliation, PR #524).** [design-fn-external-body-reconciliation.md](../../docs/design-fn-external-body-reconciliation.md) — `SurfaceItem::FnExternalBody` / `ArrowBody::Unparsed` split: parse lag (case 1), pipeline host stages (case 2a → bootstrap `ExternalRealization`), substrate accessors (case 2b → stay `Unparsed`, DB-14). Invariant `pipeline_stages_lower_to_external_realization_not_unparsed` derives stage names from `pipeline_compile_order_stage_names()` (same authority as bootstrap).

**Landing: DB-16 refined-generic substitution (S, PR #522).** Design: [design-db16-refined-generic-substitution.md](../../docs/design-db16-refined-generic-substitution.md) (R3 — unified construction authority, post-codex + post-chatgpt reviews). Materializes substituted-refined carriers at the phase boundary:
...
- **Generic-dependent `where` (lowering, PR #524).** `type_declaration_mentions_fn_type_parameter` rejects `where` when the parameter type still mentions one of the enclosing fn's type-parameter declarations (bare `T`, `List<T>`, …) with an explicit diagnostic — complementary to PR #522's infer-time materialization for substitutable refined carriers (`test_3a3_rejects_refinement_on_type_parameter`, `test_3a3_rejects_refinement_on_instantiation_with_type_param`).
...
**Deferral: `ComposedEffect` boundary cleanup (S).** PR #517 carried `ComposedEffect { operations, idempotent, breaking_operation }` forward as a temporary workflow summary shape. Acceptable only as a short-lived bridge. Before Track 17a REST wiring or Stage 2b workflow-idempotency consumers start depending on it, a follow-up PR must reshape `ComposedEffect` so workflow verdicts are structural rather than duplicated summary fields — at minimum drop redundant `idempotent`, or better encode "no breaker" versus "broken by operation X" in the type shape itself.

The thing we must not do is let Track 17a treat this convenience record as canonical by inertia. **Yellow-flag threshold:** this deferral must clear before any Track 17a consumer lands. Design anchor: [lane2-compile-time-proofs.md](../../docs/lane2-compile-time-proofs.md) Stage 2a / Stage 2b boundary note. No standalone DB yet; if cleanup needs a new durable carrier shape, write the DB in the follow-up PR instead of growing the scaffold ad hoc.

Cleared (this PR): `DerivedOpEffect { method, path_template, shape }` collapsed into `OperationEffect { operation_name, shape }`. The `method` / `path_template` fields were never consumed downstream — both the modifier check and obligation generator project through `shape` alone, and the `ReadEffect` variant already encodes "method was GET/HEAD/OPTIONS." `derive_op_effect` now returns `OperationEffect?` directly, so Stage 2b's `compose_effects` consumes the same shape derivation produces. Future diagnostic rendering that wants the originating method/path should attach a separate evidence carrier to the diagnostic, not smuggle transport facts onto the effect record.
...
**Deferral: 1c PR 2.5 — dissolve `PatternBindingRule` name-keyed recovery (S, blocks Python pilot).** PR 1 and PR 2 each implement `parse_pattern_binding_rule` by calling `named_variant_id(dag, "PatternBindingRule", "EmitBindingAlways")` et al. four times per emitter to recover `DeclarationId`s, then comparing constructor ids structurally. Typed at the compare site; name-keyed at the resolution site — the exact anti-pattern `feedback_substrate_principle_audit` Q5 ("construction authority: multiple call sites independently reconstructing the same fact") calls out. Recovery: cache typed `PatternBindingRuleVariants { emit_always, emit_underscore, emit_prefixed, not_applicable }` at bootstrap end alongside `SubstrateMarkers` / `RealizationMetas`; add `Dag::pattern_binding_rule_variants()` accessor; replace the name-lookup paths in `emit_rust.rs::parse_pattern_binding_rule` and `emit_go.rs::parse_pattern_binding_rule` with typed reads. Must land before the Python pilot so `emit_python.rs` never writes the name-keyed pattern a third time (dissolve before the consumer multiplies). **Yellow-flag threshold: 2 days.** Design anchor: PR #520 ChatGPT review ("I would merge this Go pilot, but I would not want Python to copy this pattern a second time") + `feedback_substrate_principle_audit` Q5.

**Deferral: 1c Python pilot (M).** Stage 1c PR 1 shipped the Rust pilot (`PatternBindingRule::EmitUnderscoreWhenUnused`), the `CleanEmissionContract` type system (8 rules in `src/v3/std/clean_emission.dag`), `rust_clean_emission` in `spec/rust.dag`, and E-5 in `INVARIANTS.md`. PR 2 shipped the Go pilot (`go_clean_emission` in `spec/go.dag`; `emit_go::render_sum_branch` elides unused `{name} := v.field;` prefixes and drops `v :=` when no arm binds). **Prerequisite: PR 2.5 must land first** — `emit_python.rs::parse_pattern_binding_rule` reads the typed `PatternBindingRuleVariants` cache, not the name-keyed lookup Rust + Go are currently using. Remaining scope for the Python pilot itself: add `python_clean_emission` and wire `emit_python`'s pattern-bind path. Split from the Go pilot because Rust's `_` lives inside the pattern and Go's elision lives at the statement level, while Python's pattern-bind does not emit the binding as part of the pattern at all — a structurally distinct surface, not an isomorphic mirror. Size M rather than S because Python is the first target where the contract rule shape has to generalize beyond "same pattern slot, different symbol"; a contract-shape change here (rather than a new instantiation) is evidence the contract over-fit the Rust+Go surface. **Yellow-flag threshold: 1 week** (clock starts when PR 2.5 merges). Design: [phase1-lane2-clean-emission-invariant.md §Scope split](../../docs/phase1-lane2-clean-emission-invariant.md).
