...
**Follow-ups opened by claude-review on #537** (tracked here per the "record substrate follow-ups as dissolution triggers" discipline):

1. **NSL lift for `ProductCost` / `SumCost` term lists.** `SymbolicCost::ProductCost(List<SymbolicCost>)` and `SymbolicCost::SumCost(List<SymbolicCost>)` admit `[]` and singleton inputs at construction time; `normalize` reduces both back to scalar variants, so the post-normalize canonical shape has `len ≥ 2`. Lift the field type to `NonSingletonList<SymbolicCost>` (Track 9 vocabulary) so the `≥ 2 elements` fact becomes structural. **Dissolution trigger:** the first call site that needs to pattern-match two guaranteed children without a `len() >= 2` check (likely a richer `normalize` variant or a multi-variable dominance rule).

2. **Typed degree for `PolynomialCost`.** `PolynomialCost { var, degree: Int }` — post-normalize, `degree >= 2` (degree = 1 collapses to `Linear`, degree = 0 to `Constant`). Replace `Int` with a typed carrier (`NonNegativeInt` at minimum; ideally `DegreeAtLeastTwo`) so the domain constraint is representable. **Dissolution trigger:** DB-7's degree-arithmetic surface (multi-variable polynomial composition, currently a follow-up on the lens side too).

3. **`LinearCost(v)` vs `PolynomialCost(v, 1)` canonicalization.** The two variants encode the same asymptotic fact; `dominates` and `normalize` pattern-match on both. Pick one canonical form — either dissolve `LinearCost` into `PolynomialCost(_, 1)`, or constrain `PolynomialCost.degree ≥ 2` so `Linear` is the sole degree-1 surface. **Dissolution trigger:** normalization grows a step that collapses every post-normalize `Linear` into `Polynomial(_, 1)` (or the converse), which makes the canonical form a structural invariant rather than a convention.

4. **`dominates` composite branches — .dag vs Rust divergence.** Rust `src/v3/compiler/src/dag.rs::dominates` correctly walks children for `ProductCost` / `SumCost` (fixed by the codex-flagged patch in this PR). The `.dag` authority `src/v3/std/algebra.dag::dominates` returns `False` conservatively for composites because v3's mutual-recursion termination analyzer (#519) only accepts cluster members that share a descent parameter type, and the list-iterating helper would descend on `List<SymbolicCost>` while `dominates` descends on `SymbolicCost` — different types, cluster can't form. **Dissolution trigger:** substrate extension letting a mutual-recursion cluster admit cross-type members with per-member structural-descent witnesses (list-head + list-tail descent alongside variant-payload descent). Likely a Lane 1e or later addition to the termination-proof surface.

5. **`build.rs` load-order priority list.** `src/v3/compiler/build.rs` prioritizes `list.dag` and `substrate.dag` ahead of alphabetical order so `structural_binding_info_for_variant` sees populated `Disj` connectives when downstream std files (Lane 2 Stage 2d's `algebra.dag` and the lens) are lowered. **Dissolution trigger:** termination analysis grows a type-readiness signal (phase-1 declaration with fully-populated variant-list, rather than phase-2-lowered body) so `structural_binding_info_for_variant` can succeed regardless of file order. Likely Lane 1e or later. Not urgent; the priority list is a one-line workaround that dissolves cleanly once the analyzer upgrades.
