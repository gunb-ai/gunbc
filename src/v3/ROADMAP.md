...
- **Predicate lowering.** `lower_parameter_refinement` (in `lower.rs`) lowers `SurfaceParam.refinement` to a predicate `Declaration` (`Arrow { inputs: [base], output: Bool, body: UserDefined(bind) }`) plus a refined type `Declaration` that aliases the base via `Atom(ResolvedIdentifier(base))` and carries `refinement: Some(pred)`. Threaded into `seed_function_signature`, `lower_fn_item_expr_body`, and `lower_fn_item_unparsed`.
- **Call-site discharge.** `infer::decide_transform`'s per-argument type loop calls `check_refinement_discharge` after structural equivalence passes. Compares the expected parameter's `refinement` edge against the actual argument port's top-level refinement using `predicates_structurally_equal` — a pure structural walk on the two predicate expression DAGs that pairs each side's `Bind.params[0]` as "the refined parameter" slot. No SMT, no entailment, no interning.
- **Arm-local narrowing.** `lower_expr`'s `If` arm runs `narrow_scope_for_predicate` on the cond. When the cond is a two-argument `Operator`/`Call` with **exactly one** scope-bound free variable (the candidate parameter), lowering rebuilds the then-arm's scope with that name pointing at a freshly-allocated narrowed port typed as a refinement of the outer type. Callers inside the then-arm whose callee expects the same predicate discharge structurally without an explicit guard. Multi-var predicates (e.g. `behavior_id(payload.head) == node_id` inside a match arm) skip narrowing — single-parameter refinement is the 3a.3 scope.
- **`signature_type_shape` stops at refinement carriers** so Arrow walks preserve the refined declaration id on the callee side; type equivalence still follows the `ResolvedIdentifier` alias to compare structurally.
- **Alias-chain discharge.** `check_refinement_discharge` walks the `Atom(ResolvedIdentifier(...))` chain of the actual argument's declaration so a narrowed already-refined param retains its outer refinement for discharge. Structural walk — the chain IS the conjunction.
...
Acceptance: `src/v3/compiler/tests/m2_feature_parity_test.rs::test_3a3_*` — acceptance suite now includes operator-shaped already-refined narrowing and structural-callable-identity-across-sites locks. Design: [design-m2-feature-parity.md §DB-11](../../docs/design-m2-feature-parity.md).

Remaining (blocking for ✅ Shipped): admitted surface vs supported fragment. `lower_parameter_refinement` turns any `SurfaceExpr` into a refinement predicate declaration, but `predicates_structurally_equal` only walks `Behavior::Value` and `Behavior::Transform`. A `where` expression that lowers through `Branch` (e.g. `where if cond then … else …`) is accepted at lowering and rejected at discharge as "not structurally equal" rather than "unsupported shape." Close by either rejecting out-of-fragment predicate shapes at lowering time or extending the walker. Cross-PR pattern: this is the same "lossy compression at substrate boundaries" shape catalogued in PR #511 (Cluster/IntraClusterCall) and PR #513 (ModifierCheck/EffectShape duplication).
