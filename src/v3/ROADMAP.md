...
| 3a.3 `where` refinement (DB-11, M→L overrun) | ✅ Shipped | PR #496 (foundation) + 3a.3-full follow-up | Predicate lowering, call-site structural-equality discharge, and arm-local narrowing all wired. See **Landing: 3a.3-full** below. |
...
Acceptance: `src/v3/compiler/tests/m2_feature_parity_test.rs::test_3a3_*` — six tests lock refined-parameter compile, literal-arg rejection, matching-refinement forwarding, distinct-refinement non-discharge, if-predicate narrowing, and non-narrowing call rejection. Substrate integrity test also locks `Behavior` at 5 variants. Design: [design-m2-feature-parity.md §DB-11](../../docs/design-m2-feature-parity.md).
