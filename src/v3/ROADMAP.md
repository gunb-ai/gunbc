...
**Cleared (this PR, DB-15 R2 impl):** `src/v3/std/verification.dag` now carries the R2 extensions: `TestClaim.requires: List<ResourceReference>`, the two new runtime-backed `TestPredicate` variants (`BehavioralObservation { subject, input_sample, expected_output }` and `MockBackedInvariant { subject, mock_transport, invariant }`), and `fn materialize_obligations(suite: TestSuite) -> List<TestClaim>` as the dependency-walk entry point. R2's three open questions resolved in-PR: (1) `ResourceReference` is a typed compile-time declaration sentinel (scaffold shape `{ identifier: String }` pending resources-in-v3 port, narrows to `resource`-only `DeclarationRef` post-port — not a runtime `ResourceHandle`); (2) `requires` is per-claim, not per-predicate-variant; (3) tautology avoidance is structurally enforced by the coproduct shape. `dsl/gunbc/compiler.dag`-level Stage 2c acceptance smoke at `src/v3/compiler/tests/lane2_stage_2c_verification_smoke.rs` locks each of the three answers + the materialization entry point + a user-authored suite round-trip. Design: [design-test-infra.md](../../docs/design-test-infra.md) R2.

**Remaining for Stage 2c kickoff (consumer side, separate PR):** a runner that reads `materialize_obligations(suite)` output, acquires each claim's `requires` resources, and executes the predicate. Out of scope for DB-15 per the R2 brief's consumer-contract deferral.

**Prerequisite deferral: `dsl/std/resources.dag` → v3 reconciliation (S).** Zero references to `Resource`/`acquire`/`release` under `src/v3/` today. `ResourceReference` is authored with a 🟡 scaffold marker carrying an `identifier: String` — when the port lands, it dissolves into a `DeclarationRef`-shape accepting only `resource` declarations (narrowing the accept set and removing the string carrier). Options: port declaration into `src/v3/std/resources.dag`, OR make `dsl/std/resources.dag` bootstrap-consumable by v3. Preferring the latter for single-authority. Separable from DB-15 implementation — can land independently. This is also a dissolution-of-dual-representation item; consider parking it in §Scheduled deletions if dsl/v3 duplication is the framing, or keep as a prerequisite deferral here. Preferring here for now since it's narrowly scoped.

**Known v3-compiler gap surfaced during R2 impl (not blocking Stage 2c kickoff):** user-code construction of a record whose field is a `List<T>` rejects the empty-list literal (`requires: []`) with an "implicit template binding for `TestClaim` conflicts" diagnostic, even though the bootstrap-loaded `data` form accepts `field: []` cleanly. Today the Stage 2c smoke tests sidestep this by authoring non-empty `requires` lists; generated `TestClaim` data declarations (via `lens_testgen.rs`) continue to use `requires: []` because they go through the `data` form. The gap is orthogonal to DB-15 — it is a v3 type-inference limitation on user-code empty list literals — and is not gating the Stage 2c consumer runner work.
