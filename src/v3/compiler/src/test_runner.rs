    ...
    fn eval_gate_87_cementing_projection(
        ...
        let computed =
            match lens_decl_name? {
                ...
                "gate87_parallelism_smoke_no_workflow_projection" => {
                    let Some(anchor) = program_dag.nodes().iter().find(|behavior| {
                        matches!(behavior, Behavior::Value(_) | Behavior::Bind(_))
                    }) else {
                        return Some(ClaimResult::Fail(format!(
                            "LensOutputEquals({lens_name}): no Value/Bind anchor node in `{}`",
                            file_name
                        )));
                    };
                    let report = analyze_parallelism(program_dag, anchor.id());
                    i64::from(matches!(
                        report,
