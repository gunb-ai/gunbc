...
impl<'a> TestRunner<'a> {
    ...
    fn eval_gate_87_cementing_projection(
        ...
        let computed = match lens_decl_name? {
            ...
            "gate87_parallelism_read_read_commutes" => {
                let Some(root) = program_dag
                    ...
                    return Some(ClaimResult::Fail(format!(
                        "LensOutputEquals({lens_name}): no root behavior found in `{file_name}`"
                    )));
                };
                ...
                if !dag.try_register_lane2_workflow_effect(root, workflow) {
                    return Some(ClaimResult::Fail(format!(
                        "LensOutputEquals({lens_name}): failed to register workflow effect at `{root:?}`"
                    )));
                }
                i64::from(matches!(
