...
impl<'a> TestRunner<'a> {
    ...
    fn eval_lens_output_equals(
        ...
        // T-LaneE: `ProgramOutputBind` structurally selects the generated cost lens over a named
        // output bind from the claim program. Lens identity is still the typed `DeclarationRef`
        // accepted above; dispatch no longer keys on the declaration's spelling.
            ...
            .and_then(ProgramInputRole::output_bind_name)
        {
            let Some(bind) = find_bind(&program_dag, cost_bind, &claim.file_name) else {
                return ClaimResult::Fail(format!(
                    "LensOutputEquals: output bind `{cost_bind}` not found in `{}`",
                    claim.file_name
                ));
            };
            ...
            let expected_int = match expected_decl.value_body.as_ref() {
                Some(ValueBody::Scalar(LiteralBits::Int(s))) => match s.parse::<i64>() {
                    ...
                    Err(_) => {
                        return ClaimResult::Fail(format!(
                            "LensOutputEquals: expected Int literal is not a valid i64 decimal for `{expected_name}`"
                        ));
                    }
                },
                _ => {
                    return ClaimResult::Fail(format!(
                        "LensOutputEquals: ProgramOutputBind expected_ref `{expected_name}` must be `data …: Int = <literal>` (M1(2.8); `Lookup<Int>` data literals are deferred)"
                    ));
                }
            };
            return match computed {
                ...
                CostLookup::Hit(v) => ClaimResult::Fail(format!(
                    "LensOutputEquals: expected `{expected_int}`, computed `{v}` for bind `{cost_bind}`"
                )),
                CostLookup::Miss => ClaimResult::Fail(
                    "LensOutputEquals: computed cost is Miss (malformed program)".to_string(),
                ),
            };
        }
        ...
        let input_field = if reflects_claim_program {
            // P2: reflected `List` / `Behavior` variant ids come from `self.dag`, the same graph
            // that owns `lens_id` and is passed to `apply_lens_declaration` below.
