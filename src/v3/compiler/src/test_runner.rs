    ...
    fn eval_gate_87_cementing_projection(
        ...
        let computed = match lens_decl_name? {
            ...
            "gate87_parallelism_no_workflow_projection" => {
                let Some(anchor) = first_value_or_bind_node(program_dag) else {
                    return Some(ClaimResult::Fail(format!(
                        "LensOutputEquals({lens_name}): no Value/Bind node found in `{file_name}`"
                    )));
                };
                i64::from(matches!(
                    analyze_parallelism(program_dag, anchor),
...
fn first_value_or_bind_node(dag: &Dag) -> Option<NodeId> {
    dag.nodes()
        .iter()
