...
///   the kernel `Bool` sum authored directly in `dsl/std/types.dag`.
///   The walk follows `Declaration.inhabits` when the connective chain
///   hits a `Disj` without an inline algebra.
