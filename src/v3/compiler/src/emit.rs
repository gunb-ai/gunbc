                ...
                match arguments.as_slice() {
                    ...
                    [key, value, extra] => {
                        ...
                        let value_name =
                            self.go_type_name_for_decl_at_depth(value.value, depth + 1)?;
                        let extra_name =
                            self.go_type_name_for_decl_at_depth(extra.value, depth + 1)?;
                        Ok(render_named_template(
                            &binding.carrier,
                            &[
                                ("key", &key_name),
                    ...
                    _ => Err(EmitError::UnsupportedBehavior(
                        "instantiated type carrier only supports arities 1, 2, and 3".to_string(),
                    )),
                }
            }
            TypeConnective::Cardinality(p) if p.bound() == CardinalityBound::AtMostOne => {
...
// PB-0 cycle 3 census consolidation — nested `process_exit` (see PR body receipts).
...
// PB-0 cycle 3 census consolidation — nested `omni_shape_b_openapi` (see PR body receipts).
