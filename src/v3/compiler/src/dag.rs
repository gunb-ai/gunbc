...
// Termination carriers derived from `src/v3/std/termination.dag`.
//
// The `.dag` declarations are the carrier and lattice-operation authority.
// `dag.rs` intentionally keeps only Rust carrier shapes needed by current
// consumers; executable lattice operations such as `merge_evidence` and
// `join_evidence` must stay in `std.termination`.
