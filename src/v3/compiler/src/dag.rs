...
mod computation {
    ...
    //! lived inline in `dag.rs`; this nested `mod computation` keeps them separate from
    //! the E-P induction producer in the parent module without adding a new SG-0
    //! hand-authored file path. This is **not** R3 gate
...
// `DescentEvidence` and related termination **carriers** stay on the native DAG
// for E-P / lens typing. Lattice **operations** (`merge_evidence`, …) are not
// implemented here — R3 gate `tier3_termination_mirror_dissolved`:
// `dsl/std/termination.dag` is the sole authority; executable bridge until std
// bodies lower lives in `std_termination_lattice_host`.
