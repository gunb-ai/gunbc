...
const HEADER: &str = "// AUTO-GENERATED from `src/v3/lenses/structural_resolution.dag` via\n\
...
fn main() {
    ...
    let source = std::fs::read_to_string(&lens_path).expect("read structural_resolution.dag");
    ...
    const OUT_REL: &str = "src/v3/compiler/src/lens_structural_resolution_generated.rs";
    assert!(
    ...
    std::fs::write(&out_path, &formatted).expect("write lens_structural_resolution_generated.rs");
