...
const HEADER: &str = "// AUTO-GENERATED from `src/v3/lenses/complexity.dag` via\n\
...
fn main() {
    let lens_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("lenses")
        .join("complexity.dag");
    let source = std::fs::read_to_string(&lens_path).expect("read complexity.dag");
    let dag = compile_to_dag(&source, lens_path.to_string_lossy().as_ref())
        .expect("complexity.dag compiles");
    ...
    const OUT_REL: &str = "src/v3/compiler/src/lens_cost_generated.rs";
    assert!(
