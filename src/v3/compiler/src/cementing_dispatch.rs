...
use crate::test_runner::{ClaimResult, TestClaimValue};
...
/// Evaluate `CementingDispatchMatchesProjection` for `claim` against `dag` (fixture + bootstrap).
pub(crate) fn evaluate_cementing_dispatch_projection(
    ...
    if !claim.declaration_file.ends_with(CEMENTING_DISPATCH_DECL_PATH) {
        return ClaimResult::Fail(format!(
            "CementingDispatchMatchesProjection is only accepted for claims declared in `**/{CEMENTING_DISPATCH_DECL_PATH}`; got `{}`",
    ...
    for row in &capability_rows {
        ...
        let behavioral = match record_field(fields, "behavioral") {
            ...
            None => return ClaimResult::Fail("capability row missing `behavioral` field".to_string()),
        };
        let v2 = match record_field(fields, "v2_counterpart") {
            ...
            None => return ClaimResult::Fail("capability row missing `v2_counterpart` field".to_string()),
        };
    ...
    for row in &receipt_rows {
        ...
        let kind_val = match record_field(fields, "kind") {
            Some(v) => v,
            None => return ClaimResult::Fail("cementing receipt row missing `kind`".to_string()),
        };
    ...
    if receipt_triples.len() != receipt_rows.len() {
        return ClaimResult::Fail(
            "internal error: receipt triple set size diverged from row count".to_string(),
        );
    }
    ...
    for row in &receipt_rows {
        let fields = record_fields(row).expect("rows validated above");
        let registry_name = string_field(fields, "registry_name").expect("validated");
        let module_stem = string_field(fields, "module_stem").expect("validated");
        let kind = nullary_variant_name(dag, record_field(fields, "kind").expect("validated"))
            .expect("validated");
        match kind.as_str() {
            "DagHarness" => {
