...
use crate::parse_tables::{
    ...
    primary_prefix_dispatch, soft_keyword_ident_spelling, top_level_item_dispatch, BinaryOpLevel,
...
//
// **SG-2c-1 reduction (not retirement).** The per-precedence-level functions below
...
// `parse_tables.dag` and consumed here via `binary_op_at_level`. The SG-2c
// `parse_primary` cluster: SG-2c-6 `PrimaryPrefixRow` + `primary_prefix_dispatch`
// (prefix openers) and SG-2c-7 `PrimaryAtomRow` + `primary_atom_class` (int/bool/
// string/lambda/ident atomic forms). The parser's control flow — cursor mechanics,
// recursive descent, error recovery, span fusion — remains semantic authority in
// this file.
//
...
// grammar-as-data) and `regen_parse` emits the full module without any external
// Rust body fragment. SG-2c proper is blocked on a named substrate capability —
// recursive list-body emission over `List<Token>` with cursor threading — declared
// in `src/v3/std/list.dag:13-15` ("the current compiler still lacks full structural
...
impl<'a> Parser<'a> {
    ...
    /// separated by `|`, or a **single** variant without a top-level `|` when
    /// it opens as `Variant { ... }` / `Variant(...)` — PR-A.3 EvalStrategy
    /// carrier) and TypeAlias (a single type expression). A variant looks like
    /// `Ident`, `Ident(...)`, or `Ident { ... }`. A type expression can start
    /// with `Ident<...>` (parameterized), `fn(...)` (arrow), or a bare `Ident`
    /// at RHS end (alias to another type — **not** a nullary sum; use
    /// `Variant()` for a single nullary sum variant without `|`).
    ///
    ...
    /// `parse_expr`).
    ///
    /// **Legacy `dsl/std/types.dag` constraint spellings** (`pattern("…")`,
    /// `range(min: 1, max: 5)`, `brand("…")`, bare `non_empty`, …) share this
    /// same expression surface — there is no parallel implicit-subject
    /// micro-grammar: they must tokenize as `SurfaceExpr` so SG-2 can parse
    /// the std corpus. `parse_call_args` recognizes `f(a: x, b: y)`-shaped
    /// opens so inner commas in `range(min: …, max: …)` are not mistaken for
    /// top-level `where` segment separators (they desugar to a single
    /// `Record` argument). The same path covers **one** bound, e.g.
    /// `range(min: 1)` (still `Ident` + `:` after `(`). SG-2
    /// `handwritten_parser_accepts_types_dag` tokenizes and parses the full
    /// `dsl/std/types.dag` corpus; PB-1’s committed `bootstrap_std_generated.rs`
    /// snapshot carries **no** parse-phase `Diagnostic` rows for that file.
    /// **Lowering** does not emit `Bool` predicate bodies for
    /// `dsl/std/types.dag` until helpers resolve (see
    /// `lower_type_alias_refinements_phase` / `alloc_deferred_types_dag_refinement_placeholder`
    /// in `lower.rs`): a **placeholder** `Declaration::refinement` carries the
    /// fact that a `where` was authored (P3 — not a silent `None`), while PB-1
    /// stays diagnostic-clean.
    ...
    /// Lookahead: after `=`, is the RHS a sum? True when (a) a top-level `|`
    /// exists before the next item boundary, or (b) the RHS opens as
    /// `Variant {` / `Variant(` at depth zero (single-variant sum without `|`).
    /// Tracks paren/brace depth so a `|` inside a payload list doesn't confuse
    /// the scan. [`Self::parse_sum_variants`] skips a **leading** run of `|`
    /// tokens so `| A | B` and `A | B` both parse.
    ///
    ...
    fn rhs_is_sum(&self, local_alias_reference_idents: &[String]) -> bool {
        ...
        while i < self.tokens.len() {
            if let Some(role) = bracket_role(&self.tokens[i].kind) {
                match role {
                    BracketRole::Opener => {
                        depth += 1;
                        i += 1;
            ...
            match &self.tokens[i].kind {
                ...
                TokenKind::KwWhere | TokenKind::Eof if depth == 0 => return false,
    ...
    // Per-precedence-level functions below are lowest-to-highest binding
    // (Or → And → Comparison → Additive → Multiplicative). The token →
    // `OperatorKind` map for each level is authored declaratively in
    // `parse_tables.dag` and consumed via `binary_op_at_level`; this file
    // owns control flow (chaining, non-chaining, span fusion) only.

    fn parse_logical_or(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        let mut lhs = self.parse_logical_and()?;
        while let Some(op) = binary_op_at_level(&self.peek().kind, BinaryOpLevel::Or) {
            self.bump();
    ...
    fn parse_logical_and(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        ...
        while let Some(op) = binary_op_at_level(&self.peek().kind, BinaryOpLevel::And) {
            self.bump();
    ...
    fn parse_comparison(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        ...
        if let Some(op) = binary_op_at_level(&self.peek().kind, BinaryOpLevel::Comparison) {
            self.bump();
    ...
    fn parse_additive(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        ...
        while let Some(op) = binary_op_at_level(&self.peek().kind, BinaryOpLevel::Additive) {
            self.bump();
    ...
    fn parse_term(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        ...
        while let Some(op) = binary_op_at_level(&self.peek().kind, BinaryOpLevel::Multiplicative) {
            self.bump();
    ...
    fn parse_primary(&mut self) -> Result<SurfaceExpr, Diagnostic> {
        ...
        // Map literals share the `{` prefix with record literals. Top-level `data`
        // disambiguates via `looks_like_map_literal` before `parse_record_literal`;
        // mirror that here so nested `Map<String, _>` fields (e.g. GitHub Actions
        // `with:`) can use string keys like `"fetch-depth"`.
        if matches!(peek, TokenKind::LBrace) && self.looks_like_map_literal() {
        ...
        match (cls, token.kind) {
            ...
            (PrimaryAtomClass::True, TokenKind::KwTrue) => Ok(SurfaceExpr::Literal {
                value: SurfaceLiteral::Bool(true),
                span: token.span,
            }),
            (PrimaryAtomClass::False, TokenKind::KwFalse) => Ok(SurfaceExpr::Literal {
                value: SurfaceLiteral::Bool(false),
                span: token.span,
            }),
            (PrimaryAtomClass::String, TokenKind::StringLit(value)) => Ok(SurfaceExpr::Literal {
                value: SurfaceLiteral::String(value),
                span: token.span,
            }),
            (PrimaryAtomClass::Pipe, TokenKind::Pipe) => self.parse_lambda(token.span),
            (PrimaryAtomClass::Ident, TokenKind::Ident(name)) => {
                self.parse_ident_expr(name, token.span)
            }
            (class, other) => Err(Diagnostic::ParseError {
                message: format!(
                    "internal: primary-atom class {class:?} does not match `TokenKind` {other:?} \
