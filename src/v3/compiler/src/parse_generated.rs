    ...
    fn parse_fn_item(&mut self) -> Result<SurfaceItem, Diagnostic> {
        ...
        match &self.peek().kind {
            ...
            TokenKind::LBrace => {
                ...
                //
                // When `{` begins an unambiguous record literal (`{}` or
                // `{ label: expr, ... }`), parse `SurfaceExpr::Record` so
                // `{ x: 1 }` returns remain expressible.
                //
                // Otherwise try a single complete `SurfaceExpr` through the
                // closing `}`. If parsing fails or the expression does not
                // exhaust the brace (e.g. `let`-sequence std bodies such as
                // `effects.dag` `parse_path_template`), fall back to
                // `FnExternalBody` + brace-skip so signatures and body spans
                // still flow forward (Facts Flow Forward / staged std).
