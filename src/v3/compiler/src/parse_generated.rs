...
impl<'a> Parser<'a> {
    ...
    /// **Lowering** defers those refinements for
    /// `dsl/std/types.dag` until std defines the helpers (see
    /// `lower_type_alias_refinements_phase` in `lower.rs`), which keeps PB-1
    /// bootstrap snapshots diagnostic-clean — the “broken bootstrap artifact”
    /// hazard from an earlier snapshot pass is closed on current `main`.
