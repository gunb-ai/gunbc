    ...
    fn parse_fn_item(&mut self) -> Result<SurfaceItem, Diagnostic> {
        ...
        match &self.peek().kind {
            ...
            TokenKind::LBrace => {
                ...
                //
                // Otherwise treat `{ ... }` as a single-expression block:
                // `match`, `if`, calls, bare variant constructors, etc.
                // Multi-statement blocks (`let` sequences) are still out of
                // scope — they fail at parse time rather than lowering to
                // `FnExternalBody` / `ArrowBody::Unparsed`.
