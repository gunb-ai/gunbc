...
pub mod evaluator {
        ...
        match &t.target {
            ...
            TransformTarget::Callable(callee_decl) => {
                ...
                if let TypeConnective::Arrow { body, .. } = connective {
                    if let Some(value) = try_dispatch_emit_host_go(dag, callee_decl, &operands) {
                        return value;
                    }
    ...
    fn try_dispatch_emit_host_go(
        ...
        let source = match expect_string_literal(&operands[1]) {
            Ok(source) => source,
    ...
    fn expect_string_literal(value: &Value) -> Result<&str, EvalError> {
        match value {
            Value::LiteralValue(LiteralBits::String(s)) => Ok(s),
            _ => Err(EvalError::BadTransformOperands {
                reason: "expected String literal",
            }),
        }
    }
    ...
    /// transport row. It does not introduce target semantics; it projects the existing typed
    /// `emit_host_runner` receipt into evaluator `Value` until T-22 can dispatch host transports
    /// directly from generated `.dag` eval and this helper dissolves with the surrounding
    /// `emit_host_bridge` hand-Rust.
