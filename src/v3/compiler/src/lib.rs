...
=======
        ...
        const DIRECT_DAG_FILE: &str = "lens_cost.unit";
        ...
        fn expect_cost(dag: &Dag, port: PortId) -> usize {
            match cost_of(dag, &port) {
                CostLookup::FoundCost { _0: n } => {
                    usize::try_from(n).expect("cost should be non-negative")
                }
                CostLookup::MissingCost => panic!("missing cost for port {port:?}"),
            }
        }
        ...
        fn kf_1_branch_cost_is_max_not_sum() {
            ...
            assert_eq!(baseline_cost, larger_non_max_cost);
            assert!(larger_max_cost > baseline_cost);
        }
        ...
        fn kf_1_non_max_branch_does_not_inflate_cost() {
            ...
            assert_eq!(baseline_cost, more_non_max_work_cost);
            assert!(more_max_work_cost > baseline_cost);
>>>>>>> d71635033 (WIP: Worker 7)
        }
    }
}
