...
/// Regenerated `tokenize_generated.rs` body: header + `rustfmt` stdout, no disk write.
///
/// Used by `regen_tokenize` CLI and SG-1 authority tests (in-process; avoids nested Cargo).
pub fn tokenize_generated_rust_formatted(manifest_dir: &std::path::Path) -> String {
    ...
    rustfmt_stdout(&combined)
}

/// Single-authority gate + write path for the `regen_tokenize` binary.
pub fn write_tokenize_generated_rs(manifest_dir: &std::path::Path) {
    // Single-authority gate: the output path this driver writes must
    // be registered in `REGEN_OUTPUTS` (surfaced as
    // `generated_files::GENERATED_FILES`). SG-0 treats that
    ...
    let formatted = tokenize_generated_rust_formatted(manifest_dir);
...
fn rustfmt_stdout(combined: &str) -> String {
...
fn emit_tokenize_fn(
    ...
    s.push_str("                correction: crate::diagnostics::Correction::deferred_for_diagnostic_class(\"legacy diagnostic class\"),\n");
    s.push_str("            })?;\n");
