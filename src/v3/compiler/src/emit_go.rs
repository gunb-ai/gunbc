...
pub fn emit_go(dag: &Dag) -> Result<String, EmitError> {
    emit_go_with_mode(dag, EmitGoMode::Program)
}

pub fn emit_go_module(dag: &Dag) -> Result<String, EmitError> {
    emit_go_with_mode(dag, EmitGoMode::Module)
}

fn emit_go_with_mode(dag: &Dag, mode: EmitGoMode) -> Result<String, EmitError> {
    ...
    if mode == EmitGoMode::Program && top_level_binds.is_empty() {
        return Err(EmitError::UnsupportedBehavior(
            "emit_go requires at least one top-level value Bind".to_string(),
        ));
    }
    if mode == EmitGoMode::Module && !top_level_binds.is_empty() {
        return Err(EmitError::UnsupportedBehavior(
    ...
    let mut sections = vec![if mode == EmitGoMode::Program {
        "package main".to_string()
    } else {
        "package emitted".to_string()
    }];
    if mode == EmitGoMode::Program {
