...
fn main() {
    ...
    // **Build the unshare-bootstrap helper binary on Linux** so that
    // `cargo test -p v3-compiler` (and `cargo build`) make it available at
    // `target/<profile>/gunbc_execute_command_bootstrap` for the
    // `test_runner.rs::locate_unshare_bootstrap_helper` runtime lookup.
    //
    // Without this, `cargo test -p v3-compiler` would not pull
    // `execute-command-bootstrap` into its build graph (cargo only builds
    // declared dependencies, and bin-only crates can't be added as `[dependencies]`
    // on stable), and every `evaluate_execute_command_*` call on Linux would
    // fail-closed with `SetupFailed { helper binary not found }`. The recursive
    // `cargo build -p execute-command-bootstrap` is package-scoped, so cargo's
    // per-package lockfile handles concurrent builds without deadlock.
    //
    // See PR #1049 / issue #856 for the helper substrate context.
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("linux") {
        ...
        let helper_manifest = repo_root
            .join("src")
            .join("v3")
            .join("execute_command_bootstrap")
            .join("Cargo.toml");
        println!(
            "cargo:rerun-if-changed={}",
            helper_manifest
        ...
        // Inherit the workspace target dir if set; otherwise let cargo pick the default.
        if let Ok(target_dir) = env::var("CARGO_TARGET_DIR") {
            cmd.env("CARGO_TARGET_DIR", target_dir);
        }
        let status = cmd.status().unwrap_or_else(|e| {
            panic!("failed to invoke cargo to build execute-command-bootstrap: {e}")
        });
        if !status.success() {
            panic!(
                "cargo build -p execute-command-bootstrap exited with {}",
                status
            );
        }
    }
