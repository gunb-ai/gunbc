...
/// `dag/` + thin `[[bin]]` shims) from checked-in templates under `tools/pb0_cycle6_emit_templates/`.
///
/// **Templates** live at the workspace root (outside `src/v3/compiler/`, so they stay outside the
/// SG-0 `.rs` walk). **Checked-in outputs** are tracked like other `REGEN_OUTPUTS` modules.
