...
///
/// Dissolution: edit files under `pb0_cycle4_emit_templates/` (next to this build script) and
/// rebuild; do not hand-edit `*_generated.rs` outputs.
fn emit_pb0_cycle4_host_authority_modules(repo_root: &Path, manifest_path: &Path) {
    ...
    let tmpl_dir = manifest_path.join("pb0_cycle4_emit_templates");
    let specs: &[(&str, &str)] = &[
        (
        ...
        (
            "emit_collection_ops_method_contract.rs",
...
fn main() {
    ...
    emit_pb0_cycle4_host_authority_modules(repo_root, &manifest_path);
    ...
    const REGEN_OUTPUTS: &[&str] = &[
        ...
        // `pb0_cycle4_emit_templates/`, emitted by `emit_pb0_cycle4_host_authority_modules`.
        "src/v3/compiler/src/complexity_lattice_generated.rs",
