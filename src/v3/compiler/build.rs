...
fn collect_rust_files(root: &Path, workspace_root: &Path, out: &mut Vec<String>) {
    let entries = fs::read_dir(root).unwrap_or_else(|e| panic!("read_dir {}: {e}", root.display()));
    for entry in entries {
        let path = entry.expect("read_dir entry").path();
        if path.is_dir() {
            collect_rust_files(&path, workspace_root, out);
...
fn main() {
    ...
    println!(
        "cargo:rerun-if-changed={}",
        repo_root.join("src/v3/compiler/tests").display()
    );
    ...
    let mut test_regen_outputs = Vec::new();
    collect_rust_files(
        &repo_root.join("src/v3/compiler/tests"),
        repo_root,
        &mut test_regen_outputs,
    );
    test_regen_outputs.sort();

    let mut all_regen_outputs: Vec<&str> = REGEN_OUTPUTS.to_vec();
    all_regen_outputs.extend(test_regen_outputs.iter().map(String::as_str));
    all_regen_outputs.sort();
    ...
    for path in &all_regen_outputs {
        manifest.push_str(&format!("    \"{path}\",\n"));
    }
    manifest.push_str("];\n\n");
    manifest.push_str("pub static GENERATED_RUST_TEST_FILES: &[&str] = &[\n");
    for path in &test_regen_outputs {
        manifest.push_str(&format!("    \"{path}\",\n"));
    }
    manifest.push_str("];\n\n");
    manifest.push_str("pub static GENERATED_RUST_TEST_FILE_BYTES: &[(&str, &str)] = &[\n");
    for path in &test_regen_outputs {
