...
///
/// **Templates** live under `tools/pb0_cycle4_emit_templates/*.rs.in` (workspace root — not under
/// `src/v3/compiler/`, so they are outside the SG-0 `.rs` walk and not misread as hand-authored
/// compiler sources). **Full substrate dissolution** (`.dag` / lens-producer authority replacing
...
fn emit_pb0_cycle4_host_authority_modules(repo_root: &Path) {
    ...
    let specs: &[(&str, &str)] = &[
        ...
        (
            "emit_rust_bin_shim.rs.in",
            "src/v3/compiler/src/emit_rust_bin_shim_generated.rs",
        ),
        (
            "enforced_lens_application.rs.in",
    ...
    for (tmpl_name, rel_out) in specs {
        let src_path = tmpl_dir.join(tmpl_name);
        println!("cargo:rerun-if-changed={}", src_path.display());
        let body = fs::read_to_string(&src_path).unwrap_or_else(|e| {
            panic!(
                "emit_pb0_cycle4_host_authority_modules: read template {}: {e}",
                src_path.display()
            )
        });
        ...
        if let Some(parent) = out_path.parent() {
            fs::create_dir_all(parent).unwrap_or_else(|e| {
                panic!(
                    "emit_pb0_cycle4_host_authority_modules: mkdir {}: {e}",
                    parent.display()
                )
            });
        }
        ...
        if needs_write {
            fs::write(&out_path, generated).unwrap_or_else(|e| {
                panic!(
                    "emit_pb0_cycle4_host_authority_modules: write {}: {e}",
                    out_path.display()
                )
            });
        }
...
/// Emit PB-0 cycle-6 `REGEN_OUTPUTS` modules (former `EXPECTED_HAND_AUTHORED_NON_TEST` paths under
/// `dag/` + `[[bin]]` shims) from checked-in templates under `tools/pb0_cycle6_emit_templates/`.
///
/// **Why templates sit at the workspace root.** Same hygiene as [`emit_pb0_cycle4_host_authority_modules`]:
/// paths under `tools/pb0_cycle6_emit_templates/*.rs.in` are **outside** `src/v3/compiler/`, so the
/// SG-0 `.rs` walk does not misclassify template sources as hand-authored compiler Rust (INVARIANTS
/// P2 / census partition honesty).
///
/// **P5 Progress Is Dissolution — Dispatch mechanism (b).** This `build.rs` hook is not a silent
/// authority migration: it lands in the **same PR** as the `sg0_census_test.rs` net shrink for the
/// six retired `NON_TEST` lines and the GitHub PR **§5.1** per-path `(b)/(c)` table (paired dispatch
/// per `INVARIANTS.md#p5` and `ROADMAP.md` SG-0 PR-window discipline).
///
/// **ROADMAP / program anchors.** Bounded PB-0 retirement track: `ROADMAP.md` **T-PB-A** (non-test
/// census toward **0** per `docs/design-pure-bootstrap-zero.md`), worker program
/// `docs/briefs/r3-pb0-non-test-retirement-worker.md`, and taxonomy visibility
/// `docs/audit/r3-pb0-non-test-retirement-class-taxonomy-2026-05-13.md` §1.1 / §3 class **(b)** rows
/// for the dag + bin surfaces named in the Director dispatch.
///
/// **Checkable dissolution triggers (interim vs terminal).** **Interim** receipts today: listed in
/// `REGEN_OUTPUTS` in this file (reviewed literal), `cargo:rerun-if-changed` on each template path,
/// and SG-0 tests `sg0_v3_hand_authored_census` + `sg0_generated_partition_is_producer_owned` (generated
/// Rust never masquerades as hand-authored). **Terminal** dissolutions: **dag/** bodies fold into
/// PB-Substrate / T-LP emitted Rust from the owning `.dag` substrate (not a parallel handwritten
/// module — see `docs/design-pure-bootstrap-zero-audit.md` builder/ports/effects rows). **`gunbc-ci`**
/// bin template dissolves when T-WAD BinaryShim + `dsl/gunbc/ci_emission.dag` execute the workflow gate
/// matrix without this host process edge. **`r1c_e_emit_gates`** bin template dissolves on T-Emit / R1
/// closure when the `.dag` harness no longer needs a host `ExecuteCommand` child for these emit gates
/// (ROADMAP T-Emit lane; `src/v3/compiler/src/r1c_e_gates.rs` module banner).
///
/// **Checked-in outputs:** like other `REGEN_OUTPUTS` modules — refresh committed `*_generated.rs` in
/// the same PR as `.rs.in` edits. Do not hand-edit generated shims; edit templates and rebuild.
fn emit_pb0_cycle6_regenerated_modules(repo_root: &Path) {
    const HEADER: &str = concat!(
    ...
    let tmpl_dir = repo_root.join("tools/pb0_cycle6_emit_templates");
    let specs: &[(&str, &str)] = &[
        (
            "bin_gunbc_ci.rs.in",
            "src/v3/compiler/src/bin/gunbc_ci_generated.rs",
        ),
        (
            "bin_r1c_e_emit_gates.rs.in",
            "src/v3/compiler/src/bin/r1c_e_emit_gates_generated.rs",
        ),
        (
            "dag_builder.rs.in",
            "src/v3/compiler/src/dag/builder_generated.rs",
        ),
        (
            "dag_cardinality_payload.rs.in",
            "src/v3/compiler/src/dag/cardinality_payload_generated.rs",
        ),
        (
            "dag_effects.rs.in",
            "src/v3/compiler/src/dag/effects_generated.rs",
        ),
        (
            "dag_ports.rs.in",
            "src/v3/compiler/src/dag/ports_generated.rs",
        ),
    ];
    for (tmpl_name, rel_out) in specs {
        let src_path = tmpl_dir.join(tmpl_name);
        ...
        let body = fs::read_to_string(&src_path).unwrap_or_else(|e| {
            panic!(
                "emit_pb0_cycle6_regenerated_modules: read template {}: {e}",
                src_path.display()
            )
        });
        let out_path = repo_root.join(rel_out);
        if let Some(parent) = out_path.parent() {
            fs::create_dir_all(parent).unwrap_or_else(|e| {
                panic!(
                    "emit_pb0_cycle6_regenerated_modules: mkdir {}: {e}",
                    parent.display()
                )
            });
        }
        ...
        if needs_write {
            fs::write(&out_path, generated).unwrap_or_else(|e| {
                panic!(
                    "emit_pb0_cycle6_regenerated_modules: write {}: {e}",
                    out_path.display()
                )
            });
        }
...
fn main() {
    ...
    emit_pb0_cycle4_host_authority_modules(repo_root);
    emit_pb0_cycle6_regenerated_modules(repo_root);
    ...
    println!(
        "cargo:rerun-if-changed={}",
        repo_root.join("tools/pb0_cycle4_emit_templates").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo_root.join("tools/pb0_cycle6_emit_templates").display()
    );
