...
fn read_rust_test_generator_manifest(path: &Path) -> Vec<String> {
    ...
    for (idx, raw_line) in source.lines().enumerate() {
        ...
        assert!(
            line.starts_with("src/v3/compiler/tests/") && line.ends_with(".rs"),
            "Rust test generator manifest {} line {} must be a workspace-relative test .rs path, got `{line}`",
            path.display(),
            idx + 1
        );
        if let Some(prev) = paths.last() {
            assert!(
                prev.as_str() < line,
                "Rust test generator manifest {} must be sorted and unique; `{prev}` is not before `{line}`",
                path.display()
            );
        }
...
fn main() {
    ...
    println!(
        "cargo:rerun-if-changed={}",
        rust_test_generator_manifest.display()
    );
    ...
    let test_regen_outputs = read_rust_test_generator_manifest(&rust_test_generator_manifest);
    ...
    manifest.push_str("];\n\n");
    manifest.push_str("pub static RUST_TEST_GENERATOR_MANIFEST: &[&str] = &[\n");
