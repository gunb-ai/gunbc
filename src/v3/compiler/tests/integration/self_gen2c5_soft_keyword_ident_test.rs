//! SG-2c-5 — parser-owned soft-keyword name table extraction.

use v3_compiler::parse_surface::{SurfaceExpr, SurfaceItem, VariantPayload};

#[test]
fn parser_accepts_kw_type_as_generated_soft_keyword_name_alias() {
    let source = "\
type Example = Plain | type { type: Int }\n\
data resource: Example = { type: 1 }\n";
    let tokens = v3_compiler::tokenize_for_test(source, "self_gen2c5_soft_keyword_ident.v3")
        .expect("tokenize fixture");
    let parsed = v3_compiler::parse_for_test(&tokens, "self_gen2c5_soft_keyword_ident.v3")
        .expect("parse fixture");

    match &parsed.items[0] {
        SurfaceItem::TypeSum { variants, .. } => {
            assert_eq!(variants.len(), 2, "expected two variants");
            assert_eq!(variants[1].name, "type");
            match &variants[1].payload {
                VariantPayload::Record(fields) => {
                    assert_eq!(fields.len(), 1, "expected single record field");
                    assert_eq!(fields[0].name, "type");
                }
                other => panic!("expected record payload, got {other:?}"),
            }
        }
        other => panic!("expected first item to parse as TypeSum, got {other:?}"),
    }

    match &parsed.items[1] {
        SurfaceItem::Data { body, .. } => match body {
            Some(SurfaceExpr::Record { fields, .. }) => {
                assert_eq!(fields.len(), 1, "expected single data record field");
                assert_eq!(fields[0].name, "type");
            }
            other => panic!("expected data body record literal, got {other:?}"),
        },
        other => panic!("expected second item to parse as Data, got {other:?}"),
    }
}
