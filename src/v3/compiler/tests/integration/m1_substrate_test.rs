...
fn r3_gate60_phantom_width_interaction_syntax_matches_compose_substrate() {
    // Gate #60: `Int<64>`, `Real<64>`, `Nat<8>` lower to canonical Compose × MachineWidth;
    // `MachineWidth<N>` exercises the separate rewrite branch in lowering.
    let src = "\
type Gate60_Int64_Lit = Int<64>\n\
type Gate60_Real64_Lit = Real<64>\n\
type Gate60_Nat8_Lit = Nat<8>\n\
type Gate60_MachineWidth128_Lit = MachineWidth<128>\n";
    ...
    assert_phantom_machine_width_lit_lowers_to_word(&dag, "Gate60_MachineWidth128_Lit", "Word128");
}

#[test]
fn r3_gate60_phantom_width_unsupported_magnitude_emits_diagnostic() {
    let src = "type Bad = Int<99>\n";
    let outcome = cached_compile_outcome(src, "r3_gate60_bad_phantom_width.v3");
    let dag = outcome.dag();
    assert!(
...
#[test]
fn bootstrap_string_audit_receipt_per_gate_23() {
    assert_bootstrap_string_is_free_monoid_char(&Dag::new());
}
...
fn substrate_accessor_rust_binding_invariants() {
    ...
    assert_eq!(
        checked, 9,
        "expected 9 substrate accessor bindings (port, node, resolve_producer, lane2_workflow_at, declaration_by_id, workflow_root_port, declaration_by_name, per_call_pattern_at, per_call_descent_operand_port)"
    );
