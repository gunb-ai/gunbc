...
//! pre-fold `1 + 2` subtree cost witness (`fold_pre` vs literal `fold_post`). Gate **#52** keeps the
//! cost-related `BinaryDimensionReportEquals` shape at the generic runner boundary (`Pass` stays
//! `NotYetImplemented` there); the host-side `cross_target_optimization_cost_structurally_derived_receipt`
//! test composes `Lens<SymbolicCost>` with `LanguageSpec` realization rows for the same gate.
