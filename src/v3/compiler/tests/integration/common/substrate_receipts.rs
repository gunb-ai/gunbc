...
/// construction-chain shape `Int = AbelianGroup<Nat>` (per `docs/design-numeric-construction.md`
/// §3 Option 3); the fixed-width `Int64 = OrderedRing<Word64>` row stays intact at
/// `dsl/std/integer.dag`. The `OrderedRing<Word64>` chain is now reachable through the
/// `Int64` name directly, and that's what this receipt continues to pin. The default
/// `Int` alias has its own ratchet (`int_default_alias_resolves_to_abelian_group_over_nat`).
