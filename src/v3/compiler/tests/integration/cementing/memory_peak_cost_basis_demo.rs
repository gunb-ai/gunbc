...
//! branch-style peaks compose with [`v3_compiler::memory_peak_cost::compose_sequential_memory_peak`]
...
use v3_compiler::memory_peak_cost::{
    compose_sequential_memory_peak, memory_peak_enforcement_violates,
};
...
fn memory_peak_cost_basis_demo_function_models_branch_peak_exceeding_linear_budget() {
    run_with_cost_lens_stack(|| {
        ...
        let observed_peak = compose_sequential_memory_peak(arm_heavy, arm_light);
