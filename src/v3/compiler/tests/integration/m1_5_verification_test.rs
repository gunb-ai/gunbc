...
fn rust_dag_isomorphism_executable_passes_dag_shape_report_gate() {
    // R3 gate #14: the RustDagIsomorphism consumer is executable through the
    // shared BinaryDimensionReportEquals envelope over two Dag shape-report refs.
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            assert!(
                ...
                "expected RustDagIsomorphism executable to pass through the \
                 BinaryDimensionReportEquals Dag carrier path; got {:?}",
                results[0].result
            );
        })
