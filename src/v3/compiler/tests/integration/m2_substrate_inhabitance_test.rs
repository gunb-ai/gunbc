...
/// T-Numeric-Construction Slice 4 — `Rational = Field<Int>` resolves
/// structurally as a Field instantiation over the construction-chain `Int`
/// algebra (Slice 3, #1466).
///
/// The ratchet enforces:
/// - `Rational` lives in `dsl/std/rational.dag` (single authority).
/// - The alias-chain target is a `Field` instantiation (NOT a parallel
///   algebra surface; NOT a derived-carrier opaque atom — those would be
///   Slice 4's substrate-split escalation if reviewers reject the simple
///   alias on M9 grounds).
/// - The single carrier argument resolves to `Int` (`dsl/std/integer.dag`),
///   which itself walks to `AbelianGroup<GroupCompletion<Nat>>` (Slice 3).
///
/// Note: `Field<T>` declares `reciprocal: fn(T) -> T`, which under the
/// standard parametric reading types `reciprocal: fn(Int) -> Int` —
/// denotationally false (only ±1 are units in ℤ). The header in
/// `dsl/std/rational.dag` documents this as a tracked-scaffold gap with a
/// named dissolution trigger (`FieldOfFractions<R>` derived-carrier or
/// NonZero/Result-shaped reciprocal refinement). This ratchet pins the
/// shape that lands in this slice; when the substrate sharpens, the
/// ratchet updates alongside.
#[test]
fn rational_resolves_to_field_over_int() {
    ...
    let int = dag
        .declaration_by_name("Int")
        .expect("`Int` carrier must be loaded from dsl/std/integer.dag (Slice 3)");

    // Walk the alias chain to the underlying instantiation (cycle-bounded).
    ...
    match connective {
        TypeConnective::Instantiation {
            ...
            assert_eq!(
                *template, field.id,
                "Rational must instantiate `Field`, not a parallel algebra surface"
            );
            assert_eq!(
                arguments.len(),
                1,
            ...
            assert_eq!(
                arguments[0].value, int.id,
                "Rational's Field carrier must be `Int` (the abstract integer \
                 algebra from Slice 3 #1466), not a `Word*` storage carrier \
                 or fixed-width row"
            );
        }
