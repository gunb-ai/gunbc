...
fn computation_size_bound_helpers_match_dag_authority() {
    ...
    let parser_stream = SizeBound::ParserStreamSize {
        witness: String::from("toks"),
    };
    let worklist = SizeBound::WorklistDrainSize {
    ...
    assert_eq!(size_bound_param(&parser_stream), Some("toks"));
    assert_eq!(size_bound_param(&worklist), Some("wl"));
    ...
    assert!(!is_constant_bound(&parser_stream));
    assert!(!is_constant_bound(&worklist));
