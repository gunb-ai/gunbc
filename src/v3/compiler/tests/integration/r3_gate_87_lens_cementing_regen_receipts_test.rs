...
//! `parallelism` stays an explicit `Compiles` placeholder while the register row remains
//! `LensCapabilityBehavioralPartial` (no Band-C v2-oracle dispatch row yet). Helper-only rows
//! (`infer_helpers`, `lower_helpers`, `variant_payload`) use the same placeholder shape with
//! per-file dissolution triggers in their `.dag` harness comments.
//!
