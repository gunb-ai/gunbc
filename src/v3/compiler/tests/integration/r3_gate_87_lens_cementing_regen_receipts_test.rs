...
#[test]
fn r3_gate_87_parallelism_rust_receipt_on_minimal_program() {
    let dag =
    ...
    let root = dag
        ...
        .expect("literal fixture should contain a Value or Bind node")
        .id();
    let WorkflowParallelismReport::ParallelismUnsupported(detail) = analyze_parallelism(&dag, root)
    else {
        panic!("parallelism on an unstaged literal fixture should surface ParallelismUnsupported");
    };
    assert_eq!(
        detail.kind,
