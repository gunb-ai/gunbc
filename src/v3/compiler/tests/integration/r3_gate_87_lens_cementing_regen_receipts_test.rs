...
//! Helper-only rows (`infer_helpers`, `lower_helpers`, `variant_payload`) and the **PARTIAL**
//! `parallelism` registry row stay explicit `Compiles` placeholders with per-file dissolution
