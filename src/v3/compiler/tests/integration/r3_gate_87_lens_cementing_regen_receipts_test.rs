...
//! `cost_target_realization`, `effect_enumeration`, `provenance`, `unused_parameters`, and
//! `structural_resolution` use Int-projection `.dag` claims until strict user modules can freeze
//! their full carriers without M1(2.8) opaque-body diagnostics. Helper-only rows (`infer_helpers`,
//! `lower_helpers`, `variant_payload`) stay explicit `Compiles` placeholders with paired Rust
//! source-compilation receipts and per-file dissolution triggers in their `.dag` harness comments.
//!
