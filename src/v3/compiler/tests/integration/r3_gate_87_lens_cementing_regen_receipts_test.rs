...
//! Non-complete rows without a stable public completion carrier (`infer_helpers`, `lower_helpers`,
//! `parallelism`, `variant_payload`) stay explicit `Compiles` placeholders with per-file
//! dissolution triggers in their `.dag` harness comments.
//!
