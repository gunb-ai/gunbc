...
//! `variant_payload` now uses an Int-projection `LensOutputEquals` receipt over `Gate87EmptyPayload`
//! (empty Conj) via `eval_gate_87_cementing_projection`; helper-only rows (`infer_helpers`,
//! `lower_helpers`) stay explicit `Compiles` placeholders with named blockers in their `.dag`
//! harness comments.
//!
