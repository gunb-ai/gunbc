...
const EXPECTED_HAND_AUTHORED_TEST: &[&str] = &[
    ...
    // T-4 D4: `compile_to_dag` smoke on the shared D2-resolver registry
    // `src/v4/extdeps/languages/resolver.dag` (zero module diagnostics).
    // SG-0 ratchet per INVARIANTS §P5(b) + the typescript-extdeps precedent.
    "src/v3/compiler/tests/integration/v4_extdeps_resolver_dag_smoke_test.rs",
