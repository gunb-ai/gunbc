...
const EXPECTED_HAND_AUTHORED_TEST: &[&str] = &[
    ...
    // `structural_resolution` / `unused_parameters` keep Rust pins while `.dag` claims use Int
    // projection (full-`Dag` reflection for `ProgramInput` + structural list equality is not yet
    // wired — see those harness headers). Helper regen lenses use `Compiles` + dissolution comments
    // + source-compile Rust receipts until a public output carrier is authorable in `.dag`.
