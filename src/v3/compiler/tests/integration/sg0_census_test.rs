...
const EXPECTED_HAND_AUTHORED_TEST: &[&str] = &[
    ...
    // R3 gate #87 (`lens_cementing_test_discipline_complete` / issue #2609): registry inventory
    // ratchet plus helper-only Rust compile receipts paired with
    // `tests/dag/t_r3_gate_87_cementing_regen_*.dag` + `t_pb_b_1_dag_runner_test`. Behavioral
    // Int-projection claims that are authorable as `.dag` live in the runner path, not here.
