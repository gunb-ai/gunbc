...
// by stringified message contents. Gate-#87 regen `.dag` receipt inventory is
// `R3_GATE_87_CEMENTING_REGEN_SUITES`; Rust receipt inventory is this SG-0 census list. When
// porting a Rust receipt below to a `.dag` `TestClaim`, **the same PR removes the entry from this
// list**. Don't introduce a parallel hand list (e.g. a separate "ported-but-still-listed" or
// "pending-port" set); the ratchets' point is that these two checked authorities move together.
