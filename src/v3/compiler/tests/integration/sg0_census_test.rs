...
const EXPECTED_HAND_AUTHORED_TEST: &[&str] = &[
    ...
    // §P5 Mechanism **(b)** SG-0 hand-authored integration test receipts table row for this path
    // is the planning-artifact receipt for expansions; each landing PR states exactly one checkable
    // receipt in its description (no new census path when adding tests inside this module).
