...
const EXPECTED_HAND_AUTHORED_TEST: &[&str] = &[
    ...
    // R3 BinShim carrier: hand-Rust structural ratchet for the substrate
    // carrier shape in `src/v3/std/bin_shim.dag` until reflected-Dag
    // structural assertions can consume it as a `.dag` TestClaim.
