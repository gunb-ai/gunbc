...
//!
//! Authority: `docs/briefs/r3-pr-e6-g1a-static-lens-fold-worker.md` (slice boundary + acceptance
//! gates). User `data … : Dag = …` literals remain class-5 blocked; the ratchet reifies a
//! substrate `Dag` [`v3_compiler::evaluator::Value`] via
//! [`v3_compiler::eval_substrate_reify::reify_compiled_dag_as_substrate_value`] (whole-program
//! `program.nodes()` order; no `source_file` filter). The fixture uses `std.list.fold`; the std
//! `fold` arrow remains an `ArrowBody::Unparsed` scaffold in the bootstrap dag, so the body
//! evaluator runs it through a small Rust intrinsic that mirrors the `list.dag` catamorphism.
...
fn eval_dimension_report_fn(dag: &v3_compiler::dag::Dag, fn_name: &str, d_arg: Value) -> Value {
    ...
    let frame = EvalFrame::from_bindings([(d_port, d_arg)]).expect("frame");
