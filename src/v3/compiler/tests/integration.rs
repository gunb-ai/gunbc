...
mod parse_stage4_prep {
    ...
    // SG-2b: this module snapshots the `parse.dag`-backed surface tree
    // (`parse_generated.rs` + `parse_parser_body.txt`) across the current
    // parseable corpus — structural parity ratchet for parser authority.
    ...
    fn render_manifest() -> String {
        let mut rendered = String::from(
            "# AUTO-GENERATED by `cargo test -p v3-compiler refresh_handwritten_parse_snapshot_manifest -- --ignored`\n\
             # SG-2b: snapshots parse authority output over the parse corpus.\n\
