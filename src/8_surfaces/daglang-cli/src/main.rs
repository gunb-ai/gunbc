...
//! - `daglang run [--output <path>|--output=<path>] [--dry-run] <file.dag>`: Legacy repo-specific path, unsupported in core daglang-cli

use std::path::PathBuf;

use daglang_cli::compile::{
    build_context, build_context_with_default_roots, check_from_module_graph,
    compile_from_context_with_options, render_canonical_ir_json, render_expand, render_obligations,
    render_progress_with_format, render_topology_with_format, render_triplets,
...
fn main() {
    ...
    if args.len() < 2 {
        ...
        eprintln!(
            "                      Legacy repo-specific path, unsupported in core daglang-cli"
        );
...
mod tests {
    ...
    fn normalize_path_components_handles_mixed_parent_curdir_and_redundant_separators() {
        let path = PathBuf::from(format!(
            "{}workspace//core/./../dsl//tools/../gist.dag",
            std::path::MAIN_SEPARATOR
        ));
        let normalized = normalize_path_components(&path);
        let expected = root_path().join("workspace").join("dsl").join("gist.dag");
        assert_eq!(normalized, expected);
    }
