...
mod tests {
    ...
    fn workspace_layout_from_env_manifest_contains_current_crate() {
        ...
        assert!(
            ir_dir.ends_with("src/0_foundation/ir"),
            "expected gunbc-ir dir to end with src/0_foundation/ir, got {}",
            ir_dir.display()
        );
    }
    ...
    fn relative_path_computes_nested_workspace_paths() {
        let layout = WorkspaceLayout::from_env_manifest_dir().expect("resolve workspace layout");
        let from = layout.workspace_root.join("src/0_foundation/ir");
    ...
    fn source_globs_derives_crate_sources_and_manifest() {
        ...
        assert!(
            globs.iter().any(|g| g == "src/0_foundation/ir/src/**/*.rs"),
            "expected src/0_foundation/ir source glob, got: {globs:?}"
        );
        assert!(
            globs.iter().any(|g| g == "src/0_foundation/ir/Cargo.toml"),
            "expected src/0_foundation/ir manifest glob, got: {globs:?}"
        );
    }
