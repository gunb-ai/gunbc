...
// PERF: Thread-local summary cache eliminates O(N²) HashMap cloning.
...
thread_local! {
    static SUMMARY_CACHE: RefCell<HashMap<String, Rc<ComplexitySummary>>> = RefCell::new(HashMap::new());
}

fn clear_summary_cache() {
    SUMMARY_CACHE.with(|cache| cache.borrow_mut().clear());
}
...
pub fn lookup_summary(_table: Rc<CostInternTable>, func_name: String) -> Option<Rc<ComplexitySummary>> {
    SUMMARY_CACHE.with(|cache| {
        cache.borrow().get(&func_name).cloned()
    })
}
