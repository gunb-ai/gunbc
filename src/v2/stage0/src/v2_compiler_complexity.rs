...
use crate::std_termination::DescentSource::{ChildAccessor, ListShrink, ArithmeticSubtract, ArithmeticDivide, ParserAdvance, SetRemoval, FoldIteration};
...
pub fn expr_contains_descent(expr: &Rc<Node>, param_name: &String, vars: &Rc<HashMap<String, bool>>, check_child: &bool, check_list: &bool, si: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> bool {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        if ((check_child.clone() && is_child_descent_expr(&expr, &param_name, &vars, &si)) || (check_list.clone() && is_list_shrink_expr(&expr, param_name.clone(), si.clone()))) {
            ...
            match (*expr.expr_data.clone()).clone() {
    ExprData::ExprIf => (expr_contains_descent(&if_then_branch(expr.clone()), &param_name, &vars, &check_child, &check_list, &si) && match if_else_branch(expr.clone()) {
    Some(eb) => expr_contains_descent(&eb, &param_name, &vars, &check_child, &check_list, &si),
    None => false,
}),
    ExprData::ExprBlock => match expr.children.clone().last().cloned() {
    Some(last_stmt) => expr_contains_descent(&last_stmt, &param_name, &vars, &check_child, &check_list, &si),
    None => false,
},
...
match method_size_effect(ms.clone()) {
    Some(CollectionSizeEffect::ShrinkEffect) => expr_contains_descent(&receiver, &param_name, &vars, &check_child, &check_list, &si),
    Some(CollectionSizeEffect::ProjectionEffect) => expr_contains_descent(&receiver, &param_name, &vars, &check_child, &check_list, &si),
    Some(CollectionSizeEffect::IdentityEffect) => expr_contains_descent(&receiver, &param_name, &vars, &check_child, &check_list, &si),
    None => false,
}
},
    _ => false,
...
match let_body(body.clone()) {
    Some(b) => collect_descent_vars(&b, &param_name, &next_vars, &check_child, &check_list, &si),
    None => next_vars,
}
},
    ExprData::ExprMatch => {
            let scrut = match_scrutinee(body.clone());
let scrut_is_descent = expr_contains_descent(&scrut, &param_name, &vars, &check_child, &check_list, &si);
...
match_arm_nodes(body.clone()).iter().cloned().fold(with_patterns.clone(), |acc: Rc<HashMap<String, bool>>, arm_node: Rc<Node>| {
                let arm_result = collect_descent_vars(&arm_body(&arm_node), &param_name, &with_patterns, &check_child, &check_list, &si);
v2_rt::rc_map_merge(acc, arm_result.clone())
})
},
    ExprData::ExprIf => {
            let then_vars = collect_descent_vars(&if_then_branch(body.clone()), &param_name, &vars, &check_child, &check_list, &si);
let else_vars = match if_else_branch(body.clone()) {
    Some(eb) => collect_descent_vars(&eb, &param_name, &vars, &check_child, &check_list, &si),
    None => vars.clone(),
};
v2_rt::rc_map_merge(then_vars, else_vars)
},
    ExprData::ExprBlock => body.children.clone().iter().cloned().fold(vars.clone(), |acc: Rc<HashMap<String, bool>>, stmt: Rc<Node>| collect_descent_vars(&stmt, &param_name, &acc, &check_child, &check_list, &si)),
    ExprData::ExprReturn => match body.children.clone().first().cloned() {
    Some(inner) => collect_descent_vars(&inner, &param_name, &vars, &check_child, &check_list, &si),
    None => vars.clone(),
},
    _ => vars.clone(),
}
...
pub fn all_self_calls_descend_inc(body: &Rc<Node>, func_name: &String, param_name: &String, vars: &Rc<HashMap<String, bool>>, check_child: &bool, check_list: &bool, si: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> bool {
    ...
    ExprData::ExprLet => {
            let val = let_value(body.clone());
let val_ok = all_self_calls_descend_inc(&val, &func_name, &param_name, &vars, &check_child, &check_list, &si);
...
let body_ok = match let_body(body.clone()) {
    Some(b) => all_self_calls_descend_inc(&b, &func_name, &param_name, &next_vars, &check_child, &check_list, &si),
    None => true,
};
(val_ok && body_ok)
},
    ExprData::ExprMatch => {
            let scrut = match_scrutinee(body.clone());
let scrut_ok = all_self_calls_descend_inc(&scrut, &func_name, &param_name, &vars, &check_child, &check_list, &si);
let scrut_is_descent = expr_contains_descent(&scrut, &param_name, &vars, &check_child, &check_list, &si);
let scrut_is_param = match (*scrut.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                let sname = expr_var_name_at(scrut.clone(), si.clone());
...
all_self_calls_descend_inc(&arm_body(&arm_node), &func_name, &param_name, &arm_vars, &check_child, &check_list, &si)
}) { __all = false; break; } } __all };
(scrut_ok && arms_ok)
},
    ExprData::ExprIf => {
            let cond_ok = all_self_calls_descend_inc(&if_condition(body.clone()), &func_name, &param_name, &vars, &check_child, &check_list, &si);
let then_ok = all_self_calls_descend_inc(&if_then_branch(body.clone()), &func_name, &param_name, &vars, &check_child, &check_list, &si);
let else_ok = match if_else_branch(body.clone()) {
    Some(eb) => all_self_calls_descend_inc(&eb, &func_name, &param_name, &vars, &check_child, &check_list, &si),
    None => true,
};
((cond_ok && then_ok) && else_ok)
    ...
    vars: vars.clone(),
                ...
                {
                    let stmt_ok = all_self_calls_descend_inc(&stmt, &func_name, &param_name, &acc.vars.clone(), &check_child, &check_list, &si);
let next_vars = collect_descent_vars(&stmt, &param_name, &acc.vars.clone(), &check_child, &check_list, &si);
...
(own_ok && { let mut __all = true; for child in body.children.clone().iter().cloned() { if !(all_self_calls_descend_inc(&child, &func_name, &param_name, &vars, &check_child, &check_list, &si)) { __all = false; break; } } __all })
},
    ExprData::ExprMethodCall { method_semantics: ms, .. } => {
...
if (is_iter && is_struct) {
                {
                    let recv_ok = all_self_calls_descend_inc(&receiver, &func_name, &param_name, &vars, &check_child, &check_list, &si);
...
all_self_calls_descend_inc(&lambda_body(arg_val.clone()), &func_name, &param_name, &ext_vars, &check_child, &check_list, &si)
},
    None => true,
},
    _ => all_self_calls_descend_inc(&arg_val, &func_name, &param_name, &vars, &check_child, &check_list, &si),
}
}) { __all = false; break; } } __all };
(recv_ok && args_ok)
}
            } else {
                { let mut __all = true; for child in body.children.clone().iter().cloned() { if !(all_self_calls_descend_inc(&child, &func_name, &param_name, &vars, &check_child, &check_list, &si)) { __all = false; break; } } __all }
            }
},
    _ => { let mut __all = true; for child in body.children.clone().iter().cloned() { if !(all_self_calls_descend_inc(&child, &func_name, &param_name, &vars, &check_child, &check_list, &si)) { __all = false; break; } } __all },
}
    })
}
...
if set_has(descent_vars.clone(), vname.clone()) {
    ...
    ExprData::ExprCall { .. } => if is_tree_size_preserving_wrapper(&arg_expr, si.clone()) {
                        classify_self_call_evidence(&wrapper_inner_arg(arg_expr.clone()), &param_name, &descent_vars, &check_child, &check_list, &si)
                    } else {
                        if is_sub_value_extractor(&arg_expr, si.clone()) {
                            {
                                let inner_ev = classify_self_call_evidence(&extractor_inner_arg(arg_expr.clone()), &param_name, &descent_vars, &check_child, &check_list, &si);
match inner_ev.clone() {
    ...
    Some(inner) => classify_self_call_evidence(&inner, &param_name, &descent_vars, &check_child, &check_list, &si),
    None => DescentEvidence::DescentUnknown,
}
                            } else {
                                DescentEvidence::DescentUnknown
...
let recv_ev = classify_self_call_evidence(&recv, &param_name, &descent_vars, &check_child, &check_list, &si);
match recv_ev {
...
pub fn collect_evidence_incremental(body: &Rc<Node>, func_name: &String, param_name: &String, vars: &Rc<HashMap<String, bool>>, check_child: &bool, check_list: &bool, branching_only: &bool, si: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> Option<DescentEvidence> {
    ...
    ExprData::ExprLet => {
            let val = let_value(body.clone());
let val_ev = collect_evidence_incremental(&val, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si);
let is_direct = ((check_child.clone() && is_child_descent_expr(&val, &param_name, &vars, &si)) || (check_list.clone() && is_list_shrink_expr(&val, param_name.clone(), si.clone())));
let is_var = match (*val.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => set_has(vars.clone(), expr_var_name_at(val.clone(), si.clone())),
    _ => false,
...
let body_ev = match let_body(body.clone()) {
    Some(b) => collect_evidence_incremental(&b, &func_name, &param_name, &next_vars, &check_child, &check_list, &branching_only, &si),
    None => None,
};
merge_optional_evidence(val_ev, body_ev)
    ...
    ExprData::ExprMatch => {
            let scrut = match_scrutinee(body.clone());
let scrut_ev = collect_evidence_incremental(&scrut, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si);
let scrut_is_descent = expr_contains_descent(&scrut, &param_name, &vars, &check_child, &check_list, &si);
let scrut_is_param = match (*scrut.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                let sname = expr_var_name_at(scrut.clone(), si.clone());
...
let arm_ev = collect_evidence_incremental(&arm_body(&arm_node), &func_name, &param_name, &arm_vars, &check_child, &check_list, &branching_only, &si);
...
merge_optional_evidence(scrut_ev, arms_ev)
    ...
    ExprData::ExprIf => {
            let cond_ev = collect_evidence_incremental(&if_condition(body.clone()), &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si);
...
let then_ev = if (branching_only.clone() && (then_path == 0)) {
                ...
                collect_evidence_incremental(&then_branch, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si)
            };
let else_ev = if (branching_only.clone() && (else_path == 0)) {
                None
            } else {
                match else_opt.clone() {
    Some(eb) => collect_evidence_incremental(&eb, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si),
    None => None,
}
            };
...
if (aname.clone().as_deref() == Some(param_name.clone()).as_deref()) {
                            classify_self_call_evidence(&arg_value(&arg_node), &param_name, &vars, &check_child, &check_list, &si)
                        } else {
                            acc
                        }
});
...
let child_evidence = body.children.clone().iter().cloned().fold(None, |acc: _, child: Rc<Node>| {
                let ce = collect_evidence_incremental(&child, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si);
...
merge_optional_evidence(own_evidence, child_evidence)
    ...
    vars: vars.clone(),
                ...
                let stmt_ev = collect_evidence_incremental(&stmt, &func_name, &param_name, &acc.vars.clone(), &check_child, &check_list, &branching_only, &si);
let next_vars = collect_descent_vars(&stmt, &param_name, &acc.vars.clone(), &check_child, &check_list, &si);
...
if (is_iter && is_struct) {
                {
                    let recv_ev = collect_evidence_incremental(&receiver, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si);
...
let ev = match (*arg_val.expr_data.clone()).clone() {
    ...
    Some(iter_name) => {
                            let ext_vars = v2_rt::rc_map_insert(vars.clone(), iter_name.clone(), true);
collect_evidence_incremental(&lambda_body(arg_val.clone()), &func_name, &param_name, &ext_vars, &check_child, &check_list, &branching_only, &si)
},
    None => None,
},
    _ => collect_evidence_incremental(&arg_val, &func_name, &param_name, &vars, &check_child, &check_list, &branching_only, &si),
};
...
let child_edges = Rc::new({ let mut __result = Vec::new(); for child in body.children.clone().iter().cloned() { __result.extend((*collect_scc_child_edges(&child, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si)).iter().cloned()); } __result });
v2_rt::concat(own_edges, child_edges)
},
    ExprData::ExprMethodCall { method_semantics: ms, .. } => {
            let receiver = method_receiver(body.clone());
...
if (is_iter && is_struct) {
                {
                    let recv_edges = collect_scc_child_edges(&receiver, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si);
...
collect_scc_child_edges(&lambda_body(arg_val.clone()), &caller, &param_name, &ext_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si)
},
    None => Rc::new(vec![]),
},
    _ => collect_scc_child_edges(&arg_val, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si),
}
}).iter().cloned()); } __result });
v2_rt::concat(recv_edges, args_edges)
}
            } else {
                Rc::new({ let mut __result = Vec::new(); for child in body.children.clone().iter().cloned() { __result.extend((*collect_scc_child_edges(&child, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si)).iter().cloned()); } __result })
            }
},
    ExprData::ExprLet => {
            let val = let_value(body.clone());
let val_edges = collect_scc_child_edges(&val, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si);
...
let body_edges = match let_body(body.clone()) {
    Some(b) => collect_scc_child_edges(&b, &caller, &param_name, &next_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si),
    None => Rc::new(vec![]),
};
v2_rt::concat(val_edges, body_edges)
    ...
    ExprData::ExprMatch => {
            let scrut = match_scrutinee(body.clone());
let scrut_edges = collect_scc_child_edges(&scrut, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si);
let arms_edges = Rc::new({ let mut __result = Vec::new(); for arm_node in match_arm_nodes(body.clone()).iter().cloned() { __result.extend((*collect_scc_child_edges(&arm_body(&arm_node), &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si)).iter().cloned()); } __result });
v2_rt::concat(scrut_edges, arms_edges)
    ...
    vars: descent_vars.clone(),
                ...
                let stmt_edges = collect_scc_child_edges(&stmt, &caller, &param_name, &acc.vars.clone(), &target_set, &check_child, &check_list, &scc_measure_params, &si);
let next_vars = collect_descent_vars(&stmt, &param_name, &acc.vars.clone(), &check_child, &check_list, &si);
...
result.edges.clone()
    ...
    ExprData::ExprIf => {
            let cond_edges = collect_scc_child_edges(&if_condition(body.clone()), &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si);
let then_branch = if_then_branch(body.clone());
let else_opt = if_else_branch(body.clone());
let then_edges = collect_scc_child_edges(&then_branch, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si);
let else_edges = match else_opt {
    Some(eb) => collect_scc_child_edges(&eb, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si),
    None => Rc::new(vec![]),
};
v2_rt::concat(cond_edges, v2_rt::concat(then_edges, else_edges))
},
    _ => Rc::new({ let mut __result = Vec::new(); for child in body.children.clone().iter().cloned() { __result.extend((*collect_scc_child_edges(&child, &caller, &param_name, &descent_vars, &target_set, &check_child, &check_list, &scc_measure_params, &si)).iter().cloned()); } __result }),
}
    })
}
