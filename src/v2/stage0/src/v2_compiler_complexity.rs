...
pub use crate::std_graph::{CallGraph, DfsFinishAcc, SccComponentAcc, SccCycleAcc, seed_adjacency_map, build_call_graph_from_proof_edges, dfs_finish_order, dfs_collect_component, graph_has_multi_node_scc, is_lexicographic_descent, is_valid_proof};
use self::SizeExpr::*;
use self::CostExpr::*;
use self::Certainty::*;
...
let reverse_graph = build_call_graph(&parser_entries, si.clone()).reverse.clone();
...
edges.iter().cloned().fold(Rc::new(CallGraph {
...
let local_func_set = names.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, bool>(), |acc: Rc<HashMap<String, bool>>, name: String| v2_rt::rc_map_insert(acc, name.clone(), true));
...
let result = topo_order.clone().iter().cloned().fold(Rc::new(SccBuildAcc {
    ...
    index: v2_rt::rc_empty_map::<String, Rc<SccInfo>>(),
            ...
            {
                let component = dfs_collect_component(&name, &graph.reverse.clone(), &Rc::new(SccComponentAcc {
