...
pub fn compose_effects(effects: Rc<Vec<Rc<OperationEffect>>>) -> Rc<CompositionVerdict> {
    {
        let non_idempotent = Rc::new({ let mut __result = Vec::new(); for e in effects.iter().cloned() { if match (*e.evidence.clone()).clone() {
...
pub fn derive_op_effect(operation_name: String, method: &HttpMethod, path: &Rc<PathTemplate>) -> Rc<DeriveOpEffectResult> {
    {
