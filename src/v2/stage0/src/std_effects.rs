...
pub use crate::std_http_path::{PathTemplate, PathTemplateParseResult, parse_path_template, has_path_params, last_path_param};
...
impl EffectShape {
    pub fn key_source(&self) -> Rc<KeySource> {
        match self {
            ...
            EffectShape::UpsertEffect { key_source: __val, .. } => __val.clone(),
            EffectShape::DeleteEffect { key_source: __val, .. } => __val.clone(),
...
pub fn compose_effects(effects: &Rc<Vec<Rc<OperationEffect>>>) -> Rc<ComposedEffect> {
    {
        let non_idempotent = Rc::new({ let mut __result = Vec::new(); for e in effects.clone().iter().cloned() { if match (*e.evidence.clone()).clone() {
...
pub fn derive_op_effect(operation_name: String, method: &HttpMethod, path: &Rc<PathTemplate>) -> Rc<DerivedOpEffect> {
    {
...
pub fn derive_op_effect_from_strings(operation_name: String, method_str: &String, path_str: String) -> Rc<DeriveOpEffectResult> {
    ...
    PathTemplateParseResult::ParsedPathTemplate { template: path, .. } => Rc::new(DeriveOpEffectResult::DerivedEffect {
    effect: derive_op_effect(operation_name, &method, &path),
}),
    PathTemplateParseResult::MalformedPathTemplate { raw: raw_path, segment, reason, .. } => Rc::new(DeriveOpEffectResult::MalformedPathInput {
    operation_name: operation_name,
    method: method.clone(),
...
pub fn check_modifier_vs_derivation(op: &Rc<DerivedOpEffect>, declared_idempotent: &bool, declared_readonly: &bool) -> Rc<ModifierCheck> {
    {
...
pub fn generate_idempotency_obligations(ops: Rc<Vec<Rc<DerivedOpEffect>>>) -> Rc<Vec<Rc<IdempotencyTestObligation>>> {
    Rc::new({ let mut __result = Vec::new(); for o in Rc::new({ let mut __result = Vec::new(); for o in ops.iter().cloned() { if is_idempotent_effect(o.shape.clone()) { __result.push(o); } } __result }).iter().cloned() { __result.push(Rc::new(IdempotencyTestObligation {
