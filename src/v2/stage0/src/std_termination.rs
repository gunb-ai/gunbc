...
pub enum PositiveDescentAmount {
    OneStep,
    AdditionalStep {
        previous: Rc<PositiveDescentAmount>,
    },
}
impl PositiveDescentAmount {
    pub fn previous(&self) -> Rc<PositiveDescentAmount> {
        match self {
            ...
            PositiveDescentAmount::AdditionalStep { previous: __val, .. } => __val.clone(),
        }
    }
}
...
pub fn proportional_divisor_to_int(d: Rc<ProportionalDivisor>) -> i64 {
    ...
    ProportionalDivisor::StrictlyLarger { inner: p, .. } => (1 + proportional_divisor_to_int(p.clone())),
}
    })
}
