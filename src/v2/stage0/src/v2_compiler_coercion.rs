...
pub use crate::std_types::{canonical_container_names, container_to_algebra_name};
...
pub fn render_cast(expr_str: String, type_str: String, target: RenderTarget) -> String {
    {
        ...
        v2_rt::replace(
            v2_rt::replace(syntax.template.clone(), "{expr}".to_string(), expr_str),
            "{type}".to_string(),
            type_str,
        )
    }
            ...
            if (container_name.clone().as_str() == "set".to_string().as_str()) {
                ...
                if (container_name.clone().as_str() == "map".to_string().as_str()) {
                    ...
                    if (container_name.clone().as_str() == "free_monoid".to_string().as_str()) {
                        ...
                        if (container_name.clone().as_str()
                            == "boolean_algebra".to_string().as_str())
                        {
                            "BooleanAlgebra".to_string()
                        } else {
                            if (container_name.clone().as_str()
                                == "partial_function".to_string().as_str())
                            {
...
pub fn apply_inhabitant_template2(template: String, first: String, second: String) -> String {
    v2_rt::replace(
        v2_rt::replace(template, "{0}".to_string(), first),
        "{1}".to_string(),
        second,
    )
}
...
pub fn inhabitant_tests(target: &RenderTarget) -> Rc<Vec<Rc<CoercionTestEntry>>> {
    {
        ...
        let assertions = Rc::new({
            ...
            for name in inhabitant_test_names().iter().cloned() {
                __result.extend(
                    (*match coerce_container_template(target.clone(), &name) {
                        Some(tmpl) => {
