...
pub fn structural_children_functions() -> Rc<HashMap<String, bool>> {
    thread_local! {
        static CACHED: Rc<HashMap<String, bool>> = {
            let mut __m = HashMap::new();
            __m.insert("match_arm_nodes".to_string(), true);
            __m.insert("method_arg_nodes".to_string(), true);
            __m.insert("block_stmts".to_string(), true);
            __m.insert("variant_node_fields".to_string(), true);
...
pub fn is_structural_children_function(func_name: String) -> bool {
    match v2_rt::lookup(&structural_children_functions(), func_name) {
    Some(_) => true,
    _ => false,
}
}
