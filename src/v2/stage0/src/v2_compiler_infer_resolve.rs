...
pub fn has_duplicate_type_param_name(names: &Rc<Vec<String>>) -> bool {
    { let mut __found = false; for pair in Rc::new(names.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned() { if {
        let idx = pair.0.clone();
let s = pair.1.clone();
{ let mut __found = false; for other in Rc::new(names.clone().iter().cloned().skip((idx.clone() + 1) as usize).collect::<Vec<_>>()).iter().cloned() { if (other.clone().as_str() == s.clone().as_str()) { __found = true; break; } } __found }
...
let env = tp_names.clone().iter().cloned().fold(env.clone(), |e: Rc<TypeEnv>, tp_name: String| Rc::new(TypeEnv {
    bindings: v2_rt::rc_map_insert(e.bindings.clone(), intern(&e.intern_table.clone(), &tp_name).id.clone(), Rc::new(TypeBinding {
    name: tp_name.clone(),
    resolved: Rc::new(Node {
    name: tp_name.clone(),
...
Rc::new(ItemResult {
    ...
    diagnostics: v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(collision_diags, param_diags), ret_diags), use_diags), body_diags), anno_diags), transport_diags), prop_diags), child_diags),
})
}
    })
}
