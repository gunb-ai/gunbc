...
pub use crate::v2_std_core::{
    arg_name, arg_value, arm_body, arm_guard, arm_pattern, expr_call_func, expr_method_name,
    field_init_node_name, field_init_node_value, field_node_cardinality, field_node_default_value,
    field_node_from_key, field_node_name, field_node_type_expr, foreach_variable,
    is_container_type, is_kernel_type, lambda_param_names, leaf_node, let_binding_name,
    ...
    make_transport_node, map_children, no_span, node_has_structure, node_is_product,
    param_node_default_value, param_node_name, param_node_type_expr, record_lit_type_name,
    resource_use_name, resource_use_resource, rt_node, with_optional_cardinality,
    with_required_cardinality, Cardinality, CompilerDiagnostic, ErrorNode, ExprData, InferredNode,
    MatchPattern, Node, NodeType, StringPart,
};
...
pub fn substitute_type_slots(
    n: Rc<Node>,
    slot_bindings: HashMap<String, Rc<Node>>,
    decl_name: String,
) -> Rc<Node> {
        ...
        } else {
            {
                let new_children = {
                    ...
                    for child in n.children.clone().iter().cloned() {
                        __result.push(if (child.name.clone() == decl_name.clone()) {
                            {
...
pub fn resolve_node_bounded(
                ...
                if (n.name.clone() == "Refined".to_string()) {
                ...
                } else {
                    {
                        ...
                        let all_diags = {
                            let mut __result = Vec::new();
                            for cr in child_results.clone().iter().cloned() {
                                __result.extend(cr.diagnostics.clone());
                            }
                            __result
                        };
                        Rc::new(NodeResolveResult {
    ...
    diagnostics: rt_diags.clone(),
                                        ...
                                        let field_diags = {
                                            let mut __result = Vec::new();
                                            for fr in field_results.clone().iter().cloned() {
                                                __result.extend(fr.diagnostics.clone());
                                            }
                                            __result
                                        };
                                        Rc::new(NodeResolveResult {
                        ...
                        let all_diags = {
                            let mut __result = Vec::new();
                            for vr in variant_results.clone().iter().cloned() {
                                __result.extend(vr.diagnostics.clone());
                            }
                            __result
                        };
                        Rc::new(NodeResolveResult {
...
) -> Rc<FieldInitResolveResult> {
    {
        ...
        Rc::new(FieldInitResolveResult {
            field_init: make_field_init_node(
                field_init_node_name(field_init.clone()),
                value_expr.clone(),
    ...
    if (transport.name.clone() == "local".to_string()) {
    ...
    } else {
        {
            ...
            let prop_diags = {
                let mut __result = Vec::new();
                for pr in prop_results.clone().iter().cloned() {
                    __result.extend(pr.diagnostics.clone());
                }
                __result
            };
            let child_results = {
        ...
        match (*texpr.expr_data.clone()).clone() {
                ...
                let resolved_children = {
                    ...
                    for arg_node in texpr.children.clone().iter().cloned() {
                        __result.push({
                            ...
                            make_arg_node(
                                if (arg_node.name.clone() == "".to_string()) {
                                    None
                                } else {
            ...
            ExprData::ExprIf => {
                ...
                Rc::new(ExprResolveResult {
                    ...
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(cr.diagnostics.clone(), tr.diagnostics.clone()),
                        match er.clone() {
                ...
                Rc::new(ExprResolveResult {
                    expr: make_expr_node(
                        ...
                        v2_rt::concat(vec![r.expr.clone()], {
                            let mut __result = Vec::new();
                            for p in lambda_param_names(texpr.clone()).iter().cloned() {
            ...
            ExprData::ExprSlice => {
                ...
                Rc::new(ExprResolveResult {
                    ...
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(br.diagnostics.clone(), sr.diagnostics.clone()),
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        {
            ...
            let env =
                tp_names
                    ...
                    .fold(env.clone(), |e: Rc<TypeEnv>, tp_name: String| {
                        Rc::new(TypeEnv {
                            bindings: Rc::new(v2_rt::map_insert(
                                (*e.bindings).clone(),
                                tp_name.clone(),
                                Rc::new(TypeBinding {
                                    name: tp_name.clone(),
            ...
            let use_diags = {
                let mut __result = Vec::new();
                for ur in use_results.clone().iter().cloned() {
                    __result.extend(ur.diagnostics.clone());
                }
                __result
            };
                    ...
                    v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(
                                v2_rt::concat(
                                    v2_rt::concat(
                                        v2_rt::concat(param_diags.clone(), ret_diags.clone()),
                                        use_diags.clone(),
                                    ),
                                    body_diags.clone(),
                                ),
