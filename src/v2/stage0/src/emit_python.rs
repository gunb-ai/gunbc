...
pub fn emit_single_field_product_py_type(field_node: Rc<Node>) -> String {
    if field_node.return_type.clone().is_none() {
    "dict".to_string()
} else {
    emit_node_type(rt_type(field_node.clone()), RenderTarget::Python)
}
}
...
pub fn emit_python(typed: Rc<ResolvedGraph>) -> Rc<EmitResult> {
    ...
    let files = v2_rt::concat(Rc::new(vec!(requirements.clone(), init_file.clone())), module_files.clone());
...
pub fn emit_init_py(modules: Rc<Vec<Rc<TypedModule>>>) -> Rc<TextFile> {
    ...
    Rc::new(TextFile { path: "__init__.py".to_string(), content: content.clone() })
}
...
pub fn emit_py_module(typed_module: Rc<TypedModule>, registry: Rc<HashMap<String, Rc<ItemInfo>>>) -> Rc<TextFile> {
    ...
    let imports_str = emit_py_imports(m.imports.clone());
    ...
    Rc::new(TextFile { path: v2_rt::concat(filename, ".py".to_string()), content: content.clone() })
}

pub fn emit_py_imports(imports: Rc<Vec<Rc<Import>>>) -> String {
    if ({
...
pub fn emit_py_dataclass_from_children(name: &str, children: Rc<Vec<Rc<Node>>>) -> String {
    ...
    v2_rt::concat(v2_rt::concat("@dataclass\nclass ".to_string(), name.to_string()), ":\n    pass".to_string())
} else {
    let field_lines = {
    let mut __mapped_0 = Vec::new();
    ...
    v2_rt::concat(v2_rt::concat(v2_rt::concat("@dataclass\nclass ".to_string(), name.to_string()), ":\n".to_string()), fields_str.clone())
}
}
...
pub fn emit_py_fn_def(name: &str, params: Rc<Vec<Rc<Param>>>, return_type: Rc<Node>, body: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>) -> String {
    ...
    let use_tco = match lookup_item(registry.clone(), &name) {
    Some(info) => {
...
pub fn emit_py_typed_expr(texpr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    ExprData::ExprError { kind: _, message, .. } => {
        v2_rt::concat(v2_rt::concat("raise RuntimeError(".to_string(), emit_string_literal(&message, "")), ")".to_string())
    }
    ExprData::ExprVar { name: n, binding_kind: _, .. } => {
        emit_ident(&n, RenderTarget::Python)
    }
    ExprData::ExprFieldAccess { base: b, field: f, summary, .. } => {
        if is_typed_service_call_receiver(texpr.clone()) {
    match extract_typed_service_name(texpr.clone()) {
    Some(svc_name) => {
    ...
    ExprData::ExprListLit { elements: els, .. } => {
        emit_py_typed_list_lit(els.clone(), registry.clone(), scope.clone(), depth)
    }
    ExprData::ExprBinOp { op: o, left: l, right: r, .. } => {
        emit_py_typed_bin_op(o.clone(), l.clone(), r.clone(), registry.clone(), scope.clone(), depth)
    }
    ExprData::ExprUnaryOp { op: o, operand: e, .. } => {
        emit_py_typed_unary_op(o.clone(), e.clone(), registry.clone(), scope.clone(), depth)
    }
    ExprData::ExprLambda { params: ps, body: bd, semantics: _, .. } => {
        emit_py_typed_lambda(ps.clone(), bd.clone(), registry.clone(), scope.clone(), depth)
    }
    ...
    ExprData::ExprBlock { stmts: ss, .. } => {
        emit_py_typed_block(ss.clone(), registry.clone(), scope.clone(), depth)
    }
    ...
    ExprData::ExprSlice { base: b, start: s, end: e, .. } => {
        emit_py_typed_slice(b.clone(), s.clone(), e.clone(), registry.clone(), scope.clone(), depth)
    }
    ExprData::ExprReturn { value: v, .. } => {
        v2_rt::concat("return ".to_string(), emit_py_typed_expr(v.clone(), registry.clone(), scope.clone(), depth))
    }
    ExprData::NoExprData => {
        "".to_string()
    }
...
pub fn emit_py_typed_list_lit(elements: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        if ({
    let __len_5 = elements.clone().len();
    __len_5 as i64
}) == 0_i64 {
    "[]".to_string()
} else {
    ...
    for __elem_1 in elements.iter().cloned() {
        __mapped_0.push(emit_py_typed_expr(__elem_1.clone(), registry.clone(), scope.clone(), depth.clone()));
    }
    Rc::new(__mapped_0)
};
    let els_str = {
...
pub fn emit_py_typed_bin_op(op: BinOpKind, left: Rc<Node>, right: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("(".to_string(), l_str.clone()), " if ".to_string()), l_str.clone()), " is not None else ".to_string()), r_str), ")".to_string())
} else {
    let op_str = emit_bin_op_symbol(op.clone(), RenderTarget::Python);
    v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("(".to_string(), l_str.clone()), " ".to_string()), op_str), " ".to_string()), r_str), ")".to_string())
}
    })
}

pub fn emit_py_typed_unary_op(op: UnaryOpKind, operand: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let expr_str = emit_py_typed_expr(operand.clone(), registry.clone(), scope.clone(), depth);
        match op {
    UnaryOpKind::Not => {
        v2_rt::concat(emit_keyword("not", RenderTarget::Python), expr_str)
    }
    UnaryOpKind::Neg => {
        v2_rt::concat("-".to_string(), expr_str)
    }
}
    })
}

pub fn emit_py_typed_lambda(params: Rc<Vec<String>>, body: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let param_strs = {
    let mut __mapped_0 = Vec::new();
    for __elem_1 in params.iter().cloned() {
        __mapped_0.push(emit_ident(&__elem_1, RenderTarget::Python));
    }
    Rc::new(__mapped_0)
};
        let params_str = {
    let mut __joined_2 = String::new();
    let mut __first_4 = true;
    for __elem_3 in param_strs.iter().cloned() {
        if !__first_4 {
    __joined_2.push_str(&", ".to_string());
};
        __first_4 = false;
    ...
    __joined_2
        ...
        let body_str = emit_py_typed_expr(body.clone(), registry.clone(), scope.clone(), depth);
        v2_rt::concat(v2_rt::concat(v2_rt::concat("lambda ".to_string(), params_str.clone()), ": ".to_string()), body_str)
    })
}
...
pub fn emit_py_typed_tco_expr(texpr: Rc<Node>, fn_name: &str, params: Rc<Vec<Rc<Param>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    ExprData::ExprCall { func: f, args: a, call_semantics: _, .. } => {
        if f.clone() == fn_name {
    emit_py_typed_tco_reassign(a.clone(), params.clone(), registry.clone(), scope.clone(), depth.clone())
} else {
    let call_str = emit_py_typed_call(&f, a.clone(), registry.clone(), scope.clone(), depth.clone());
    ...
    let then_str = emit_py_typed_tco_expr(t.clone(), &fn_name, params.clone(), registry.clone(), scope.clone(), depth.clone() + 1_i64);
    match e.as_ref().map(|__rc| __rc.as_ref()) {
    Some(eb) => {
        let eb = Rc::new(eb.clone());
        {
    let else_str = emit_py_typed_tco_expr(eb.clone(), &fn_name, params.clone(), registry.clone(), scope.clone(), depth.clone() + 1_i64);
    ...
    for __elem_1 in arm_list.iter().cloned() {
        __mapped_0.push(emit_py_typed_tco_match_arm(__elem_1.clone(), &fn_name, params.clone(), registry.clone(), scope.clone(), depth.clone()));
    }
    Rc::new(__mapped_0)
};
    let arms_str = {
    ...
    Some(b) => {
        ...
        v2_rt::concat(v2_rt::concat(let_line, "\n".to_string()), emit_py_typed_tco_expr(b.clone(), &fn_name, params.clone(), registry.clone(), next_scope.clone(), depth.clone()))
    }
    None => {
        let_line
    }
    ...
    Some(last_expr) => {
        emit_py_typed_tco_expr(last_expr.clone(), &fn_name, params.clone(), registry.clone(), init_state.scope.clone(), depth.clone())
    }
    None => {
        "return None".to_string()
    }
    ...
    if ({
    let __len_11 = init_state.text.clone().len();
    __len_11 as i64
}) == 0_i64 {
    last_str.clone()
} else {
    ...
    let mut __joined_8 = String::new();
    let mut __first_10 = true;
    for __elem_9 in init_state.text.iter().cloned() {
        if !__first_10 {
    __joined_8.push_str(&"\n".to_string());
};
        __first_10 = false;
    ...
    _ => {
        {
    let val_str = emit_py_typed_expr(texpr.clone(), registry.clone(), scope.clone(), depth.clone());
...
pub fn emit_py_typed_tco_reassign(args: Rc<Vec<Rc<NamedArg>>>, params: Rc<Vec<Rc<Param>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    for (__idx_5, __elem_6) in ordered_args.clone().iter().enumerate() {
        __enumerated_4.push((__idx_5 as i64, __elem_6.clone()));
    }
    Rc::new(__enumerated_4)
}).iter().cloned() {
        __mapped_7.push(v2_rt::concat(v2_rt::concat(v2_rt::concat("__tco_".to_string(), v2_rt::to_string(__elem_8.0.clone())), " = ".to_string()), __elem_8.1.clone()));
    }
    Rc::new(__mapped_7)
};
    let assigns = {
    ...
    for (__idx_10, __elem_11) in param_names.clone().iter().enumerate() {
        __enumerated_9.push((__idx_10 as i64, __elem_11.clone()));
    }
    Rc::new(__enumerated_9)
}).iter().cloned() {
        __mapped_12.push(v2_rt::concat(v2_rt::concat(__elem_13.1.clone(), " = __tco_".to_string()), v2_rt::to_string(__elem_13.0.clone())));
    }
    Rc::new(__mapped_12)
};
    let all_lines = v2_rt::concat(v2_rt::concat(temp_lets.clone(), assigns.clone()), Rc::new(vec!("continue".to_string())));
    {
    let mut __joined_14 = String::new();
    let mut __first_16 = true;
    for __elem_15 in all_lines.iter().cloned() {
        if !__first_16 {
    __joined_14.push_str(&"\n".to_string());
};
        __first_16 = false;
...
pub fn emit_py_service_def(item: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>) -> String {
    ...
    let transport = if item.transport.clone().is_none() {
    local_transport_node(item.span.clone())
} else {
    item.transport.clone().unwrap()
};
    let init_method = emit_py_service_init(transport.clone(), item.config.clone());
...
pub fn emit_py_service_init(transport: Rc<Node>, config: Option<Rc<ServiceConfig>>) -> String {
...
pub fn emit_py_operation_method(service_name: &str, transport: Rc<Node>, op_node: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, depth: i64) -> String {
    ...
    let effective_transport = match op_node.transport.as_ref().map(|__rc| __rc.as_ref()) {
    Some(op_transport) => {
    ...
    let body = emit_py_transport_call(effective_transport.clone(), &op_node.name, registry.clone());
