...
pub fn emit_py_typed_list_lit(elements: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    emit_list_lit_expr(el_strs.clone(), RenderTarget::Python)
}
...
pub fn emit_py_typed_unary_op(op: UnaryOpKind, operand: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    ...
    emit_unary_op(op, &expr_str, RenderTarget::Python)
}
...
pub fn emit_py_typed_lambda(params: Rc<Vec<String>>, body: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    let params_str = emit_lambda_params(params.clone(), RenderTarget::Python);
    let body_str = emit_py_typed_expr(body.clone(), registry.clone(), scope.clone(), depth);
    emit_lambda(&params_str, &body_str, RenderTarget::Python)
}
...
pub fn emit_py_tco_match(frame: Rc<TcoFrame>, fn_name: &str, params: Rc<Vec<Rc<Param>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>) -> String {
    ...
    for __elem_1 in arm_list.iter().cloned() {
        __mapped_0.push(emit_py_typed_tco_match_arm(__elem_1.clone(), &fn_name, params.clone(), registry.clone(), frame.scope.clone(), frame.depth.clone()));
    }
    Rc::new(__mapped_0)
};
    let arms_str = {
