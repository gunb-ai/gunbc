...
pub use crate::v2_std_core::{
    authored_name_at, expr_call_func_at, make_error_node, no_span, DeclaredFuncSig, ErrorNode,
...
pub fn topo_resolve_loop(
    ...
    loop {
        if ((remaining.clone().len() as i64) == 0) {
            {
                let all_resolved = Rc::new(v2_rt::map_values(&declared_sigs))
                    .iter()
                    .cloned()
                    .fold(
                        resolved.clone(),
                ...
                if {
                    ...
                    });
                    {
                        ...
                        for c in local_callees.clone().iter().cloned() {
                            if !(v2_rt::map_get(&resolved, c.clone()) != None) {
                                __all = false;
                                break;
                            }
                        }
        ...
        if ((ready.clone().len() as i64) == 0) {
            {
                let cycle_accum = remaining.clone().iter().cloned().fold(
                    ...
                    |acc: Rc<SigsAccum>, fn_name: String| match v2_rt::map_get(
                        &declared_sigs,
                        fn_name.clone(),
                    ) {
                        Some(dsig) => {
                            if (dsig.inferred.clone() != None) {
                                ...
                                Rc::new(SigsAccum {
                                    ...
                                    diagnostics: v2_rt::rc_list_push(
