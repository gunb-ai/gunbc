...
pub use crate::v2_core::{Node, Param, ExprData, arg_value, arm_body, field_access_base, if_condition, if_then_branch, if_else_branch, let_value, let_body, match_scrutinee, match_arm_nodes, method_receiver, method_arg_nodes};
use crate::v2_core::ExprData::{NoExprData, ExprError, ExprLiteral, ExprVar, ExprFieldAccess, ExprCall, ExprMethodCall, ExprMatch, ExprIf, ExprLet, ExprBlock, ExprReturn};
use EdgeKind::*;
...
pub struct OperationalFacts {
    pub binding_fan_out: HashMap<String, i64>,
}

pub fn empty_operational_facts() -> Rc<OperationalFacts> {
    Rc::new(OperationalFacts {
    binding_fan_out: <HashMap<_, _>>::new(),
})
}

pub fn compute_operational_facts(proofs: Vec<Rc<OwnershipProof>>) -> Rc<OperationalFacts> {
    {
        let fan_out_map = proofs.clone().iter().cloned().fold(<HashMap<String, i64>>::new(), |acc: _, proof: Rc<OwnershipProof>| v2_rt::map_values(&proof.bindings.clone()).iter().cloned().fold(acc.clone(), |m: _, usage: Rc<BindingUsage>| v2_rt::map_insert(m.clone(), v2_rt::concat(v2_rt::concat(proof.func_name.clone(), "|".to_string()), usage.name.clone()), binding_fan_out(usage.clone()))));
Rc::new(OperationalFacts {
    binding_fan_out: fan_out_map.clone(),
})
}
}
