...
pub fn node_contains_call_generic(
    n: Rc<Node>,
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        let is_bare = ((((n.children.clone().len() as i64) == 0)
            ...
            && ((n.params.clone().len() as i64) == 0));
        let bare_generic = (is_bare && {
            let mut __found = false;
            for g in generic_names.clone().iter().cloned() {
                if (g.clone().as_str() == nm.clone().as_str()) {
                    __found = true;
                    break;
                }
            }
        ...
        let inferred_generic = match n.inferred.clone().as_deref().cloned() {
            Some(InferredNode::Resolved { node: rt, .. }) => node_contains_call_generic(
                rt.clone(),
                generic_names.clone(),
                source_indices.clone(),
            ),
        ...
        (((bare_generic || inferred_generic) || {
            let mut __found = false;
            for ch in n.children.clone().iter().cloned() {
                if node_contains_call_generic(
                    ch.clone(),
                    generic_names.clone(),
                    source_indices.clone(),
                ) {
