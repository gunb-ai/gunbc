...
use crate::v2_std_core::VarBindingKind::{
    FunctionValueBinding, LocalValueBinding, VariantValueBinding,
};
...
pub use crate::v2_compiler_infer_patterns::{
    ...
    lookup_variant_in_type, pattern_binding_type, pattern_subject_from_node,
    pattern_subject_from_node_type, NodeLookupResult, PatternSubject,
};
...
pub use crate::v2_compiler_infer_types::{
    ...
    is_bridge_placeholder_type_name, node_contains_bridge_placeholder_type,
...
pub fn merge_scope_from_imports(
    mut remaining: Vec<Rc<ResolvedImport>>,
    mut parent_index: HashMap<String, Rc<TypedModule>>,
    mut env: Rc<TypeEnv>,
    mut func_sigs: HashMap<String, Rc<DeclaredFuncSig>>,
    mut svc_registry: HashMap<String, Vec<Rc<OpEntry>>>,
    mut svc_locals: HashMap<String, Rc<TypeBinding>>,
    ...
    loop {
        match remaining.clone().first().cloned() {
            ...
            Some(imp) => match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
                Some(typed_parent) => {
                    let next_func_sigs =
                        v2_rt::map_values(&typed_parent.func_env.clone().signatures.clone())
                            .iter()
                            .cloned()
                            .fold(func_sigs.clone(), |acc: _, rsig: Rc<ResolvedFuncSig>| {
                            ...
                            {
                                {
                                    ...
                                    Rc::new(InferScopeComponents {
                                        ...
                                        svc_registry: v2_rt::map_insert(
                                            acc.svc_registry.clone(),
                                            titem.name.clone(),
                                            entries.clone(),
                                        ),
                                        svc_locals: v2_rt::map_insert(
                                ...
                                if (((((titem.body.clone() != None)
                                    ...
                                    && (node_has_structure(titem.clone()) == false))
...
pub fn namespace_root_from_properties(properties: Vec<Rc<Node>>, name: String) -> String {
    match {
        ...
        for p in properties.clone().iter().cloned() {
            if (field_init_node_name(p.clone()) == "namespace_root".to_string()) {
                __result.push(p);
            }
        }
        __result
            ...
            Some(parent) => {
                if node_is_coproduct(parent.clone()) {
                    if {
                        ...
                        for child in parent.children.clone().iter().cloned() {
                            if (child.name.clone() == name.clone()) {
                                __found = true;
                                break;
                            }
                        }
...
pub fn inferred_from_node_type(
    result: Rc<NodeType>,
    ...
    match (*result.clone()).clone() {
        ...
        NodeType::InferError {
            ...
            span: span,
            ..
        } => Rc::new(InferredNode::CompilerError {
            message: message.clone(),
            span: span.clone(),
        }),
...
pub fn lambda_param_types_from_scope(scope: Rc<InferScope>, params: Vec<String>) -> Vec<Rc<Node>> {
    {
        let mut __result = Vec::new();
        for param_name in params.clone().iter().cloned() {
            __result.push(
                match v2_rt::map_get(&scope.locals.clone(), param_name.clone()) {
            ...
            MatchPattern::VariantPattern {
                ...
                let inferred_parent = match (*resolved_scrut.clone()).clone() {
                    PatternSubject::PatternResolved {
                        ...
                        if (node_is_optional(resolved_scrut_node.clone())
                            && ((variant_name.clone() == "Some".to_string())
                                || (variant_name.clone() == "None".to_string())))
                        {
                    ...
                    for binding in bindings.clone().iter().cloned() {
                        __result.push({
                            let field_lookup = lookup_field_in_variant(
                                ...
                                field_binding_name(binding.clone()),
                                scope.module_name.clone(),
                            );
    ...
    if (is_fold && ((method_args.clone().len() as i64) >= min_args.clone())) {
        {
            let init_arg = match {
                ...
                for a in method_args.clone().iter().cloned() {
                    if arg_has_name(a.clone(), "init".to_string()) {
                        __result.push(a);
                    }
                }
                __result
    ...
    match semantics.clone().as_deref().cloned() {
        Some(MethodSemantics::AlgebraMethodSemantics {
            ...
            method_type
                ...
                .cloned()
                .map(|p| {
                    let param_type = param_node_type_expr(p.clone());
                    let resolved_param_type = rt_type(param_type.clone());
                    if ((resolved_param_type.params.clone().len() as i64) > 0) {
    ...
    let is_fold = method_name_is(method_name.clone(), "fold".to_string());
    {
            ...
            .cloned()
        {
            __result.push({
                ...
                let is_init_arg = (arg_has_name(a.clone(), "init".to_string())
                    || ((arg_name(a.clone()) == None) && (idx.clone() == 0)));
                            ...
                            Some(formal_type) => {
                                    ...
                                    && ((formal_type.params.clone().len() as i64) > 0))
                                {
                                    ...
                                    if is_lambda_expr(arg_value(a.clone())) {
                                        ...
                                        let inferred_type = rt_type(ar.typed.clone());
                                        let callable_diags = if (((formal_type
                                            .params
                                            .clone()
                                            .len() as i64)
                                            > 0)
                                            ...
                                            vec![inference_error(
                                                v2_rt::concat(
                                                    v2_rt::concat(
                                                        v2_rt::concat(
                                                            v2_rt::concat(
                                                                "callable argument type mismatch: expected '"
                                                                    .to_string(),
                                                                node_type_shape(formal_type.clone()),
                                                            ),
                                                            "', got '".to_string(),
                                                        ),
                                                        node_type_shape(inferred_type.clone()),
                                                    ),
                                                    "'".to_string(),
                                                ),
                                                a.span.clone(),
                                        ...
                                        Rc::new(ArgInferResult {
                                            ...
                                            diagnostics: v2_rt::concat(
                                                ar.diagnostics.clone(),
                                                callable_diags.clone(),
                                            ),
                                        })
                                    }
                                }
...
pub fn infer_lambda_with_element_type(
    lambda_expr: Rc<Node>,
    element_type: Rc<Node>,
    scope: Rc<InferScope>,
) -> Rc<InferResult> {
...
pub fn infer_lambda_with_callable_type(
    lambda_expr: Rc<Node>,
    callable_type: Rc<Node>,
    ...
    match (*lambda_expr.expr_data.clone()).clone() {
        ExprData::ExprLambda { .. } => {
            ...
            let callable_return = callable_inferred(callable_type.clone());
            let callable_return_has_placeholder =
                node_contains_bridge_placeholder_type(callable_return.clone());
            let param_types = {
                let mut __result = Vec::new();
                for pair in lam_params
                    .clone()
            ...
            let body_result = infer_expr(
                lam_body.clone(),
                typed_scope.clone(),
                if callable_return_has_placeholder.clone() {
                    None
                } else {
                    Some(callable_return.clone())
                },
            );
            let body_typed = body_result.typed.clone();
            let arity_diags = if ((lam_params.clone().len() as i64)
                == (callable_params.clone().len() as i64))
            {
                vec![]
            } else {
                vec![inference_error(
                    "callable argument arity mismatch".to_string(),
                    span.clone(),
                    scope.module_name.clone(),
                )]
            };
            let body_type = rt_type(body_typed.clone());
            let return_diags = if node_type_compatible_with_formal_placeholders(
                body_type.clone(),
                callable_return.clone(),
            ) {
                vec![]
            } else {
                vec![inference_error(
                    v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(
                                v2_rt::concat(
                                    "callable return type mismatch: expected '".to_string(),
                                    node_type_shape(callable_return.clone()),
                                ),
                                "', got '".to_string(),
                            ),
                            node_type_shape(body_type.clone()),
                        ),
                        "'".to_string(),
                    ),
                    lam_body.span.clone(),
            ...
            Rc::new(ArgInferResult {
                ...
                diagnostics: v2_rt::concat(
                    arity_diags.clone(),
                    v2_rt::concat(body_result.diagnostics.clone(), return_diags.clone()),
                ),
            })
        }
        _ => {
...
pub fn refine_collection_result_type(
    semantics: Option<Rc<MethodSemantics>>,
    typed_args: Vec<Rc<Node>>,
...
) -> Rc<Node> {
    {
        ...
        if method_name_is(method_name.clone(), "map".to_string()) {
            ...
            if method_name_is(method_name.clone(), "flat_map".to_string()) {
                ...
                if (method_name_is(method_name.clone(), "filter".to_string())
                    || method_name_is(method_name.clone(), "sort_by".to_string())
                    || method_name_is(method_name.clone(), "unique".to_string())
                    || method_name_is(method_name.clone(), "reverse".to_string()))
                {
                    ...
                    if method_name_is(method_name.clone(), "fold".to_string()) {
                            ...
                            && ((receiver_type.children.clone().len() as i64) == 0))
                        {
                            match typed_args.clone().first().cloned() {
                                Some(item_arg) => {
                                    match (*rt_node(arg_value(item_arg.clone()))).clone() {
                                        NodeType::Typed { node: rt, .. } => {
                                            container_node("List".to_string(), rt.clone())
                                        }
                                        NodeType::InferError { .. } => fallback.clone(),
                                        NodeType::Untyped => fallback.clone(),
                                    }
                            ...
                            {
                                {
                                    let key_type = match typed_args.clone().first().cloned() {
                                        Some(key_arg) => {
                                            match (*rt_node(arg_value(key_arg.clone()))).clone() {
                                                NodeType::Typed { node: rt, .. } => rt.clone(),
                                                NodeType::InferError { .. } => fallback.clone(),
                                                NodeType::Untyped => fallback.clone(),
                                            }
                                    ...
                                    match typed_args
                                        ...
                                        .cloned()
                                    {
                                        Some(val_arg) => map_node(
                                            key_type.clone(),
                                            match (*rt_node(arg_value(val_arg.clone()))).clone() {
                                                NodeType::Typed { node: rt, .. } => rt.clone(),
                                                NodeType::InferError { .. } => fallback.clone(),
                                                NodeType::Untyped => fallback.clone(),
                                            },
                                ...
                                if ((method_name_is(method_name.clone(), "map_merge".to_string())
                                    && receiver_is_map.clone())
                                    && ((typed_args.clone().len() as i64) >= 1))
                                {
                                    {
                                        ...
                                        {
                                            Some(overlay_arg) => {
                                                match (*rt_node(arg_value(overlay_arg.clone())))
                                                    .clone()
                                                {
                                                    NodeType::Typed { node: rt, .. } => rt.clone(),
                                                    NodeType::InferError { .. } => {
                                        ...
                                        if (((receiver_type.children.clone().len() as i64) == 0)
                                            && ((overlay_type.children.clone().len() as i64) > 0))
                                        {
                                            overlay_type.clone()
                                        } else {
                                            if ((receiver_type.children.clone().len() as i64) > 0) {
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*texpr.expr_data.clone()).clone() {
            ...
            ExprData::ExprVar { .. } => {
                ...
                match v2_rt::map_get(&scope.locals, name.clone()) {
                    ...
                    None => match lookup_func_sig(scope.func_env.clone(), name.clone()) {
                        Some(fsig) => ok_infer(make_named_expr_node(
                            name.clone(),
                            Rc::new(ExprData::ExprVar {
            ...
            ExprData::ExprFieldAccess { .. } => {
                ...
                if (resolved_base.name.clone() == "Error".to_string()) {
                    ...
                    match lookup_field_type_node(
                        ...
                        None => {
                            if (resolved_base.name.clone() == "Dynamic".to_string()) {
                                {
                                    let fa_texpr = make_named_expr_node(
                                        field_name.clone(),
                                        Rc::new(ExprData::ExprFieldAccess {
            ...
            ExprData::ExprCall { .. } => {
                ...
                let call_method_args = call_args
                    .clone()
                    .iter()
                    .cloned()
                    .skip(1 as usize)
                        ...
                        Some(bridge) => {
                                ...
                                && ((call_args.clone().len() as i64) >= 2))
                            {
                                ...
                                v2_rt::concat(
                                    ...
                                    {
                                        let mut __result = Vec::new();
                                        for a in call_method_args.clone().iter().cloned() {
                                            __result.push({
                                                let ar = infer_expr(
                                                    arg_value(a.clone()),
                                                    scope.clone(),
                ...
                } else {
                    {
                            ...
                            .cloned()
                        {
                            __result.push({
                                ...
                                let is_callable_formal =
                                    (formal_param_type.params.clone().len() as i64) > 0;
                                if (is_lambda_expr(arg_value(a.clone()))
                                    && is_callable_formal.clone())
                                {
                                    infer_lambda_with_callable_type(
                                        arg_value(a.clone()),
                ...
                let arg_diags = {
                    let mut __result = Vec::new();
                    for air in arg_infer_results.clone().iter().cloned() {
                        __result.extend(air.diagnostics.clone());
                    }
                    __result
                };
                        ...
                        if (is_known_method.clone() && ((typed_args.clone().len() as i64) > 0)) {
                            {
                                ...
                                let bridge_result_type = refine_collection_result_type(
                            ...
                            if (func_name.clone() == "empty_map".to_string()) {
                                ...
                                if (infer_builtin_call_type(func_name.clone()) != None) {
                                    {
                                        let bt = if ((func_name.clone() == "lookup".to_string())
                                            || (func_name.clone() == "map_get".to_string()))
                                        {
                                            match typed_args.clone().first().cloned() {
                                ...
                                } else {
                                    {
                                        let callable_local = match v2_rt::map_get(
                                            ...
                                            Some(binding) => {
                                                if (binding.resolved.clone().name.clone()
                                                    == "Callable".to_string())
                                                {
            ...
            ExprData::ExprMethodCall { .. } => {
                ...
                let mc_texpr = make_named_expr_node(
                    ...
                    v2_rt::concat(vec![recv_typed.clone()], typed_mc_arg_nodes.clone()),
                    Some(Rc::new(InferredNode::Resolved {
            ...
            ExprData::ExprMatch => {
                ...
                let arm_diags = {
                    let mut __result = Vec::new();
                    for ar in arm_infer_results.clone().iter().cloned() {
                        __result.extend(ar.diagnostics.clone());
                    }
                    __result
                };
                ...
                let match_texpr = make_expr_node(
                    ...
                    v2_rt::concat(vec![scrut_typed.clone()], typed_arms.clone()),
                    Some(Rc::new(InferredNode::Resolved {
                        node: result_type.clone(),
                    })),
                    span.clone(),
                ...
                Rc::new(InferResult {
                    ...
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(scrut_diags.clone(), arm_diags.clone()),
                ...
                match else_expr.clone() {
                    Some(else_branch) => {
                                    ...
                                    v2_rt::concat(
                                        v2_rt::concat(
                                            "if branches resolve to incompatible types: "
                                                .to_string(),
                                            node_type_shape(then_rt.clone()),
                                        ),
                                        " vs ".to_string(),
                                    ),
                                    node_type_shape(else_rt.clone()),
                                ),
                                span.clone(),
                                scope.module_name.clone(),
                            )]
                        ...
                        Rc::new(InferResult {
                            ...
                            diagnostics: v2_rt::concat(
                                v2_rt::concat(
                                    v2_rt::concat(cond_diags.clone(), then_diags.clone()),
                                    else_diags.clone(),
                                ),
                                branch_diags.clone(),
                            ),
            ...
            ExprData::ExprListLit => {
                ...
                let elem_diags = {
                    let mut __result = Vec::new();
                    for r in elem_results.clone().iter().cloned() {
                        __result.extend(r.diagnostics.clone());
                    }
                    __result
                };
            ...
            ExprData::ExprBinOp { op: op, .. } => {
                ...
                let bo_texpr = make_expr_node(
                    Rc::new(ExprData::ExprBinOp { op: op.clone() }),
            ...
            ExprData::ExprStringInterp => {
                ...
                let interp_diags = {
                    let mut __result = Vec::new();
                    for part_node in texpr.children.clone().iter().cloned() {
                        __result.extend(match (*part_node.expr_data.clone()).clone() {
            ...
            ExprData::ExprIndex => {
                ...
                let index_check = match (*base_type_result.clone()).clone() {
                    NodeType::Typed {
                        node: base_type, ..
                    } => match (*index_type_result.clone()).clone() {
                        NodeType::Typed {
                            node: index_type, ..
                ...
                let idx_inferred = match index_check.clone() {
                    ...
                    None => Some(inferred_from_node_type(
                        access_failure_source.clone(),
                ...
                Rc::new(InferResult {
                    ...
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(base_diags.clone(), index_diags.clone()),
            ...
            ExprData::ExprSlice => {
                ...
                let slice_check = match (*base_type_result.clone()).clone() {
                    NodeType::Typed {
                        node: base_type, ..
                    } => match (*start_type_result.clone()).clone() {
                        NodeType::Typed {
                            node: start_type, ..
                        } => match (*end_type_result.clone()).clone() {
                ...
                Rc::new(InferResult {
                    ...
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(base_diags.clone(), start_diags.clone()),
                            end_diags.clone(),
                        ),
...
pub fn infer_record_lit(
...
) -> Rc<InferResult> {
    {
        ...
        let fi_diags = {
            let mut __result = Vec::new();
            for fir in fi_infer_results.clone().iter().cloned() {
                __result.extend(fir.diagnostics.clone());
            }
            __result
        };
        if (type_name.clone() == None) {
        ...
        } else {
            {
                ...
                let is_some_ctor = ((type_name.clone().unwrap() == "Some".to_string())
                    && (local_variant_parent.clone() == expected_optional_parent.clone()));
                let resolved_node = if is_some_ctor.clone() {
                    {
                        let val_field = {
                            let mut __result = Vec::new();
                            for fir in fi_infer_results.clone().iter().cloned() {
                                if (fir.typed_field.clone().name.clone() == "value".to_string()) {
                                    __result.push(fir);
                                }
                            }
                            __result
                        ...
                        match val_field.clone() {
                            Some(val_fir) => with_optional_cardinality(rt_type(
                ...
                if ((item.body.clone() != None) && ((item.params.clone().len() as i64) > 0)) {
                    {
                        ...
                        let fn_scope = item.uses.iter().cloned().fold(
                            fn_scope.clone(),
                            |s: Rc<InferScope>, u: Rc<Node>| {
                                extend_scope(
                                    s.clone(),
                                    resource_use_name(u.clone()),
                        ...
                        Rc::new(TypedItemResult {
                            item: Rc::new(Node {
                                ...
                                properties: item.properties.clone(),
                                type_annotation: typed_anno.clone(),
                                is_self_recursive: expr_has_self_call(
                                    body_typed.clone(),
                                    item.name.clone(),
                                ),
                                    ...
                                    Rc::new(TypedItemResult {
                                        item: Rc::new(Node {
                                            ...
                                            children: {
                                                ...
                                                for c in item.children.clone().iter().cloned() {
                                                    __result.push(
                                                        infer_item(c.clone(), scope.clone())
                                                            .item
                                                            .clone(),
                                                    );
                                                }
...
pub fn build_type_env(
...
) -> Rc<BuildTypeEnvResult> {
    {
        ...
        let source_index = v2_rt::map_get(
            &source_indices,
            module.module.clone().span.clone().file.clone(),
        );
        let kernel_bindings = KERNEL_TYPE_SET
            .keys()
            .cloned()
            .collect::<Vec<_>>()
            .iter()
        ...
        let kernel_bindings = v2_rt::map_insert(
        ...
        let std_import_bindings = std_types_parent_env.clone().iter().cloned().fold(
            <HashMap<String, Rc<TypeBinding>>>::new(),
        ...
        let import_bindings = module
            ...
            .iter()
            .cloned()
            .fold(std_import_bindings.clone(), |acc: _, imp: Rc<ResolvedImport>| {
                match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
                    Some(typed_parent) => {
                        if (imp.module_path.clone() == "std.types".to_string()) {
                            typed_parent
                                ...
                                .fold(acc.clone(), |bacc: _, name: String| {
                                    if is_bridge_placeholder_type_name(name.clone()) {
                                        ...
                                        match v2_rt::map_get(
                                            &typed_parent.type_env.bindings,
                                            name.clone(),
                                        ) {
                                            Some(binding) => v2_rt::map_insert(
                                                bacc.clone(),
                                                name.clone(),
                                                binding.clone(),
                                            ),
                            ...
                            v2_rt::map_merge(acc.clone(), (*typed_parent.type_env.bindings).clone())
                        }
                    }
                    None => acc.clone(),
                }
        ...
        let import_diags = {
            let mut __result = Vec::new();
            for imp in module.resolved_imports.clone().iter().cloned() {
                __result.extend(match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
    Some(_) => vec![],
    None => vec![make_error_node(Rc::new(CompilerDiagnostic::InternalError { message: format!("missing parent environment for imported module '{}' while typechecking '{}'", imp.module_path, module.module.name), span: imp.target_span.clone().clone().unwrap() }), module.module.clone().name.clone())],
        ...
        let local_bindings = module_items(module.module.clone()).iter().cloned().fold(
            <HashMap<String, Rc<TypeBinding>>>::new(),
            |acc: _, item: Rc<Node>| {
                if node_has_structure(item.clone()) {
                    {
                        ...
                        && (item.body.clone() == None))
                    {
                            ...
                            && ((item.children.clone().len() as i64) > 0))
                        {
                            ...
                            if (((((item.params.clone().len() as i64) > 0)
                                && (node_has_structure(item.clone()) == false))
                                && (item.body.clone() == None))
        ...
        let local_name_set = v2_rt::map_values(&local_bindings).iter().cloned().fold(
            <HashMap<String, bool>>::new(),
...
pub fn build_item_info(item: Rc<Node>) -> Rc<ItemInfo> {
    {
        ...
        let res_names = {
            ...
            for u in item.uses.clone().iter().cloned() {
                __result.push(resource_use_name(u.clone()));
            }
            __result
        };
        match kind.clone() {
...
pub fn analyze_item(item: Rc<Node>, env: Rc<TypeEnv>, module_name: String) -> Rc<ItemContribution> {
    {
            ...
            if ((ritem.transport.clone() != None) && ((ritem.children.clone().len() as i64) > 0)) {
                {
                    ...
                    for c in ritem.children.clone().iter().cloned() {
                        __result.push(service_op_entry(c.clone()));
                    }
                    __result
                }
            } else {
            ...
            && ((ritem.children.clone().len() as i64) > 0))
        {
            {
                let root =
                    namespace_root_from_properties(ritem.properties.clone(), ritem.name.clone());
...
pub fn fold_module_contributions(
    mut remaining: Vec<Rc<ItemContribution>>,
    mut resolved_items: Vec<Rc<Node>>,
    mut func_sigs: HashMap<String, Rc<DeclaredFuncSig>>,
    mut svc_registry: HashMap<String, Vec<Rc<OpEntry>>>,
    mut svc_locals: HashMap<String, Rc<TypeBinding>>,
    mut item_registry: HashMap<String, Rc<ItemInfo>>,
    mut diag_chunks: Vec<Vec<Rc<ErrorNode>>>,
        ...
        match remaining.clone().first().cloned() {
            ...
            Some(contribution) => {
                let next_func_sigs = match contribution.func_sig.clone() {
                    Some(sig) => {
                        v2_rt::map_insert(func_sigs.clone(), sig.name.clone(), sig.clone())
                    }
                ...
                let next_svc_registry = if ((contribution.svc_entries.clone().len() as i64) > 0) {
                    v2_rt::map_insert(
                        svc_registry.clone(),
                        contribution.resolved_item.clone().name.clone(),
                ...
                let next_svc_locals = match contribution.svc_local.clone() {
                    Some(binding) => {
                        v2_rt::map_insert(svc_locals.clone(), binding.name.clone(), binding.clone())
                    }
                ...
                };
                {
                    ...
                    let __tco_5 = v2_rt::map_insert(
                        item_registry.clone(),
                        contribution.item_info.clone().name.clone(),
                        contribution.item_info.clone(),
                    );
                    let __tco_6 = v2_rt::list_push(
                        diag_chunks.clone(),
                        contribution.resolve_diagnostics.clone(),
                    );
                    remaining = __tco_0;
...
pub fn build_module_context(
    contributions: Vec<Rc<ItemContribution>>,
    parent_index: HashMap<String, Rc<TypedModule>>,
    resolved_imports: Vec<Rc<ResolvedImport>>,
    env: Rc<TypeEnv>,
    module_name: String,
) -> Rc<ModuleContext> {
    {
        ...
        let all_declared_sigs =
            v2_rt::map_merge(merged_scope.func_sigs.clone(), local.func_sigs.clone());
        let resolve_result = resolve_func_sigs(
            all_declared_sigs.clone(),
            local.resolved_items.clone(),
            module_name.clone(),
        );
        let all_locals = v2_rt::map_values(&merged_scope.svc_locals.clone())
            .iter()
            .cloned()
            .fold(
...
pub fn typecheck_module(
...
) -> Rc<TypecheckModuleResult> {
    {
        ...
        let infer_diags = {
            let mut __result = Vec::new();
            for tir in typed_item_results.clone().iter().cloned() {
                __result.extend(tir.diagnostics.clone());
            }
            __result
        };
        let typed_module = module_node(
            resolved.module.clone().name.clone(),
...
pub fn resolve_env_bindings(
    env: Rc<TypeEnv>,
    module_name: String,
    local_names: HashMap<String, bool>,
...
) -> Rc<EnvResolveResult> {
    {
        let remaining = {
            ...
            for b in {
                let mut __result = Vec::new();
                for b in v2_rt::map_values(&env.bindings.clone()).iter().cloned() {
                    if emit_map_has(local_names.clone(), b.name.clone()) {
                        __result.push(b);
                    }
                }
                __result
...
pub fn topo_resolve_types(
    ...
    mut diagnostics: Vec<Rc<ErrorNode>>,
    mut local_names: HashMap<String, bool>,
    mut deps_map: HashMap<String, Vec<String>>,
) -> Rc<EnvResolveResult> {
    loop {
                    ...
                    Some(deps) => {
                        ...
                        for dep in deps.clone().iter().cloned() {
                            if !((((is_kernel_type(dep.clone())
                                ...
                                || (dep.clone() == "".to_string()))
                                || is_recursive_type(env.clone(), dep.clone()))
        ...
        let ready_accum = ready.clone().iter().cloned().fold(
            ...
            |acc: _, name: String| match v2_rt::map_get(&env.bindings.clone(), name.clone()) {
                Some(binding) => {
                    let result =
                        resolve_node(binding.resolved.clone(), env.clone(), module_name.clone());
                    Rc::new(BindingsAccum {
                        bindings: v2_rt::map_insert(
                            acc.bindings.clone(),
                            name.clone(),
...
pub fn collect_parent_envs(
    resolved: Rc<ResolvedModule>,
    module_index: HashMap<String, Rc<TypedModule>>,
) -> Rc<ParentModulesResult> {
    {
        ...
        let diagnostics = {
            let mut __result = Vec::new();
            for imp in resolved.resolved_imports.clone().iter().cloned() {
                __result.extend(match v2_rt::map_get(&module_index, imp.module_path.clone()) {
    Some(_) => vec![],
...
pub fn build_emit_graph_info(modules: Vec<Rc<TypedModule>>) -> Rc<EmitGraphInfo> {
    {
        ...
        let built = modules.clone().iter().cloned().fold(
            init.clone(),
            |state: _, typed_module: Rc<TypedModule>| {
                typed_module.items.clone().iter().cloned().fold(
                    state.clone(),
                    |inner_state: _, item: Rc<Node>| {
                        add_emit_item_summary(inner_state.clone(), item.clone())
                    },
                )
            },
        );
...
pub fn typecheck_modules(
    mut remaining: Vec<Rc<ResolvedModule>>,
    mut modules: Vec<Rc<TypedModule>>,
    mut module_index: HashMap<String, Rc<TypedModule>>,
    mut item_registry: HashMap<String, Rc<ItemInfo>>,
    mut diag_chunks: Vec<Vec<Rc<ErrorNode>>>,
        ...
        match remaining.clone().first().cloned() {
            ...
            Some(resolved) => {
                ...
                let tc_diags = tc_result.diagnostics.clone();
                {
                    ...
                    let __tco_2 = v2_rt::map_insert(
                        module_index.clone(),
                        typed.module.clone().name.clone(),
                        typed.clone(),
                    );
                    let __tco_3 =
                        v2_rt::map_merge(item_registry.clone(), typed.item_registry.clone());
                    let __tco_4 = v2_rt::list_push(
                        v2_rt::list_push(diag_chunks.clone(), parent_result.diagnostics.clone()),
                        tc_diags.clone(),
                    );
