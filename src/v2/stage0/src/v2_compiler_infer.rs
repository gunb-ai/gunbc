...
pub fn list_kernel_ty_from_element(element: Rc<Node>, span: Rc<SourceSpan>, module_name: String) -> Rc<KernelListTyDiag> {
    match make_container_type(&"List".to_string(), element) {
    Some(n) => Rc::new(KernelListTyDiag {
    ty: n.clone(),
    miss_diags: Rc::new(vec![]),
}),
    None => Rc::new(KernelListTyDiag {
    ty: missing_kernel_container_profile_type("List".to_string()),
    miss_diags: Rc::new(vec![inference_error("kernel List container profile missing (algebra/kernel data mismatch)".to_string(), span, module_name)]),
}),
}
}
...
pub fn infer_tier2b_builtin_with_kernel_diags(func_name: &String, typed_args: Rc<Vec<Rc<Node>>>, scope: &Rc<InferScope>, span: Rc<SourceSpan>) -> Rc<Tier2bBt> {
    ...
    Some(key_type) => {
                let lk = list_kernel_ty_from_element(key_type.clone(), span, scope.module_name.clone());
Rc::new(Tier2bBt {
    bt: lk.ty.clone(),
    ...
    Some(InferredNode::Resolved { node: receiver_type, .. }) => match map_value_type_in_env(receiver_type.clone(), &scope.type_env.clone()) {
    Some(value_type) => {
                    let lv = list_kernel_ty_from_element(value_type.clone(), span, scope.module_name.clone());
Rc::new(Tier2bBt {
    bt: lv.ty.clone(),
...
match bare_m {
    ...
    diagnostics: v2_rt::concat(arg_diags, Rc::new(vec![inference_error("empty_map(): Map kernel container profile missing".to_string(), span.clone(), scope.module_name.clone())])),
                            ...
                            if (infer_builtin_call_type(func_name.clone()) != None) {
                                {
                                    let tier2b = infer_tier2b_builtin_with_kernel_diags(&func_name, typed_args.clone(), &scope, span.clone());
...
let pipe_fb = match method_resolution.result_type.clone() {
    ...
    None => method_pipe_map_keys_values_fallback(&recv_rt, &method_name, &scope, span.clone()),
};
...
Rc::new(InferResult {
    typed: mc_texpr,
    diagnostics: v2_rt::concat(v2_rt::concat(recv_diags, mc_arg_diags), pipe_fb.kernel_diags.clone()),
})
},
    ExprData::ExprMatch => {
            let span = texpr.span.clone();
...
let list_k = list_kernel_ty_from_element(elem_type_node.clone(), span.clone(), scope.module_name.clone());
