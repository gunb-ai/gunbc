...
let then_rt = rt_type(then_result.typed.clone());
let else_rt = rt_type(else_result.typed.clone());
...
let result_type = infer_binop_type_node(op.clone(), rt_type(left_typed.clone()), scope.type_env.clone().source_index.clone());
