            ...
            Some(decl) => match v2_rt::map_get(&env.bindings.clone(), decl.binding_key.clone()) {
                Some(binding) => {
                    if node_has_binding_id(binding.resolved.clone(), binding_id.clone()) {
                        ...
                        v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.clone())
                    }
                }
                None => v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.clone()),
            },
            None => v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.clone()),
        },
        None => None,
    ...
    envs.iter().cloned().fold(
        ...
        |acc: Rc<HashMap<BindingId, Rc<TypeBinding>>>, env: Rc<TypeEnv>| {
            ...
            Rc::new(v2_rt::map_values(&env.bindings.clone()))
                ...
                .fold(
                    with_existing.clone(),
                    |bacc: Rc<HashMap<BindingId, Rc<TypeBinding>>>, binding: Rc<TypeBinding>| {
...
pub fn carrier_closure_direct_type_deps(
...
) -> Rc<Vec<String>> {
    {
        ...
        let decl_ref = match n.binding_id.clone() {
            Some(binding_id) => {
                match v2_rt::map_get(&dep_env.decl_registry.clone(), binding_id.clone()) {
                    Some(decl) => {
...
pub fn add_type_binding_dependency_closure(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let already_seen = match binding.resolved.clone().binding_id.clone() {
            Some(binding_id) => {
                v2_rt::map_contains_key(&state.seen_binding_ids.clone(), binding_id.clone())
            }
