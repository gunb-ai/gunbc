...
pub fn rejects_legacy_anthropic_tool_result_string(
...
) -> bool {
    {
        let is_tool_result_content = match type_name {
            Some(tn) => {
                ((tn.clone().as_str() == "UserToolResultBlock".to_string().as_str())
                    && (field_name.as_str() == "content".to_string().as_str()))
            }
            None => false,
        };
        let value_is_string_literal = match (*value.expr_data.clone()).clone() {
        ...
        let fi_infer_results = Rc::new({
            ...
            for fi in field_inits.iter().cloned() {
                __result.push({
                    ...
                    let legacy_tool_result_diags = if rejects_legacy_anthropic_tool_result_string(
                        type_name.clone(),
                        fi_name.clone(),
                        ar_typed.clone(),
                    ) {
                        Rc::new(vec![type_mismatch_error(
                            "AnthropicToolResultContent?".to_string(),
                            "String".to_string(),
                            ar_typed.span.clone(),
                            scope.module_name.clone(),
                        )])
                    ...
                    Rc::new(FieldInferResult {
                        ...
                        diagnostics: v2_rt::concat(
                            v2_rt::concat(ar_diags.clone(), field_type_diags.clone()),
                            legacy_tool_result_diags.clone(),
                        ),
                    })
                });
            }
