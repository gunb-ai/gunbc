...
pub use crate::v2_compiler_infer_types::{nominal_type_ref, make_container_type, make_callable_type, KernelTypeBuild, node_is_keyed_collection, node_is_element_collection, node_is_collection, node_is_set_collection, is_fully_resolved, resolve_type_variables_from_template, template_return_has_variables, template_return_is_receiver_self, bare_map_node, bare_set_node, callable_inferred, normalize_access_type_node, node_type_shape, node_type_compatible, node_type_equals, prefer_specific_type, node_type_deps, method_receiver_element_node, infer_literal_node, infer_binop_type_node, extract_optional_inner_node, for_each_element_type_node, resolved_type, child_type_node, emit_map_has};
pub use crate::v2_compiler_infer_method::{builtin_kernel_seed_diagnostics, infer_builtin_call_type, resolve_builtin_call_type};
pub use crate::v2_compiler_infer_cycle::{detect_type_cycles_kahn};
pub use crate::v2_compiler_infer_env::{TypeEnv, TypeBinding, is_recursive_type, is_recursive_type_by_name, lookup_type, lookup_type_by_name, lookup_type_for, merge_envs, put_inductive_field, put_inductive_field_cross, merge_inductive_fields, inductive_fields_for, inductive_fields_list_to_map};
pub use crate::std_node::{compiler_inductive_fields, compiler_recursive_types};
pub use crate::std_induction::{InductiveField, RecursionShape, SubValueRelation, ShrinkFactor, sub_value_to_evidence, meet_sub_value, join_sub_value, compose_sub_value, compose_sub_value_relations};
...
pub use crate::std_termination::{positive_descent_amount_from_positive_int, proportional_divisor_from_int_at_least_two, PositiveDescentAmount};
...
pub use crate::v2_compiler_infer_lookup::{KnownMethodResolution, resolve_known_method_node, resolve_scrutinee_type_node, field_summary_for_type, lookup_field_type_node, lookup_coproduct_common_field_node, map_value_type_in_env, map_key_type_in_env, set_element_type_in_env, lookup_in_scope, lookup_func_sig};
...
v2_rt::rc_map_insert(acc, sig_key.clone(), Rc::new(DeclaredFuncSig {
    name: sig_key.clone(),
...
let parent_result = typed_parent.items.clone().iter().cloned().fold(Rc::new(InferScopeComponents {
    ...
    name: authored_name_at(env.source_indices.clone(), &c),
    outputs: inferred_to_outputs(&c.inferred.clone(), c.span.clone(), env.source_indices.clone()),
    params: c.params.clone(),
})); } __result });
let root = namespace_root_from_properties(titem.properties.clone(), authored_name_at(env.source_indices.clone(), &titem), env.source_indices.clone());
Rc::new(InferScopeComponents {
    func_sigs: acc.func_sigs.clone(),
    svc_registry: v2_rt::rc_map_insert(acc.svc_registry.clone(), authored_name_at(env.source_indices.clone(), &titem), entries.clone()),
    ...
    svc_locals: v2_rt::rc_map_insert(acc.svc_locals.clone(), authored_name_at(env.source_indices.clone(), &titem), Rc::new(TypeBinding {
    name: authored_name_at(env.source_indices.clone(), &titem),
...
pub fn classify_field_recursion(field_node: &Rc<Node>, parent_name: &String, recursive_type_set: Rc<std::collections::BTreeSet<String>>, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> Option<Rc<FieldRecursionResult>> {
    {
...
let field_type_name = authored_name_at(source_indices.clone(), &type_expr);
...
let value_type = match type_expr.children.clone().get(value_index as usize).cloned() {
    Some(t) => authored_name_at(source_indices.clone(), &t),
    None => "".to_string(),
};
let is_recursive_element = ((value_type.clone().as_str() == parent_name.clone().as_str()) || set_contains(recursive_type_set, value_type.clone()));
...
match field_type_name.clone().as_str() {
    ...
    _ => None,
                ...
                if set_contains(recursive_type_set, field_type_name.clone()) {
                    match field_node.return_cardinality.clone() {
    Cardinality::CardOptional => Some(Rc::new(FieldRecursionResult {
    shape: RecursionShape::OptionalRecursion,
...
pub fn collect_fields_inductive(fields: Rc<Vec<Rc<Node>>>, parent_name: String, variant_name: String, recursive_type_set: Rc<std::collections::BTreeSet<String>>, acc: Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>> {
    fields.iter().cloned().fold(acc.clone(), |field_acc: Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>>, field_node: Rc<Node>| match classify_field_recursion(&field_node, &parent_name, recursive_type_set.clone(), &source_indices) {
    Some(result) => put_inductive_field_cross(&field_acc, &parent_name, variant_name.clone(), authored_name_at(source_indices.clone(), &field_node), result.shape.clone(), result.element_type.clone()),
    None => field_acc.clone(),
})
}

pub fn collect_item_inductive_fields(item: &Rc<Node>, recursive_type_set: Rc<std::collections::BTreeSet<String>>, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>> {
    {
        let item_name = authored_name_at(source_indices.clone(), &item);
match item.connective.clone() {
    Connective::Disj => item.children.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Vec<Rc<InductiveField>>>>(), |variant_acc: Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>>, variant_node: Rc<Node>| collect_fields_inductive(variant_node.children.clone(), item_name.clone(), authored_name_at(source_indices.clone(), &variant_node), recursive_type_set.clone(), variant_acc, source_indices.clone())),
    Connective::Conj => collect_fields_inductive(item.children.clone(), item_name.clone(), "".to_string(), recursive_type_set.clone(), v2_rt::rc_empty_map::<String, Rc<Vec<Rc<InductiveField>>>>(), source_indices.clone()),
    _ => v2_rt::rc_empty_map::<String, Rc<Vec<Rc<InductiveField>>>>(),
}
}
}

pub fn build_item_inductive_fields(items: Rc<Vec<Rc<Node>>>, recursive_type_set: Rc<std::collections::BTreeSet<String>>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>> {
    items.iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Vec<Rc<InductiveField>>>>(), |acc: Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>>, item: Rc<Node>| merge_inductive_fields(acc, &collect_item_inductive_fields(&item, recursive_type_set.clone(), &source_indices)))
}
...
pub fn namespace_root_from_properties(properties: Rc<Vec<Rc<Node>>>, name: String, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> String {
    match Rc::new({ let mut __result = Vec::new(); for p in properties.iter().cloned() { if (field_init_node_name_at(p.clone(), source_indices.clone()).as_str() == "namespace_root".to_string().as_str()) { __result.push(p); } } __result }).first().cloned() {
...
if is_coproduct {
            if has_child_named(parent.clone(), name.clone(), scope.type_env.clone().source_indices.clone()) {
                Some(authored_name_at(scope.type_env.clone().source_indices.clone(), &binding.resolved.clone()))
            } else {
                None
            }
        } else {
...
pub fn type_mismatch_error(expected: String, got: String, span: Rc<SourceSpan>, module_name: String) -> Rc<ErrorNode> {
    make_error_node(Rc::new(CompilerDiagnostic::TypeMismatch {
    expected: expected,
    got: got,
    span: span,
}), module_name)
...
pub fn rejects_string_for_optional_coproduct_field(expected: &Rc<Node>, got: Rc<Node>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> bool {
    {
        let expected_inner = with_required_cardinality(&expected);
let expected_is_optional_coproduct = ((expected.return_cardinality.clone() == Cardinality::CardOptional) && (expected_inner.connective.clone() == Connective::Disj));
let got_is_string = (authored_name_at(source_indices, &got).as_str() == "String".to_string().as_str());
(expected_is_optional_coproduct && got_is_string)
}
}

pub fn record_lit_expected_fields(type_name: Option<String>, scope: &Rc<InferScope>) -> Rc<Vec<Rc<Node>>> {
    match type_name {
    ...
    Some(parent_name) => match lookup_type_by_name(&scope.type_env.clone(), parent_name.clone()) {
    Some(parent) => match Rc::new({ let mut __result = Vec::new(); for v in parent.children.clone().iter().cloned() { if (authored_name_at(scope.type_env.clone().source_indices.clone(), &v).as_str() == tn.clone().as_str()) { __result.push(v); } } __result }).first().cloned() {
...
pub fn infer_tier2b_builtin_with_kernel_diags(func_name: &String, typed_args: Rc<Vec<Rc<Node>>>, scope: Rc<InferScope>, span: Rc<SourceSpan>) -> Rc<Tier2bBt> {
    if ((func_name.clone().as_str() == "lookup".to_string().as_str()) || (func_name.clone().as_str() == "map_get".to_string().as_str())) {
        Rc::new(Tier2bBt {
...
Rc::new(Tier2bBt {
    ...
    kernel_diags: Rc::new(vec![]),
                ...
                if ((func_name.clone().as_str() == "set_insert".to_string().as_str()) || (func_name.clone().as_str() == "set_union".to_string().as_str())) {
                    Rc::new(Tier2bBt {
    bt: match typed_args.first().cloned() {
    Some(receiver_arg) => match arg_value(&receiver_arg).inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: receiver_type, .. }) => receiver_type.clone(),
    _ => resolve_builtin_call_type(func_name.clone()),
},
    None => resolve_builtin_call_type(func_name.clone()),
},
...
pub fn method_pipe_map_keys_values_fallback(recv_rt: &Rc<Node>, method_name: &String, scope: Rc<InferScope>, span: Rc<SourceSpan>) -> Rc<MethodPipeFallback> {
...
pub fn validate_cast(source_inferred: Option<Rc<InferredNode>>, target_name: &String, span: Rc<SourceSpan>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>, module_name: String) -> Option<Rc<ErrorNode>> {
    match source_inferred.as_deref().cloned() {
    Some(InferredNode::Resolved { node: src_node, .. }) => {
        let source_name = authored_name_at(source_indices, &src_node);
...
if is_coproduct {
                        Some(authored_name_at(scope.type_env.clone().source_indices.clone(), &resolved_scrut_node))
                    } else {
                        None
                    }
}
...
make_field_binding_node(&field_binding_name_at(binding.clone(), scope.type_env.clone().source_indices.clone()), annotate_pattern_parent_enums(field_binding_pattern(binding.clone()), field_subject.clone(), &scope), binding.span.clone(), node_name_span(&binding))
}); } __result });
...
pub fn build_params_scope(scope: &Rc<InferScope>, params: Rc<Vec<Rc<Node>>>) -> Rc<InferScope> {
    {
        let new_locals = params.iter().cloned().fold(scope.locals.clone(), |acc: Rc<HashMap<String, Rc<TypeBinding>>>, p: Rc<Node>| v2_rt::rc_map_insert(acc, param_node_name_at(p.clone(), scope.type_env.clone().source_indices.clone()), Rc::new(TypeBinding {
    name: param_node_name_at(p.clone(), scope.type_env.clone().source_indices.clone()),
...
pub fn extend_scope_with_params(scope: &Rc<InferScope>, params: Rc<Vec<String>>) -> Rc<InferScope> {
    {
        let new_locals = params.iter().cloned().fold(scope.locals.clone(), |acc: Rc<HashMap<String, Rc<TypeBinding>>>, p: String| v2_rt::rc_map_insert(acc, p.clone(), Rc::new(TypeBinding {
    name: p.clone(),
...
extend_scope(&scope, &let_binding_name_at(stmt.clone(), scope.type_env.clone().source_indices.clone()), stmt_type, classify_binding_provenance(&val, &scope))
}
    } else {
        scope.clone()
    },
...
pub fn derive_field_provenance(scope: &Rc<InferScope>, scrutinee_provenance: &Rc<SubValueRelation>, scrutinee_subject: Rc<PatternSubject>, variant_name: String, field_name: String) -> Rc<SubValueRelation> {
    ...
    PatternSubject::PatternResolved { node: n, .. } => authored_name_at(scope.type_env.clone().source_indices.clone(), &resolved_type(n.clone())),
    _ => "".to_string(),
};
...
bindings.clone().iter().cloned().fold(Rc::new(PatternScopeResult {
    ...
    diagnostics: variant_diags,
                ...
                let field_name = field_binding_name_at(fb.clone(), scope.type_env.clone().source_indices.clone());
let field_lookup = lookup_field_in_variant(variant_subject.clone(), &field_name, scope.module_name.clone(), &scope.type_env.clone().source_indices.clone());
let field_subject = lookup_result_subject(field_lookup.clone());
let field_type = pattern_binding_type(field_subject.clone());
let field_provenance = derive_field_provenance(&scope, &scrutinee_provenance, scrutinee_subject.clone(), vname.clone(), field_name.clone());
...
pub fn arg_has_name(arg: Rc<Node>, name: String, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> bool {
    {
...
if (is_fold && ((method_args.clone().len() as i64) >= min_args)) {
            {
                let init_arg = match Rc::new({ let mut __result = Vec::new(); for a in method_args.clone().iter().cloned() { if arg_has_name(a.clone(), "init".to_string(), scope.type_env.clone().source_indices.clone()) { __result.push(a); } } __result }).first().cloned() {
    Some(a) => Some(a.clone()),
...
Some(Rc::new(ArgInferResult {
    typed_arg: make_arg_node(arg_name_at(ia.clone(), scope.type_env.clone().source_indices.clone()), init_result.typed.clone(), ia.span.clone(), ia.span.clone()),
...
match (*coll_prov).clone() {
    SubValueRelation::SubValueUnknown => Rc::new(SubValueRelation::SubValueUnknown),
    _ => if (authored_name_at(scope.type_env.clone().source_indices.clone(), &elem_type).as_str() == "".to_string().as_str()) {
            Rc::new(SubValueRelation::SubValueUnknown)
        } else {
            {
                let ind_fields = inductive_fields_for(scope.type_env.clone(), authored_name_at(scope.type_env.clone().source_indices.clone(), &elem_type));
...
let is_init_arg = (arg_has_name(a.clone(), "init".to_string(), scope.type_env.clone().source_indices.clone()) || ((arg_name_at(a.clone(), scope.type_env.clone().source_indices.clone()) == None) && (idx.clone() == 0)));
...
let prov_map = match (*elem_provenance.clone()).clone() {
    SubValueRelation::SubValueUnknown => v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
    _ => v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), "elem".to_string(), elem_provenance.clone()),
};
...
pub fn seed_override_map() -> Rc<HashMap<String, Rc<Node>>> {
    Rc::new(vec!["".to_string()]).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Node>>(), |acc: Rc<HashMap<String, Rc<Node>>>, k: String| v2_rt::rc_map_insert(acc, k.clone(), unit_type()))
}
...
Rc::new(ArgInferResult {
    typed_arg: make_arg_node(arg_name_at(arg.clone(), scope.type_env.clone().source_indices.clone()), ar.typed.clone(), arg.span.clone(), arg.span.clone()),
    diagnostics: ar.diagnostics.clone(),
})
}
}
...
Rc::new(InferResult {
    ...
    ExprData::ExprVar { .. } => {
            let name = expr_var_name_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
match v2_rt::map_get(&scope.locals.clone(), name.clone()) {
    Some(binding) => {
...
Rc::new(InferResult {
    ...
    ExprData::ExprFieldAccess { .. } => {
            let field_name = field_access_field_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
...
if resolved_base_is_error {
    ...
    diagnostics: base_diags,
                ...
                match lookup_field_type_node(&resolved_base, &field_name, scope.type_env.clone().source_indices.clone()) {
    Some(field_type) => {
...
Rc::new(InferResult {
    ...
    None => {
                            let error_message = v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("no field '".to_string(), field_name.clone()), "' on type '".to_string()), authored_name_at(scope.type_env.clone().source_indices.clone(), &resolved_base)), "'".to_string());
...
Rc::new(InferResult {
    ...
    ExprData::ExprCall { .. } => {
            let func_name = expr_call_func_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
...
let elem_type = for_each_element_type_node(first_type, scope.type_env.clone().source_indices.clone());
...
v2_rt::concat(Rc::new(vec![Rc::new(ArgInferResult {
    typed_arg: make_arg_node(arg_name_at(first_arg.clone(), scope.type_env.clone().source_indices.clone()), first_result.typed.clone(), first_arg.span.clone(), first_arg.span.clone()),
...
let call_acc_is_under_resolved = !is_fully_resolved(&call_fold_acc_type, &scope.type_env.clone().source_indices.clone());
...
let method_tv = if (method_resolution.semantics.clone() == None) {
    ...
    Some(t) => if (template_return_has_variables(t.clone()) || !is_fully_resolved(&base_result_type, &scope.type_env.clone().source_indices.clone())) {
                                    {
...
resolve_type_variables_from_template(&t, call_arg_types, &first_arg_type, fold_overrides, &scope.type_env.clone().source_indices.clone())
}
                                } else {
                                    Rc::new(KernelTypeBuild {
    ty: base_result_type.clone(),
...
let final_receiver = if ((returns_receiver_self && is_fully_resolved(&bridge_result_type, &scope.type_env.clone().source_indices.clone())) && !is_fully_resolved(&first_arg_type, &scope.type_env.clone().source_indices.clone())) {
                                Rc::new(Node {
...
let remaining_arg_nodes = Rc::new({ let mut __result = Vec::new(); for ta in remaining.clone().iter().cloned() { __result.push(make_arg_node(arg_name_at(ta.clone(), scope.type_env.clone().source_indices.clone()), arg_value(&ta), span.clone(), span.clone())); } __result });
Rc::new(InferResult {
    ...
    diagnostics: v2_rt::concat(v2_rt::concat(arg_diags, method_resolution.diagnostics.clone()), template_subst_diags),
})
}
                    } else {
...
match bare_s {
    ...
    diagnostics: v2_rt::concat(arg_diags, Rc::new(vec![inference_error("empty_set(): Set kernel container profile missing".to_string(), span.clone(), scope.module_name.clone())])),
                                ...
                                if (infer_builtin_call_type(func_name.clone()) != None) {
                                    {
                                        let tier2b = infer_tier2b_builtin_with_kernel_diags(&func_name, typed_args.clone(), scope.clone(), span.clone());
...
Rc::new(InferResult {
    ...
    diagnostics: v2_rt::concat(arg_diags, tier2b.kernel_diags.clone()),
})
}
                                } else {
                                    {
...
Rc::new(InferResult {
    ...
    diagnostics: arg_diags,
                                            ...
                                            {
                                                let type_match = lookup_type_by_name(&scope.type_env.clone(), func_name.clone());
...
Rc::new(InferResult {
    ...
    ExprData::ExprMethodCall { .. } => {
            let method_name = expr_method_name_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
...
let recv_elem_type = for_each_element_type_node(recv_rt.clone(), scope.type_env.clone().source_indices.clone());
...
let mc_acc_is_under_resolved = !is_fully_resolved(&fold_acc_type, &scope.type_env.clone().source_indices.clone());
...
let pipe_fb = match method_resolution.result_type.clone() {
    ...
    None => method_pipe_map_keys_values_fallback(&recv_rt, &method_name, scope.clone(), span.clone()),
};
...
resolve_type_variables_from_template(&t, mc_arg_types, &recv_rt, mc_fold_overrides, &scope.type_env.clone().source_indices.clone())
}
                } else {
                    Rc::new(KernelTypeBuild {
    ty: base_result_type.clone(),
...
Rc::new(InferResult {
    typed: mc_texpr,
    diagnostics: v2_rt::concat(v2_rt::concat(v2_rt::concat(recv_diags, mc_arg_diags), pipe_fb.kernel_diags.clone()), v2_rt::concat(method_resolution.diagnostics.clone(), mc_template_diags)),
})
},
    ExprData::ExprMatch => {
            let span = texpr.span.clone();
...
let unified_arm_type = match arm_body_types.clone().first().cloned() {
    Some(first_type) => Rc::new(arm_body_types.clone().iter().cloned().skip(1 as usize).collect::<Vec<_>>()).iter().cloned().fold(first_type, |acc: Rc<Node>, t: Rc<Node>| prefer_specific_type(acc, t.clone(), &scope.type_env.clone().source_indices.clone())),
    None => scrut_rt.clone(),
};
let arm_infer_results = if is_fully_resolved(&unified_arm_type, &scope.type_env.clone().source_indices.clone()) {
...
if !is_fully_resolved(&original.body_type.clone(), &scope.type_env.clone().source_indices.clone()) {
                        {
                            let arm_pat = arm_pattern(arm_node.clone());
let arm_g = arm_guard(&arm_node);
let arm_b = arm_body(&arm_node);
...
let then_result = if (!is_fully_resolved(&then_rt, &scope.type_env.clone().source_indices.clone()) && is_fully_resolved(&else_rt, &scope.type_env.clone().source_indices.clone())) {
                    infer_expr(&then_expr, &scope, &Some(else_rt.clone()))
                } else {
                    then_result.clone()
                };
let else_result = if (!is_fully_resolved(&else_rt, &scope.type_env.clone().source_indices.clone()) && is_fully_resolved(&then_rt, &scope.type_env.clone().source_indices.clone())) {
...
let unified = prefer_specific_type(then_rt.clone(), else_rt.clone(), &scope.type_env.clone().source_indices.clone());
let branch_diags = if node_type_compatible(then_rt.clone(), else_rt.clone(), scope.type_env.clone().source_indices.clone()) {
                    Rc::new(vec![])
                } else {
                    Rc::new(vec![inference_error(v2_rt::concat(v2_rt::concat(v2_rt::concat("if branches resolve to incompatible types: ".to_string(), node_type_shape(&then_rt, &scope.type_env.clone().source_indices.clone())), " vs ".to_string()), node_type_shape(&else_rt, &scope.type_env.clone().source_indices.clone())), span.clone(), scope.module_name.clone())])
                };
...
Rc::new(InferResult {
    ...
    ExprData::ExprLet => {
            let let_name = let_binding_name_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
...
let is_tail_return = match body_expr.clone() {
    ...
    ExprData::ExprVar { .. } => (authored_name_at(scope.type_env.clone().source_indices.clone(), &bd).as_str() == let_name.clone().as_str()),
    _ => false,
},
    None => false,
};
...
infer_record_lit(&record_lit_type_name_at(texpr.clone(), scope.type_env.clone().source_indices.clone()), field_inits, &span, node_name_span(&texpr), &scope)
},
    ExprData::ExprListLit => {
            let span = texpr.span.clone();
let elements = texpr.children.clone();
let elem_expected = match expected.clone() {
    Some(exp) => if node_is_element_collection(&exp, scope.type_env.clone().source_indices.clone()) {
...
Rc::new(InferResult {
    typed: ll_texpr,
    diagnostics: v2_rt::concat(v2_rt::concat(elem_diags, empty_list_diags), list_k.miss_diags.clone()),
})
},
...
let binop_info = infer_binop_type_node(&op, &resolved_type(left_typed.clone()), scope.type_env.clone().source_indices.clone());
...
Rc::new(InferResult {
    ...
    ExprData::ExprForEach => {
            let variable = foreach_variable_at(texpr.clone(), scope.type_env.clone().source_indices.clone());
let span = texpr.span.clone();
...
let elem_type_node = for_each_element_type_node(resolved_type(coll_typed.clone()), scope.type_env.clone().source_indices.clone());
...
let index_check = match base_typed.inferred.clone().as_deref().cloned() {
    ...
    Some(InferredNode::Resolved { node: index_type, .. }) => Some(check_index_access_node(base_type.clone(), index_type.clone(), &span, scope.module_name.clone(), &scope.type_env.clone().source_indices.clone())),
    _ => None,
},
    _ => None,
};
...
let slice_check = match base_typed.inferred.clone().as_deref().cloned() {
    ...
    Some(InferredNode::Resolved { node: end_type, .. }) => Some(check_slice_access_node(base_type.clone(), start_type.clone(), end_type.clone(), &span, &scope.module_name.clone(), &scope.type_env.clone().source_indices.clone())),
    _ => None,
},
    _ => None,
},
...
let fi_infer_results = Rc::new({ let mut __result = Vec::new(); for fi in field_inits.iter().cloned() { __result.push({
            let fi_name = field_init_node_name_at(fi.clone(), scope.type_env.clone().source_indices.clone());
let field_expected = match Rc::new({ let mut __result = Vec::new(); for sf in struct_fields.clone().iter().cloned() { if (authored_name_at(scope.type_env.clone().source_indices.clone(), &sf).as_str() == fi_name.clone().as_str()) { __result.push(sf); } } __result }).first().cloned() {
    Some(sf) => {
...
if rejects_string_for_optional_coproduct_field(&expected_node, got_node.clone(), scope.type_env.clone().source_indices.clone()) {
                    Rc::new(vec![type_mismatch_error(node_type_shape(&expected_node, &scope.type_env.clone().source_indices.clone()), node_type_shape(&got_node, &scope.type_env.clone().source_indices.clone()), ar_typed.span.clone(), scope.module_name.clone())])
                } else {
                    Rc::new(vec![])
                }
},
...
if (type_name.clone() == None) {
    ...
    name: field_init_node_name_at(fir.typed_field.clone(), scope.type_env.clone().source_indices.clone()),
...
Rc::new(InferResult {
    ...
    diagnostics: fi_diags,
            ...
            {
                let type_lookup = lookup_type_by_name(&scope.type_env.clone(), type_name.clone().unwrap());
...
let resolved_node = if is_some_ctor {
                    {
                        let val_field = Rc::new({ let mut __result = Vec::new(); for fir in fi_infer_results.clone().iter().cloned() { if (field_init_node_name_at(fir.typed_field.clone(), scope.type_env.clone().source_indices.clone()).as_str() == "value".to_string().as_str()) { __result.push(fir); } } __result }).first().cloned();
match val_field {
...
pub fn classify_size_expr(val: &Rc<Node>, ctx: &Rc<DescentContext>) -> Option<Rc<SizeExpr>> {
    ...
    ExprData::ExprMethodCall { .. } => {
        let mname = expr_method_name_at(val.clone(), ctx.type_env.clone().source_indices.clone());
if (mname.as_str() == "count".to_string().as_str()) {
            {
                let receiver = method_receiver(val.clone());
match (*receiver.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                    let rname = expr_var_name_at(receiver.clone(), ctx.type_env.clone().source_indices.clone());
...
pub fn proportional_skip_alias_plus_literal(alias_expr: &Rc<Node>, lit_expr: Rc<Node>, param_name: &String, ctx: &Rc<DescentContext>) -> Option<Rc<SubValueRelation>> {
    match (*alias_expr.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
        let aname = expr_var_name_at(alias_expr.clone(), ctx.type_env.clone().source_indices.clone());
...
pub fn classify_collection_shrink(arg_expr: &Rc<Node>, param_name: &String, ctx: &Rc<DescentContext>) -> Rc<SubValueRelation> {
    ...
    ExprData::ExprMethodCall { .. } => {
        let mname = expr_method_name_at(arg_expr.clone(), ctx.type_env.clone().source_indices.clone());
...
Rc::new(SubValueRelation::StrictSubValue {
    ...
    ExprData::ExprVar { .. } => {
                                    let aname = expr_var_name_at(arg_val.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.size_aliases.clone(), aname).as_deref().cloned() {
    Some(SizeExpr::DividedSize { param: p, divisor: d, .. }) => if (p.clone().as_str() == param_name.clone().as_str()) {
                                        match proportional_divisor_from_int_at_least_two(d.clone()) {
    Some(div_w) => {
...
Rc::new(SubValueRelation::StrictSubValue {
    ...
    BinOp::Add => if (mname.clone().as_str() == "skip".to_string().as_str()) {
                                    {
...
match proportional_skip_alias_plus_literal(&left, right.clone(), &param_name, &ctx) {
    ...
    BinOp::Sub => Rc::new(SubValueRelation::SubValueUnknown),
    _ => Rc::new(SubValueRelation::SubValueUnknown),
},
    _ => Rc::new(SubValueRelation::SubValueUnknown),
}
...
pub fn maybe_insert_composed_field_relation(acc: Rc<HashMap<String, Rc<SubValueRelation>>>, fname: String, composed: Option<Rc<SubValueRelation>>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    match composed {
    Some(rel) => match (*rel.clone()).clone() {
    SubValueRelation::SubValueUnknown => acc,
    _ => v2_rt::rc_map_insert(acc, fname, rel.clone()),
},
    None => acc,
}
}

pub fn maybe_insert_output_field_relation(acc: Rc<HashMap<String, Rc<SubValueRelation>>>, idx: i64, fname: String, sig: Rc<ResolvedFuncSig>, val: Rc<Node>, ctx: Rc<DescentContext>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    {
        let composed = match sig.output_provenance.clone().get(idx as usize).cloned() {
    Some(param_map) => classify_call_via_provenance(val, &param_map, ctx),
    None => None,
};
...
pub fn maybe_insert_match_field_relation(fp_acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: &String, field_label: String, vname: String, scrut_rel: Option<Rc<SubValueRelation>>, param_types: Rc<HashMap<String, String>>, type_env: Rc<TypeEnv>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    match scrut_rel {
    Some(rel) => {
...
if (lookup_type.clone().as_str() != "".to_string().as_str()) {
            {
                let ind_fields = inductive_fields_for(type_env, lookup_type.clone());
let matching = Rc::new({ let mut __result = Vec::new(); for f in ind_fields.iter().cloned() { if ((f.field_name.clone().as_str() == field_label.clone().as_str()) && ((f.variant_name.clone().as_str() == vname.clone().as_str()) || (f.variant_name.clone().as_str() == "".to_string().as_str()))) { __result.push(f); } } __result }).first().cloned();
match matching {
    Some(ind_field) => v2_rt::rc_map_insert(fp_acc, pname.clone(), compose_sub_value(rel.clone(), ind_field.clone())),
    None => fp_acc,
}
}
        } else {
...
pub fn build_per_field_for_let(val: &Rc<Node>, binding_name: String, ctx: &Rc<DescentContext>) -> Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    match (*val.expr_data.clone()).clone() {
    ExprData::ExprCall { .. } => {
        let callee = expr_call_func_at(val.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.func_sigs.clone(), callee) {
    Some(sig) => if ((sig.output_provenance.clone().len() as i64) > 1) {
            {
                let return_type = sig.inferred.clone();
let field_map = Rc::new(return_type.children.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pair: (i64, Rc<Node>)| {
                    let idx = pair.0.clone();
let field_node = pair.1.clone();
let fname = authored_name_at(ctx.type_env.clone().source_indices.clone(), &field_node);
maybe_insert_output_field_relation(acc, idx.clone(), fname.clone(), sig.clone(), val.clone(), ctx.clone())
});
if ((Rc::new(v2_rt::map_keys(&field_map)).len() as i64) > 0) {
                    v2_rt::rc_map_insert(ctx.per_field_vars.clone(), binding_name, field_map.clone())
                } else {
    ...
    ExprData::ExprMethodCall { .. } => {
        let callee = expr_method_name_at(val.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.func_sigs.clone(), callee) {
    Some(sig) => if ((sig.output_provenance.clone().len() as i64) > 1) {
            {
                let return_type = sig.inferred.clone();
...
let fname = field_access_field_at(expr.clone(), ctx.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => { let bname = expr_var_name_at(base.clone(), ctx.type_env.clone().source_indices.clone());
...
if (base_type.clone().as_str() != "".to_string().as_str()) {
    ...
    ExprData::ExprVar { .. } => { let rname = expr_var_name_at(expr.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.sub_value_vars.clone(), rname) {
    ...
    ExprData::ExprMethodCall { .. } => { let inner_mname = expr_method_name_at(expr.clone(), ctx.type_env.clone().source_indices.clone());
...
pub fn classify_binding_provenance(val: &Rc<Node>, scope: &Rc<InferScope>) -> Rc<SubValueRelation> {
    match (*val.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
        let vname = expr_var_name_at(val.clone(), scope.type_env.clone().source_indices.clone());
...
let field = field_access_field_at(val.clone(), scope.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
            let bname = expr_var_name_at(base.clone(), scope.type_env.clone().source_indices.clone());
match v2_rt::map_get(&scope.locals.clone(), bname) {
    Some(binding) => {
                let fields = inductive_fields_for(scope.type_env.clone(), authored_name_at(scope.type_env.clone().source_indices.clone(), &binding.resolved.clone()));
...
match v2_rt::map_get(&ctx.scope_locals.clone(), vname) {
    ...
    ExprData::ExprFieldAccess { .. } => {
        let base = field_access_base(val.clone());
let field = field_access_field_at(val.clone(), ctx.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
            let bname = expr_var_name_at(base.clone(), ctx.type_env.clone().source_indices.clone());
let from_per_field = match v2_rt::map_get(&ctx.per_field_vars.clone(), bname.clone()) {
    Some(field_map) => v2_rt::map_get(&field_map, field.clone()),
    None => None,
};
...
match (*inner.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                        let iname = expr_var_name_at(inner.clone(), ctx.type_env.clone().source_indices.clone());
...
match cf {
    ...
    Some(InferredNode::Resolved { node: rt, .. }) => if is_container_type(authored_name_at(ctx.type_env.clone().source_indices.clone(), &rt)) {
            {
                let elem = match rt.children.clone().first().cloned() {
    Some(c) => authored_name_at(ctx.type_env.clone().source_indices.clone(), &c),
    None => "".to_string(),
};
if ((elem.clone().as_str() != "".to_string().as_str()) && is_recursive_type_by_name(&ctx.type_env.clone(), elem.clone())) {
                    {
...
if ((v2_rt::map_get(&ctx.param_names.clone(), lname.clone()) != None) || (v2_rt::map_get(&ctx.sub_value_vars.clone(), lname.clone()) != None)) {
    ...
    ExprData::ExprLiteral { ref value, .. } => { let LiteralValue::LitInt { value: k, .. } = value.as_ref() else { unreachable!() }; match positive_descent_amount_from_positive_int(k.clone()) {
...
match (*inner_left.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                        let lname = expr_var_name_at(inner_left.clone(), ctx.type_env.clone().source_indices.clone());
if ((v2_rt::map_get(&ctx.param_names.clone(), lname.clone()) != None) || (v2_rt::map_get(&ctx.sub_value_vars.clone(), lname.clone()) != None)) {
                            lname.clone()
                        } else {
                            "".to_string()
...
if (left_name.clone().as_str() != "".to_string().as_str()) {
                    match (*right.expr_data.clone()).clone() {
    ExprData::ExprLiteral { ref value, .. } => { let LiteralValue::LitInt { value: k, .. } = value.as_ref() else { unreachable!() }; match proportional_divisor_from_int_at_least_two(k.clone()) {
    Some(div_w) => Some(Rc::new(SubValueRelation::ArithmeticDescent {
...
let arg_by_name = call_args.iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Node>>(), |acc: Rc<HashMap<String, Rc<Node>>>, arg_node: Rc<Node>| match arg_name_at(arg_node.clone(), ctx.type_env.clone().source_indices.clone()) {
...
pub fn classify_call_arg_provenance(arg_expr: &Rc<Node>, ctx: &Rc<DescentContext>) -> Option<Rc<SubValueRelation>> {
    match (*arg_expr.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
        let name = expr_var_name_at(arg_expr.clone(), ctx.type_env.clone().source_indices.clone());
...
pub fn merge_argument_relations(rels: &Rc<Vec<Rc<SubValueRelation>>>) -> Rc<SubValueRelation> {
    ...
    None => Rc::new(SubValueRelation::SubValueUnknown),
    Some(first_rel) => Rc::new(rels.clone().iter().cloned().skip(1 as usize).collect::<Vec<_>>()).iter().cloned().fold(first_rel.clone(), |acc: Rc<SubValueRelation>, rel: Rc<SubValueRelation>| meet_sub_value(acc, rel.clone())),
}
}
...
pub fn classify_argument(arg_expr: &Rc<Node>, param_name: &String, ctx: &Rc<DescentContext>) -> Rc<SubValueRelation> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*arg_expr.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
            let vname = expr_var_name_at(arg_expr.clone(), ctx.type_env.clone().source_indices.clone());
...
let field = field_access_field_at(arg_expr.clone(), ctx.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                let bname = expr_var_name_at(base.clone(), ctx.type_env.clone().source_indices.clone());
...
match (*inner_rel.clone()).clone() {
    ...
    SubValueRelation::PreservedValue => {
                    let callee = expr_call_func_at(arg_expr.clone(), ctx.type_env.clone().source_indices.clone());
let from_provenance = match v2_rt::map_get(&ctx.func_sigs.clone(), callee.clone()) {
    Some(sig) => match sig.output_provenance.clone().first().cloned() {
...
match op.clone() {
    ...
    ExprData::ExprVar { .. } => ((expr_var_name_at(left.clone(), ctx.type_env.clone().source_indices.clone()).as_str() == param_name.clone().as_str()) || (v2_rt::map_get(&ctx.sub_value_vars.clone(), expr_var_name_at(left.clone(), ctx.type_env.clone().source_indices.clone())) != None)),
    _ => false,
};
if left_is_param {
                    match (*right.expr_data.clone()).clone() {
    ExprData::ExprLiteral { ref value, .. } => { let LiteralValue::LitInt { value: k, .. } = value.as_ref() else { unreachable!() }; match positive_descent_amount_from_positive_int(k.clone()) {
    Some(steps) => Rc::new(SubValueRelation::ArithmeticDescent {
    param: param_name.clone(),
    factor: Rc::new(ShrinkFactor::ConstantShrink {
    steps: steps.clone(),
}),
    ...
    ExprData::ExprBinOp { op: BinOp::Sub, .. } => {
                    let inner_left = binop_left(left.clone());
match (*inner_left.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => ((expr_var_name_at(inner_left.clone(), ctx.type_env.clone().source_indices.clone()).as_str() == param_name.clone().as_str()) || (v2_rt::map_get(&ctx.sub_value_vars.clone(), expr_var_name_at(inner_left.clone(), ctx.type_env.clone().source_indices.clone())) != None)),
    _ => false,
}
},
    _ => false,
...
if left_is_param {
                    match (*right.expr_data.clone()).clone() {
    ExprData::ExprLiteral { ref value, .. } => { let LiteralValue::LitInt { value: k, .. } = value.as_ref() else { unreachable!() }; match proportional_divisor_from_int_at_least_two(k.clone()) {
    Some(div_w) => Rc::new(SubValueRelation::ArithmeticDescent {
    param: param_name.clone(),
    factor: Rc::new(ShrinkFactor::ProportionalShrink {
    divisor: div_w.clone(),
}),
...
match v2_rt::map_get(&ctx.scope_locals.clone(), bname) {
    ...
    SubValueRelation::SubValueUnknown => classify_argument(&arg_expr, &param_name, &ctx),
    _ => {
                let fields = inductive_fields_for(ctx.type_env.clone(), authored_name_at(ctx.type_env.clone().source_indices.clone(), &binding.resolved.clone()));
let matching = Rc::new({ let mut __result = Vec::new(); for f in fields.iter().cloned() { if (f.field_name.clone().as_str() == field.clone().as_str()) { __result.push(f); } } __result }).first().cloned();
match matching {
    Some(ind_field) => compose_sub_value(binding.provenance.clone(), ind_field.clone()),
    None => Rc::new(SubValueRelation::SubValueUnknown),
...
pub fn build_call_evidence(call_node: &Rc<Node>, ctx: &Rc<DescentContext>) -> Rc<Vec<Rc<SubValueRelation>>> {
    {
        let arg_map = Rc::new(call_node.children.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Node>>(), |acc: Rc<HashMap<String, Rc<Node>>>, pair: (i64, Rc<Node>)| {
            let idx = pair.0.clone();
let arg_node = pair.1.clone();
let aname = arg_name_at(arg_node.clone(), ctx.type_env.clone().source_indices.clone());
match aname.clone() {
    Some(pname) => v2_rt::rc_map_insert(acc.clone(), pname.clone(), arg_value(&arg_node)),
    None => acc.clone(),
}
});
let positional_args = Rc::new({ let mut __result = Vec::new(); for arg_node in Rc::new({ let mut __result = Vec::new(); for arg_node in call_node.children.clone().iter().cloned() { if (arg_name_at(arg_node.clone(), ctx.type_env.clone().source_indices.clone()) == None) { __result.push(arg_node); } } __result }).iter().cloned() { __result.push(arg_value(&arg_node)); } __result });
...
match (*arg_val.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                let vname = expr_var_name_at(arg_val.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.param_names.clone(), vname) {
    Some(t) => t.clone(),
    None => "".to_string(),
}
},
...
let source_type = body.children.clone().iter().cloned().fold("".to_string(), |found: String, arg_node: Rc<Node>| descent_source_type(found, &arg_node, &ctx));
...
make_arg_node(arg_name_at(child.clone(), ctx.type_env.clone().source_indices.clone()), annotate_descent(&arg_val, &lambda_ctx), child.span.clone(), child.span.clone())
}
                } else {
                    make_arg_node(arg_name_at(child.clone(), ctx.type_env.clone().source_indices.clone()), annotate_descent(&arg_val, &ctx), child.span.clone(), child.span.clone())
                },
    _ => make_arg_node(arg_name_at(child.clone(), ctx.type_env.clone().source_indices.clone()), annotate_descent(&arg_val, &ctx), child.span.clone(), child.span.clone()),
}
}); } __result });
Rc::new(Node {
...
let scrut_type = match (*scrut.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                let sname = expr_var_name_at(scrut.clone(), ctx.type_env.clone().source_indices.clone());
...
let fname = field_access_field_at(scrut.clone(), ctx.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                    let bname = expr_var_name_at(base.clone(), ctx.type_env.clone().source_indices.clone());
let base_type = match v2_rt::map_get(&ctx.param_names.clone(), bname.clone()) {
...
Rc::new({ let mut __result = Vec::new(); for f in ind_fields.clone().iter().cloned() { if (f.field_name.clone().as_str() == fname.clone().as_str()) { __result.push(f); } } __result }).first().cloned()
    ...
    ExprData::ExprMethodCall { .. } => {
                let mname = expr_method_name_at(scrut.clone(), ctx.type_env.clone().source_indices.clone());
...
let scrut_variant_sig = match (*scrut.expr_data.clone()).clone() {
    ExprData::ExprCall { .. } => {
                let callee = expr_call_func_at(scrut.clone(), ctx.type_env.clone().source_indices.clone());
match v2_rt::map_get(&ctx.func_sigs.clone(), callee) {
    Some(sig) => if ((Rc::new(v2_rt::map_keys(&sig.variant_provenance.clone())).len() as i64) > 0) {
                    Some(sig.clone())
                } else {
                    None
                },
...
let annotated_arms = Rc::new({ let mut __result = Vec::new(); for arm_node in match_arm_nodes(body.clone()).iter().cloned() { __result.push({
    ...
    Some(sig) => arm_ctx_from_variant_provenance(arm_node.clone(), sig.clone(), scrut.clone(), &ctx),
    None => None,
};
...
let binding_name = let_binding_name_at(body.clone(), ctx.type_env.clone().source_indices.clone());
...
let inner_per_field = build_per_field_for_let(&val, binding_name.clone(), &ctx);
...
let bname = let_binding_name_at(child.clone(), ctx.type_env.clone().source_indices.clone());
...
Rc::new(Node {
    ...
    ExprData::ExprMethodCall { method_semantics: ms, .. } => {
            let mname = expr_method_name_at(body.clone(), ctx.type_env.clone().source_indices.clone());
...
let receiver = match (*raw_receiver.expr_data.clone()).clone() {
    ExprData::ExprMethodCall { .. } => {
                        let inner_mname = expr_method_name_at(raw_receiver.clone(), ctx.type_env.clone().source_indices.clone());
...
let fname = field_access_field_at(receiver.clone(), ctx.type_env.clone().source_indices.clone());
match (*base.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => {
                            let bname = expr_var_name_at(base.clone(), ctx.type_env.clone().source_indices.clone());
let base_type = match v2_rt::map_get(&ctx.param_names.clone(), bname.clone()) {
...
let lparams = lambda_param_names_at(lambda_node, ctx.type_env.clone().source_indices.clone());
lparams.clone().get(elem_position.clone().unwrap() as usize).cloned()
},
    None => None,
};
...
let inner_ctx = match lambda_elem_info {
    ...
    sub_value_vars: if is_fold {
                        v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), elem_name.clone(), Rc::new(SubValueRelation::IteratedSubValue {
    field: ind_field.clone(),
}))
                    } else {
...
if is_lambda.clone() {
                            make_arg_node(match arg_name_at(arg_node.clone(), ctx.type_env.clone().source_indices.clone()) {
    Some(n) => Some(n.clone()),
    None => None,
...
let fe_var = foreach_variable_at(body.clone(), ctx.type_env.clone().source_indices.clone());
...
let inner_ctx = match coll_field {
    ...
    param_order: ctx.param_order.clone(),
    type_env: ctx.type_env.clone(),
    sub_value_vars: v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), fe_var.clone(), Rc::new(SubValueRelation::IteratedSubValue {
    field: ind_field.clone(),
})),
    size_aliases: ctx.size_aliases.clone(),
    scope_locals: ctx.scope_locals.clone(),
    ...
    param_order: ctx.param_order.clone(),
    type_env: ctx.type_env.clone(),
    sub_value_vars: v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
    size_aliases: ctx.size_aliases.clone(),
    scope_locals: ctx.scope_locals.clone(),
    func_sigs: ctx.func_sigs.clone(),
    per_field_vars: ctx.per_field_vars.clone(),
...
pub fn resolved_type_name(n: &Rc<Node>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> String {
    match n.inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: rt, .. }) => authored_name_at(source_indices, &rt),
    _ => match n.children.clone().first().cloned() {
    Some(c) => authored_name_at(source_indices, &c),
    None => "".to_string(),
},
}
}
...
pub fn infer_output_provenance(body: &Rc<Node>, params: &Rc<Vec<Rc<Node>>>, type_env: &Rc<TypeEnv>, scope_locals: Rc<HashMap<String, Rc<TypeBinding>>>, func_sigs: &Rc<HashMap<String, Rc<ResolvedFuncSig>>>) -> Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    {
        let param_names = Rc::new({ let mut __result = Vec::new(); for p in params.clone().iter().cloned() { __result.push(param_node_name_at(p.clone(), type_env.source_indices.clone())); } __result });
let param_types = params.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, String>(), |acc: Rc<HashMap<String, String>>, p: Rc<Node>| {
            let pname = param_node_name_at(p.clone(), type_env.source_indices.clone());
let ptype = resolved_type_name(&p, type_env.source_indices.clone());
...
let scalar = classify_body_provenance(&body, &param_names, &param_types, &type_env, &func_sigs, &v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>());
if ((Rc::new(v2_rt::map_keys(&scalar)).len() as i64) > 0) {
            Rc::new(vec![scalar.clone()])
        } else {
            {
                let per_field = classify_body_per_field(body.clone(), param_names.clone(), param_types.clone(), &type_env, func_sigs.clone(), v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>());
if (((per_field.clone().len() as i64) > 0) && { let mut __found = false; for m in per_field.clone().iter().cloned() { if ((Rc::new(v2_rt::map_keys(&m)).len() as i64) > 0) { __found = true; break; } } __found }) {
                    per_field.clone()
                } else {
                    Rc::new(vec![])
                }
...
match v2_rt::map_get(&let_prov, vname.clone()) {
    ...
    None => if { let mut __found = false; for pn in param_names.clone().iter().cloned() { if (pn.clone().as_str() == vname.clone().as_str()) { __found = true; break; } } __found } {
                v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), vname.clone(), Rc::new(SubValueRelation::PreservedValue))
            } else {
                v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>()
            },
}
...
let base_prov = classify_body_provenance(&base, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
Rc::new(v2_rt::map_keys(&base_prov)).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| match v2_rt::map_get(&base_prov, pname.clone()) {
    Some(base_rel) => {
...
compose_callee_provenance(callee, call_args_by_name(expr.clone(), type_env.source_indices.clone()), param_names.clone(), param_types.clone(), type_env.clone(), &func_sigs, let_prov.clone())
},
    ExprData::ExprMethodCall { method_semantics: ms, .. } => {
            let mname = expr_method_name_at(expr.clone(), type_env.source_indices.clone());
let callee_result = compose_callee_provenance(mname.clone(), method_call_args_by_name(&expr, func_sigs.clone(), &type_env.source_indices.clone()), param_names.clone(), param_types.clone(), type_env.clone(), &func_sigs, let_prov.clone());
if ((Rc::new(v2_rt::map_keys(&callee_result)).len() as i64) > 0) {
                callee_result.clone()
            } else {
                {
                    let is_shrink = match ms.clone().as_deref().cloned() {
    Some(MethodSemantics::AlgebraMethodSemantics { size_effect: se, .. }) => match se.clone() {
    Some(CollectionSizeEffect::ShrinkEffect) => true,
    _ => false,
},
    _ => false,
};
if (is_shrink && match method_arg_nodes(expr.clone()).first().cloned() {
...
let recv_prov = classify_body_provenance(&recv, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
Rc::new(v2_rt::map_keys(&recv_prov)).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| match v2_rt::map_get(&recv_prov, pname.clone()) {
    Some(recv_rel) => {
                                let synth_field = Rc::new(InductiveField {
...
v2_rt::rc_map_insert(acc.clone(), pname.clone(), compose_sub_value(recv_rel.clone(), synth_field.clone()))
},
    None => acc.clone(),
})
}
...
let scrut_prov = classify_body_provenance(&scrut, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
let arms = match_arm_nodes(expr.clone());
let arm_provs = Rc::new({ let mut __result = Vec::new(); for arm in arms.iter().cloned() { __result.push({
...
if ((bind_name.clone().as_str() != "".to_string().as_str()) && (field_label.clone().as_str() != "".to_string().as_str())) {
                        {
                            let field_prov = Rc::new(v2_rt::map_keys(&scrut_prov)).iter().cloned().fold(empty_prov_map(), |fp_acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| maybe_insert_match_field_relation(fp_acc, &pname, field_label.clone(), vname.clone(), v2_rt::map_get(&scrut_prov, pname.clone()), param_types.clone(), type_env.clone()));
v2_rt::rc_map_insert(acc.clone(), bind_name.clone(), field_prov.clone())
}
                    } else {
                        acc.clone()
                    }
}),
    MatchPattern::Bind { name: bname, .. } => v2_rt::rc_map_insert(let_prov.clone(), bname.clone(), scrut_prov.clone()),
    _ => let_prov.clone(),
};
classify_body_provenance(&arm_body(&arm), &param_names, &param_types, &type_env, &func_sigs, &arm_let_prov)
}); } __result });
meet_body_provenance_maps(&arm_provs)
},
    ExprData::ExprIf => {
            let then_prov = classify_body_provenance(&if_then_branch(expr.clone()), &param_names, &param_types, &type_env, &func_sigs, &let_prov);
match if_else_branch(expr.clone()) {
    Some(else_expr) => {
                let else_prov = classify_body_provenance(&else_expr, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
...
let value_prov = classify_body_provenance(&value, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
let updated = v2_rt::rc_map_insert(let_prov.clone(), bname.clone(), value_prov.clone());
match let_body(expr.clone()) {
    Some(body) => classify_body_provenance(&body, &param_names, &param_types, &type_env, &func_sigs, &updated),
    None => v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
}
},
    ExprData::ExprBlock => {
            let block_result = expr.children.clone().iter().cloned().fold(Rc::new(BlockProvFoldState {
    prov: let_prov.clone(),
    last: empty_prov_map(),
}), |state: Rc<BlockProvFoldState>, stmt: Rc<Node>| match (*stmt.expr_data.clone()).clone() {
    ExprData::ExprLet => {
                let bname = let_binding_name_at(stmt.clone(), type_env.source_indices.clone());
let value = let_value(stmt.clone());
let value_prov = classify_body_provenance(&value, &param_names, &param_types, &type_env, &func_sigs, &state.prov.clone());
let updated_prov = v2_rt::rc_map_insert(state.prov.clone(), bname.clone(), value_prov.clone());
match let_body(stmt.clone()) {
    Some(body) => Rc::new(BlockProvFoldState {
    prov: state.prov.clone(),
    last: classify_body_provenance(&body, &param_names, &param_types, &type_env, &func_sigs, &updated_prov),
}),
    ...
    prov: state.prov.clone(),
    last: classify_body_provenance(&stmt, &param_names, &param_types, &type_env, &func_sigs, &state.prov.clone()),
}),
});
block_result.last.clone()
},
    ExprData::ExprCast => classify_body_provenance(&cast_expr(expr.clone()), &param_names, &param_types, &type_env, &func_sigs, &let_prov),
    ExprData::ExprReturn => match expr.children.clone().first().cloned() {
    Some(inner) => classify_body_provenance(&inner, &param_names, &param_types, &type_env, &func_sigs, &let_prov),
    None => v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
},
    _ => v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
}
    })
}
...
pub fn call_args_by_name(call: Rc<Node>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<HashMap<String, Rc<Node>>> {
    call.children.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Node>>(), |acc: Rc<HashMap<String, Rc<Node>>>, arg_node: Rc<Node>| match arg_name_at(arg_node.clone(), source_indices.clone()) {
    Some(aname) => v2_rt::rc_map_insert(acc.clone(), aname.clone(), arg_value(&arg_node)),
    None => acc.clone(),
})
}

pub fn method_call_args_by_name(call: &Rc<Node>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<HashMap<String, Rc<Node>>> {
    {
...
let base = match v2_rt::map_get(&func_sigs, mname) {
    Some(sig) => match sig.params.clone().first().cloned() {
    Some(first_p) => {
            let first_pname = param_node_name_at(first_p.clone(), source_indices.clone());
if (first_pname.clone().as_str() != "".to_string().as_str()) {
                v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<Node>>(), first_pname.clone(), recv)
            } else {
...
pub fn compose_callee_provenance(callee: String, arg_by_name: Rc<HashMap<String, Rc<Node>>>, param_names: Rc<Vec<String>>, param_types: Rc<HashMap<String, String>>, type_env: Rc<TypeEnv>, func_sigs: &Rc<HashMap<String, Rc<ResolvedFuncSig>>>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    match v2_rt::map_get(&func_sigs, callee) {
    Some(sig) => match sig.output_provenance.clone().first().cloned() {
    Some(callee_prov) => Rc::new(v2_rt::map_keys(&callee_prov)).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, callee_pname: String| match v2_rt::map_get(&callee_prov, callee_pname.clone()) {
    Some(callee_rel) => match v2_rt::map_get(&arg_by_name, callee_pname.clone()) {
    Some(arg) => {
        let arg_prov = classify_body_provenance(&arg, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
...
pub fn classify_body_per_field(expr: Rc<Node>, param_names: Rc<Vec<String>>, param_types: Rc<HashMap<String, String>>, type_env: &Rc<TypeEnv>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>) -> Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    {
...
classify_terminal_per_field(&terminal, param_names, param_types, &type_env, func_sigs, terminal_prov)
}
}

pub fn classify_terminal_per_field(expr: &Rc<Node>, param_names: Rc<Vec<String>>, param_types: Rc<HashMap<String, String>>, type_env: &Rc<TypeEnv>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>) -> Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    match (*expr.expr_data.clone()).clone() {
    ExprData::ExprRecordLit { .. } => Rc::new({ let mut __result = Vec::new(); for fi in expr.children.clone().iter().cloned() { __result.push(classify_body_provenance(&field_init_node_value(&fi), &param_names, &param_types, &type_env, &func_sigs, &let_prov)); } __result }),
    ExprData::ExprMatch => {
        let arm_results = Rc::new({ let mut __result = Vec::new(); for arm in match_arm_nodes(expr.clone()).iter().cloned() { __result.push({
            let arm_unwrapped = unwrap_body_terminal(&arm_body(&arm), let_prov.clone(), type_env.clone());
match (*arm_unwrapped.expr.clone().expr_data.clone()).clone() {
    ExprData::ExprRecordLit { .. } => Rc::new({ let mut __result = Vec::new(); for fi in arm_unwrapped.expr.clone().children.clone().iter().cloned() { __result.push(classify_body_provenance(&field_init_node_value(&fi), &param_names, &param_types, &type_env, &func_sigs, &arm_unwrapped.let_prov.clone())); } __result }),
    _ => Rc::new(vec![]),
}
}); } __result });
meet_per_field_results(&arm_results)
    ...
    ExprData::ExprIf => {
        let then_unwrapped = unwrap_body_terminal(&if_then_branch(expr.clone()), let_prov.clone(), type_env.clone());
let then_result = match (*then_unwrapped.expr.clone().expr_data.clone()).clone() {
    ExprData::ExprRecordLit { .. } => Rc::new({ let mut __result = Vec::new(); for fi in then_unwrapped.expr.clone().children.clone().iter().cloned() { __result.push(classify_body_provenance(&field_init_node_value(&fi), &param_names, &param_types, &type_env, &func_sigs, &then_unwrapped.let_prov.clone())); } __result }),
    _ => Rc::new(vec![]),
};
match if_else_branch(expr.clone()) {
    Some(else_expr) => {
            let else_unwrapped = unwrap_body_terminal(&else_expr, let_prov.clone(), type_env.clone());
let else_result = match (*else_unwrapped.expr.clone().expr_data.clone()).clone() {
    ExprData::ExprRecordLit { .. } => Rc::new({ let mut __result = Vec::new(); for fi in else_unwrapped.expr.clone().children.clone().iter().cloned() { __result.push(classify_body_provenance(&field_init_node_value(&fi), &param_names, &param_types, &type_env, &func_sigs, &else_unwrapped.let_prov.clone())); } __result }),
    _ => Rc::new(vec![]),
};
...
pub fn unwrap_body_terminal(expr: &Rc<Node>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, type_env: Rc<TypeEnv>) -> Rc<BodyTerminal> {
    match (*expr.expr_data.clone()).clone() {
...
pub fn meet_per_field_results(results: &Rc<Vec<Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>>>>) -> Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    match results.clone().first().cloned() {
    None => Rc::new(vec![]),
    Some(first_result) => if ((first_result.clone().len() as i64) == 0) {
        Rc::new(vec![])
    } else {
        {
            let all_same_count = { let mut __all = true; for r in results.clone().iter().cloned() { if !(((r.clone().len() as i64) == (first_result.clone().len() as i64))) { __all = false; break; } } __all };
if !all_same_count {
                Rc::new(vec![])
            } else {
                Rc::new({ let mut __result = Vec::new(); for pair in Rc::new(first_result.clone().iter().cloned().enumerate().map(|(i, v)| (i as i64, v)).collect::<Vec<_>>()).iter().cloned() { __result.push({
                    let idx = pair.0.clone();
let field_provs = Rc::new({ let mut __result = Vec::new(); for r in results.clone().iter().cloned() { __result.push(match r.clone().get(idx.clone() as usize).cloned() {
    Some(m) => m.clone(),
    None => empty_prov_map(),
}); } __result });
meet_body_provenance_maps(&field_provs)
}); } __result })
            }
}
    },
...
pub fn arm_ctx_from_variant_provenance(arm_node: Rc<Node>, sig: Rc<ResolvedFuncSig>, scrut: Rc<Node>, ctx: &Rc<DescentContext>) -> Option<Rc<DescentContext>> {
    match (*arm_pattern(arm_node)).clone() {
    MatchPattern::VariantPattern { name: vname, field_bindings: bindings, .. } => match v2_rt::map_get(&sig.variant_provenance.clone(), vname.clone()) {
    Some(variant_field_map) => {
        let call_args = call_args_by_name(scrut, ctx.type_env.clone().source_indices.clone());
let extended = bindings.clone().iter().cloned().fold(ctx.clone(), |c: Rc<DescentContext>, fb: Rc<Node>| {
            let field_label = field_binding_name_at(fb.clone(), ctx.type_env.clone().source_indices.clone());
let bind_name = match (*field_binding_pattern(fb.clone())).clone() {
    MatchPattern::Bind { name: bn, .. } => bn.clone(),
    _ => "".to_string(),
};
if ((bind_name.clone().as_str() == "".to_string().as_str()) || (field_label.clone().as_str() == "".to_string().as_str())) {
                ...
                match v2_rt::map_get(&variant_field_map, field_label.clone()) {
    Some(param_map) => {
                    let composed = compose_callee_param_map(&param_map, call_args.clone(), c.param_order.clone(), c.param_names.clone(), c.type_env.clone(), c.func_sigs.clone(), v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>());
let all_rels = Rc::new({ let mut __result = Vec::new(); for k in Rc::new(v2_rt::map_keys(&composed)).iter().cloned() { __result.push(match v2_rt::map_get(&composed, k.clone()) {
    Some(r) => r.clone(),
    None => Rc::new(SubValueRelation::SubValueUnknown),
}); } __result });
let best = merge_argument_relations(&all_rels);
match (*best.clone()).clone() {
    ...
    param_order: c.param_order.clone(),
    type_env: c.type_env.clone(),
    sub_value_vars: v2_rt::rc_map_insert(c.sub_value_vars.clone(), bind_name.clone(), best.clone()),
    size_aliases: c.size_aliases.clone(),
    scope_locals: c.scope_locals.clone(),
    func_sigs: c.func_sigs.clone(),
    per_field_vars: c.per_field_vars.clone(),
...
pub fn compose_callee_param_map(callee_prov: &Rc<HashMap<String, Rc<SubValueRelation>>>, arg_by_name: Rc<HashMap<String, Rc<Node>>>, param_names: Rc<Vec<String>>, param_types: Rc<HashMap<String, String>>, type_env: Rc<TypeEnv>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    Rc::new(v2_rt::map_keys(&callee_prov)).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, callee_pname: String| match v2_rt::map_get(&callee_prov, callee_pname.clone()) {
    Some(callee_rel) => match v2_rt::map_get(&arg_by_name, callee_pname.clone()) {
    Some(arg) => {
        let arg_prov = classify_body_provenance(&arg, &param_names, &param_types, &type_env, &func_sigs, &let_prov);
Rc::new(v2_rt::map_keys(&arg_prov)).iter().cloned().fold(acc.clone(), |inner: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| match v2_rt::map_get(&arg_prov, pname.clone()) {
...
match (*composed.clone()).clone() {
    SubValueRelation::SubValueUnknown => inner.clone(),
    _ => v2_rt::rc_map_insert(inner.clone(), pname.clone(), composed.clone()),
}
},
    None => inner.clone(),
})
...
pub fn compute_variant_provenance(body: Rc<Node>, return_type: Rc<Node>, params: &Rc<Vec<Rc<Node>>>, type_env: &Rc<TypeEnv>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>) -> Rc<HashMap<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>> {
    {
        let rt_name = authored_name_at(type_env.source_indices.clone(), &return_type);
let rt_def = lookup_type_by_name(&type_env, rt_name.clone());
match rt_def {
...
collect_variant_constructors(&body, rt_name.clone(), &param_names, &param_types, &type_env, &func_sigs, v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>(), &v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>())
},
    _ => v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>(),
},
    None => v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>(),
}
}
}

pub fn collect_variant_constructors(body: &Rc<Node>, parent_enum: String, param_names: &Rc<Vec<String>>, param_types: &Rc<HashMap<String, String>>, type_env: &Rc<TypeEnv>, func_sigs: &Rc<HashMap<String, Rc<ResolvedFuncSig>>>, let_prov: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, acc: &Rc<HashMap<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>>) -> Rc<HashMap<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*body.expr_data.clone()).clone() {
    ExprData::ExprRecordLit { parent_enum: pe, .. } => match pe.clone() {
    Some(enum_name) => if (enum_name.clone().as_str() == parent_enum.clone().as_str()) {
            {
                let variant_name = body.name.clone();
let field_prov_map = body.children.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>(), |m: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, fi: Rc<Node>| {
                    let fname = field_init_node_name_at(fi.clone(), type_env.source_indices.clone());
let fprov = classify_body_provenance(&field_init_node_value(&fi), &param_names, &param_types, &type_env, &func_sigs, &let_prov);
if (fname.clone().as_str() != "".to_string().as_str()) {
                        v2_rt::rc_map_insert(m.clone(), fname.clone(), fprov.clone())
                    } else {
                        m.clone()
                    }
});
match v2_rt::map_get(&acc, variant_name.clone()) {
    Some(existing) => {
                    let merged = merge_field_prov_maps(&existing, &field_prov_map);
v2_rt::rc_map_insert(acc.clone(), variant_name.clone(), merged)
},
    None => v2_rt::rc_map_insert(acc.clone(), variant_name.clone(), field_prov_map),
}
}
        } else {
            acc.clone()
    ...
    ExprData::ExprMatch => match_arm_nodes(body.clone()).iter().cloned().fold(acc.clone(), |a: Rc<HashMap<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>>, arm: Rc<Node>| collect_variant_constructors(&arm_body(&arm), parent_enum.clone(), &param_names, &param_types, &type_env, &func_sigs, let_prov.clone(), &a)),
    ExprData::ExprIf => {
            let then_acc = collect_variant_constructors(&if_then_branch(body.clone()), parent_enum.clone(), &param_names, &param_types, &type_env, &func_sigs, let_prov.clone(), &acc);
match if_else_branch(body.clone()) {
    Some(eb) => collect_variant_constructors(&eb, parent_enum.clone(), &param_names, &param_types, &type_env, &func_sigs, let_prov.clone(), &then_acc),
    None => then_acc,
}
},
...
let value_prov = classify_body_provenance(&let_value(body.clone()), &param_names, &param_types, &type_env, &func_sigs, &let_prov);
let new_lp = v2_rt::rc_map_insert(let_prov.clone(), bname.clone(), value_prov);
collect_variant_constructors(&b, parent_enum.clone(), &param_names, &param_types, &type_env, &func_sigs, new_lp, &acc)
},
    None => acc.clone(),
},
    ExprData::ExprBlock => {
...
if (stmt_count.clone() == 0) {
                ...
                {
                    let final_lp = Rc::new(stmts.clone().iter().cloned().take((stmt_count.clone() - 1) as usize).collect::<Vec<_>>()).iter().cloned().fold(let_prov.clone(), |lp: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, stmt: Rc<Node>| match (*stmt.expr_data.clone()).clone() {
    ExprData::ExprLet => {
                        let bname = let_binding_name_at(stmt.clone(), type_env.source_indices.clone());
let vprov = classify_body_provenance(&let_value(stmt.clone()), &param_names, &param_types, &type_env, &func_sigs, &lp);
v2_rt::rc_map_insert(lp.clone(), bname.clone(), vprov.clone())
},
    _ => lp.clone(),
});
match stmts.clone().get((stmt_count.clone() - 1) as usize).cloned() {
    Some(last) => collect_variant_constructors(&last, parent_enum.clone(), &param_names, &param_types, &type_env, &func_sigs, final_lp, &acc),
    None => acc.clone(),
}
}
            }
...
pub fn merge_field_prov_maps(a: &Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, b: &Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>) -> Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    {
        let from_a = Rc::new(v2_rt::map_keys(&a)).iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>(), |result: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, fname: String| match v2_rt::map_get(&a, fname.clone()) {
    Some(a_prov) => match v2_rt::map_get(&b, fname.clone()) {
    Some(b_prov) => v2_rt::rc_map_insert(result.clone(), fname.clone(), union_prov_maps(&a_prov, &b_prov)),
    None => v2_rt::rc_map_insert(result.clone(), fname.clone(), a_prov.clone()),
},
    None => result.clone(),
});
Rc::new(v2_rt::map_keys(&b)).iter().cloned().fold(from_a, |result: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>, fname: String| if (v2_rt::map_get(&result, fname.clone()) != None) {
            result.clone()
        } else {
            match v2_rt::map_get(&b, fname.clone()) {
    Some(b_prov) => v2_rt::rc_map_insert(result.clone(), fname.clone(), b_prov.clone()),
    None => result.clone(),
}
        })
}
...
pub fn union_prov_maps(a: &Rc<HashMap<String, Rc<SubValueRelation>>>, b: &Rc<HashMap<String, Rc<SubValueRelation>>>) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    {
        let from_a = Rc::new(v2_rt::map_keys(&a)).iter().cloned().fold(empty_prov_map(), |result: Rc<HashMap<String, Rc<SubValueRelation>>>, key: String| match v2_rt::map_get(&a, key.clone()) {
    Some(a_rel) => match v2_rt::map_get(&b, key.clone()) {
    Some(b_rel) => v2_rt::rc_map_insert(result.clone(), key.clone(), join_sub_value(a_rel.clone(), b_rel.clone())),
    None => v2_rt::rc_map_insert(result.clone(), key.clone(), a_rel.clone()),
},
    None => result.clone(),
});
Rc::new(v2_rt::map_keys(&b)).iter().cloned().fold(from_a, |result: Rc<HashMap<String, Rc<SubValueRelation>>>, key: String| if (v2_rt::map_get(&result, key.clone()) != None) {
            result.clone()
        } else {
            match v2_rt::map_get(&b, key.clone()) {
    Some(b_rel) => v2_rt::rc_map_insert(result.clone(), key.clone(), b_rel.clone()),
    None => result.clone(),
}
        })
}
...
pub fn populate_output_provenance(typed_items: Rc<Vec<Rc<Node>>>, func_env: Rc<ResolvedFuncEnv>, type_env: Rc<TypeEnv>, locals: Rc<HashMap<String, Rc<TypeBinding>>>) -> Rc<ResolvedFuncEnv> {
    {
        let updated_sigs = typed_items.iter().cloned().fold(func_env.signatures.clone(), |acc: Rc<HashMap<String, Rc<ResolvedFuncSig>>>, item: Rc<Node>| if ((item.params.clone().len() as i64) > 0) {
            {
                let fn_name = authored_name_at(type_env.source_indices.clone(), &item);
...
let ptype = resolved_type_name(&first_param, type_env.source_indices.clone());
...
match list_field.clone() {
    Some(ind_field) => {
                                    let provenance = Rc::new(vec![v2_rt::rc_map_insert(v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(), pname.clone(), Rc::new(SubValueRelation::StrictSubValue {
    field: ind_field.clone(),
    factor: Rc::new(ShrinkFactor::UnitShrink),
}))]);
match v2_rt::map_get(&acc, fn_name.clone()) {
    ...
    Some(body) => if !expr_has_self_call(&body, &fn_name, &type_env.source_indices.clone()) {
                        {
                            let provenance = infer_output_provenance(&body, &item.params.clone(), &type_env, locals.clone(), &acc);
let variant_prov = match item.inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: rt, .. }) => compute_variant_provenance(body.clone(), rt.clone(), &item.params.clone(), &type_env, acc.clone()),
    _ => v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>(),
};
...
pub fn annotate_descent_evidence(body: Rc<Node>, fn_name: String, params: &Rc<Vec<Rc<Node>>>, type_env: &Rc<TypeEnv>, scope_locals: Rc<HashMap<String, Rc<TypeBinding>>>, func_sigs: Rc<HashMap<String, Rc<ResolvedFuncSig>>>) -> Rc<Node> {
    {
        let param_name_map = params.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, String>(), |acc: Rc<HashMap<String, String>>, p: Rc<Node>| {
            let pname = param_node_name_at(p.clone(), type_env.source_indices.clone());
let ptype = resolved_type_name(&p, type_env.source_indices.clone());
v2_rt::rc_map_insert(acc, pname.clone(), ptype.clone())
});
let param_order_list = Rc::new({ let mut __result = Vec::new(); for p in params.clone().iter().cloned() { __result.push(param_node_name_at(p.clone(), type_env.source_indices.clone())); } __result });
let ctx = Rc::new(DescentContext {
    ...
    sub_value_vars: v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
    size_aliases: v2_rt::rc_empty_map::<String, Rc<SizeExpr>>(),
    scope_locals: scope_locals,
    func_sigs: func_sigs,
    per_field_vars: v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>(),
});
...
pub fn infer_auth_source_properties(props: Rc<Vec<Rc<Node>>>, scope: Rc<InferScope>) -> Rc<InferPropertiesResult> {
    {
        let results = Rc::new({ let mut __result = Vec::new(); for p in props.iter().cloned() { __result.push(if (field_init_node_name_at(p.clone(), scope.type_env.clone().source_indices.clone()).as_str() == "svc_auth_source".to_string().as_str()) {
            {
                let val = field_init_node_value(&p);
let val_result = infer_expr(&val, &scope, &None);
(Rc::new(Node {
...
let fn_scope = item.uses.clone().iter().cloned().fold(fn_scope.clone(), |s: Rc<InferScope>, u: Rc<Node>| extend_scope(&s, &resource_use_name_at(u.clone(), scope.type_env.clone().source_indices.clone()), resource_use_resource(&u), Rc::new(SubValueRelation::SubValueUnknown)));
...
Rc::new(TypedItemResult {
    ...
    has_non_tail_self_call: expr_has_non_tail_self_call(&body_typed, &authored_name_at(scope.type_env.clone().source_indices.clone(), &item), true, &scope.type_env.clone().source_indices.clone()),
    match_pattern: None,
    expr_data: Rc::new(ExprData::NoExprData),
    ident: None,
}),
...
pub fn collect_parent_bindings_filtered(envs: Rc<Vec<Rc<TypeEnv>>>) -> Rc<HashMap<i64, Rc<TypeBinding>>> {
    envs.iter().cloned().fold(v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(), |acc: Rc<HashMap<i64, Rc<TypeBinding>>>, env: Rc<TypeEnv>| Rc::new(v2_rt::map_keys(&env.bindings.clone())).iter().cloned().fold(acc, |bacc: Rc<HashMap<i64, Rc<TypeBinding>>>, ident: i64| {
...
if is_type_variable_name(&name) {
    ...
    Some(binding) => v2_rt::rc_map_insert(bacc.clone(), ident.clone(), binding.clone()),
    None => bacc.clone(),
}
        }
}))
...
pub fn build_type_env(module: &Rc<ResolvedModule>, parent_index: &Rc<HashMap<String, Rc<TypedModule>>>, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>, intern_table: &Rc<InternTable>) -> Rc<BuildTypeEnvResult> {
    {
...
let std_types_parent_env = if ((module_name_str.clone().as_str() != "std.types".to_string().as_str()) && (imports_std_types == false)) {
...
if is_type_variable_name(&name) {
                    ...
                    match v2_rt::map_get(&typed_parent.type_env.clone().bindings.clone(), ident.clone()) {
    Some(binding) => v2_rt::rc_map_insert(bacc.clone(), ident.clone(), binding.clone()),
    None => bacc.clone(),
}
                }
...
let import_recursive = parent_envs.clone().iter().cloned().fold(Rc::new(vec![]), |acc: _, env: Rc<TypeEnv>| v2_rt::concat(acc, env.recursive_types.clone()));
let import_recursive_set = parent_envs.clone().iter().cloned().fold(v2_rt::rc_empty_map::<i64, bool>(), |acc: Rc<HashMap<i64, bool>>, env: Rc<TypeEnv>| v2_rt::rc_map_merge(acc, env.recursive_type_set.clone()));
let import_inductive_fields = parent_envs.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, Rc<Vec<Rc<InductiveField>>>>(), |acc: Rc<HashMap<String, Rc<Vec<Rc<InductiveField>>>>>, env: Rc<TypeEnv>| merge_inductive_fields(acc, &env.inductive_fields.clone()));
...
let import_diags = Rc::new({ let mut __result = Vec::new(); for imp in module.resolved_imports.clone().iter().cloned() { __result.extend((*match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
    ...
    message: v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("missing parent environment for imported module '".to_string(), imp.module_path.clone()), "' while typechecking '".to_string()), module_name_str.clone()), "'".to_string()),
...
v2_rt::rc_map_insert(acc.clone(), item_ident.clone(), Rc::new(TypeBinding {
...
v2_rt::rc_map_insert(acc.clone(), item_ident.clone(), Rc::new(TypeBinding {
    ...
    provenance: Rc::new(SubValueRelation::SubValueUnknown),
                    ...
                    if ((item.transport.clone() == None) && ((item.children.clone().len() as i64) > 0)) {
                        {
                            let ref_node = nominal_ref_node(&authored_name_at(source_indices.clone(), &item), item.span.clone(), item.ident_span.clone());
v2_rt::rc_map_insert(acc.clone(), item_ident.clone(), Rc::new(TypeBinding {
    name: authored_name_at(source_indices.clone(), &item),
...
v2_rt::rc_map_insert(acc.clone(), item_ident.clone(), Rc::new(TypeBinding {
    ...
    provenance: Rc::new(SubValueRelation::SubValueUnknown),
                            ...
                            if ((((((item.properties.clone().len() as i64) > 0) && (item.connective.clone() == Connective::NoConnective)) && (item.transport.clone() == None)) && (item.inferred.clone() == None)) && ((item.params.clone().len() as i64) == 0)) {
                                v2_rt::rc_map_insert(acc.clone(), item_ident.clone(), nominal_type_binding(&authored_name_at(source_indices.clone(), &item)))
                            } else {
                                acc.clone()
                            }
                        }
...
let local_name_set = Rc::new(v2_rt::map_keys(&local_bindings)).iter().cloned().fold(v2_rt::rc_empty_map::<i64, bool>(), |acc: Rc<HashMap<i64, bool>>, ident: i64| v2_rt::rc_map_insert(acc, ident.clone(), true));
...
let cross_type_all_names = v2_rt::concat(cycle_set_str.clone(), Rc::new(v2_rt::map_keys(&compiler_recursive_types())));
let cross_type_set_str = cross_type_all_names.clone().iter().cloned().fold(empty_set(), |acc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, name: String| set_insert(acc, name.clone()));
let cross_type_set = cross_type_all_names.clone().iter().cloned().fold(v2_rt::rc_empty_map::<i64, bool>(), |acc: Rc<HashMap<i64, bool>>, name: String| v2_rt::rc_map_insert(acc, intern(&intern_table, &name).id.clone(), true));
let local_inductive_fields = build_item_inductive_fields(module_items(module.module.clone()), cross_type_set_str, source_indices.clone());
...
let resolved = resolve_env_bindings(&unresolved_env, module_name_str.clone(), &local_name_set, all_deps_map.clone());
...
let imports_std_types = { let mut __found = false; for imp in module.resolved_imports.clone().iter().cloned() { if (imp.module_path.clone().as_str() == "std.types".to_string().as_str()) { __found = true; break; } } __found };
let module_name_str = authored_name_at(source_indices.clone(), &module.module.clone());
let std_types_parent_env = if ((module_name_str.as_str() != "std.types".to_string().as_str()) && (imports_std_types == false)) {
            match v2_rt::map_get(&parent_index, "std.types".to_string()) {
    Some(typed_std) => Rc::new(vec![typed_std.type_env.clone()]),
    None => Rc::new(vec![]),
}
...
let item_name_str = authored_name_at(source_indices.clone(), &item);
match kind.clone() {
    ...
    service_names: if (item.body.clone() == None) {
            ...
            collect_typed_service_calls(item.body.clone().clone().unwrap(), source_indices.clone())
        },
    ...
    is_self_recursive: if (item.body.clone() == None) {
            ...
            expr_has_self_call(&item.body.clone().clone().unwrap(), &item_name_str, &source_indices)
        },
    has_non_tail_self_call: if (item.body.clone() == None) {
            false
        } else {
            expr_has_non_tail_self_call(&item.body.clone().clone().unwrap(), &item_name_str, true, &source_indices)
        },
}),
    _ => Rc::new(ItemInfo {
    name: item_name_str.clone(),
...
Some(Rc::new(DeclaredFuncSig {
    name: authored_name_at(env.source_indices.clone(), &ritem),
    params: ritem.params.clone(),
    inferred: declared_rt,
    is_async: ((ritem.uses.clone().len() as i64) > 0),
    output_provenance: Rc::new(vec![]),
    variant_provenance: v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>>(),
}))
}
        } else {
            None
...
let svc_local = if ((ritem.transport.clone() != None) && ((ritem.children.clone().len() as i64) > 0)) {
            {
                let root = namespace_root_from_properties(ritem.properties.clone(), authored_name_at(env.source_indices.clone(), &ritem), env.source_indices.clone());
...
let next_svc_registry = if ((contribution.svc_entries.clone().len() as i64) > 0) {
            v2_rt::rc_map_insert(svc_registry, authored_name_at(source_indices.clone(), &contribution.resolved_item.clone()), contribution.svc_entries.clone())
        } else {
            svc_registry
        };
...
pub fn build_module_context(contributions: Rc<Vec<Rc<ItemContribution>>>, parent_index: Rc<HashMap<String, Rc<TypedModule>>>, resolved_imports: &Rc<Vec<Rc<ResolvedImport>>>, env: &Rc<TypeEnv>, module_name: &String) -> Rc<ModuleContext> {
    {
        let local = fold_module_contributions(contributions, Rc::new(vec![]), v2_rt::rc_empty_map::<String, Rc<DeclaredFuncSig>>(), v2_rt::rc_empty_map::<String, Rc<Vec<Rc<OpEntry>>>>(), v2_rt::rc_empty_map::<String, Rc<TypeBinding>>(), v2_rt::rc_empty_map::<String, Rc<ItemInfo>>(), Rc::new(vec![]), env.source_indices.clone());
let imported_enum_names = resolved_imports.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, bool>(), |acc: Rc<HashMap<String, bool>>, imp: Rc<ResolvedImport>| imp.specific_names.clone().iter().cloned().fold(acc, |inner: Rc<HashMap<String, bool>>, n: String| v2_rt::rc_map_insert(inner, n.clone(), true)));
...
if is_coproduct.clone() {
                {
                    let curr_is_imported = (v2_rt::map_get(&imported_enum_names, authored_name_at(env.source_indices.clone(), &binding.resolved.clone())) != None);
binding.resolved.clone().children.clone().iter().cloned().fold(acc.clone(), |vacc: Rc<VariantFoldState>, child: Rc<Node>| match v2_rt::map_get(&vacc.locals.clone(), authored_name_at(env.source_indices.clone(), &child)) {
    Some(prev) => {
                        let prev_is_imported = (v2_rt::map_get(&imported_enum_names, authored_name_at(env.source_indices.clone(), &prev.resolved.clone())) != None);
if (curr_is_imported.clone() && prev_is_imported.clone()) {
    ...
    variant: authored_name_at(env.source_indices.clone(), &child),
    ...
    span: no_span(),
                            ...
                            if curr_is_imported.clone() {
                                Rc::new(VariantFoldState {
    locals: v2_rt::rc_map_insert(vacc.locals.clone(), authored_name_at(env.source_indices.clone(), &child), Rc::new(TypeBinding {
    name: authored_name_at(env.source_indices.clone(), &child),
...
let all_locals = Rc::new(v2_rt::map_values(&merged_scope.svc_locals.clone())).iter().cloned().fold(env_variant_locals, |acc: Rc<HashMap<String, Rc<TypeBinding>>>, binding: Rc<TypeBinding>| v2_rt::rc_map_insert(acc, binding.name.clone(), binding.clone()));
...
pub fn typecheck_module(resolved: &Rc<ResolvedModule>, parent_index: &Rc<HashMap<String, Rc<TypedModule>>>, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>, intern_table: Rc<InternTable>) -> Rc<TypecheckModuleResult> {
    {
        let env_result = build_type_env(&resolved, &parent_index, &source_indices, &intern_table);
...
let resolved_module_name = authored_name_at(source_indices.clone(), &resolved.module.clone());
let contributions = Rc::new({ let mut __result = Vec::new(); for item in module_items(resolved.module.clone()).iter().cloned() { __result.push(analyze_item(item.clone(), &env, &resolved_module_name)); } __result });
let ctx = build_module_context(contributions, parent_index.clone(), &resolved.resolved_imports.clone(), &env, &resolved_module_name);
let data_locals = ctx.resolved_items.clone().iter().cloned().fold(ctx.locals.clone(), |acc: Rc<HashMap<String, Rc<TypeBinding>>>, item: Rc<Node>| if ((((item.body.clone() != None) && ((item.params.clone().len() as i64) == 0)) && (item.inferred.clone() == None)) && (item.type_annotation.clone() != None)) {
            v2_rt::rc_map_insert(acc.clone(), authored_name_at(source_indices.clone(), &item), Rc::new(TypeBinding {
    name: authored_name_at(source_indices.clone(), &item),
...
let reannotated_items = Rc::new({ let mut __result = Vec::new(); for item in typed_items.clone().iter().cloned() { __result.push(if (item.is_self_recursive.clone() && (item.body.clone() != None)) {
    ...
    return_cardinality: item.return_cardinality.clone(),
    uses: item.uses.clone(),
    body: Some(annotate_descent_evidence(item.body.clone().clone().unwrap(), authored_name_at(source_indices.clone(), &item), &item.params.clone(), &env, data_locals.clone(), updated_func_env.signatures.clone())),
    connective: item.connective.clone(),
...
let typed_base = module_node(&resolved_module_name, module_imports(resolved.module.clone()), ctx.resolved_items.clone(), &resolved.module.clone().span.clone());
...
Rc::new(TypecheckModuleResult {
    ...
    diagnostics: v2_rt::concat(v2_rt::concat(v2_rt::concat(env_diags.clone(), ctx.diagnostics.clone()), infer_diags), seed_diags.clone()),
})
}
}
...
pub fn topo_resolve_types(mut remaining: Rc<Vec<String>>, mut env: Rc<TypeEnv>, mut module_name: String, mut diagnostics: Rc<Vec<Rc<ErrorNode>>>, mut local_names: Rc<HashMap<i64, bool>>, mut deps_map: Rc<HashMap<String, Rc<Vec<String>>>>, mut fuel: i64) -> Rc<EnvResolveResult> {
    loop {
...
let remaining_set = remaining.clone().iter().cloned().fold(empty_set(), |acc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, name: String| set_insert(acc, name.clone()));
let ready = Rc::new({ let mut __result = Vec::new(); for name in remaining.clone().iter().cloned() { if match v2_rt::map_get(&deps_map, name.clone()) {
    Some(deps) => { let mut __all = true; for dep in deps.clone().iter().cloned() { if !(((((is_kernel_type(dep.clone()) || (dep.clone().as_str() == "None".to_string().as_str())) || (dep.clone().as_str() == "".to_string().as_str())) || is_recursive_type_by_name(&env, dep.clone())) || (set_contains(remaining_set.clone(), dep.clone()) == false))) { __all = false; break; } } __all },
    None => true,
...
Rc::new(BindingsAccum {
    bindings: v2_rt::rc_map_insert(acc.bindings.clone(), ident.clone(), Rc::new(TypeBinding {
    name: name.clone(),
...
let ready_set = ready.clone().iter().cloned().fold(v2_rt::rc_empty_map::<String, bool>(), |acc: Rc<HashMap<String, bool>>, name: String| v2_rt::rc_map_insert(acc, name.clone(), true));
...
pub fn collect_parent_envs(resolved: &Rc<ResolvedModule>, module_index: Rc<HashMap<String, Rc<TypedModule>>>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Rc<ParentModulesResult> {
    {
...
let diagnostics = Rc::new({ let mut __result = Vec::new(); for imp in resolved.resolved_imports.clone().iter().cloned() { __result.extend((*match v2_rt::map_get(&module_index, imp.module_path.clone()) {
    Some(_) => Rc::new(vec![]),
    None => Rc::new(vec![make_error_node(Rc::new(CompilerDiagnostic::InternalError {
    message: v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("missing parent environment for imported module '".to_string(), imp.module_path.clone()), "' while ordering '".to_string()), resolved_mod_name.clone()), "'".to_string()),
    span: imp.target_module.clone().clone().unwrap().span.clone(),
...
pub fn infer_literal_string_value(n: Rc<Node>) -> Option<String> {
...
pub fn infer_field_value_by_name(record: Rc<Node>, field_name: String, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Option<Rc<Node>> {
    match Rc::new({ let mut __result = Vec::new(); for fi in record.children.clone().iter().cloned() { if (field_init_node_name_at(fi.clone(), source_indices.clone()).as_str() == field_name.clone().as_str()) { __result.push(fi); } } __result }).first().cloned() {
    Some(fi) => Some(field_init_node_value(&fi)),
    None => None,
}
}

pub fn infer_record_string_field(record: Rc<Node>, field_name: String, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> Option<String> {
    match infer_field_value_by_name(record, field_name, source_indices) {
    Some(vn) => infer_literal_string_value(vn.clone()),
    None => None,
}
}

pub fn build_untagged_json_tuple_variants(modules: Rc<Vec<Rc<TypedModule>>>) -> Rc<std::collections::BTreeSet<String>> {
    modules.iter().cloned().fold(empty_set(), |acc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, m: Rc<TypedModule>| {
        let si = m.type_env.clone().source_indices.clone();
m.items.clone().iter().cloned().fold(acc, |inner: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, item: Rc<Node>| if (item_kind(&item) == ItemKind::DataItem) {
            match item.body.clone() {
    Some(body) => match infer_record_string_field(body.clone(), "coproduct".to_string(), si.clone()) {
    Some(coproduct_name) => match infer_field_value_by_name(body.clone(), "encoding".to_string(), si.clone()) {
    Some(enc_node) => if (authored_name_at(si.clone(), &enc_node).as_str() == "UntaggedJsonStringOrArray".to_string().as_str()) {
                match infer_record_string_field(enc_node.clone(), "string_variant".to_string(), si.clone()) {
    Some(sv) => match infer_record_string_field(enc_node.clone(), "array_variant".to_string(), si.clone()) {
    Some(av) => {
                    let k1 = v2_rt::concat(v2_rt::concat(coproduct_name.clone(), "::".to_string()), sv.clone());
let k2 = v2_rt::concat(v2_rt::concat(coproduct_name.clone(), "::".to_string()), av.clone());
set_insert(set_insert(inner.clone(), k1.clone()), k2.clone())
},
    None => inner.clone(),
},
    None => inner.clone(),
...
pub fn build_fielded_variants(modules: Rc<Vec<Rc<TypedModule>>>, type_summaries: Rc<HashMap<String, Rc<TypeSummary>>>) -> Rc<std::collections::BTreeSet<String>> {
    {
        let result = modules.iter().cloned().fold(empty_set(), |acc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, m: Rc<TypedModule>| {
            let items = m.items.clone();
let si = m.type_env.clone().source_indices.clone();
items.clone().iter().cloned().fold(acc, |inner: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, item: Rc<Node>| {
                let is_enum = match v2_rt::map_get(&type_summaries, authored_name_at(si.clone(), &item)) {
...
variants.clone().iter().cloned().fold(inner.clone(), |vacc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, variant: Rc<Node>| {
                            let has_fields = ((variant.children.clone().len() as i64) > 0);
if has_fields.clone() {
                                set_insert(vacc.clone(), v2_rt::concat(v2_rt::concat(enum_name.clone(), "::".to_string()), authored_name_at(si.clone(), &variant)))
                            } else {
                                vacc.clone()
                            }
})
...
let built = modules.clone().iter().cloned().fold(init.clone(), |state: Rc<EmitInfoBuildState>, typed_module: Rc<TypedModule>| typed_module.items.clone().iter().cloned().fold(state, |inner_state: Rc<EmitInfoBuildState>, item: Rc<Node>| add_emit_item_summary(inner_state, &item, &typed_module.type_env.clone().source_indices.clone())));
let all_recursive = modules.clone().iter().cloned().fold(empty_set(), |acc: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, m: Rc<TypedModule>| Rc::new(v2_rt::map_keys(&m.type_env.clone().recursive_type_set.clone())).iter().cloned().fold(acc, |inner: Rc<std::collections::BTreeSet<compile_error!("UNRESOLVED_TypeVariable")>>, ident: i64| set_insert(inner, intern_str(m.type_env.clone().intern_table.clone(), ident.clone()))));
...
pub fn typecheck(graph: Rc<ModuleGraph>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>, intern_table: Rc<InternTable>) -> Rc<TypedGraph> {
    typecheck_modules(graph.modules.clone(), Rc::new(vec![]), v2_rt::rc_empty_map::<String, Rc<TypedModule>>(), v2_rt::rc_empty_map::<String, Rc<ItemInfo>>(), Rc::new(vec![]), source_indices, intern_table)
}

pub fn typecheck_modules(mut remaining: Rc<Vec<Rc<ResolvedModule>>>, mut modules: Rc<Vec<Rc<TypedModule>>>, mut module_index: Rc<HashMap<String, Rc<TypedModule>>>, mut item_registry: Rc<HashMap<String, Rc<ItemInfo>>>, mut diag_chunks: Rc<Vec<Rc<Vec<Rc<ErrorNode>>>>>, mut source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>, mut intern_table: Rc<InternTable>) -> Rc<TypedGraph> {
    loop {
        match remaining.clone().first().cloned() {
...
break Rc::new(TypedGraph {
    ...
    Some(resolved) => { let parent_result = collect_parent_envs(&resolved, module_index.clone(), source_indices.clone());
let tc_result = typecheck_module(&resolved, &module_index, &source_indices, intern_table.clone());
...
let __tco_2 = v2_rt::rc_map_insert(module_index, authored_name_at(source_indices.clone(), &typed.module.clone()), typed.clone());
...
pub fn reconcile(graph: Rc<ModuleGraph>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>, intern_table: Rc<InternTable>) -> Rc<ResolvedGraph> {
    {
