...
pub use crate::v2_compiler_infer_env::{
    alloc_binding_id, empty_binding_id_allocator, inductive_fields_for,
    inductive_fields_list_to_map, is_recursive_type, is_recursive_type_by_name, lookup_type,
    lookup_type_by_name, lookup_type_for, merge_envs, merge_inductive_fields, node_has_binding_id,
    put_inductive_field, put_inductive_field_cross,
};
pub use crate::v2_compiler_infer_env::{
    BindingIdAllocator, TypeBinding, TypeDeclAuthority, TypeDeclBinding, TypeEnv,
};
...
pub use crate::v2_compiler_infer_resolve::{
    peel_nominal_alias_identity, peel_nominal_alias_identity_result,
    preserve_nominal_brand_on_resolve, resolve_item_types, resolve_node,
};
...
use crate::v2_std_core::ExprErrorKind::{InternalExprError, ParseRecoveryError, SemanticExprError};
...
pub use crate::v2_std_core::{
    ...
    map_node_subtrees, match_arm_nodes, match_scrutinee, method_arg_nodes, method_receiver,
    ...
    slice_end, slice_start, string_type, unaryop_operand, unit_type, variant_node_fields,
    with_optional_cardinality, with_required_cardinality,
};
pub use crate::v2_std_core::{
    BindingId, CallSemantics, Cardinality, CompilerDiagnostic, Connective, DeclaredFuncEnv,
    DeclaredFuncSig, ErrorNode, ExprData, ExprErrorKind, FieldAccessStyle, FieldSummary,
    ...
    loop {
        match remaining.clone().first().cloned() {
            ...
            Some(imp) => match v2_rt::map_get(&parent_index, imp.module_path.clone()) {
                Some(typed_parent) => {
                    let next_func_sigs = Rc::new(v2_rt::map_values(
            ...
            if is_container_type(field_type_name.clone()) {
                {
                    ...
                    if is_recursive_element {
                        {
                            let elem =
                                if (value_type.clone().as_str() == parent_name.clone().as_str()) {
                                    parent_name.clone()
                                } else {
                                    value_type.clone()
                                };
    ...
    match Rc::new({
        ...
        for p in properties.iter().cloned() {
            if (field_init_node_name_at(p.clone(), source_indices.clone()).as_str()
                == "namespace_root".to_string().as_str())
            {
                __result.push(p);
            }
        }
...
pub fn infer_null_literal_node(expected: Option<Rc<Node>>) -> Rc<Node> {
    match expected {
        Some(exp) => match exp.return_cardinality.clone() {
            Cardinality::CardOptional => exp.clone(),
            _ => infer_literal_node(Rc::new(LiteralValue::LitNull)),
        },
        None => infer_literal_node(Rc::new(LiteralValue::LitNull)),
    }
}
...
pub fn infer_var_binding_kind(scope: Rc<InferScope>, name: String) -> Rc<VarBindingKind> {
    match lookup_variant_parent_enum(scope.clone(), name.clone()) {
        ...
        None => match v2_rt::map_get(&scope.locals.clone(), name.clone()) {
            Some(_) => {
                if v2_rt::map_has(&scope.match_bound_names.clone(), name.clone()) {
...
pub fn expected_coproduct_variant(
    parent: Rc<Node>,
    variant_name: String,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Option<Rc<Node>> {
    match parent.connective.clone() {
        Connective::Disj => find_child_named(parent.clone(), variant_name, source_indices),
        _ => None,
    }
}

pub fn expected_has_coproduct_variant(
    ...
    match expected {
        Some(expected_node) => {
            match expected_coproduct_variant(expected_node.clone(), variant_name, source_indices) {
                Some(_) => true,
                None => false,
            }
        }
...
pub fn direct_call_actual_is_none(
    actual_expr: Rc<Node>,
    actual_type: Rc<Node>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> bool {
    ((authored_name_at(source_indices.clone(), actual_expr).as_str()
        ...
        || (authored_name_at(source_indices.clone(), actual_type).as_str()
            == "None".to_string().as_str()))
}
...
pub fn direct_call_arg_is_typed_deferral(
    ...
    {
        let formal_is_callable = (type_node_is_callable(formal_raw.clone())
            || (authored_name_at(source_indices.clone(), formal_raw.clone()).as_str()
                == "Callable".to_string().as_str()));
        let actual_is_callable_literal = is_lambda_expr(actual_expr.clone());
        let formal_is_optional = ((formal_raw.return_cardinality.clone()
            == Cardinality::CardOptional)
            || (authored_name_at(source_indices.clone(), formal_raw.clone()).as_str()
...
pub fn node_contains_generic_name(
    n: Rc<Node>,
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        let is_generic_leaf = ((((n.connective.clone() == Connective::NoConnective)
            && ((n.children.clone().len() as i64) == 0))
            && ((n.params.clone().len() as i64) == 0))
            && {
                let mut __found = false;
                for g in generic_names.clone().iter().cloned() {
                    if (g.clone().as_str() == n_name.clone().as_str()) {
                        __found = true;
                        break;
                    }
                }
        ...
        (((is_generic_leaf || {
            ...
            Some(InferredNode::Resolved { node: rt, .. }) => node_contains_generic_name(
                rt.clone(),
                generic_names.clone(),
                source_indices.clone(),
            ),
...
pub fn direct_call_arg_decl_identity(n: Rc<Node>, env: Rc<TypeEnv>) -> Rc<Node> {
    match n.binding_id.clone() {
        ...
        None => match lookup_type_for(env, n.clone()) {
            Some(resolved) => {
                let resolved_with_cardinality = with_optional_cardinality(resolved.clone());
                match n.return_cardinality.clone() {
                    Cardinality::CardOptional => resolved_with_cardinality,
                    _ => resolved.clone(),
                }
            }
            None => n.clone(),
...
pub fn direct_call_node_binding(env: Rc<TypeEnv>, n: Rc<Node>) -> Option<Rc<TypeBinding>> {
    match n.binding_id.clone() {
        Some(binding_id) => {
            match v2_rt::map_get(&env.decl_registry.clone(), binding_id.value.clone()) {
                Some(decl) => match v2_rt::map_get(&env.bindings.clone(), decl.binding_key.clone())
                {
                    Some(binding) => {
                        if node_has_binding_id(binding.resolved.clone(), binding_id.clone()) {
                            Some(binding.clone())
                        } else {
                            v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.value.clone())
                        }
                    }
                    None => v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.value.clone()),
                },
                None => v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.value.clone()),
            }
        }
...
pub fn direct_call_bare_alias_target_name(
...
pub fn direct_call_carrier_transparent_alias_to(
    env: Rc<TypeEnv>,
    alias_node: Rc<Node>,
    target_node: Rc<Node>,
) -> bool {
    match alias_node.binding_id.clone() {
        Some(alias_binding_id) => match v2_rt::map_get(
            &env.carrier_bindings.clone(),
            alias_binding_id.value.clone(),
        ) {
            Some(carrier) => {
                    ...
                    None => {
                        let carrier_is_structural_shell =
                            (((carrier.resolved.clone().connective.clone()
                                ...
                                || ((carrier.resolved.clone().children.clone().len() as i64) > 0))
                                || ((carrier.resolved.clone().params.clone().len() as i64) > 0));
                        let carrier_has_alias_id = match carrier.resolved.clone().binding_id.clone()
                        {
                ...
                Some(binding) => match direct_call_bare_alias_target_node(binding.clone()) {
                    ...
                    None => {
                        ...
                        let alias_decl_name = match alias_node.binding_id.clone() {
                            Some(binding_id) => match v2_rt::map_get(
...
pub fn nominal_call_arg_brand_mismatch(
...
) -> bool {
    {
        ...
        let distinct_brand = match formal.binding_id.clone() {
            Some(formal_binding_id) => match actual.binding_id.clone() {
                Some(actual_binding_id) => (formal_binding_id.clone() != actual_binding_id.clone()),
                None => (formal_name.clone().as_str() != actual_name.clone().as_str()),
            },
            None => (formal_name.clone().as_str() != actual_name.clone().as_str()),
        };
...
pub fn direct_call_plain_kernel_leaf_mismatch(
...
) -> bool {
    {
        let formal_name = authored_name_at(source_indices.clone(), formal.clone());
        let actual_name = authored_name_at(source_indices.clone(), actual.clone());
        (((((((((((formal.binding_id.clone() == None)
            && (actual.binding_id.clone() == None))
            && (formal.connective.clone() == Connective::NoConnective))
            && (actual.connective.clone() == Connective::NoConnective))
            && ((formal.children.clone().len() as i64) == 0))
...
pub fn direct_call_product_field_names_match(
    formal: Rc<Node>,
    actual: Rc<Node>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> bool {
    {
        ...
        .cloned()
        {
            ...
            {
                Some(actual_field) => {
                    (field_node_name_at(pair.1.clone(), source_indices.clone()).as_str()
                        == field_node_name_at(actual_field.clone(), source_indices.clone())
                            .as_str())
                }
                None => false,
            }) {
...
) -> bool {
    {
        let same_structural_carrier =
            (structural_carrier_template_name(formal.clone(), source_indices.clone()).as_str()
                == structural_carrier_template_name(actual.clone(), source_indices.clone())
                    .as_str());
        let both_declared_identity = (((formal.binding_id.clone() != None)
            && (actual.binding_id.clone() != None))
            && same_structural_carrier.clone());
        ...
        let record_twin_carriers = ((((((((formal.connective.clone() == Connective::Conj)
            ...
            && direct_call_product_field_names_match(
                formal.clone(),
                actual.clone(),
                source_indices.clone(),
            ));
            ...
            .cloned()
            {
                __result.extend(
                    (*{
                        ...
                        let formal_peeled_result = peel_nominal_alias_identity_result(
                            formal_subst.clone(),
                            type_env.clone(),
                            module_name.clone(),
                        );
                        let formal_peeled = formal_peeled_result.node.clone();
                        let formal_identity =
                            direct_call_arg_decl_identity(formal_peeled.clone(), type_env.clone());
                        ...
                        match typed_args.clone().get(pair.0.clone() as usize).cloned() {
                            Some(ta) => {
                                ...
                                let actual_peeled_result = peel_nominal_alias_identity_result(
                                    actual_raw.clone(),
                                    type_env.clone(),
                                    module_name.clone(),
                                );
                                ...
                                let peel_diags = v2_rt::concat(
                                    formal_peeled_result.diagnostics.clone(),
                                    actual_peeled_result.diagnostics.clone(),
                                );
                                let arg_span = arg_value(ta.clone()).span.clone();
                                if ((peel_diags.clone().len() as i64) > 0) {
                                    peel_diags.clone()
                                } else {
                                    if ((((direct_call_arg_is_typed_deferral(
                                        ...
                                        actual_raw.clone(),
                                        source_indices.clone(),
                                    ) || node_contains_type_variable(
                                        formal_subst.clone(),
                                    )) || node_contains_type_variable(actual_raw.clone()))
                                        || node_contains_generic_name(
                                            formal_subst.clone(),
                                            generic_names.clone(),
                                            source_indices.clone(),
                                        ))
                            ...
                            None => formal_peeled_result.diagnostics.clone(),
                        }
                    })
                    .iter()
                    .cloned(),
        ...
        if (func_name.clone().as_str() == "map_keys".to_string().as_str()) {
            ...
            if (func_name.clone().as_str() == "map_values".to_string().as_str()) {
                ...
                if (((func_name.clone().as_str() == "set_insert".to_string().as_str())
                    ...
                    || (func_name.clone().as_str() == "set_contains".to_string().as_str()))
                {
                    {
                        ...
                        let union_other_bad =
                            if (func_name.clone().as_str() == "set_union".to_string().as_str()) {
                                match typed_args.clone().get(1 as usize).cloned() {
                        ...
                        let result_bt = if (func_name.clone().as_str()
                            == "set_contains".to_string().as_str())
                        {
                        ...
                        let build_diags = if (func_name.clone().as_str()
                            == "set_contains".to_string().as_str())
                        {
                            Rc::new(vec![])
                        } else {
                        ...
                        Rc::new(Tier2bBt {
                            bt: result_bt,
                            kernel_diags: v2_rt::concat(
                                wrong_receiver_diags,
                                v2_rt::concat(
                                    wrong_union_other_diags,
                                    v2_rt::concat(mismatch_diags, build_diags),
                                ),
                            ),
                        })
                    }
...
pub fn seed_override_map() -> Rc<HashMap<String, Rc<Node>>> {
    Rc::new(vec!["".to_string()]).iter().cloned().fold(
        ...
        |acc: Rc<HashMap<String, Rc<Node>>>, k: String| {
            v2_rt::rc_map_insert(acc, k.clone(), unit_type())
        },
    )
}
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*texpr.expr_data.clone()).clone() {
            ExprData::ExprLiteral { value: lit, .. } => {
                ...
                let inferred_lit = match (*lit.clone()).clone() {
                    LiteralValue::LitNull => infer_null_literal_node(expected.clone()),
                ...
                if (collision.clone() != None) {
                    ...
                    match v2_rt::map_get(&scope.locals.clone(), name.clone()) {
                        Some(binding) => {
                            ...
                            let expected_local_diags = match expected.clone() {
                                Some(expected_node) => {
                            ...
                            Rc::new(InferResult {
                                typed: local_typed,
                                diagnostics: expected_local_diags,
                            })
                        }
                        None => {
                            let expected_variant_parent = match expected.clone() {
                                Some(expected_node) => match expected_coproduct_variant(
                                    expected_node.clone(),
                                    name.clone(),
                                    scope.type_env.clone().source_indices.clone(),
                                ) {
                                    Some(_) => Some(authored_name_at(
                                        scope.type_env.clone().source_indices.clone(),
                                        expected_node.clone(),
                                    )),
                                    None => None,
                            ...
                            match expected_variant_parent {
                                Some(parent_enum) => ok_infer(make_named_expr_node(
                                    name.clone(),
                                    Rc::new(ExprData::ExprVar {
                                            ...
                                            } else {
                                                {
                                                    ...
                                                    if (collision.clone() != None) {
                                                        variant_collision_result(
                                                            func_name.clone(),
                                                            span.clone(),
                                                            collision.clone().unwrap(),
                                                        )
                                                    } else {
                                                        {
                                                            let ctor =
                                                                infer_variant_constructor_call(
                                                                    func_name.clone(),
                                                                    call_args.clone(),
                                                                    span.clone(),
                                                                    node_name_span(texpr.clone()),
    ...
    diagnostics: v2_rt::concat(arg_diags, call_diags),
            ...
            ExprData::ExprMethodCall {
                ...
                let mc_texpr = make_named_expr_node(
                    ...
                    v2_rt::concat(Rc::new(vec![recv_typed.clone()]), typed_mc_arg_nodes),
            ...
            ExprData::ExprMatch => {
                    ...
                    for arm_node in arm_nodes.clone().iter().cloned() {
                        __result.push({
                            ...
                            Rc::new(ArmInferResult {
                                ...
                                diagnostics: v2_rt::concat(
                                    pattern_diags.clone(),
                                    v2_rt::concat(guard_diags.clone(), body_diags.clone()),
                                ),
                ...
                let match_texpr = make_expr_node(
                    ...
                    v2_rt::concat(Rc::new(vec![scrut_typed.clone()]), typed_arms.clone()),
                    Some(Rc::new(InferredNode::Resolved { node: result_type })),
                    span.clone(),
                );
                Rc::new(InferResult {
                    typed: match_texpr,
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(v2_rt::concat(scrut_diags, arm_diags), empty_arms_diags),
            ...
            ExprData::ExprIf => {
                ...
                match else_expr {
                    Some(else_branch) => {
                        ...
                        let then_compare_stamped = stamp_hidden_dependency_refs(
                            then_compare_raw,
                            scope.type_env.clone(),
                            scope.type_env.clone().source_indices.clone(),
                        );
                        let else_compare_stamped = stamp_hidden_dependency_refs(
                            else_compare_raw,
                            scope.type_env.clone(),
                            scope.type_env.clone().source_indices.clone(),
                        );
                        let then_compare = if (else_is_none.clone() && !then_is_none.clone()) {
                            with_optional_cardinality(then_compare_stamped)
                        } else {
                            then_compare_stamped
                        };
                        let else_compare = if (then_is_none.clone() && !else_is_none.clone()) {
                            with_optional_cardinality(else_compare_stamped)
                        } else {
                            else_compare_stamped
                        };
                        ...
                        let unified = if (then_is_none.clone() && !else_is_none.clone()) {
                            with_optional_cardinality(else_rt.clone())
                        } else {
                            if (else_is_none.clone() && !then_is_none.clone()) {
                        ...
                        Rc::new(InferResult {
                            typed: if_texpr,
                            diagnostics: v2_rt::concat(
                                v2_rt::concat(v2_rt::concat(cond_diags, then_diags), else_diags),
                                branch_diags,
                            ),
                        })
                    }
            ...
            ExprData::ExprLambda => {
                ...
                let elem_prov = match v2_rt::map_get(
                ...
                let typed_param_nodes = Rc::new({
                    ...
                    .cloned()
                    {
                        __result.push({
                            ...
                            let param_type =
                                match v2_rt::map_get(&lam_scope.locals.clone(), param_name.clone())
                                {
                ...
                let lam_texpr = make_expr_node(
                    ...
                    v2_rt::concat(Rc::new(vec![body_typed.clone()]), typed_param_nodes),
                    body_typed.inferred.clone(),
                    span.clone(),
                );
                Rc::new(InferResult {
            ...
            ExprData::ExprIndex => {
                ...
                Rc::new(InferResult {
                    typed: idx_texpr,
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(base_diags, index_diags),
                        index_access_diags,
                    ),
                })
            }
            ExprData::ExprSlice => {
                ...
                Rc::new(InferResult {
                    typed: slc_texpr,
                    diagnostics: v2_rt::concat(
                        v2_rt::concat(v2_rt::concat(base_diags, start_diags), end_diags),
                        slice_access_diags,
                    ),
                })
            }
...
pub fn infer_record_lit(
    ...
    {
        let struct_fields =
            record_lit_expected_fields(type_name.clone(), scope.clone(), expected.clone());
            ...
            ) != None))
        {
            ...
            if (type_name.clone() == None) {
            ...
            } else {
                {
                    ...
                    let env_variant_parent = match lookup_coproduct_parent_binding_by_variant(
                        scope.type_env.clone(),
                        type_name.clone().unwrap(),
                    ) {
                        Some(parent_binding) => Some(parent_binding.name.clone()),
                        None => None,
                    };
                    let effective_variant_parent = match local_variant_parent.clone() {
                        Some(_) => local_variant_parent.clone(),
                        None => match env_variant_parent.clone() {
                            Some(_) => env_variant_parent.clone(),
                            None => expected_variant_parent,
                        },
                    };
                        ...
                        None => {
                            ...
                            match local_lookup {
                                ...
                                None => match lookup_coproduct_parent_binding_by_variant(
                                    ...
                                    Some(parent_binding) => Some(parent_binding.resolved.clone()),
                                    None => match expected.clone() {
                                        Some(expected_node) => match expected_coproduct_variant(
                                            ...
                                            Some(_) => Some(expected_node.clone()),
                                            None => None,
                                        },
                                        None => None,
                                    },
                    ...
                    let type_diags = match effective_lookup.clone() {
                        ...
                        None => Rc::new(vec![inference_error(
                            v2_rt::concat(
                                v2_rt::concat("type '".to_string(), type_name.clone().unwrap()),
                    ...
                    let texpr = make_named_expr_node(
                        ...
                        Rc::new(ExprData::ExprRecordLit {
                            parent_enum: effective_variant_parent.clone(),
                        }),
                        typed_field_nodes2,
                        Some(Rc::new(InferredNode::Resolved {
                            node: resolved_node,
            ...
            match (*val.expr_data.clone()).clone() {
                    ...
                    .clone()
                {
                    ExprData::ExprVar {
                        ...
                        match v2_rt::map_get(&ctx.size_aliases.clone(), lname)
                            .as_deref()
                            .cloned()
                        {
...
pub fn classify_collection_shrink(
    ...
    match (*arg_expr.expr_data.clone()).clone() {
        ExprData::ExprMethodCall {
            ...
            if ((mname.clone().as_str() == "take".to_string().as_str())
        ...
        Some(rel) => {
            let lookup_type = match (*rel.clone()).clone() {
                SubValueRelation::PreservedValue => {
                    match v2_rt::map_get(&param_types, pname.clone()) {
                        Some(t) => t.clone(),
                        None => "".to_string(),
                    }
                }
            ...
            if (lookup_type.clone().as_str() != "".to_string().as_str()) {
                {
                    ...
                    let matching = Rc::new({
                        let mut __result = Vec::new();
                        for f in ind_fields.iter().cloned() {
                            if ((f.field_name.clone().as_str() == field_label.clone().as_str())
                                && ((f.variant_name.clone().as_str() == vname.clone().as_str())
                                    || (f.variant_name.clone().as_str()
                                        == "".to_string().as_str())))
                            {
                                __result.push(f);
                            }
                    ...
                    .cloned();
                    match matching {
                        Some(ind_field) => v2_rt::rc_map_insert(
                            fp_acc,
                            pname.clone(),
            ...
            ExprData::ExprFieldAccess { summary: _, .. } => {
                ...
                match (*base.expr_data.clone()).clone() {
                    ExprData::ExprVar {
                        ...
                        let base_type =
                            match v2_rt::map_get(&ctx.param_names.clone(), bname.clone()) {
                                Some(t) => t.clone(),
                                None => {
                                    match v2_rt::map_get(&ctx.sub_value_vars.clone(), bname.clone())
                                    {
                                        Some(rel) => match (*rel.clone()).clone() {
                        ...
                        if (base_type.clone().as_str() != "".to_string().as_str()) {
                            break Rc::new({
                                ...
                                for f in Rc::new({
                                    ...
                                    .cloned()
                                    {
                                        if (f.field_name.clone().as_str() == fname.clone().as_str())
                                        {
                                            __result.push(f);
                                        }
                                    }
                        ...
                        Some(binding) => {
                            ...
                            let matching = Rc::new({
                                ...
                                for f in fields.iter().cloned() {
                                    if (f.field_name.clone().as_str() == field.clone().as_str()) {
                                        __result.push(f);
                                    }
                                }
                                __result
        ...
        ExprData::ExprFieldAccess { summary: _, .. } => {
            ...
            match (*base.expr_data.clone()).clone() {
                ExprData::ExprVar {
                    ...
                    let from_per_field =
                        match v2_rt::map_get(&ctx.per_field_vars.clone(), bname.clone()) {
                    ...
                    match from_per_field {
                        ...
                        None => {
                            let type_name =
                                match v2_rt::map_get(&ctx.scope_locals.clone(), bname.clone()) {
                                ...
                                Some(first_arg) => {
                                    ...
                                    match (*inner.expr_data.clone()).clone() {
                                        ExprData::ExprVar {
                                            ...
                                            let inner_type = match v2_rt::map_get(
                                                &ctx.param_names.clone(),
                                                iname.clone(),
                                            ) {
                                                Some(t) => t.clone(),
                ...
                Some(InferredNode::Resolved { node: rt, .. }) => {
                    if is_container_type(authored_name_at(
                    ...
                    )) {
                        {
                            ...
                            if ((elem.clone().as_str() != "".to_string().as_str())
            ...
            match type_based.clone() {
                ...
                None => match (*val.expr_data.clone()).clone() {
                        ...
                        match op.clone() {
                            BinOp::Sub => match (*left.expr_data.clone()).clone() {
                                ExprData::ExprVar {
                                    ...
                                    if ((v2_rt::map_get(&ctx.param_names.clone(), lname.clone())
                                        != None)
                                        || (v2_rt::map_get(
                                            &ctx.sub_value_vars.clone(),
                                            lname.clone(),
                    ...
                    ExprData::ExprMethodCall {
                        ...
                        let is_collection_preserving = (((((mname.clone().as_str()
                            == "skip".to_string().as_str())
                        ...
                        if is_collection_preserving {
                            {
                                ...
                                match coll_field {
                                    ...
                                    None => {
                                        if (mname.clone().as_str() == "skip".to_string().as_str()) {
                                            {
...
) -> Option<Rc<SubValueRelation>> {
    {
        ...
        let arg_by_name = call_args.iter().cloned().fold(
            ...
            |acc: Rc<HashMap<String, Rc<Node>>>, arg_node: Rc<Node>| match arg_name_at(
                ...
                Some(aname) => {
                    v2_rt::rc_map_insert(acc.clone(), aname.clone(), arg_value(arg_node.clone()))
                }
                None => acc.clone(),
            },
        );
        Rc::new(v2_rt::map_keys(&param_map)).iter().cloned().fold(
            None,
            |result: _, callee_pname: String| match v2_rt::map_get(&param_map, callee_pname.clone())
            {
                    ...
                    Some(first_arg) => {
                        ...
                        match (*inner_rel.clone()).clone() {
                            ...
                            SubValueRelation::PreservedValue => {
                                ...
                                let from_provenance =
                                    match v2_rt::map_get(&ctx.func_sigs.clone(), callee.clone()) {
                                        Some(sig) => {
                                ...
                                match from_provenance {
                                    ...
                                    None => {
                                        ...
                                        {
                                            {
                                                let type_name = match v2_rt::map_get(
                                                    &ctx.param_names.clone(),
                                                    param_name.clone(),
                                                ) {
                                                    Some(t) => t.clone(),
...
) -> Rc<Vec<Rc<SubValueRelation>>> {
    {
        ...
        .fold(
            ...
            |acc: Rc<HashMap<String, Rc<Node>>>, pair: (i64, Rc<Node>)| {
                ...
                match aname.clone() {
                    Some(pname) => v2_rt::rc_map_insert(
                        acc.clone(),
                        pname.clone(),
                ...
                let annotated_children = Rc::new({
                    ...
                    for child in body.children.clone().iter().cloned() {
                        __result.push({
                            ...
                            match (*arg_val.expr_data.clone()).clone() {
                                ExprData::ExprLambda => {
                                    if (source_type.clone().as_str() != "".to_string().as_str()) {
                                        {
                                            ...
                                            let lambda_ctx = Rc::new(DescentContext {
                                                ...
                                                param_names: if (p2.clone().as_str()
                                                    != "".to_string().as_str())
                                                {
                                                    v2_rt::rc_map_insert(
                                                        v2_rt::rc_map_insert(
                                                ...
                                                sub_value_vars: if (p1.clone().as_str()
                                                    != "".to_string().as_str())
                                                {
                                                    {
            ...
            ExprData::ExprMatch => {
                ...
                let scrut_type = match (*scrut.expr_data.clone()).clone() {
                    ExprData::ExprVar {
                        ...
                        match v2_rt::map_get(&ctx.param_names.clone(), sname.clone()) {
                            Some(t) => t.clone(),
                            None => {
                                match v2_rt::map_get(&ctx.sub_value_vars.clone(), sname.clone()) {
                                    Some(rel) => match (*rel.clone()).clone() {
                                        SubValueRelation::StrictSubValue { field: f, .. } => {
                                            f.element_type.clone()
                                        }
                ...
                let scrut_inducing_field = match (*scrut.expr_data.clone()).clone() {
                    ...
                    ExprData::ExprMethodCall {
                        ...
                        if ((mname.clone().as_str() == "first".to_string().as_str())
                ...
                let scrut_has_inductive = (((scrut_type.clone().as_str()
                    != "".to_string().as_str())
                ...
                let scrut_variant_sig = match (*scrut.expr_data.clone()).clone() {
                    ExprData::ExprCall { .. } => {
                        ...
                        match v2_rt::map_get(&ctx.func_sigs.clone(), callee) {
                            Some(sig) => {
                                if ((Rc::new(v2_rt::map_keys(&sig.variant_provenance.clone())).len()
                                    as i64)
                                    > 0)
                                {
                                    Some(sig.clone())
                ...
                let inner_ctx = match val_relation {
                    Some(rel) => {
                        ...
                        Rc::new(DescentContext {
                            ...
                            scope_locals: v2_rt::rc_map_insert(
                                ...
                                match val_rel.clone() {
                                    Some(rel) => {
                                        ...
                                        Rc::new(DescentContext {
                                            ...
                                            param_names: if (sub_type.clone().as_str()
                                                != "".to_string().as_str())
                                            {
                                                v2_rt::rc_map_insert(
                ...
                if (elem_position.clone() != None) {
                    {
                            ...
                            ExprData::ExprFieldAccess { summary: _, .. } => {
                                ...
                                match (*base.expr_data.clone()).clone() {
                                    ExprData::ExprVar {
                                        ...
                                        let base_type = match v2_rt::map_get(
                                            &ctx.param_names.clone(),
                                            bname.clone(),
                                        ) {
                                            Some(t) => t.clone(),
                        ...
                        let inner_ctx = match lambda_elem_info {
                            Some(elem_name) => match receiver_field {
                                Some(ind_field) => Rc::new(DescentContext {
                                    fn_name: ctx.fn_name.clone(),
                                    param_names: v2_rt::rc_map_insert(
                                        ctx.param_names.clone(),
                                        elem_name.clone(),
...
) -> Rc<Vec<Rc<HashMap<String, Rc<SubValueRelation>>>>> {
    {
        ...
        let param_types = params.clone().iter().cloned().fold(
            ...
            |acc: Rc<HashMap<String, String>>, p: Rc<Node>| {
                ...
                if ((pname.clone().as_str() != "".to_string().as_str())
        ...
        } else {
            {
                ...
                if (((per_field.clone().len() as i64) > 0) && {
                    ...
                    for m in per_field.clone().iter().cloned() {
                        if ((Rc::new(v2_rt::map_keys(&m)).len() as i64) > 0) {
                            __found = true;
                            break;
                        }
                    }
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*expr.expr_data.clone()).clone() {
                    ...
                    None => {
                        if {
                            ...
                            for pn in param_names.clone().iter().cloned() {
                                if (pn.clone().as_str() == vname.clone().as_str()) {
                                    __found = true;
                                    break;
                                }
                            }
            ...
            ExprData::ExprFieldAccess { summary: _, .. } => {
                ...
                Rc::new(v2_rt::map_keys(&base_prov)).iter().cloned().fold(
                    v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
                    |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| {
                        match v2_rt::map_get(&base_prov, pname.clone()) {
                            Some(base_rel) => {
                        ...
                        {
                            {
                                ...
                                Rc::new(v2_rt::map_keys(&recv_prov)).iter().cloned().fold(
                                    v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
                                    |acc: Rc<HashMap<String, Rc<SubValueRelation>>>,
                                     pname: String| {
                                        match v2_rt::map_get(&recv_prov, pname.clone()) {
                                            Some(recv_rel) => {
                                                let synth_field = Rc::new(InductiveField {
...
if ((bind_name.clone().as_str() != "".to_string().as_str()) && (field_label.clone().as_str() != "".to_string().as_str())) {
                        {
                            let field_prov = Rc::new(v2_rt::map_keys(&scrut_prov)).iter().cloned().fold(empty_prov_map(), |fp_acc: Rc<HashMap<String, Rc<SubValueRelation>>>, pname: String| maybe_insert_match_field_relation(fp_acc, pname.clone(), field_label.clone(), vname.clone(), v2_rt::map_get(&scrut_prov, pname.clone()), param_types.clone(), type_env.clone()));
...
classify_body_provenance(arm_body(arm.clone()), param_names.clone(), param_types.clone(), type_env.clone(), func_sigs.clone(), arm_let_prov.clone())
            ...
            ExprData::ExprLet => {
                ...
                let updated =
                    v2_rt::rc_map_insert(let_prov.clone(), bname.clone(), value_prov.clone());
                ...
                let block_result = expr.children.clone().iter().cloned().fold(
                        ...
                        .clone()
                    {
                        ExprData::ExprLet => {
                            ...
                            let updated_prov = v2_rt::rc_map_insert(
                                state.prov.clone(),
                                bname.clone(),
                                value_prov.clone(),
                            );
...
pub fn compose_callee_provenance(
    ...
    match v2_rt::map_get(&func_sigs, callee) {
        Some(sig) => match sig.output_provenance.clone().first().cloned() {
            Some(callee_prov) => Rc::new(v2_rt::map_keys(&callee_prov)).iter().cloned().fold(
                v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
                |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, callee_pname: String| {
                    match v2_rt::map_get(&callee_prov, callee_pname.clone()) {
                        Some(callee_rel) => {
                            match v2_rt::map_get(&arg_by_name, callee_pname.clone()) {
                                Some(arg) => {
                                    ...
                                    Rc::new(v2_rt::map_keys(&arg_prov)).iter().cloned().fold(
                                        acc.clone(),
                                        |inner: Rc<HashMap<String, Rc<SubValueRelation>>>,
                                         pname: String| {
                                            match v2_rt::map_get(&arg_prov, pname.clone()) {
                                                Some(arg_rel) => {
...
pub fn intersect_prov_maps(
    ...
    Rc::new(v2_rt::map_keys(&a)).iter().cloned().fold(
        empty_prov_map(),
        |result: Rc<HashMap<String, Rc<SubValueRelation>>>, key: String| match v2_rt::map_get(
            &b,
            key.clone(),
        ) {
            Some(b_rel) => match v2_rt::map_get(&a, key.clone()) {
                Some(a_rel) => v2_rt::rc_map_insert(
                    result.clone(),
                    key.clone(),
...
pub fn arm_ctx_from_variant_provenance(
    ...
    match (*arm_pattern(arm_node)).clone() {
        MatchPattern::VariantPattern {
            ...
            field_bindings: bindings,
            ..
        } => match v2_rt::map_get(&sig.variant_provenance.clone(), vname.clone()) {
                            ...
                            || (field_label.clone().as_str() == "".to_string().as_str()))
                        {
                            ...
                            match v2_rt::map_get(&variant_field_map, field_label.clone()) {
                                Some(param_map) => {
                                    ...
                                    let all_rels = Rc::new({
                                        let mut __result = Vec::new();
                                        for k in Rc::new(v2_rt::map_keys(&composed)).iter().cloned()
                                        {
                                            __result.push(
                                                match v2_rt::map_get(&composed, k.clone()) {
                                                    Some(r) => r.clone(),
                                                    None => {
                                                        Rc::new(SubValueRelation::SubValueUnknown)
                                                    }
...
pub fn compose_callee_param_map(
    ...
    Rc::new(v2_rt::map_keys(&callee_prov)).iter().cloned().fold(
        v2_rt::rc_empty_map::<String, Rc<SubValueRelation>>(),
        |acc: Rc<HashMap<String, Rc<SubValueRelation>>>, callee_pname: String| match v2_rt::map_get(
            &callee_prov,
            callee_pname.clone(),
        ) {
            Some(callee_rel) => match v2_rt::map_get(&arg_by_name, callee_pname.clone()) {
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*body.expr_data.clone()).clone() {
            ExprData::ExprRecordLit {
                ...
                Some(enum_name) => {
                    if (enum_name.clone().as_str() == parent_enum.clone().as_str()) {
                        {
...
pub fn merge_field_prov_maps(
    ...
    {
        let from_a = Rc::new(v2_rt::map_keys(&a)).iter().cloned().fold(
            v2_rt::rc_empty_map::<String, Rc<HashMap<String, Rc<SubValueRelation>>>>(),
            |result: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>,
             fname: String| match v2_rt::map_get(&a, fname.clone()) {
                Some(a_prov) => match v2_rt::map_get(&b, fname.clone()) {
                    Some(b_prov) => v2_rt::rc_map_insert(
                        result.clone(),
                        fname.clone(),
        ...
        Rc::new(v2_rt::map_keys(&b)).iter().cloned().fold(
            from_a,
            |result: Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>,
             fname: String| {
                if (v2_rt::map_get(&result, fname.clone()) != None) {
                    result.clone()
                } else {
                    match v2_rt::map_get(&b, fname.clone()) {
                        Some(b_prov) => {
                            v2_rt::rc_map_insert(result.clone(), fname.clone(), b_prov.clone())
                        }
                        None => result.clone(),
                    }
                }
...
) -> Rc<HashMap<String, Rc<SubValueRelation>>> {
    {
        let from_a = Rc::new(v2_rt::map_keys(&a)).iter().cloned().fold(
            ...
            |result: Rc<HashMap<String, Rc<SubValueRelation>>>, key: String| match v2_rt::map_get(
                ...
                Some(a_rel) => match v2_rt::map_get(&b, key.clone()) {
                    Some(b_rel) => v2_rt::rc_map_insert(
                        result.clone(),
                        key.clone(),
        ...
        Rc::new(v2_rt::map_keys(&b)).iter().cloned().fold(
            ...
            |result: Rc<HashMap<String, Rc<SubValueRelation>>>, key: String| {
                if (v2_rt::map_get(&result, key.clone()) != None) {
                    result.clone()
                } else {
                    match v2_rt::map_get(&b, key.clone()) {
                        Some(b_rel) => {
                            v2_rt::rc_map_insert(result.clone(), key.clone(), b_rel.clone())
                        }
                        None => result.clone(),
                    }
                }
        ...
        let results = Rc::new({
            ...
            for p in props.iter().cloned() {
                __result.push(
                    ...
                    .as_str()
                        == "svc_auth_source".to_string().as_str())
                    {
                        {
                            let val = field_init_node_value(p.clone());
                            let val_result = infer_expr(val.clone(), scope.clone(), None);
                    ...
                    |bacc: Rc<HashMap<i64, Rc<TypeBinding>>>, ident: i64| {
                        ...
                        if is_type_variable_name(name.clone()) {
                            ...
                            match v2_rt::map_get(&env.bindings.clone(), ident.clone()) {
                                Some(binding) => v2_rt::rc_map_insert(
                                    bacc.clone(),
                                    ident.clone(),
                                    binding.clone(),
                                ),
    ...
    envs.iter().cloned().fold(
        ...
        |acc: Rc<HashMap<i64, Rc<TypeBinding>>>, env: Rc<TypeEnv>| {
            ...
            Rc::new(v2_rt::map_values(&env.bindings.clone()))
                ...
                .fold(
                    with_existing.clone(),
                    |bacc: Rc<HashMap<i64, Rc<TypeBinding>>>, binding: Rc<TypeBinding>| {
                        match binding.resolved.clone().binding_id.clone() {
                            Some(binding_id) => v2_rt::rc_map_insert(
                                bacc.clone(),
                                binding_id.value.clone(),
                                binding.clone(),
                            ),
...
pub fn lookup_coproduct_parent_binding_by_variant(
    env: Rc<TypeEnv>,
    variant_name: String,
) -> Option<Rc<TypeBinding>> {
    {
        let visible_matches = Rc::new({
            let mut __result = Vec::new();
            for binding in Rc::new(v2_rt::map_values(&env.bindings.clone()))
                .iter()
                .cloned()
        ...
        let matches = v2_rt::concat(visible_matches, carrier_matches);
        if ((matches.clone().len() as i64) == 1) {
            matches.clone().first().cloned()
        } else {
            None
        }
    }
...
pub fn stamp_hidden_dependency_refs(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let rebuilt = map_node_subtrees(n, |child| {
            stamp_hidden_dependency_refs(child.clone(), dep_env.clone(), source_indices.clone())
        });
...
pub fn stamp_type_binding_dependency_refs(
    ...
    Rc::new(TypeBinding {
        ...
        resolved: stamp_hidden_dependency_refs(binding.resolved.clone(), dep_env, source_indices),
...
pub struct CarrierClosureState {
    pub carrier_bindings: Rc<HashMap<i64, Rc<TypeBinding>>>,
    pub seen_names: Rc<HashMap<String, bool>>,
    pub seen_binding_ids: Rc<HashMap<i64, bool>>,
}

#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct CarrierClosureDepsContext {
    pub n: Rc<Node>,
    pub dep_env: Rc<TypeEnv>,
                        ...
                        match v2_rt::map_get(&env.bindings.clone(), decl.binding_key.clone()) {
                            Some(binding) => {
                                if node_has_binding_id(
                                    ...
                                    v2_rt::map_get(
                                        &env.carrier_bindings.clone(),
                                        decl.binding_id.clone().value.clone(),
                                    )
                                }
                            }
                            None => v2_rt::map_get(
...
pub fn carrier_closure_direct_type_deps(
...
) -> Rc<Vec<String>> {
    {
        ...
        let decl_ref = match n.binding_id.clone() {
            Some(binding_id) => {
                match v2_rt::map_get(&dep_env.decl_registry.clone(), binding_id.value.clone()) {
                    Some(decl) => {
...
pub fn carrier_closure_field_node_type_deps(
    child: Rc<Node>,
    dep_env: Rc<TypeEnv>,
    source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>,
) -> Rc<Vec<String>> {
    carrier_closure_node_type_deps(field_node_type_expr(child), dep_env, source_indices)
}

pub fn carrier_closure_node_conj_child_deps(ctx: Rc<CarrierClosureDepsContext>) -> Rc<Vec<String>> {
    {
        let children = ctx.n.clone().children.clone();
        Rc::new({
            let mut __result = Vec::new();
            for child in children.iter().cloned() {
                __result.extend(
                    (*carrier_closure_field_node_type_deps(
                        child.clone(),
                        ctx.dep_env.clone(),
...
pub fn carrier_closure_node_plain_child_deps(
    ctx: Rc<CarrierClosureDepsContext>,
) -> Rc<Vec<String>> {
    {
        let children = ctx.n.clone().children.clone();
        Rc::new({
...
pub fn carrier_closure_node_type_deps(
    ...
    {
        let base =
            carrier_closure_direct_type_deps(n.clone(), dep_env.clone(), source_indices.clone());
        let ctx = Rc::new(CarrierClosureDepsContext {
            n: n.clone(),
            dep_env: dep_env.clone(),
            source_indices: source_indices.clone(),
        });
        let conj_deps = carrier_closure_node_conj_child_deps(ctx.clone());
        let plain_deps = carrier_closure_node_plain_child_deps(ctx.clone());
        let child_deps = match n.connective.clone() {
            Connective::Conj => conj_deps,
            _ => plain_deps,
        };
...
pub fn add_type_binding_dependency_closure(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        let already_seen = match binding.resolved.clone().binding_id.clone() {
            Some(binding_id) => {
                v2_rt::map_contains_key(&state.seen_binding_ids.clone(), binding_id.value.clone())
            }
        ...
        if already_seen {
        ...
        } else {
            {
                ...
                let with_binding = match stamped.resolved.clone().binding_id.clone() {
        ...
        let closure_state = Rc::new(v2_rt::map_values(&env.bindings.clone()))
            ...
            .fold(
                Rc::new(CarrierClosureState {
                    ...
                    seen_binding_ids: v2_rt::rc_empty_map::<i64, bool>(),
                }),
...
pub fn collect_import_carrier_bindings_for_imported_funcs(
...
) -> Rc<HashMap<i64, Rc<TypeBinding>>> {
    {
        ...
        let closure_state = Rc::new(v2_rt::map_values(
            &typed_parent.func_env.clone().signatures.clone(),
        ))
        .iter()
        .cloned()
...
pub fn import_allows_type_binding(
    ...
    if import_allows_binding_name(imp.clone(), binding.name.clone()) {
    ...
    } else {
        {
            ...
            ((binding.resolved.clone().connective.clone() == Connective::Disj) && allowed_child)
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct TypeDeclRegistrationAccum {
    pub bindings: Rc<HashMap<i64, Rc<TypeBinding>>>,
    pub decl_registry: Rc<HashMap<i64, Rc<TypeDeclBinding>>>,
    pub allocator: BindingIdAllocator,
}

pub fn register_type_decl_binding(
    acc: Rc<TypeDeclRegistrationAccum>,
    item_ident: i64,
    name: String,
    module_name: String,
    decl_span: Rc<SourceSpan>,
    resolved: Rc<Node>,
    provenance: Rc<SubValueRelation>,
) -> Rc<TypeDeclRegistrationAccum> {
    {
        ...
        Rc::new(TypeDeclRegistrationAccum {
            ...
            decl_registry: v2_rt::rc_map_insert(
                ...
                binding_alloc.binding_id.clone().value.clone(),
                Rc::new(TypeDeclBinding {
                    binding_id: binding_alloc.binding_id.clone(),
                    authority: Rc::new(TypeDeclAuthority {
            ...
            allocator: binding_alloc.allocator.clone(),
        })
    }
}
...
pub fn kernel_span(name: String) -> Rc<SourceSpan> {
    Rc::new(SourceSpan {
        file: v2_rt::concat(
            v2_rt::concat("<kernel:".to_string(), name.clone()),
            ">".to_string(),
        ),
        start: 0,
...
pub fn build_type_env(
    ...
    {
        let source_indices = Rc::new(v2_rt::map_keys(&kernel_type_set()))
            .iter()
            .cloned()
            .fold(
                source_indices.clone(),
        ...
        let intern_table = Rc::new(v2_rt::map_keys(&kernel_type_set()))
            .iter()
            .cloned()
        ...
        let intern_table = Rc::new(v2_rt::map_keys(&compiler_recursive_types()))
            .iter()
            .cloned()
            .fold(intern_table.clone(), |t: Rc<InternTable>, name: String| {
                intern(t, name.clone()).table.clone()
            });
        let kernel_bindings_base = Rc::new(v2_rt::map_keys(&kernel_type_set()))
            .iter()
            .cloned()
            .fold(
                v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(),
        ...
        let kernel_recursive_types = Rc::new({
            let mut __result = Vec::new();
            for name in Rc::new(v2_rt::map_keys(&compiler_recursive_types()))
                .iter()
                .cloned()
            {
        ...
        let kernel_recursive_type_set = Rc::new(v2_rt::map_keys(&compiler_recursive_types()))
            .iter()
            .cloned()
            .fold(
        ...
        let kernel = Rc::new(TypeEnv {
            ...
            carrier_bindings: v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(),
        ...
        let import_bindings = module.resolved_imports.clone().iter().cloned().fold(
            std_import_bindings,
            |acc: Rc<HashMap<i64, Rc<TypeBinding>>>, imp: Rc<ResolvedImport>| match v2_rt::map_get(
                &parent_index,
                imp.module_path.clone(),
            ) {
                Some(typed_parent) => collect_import_bindings_for_import(
                    acc.clone(),
                    imp.clone(),
                    typed_parent.type_env.clone(),
                ),
        ...
        let local_registration = module_items(module.module.clone()).iter().cloned().fold(
            Rc::new(TypeDeclRegistrationAccum {
                bindings: v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(),
                decl_registry: v2_rt::rc_empty_map::<i64, Rc<TypeDeclBinding>>(),
                allocator: allocator.clone(),
            }),
            |acc: Rc<TypeDeclRegistrationAccum>, item: Rc<Node>| {
                                ...
                                && (item.transport.clone() == None))
                            {
                                    ...
                                    && ((item.params.clone().len() as i64) == 0))
                                {
                                    register_type_decl_binding(
                                        ...
                                        module_name_str.clone(),
                                        item.span.clone(),
                                        nominal_leaf_type(item_name.clone()),
                                        Rc::new(SubValueRelation::SubValueUnknown),
                                    )
                                } else {
                                    acc.clone()
        ...
        let local_bindings = local_registration.bindings.clone();
        let local_decl_registry = local_registration.decl_registry.clone();
        let allocator = local_registration.allocator.clone();
        let param_bindings = module_items(module.module.clone()).iter().cloned().fold(
            v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(),
            |acc: Rc<HashMap<i64, Rc<TypeBinding>>>, item: Rc<Node>| {
                let is_type_decl = match v2_rt::map_get(
                    &local_bindings,
                    intern(
                        intern_table.clone(),
                        authored_name_at(source_indices.clone(), item.clone()),
        ...
        let local_name_set = Rc::new(v2_rt::map_keys(&local_bindings))
            .iter()
            .cloned()
            .fold(
                v2_rt::rc_empty_map::<i64, bool>(),
                |acc: Rc<HashMap<i64, bool>>, ident: i64| {
                    v2_rt::rc_map_insert(acc, ident.clone(), true)
                },
            );
        ...
        let str_bindings = Rc::new(v2_rt::map_values(&merged.bindings.clone()))
            .iter()
            .cloned()
            .fold(
                v2_rt::rc_empty_map::<String, Rc<TypeBinding>>(),
                |acc: Rc<HashMap<String, Rc<TypeBinding>>>, b: Rc<TypeBinding>| {
                    v2_rt::rc_map_insert(acc, b.name.clone(), b.clone())
                },
            );
...
pub fn build_type_env_unresolved(
...
) -> Rc<BuildTypeEnvResult> {
    {
        ...
        let kernel_bindings = Rc::new(v2_rt::map_keys(&kernel_type_set()))
            .iter()
            .cloned()
            .fold(
                v2_rt::rc_empty_map::<i64, Rc<TypeBinding>>(),
        ...
        Rc::new(BuildTypeEnvResult {
            env: unresolved_env,
            allocator: empty_binding_id_allocator(),
            diagnostics: Rc::new(vec![]),
        })
    }
}
        ...
        let func_sig = if (is_func || is_zero_arg_func) {
            {
                ...
                Some(Rc::new(DeclaredFuncSig {
                    ...
                    variant_provenance: v2_rt::rc_empty_map::<
                        String,
                        Rc<HashMap<String, Rc<HashMap<String, Rc<SubValueRelation>>>>>,
                    >(),
                }))
        ...
        match remaining.clone().first().cloned() {
            ...
            Some(contribution) => {
                ...
                let next_svc_locals = match contribution.svc_local.clone() {
                    Some(binding) => {
                        v2_rt::rc_map_insert(svc_locals, binding.name.clone(), binding.clone())
                    }
                    None => svc_locals,
                };
                {
                    ...
                    let __tco_1 =
                        v2_rt::rc_list_push(resolved_items, contribution.resolved_item.clone());
                    let __tco_2 = next_func_sigs;
                    let __tco_3 = next_svc_registry;
                    let __tco_4 = next_svc_locals;
                    let __tco_5 = v2_rt::rc_map_insert(
                        item_registry,
                        contribution.item_info.clone().name.clone(),
                        contribution.item_info.clone(),
                    );
                    let __tco_6 =
                        v2_rt::rc_list_push(diag_chunks, contribution.resolve_diagnostics.clone());
                    remaining = __tco_0;
...
pub fn build_module_context(
...
) -> Rc<ModuleContext> {
    {
        let local = fold_module_contributions(
            contributions,
            Rc::new(vec![]),
            v2_rt::rc_empty_map::<String, Rc<DeclaredFuncSig>>(),
            v2_rt::rc_empty_map::<String, Rc<Vec<Rc<OpEntry>>>>(),
            v2_rt::rc_empty_map::<String, Rc<TypeBinding>>(),
            v2_rt::rc_empty_map::<String, Rc<ItemInfo>>(),
            Rc::new(vec![]),
            env.source_indices.clone(),
        );
        let imported_enum_names = resolved_imports.clone().iter().cloned().fold(
            v2_rt::rc_empty_map::<String, bool>(),
            |acc: Rc<HashMap<String, bool>>, imp: Rc<ResolvedImport>| match v2_rt::map_get(
                &parent_index,
                imp.module_path.clone(),
            ) {
        ...
        let imported_variant_parent_names = resolved_imports.clone().iter().cloned().fold(
            v2_rt::rc_empty_map::<String, bool>(),
            |acc: Rc<HashMap<String, bool>>, imp: Rc<ResolvedImport>| match v2_rt::map_get(
                &parent_index,
                imp.module_path.clone(),
...
let curr_is_imported = (curr_parent_imported.clone() || curr_variant_imported.clone());
match v2_rt::map_get(&vacc.locals.clone(), child_name.clone()) {
    Some(prev) => {
                            let prev_parent_name = authored_name_at(env.source_indices.clone(), prev.resolved.clone());
...
let same_parent_binding = match binding.resolved.clone().binding_id.clone() {
    ...
    Some(prev_binding_id) => ((curr_binding_id.clone() == prev_binding_id.clone()) || match v2_rt::map_get(&env.decl_registry.clone(), curr_binding_id.value.clone()) {
    Some(curr_decl) => match v2_rt::map_get(&env.decl_registry.clone(), prev_binding_id.value.clone()) {
...
if (((((((curr_variant_imported.clone() && prev_variant_imported.clone()) && (same_parent_binding.clone() == false)) && (is_bootstrap_duplicate_variant_name(child_name.clone()) == false)) && (curr_parent_imported.clone() == false)) && (prev_parent_imported.clone() == false)) && curr_is_imported.clone()) && prev_is_imported.clone()) {
    ...
    enum1: prev_parent_name.clone(),
    enum2: curr_parent_name.clone(),
    span: no_span(),
}), module_name.clone())),
})
                            } else {
    ...
    collisions: vacc.collisions.clone(),
        ...
        let all_declared_sigs =
            v2_rt::rc_map_merge(merged_scope.func_sigs.clone(), local.func_sigs.clone());
        ...
        let all_locals = Rc::new(v2_rt::map_values(&merged_scope.svc_locals.clone()))
            .iter()
            .cloned()
            .fold(
                env_variant_locals,
                |acc: Rc<HashMap<String, Rc<TypeBinding>>>, binding: Rc<TypeBinding>| {
                    v2_rt::rc_map_insert(acc, binding.name.clone(), binding.clone())
                },
            );
...
) -> Rc<ParentModulesResult> {
    {
        let modules = Rc::new({
            ...
            for imp in resolved.resolved_imports.clone().iter().cloned() {
                __result.extend(
                    (*match v2_rt::map_get(&module_index, imp.module_path.clone()) {
...
pub fn typecheck_modules(
    ...
    mut allocator: BindingIdAllocator,
) -> Rc<TypedGraph> {
    loop {
        match remaining.clone().first().cloned() {
            None => {
            ...
            Some(resolved) => {
                ...
                allocator = tc_result.allocator.clone();
                {
                    let __tco_0 = Rc::new(
                        remaining
                            .iter()
                    ...
                    let __tco_2 = v2_rt::rc_map_insert(
                        module_index,
                    ...
                    let __tco_4 = v2_rt::rc_list_push(
                        v2_rt::rc_list_push(diag_chunks, parent_result.diagnostics.clone()),
                        tc_diags,
                    );
                    remaining = __tco_0;
                    modules = __tco_1;
