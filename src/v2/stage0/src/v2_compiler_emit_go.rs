...
pub fn emit_go(typed: &Rc<ResolvedGraph>) -> Rc<EmitResult> {
    {
        ...
        let module_files = Rc::new({
            ...
            for tm in typed.modules.clone().iter().cloned() {
                __result.push(emit_go_module(&tm, registry.clone()));
            }
            __result
        });
    ...
    } else {
        {
            ...
            let tests_str = Rc::new({
                ...
                for p in projections.clone().iter().cloned() {
                    __result.push(emit_go_operation_test(&p, 0));
                }
                __result
            })
            .join(&"\n\n".to_string());
                                            ...
                                            v2_rt::concat(
                                                v2_rt::concat(
                                                    v2_rt::concat(
                                                        ...
                                                        go_test_signature_comment(&projection),
                                                    ),
                                                    "\n".to_string(),
                                                ),
                                                indent.clone(),
...
) -> Rc<TextFile> {
    {
        ...
        let items_str = Rc::new({
            ...
            for item in typed_module.items.clone().iter().cloned() {
                __result.push(emit_go_typed_item(&item, registry.clone(), &scope));
            }
            __result
        })
        .join(&"\n\n".to_string());
    ...
    } else {
        {
            let field_lines = Rc::new({
                ...
                for child in children.clone().iter().cloned() {
                    __result.push(emit_go_struct_field_from_child(&child, &env));
                }
                __result
            });
...
pub fn emit_go_struct_field_from_child(child: &Rc<Node>, env: &Rc<TypeEnv>) -> String {
    {
        ...
        let json_tag = v2_rt::concat(
            v2_rt::concat(" `json:\"".to_string(), to_snake(child_text.clone())),
            "\"`".to_string(),
        );
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat("\t".to_string(), go_export_ident(child_text.clone())),
                    " ".to_string(),
                ),
                ty,
            ),
        ...
        if has_data {
            {
                ...
                let variant_structs = Rc::new({
                    let mut __result = Vec::new();
                    for child in children.clone().iter().cloned() {
                        __result.push(emit_go_variant_struct(&name, &child, &env));
                    }
                    __result
                });
...
pub fn emit_go_variant_struct(parent_name: &String, child: &Rc<Node>, env: &Rc<TypeEnv>) -> String {
    {
        ...
        let marker_method = v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat(
        ...
        } else {
            {
                let field_lines = Rc::new({
                    ...
                    for f in child.children.clone().iter().cloned() {
                        __result.push(emit_go_struct_field_from_child(&f, &env));
                    }
                    __result
                });
                let fields_str = field_lines.join(&"\n".to_string());
...
pub fn emit_go_func_def(
...
) -> String {
    {
        ...
        let body_str =
            emit_unified_typed_func_body(&body, &RenderTarget::Go, &registry, &body_scope, 1);
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat(
...
) -> String {
    {
        let param_strs = Rc::new({
            ...
            for p in params.iter().cloned() {
                __result.push(emit_param_shared(&p, &RenderTarget::Go, &source_indices));
            }
            __result
        });
...
pub fn emit_go_transport_body(
    ...
    emit_unified_transport_dispatch(
        ...
        depth,
        RenderTarget::Go,
        |n, t, d, si| emit_go_rest_call(n.clone(), &t, d.clone(), &si),
        |n, t, d, si| emit_go_shell_call(n.clone(), t.clone(), d.clone(), &si),
...
pub fn emit_go_rest_call(
...
) -> String {
    {
        ...
        let url_line = v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(prefix.clone(), "url := fmt.Sprintf(\"%s/".to_string()),
                to_snake(op_name),
            ),
            "\", c.BaseURL)".to_string(),
        );
        let body_line = v2_rt::concat(
            prefix.clone(),
            "reqBody := bytes.NewBuffer(nil)".to_string(),
        );
        let req_line = v2_rt::concat(
            v2_rt::concat(
                prefix.clone(),
        ...
        let auth_line = if transport_has_auth(transport.clone(), source_indices.clone()) {
            {
                ...
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat(prefix.clone(), "req.Header.Set(\"".to_string()),
                        header_name,
                    ),
...
pub fn emit_go_shell_call(
    ...
    {
        let prefix = make_indent(depth);
        let cmd_line = v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(prefix.clone(), "cmd := exec.Command(\"".to_string()),
                to_snake(op_name),
            ),
            "\")".to_string(),
        );
        ...
        let run_lines = v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(prefix.clone(), "output, err := cmd.Output()\n".to_string()),
        ...
        let all_lines = v2_rt::concat(
            v2_rt::concat(Rc::new(vec![cmd_line, dir_line]), env_lines),
        ...
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(prefix, "path := fmt.Sprintf(\"%s/".to_string()),
                            to_snake(op_name),
                        ),
                        "\", c.BasePath)\n".to_string(),
                    ),
...
pub fn emit_go_resource_def(item: &Rc<Node>, env: &Rc<TypeEnv>) -> String {
    {
        ...
        let methods = Rc::new({
            ...
            for c in cap_children.iter().cloned() {
                __result.push(emit_go_capability_method(&c, 1, &env));
            }
            __result
        });
...
) -> String {
    {
        ...
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat("var ".to_string(), go_export_ident(name)),
                        " ".to_string(),
                    ),
                    ty_str,
                ),
