...
pub fn keyed_collection_parts(n: &Rc<Node>, source_index: Option<Rc<NewlineIndex>>) -> Option<Rc<KeyedCollectionParts>> {
    if (node_is_keyed_collection(&n, source_index) && ((n.children.clone().len() as i64) >= 2)) {
...
let base_is_string = node_type_equals(&normed, &string_type(), source_index.clone());
let index_is_int = node_type_equals(&normed_index, &int_type(), source_index.clone());
if base_is_string {
            {
...
access_result(string_type(), &diags, span.clone(), "invalid string index access".to_string())
            ...
            match keyed_collection_parts(&normed, source_index.clone()) {
    Some(parts) => {
...
access_result(with_optional_cardinality(&parts.value_type.clone()), &key_diags, span.clone(), "invalid keyed collection index access".to_string())
    ...
    None => if node_is_keyed_collection(&normed, source_index.clone()) {
                {
...
access_result(unit_type(), &malformed_diags, span.clone(), "malformed keyed collection type in index access".to_string())
                ...
                if ((is_ordered_element_collection(normed.name.clone()) && node_is_element_collection(&normed, source_index.clone())) && index_is_int) {
                    {
