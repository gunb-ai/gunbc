...
use crate::v2_std_core::TokenShape::{
    ...
    ShGe, ShGt, ShIdent, ShKwAcquire, ShKwCapability, ShKwData, ShKwElse, ShKwExtern, ShKwFalse,
    ShKwFn, ShKwFor, ShKwFunc, ShKwHermetic, ShKwIdempotent, ShKwIf, ShKwImport, ShKwIn, ShKwInput,
    ShKwInterface, ShKwLet, ShKwMatch, ShKwModule, ShKwNone, ShKwOperation, ShKwOutput,
    ShKwPattern, ShKwPipeline, ShKwProfile, ShKwReadonly, ShKwRelease, ShKwResource, ShKwReturn,
    ShKwService, ShKwTrue, ShKwType, ShKwWhere, ShKwWith, ShLBrace, ShLBracket, ShLParen, ShLe,
    ShLitFloat, ShLitInt, ShLitStr, ShLt, ShMinus, ShNe, ShNewline, ShNullCoalesce, ShOr,
    ShPercent, ShPipe, ShPipeArrow, ShPlus, ShQuestion, ShRBrace, ShRBracket, ShRParen, ShSlash,
    ShStar, ShStrBegin, ShStrEnd, ShStrMid, ShUnknown,
};
...
pub fn tokenize(source: String, file: String) -> Vec<Rc<Token>> {
    {
        ...
        let eof_span = SourceSpan::with_file(
            src.file.clone(),
...
pub fn emit(
...
) -> Rc<ScanResult> {
    {
        let token = make_token(
            text.clone(),
            SourceSpan::with_file(file, pos.pos.clone(), (pos.pos.clone() + len.clone())),
            shape.clone(),
        );
        Rc::new(ScanResult {
...
pub fn scan_ident(source: Rc<SourceRef>, pos: Rc<TokPos>) -> Rc<ScanResult> {
    {
        ...
        let token = make_token(
            text.clone(),
            SourceSpan::with_file(source.file.clone(), pos.pos.clone(), end.clone()),
            shape.clone(),
        );
        Rc::new(ScanResult {
            pos: end.clone(),
...
pub fn scan_number(source: Rc<SourceRef>, pos: Rc<TokPos>) -> Rc<ScanResult> {
    {
        ...
        if ((((int_end.clone() + 1) < string_length_ref(source.as_ref()))
            && (char_at_ref(source.as_ref(), int_end.clone()) == ".".to_string()))
            && is_digit(char_at_ref(source.as_ref(), (int_end.clone() + 1))))
        {
            {
                ...
                let token = make_token(
                    number_text.clone(),
                    SourceSpan::with_file(source.file.clone(), pos.pos.clone(), frac_end.clone()),
        ...
        let token = make_token(
            text.clone(),
            SourceSpan::with_file(source.file.clone(), pos.pos.clone(), int_end.clone()),
            shape.clone(),
        );
        Rc::new(ScanResult {
            pos: int_end.clone(),
...
pub fn scan_string(source: Rc<SourceRef>, pos: Rc<TokPos>) -> Rc<ScanResult> {
    {
        ...
        match (*result.clone()).clone() {
            ...
            StringScanResult::UnterminatedString {
                ...
                let token = make_token(
                    processed.clone(),
                    SourceSpan::with_file(source.file.clone(), span_start.clone(), end_pos.clone()),
                    TokenShape::ShUnknown,
                );
                Rc::new(ScanResult {
                    pos: end_pos.clone(),
...
pub fn process_escapes_loop(mut source: String, mut pos: i64, mut acc: Vec<String>) -> String {
    ...
    loop {
        if (pos.clone() >= string_length_text_ref(text)) {
            ...
            if ((ch.clone() == "\\".to_string())
                && ((pos.clone() + 1) < string_length_text_ref(text)))
            {
...
pub fn skip_spaces_and_comments(source: Rc<SourceRef>, pos: Rc<TokPos>) -> Rc<TokPos> {
    ...
    loop {
        ...
        if char_at_ref(source.as_ref(), p) == "/".to_string()
            && char_at_ref(source.as_ref(), p + 1) == "/".to_string()
        {
