...
}), |inner: Rc<DfsFinishAcc>, neighbor: String| dfs_finish_order(neighbor.clone(), &adjacency, &inner));
Rc::new(DfsFinishAcc {
...
}), |inner: Rc<SccComponentAcc>, neighbor: String| dfs_collect_component(neighbor.clone(), &adjacency, &inner))
}
        }
    })
}
