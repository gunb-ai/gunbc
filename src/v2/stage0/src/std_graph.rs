...
Rc::new({ let mut __result = Vec::new(); for e in edges.iter().cloned() { if (e.caller.clone().as_str() != e.callee.clone().as_str()) { __result.push(e); } } __result }).iter().cloned().fold(Rc::new(CallGraph {
...
let nd_graph = build_call_graph_from_proof_edges(&members, non_descending.clone());
