...
pub fn emit_go_mod(module_name: &str) -> Rc<TextFile> {
    ...
    Rc::new(TextFile { path: GO_MANIFEST_FILE.to_string(), content: content.clone() })
}
...
pub fn emit_go_module(typed_module: Rc<TypedModule>, registry: Rc<HashMap<String, Rc<ItemInfo>>>) -> Rc<TextFile> {
    ...
    Rc::new(TextFile { path: v2_rt::concat(filename, GO_SOURCE_EXTENSION.to_string()), content: content.clone() })
}
...
pub fn emit_go_typed_expr(texpr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64) -> String {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        let shared = emit_shared_expr(texpr.clone(), RenderTarget::Go, |t, s, d| emit_go_typed_expr(t.clone(), registry.clone(), s.clone(), 0_i64), scope.clone(), depth.clone());
        match shared {
    Some(result) => {
        if result.clone() == "" {
    "".to_string()
} else {
    v2_rt::concat(prefix, result.clone())
}
    }
    None => {
    ...
    ExprData::ExprLet { name: n, value: v, body: bd, .. } => {
        emit_go_typed_let(&n, v.clone(), bd.clone(), registry.clone(), scope.clone(), depth.clone())
    }
    ...
    ExprData::ExprBlock { stmts: ss, .. } => {
        emit_go_typed_block(ss.clone(), registry.clone(), scope.clone(), depth.clone())
    }
    ...
    ExprData::ExprForEach { variable: v, collection: c, body: bd, .. } => {
        emit_go_typed_for_each(&v, c.clone(), bd.clone(), registry.clone(), scope.clone(), depth.clone())
    }
