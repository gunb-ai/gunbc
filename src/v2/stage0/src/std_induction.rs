...
pub fn exponent_rank(e: Rc<PolynomialExponent>) -> i64 {
    match (*e).clone() {
    PolynomialExponent::IntegerExp { value: v, .. } => (v.clone() * 10),
    PolynomialExponent::FractionExp { numerator: n, root: r, .. } => if (r.clone() > 0) {
        ((n.clone() * 10) / r.clone())
} else {
        0
},
    PolynomialExponent::LogBasedExp { base: b, argument: a, .. } => if (a.clone() <= b.clone()) {
        10
} else {
        15
},
...
pub fn atomic_rank(c: Rc<AtomicCost>) -> i64 {
    match (*c).clone() {
    AtomicCost::LogCost { .. } => 1,
    AtomicCost::PolyCost { exponent: e, .. } => exponent_rank(e.clone()),
}
}

pub fn bound_rank(b: Rc<CostBound>) -> i64 {
    match (*b).clone() {
    CostBound::ConstantBound => 0,
    CostBound::AtomicBound { cost: c, .. } => atomic_rank(c.clone()),
    CostBound::ProductBound { factors: fs, .. } => fs.clone().iter().cloned().fold(0, |acc: i64, f: Rc<AtomicCost>| (acc.clone() + atomic_rank(f.clone()))),
    CostBound::SumOfProductsBound { terms: ts, .. } => ts.clone().iter().cloned().fold(0, |acc: i64, term: Rc<Vec<Rc<AtomicCost>>>| {
        let term_rank = term.clone().iter().cloned().fold(0, |tacc: i64, f: Rc<AtomicCost>| (tacc.clone() + atomic_rank(f.clone())));
if (term_rank.clone() > acc.clone()) {
            term_rank.clone()
} else {
            acc.clone()
}
}),
    CostBound::ForeverBound => 1000,
    CostBound::ErrorBound => 1001,
}
}

pub fn cost_dominates(a: Rc<CostBound>, b: Rc<CostBound>) -> bool {
    (bound_rank(a) < bound_rank(b))
}
