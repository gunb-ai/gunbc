...
pub fn go_spec() -> Rc<LanguageSpec> {
    Rc::new(LanguageSpec { target_name: "go".to_string(), reserved_words: Rc::new(ReservedWords { keywords: Rc::new(GO_RESERVED.iter().map(|s| s.to_string()).collect::<Vec<_>>()), strategy: Rc::new(ReservedWordStrategy::SuffixEscape { suffix: GO_RESERVED_ESCAPE_SUFFIX.to_string() }) }), scaffold: Rc::new(ProjectScaffold { manifest_file: Some(GO_MANIFEST_FILE.to_string()), module_init_file: None, source_file_extension: ".go".to_string(), source_dir: None }), serialization: Rc::new(SerializationSpec { struct_derives: None, struct_derives_copy: None, enum_derives: None, enum_derives_copy: None, tag_attribute: None, rename_attribute_template: None, derive_attribute: None, default_value: None }), test_conventions: Rc::new(TestConventions { file_prefix: "".to_string(), file_suffix: "_test".to_string(), file_dir: None, function_prefix: "Test".to_string(), name_style: TestNameStyle::PascalCaseTestNames, async_decorator: None }), top_level_visibility: "".to_string() })
}
