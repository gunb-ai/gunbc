            ...
            Some(first_seg) => match parse_segment_tokens(&first_seg) {
                ...
                Some(first_tokens) => {
                    ...
                    .fold(
                        ...
                        |acc: Option<Rc<Vec<Rc<UrlPathToken>>>>, seg: String| match acc.clone() {
                            None => None,
                            Some(parsed) => match parse_segment_tokens(&seg) {
                                Some(seg_tokens) => {
                                    Some(v2_rt::concat(parsed.clone(), seg_tokens.clone()))
                                }
                                None => None,
                            },
                        },
...
pub fn parse_segment_tokens(seg: &String) -> Option<Rc<Vec<Rc<UrlPathToken>>>> {
    if !v2_rt::contains(seg.clone(), "{".to_string()) {
    ...
    } else {
        {
            ...
            if (v2_rt::contains(prefix.clone(), "}".to_string())
            ...
            if (((param_name.clone().as_str() == "".to_string().as_str())
                || v2_rt::contains(suffix.clone(), "{".to_string()))
                || v2_rt::contains(suffix.clone(), "}".to_string()))
            {
                None
            } else {
                Some(v2_rt::concat(
                    v2_rt::concat(prefix_tokens, param_tokens),
                    suffix_tokens,
                ))
            }
        }
