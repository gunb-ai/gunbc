...
use UrlPathToken::*;
use PathSegmentTokensResult::*;
use PathTemplateParseResult::*;
...
match segments.clone().first().cloned() {
    ...
    PathSegmentTokensResult::MalformedPathSegment { segment: s, reason: r, .. } => Rc::new(PathTemplateParseResult::MalformedPathTemplate {
    raw: raw.clone(),
    segment: s.clone(),
    reason: r.clone(),
}),
    PathSegmentTokensResult::ParsedSegmentTokens { tokens: first_tokens, .. } => {
            let parsed = Rc::new(segments.clone().iter().cloned().skip(1 as usize).collect::<Vec<_>>()).iter().cloned().fold(Rc::new(PathTemplateParseResult::ParsedPathTemplate {
    template: Rc::new(PathTemplate {
    ...
    PathTemplateParseResult::ParsedPathTemplate { template: path, .. } => match (*parse_segment_tokens(&seg)).clone() {
    PathSegmentTokensResult::MalformedPathSegment { segment: s, reason: r, .. } => Rc::new(PathTemplateParseResult::MalformedPathTemplate {
    raw: raw.clone(),
    segment: s.clone(),
    reason: r.clone(),
}),
    PathSegmentTokensResult::ParsedSegmentTokens { tokens: seg_tokens, .. } => Rc::new(PathTemplateParseResult::ParsedPathTemplate {
    template: Rc::new(PathTemplate {
    tokens: v2_rt::concat(path.tokens.clone(), seg_tokens.clone()),
}),
}),
},
});
...
if (((param_name.clone().as_str() == "".to_string().as_str()) || v2_rt::contains(suffix.clone(), "{".to_string())) || v2_rt::contains(suffix.clone(), "}".to_string())) {
    ...
    reason: "invalid parameter segment structure".to_string(),
                ...
                Rc::new(PathSegmentTokensResult::ParsedSegmentTokens {
    tokens: v2_rt::concat(v2_rt::concat(prefix_tokens, param_tokens), suffix_tokens),
})
            }
}
    }
