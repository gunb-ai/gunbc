...
pub fn build_type_summary(item: Rc<Node>) -> Option<Rc<TypeSummary>> {
    ...
    Some(Rc::new(TypeSummary { name: item_name, repr: Rc::new(TypeRepr::EnumRepr { unit_only: unit_only.clone() }), field_summaries: build_enum_field_summaries(item.children.clone()) }))
}
}
...
pub fn add_emit_item_summary(state: Rc<EmitInfoBuildState>, item: Rc<Node>) -> Rc<EmitInfoBuildState> {
    ...
    let __rc_3 = __acc_0;
    let mut __map_ins_2 = Rc::try_unwrap(__rc_3).unwrap_or_else(|rc| (*rc).clone());
    __map_ins_2.insert(variant_name.clone(), Rc::new(TypeSummary { name: variant_name.clone(), repr: Rc::new(TypeRepr::StructRepr), field_summaries: build_struct_field_summaries(__elem_1.children.clone()) }));
    Rc::new(__map_ins_2)
}
} else {
    __acc_0.clone()
    ...
    __map_ins_25.insert(v2_rt::concat(v2_rt::concat(variant_name.clone(), "|".to_string()), __elem_24.name.clone()), resolved_name.clone());
    Rc::new(__map_ins_25)
}
} else {
    __acc_23.clone()
