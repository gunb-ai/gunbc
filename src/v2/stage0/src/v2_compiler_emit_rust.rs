...
pub use crate::v2_compiler_infer_types::{resolved_type, normalize_access_type_node, for_each_element_type_node, child_type_node, emit_map_has, node_is_keyed_collection, node_is_element_collection, node_is_collection};
...
items.iter().cloned().fold(Rc::new(OwnershipBuildResult {
    ...
    fold_eligible_index: v2_rt::rc_empty_map::<Rc<HashMap<String, bool>>>(),
            ...
            let proof = analyze_ownership(item.name.clone(), item.params.clone(), item.body.clone().clone().unwrap(), None);
...
let local_enum_uses = Rc::new({ let mut __result = Vec::new(); for item in Rc::new({ let mut __result = Vec::new(); for item in typed_module.items.clone().iter().cloned() { if (is_type_def_item(item.clone()) && (item.connective.clone() == Connective::Disj)) { __result.push(item); } } __result }).iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat("use ".to_string(), authored_name(scope.type_env.clone(), item.clone())), "::*;".to_string())); } __result });
...
let rename_attr = match Rc::new({ let mut __result = Vec::new(); for p in child.properties.clone().iter().cloned() { if (field_init_node_name_at(p.clone(), None).as_str() == "from_key".to_string().as_str()) { __result.push(p); } } __result }).first().cloned() {
...
match (*body.expr_data.clone()).clone() {
    ExprData::ExprLet => {
                let n = let_binding_name_at(body.clone(), None);
...
let guards = Rc::new({ let mut __result = Vec::new(); for fb in str_bindings.clone().iter().cloned() { __result.push(match (*field_binding_pattern(fb.clone())).clone() {
    MatchPattern::LitPattern { ref value, .. } => { let LiteralValue::LitStr { value: s, .. } = value.as_ref() else { unreachable!() }; v2_rt::concat(v2_rt::concat(emit_ident(field_binding_name_at(fb.clone(), None), RenderTarget::Rust), " == ".to_string()), emit_string_literal(s.clone(), "".to_string())) },
    _ => "".to_string(),
}); } __result });
...
if is_fielded2 {
...
} else {
                        {
                            let binding_strs = Rc::new({ let mut __result = Vec::new(); for fb in effective_bindings.clone().iter().cloned() { __result.push({
                                let fb_name = field_binding_name_at(fb.clone(), None);
let fb_pat = field_binding_pattern(fb.clone());
if is_string_lit_pattern(fb_pat.clone()) {
...
let ref_bound_fields = Rc::new({ let mut __result = Vec::new(); for fb in fbs.clone().iter().cloned() { __result.extend((*if analyze_rc_pattern(field_binding_pattern(fb.clone()), "".to_string(), shared_types.clone(), emit_info.clone()).matches_rc_variant.clone() {
                    Rc::new(vec![field_binding_name_at(fb.clone(), None)])
} else {
                    Rc::new(vec![])
}).iter().cloned()); } __result });
Rc::new(RcPatternAnalysis {
...
continue;
    ...
    None => { break "".to_string(); },
            ...
            let preludes = Rc::new({ let mut __result = Vec::new(); for fb in fbs.clone().iter().cloned() { __result.extend((*if field_needs_rc_ref(field_binding_name_at(fb.clone(), None), rc_analysis.clone()) {
                Rc::new(vec![v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("let ".to_string(), emit_pattern(field_binding_pattern(fb.clone()), shared_types.clone(), "".to_string(), source_index.clone(), emit_info.clone())), " = ".to_string()), emit_ident(field_binding_name_at(fb.clone(), None), RenderTarget::Rust)), ".as_ref() else { unreachable!() };".to_string())])
} else {
                Rc::new(vec![])
}).iter().cloned()); } __result });
...
match (*texpr.expr_data.clone()).clone() {
    ExprData::ExprVar { binding_kind, .. } => {
            let n = expr_var_name_at(texpr.clone(), None);
...
match summary {
    ...
    ExprData::ExprVar { .. } => {
                        let base_name = expr_var_name_at(base.clone(), None);
...
pub fn emit_rust_expr_var(expr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>, source_index: Option<Rc<NewlineIndex>>) -> String {
    ...
    ExprData::ExprVar { binding_kind, .. } => {
        let n = expr_var_name_at(expr.clone(), None);
...
pub fn emit_rust_expr_call(expr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    match (*expr.expr_data.clone()).clone() {
    ExprData::ExprCall { .. } => {
        let f = expr_call_func_at(expr.clone(), None);
...
let m = expr_method_name_at(expr.clone(), None);
...
pub fn emit_rust_expr_let(expr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    match (*expr.expr_data.clone()).clone() {
    ExprData::ExprLet => {
        let n = let_binding_name_at(expr.clone(), None);
...
let field_strs = match (*update_arg.expr_data.clone()).clone() {
    ExprData::ExprRecordLit { .. } => Rc::new({ let mut __result = Vec::new(); for f in update_arg.children.clone().iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat(emit_ident(field_init_node_name_at(f.clone(), None), RenderTarget::Rust), ": ".to_string()), emit_typed_expr(field_init_node_value(f.clone()), registry.clone(), scope.clone(), depth.clone(), shared_types.clone(), emit_info.clone(), 1024))); } __result }),
...
match call_args.last().cloned() {
    ...
    ExprData::ExprLambda { semantics, .. } => {
                    let lps = lambda_param_names_at(arg_value(a.clone()), None);
...
let acc_param_name = match (*fold_lambda_node.expr_data.clone()).clone() {
    ExprData::ExprLambda { .. } => match lambda_param_names_at(fold_lambda_node.clone(), None).first().cloned() {
    Some(n) => n.clone(),
    None => "".to_string(),
},
    _ => "".to_string(),
...
let fold_emit_info = if acc_unwrap {
    ...
    ExprData::ExprLambda { .. } => {
                let ps = lambda_param_names_at(fold_lambda_node.clone(), None);
...
let init_str = match args.clone().first().cloned() {
    ...
    ExprData::ExprCall { .. } => {
            let init_func = expr_call_func_at(arg_value(init_arg.clone()), None);
...
let ps = lambda_param_names_at(arg_value(a.clone()), None);
...
pub fn emit_rust_first_method_call(receiver: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    ...
    ExprData::ExprMethodCall { .. } => if (expr_method_name_at(receiver.clone(), None).as_str() == "skip".to_string().as_str()) {
        {
...
let body_str = match (*arm_b.expr_data.clone()).clone() {
    ExprData::ExprVar { binding_kind: body_binding_kind, .. } => {
            let body_name = expr_var_name_at(arm_b.clone(), None);
...
if (is_product && (resolved_type.ident_span.clone() == None)) {
    ...
    None => "compile_error!(\"empty anonymous record literal\")".to_string(),
                    ...
                    {
                        let lit_field_names = Rc::new({ let mut __result = Vec::new(); for f in fields.clone().iter().cloned() { __result.push(field_init_node_name_at(f.clone(), None)); } __result });
match find_struct_name_by_fields(lit_field_names, emit_info.type_summaries.clone()) {
    Some(resolved_sn) => {
                            let field_strs = Rc::new({ let mut __result = Vec::new(); for f in fields.clone().iter().cloned() { __result.push({
                                let fname = field_init_node_name_at(f.clone(), None);
...
empty_raw
                    ...
                    {
                        let field_strs = Rc::new({ let mut __result = Vec::new(); for f in fields.clone().iter().cloned() { __result.push({
                            let f_name = field_init_node_name_at(f.clone(), None);
...
pub fn emit_typed_cast(expr: Rc<Node>, target: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    {
        let expr_str = emit_typed_expr(expr.clone(), registry, scope.clone(), depth, shared_types.clone(), emit_info, 1024);
let ty_str = render_rust_type(target.clone(), shared_types.clone(), scope.type_env.clone().source_index.clone());
if is_primitive_numeric_node(target.clone()) {
    ...
    Some(InferredNode::Resolved { node: n, .. }) => render_rust_type(n.clone(), shared_types.clone(), scope.type_env.clone().source_index.clone()),
    _ => "".to_string(),
};
...
pub fn emit_tco_init_stmt(stmt: Rc<Node>, params: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    ...
    ExprData::ExprLet => {
        let n = let_binding_name_at(stmt.clone(), None);
let v = let_value(stmt.clone());
let val_str = emit_typed_expr(v, registry, scope, depth, shared_types, emit_info, 1024);
let is_param = { let mut __found = false; for p in params.iter().cloned() { if (param_node_name_at(p.clone(), None).as_str() == n.clone().as_str()) { __found = true; break; } } __found };
if is_param {
...
pub fn emit_rust_tco_let(frame: Rc<TcoFrame>, fn_name: String, params: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    match (*frame.expr.clone().expr_data.clone()).clone() {
    ExprData::ExprLet => {
        let n = let_binding_name_at(frame.expr.clone(), None);
...
let is_param = { let mut __found = false; for p in params.clone().iter().cloned() { if (param_node_name_at(p.clone(), None).as_str() == n.clone().as_str()) { __found = true; break; } } __found };
...
pub fn expr_references_var(node: Rc<Node>, var_name: String) -> bool {
    ...
    ExprData::ExprVar { .. } => (expr_var_name_at(node.clone(), None).as_str() == var_name.clone().as_str()),
...
let mock_props = Rc::new({ let mut __result = Vec::new(); for p in op_node.properties.clone().iter().cloned() { if has_mock_prefix(field_init_node_name_at(p.clone(), None)) { __result.push(p); } } __result });
...
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("            __m.insert(\"".to_string(), field_init_node_name_at(f.clone(), None)), "\".to_string(), ".to_string()), val_str.clone()), ");".to_string())
}); } __result });
...
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("let ".to_string(), field_init_node_name_at(mock_prop.clone(), None)), " = ".to_string()), body_str), ";".to_string())
}
}
