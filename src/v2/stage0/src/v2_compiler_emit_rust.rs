...
pub use crate::v2_compiler_emit::{
    ...
    extract_modifier_names, extract_test_projections, has_mock_prefix, has_nested_records_node,
    has_service_items, is_data_def_item, is_function_item, is_null_coalesce, is_resource_def_item,
    is_self_recursive, is_service_def_item, is_service_item, is_tco_eligible,
    is_tco_identity_passthrough, is_type_alias_item, is_type_alias_return_node, is_type_decl_item,
    is_type_def_item, language_spec, lookup_item, module_emit_scope, order_typed_call_args,
    render_node_type, rust_literal_for_pattern, scope_after_expr, seed_bindings,
    service_fallback_transport, service_field_ctors, service_field_decls, tco_reassign_core,
    test_function_name, to_pascal, to_string, to_string_helper, typed_named_arg_matches,
    unique_strings, wrap_shared_type,
};
pub use crate::v2_compiler_emit::{
...
pub use crate::v2_compiler_infer_emit_info::{EmitGraphInfo, TypeRepr, TypeSummary};
pub use crate::v2_compiler_infer_env::{authored_name, lookup_type_by_name};
...
pub use crate::v2_compiler_ownership::OwnershipProof;
            ...
            {
                {
                    ...
                    let base = if (def_mod.clone().as_str() != local_mod.as_str()) {
                        v2_rt::concat(
                            v2_rt::concat(
                                v2_rt::concat("crate::".to_string(), def_mod.clone()),
                                "::".to_string(),
                            ),
                            name.clone(),
                        )
...
pub fn build_module_export_sets(
    ...
    modules.iter().cloned().fold(
        ...
        |acc: Rc<HashMap<String, Rc<HashMap<String, bool>>>>, tm: Rc<TypedModule>| {
            ...
            v2_rt::rc_map_insert(acc, m_name.clone(), exported_set.clone())
        },
    )
}
        ...
        let by_filename = modules.clone().iter().cloned().fold(
            ...
            |acc: Rc<HashMap<String, Rc<Vec<Rc<TypedModule>>>>>, tm: Rc<TypedModule>| {
                ...
                match v2_rt::map_get(&acc, fname.clone()) {
                    Some(prior) => v2_rt::rc_map_insert(
                        acc.clone(),
                        fname.clone(),
                        v2_rt::rc_list_push(prior.clone(), tm.clone()),
                    ),
                    None => {
...
pub fn name_in_transitive_export_surface(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        if {
            ...
            {
                let in_local = match v2_rt::map_get(&export_sets, module_name.clone()) {
                            ...
                            None => {
                                ...
                                .cloned()
                                {
                                    Some(src) => {
                                        ...
                                        let __tco_1 =
                                            v2_rt::concat(visited, Rc::new(vec![import_module]));
...
pub fn import_module_enum_scope(
...
) -> Rc<Vec<String>> {
    {
        ...
        unique_strings(v2_rt::concat(physical, reexport_parents))
    }
}
                    ...
                    .cloned()
                    {
                        __result.extend(
                            (*{
                                ...
                                let direct = match v2_rt::map_get(&export_sets, src_mod.clone()) {
...
pub fn wildcard_reexport_surface_names(
...
) -> Rc<Vec<String>> {
    {
        ...
        unique_strings(Rc::new({
            let mut __result = Vec::new();
            for n in v2_rt::concat(local_candidates, wildcard_pool_candidates)
                .iter()
                .cloned()
            {
                if match reexport_source_module_name(
        ...
        if ((deduped_names.clone().len() as i64) == 0) {
        ...
        } else {
            {
                ...
                let merged_direct_names =
                    unique_strings(v2_rt::concat(direct_names.clone(), local_parent_names));
...
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(rust_visibility_prefix(), "use crate::".to_string()), def_mod.clone()), "::{".to_string()), names_str.clone()), "};".to_string())
}
                    } else {
                        "".to_string()
                    }
