...
pub fn emit_tco_param(
...
) -> String {
    {
        ...
        let ty = emit_rust_param_type(n, shared_types, source_indices.clone(), Rc::new(vec![]));
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    "mut ".to_string(),
...
pub fn emit_param(
...
) -> String {
    {
        ...
        let is_borrowable = ((v2_rt::set_contains(&read_only_params, pname.clone())
            ...
            && needs_reference_node(n.clone(), source_indices.clone()));
        ...
        let param_args = Rc::new({
            ...
            for p in wf.params.clone().iter().cloned() {
                __result.push({
                    ...
                    let n = param_node_type_expr(p.clone());
                    let is_borrowable =
                        ((v2_rt::set_contains(&wf.read_only_params.clone(), pname.clone())
                            && ((n.params.clone().len() as i64) == 0))
                            && needs_reference_node(n.clone(), wf.source_indices.clone()));
                    if is_borrowable.clone() {
