...
pub use crate::v2_compiler_infer_types::{normalize_access_type_node, for_each_element_type_node, decl_resolved_type, emit_guarded_type, child_type_node, emit_map_has, node_is_keyed_collection, node_is_element_collection, node_is_collection};
...
if is_type_def_item(item.clone()) {
            ...
            if is_type_alias_item(item.clone()) {
                v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(rust_visibility_prefix(), "type ".to_string()), item_text.clone()), " = ".to_string()), render_rust_type(decl_resolved_type(item.clone()), shared_types)), ";".to_string())
} else {
...
if ((item.uses.clone().len() as i64) > 0) {
                                emit_func_def(item_text.clone(), item.params.clone(), decl_resolved_type(item.clone()), item.uses.clone(), item.body.clone().clone().unwrap(), registry.clone(), scope.clone(), shared_types, fn_emit_info)
} else {
                                emit_fn_def(item_text.clone(), item.params.clone(), decl_resolved_type(item.clone()), item.body.clone().clone().unwrap(), registry.clone(), scope.clone(), shared_types, fn_emit_info)
}
}
} else {
...
pub fn emit_struct_field_from_child(child: Rc<Node>, recursive_types: Rc<HashMap<String, bool>>, shared_types: Rc<HashMap<String, bool>>, env: Rc<TypeEnv>) -> String {
    {
        let rt_child = decl_resolved_type(child.clone());
...
let shared = Rc::new({ let mut __result = Vec::new(); for fname in all_shared.clone().iter().cloned() { if {
    ...
    Some(f) => render_rust_type(decl_resolved_type(f.clone()), shared_types.clone()),
    None => "".to_string(),
}); } __result });
...
if ((child.children.clone().len() as i64) == 0) {
...
} else {
            {
                let field_lines = Rc::new({ let mut __result = Vec::new(); for f in child.children.clone().iter().cloned() { __result.push({
                    let rt_f = decl_resolved_type(f.clone());
...
if base_is_anon_record {
            {
                let bt = emit_guarded_type(base.clone());
...
{
                        let __tco_0 = emit_guarded_type(normed.clone());
...
let raw = emit_typed_record_lit(tn.clone(), fs, parent_enum.clone(), emit_guarded_type(expr.clone()), registry, scope.clone(), depth, shared_types.clone(), emit_info);
...
let type_name = if (base_arg.inferred.clone() != None) {
                    emit_guarded_type(base_arg.clone()).name.clone()
} else {
...
let base_node = normalize_access_type_node(emit_guarded_type(base.clone()));
...
if ((is_under_resolved_map || is_under_resolved_list) || is_under_resolved_non_collection) {
    ...
    Some(init_arg) => if (arg_value(init_arg.clone()).inferred.clone() != None) {
            emit_guarded_type(arg_value(init_arg.clone()))
} else {
...
let match_result_type = match arms.clone().first().cloned() {
    Some(first_arm) => emit_guarded_type(arm_body(first_arm.clone())),
...
pub fn lookup_struct_field_type_name(struct_node: Rc<Node>, field_name: String, variant_name: Option<String>) -> Option<String> {
    ...
    Some(field_child) => if (field_child.inferred.clone() != None) {
            {
                let field_type = decl_resolved_type(field_child.clone());
...
let raw = emit_typed_record_lit(tn.clone(), inner_fields, corrected_parent.clone(), emit_guarded_type(field_value.clone()), registry, scope.clone(), depth, shared_types.clone(), emit_info.clone());
...
let ret_type = render_rust_type(decl_resolved_type(op_node.clone()), shared_types.clone());
let eff_transport = effective_operation_transport(op_node.clone(), transport);
let op_inferred = decl_resolved_type(op_node.clone());
...
let dry_run_body = emit_dry_run_branch_from_props(op_text.clone(), decl_resolved_type(op_node.clone()), mock_props, registry.clone());
...
if ((is_product && (n.name.clone().as_str() == "".to_string().as_str())) && ((n.children.clone().len() as i64) == 1)) {
    ...
    Some(field_node) => decl_resolved_type(field_node.clone()),
    None => n.clone(),
}
} else {
            n.clone()
...
let ret = render_rust_type(decl_resolved_type(cap_node.clone()), shared_types.clone());
...
Rc::new(WorkflowFunc {
    ...
    inferred: decl_resolved_type(item.clone()),
