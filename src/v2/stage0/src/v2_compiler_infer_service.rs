...
pub use crate::v2_std_core::{
    expr_call_func, expr_var_name, field_access_base, field_access_field, field_node_type_expr,
    leaf_node, method_receiver, no_span, node_has_structure, Cardinality, Connective, ExprData,
    InferredNode, Node,
};
        ...
        ExprData::ExprFieldAccess { .. } => {
            ...
            match (*b.expr_data.clone()).clone() {
                ExprData::ExprVar { .. } => {
                    ...
                    Some(v2_rt::concat(
                        v2_rt::concat(ns.clone(), ".".to_string()),
                        f.clone(),
                    ))
                }
                _ => None,
...
pub fn collect_typed_service_calls_into(texpr: Rc<Node>, acc: Rc<UniqueAccum>) -> Rc<UniqueAccum> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
                    ...
                    match extract_typed_service_name(r.clone()) {
                        Some(service_name) => {
                            if emit_map_has(acc.seen.clone(), service_name.clone()) {
                                ...
                                Rc::new(UniqueAccum {
                                    seen: v2_rt::map_insert(
                                        acc.seen.clone(),
                                        service_name.clone(),
                                        true,
                                    ),
                                    result: v2_rt::list_push(
                                        acc.result.clone(),
                                        service_name.clone(),
                                    ),
                                })
        ...
        let result = texpr
            ...
            .iter()
            .cloned()
            .fold(this_acc.clone(), |a: _, child: Rc<Node>| {
                collect_typed_service_calls_into(child.clone(), a.clone())
            });
        result.clone()
    })
}
...
pub fn collect_called_func_names_into(texpr: Rc<Node>, acc: Rc<UniqueAccum>) -> Rc<UniqueAccum> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        let result = texpr
            ...
            .cloned()
            .fold(this_acc.clone(), |a: _, child: Rc<Node>| {
                collect_called_func_names_into(child.clone(), a.clone())
            });
        result.clone()
    })
}
...
pub fn expand_transitive_services_once(
...
) -> HashMap<String, Rc<ItemInfo>> {
    {
        ...
        all_items
            ...
            .fold(
                registry.clone(),
                |reg: _, item: Rc<Node>| match v2_rt::map_get(&reg, item.name.clone()) {
                    Some(info) => {
                        ...
                        if is_not_func.clone() {
                            ...
                            if has_no_body.clone() {
                            ...
                            } else {
                                {
                                    ...
                                    let extra = {
                                        let mut __result = Vec::new();
                                        for callee_name in called.iter().cloned() {
                                            __result.extend(
                                                match v2_rt::map_get(&reg, callee_name.clone()) {
                                                    Some(callee_info) => {
...
pub fn total_service_count(registry: HashMap<String, Rc<ItemInfo>>) -> i64 {
    v2_rt::map_values(&registry)
        .iter()
        .cloned()
        .fold(0, |acc: _, info: Rc<ItemInfo>| {
            (acc.clone() + (info.service_names.clone().len() as i64))
        })
}
    ...
    {
        {
            let path = v2_rt::concat(
                v2_rt::concat(base_type.name.clone(), ".".to_string()),
                field.clone(),
            );
                        ...
                        if ((op.outputs.clone().len() as i64) == 0) {
                            ...
                            Some(Rc::new(ServiceMethodResult {
                                result_type: Rc::new(Node {
                                    ...
                                    children: {
                                        let mut __result = Vec::new();
                                        for f in op.outputs.iter().cloned() {
