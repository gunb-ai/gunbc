...
pub fn service_return_str(spec: Rc<LanguageSpec>, ret_type: String) -> String {
    match (*spec.service_return.clone()).clone() {
        ...
        ServiceReturnStrategy::ErrorTupleReturn { error_type: et, .. } => v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(v2_rt::concat(" (".to_string(), ret_type), ", ".to_string()),
                et.clone(),
            ),
            ")".to_string(),
        ),
...
pub fn python_spec() -> Rc<LanguageSpec> {
    Rc::new(LanguageSpec {
        ...
        bridge_method_prefix: "".to_string(),
        bridge_method_case: NamingCase::SnakeCase,
        bridge_method_overrides: v2_rt::rc_map_insert(
...
pub fn target_keyword(target: RenderTarget, key: String) -> String {
    match target {
        RenderTarget::Rust => match v2_rt::lookup(&rust_keywords(), key.clone()) {
            Holds { value: kw, .. } => kw.clone(),
            Violates { diagnostic: _, .. } => key.clone(),
        },
