...
use crate::v2_rt::{self, Hash};
use crate::v2_std_core::NewlineIndex;
...
pub fn closure_content_digest(sources: &[Rc<SourceFile>]) -> Hash {
    ...
    let mut acc = v2_rt::atom_identity_hash("resolved-graph-closure-v1".to_string());
    for (module, content) in pairs {
        acc = v2_rt::hash_combine(acc, v2_rt::atom_identity_hash(module));
        acc = v2_rt::hash_combine(acc, v2_rt::atom_identity_hash(content.to_string()));
    }
    acc
}
...
pub fn subject_digest_for_closure(sources: &[Rc<SourceFile>]) -> Hash {
    ...
    digest = v2_rt::hash_combine(
        digest,
        v2_rt::atom_identity_hash(RESOLVE_LOGIC_VERSION.to_string()),
    );
    v2_rt::hash_combine(
        digest,
...
fn payload_content_digest(payload_bytes: &[u8]) -> Hash {
    v2_rt::atom_identity_hash(String::from_utf8_lossy(payload_bytes).into_owned())
}
...
pub fn lookup(cache_root: &Path, subject_digest: &str) -> CacheLookupResult {
    if !v2_rt::is_hash_digest(subject_digest) {
        return CacheLookupResult::RejectedHit(CacheRejectReason::BackendKeyMalformed);
    }
