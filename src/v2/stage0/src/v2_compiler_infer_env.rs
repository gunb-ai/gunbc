...
pub fn merge_decl_registry_state(
    ...
    Rc::new(v2_rt::map_keys(&incoming)).iter().cloned().fold(
        state,
        |acc: Rc<DeclRegistryMergeState>, key: i64| match v2_rt::map_get(&incoming, key.clone()) {
            Some(binding) => match v2_rt::map_get(&acc.decl_registry.clone(), key.clone()) {
                Some(existing) => {
...
pub fn lookup_carrier_type_for(env: Rc<TypeEnv>, node: Rc<Node>) -> Option<Rc<Node>> {
    match node.binding_id.clone() {
        Some(binding_id) => {
            match v2_rt::map_get(&env.carrier_bindings.clone(), binding_id.value.clone()) {
                Some(binding) => Some(binding.resolved.clone()),
                None => None,
            }
        }
