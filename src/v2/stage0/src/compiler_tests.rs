...
mod compiler_tests {
    ...
    // ------------------------------------------------------------------
    // Variant-constructor name collisions (DB-5 / P3 fail-closed teeth)
    //
    // Today the resolver registers variant constructors in a flat name->binding
    // map keyed by the bare name (variant_locals_from_items, src/v2/04_items.dag),
    // last-write-wins with no collision check. When two coproduct types declare
    // the same constructor name, one silently clobbers the other (see
    // docs/v4-variant-name-resolution.md; 37 such names exist in src/v4 today).
    //
    // This is the *spec* for the fail-closed check that belongs in the resolver:
    // it runs the exact detection (a constructor name claimed by >=2 distinct
    // parent types is a collision) over REAL parsed Nodes, proving the teeth bite.
    // It deliberately does NOT wire the check into the live resolver path — firing
    // it tree-wide reddens every v4 compile (all 37) and is the post-R2 "fire in
    // its slot" step. The check below is the property the wired version must satisfy.

    /// Parse a source string through the real v2 parser, returning the module Node
    /// and the source_indices needed to read authored names back out.
    fn parse_module_with_indices(
        path: &str,
        content: &str,
    ) -> (
        std::rc::Rc<crate::v2_std_core::Node>,
        std::rc::Rc<HashMap<String, std::rc::Rc<crate::v2_std_core::NewlineIndex>>>,
    ) {
        let tokens = tokenize(&content.to_string(), path.to_string());
        let mut source_indices = HashMap::new();
        source_indices.insert(
        ...
        let module = parsed
            ...
            .module
            .clone()
            .unwrap_or_else(|| panic!("{} produced no module", path));
        (module, source_indices)
    }

    /// The fail-closed detection: map each constructor name to the set of distinct
    /// parent coproduct types that declare it; any name with >=2 parents is a
    /// collision. This mirrors what variant_locals_from_items walks, but instead of
    /// silently overwriting on a repeated key it records the conflict.
    fn constructor_collisions(
        module: std::rc::Rc<crate::v2_std_core::Node>,
        source_indices: std::rc::Rc<HashMap<String, std::rc::Rc<crate::v2_std_core::NewlineIndex>>>,
    ) -> std::collections::BTreeMap<String, std::collections::BTreeSet<String>> {
        use std::collections::{BTreeMap, BTreeSet};
        let mut owners: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
        for item in crate::v2_std_core::module_items(module).iter() {
            if item.connective != crate::v2_std_core::Connective::Disj {
                continue;
            }
            let parent = crate::v2_std_core::authored_name_at(source_indices.clone(), item.clone());
            for child in item.children.iter() {
                let ctor =
                    crate::v2_std_core::authored_name_at(source_indices.clone(), child.clone());
                owners.entry(ctor).or_default().insert(parent.clone());
            }
        }
        owners
            .into_iter()
    ...
    #[test]
    fn fail_closed_check_catches_constructor_collision() {
        // Mirrors the real DecimalDigit / NonZeroDecimalDigit collision in
        // src/v4/std/integer.dag: D1, D2 are claimed by both; D0 by only one.
        let src = "module test.dup\n\
                   type DecimalDigit = D0 | D1 | D2\n\
                   type NonZeroDecimalDigit = D1 | D2\n";
        let (module, indices) = parse_module_with_indices("test_dup.dag", src);
        let collisions = constructor_collisions(module, indices);

        // The teeth bite: D1 and D2 are caught, each owned by both types.
        assert!(
            collisions.contains_key("D1"),
            "D1 collision not caught; collisions = {collisions:?}"
        );
        assert!(
            collisions.contains_key("D2"),
            "D2 collision not caught; collisions = {collisions:?}"
        );
        assert_eq!(
            collisions.get("D1").map(|p| p.len()),
            Some(2),
            "D1 should be owned by both coproducts"
        );
        // No false positive: D0 has a single owner, so it is NOT a collision.
        assert!(
            !collisions.contains_key("D0"),
            "D0 is single-owner and must not be flagged"
        );
    }

    #[test]
    fn no_collision_when_constructor_names_are_distinct() {
        // Negative case: disjoint constructor names must produce zero collisions,
        // so the fail-closed check stays quiet on well-formed programs.
        let src = "module test.ok\n\
                   type Color = Red | Green | Blue\n\
                   type Shape = Circle | Square\n";
        let (module, indices) = parse_module_with_indices("test_ok.dag", src);
        let collisions = constructor_collisions(module, indices);
        assert!(
            collisions.is_empty(),
            "expected no collisions for distinct names, got {collisions:?}"
        );
    }
}
