...
mod compiler_tests {
    ...
    fn witness_rust_canonical_primitives() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Int".into()).and_then(|cp| cp.default_expr.clone()), Some("0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Float".into()).and_then(|cp| cp.default_expr.clone()), Some("0.0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Bool".into()).and_then(|cp| cp.default_expr.clone()), Some("false".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Unit".into()).and_then(|cp| cp.default_expr.clone()), Some("()".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "String".into()).and_then(|cp| cp.default_expr.clone()), Some("String::new()".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Bytes".into()).and_then(|cp| cp.default_expr.clone()), Some("Vec::new()".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Rust, "Json".into()).and_then(|cp| cp.default_expr.clone()), Some("serde_json::Value::Null".into()));
    }
    ...
    fn witness_python_canonical_primitives() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Int".into()).and_then(|cp| cp.default_expr.clone()), Some("0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Float".into()).and_then(|cp| cp.default_expr.clone()), Some("0.0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Bool".into()).and_then(|cp| cp.default_expr.clone()), Some("False".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Unit".into()).and_then(|cp| cp.default_expr.clone()), Some("None".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "String".into()).and_then(|cp| cp.default_expr.clone()), Some("\"\"".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Bytes".into()).and_then(|cp| cp.default_expr.clone()), Some("b\"\"".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Secret".into()).and_then(|cp| cp.default_expr.clone()), Some("\"\"".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Python, "Json".into()).and_then(|cp| cp.default_expr.clone()), Some("{}".into()));
    }
    ...
    fn witness_go_canonical_primitives() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Int".into()).and_then(|cp| cp.default_expr.clone()), Some("0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Float".into()).and_then(|cp| cp.default_expr.clone()), Some("0.0".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Bool".into()).and_then(|cp| cp.default_expr.clone()), Some("false".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Unit".into()).and_then(|cp| cp.default_expr.clone()), Some("struct{}{}".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "String".into()).and_then(|cp| cp.default_expr.clone()), Some("\"\"".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Bytes".into()).and_then(|cp| cp.default_expr.clone()), Some("nil".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Secret".into()).and_then(|cp| cp.default_expr.clone()), Some("\"\"".into()));
        assert_eq!(lookup_checkpoint(RenderTarget::Go, "Json".into()).and_then(|cp| cp.default_expr.clone()), Some("nil".into()));
    }
    ...
    fn witness_rust_container_identities() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_inhabitant(RenderTarget::Rust, "FreeMonoid".into()).and_then(|inh| inh.identity_expr.clone()), Some("Vec::new()".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Rust, "BooleanAlgebra".into()).and_then(|inh| inh.identity_expr.clone()), Some("BTreeSet::new()".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Rust, "PartialFunction".into()).and_then(|inh| inh.identity_expr.clone()), Some("HashMap::new()".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Rust, "OrderedRing".into()).and_then(|inh| inh.identity_expr.clone()), Some("0i64".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Rust, "ApproximateField".into()).and_then(|inh| inh.identity_expr.clone()), Some("0.0f64".into()));
    }
    ...
    fn witness_python_container_identities() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_inhabitant(RenderTarget::Python, "FreeMonoid".into()).and_then(|inh| inh.identity_expr.clone()), Some("[]".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Python, "BooleanAlgebra".into()).and_then(|inh| inh.identity_expr.clone()), Some("set()".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Python, "PartialFunction".into()).and_then(|inh| inh.identity_expr.clone()), Some("{}".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Python, "OrderedRing".into()).and_then(|inh| inh.identity_expr.clone()), Some("0".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Python, "ApproximateField".into()).and_then(|inh| inh.identity_expr.clone()), Some("0.0".into()));
    }
    ...
    fn witness_go_container_identities() {
        use crate::v2_compiler_coercion::*;
        assert_eq!(lookup_inhabitant(RenderTarget::Go, "FreeMonoid".into()).and_then(|inh| inh.identity_expr.clone()), Some("nil".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Go, "BooleanAlgebra".into()).and_then(|inh| inh.identity_expr.clone()), Some("nil".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Go, "PartialFunction".into()).and_then(|inh| inh.identity_expr.clone()), Some("nil".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Go, "OrderedRing".into()).and_then(|inh| inh.identity_expr.clone()), Some("0".into()));
        assert_eq!(lookup_inhabitant(RenderTarget::Go, "ApproximateField".into()).and_then(|inh| inh.identity_expr.clone()), Some("0.0".into()));
    }
