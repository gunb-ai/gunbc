...
/// Backend-facing emit symbols routed through `v2_compiler_emit_core_support`.
const EMIT_CORE_SUPPORT_SYMBOLS: &[&str] = &[
    "EmitResult",
    "TestProjection",
    "apply_named_template",
...
fn run() -> Result<(), String> {
    ...
    time_phase(&mut phases, "patch_bootstrap_emit_core_support", || {
        patch_bootstrap_emit_core_support(&fresh_dir.join("src"))
    })?;
...
fn patch_bootstrap_emit_core_support(src_dir: &Path) -> Result<(), String> {
    let lib_path = src_dir.join("lib.rs");
    let mut lib_text =
        fs::read_to_string(&lib_path).map_err(|e| format!("read {}: {e}", lib_path.display()))?;
    if !lib_text.contains("pub mod v2_compiler_emit_core_support;") {
        lib_text = lib_text.replace(
    ...
    fs::write(
        src_dir.join("v2_compiler_emit_core_support.rs"),
        render_emit_core_support(),
    )
    .map_err(|e| format!("write emit core support: {e}"))?;
    ...
    for file_name in [
        ...
        let text =
            fs::read_to_string(&path).map_err(|e| format!("read {}: {e}", path.display()))?;
        let patched = patch_emit_backend_imports_text(&text)?;
        fs::write(&path, patched).map_err(|e| format!("write {}: {e}", path.display()))?;
    }
    Ok(())
}

fn render_emit_core_support() -> String {
    format!(
        "{}\n{}\n\n{}",
        "// Generated by v2 compiler -- do not edit.",
        "// Source module: v2.compiler.emit (backend-facing support surface).",
        render_pub_use("v2_compiler_emit", EMIT_CORE_SUPPORT_SYMBOLS)
    )
}

fn patch_emit_backend_imports_text(text: &str) -> Result<String, String> {
    if text.contains("pub use crate::v2_compiler_emit_core_support") {
        return Err(
            "patch_emit_backend_imports_text: backend already contains emit core support delegation"
                .to_string(),
        );
    }

    let support_symbols: BTreeSet<&str> = EMIT_CORE_SUPPORT_SYMBOLS.iter().copied().collect();
    let marker = "pub use crate::v2_compiler_emit::{";
    let mut patched = String::new();
    let mut rest = text;
    let mut inserted_support = false;
    ...
    while let Some(start) = rest.find(marker) {
        ...
        let end = after_marker.find("};").ok_or_else(|| {
            "patch_emit_backend_imports_text: missing v2_compiler_emit use terminator".to_string()
        })?;
        let block = &after_marker[..end];
        let symbols: Vec<String> = block
            .split(',')
            .map(str::trim)
        ...
        for symbol in symbols {
            if support_symbols.contains(symbol.as_str()) {
                delegated_count += 1;
            } else {
                retained.push(symbol);
            }
        }
        if delegated_count > 0 && !inserted_support {
            patched.push_str(&render_pub_use(
                "v2_compiler_emit_core_support",
                EMIT_CORE_SUPPORT_SYMBOLS,
            ));
            inserted_support = true;
        }
        if !retained.is_empty() {
            let retained_refs: Vec<&str> = retained.iter().map(String::as_str).collect();
            patched.push_str(&render_pub_use("v2_compiler_emit", &retained_refs));
        }
    ...
    if delegated_count == 0 {
        return Err(
            "patch_emit_backend_imports_text: no emit core support symbols found".to_string(),
        );
    }
    Ok(patched)
}

fn render_pub_use(module: &str, symbols: &[&str]) -> String {
    format!(
...
"#;
    ...
    #[test]
    fn patch_emit_backend_imports_routes_selected_symbols_through_support() {
        let emitted = r#"
pub use crate::v2_compiler_emit::{
    apply_named_template, emit_error_expr, unique_strings,
};
pub use crate::v2_compiler_emit::{
    EmitResult, InterpPart, TestProjection,
};
"#;

        let patched =
            patch_emit_backend_imports_text(emitted).expect("patch emitted backend imports");

        assert!(patched.contains("pub use crate::v2_compiler_emit_core_support"));
        assert!(patched.contains("apply_named_template"));
        assert!(patched.contains("unique_strings"));
        assert!(patched.contains("EmitResult"));
        assert!(patched.contains("TestProjection"));
        assert!(patched.contains("emit_error_expr"));
        assert!(patched.contains("InterpPart"));
        assert!(!patched.contains("pub use crate::v2_compiler_emit::{\n    apply_named_template"));
    }

    #[test]
    fn patch_emit_backend_imports_rejects_already_patched_input() {
        let emitted = r#"
...
"#;
        ...
        let err = patch_emit_backend_imports_text(emitted).expect_err("already patched input");
        assert!(err.contains("already contains emit core support delegation"));
    }
}
