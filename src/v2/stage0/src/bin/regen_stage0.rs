...
fn assert_output_set_matches_registry(committed_src: &Path, fresh_src: &Path) -> Result<(), String> {
    ...
    let unregistered_fresh: Vec<_> = fresh_files
        .iter()
        .filter(|file| !registered.contains(file.as_str()) && !hand_maintained.contains(file.as_str()))
        .cloned()
        .collect();
