...
pub use crate::v2_std_core::{
    ...
    module_imports, module_items, module_node, no_span, Connective, ErrorNode, NewlineIndex, Node,
};
...
pub fn get_exported_names(
...
) -> Rc<Vec<String>> {
    {
        ...
        v2_rt::concat(
            v2_rt::concat(v2_rt::concat(item_names, variant_names), imported_names),
...
pub fn adjacency_add_edge(
...
) -> Rc<HashMap<String, Rc<Vec<String>>>> {
    {
        ...
        v2_rt::rc_map_insert(
            adjacency.clone(),
            from_module.clone(),
            v2_rt::rc_list_push(existing, to_module),
        )
    }
}
...
pub fn topological_sort(
...
) -> Rc<TopoResult> {
    {
        ...
        let adjacency = v2_rt::concat(explicit_edges, implicit_std_types_edges)
            .iter()
            .cloned()
            .fold(
        ...
        let initial_queue = Rc::new({
            let mut __sorted: Vec<_> = Rc::new({
                let mut __result = Vec::new();
                for name in module_names.clone().iter().cloned() {
                    if match v2_rt::map_get(&in_degree_map, name.clone()) {
                        Some(0) => true,
                        _ => false,
                    } {
                        __result.push(name);
        ...
        if ((result.sorted.clone().len() as i64) == module_count.clone()) {
        ...
        } else {
            {
                ...
                let cycle_members = Rc::new({
                    let mut __result = Vec::new();
                    for name in module_names.clone().iter().cloned() {
                        if (v2_rt::map_has(&sorted_set, name.clone()) == false) {
                            __result.push(name);
                        }
                    }
                    __result
        ...
        let new_zero_set = Rc::new({
            ...
            for neighbor in Rc::new({
                ...
                for node in queue.clone().iter().cloned() {
                    __result.extend(
                        (*match v2_rt::map_get(&adjacency, node.clone()) {
                            Some(ns) => ns.clone(),
                            None => Rc::new(vec![]),
                        })
                        .iter()
            ...
            .cloned()
            {
                if match v2_rt::map_get(&batch_result.in_degree_map.clone(), neighbor.clone()) {
                    Some(0) => true,
                    _ => false,
                } {
