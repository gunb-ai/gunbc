...
let exported = get_exported_names(&m, source_index.clone());
...
let resolve_accum = modules.clone().iter().cloned().fold(Rc::new(ResolveAccum {
    ...
    diagnostics: Rc::new(vec![]),
            ...
            let source_index = v2_rt::map_get(&source_indices, m.span.clone().file.clone());
let result = resolve_module_imports(&m, module_index.clone(), export_sets.clone(), source_index.clone());
Rc::new(ResolveAccum {
...
let name_diags = if import_is_all(import.clone()) {
    ...
    name: authored_name_at(source_index.clone(), &child),
    module_path: import.name.clone(),
    importing_module: importing_module.clone(),
...
let variant_names = Rc::new({ let mut __result = Vec::new(); for item in module_items(module.clone()).iter().cloned() { __result.extend((*get_variant_names(&item, source_index.clone())).iter().cloned()); } __result });
...
if is_coproduct {
            Rc::new({ let mut __result = Vec::new(); for c in item.children.clone().iter().cloned() { __result.push(authored_name_at(source_index.clone(), &c)); } __result })
        } else {
            Rc::new(vec![])
        }
}
