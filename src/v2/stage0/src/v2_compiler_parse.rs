...
pub use crate::v2_std_core::{
    arg_name, arg_value, expr_call_func, expr_var_name, field_access_field, field_binding_name,
    field_binding_pattern, field_node_cardinality, field_node_default_value, field_node_from_key,
    field_node_name, field_node_type_expr, file_transport_node, import_node, leaf_node,
    ...
    make_resource_use_node, make_text_part_node, make_variant_node, module_node, no_span,
    node_is_coproduct, node_is_product, param_node_default_value, param_node_name,
    param_node_type_expr, rest_transport_node, rt_node, service_config_properties,
    shell_transport_node, variant_node_fields, variant_node_name, with_required_cardinality,
    BinOpKind, Cardinality, CompilerDiagnostic, Connective, ErrorNode, ExprData, ExprErrorKind,
    InferredNode, LiteralValue, MatchPattern, Node, NodeType, OperationModifier, SourceSpan,
        ...
        match sh.clone() {
            ...
            _ => {
                ...
                Rc::new(NameResult {
                    ...
                    state: state.clone(),
                    err: Some(parse_error(
                        v2_rt::concat("expected identifier, found ".to_string(), found.clone()),
                        current_span(tokens.clone(), state.clone()),
                    )),
                })
            }
                ...
                match kw_name.clone() {
                    ...
                    None => {
                        ...
                        Rc::new(NameResult {
                            ...
                            state: state.clone(),
                            err: Some(parse_error(
                                v2_rt::concat("expected name, found ".to_string(), found.clone()),
                                current_span(tokens.clone(), state.clone()),
                            )),
                        })
                    }
...
pub fn parser_passthrough_state_expr(expr: Rc<Node>) -> Option<Rc<Node>> {
    match (*expr.expr_data.clone()).clone() {
        ExprData::ExprCall { .. } => match parser_helper_identity(expr_call_func(expr.clone())) {
            ...
            Some(ParserHelperIdentity::ParserHelperWith) => match expr.children.first().cloned() {
...
pub fn node_inferred_to_outputs(rt: Rc<Node>) -> Vec<Rc<Node>> {
    if is_conj_with_children(rt.clone()) {
        let typed_children = rt
            .children
            .iter()
            .all(|ch| matches!((*rt_node(ch.clone())).clone(), NodeType::Typed { .. }));
        if !typed_children {
            return vec![];
        }
        {
...
pub fn parse(tokens: Vec<Rc<Token>>) -> Rc<ParseResult> {
    {
        ...
        let state = Rc::new(ParserState { pos: 0 });
...
pub fn variant_to_child_node(variant: Rc<Node>) -> Rc<Node> {
    {
        ...
        let children = {
            ...
            for f in fields.clone().iter().cloned() {
                __result.push(field_to_child_node(f.clone()));
            }
            __result
        };
        Rc::new(Node {
        ...
        if e.consumed.clone() {
            {
                let r = parse_field_list(
                    tokens.clone(),
                    skip_newlines(tokens.clone(), e.state.clone()),
                );
        ...
        if is_ident(tokens.clone(), state.clone()) {
            {
                ...
                if (peek_is_lbrace(tokens.clone(), s.clone())
                    || peek_is_pipe(tokens.clone(), s.clone()))
                {
                    {
                        ...
                        let type_children = {
                            ...
                            for v in variants.clone().iter().cloned() {
                                __result.push(variant_to_child_node(v.clone()));
                            }
                            __result
                        };
                        let item = Rc::new(Node {
            ...
            match pred_name.clone().as_str() {
                    ...
                    Rc::new(PredResult {
                        predicate: make_field_init_node(
                            ...
                            make_expr_node(
                                ...
                                v2_rt::concat(min_fields.clone(), max_fields.clone()),
                                None,
                                zero_span.clone(),
                            ),
                            no_span(),
                ...
                _ => Rc::new(PredResult {
                    ...
                    err: Some(parse_error(
                        ...
                        current_span(tokens.clone(), e.state.clone()),
                    )),
                }),
            }
        } else {
...
pub fn parse_type_expr(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<TypeResult> {
    {
        ...
        match sh.clone() {
            Some(TokenShape::ShLBrace) => {
                ...
                let r = parse_field_list(
                    tokens.clone(),
                    skip_newlines(tokens.clone(), adv.state.clone()),
                );
                if has_err(r.err.clone()) {
                    return Rc::new(TypeResult {
        ...
        if e.consumed.clone() {
            {
                ...
                let s = r.state.clone();
                let type_args =
                    collect_type_args(tokens.clone(), s.clone(), vec![first_arg.clone()]);
...
pub fn parse_optional_type_params(
...
) -> Rc<TypeParamsResult> {
    {
        ...
        if e.consumed.clone() {
            {
                let params_result =
                    collect_type_param_names(tokens.clone(), e.state.clone(), vec![]);
                let r = expect(
                    tokens.clone(),
                    params_result.state.clone(),
                    ExpectedToken::ExpectGt,
                );
...
pub fn parse_field_list_acc(
    ...
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
        if ((peek_is_rbrace(tokens.clone(), s.clone())
            || peek_is_rparen(tokens.clone(), s.clone()))
            || at_end(tokens.clone(), s.clone()))
        {
...
pub fn parse_func_def(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ItemResult> {
    {
        ...
        let r = match sh.clone() {
            ...
            Some(TokenShape::ShKwPattern) => expect(
                tokens.clone(),
                state.clone(),
                ExpectedToken::ExpectKwPattern,
            ),
            Some(TokenShape::ShKwInterface) => expect(
                tokens.clone(),
                state.clone(),
...
pub fn parse_uses_entry(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ResUseResult> {
    {
        ...
        if e_lp.consumed.clone() {
            ...
            let res_node = Rc::new(Node {
                name: r3.type_expr.name.clone(),
                span: r3.type_expr.span.clone(),
                ident_span: r3.type_expr.ident_span.clone(),
                children: r3.type_expr.children.clone(),
                params: r3.type_expr.params.clone(),
                inferred: r3.type_expr.inferred.clone(),
                return_cardinality: r3.type_expr.return_cardinality.clone(),
                uses: r3.type_expr.uses.clone(),
                body: r3.type_expr.body.clone(),
                connective: r3.type_expr.connective.clone(),
                transport: r3.type_expr.transport.clone(),
                ...
                match_pattern: r3.type_expr.match_pattern.clone(),
                expr_data: r3.type_expr.expr_data.clone(),
            });
...
pub fn parse_resource_config_acc(
    ...
    if ec.consumed.clone() {
        parse_resource_config_acc(
            tokens.clone(),
            skip_newlines(tokens.clone(), ec.state.clone()),
            acc,
        )
    } else {
        Rc::new(ResConfigResult {
            fields: acc.clone(),
            state: r3.state.clone(),
            err: None,
        })
...
pub fn parse_service_body(
    ...
    parse_service_entries(
        ...
        state.clone(),
        None,
        local_transport_node(current_span(tokens.clone(), state.clone())),
        vec![],
    )
}
                ...
                Some(TokenShape::ShIdent) => {
                    ...
                    if (id.clone() == "config".to_string()) {
                        ...
                        let r2 = parse_service_config_block(
                            tokens.clone(),
                            skip_newlines(tokens.clone(), r.state.clone()),
                        );
                        if has_err(r2.err.clone()) {
                            return Rc::new(ServiceBodyResult {
                                config: config.clone(),
                ...
                if (tok_text.clone() == "rest".to_string()) {
                    {
                        ...
                        let r2 = parse_rest_binding_body(
                            tokens.clone(),
                            skip_newlines(tokens.clone(), r.state.clone()),
                        );
                        if has_err(r2.err.clone()) {
                        ...
                        let r3 = expect(
                            tokens.clone(),
                            skip_newlines(tokens.clone(), r2.state.clone()),
...
pub fn parse_op_body_entries(
    mut tokens: Rc<Vec<Rc<Token>>>,
    mut state: Rc<ParserState>,
    mut inputs: Vec<Rc<Node>>,
    mut outputs: Vec<Rc<Node>>,
    mut modifier_props: Vec<Rc<Node>>,
    mut transport: Option<Rc<Node>>,
    mut exit_props: Vec<Rc<Node>>,
    mut response_props: Vec<Rc<Node>>,
    mut mock_props: Vec<Rc<Node>>,
) -> Rc<OpBodyResult> {
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
                                ...
                                if (id.clone() == "mock_response".to_string()) {
                                    ...
                                    if peek_is_colon_after_ident(tokens.clone(), s.clone()) {
                                        ...
                                        }
                                        {
                                            let __tco_0 = tokens.clone();
                                            let __tco_1 =
                                                skip_newlines(tokens.clone(), r3.state.clone());
                                            let __tco_2 = inputs.clone();
                                            let __tco_3 = outputs.clone();
                                            let __tco_4 = modifier_props.clone();
                                            let __tco_5 = transport.clone();
...
pub fn modifiers_to_props(
    modifiers: Vec<OperationModifier>,
    span: Rc<SourceSpan>,
) -> Vec<Rc<Node>> {
    {
        let mut __result = Vec::new();
                ...
                if ((state.pos.clone() + 1) < (tokens.clone().len() as i64)) {
                    {
                        let next_tok = tokens
                            .clone()
                            .get((state.pos.clone() + 1) as usize)
                            .cloned();
                        match next_tok.clone() {
                            Some(t) => {
                                if (is_ident_shape(t.shape.clone())
                                    && (t.text.clone() == "xx".to_string()))
                                {
                                    {
                                        let span = current_span(tokens.clone(), state.clone());
                                        let adv = advance(tokens.clone(), state.clone());
        ...
        if is_mock.clone() {
            {
                ...
                let r2 = parse_mock_response_entries(
                    tokens.clone(),
                    skip_newlines(tokens.clone(), r.state.clone()),
                );
                if has_err(r2.err.clone()) {
...
pub fn parse_resource_entries(
    mut tokens: Rc<Vec<Rc<Token>>>,
    mut state: Rc<ParserState>,
    mut properties: Vec<Rc<Node>>,
    mut capabilities: Vec<Rc<Node>>,
) -> Rc<ResPropResult> {
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
        if (peek_is_rbrace(tokens.clone(), s.clone()) || at_end(tokens.clone(), s.clone())) {
            ...
            match sh.clone() {
                ...
                Some(TokenShape::ShIdent) => {
                    if peek_is_colon_after_ident(tokens.clone(), s.clone()) {
                        ...
                        let fname = r.name.clone();
                        let r2 =
                            expect(tokens.clone(), r.state.clone(), ExpectedToken::ExpectColon);
                        if has_err(r2.err.clone()) {
                            return Rc::new(ResPropResult {
                                properties: properties.clone(),
                                capabilities: capabilities.clone(),
        ...
        if peek_is_lbrace(tokens.clone(), s.clone()) {
            {
                ...
                let io = parse_input_output_blocks(
                    tokens.clone(),
                    skip_newlines(tokens.clone(), r2.state.clone()),
                );
                ...
                let r3 = expect(
                    tokens.clone(),
                    skip_newlines(tokens.clone(), io.state.clone()),
                    ExpectedToken::ExpectRBrace,
                );
                if has_err(r3.err.clone()) {
                    return Rc::new(CapResult {
...
pub fn parse_extern_decl(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ItemResult> {
    {
        ...
        match sh.clone() {
            ...
            _ => Rc::new(ItemResult {
                ...
                state: r.state.clone(),
                err: Some(parse_error(
                    "expected fn or func after extern".to_string(),
                    current_span(tokens.clone(), r.state.clone()),
                )),
            }),
        }
    }
...
pub fn parse_stmts_acc(
    ...
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
        if ((peek_is_rbrace(tokens.clone(), s.clone()) || at_end(tokens.clone(), s.clone()))
            || peek_is_eof(tokens.clone(), s.clone()))
        {
...
pub fn parse_stmt(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ExprResult> {
    {
        ...
        match sh.clone() {
            ...
            Some(TokenShape::ShIdent) => {
                if peek_text_is(tokens.clone(), state.clone(), "node".to_string())
                    && peek_is_node_decl(tokens.clone(), state.clone())
                {
            ...
            _ => {
                if let Some(ref shape) = sh {
                    if is_name_keyword_shape(shape.clone())
                        && peek_is_eq_after_ident(tokens.clone(), state.clone())
                    {
                        parse_bare_assignment(tokens.clone(), state.clone())
                    } else {
                        parse_expr(tokens.clone(), state.clone())
...
pub fn is_constraint_bracket(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> bool {
    if ((state.pos.clone() + 1) < (tokens.clone().len() as i64)) {
        ...
        match next_tok {
            Some(t) => {
                if is_ident_shape(t.shape.clone()) {
                    t.text.clone() == "after" || t.text.clone() == "when"
                } else {
                    false
                }
            }
...
pub fn parse_constraint_list(
    ...
    if ec.consumed.clone() {
        parse_constraint_list(tokens.clone(), ec.state.clone(), acc)
    } else {
        Rc::new(ConstraintsResult {
            constraints: acc.clone(),
            state: r.state.clone(),
...
pub fn is_constraint_bracket_after_ident(
    tokens: Rc<Vec<Rc<Token>>>,
    state: Rc<ParserState>,
) -> bool {
...
pub fn peek_is_node_decl(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> bool {
    if ((state.pos.clone() + 2) < (tokens.clone().len() as i64)) {
        ...
        let t2 = tokens
            .clone()
            .get((state.pos.clone() + 2) as usize)
            .cloned();
        let name_ok = match t1 {
            Some(t) => is_ident_shape(t.shape.clone()) || is_name_keyword_shape(t.shape.clone()),
            None => false,
        };
...
pub fn peek_text_is(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>, expected: String) -> bool {
    match peek(tokens.clone(), state.clone()) {
                                ...
                                if is_dot_shape(op_shape.clone()) {
                                    ...
                                    if is_pipe_arrow_shape(op_shape.clone()) {
                                        ...
                                        let new_lhs = make_expr_node(
                                            Rc::new(ExprData::ExprBinOp { op: binop.clone() }),
...
pub fn infix_bp(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Option<Rc<BindingPower>> {
    {
        let sh = peek_shape(tokens.clone(), state.clone());
        match sh.clone() {
            Some(TokenShape::ShNullCoalesce) => Some(Rc::new(BindingPower { left: 3, right: 4 })),
            Some(TokenShape::ShOr) => Some(Rc::new(BindingPower { left: 5, right: 6 })),
            Some(TokenShape::ShAnd) => Some(Rc::new(BindingPower { left: 7, right: 8 })),
            Some(TokenShape::ShEqEq) => Some(Rc::new(BindingPower { left: 9, right: 10 })),
            Some(TokenShape::ShNe) => Some(Rc::new(BindingPower { left: 9, right: 10 })),
        ...
        if peek_is_lparen(tokens.clone(), s.clone()) {
            {
                ...
                Rc::new(ExprResult {
                    expr: make_named_expr_node(
                        ...
                        v2_rt::concat(vec![receiver.clone()], r2.args.clone()),
                        None,
                        span.clone(),
                    ),
                    state: r2.state.clone(),
...
pub fn parse_lambda_stmts(
    ...
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
        if (((peek_is_rparen(tokens.clone(), s.clone())
            || peek_is_rbrace(tokens.clone(), s.clone()))
            || at_end(tokens.clone(), s.clone()))
            || peek_is_eof(tokens.clone(), s.clone()))
        {
            break Rc::new(StmtsResult {
        ...
        if peek_is_fat_arrow(tokens.clone(), s.clone()) {
            {
                ...
                Rc::new(ExprResult {
                    expr: make_expr_node(
                        Rc::new(ExprData::ExprLambda { semantics: None }),
                        v2_rt::concat(vec![r.expr.clone()], vec![leaf_node(name.clone())]),
                        None,
                        span.clone(),
                    ),
                    state: r.state.clone(),
...
pub fn parse_single_arg(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ArgResult> {
    {
        ...
        let is_name_token = (is_ident(tokens.clone(), state.clone())
            || is_keyword_name(tokens.clone(), state.clone()));
        if is_name_token.clone() {
            {
                ...
                if has_err(name_r.err.clone()) {
                    ...
                    if peek_is_colon(tokens.clone(), name_r.state.clone()) {
                        ...
                        {
                            let restored = Rc::new(ParserState {
                                pos: save_pos.clone(),
                                ..(*state.clone()).clone()
                            });
...
pub fn parse_match(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ExprResult> {
    {
        ...
        Rc::new(ExprResult {
            expr: make_expr_node(
                ...
                v2_rt::concat(vec![scrutinee.clone()], arms.clone()),
                None,
                span.clone(),
            ),
            state: r.state.clone(),
...
pub fn parse_expr_loop_no_brace(
    ...
    loop {
        if ((at_end(tokens.clone(), state.clone()) || peek_is_eof(tokens.clone(), state.clone()))
            || peek_is_lbrace(tokens.clone(), state.clone()))
        {
            break Rc::new(ExprResult {
                expr: lhs.clone(),
                state: state.clone(),
            ...
            if (peek_is_lparen(tokens.clone(), s.clone()) && (14 >= min_bp.clone())) {
                ...
                if (peek_is_lbracket(tokens.clone(), s.clone()) && (14 >= min_bp.clone())) {
                    let span = current_span(tokens.clone(), s.clone());
                    let r =
                        parse_index_or_slice(tokens.clone(), s.clone(), lhs.clone(), span.clone());
                    if has_err(r.err.clone()) {
                        return Rc::new(ExprResult {
                            expr: r.expr.clone(),
                            state: r.state.clone(),
    ...
    if ((state.pos.clone() + offset.clone()) < (tokens.clone().len() as i64)) {
        {
            let tok = tokens
                .clone()
                .get((state.pos.clone() + offset.clone()) as usize)
                .cloned();
            match tok.clone() {
...
pub fn scan_for_fat_arrow_after_braces(
    ...
    scan_braces_depth(
        tokens.clone(),
        state.clone(),
        (state.pos.clone() + start_offset.clone()),
        1,
    )
}

pub fn scan_braces_depth(
    mut tokens: Rc<Vec<Rc<Token>>>,
    mut state: Rc<ParserState>,
    mut idx: i64,
    mut depth: i64,
...
pub fn parse_field_init(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<FieldInitResult> {
    {
        let dummy_fi =
            make_field_init_node("".to_string(), parse_recovery_placeholder(), no_span());
        if (is_ident(tokens.clone(), state.clone())
            || is_keyword_name(tokens.clone(), state.clone()))
        {
            {
            ...
            if (peek_is_lit_str(tokens.clone(), state.clone())
                && peek_is_colon_after_ident(tokens.clone(), state.clone()))
            {
                {
...
pub fn parse_expr_list_until_acc(
    mut tokens: Rc<Vec<Rc<Token>>>,
    mut state: Rc<ParserState>,
    mut end_expected: ExpectedToken,
    mut acc: Vec<Rc<Node>>,
) -> Rc<ExprsResult> {
    loop {
        let s = skip_newlines(tokens.clone(), state.clone());
...
pub fn parse_fn_lambda(tokens: Rc<Vec<Rc<Token>>>, state: Rc<ParserState>) -> Rc<ExprResult> {
    {
        ...
        Rc::new(ExprResult {
            expr: make_expr_node(
                Rc::new(ExprData::ExprLambda { semantics: None }),
                v2_rt::concat(vec![body_r.expr.clone()], {
                    let mut __result = Vec::new();
        ...
        match interp_sh.clone() {
            Some(TokenShape::ShStrMid) => {
                ...
                let mid_parts = if (v2_rt::string_length(mid.clone()) > 0) {
                    v2_rt::list_push(
                        new_parts.clone(),
                        Rc::new(StringPart::Text { value: mid.clone() }),
                    )
                } else {
                    new_parts.clone()
                };
        ...
        if peek_is_rbrace(tokens.clone(), s.clone()) {
        ...
        } else {
            {
                ...
                match sh.clone() {
                    ...
                    _ => {
                        let ident_or_keyword = (is_ident(tokens.clone(), s.clone())
                            || is_keyword_name(tokens.clone(), s.clone()));
                        if (ident_or_keyword.clone()
                            && peek_is_colon_after_ident(tokens.clone(), s.clone()))
                        {
                            {
                                let r = parse_field_init_list(tokens.clone(), s.clone());
                                if has_err(r.err.clone()) {
                                ...
                                let s2 = skip_newlines(tokens.clone(), r.state.clone());
                                let r2 =
                                    expect(tokens.clone(), s2.clone(), ExpectedToken::ExpectRBrace);
                                if has_err(r2.err.clone()) {
                                    return Rc::new(ExprResult {
                                        expr: dummy_expr.clone(),
                                        state: r2.state.clone(),
                            ...
                            if (peek_is_lit_str(tokens.clone(), s.clone())
                                && peek_is_colon_after_ident(tokens.clone(), s.clone()))
                            {
                                {
                                    let r = parse_field_init_list(tokens.clone(), s.clone());
