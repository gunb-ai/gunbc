...
pub fn node_is_collection(n: Rc<Node>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> bool {
    ((((n.children.clone().len() as i64) > 0) && (n.connective.clone() == Connective::NoConnective)) && is_container_type(authored_name_at(source_indices, n.clone())))
}
...
if self_is_type_var {
                ...
                {
                    let under_param = match container_expected_arity(authored_name_at(source_indices.clone(), n.clone())) {
...
if under_param {
                        ...
                        { let mut __all = true; for ch in n.children.clone().iter().cloned() { if !(is_fully_resolved(child_type_node(ch.clone()), source_indices.clone())) { __all = false; break; } } __all }
                    }
}
            }
}
...
if __is_leaf {
    ...
    Some(InferredNode::Resolved { node: rt, .. }) => (((((rt.connective.clone() == Connective::NoConnective) && ((rt.children.clone().len() as i64) == 0)) && (rt.ident_span.clone() != None)) && (authored_name_at(source_indices.clone(), rt.clone()).as_str() != "None".to_string().as_str())) && (is_kernel_type(authored_name_at(source_indices.clone(), rt.clone())) == false)),
    _ => false,
};
if __is_named_ref {
...
if is_product {
                        ...
                        if is_coproduct {
                            ...
                            {
                                let __is_container = node_is_element_collection(n.clone(), source_indices.clone());
...
if __is_container {
    ...
    Some(el) => node_type_shape(child_type_node(el.clone()), source_indices.clone()),
    None => "?".to_string(),
};
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("Container(".to_string(), n_name), ",".to_string()), elem_shape), ")".to_string())
                                    ...
                                    if is_optional {
                                        {
                                            let inner_shape = node_type_shape(with_required_cardinality(n.clone()), source_indices.clone());
...
if (same_kind && left_is_unit_inner) {
            ...
            if (is_fully_resolved(right.clone(), source_indices.clone()) && !is_fully_resolved(left.clone(), source_indices.clone())) {
                right.clone()
            } else {
                left.clone()
            }
                ...
                if (left_tv.clone() || right_tv.clone()) {
                    ...
                    if (left_opt.clone() && right_is_unit_eq) {
                        ...
                        if (left_is_unit_eq && right_opt.clone()) {
                            ...
                            if (left_opt.clone() && right_opt.clone()) {
                                node_type_equals_core(with_required_cardinality(left.clone()), with_required_cardinality(right.clone()), source_indices)
                            } else {
...
if (left_leaf.clone() && right_leaf.clone()) {
    ...
    Some(right_child) => node_type_equals(pair.1.clone(), right_child.clone(), source_indices.clone()),
    None => false,
}) { __all = false; break; } } __all }
                        }
                    }
...
if (left_is_container && right_is_container) {
                                if (left.name.clone().as_str() != right.name.clone().as_str()) {
                                    ...
                                    match left.children.clone().first().cloned() {
    Some(left_ch) => match right.children.clone().first().cloned() {
    Some(right_ch) => node_type_equals(child_type_node(left_ch.clone()), child_type_node(right_ch.clone()), source_indices.clone()),
    None => false,
},
    None => false,
}
                                ...
                                {
                                    let both_maps = (node_is_keyed_collection(left.clone(), source_indices.clone()) && node_is_keyed_collection(right.clone(), source_indices.clone()));
if both_maps {
    ...
    Some(right_second) => (node_type_equals(child_type_node(left_first.clone()), child_type_node(right_first.clone()), source_indices.clone()) && node_type_equals(child_type_node(left_second.clone()), child_type_node(right_second.clone()), source_indices.clone())),
    None => false,
},
    None => false,
},
    ...
    Some(right_param) => node_type_equals(param_node_type_expr(pair.1.clone()), param_node_type_expr(right_param.clone()), source_indices.clone()),
    None => false,
}) { __all = false; break; } } __all };
if (params_eq == false) {
                                                        false
    ...
    Some(InferredNode::Resolved { node: right_ret, .. }) => node_type_equals(left_ret.clone(), right_ret.clone(), source_indices.clone()),
    None => false,
    _ => false,
},
...
if __is_named_ref {
                match n.inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: rt, .. }) => Rc::new(vec![authored_name_at(source_indices.clone(), rt.clone())]),
    _ => Rc::new(vec![]),
}
            } else {
                if has_structure {
    ...
    Some(InferredNode::Resolved { node: rt, .. }) => node_type_deps(rt.clone(), source_indices.clone()),
    ...
    _ => Rc::new(vec![]),
                        ...
                        if ((n.children.clone().len() as i64) > 0) {
                            {
                                let child_deps = Rc::new({ let mut __result = Vec::new(); for child in n.children.clone().iter().cloned() { __result.extend((*node_type_deps(child.clone(), source_indices.clone())).iter().cloned()); } __result });
