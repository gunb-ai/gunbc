...
pub use crate::v2_std_core::{
    ...
    make_expr_error_node, make_expr_node, make_param_node, make_span, no_span, none_type,
    param_node_type_expr, string_type, unit_type, with_optional_cardinality,
    with_required_cardinality, AlgebraFieldKind, BinOp, Cardinality, Connective, ExprData,
    ExprErrorKind, InferredNode, LiteralValue, NewlineIndex, Node,
};
...
pub fn node_is_collection(
    ...
    ((((n.children.clone().len() as i64) > 0)
        ...
        && is_container_type(authored_name_at(source_indices, &n)))
}
...
pub fn is_fully_resolved(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        if self_is_type_var {
            ...
            {
                let under_param =
                    match container_expected_arity(authored_name_at(source_indices.clone(), &n)) {
                ...
                if under_param {
                ...
                } else {
                    {
                        ...
                        for ch in n.children.clone().iter().cloned() {
                            if !(is_fully_resolved(&child_type_node(&ch), &source_indices)) {
                                __all = false;
                                break;
                            }
                        }
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        ...
        match (*template).clone() {
            ...
            AlgebraTypeTemplate::AlgebraTypeVariable { id: var_id, .. } => {
                type_variable_node(var_id.clone())
            }
            AlgebraTypeTemplate::ContainerOf {
                ...
                let kind_name = match (*src.clone()).clone() {
                    ContainerSource::SameAsReceiver => {
                        authored_name_at(source_indices.clone(), &base)
                    }
            ...
            AlgebraTypeTemplate::OptionalOf { inner, .. } => with_optional_cardinality(
                &instantiate_algebra_type(inner.clone(), &base, &source_indices),
            ),
            AlgebraTypeTemplate::TupleOf { first, second, .. } => make_tuple_type(
                instantiate_algebra_type(first.clone(), &base, &source_indices),
                instantiate_algebra_type(second.clone(), &base, &source_indices),
            ),
            AlgebraTypeTemplate::CallableOf {
                ...
                Rc::new({
                    ...
                    for tp in p.clone().iter().cloned() {
                        __result.push(instantiate_algebra_type(tp.clone(), &base, &source_indices));
                    }
                    __result
                }),
                instantiate_algebra_type(r.clone(), &base, &source_indices),
            ),
        }
    })
}
...
pub fn instantiate_algebra_field(
...
) -> Rc<Node> {
    {
        ...
        let return_type =
            instantiate_algebra_type(template.return_type.clone(), &base, &source_indices);
            ...
            Some(p) => {
                let fields = Rc::new({
                    ...
                    for template in algebra_templates_for_profile(p.clone()).iter().cloned() {
                        __result.push(instantiate_algebra_field(&template, &base, &source_indices));
                    }
                    __result
                });
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || match (*template).clone() {
        ...
        AlgebraTypeTemplate::ReceiverSelf => {
            let s1 = match concrete.children.clone().first().cloned() {
                Some(k) => {
                    if (v2_rt::map_get(&subst, "__key__".to_string()) != None) {
                        subst.clone()
                    } else {
                        v2_rt::rc_map_insert(
                            subst.clone(),
            ...
            match concrete.children.clone().get(1 as usize).cloned() {
                Some(v) => {
                    if (v2_rt::map_get(&s1, "__value__".to_string()) != None) {
                        s1.clone()
                    } else {
                        v2_rt::rc_map_insert(
                            s1.clone(),
        ...
        AlgebraTypeTemplate::ReceiverValue => {
            if (v2_rt::map_get(&subst, "__value__".to_string()) != None) {
                subst.clone()
            } else {
        ...
        AlgebraTypeTemplate::ReceiverElement => {
            if (v2_rt::map_get(&subst, "__element__".to_string()) != None) {
                subst.clone()
            } else {
        ...
        AlgebraTypeTemplate::ContainerOf {
            ...
            let expected_name = match (*src.clone()).clone() {
                ContainerSource::SameAsReceiver => {
                    authored_name_at(source_indices.clone(), &receiver)
                }
                ContainerSource::Named { name: n, .. } => n.clone(),
            };
            if (authored_name_at(source_indices.clone(), &concrete).as_str()
...
pub fn apply_type_substitution(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || match (*template).clone() {
        AlgebraTypeTemplate::AlgebraTypeVariable { id: var_id, .. } => {
            match v2_rt::map_get(&subst, var_id.clone()) {
                    ...
                    if (arity.clone() == Some(2)) {
                        match v2_rt::map_get(&subst, "__key__".to_string()) {
                            Some(key) => {
                                ...
                                match make_map_type(key.clone(), val) {
                                    Some(m) => m.clone(),
                                    None => {
                                        missing_kernel_container_profile_type("Map".to_string())
                                    }
                                }
                            }
                            None => receiver.clone(),
                        ...
                        if (arity.clone() == Some(1)) {
                            match v2_rt::map_get(&subst, "__element__".to_string()) {
                                Some(elem) => {
                                    match make_container_type(&receiver_name_str, elem.clone()) {
                                        Some(c) => c.clone(),
                                        None => missing_kernel_container_profile_type(
                                            receiver_name_str.clone(),
                                        ),
                                    }
                                }
                                None => receiver.clone(),
        ...
        AlgebraTypeTemplate::ReceiverElement => match receiver.children.clone().first().cloned() {
            ...
            None => match v2_rt::map_get(&subst, "__element__".to_string()) {
                ...
                None => match container_param_name(
                    authored_name_at(source_indices.clone(), &receiver),
                    0,
                ) {
                    Some(n) => type_variable_node(n.clone()),
                    None => missing_kernel_container_profile_type(authored_name_at(
                        source_indices.clone(),
                        &receiver,
                    )),
                },
        ...
        AlgebraTypeTemplate::ContainerOf {
            ...
            let inner_applied =
                apply_type_substitution(inner.clone(), &subst, receiver.clone(), &source_indices);
        ...
        AlgebraTypeTemplate::TupleOf {
            ...
            apply_type_substitution(ft.clone(), &subst, receiver.clone(), &source_indices),
            apply_type_substitution(st.clone(), &subst, receiver.clone(), &source_indices),
        ),
        AlgebraTypeTemplate::CallableOf {
            params: p,
            return_type: r,
            ...
            make_callable_type(
                ...
                apply_type_substitution(r.clone(), &subst, receiver.clone(), &source_indices),
            )
        }
    })
}
        ...
        if __is_leaf {
            {
                let __is_named_ref = match n.inferred.clone().as_deref().cloned() {
                    Some(InferredNode::Resolved { node: rt, .. }) => {
                        (((((rt.connective.clone() == Connective::NoConnective)
                            ...
                            && (authored_name_at(source_indices.clone(), &rt).as_str()
                                != "None".to_string().as_str()))
                            && (is_kernel_type(authored_name_at(source_indices.clone(), &rt))
                                == false))
                    }
                    _ => false,
                };
                if __is_named_ref {
                    ...
                    v2_rt::concat(
                        v2_rt::concat("Primitive(".to_string(), n_name),
                        ")".to_string(),
                    )
                }
            }
        } else {
            {
                ...
                if is_product {
                    if (n.ident_span.clone() == None) {
                        ...
                        v2_rt::concat(
                            v2_rt::concat("Product(".to_string(), n_name),
                            ")".to_string(),
                        )
                    }
                } else {
                    if is_coproduct {
                        if (n.ident_span.clone() == None) {
                            ...
                            v2_rt::concat(
                                v2_rt::concat("Coproduct(".to_string(), n_name),
                                ")".to_string(),
                            )
                        }
                    } else {
                            ...
                            if __is_container {
                                {
                                    let elem_shape = match n.children.clone().first().cloned() {
                                        Some(el) => {
                                            node_type_shape(&child_type_node(&el), &source_indices)
                                        }
                                        None => "?".to_string(),
                                    };
                                    v2_rt::concat(
                                        v2_rt::concat(
                                            v2_rt::concat(
                                                v2_rt::concat("Container(".to_string(), n_name),
                                                ",".to_string(),
                                            ),
                                            elem_shape,
                                        ),
                                ...
                                if is_optional {
                                    {
                                        ...
                                        v2_rt::concat(
                                            v2_rt::concat("Optional(".to_string(), inner_shape),
                                            ")".to_string(),
                                        )
                                    }
                                } else {
                                    if is_map {
                                        ...
                                        v2_rt::concat(
                                            v2_rt::concat("Node(".to_string(), n_name),
                                            ")".to_string(),
                                        )
                                    }
                                }
                ...
                if (left_opt.clone() && right_is_unit) {
                    ...
                    if (left_is_unit && right_opt.clone()) {
                        ...
                        let left_is_container =
                            node_is_element_collection(&left, source_indices.clone());
                        let right_is_container =
                            node_is_element_collection(&right, source_indices.clone());
                        if (left_is_container && right_is_container) {
                            if (authored_name_at(source_indices.clone(), &left).as_str()
                                != authored_name_at(source_indices.clone(), &right).as_str())
                            {
                                break false;
                            } else {
                                match left.children.clone().first().cloned() {
                                    Some(left_ch) => {
                                        match right.children.clone().first().cloned() {
                                            Some(right_ch) => {
                                                let left_el = child_type_node(&left_ch);
                            ...
                            if (left_opt.clone() && right_opt.clone()) {
                                ...
                                if (left_opt.clone() || right_opt.clone()) {
                                    ...
                                    break (authored_name_at(source_indices.clone(), &left)
                                        .as_str()
                                        == authored_name_at(source_indices.clone(), &right)
                                            .as_str());
                                }
                            }
                        }
...
pub fn prefer_specific_type(
...
) -> Rc<Node> {
    {
        ...
        if (same_kind && left_is_unit_inner) {
            ...
            if (is_fully_resolved(&right, &source_indices)
                && !is_fully_resolved(&left, &source_indices))
            {
                right.clone()
            } else {
                left.clone()
                        ...
                        if (left_is_unit_eq && right_opt.clone()) {
                            ...
                            if (left_opt.clone() && right_opt.clone()) {
                                node_type_equals_core(
                                    &with_required_cardinality(&left),
                                    &with_required_cardinality(&right),
                                    &source_indices,
                                )
                            } else {
                    ...
                    if (left_struct.clone() && right_leaf.clone()) {
                    ...
                    } else {
                        {
                            ...
                            if (left_is_container && right_is_container) {
                                        ...
                                        Some(left_ch) => {
                                            match right.children.clone().first().cloned() {
                                                Some(right_ch) => node_type_equals(
                                                    &child_type_node(&left_ch),
                                                    &child_type_node(&right_ch),
                                                    source_indices.clone(),
                                                ),
                                                None => false,
                                            }
                                ...
                                {
                                    let both_maps =
                                        (node_is_keyed_collection(&left, source_indices.clone())
                                                    ...
                                                    .cloned()
                                                {
                                                        ...
                                                        .cloned()
                                                    {
                                                            ...
                                                            .cloned()
                                                        {
                                                            Some(right_second) => {
                                                                (node_type_equals(
                                                                    &child_type_node(&left_first),
                                                                    &child_type_node(&right_first),
                                                                    source_indices.clone(),
                                                                ) && node_type_equals(
                                                                    &child_type_node(&left_second),
                                                                    &child_type_node(&right_second),
                                                                    source_indices.clone(),
                                                                ))
                                                            }
                                                            None => false,
...
pub fn node_type_deps(
    ...
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
            ...
            match n.inferred.clone().as_deref().cloned() {
                Some(InferredNode::Resolved { node: rt, .. }) => {
                    (((((rt.connective.clone() == Connective::NoConnective)
                        ...
                        && (authored_name_at(source_indices.clone(), &rt).as_str()
                            != "None".to_string().as_str()))
                        && (is_kernel_type(authored_name_at(source_indices.clone(), &rt)) == false))
                }
                _ => false,
            }
        };
        ...
        if __is_named_ref {
            match n.inferred.clone().as_deref().cloned() {
                Some(InferredNode::Resolved { node: rt, .. }) => {
                    Rc::new(vec![authored_name_at(source_indices.clone(), &rt)])
                }
                _ => Rc::new(vec![]),
            }
        } else {
                    ...
                    for child in n.children.clone().iter().cloned() {
                        __result.extend(
                            (*match child.inferred.clone().as_deref().cloned() {
                                Some(InferredNode::Resolved { node: rt, .. }) => {
                                    node_type_deps(&rt, &source_indices)
                                }
                                _ => {
                    ...
                    if ((n.children.clone().len() as i64) > 0) {
                        {
                            let child_deps = Rc::new({
                                ...
                                for child in n.children.clone().iter().cloned() {
                                    __result.extend(
                                        (*node_type_deps(&child, &source_indices)).iter().cloned(),
                                    );
                                }
                                __result
                            });
        ...
        match extracted {
            ...
            None => {
                if ((((normed.connective.clone() == Connective::NoConnective)
                    ...
                    && (authored_name_at(source_indices, &normed).as_str()
