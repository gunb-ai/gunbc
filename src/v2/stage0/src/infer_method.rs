...
pub use crate::v2_core::{Node, InferredNode, Connective, CollectionKind, Cardinality, ExprData, IntrinsicMethod, RuntimeBridgeMethod, leaf_node, with_optional_cardinality};
use crate::v2_core::InferredNode::{TypeVariable};
use crate::v2_core::Connective::{NoConnective};
use crate::v2_core::CollectionKind::{NoCollection};
use crate::v2_core::Cardinality::{Required};
use crate::v2_core::ExprData::{NoExprData};
...
use crate::v2_core::RuntimeBridgeMethod::{BridgeGet, BridgeWith, BridgeListPush, BridgeMapInsert, BridgeMapMerge, BridgeMapGet, BridgeMapHas, BridgeEmitMapHas, BridgeMapValues, BridgeMapKeys, BridgeMapContainsKey, BridgeCharAt, BridgeStringAt, BridgeStringLength, BridgeLength, BridgeStartsWith, BridgeEndsWith, BridgeToString, BridgeTrim, BridgeToLower, BridgeToUpper, BridgeReplace, BridgeSubstring, BridgeToInt, BridgeEmptyMap, BridgeContains, BridgeReverse, BridgeLookup};
pub use crate::infer_types::{container_node, tuple_node, error_type_node, bare_map_node, method_receiver_element_node};
            ...
            if (((((name.clone() == "char_at".to_string()) || (name.clone() == "substring".to_string())) || (name.clone() == "from_code_point".to_string())) || (name.clone() == "to_string".to_string())) || (name.clone() == "concat".to_string())) {
                ...
                if ((((name.clone() == "scan_while".to_string()) || (name.clone() == "scan_string_end".to_string())) || (name.clone() == "scan_to_eol".to_string())) || (name.clone() == "skip_horizontal_ws".to_string())) {
                    ...
                    if ((name.clone() == "lookup".to_string()) || (name.clone() == "map_get".to_string())) {
                        Some(with_optional_cardinality(type_variable_node("map_value".to_string())))
} else {
                        if (((name.clone() == "map_insert".to_string()) || (name.clone() == "map_merge".to_string())) || (name.clone() == "with".to_string())) {
                            ...
                            if (((name.clone() == "map_contains_key".to_string()) || (name.clone() == "map_has".to_string())) || (name.clone() == "emit_map_has".to_string())) {
                                ...
                                if ((((name.clone() == "map_keys".to_string()) || (name.clone() == "map_values".to_string())) || (name.clone() == "reverse".to_string())) || (name.clone() == "list_push".to_string())) {
                                    Some(container_node("List".to_string(), type_variable_node("collection_element".to_string())))
} else {
                                    if (name.clone() == "Some".to_string()) {
                                        Some(with_optional_cardinality(type_variable_node("some_inner".to_string())))
} else {
                                        None
}
}
