...
let nname = authored_name_at(source_indices.clone(), &n);
...
let child_diags = Rc::new({ let mut __result = Vec::new(); for c in n.children.clone().iter().cloned() { __result.extend((*check_bare_containers(&c, &module_name, &source_indices)).iter().cloned()); } __result });
let param_diags = Rc::new({ let mut __result = Vec::new(); for p in n.params.clone().iter().cloned() { __result.extend((*check_bare_containers(&p, &module_name, &source_indices)).iter().cloned()); } __result });
let type_ann_diags = match n.type_annotation.clone() {
    Some(ta) => check_bare_containers(&ta, &module_name, &source_indices),
    None => Rc::new(vec![]),
};
let body_diags = match n.body.clone() {
    Some(b) => check_bare_containers(&b, &module_name, &source_indices),
    None => Rc::new(vec![]),
};
let inferred_diags = if ((n.params.clone().len() as i64) > 0) {
    ...
    InferredNode::Resolved { node: rn, .. } => check_bare_containers(&rn, &module_name, &source_indices),
    _ => Rc::new(vec![]),
},
    None => Rc::new(vec![]),
}
            };
let uses_diags = Rc::new({ let mut __result = Vec::new(); for u in n.uses.clone().iter().cloned() { __result.extend((*check_bare_containers(&u, &module_name, &source_indices)).iter().cloned()); } __result });
let prop_diags = Rc::new({ let mut __result = Vec::new(); for p in n.properties.clone().iter().cloned() { __result.extend((*check_bare_containers(&p, &module_name, &source_indices)).iter().cloned()); } __result });
