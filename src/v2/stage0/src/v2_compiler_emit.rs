...
pub use crate::v2_compiler_languages::{LanguageSpec, ReservedWordStrategy, language_spec_for_target, test_conventions_for_target, target_keyword, target_primitive_type, target_container_template, TestNameStyle};
...
if is_typed_service_call_receiver(expr.clone()) {
    ...
    None => v2_rt::concat(v2_rt::concat(emit_simple_expr(b.clone(), target.clone()), ".".to_string()), emit_ident(f.clone(), target.clone())),
}
} else {
                v2_rt::concat(v2_rt::concat(emit_simple_expr(b.clone(), target.clone()), ".".to_string()), emit_ident(f.clone(), target.clone()))
}
},
...
if has_unnamed.clone() {
    ...
    None => args.clone(),
    Some(sig) => {
                let arg_map = args.clone().iter().cloned().fold(<HashMap<String, Rc<Node>>>::new(), |acc: _, arg: Rc<Node>| if (arg_name(arg.clone()) != None) {
                    v2_rt::map_insert(acc.clone(), arg_name(arg.clone()).clone().unwrap(), arg.clone())
} else {
                    acc.clone()
});
...
let leftovers = { let mut __result = Vec::new(); for arg in args.clone().iter().cloned() { if if (arg_name(arg.clone()) == None) {
                    true
} else {
                    (emit_map_has(param_name_set.clone(), arg_name(arg.clone()).clone().unwrap()) == false)
...
pub fn escape_json_string(s: String) -> String {
    s.clone().split(&"\\".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&"\\\\".to_string()).split(&"\"".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&"\\\"".to_string()).split(&"
...
pub fn module_to_filename(name: String) -> String {
    name.clone().split(&".".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&"_".to_string())
}
...
{ let mut __result = Vec::new(); for ch in snake.clone().chars().map(|c| c.to_string()).collect::<Vec<_>>().iter().cloned() { __result.push(to_upper_char(ch.clone())); } __result }.join(&"".to_string())
}
}
...
pub fn apply_type_template1(template: String, arg0: String) -> String {
    template.clone().split(&"{0}".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&arg0.clone())
}

pub fn apply_type_template2(template: String, arg0: String, arg1: String) -> String {
    template.clone().split(&"{0}".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&arg0.clone()).split(&"{1}".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&arg1.clone())
}
...
pub fn emit_string_literal(s: String, suffix: String) -> String {
    {
        let escaped = s.clone().split(&"\\".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&"\\\\".to_string());
let escaped2 = escaped.clone().split(&"\"".to_string()).map(|s| s.to_string()).collect::<Vec<_>>().join(&"\\\"".to_string());
v2_rt::concat(v2_rt::concat(v2_rt::concat("\"".to_string(), escaped2.clone()), "\"".to_string()), suffix.clone())
}
}
...
if (((n.name.clone() == "Dynamic".to_string()) || (n.name.clone() == "Error".to_string())) && ((n.children.clone().len() as i64) == 0)) {
                if is_rust.clone() {
                    return v2_rt::concat(v2_rt::concat("compile_error!(\"unresolved ".to_string(), n.name.clone()), " type reached emit\")".to_string())
} else {
...
match target.clone() {
    RenderTarget::Go => v2_rt::concat(v2_rt::concat(v2_rt::concat("func(".to_string(), param_str.clone()), ")".to_string()), if (ret_str.clone() != "".to_string()) {
...
let kind = if (item_has_structure.clone() && (item.transport.clone() == None)) {
            ...
            if ((((((item_has_structure.clone() == false) && (item.body.clone() == None)) && ((item.params.clone().len() as i64) == 0)) && (item.transport.clone() == None)) && ((item.children.clone().len() as i64) == 0)) && is_type_alias_return_node(rt_type(item.clone()))) {
