...
if is_pair {
    ...
    Some(c) => if (c.inferred.clone() != None) {
                                render_node_type(resolved_type_or_error(c.clone()), target.clone(), shared_types.clone())
} else {
...
let parts = Rc::new({ let mut __result = Vec::new(); for child in n.children.clone().iter().cloned() { __result.push(render_node_type(resolved_type_or_error(child.clone()), target.clone(), shared_types.clone())); } __result });
