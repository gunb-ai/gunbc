...
pub use crate::v2_compiler_infer_types::{rt_type, child_type_node, emit_map_has, node_is_collection, node_is_keyed_collection, node_is_element_collection};
...
if is_pair {
    ...
    Some(c) => if (c.inferred.clone() != None) {
                                render_node_type(rt_type(c.clone()), target.clone(), shared_types.clone())
} else {
