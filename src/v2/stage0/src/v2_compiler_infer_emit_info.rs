...
let key = authored_name_at(source_index.clone(), &child);
...
pub fn enum_field_type_consistent(variants: Rc<Vec<Rc<Node>>>, field_name: String, expected: Rc<Node>, source_index: Option<Rc<NewlineIndex>>) -> bool {
    ...
    Some(field_child) => node_type_equals(&child_type_node(&field_child), &expected, source_index.clone()),
    None => false,
...
pub fn build_enum_field_summaries(variants: &Rc<Vec<Rc<Node>>>, source_index: Option<Rc<NewlineIndex>>) -> Rc<HashMap<String, Rc<FieldSummary>>> {
    ...
    Some(first_variant) => Rc::new({ let mut __result = Vec::new(); for f in first_variant.children.clone().iter().cloned() { __result.push(authored_name_at(source_index.clone(), &f)); } __result }),
    None => Rc::new(vec![]),
};
...
let consistent = Rc::new({ let mut __result = Vec::new(); for field_name in shared.iter().cloned() { if match find_first_enum_field_node(variants.clone(), field_name.clone(), source_index.clone()) {
    Some(first_field) => enum_field_type_consistent(variants.clone(), field_name.clone(), child_type_node(&first_field), source_index.clone()),
    None => false,
...
if is_product {
    ...
    field_summaries: build_struct_field_summaries(&item, source_index.clone()),
...
Some(Rc::new(TypeSummary {
    ...
    variant_name_set: item.children.clone().iter().cloned().fold(v2_rt::rc_empty_map::<bool>(), |acc: Rc<HashMap<String, bool>>, child: Rc<Node>| v2_rt::rc_map_insert(acc.clone(), authored_name_at(source_index.clone(), &child), true)),
    generic_param_names: gpn,
    has_fn_fields: has_fn,
}))
}
...
pub fn add_emit_item_summary(state: Rc<EmitInfoBuildState>, item: &Rc<Node>, source_index: &Option<Rc<NewlineIndex>>) -> Rc<EmitInfoBuildState> {
    match build_type_summary(&item, &source_index) {
    Some(summary) => {
...
let vname = authored_name_at(source_index.clone(), &variant);
v2_rt::rc_map_insert(acc.clone(), vname.clone(), Rc::new(TypeSummary {
    name: vname.clone(),
    repr: Rc::new(TypeRepr::StructRepr),
    field_summaries: build_struct_field_summaries(&variant, source_index.clone()),
