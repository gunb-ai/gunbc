...
let test_files = Rc::new({ let mut __result = Vec::new(); for f in Rc::new({ let mut __result = Vec::new(); for tm in typed.modules.clone().iter().cloned() { __result.push(emit_py_test_file(&authored_name_at(tm.type_env.clone().source_indices.clone(), &tm.module.clone()), &Rc::new({ let mut __result = Vec::new(); for p in test_projections.clone().iter().cloned() { if (p.module_name.clone().as_str() == authored_name_at(tm.type_env.clone().source_indices.clone(), &tm.module.clone()).as_str()) { __result.push(p); } } __result }))); } __result }).iter().cloned() { if (f.path.clone().as_str() != "".to_string().as_str()) { __result.push(f); } } __result });
...
pub fn emit_init_py(modules: Rc<Vec<Rc<TypedModule>>>) -> Rc<TextFile> {
    {
        let import_lines = Rc::new({ let mut __result = Vec::new(); for tm in modules.iter().cloned() { __result.push({
            let mod_name = module_to_filename(authored_name_at(tm.type_env.clone().source_indices.clone(), &tm.module.clone()));
...
pub fn python_test_signature_comment(projection: &Rc<TestProjection>) -> String {
    {
        let params_str = Rc::new({ let mut __result = Vec::new(); for p in projection.params.clone().iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat(param_node_name_at(p.clone(), projection.source_indices.clone()), ": ".to_string()), emit_node_type(param_node_type_expr(&p), RenderTarget::Python, projection.source_indices.clone()))); } __result }).join(&", ".to_string());
...
pub fn emit_py_test_file(module_name: &String, projections: &Rc<Vec<Rc<TestProjection>>>) -> Rc<TextFile> {
    ...
    content: "".to_string(),
        ...
        {
            let tests_str = Rc::new({ let mut __result = Vec::new(); for p in projections.clone().iter().cloned() { __result.push(emit_py_operation_test(&p, 0)); } __result }).join(&"\n\n".to_string());
...
let mock_setup = Rc::new({ let mut __result = Vec::new(); for mp in projection.mock_field_inits.clone().iter().cloned() { __result.push(emit_py_mock_prop_setup(&mp, (depth.clone() + 1), &projection.source_indices.clone())); } __result }).join(&"\n".to_string());
...
pub fn emit_py_mock_prop_setup(mock_prop: &Rc<Node>, depth: i64, source_indices: &Rc<HashMap<String, Rc<NewlineIndex>>>) -> String {
    v2_rt::concat(v2_rt::concat(emit_ident(field_init_node_name_at(mock_prop.clone(), source_indices.clone()), RenderTarget::Python), " = ".to_string()), emit_simple_expr(&field_init_node_value(&mock_prop), &RenderTarget::Python, &source_indices))
}
...
let items_str = Rc::new({ let mut __result = Vec::new(); for item in typed_module.items.clone().iter().cloned() { __result.push(emit_py_typed_item(&item, registry.clone(), &scope)); } __result }).join(&"\n\n\n".to_string());
...
let has_structs = { let mut __found = false; for item in items.clone().iter().cloned() { if (is_type_def_item(&item) && (item.connective.clone() == Connective::Conj)) { __found = true; break; } } __found };
let has_enums = { let mut __found = false; for item in items.clone().iter().cloned() { if (is_type_def_item(&item) && (item.connective.clone() == Connective::Disj)) { __found = true; break; } } __found };
let has_services = { let mut __found = false; for item in items.clone().iter().cloned() { if is_service_item(&item) { __found = true; break; } } __found };
                ...
                if is_type_decl_item(&item, env.source_indices.clone()) {
                    ...
                    if is_function_item(&item) {
                        ...
                        if is_data_def_item(&item) {
                            emit_py_data_def(item_text, item.type_annotation.clone().clone().unwrap(), item.body.clone().clone().unwrap(), registry, &scope)
                        } else {
...
pub fn emit_py_dataclass_from_children(name: String, children: &Rc<Vec<Rc<Node>>>, env: Rc<TypeEnv>) -> String {
    if ((children.clone().len() as i64) == 0) {
        ...
        {
            let field_lines = Rc::new({ let mut __result = Vec::new(); for child in children.clone().iter().cloned() { __result.push(emit_py_dataclass_field_from_child(&child, &env)); } __result });
...
if has_data {
            {
                let variant_classes = Rc::new({ let mut __result = Vec::new(); for child in children.clone().iter().cloned() { __result.push(emit_py_variant_class_from_child(name.clone(), &child, &env)); } __result });
...
if ((child.children.clone().len() as i64) == 0) {
            ...
            {
                let field_lines = Rc::new({ let mut __result = Vec::new(); for f in child.children.clone().iter().cloned() { __result.push(emit_py_dataclass_field_from_child(&f, &env)); } __result });
let fields_str = field_lines.join(&"\n".to_string());
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(py_derive_attribute(), "\nclass ".to_string()), class_name), ":\n".to_string()), fields_str)
}
        }
...
if use_tco {
            {
                let body_str = emit_py_typed_tco_body(body.clone(), &name, &params, &registry, &body_scope, (depth.clone() + 1));
...
let resource_strs = Rc::new({ let mut __result = Vec::new(); for u in uses.iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat(emit_ident(resource_use_name_at(u.clone(), source_indices.clone()), RenderTarget::Python), ": ".to_string()), emit_node_type(resource_use_resource(&u), RenderTarget::Python, source_indices.clone()))); } __result });
...
pub fn emit_py_pattern(pattern: Rc<MatchPattern>, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>) -> String {
    ...
    MatchPattern::VariantPattern { name: n, field_bindings: fbs, .. } => emit_py_variant_pattern(n.clone(), &fbs, source_indices),
...
pub fn emit_py_typed_expr(texpr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: &Rc<InferScope>, depth: i64, fuel: i64) -> String {
    emit_unified_typed_expr(texpr, &RenderTarget::Python, registry, &scope, depth, fuel, |pat| emit_py_pattern(pat.clone(), scope.type_env.clone().source_indices.clone()))
}
...
if ((init_state.text.clone().len() as i64) == 0) {
    ...
    _ => v2_rt::concat("return ".to_string(), emit_py_typed_expr(body.clone(), registry.clone(), &scope, depth.clone(), 1024)),
}
}
    })
}
...
pub fn emit_py_typed_tco_body(texpr: Rc<Node>, fn_name: &String, params: &Rc<Vec<Rc<Node>>>, registry: &Rc<HashMap<String, Rc<ItemInfo>>>, scope: &Rc<InferScope>, depth: i64) -> String {
    emit_unified_tco_body(texpr, fn_name.clone(), params.clone(), &RenderTarget::Python, registry.clone(), scope.clone(), depth.clone(), |expr, scope, depth| emit_py_typed_expr(expr.clone(), registry.clone(), &scope, depth.clone(), 1024), |frame| emit_py_tco_match(&frame, fn_name.clone(), params.clone(), &registry, |stmts, scope, depth| emit_py_init_block_stmts(stmts.clone(), Rc::new(vec![]), scope.clone(), registry.clone(), depth.clone())), |stmts, scope, depth| emit_py_init_block_stmts(stmts.clone(), Rc::new(vec![]), scope.clone(), registry.clone(), depth.clone()))
}

pub fn emit_py_tco_match(frame: &Rc<TcoFrame>, fn_name: String, params: Rc<Vec<Rc<Node>>>, registry: &Rc<HashMap<String, Rc<ItemInfo>>>, render_init_stmts: impl Fn(Rc<Vec<Rc<Node>>>, Rc<InferScope>, i64) -> Rc<BlockEmitState> + Clone) -> String {
...
let arm_strs = Rc::new({ let mut __result = Vec::new(); for arm in arm_list.iter().cloned() { __result.push(emit_py_typed_tco_match_arm(&arm, &fn_name, &params, &registry, &frame.scope.clone(), frame.depth.clone(), render_init_stmts.clone())); } __result });
...
pub fn emit_py_typed_tco_match_arm(arm: &Rc<Node>, fn_name: &String, params: &Rc<Vec<Rc<Node>>>, registry: &Rc<HashMap<String, Rc<ItemInfo>>>, scope: &Rc<InferScope>, depth: i64, render_init_stmts: impl Fn(Rc<Vec<Rc<Node>>>, Rc<InferScope>, i64) -> Rc<BlockEmitState> + Clone) -> String {
    {
        let bs = language_spec(RenderTarget::Python).block_syntax.clone();
...
let body_str = emit_unified_tco_expr(&Rc::new(TcoFrame {
    ...
    scope: scope.clone(),
    depth: (depth.clone() + 2),
}), &fn_name, params.clone(), &RenderTarget::Python, registry.clone(), |expr, scope, depth| emit_py_typed_expr(expr.clone(), registry.clone(), &scope, depth.clone(), 1024), |frame| emit_py_tco_match(&frame, fn_name.clone(), params.clone(), &registry, render_init_stmts.clone()), render_init_stmts.clone());
...
pub fn emit_py_transport_body(transport: Rc<Node>, op_name: String, source_indices: Rc<HashMap<String, Rc<NewlineIndex>>>, depth: i64) -> String {
    emit_unified_transport_dispatch(&transport, op_name, &source_indices, depth, RenderTarget::Python, |n, t, d, si| emit_py_rest_call(n.clone(), t.clone(), si.clone()), |n, t, d, si| emit_py_shell_call(n.clone(), t.clone(), &si), |n, d| emit_py_file_call(n.clone()), |n, d| emit_py_local_call(n.clone()))
}

pub fn emit_py_service_def(item: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, env: Rc<TypeEnv>) -> String {
    emit_unified_service_def(&item, &RenderTarget::Python, registry, &env, |name, transport, ops, si| emit_py_service_init(transport.clone(), ops.clone(), si.clone()), |transport, op_name, si, depth| emit_py_transport_body(transport.clone(), op_name.clone(), si.clone(), depth.clone()))
}
...
let header_entries = Rc::new({ let mut __result = Vec::new(); for h in hdrs.iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat(v2_rt::concat("\"".to_string(), field_init_node_name_at(h.clone(), source_indices.clone())), "\": ".to_string()), emit_simple_expr(&field_init_node_value(&h), &RenderTarget::Python, &source_indices))); } __result });
...
let env_dict_entries = Rc::new({ let mut __result = Vec::new(); for e in envs.iter().cloned() { __result.push(v2_rt::concat(v2_rt::concat(v2_rt::concat("\"".to_string(), field_init_node_name_at(e.clone(), source_indices.clone())), "\": ".to_string()), emit_simple_expr(&field_init_node_value(&e), &RenderTarget::Python, &source_indices))); } __result });
...
let methods = Rc::new({ let mut __result = Vec::new(); for c in cap_children.iter().cloned() { __result.push(emit_py_capability_method(&c, &env)); } __result });
