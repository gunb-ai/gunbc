...
pub use crate::v2_compiler_emit::{
    ...
    empty_emit_scope, escape_json_string, escape_python_interp_text, escape_string_literal_body,
    extract_string_interp_parts, extract_test_projections, has_nested_records_node,
    has_service_items, is_data_def_item, is_function_item, is_null_coalesce, is_resource_def_item,
...
pub use crate::v2_std_core::{
    ...
    transport_has_auth, transport_headers, with_required_cardinality, Connective, DeclaredFuncSig,
    ExprData, FieldSummary, InferredNode, MatchPattern, MethodSemantics, NewlineIndex, Node,
...
pub fn emit_python(typed: Rc<ResolvedGraph>) -> Rc<EmitResult> {
    {
        ...
        let files = v2_rt::concat(
            v2_rt::concat(Rc::new(vec![requirements, init_file]), module_files),
            test_files,
        );
        Rc::new(EmitResult {
            files: files,
            ...
            for tm in modules.iter().cloned() {
                __result.push({
                    ...
                    v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat(
                                v2_rt::concat(items.import_from_keyword.clone(), " . ".to_string()),
...
pub fn emit_py_prelude(typed_module: Rc<TypedModule>) -> String {
    {
        ...
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(v2_rt::concat(future_import, dc_import), enum_import),
                typing_import,
            ),
            async_import,
        )
...
pub fn emit_py_dataclass_from_children(
    ...
    if ((children.clone().len() as i64) == 0) {
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(py_derive_attribute(), "\nclass ".to_string()),
                name,
            ),
            ":\n    pass".to_string(),
        )
...
) -> String {
    {
        ...
        let service_strs = Rc::new({
            ...
            for sn in service_names.iter().cloned() {
                __result.push(v2_rt::concat(
                    v2_rt::concat(service_var_name(sn.clone()), ": ".to_string()),
...
pub fn emit_py_service_init(
...
) -> String {
    {
        ...
        if ((params.clone().len() as i64) == 0) {
            ...
            {
                let params_str =
                    v2_rt::concat("self, ".to_string(), params.clone().join(&", ".to_string()));
                let assigns_str = Rc::new({
                    let mut __result = Vec::new();
                    for a in assigns.iter().cloned() {
                        __result.push(v2_rt::concat("    ".to_string(), a.clone()));
                    }
                    __result
                })
                .join(&"\n".to_string());
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat("def __init__(".to_string(), params_str),
                        "):\n".to_string(),
                    ),
                    assigns_str,
                )
...
pub fn emit_py_rest_call(
...
) -> String {
    {
        let self_base_url = v2_rt::concat(
            v2_rt::concat("{".to_string(), "self.base_url".to_string()),
            "}".to_string(),
        );
        let url_line = v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat("url = f\"".to_string(), self_base_url),
                    "/".to_string(),
                ),
                emit_ident(op_name, RenderTarget::Python),
            ),
            "\"".to_string(),
        );
        ...
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(v2_rt::concat(url_line, "\n".to_string()), headers_dict),
                "\n".to_string(),
            ),
            session_lines,
        )
...
pub fn emit_py_headers_dict(
...
) -> String {
    {
        let auth_entry = if transport_has_auth(transport.clone(), source_indices.clone()) {
            {
                ...
                v2_rt::concat(
                    v2_rt::concat("\"".to_string(), header_name),
        ...
        v2_rt::concat(
            v2_rt::concat("headers = {".to_string(), all_entries),
            "}".to_string(),
        )
    }
}
...
pub fn emit_py_shell_call(
...
) -> String {
    {
        ...
        let env_str = v2_rt::concat(
            v2_rt::concat("{".to_string(), env_dict_entries.join(&", ".to_string())),
            "}".to_string(),
        );
        v2_rt::concat(
            v2_rt::concat(
...
pub fn emit_py_file_call(op_name: String) -> String {
    {
        let self_base_path = v2_rt::concat(
            v2_rt::concat("{".to_string(), "self.base_path".to_string()),
            "}".to_string(),
        );
        v2_rt::concat(
            v2_rt::concat(
                v2_rt::concat(
                    v2_rt::concat(
                        v2_rt::concat(
                            v2_rt::concat("path = f\"".to_string(), self_base_path),
                            "/".to_string(),
                        ),
                        emit_ident(op_name, RenderTarget::Python),
                    ),
