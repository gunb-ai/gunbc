...
fn compile_dag_with_complexity(
    source: &str,
) -> Rc<v2_compiler::v2_compiler_complexity::ComplexityReport> {
...
// Complexity-counter ratchet: tokenizer text lookup stays O(1) in file size.
//
// `01_tokenize.dag` precomputes `source_chars` once and indexes it for every
// scan step. A recurrence that re-walks the raw `String` per lookup turns
// tokenization super-linear. This is a focused slice of the retired
// `strict_complexity_violation_count` gate — hot-path lookup helpers only.
#[test]
fn complexity_counter_tokenizer_text_lookup_flat_in_file_size() {
    let tokenize_dag = read_v2_file("src/v2/01_tokenize.dag");
    let tokenize_rs = read_v2_file("src/v2/stage0/src/v2_compiler_tokenize.rs");

    assert!(
        tokenize_dag.contains("source.source_chars[pos]"),
        "01_tokenize.dag must index precomputed codepoints for O(1) text lookup"
    );
    assert!(
        tokenize_dag.contains("chars_to_string(chars: source.source_chars"),
        "01_tokenize.dag must slice the codepoint vec for substring text, not the raw String"
    );
    assert!(
        tokenize_rs.contains("v2_rt::chars_to_string"),
        "stage0 tokenizer must realize substring via chars_to_string on source_chars"
    );
    assert!(
        !tokenize_rs.contains("v2_rt::substring(&source.text"),
        "stage0 tokenizer must not route hot-path substring through v2_rt::substring on raw text"
    );

    let result = crate::helpers::resolve_imports_transitively(
        "src/v2/01_tokenize.dag",
        &tokenize_dag,
    );
    let file_refs: Vec<(&str, &str)> = result
        .iter()
        .map(|src| (src.path.as_str(), src.content.as_str()))
        .collect();
    ...
    let lookup_violations: Vec<_> = compiled
        ...
        .violations
        .iter()
        .filter(|v| TEXT_LOOKUP_FUNCS.contains(&v.func_name.as_str()))
        .map(|v| format!("{}: {}", v.func_name, v.reason))
        .collect();

    eprintln!(
        "tokenizer text-lookup complexity violations: {} (total compile violations: {})",
        lookup_violations.len(),
        compiled.complexity.violations.len()
    );
    for line in &lookup_violations {
        eprintln!("  {line}");
    }

    // Ratchet: hot-path text lookup helpers must stay violation-free. Raise only
    // when the analyzer gap is closed, never when reintroducing O(pos) lookup.
    const TEXT_LOOKUP_VIOLATION_RATCHET: usize = 0;
    assert!(
        lookup_violations.len() <= TEXT_LOOKUP_VIOLATION_RATCHET,
        "tokenizer text lookup must stay flat in file size (O(1) list index); got {} violations: {:?}",
        lookup_violations.len(),
        lookup_violations
    );
}
...
fn python_div_uses_algebra_aware_dispatch() {
    ...
    use v2_compiler::std_syntax::{AlgebraFieldKind, BinOp};
    use v2_compiler::v2_compiler_artifact::RenderTarget;
...
fn diag_emitter_scc() {
    ...
    // Build func_index
    let func_index: HashMap<String, Rc<v2_compiler::v2_compiler_compile::FuncEntry>> = func_entries
        .iter()
        ...
        let unknown_edges: Vec<_> = edges
            ...
            .filter(|e| {
                e.evidence.iter().any(|ev| {
                    matches!(
                        ev,
                        v2_compiler::std_termination::DescentEvidence::DescentUnknown
                    )
                })
            })
            .collect();
    ...
    if let Some(entry) = entry {
        ...
        let path_calls = v2_compiler::v2_compiler_complexity::max_path_self_calls(
            entry.body.clone(),
            "apply_named_template_nested".to_string(),
            Rc::new(HashMap::new()),
        );
