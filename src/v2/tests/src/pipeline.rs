...
// ── KF-8: Optimality gate regression tests ──────────────────────────────

#[test]
fn optimality_gate_linear_no_violation() {
    // O(n) catamorphism on a list — optimal. No violation expected.
    let source = r#"module opt_ok

type MyList<T> = Nil | Cons { head: T, tail: MyList<T> }

fn sum(xs: MyList<Int>) -> Int {
  match xs {
    Nil => 0
    Cons { head: h, tail: rest } => h + sum(xs: rest)
  }
}
"#;
    let complexity = compile_dag_with_complexity(source);
    let opt_violations: Vec<_> = complexity.violations.iter()
        .filter(|v| v.reason.contains("optimality"))
        .collect();
    assert!(opt_violations.is_empty(), "O(n) function should have no optimality violation");
}

#[test]
fn optimality_gate_log_no_violation() {
    // O(log n) binary search — better than O(n). No violation expected.
    let source = r#"module opt_log
...
"#;
    ...
    let opt_violations: Vec<_> = complexity.violations.iter()
        .filter(|v| v.reason.contains("optimality"))
        .collect();
    assert!(opt_violations.is_empty(), "O(log n) function should have no optimality violation");
}

#[test]
fn optimality_gate_infra_cost_dominates() {
    // Directly verify the cost ordering: O(log n) < O(n) < O(n^2) < O(Forever)
    use v2_compiler::std_induction::*;
    let o1 = Rc::new(CostBound::ConstantBound);
    let ologn = cost_log("n".to_string());
    let on = cost_linear("n".to_string());
    let on2 = cost_poly("n".to_string(), 2);
    let onlogn = cost_nlogn("n".to_string());
    let forever = Rc::new(CostBound::ForeverBound);

    assert!(cost_dominates(o1.clone(), ologn.clone()), "O(1) < O(log n)");
    assert!(cost_dominates(ologn.clone(), on.clone()), "O(log n) < O(n)");
    assert!(cost_dominates(on.clone(), onlogn.clone()), "O(n) < O(n log n)");
    assert!(cost_dominates(onlogn.clone(), on2.clone()), "O(n log n) < O(n^2)");
    assert!(cost_dominates(on2.clone(), forever.clone()), "O(n^2) < O(Forever)");
    // Not dominated in the wrong direction
    assert!(!cost_dominates(on.clone(), ologn.clone()), "O(n) does NOT dominate O(log n)");
    assert!(!cost_dominates(on.clone(), on.clone()), "O(n) does NOT strictly dominate itself");
}
