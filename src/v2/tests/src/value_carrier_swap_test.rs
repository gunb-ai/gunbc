...
//!
//! The v4.std.value_carrier sharing/identity laws have executable witnesses
//! in src/v4/test/claim/std_grounding/value_carrier_laws.dag; those run at
...
fn run_bool(src: &str, entry: &str) {
    ...
    match v2_interpreter::run(graph, resolved.source_indices.clone(), entry) {
        Ok(Value::Bool(true)) => {}
...
"#
    ...
    let graph = resolved.graph.as_ref().expect("graph");
    let first =
        v2_interpreter::run(graph, resolved.source_indices.clone(), "build").expect("first build");
    let second =
        v2_interpreter::run(graph, resolved.source_indices.clone(), "build").expect("second build");
    match (&first, &second) {
