...
fn coproduct_reflection_std_node_bridge_fns_are_intercept_wired() {
    ...
    for name in v2_interpreter::std_node_query_bridge_fn_names() {
        assert!(
            source.contains(&format!("\"{name}\" =>")),
            "eval_call intercept must wire v4.std.node_query bridge `{name}`"
        );
    }
    assert!(
    ...
    assert!(
        ...
        "eval_call must gate v4.std.node_query bridge dispatch"
    );
}
...
/// Regression for #4951: `kernel_binding_symbol` in `marshal_type_expr_ref`
...
fn marshal_type_expr_ref_preserves_authored_names_for_claim_corpus() {
    ...
    let end = rest
        .find("\nfn kernel_binding_symbol")
        .expect("kernel_binding_symbol sibling");
    let body = &rest[..end];
    assert!(
