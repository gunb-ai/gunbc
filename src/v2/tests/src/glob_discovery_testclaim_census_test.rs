...
/// Extract the roster's witness *function* names from `function: "..."` rows. This is the
/// hand run-roster the brief asked to equate with discovered `TestClaimRun` decls.
fn roster_function_names() -> BTreeSet<String> {
...
fn glob_discovery_testclaim_census_and_d2_contradiction() {
    ...
    let roster = roster_function_names();
    ...
    println!("v4_roster_pilot witness fns     : {}", roster.len());
    ...
    let intersection: Vec<&String> = testclaimrun.intersection(&roster).collect();
    ...
    assert!(
        roster.len() >= 30,
        "roster witness-fn extraction degenerate: found only {} (expected ~39)",
        roster.len()
    );
}
