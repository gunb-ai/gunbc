//! Generated by regen_stage0 -- do not edit.
//!
//! Crate boundary for generated v2 stage0 core authorities.
//!
//! The module bodies remain generated into `src/v2/stage0/src`. This crate
//! exposes the lower std/runtime/artifact/language/infer support surface
//! without copying generated module bodies. The root `NonEmpty*` wrappers are
//! mirrored from the stage0 lib root because generated modules address them as
//! `crate::NonEmptyVec` and `crate::NonEmptyBTreeSet`.

#![allow(
    unused_imports,
    unused_variables,
    unused_mut,
    unused_parens,
    dead_code,
    unreachable_patterns,
    non_shorthand_field_patterns,
    suspicious_double_ref_op,
    clippy::all
)]

#[path = "../../stage0/src/extdeps_languages_dag_emit.rs"]
pub mod extdeps_languages_dag_emit;
#[path = "../../stage0/src/extdeps_languages_dag_syntax.rs"]
pub mod extdeps_languages_dag_syntax;
#[path = "../../stage0/src/extdeps_languages_dag_types.rs"]
pub mod extdeps_languages_dag_types;
#[path = "../../stage0/src/extdeps_languages_go_emit.rs"]
pub mod extdeps_languages_go_emit;
#[path = "../../stage0/src/extdeps_languages_go_syntax.rs"]
pub mod extdeps_languages_go_syntax;
#[path = "../../stage0/src/extdeps_languages_go_types.rs"]
pub mod extdeps_languages_go_types;
#[path = "../../stage0/src/extdeps_languages_python_emit.rs"]
pub mod extdeps_languages_python_emit;
#[path = "../../stage0/src/extdeps_languages_python_syntax.rs"]
pub mod extdeps_languages_python_syntax;
#[path = "../../stage0/src/extdeps_languages_python_types.rs"]
pub mod extdeps_languages_python_types;
#[path = "../../stage0/src/extdeps_languages_rust_emit.rs"]
pub mod extdeps_languages_rust_emit;
#[path = "../../stage0/src/extdeps_languages_rust_syntax.rs"]
pub mod extdeps_languages_rust_syntax;
#[path = "../../stage0/src/extdeps_languages_rust_types.rs"]
pub mod extdeps_languages_rust_types;
#[path = "../../stage0/src/std_algebra.rs"]
pub mod std_algebra;
#[path = "../../stage0/src/std_coercion.rs"]
pub mod std_coercion;
#[path = "../../stage0/src/std_computation.rs"]
pub mod std_computation;
#[path = "../../stage0/src/std_effects.rs"]
pub mod std_effects;
#[path = "../../stage0/src/std_error_primitives.rs"]
pub mod std_error_primitives;
#[path = "../../stage0/src/std_graph.rs"]
pub mod std_graph;
#[path = "../../stage0/src/std_http_path.rs"]
pub mod std_http_path;
#[path = "../../stage0/src/std_induction.rs"]
pub mod std_induction;
#[path = "../../stage0/src/std_iteration.rs"]
pub mod std_iteration;
#[path = "../../stage0/src/std_node.rs"]
pub mod std_node;
#[path = "../../stage0/src/std_syntax.rs"]
pub mod std_syntax;
#[path = "../../stage0/src/std_termination.rs"]
pub mod std_termination;
#[path = "../../stage0/src/std_types.rs"]
pub mod std_types;
#[path = "../../stage0/src/v2_compiler_artifact.rs"]
pub mod v2_compiler_artifact;
#[path = "../../stage0/src/v2_compiler_infer_emit_info.rs"]
pub mod v2_compiler_infer_emit_info;
#[path = "../../stage0/src/v2_compiler_infer_env.rs"]
pub mod v2_compiler_infer_env;
#[path = "../../stage0/src/v2_compiler_infer_items.rs"]
pub mod v2_compiler_infer_items;
#[path = "../../stage0/src/v2_compiler_infer_service.rs"]
pub mod v2_compiler_infer_service;
#[path = "../../stage0/src/v2_compiler_infer_sigs.rs"]
pub mod v2_compiler_infer_sigs;
#[path = "../../stage0/src/v2_compiler_infer_types.rs"]
pub mod v2_compiler_infer_types;
#[path = "../../stage0/src/v2_compiler_languages.rs"]
pub mod v2_compiler_languages;
#[path = "../../stage0/src/v2_rt.rs"]
pub mod v2_rt;
#[path = "../../stage0/src/v2_std_core.rs"]
pub mod v2_std_core;

#[derive(Debug, Clone, PartialEq)]
pub struct NonEmptyVec<T>(Vec<T>);

impl<T> NonEmptyVec<T> {
    pub fn new(items: Vec<T>) -> Result<Self, &'static str> {
        if items.is_empty() {
            Err("NonEmptyVec requires at least one element")
        } else {
            Ok(Self(items))
        }
    }

    pub fn as_slice(&self) -> &[T] {
        &self.0
    }

    pub fn into_vec(self) -> Vec<T> {
        self.0
    }
}

#[derive(Debug, Clone, PartialEq)]
pub struct NonEmptyBTreeSet<T: Ord>(std::collections::BTreeSet<T>);

impl<T: Ord> NonEmptyBTreeSet<T> {
    pub fn new(items: std::collections::BTreeSet<T>) -> Result<Self, &'static str> {
        if items.is_empty() {
            Err("NonEmptyBTreeSet requires at least one element")
        } else {
            Ok(Self(items))
        }
    }

    pub fn as_set(&self) -> &std::collections::BTreeSet<T> {
        &self.0
    }

    pub fn into_set(self) -> std::collections::BTreeSet<T> {
        self.0
    }
}
