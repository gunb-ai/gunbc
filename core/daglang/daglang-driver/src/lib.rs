...
use daglang_emit::rust_exec_runtime::emit_exec_runtime_via_ir;
use daglang_emit::{
...
fn emit_with_options(
    ...
    match (options.target, options.layer) {
        ...
        (CodegenTarget::Rust, CodegenLayer::ExecRuntime) => {
            ...
            let files = emit_exec_runtime_via_ir(dag, module_name).map_err(|error| {
...
mod tests {
    use super::*;
    use daglang_emit::rust_exec_runtime::{emit_exec_runtime, emit_exec_runtime_via_ir};
...
"#,
    ...
    // =======================================================================
    // D2.2 — Verify IR path produces structurally equivalent output
    // =======================================================================

    /// D2.2 — Verify that the IR path for makegen produces structurally
    /// equivalent output to the D1 standalone path.
    #[test]
    fn makegen_ir_path_structurally_equivalent_to_d1() {
        let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../..");
        let root = workspace.join("dsl");
        let file = root.join("tools/makegen.dag");
        ...
        // Get the module name (same logic as emit_with_options).
        ...
        let d1_files = emit_exec_runtime(&output.lowered_dag, module_name)
            .expect("D1 emit should succeed");
        let ir_files = emit_exec_runtime_via_ir(&output.lowered_dag, module_name)
            .expect("IR emit should succeed");
        ...
        assert_eq!(
            d1_nodes, ir_nodes,
            "same number of add_node calls (D1={d1_nodes} IR={ir_nodes})"
        );
        ...
        assert_eq!(
            d1_edges, ir_edges,
            "same number of add_edge calls (D1={d1_edges} IR={ir_edges})"
        );

        // ---- All DAG node IDs present in both ----
        for node in &output.lowered_dag.nodes {
            assert!(
                ir_main.contains(&node.id.0),
                "IR path should reference node `{}`",
                node.id.0
            );
        }
        ...
        for variant in &[
            ...
            assert!(
                ir_main.contains(variant),
                "IR path should contain handler variant `{variant}`"
            );
        }
        ...
        assert!(ir_main.contains("execute_with_mode_and_inputs"));

        // ---- Same handler body semantics ----
        assert!(
            ir_main.contains("Generated by daglang"),
            "render_makefile handler should contain Makefile header"
        );
        assert!(
            ir_main.contains("Value::Skipped"),
            "execute_transport handler should handle skip"
        );
        assert!(
            ir_main.contains("fn execute_file_request"),
            "should include file request helper"
        );
    }

    /// D2.2 — Verify that the IR path for pragma produces structurally
    /// equivalent output to the D1 standalone path.
    #[test]
    fn pragma_ir_path_structurally_equivalent_to_d1() {
        let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../..");
        let root = workspace.join("dsl");
        let file = root.join("tools/pragma.dag");
        ...
        assert_eq!(d1_files[1].content, ir_files[1].content);
        ...
        // ---- Pragma helpers ----
        assert!(ir_main.contains("PragmaDirectiveRuntime"));
        assert!(ir_main.contains("parse_pragma_directives"));

        // ---- Same handler body semantics ----
        assert!(ir_main.contains("clippy"));
        assert!(ir_main.contains("disallowed_method"));
        assert!(ir_main.contains("lint"));
    }
}
