...
pub fn emit_transport_mock_tests(backend: &str, dag: &Dag<LoweredOp>) -> Option<EmittedFile> {
    let mut transport_nodes = dag
        .nodes
        .iter()
        .filter_map(|node| match classify_computation(node) {
            Ok(Computation::Transport { .. }) => Some(node.id.0.clone()),
            _ => None,
        })
        .collect::<Vec<_>>();
    transport_nodes.sort();
    transport_nodes.dedup();

    if transport_nodes.is_empty() {
        return None;
    }
    ...
    match backend {
        "rust" => {
            ...
            for node_id in &transport_nodes {
                let test_name = sanitize_identifier(&format!("mock_transport_{node_id}"));
                let _ = writeln!(
                    content,
                    "#[test]\nfn {test_name}() {{\n    let mut mocks = std::collections::BTreeMap::new();\n    mocks.insert(\"{node_id}\", \"mock-response\");\n    assert_eq!(mocks.get(\"{node_id}\"), Some(&\"mock-response\"));\n}}\n"
                );
            }
            Some(EmittedFile {
        ...
        "go" => {
            ...
            for node_id in &transport_nodes {
                ...
                let _ = writeln!(
                    content,
                    "func {test_name}(t *testing.T) {{\n    mocks := map[string]string{{\"{node_id}\": \"mock-response\"}}\n    got, ok := mocks[\"{node_id}\"]\n    if !ok || got != \"mock-response\" {{\n        t.Fatalf(\"missing mock for {node_id}\")\n    }}\n}}\n"
                );
            }
            Some(EmittedFile {
        ...
        "c" => {
            ...
            for node_id in &transport_nodes {
                ...
                let _ = writeln!(
                    content,
                    "    const char* {var_name} = \"mock-response\";\n    assert(strcmp({var_name}, \"mock-response\") == 0);"
                );
            }
...
mod tests {
    ...
    fn emit_rust_transport_mock_tests_per_transport_node() {
        ...
        assert!(emitted.content.contains("#[test]"));
        assert!(emitted.content.contains("mock-response"));
    }
