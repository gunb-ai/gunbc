...
fn makegen_e2e_generated_binary_produces_correct_makefile() {
    ...
    let run_output = Command::new(out_dir.join("target/debug/tools-makegen"))
        ...
        // Lowered layer-1 runtime currently exposes the makegen path both on
        // the callable entrypoint and the synthesized param_source node.
        .arg(makefile_path.to_string_lossy().as_ref())
        .output()
    ...
    assert!(
        ...
        "Makefile should contain makegen generator header"
    );
    assert!(
