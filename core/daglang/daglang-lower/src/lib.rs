...
fn add_service_call_edges(
    ...
    // Track transport endpoint usage across ALL callables in ALL modules so
    ...
    // edges to the original.  Previously this was per-module, which meant
    // two modules calling the same operation both wired to the shared
    // prepare node — causing "multiple upstream edges" at execution time
    // (BT-E1).
