...
fn add_service_call_edges(
    ...
    // Track transport endpoint usage across ALL modules so that any callable
    // referencing a service operation already wired by a prior module gets a
    // cloned triplet (_c1, _c2, …) instead of wiring duplicate scalar edges
    // to the original.
