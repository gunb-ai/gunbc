...
use gunbc_exec::{execute_dag, ExecuteConfig, Executable, ExecutionLog, ExecutionMode, LogEntry};
...
pub fn build_corpus_report<T: Executable + Clone + Send>(
    ...
    for (info, dag, mock_spec) in workflows {
        ...
        let log = match execute_dag(*dag, ExecuteConfig { mode, ..Default::default() }) {
            Ok(log) => log,
            Err(err) => {
