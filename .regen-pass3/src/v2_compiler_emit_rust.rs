...
pub use crate::v2_compiler_ownership::{OwnershipProof, FoldAccUnwrapProof, analyze_ownership, build_movable_set, build_last_use_set, build_read_only_params};
...
pub struct OwnershipBuildResult {
    ...
    pub read_only_index: Rc<HashMap<String, Rc<HashMap<String, bool>>>>,
}
...
pub fn build_ownership_results(modules: Rc<Vec<Rc<TypedModule>>>) -> Rc<OwnershipBuildResult> {
    {
        let proofs = Rc::new({ let mut __result = Vec::new(); for m in modules.iter().cloned() { __result.extend((*Rc::new({ let mut __result = Vec::new(); for item in Rc::new({ let mut __result = Vec::new(); for item in m.items.clone().iter().cloned() { if (item.body.clone() != None) { __result.push(item); } } __result }).iter().cloned() { __result.push(Rc::new(OwnershipProofEntry {
    name: item.name.clone(),
    proof: analyze_ownership(item.name.clone(), item.params.clone(), item.body.clone().clone().unwrap(), m.type_env.clone().source_index.clone()),
...
proofs.iter().cloned().fold(Rc::new(OwnershipBuildResult {
    ...
    read_only_index: v2_rt::rc_empty_map::<Rc<HashMap<String, bool>>>(),
    ...
    last_use_index: v2_rt::rc_map_insert(acc.last_use_index, entry.name.clone(), build_last_use_set(entry.proof.clone())),
    read_only_index: v2_rt::rc_map_insert(acc.read_only_index, entry.name.clone(), build_read_only_params(entry.proof.clone())),
}) })
}
}
...
let emit_info = Rc::new(EmitGraphInfo {
    ...
    last_use_index: ownership.last_use_index.clone(),
    last_use_spans: v2_rt::rc_empty_map::<i64>(),
    read_only_index: v2_rt::rc_empty_map::<Rc<HashMap<String, bool>>>(),
    read_only_params: v2_rt::rc_empty_map::<bool>(),
});
let filtered_ro_index = typed.modules.clone().iter().cloned().fold(v2_rt::rc_empty_map::<Rc<HashMap<String, bool>>>(), |acc: Rc<HashMap<String, Rc<HashMap<String, bool>>>>, m: Rc<TypedModule>| Rc::new({ let mut __result = Vec::new(); for item in m.items.clone().iter().cloned() { if (item.body.clone() != None) { __result.push(item); } } __result }).iter().cloned().fold(acc.clone(), |acc2: Rc<HashMap<String, Rc<HashMap<String, bool>>>>, item: Rc<Node>| {
            let raw_ro = match v2_rt::map_get(&ownership.read_only_index.clone(), item.name.clone()) {
    Some(ro) => ro.clone(),
    None => v2_rt::rc_empty_map::<bool>(),
};
let filtered_ro = item.params.clone().iter().cloned().fold(v2_rt::rc_empty_map::<bool>(), |ro_acc: Rc<HashMap<String, bool>>, p: Rc<Node>| {
                let pname = param_node_name_at(p.clone(), m.type_env.clone().source_index.clone());
let is_ro = emit_map_has(raw_ro.clone(), pname.clone());
let is_shared = is_shared_param_type(p.clone(), shared.clone());
if (is_ro.clone() && is_shared.clone()) {
                    v2_rt::rc_map_insert(ro_acc.clone(), pname.clone(), true)
} else {
                    ro_acc.clone()
}
});
if ((Rc::new(v2_rt::map_keys(&filtered_ro)).len() as i64) > 0) {
                v2_rt::rc_map_insert(acc2.clone(), item.name.clone(), filtered_ro.clone())
} else {
                acc2.clone()
}
}));
let emit_info = Rc::new(EmitGraphInfo {
    ...
    last_use_index: emit_info.last_use_index.clone(),
    last_use_spans: emit_info.last_use_spans.clone(),
    read_only_index: filtered_ro_index,
...
let emit_info = Rc::new(EmitGraphInfo {
    ...
    fold_eligible: v2_rt::rc_empty_map::<bool>(),
    last_use_index: base_info.last_use_index.clone(),
    last_use_spans: v2_rt::rc_empty_map::<i64>(),
    read_only_index: base_info.read_only_index.clone(),
    read_only_params: v2_rt::rc_empty_map::<bool>(),
});
let shared_types = emit_info.shared_types.clone();
...
if is_type_def_item(item.clone()) {
            ...
            if is_type_alias_item(item.clone(), env.source_index.clone()) {
                v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(rust_visibility_prefix(), rust_items().type_alias_keyword.clone()), " ".to_string()), item_text), " = ".to_string()), render_rust_type(resolved_type(item.clone()), shared_types, env.source_index.clone())), ";".to_string())
} else {
                if is_type_decl_item(item.clone(), env.source_index.clone()) {
                    ...
                    if is_function_item(item.clone()) {
                        {
                            let fn_movable = match v2_rt::map_get(&emit_info.ownership_index.clone(), item.name.clone()) {
    Some(m) => m.clone(),
    None => v2_rt::rc_empty_map::<bool>(),
};
...
let fn_last_use = match v2_rt::map_get(&emit_info.last_use_index.clone(), item.name.clone()) {
    Some(m) => m.clone(),
    None => v2_rt::rc_empty_map::<i64>(),
};
let fn_read_only = match v2_rt::map_get(&emit_info.read_only_index.clone(), item.name.clone()) {
    Some(m) => m.clone(),
    None => v2_rt::rc_empty_map::<bool>(),
};
let fn_emit_info = Rc::new(EmitGraphInfo {
    ...
    last_use_index: emit_info.last_use_index.clone(),
    last_use_spans: fn_last_use,
    read_only_index: emit_info.read_only_index.clone(),
...
if is_effectful {
                                emit_func_def(item_text, item.params.clone(), resolved_type(item.clone()), item.uses.clone(), item.body.clone().clone().unwrap(), registry.clone(), scope.clone(), shared_types, fn_emit_info)
} else {
                                emit_fn_def(item_text, item.params.clone(), resolved_type(item.clone()), item.body.clone().clone().unwrap(), registry.clone(), scope.clone(), shared_types, fn_emit_info)
}
}
} else {
                        if is_data_def_item(item.clone()) {
                            emit_data_def(item_text, item.type_annotation.clone().clone().unwrap(), item.body.clone().clone().unwrap(), registry.clone(), scope.clone(), 0, shared_types, emit_info.clone())
} else {
...
let rename_attr = match Rc::new({ let mut __result = Vec::new(); for p in child.properties.clone().iter().cloned() { if (field_init_node_name_at(p.clone(), env.source_index.clone()).as_str() == "from_key".to_string().as_str()) { __result.push(p); } } __result }).first().cloned() {
...
let enum_def = v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat(derives, "\n".to_string()), rust_serde_tag_attr()), "\n".to_string()), rust_visibility_prefix()), rust_items().enum_keyword.clone()), " ".to_string()), name.clone()), type_params.clone()), " {\n".to_string()), variants_str), "\n}".to_string());
...
let fn_read_only = if use_tco.clone() {
            v2_rt::rc_empty_map::<bool>()
} else {
            emit_info.read_only_params.clone()
};
let params_str = emit_params(params.clone(), shared_types.clone(), si.clone(), fn_read_only);
...
let params_str = emit_func_params(params.clone(), uses.clone(), service_names, shared_types.clone(), scope.type_env.clone().source_index.clone(), emit_info.read_only_params.clone());
...
let body_str = emit_func_body(body, registry.clone(), body_scope, (depth.clone() + 1), shared_types.clone(), emit_info.clone());
...
v2_rt::concat(v2_rt::concat(v2_rt::concat("mut ".to_string(), emit_ident(param_node_name_at(param.clone(), source_index.clone()), RenderTarget::Rust)), rust_items().param_type_sep.clone()), ty)
}
}

pub fn emit_func_params(params: Rc<Vec<Rc<Node>>>, uses: Rc<Vec<Rc<Node>>>, service_names: Rc<Vec<String>>, shared_types: Rc<HashMap<String, bool>>, source_index: Option<Rc<NewlineIndex>>, read_only_params: Rc<HashMap<String, bool>>) -> String {
    {
        let param_strs = Rc::new({ let mut __result = Vec::new(); for p in params.iter().cloned() { __result.push(emit_param(p.clone(), shared_types.clone(), source_index.clone(), read_only_params.clone())); } __result });
...
pub fn emit_params(params: Rc<Vec<Rc<Node>>>, shared_types: Rc<HashMap<String, bool>>, source_index: Option<Rc<NewlineIndex>>, read_only_params: Rc<HashMap<String, bool>>) -> String {
    {
        let strs = Rc::new({ let mut __result = Vec::new(); for p in params.iter().cloned() { __result.push(emit_param(p.clone(), shared_types.clone(), source_index.clone(), read_only_params.clone())); } __result });
strs.join(&", ".to_string())
}
}
...
pub fn is_shared_param_type(param: Rc<Node>, shared_types: Rc<HashMap<String, bool>>) -> bool {
    {
        let type_expr = param_node_type_expr(param);
let type_name = type_expr.name.clone();
let resolved_name = match type_expr.inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: rt, .. }) => rt.name.clone(),
    _ => type_name.clone(),
};
(emit_map_has(shared_types.clone(), type_name.clone()) || emit_map_has(shared_types.clone(), resolved_name))
}
}

pub fn emit_param(param: Rc<Node>, shared_types: Rc<HashMap<String, bool>>, source_index: Option<Rc<NewlineIndex>>, read_only_params: Rc<HashMap<String, bool>>) -> String {
    {
        let n = param_node_type_expr(param.clone());
let ty = emit_rust_param_type(n, shared_types.clone(), source_index.clone());
let pname = param_node_name_at(param.clone(), source_index.clone());
let is_read_only = emit_map_has(read_only_params, pname.clone());
let is_shared = is_shared_param_type(param.clone(), shared_types.clone());
let prefix = if (is_read_only && is_shared) {
            "&".to_string()
} else {
            "".to_string()
};
v2_rt::concat(v2_rt::concat(v2_rt::concat(emit_ident(pname.clone(), RenderTarget::Rust), rust_items().param_type_sep.clone()), prefix), ty)
}
}
...
if is_fielded2 {
...
} else {
                        {
                            let binding_strs = Rc::new({ let mut __result = Vec::new(); for fb in effective_bindings.clone().iter().cloned() { __result.push({
                                let fb_name = field_binding_name_at(fb.clone(), source_index.clone());
let fb_pat = field_binding_pattern(fb.clone());
if is_string_lit_pattern(fb_pat.clone()) {
...
pub fn analyze_rc_pattern(pattern: Rc<MatchPattern>, scrut_type: String, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>, source_index: Option<Rc<NewlineIndex>>) -> Rc<RcPatternAnalysis> {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*pattern).clone() {
    MatchPattern::VariantPattern { name: n, parent_enum, field_bindings: fbs, .. } => if ((n.clone().as_str() == "Some".to_string().as_str()) || (n.clone().as_str() == "None".to_string().as_str())) {
            if ((fbs.clone().len() as i64) == 1) {
                match fbs.clone().first().cloned() {
    Some(fb) => {
                    let inner = analyze_rc_pattern(field_binding_pattern(fb.clone()), "".to_string(), shared_types.clone(), emit_info.clone(), source_index.clone());
Rc::new(RcPatternAnalysis {
    matches_rc_variant: false,
...
let ref_bound_fields = Rc::new({ let mut __result = Vec::new(); for fb in fbs.clone().iter().cloned() { __result.extend((*if analyze_rc_pattern(field_binding_pattern(fb.clone()), "".to_string(), shared_types.clone(), emit_info.clone(), source_index.clone()).matches_rc_variant.clone() {
                    Rc::new(vec![field_binding_name_at(fb.clone(), source_index.clone())])
} else {
                    Rc::new(vec![])
}).iter().cloned()); } __result });
Rc::new(RcPatternAnalysis {
...
pub fn analyze_rc_match(scrutinee: Rc<Node>, arms: Rc<Vec<Rc<Node>>>, scrut_type: String, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>, source_index: Option<Rc<NewlineIndex>>) -> RcMatchAnalysis {
    {
        let arm_analyses = Rc::new({ let mut __result = Vec::new(); for arm in arms.iter().cloned() { __result.push(analyze_rc_pattern(arm_pattern(arm.clone()), scrut_type.clone(), shared_types.clone(), emit_info.clone(), source_index.clone())); } __result });
...
if is_string_lit_pattern(fb_pat.clone()) {
                    ...
                    {
                        let inner_analysis = analyze_rc_pattern(fb_pat.clone(), scrut_type.clone(), shared_types.clone(), emit_info.clone(), source_index.clone());
...
if is_shorthand.clone() {
                                                ...
                                                {
                                                    let inner_analysis = analyze_rc_pattern(fb_pat.clone(), "".to_string(), shared_types.clone(), emit_info.clone(), source_index.clone());
...
pub fn emit_var_ref(name: String, binding_kind: Option<Rc<VarBindingKind>>, resolved_type: Option<Rc<InferredNode>>, shared_types: Rc<HashMap<String, bool>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, emit_info: Rc<EmitGraphInfo>, span_start: i64) -> String {
    if ((name.clone().as_str() == "none".to_string().as_str()) || (name.clone().as_str() == "None".to_string().as_str())) {
        ...
        if ((name.clone().as_str() == "true".to_string().as_str()) || (name.clone().as_str() == "false".to_string().as_str())) {
            ...
            {
                let sharing = language_spec(RenderTarget::Rust).sharing.clone();
let is_borrowed_param = emit_map_has(emit_info.read_only_params.clone(), name.clone());
let is_last_use = if is_borrowed_param.clone() {
                    false
} else {
                    match v2_rt::map_get(&emit_info.last_use_spans.clone(), name.clone()) {
    Some(last_span) => ((last_span.clone() == span_start.clone()) && (span_start.clone() > 0)),
    None => false,
}
};
...
let is_movable = if is_borrowed_param.clone() {
                                false
} else {
                                (emit_map_has(emit_info.movable.clone(), name.clone()) || is_last_use)
};
...
match (*texpr.expr_data.clone()).clone() {
    ExprData::ExprVar { binding_kind, .. } => {
            let n = expr_var_name_at(texpr.clone(), si);
...
match summary {
    ...
    ExprData::ExprVar { .. } => {
                        let base_name = expr_var_name_at(base.clone(), scope.type_env.clone().source_index.clone());
...
emit_var_ref(n, binding_kind.clone(), expr.inferred.clone(), shared_types, registry, emit_info, expr.span.clone().start.clone())
},
...
emit_typed_method_call(r, m, a, expr.inferred.clone(), method_semantics.clone(), registry, scope.clone(), depth, shared_types, emit_info)
},
...
emit_typed_let(n, v, bd, registry, scope.clone(), depth, shared_types, emit_info)
},
...
pub fn emit_borrow_arg(texpr: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>, depth: i64) -> String {
    match (*texpr.expr_data.clone()).clone() {
    ExprData::ExprVar { binding_kind, .. } => {
        let n = expr_var_name_at(texpr.clone(), scope.type_env.clone().source_index.clone());
if ((((n.clone().as_str() == "none".to_string().as_str()) || (n.clone().as_str() == "None".to_string().as_str())) || (n.clone().as_str() == "true".to_string().as_str())) || (n.clone().as_str() == "false".to_string().as_str())) {
            emit_var_ref(n.clone(), binding_kind.clone(), texpr.inferred.clone(), shared_types, registry, emit_info, texpr.span.clone().start.clone())
} else {
            {
                let ident = emit_ident(n.clone(), RenderTarget::Rust);
let caller_param_borrowed = emit_map_has(emit_info.read_only_params.clone(), n.clone());
if caller_param_borrowed {
                    ident
} else {
                    v2_rt::concat("&".to_string(), ident)
}
}
}
},
    _ => {
        let expr_str = emit_typed_expr(texpr.clone(), registry, scope, depth, shared_types, emit_info, 1024);
...
pub fn is_map_typed_expr(texpr: Rc<Node>, source_index: Option<Rc<NewlineIndex>>) -> bool {
    match texpr.inferred.clone().as_deref().cloned() {
    Some(InferredNode::Resolved { node: n, .. }) => node_is_keyed_collection(n.clone(), source_index),
    _ => false,
}
}
...
match call_args.last().cloned() {
    ...
    ExprData::ExprLambda { semantics, .. } => {
                    let lps = lambda_param_names_at(arg_value(a.clone()), scope.type_env.clone().source_index.clone());
...
let callee_read_only = if is_rt.clone() {
            v2_rt::rc_empty_map::<bool>()
} else {
            match v2_rt::map_get(&emit_info.read_only_index.clone(), func.clone()) {
    Some(m) => m.clone(),
    None => v2_rt::rc_empty_map::<bool>(),
}
};
...
v2_rt::concat("&".to_string(), base.clone())
                ...
                {
                    let aname = match arg_name_at(a.clone(), collection_scope.type_env.clone().source_index.clone()) {
    Some(n) => n.clone(),
    None => "".to_string(),
};
let callee_param_read_only = emit_map_has(callee_read_only.clone(), aname.clone());
if callee_param_read_only.clone() {
                        emit_borrow_arg(arg_value(a.clone()), registry.clone(), collection_scope.clone(), shared_types.clone(), emit_info.clone(), depth.clone())
} else {
...
extend_scope(acc.clone(), param_name.clone(), param_type.clone(), Rc::new(SubValueRelation::SubValueUnknown))
}),
    None => scope,
}
}
...
let acc_param_name = match (*fold_lambda_node.expr_data.clone()).clone() {
    ExprData::ExprLambda { .. } => match lambda_param_names_at(fold_lambda_node.clone(), scope.type_env.clone().source_index.clone()).first().cloned() {
    Some(n) => n.clone(),
    None => "".to_string(),
},
    _ => "".to_string(),
...
let init_str = match args.clone().first().cloned() {
    ...
    ExprData::ExprCall { .. } => {
            let init_func = expr_call_func_at(arg_value(init_arg.clone()), scope.type_env.clone().source_index.clone());
...
if is_map_typed_expr(receiver.clone(), scope.type_env.clone().source_index.clone()) {
...
pub fn emit_rust_first_method_call(receiver: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    ...
    ExprData::ExprMethodCall { .. } => if (expr_method_name_at(receiver.clone(), scope.type_env.clone().source_index.clone()).as_str() == "skip".to_string().as_str()) {
        {
...
let rc_match = analyze_rc_match(scrutinee.clone(), arms.clone(), scrut_type.clone(), shared_types.clone(), emit_info.clone(), scope.type_env.clone().source_index.clone());
...
let si = scope.type_env.clone().source_index.clone();
let rc_analysis = analyze_rc_pattern(arm_pat.clone(), scrut_type.clone(), shared_types.clone(), emit_info.clone(), si.clone());
...
})), shared_types.clone(), registry.clone(), emit_info.clone(), arm_b.span.clone().start.clone())
} else {
...
if (is_product && (resolved_type.ident_span.clone() == None)) {
    ...
    None => "compile_error!(\"empty anonymous record literal\")".to_string(),
                    ...
                    {
                        let lit_field_names = Rc::new({ let mut __result = Vec::new(); for f in fields.clone().iter().cloned() { __result.push(field_init_node_name_at(f.clone(), scope.type_env.clone().source_index.clone())); } __result });
...
pub fn emit_tco_init_stmt(stmt: Rc<Node>, params: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>) -> String {
    ...
    ExprData::ExprLet => {
        let n = let_binding_name_at(stmt.clone(), scope.type_env.clone().source_index.clone());
let v = let_value(stmt.clone());
let val_str = emit_typed_expr(v, registry, scope.clone(), depth, shared_types, emit_info, 1024);
let is_param = { let mut __found = false; for p in params.iter().cloned() { if (param_node_name_at(p.clone(), scope.type_env.clone().source_index.clone()).as_str() == n.clone().as_str()) { __found = true; break; } } __found };
if is_param {
    ...
    _ => emit_typed_expr(stmt.clone(), registry, scope.clone(), depth, shared_types, emit_info, 1024),
}
}
...
let rc_match = analyze_rc_match(s.clone(), arm_list.clone(), tco_scrut_type.clone(), shared_types.clone(), emit_info.clone(), frame.scope.clone().type_env.clone().source_index.clone());
...
let is_param = { let mut __found = false; for p in params.clone().iter().cloned() { if (param_node_name_at(p.clone(), frame.scope.clone().type_env.clone().source_index.clone()).as_str() == n.clone().as_str()) { __found = true; break; } } __found };
...
pub fn expr_references_var(node: Rc<Node>, var_name: String, source_index: Option<Rc<NewlineIndex>>) -> bool {
    stacker::maybe_grow(512 * 1024, 2 * 1024 * 1024, || {
        match (*node.expr_data.clone()).clone() {
    ExprData::ExprVar { .. } => (expr_var_name_at(node.clone(), source_index.clone()).as_str() == var_name.clone().as_str()),
    _ => { let mut __found = false; for c in node.children.clone().iter().cloned() { if expr_references_var(c.clone(), var_name.clone(), source_index.clone()) { __found = true; break; } } __found },
}
    })
}
...
pub fn emit_dry_run_branch_from_props(op_name: String, inferred: Rc<Node>, mock_props: Rc<Vec<Rc<Node>>>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, source_index: Option<Rc<NewlineIndex>>) -> String {
    {
...
match first_mock {
    Some(mp) => {
                    let mock_json = emit_data_value_json(field_init_node_value(mp.clone()), source_index);
...
let return_line = emit_exit_code_handling(op_node.clone(), inferred, source_index.clone());
...
if is_bool {
            ...
            {
                let __eff_is_container = node_is_element_collection(effective.clone(), source_index);
...
pub fn emit_data_def_body(type_node: Rc<Node>, value: Rc<Node>, registry: Rc<HashMap<String, Rc<ItemInfo>>>, scope: Rc<InferScope>, depth: i64, shared_types: Rc<HashMap<String, bool>>, emit_info: Rc<EmitGraphInfo>, needs_rc: bool) -> String {
    if (has_nested_records_node(type_node.clone(), scope.type_env.clone().source_index.clone()) && !data_value_has_cross_refs(value.clone())) {
        {
...
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("            __m.insert(\"".to_string(), field_init_node_name_at(f.clone(), scope.type_env.clone().source_index.clone())), "\".to_string(), ".to_string()), val_str.clone()), ");".to_string())
}); } __result });
...
pub fn emit_mock_prop_setup(mock_prop: Rc<Node>, depth: i64, source_index: Option<Rc<NewlineIndex>>) -> String {
    {
        let body_str = emit_simple_expr(field_init_node_value(mock_prop.clone()), RenderTarget::Rust, source_index.clone());
v2_rt::concat(v2_rt::concat(v2_rt::concat(v2_rt::concat("let ".to_string(), field_init_node_name_at(mock_prop.clone(), source_index.clone())), " = ".to_string()), body_str), ";".to_string())
}
}
