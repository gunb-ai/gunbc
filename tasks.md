...
| CP-47 | `RuntimeBindings` replaces `ExternResolver` trait | M | CP-40 | CP | **Bridge landed** — `RuntimeBindings` centralizes registration in `core/resolve/src/lib.rs` with typed `ProgramSymbolId` keys, and a single cached binding table now powers both `GunbcExternResolver` and `resolve_extern_symbol()`. The resolver still implements the old `ExternResolver` trait as a migration bridge; ExternId-keyed total binding remains open. |
