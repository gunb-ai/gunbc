...
| 4a | RT4a | **Complex return expression lowering.** `resolve_return_expr_source()` only handles Ident/FieldAccess/Call/Literal — `BinOp`, `UnaryOp`, `If`, `Match`, `Pipe` all fall through to `_ => None` (silent drop). Fix: synthesize compute nodes in IR for complex expressions, wire result to `__out:*`. Touches: `daglang-lower/src/lib.rs:7768-7841`. Test: `build.dag` `overall_success` evaluates to `Bool(true)` when all stages pass. Revert ci.rs workaround after. See POSTMORTEM below. | M | Done | — |
| 4b | RT4b | **Passthrough missing-input diagnostic.** `execute_with_declared_output_passthrough()` silently falls back to `Value::Skipped` when `__out:*` input is missing. Fix: emit diagnostic warning (or error in strict mode) when a declared output port has no wired input. Touches: `gunbc-dag/src/resolve.rs:71-91`. | S | Done | — |
| 4c | RT4c | **Lowering completeness gate.** `wire_callable_return_outputs()` silently `continue`s when `resolve_return_expr_source()` returns `None`. Fix: emit `LowerWarning` for unwired non-optional return outputs. Track unwired count as metric. Touches: `daglang-lower/src/lib.rs:7844-7902`. | S | Done | — |
...
| # | ID | What | Size | Status | Deps |
|---|-----|------|------|--------|------|
| 38 | RT38 | **No panics in lowering for user-authored DSL.** `derive_file_spec()` calls `FileOp::from_dsl_str(..).unwrap_or_else(\|\| panic!(...))` (`daglang-lower/src/lib.rs:5051-5052`). Replaced with `LowerError::InvalidFileOp` variant returning clean error message. | S | Done | — |
| 39 | RT39 | **`auth_input` must parse or error.** Parser at `daglang-syntax/src/parser.rs:338-343` silently advances on non-ident `auth_input` values. Replaced both `else { self.advance(); }` with `record_err("expected identifier for auth/auth_input")` + advance. Non-ident values now produce parse errors. | S | Done | — |
| 40 | RT40 | **Structural optionality for shell TrimStdout.** Added `is_optional: bool` field to `OutputFieldSpec`. Set during lowering from `TypeExpr::Optional`. Consumer in `resolve_service.rs` uses `f.is_optional` instead of `f.type_id.ends_with('?')`. Updated all construction sites across `service_emit.rs`, `resolve_service.rs`, `resolve.rs`. | S | Done | — |
...
| 42 | RT42 | **Pipe methods first-class in AST.** Added `PipeMethod` enum with 24 variants in `daglang-syntax/src/lib.rs` with `from_name()`/`as_str()` methods. Replaced 25-entry string allowlist in `should_track_call_name()` with `PipeMethod::from_name(name).is_some()`. Single authoritative registry. | M | Done | — |
| 43 | RT43 | **Nested field access in lowerer.** Updated `resolve_return_expr_source` to route nested `FieldAccess` through `synthesize_return_compute_node` (ReturnExprCompute path). Deleted `extract_secret` workaround from `credential.dag`. N-level field access (e.g. `cred.token.token`) now works via the compute node fallback. | M | Done | — |
| 44 | RT44 | **Expand `evaluate_fn_body` coverage.** Root cause: `local_auth()` body was lossy because parser rejected `?.` (optional chaining). Fixed by adding `?.` support to parser — desugars `expr?.field` to `if expr != None { expr.field } else { None }`. `local_auth()` now parses non-lossily; lowerer extracts transport nodes at compile time. Also: added `max_by` pipe method to evaluator, 14 new eval tests (null coalesce, variant construct, match on enum, record update, for, nested field access, etc.). | M | Done | — |
| 45 | RT45 | **Typed enum values from `evaluate_fn_body`.** Added `Value::Enum { variant: String }` to `value.rs`. Updated `VariantConstruct` in evaluator to emit `Value::Enum` for unit variants. Added `as_enum_variant()` helper. Updated all consumer sites: `fidelity.rs`, `value_bridge.rs`, `value_expr.rs`, `progress.rs`, `resolve.rs`. `match_pattern` handles both `Value::Enum` and legacy `Value::Str`. | M | Done | — |
| 46 | RT46 | **Delete fidelity silent fallbacks.** Replaced `unwrap_or(TestClass::Unit)` and `unwrap_or(FermiCost::XS)` with `unwrap_or_else(\|\| panic!(...))` with descriptive error messages. Classification failure now surfaces as a clear error indicating DSL evaluator bug. | S | Done | RT45 |
| 47 | RT47 | **Proper multi-param lambda parsing.** Extracted `try_parse_multi_param_lambda()` helper with read-only lookahead (no parser state mutation). Two-phase: scan for `(ident, ident, ...) =>` pattern, then commit. Simplified LParen parse branch from ~55 lines to ~10 lines. | M | Done | — |
...
| ~~Validation at use site~~ | ~~`GenericRestParseOp` doesn't check HTTP status code.~~ Already fixed: `rest.is_success()` checks status at entry (line 185). Non-success returns `Err` with status, body excerpt, and auth context. | ~~`gunbc-dag/src/resolve_service.rs`~~ | RT-I4 proof | **Done** |
...
| ~~Validation at use site~~ | ~~`GNUmakefile` had `--mode=ensure` flag.~~ Already fixed: line 26 runs `cargo run -p gunbc-dag --bin gunbc-bootstrap` without invalid flags. | ~~`GNUmakefile:26`~~ | CI fix | **Done** |
| ~~Validation at use site~~ | ~~`find crates -type d` in bootstrap returns exit 1 when `crates/` doesn't exist.~~ Fixed by RT-I4: SplitLines returns empty list on non-zero exit for list-producing ops. | ~~`dsl/tools/bootstrap.dag`, `gunbc-dag/src/resolve_service.rs:523-543`~~ | CI fix | **Done** |
...
| Heuristic reimplementation | `looks_effectful_without_kind()` uses port type strings to detect misclassified nodes. NOT dead code after RT6 — it's a validation safety net used by `validate_node_kinds_for_interception()`. Port type strings (`TransportRequest`, `ToolHandle`, etc.) are DSL type names, not easily enumifiable. Low priority. | `core/exec/src/execute/mod.rs:2062-2090` | RF-H4 PR scout | 2026-02-26 |
