...
| Source Repo | Location | What to Port |
|-------------|----------|-------------|
| `the-gunbai` | `docs/understandings/*.md` | 40 understandings: behavioral property structure (9 dims per op), secret providers (5), coordination stores (3), CLI tools (17) |
| `gunb.ai` | `tools/extdeps/proto/*.proto` | Rate limits, capability contracts, infrastructure scopes, API versioning, LLM model specs with pricing |
| `gunb.ai` | `tools/extdeps/*_api.go` | GCP/AWS/GitHub endpoint detail, retry configs, org policies, auth quirks |
...
| 7 | DM-7 | **`extdeps/secrets/gcp_secret_manager.dag`** — "What is GCP Secret Manager?" Types: `GcpSecret`, `GcpSecretVersion`, `GcpReplicationPolicy` (Automatic \| UserManaged). Data: 8 `OperationBehavior` records (AccessVersion, CreateSecret, AddVersion, ListSecrets, GetSecret, DestroyVersion, DisableVersion, EnableVersion), rate limits per method, propagation delays. Port from `the-gunbai/docs/understandings/gcp-secret-manager.md`. | M | Pending | DM-6 |
| 8 | DM-8 | **`extdeps/secrets/github_secrets.dag`** — "What is GitHub Secrets?" Types: `GitHubSecretScope` (Repository \| Environment \| Organization), `GitHubPublicKey` { key_id, key }, `OidcTokenClaims`. Data: 8 `OperationBehavior` records (Get, Create, Update, Delete, ListRepo, ListEnv, GetPublicKey, GetOidcToken), encryption model (libsodium sealed box). Port from `the-gunbai/docs/understandings/github-secrets.md`. | M | Pending | DM-6 |
| 9 | DM-9 | **`extdeps/secrets/env_file.dag`** — "What is a .env file?" Types: `EnvEntry` { key, value, comment? }, `EnvFile`, `EnvValidation` { required_keys, format_rules }. Data: 6 `OperationBehavior` records (Load, Get, Set, List, Validate, GenerateExample), parsing rules. Port from `the-gunbai/docs/understandings/env-file.md`. | S | Pending | DM-6 |
| 10 | DM-10 | **`extdeps/secrets/vault.dag`** — "What is HashiCorp Vault?" Types: `VaultEngine` (Kv2 \| Transit \| Database \| Pki), `VaultAuth` (AppRole \| Oidc \| Token \| Kubernetes), `VaultLease` { id, ttl_seconds, renewable }, `VaultPolicy`, `DynamicCredential` { engine, role, ttl }. Data: 10 `OperationBehavior` records (KvGet, KvPut, KvDelete, KvList, Authenticate, RenewLease, RevokeLease, GenerateDynamicCred, RotateRoot, WrapToken), lease lifecycle. Port from `the-gunbai/docs/understandings/hashicorp-vault.md`. | L | Pending | DM-6 |
