...
4. **RCI-4 (P1) — DONE (2026-03-06)**: Applied the same shared-schema plus leaf-serializer cleanup to `makegen` and `justgen`. `dsl/extdeps/build_targets.dag` is now the single repo build-target schema, `dsl/extdeps/{make_render,justfile_render}.dag` own output syntax, and `dsl/tools/{makegen,justgen}.dag` are thin typed wrappers over discovery + serializer calls + `content_upsert`. Ratchets now cover both generated outputs (`gunbc-app/tests/render_projection_contracts.rs`, `core/daglang/daglang-cli/tests/snapshots/makegen_expand.txt`, and `core/daglang/daglang-resolve/tests/module_graph.rs`). Remaining adjacent render-lane work is CLI gen, markdown, CI reports, and similar text emitters.
...
|----|------|------|------|--------|--------|
| CP-46 | Structured `LowerError` enum with spans (subsumes CP-14) | M | CP-48 | CP | **Phase 1 complete** — `LowerError` has stable codes/help and now converts directly into shared `daglang-contract::Diagnostic` at the driver boundary. Variant-level mandatory spans are still deferred to CP-63. |
| CP-49 | Thread spans through `TypeError` (35+ variants → all carry Span) | M | CP-48 | CP | **Phase 1 complete** — `TypeError` now converts directly into shared `daglang-contract::Diagnostic` at the driver boundary, replacing the dead wrapper path. Variant-level mandatory spans are still deferred to CP-63. |
| CP-50 | Help text on common errors (10+ most-hit paths) | M | CP-48 | CP | **Done** — `help()` methods on TypeError (11 variants), LowerError (6 variants), and VerifyError (4 variants) now flow through shared driver diagnostics. |
...
| CP-29 | Validate required inputs after lower (catches `make gist` class) | S | CP-46 | CP | **Done** — `validate_required_inputs()` public function walks DAG nodes and is included in `verify_dag()`, which now runs on every compile path. Direct lowerer-only helpers still bypass verification unless the caller opts in. 3 unit tests. |
...
| ID | What | Size | Deps | Source | Status |
|----|------|------|------|--------|--------|
| CP-8 | Flip `skip_verification` to false | S | CP-6,7 + Bridge 1+2 | CP | **Done** — the verification bypass is deleted from `CompileOptions`; `verify_dag()` now runs on every compilation. Function-typed ports (`fn(T)->R`) still skip type expression validation (WS3-2 prerequisite). |
| CP-23 | `VerifiedDag<T>` type wrapper (gates Resolve + Emit) | M | CP-8 | CP | **Done** — `VerifiedDag<T>` newtype in `core/ir/src/verified.rs`. Private inner field (`Dag<T>`) now prevents construction without `verify()`. `Deref<Target=Dag<T>>` for transparent downstream access. `CompileOutput.lowered_dag: VerifiedDag<LoweredOp>` remains the driver gate. |
| CP-41 | Merge `validate_structural_primitive_wiring()` into `verify()` | S | CP-23 | CP | **Done** — structural primitive wiring errors and generic IR verification now share the single driver verification failure path. There is no compile-time verification bypass left. |
...
| CP-47 | `RuntimeBindings` replaces `ExternResolver` trait | M | CP-40 | CP | **Phase 2 complete** — resolve/build now accept `RuntimeBindings` directly, the old `ExternResolver` trait and `NullExternResolver` are deleted, and app/runtime callers use the single cached binding table. Remaining gap to target-state is ExternId-keyed total binding; current keys are still typed `ProgramSymbolId`. |
...
| RT-6 | **Wire `credential_chain` into gist** — replace raw `shell.GCloud.SecretManagerAccessVersion` with `credential_chain(runtime: LocalDev)` in all 3 gist entrypoints. | M | RT-4 | **Done** — `extdeps.github.auth::github_token()` now lowers through `std.patterns::credential_chain(runtime: LocalDev)` instead of the shell Secret Manager helper, so `tools.gist` and `shared.gist_modes` route GitHub auth through `local_auth` → `gcp.STS.Exchange` → REST `gcp.SecretManager.AccessVersion`. `gunbc-app/tests/gist_recent_regressions.rs` asserts the new path and the absence of shell `SecretManagerAccessVersion`. |
...
| RT-8 | **Verify `credential_chain` e2e** — pattern at `dsl/std/patterns.dag:236-283` references `gcp.STS.Exchange`, `local_auth()`, `gcp.SecretManager.AccessVersion` (REST). Verify all lower correctly with auth fixes. | M | RT-6 | **Phase 1 complete** — lowering/build coverage now proves the gist auth path contains `local_auth`, `gcp.STS.Exchange`, and REST `gcp.SecretManager.AccessVersion` (`gunbc-app/tests/gist_recent_regressions.rs`, `daglang-lower` scope/pattern tests). Remaining gap is an unignored end-to-end execution proof after the separate DryRun/GetField bridge is fixed. |
