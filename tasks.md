...
| 83 | RT83 | **Passthrough missing-input enforcement.** Strengthen RT4b from diagnostic to enforcement. When `execute_with_declared_output_passthrough()` falls back to `Value::Skipped` for a *required* output port (non-`?` type), return `ExecError` instead of silently substituting Skipped. Optional ports (`T?`) may still skip. CI test: required output ports never produce Skipped in dry-run of any `.dag` module. **Current state**: eprintln diagnostic added then suppressed (noisy before lowerer wires dag_util if/else branches). Restore as hard ExecError once lowerer fully wires `shared/dag_util.dag` return expressions. Code ref: `resolve.rs:99` TODO. | S | Pending | RT4a |
...
| 89 | RT89 | **REST status-code checking (fail closed).** `GenericRestParseOp` doesn't check HTTP status code — it just tries to extract fields from the response body. A 401 with `{"message":"Bad credentials"}` only fails because `html_url` is missing. (a) Add status code check before field extraction: non-2xx → parse error. (b) Document which operations intentionally accept non-2xx (e.g., 404 → `found: false`). (c) Wire to PC-6 for per-operation response classification. Prevents: silent success-with-garbage on error responses. Source: `TODO/testgen-proof-analysis.md` Proof 2. | M | Pending | — |
| 90 | RT90 | **Error-case mocks in auto_mock_spec pipeline.** For each transport mock in `auto_mock_spec()`, generate at least one error variant alongside the success mock: (a) REST: 401 for auth-bearing services, 500 for all. (b) Shell: exit 1 with stderr. These feed into Bucket C `SingleTransportFailure` to replace the `"<TRANSPORT_FAILURE>"` sentinel with realistic errors. Does NOT require `response {}` blocks (pre-PC-1). Source: `TODO/testgen-proof-analysis.md` Proof 3. | M | Pending | RT85 |
| 91 | RT91 | **Credential expiry enforcement.** `resource AuthContext` declares `expires: true` in `dsl/std/resources.dag:87-98`. The lowerer silently discards this. `TransportOps::Execute` never calls `credential.is_valid()`. Three fixes: (a) Lowerer reads `expires` property → sets `has_expiry` on resource spec. (b) Executor checks `credential.is_valid()` before sending request. (c) Testgen generates expiry-scenario obligations for resources with `expires: true`. Source: `TODO/testgen-proof-analysis.md` RT-A5. | M | Pending | — |
| 92 | RT-PC1 | **Parameterized header injection.** GCS providers declare CAS via `generation: Int` input fields, but the compiler generates static headers only. The lowerer needs: (a) a DSL construct to map input fields to HTTP headers (e.g., `headers { "x-goog-if-generation-match": expected_generation }`), (b) codegen to emit runtime header construction from input values. Without this, CAS preconditions must be hand-wired in the Rust worker. Source: review feedback on GCS provider semantics. | L | Pending | PC-1 |
| 93 | RT93 | **Stdlib host + caching.** Replace per-module "compile .dag + extract fn bodies + eval" pattern with a single cached stdlib interface. `fidelity.rs` compiles `std/fidelity.dag` on every call with no `OnceLock`; pragma has the same pattern. Fix: (a) `OnceLock<HashMap<String, LoweredFnBody>>` for immediate caching, (b) `include_str!` to embed stdlib sources (no `../../dsl` disk paths), (c) long-term: compile at build time to static table. Acceptance: no `daglang_driver` calls at runtime from `core/codegen`; classification is deterministic and hermetic. Design: `docs/design/v4/red-team-two-lane-plan.md` B13. | M | Pending | — |
| 94 | RT94 | **Resolve ReturnExprCompute split-brain.** Emitted runtime (exec-runtime) classifies `ReturnExprCompute` as `MetadataOnly` (skipped), while interpreted executor evaluates them via `ReturnExprComputeOp`. Graphs compiled through the emit backend silently drop compute nodes and their edges. Fix: (a) emit handler that evaluates stored `fn_body`, or (b) compile error if emit path encounters them, or (c) lower desugars complex returns into explicit DAG nodes. Add e2e test: `overall_success = a && b` computes correctly in emitted runtime. Design: `docs/design/v4/red-team-two-lane-plan.md` B17. | M | Pending | — |
| 95 | RT95 | **Transport class in node metadata.** Virtual backend requirements derive transport class from `node_id.contains("shell")` substring heuristics (`registry_gen.rs`). Store `ServiceTransportClass` explicitly in node metadata during lowering. Registry generation reads metadata, not names. Acceptance: rename a node ID → classification unchanged. Design: `docs/design/v4/red-team-two-lane-plan.md` B16. | S | Pending | — |
| 96 | RT96 | **Kill `propagate_to_param_sources`.** Boundary mocks propagation (`intercept.rs`) is a patch over inconsistent boundary model — param source nodes appear as separate entrypoints requiring manual propagation. Fix boundary detection so generated CLI code doesn't need it. Acceptance: `propagate_to_param_sources` deleted; entrypoint detection yields single canonical boundary port per input. Design: `docs/design/v4/red-team-two-lane-plan.md` B18. | M | Pending | — |
