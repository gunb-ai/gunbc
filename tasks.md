...
|---|-----|------|------|--------|------|
| 1 | RT1 | **Credential wiring.** `config { auth: BearerToken }` + `auth_token: Secret` → token never reaches execute node → unauthenticated request, silent. Fix: `GenericRestPrepareOp` identifies Secret-typed credential field when `auth_scheme` is set, excludes it from JSON body, and sets `request.auth` with proper `Credential`. The execute node's `r.auth.take()` fallback applies it at transport boundary. Touches: resolve_service.rs. | M | **Done** | — |
| 2 | RT2 | **Execute node silent fallthrough.** Missing `res:credential` → sends unauthenticated, no error. Fix: `GenericRestPrepareOp` now fails-closed when `auth_scheme` is declared but credential field is empty or missing. Returns hard error instead of producing unauthenticated request. Touches: resolve_service.rs. | S | **Done** | — |
| 3 | RT3 | **File transport completeness.** Extended `GenericFilePrepareOp` from 3 ops (READ/READ_BYTES/WRITE) to all 9 (+ APPEND, DELETE, EXISTS, CREATE_DIR, GLOB, METADATA). Extended `GenericFileParseOp` to handle exists/paths/modified_ms/deleted/created/appended output fields. Touches: resolve_service.rs. | M | **Done** | — |
| 4 | RT4 | **Transport block validation in lowerer.** `LowerError::MissingTransport` activated — non-interface service operations without transport blocks now fail at lowering. Interface-implementing services (`service X : Y`) map to `InterfaceStub` when transport absent. Added transport blocks to `pull_request.dag` and `openai.dag`. Touches: daglang-lower, dsl/services/. | M | **Done** | — |
...
| Fallback arm | HTTP method `_ => RestRequest::post(&url)` — typo silently becomes POST. | `gunbc-dag/src/resolve_service.rs:72-79` | R1 scout | 2026-02-26 |
...
| Validation at use site | `input_as_string()` returns `"(unresolved)"` magic string. | `gunbc-dag/src/resolve_service.rs:634-641` | R1 scout | 2026-02-26 |
