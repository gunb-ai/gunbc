...
| 44 | RT44 | **Expand `evaluate_fn_body` coverage.** Investigation: `local_auth()` is inherently runtime-only (service calls require I/O); body is lossy due to `?.` parser limitation. Added `max_by` pipe method to evaluator. Added 14 new eval tests: null coalesce, variant construct (unit/payload), match on enum, record update, for expression, nested field access, unit field access passthrough, unknown function error, list literal, unary not, filter with lambda. Root cause (`?.` parser support) tracked separately. | M | Done | — |
