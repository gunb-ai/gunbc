...
- [x] CI workflow execution and local execution both use the same underlying progress/event model, with only surface config differences. _(2026-02-18: `DisplayConfig { mode, verbosity }` now selects `Animated`/`Plain`/`CiPlain` surfaces while preserving shared execution path.)_
- [x] There are no remaining direct per-binary node-log print loops outside shared display infrastructure. _(2026-02-18: audited `gunbc-dag/src/bin/*` for `log.entries`/manual node-loop printing; binaries route through shared `execute_and_display*` paths.)_
