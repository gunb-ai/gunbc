...
- [x] Add `testgen --mode=verify` (deprecated `--check`) mode that verifies files match — *implemented in `testgen.rs`*
- [x] `make test` runs verification before `cargo test` — *via MetaTarget resource needs*
...
- Phase 10.2: ConditionalMock predicate-based response selection (optional, not needed yet)
- Phase 11: IR completeness is ongoing — tracked in `design-codegen-quality.md`
...
- [x] This replaces per-tool signature tests (Pattern E, consolidation §7 Pattern 3)
...
**Done**: DagSpecDef type in `core/testgen-registry`, `iter_dag_specs()` consumed
by testgen, makegen, and CI. Phase 8 cleanup complete.
