...
Tests that DAG signature validates. Generated by testgen when a
`TestgenTargetDef` provides a signature path.
...
- [x] `infer_signature(&dag)` matches declared signature
...
- [x] Migrate Pattern B tests to NodeExamples — then delete
- [x] Delete Pattern E tests — once TODO 8.2 lands
- [x] Goal: graph_mock.rs files contain only `pub fn mock_spec()` + data
...
**Blocked on**: Implementing the Phase 9 migration plan (DagSpec inventory
and consumers). The Phase 8 cleanup is complete.

**TODO 9.1: Design DagSpec type**
- [x] Define what fields DagSpec carries
- [x] Resolve circular dep (DagSpec metadata in testgen-registry;
      builder/mocks stay in tool crate via inventory registration)
...
**Type placement**: `core/testgen-registry` (already depends on codegen + inventory).
Tool crates already depend on this crate via `#[testgen_target]`, so no new
dependency cycle is introduced.

**Two-layer model**:
- `DagSpec<T>` (typed, local use in tool crates if needed):
  - `builder: fn() -> Dag<T>`
  - `mock_spec: fn() -> MockSpec`
  - `signature: Option<fn() -> WorkflowSignature>`
  - `meta: DagSpecMeta`
- `DagSpecDef` (type-erased, inventory registration + cross-tool consumption):
  - `origin_crate: &'static str`
  - `name: &'static str`
  - `dag_builder_call: &'static str` (string for codegen output)
  - `mock_spec_path: &'static str`
  - `signature_path: Option<&'static str>`
  - `output_path: &'static str`
  - `module_name: &'static str`
  - `tool_name: Option<&'static str>`
  - `testgen: DagSpecTestgen` (boundary/chain/flow, window_max_nodes, class, fermi, requires, secrets)
  - `generate: fn(&TestgenTargetDef) -> String` (same as current TestgenTarget)

**Why this resolves the circular dep**:
- Tool crates register `DagSpecDef` via inventory (strings + function pointer)
  with no direct dependency on gunbc-codegen types beyond `testgen-registry`.
- `testgen-registry` translates `DagSpecDef` → `TestgenTargetDef` for codegen.

**Migration plan**:
1. Introduce `DagSpecDef` + `DagSpecMeta` alongside existing `TestgenTarget`
   (compat layer: `TestgenTarget` becomes a type alias or thin wrapper).
2. Update `#[testgen_target]` macro to emit `DagSpecDef` instead of `TestgenTarget`.
3. Add `dag_specs()` helper in each tool crate (or rely directly on inventory).
4. Switch testgen/makegen/ci to iterate DagSpec inventory.
