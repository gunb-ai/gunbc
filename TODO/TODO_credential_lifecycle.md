...
#### Cloud baseline + provider stubs

To avoid locking into a single cloud, we keep a provider-neutral baseline:

- `CloudSecretConfig` + `CloudSecretRef` live in `core/ir/src/transport/cloud.rs`.
- Provider stubs exist for AWS and Azure so DAGs and tests can reference
  a stable interface even before full implementations land.
...
- Provider-neutral cloud config exists (baseline), with AWS/Azure stubs
  present so we can extend without reworking DAG interfaces.
...
### Phase 2a: Cloud baseline + stubs (additive)

- [x] Add `CloudSecretConfig` + `CloudSecretRef` in `core/ir` (provider-neutral).
- [x] Add stub crates `lib/aws-ops` and `lib/azure-ops` with placeholder DAGs.
