...
| **GD-4** | **Delete section 9C legacy tool graph stacks**: delete inventory section 9C `MIGRATE_DELETE` files after parity checks and generated contracts are green. | GD-2, GD-3 | L | Done (2026-02-22) -- deleted remaining handwritten `gunbc-dag/src/testgen_dag/{graph,mod,ops}.rs` stack; removed `gunbc-dag` library exports tied to legacy testgen GraphIR ops; `gunbc-testgen` now executes registry-driven generation using transport-backed file I/O (no legacy testgen GraphIR dependency); validated via `cargo test -p gunbc-dag --bin gunbc-testgen`, `cargo test -p gunbc-dag --test resource_registry_coverage`, `cargo test -p gunbc-dag --no-run`, and `cargo clippy -p gunbc-dag --lib --bins -- -D warnings` |
