...
- **Imports a per-shim row needs:** `import v3.std.bin_shim { BinShim }` (live), `import std.process { ProcessExit }` (live in `dsl/std/process.dag`), and the `<bin_name>_main` entry-function declaration (see §"Substrate prerequisite (STOP+PING)" — *not* yet live for any existing PB-owned bin).
...
- **`std.process.ProcessExit`** — LIVE in `dsl/std/process.dag` (`type ProcessExit = ExitSuccess | ExitFailure { ... }`).
...
- Live `ProcessExit`: [`dsl/std/process.dag`](../../process.dag).
