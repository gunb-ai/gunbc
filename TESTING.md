...
| `parallelism` | Int projection (`NoWorkflowProjection` scaffold on `lit`) | `t_r3_gate_87_cementing_regen_parallelism.dag` | `r3_gate_87_parallelism_rust_receipt_on_literal_bind` | `WorkflowParallelismReport` literals stable in `.dag` (including typed pairwise evidence per the lens register) → `LensOutputEquals` on the report carrier; delete or narrow the Rust pin in the same PR. |
