...
> - **External-toolchain boundary tests migrate to `ExecuteCommand`-based `.dag` `TestClaim` declarations** as the cascade-named successor pattern. **Capability state (2026-04-25, PB-Runtime `ExecuteCommand` extension landed):** the schema is still `src/v3/std/verification.dag:115-119` (PR #678). The Rust `TestRunner` and the M1.5 testgen harness now share one `std::process` path for arbitrary `command` + `args` + `expect_exit_code` — `evaluate_execute_command_exit_code` (and, for the M1.5 propositional true/false lens, `evaluate_execute_command_m1_5` which rejects collapsing non-exit `Fail` into `false`) and `parse_execute_command_fields` in `src/v3/compiler/src/test_runner.rs`; exit mismatches, spawn errors, wall-clock **timeout**, and **policy** fails (e.g. common shells with `-c` and a **bare** `&` that may be shell background, after a small elision of `&&` and common `n>&m` / `&>`-style token spellings; the same check applies when a known shell appears in `args`, e.g. `env sh -c …`, not only when `command` is a shell — PR #792) are distinguishable `ClaimResult::Fail` messages. **Linux** (when the host allows `unshare(1)`) also wraps the logical command in `unshare(1) -c -f -p` (user + PID namespace) so exiting the namespace **init** can tear down a contained process **subtree**; on **any** `unshare(1)` **spawn** failure (e.g. not on `PATH`, or `EPERM` / similar at `execve` — as in some container defaults), the runner **falls back** to the same direct `Child`+wall+null stdio path as non-Linux, without that isolation. **Non-Linux** uses that direct path. The child’s stdin/stdout/stderr are **not** captured to memory for the claim (null device on the direct path; on the `unshare(1)` path a `sh` bootstrap re-`exec`s the user `command`+`args` with **logical** `stderr` to the null device, while a parent pipe is still used for util-linux and early-bootstrap diagnostics only, plus nonblocking **drain** in the wait loop in case a wrapper path is chatty; the claim is still not `output()`-style full-buffer `capture` of stderr) so exit-code–only claims cannot OOM the runner on huge output. A fixed wall bound (`EXECUTE_COMMAND_WALL_TIMEOUT` in the same file) fail-closes unbounded hang; on Unix, timeout also signals the child’s process group. `GUNBC_EXECUTE_COMMAND_UNSHARE_EMPTY_STDERR_RELAUNCH` (truthy `1` or `true`) is the opt-in to allow the unshare empty-stderr re-exec in `cargo build` the same as in `cargo test` (PR #792, `test_runner.rs` `unshare_empty_stderr_relaunch_authorized`). The M1.5 allowlist + fail-closed panic for disallowed `ExecuteCommand` shapes are **retired** (see `src/v3/compiler/tests/integration/m1_5_testgen_test.rs` — the harness is bool-only for *exit mismatch* vs pass; other outcomes panic; full triage strings are `TestRunner`’s). A landed T-PB-B-1 boundary migration example is `src/v3/compiler/tests/dag/t_pb_b_1_execute_command_boundary.dag` (`t_pb_b_1_dag_runner_test`), structurally paralleling the class-5 *spawn + exit* receipt from e.g. `m1_4_emit_python_test::python_stdout` without requiring CPython in every CI run. Residual **bulk** migration of older Rust `Command::new("rustc" | "python3" | …)` boundary tests to `.dag` remains its own work item on ROADMAP. Full process-tree and sandbox **policy** (beyond `&` in shell `-c` and pgrp+timeout) is a separate, Director-level track.
>
...
**P5 / single-receipt closure.** When the `.dag` harness subsumes what the Rust
co-receipt pins, the **same PR** **deletes** that paired `#[test]` from
`r3_gate_87_lens_cementing_regen_receipts_test.rs` and carries the **checkable net
paydown** in the PR description per `INVARIANTS.md` §P5(b) (for example SG-0
`EXPECTED_HAND_AUTHORED_TEST` line removal when the test was census-listed). If a
thin Rust check must remain, the PR description must **name the irreducible
contract** it still enforces so the row does not silently keep parallel
authority.

| `regen.dag` lens | Placeholder shape | Harness | Rust co-receipt (if any) | Dissolution trigger |
|---|---|---|---|---|
| `infer_helpers` | `Compiles` smoke | `t_r3_gate_87_cementing_regen_infer_helpers.dag` | `r3_gate_87_infer_helpers_lens_source_compiles` | Public `infer_helpers` output carrier authorable as `.dag` data → `LensOutputEquals` (or equivalent) in the harness; **same PR:** delete `r3_gate_87_infer_helpers_lens_source_compiles` + §P5(b) paydown line. |
| `lower_helpers` | `Compiles` smoke | `t_r3_gate_87_cementing_regen_lower_helpers.dag` | `r3_gate_87_lower_helpers_lens_source_compiles` | Same as `infer_helpers` for `lower_helpers` / `r3_gate_87_lower_helpers_lens_source_compiles`. |
| `parallelism` | Int projection (`NoWorkflowProjection` scaffold on `lit`) | `t_r3_gate_87_cementing_regen_parallelism.dag` | `r3_gate_87_parallelism_rust_receipt_on_literal_bind` | `WorkflowParallelismReport` literals stable in `.dag` (typed pairwise evidence per the lens register when in scope) → `LensOutputEquals` on the report carrier; **same PR:** delete `r3_gate_87_parallelism_rust_receipt_on_literal_bind` when subsumed + §P5(b) paydown line. |
| `variant_payload` | `Compiles` smoke | `t_r3_gate_87_cementing_regen_variant_payload.dag` | `r3_gate_87_variant_payload_lens_source_compiles` | Stable variant-declaration fixture plus `VariantPayloadShapeLookup` literal in `.dag` → `LensOutputEquals`; **same PR:** delete `r3_gate_87_variant_payload_lens_source_compiles` + §P5(b) paydown line. |
| `unused_parameters` | Int projection (`LensOutputEquals` → `0` / `1`) | `t_r3_gate_87_cementing_regen_unused_parameters.dag` | `r3_gate_87_unused_parameters_rust_receipt_on_literal_program` | `List<UnusedParameter>` (or equivalent) literals stable as authored `.dag` data without M1(2.8) opaque-body diagnostics → replace the Int witness with direct carrier `LensOutputEquals`; **same PR:** delete `r3_gate_87_unused_parameters_rust_receipt_on_literal_program` when subsumed + §P5(b) paydown line. |
| `structural_resolution` | Int projection | `t_r3_gate_87_cementing_regen_structural_resolution.dag` | `r3_gate_87_structural_resolution_rust_receipt_on_literal_program` | `List<UnresolvedArrowBody>` literals stable in `.dag` under the same M1(2.8) constraint → full carrier `LensOutputEquals`; **same PR:** delete `r3_gate_87_structural_resolution_rust_receipt_on_literal_program` when subsumed + §P5(b) paydown line. |
| `effect_enumeration` | Int projection (`NoTransaction` scaffold) | `t_r3_gate_87_cementing_regen_effect_enumeration.dag` | `r3_gate_87_effect_enumeration_rust_receipt_on_minimal_program` | `EffectEnumerationReport` literals stable in `.dag` → behavioral `LensOutputEquals` on the report shape; **same PR:** delete `r3_gate_87_effect_enumeration_rust_receipt_on_minimal_program` when subsumed + §P5(b) paydown line. |
| `provenance` | Int projection | `t_r3_gate_87_cementing_regen_provenance.dag` | `r3_gate_87_provenance_origin_rust_receipt_on_literal_bind` | `Origin` literals comparable by `LensOutputEquals` → direct carrier compare in the harness; **same PR:** delete `r3_gate_87_provenance_origin_rust_receipt_on_literal_bind` when subsumed + §P5(b) paydown line. |
| `cost_target_realization` | Int projection | `t_r3_gate_87_cementing_regen_cost_target_realization.dag` | `r3_gate_87_cost_target_realization_rust_receipt_resolves_type_realization_row` | Full realization-meta / `Declaration?` receipt expressible as `.dag` data → replace the Int witness; **same PR:** delete `r3_gate_87_cost_target_realization_rust_receipt_resolves_type_realization_row` when subsumed + §P5(b) paydown line, **or** keep only a **named** irreducible remainder and cite it in the PR description (no silent parallel pin). |
