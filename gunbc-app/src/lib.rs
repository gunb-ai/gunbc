//! gunbc-app: Repo-specific DAG configuration for gunbc.
//!
...
//! used by any project. This crate (`gunbc-app`) contains configuration
//! specific to the gunbc repository itself.
//!
//! For example:
//! - `gunbc-clippy` (in lib/tools/) wraps the clippy CLI tool (general)
//! - `gunbc-app::ci` defines gunbc's CI pipeline (repo-specific)
...
// to be linked. gunbc-app depends on all three, so they run here.
