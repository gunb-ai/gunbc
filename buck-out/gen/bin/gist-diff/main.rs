...
fn main() {
    ...
    // Build the graph
    let dag = match build_gist_graph(GistMode::Diff { base_ref: base_ref.clone() }, extensions.clone(), public) {
        Ok(d) => d,
        Err(e) => {
...
fn print_help() {
    println!("gist-diff - Create a GitHub gist from branch diff");
    println!();
    println!("USAGE:");
    println!("    gist-diff [OPTIONS]");
    println!();
    println!("OPTIONS:");
    println!("    -r, --repo-path <VAL>       Repository path to scan");
    println!("    -b, --base-ref <VAL>        Base branch for diff");
