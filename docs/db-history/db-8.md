...
### R3 T-FixedPoint — §1.8 gate #16 (`pb_self_compile_fixed_point` R3 horizon)

The R1 lane gate and R3 thesis facet 2 share the **predicate name** but not the acceptance bar — see [`docs/r3-structure.md`](../r3-structure.md) §"T-FixedPoint" and [`docs/r2-structure.md`](../r2-structure.md) (R1 closure criteria). This DB-8 stack (`self_host_fixed_point`, `determinism_test`, receipt contract) is the **mechanical ratchet** aligned with [`docs/design-fixed-point-ratchet.md`](../design-fixed-point-ratchet.md); the future strong `.dag` suite (`pb_self_compile_fixed_point_strong`, Row A/B/C per [`docs/briefs/r3-pb-t-fixedpoint-worker.md`](../briefs/r3-pb-t-fixedpoint-worker.md)) is **dispatch-gated** and must not be authored in `verification.dag` until joint preconditions in that brief read green. Wave-1 P0 worker brief: [`docs/briefs/r3-wave1-pb2-fixedpoint-gate16-r3-horizon-worker.md`](../briefs/r3-wave1-pb2-fixedpoint-gate16-r3-horizon-worker.md).
