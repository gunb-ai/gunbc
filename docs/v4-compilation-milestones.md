...
It does not execute the TestClaim runner or prove verdicts. What has **not** been
done in CI is: compiling full emitted Rust with `rustc`/`cargo` to produce a
runnable `v4-stage0` binary.
...
3. **dag.dag language model wiring.** `dag_wave1_grammar()` (ModeledGrammar with production bodies) exists in dag.dag, but both pipeline-facing language model functions — `dag_language_model_wave1_void()` and `dag_language_model_surface_empty_prelude()` — use `dag_wave1_g0_void_grammar()` (VoidGrammar). `parse()` dispatches VoidGrammar to the accept-empty arm: it accepts empty token streams and rejects any real program. `dag_language_model_wave1()` already exists at dag.dag:687 (uses `dag_wave1_grammar()`); what is missing is **pipeline wiring** — compile anchors currently pass `dag_language_model_wave1_void()` instead (see `resolve_compile_anchor.dag:60`). Switching the compile pipeline to use `dag_language_model_wave1()` is the required step. This is **not** T-7 scope — it is dag.dag data wiring, a separate follow-on task.
...
- Wire `dag_language_model_wave1()` (already exists at dag.dag:687, uses `dag_wave1_grammar()`) into the compile pipeline — replace `dag_language_model_wave1_void()` references in compile anchors with `dag_language_model_wave1()`
...
- T-9 infer: modeled and in the chain; requires T-2 + T-4 algebra grounding (formal TASKS.md prerequisite) — T-9 fail-closes on bare-Atom bridge symbols until T-2 exports Node constructors and T-4 Wave 2b uses them in rust.dag
- T-2 std/algebra.dag: export Node constructors for each algebra type (e.g. `ordered_ring_node(inhabitant: Node) -> Node`) — explicit gate for T-4 Wave 2b per TASKS.md
- T-4 algebra fact-bundle for Rust target: use T-2 Node constructors to populate `AlgebraInhabitanceDecl.algebra` in rust.dag so T-9 can resolve algebra references instead of rejecting via `infer_algebra_ref_ungrounded`
...
**Sequential dependency:** [`compile_ingest_staging` wiring + dag.dag LM wiring (`dag_language_model_wave1()`)] → T-6 walk → T-7 `parse_expr` → T-8 normalize/resolve (modeled) → T-9 infer → T-10 emit → M2.
...
## TASKS.md update required (2026-05-25 planning session)

The following decisions were ratified during this milestone planning session and
**must be committed to their authoritative homes in `src/v4/TASKS.md` before
workers execute the affected tasks.** This entry is a transfer receipt, not a
dispatch authority — `TASKS.md` task bodies are the source of truth.

- **T-2 §modeling decisions:** Node constructor shape is `Conj` + named edges, not `Instantiation`.
- **T-25-core §approach:** strengthen home types before reaching for refinements; `posix.dag` is the reference.
- **T-30 §mechanism:** affected-lens `TestClaim` driven by T-21; not a separate lint or ci.dag build step.
- **T-4 §authoring contract:** per-language conformance test is an explicit deliverable, not a follow-on; T-19 + T-21 are prerequisites.
- **T-24 §gate:** lens verdict via `run_required_lens_gates`, not build exit code alone.
