...
**SUCCEEDED BY (2026-07-04):** [resolver graph-major design](resolver-graph-major-design.md) — the module-grained generalization this doc's own M1 dissolution trigger anticipated ("dependency-batched resolve"). M1's entry-keyed memo and the dormant #4867 entry-keyed disk cache are both the wrong-unit special case measured there (entry-grained sharing captures ~105s of a multi-thousand-second redundancy; the unit must be the module). M2's `RunnableCompile` shape is absorbed as that design's module node; its determinism gate (#5941) now scopes only the persistent tier.
...
Related: [lens input authority — resolve once, share by reference](lens-input-authority-design.md) — the lens-runner binding of the same computed-once/read-by-reference discipline (P1 landed #6426/#6432: `gunbc.floor_materialization` declares the floor index share with a RED control). See also [floor memory 3× regression diagnosis](floor-memory-pool-parse-regression-diagnosis.md) — the whole-pool full-body `pool_parse` baseline (#6848) is a per-process instance of exactly this double-paid-compute shape, priced with a receipt (RSS ≈ 225 + 0.38 MiB/pool-module).
