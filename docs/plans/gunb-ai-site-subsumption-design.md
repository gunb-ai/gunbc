...
- **Phase 0 — model + moodboard.** `dag/gunbc/site/` module home; `Material` tokens + derivation fns + the §4i craft-floor scales (spacing · type · stroke · radius · contrast · luminance budget, as typed rows); panel/type/SVG/ornament typed vocabulary; the §4f component rows with their depth ladders (`render(fact, depth)` fold shape); the §4e mark candidates (A–D) rendered from specs at 16/24/48/120 px; the §4l behavior rows (verbs · responses · timing tokens) + the hero-constellation and hash-nebula generators (frieze/coffering parked per §4l); emit `moodboard.html` + `primitives.html` (every primitive at every depth D0–D3; D4 = its emitted source form) as committed, drift-gated artifacts (published at non-linked paths once Phase 2 lands; reviewable on the srv1/tailnet path before that). **Gate: the §4k checklist signed row-by-row (approve/strike/amend), then register studies re-cut to the signed rows only — small batches, built to loop cheaply; nothing proceeds on unsigned rows.** No public-site change.
  ...
  - **Slice 3 LANDED (ninth + tenth passes, 2026-07-21 — the accent decision grounded and rendered; facts re-homed to extdeps per operator).** The classification is a real upstream framework, so its facts live in **`dag/extdeps/astronomy/stellar_classification.dag`** (§3: cite the source, keep its real names): `extdeps_external_authority_anchor` → `en.wikipedia.org/wiki/Stellar_classification`; the Harvard/MK main-sequence table as rows — classes O B A F G K M, effective-temperature ranges (typed `BoundedRange | OpenAbove`, no fabricated ceilings), **both** chromaticity columns verbatim, hydrogen-line strength, main-sequence share (source-verbatim text), plus the color-perception quote ("no green, cyan, indigo, or violet stars"). `gunbc.site.starlight` keeps only the **perceptual projection**: one envelope-constrained `Material` per class (`StarlightRender`), 1:1 coverage of the classification witnessed. Physics walls: **no green class** (checked against the *source's* chromaticity strings AND the projection's hues), and **M prunes itself** (lit variant fails the glare envelope). `gunbc.site.accent_study` emits the comparison page with labels **derived from the cited rows** (letter · Vega chromaticity · temperature range) over identical constellation fact rows, plus the verdigris-as-material exhibit, the accent-independent day atlas, and the source quote. Witnesses: `site_accent_study_witness_test.dag` keystone green (classification well-formed/hot-to-cool · projection covers classification · no-green in source and projection · M-pruned · G == live night figure · page emits with cited labels). Preview-only; the signed class's rows land in `tokens.night_theme` and the study dissolves. Seed-parser note for future authors: `X as T` inside an `if` condition of a *braceless* lambda body is a parse error in the v1 seed — bind the cast with `let` first (two sites fixed here).
