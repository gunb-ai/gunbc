...
The operator asked for the register as a *verbal checklist* to keep taste consistent across passes ("i'm starting to doubt myself"). The audit of all four briefs finds them **convergent, not contradictory** — "futuristic," "fractal," "calm," "glowfish," and "simpler/safer" compose into one thesis because the LOD grammar (§4d) assigns them to different depths:

> **Quiet at arm's length; intricate up close; every intricate thing true.**

Simplicity is a D0/D1 property; intricacy is a D2/D3 property; §4c makes the intricacy derived. The banknote is the standing proof this composes: mostly empty paper, one engraved figure, nothing fake. Exactly **two** things actually changed across the four briefs, both recorded rather than absorbed: the palette pivot (warm → indigo/starlight, rows 3–5) and the type treatment (engraved caps proposed, rejected, withdrawn — row 7). The signal has otherwise been stable.

The consistency test for any future proposal is two questions: **does it keep D0/D1 quiet?** and **is its D2/D3 intricacy derived from a real fact?** Yes + yes = consistent with every brief so far; anything that makes D0 louder is rejected regardless of how good it looks in isolation.
...
| 2 | intricacy placement | concentrated: hero constellation · section friezes · the mark · D3 reveals — body and panels stay plain | "intricate," "fractal" |
...
| 9 | motion | ~none: opacity-only reveals; `prefers-reduced-motion` = instant | "calm" |
