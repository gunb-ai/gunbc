...
**RE-DERIVED 2026-09-02 at main `de531c35496`; the 2026-08-15 figures below are a dated record, not standing.** Instrument, stated so it can be re-run rather than believed: build `gunbc` from the measured commit (`cargo build --release -p v1-compiler --bin gunbc`), delete every import statement brace-balanced from `dag/`, `src/v2/` and `src/v1/` in a detached scratch worktree (a line-based delete strands the member lists), then run one compile over the compile-clean root set — `gunbc compile --source-root dag --source-root src/v2 --source-root src/v1 --dependency-pool-index primary-precedence --target dag --output-dir <tmp>` (`gunbc.ci_layer_roots` `compile_clean_source_roots`) — and read its terminal `N blocking error(s)` line and `error[file:line:col]: msg` rows. The unstripped tree under the same invocation is the control.
...
- **By class:** unresolved type 3,247 · no-field 2,468 · ambiguous reference 1,701 · undefined variable 1,054 · method-unresolved 839 · function-not-found 777 · variant-not-found 180 · other 164 (6 of the 10,436 carry a `<kernel:…>` span rather than a file).
- **Top names:** `children` 704 · `Nat` 675 · `FilePath` 640 · `decl` 623 · `row` 598 · `trim` 528 · `Empty` 525 · `Node` 464.

**The 5,531/280/935 census does NOT still hold, and the way it fails is the finding.** The *spread* barely moved — 935 → 923 files, 280 → 272 distinct names — while the *volume* nearly doubled, 5,531 → 10,436. So the strip does not reach materially more of the corpus than it did eight days and 3,226 changed `.dag` files ago; the same population of sites now draws about twice the diagnostics. The class mix moved further than the totals: function-not-found collapsed 2,065 → 777 while unresolved type rose 1,742 → 3,247, undefined variable is flat (1,067 → 1,054), and two classes that carried no census rows in August — ambiguous reference (1,701) and method-unresolved (839) — are now a quarter of the total. Ambiguous reference in particular is a class the 2026-08-15 census could not have counted at its measured size: it is the sibling-homonym question that census left open, arriving as a diagnostic rather than as a silent mis-binding. Step 4's class-grain fix-forward should be re-prioritized off these classes, not off the August ones.
