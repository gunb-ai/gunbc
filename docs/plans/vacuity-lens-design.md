...
So the lens **must** read the live corpus, which means a host bridge. **Feasibility correction (2026-07-14):** the tree has **no `*_project.rs` files** — the established corpus-gate bridges (`inert_carrier_names_live` `cli_run.rs:13897`, `doc_graph_orphan_count` `cli_run.rs:14595`) are **additive functions inside `cli_run.rs`**, registered across `v1_compiler_infer_method.rs` (builtin name) + `v1_interpreter.rs` (dispatch). `cli_run.rs` is a **DESIGN-named load-bearing file** (the #5433 closure) → **escalate/confirm before editing** (inert-layer doc §7). The vacuity bridge is *additive beside* those functions, not a change to the reachability closure — but it still touches the load-bearing file, so it is an explicit checkpoint, not a silent edit.

- **bridge functions in `cli_run.rs` (additive)** — walk `*_test.dag`, enumerate each `test fn` body / `TestClaim` assertion, expose scalar/list verdicts through the **same additive builtin seam** as `inert_carrier_names_live`. Signals #1/#2 need each assertion's operand ASTs + a per-operand call-graph; #3 needs the projection→data-decl def-use edge (heavier). The #3 hardcoded-identity subclass is the most tractable *and* the operator's named pain — a `*_test.dag` asserting a string literal equal to its own declared module path is a self-identity read-back detectable without the full call-graph.
...
| host-fed corpus enumeration through the additive builtin seam (⚠ load-bearing `cli_run.rs`) | `inert_carrier_names_live` / `doc_graph_orphan_count` (additive fns) + builtin registration | `cli_run.rs:13897,14595` · `v1_compiler_infer_method.rs:333` · `v1_interpreter.rs:7754` |
