...
**Vacuity is not a new concept — it is the `materialization_ladder` law on the authored-fact substrate.** The operator's *state × decision law for redundant computation* (`dag/std/materialization_ladder.dag`, 2026-07-09) has a primitive — **plurality of demand per computation-identity** — and degenerate verdicts: `DeadComputation` (0×pure = authored waste), `AuthoredDuplication` (≥2 demands, shared-state LCA → **rewire/`Share`, never a cache**). Generalize the primitive from *plurality of demand per computation-identity* to **plurality of authority per fact-identity**, and a vacuous test *is* `AuthoredDuplication`: the fact has one authority (the model field / grounded constant / structural count), and the test's hardcoded expected value is a *second* authority for it. The ladder already runs a **detectors-feed-one-verdict-vocabulary** architecture (`v2.std.materialize` + `v2.lens.duplicate_computation` are detectors; `std.realization.Materialization` is the verdict) — the vacuity lens is a **new detector for the authored-fact substrate feeding the same verdict**, not a forked law (§3). The "cut": the ladder's `AuthoredDuplication` fix is *rewire to the single authority, never a second copy* — which on the fact substrate **is** §5 construction-over-validation (derive the structural fact from one authority; do not mirror-test it). This is why the datasheet case is vacuous: `watt_count(...) == 183` is a second authority for a fact the model already owns (grounded to the datasheet), exactly `AuthoredDuplication`.
