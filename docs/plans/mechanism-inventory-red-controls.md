...
Census of every wired gate/lens/cache reachable from the CI floor plan (`src/v2/workflow/ci_floor_plan.dag`) and the commit workflow (`dag/gunbc/commit_workflow.dag`): what each claims, its discriminating RED witness (the control that goes red when the gated behavior is wrong — DESIGN §5 spec-without-execution), and its last execution receipt. **Audit only — nothing here is a fix.** One pilot red-control ships with this audit (§6).

Receipts baseline: main tip `ef814b1f67` at census time (2026-07-01). **Main-red incident receipt (resolved same day)**: main CI was red from the last green floor run `28486103773` (00:59 UTC) until #6112 merged (22:44 UTC, `fb35cd3815`); run `28544311796` (20:02 UTC) shows batch 2 failing on `extdeps_external_authority_gate_passes` (false), `generated_artifact_drift_gate_passes` (false), and the discovery corpus itself (resolve error: `dag/extdeps/tools/tools.dag:56:20: no field 'Which' on type 'shell'`, surfaced via `nbd_proxy_serve_transport_witness_test.dag`). #6112 root-caused all three to an `extdeps.shell` module collision + the `nbd_proxy_serve` anchor + `.gitignore` drift. Two readings of the ~22h window, both true: the gates demonstrably fire on real breakage (not vacuous), and while main stayed red the floor protected nothing incrementally — every PR CI run inherited the red, so a new regression was indistinguishable from the standing one. That window is the live cost the "gate must be green-on-main at merge" wall (§5 finding 2) would displace.

## 1. Floor gates (10, `Gate` coproduct `dag/gunbc/ci_gate.dag:3-13`, runnables `ci_floor_plan.dag:157-179`)
...
|---|---|---|---|
| RustMonolithGate | changed `.rs` pass fmt/clippy/nextest (`dag/tools/rust_gates_ci.dag:42-68`) | real cargo failures only; no synthetic perturb. Change-gated skip-to-green when no rust paths changed (`:63-66`); fail-closed on git error (`rust_stage0_gates.dag:100-105`) | wired, red-by-consequence only |
| EmitHostGate | rust/py/go/ts/node smoke rows emit expected byte counts / HTTP 200 (`dag/tools/emit_host_gate.dag:12-25`) | exact byte-count `test -eq` is the discriminator (`emit_host_transport.dag:34-57`); no planted-wrong-emit fixture | wired, exact-oracle |
| LayeringImportsGate | clean tree + scanner detects 5 planted violations (`dag/tools/layering_imports_gate.dag:9-19`) | **yes** — 5 perturb `detects_holds` claims (`layering_imports_transport.dag:46-52`) | wired + red-control |
| ExtdepsExternalAuthorityGate | URI witnesses + live clean corpus + red fixtures (`dag/tools/extdeps_external_authority_gate.dag:10-21`) | **yes, strongest** — `missing_anchor_red`, `bogus_scheme_red`, `file_anchor_red`, `anchor_drop_red` asserted RED (`:17-33,67-83`). Receipt: went red for real on main 2026-07-01 | wired + red-control (currently firing) |
| DagCompileCleanGate | whole tree compiles; perturbs go red (`dag/tools/dag_compile_clean_gate.dag:12-25`) | **yes** — planted skew/unresolved-import must FAIL compile (`dag_compile_clean_transport.dag:70-128`); `red_empty_shell_program_fails_dcc_invocation_check` (`:256`) | wired + red-control |
| GeneratedArtifactDriftGate | committed artifacts == regen, and a perturbed copy drifts (`dag/tools/generated_artifact_gate.dag:22-42`) | **yes** — `artifact_red_receipt_ok` (`:33-38`); empty-roster vacuity (`all([],…)⇒true`, `:51`) is closed by `generated_artifact_drift_test.dag:24` (`count > 0`). Receipt: red on main 2026-07-01 | wired + red-control (currently firing) |
...
| RegenVerifyGate | committed stage0 == fresh regen (`dag/tools/regen_verify_gate.dag:6-14`) | discrimination lives inside the host binary's `--verify` (`regen_verify_transport.dag:28`); no `.dag`-level negative control | wired, host-trusted red |
...
| determinism_compile_gate | a bare `map_keys` reach into the compiled tree fails compilation (`src/v2/lens/determinism.dag`, enrolled in `v2.compiler.compile` `always_required_root_lenses`) | red control = planted-`map_keys` reach fixture (`src/v2/lens/determinism/reach_witness_test.dag`); replaced the deleted x2 EmitDeterminismGate (#6594) | wired, construction gate |

Corpus runnables (rewritten 2026-07-04, witness opt-in inversion): witness enrollment in CI is now **opt-in by declared roster row** — `CiSpec.witness_entries` (`CommitWitnessClaim` rows on the `GithubActionsCiJob` surface of `commit_gate_roster`, projected by `project_ci_floor_witness_entries`), partitioned into `floor_corpus_node` (explicit entries, no scan dirs) and `execution_corpus_node` (rows under `src/v2/test/claim/execution/`); an **empty roster puts no corpus node in the plan at all**. The glob discovery over `witness_layer_roots` + scan dirs remains the local `claim_batch --roster-from-discovery` path only. The prior hazard this section flagged — **enrollment opt-out by path substring** silently un-enrolling witnesses with no red signal — is dissolved in CI by construction: nothing is enrolled by scanning, so nothing can be silently *un*-enrolled by an exclusion (`witness_exclusion_substrings` now only shapes the local discovery roster). The `grounding_whole_tree` dedicated single claim was dissolved into the roster; re-enrolling it is a declared `CommitWitnessClaim` row for `src/v2/test/manual/grounding_lens_whole_tree_test.dag`.
...
| merge-admission **gate** (`ci.yml:95-99`; `merge_admission_gate.dag:47-88`) | every CI run | receipt-missing/invalid paths can red (`:51-64`). **The staleness discrimination is switched OFF**: `merge_admission_blocks_merge` (`ci_failure_class.dag:173-184`) conjoins on `gating_is_enforced(merge_freshness_gating_status)` and `merge_freshness_gating_status = GatingComputedDeferred` (`:149`) ⇒ always admits | **the central coverage-by-illusion finding.** The pure predicate is fully red-witnessed (`merge_admission_witness_test.dag:62-100` — stale-base/stale-roster block, fresh admits, roster-hash sensitivity), so the model looks proven while the deployed wrapper is inert. Pilot red-control §6 pins this by execution. Honestly scaffold-marked (`ci_failure_class.dag:152-159`) with a named activation trigger; this is roadmap node **2-merge-admission (HELD)** — the pilot control (§6) is its missing receipt pair |
...
The #5433 inert-lens backstop **does execute** in the floor (`dag/test/claim/inert_lens_hygiene_witness_test.dag`, builtins `cli_run.rs:3150,3165`) but only checks module **import-reachability** from a discovered witness + non-empty universe. A lens that is imported and green-only (or whose live witness is exclusion-hidden) passes it — the gap its own plan (`dag/gunbc/plans/inert_layer_lens.dag` §9) names.
...
**B — wired, live-green-only (red is synthetic) (7):** layering_imports, realization_vocabulary_containment, intent_linearity, module_graph, complexity_linearity_audit, non_fold_residue, no_dual_representation_test. A live pass runs green over the tree; the only red-going case is a planted fixture — a real corpus violation of a subtly different shape can slip. (`medium_structure_containment` deleted #6831.)
...
**D — inert (3):** **grounding** (its live whole-tree witness `src/v2/test/manual/grounding_lens_whole_tree_test.dag` is dark to local discovery via the `test/manual/` exclusion, and since the 2026-07-04 opt-in inversion the floor's dedicated `grounding_whole_tree_runnable` is gone too — the discovery-exclusion + explicit-runnable dual-representation is dissolved; re-enrollment is a single declared `CommitWitnessClaim` roster row on the `GithubActionsCiJob` surface), **fact_density** (fires inside the compile-lens bundle but no discovered `test fn` gates its verdict), **affected_set_examples** (fixture carrier, no witness).
...
| BuildBuddy CAS | INERT — modeled catalog only (`dag/extdeps/cache/buildbuddy.dag`) | none | none | placeholder for §2 P2 `realize(subject)` |
...
2. **Main-red erodes the whole floor** (incident receipt, resolved by #6112 same day): for ~22h every gate's per-PR discrimination was suspended (a PR could not distinguish its own breakage from the standing one). The gate-hygiene ROADMAP item ("floor-enrolled gate must be green-on-main at merge") is exactly the missing wall.
...
`dag/test/claim/merge_admission_deployed_gate_control_test.dag` — floor-discovered, pure `.dag`, no host effect. Two claims over the same stale-base scenario the existing witnesses use:
