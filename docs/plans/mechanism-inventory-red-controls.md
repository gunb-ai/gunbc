...
| EmitDeterminismGate | two sequential emits byte-identical (`dag/tools/emit_determinism_gate.dag:6-15`) | `diff -r` is the discriminator (`emit_determinism_transport.dag:48`); no injected-nondeterminism control; theoretical vacuity if the corpus emitted nothing (diff of two empty dirs passes) | wired, self-diff only |
