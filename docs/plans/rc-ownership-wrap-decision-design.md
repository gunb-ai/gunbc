...
**That denominator is keyed on rendered diagnostic text, and it is known incomplete by execution.** Four facts, measured by `smart-ibex-716` and stated without a fifth. (1) A two-arm run holding the corpus byte-identical at `a6bceb6903` and varying only the binary across a `RustCorpusRepr` carrier deletion left **18 `01_tokenize` sites textually identical in both arms** — `expected Rc<Nat>, found i64` — while `Nat` went from `Rc<CommutativeSemiring<Magnitude>>` to `i64` underneath them. The carrier half of a fused disagreement dissolved at its own authority; the wrap half survived, and rustc printed the same string on both sides because it prints the alias. Those 18 are **disjoint from the 55** (measured against the 1,883-row enumeration, 0 of 18 present), so the census misses them: **+18 proven at that one signature.** (2) The same mechanism appears under **two** signatures and the renderer chooses between them inconsistently — `Rc<Nat>` and `Rc<Rc<CommutativeSemiring<Magnitude>>>` were the same type in the before arm, and 43 sites of one mechanism landed in two different roots by rendering alone. A text-keyed census therefore inherits not merely a collapse but an *inconsistently applied* one, which is worse, because the resulting buckets read as distinct populations with distinct sizes. (3) Rows whose poles differ by exactly one `Rc<…>` layer while not being textually equal number **306**, none currently filed here — a candidate pool, emphatically **not** 306 hidden sites: its largest block is 102 `Rc<Fnv1a64Structural>` against `String`, a genuine carrier disagreement merely wearing the one-layer shape. Only the 18 carry executed evidence. (4) **The error direction is unestablished.** Masking can hide a wrap disagreement inside a carrier-named bucket, which is what was measured, and can equally render a carrier disagreement as a one-layer pair, which would inflate this root rather than deflate it; one measured instance of one direction is not a direction. The sound discriminator is structural — one `Rc` layer apart **after alias expansion** — which the diagnostic text cannot supply and the emitted source can. None of this bears on the mechanism below, which never reads a printed type name.
