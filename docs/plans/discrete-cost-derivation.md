...
So the displaced cost is concrete and already priced by the tree: **three live scaffolds are blocked on one missing derivation**, and each additional witness that disappears into `long/` is a row that per-PR CI stops proving while a green CI keeps implying it did (`compiler_frontier_per_module_probe_exclusion_note` states this outright: *'green CI says nothing about survey numbers unless the local recipe below ran'*). The benefit is not an elegant cost algebra; it is retiring a proxy that silently removes coverage.
...
There is also a live admission law: `v2.lens.witness_cost_locality` already classifies a witness `Local | CouplesToAmbient` from its import closure and routes it `AdmitContinuous | RouteScheduledLane` against a `WitnessInputEnvelope`. That is the consumer seam this note's output plugs into — see §10 C5.
...
| `SymbolicCost` | sizes remain free variables | the expression + the unresolved variables, named |
...
The measured corpus for calibration already exists and is populated: `dag/gunbc/witness_row_cost_basis.tsv` with `WitnessRowCostVerdict` drift comparison. C4 does not need to build a measurement pipeline; it needs to join to one.
...
1. **C0 — authority convergence (ruling, no new taxonomy).** Reconcile `v2.lens.cost` `SymbolicCost`, `v1.compiler.complexity` `CostExpr` / `ComplexitySummary`, `std.realization_schedule` `CostAccount`, `std.realization_measurement`, and `gunbc.ci_input_envelope` `InputEnvelope`. **Ruling to sign:** `v2.lens.cost` is the symbolic and discrete work authority; the useful `CostExpr` work/span/binder machinery migrates out of the v1 seed into it; `CostAccount` remains the downstream realized time/space/power projection; measured receipts stay falsifiers. **Accept:** one authority per concept, and correction 4's `llvm_instruction_cost` has a declared destination.
2. **C1 — the valuation environment and exact pure work.** The seam correction 1 says is absent: bind `SizeVariable` to values, and evaluate to `ExactCost | BoundedCost | SymbolicCost | CostRefused` over sequential nodes, branches with known conditions, known-finite lists, bounded folds and direct calls. **Accept:** doubling a bounded list doubles the derived fold-body work, by execution; a missing loop bound yields `CostRefused`, not zero; pilot on a real admission function whose cost is independently obvious.
3. **C2 — loops, recursion, sharing, and span.** Consume `loop_bound_measure`, `DescentEvidence` and the SCC/descent machinery; add the binder-carrying sum (correction 3); derive `span` alongside `work`; make `Materialization` a derivation input (§8). **Accept:** sequential and parallel graphs with equal work show different span; memoized and recomputed realizations of one graph derive different work; unknown descent refuses.
...
- A branch with a known-true condition charges only the taken arm; with an unknown condition it returns `BoundedCost` or `SymbolicCost` — **never a fabricated exact number**.
