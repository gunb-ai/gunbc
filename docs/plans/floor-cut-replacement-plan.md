...
3. regen self-host fixed point (the modules `regen_stage0.rs`, `regen_floor_skip_witness.rs`, `dag/tools/regen_verify_gate.dag`, `regen_verify_transport.dag` survive throughout; their **job** returns here) — **ORDER-DEPENDENT (corrected 2026-08-15): "survive throughout" holds on this branch only.** `integration/v1-cut` (gunbc#8293) deletes `regen_stage0.rs` and its roster outright, because the regeneration loop's subject — the `src/v1/**/*.dag` authority — is what that cut removes. So if the v1 cut reaches main first, **this queue item retires rather than re-adds**: there is no job to return, and re-adding one would restore a consumer of a deleted producer. Whichever of the two cuts merges second owns reconciling this; it is not a defect in either plan, and it must not be resolved by silently re-adding the job.
...
`gunbc.replacement_cut` row `FLOOR-Y` — authored as a follow-up once gunbc#8276 (the carrier) merges to main.
