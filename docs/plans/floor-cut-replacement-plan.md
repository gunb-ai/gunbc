...
Side effect: with `ci.yml` deleted on the branch, PR #8283 stops running CI (the merge ref carries no workflow file) — the known-red runner burn stops.
...
"The deletion is the census" holds for consumers that **survive to refuse**. Executing the doctrine on five branches at once surfaced three populations it cannot reach, each silent — no diagnostic, no unresolved reference, no red — so a green tree is complete over external loads and says nothing about these.
...
2. **A guard that leaves with its subject takes an obligation with it.** Separate **guard** from **peer** — peers dying together as one subsystem is the cut working (16 of 42 deleted witnesses here); a guard exists to outlive what it constrains. Specimens: `arm_declares_dissolution_trigger`, deleted alongside the dispatch arms it asserted about, and `SELF_COMPILE_ERROR_RATCHET`, a self-compile error bound declared and enforced in one deleted file and now absent tree-wide. A vanished guard produces **no artifact**, so an artifact-keyed landing ledger is blind to this class too.
3. **A surviving thing can stop being reached.** Nothing asks whether a survivor still has a consumer. Measured: 8 orphan candidates by import graph → **3** after joining roster rows, `*_test.dag` discovery, workflow argv and Cargo `[[bin]]` — a 5-in-8 false-positive rate, and three of the five were reached by the discovery fold this cut itself delivers. **A reach measurement over one medium reads exactly like a reach measurement**, and nothing in the query announces the media it omitted; the medium join is part of the query, never a follow-up pass.

Corollary for Step 3: **a re-add is discharged by the artifact *and* its guard.** Four queued gates — `dag_compile_clean_gate`, `generated_artifact_gate`, `regen_verify_gate`, `regen_verify_transport` — had their guards deleted by this same cut, so a re-add judged by "the gate is back" silently restores an unwitnessed gate.

## Step 3 — the re-add queue (one job at a time, operator agreement each, **recursively delete-first**)

The orphaned obligations, from the wipe's rung-drop declaration. Nothing returns unagreed — and a re-add is **not** a restore-from-quarry: each returning obligation gets the same sequence recursively (operator ruling, 2026-08-15: "they all have a lot of cruft — the same sequence has to be followed recursively"). Per re-add: state the obligation from first principles, design the minimal job that discharges it, mine the quarry as oracle only, and expect the old job's structure to die. Two cruft specimens: the old build job's 19-binary roster is mostly floor/falsifier bins this cut deletes, and its `CARGO_BUILD_JOBS=1`-then-unset-`RUSTC_WRAPPER` retry arm is the absorbing fallback DESIGN's srvN build-cache thread names as masking the sccache deficit. Neither survives re-derivation. The queue:
...
3. regen self-host fixed point — **this item's disposition is NOT carried here.** The executing lane records it on `gunbc.witness_floor_workflow` — **a carrier that exists only on `integration/floor-cut`, not on `main`, until this cut merges** — and that carrier is the authority; this plan names it and asserts nothing about its contents, since summarizing an authority makes the citation a second copy (§3). The branch qualifier is load-bearing: an unqualified symbol invites a `main`-side grep that cannot resolve it — the stale-citation class this plan argues against.

   What this plan contributes is the **cross-branch order-dependency**, a merge-graph fact no single branch can observe: `regen_stage0` is alive on `main` and deleted only by `integration/v1-cut` (gunbc#8293), whose cut removes the regeneration loop's subject — the `src/v1/**/*.dag` authority. The item's disposition is a function of merge order, and all three arms are real: v1-cut first ⇒ the obligation retires with its mechanism; **floor-cut first ⇒ main carries a live regen authority with no required job invoking it — a gap someone owns, not a settled absence**; v1-cut never lands ⇒ the item returns to this queue as an ordinary unmet obligation. Whichever cut merges second owns the reconciliation, and must not resolve it by silently re-adding a job whose producer is gone.
...
Observed at the wipe boundary (identity-grain, 2026-08-15), beyond the predicted queue: (a) with the drift gate gone, `.gitattributes` and the surviving generated artifacts can drift **silently** until item 4 returns; (b) `tools.merge_admission_gate` reads a receipt the floor run used to stamp — merge admission has **no producer** until re-added; (c) the falsifier rows carried the only unselected whole-corpus cold control; (d) fmt exists only as the bypassable opt-in pre-push hook. The full ledger is the lane's boundary report and `commit_workflow.dag`'s gate roster, retained deliberately — it IS the ledger.

## Contested / do-not-delete (sweep-verified non-floor consumers)

- **Keep whole:** `src/v1/stage0/src/bin/claim_batch.rs` (the local path) · `src/v2/workflow/floor_discovery.dag`, `floor_discovery_producer.dag`, `floor_discovery_transport.dag` (reached by the local producer path in cli_run; since 2026-08-29 the required floor's roster is the producer's own per-file fold — `discover_floor_rows_for_source` — over its prepared sources, so it is a floor authority as well) · `src/v2/workflow/floor_naming_hygiene.dag` (surviving `test fn` placement rule; non-floor importer) · `src/v2/workflow/executor.dag`, `scheduler.dag`, `batch_runner.dag` (generic substrate) · the fleet-converge pair · the regen bins
...
`gunbc.replacement_cut` row `FLOOR-Y` — the carrier merged to main in gunbc#8276 (2026-08-15), so the row is authorable and an open follow-up for the executing session, not blocked. (Corrected 2026-08-15: this line still read *"once gunbc#8276 merges"* while the sibling namespace plan recorded the merge — two cut plans asserting opposite states of one fact, §3 single authority, caught in review on gunbc#8297. `dag/gunbc/replacement_cut.dag` is present on `origin/main`.)
