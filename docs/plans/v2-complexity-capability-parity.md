...
3. **C0(b2) — DELIVERED 2026-08-05** — declaration-to-capability totality. Carrier: `gunbc.v1_complexity_decl_classification` derives the `v1.compiler.complexity` population from `DeclFact` (filtered by `src/v1/complexity.dag` rel_path) and classifies every declaration exactly once via `gunbc.v1_complexity_decl_classification_roster` (201 rows at this revision). **Accept:** bijection between derived population and roster (long lane), every semantic capability has at least one `AuthorityOf` declaration, every inventory authority carrier matches, zero stale rows, distinct names.
