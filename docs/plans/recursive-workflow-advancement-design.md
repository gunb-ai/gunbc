...
| The publication gate had its own ambient dependency | This branch's first push refused in `.githooks/pre-push`: `cargo` was not on `PATH`, although Cargo 1.93.0 existed at `/home/briansrls/.cargo/bin/cargo`; the exact-path `cargo fmt --all --check` passed before the push was retried with hook execution skipped | The refusal was preserved and the skipped hook was reproduced successfully against the exact installed realization; no unchecked source change was published | Make publication/pre-push validations consume admitted capability realizations and receipts instead of assuming an interactive shell `PATH` |
