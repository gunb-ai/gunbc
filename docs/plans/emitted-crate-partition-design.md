...
- **Shared home (language lane owns; import target, never imports upward):**
  - `CompilationUnit` / `CompilationUnitId` — the agnostic unit. Fields: `id` (the unit's **one** agnostic logical identity — **input**, supplied via the assignment map), `members` (module-DAG nodes in this unit — **input**), `interface` (exported surface other units may reference — **R5-derived output**), `deps` (units this depends on — **R5-derived output**). There is **no separate agnostic artifact identity**: every realization derives its target spelling (crate name, `.h`/`.c`/`.o` basename, package name) from `id`. *(Corrected 2026-08-01, review on gunbc#7480: this line prescribed a second `artifact` field. A second independently writable identity beside `id` is the dual-representation §3 forbids — a `CompilationUnit` could be written with `id` and `artifact` disagreeing, and nothing derived them from one another. The field was removed from `v2.std.compilers.compilation_unit` `CompilationUnit`; this locked interface line is corrected to match the carrier rather than left prescribing the deleted shape.)*
...
7. **Rust R3 render:** each `CompilationUnit` → `Cargo.toml` + `lib.rs` via the existing renderers; the crate-name spelling is **derived from `id`** here, in the Rust realization — exactly as C derives its `.h`/`.c`/`.o` basename from the same `id` through `c_translation_unit_basename`.
...
- **Do not:** create a structure parallel to `stage0_crate_plan()`; hand-author the partition; hand-set `interface`/`deps`; store a Rust crate name (or any other target spelling) on the shared unit — derive it from `id` in the realization; embed `PartitionPolicy` in the shared parameter; couple `K` to the live host.
