...
- **srv9–srv12** carry neither.

So the admissible set for a new serving shape today is four hosts — srv9–srv12 — and the exclusion authority is the pair-serving group binding, not a reservation and not the role roster. The repair is to make the fit screen consume the **host binding**: the set a candidate is screened against is the roster minus hosts bound by a desired serving deployment, minus role assignments, minus undischarged reservations, **with each exclusion naming the authority that actually produced it and each such authority checked live rather than transcribed here**. A shape needing more admissible hosts than exist fails a hard constraint and is excluded before Pareto, never silently fitted against 8; if that leaves no candidate, the result is `NoFeasibleRealization` with the exclusions named (§7), not a refusal — the selection was well-formed, the fleet was full.

That last clause is the durable lesson and the reason this document must not carry the host list as data. Two of the three exclusion terms in that formula are *currently empty* — no undischarged reservation, and role assignment is a strict subset of what pair-serving already binds — so a screen built from a transcription of today's state would be wrong the moment any of them moves. The admissible count is **not a constant**: releasing Group A returns four hosts, and promoting a second group removes four more. That is why the binding is derived from the authorities rather than stated, and it is exactly what this section got wrong when it stated it.
