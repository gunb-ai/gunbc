...
**Status:** **typed affected-CI audit banked and denominator reconciled** (`gunbc.ci_cost_arc_closeout_receipt`). P1 REJECT (#7725); selection honesty landed (#7722/#7729). `retention_settle=ClosedReject`. **End-to-end residual:** `end_to_end_residual=ReconciledCiJobWall`; 15 adjacent elapsed intervals on the harvested whole-second event grid assign 100% of the affected subject's CI-job wall and `unattributed_ci_wall=0`. The millisecond value is a projection from that grid, not subsecond source precision. **Compiler next measured target:** `compiler_next_measured_target=PerEntryAssembly`; its next slice identifies duplicated construction before selecting a mechanism. Not softer eviction.
...
**P1 follow-on — entry-view assembly memos REJECT** (measured 2026-08-06, PR **#7867**): three process-lifetime memos (`closure_assembly_env`, `root_assembly_env`, `rewired_modules`) on the same 50-entry cohort, one process and one `MultiEntryIndex`, interleaved base/after × two reproductions: Σ exclusive assembly **−4.7%**, elapsed wall **−1.5%**, peak RSS **+1.8%**; **`root_env` hit 92% (672/57) and wall still moved only −1.5%** — the load-bearing finding: entry-assembly memoization as a class is not where time goes, even when the cache works (`closure`/`rewired` 0 hits are secondary — keys do not repeat on this roster). Representative affected-floor run not started — cohort miss is decisive. Implementation **deleted** on merge; retained evidence only. → [entry-view-assembly-direction-receipt](entry-view-assembly-direction-receipt.md) · raw arms, roster, subject and derivation in `receipts/entry-view-assembly-direction/`

**P1 next measurement — import-string rewire control LIVENESS-CONFOUNDED** (2026-08-10): the first two frozen-cohort repetitions are invalidated because the host retained closure-wide plan/result batches and charged always-on immutable counters inside the phase it called dominant. Production now keeps the original streaming, counter-free wrapper, and the default-off observer also prepares/applies/releases one module at a time. The corrected same-head A–B–B–A completed baseline-r1 and both observed arms with identical outcome/subject/closure hashes, but baseline-r2 failed to return twice, including after a declared 900-second bound; no fourth row is invented and no instrumentation-tax or optimization verdict is selected. The observed arms classify 6,572,738 of 6,606,131 ancestry insert attempts (99.49%) as same-`Rc`-identity and 33,393 (0.51%) as changed, making no-op insertion a candidate for a discriminating follow-on rather than an authorized optimization. Prior testing already rules out precomputing direct-import ambiguity; entry-view memoization remains rejected. → [import-string rewire decomposition](import-string-rewire-decomposition-measurement.md) · receipts and runner in `receipts/import-string-rewire-decomposition/`
