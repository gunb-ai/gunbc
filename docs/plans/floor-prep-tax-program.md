...
**P1 redirect — first slice measured:** the redirect's first mechanism is the **fill composition overlay direction** (`v1.compiler.infer` `symbol_index_with_qualified_fill` / `symbol_index_with_bare_fill`): both composed an entry's closure census with a per-index underlay memo by walking the *underlay's* key list, so every entry paid O(|whole pool|) to build an index that differs only by its own closure. Overlaying the closure onto the shared memo is the same denotation at O(|closure|). Measured on a fixed 50-entry cohort, two reproductions per arm: symbol-index merge **−97.9%**, per-root symbol-index composition **−97.0%**, Σ exclusive assembly **−54.7%**, additive resolve **−30.2%**, outcomes byte-identical (48 PASS / 2 FAIL, same names), `regen_divergence_count=0` with the seed rebuilt from the changed sources. This does **not** reach the shared-entry-view bar (assembly ≥60% on this cohort) — the surviving rows are import-string rewiring, the per-entry closure census, and the per-root variant base, which are different defects and need module-grain reuse. No fleet wall is claimed (additive-vs-elapsed denominator rule). → [fill-composition-overlay-direction-receipt](fill-composition-overlay-direction-receipt.md)
