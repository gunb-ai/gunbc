...
**P1 next measurement — import-string rewire decomposed** (measured 2026-08-10): the same frozen 50-entry cohort, two repetitions on one binary/subject. Binding application is **89.15–90.68%** of the import-string row; both index builds together are below 3.5%, and module preparation is below 8%. Both runs perform the identical 6,928,314 inherited-key ambiguity checks and 6,606,131 ancestry-map write attempts. This selects the binding-application phase for one more model-first decomposition; it does **not** yet select a cache or map mechanism because ambiguity membership work and persistent-map updates remain combined. → [import-string rewire decomposition](import-string-rewire-decomposition-measurement.md) · exact receipts in `receipts/import-string-rewire-decomposition/`
