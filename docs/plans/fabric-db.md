...
- **Weak digest:** object refs use the non-cryptographic structural digest (`std.content_hash` `content_hash_of_value`). A collision is refused at put (`FabricObjectCorrupt`), never read through. Moving to SHA-256 needs a SHA-256 over bytes the substrate can compute; at present it can only parse SHA-256 hex.
