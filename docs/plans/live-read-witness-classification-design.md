...
> **Status:** P0–P2 landed (2026-07-14) — `v2.lens.live_read_classification` + fixture witnesses green by execution; P1 G2 call-reachability lens; P2 floor axis (iv) `runtime_data_dependency_touched` (#6630); P3 (candidate, unproven) wires the G2 manifest into floor admission. **P3 CANDIDATE, not landed** — the consumption half is written (axis (iv) decided by `LiveReadSelectionContext::runtime_dependency_touched_for_entry`, consulted by both the per-row selection and the precompute), but it is NOT sufficient on its own and must not be read as delivered. The producer's fact universe is entry-closure scoped: `fn_arrow_decl_facts_live()` reflects the eval context's modules, and that context resolves the lens entry alone, so a request root outside the lens's own closure cannot bind and classifies as a modelled refusal. Consuming that manifest widens selection rather than narrowing it. Producer universe completeness and consumption are ONE atomic change; neither half lands alone. Model-before-implement authority for how runtime tree/host reads become visible to affected-set attribution. Companion to [affected-set precompute pruning](affected-set-precompute-pruning.md), [witness cost locality](../src/v2/lens/witness_cost_locality.dag) (`v2.lens.witness_cost_locality`), and [v1 run-stability throughline](v1-run-stability-throughline.md) §1.
...
**Blocked on:** G2 path-intersection (not G1 module-closure alone) wired into `cli_run.rs` floor skip + carrier memoization receipts (§8a resolved-graph memo; §8b single-roster memo). IN PROGRESS (not resolved): the G1-only `runtime_data_dependency_touched_via_carrier_closure` interim named here is deleted on the P3 candidate branch, but deletion alone does not discharge this blocker — see the P3 status above. Axis (iv) now answers from the declaration-grain G2 manifest, so a carrier-importing witness is selected only when a diff touches a runtime read its own call-reachable declarations perform — the over-selection that made re-enrollment cost ~15m roster-gate / 85m+ lever_a witnesses on most `.dag`-touching PRs.
