...
**Blocked on:** G2 path-intersection (not G1 module-closure alone) wired into `cli_run.rs` floor skip + carrier memoization receipts (§8a resolved-graph memo; §8b single-roster memo). RESOLVED: the G1-only `runtime_data_dependency_touched_via_carrier_closure` interim named here is deleted. Axis (iv) now answers from the declaration-grain G2 manifest, so a carrier-importing witness is selected only when a diff touches a runtime read its own call-reachable declarations perform — the over-selection that made re-enrollment cost ~15m roster-gate / 85m+ lever_a witnesses on most `.dag`-touching PRs.
