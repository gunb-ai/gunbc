...
1 positional citation in this plan cannot be resolved against the current tree and is deliberately LEFT UNCONVERTED rather than repointed (DESIGN §3 standing rule; repointing to a near-miss symbol would falsify the claim). typescript.dag:1381-1414 claims that the ts_translation_rules_witness bundle expects 3 rows (add + type_alias + pr3), but the current v2.extdeps.languages.typescript ts_translation_rules_witness has only 2 rows (add + type_alias) and no pr3 row. This is evidence that the plan describes a superseded tree state, not merely a stale pointer; resolving it requires re-grounding the plan, which is out of scope for a citation sweep. dissolve-on: feature:typescript-gap-census-regrounded — the plan's claims are re-verified against the live tree, at which point this citation is either converted or the row deletes with the claim it supported.
