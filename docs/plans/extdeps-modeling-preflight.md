...
- **Enrollment is staged** behind `gunbc.extdeps_scope_frontier`, with the grain stated (codex `review 46258`): per-PR merge-gated (hermetic) — every rostered path resolves on disk, the three rosters stay disjoint, carrier files byte-declare the scope, RED controls live; wet-lane (discovery-excluded, documented local recipe) — the live `dag/extdeps` enumeration that refuses a file in none of the three, and the manifest freeze diff. A new module can never join the frozen manifest (remove-only, count derived), so its only landing states are carrier or machinery; per-PR merge-gating of the new-file direction arrives at the `v2.lens.mandatory_tag` region promotion this frontier dissolves into.
