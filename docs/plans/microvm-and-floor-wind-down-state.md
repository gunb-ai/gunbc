...
`ConvergedSlotNetwork` (that row stood until the converged run below).
...
**(4) has executed, and srv1's slot network is converged.** fleet-converge run 36043010087
(`microvm_network_apply`, srv1, 2026-09-24) staged, installed and read the ruleset back under
the administrator principal (listing digest `da7a00c5…9099`); run 36045264008
(`microvm_network_observe`, srv1) then minted a `ConvergedSlotNetwork` at generation
36045264008 carrying the same digest, fourteen taps with v6 disabled, and zero helpers.
`microvm_network_host_mutation_principal_frontier`, `host_ruleset_installer_frontier` and
`converged_slot_network_producer_frontier` were deleted on those receipts.
