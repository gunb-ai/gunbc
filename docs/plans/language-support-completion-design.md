...
**Status: DRAFT for operator review (2026-08-02) — design-note-first; no code lands from this note.** Roadmap registration and dispatch are operator sign-off steps. The operator's framing that seeded this program: language support in this repo repeatedly reaches "80% and good enough" and stops — hand-authored `ts_*` target AST at the product layer being the live specimen — and that is tech debt, not support.

## 0. The grounding receipt (executed 2026-08-02, this is not a feeling)

- `realization_vocab_live_corpus_receipt_holds` (`src/v2/test/claim/long/realization_vocabulary_containment_witness_test.dag`) — the live-corpus containment receipt for the target-AST wall — **executed FAIL** against main. Its own note declares it NOT roster-enrolled and excluded from discovery AND the affected-set falsifier ("local recipe until then"), so the wall's live consumer has been offline since the fast-lane re-home: DESIGN §6's coverage-by-illusion tier, executing.
- The unexempted leak population, exactly three paths: `dag/gunbc/roadmap_component.dag` (imports `extdeps.languages.typescript.program`; **2,025 `ts_*` call sites** in a 2,689-line product module), `dag/gunbc/design/theme_transition.dag` (same import; media-query/timer/listener behavior authored as TS AST), `src/v2/workflow/effect_plan_bash_materialize.dag` (imports `v2.extdeps.languages.bash_build` outside every realization-edge prefix). The exception roster carries only `node_http_server_emit.dag` and `dag/tools/build_step.dag`.
...
- **LANG-2 no-new-debt gate.** Enroll the live-corpus containment receipt on a scheduled CI lane (its note's named dissolve-on, still unfired); add the three leak paths as declared exception rows with dissolution triggers; a NEW leak refuses. This mirrors the shell→intent freeze and precedes any migration — stop growth before paying debt.
...
- **TS-0 freeze.** The gate lands first: roadmap_component + design/theme_transition (+ the bash materializer) become declared frontier rows with dissolution triggers; new leaks refuse. No migration before growth stops.
...
No code, no lens edits, no roadmap nodes, no registry rows — this md is the sign-off artifact. The first implementation PR after sign-off is Wave 1's gate slice (enroll the live receipt + freeze the three leaks), because it is the only slice that stops the debt from growing while everything else is designed. **Dissolution:** this note dissolves into the typed registry rows and a registered `gunbc.plan.Plan` when Wave 1 lands, per house pattern.
