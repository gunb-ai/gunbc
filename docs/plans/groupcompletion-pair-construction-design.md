...
- **Deep-seven burn-down measurement:** rerun the classifier-v3 probe (`gunbc compile --source-root dag --source-root src/v2 --entry <module> --target rust --dependency-pool-index primary-precedence` → `cssl_assemble` → `cargo build --release --lib`, `CSSL_STD_SEED_LINK=1`) across the same 7 modules (`06_translate`, `04_infer`, `05_eval`, `05_emit`, `emit_host`, `emit_module`, `materialization_carriers`), before/after, and report the measured delta in the `COPRODUCT_NATIVE_NUMERIC` E0308 bucket plus the `GroupCompletion<Rc<Nat>>` arithmetic-trait E0369 residue from Root 4 — the 2026-07-24 doc's baseline (3 occurrences of the `expected i64 found GroupCompletion<Rc<Nat>>` shape in 06_translate alone, 1–5% share across all 6 deep-family modules) is the pre-fix number I'll cite; a reused-authority-pattern-plus-cargo-checks-pass without this rerun is explicitly not accepted as a fix per your mandate.
