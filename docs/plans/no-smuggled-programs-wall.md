...
1. **This PR — design doc + corpus census receipt, sign-ready.** No code. The emit-seam type change (§3) is load-bearing (`06_translate`/`05_emit_orchestration` are DESIGN-named); **the operator signs this doc before the type change lands.**
2. **PR 2 — HALF A Slice A0** (orchestration→bash carrier + grammar-owned residual joins), byte-identical-gated (§3.4), behind the sign.
3. **PR 3 — HALF B lens** (grammar-classifier swap + `AmbiguousParse` bucket + census receipt), `AuditOnly` → census-clean → `Blocking`.
4. **PR 4+ — HALF A Slice A1** (`06_translate` serialize family), its own load-bearing sign checkpoint.
