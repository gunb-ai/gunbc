...
2. **Gate integration is small and precise:** extend `should_run_gates` / `any_rust_monolith` (`dag/tools/rust_stage0_gates.dag`) so a changed `.dag` path in the union of declared rust-test closures ALSO returns true.
