...
6 positional citations in this plan cannot be resolved against the current tree and are deliberately LEFT UNCONVERTED rather than repointed (DESIGN §3 standing rule; repointing to a near-miss symbol would falsify the claim). machine_constraints.dag:34-46 claims a TRACKED PARAMETER GAP text absent from std.machine_constraints; bit.dag:25 also claims a cardinality/header concession absent from std.bit; measure.dag:8,245 claims a deferred Refined<Measure> site absent from std.measure; machine_constraints.dag:109 claims parse_type_rhs_after_eq, absent from that module and the current source tree; 03_normalize.dag:90 claims a dag_surface_type_alias_rhs normalize hook absent from v2.compiler.normalize; and machine_constraints.dag:43-46 claims a phantom-to-value bridge text absent from std.machine_constraints. This is evidence that the plan describes a superseded tree state, not merely a stale pointer; resolving it requires re-grounding the plan, which is out of scope for a citation sweep. dissolve-on: feature:cardinality-refinement-regrounded — the plan's claims are re-verified against the live tree, at which point each citation is either converted or the row deletes with the claim it supported.
