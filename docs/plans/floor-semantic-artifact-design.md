...
`gunbc.witness_floor_workflow` emits `.github/workflows/witnesses.yml`: checkout, toolchain, build, then one step invoking `claim_executor --required-floor` over two source roots. Read on main, four load-bearing properties hold:

- **There is no artifact step.** The floor's typed, located, counted diagnostics exist only as bytes in a foreign executor's job log; semantic output has no durable carrier.
...
That is the state-space conflation, not a presentation inconvenience. The symptom that surfaced it: the `gh` CLI and the API job-log endpoint return different prefixes for one long step, so the terminal refusal that decided a run can be absent from the surface a reader reaches for. **Precision, because the weaker claim is the true one:** this establishes that the retrieval surfaces share no dependable completeness contract, not that GitHub's stored log discards bytes. The conclusion stands — a channel whose consumers can receive different prefixes with no completeness verdict is unfit to carry semantic authority.
...
GitHub Actions executes and stores the first instance; it does not define the channel. Chosen ahead of live tailing because it proves the strongest invariant first: **a complete semantic answer either arrives as a verified whole, or refuses.**

## 3. Channel roles

Roles are content of the Work's output contract, reached through the existing `OutputContractRef` — not a new field on `Work` and **not** a Demand fact: a Demand may choose a channel, a destination and a completeness requirement; it may never relabel an output from diagnostic to semantic.
...
Ordering is **total within one emission log and incomparable across logs**. Comparison is three-valued — before / equal / after / incomparable-carrying-both-log-refs — for the reason `std.content_hash` `compare_content_hash` already gives for cross-family digests: two cursors from different logs are not known-different but incomparable, and collapsing that into a definite answer reports a comparison that never happened.

Offset ordering, range validity, containment, continuity, gap and overlap live **once** in the ordered-emission authority. Handlers consume an already-decided range and never define cursor arithmetic. Were the cursor an opaque brand, each handler would supply its own comparison and two would eventually disagree about whether a gap exists — a silently missing frame, the one property this model exists to prevent.

## 5. Production before rendering

`claim_executor` must not write text and reparse or split it. Where the floor holds the typed row outcome it emits an ordered semantic frame and **independently** projects a human line to stdout. `std.observation` already carries typed process events and outcomes as distinct model states rather than glyphs; reuse that vocabulary where it preserves the real facts, and wrap the observation transition around the exact floor result carrier rather than flattening richer refusal coordinates into a prose field.
...
An events file plus a terminal manifest, written after everything else and carrying schema, subject identity, channel identity, roster digest, the range `[0, N)`, the events blob digest, terminal outcome and counts.
...
Uploading a sidecar while the job result still comes from the process exit code leaves the artifact observational, not authoritative. Capture the host termination, verify locally, upload, then derive the conclusion:
...
Ours: the semantic frame schema, sequencing, terminalization, the completeness decision, content identity and verification. The first storage and transfer binding may be GitHub's artifact service — making GitHub Actions a foreign executor plus a materialization provider plus a diagnostic console, **not** the semantic-output authority. The same artifact later moves to a content-addressed store with no change to Work, Demand, cursor, frame or verification semantics.
...
No code lands from it. Nothing here is measured except the four workflow properties in §1 and the retrieval-surface disagreement, which is a retrieval fact rather than a storage fact. No frame schema is specified. The relationship to the fabric's delivery carrier is a reference, not an implementation.
