...
| "one mechanism dominates all top-25 (~80%+), so fold ≤60s after W2" (this doc, first draft) | **superseded at phase grain** — post-#6750 slow-module split is 60.5% inference / 39.4% provenance (probe-grain review). Survives only as the mechanism-grain hypothesis (one un-visited walk under both phases, §2.4), which W2 step 0 must confirm; §6 fold target marked provisional accordingly. |
...
- **W2 — kernel root fix (days).** Primary suspect **named and verified in-source** (§2.4): the un-visited sigs-env DAG walk in `lookup_resolved_sig`/`lookup_in_parent_chain`, plus the quadratic `take` prefix copy. Step 0 = re-run the mechanism split on the then-current tree and confirm the single-mechanism hypothesis (both phase shares riding the walk — §2.4); then fix in `04_sigs.dag` authoring + regen. Candidate fix shapes, precedent-guided: (a) **merge-at-construction** — one deduped lookup structure per module env, the `merge_envs` shape (reconcile 81%→6%), carrying M1's caution that construction along the import DAG must *share*, never duplicate (content-key the merged env); (b) **walk-time visited-set** keyed by module identity — the `func_reaches_self` pattern already in the same file. Priced by before/after **probe counters on identical input** (§2.4 oracle: 902.8M-class → ~53.3M-class) plus fold wall; RED = planted diamond-import fixture asserting the per-path→per-node collapse. Fold target: **re-derived at step 0** from the measured probe reduction (§6 note — the prior ≤60s rested on session-base single-phase dominance, which §2.4 shows does not hold post-#6750); the `ci_spec`-class ~8ms→µs field-check shape goal stands.
...
| + W2 (kernel root fix — the §2.4 walk) | **~35–45 min** | ~60 min | ~45 min | **≤60s (provisional — see note)** |
...
Anything that misses its row's number by >30% triggers a written root-cause note in this doc before the next workstream proceeds (no silent target drift). **The W2 fold target is provisional until W2 step 0**: §2.4's post-#6750 measurement (60.5/39.4 phase split on slow modules) already fails the single-PHASE-dominance premise the ≤60s row was first derived under; the surviving hypothesis is single-MECHANISM dominance (one un-visited walk under both phases), and step 0 either confirms it — target re-derived from the measured probe-count reduction — or this table re-issues under the >30%-miss rule. The build job (~13 min) and regen (~21 min) become the dominant terms at the end state; both have known follow-ups (prebuilt-bin cache; regen input-closure scoping already landed #6732) explicitly out of this plan's scope.
