...
**Conclusions.** (a) The environment gap is gone entirely (§3.2 parity) — CI's 47-min gate ≈ today's tree's 40-min fold × nothing. (b) "CI getting longer and longer" is **R1 × R2 compounding live**: ~2.2k lines of tower additions made importer modules 6–38× slower in one day, because the kernel's cost is superlinear in the visible tower. The blown-up modules' own files did not change. Whole-fold: **346s → 2,413s in one day of merges** (top-10 modules = 49%, top-25 = 78% of the new fold; new #1 `host_identity_assimilation` 214.5s). (c) Bisect: blowup **fully present at #6770** (leg-1 receipt: same 142/172/218s markers) ⇒ guilty ∈ {#6769, #6759, #6757, #6750, #6770}; leg 2 (at #6759) in flight — the exact merge gets named here before this PR merges. Candidates by file trail: #6759 (REST transport axes — `host_effect_realize` imports redfish/REST) and #6757 (+96-line `host_target_carriers_behavioral_transport` + a `ci_layer_roots` hub edit). (d) Until W2 lands, every further tower merge repeats this — the fleet lane should see this table.
...
| + W2a (revert/repair the guilty merge's cost shape) | back to ~pre-growth (~65 min) | ~85 min | ~70 min | ~350s |
