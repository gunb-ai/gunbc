...
#6506 landed the first mechanisms. Treat them as the current baseline, and as *approximations* of the model — good-enough interims, each standing in for a modeled quantity that Stages 3–5 will derive:

- **Explicit budget exists, but hand-set.** `gunbc_ci_fast_lane_witness_eval_budget = 5s` (`ci_floor_plan.dag`), enforced fail-closed as a typed `EvalBudgetExceeded` (in-eval poll + completion-side). This *approximates* the cost axis: a single flat threshold stands in for a per-claim cost estimate. It is the §5 smuggled-heuristic (a bare threshold selecting an arm), honest only because it refuses loudly and countably — retired by Stage 4 (derived cost).
