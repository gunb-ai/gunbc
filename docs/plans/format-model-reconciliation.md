...
Delete this doc when the remaining real record emitters (the C5 digest/accelerator tail) route through `std.serialize` `serialize_record_doc` with byte-identical witnesses. The legacy FormatModel/OutputFormat collapse, runner manifest/JSON swap, gitignore line-layout de-fork, dnsmasq boundary, and CSS typed-row migration are already complete.
