...
- **Weak digest:** object refs use the non-cryptographic structural digest (`std.content_hash` `content_hash_of_value`). A collision is refused at put (`FabricObjectCorrupt`), never read through, and it is a **content-identity** failure — it can enable an isolation failure given authorization and sharing, but tenant isolation stands on its own and never on the digest. The sentence this bullet used to carry — that moving to SHA-256 needs a SHA-256 over bytes the substrate can compute, which it lacked — was true when written and is no longer: `extdeps.crypto.sha2` `sha256` / `sha256_hex` compute FIPS 180-4 SHA-256 over `List<UInt8>`, witnessed by `test.claim.sha256_fips_witness_test`. What is still open is **which realization binds that digest to `fabric_object_ref_of`**: the pure kernel is emit-blocked (`std.operator_realization` refuses infix arithmetic on `Compose<UInt, MachineWidth<N>>`, so `std.bitwise` `word32_add` runs only interpreted), while `extdeps.crypto.hash` `sha256sum_file_command` / `sha256sum_line_digest` executes today at one process spawn per object and would make minting effectful — which is what makes `fabric_object_verified` a pure check today. Per DESIGN §3 that is a realization choice behind one interface, decided on measured throughput against object size, not a prerequisite ordering.
