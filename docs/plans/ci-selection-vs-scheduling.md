...
## 7. 2026-07-04 — the run-all baseline stood down IN CI (opt-in inversion)

Step 1's "sound baseline" stopped being a baseline the day the floor stopped *completing*: at ~1,551 discovered witnesses the composed pass hit the 90-minute job timeout on every run — maximum cost, **zero delivered signal**, and every merge effectively fail-open (a gate that always times out gates nothing). A run-all that never finishes is not a superset of the affected set; it is the empty set billed at full price. Operator decision: invert to **declared opt-in enrollment** — `CiSpec.witness_entries`, projected from `CommitWitnessClaim` rows on the `GithubActionsCiJob` surface of `commit_gate_roster`; an empty roster puts zero witness-corpus nodes in the floor plan. This is NOT the §0 cost-routed selection this doc rejects: no affected test is demoted by expense, because *no* witness currently gates by discovery — the inversion replaces an always-red-by-timeout gate with an honest, completing floor (gates + declared witnesses), and the discovery roster remains the local path (`claim_batch --roster-from-discovery`). The §3 ladder is unchanged as the *destination*: land affected-set selection + floor memoization, then re-enroll the corpus through the affected set and dissolve the opt-in roster (dissolve-on marker lives on `ci_witness_optin_inversion` in `gunbc.commit_workflow`). Found during the inversion: `resolve_floor_runner_context` had been failing on a comment block the v1 grammar cannot parse (`affected_set_floor_runner.dag`), so the affected-set skip silently fell back to full-corpus on every run — step 2 was dead in CI while the doc described it as CI-enrolled.
