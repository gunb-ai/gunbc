...
| **F5** | self-host frontier runner (`v2.compiler.emit_host` `run_test_claim_module_emit_vs_eval`: emit → build → run → compare-to-eval) | ✓ machinery **for Rust only**, run **OFFLINE** (excluded from discovery). Rust is **not** self-hosted, and after the roster cut there is **no symbol that counts this** — `compiler_frontier_emitter_produced_count` is deleted and nothing replaced it, so the bar is stated qualitatively on purpose: **no module carries a producer-qualified promotion.** The surviving authority is `v2.compiler.self_host.emitter_producer_provenance`, which holds the producer facts as structure rather than as a tally — `EmitterProducer` x `EmissionQualification`, `mint_producer_emission_receipt` as the only route to a receipt, and the realized-closure wall (`seed_emit_rust_reachable`) under which a `V2Emitter*` mint is unwritable while `v1.compiler.emit_rust` is in the closure. Re-deriving a count is a future obligation, not a present fact. |
...
| --- | --- | --- | --- | --- | --- |
| **rust** | compiled | compiled; bar-c; F2 **partial** (`^rust_token_unwired_{else,loop,match,fat_arrow}`, `v2.extdeps.languages.rust` `rust_value_expression_projection`); F5 emit-vs-eval fixture **OFFLINE** | **zero producer-qualified emissions, and no roster to count them against** — the 27-row `SeedRetained` roster is DELETED, so this cell no longer has a denominator and does not invent one. `v2.compiler.self_host.emitter_producer_provenance` `mint_producer_emission_receipt` is the only route to a producer-qualified receipt and no v2-emitter-produced promotion exists; behavioral-equivalence standing is carried by `BehavioralProbeBinding`, whose `BehavioralProbeUnbound` arm is the honest state for a receipt that has produced source and nothing more. Reference model, **NOT self-hosted** | F2 completion; F5 self-host generalization | (reference model) |
