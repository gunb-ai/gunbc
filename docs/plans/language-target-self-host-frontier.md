...
2 positional citations in this plan cannot be resolved against the current tree and are deliberately LEFT UNCONVERTED rather than repointed (DESIGN §3 standing rule; repointing to a near-miss symbol would falsify the claim). Both emit_host.dag:164-172 citations claim a central host_tool_program_name switch, but that symbol is absent from v2.compiler.emit_host; the current process_program_name is generic and therefore is not a valid replacement for a citation supporting the claimed defect. This is evidence that the plan describes a superseded tree state, not merely a stale pointer; resolving it requires re-grounding the plan, which is out of scope for a citation sweep. dissolve-on: feature:language-target-self-host-frontier-regrounded — the plan's claims are re-verified against the live tree, at which point each citation is either converted or the row deletes with the claim it supported.
...
| **spice** | analog format | **bar-b only** — the witness compares emitted text to a **golden** (`v2.test.formats.spice_rc_ngspice_oracle` `spice_rc_ngspice_op_holds`); it **never runs ngspice** | bar-c green + frontier row `self-host: N/A (analog)`. **No Modelica carrier in-tree** — dual-emit is future, not present | F4(`ngspice`) row-bound (bar-b → bar-c); Modelica carrier does not yet exist | **emit-generality (continuous)** |
