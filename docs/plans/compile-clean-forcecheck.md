...
`compiler_tests.rs` is GENERATED, not hand-written: it is emitted from `src/v1/compiler_tests_rust.dag` by the stage0 emit and is claimed by the derived generated population (`gunbc.stage0_rust_source_lifecycle_scaffold derived_generated_stage0_repo_paths`), so its line growth is witness text authored in `.dag` and is not hand-Rust surface. The authority for this receipt is the carrier, `v1.compiler.core` `compiler_diagnostic_seed_projection_note`, beside the coproduct whose extension forces the arms. THIS DOC IS A PROJECTION of this plan and is regenerated by the generated-artifact gate, so the receipt is authored here; a receipt written into the .md alone is reverted on the next heal.
