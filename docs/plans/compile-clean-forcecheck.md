...
The (B) wall adds SIX variants to the `CompilerDiagnostic` coproduct in `.dag` (`MethodNotFound`, `MethodExistenceUndecided`, `MethodExistenceFrontierAdmitted`, `ReceiverTypeUnestablished`, `DeclaredTypeConformanceUnjudged`, `FrontierOccurrenceBudgetExceeded`). Each forces an arm in two TOTAL matches in the hand-maintained seed transport `cli_run.rs` — `compile_clean_diagnostic_histogram_key` and its method-name extractor. Without those arms the seed does not compile, so they are the mechanical consequence of the `.dag` change rather than host capability chosen in Rust.
...
**Checkable receipt — the hand-Rust carrier census is flat; only arm count moved.** Hand-written `fn` count in `cli_run.rs` is 742 at `origin/main` and 742 on this branch, and `git diff origin/main -- src/v1/stage0/src/cli_run.rs` is 18 added lines containing zero `fn`. Both figures are re-derived from those two commands on every variant addition rather than carried forward — the receipt read five variants and 13 lines after a sixth landed, which made the receipt itself the stale-prose defect it exists to prevent (review 45501).
