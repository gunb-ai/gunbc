...
Status: parked design row. Pins the load-bearing operational facts that the `v1_compiler_emit_rust.rs` header carried before the codebase-wide comment ban deleted it (deletion was operator-directed; git history holds the full original at `git show <pre-ban-sha>:src/v1/stage0/src/v1_compiler_emit_rust.rs`). A comment described the regen hazard but enforced nothing — the structural home for the fact is this gate. Breadcrumb: `node://adhoc-80af9ff8-40f` (#5514 deferral).

## Why this row exists

`src/v1/stage0/src/v1_compiler_emit_rust.rs` is a **hand-synced mirror** of the `.dag` authority, **not** a clean regen fixpoint. There is no `regen --verify` gate, so a blind regen would silently overwrite the two hand-resolved drifts below and regress the wire-policy fix. The protection was always absent; this row makes the fact durable so it rides the gate when it lands.

## The two hand-resolved local drifts (must be reproduced by a faithful regen)

1. **cargo header inlined** — the file inlines its cargo header rather than importing the deliberately-unwired `extdeps_cargo_version` orphan module.
2. **policy ref/value alignment at `wire_value_serialize.rs`** — the wire-policy reference/value alignment is hand-resolved here; a naive regen loses it.
...
## The third drift — 29 lens/census builtin type-rows the faithful regen DROPS (AUTHORED into the `.dag` authority; verify-flip still operator-owned)

Static analysis (no code flipped, no groundings authored) of the `builtin_function_registry()` §3 fork located the drift: the registry is duplicate authority — the `.dag` side `src/v1/04_method.dag` (`fn builtin_function_registry`) and the hand-synced Rust mirror `src/v1/stage0/src/v1_compiler_infer_method.rs`. The Rust mirror registered **108** name→return-type rows while the `.dag` authority registered only **79**; **exactly 29 type-rows lived in the Rust seed and were absent from the `.dag` authority** (24 Class-R map-insert keys + their 5 Class-C `_elem` type-variable companions, carried inside the `_facts` parents' return types). A faithful `regen_stage0` (which emits `v1_compiler_infer_method.rs` from `04_method.dag`) DROPPED them, and every lens/census `.dag` witness resolving a dropped name then failed closed ("function not found in scope", `compile_clean_forcecheck.md` probe 2).

**PREP landed (this row):** the 24 Class-R type-rows are now authored into `04_method.dag`'s `builtin_function_registry` in the **exact Rust-mirror insert order**, so the `.dag` authority registers all 95 keys and the registry-drift component of the regen is now lossless by construction. This is a *type-registration only* — the realization stays the seed host builtin (no pure-`.dag` def authored; that remains the trap, dissolution gunbc#5364). The rows are added to the **global** registry, so they widen the already-flagged seed-intrinsic leak (`compile_clean_forcecheck.dag`: the registry is a marked BRIDGE, not tree-scoped); scoping the registry is the same gunbc#5364 dissolution, out of scope here. **The enforcing `regen --verify` flip is NOT landed** — it returns to the operator for sign-off (it also depends on the cargo-header and wire-policy drifts above and the ~150 emitter-completeness gaps).
...
**Why this is the third drift, not new debt:** like the cargo-header and wire-policy drifts above, the 29 rows were a hand-sync the Rust mirror carried that the `.dag` authority lacked. The `regen --verify` wall (a clean regen must reproduce the committed seed bit-identically) cannot go green until the mirror is faithful in **both** directions — and the construction-correct direction was to add the rows to the `.dag` authority (so regen emits them), not to special-case them out of the verify. That direction is now taken for the registry component (PREP above); the wall itself stays deferred to the operator and to §7 fixpoint convergence.
