...
std-surface / extdeps-cited-rows split is *not* a fork and was refuted).

| module | location | the fork | verify |
| --- | --- | --- | --- |
| `std.languages` | data rust_language/rust_spec/rust_* (and go_*/python_*/typescript_*) ↔ dag/extdeps/languages/{rust,go,python,typescript}/{syntax,types,emit,primitives}.dag | std/languages.dag re-models Rust/Go/Python/TS syntax, type-mappings and primitives that dag/extdeps/languages/<lang>/ already model — two homes for one per-language spec (verified: | CONFIRMED |
| `std.types` | Milliseconds (line 151), Seconds (line 152), Duration (line 150) | Duplicate-models time units that std.measure already owns: std.measure.Millisecond (Measure<Time,Milli,Nat>) and std.measure.Second (Measure<Time,One,Nat>) are the single authority | CONFIRMED |
| `std.types` | SemVer (line 127) | `SemVer = String` forks extdeps.version.semver, which already models the semver.org spec faithfully as SemVerVersion { major, minor, patch, prerelease, build } cited to `semver.org | CONFIRMED |
| `extdeps.formats.elf.segments` | PhdrDecodeOutcome<T> / PhdrDecodeReason | Parallel decode machinery duplicating types.dag's generic ElfDecodeOutcome<T>/ElfDecodeReason: PhdrDecoded/PhdrRejected mirror ElfDecoded/ElfRejected, PhdrTruncated{required_bytes, | CONFIRMED |
| `extdeps.gnu_coreutils` | diff_recursive_flags / diff_recursive_brief_flags | The diff operation is forked here: `diff -r` / `diff -rq` flags live in the coreutils module while extdeps.diffutils (extdeps/shell/diffutils.dag) already models diff as the `shell | CONFIRMED |
| `extdeps.gunbc` | packages (List<WorkspacePackage>) / WorkspacePackage / CrateRole | A hand-drawn roster of the Rust workspace crate layout (gunbc-*, daglang-* with CrateRole) duplicate-models the crate-partition concept whose derived authority is src/v2/workflow/r | REFUTED |
| `extdeps.languages.rust.types` | integer_types / float_types | Bare String lists of Rust integer/float type names duplicate the type-name catalog already modeled as RustPrimitive.target_name rows in extdeps.languages.rust.primitives (rust_grou | CONFIRMED |
| `extdeps.llm.cli` | service claude.Invoke (operation Run, cli.dag:152) | The claude.Invoke.Run shell-transport argv re-spells the exact claude-session invocation flag sequence (permission_args ++ --settings/--effort/--session-id/-n/--append-system-promp | CONFIRMED |
| `std.baseboard` | BaseboardManufacturer = AsrockRack | Re-coins a hardware vendor as a freshly minted single-variant enum; DESIGN §3 dissolves 'vendor' into a single Vendor<Domain> authority whose cited company entities live one-file-e | CONFIRMED |
| `std.markdown_markup` | markdown_inline_to_markup_nodes | Re-implements the MarkdownInline -> HTML-element mapping (code/em/strong/a/img tag map) already present in std.markdown.markdown_inline_to_fragments, differing only in target vocab | CONFIRMED |
| `std.markup` | MarkupNode/ElementNode/TextNode/MarkupAttr vs Fragment/Element/FragmentText/Attribute | Two byte-identical-shaped vocabularies for the same HTML element tree: ElementNode{tag,attrs,children} == Element{tag,attributes,children}, TextNode == FragmentText, MarkupAttr ==  | CONFIRMED |
| `extdeps.cloud.gcp.secret_manager` | SmReplica.location: String | A replica location is a GCP region, a concept already modeled in extdeps.cloud.gcp.gcp (GcpRegion + the cited `regions` rows), but here it is re-carried as a bare String instead of | REFUTED |
| `std.bit` | Word8 / Word16 / Word32 / Word64 / Word128 | Five separate hand-rolled width types duplicate the machine-width axis std.machine_constraints already parameterizes as MachineWidth<N> (std.integer: Int8..UInt128 = Compose<Int, M | CONFIRMED |
...
| `std.filesystem` | fn is_text_encoding(e: Encoding?) | std.filesystem re-defines is_text_encoding, which already exists as std.encoding.is_text_encoding(e: Encoding); this is a second (Optional-lifted) copy of the same text-vs-binary p | CONFIRMED |
| `std.node` | compiler_inductive_fields (InductiveField rows) | Hand-authored table restates the inductive-field structure the Node/InferredNode/MatchPattern/MethodSemantics type declarations already carry (dual representation), using bare-Stri | CONFIRMED |
...
the cited table moves. The house exemplars (unicode blocks, RFC method tables).

| module | location | the table | verify | suggested |
| --- | --- | --- | --- | --- |
| `std.baseboard` | BaseboardManufacturer (= AsrockRack) and BaseboardModel (= AsrockRackAltrad8ud1l2t) | Vendor-specific closed enums naming one real board (ASRock Rack ALTRAD8UD-1L2T) are cited catalog data sitting in std; extdeps/boards/asrock | CONFIRMED | Move the vendor model/manufacturer enum values (and the catalog rows) to extdeps/boards; keep only the agnostic shape (S |
| `std.bmc` | BmcFirmwareFamily = \| OpenBmc | A specific cited firmware project (OpenBMC) pinned as a std enum value and matched-on for dispatch, while the cited OpenBMC facts (factory l | PARTIAL | Home the firmware-family enum values + their dispatch in extdeps/bmc (per DESIGN §3 os pattern: std projects the agnosti |
| `std.languages` | all `data` rows (rust_language/go_language/python_language/typescript_language, rust_spec/go_spec/python_spec, *_statements/*_expressions/*_control_flow/*_literals/*_reserved_words/*_ops, markdown_format/html_format/yaml_format/toml_format/makefile_format/gitignore_format/css_format, json_record_knobs — lines ~365-1201) | A dag/std/* module carries the concrete per-language and per-format cited syntax/type/reserved-word tables; §3 keeps the framework types (La | CONFIRMED | Move every `data *_language/*_spec/*_format/*_ops/*_syntax/*_reserved_words` row into dag/extdeps/languages/{rust,go,pyt |
| `std.cache_identity` | CacheInterfaceProduct | A dag/std module carries a roster of specific real vendor cache products; per §3 the cited product enumeration is data that belongs in extde | PARTIAL | Relocate the vendor variants to extdeps (e.g. extdeps/cache/{gha,sccache,buildbuddy,cargo,rustup}.dag as cited rows); le |
| `std.os.types` | UbuntuDistributionCatalogRow / WindowsDistributionCatalogRow / MacosDistributionCatalogRow + UbuntuDistribution/WindowsDistribution/MacosDistribution variant enums + OperatingSystemProduct | Vendor-specific catalog-row shapes and cited distribution variant identities (NobleNumbat2404Lts, Windows1124H2Build26100, Sequoia15) are a  | REFUTED | Move the vendor catalog-row types + distribution variant enums to extdeps/os/{ubuntu,windows,macos}.dag beside the cited |
| `std.unicode` | wide_blocks (line 58), zero_width_blocks (line 42), zero_width_codepoints (line 51) | A dag/std/* module carries cited upstream data rows (Unicode block start/end codepoint ranges from UAX #11 / UCD); per §3 the cited table be | CONFIRMED | Move the block-range rows to a cited extdeps module (e.g. extdeps/unicode, cite UAX #11 + Unicode version); keep Display |
| `std.currency` | type CurrencyCode = Eur \| Usd + fn currency_minor_unit_exponent (Eur=>2, Usd=>2) | A cited ISO 4217 code list plus its minor-unit exponent data table sits in dag/std/; the framework (Currency Quantity, MoneyAmount<S>=Measur | CONFIRMED | Relocate the code enum + minor-unit table to extdeps/currency as cited ISO 4217 rows (beside the extdeps/pricing/* consu |
| `std.os` | operating_system_surface_ubuntu / _windows / _macos (distro_or_product literals) | Cited vendor product-marketing names ("Ubuntu", "Windows 11", "macOS Sequoia") and the Windows "(OS build ...)" version format are hardcoded | PARTIAL | Add a cited product_name (and OS-build display) field to the extdeps DistributionCatalogRow in extdeps/os/{ubuntu,window |
...
| --- | --- | --- |
| `extdeps.runtime.syscalls.linux` | sys_aarch64_read / sys_aarch64_write / sys_aarch64_mmap / sys_aarch64_execve / sys_aarch64_exit (linux_aarch64_syscall_subset) | The aarch64 syscall numbers (read=63, write=64, mmap=222, execve=221, exit=93) come from the generic table include/uapi/asm-generic/unistd.h, but the only cited source is the x86-specific arch/x86/... |
...
| `std.approximate_field` | RoundingMode / Precision / NanPolicy / InfinityPolicy / SignedZeroPolicy / SubnormalPolicy | Models the IEEE 754 floating-point standard verbatim (rounding-direction attributes ToNearestEven/ToPositiveInfinity/…, NaN/infinity/signed-zero/subnormal policies, GradualUnderflow/FlushToZero) but c |
...
| `std.bmc` | BmcProtocol (= Redfish \| Ipmi) and BmcFirmwareFamily (= OpenBmc) | Models real industry standards (DMTF Redfish, IPMI) and the OpenBMC firmware project with no data-note or ExternalAuthority in std; the extdeps twins (extdeps/bmc.dag cites the BMC authority, extdeps/ |
...
| `std.encoding` | base64_alphabet / base64_encode / base64_decode / Base64Variant | The Base64 codec and its two verbatim alphabet strings (Standard and UrlSafe) are exactly the RFC 4648 tables, but the module carries no citation naming RFC 4648 or its version; only a Terminal debt m |
| `std.exec_format` | ExecutableFormatFamily (Elf \| Pe \| MachO) / Relocation / SegmentPermission / LoadSegment | The surface names three real binary-format specs and models spec-derived structures with no in-module citation; only ELF has a cited downstream home (extdeps/formats/elf cites the gABI), while Pe (PE/ |
| `std.languages` | data rust_reserved_words / go_reserved_words / python_reserved_words (and every *_spec / *_language row) | Reserved-word lists, naming conventions and syntax templates are taken verbatim from each language's official reference with no data-note naming the upstream source or version. |
| `std.types` | HttpMethod (line 184), HttpStatus (line 118), TransportRequest/TransportResponse (lines 211,218) | std.types defines the HTTP method enum, status-code range, and transport request/response shapes — a real upstream spec (IETF HTTP, RFC 9110) — with no source/version note; the RFC 9110 citation lives |
| `std.unicode` | wide_blocks (line 58), zero_width_blocks (line 42), zero_width_codepoints (line 51), char_in_class (line 6) | The codepoint-range block tables and inline ASCII/codepoint ranges are transcribed from the Unicode Standard (UAX #11 East Asian Width + Unicode Character Database block ranges) with no data-note nami |
| `extdeps.bmc.types` | extdeps_external_authority_anchor (module extdeps.bmc.types) | Encodes version-specific Redfish schema enum registries (RedfishResetType, RedfishBootSourceOverrideTarget/Enabled/Mode, RedfishPowerState, RedfishPrivilege, RedfishAccountRole) but the anchor `www.dm |
...
| `std.markdown` | module std.markdown (HeadingLevel H1..H6, MarkdownAlignment, TaskListBlock, TableBlock) | Models CommonMark plus GitHub-Flavored-Markdown extensions (task lists, tables, column alignments, 6 heading levels) but carries no data-note naming the CommonMark/GFM spec or its version. |
| `std.markup` | escape_text / escape_attribute / escape_url (and unescape_* counterparts) | Implements HTML metacharacter escaping (&amp;/&lt;/&gt;/&quot;) and URL percent-encoding (%25/%20/%3C/%22) — real upstream specs — with no citation. |
| `std.numerical_contract` | FmaContractionPolicy / NumericalContract.FloatApproximate | Models IEEE-754 floating-point contract concepts (rounding attribute, fused-multiply-add contraction) with no data-note naming the standard; FMA contraction is a real IEEE-754 / C-standard concept, no |
| `std.process` | exit_code_success / exit_code_general_error / exit_code_misuse (0/1/2) + ProcessExit.ExitFailure.code | Models the reserved process exit-status convention (0 = success, 1 = general error, 2 = misuse of shell builtins) — a real upstream POSIX/shell standard — with no data-note naming the source or versio |
