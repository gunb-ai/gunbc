...
30. **A function name's meaning depends on the CONSUMER'S TRANSITIVE IMPORT CLOSURE, so adding
 ...
 `floor_preparation` is COLLATERAL — it reaches `object_store`
   ...
   *ambiguous* are different states with different repairs (one says no authority answers, the
   ...
   THIS ROW AND THE TRANSPORT-ARGV ROW MUST LAND AS ONE CUT, NOT TWO. (That row is numbered 29
   and is carried by gunbc#8916, unmerged at the time of writing — cited by PR rather than by
   number alone so the reference does not dangle if the numbering shifts on landing.) Row 29's fix is to run
