...
**Three rows below share one shape, and they are a stronger case together than separately: the v2 `Bool` representation fork, the missing-import class, and the Optional cast are ALL DECIDABLE FROM LOCAL OR ONE-MODULE INFORMATION, all currently held by authors looking, and all recurred under full awareness of the rule.** Each is filed with its own trigger; what they have in common is that the mitigation in place is diligence, and diligence has now demonstrably failed on each.

**HOW EACH RECURRENCE WAS ACTUALLY CAUGHT, because the absence is the argument.** The `Bool` fork: by a probe someone wrote after a witness returned a value they did not expect — no probe, no finding. The missing imports: one by a person reading a diff, one by a scheduled review agent reading the file; both files were GREEN, and nothing in the build, the typechecker or the witness roster could have surfaced either. The Optional casts: all three by the author re-reading code they had just written, none by execution — every instance sat on an arm no test reaches, which is exactly why they type-check and pass.

**None was caught by a decidable check.** The nearest thing to a mechanism in that list is a review agent, which is automated but advisory and probabilistic — it found one instance of one class on one file, and nothing about it makes the next instance findable. Read the other way: on every one of these, a fully green run was compatible with the defect being present, which is the property that makes them worth walls rather than reminders.
...
| v2 `Bool` representation fork (`==` result vs the declared `True \| False` coproduct) | **Below floor — one defect is silent (measured 2026-08-22).** v2 carries TWO representations of `Bool` with no bridge, and each is unusable by the other's eliminator. (1) THE ROOT, a §3 fork: `==` yields a host `true`/`false`; `v2.std.logic` declares `type Bool = True \| False`. (2) LOUD: matching the coproduct's constructors over a comparison-derived Bool is reported **non-exhaustive** — a match covering BOTH arms of a two-constructor closed coproduct — and returning a coproduct Bool is refused at the return boundary as "returned `Bool`, not Bool". (3) **BELOW FLOOR:** `!` applied to a coproduct Bool answers `false` for both `top` and `bottom` — no diagnostic, no refusal, a fabricated plausible answer over a representation it cannot read. Only (3) is outside the ladder; (1) is what makes (2) and (3) possible, so a fix that only teaches `!` to refuse non-Bools repairs the symptom and leaves the fork | **R3 / structurally impossible** — one representation, or a declared total coercion between them | Nothing yet: decidable and fully grounded, so this is a **wall, not a ratchet**. No modeling gap blocks it | Two independent probe modules, 2026-08-22 (fierce-lynx-647 + bright-ferret-335), each with a passing control in the same run: `!(0 == 1)` → `true` beside `!bool_boolean_algebra.bottom` → `false`; `if 0 == 0` passes beside `match b { True .. False .. }` → non-exhaustive on `true`. **Zero live exposure, and the reason is the finding:** `bool_boolean_algebra`'s own `meet`/`join`/`complement` are all written as `match a { True => .. }` — the std Boolean algebra cannot accept a Bool v2 produces — and 10 `extdeps/languages/*` modules declare it as their `BooleanAlgebra<Bool>` grounding, while `.meet(`/`.complement(` have **zero call sites** corpus-wide. `required_floor` `long_home_storage_agreement` matches on `Bool` params and has no v2 caller. Measured 1785 `if` sites vs 29 `True =>` in src/v2, most of the latter being Rust source-text fixtures or a different coproduct — so the exercised form is `if`, and the coproduct side is unexercised rather than harmless. **NOT ENROLLED:** the probes were run one-off and deleted; nothing reds if this changes | pick one representation (or author the coercion), then re-run both probes as permanent regression controls per §4b meta-obligation 4 |
| Referenced symbol with no import (v2 whole-pool resolution) | **Mitigatable, and the mitigation is an author looking (measured 2026-08-22).** A v2 module may reference a cross-module symbol it never imports: `resolve_in` finds it in the assembled POOL, so the module typechecks and every witness passes. The file is green by accident of closure composition, and a narrower closure DOES refuse it — **measured 2026-08-22**, two arms over byte-identical source: with the declaring module present in the source root but unimported, `fn identity(w: Widget) -> Widget` resolves and reaches evaluation (failing only on a missing argument); with that module removed from the root, the same file refuses `unresolved type 'Widget'` at both positions. **Scope of the measurement, because the two halves differ:** a bare TYPE reference resolves pool-wide — which is the shape both live instances took — while a bare VARIANT CONSTRUCTOR does not (`MadeWidget` refused `undefined variable` even in the wide root), so the silent surface is type positions, not every bare name. This is the ordinary compiler floor ("names resolve"), and it is **silent** — no diagnostic, no count, so the deficit's frequency is zero by construction. **Why it is filed as a class rather than a defect:** two instances landed in one module inside an hour — `Symbol` used 3x, then `LedgerPublication` used as a parameter type one commit later — the second AFTER the class had been named to the author and the reason a green suite cannot see it explained. Vigilance demonstrably does not hold it | **R3 / structurally guaranteed** — a module's referenced symbols and its import block are both in the `Node` tree, so "every referenced symbol is imported or locally declared" is decidable from one module's own source at ingestion. A **wall, not a ratchet**, and no modeling gap blocks it | **Unbuilt, not blocked** — decidable and fully grounded today, so by §4b(2) this is *can climb now but unbuilt*, the only one of the three stall states that is nobody's dependency. Its reach is the reason it is not free: the wall applies to every v2 module, not to this class's two instances | Both instances found by a READER, neither by execution: `Symbol` (3 uses, import deleted by a `sed`) and `LedgerPublication` (parameter type, `floor_terminal_ledger_test.dag`) each passed a full green run. The counter-evidence that makes this invisible rather than merely missed: the resolver DID catch a stale import of a RENAMED variant in the same module, which reads as import coverage — it covers WRONG imports, not MISSING ones. Related from the other direction: §6 records the corpus-wide import censuses DELETED 2026-08-11, and `dag/**` import-stripping is blocked on this same Class B. **AND THE SAME MECHANISM IS A §6 COST-SHAPE DEFECT, measured 2026-08-22** — one cause, a correctness hole and a cost shape. Because a bare name MAY resolve pool-wide, every resolve must first build a corpus-wide bare-reference index: on a `claim_batch` cost-partition, `load_bare_edge_index` was **41.7s of a 47.9s parent** and `edge_index_tree_census` alone **32.6s**, over `source_files=3857` / `bare_eligible=731`; end to end a cold narrow-entry resolve measured **93.4s wall / 1.93 GB peak RSS** on a contended session container. **Which claim this row makes, because the two are related and not identical:** the row claims the WALL (a referenced symbol must be imported or locally declared), not the index removal. The wall is what makes the index unnecessary; removing the index is a separate change that becomes available once the wall holds, and nothing here establishes that the index has no other consumer. **Consequence already load-bearing elsewhere:** this makes the wall a DEPENDENCY of the terminal-ledger artifact's cheaper design, not a neighbour of it — holding a resolved context across the floor fold costs ~1.9GB today and becomes affordable only when a resolve stops being a whole-corpus walk | ingestion-time check on the module's own declaration, where the module is parsed anyway — one module's facts from one module's source, not a corpus-wide reconstruction (the cost shape §6 names). **Note for whoever builds it:** this row's wall covers MISSING imports only. Running the check by hand on two modules also surfaced four IMPORTED-BUT-UNUSED names — the opposite direction, which the wall as specified would not catch. It is not a defect in this row, and it is nearly free: both directions are decidable from the same one-module `Node` tree the wall already walks, so a builder who implements only one has left the cheaper half on the floor |
| `x as T` where `x: T?` (Optional cast) | **Below floor — silent, measured 2026-08-22.** Casting an Optional to its payload type does NOT unwrap and does NOT refuse: `none as String` returns **`null`** from a `String`-typed function, and `Present { value: "real" } as String` returns a value that formats as **`Optional`** — the payload is lost in both directions, with no diagnostic at either. A plain-`String` control in the same run returned `plain`, so the loss is the cast's, not the harness's. This is silent wrongness (§5), outside the ladder rather than low on it: a `String`-typed carrier holding `null` flows onward as a value. **Note against an earlier private record:** it had this class rendering the literal wrapper text `Present { value: … }`; in return position it does not, so the two observations describe different positions and only the measured one is cited here. **Why it recurs, and this is the row's justification:** three instances in one day, all by an author who had already caught the class twice, all at the same position — the absent arm of a `match` on an Optional. In the author's words, *"I reach for a cast when a match arm has NO SENSIBLE VALUE, and `.dag` lets me."* That is a LANGUAGE AFFORDANCE producing a defect at a predictable position: the cast is the only spelling that type-checks there, and the type system offers no other exit, so knowing the rule does not help | **R3 / structurally guaranteed** — the operand's type is known at the cast site, so "this cast cannot succeed and is not an unwrap" is decidable from local information alone. A **wall, not a ratchet** | **Unbuilt, not blocked** — decidable today, and §4b(2)'s *can climb now but unbuilt*. Recurrence under full awareness is the evidence that the mitigation currently in place (an author remembering) does not hold | Executed probe, three arms one run with a passing control: `cast_absent` → `null`, `cast_present` → `Optional`, `control_plain_string` → `plain`. **Independently confirmed by a second probe in a separate session, and the two measure DIFFERENT HALVES — cited separately rather than as two measurements of one thing.** The second used assertions, not prints: `a_control_plain_is_plain` PASSED, `b_cast_present_keeps_payload` FAILED (`(Present { value: "real" } as String) == "real"` is false), `c_cast_absent_is_not_empty_string` FAILED. `b` is what settles the class independently: a PRESENT Optional cast does not equal its own payload, so this is not an absent-arm quirk — the cast discards the payload even when there is one to keep. What that probe CANNOT establish is the renderings: asserting `c` against `""` shows only that the absent cast is not the empty string, which is consistent with `null` and with much else. The values above come from the printing probe alone; the both-directions loss is attested by both. **LIVE EXPOSURE — stated because a silent-wrongness row without it reads as either an emergency or an abstraction, and it is neither.** THE THREE INSTANCES: all three were authored the same day and ALL WERE REMOVED BEFORE LANDING — none shipped, so they contribute zero exposure. A NARROW PROBE FOR THE IDIOMATIC PRODUCERS: a pattern matching `.first()` or `.last()` immediately followed by a cast to a capitalized type, over `dag/` and `src/v2/`, finds ZERO live code sites corpus-wide (its one hit is a carrier comment warning against the spelling). THE GENERAL POPULATION IS **UNMEASURED**, and that is the honest word rather than "latent": 1097 sites match a closing parenthesis followed by a cast to a capitalized type, and which of those operands are `Optional`-typed needs type information a grep does not have. **Both facts point opposite ways and both belong here:** latency makes the fix cheap, and it also means NOTHING WILL EVER SURFACE IT — an arm no test reaches and no run takes has a defect frequency of zero by construction, until the day it does not. The three instances are themselves the evidence that the arm gets written: each was authored by someone who knew the rule, at the position where the cast is the only spelling that type-checks. Three production near-misses the same day, all caught by review or by a reader, none by execution — two in `.dag` test fixtures for the terminal ledger, one in a parser that the same module's own carrier note forbids | refuse the cast at typecheck when the operand is an `Optional` and the target is its payload type, with the unwrap forms (`match`, or a total helper returning a type that can express the absence) as the sanctioned spellings |
