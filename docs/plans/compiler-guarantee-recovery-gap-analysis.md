...
| …in compiler source | **ENFORCED as of 2026-08-22 — the exemption is DELETED** (was: UNENFORCED by exemption). The arm skipped `v2.*` callers; its `v1.compiler.*` half was dead and was deleted at gunbc#8157. It is gone, so the direct-call argument-type judgment runs over every module including the compiler's own sources. **Do not read this row as closing its class** — the argument seam is one of at least two, and record-literal FIELD conformance (gunbc#8865) was never gated by this arm; see the seam-split row in §3 | closed by measurement, not by argument: gunbc#8864 shadowed the disabled judgment (115 would-be diagnostics at 78 sites on the `03_ingest` closure, all one class — a transparent type alias — residue ZERO), gunbc#8873 carried transparent-declaration identity forward so that class stopped firing and re-measured at zero, then the arm was deleted | wall landed |
...
1. ~~**v1 admits v2 source unchecked** — `module_skips_direct_call_arg_check` exempts `v2.*`.~~ **CLOSED 2026-08-22: the arm is deleted and the direct-call argument-type judgment now runs over `v2.*` callers.** Struck rather than removed because item 2 below is the half that compounds with it and is still open, and a reader who finds only one numbered item will not know which one closed. Scope: this closes the DIRECT-CALL ARGUMENT seam only — a record literal's field conformance is a separate seam that this arm never gated (gunbc#8865).
...
**Clause 4 WAS violated by name, and no longer is (2026-08-22).** `v1.compiler.infer`
`module_skips_direct_call_arg_check` was exactly an escape hatch, exempting the module trees
that most needed the dimension; it is deleted. The sentence is kept in the past tense rather
dropped because the clause's force comes from having had a named violation and closed it — a
section asserting "no escape hatch" with no history of one reads as untested.
