...
10. **Unenrolled floor measurements** (opened 2026-08-01 by codex review 46306, which caught
   ...
   **NOT DISCHARGED — HALF-DISCHARGED 2026-08-22, and the unfinished half is the half that mattered.** The AUTHORSHIP this item asks for is done, for SIX classes rather than the four it names; the ENROLMENT is not, because the witness carrying it is declined by the floor before it runs (§11 item 23). Read the rest of this paragraph as a description of authored artifacts, never of executing ones. The
   authorship landed as `dag/test/claim/guarantee_floor_class_probe_witness_test.dag` over six
   ...
   `floor-record-field-completeness`, `floor-parse-list-separator`,
   `floor-declared-conformance-v2-module-exemption`) — this item's four, plus the parse-separator
   row whose evidence was a throwaway session probe and the `v2.`-exemption row whose cell said no
   probe asserted the exemption's reach. All six were the same shape: a rung standing on a
   measurement nothing re-runs — and all six STILL ARE, because a declined witness re-runs nothing either. **What this does NOT claim.** Four of the six are
