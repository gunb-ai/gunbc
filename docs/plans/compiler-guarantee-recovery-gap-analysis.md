...
**READ EVERY EVIDENCE CELL BELOW THROUGH §11 ITEM 23 (added 2026-08-22).** Where a row's rung
rests on a probe pair in the guarantee-probe corpus — the call label/surplus, method
established surface, declared-conformance ground fragment, bare-none field admission,
sole-constructor cross-module and v2 self-grounding frontier classes, and the six floor
classes authored beside them — that probe is **AUTHORED, NOT EXECUTING**, so 100 identities of
guarantee evidence across five carriers run in no required CI leg. The walls those rows describe
LANDED and the measurements were taken; what does not happen is the re-taking. Read each such rung
as *established once, unguarded since* — the §4b meta-obligation-4 shape, stated here once rather
than repeated in every cell it touches.

**THE REASON THIS PARAGRAPH ORIGINALLY GAVE IS SUPERSEDED (corrected 2026-09-22).** It said the
required floor discovers each carrier and declines it (`DeclinedLiveTree`), and that the condition
lasts "until the decline arm is deleted". That arm WAS deleted at the root of
`v2.workflow.required_floor`, and the carriers still do not execute: the 2026-09-19 nominal-roster
ruling cut the required gate to `required_gate_prefixes` and `required_gate_authored_modules`, and
these carriers match neither, so their identities disposition `DeclinedOutsideRequiredGate`. §11
item 28's trigger was therefore satisfied without producing the outcome it promised — the shape
DESIGN §4b(3) warns about, a trigger naming an artifact rather than the capability it was meant to
restore. THE CAPABILITY is a required gate that plans these identities, and the two routes to it
are a module name on an already-admitted prefix (taken by the record-completeness zero-field family,
which now executes) and the derived changed-population selection that `v2.workflow.required_floor`
names as `gunbc.rung_drop` `required_gate_bankruptcy`'s restoration trigger. Growing either gate
roster is not a route: it is closed without operator sign-off.
...
| Generic instantiation | **Below floor — silent** (measured 2026-08-01: `type Boxed<T> { inner: T }` constructed as `Boxed { inner: "not an int" }` at declared return `Boxed<Int>` compiles with zero diagnostics of any severity) | R2 | substitution unproven | first measured by a one-off execution 2026-08-01 via `compile_dag_diagnostic_census` on the v1 CompileAccept path (source and result in the §10 eighth-pass ledger), and **AUTHORED AS A PROBE PAIR 2026-08-22 — AUTHORED, NOT EXECUTING, AND THE DISTINCTION IS THE WHOLE POINT** (see §11 item 28): the measurement is now written as `test.claim.guarantee_floor_class_probe_witness` `floor_generic_instantiation_is_still_silently_accepted`, against the registry row `gunbc.guarantee_probe_corpus` class `floor-generic-instantiation-inhabitance`, with `floor_generic_instantiation_monomorphic_control_refuses` (the same bad value in a NON-generic record refuses `TypeMismatch`, blocking, on this same harness and run — so the zero is the compiler's silence, not the probe failing to reach the judgment) and `floor_generic_instantiation_green_control_is_clean` as its probe-adequacy control. **It does not run in CI.** The witness declares `ReadsLiveTree` — truthfully, because `compile_dag_diagnostic_census` resolves its synthetic source against the live checkout. **THE CONCLUSION STILL HOLDS AND THE REASON WRITTEN HERE NO LONGER DOES:** this cell said the required floor DECLINES every `ReadsLiveTree` module before the fold sees it (`v2.workflow.required_floor` `RequiredFloorDisposition` `DeclinedLiveTree`), and that arm was DELETED at the root, so a live-tree read is not a decline today. What excludes the probe now is the 2026-09-19 nominal-roster ruling: `test.claim.guarantee_floor_class_probe_witness` matches no row of `required_gate_prefixes` and no row of `required_gate_authored_modules`, so its identities disposition `DeclinedOutsideRequiredGate` and are never planned. The remedy is a module name on an admitted prefix, which the record-completeness zero-field family took (see that row); it is NOT a declaration change, and it is not a roster row, which is closed to growth without operator sign-off. It is green by execution LOCALLY (`gunbc run --claim-run`, 2026-08-22, this session) and that is a strictly weaker fact. So this cell's rung is unchanged from the day before the probe was authored: it still rests on a measurement nothing re-runs. What the probe pair buys is that on the day the decline arm is deleted the measurement becomes standing evidence with no further authorship. Trigger: the `DeclinedLiveTree` deletion (§11 item 28), NOT this cell (§4b meta-obligation 4; the original gap was codex review 46306, §11 item 10) | inhabitance at instantiation |
| Field through generics | **Split.** Enclosing unconstrained T: **blocking InternalError** on the seed CompileAccept path (gunbc#11285; `fn get_field<T>(t: T) -> Int { t.no_such_field }` / `asks<T>{ item.value }`). Callee-generic and placeholder bases still defer: `field_of_type_var` is still minted when the base is not yet known at this infer_expr (`callable_param`, uninstantiated constructors, in-flight lambda-parameter binders (`in_flight_lambda_param_names`)). Historical silent measurement 2026-08-01 remains the receipt that the enclosing arm climbed. | R2 for remaining callee-generic deferral; enclosing unconstrained T is blocking `InternalError` | remaining `field_of_type_var` mint is callee-generic / placeholder deferral, not enclosing T | first measured silent 2026-08-01 via `compile_dag_diagnostic_census` on the v1 CompileAccept path (source and result in the §10 eighth-pass ledger). Authored as a probe pair 2026-08-22: `test.claim.guarantee_floor_class_probe_witness` `floor_field_through_generics_type_variable_refuses` against `gunbc.guarantee_probe_corpus` class `floor-field-through-generics`, with `floor_field_through_generics_declared_record_control_refuses` (declared record, blocking `InternalError`, observed not endorsed) and `floor_field_through_generics_green_control_is_clean`. gunbc#11285 climbed the enclosing-T hole: `floor_field_through_generics_hole_probe` is `ExpectBlockingRefusal` `{ diagnostic_class: InternalError }`. DESIGN §4b(4): that RED stays enrolled. The witness module still carries `ReadsLiveTree` / `DeclinedLiveTree` history (§11 item 28); that is an execution-route fact about the module, not a claim that enclosing T is still silent. Remaining `field_of_type_var` is the callee-generic deferral arm. Trigger for that remainder: discharge after the two-round argument fold, not a return to silent enclosing T. | constraint carried + unique discharge |
| Closed-match exhaustiveness | **Path-split, measured 2026-08-01 — the class is not one rung.** Coproduct-typed scrutinee: **R2** (a missing arm on a declared closed variant refuses `NonExhaustiveMatch`, blocking, naming the absent variant). Type-variable scrutinee: **below floor — silent** (`fn pick<T>(t: T) -> Int { match t { Red => 1 } }` compiles with zero diagnostics — one arm, an unconstrained subject, and a variant belonging to an unrelated type). Class rung is the minimum, so **below floor** | R3 (full arm population at elimination) | the silent arm is `PatternDynamic { span: _ } => []`, **not** `PatternLookupBlocked => []` as this row previously said — `pattern_subject_from_node` reaches `PatternLookupBlocked` only when the scrutinee's inferred type `is_compiler_error`, i.e. where a diagnostic already exists, so that arm is not the silent one and its silence is **not** established by these probes | first measured by a one-off execution 2026-08-01 via `compile_dag_diagnostic_census` on the v1 CompileAccept path (source and result in the §10 eighth-pass ledger), and **AUTHORED AS A PROBE PAIR 2026-08-22 — AUTHORED, NOT EXECUTING, AND THE DISTINCTION IS THE WHOLE POINT** (see §11 item 28): the measurement is now written as `test.claim.guarantee_floor_class_probe_witness` `floor_exhaustiveness_on_a_type_variable_scrutinee_is_still_silent`, against the registry row `gunbc.guarantee_probe_corpus` class `floor-closed-match-exhaustiveness`, with the coproduct-scrutinee arm already enrolled at `test.claim.match_exhaustiveness_coproduct_witness` `w_missing_coproduct_arm_reports_one_non_exhaustive`, on the same harness and the same floor run — not re-authored here, because a second copy would be a second authority for one fact as its probe-adequacy control. **It does not run in CI.** The witness declares `ReadsLiveTree` — truthfully, because `compile_dag_diagnostic_census` resolves its synthetic source against the live checkout. **THE CONCLUSION STILL HOLDS AND THE REASON WRITTEN HERE NO LONGER DOES:** this cell said the required floor DECLINES every `ReadsLiveTree` module before the fold sees it (`v2.workflow.required_floor` `RequiredFloorDisposition` `DeclinedLiveTree`), and that arm was DELETED at the root, so a live-tree read is not a decline today. What excludes the probe now is the 2026-09-19 nominal-roster ruling: `test.claim.guarantee_floor_class_probe_witness` matches no row of `required_gate_prefixes` and no row of `required_gate_authored_modules`, so its identities disposition `DeclinedOutsideRequiredGate` and are never planned. The remedy is a module name on an admitted prefix, which the record-completeness zero-field family took (see that row); it is NOT a declaration change, and it is not a roster row, which is closed to growth without operator sign-off. It is green by execution LOCALLY (`gunbc run --claim-run`, 2026-08-22, this session) and that is a strictly weaker fact. So this cell's rung is unchanged from the day before the probe was authored: it still rests on a measurement nothing re-runs. What the probe pair buys is that on the day the decline arm is deleted the measurement becomes standing evidence with no further authorship. Trigger: the `DeclinedLiveTree` deletion (§11 item 28), NOT this cell (§4b meta-obligation 4; the original gap was codex review 46306, §11 item 10) | `ExhaustivenessUnknown` refuses on the dynamic subject |
| Record completeness | **Form-split, re-measured 2026-09-10 — the class is not one rung, and the minimum governs it (§4b meta-obligation 1).** PARTIAL OMISSION: **R2** (a literal omitting a declared required field refuses `MissingField`, blocking, naming the field and type — measured 2026-08-01; the class had been carried as `Unknown — unmeasured` and that measurement raised it). EMPTY BRACES ON A PRODUCT: **R2** (`Pair { }` refuses one blocking `MissingField` per declared field). EMPTY BRACES ON A COPRODUCT VARIANT, and a BARE NAME in either kind: **OUTSIDE THE LADDER — accepted, silent, and the declared fields then read `null`** (measured 2026-09-10, eight sources on one binary through `compile_dag_diagnostic_census`; `Circle { }` and bare `Circle`/`Pair` each compile with zero diagnostics of any severity). A value inhabiting neither `Int` nor `String` at a field declared `Int` is the floor property `values inhabit declared types` failing, so this is silent wrongness rather than a low rung. **The 2026-08-01 R2 stood on a fixture that supplies SOME fields, which cannot express the zero-field form — the red it could author was never the red that was missing.** Authority for the class: `gunbc.recurring_failure_mode.zero_field_construction_escapes_the_field_set_check` | R3 | judgment is per-literal; construction-side and generic-instantiation completeness are separate and the latter measures **below floor** in the row above | first measured by a one-off execution 2026-08-01 via `compile_dag_diagnostic_census` on the v1 CompileAccept path (source and result in the §10 eighth-pass ledger), and **AUTHORED AS A PROBE PAIR 2026-08-22 — AUTHORED, NOT EXECUTING, AND THE DISTINCTION IS THE WHOLE POINT** (see §11 item 28): the measurement is now written as `test.claim.guarantee_floor_class_probe_witness` `floor_record_completeness_missing_field_refuses_blocking`, against the registry row `gunbc.guarantee_probe_corpus` class `floor-record-field-completeness`, with `floor_record_completeness_green_control_is_clean` as its probe-adequacy control. **It does not run in CI.** The witness declares `ReadsLiveTree` — truthfully, because `compile_dag_diagnostic_census` resolves its synthetic source against the live checkout. **THE CONCLUSION STILL HOLDS AND THE REASON WRITTEN HERE NO LONGER DOES:** this cell said the required floor DECLINES every `ReadsLiveTree` module before the fold sees it (`v2.workflow.required_floor` `RequiredFloorDisposition` `DeclinedLiveTree`), and that arm was DELETED at the root, so a live-tree read is not a decline today. What excludes the probe now is the 2026-09-19 nominal-roster ruling: `test.claim.guarantee_floor_class_probe_witness` matches no row of `required_gate_prefixes` and no row of `required_gate_authored_modules`, so its identities disposition `DeclinedOutsideRequiredGate` and are never planned. The remedy is a module name on an admitted prefix, which the record-completeness zero-field family took (see that row); it is NOT a declaration change, and it is not a roster row, which is closed to growth without operator sign-off. It is green by execution LOCALLY (`gunbc run --claim-run`, 2026-08-22, this session) and that is a strictly weaker fact. So this cell's rung is unchanged from the day before the probe was authored: it still rests on a measurement nothing re-runs. What the probe pair buys is that on the day the decline arm is deleted the measurement becomes standing evidence with no further authorship. Trigger: the `DeclinedLiveTree` deletion (§11 item 28), NOT this cell (§4b meta-obligation 4; the original gap was codex review 46306, §11 item 10). **The zero-field forms are enrolled 2026-09-10** as `floor_record_completeness_zero_field_variant_is_still_silent` and `floor_record_completeness_bare_name_is_still_silent` (both `ExpectSilentAcceptanceHole`), with `floor_record_completeness_zero_field_product_refuses_every_field` as the adequacy control proving the check EXISTS and handles zero supplied fields on the product path — without that control the split reads as an absent wall rather than a gap on two paths. They carry the same `ReadsLiveTree` decline as their siblings above, so they are authored-not-executing in CI on the same trigger. **WALL LANDED 2026-09-11 — every form now R2 on this path.** The variant-empty-braces silence was not a missed path: `Circle { }` reached the one completeness check and was exempted there by `is_zero_field_variant_tag_reference`, an implementation of operator Ruling 1a (2026-07-15, the `discriminant(v: X {})` tag-reference idiom) that was WIDER than the ruling — every zero-field variant literal anywhere. The exemption is narrowed to the one seam the ruling names, the `discriminant` argument (`v1.compiler.infer` `variant_tag_reference_argument_results`, the same shape the emitter folds in `discriminant_zero_field_variant_tag`); the bare-name form is judged on its own two routes (`bare_variant_reference_missing_field_diagnostics` on a `VariantValueBinding` reference, `bare_product_reference_missing_field_diagnostics` on a plain global binding) against the ONE required-field predicate the literal path uses (`declared_field_is_required`). Pattern position never reaches any of the three, so `ParamToken => true` stays legitimate by construction. Corpus census by the wall's own refusals through the required build lane (no writable cgroup on BuildBuddy runners, so CI was the instrument): 17 sites were the sanctioned idiom (all `src/v2/std/compilers/target_model.dag`, kept by the narrowed seam) and 1 was the defect itself — `gunbc.fleet.fleet_posix_accounts` minted `PrincipalUnobserved` bare with `allocation` in hand, repaired in the same change. The two hole witnesses flipped to `floor_record_completeness_zero_field_variant_refuses_every_field` and `floor_record_completeness_bare_name_refuses_every_field` (`ExpectBlockingRefusal`, permanent regression controls per §4b(4)), and the seam has its own pair: `..._discriminant_tag_reference_stays_clean` / `..._zero_field_literal_off_the_seam_refuses`, so the exemption can neither vanish nor widen silently. **THE RUNG IS ESTABLISHED ON THE ACCEPTANCE PATH, AND THE ZERO-FIELD EVIDENCE EXECUTES ON THE REQUIRED GATE — BUT BY A MOVE, NOT BY THE DECLINE DELETION THIS CELL ONCE CREDITED.** `DeclinedLiveTree` was deleted at the root (`v2.workflow.required_floor`, the paragraph beginning "`DeclinedLiveTree` IS DELETED HERE"), and for eight days that was cited here as making `claim_executor --required-floor` plan and execute `test.claim.guarantee_floor_class_probe_witness`. The 2026-09-19 nominal-roster ruling then cut the required gate to `required_gate_prefixes` and `required_gate_authored_modules`, which that module matches neither of, so its identities disposition `DeclinedOutsideRequiredGate` and a floor green says nothing about them. The five zero-field witnesses were therefore re-homed onto an admitted prefix, at `test.claim.infer_record_field_completeness_zero_field_witness_test` (`test.claim.infer_`, the inference family the wall itself lives in), where admission is by construction rather than by a roster edit; read the landing run's per-identity disposition for them, not the check name. The PARTIAL-OMISSION red and its green control stay in the unadmitted module and are authored-not-gate-executing, as the sentences above this one say | required-field construction at every construction form on the v1 compile path — partial omission, empty braces and bare name, product and coproduct variant alike — with the `discriminant` argument position the one declared, measured exemption |
| Parse: list separator dropped | **Below floor — silent** (measured: `[ {a}, {b}  {c} ]` compiles with zero diagnostics — a dropped comma is a silent semantic change, two- vs three-element list; survived regen, whole-corpus compile, fixed-point verify and a 15-case matrix, caught only by a human diff read) | R3 (decidable grammar fact) | separator omission parses as element juxtaposition | First measured by tidy-deer-730's throwaway probe on gunbc#7484 + review 45347, 2026-07-31. **PROBE PAIR AUTHORED 2026-08-22, NOT EXECUTING** (§11 item 28 — the witness is declined by the floor's `DeclinedLiveTree` arm and runs only under a local `gunbc run --claim-run`): `test.claim.guarantee_floor_class_probe_witness` `floor_parse_list_separator_omission_is_still_silent` re-executes it on every required-floor run against `gunbc.guarantee_probe_corpus` class `floor-parse-list-separator`, with `floor_parse_list_separator_doubled_separator_control_refuses` (a DOUBLED separator in the same position refuses `ParseError`, blocking) and `floor_parse_list_separator_green_control_is_clean` as its probe-adequacy controls, all three green locally and none of them executing in CI — so the omission's silence is measured against a parser that demonstrably refuses malformed separators | the probe pair is now in the corpus and pins the hole; what remains is the refusal in the list production (§11 item 6), on which the enrolled probe flips RED and its row is rewritten as `ExpectBlockingRefusal` |
| A value that does not inhabit its declared type is accepted — **seam-split, enumeration UNFINISHED** (was filed as "Direct-call argument TYPE conformance, v2 corpus"; renamed 2026-08-22, see the trigger cell) | **Below floor — the judgment never runs.** Not a hole a bad value slipped through: `v1.compiler.04_infer` gates `arg_compat_diags` on `module_skips_direct_call_arg_check(module_name: scope.module_name)`, which returns true for every module whose name begins with `v2.`. The key is the CALLER's module, so the entire active v2 corpus — compiler stages, extdeps, std, witnesses — has the direct-call argument-type judgment switched off. Measured consequence, 2026-08-22: `v2.extdeps.languages.dag` `dag_int_literal_node_from_lexeme` / `dag_int_literal_node_from_magnitude` declared `occurrence_id: OccurrenceId` since gunbc#6558 while storing that parameter straight into `Node.occurrence_id`, declared `NodeOccurrenceId`. For two months 15 call sites passed `SyntheticOccurrence` (a NodeOccurrenceId, contradicting the declaration) and nothing complained; gunbc#8833 made one call site OBEY the declaration and nothing complained about that either. The error surfaced only as an interpreter `non-exhaustive pattern match on: OccurrenceId { value: 79 }` across 16 witnesses, reddening main. **The inversion worth naming: a declaration that lies is inert while every caller contradicts it in the same direction, and detonates on the first caller who takes it at its word.** So the at-risk population is not "callers who got it wrong" but callers who might get it right — a genuinely counterintuitive census. Note this is the TYPE judgment only; the SHAPE judgment (`direct_call_shape_diags`, labels/arity) is exemption-free and does fire | R2 (a declared parameter type is a decidable conformance check the seam already computes — `direct_call_arg_mismatch_diags` exists, is written, and is simply not called, so the distance to the next rung is an if-statement plus a triage, not an implementation) | **THIS ROW IS A CORRECTION TO DESIGN §4b, NOT A NEW OBSERVATION BESIDE IT — read them together or the first will look like it already settled this.** DESIGN §4b names this exact symbol and reports that it "was found (code read, not execution) to be scoped entirely to the direct-call argument-type judgment and does not reach `sole_constructor`'s construction check at either call site — a positive finding that retires this axis, not an absence of any exemption anywhere." That finding is correct and is not disputed here. The question it asked was whether the exemption LEAKS into `sole_constructor`; the question it never asked is what the exemption COSTS inside the judgment it is scoped to. "Scoped entirely to the direct-call argument-type judgment" reads as reassuring only until that judgment is measured, and it is the argument-type check for the entire active v2 corpus. **Confinement was measured and then treated as safety, and the axis that got retired was not the one that mattered.** Two facts about why the seam nonetheless reads as covered: the SHAPE judgment (`direct_call_shape_diags` — labels, arity, duplicate binding) is exemption-free and DOES fire over every module including the compiler's own sources, so half-live is more deceptive than dead — every casual check finds something working. And the arm is NOT unreasoned: `direct_call_shape_wall_note` states the TYPE judgment's false-positive classes are representation gaps (brand aliases, optionality's two forms, anonymous literals, expansion depth — the conformance wall's four measured classes), which is a real stated reason and is why this row asks for a measurement rather than a deletion. What is unmeasured is whether those classes still fire in v2 today and at what rate. The neighbouring compiler-module arm of this same exemption was deleted in place after being found never to have been in force; the `v2.` arm was left standing at that moment without separate justification of its own | This incident, measured end to end: CI run 32542017600 (main, 67437fcbe9) 16 FAIL with the raw-`OccurrenceId` match error; caller census of both functions read at call grain (17 sites: 15 `SyntheticOccurrence`, 1 `node.occurrence_id`, 1 internal pass-through, 1 raw `minted.id`); exemption mechanism read at `v1.compiler.04_infer` `module_skips_direct_call_arg_check` and its single call site gating `arg_compat_diags` on `scope.module_name`. **NOT ENROLLED**: no probe pair asserts this exemption's reach, so nothing reds if it changes | **THE TRIGGER THIS ROW SHIPPED WITH WAS INCOMPLETE AND IS CORRECTED HERE (2026-08-22, swift-badger-524 retracting their own ruling).** As merged in gunbc#8854 this cell named ONE closing condition — delete the `v2.` exemption — which is a NECESSARY condition presented as a sufficient one. The class is not "direct-call argument type conformance"; it is **a value that does not inhabit its declared type is accepted**, and it has at least TWO seams: **(1) direct-call arguments** — gated off for `v2.*` by `module_skips_direct_call_arg_check`, the mechanism this row measures; **(2) record-literal fields** — gated by NOTHING, below floor in ORDINARY NON-v2 MODULES, receipt gunbc#8865 (gentle-eagle-360). Its minimal pair, no compiler internals involved: `CppHolder { subject: CppWrapped { inner: cpp_inner() } }` accepted and correct, against `CppHolder { subject: cpp_inner() }` — a coproduct PAYLOAD inhabiting a field declared as its parent COPRODUCT — ACCEPTED BY TYPING and dying at runtime as `PatternMatchFailure`. A record literal is not the direct-call seam, so deleting the exemption would not close it. **SEAM (2) NOW HAS A LIVE PRODUCTION SPECIMEN, WHICH RANKS IT DIFFERENTLY FROM A SYNTHETIC PAIR** (gentle-eagle-360, 2026-08-22, found while checking for a second content-identity authority before building on one): `v2.std.materialize` declares `MaterializedNode.hash` as `ContentHash` — the subject-generic union — and `materialize_fold_step` stores `content_hash(n)` into it, which returns `Fnv1a64Structural`, the PAYLOAD carried inside the union's `Fnv1a64` arm. Receipt, by execution against the module as it stands on main: taking the value materialize ACTUALLY STORED and asking the union's own family authority `content_hash_family` about it yields `PatternMatchFailure { value: "Fnv1a64Structural { digest: 7ac77ab0e29c6bd8 }" }`; the probe's fallback was a correctly wrapped `Fnv1a64(...)`, so an empty result or a well-formed value would have PASSED and only a stored value can produce that red. **The defect is LATENT and that is the instructive part**: peer lookup compares `e.hash == h`, raw payload against raw payload, which agrees with itself — so every current path is green and the error surfaces only when a materialize hash is routed through the union's own machinery (`content_hash_family`, or `compare_content_hash`, which exists precisely to refuse cross-family collapse). That is the same shape as this row's own originating incident: **a declaration that lies is inert while every consumer contradicts it in the same direction, and detonates on the first consumer that takes it at its word** — here the first cross-family artifact to reach the carrier. Disposition ruled ARM A, wrap the construction (`MaterializedNode { hash: Fnv1a64(h), ... }`), NOT narrow the declaration: narrowing would sever `MaterializedNode.hash` from `RealizationPlan.target`, and §2's Realization spans resolve-cost, sccache and OS provisioning on ONE content hash, so a structural-only key could never name a realization of a fetched artifact or a provisioned image (swift-badger-524 ruling, 2026-08-22; handed to silent-bear-842 with the receipt, unfixed here because it is that lane's module). **THE SEAM ITSELF IS THE FINDING, not this one site**: gunbc#7480 corrected the design document for asserting the wrong type at exactly this `Hash`/`ContentHash` boundary, and a production module now carries the mirror-image error — a boundary documentation has to keep re-explaining is a boundary that wants construction. ****THAT HAND-OFF IS SUPERSEDED AND THE SPECIMEN IS REPAIRED — corrected in place rather than left standing, because the sentence above says a live production defect is UNFIXED and awaiting another lane, which is the present-tense claim a reader would plan against (gunbc#8876, 2026-08-22).** The wrap landed in the wall's own change by operator ruling: a wall and the repair it forces must land together, since with the wall on and the wrap absent main is red, so only one PR can contain both. `v2.std.materialize` `materialize_fold_step` and `distinct_hashes` now build `Fnv1a64(content_hash(n))`, and the same construction was repaired at four further sites the wall refused for the identical reason — `v2.workflow.operand_flow`, `materialize_witness_test`, `materialization_provider_witness_test` and `heal_revalidation_witness_test`. The receipt above stays as the historical demonstration that the defect was real and reachable; it is no longer a description of the tree. THE ENUMERATION IS UNFINISHED: two is what has been measured, not the count.** Assume a third seam exists until someone enumerates them; note that this row's own originating incident (#8854) reached its victim through a record literal one hop downstream of the direct call, so the two seams are not even cleanly separable at a site. **SEAM (1) WAS EXECUTED TO A MEASURED, BLOCKED STOP ON 2026-08-22 — the arm is NOT deleted, and this paragraph is the §4b row for that state rather than a plan.** Rung: **below floor, unchanged** — the judgment still does not run for any `v2.*` caller. Ceiling: **R2** (a declared parameter type is a decidable conformance check the seam already computes). Next trigger, stated as §4b(2) requires and with its disposition: *a formal parameter declared as an applied generic carries its applied form through resolution rather than reaching the comparison seam as the bare constructor* — **DECLINED at the owning layer on 2026-08-22**, on the ground that a bounded compiler-floor packet whose completion condition has moved should close and hand its residue forward rather than extend into a type-system project. A declined trigger is a legitimate §4b state; **a blocked class with no row is how this exemption survived years in the first place**, which is the reason this row exists at all. Enrolled evidence, so the class stays countable while blocked: `direct_call_arg_type_v2_module_red_probe` (the discriminating RED, rostered in `v2.workflow.floor_expected_red` — it cannot pass while the arm stands and PASSES on a tree with the arm deleted, so it is a satisfiable assertion held open by a named cause), plus two controls that pass on main today (`direct_call_arg_type_ordinary_module_red_probe`, the paired nonzero the exemption never covered; and `direct_call_arg_type_v2_green_control_probe`). **What was measured before the stop, and how to read it:** deleting the arm produces **285+k** blocking diagnostics and **67+k** with gunbc#8873 merged, for some k ≥ 1 unmeasured — these are ROSTER-RESOLVED counts, not corpus counts, and `src/v2/extdeps/formatters/lean4_format.dag:184` is the proof: a live site, structurally identical to eight that refused, contributing zero to *both* arms and therefore invisible to their difference. **Zero genuine call-site defects were found in the residue** — every diagnostic examined was a deficit in the type judgment, so no call sites were edited, because 67 mechanical edits would have cemented three compiler deficits permanently. The per-mechanism split of that residue is deliberately NOT recorded here: it was published as 48/11/8, and the rule the 48 were attributed to was then found to admit nothing in this corpus, so the attribution was retracted before it was cited. Trigger for the count to be restated: the residue re-measured with its `why` column. **The counterexample that retracted it is worth carrying on its own, because it will bite anyone who reaches for a name-keyed alias relation:** `type Float = Float64` is declared in BOTH `dag/std/float.dag:18` and `src/v2/std/float.dag:33`, spelling the target identically — but `dag/std/float.dag:16` has `type Float64 = Real64` (a transparent alias) while `src/v2/std/float.dag:30` has `Float64` as a **record**. So the two declarations agree on a *spelling* and disagree on a *type*, and a unanimity rule keyed on the target's NAME admits them as the same concept. Requiring the target itself to be census-unique closes it — and once closed, that rule admits nothing in this corpus, which is what falsified the attribution above. Found by still-carp-717 while building the rule, before it shipped. **The original trigger text, kept because the sequence it specifies was followed and the record should show what was attempted:** seam (1) — turn the exemption off for `v2.` behind a measured diagnostic count, triage, then delete the arm, the disposition the compiler-module arm received; the measured prerequisite is ALIAS TRANSPARENCY, because proud-ant-819's report-only shadow on the 03_ingest closure found 115 `WouldDiagnose` relations at 78 sites of which 115 reduce to a transparent `type A = B` (92 via `type Hash = Fnv1a64Structural`, 23 via the `Node` phase carriers), residue ZERO — so on that closure the exemption is currently suppressing false positives, not defects. Seam (2) — its own wall at record-literal field conformance; nothing to turn off, it was never on. **Closing seam (1) MUST NOT be read as closing the class**, which is the specific failure this correction exists to prevent: a class that looks like it has one closing condition gets closed when that condition fires. Two blind spots neither instrument covers, named rather than left to be rediscovered: production itself SKIPS an anonymous record literal standing as an ACTUAL at a direct call, so its argument type is never judged (521 relations, 2.7%, `Unadjudicated` in proud-ant-819's shadow — upstream of the guard, so invisible to a flip-off arm too); and a function storing into a field from its own differently-named parameter is this class at yet another site, which a callee-name-keyed census cannot see. **THAT FIRST BLIND SPOT IS NOT SEAM (2), AND THE TWO WILL BE MERGED BY ANY READER WHO DOES NOT SEE THIS SENTENCE** (caught by proud-ant-819, who put the mirror-image clause in their own row): both descriptions begin "a record literal" and name DIFFERENT POSITIONS. The blind spot is a record literal at an ARGUMENT position whose type goes unjudged at the direct-call seam — seam (1)'s territory, and a population of 521 relations. Seam (2) / gunbc#8865 is a record literal's FIELD whose value does not inhabit the field's declared type — a different seam with a different mechanism and no measured population. Reading them as one would make the 521 look like evidence for gunbc#8865, or gunbc#8865's receipt look like it bounds the 521. Neither is true. **A CLAIM ABOUT THIS ROW'S EVIDENCE WAS MADE HERE AND IS RETRACTED, MEASURED WRONG (2026-08-22, the §11 item 28 lane, retracting itself).** The retracted claim was that these probes sit in the declined `ReadsLiveTree` carrier and therefore never execute, so the expected-red enrolment could only be `not_evaluated`. **It is false, and the measurement that refutes it is the run that was supposed to confirm it:** run `32586093086` reports `[expected-red-roster-join] roster=207 still_red=207 now_passes=0 not_evaluated=0` with `known_red_held=207`. The v2-module red EXECUTES and is held still-red exactly as its row intends. The error was reading an IMPORT as a location: `guarantee_probe_corpus_witness` imports the three probe ids for a roster assertion, and I took that for where the probes run; they run in `test.claim.direct_call_argument_type_witness`, a separate file. An import is evidence of visibility, never authority for where a name lives. **What survives is a different and sharper finding.** That file executes because it declares `SubstrateInputsOnly` while calling `compile_dag_diagnostic_census` — the same declaration `gunbc.compile_diagnostic_census`'s own live-tree note already adjudicated as a MIS-declaration when it found `transport_script_wall_compile_red_test` doing it (that census walks `build_module_path_index_from_witness_roots`, i.e. the live checkout). So the specimen population for that mis-declaration is now TWO, the second authored on 2026-08-22 after the first was adjudicated, and both are the only reason any census-compiling probe executes at all. Either these files' declaration is wrong or that authority's note is; they cannot both be right, and which one it is decides the size of §11 item 28's population. That is a question for the arm's owner, not a defect in this row's measurement, which stands. **SEAM (2) IS NOW CLOSED, AND CLOSING IT ENUMERATED TWO MORE — the count is FIVE measured, still not the total (gunbc#8876, 2026-08-22).** **OWNERSHIP, STATED PER SEAM BECAUSE THE CLASS NOW SPANS TWO LANES AND NEITHER CAN CLOSE IT ALONE:** seam (1) is the exemption-gated direct-call seam owned by the paragraph immediately preceding this one, whose next trigger is recorded there as DECLINED rather than pending; seams (2) through (5) belong to the field-inhabitance lane (snappy-tern-856). A lane that lands its last seam will be tempted to close this row. NEITHER MAY: 'my seams are done' is not 'the class is closed', and the row closes only when every seam does — including any seam not yet enumerated. The record-literal FIELD seam is walled: `v1.compiler.infer` `coproduct_payload_where_parent_required` refuses a value whose type is one of the declared coproduct's own variant payload types, consulting `transparent_alias_identity_agrees` over the `SymbolIndex` so an alias-mediated producer is caught too. Rung at that seam: **structurally guaranteed** — no `Accepted` program contains the state there. NOT structurally impossible, because the bad literal is still writable and the compiler refuses it; and NOT the class, which is why the two rows below exist. Evidence, enrolled permanently per §4b(4): `test.claim.coproduct_payload_soundness_witness_test` — the direct RED, the alias-mediated RED, and the positive control that stops a refuse-everything compiler from satisfying either. Turning the wall on WAS the census: EIGHT live sites, none previously known, all repaired in the same change — `ContentHash` from `Fnv1a64Structural` at `materialization_provider_witness_test`, `heal_revalidation_witness_test`, `materialize_witness_test`, `v2.std.materialize` and `v2.workflow.operand_flow`; `LexRules` from `LexRuleSet` at two `src/v2/test/claim/manual` fold tests. **SEAM (3), UNWALLED AND DECLARED, owner the field-inhabitance lane: the module-scope `data` initializer.** `data X: ContentHash = content_hash(...)` stores the payload into a `ContentHash`-declared row and is NOT refused, because a `data` initializer is not a record-literal field. Rung: **below floor**, exactly as seam (2) was. Unguarded: every module-scope `data` row in the corpus. Trigger: the same conformance judgment applied at the data-initializer seam, which needs no new relation — the declared type and the initializer's inferred type are both already in hand there. **SEAM (4), UNWALLED AND DECLARED, owner the field-inhabitance lane, AND IT IS RANKED FIRST OF THE TWO: the list element.** `v2.workflow.locality_affinity` built consumer identities as `list_add_distinct_hash(xs: cs, v: content_hash(n: e.dependent))` — a raw payload into a `List<ContentHash>`. Rung: **below floor**, and it does not fail loudly. Measured: repairing the comparands while that producer stayed raw turned FOUR `locality_affinity` witnesses to `Bool(false)` with NOTHING refused at compile time — a wrong answer discovered later, not a diagnostic. That is what makes it rank above seam (3), and the ranking is fixed by a RULE rather than by judgment (operator ratification, 2026-08-22): seam (3) admits a wrong value a correctly-built peer can still catch by comparison — harmful, detectable, recoverable — while seam (4) admits a wrong value AND DISABLES THE COMPARISON THAT WOULD HAVE CAUGHT IT. That is a silent wrong answer, which DESIGN places OUTSIDE the guarantee ladder and forbids outright rather than ranking low, so the two are in different categories and no population argument reaches across the line: a larger set of loud failures does not outrank a smaller set of silent ones. The ranking is a priority order for collisions, NOT a serial queue — seam (3) needs no new relation and may land whenever it is ready. Unguarded: every list, map and collection element position whose element type is a coproduct. Trigger: element-position conformance against the container's declared element type. **SEAM (5), UNWALLED AND DECLARED, owner the field-inhabitance lane: the declared RETURN type.** `fn f(x: Node) -> ContentHash { content_hash(n: x) }` returns the payload where the parent coproduct is declared and is NOT refused. Rung: **below floor**. Measured 2026-08-22 on the gunbc#8876 merge commit, with a liveness control in the SAME compile that produced exactly one diagnostic (the record-literal RED), so the silence is a measurement and not an untested assumption. HOW IT WAS FOUND IS WORTH RECORDING, because it says something about the enumeration itself: it was found by a MIS-WRITTEN PROBE — the author intended an alias-mediated FIELD case and wrote a return position by mistake. Four of the five seams in this row were discovered by accident or by a downstream red rather than by anyone enumerating positions, which is the standing evidence for #8868's instruction to assume a further seam. Unguarded: every declared return type whose type is a coproduct. Trigger: `declared_type_conformance_diags` already runs at this seam and already holds both the declared and the inferred type, so the distance is a payload-membership arm on an existing judgment, not a new walk. **DO NOT READ SEAM (2)'s CLOSURE AS THE CLASS'S.** One seam of four measured is walled; a reader who sees only 'declared-field inhabitance wall landed' will conclude the class is closed, and the next lane would then build on a guarantee covering a quarter of it. Neither seam (3) nor seam (4) nor seam (5) is closed by gunbc#8876, which deliberately did not widen to chase them. **THIS ROW'S CEILING CELL SAYS THE DISTANCE TO R2 IS "AN IF-STATEMENT PLUS A TRIAGE". THAT IS MEASURED FALSE FOR THE PAYLOAD-WHERE-PARENT SHAPE, AND THE CORRECTION MATTERS BECAUSE IT IS THE SECOND NECESSARY-CONDITION-STATED-AS-SUFFICIENT TRIGGER THIS ONE ROW HAS CARRIED (snappy-tern-856, 2026-08-22).** The cell reasons that `direct_call_arg_mismatch_diags` exists, is written, and is simply not called, so flipping `module_skips_direct_call_arg_check` off closes seam (1). Flipping it off is NECESSARY. It is not SUFFICIENT, and the discriminating measurement is a module the exemption never applied to in the first place: in an ORDINARY NON-`v2.*` module, where the guard is false and the judgment demonstrably runs, `c4_takes(o: 5)` against `fn c4_takes(o: EOuter)` REFUSES (`expected 'Coproduct(EOuter)', got 'Primitive(Int)'`) while `c4_takes(o: e_inner())` — the coproduct's own payload type — is ACCEPTED in the same compile. The judgment is live at that seam and misses this shape, so deleting the arm leaves it accepted. **Read the two facts together or the row misleads:** the exemption really does switch the judgment off for every `v2.*` caller (that half is unchanged and still below floor), AND the judgment it switches off would not catch this shape anyway. A reader who acts only on the first will delete the arm, watch the diagnostics, and conclude the seam is closed. **THE DENOMINATOR THIS ROW HAS LACKED SINCE IT WAS WRITTEN NOW EXISTS, and it is what makes the sentence above checkable rather than another anecdote.** The row has said "assume a third seam until someone enumerates them" through two revisions while the count grew from two to five BY ACCIDENT — four of the five were found by a downstream red or by a mis-written probe, never by anyone enumerating positions, which samples the reviewer rather than the population. The positions are now enumerated FROM THE GRAMMAR: every call site of `parse_type_expr` in `v1.compiler.parse`, which is the finite set of places the language attaches a declared type. That is **14 sites**, and all five previously-known seams FALL OUT of it as rows rather than having been seeded into it — the falsifier the enumeration was required to pass. The list element seam falls out only because a container's element type is reached as a TYPE ARGUMENT rather than as a declaration of its own, which is why nobody had named it as a position before. **METHOD, because "not refused" is worthless without it.** Each position carries three arms: the payload-where-parent shape, a KERNEL mismatch (`Int` where the coproduct is declared), and an UNDEFINED NAME. The third is the reachability control: if it refuses, inference REACHES that position, so a passing kernel arm establishes that NO TYPE JUDGMENT RUNS there rather than that the position went unanalysed. All seven reachability controls refused. Dispositions, measured: **refuses** — record/variant NAMED field (`parse_field`), and only because gunbc#8876 walls it. **Judgment runs, misses the shape** — fn parameter at the direct-call seam. **Not refused, no judgment runs** — container type argument (list element), variant POSITIONAL payload, fn declared return, module-scope `data` initializer, annotated `let`. **Not refused** — `as` cast. **Not reachable from a source value** — type-alias RHS, `uses` resource type, service `exit` and `response` types (the last two are the reconstruction path's, not this row's). **Not refused, no judgment runs** (measured after the first pass, closing the census at 14 of 14) — the callable-type RETURN position (`cb: fn(Int) -> EOuter` given a lambda whose body yields the payload) and the callable-type PARAMETER position (`cb: fn(EOuter) -> Int` called with the payload); both reachability controls refused, so both positions are analysed. Three of these positions were previously unknown: variant positional payload, `as` cast, and annotated `let`. **AND THE ENUMERATION FOUND SOMETHING WIDER THAN THIS ROW'S CLASS, WHICH IS DELIBERATELY NOT CLAIMED HERE.** `data c6b: EOuter = 5` and `fn c5b() -> EOuter { 5 }` are both ACCEPTED, with the reachability control proving both positions are analysed. That is not payload-versus-parent; it is a plain kernel value where a declared coproduct is required, which is the ordinary-floor invariant DESIGN §4b names beside name resolution and exact-bijection binding. On the measured census that is **seven of fourteen grammar-admitted positions accepting a kernel value where a declared coproduct is required** — variant positional payload, fn declared return, module-scope `data` initializer, container type argument, annotated `let`, callable-type return, callable-type parameter — with the single refusing position owing its refusal to a PR that is held at the time of writing. (The count was first published as FIVE of fourteen, before the two callable-type positions were measured; it is corrected here rather than left standing, because an understated denominator on a floor breach is the same defect as an overstated rung. The `as` cast is deliberately not in the seven: it was never given a kernel arm, since a cast is an authored assertion and whether a kernel-to-coproduct cast should refuse is a different question this census did not ask.) This row's class is one INSTANCE of that breach rather than the breach itself, so the finding is routed for its own §4b row and its own owner rather than absorbed here — recording it inside this row would repeat the mistake this correction exists to fix, by making a wider unwalled invariant look like a seam of a narrower one  **THAT WIDER BREACH NOW HAS ITS OWN OWNER AND FOUR OF ITS SEVEN POSITIONS ARE WALLED (2026-08-24, the declared-type-inhabitance lane).** Scope, stated first because the class and this row's class are neighbours that will be merged by anyone reading quickly: the subject is A PLAIN KERNEL VALUE (`Int`, `String`, `Bool`) where a declared record or coproduct is required — NOT a coproduct payload where its parent is required, which is this row's seam (2) and is separately walled. MEASURED BEFORE, on the pre-change binary, one fixture per position with an undefined-name reachability control beside it: all SEVEN positions compiled `0 diagnostics` and EMITTED 9 files (fn declared return, module-scope `data` annotation, annotated `let`, container element, variant positional payload, callable-type return, callable-type parameter); every reachability control refused (`function 'no_such_name_at_all' not found in scope`), so the silence measured an absent judgment rather than an unanalysed position; and the record-literal NAMED field refused (`expected 'Coproduct(KOuter)', got 'Primitive(Int)'`), which is what identified the relation that already existed. MEASURED AFTER, same fixtures, same flags: the declared return, the `data` annotation, the annotated `let` and the container element REFUSE with a located `TypeMismatch` (`expected 'Coproduct(KOuter)', got 'Primitive(Int)'`; the element position as `expected 'Container(List,Coproduct(KOuter))', got 'Container(List,Primitive(Int))'`); the conforming spelling at all four positions still compiles `0 diagnostics`; the named field refuses identically, which is the control proving the reused relation has not forked; and `fn f() -> KInner { 5 }` — a RECORD rather than a coproduct — now refuses too (`expected 'Product(KInner)', got 'Primitive(Int)'`), so the wall is the declared-type class and not a coproduct special case. NO SECOND RELATION WAS MINTED (DESIGN §3): `kernel_value_declared_type_mismatch` was already the corpus authority for this judgment and was consulted at exactly one of the fourteen positions; what landed is its peeling prologue lifted into `declared_type_conformance_diags`, so the fn return, the early `return`, the `data` annotation and the newly-wired annotated `let` all widen with that gate rather than beside it. FALSE-POSITIVE CENSUS, AND ITS FIRST STATEMENT HERE WAS WRONG ABOUT ITS OWN DENOMINATOR — corrected in place because the error is the instructive part. The regen corpus compile (`dag` + `src/v2`, `first_generation_equal=true`, `planned=133 executed=133`) reports zero hard diagnostics under the wall, and this row first read that as 'no live site was repaired'. The required floor then refused: its prepared subject resolves **3872 modules**, including `src/v2/test/**`, which the regen closure does not reach, and the wall found **six live sites in one file** — `src/v2/test/claim/execution/emit_on_demand_match_loop_fold_family_witness_test` declared six octet fixtures and one function parameter as `List<Byte>` while holding integer literals (`[0, 255, 255, 255, 255]`) and forwarding them to `v2.compiler.emit_host` `emit_host_octets_byte_string`, whose declared parameter is `List<Int>` and which builds each `Byte` itself. `Byte` is a record over `List<Bit>` and no `Int -> Byte` cast rule exists, so every one of those declarations was a lie that no consumer contradicted — the latent shape this row's originating incident already names, at the element position the wall now judges. Repaired to `List<Int>` (the entry then compiles with 0 blocking diagnostics), which makes TURNING THE WALL ON the census, exactly as gunbc#8876's field seam was. THE GENERAL LESSON, worth more than the six sites: a green whole-corpus compile is scoped to ITS OWN closure, and quoting it as 'the corpus' silently excludes whatever that closure omits; the floor's prepared subject is the wider denominator and disagreeing with it is how the gap was found. Zero further live sites were found beyond that file, which is a fact about this class's live population and not evidence that the other three positions are empty. RUNG: the four walled positions are **structurally guaranteed** (the bad value is still writable and the compiler refuses it); the class as a whole stays **below floor**, because three positions still admit it. STILL OPEN, each with its trigger, and NONE of them closed by this change: (a) the VARIANT POSITIONAL PAYLOAD — `KHeld(5)` routes through `infer_variant_constructor_call` into `infer_record_lit` under the synthetic field name `0`, and the declared type of that positional slot does not reach the field-conformance seam the NAMED field reaches; trigger is that slot resolving to its declared payload type there. (b) and (c) the CALLABLE-TYPE RETURN and PARAMETER — a lambda's body against `fn(Int) -> KOuter`, and an actual against `fn(KOuter) -> Int`; both reachability controls refused, so both positions are analysed and neither carries the callable's own declared component types into a conformance judgment; trigger is threading each callable boundary's declared parameter and return through the same relation, which is also what `explicit_return_conformance_note` names as its lambda narrowing. A NESTED container (`List<List<KOuter>>`) is likewise not judged: the element peel is ONE level, which is the depth measured, and deeper is unmeasured rather than covered. Enrolled evidence, permanent per §4b(4): `test.claim.declared_type_kernel_inhabitance_witness_test` — four discriminating REDs, the already-closed named-field RED as the no-fork control, and one positive control carrying the conforming spelling at all four closed positions so a refuse-everything compiler cannot satisfy them **A THIRD SEAM, MEASURED 2026-09-01 (eager-hawk-376), AND IT IS NOT A SEAM AT A POSITION — IT IS A PROPERTY OF THE DECLARED TYPE.** The row above says to assume a third seam exists until someone enumerates them; this is one, and it is orthogonal to (a)/(b)/(c), which all vary the POSITION while holding the declared type ordinary. Here the position is the already-covered one and the DECLARED TYPE is bodyless: **any `type X` declared with no body admits any argument in parameter position, silently.** Receipt, four fixtures in an ORDINARY (non-`v2.*`) module, so seam (1)'s `module_skips_direct_call_arg_check` exemption is not in play, against a binary carrying main's semantics: `fn takes(x: Compose<Int, MachineWidth<8>>) -> Int` given a `String` — ACCEPTED, zero diagnostics naming the file; the same with a bare `Phantom` parameter — ACCEPTED; with a locally declared `type Opaque` and no `Phantom` anywhere — ACCEPTED; and the control `type MyRec = Word8`, an alias to a record — REFUSES with both `type mismatch: expected 'Primitive(MyRec)', got 'Primitive(String)'` and the paired inhabitance diagnostic. **The control is what makes this a finding rather than a silence: the judgment RUNS in that module and admits, which is a different mechanism from seam (1), where the judgment never runs.** It is not `Phantom`-specific and not `Compose`-specific — both were the obvious readings and both are refuted by the third fixture. POPULATION: 69 bodyless declarations corpus-wide, including `std.constructors` `Phantom`, `Product` and `Coproduct`, and `std.machine_constraints` `MachineWidth` / `PointerWidth`. **That 69 is the SYNTACTIC population and an upper bound on nothing else** — a bodyless type never used in parameter position cannot exhibit the hole, so the exposed subset is smaller and unmeasured. **THE EXPOSED SUBSET IS NOW MEASURED (2026-09-01, vivid-deer-102), AND IT IS FOUR NAMES OF 66.** Re-derived independently rather than quoted, per this row's own instruction, over `dag/`, `src/v1/` and `src/v2/` at `origin/main` `6741a3953`: **69 bodyless declarations / 66 distinct names**, which JOINS THIS ROW'S 69 EXACTLY once the two censuses are reconciled at identity grain rather than at count — the first pass matched `^type X$` and found 62, and the missing 7 are the PARAMETERIZED bodyless declarations (`Compose<carrier, dimension>`, `MachineWidth<bits>` twice, `ReferenceModel<T>`, `CppTargetDataModel<...>`, `CppTargetProfile<...>`, `ConfigPatchRecord<Config>`), so 62 + 7 = 69 and the two lanes agree on membership, not merely on a total. REACHABILITY IN PARAMETER POSITION, which is where a non-inhabiting value can actually flow in: **`Symbol` (`v2.std.node`) 1045 parameter sites; `SourceSpan` (`v2.lens.application`) 129; `Bytes` (`std.types`) 4; `ConfigPatchRecord` (`v2.std.patch`) 1** — and nothing else. 33 names appear only in field/record position; **27 appear in no parameter, field or return position at all — but see (b) below: those 27 are EXPOSURE-ON-FIRST-USE, not inert**, so the honest exposure split is three-way: exposed now (4 names, `Symbol` dominating), exposure-on-first-use (the 27), grammar-absent (none found). `Compose` and `MachineWidth` are among the unexposed: they are type CONSTRUCTORS applied as type arguments, never parameter types, which is why adding the parameterized 7 to the population moves the exposure count by one. **So the headline is not 69: the class is small and its exposure is dominated by ONE declaration**, `Symbol`, the substrate's own identity field, at an order of magnitude more sites than everything else combined — and a census reported as 69 exposed positions would bury that. METHOD LIMITS, carried rather than left to be discovered: parameter sites are counted only on lines beginning `fn `/`func `, so MULTI-LINE SIGNATURES ARE UNDERCOUNTED; the field-position figure uses a looser match and may over-count. Both are bounds, not exact counts — the SHAPE (four exposed names, `Symbol` dominating) is robust to both errors. THE CEILING IS NOT ONE VALUE, because the population splits three ways and the row would overstate itself by reporting a single one. **(a) DECLARED-ABSTRACT BY RULING — `Symbol`, `Unit`:** the operator ruling of 2026-09-01 keeps `Symbol` opaque and projected at the `Node` boundary, so the attainable ceiling is NOT structural impossibility via arms; it is a CONSTRUCTION DISCIPLINE at the boundary — values enter only through a declared minting authority. `v2.std.compilers.lexing` `symbol_intern_lexeme` IS such an authority, which is why gunbc#9854's repair of `target_value_expression_char_unicode_scalar` has the right shape where minting a host `String` through `from_code_point` did not. R3 for sites that go through such an authority, R2 corpus-wide. **(b) TYPE-LEVEL TAGS — the 27 never in parameter, field or return position** (`cpp_abi`'s `CppMachineWidth8/16/32/64` and `CppDataModelFamily*`, `std.measure`'s `PerSecond`/`PerMonth`/`PerKilowattHour`, `std.constructors`' `Product`/`Coproduct`/`Phantom`). **AN EARLIER REVISION OF THIS CELL FILED THESE AS INERT — 'the hole is not reachable, no climb needed' — AND THAT WAS WRONG. IT IS CORRECTED HERE BY EXECUTION (2026-09-01), ON eager-hawk-376's CHALLENGE.** §4b judges reachability BY WHAT A FIXTURE MAY AUTHOR, never by what the accepted corpus contains, so the question is not whether the corpus uses these in parameter position but whether the grammar ADMITS them there. It does. Measured in an ORDINARY (non-`v2.*`) module, so seam (1)'s `module_skips_direct_call_arg_check` exemption is not in play, against the pristine-`origin/main` binary: `fn vd_takes_phantom(x: Phantom) -> Int` fed `"abc"` and `fn vd_takes_persecond(x: PerSecond) -> Int` fed `7` both resolve CLEAN and **EXECUTE**, returning `1` and `2` — the run reaches the body, so this is admission proven by execution and not merely by a silent compile. The planted positive control in the same module (`length(xs: n)` on an `Int`) REFUSES, so the judgment is live there and the silence is a measurement rather than a judgment that never ran. **These are therefore CORPUS-UNOCCUPIED, NOT GRAMMAR-ABSENT: exposure-on-first-use, acquiring exposure the moment anyone writes the construct, with nothing to refuse them.** Filing them as inert was the bounded-population-reads-as-progress trap — the population looked closed because nothing had walked into it yet. Two of the 27 are measured (`Phantom`, `PerSecond`, from two different declaring modules); the remaining 25 are the same shape and are NOT individually executed, so they are recorded as exposure-on-first-use BY CLASS rather than each by receipt. Ceiling: the same as (c) — an arms-or-construction decision — not 'no climb needed'. **(c) GENUINELY EXPOSED AND UNRULED — `SourceSpan` (129 sites) and `Bytes` (4):** an arms-or-construction decision is OPEN on both and nobody has ruled; recorded here so they are countable rather than forgotten. CLASS RUNG IS THE MINIMUM ACROSS THE PARTS, so the class stays **below floor**. NEXT-RUNG TRIGGER, naming the CAPABILITY rather than an artifact as §4b(3) requires: **a bodyless declared type can be inhabited only through a declared minting authority, AND the direct-call inhabitance judgment consults that authority.** `test.claim.target_model_survivor_witness_test` `char_unicode_scalar_expression_carries_a_symbol_identity` (gunbc#9854) is a wall for ONE SITE and **does not retire this row** — the invalid state stays writable at the other 1044 `Symbol` parameter sites. A naive line-oriented count returns 5227 because the multi-line coproduct spelling (`type X` then `= A \| B` on the following line) reads as bodyless; re-derive rather than quoting either number. Rung: **below floor** — a value that does not inhabit its declared type is accepted, in an ordinary module, with the judgment live. Ceiling: **R2** — membership is decidable from the declaration, since the refusing control proves the comparison seam already reaches this position and already has both types in hand. **NOT ENROLLED**: no probe asserts this, so nothing reds if it changes; the fixtures above are reconstructable but were deleted with the worktree. **AND THAT STATE IS NOW A CHOICE RATHER THAN A CONSTRAINT, WHICH §4b WOULD NOT LET STAND SILENTLY:** `test.claim.declared_type_expected_type_path_witness` landed on main the same night, and it compiles caller-authored SOURCE STRINGS through `gunbc.compile_diagnostic_census`, asserting on `census_ran` plus a blocking refusing-count rather than on a diagnostic spelling. The four fixtures above are ordinary source strings, so that harness can express them — which means the refusal is authorable, and §4b's own rule is that declining enrollment where the RED is authorable is specification-without-execution. The reason it is not enrolled in this diff is not that it cannot be: the seam ADMITS today, so the assertion is red on main and enrollment requires a `v2.workflow.floor_expected_red` row — a roster another lane edited the same night, and a decision about the required gate rather than about this class. That decision is named here as the immediate next step, owner-ruled, so the gap is countable rather than quietly permanent. Next trigger, at capability grain rather than artifact grain per §4b(3): *the conformance seam decides a bodyless declared type by a stated rule — admit-nothing-but-its-own-constructions, or a declared and typed opacity — rather than by falling through*, with the four fixtures above enrolled as the discriminating REDs and the alias-to-record case kept as the no-fork control. **A SECOND, INDEPENDENT DEFECT FOUND IN THE SAME PASS, RECORDED HERE BECAUSE IT IS THE INSTRUMENT THAT WOULD OTHERWISE BE CITED AS THIS CLASS'S CENSUS:** `tools.opaque_realization_census` `declaration_body_standing_in` decides opacity from the `type X` line alone and carries the annotation *"No such spelling occurs in this corpus today, so the reading is exact for the corpus it is run over"* about the multi-line body form. **That premise is false, and by a wide margin** — the multi-line spelling is one of the corpus's ordinary forms for a coproduct, and the function classifies every occurrence of it `DeclaredOpaque`; and separately it matches the declaration by bare prefix, with no name-boundary check, so a longer same-prefix name declared EARLIER in the file decides a shorter name's standing — specimen `Userset` shadowing `User` in `extdeps.access.zanzibar`, and the pairs are plentiful rather than isolated. Both are read from the function, not from its annotation. **THE ANNOTATION IS THE FINDING, AND DESIGN §4c ALREADY NAMES ITS SHAPE:** an annotation is never evidence that a machine claim holds, because no `Accepted` program can read one. This one does not merely rot — it CERTIFIES the exact trap its function falls into, so a reader doing the diligent thing and checking whether the author considered the continuation form finds a sentence saying yes. It is therefore a receipt for §4c's rule and not only a defect in one census. **Of the two, the bare-prefix bug ranks first for repair**, because they fail differently: the continuation misclassification is systematic and one control exposes it, while `Userset` deciding `User`'s standing is silent, name-dependent, and yields a different wrong answer according to declaration ORDER within a file — so it survives a fix of the continuation reading alone, and the shadowed pairs are numerous enough that such a fix would look correct on a spot check. The census gates nothing today, so this is a wrong instrument rather than a wrong wall — but it is the instrument this row's population question would be handed to. **A NOTE ON THE FIGURES IN THIS ROW, FOLLOWING review 58065 (2026-09-01):** that review applied §6's *name the instrument, never transcribe its output* to a count of mine and was right, so every figure of mine that came from a one-off scan with no committed producer has been replaced by cited members, named specimens and stated magnitudes. **The `69` / `66` join above is deliberately NOT treated that way**, because it is not a transcribed one-off: two lanes derived it independently and reconciled it AT IDENTITY GRAIN against a named ref, which is the re-derivation §6 asks for rather than the copying it forbids. What both censuses still owe is the same thing: a committed producer, so the next reader re-derives instead of trusting either lane. **THE SCOPE OF THE OBLIGATION CENSUS ITSELF, MEASURED 2026-09-01 (neat-hawk-884 found it; eager-hawk-376 verified and censused it) — TEN OF TWELVE GRAMMAR POSITIONS HAVE NO OBLIGATION PRODUCER.** `v1.compiler.04_infer` `DeclaredTypePosition` declares twelve positions that can receive a source value; `DeclaredTypeObligation` is CONSTRUCTED at exactly two sites, for `PositionDirectCallArgument` and `PositionListElement`. Verified independently of the carrier's own annotation by counting construction sites of that type in that module: one declaration, two constructions. Every other member occurs only in `declared_type_position_label`, which renders a name and judges nothing. **THE CARRIER ALREADY STATES THIS IN §4b SHAPE IN ITS OWN VOICE — including the enumeration, the refusal to file the ten as `InhabitanceUndecidable`, and a per-position trigger — so this row POINTS AT IT rather than restating it**, per §6's rule that the mark on the carrier is the authority and not a parallel ledger. What is recorded here is only what the carrier does not say. **(1) IT IS A THIRD CAUSE OF SILENCE, and that is the load-bearing consequence.** The board has been reasoning with two — the judgment ran and accepted, or ran and declined — and the bodyless finding above established the first is real. This adds *no producer at that seam at all*. The three are indistinguishable from outside the compiler, which is why an admission cannot presently be attributed to a cause without reading the wiring; neat-hawk-884 reports 45 of 64 board rows UNDECIDED on exactly this. **(2) OCCUPANCY, POSITION BY POSITION, AND IT IS NOT THE SAME QUESTION AS REACHABILITY.** Occupied today, by cited specimen rather than by count: `PositionDataInitializer` and `PositionRecordLiteralField` (`extdeps.abi` `extdeps_external_authority_anchor`), `PositionDeclaredReturn` (`std.access` `access_refusal_reason`), `PositionLetAnnotation` (`extdeps.render.terminal`, 92 sites), `PositionGenericTypeArgument` and `PositionVariantPayload` (`std.algebra` `CallableOf`, `Present` payloads), `PositionMapValue` (`std.algebra` `kernel_algebra_profile`), `PositionCastTarget` (`extdeps.access.posix_effective_principal`, `(name as NonEmptyStr) as EffectivePosixPrincipalName`), and `PositionCallableReturn` (`extdeps.bazel.label`, `fn(acc, d)` actuals). **`PositionParameterDefault` is grammar-admitted and its corpus occupancy is NOT ESTABLISHED HERE, deliberately.** Grammar admission is decidable by symbol and is cited: `v1.compiler.02_parse` parses `name: Type = expr` in a parameter list and is the only path constructing a param node with a non-`none` default. Occupancy was checked once, by a paren-balanced scan over the tracked `.dag` corpus, and came back empty — **and that measurement is deliberately not quoted as a figure in this row, because it has no committed producer.** §6 is explicit that a measurement is cited by naming the producer that re-derives it and never by copying its output, and its own test decides the case: occupancy of a grammar position IS worth re-deriving, since it changes the day someone authors the construct, so it is worth an entry point and is therefore an instrument this row is OWED rather than a number this row may carry. **The conclusion does not depend on that measurement in either direction**, which is why removing the figure costs the row nothing: reachability is judged by what a FIXTURE MAY AUTHOR, never by what the accepted corpus contains, so a grammar-admitted position has an authorable RED whether the corpus occupies it or not. An empty corpus would mean no exposure TODAY and exposure on first use, with nothing to refuse the author; a non-empty one would mean exposure now. Neither reading makes the position inert, and neither changes the rung, the ceiling, or the trigger below. The instrument this owes: a committed producer that reports, per `DeclaredTypePosition` member, whether the tracked corpus occupies that position — which would also give the nine specimens above a re-derivable form instead of nine cited sites. **(3) CEILINGS DO NOT DIFFER BY POSITION, AND THE REASON MATTERS MORE THAN THE ANSWER.** None of the ten is grammar-absent, so none is unreachable, so none has a lower ceiling for that reason. The decision procedure is shared and position-agnostic, so each unwired position is a construction site rather than a rule — which is why the carrier prices the residual ten at a call each. The one place a ceiling genuinely bites is inside the relation and not at a position: `InhabitanceUndecidableReason` already carries `UndecidableGenericFormal`, so `PositionGenericTypeArgument` will reach a decidability limit the others do not, and that limit is a property of the facts rather than of the wiring. Rung: **below floor** for all ten — absent from the census, no diagnostic of either severity. Ceiling **R2**, the same as the two wired positions, for the reason above. Next trigger at capability grain rather than one artifact: *every grammar position that can receive a source value constructs a `DeclaredTypeObligation` and hands it to the shared decision procedure* — the carrier's ten per-position triggers are the steps, and satisfying one of them retires one step and not this row. **NOT ENROLLED, by ruling rather than by omission** (bright-ram-778, 2026-09-01): enrollment of any of these as an expected red is a decision about the REQUIRED GATE and belongs to the owner of `v2.workflow.floor_expected_red`, not to the author who found the gap. Named here so the gap stays countable. No producers were written: this row is a census, and if a producer is trivially addable that is evidence for the trigger rather than licence to add it. |
| A sealed carrier is invisible to every judgment that reads its type HEAD — **`sole_constructor` erases structure**, whole corpus, not v2-scoped | **Below floor — the judgment runs and cannot see the subject** (measured by execution 2026-09-01 on main `37e705c42e`, six-cell matrix, one flat `--source-dir` compile). A `sole_constructor` carrier bound at a formal declared as its own payload type compiles with ZERO diagnostics; the reverse direction and the same carrier at a KERNEL formal are equally silent. This is not the `v2.*` exemption: the identical program in an ordinary module is silent too, and so is the whole matrix on a tree with `module_skips_direct_call_arg_check` deleted — while the SAME run gains a `TypeMismatch` on a kernel red in the v2 module, so the exemption arm is provably live and simply orthogonal. **The discriminating pair is the finding**: `type PlainW { root: Inner }` and `type Wrapped sole_constructor { root: Inner }` differ by one token, and only the sealed one is admitted at an `Inner` formal. | **R3 — structurally guaranteed.** Two declared types either are the same declaration or are not; nothing about a construction restriction makes that undecidable, and the relation that decides it (`nominal_product_inhabitance_refusal`) already exists and already runs exemption-free. What stood between here and there was not decidability but VISIBILITY. Not R4: the bad binding stays writable in source, which is exactly why its RED is authorable as a fixture. | `v1.compiler.parse` `parsed_sole_constructor_properties` stores the seal as a PROPERTY on the declaration node, and `v1.compiler.infer` `exposure_view_for_node` answered `OpaqueTypeHead` for any node whose `properties` list was non-empty — so `nominal_product_head_name` answered `""`, `nominal_product_inhabitance_refusal` short-circuited on the empty head, `declared_type_inhabitance` fell through to `Inhabits`, and `record_at_scalar_needs_identity` / `coproduct_at_record_declared_type` were blinded by the same read. **THE INVERSION**: BL-1 sealed `v2.compiler.normalized_tree` `NormalizedTree` from an alias for `Node` into a record precisely to make a wrapper-retention drop unwritable, and the act of sealing LOWERED the carrier's rung on the application-binding axis, undeclared, because the two axes are read by different consumers. `v2.compiler.source_authority` `normalized_source_ast_equal_witness` then bound a `NormalizedTree` at a `Node` formal and was accepted. **WHY gunbc#8886's 285-row census could not contain this shape**: its 148 `Node`-vs-`ResolvedTree`/`ParseTree` rows are transparent-ALIAS false positives — a census enumerates what the relation REFUSES, so a defect the relation is blind to is invisible to it by construction. Filed as recurring failure mode `sealing_property_erases_structure`. | `test.claim.sole_constructor_type_head_exposure_witness_test` — THREE discriminating REDs (sealed-at-payload, payload-at-sealed, sealed-at-kernel), of which the first two GREEN with this repair and the third FAILS and is enrolled in `v2.workflow.floor_expected_red`. **The distinction is carried by the enrolment rather than by the assertion, and this cell records two corrections rather than softening either**: an earlier revision counted the kernel arm as a third landed wall, which it is not (§4b(1) rung inflation in the evidence sentence); the revision after that asserted the compiler's CURRENT ACCEPTING answer to keep the hole visible, which made a below-floor defect required green behaviour and contradicted §4b(4). The roster is the arm that was missing from both: an enrolled row still executes and is still asserted, its failure counts as agreement, and it REDS THE BUILD the moment it starts passing — so the wall is counted the moment it lands instead of being absorbed into a green run. The unsealed twin as the no-fork control (it refused BEFORE the repair and must keep refusing, or the repair widened opacity instead of narrowing it), a conforming projection through the carrier as the positive control, and an in-module `sole_constructor` construction as the sealing-still-works control. Run through `compile_dag_diagnostic_census`, so the REDs execute rather than being described. | **For this marker: the record→record arms land with the row; the KERNEL arm does not and is NOT this marker's remaining work — it belongs to the already-filed row *Direct-call record→scalar inhabitance with erased declaration identity*, which measures below floor on binding for want of a reliable declaration identity at the binding edge, and it is carried here as an enrolled expected-red row so it is counted from both ends rather than falling between them** (`type_head_preserving_property`, established positively: the parser builds the identical `Conj`-with-children node and only the properties list differs, so the seal restricts CONSTRUCTION and not structure). **For the class: every OTHER property in that channel — `namespace_root` and the deferred service-config rows — remains opaque and UNMEASURED at this seam**, named rather than swept in; its trigger is a discriminating pair authored per marker. The capability trigger that retires the shape entirely is markers carried in the declaration's own type rather than in an untyped side-channel, so a consumer branching on the channel cannot compile. |
...
| Direct-call argument TYPE conformance — **applied-generic alias**, v2 corpus | **Below floor — the semantic class is unrepaired and structurally unreachable from the comparison seam; its CORPUS POPULATION IS NOW ZERO.** Read the two populations in this cell as what they are: **PREVIOUS (measured 2026-08-22): 9 declarations, 8 observed false refusals. CURRENT (gunbc#11749, CENSUS-REFUSED-10): 0.** The rung did not move — only the corpus did. An alias whose right-hand side is a generic application (`type BlackConfigPatch = ConfigPatchRecord<BlackConfig>`) refuses at every call site passing it to the matching formal. Specimen, measured by execution 2026-08-22 on a fixture diffed byte-for-byte against the live `v2.std.patch` declaration, with the exemption deleted in the Rust MIRROR and `exempt=judged` on all 20352 ledger rows as the control: `formal_type = Primitive(ConfigPatchRecord)`, `actual_type = Node(BlackConfigPatch)`, `nominal=true container=false kernel=false`, `fname=ConfigPatchRecord aname=BlackConfigPatch`. Population as measured 2026-08-22: 9 declarations, all under `src/v2/extdeps/formatters/` (`black`, `clang_format`, `gofmt`, `google_java_format`, `ktfmt`, `lean4_format`, `prettier`, `rustfmt`, `swift_format`), of which 8 produced diagnostics in the CI arm — `lean4_format` is structurally identical and produced none in EITHER arm, so the population was 9 and the measured count 8+k. **THE CURRENT POPULATION, RESTATED: ZERO, AND THE CLASS IS UNCHANGED.** All 9 declarations were deleted and their 10 use sites now name `ConfigPatchRecord<XConfig>` directly, because a single-use alias of a generic instantiation is the nickname DESIGN §6 forbids ("never bare-alias") sitting on top of the shared authority — the §2-horizontal modelling praised in §12's Seam A is `v2.std.patch` itself, which that change preserves and consumes more directly. **THAT IS A CHANGE OF POPULATION, NOT A CLIMB**: resolve still discards the type argument at the formal position, the relation still cannot hold an argument on the formal side, and the rung and the trigger below are unchanged. A reader following the old positive control (`black.dag` must refuse) would now read a LEGITIMATE zero as a false zero, which is why the control is restated in the evidence cell rather than left pointing at a deleted site | R2, and it is decidable — the same equality already accepts the applied form when it is written directly at the formal | **THE FORMAL SIDE IS WHERE THE INFORMATION DIES, and that is the sentence that should stop the next attempt from this direction.** The formal is DECLARED `ConfigPatchRecord<Config>` and REACHES the comparison as the bare constructor, with the type argument already discarded. So the equality has one side that structurally cannot hold an argument, and the case split over any representative is EXHAUSTIVE: (a) representative = the applied form with every argument retained never equals the bare constructor, so it admits nothing and all 9 sites keep refusing; (b) representative = the bare constructor admits `ConfigPatchRecord<X>` at `ConfigPatchRecord<Y>` for any X and Y, which is exactly the over-peel that erases the distinction. There is no third representative, and widening the carrier does not help — a node-valued map fails identically, because the missing information is not on the relation's side of the map. **Not to be confused with gunbc#8879**, the withdrawn repair that widened transparency AT RESOLVE and was refused by CI in three classes at once (20 direct-call comparisons, 18 variant projections, 3 files of regen drift). What that established is that resolve is the wrong place for a transparency JUDGMENT; preserving an applied form is resolve DISCARDING LESS, which hands the same consumers MORE structure, and is a different claim that has not been measured | executed fixture + shadow ledger. **THE POSITIVE CONTROL MOVED WITH THE POPULATION (gunbc#11749).** It WAS `black.dag` must refuse; a zero there is decidably wrong. It no longer is, and citing it now inverts its meaning: `black.dag` is expected to compile clean at this head. The surviving discriminator is a closure-free fixture, which is where it should have been all along — the RED is still authorable because the invalid state is still writable in source: with `type PatchRecord<Config>` and `fn patch_layer<Config>(base: Config, patch: PatchRecord<Config>) -> Config`, a parameter declared by `type PConfigPatch = PatchRecord<PConfig>` refuses with `expected 'Primitive(PatchRecord)', got 'Node(PConfigPatch<Primitive(PConfig)>)'` while the same parameter spelled `PatchRecord<PConfig>` compiles clean. The green control that shows the refusal is a FALSE one, rather than a brand the corpus intends: an UNPARAMETERIZED alias (`type PlainAlias = Inner`) IS admitted at an `Inner` formal, so the corpus's alias semantics is transparency and the applied form is the only spelling it refuses. **NOT ENROLLED**: no probe pair asserts this class, so nothing reds if it changes. Three false zeros were produced while measuring it — twice from patching the `.dag` authority while the binary is built from the emitted Rust mirror, once from a run that PANICKED and reported zero diagnostics — each caught only by a positive control or by another session's raw per-line export, never by an aggregate | **RESOLVE CARRIES APPLIED FORMS AT THE FORMAL POSITION.** Explicitly NOT scoped as of 2026-08-22 (operator-lane ruling, swift-badger-524): a bounded floor-recovery lane whose completion condition has moved is closed honestly rather than extended, and nothing in that ruling says the change is wrong. Until it lands, the `v2.` exemption stays whole and gunbc#8886 stays blocked |
| Direct-call argument TYPE conformance — **container element**, non-exempt module; and the witness family that reported it was **rung INFLATION** (DESIGN §4b(1) — *not* §4b(3): nothing regressed, the wall was never there, so "restore the previous behaviour" is not the remedy and a regression row would have read as authorising it) | **Below floor — the container-element judgment does not fire at a direct call argument.** Measured 2026-08-27 with the witness's OWN oracle (`compile_dag_rust_emit_check`) alongside `compile_dag_diagnostic_census`, over two corpora in two remote dispatches: `main` @ `ddb7f533e4e` and gunbc#9428's head. A fixture handing `FreeMonoid<LocalNode>` to a declared `FreeMonoid<NormalizedTree>` parameter — locally declared record, corpus-unique wrapper name, no `Node`, no `v2.std.node`, no occurrence identity anywhere in it — **compiles clean ON `main`**, 0 blocking; the argument surfaces only as advisory `DeclaredTypeInhabitanceUndecided` at "direct call argument". Distinct from the `module_skips_direct_call_arg_check` row above: that exemption keys on the CALLER's module name and this fixture's module is not `v2.`-prefixed, so the judgment is not switched off here — it is written, reached, and answers no. **THE INFLATION:** the class read as mechanically preventable on the strength of `test.claim.record_literal_call_arg_handoff_witness.record_literal_container_element_mismatch_at_call_arg_still_refuses` being green. That witness was green on an UNRELATED defect in its own closure — `v2.std.node` referenced `std.occurrence_identity.OccurrenceId` and `ScopedOccurrenceRef` fully qualified WITHOUT importing that module, so every closure importing `v2.std.node` inherited two `UnresolvedType` diagnostics and refused for that reason; measured on `main`, the fixture's refusal is exactly `UnresolvedType/std.occurrence_identity.OccurrenceId` ×2 + `ScopedOccurrenceRef` ×1, and contains no type-mismatch diagnostic at all. gunbc#9428 adds the import, those resolve, and the accidental refusal goes with them. **A witness green on a defect in its own closure is not weak coverage, it is NEGATIVE coverage** — it is cited as evidence the wall is live, so the class never ranks for climbing. That is DESIGN §4b's worse-than-absent case reached from a direction the document does not yet record: not an unauthorable RED, but an AUTHORABLE one satisfied by the WRONG CAUSE, which no reader of the witness can distinguish from the right one. The nominal sibling `record_literal_nominal_mismatch_at_call_arg_still_refuses` stays green through a DIFFERENT path (`literal_introduction_type_mismatch`, whose actual is a record LITERAL rather than a call), which is why the family read as coherent and was not | R2 (two distinct nominal element types under one container template is a decidable comparison the seam already reaches) | `container_element_nominal_brand_mismatch` is written and IS reached from `direct_call_arg_type_mismatch`, but it delegates to `nominal_call_arg_brand_mismatch`, which requires `structural_carrier_template_name(formal_el) == structural_carrier_template_name(actual_el)`; that name falls through `canonical_template_name` to the element's own authored name for any non-container type, so two DISTINCT nominal element types can never satisfy the conjunct that the mismatch arm needs. **Stated as a CODE READ, consistent with the measurement but not independently executed** — what is executed is that the compile is clean, not which conjunct declined | Measurement above (both corpora, `emit_check` + census). **Instrument not committed:** a throwaway cargo example over the two `pub` entry points in `cli_run`, kept out of the tree per the no-ad-hoc-script rule; re-deriving it needs a `.dag` entry point, which is this row's standing "name the instrument" debt | `container_element_nominal_brand_mismatch` reaching the direct-call-argument position — i.e. `nominal_call_arg_brand_mismatch` admitting distinct nominal element types under a shared container template. The witness is the ready-made RED and needs no new fixture; it is enrolled as expected-red in gunbc#9428, where it FAILS (on `main` it still passes on the accident, so enrolling it here would trip the roster's stale-quarantine arm and red main's own floor) |
| A diagnostic reports a type the source never declares — **a nullary alias rendered with its BODY's type argument applied to it** | **Below floor — not a wrong answer about a program, a wrong answer about the compiler's own report.** Two instances measured 2026-09-01 (vivid-deer-102), different aliases, one shape: `type LexText = FreeMonoid<Char>` is NULLARY, and a refusal at a `LexText`-declared parameter printed `expected 'Node(LexText<Product(Char)>)'` — the alias NAME kept while the body's type argument is rendered onto it; and a repair to `v1.compiler.emit_rust` `is_host_text_carrier_type` emitted `pub fn string_is_empty(s: String<i64>)` into `v2_std_text.rs`, which is not valid Rust. THE HARM IS NOT THE MALFORMED SPELLING, IT IS THAT THE MALFORMED TYPE READS AS A REAL REFUSAL: the `LexText` arm reported several times as many refusals as the qualified-spelling arm, and that inflated figure was reported to, and endorsed by, the XL-N manager as a stricter wall over a larger population before the diagnostic text was read closely enough to notice the alias could not take an argument. A population inflated by a printer defect is worse than a missing measurement, because it is quoted. **THE FIGURES THEMSELVES ARE DELIBERATELY NOT CARRIED HERE, and that is this row's own finding turned on itself:** the original figures were a one-off scan with no committed producer, so restating them would make this row a second quotation of exactly the kind of number it exists to warn about. What is durable is the SHAPE — one arm inflated relative to the other, endorsed before it was read — and that survives without a literal. | R3 (a nullary declaration's arity is decidable at the declaration; applying an argument to it is unwritable in a well-formed applier) | **This row does NOT claim the repair site.** The defect is in the type applier or the diagnostic printer and three lanes are converging on `05_emit_rust.dag`; naming a site here would be a second authority for a fix nobody has scoped. What IS established: the two instances share a shape, it reaches BOTH a resolve-stage diagnostic and an emitted Rust signature, so it is not confined to reporting. Distinguish it from the neighbouring non-confluence rows: those are two authorities disagreeing in two PLACES; this is **two decisions inside ONE rendered type expression** — the base name decided by one authority and the type argument by another — which is why it survives every emitter-vs-resolver question currently open. | Both instances reproduced on a binary built from a PRISTINE `origin/main` export (`6741a3953`, `modified-mirror-files=0`, the `is_host_text_carrier_type` repair verified ABSENT from the compiled mirror), each arm carrying a firing positive control, so neither is an artifact of the measuring tree. The `LexText` arm's refusals fall in three cited files — `extdeps/languages/bash.dag` carrying the large majority, `bash_orch_if.dag` most of the remainder, `bash_orch_tier2.dag` a trace — the specimens being the durable half and the per-file counts having no committed producer to re-derive them. **NOT ENROLLED**: no probe asserts either instance, so nothing reds if the applier changes. **AND THE MISSING PRODUCER IS ITSELF PART OF THIS ROW'S TRIGGER** (§6, and review 58203 which made the call): a per-file refusal census for a named diagnostic arm is worth re-deriving — it changes the moment the applier changes, which is the event this row is about — so it is worth an entry point, and its absence is the reason the figures above are cited as specimens rather than counts. | A nullary declared type cannot have a type argument applied to it in any rendered type expression — diagnostic or emitted target — which is a well-formedness property of the applier, not a spelling rule for the printer |
| Direct-call declared-type inhabitance — decidable outer-shape disjointness | **Class rung remains below floor (the minimum across all rows); two measured arms rise to R2.** INVALID STATE: an application binds a `List` value to `NonEmptyStr` or a coproduct value to a record. HARM: the live fabric specimen passed `List<FabricCellMember>` to `content_hash_of_value(value: NonEmptyStr)` and failed only inside `atom_identity_hash` at runtime. DISTINGUISHING FACTS: `build_call_application_plan` already builds an obligation for every non-callable formal, so the seam was not bypassed; the relation ran but could not decide these shapes. This change makes the two arms blocking R2 refusals, located at the application and naming the parameter, declared type, and supplied type. The rows below still admit, so the general property is not claimed enforced. | **Per-arm:** R2 for List→scalar and coproduct→record. **Class ceiling:** R2 after every boundary population is grounded; R3 requires normalized application IR whose binding edge cannot carry a non-inhabitant. | Other shapes remain in separately counted rows below; their admitting path fixes the class minimum below floor. | Binding REDs plus a conforming control in `test.claim.declared_type_inhabitance_direct_call_witness`. Production incident: `gunbc.instruments.fabric_control_plane_live_probe` `fci1_cell_observation_wire` calling `std.content_hash.content_hash_of_value`. | Retire each separately-triggered row below; only then may the class rise from its minimum below-floor rung. |
| Direct-call generic-formal inhabitance | **Declared counted boundary; class path admits.** The formal still contains a type variable and the binding edge carries no instantiated substitution proof, so asserting either inhabitance answer here would fabricate it. | R2 when instantiated substitution proof reaches the obligation; R3 when normalized application IR makes the proof intrinsic | This may be genuinely undecidable at the present seam, unlike the two wall-after-grounding rows below. | `compile_dag_diagnostic_census` counts `DeclaredTypeInhabitanceUndecided`; `test.claim.declared_type_inhabitance_direct_call_witness` executes a generic-formal fixture. The whole `dag` + `src/v2` population and per-reason counts are re-derived by `claim_executor --required-regen --source-root dag --source-root src/v2` with the undecided class temporarily blocking. | Carry the instantiated generic substitution proof to every binding obligation, then decide and retire `UndecidableGenericFormal`. |
| Direct-call record→scalar inhabitance with erased declaration identity | **Below floor on binding; counted rather than silent in this change.** INVALID STATE: a nominal record reaches a scalar parameter. HARM: the record can fail only inside a scalar primitive. DISTINGUISHING FACTS: the binding edge exposes `Product(name)` but not a reliable declaration identity; the aggravating fact is that `dag/std/types.FilePath` is a String refinement while `v2.extdeps.file_system.FilePath` is an unrelated record. Namespacing makes both legal, and bare-head lookup cannot distinguish them. A blocking arm exposed compiler-source sites whose actual identity cannot be recovered here. This change classifies the shape as `UndecidableProducedIdentityErased`, making its admission visible without inventing either answer. | R2 once the edge carries fully-qualified declaration identity; R3 when normalized IR makes that identity intrinsic | A spelling check or `FilePath` exemption would guess across the known homonym fork. | `test.claim.declared_type_inhabitance_direct_call_witness` counts the boundary. `claim_executor --required-regen --source-root dag --source-root src/v2` with undecided diagnostics temporarily blocking ranges over the whole named source-root population and re-derives the compiler-source occurrences; the row does not claim that compiler source is the universe. | Carry fully-qualified produced declaration identity through inference and application planning; then decide record→scalar from the exact type-head census and make the refusal blocking. |
| Direct-call Optional→required inhabitance after a nonempty-list guard | **Below floor on binding; R2 on cast; class rung is the binding-path minimum.** INVALID STATE: an `Optional<T>` reaches a required `T` parameter without elimination. HARM: absence can cross the call boundary and fail only at use. DISTINGUISHING FACTS: the cast path blocks the shape as `OptionalCastNotEliminated`, while the binding path emits non-blocking `DeclaredTypeInhabitanceUndecided` and admits it. Making the binding verdict blocking exposed corpus sites that call `List.first()` only after `count()`/`length()` proves nonempty: those sites depend on a real nonempty-list fact that is present in author control flow but absent from the model. This is a wall after grounding, not a permanently undecidable boundary. | R2 when nonempty-list flow evidence grounds `first()` as `T`; R3 requires the normalized binding edge to make an uneliminated optional unwritable | Blocking immediately would correctly reject what the model cannot prove but would also reject every guarded `first()` use; hand-writing Present/Absent at each site is repeated mitigation and does not model the fact for the next site. | `test.claim.declared_type_inhabitance_direct_call_witness` executes both answers separately: binding is admitted and counted as `DeclaredTypeInhabitanceUndecided`; the same shape through cast blocks as `OptionalCastNotEliminated`. The corpus population is re-derived by `claim_executor --required-regen --source-root dag --source-root src/v2` with the undecided class temporarily blocking; no stale numeric count is stored here. | Model a nonempty-list fact in control flow so `first()` on a proven-nonempty list yields `T` rather than `T?`; then make Optional→required binding a blocking refusal with no site exemption. |
...
| Duplicate top-level declaration in ONE module | **Was BELOW FLOOR — silent, with LIVE exposure; wall landed in this change.** INVALID STATE: one module declaring one name twice. HARM: `v1.compiler.env` `symbol_index_insert` is a `map_insert` on the qualified name, so the second declaration REPLACED the first and every reference bound the last one written, with no diagnostic of any severity. MEASURED BY EXECUTION through our own binary before the wall: `fn probe_dup() -> Int { 1 }` then `fn probe_dup() -> Int { 2 }` ran the caller to `2`; the single-definition control ran to `1`; same harness, duplication the only variable. The collision is NOT fn/fn only — a `data` and a `fn` of one name share the key, and a bare reference to the data yielded `<fn thing>`. DISTINGUISHING FACT vs the census-ambiguous last-import-wins row above: that one is ACROSS modules, this one is INSIDE one, so it needs no corpus walk and no import resolution — it is decidable from one module's own item list. LIVE EXPOSURE, denominator stated: a scan of 4163 authored `.dag` files found 2 modules carrying it — `dag/gunbc/systemctl_list_units.dag` (`data systemctl_list_units_all_states` shadowed by `fn systemctl_list_units_all_states`, so the ListUnits `state:` argument bound the FUNCTION, not the `"active,inactive,failed"` string) and `dag/test/claim/spark_managed_grant_reconcile_witness_test.dag` (one `data` row declared twice with an identical value). Both are repaired in this change; the deletion IS the census. THE BOUND THAT MATTERS MOST is not the compiler's: where the two definitions are semantically EQUIVALENT no behavioural witness can discriminate them whichever wins, so green-by-execution — which this repository leans on everywhere — is STRUCTURALLY blind to duplicate authorship. That is why the evidence is a compile-seam refusal and not a runtime probe. | R2 (mechanically preventable) reached; R4 (unwritable) is the honest ceiling only once ingestion carries a keyed declaration roster that cannot hold two entries for one name | the refusal is a fold over the module's item list in `v1.compiler.resolve` `check_duplicate_declarations`, not a property of the carrier: the duplicate is still AUTHORABLE and is refused after it is built | wall: `v1.compiler.resolve` `check_duplicate_declarations` emitting `DuplicateDeclaration` (blocking, default arm of `is_interpreter_blocking_diagnostic`). RED + positive control: `duplicate_declaration_wall_witness` in `v1.compiler.compiler_tests_rust`, authored at the FIXTURE boundary (two REDs — fn/fn and data/fn — and a green control on distinct names). ENROLMENT, stated rather than inherited: the Rust suite was removed from CI 2026-07-11, so that witness executes LOCALLY only and guards nothing on a push; `dag/test/claim/duplicate_definition_binding_probe_test.dag` asserts the same refusal in `.dag` and is `ReadsLiveTree`, hence `DeclinedLiveTree` in the required floor — discovered, counted, never run. So the wall is real and its EVIDENCE is unenrolled, which is a weaker claim than R2-with-a-guard and is written that way deliberately | next trigger, two and separable: (a) an enrolled route for a compiler FIXTURE red — either the `DeclinedLiveTree` arm's deletion (which `v2.workflow.required_floor` already carries as staged) flipping the `.dag` probe into the floor, or a hermetic census that does not walk the live tree; (b) for the R4 climb, an ingestion-time keyed declaration roster (`v1_compiler.declaration_index` is the nearest existing construction) |
...
| Gate whose only independently discriminating conjunct is not the one it is named for (**filed 2026-08-29**, specimen `gunbc.host_axis_caps` `deploy_memory_cap_script_author_precedes_shadow_revert`) | **R1, and the rung is carried by the fixture REDs rather than by the production call.** The gate returns `list_length(units) > 0 && script == ordered`, where `ordered` is the author-then-revert composition rebuilt inside the gate from `deploy_runner_memory_cap_author_stmts()` and `deploy_memory_cap_shadow_revert_stmts(units)`. Both conjuncts are real, and they are not equally informative. The **nonempty-unit conjunct is valid and must be kept**: whether a host has at least one runner unit is a fact the gate does not itself produce. The **ordering conjunct is a duplicate reconstruction** — at the production call site (`gunbc.live_deploy.emit` `deploy_memory_cap_apply_steps`) the `script` argument is `deploy_memory_cap_apply_script_for_host(host)`, which composes author-then-revert from the same two producers over the same `units`, so `script == ordered` is tautological on the production construction and cannot discriminate. With `host_axis_caps_author_before_revert_enforced` constant true, the gate's production behaviour is decided entirely by the unit count. The ordering is genuinely checked only where a **fixture** supplies a script the gate did not build — `witness_reversed_deploy_script_fails_author_before_shadow_revert_gate` and `witness_padded_wrong_order_script_fails_author_before_shadow_revert_gate`, both executing REDs. | R2 — ordering becomes **structural in the script constructor**: one producer emits author-then-revert as a single composed value, so a wrong order has no writable form and needs no predicate to rule it out | Not raised here, and deliberately **not** "fixed" by deleting the check. §4b rules that a fixture-authorable RED is real evidence, so the gate is not decoration as a function; what is inert is the *ordering half at the production call site*, which reads as enforcement of an order that cannot vary there. Validating an order by rebuilding the script a second way is also the §5 tell — a check re-stating a constraint the construction already carries — and the repair is construction, not a better predicate. It is **not** a cost defect: measured, the two derivations are 1183ms and 1166ms alone but 1287ms together, so the interpreter already memoizes the second, and an earlier reading of this as duplicated work was refuted by that measurement. | The two fixture witnesses above (executing REDs); cost partition on this branch via `claim_batch` per-witness `cpu_ms` | The composed author-then-revert constructor. Until it exists, this row records that the production call enforces a unit count and *couples* two functions that coincide today — it does not enforce an ordering. |
...
| **P0.1** | Field access has a receiver proven to carry that field | **Split.** Enclosing unconstrained type parameter: seed refuses (`InternalError` / `floor_field_through_generics_type_variable_refuses`, gunbc#11285). Remaining `field_of_type_var` mint is the callee-generic deferral arm, not that hole. | Unchanged for the deferral population | **R2** | R3 field-carrying bound. Baseline → generic-field constraint wall (enclosing T climbed; remaining is in-flight callee discharge) |
...
| Field typos | **PARTIAL** — concrete types checked; enclosing unconstrained type parameter refuses (`InternalError`, gunbc#11285); through an in-flight callee generic or inference placeholder, still deferred | `v1.compiler.infer` `is_never_constrained_enclosing_generic_base` takes the existing no-field `InternalError`; `field_of_type_var` remains only for bases not yet known at this infer_expr (`src/v1/04_infer.dag` `is_deferred_field_access_base`) | wall after grounding (deferral population) |
...
10. **Unenrolled floor measurements** (opened 2026-08-01 by codex review 46306, which caught
   ...
   **NOT DISCHARGED — HALF-DISCHARGED 2026-08-22, and the unfinished half is the half that mattered.** The AUTHORSHIP this item asks for is done, for FIVE classes rather than the four it names; the ENROLMENT is not, because the witness carrying it is declined by the floor before it runs (§11 item 28). Read the rest of this paragraph as a description of authored artifacts, never of executing ones. The
   authorship landed as `dag/test/claim/guarantee_floor_class_probe_witness_test.dag` over five
   registry classes in `gunbc.guarantee_probe_corpus` (`floor-generic-instantiation-inhabitance`,
   `floor-field-through-generics`, `floor-closed-match-exhaustiveness`,
   `floor-record-field-completeness`, `floor-parse-list-separator`) — this item's four plus the
   parse-separator row whose evidence was a throwaway session probe. All five were the same shape:
   a rung standing on a measurement nothing re-runs — and all five STILL ARE, because a declined
   witness re-runs nothing either. **A SIXTH WAS AUTHORED IN THE SAME PASS AND WITHDRAWN BEFORE
   MERGE**, and it is recorded because the withdrawal is the §3 move rather than a loss: the
   `v2.`-module direct-call argument-type exemption was built here as
   `floor-declared-conformance-v2-module-exemption` and deleted when gunbc#8902 landed
   `direct-call-argument-type` on main first with three probes over the same seam. One concept,
   one authority — that class is theirs, this lane carries no second name for it, and the §1c row
   for the seam cites their rows (with the note that their probes sit in the same declined arm).
   **What this does NOT claim.** Four of the five are
   BELOW-FLOOR HOLES, and enrolling a hole raises nothing: the probes pin the defect so it cannot
   change unnoticed, they do not close it. The rungs are unmoved by this change — what moves is
   that they are now *observed* rather than *remembered*. To keep a hole's green from reading as a
   guarantee the expectation vocabulary gained `ExpectSilentAcceptanceHole`, a variant that holds
   on exactly the same observation as `ExpectZeroDiagnostics` and exists precisely because that
   coincidence would otherwise make walls and defects indistinguishable in the registry; the
   corpus can now be asked how many of its greens are holes
   (`guarantee_probe_below_floor_hole_rows`). Each hole ships with a discriminating control that
   refuses on the same harness in the same run, per the 2026-08-01 probe-adequacy mandate, so a
   zero is never the harness failing to reach the judgment. **Still open beside this:** the enrolment itself, on §11 item 28's trigger; item 9
   (the dark Rust suite) is untouched — this was authorship, not a move; the record-literal FIELD
   seam of the inhabitance class (gunbc#8865) still has no probe; and every hole's next-rung
   trigger is unchanged, with the probe flipping RED and its row being rewritten as
   `ExpectBlockingRefusal` on the day its wall lands rather than being deleted (§4b
   meta-obligation 4).
...
28. **The guarantee-probe corpus is DISCOVERED AND DECLINED: five carriers, 100 identities, and
   six §4b classes whose executing evidence does not execute** (opened 2026-08-22, this lane,
   from a specimen established by session silent-badger-817 and relayed through
   eager-crane-282 — recorded here rather than repaired, because the repair is a fleet-blocking
   sequencing decision that is explicitly not this lane's to take).

   INVALID STATE. `v2.workflow.required_floor` `RequiredFloorDisposition` carries a
   `DeclinedLiveTree` arm: a witness file declaring
   `data live_tree_disposition: LiveTreeDisposition = ReadsLiveTree` is DISCOVERED by the floor,
   counted in `declined_live`, and never folded. The guarantee-recovery evidence base declares
   exactly that, truthfully, and is therefore the worst-hit slice of the arm:

   | carrier | identities | what cites it |
   | --- | --- | --- |
   | `test.claim.guarantee_probe_corpus_witness` | 50 | the six landed classes — bare-none field admission, call label/surplus, method established surface, declared-conformance ground fragment, sole-constructor cross-module, v2 self-grounding frontier |
   | `test.claim.sole_constructor_completeness_audit_probe_test` | 26 | DESIGN §4b's `sole_constructor` completeness audit paragraph and this document's §10 2026-08-20 pass |
   | `v2.test.claim.infer_self_grounding_wall_test` | 12 | the `v2-infer-eval` path's receipts |
   | `test.claim.compile_diagnostic_census_witness` | 8 | the OBSERVATION SURFACE every probe above is read through |
   | `test.claim.optional_cast_wall_witness` | 4 | the optional-cast wall |

   HARM, and it is this document's own evidence base rather than a compiler behaviour. §4b
   meta-obligation 4 requires a class's discriminating RED to REMAIN ENROLLED as the executing
   proof that its rung stays real. A declined witness satisfies the letter of "a probe exists"
   and none of the substance: if any of those six walls regressed tomorrow, nothing would red.
   That is rung inflation applied to the guarantee-recovery ledger itself — worse than sitting
   low, because an inflated class never ranks for climbing.

   THE DECLARATIONS ARE HONEST AND MUST NOT BE RETYPED. The probes do read the live tree:
   `compile_dag_diagnostic_census` resolves its synthetic source against
   `build_module_path_index_from_witness_roots`, which walks the checkout, and the witnesses
   additionally `filesystem_read` their harness sources to mint a harness revision.
   Relabelling one `SubstrateInputsOnly` to make it execute would be a fabricated declaration —
   the inverse defect, and worse than the one it fixes, because it makes a live-tree probe look
   inert to every other consumer of that declaration (affected-set selection eligibility, which
   is the fact `live_tree_disposition` actually owns).

   THE DISCRIMINATING SPECIMEN, and it is why the arm is wrong rather than merely expensive.
   `test.claim.match_exhaustiveness_coproduct_witness` calls the SAME
   `compile_dag_diagnostic_census` builtin, declares `SubstrateInputsOnly`, and EXECUTES AND
   PASSES on the floor today. Same host call, opposite declaration, opposite fate. Two things
   follow, and the second matters more than the census: the inverse defect above is already live
   in the corpus and is currently the ONLY reason any census-compiling probe runs at all; and
   the decline is therefore a PREDICTION about capability, refuted by execution — a
   census-compiling witness demonstrably can run under the hermetic floor, via the checkout-read
   carve-out (`v1_interpreter` `hermetic_checkout_read_disposition_under`) that landed AFTER
   ...
   `declined_live=830`. The 20-identity (2.4%) gap is NOT reconciled — the candidate causes are
   `modules_excluded=4` and the fact that this census matched long-home prefixes syntactically
   rather than through `long_home_prefixes()`. Same order, not the same number, and it is
   ...
   THE RECEIPT IS PINNED TO THE HEAD THAT PRODUCED IT, AND THAT HEAD IS NO LONGER THIS BRANCH'S
   TIP. Both runs above measured a witness file carrying 20 identities. Merging main afterwards
   removed three of them — the `v2.`-module exemption probes, withdrawn because gunbc#8902 landed
   the same seam first (§11 item 10's withdrawal paragraph) — so the file now carries 17 and the
   next run's `declined_live` will read 17 above whatever main reports at that moment, not 20
   above 830. The +20/+0 receipt is left stated as measured rather than rewritten to the new
   number, because it is a record of an execution and not an estimate of the current tree; what
   the merge changes is the arithmetic a reader should expect next, not what was observed.
   ...
   cite these probes now say authored-not-executing in line.

   NEXT-RUNG TRIGGER — SUPERSEDED, AND THE WAY IT FAILED IS THE LESSON (corrected 2026-09-22).
   As written it was one deletion and not this lane's: delete the `DeclinedLiveTree` arm at the
   root, as `required_floor`'s header already staged it. THAT DELETION HAPPENED AND THESE
   CARRIERS STILL DO NOT EXECUTE — the 2026-09-19 nominal-roster ruling cut the required gate to
   `required_gate_prefixes` / `required_gate_authored_modules`, which they match neither of, so
   they disposition `DeclinedOutsideRequiredGate` instead. A trigger that names an artifact
   rather than the capability is satisfied while the capability stays dead (DESIGN §4b(3)); the
   capability here is a required gate that PLANS these identities. The trigger is restated as
   that capability, reachable by a module name on an already-admitted prefix (the route the
   record-completeness zero-field family took) or by the derived changed-population selection
   that `required_floor` names as `required_gate_bankruptcy`'s restoration trigger — never by a
   roster row, which is closed to growth without operator sign-off. The paragraph below is kept
   as the record of why the deletion was sequenced rather than as a live instruction. Its cost measurement is
   already taken and cited there: run 32345970386 measured 626/783 passing under execution, so
   roughly 157 identities would red and need triage or expected-red enrolment IN THE SAME
   CHANGE. WHY IT IS SEQUENCED RATHER THAN DONE HERE (operator-lane ruling relayed by
...
30. **The `v2.*` argument-type exemption's residue is TWO COMPILER SEAMS, both reproduced minimally,
   ...
   **THE 19 IS A COUNT OVER A MOVING CORPUS, so it is bound to its runs rather than stated as a
   ...
   - **Seam A — a transparent alias whose RHS is a GENERIC INSTANTIATION is not peeled.** Population
     ...
     authority in `v2.std.patch`, instantiated eight times. **ZERO SITES AS OF gunbc#11749**: the
     aliases were single-use nicknames on top of that shared authority and were deleted, so the use
     sites now name `ConfigPatchRecord<XConfig>` directly — the shared authority is untouched and
     more directly consumed, and the seam is unrepaired. The reproduction below is now the ONLY
     discriminator for this seam, and it still reds. **Reproduction, 12 lines, one red cell and
...
33. **Two definitions of ONE NAME in ONE MODULE are accepted with zero diagnostics, and the
   ...
   **The evidence, and the honest statement of where it executes.**
   `dag/test/claim/duplicate_definition_binding_probe_test.dag` asserts the CORRECT behaviour — the
   ...
   declares `ReadsLiveTree` — and `v1_compiler.cli_run` `run_required_floor` routed every such
   ...
   condition. **THE NON-ENROLMENT STILL HOLDS AND ITS REASON CHANGED (corrected 2026-09-22):**
   `DeclinedLiveTree` is deleted at the root, so a live-tree read no longer declines anything;
   `test.claim.duplicate_definition_binding_probe_test` matches no row of
   `v2.workflow.required_floor` `required_gate_prefixes` or `required_gate_authored_modules`
   after the 2026-09-19 nominal-roster ruling, so it dispositions `DeclinedOutsideRequiredGate`
   and the enrolment would be false for the same reason under a different mechanism. See the
   correction on item 28's trigger.
