...
29. **THE POSITION CENSUS, RE-MEASURED INDEPENDENTLY, AND THE TWO CELLS IT ADDS: a parameter's
...
30. **DECLARATION-SITE DEFAULT-VALUE EXPRESSIONS are RESOLVED but never INFERRED, so an undefined
   name at one is accepted — and this is a SECOND AXIS, cut by pass coverage rather than by
   grammar position** (opened 2026-08-22, session quiet-boar-696, split out of item 29 because
   severity does not follow position-count: at item 29's seven positions a judgment runs and
   ...
   *Walked, typed* — `fn` body against the declared return; `data` initializer against its
   ...
   WHY THE SECOND ARM IS THE FINDING AND THE FIRST IS NOT. At item 29's positions the reachability
   ...
   the same territory as item 29's other positions rather than upstream of it, and the
   ...
   *unanalysed* and *analysed by a pass that does not judge this* are different defects with
   ...
   declared type, which makes both members ordinary consumers of item 29's obligation rather than
   checks of their own.

31. **WITHDRAWN BEFORE IT WAS ACTED ON: `gunbc compile` does NOT report a budget refusal as
