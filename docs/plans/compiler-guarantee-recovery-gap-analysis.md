...
31. **A function name's meaning depends on the CONSUMER'S TRANSITIVE IMPORT CLOSURE, so adding
 ...
 `floor_preparation` is COLLATERAL — it reaches `object_store`
   ...
   *ambiguous* are different states with different repairs (one says no authority answers, the
   ...
   THIS ROW AND THE TRANSPORT-ARGV ROW MUST LAND AS ONE CUT, NOT TWO. (That row is carried by
   gunbc#8916, unmerged at the time of writing, and is cited BY PR rather than by row number
   because the numbering has already shifted twice under both rows while they sat in review —
   this row was drafted as 30, main independently landed a different 29 and 30, and it is now 31.
   A row number is a POSITION in a file that many lanes append to concurrently, so citing one is
   the §3 positional-citation defect with an unusually short half-life: the reference rots without
   anyone touching either row. The PR number is the symbol here.) The transport-argv row's fix (gunbc#8916) is to run
   the existing expression-resolution authority over the transport argv expression tree. If that
   authority still answers with a silently-chosen name, that repair INHERITS this defect at
   ...
   FIRED IS NOT THE SAME COUNT AS REPAIRED, and both belong in the record. THREE sites errored —
   those are the diagnostics. FOUR were repaired: the fourth,
   `v2.workflow.floor_preparation` `prepared_floor_probe_for_witness`, returns from a bare
   `map_get` and did NOT appear in any diagnostic, because its closure had not been perturbed in
   the way that would expose it. It is the class waiting for the NEXT edge rather than broken by
   this one, and it is a small live demonstration of the 46-file claim: **the diagnostics are not
   the population, they are the population's intersection with today's import graph.**
