...
**The two directions are not symmetric, and conflating them would overstate the finding.** A `Node` reaching a `NormalizedTree` position is **invisible** — accepted at typecheck, failing at runtime, and the whole subject of this receipt. The reverse, a `.root` projection applied to a genuine `Node`, is **caught statically at resolve**. Both occurred here: the conflation repeatedly, and the projection twice, when bulk edits applied `.root` to two sites whose values came from `resolve` (`ResolvedTree = Node`) rather than from `normalize`. The second class was found by the compiler within one cycle and never reached a wrong answer. So the honest claim is narrow: the checker does not distinguish a wrapper from its payload *when the payload is supplied where the wrapper is declared*, and it does distinguish the opposite. A receipt that cited both as blindness would be inflating its own class.
