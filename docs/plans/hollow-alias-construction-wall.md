...
**Status:** DRAFT, design-note-first per sharp-bee-290 mandate `msg_7a83651e-f370-4b2a-8786-e5c0d81aeaa5` (nod 2). Capstone of the completion-pattern lane — the operator sequenced the wall behind the [GroupCompletion/FieldOfFractions specimens](groupcompletion-pair-construction-design.md), not behind their full implementation, so this note can proceed while `FieldOfFractions` realization (§7.3 there) is still held for sign. Not implemented; no code lands from this note yet.
...
## 5. Discriminating RED (to be written when the lens lands — none exists yet, this is a note)
