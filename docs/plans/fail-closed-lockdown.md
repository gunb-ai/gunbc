...
| RustMonolithGate | `.rs` + manifest | `gunbc.ci_gate` `RustMonolithGate`; `tools.rust_gates_ci` `run_gates` |
...
| **EmitHostGate** | wired but thin | exactly **4 MVP fixtures** (rust/python/go/ts), not the emit tree (`tools.emit_host_gate` `emit_host_gate_passes`) |
...
| --- | --- | --- |
| `resolved_graph_cache.rs` `closure_content_digest` | **cache flake** | content digest = hash of `from_utf8_lossy(bytes)` — **lossy decode, not raw bytes** → warm≠cold / wrong-hit physically possible. **VERIFIED.** |
| `resolved_graph_cache.rs` `read_cached_header` | cache key | subject/content digests read via `from_utf8(..).unwrap_or("")` — malformed → `""` → collision |
| `v1_interpreter.rs` `parse_table_memo_scope_and_key` | under-keyed | key `(grammar_digest, token_digest, position, production)` — cross-file position collision; no red witness |
...
| `v1_interpreter.rs` `eval_shell_exec`, `eval_service_call` | fail-open IO | `from_utf8_lossy` / `unwrap_or_default` fabricate plausible text from malformed output |
...
5. **CI-coverage-completeness audit** — tests/gates that *exist but don't run*. Confirmed instance: the rust gate runs a "known-green subset" (`interp_recorded_fixture wet_hermetic resolve_expr_types_retraversal`, `gunbc.ci_spec` `gunbc_ci_floor_spec`) of **60** `src/v1/tests` files — the rest rot silently (how the Behavior-arm test went stale). Also: the `discrimination` lens (the §5 discriminating-witness enforcer) is itself roster-only, not whole-corpus — the enforcer is vacuous.
