...
| Step 5 roster — **CI skip-decision surfaces** (added PR #7432) | **RETAINED — explicit deferral** | `cli_run::selection_control_input_sources` + `cli_run::selection_control_skip_label_for_ci` (with `SelectionControlRefusalCause` / `SelectionControlSkipRefusal`) and the `selection_control_skip_witness` bin. HAND-RUST GATE explicit deferral (review 44745): host-side selection Rust of the same class Step 5 retires — a path/import-closure decision living in the seed rather than in `.dag`. Why deferred rather than modeled now: the decision must run BEFORE the floor resolves anything (it exists to avoid that resolve), so a `.dag` consumer would pay the ~100s cold whole-pool resolve the skip is meant to save — the deferral dissolves with the same persistent content-keyed node store `gunbc.ci_materialization`'s resolve-receipt note names, not on its own schedule. Near-term dissolution: unify the import walk with `cli_run::regen_input_sources` behind one parameterized helper (duplicate policy + entry source as arguments) once regen's byte-identical oracle can re-green on it. Checkable receipt: 9 label arms green by execution in `cli_run.rs` (7 structural + 2 refusal, incl. `selection_control_skip_refuses_when_diff_observation_fails` as the discriminating RED) plus 3 bin unit tests; the surface is counted here rather than left implicit. |
