...
1. **Bounded input before cost before schedule.** A `WorkDemand` without a declared input ceiling is fail-open (§5): the scheduler has no principled admission or cost basis. `InputEnvelope = BoundedInput | EnvelopeUnknown`; `EnvelopeUnknown` is honest bottom — admission refuses (`RefusedUndeclared`), never silent green.
...
**Authority:** `gunbc.ci_input_envelope` (`InputSizeAxis` · `InputBound` · `InputEnvelope` · `input_admitted` · `gunbc_ci_corpus_envelope`). **P1 landed:** types + admission witness (`dag/test/claim/input_envelope_admission_test.dag`). Scaffold ceilings in `gunbc_ci_corpus_envelope` dissolve when P2 derives them from corpus discovery (witness roster count, source-node count, corpus-node count) — operator-set operating-point ceilings in P1 are explicitly **not** measurements.
...
| **SCHEDULING** (§1/§2) | given admitted work, how wide / where / when? | CostEnvelope (Predicted) + host `ResourceEnvelope` / budget tree | over-commit → refuse or shrink width, never silent OOM |
...
- **Host / run budget** — `product.budget_tree` + `compute_fabric.ResourceEnvelope` (BUILD vs RUN phases stay distinct — two invariants, not one product)
- **Measured observation** — `compute_fabric.PerformanceReceipt` → roll-up to `CostAccount` with `basis = Measured` (falsifier only)
...
4. **P3 — Measured falsifier + calibration loop.** `PerformanceReceipt` / per-shard RSS (`resource-aware-scheduler.md` Node A) feeds `CostBasis = Measured`; mismatch vs Predicted → typed diagnostic. Calibration updates symbolic formula inputs, not the Predicted row directly. **Accept (T3):** planted underestimate in Predicted → falsifier RED.
5. **P4 — Runtime admission gate.** Wire `input_admitted` into scheduler / `WorkDemand` construction so runtime refuses before schedule build. **Accept (T2):** corpus exceeding envelope → schedule refuses, not OOM.
6. **P5 — P2 ceiling derivation.** Replace scaffold ceilings in `gunbc_ci_corpus_envelope` with values derived from discovery roster + measured node counts; dissolve `Scaffold` disposition on the envelope data.

## 8. Relationship to companion docs

- [`input-envelope-roadmap.md`](input-envelope-roadmap.md) — P0/P5 detail for InputEnvelope; defers to §3a here for authority
- [`resource-aware-scheduler.md`](resource-aware-scheduler.md) — Nodes A–D implementation tracker for P2–P3; defers to §5–§7 here
- [`ci-selection-vs-scheduling.md`](ci-selection-vs-scheduling.md) — selection axis framing; §4 pipeline table is the composed view
- [`compute-envelope-model.md`](compute-envelope-model.md) — host `ResourceEnvelope` and BUILD-phase fan-out; RUN-phase width is §5 here
...
Delete this doc when every `WorkDemand` carries `BoundedInput` or an explicit `EnvelopeUnknown` escalation, every floor `Runnable` carries Predicted `CostEnvelope`-derived cost, `spawn_width` is derived from `WidthBoundedRealization` (no side-channel), Measured falsifies Predicted with a discriminating RED, and the selection/scheduling axis split is witnessed on the live floor — at which point the carriers state the policy and companion trackers (`resource-aware-scheduler.md`, `input-envelope-roadmap.md`) may dissolve.
