...
Typecheck is 69% of it, load 36%. Named `resolve` — the step the lane is named for — is 0.2%.
Anyone attacking "resolve" by its name will optimize the wrong thing; the cost is in *typecheck* and
*load*, and the lane name is a nickname for the whole entry-graph construction.
