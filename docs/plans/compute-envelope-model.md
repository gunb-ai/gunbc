...
Collapse the N hand-tuned dimensions into **one authority** — the host `ResourceEnvelope` (cores, memory, pids capacity; a type in `gunbc.fleet_container`, which is where #5904 moved it and where it lives now that `product.compute_fabric` is deleted) — and make **every knob a function of it**. Then the dimensions stop being N things to juggle: they are one measured fact with derivations, and the fleet gets **exactly one coherent operating point**. It *cannot* be both starved and thrashing, because both the width floor and the fan-out ceiling come from the same envelope.
...
- **PUBLIC (central, topology):** the envelope *shape*, the width *derivation*, the codegen-units cap — all live in `gunbc.fleet_container` / `std/realization_width.dag` / the `[profile.test]` block. These are host-agnostic functions: "given an envelope, here is the width / the fan-out."
...
1. **Ground the envelope in measured truth** — kill the 64c/256GB lie (real: 128c/128GB, both hosts). *Everything downstream inherits this; you cannot derive width from a lying envelope.* (ctrl + the `gunbc.fleet_container` envelope facts.)
