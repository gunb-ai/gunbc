...
**Keys are algorithmic.** Each identity is a total derivation into a canonical encoding that is injective over its domain. A finite hash of that encoding is never injective. The hash family is chosen by sharing scope, and each scope declares a collision disposition: within one run, a structural fingerprint (`Fnv1a64Structural`) is an index and a hit verifies by content before serving; across a run, host or store, a collision-resistant family (the SHA families of `std.content_hash`) is required, and a hit still verifies the canonical preimage before serving. Hand-authored, positional or lossy encodings are defects; C1 is the recorded specimen -- a live cross-parse memo identity whose digest used the wrong unary byte domain. The `conformance-identity` domain owns the key RELATION (what relation, what scope, what complete canonical preimage derives it, what equality and collision disposition govern it); identity owns whether the declared inputs are complete. `conformance-realization` owns whether a handler, provider or placement faithfully realizes an admitted identity and result contract (completeness, readback, retention, transport, refusal, binding obligations) -- whether the handler consumes the admitted key without substituting, truncating or bypassing it. Cache purity is one discriminator across that boundary, not the whole realization domain.
...
- **Cross-run hit protocol.** Two routes are admissible. Manifest-validation (first consumer): DemandIdentity looks up a MUTABLE evaluation-index record {read-set manifest, EvaluationIdentity, ResultContentIdentity}, validates that current member identities match the sealed read set, then serves the IMMUTABLE content-addressed result or re-evaluates. The index is a mutable alias with compare-and-set/generation on update: two evaluations racing for the same DemandIdentity CAS the index; the loser re-reads and either attaches to the winner's verified binding or re-evaluates if the winner aborted; a stale manifest (current members disagree with the recorded read set) is a miss and re-evaluates; a missing or corrupt index is not an absent materialization -- the immutable store is still queried by ResultContentIdentity when known, otherwise the demand re-evaluates; same EvaluationIdentity with different ResultContentIdentity is a conflict, never a CAS winner. The immutable store stays content-addressed independently of the mutable alias. Discovery-first: re-run pure dependency discovery, form the exact read set, derive EvaluationIdentity, then immutable lookup. **Ruled for the first consumer: manifest-validation.** Discovery-first re-executes the dependency pass the demand engine already performs and remains an admissible later handler.
...
| `v2.workflow.scheduler` / `executor` / `runtime_run`, `module_resolution_plan`; `v2.workflow.realization_runner` scheduling surfaces | **Scheduling replaced (M3).** Layer planner, batches and sequential runners are deleted. Relocated from `realization_runner` (not deleted): `NodeKeyedGraphArtifact`, `NodeKeyedGraphRow`, `NodeKeyedGraphArtifactEntry`, `node_keyed_graph_row_lookup` / `NodeKeyedGraphRowMissing`, `node_keyed_graph_transitive_bytes` / `NodeKeyedGraphTransitiveBytesMissingRow` (missing-reference refusals and transitive-byte derivation). Deleted from that module: `runner_dispatch_*`, `runner_walk_batches`, `RunnerSchedulePlan`, `runner_schedule_plan_from_dependencies` and the other runner scheduling surfaces. `module_resolution_plan` and its mirrored host realization move onto the engine. The interpreter is not deleted at M3. |
...
Order: M0 -> M1 -> M2 -> M3 -> M4 -> M5 -> M6 -> M7. Law-bearing decomposition is last: do not optimize stage-wide folds that M6 deletes. Each lists what it makes true, what it deletes in the same change, its production consumer, its qualification and its exit.
...
**Makes true.** Section 2.5 with Share and explicit Recompute: demand closure, suspension, availability states, one ready queue, one dispatch policy, typed blocked states, in-flight attachment. The native fold's driver becomes a `.dag` demand over its universe; the emitted main reduces to the host-effect boundary. M3 deletes SCHEDULING authority only. The current interpreter remains the production semantic handler until emitted Rust implements the complete judgment and refusal contract.

**Deletes in the same change.** `v2.workflow.scheduler` layer planning, `v2.workflow.executor` batches, `v2.workflow.realization_runner` scheduling surfaces (`runner_dispatch_job`, `runner_dispatch_jobs`, `runner_dispatch_batch`, `runner_walk_batches`, `RunnerSchedulePlan`, `runner_schedule_plan_from_dependencies`, `runner_schedule_store_covers_nodes`, `runner_schedule_batch_count`, `runner_schedule_contains_batch`, `runner_execution_dependencies` as a scheduler wrap), `v2.workflow.runtime_run`'s sequential fold, `module_resolution_plan`'s antichain-batch walk and its mirrored host realization, and the Rust module loop in `v1.compiler.emit_rust` `emit_source_root_eval_driver_main_rs`. Relocated, not deleted: `NodeKeyedGraphArtifact`, `NodeKeyedGraphRow`, `NodeKeyedGraphArtifactEntry`, intern/lookup rows, `node_keyed_graph_transitive_bytes` and the missing-row refusal arms (`NodeKeyedGraphRowMissing`, `NodeKeyedGraphTransitiveBytesMissingRow`). The interpreter is not deleted and is not demoted to an oracle at this cut.
...
D-row instrument: total=8 / ruled-or-deferred=8 / unresolved=0.
...
8. **D8 — First authority cut.** The v2 native fold's driver (M3). Recommendation: confirm. **Deferred:** trigger is M3's detailed plan confirming that driver as the first production scheduling cut; the consumer is not changed here.

## Dissolution trigger (DESIGN §6)

Delete this document when M7 exit holds and every earlier milestone's deletions are census-verified: program runs go through the demand engine, no production key lies outside a named keying relation, no production dependency relation restates derivable program flow, no width integer exists in production (M5), the store serves cross-run repeats on the landing-receipt path (M5), the compiler's stages are keyed computations (M6), and every production decomposition cites law evidence (M7) -- at which point the carriers state the design and this document is a second copy of it.
