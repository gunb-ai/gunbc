...
**Keys are algorithmic.** Each identity is a total derivation into a canonical encoding that is injective over its domain. A finite hash of that encoding is never injective. The hash family is chosen by sharing scope, and each scope declares a collision disposition: within one run, a structural fingerprint (`Fnv1a64Structural`) is an index and a hit verifies by content before serving; across a run, host or store, a collision-resistant family (the SHA families of `std.content_hash`) is required, and a hit still verifies the canonical preimage before serving. Hand-authored, positional or lossy encodings are defects; the C1 token-stream digest, which mapped every value above 255 to one tag, is the recorded specimen (and after #11401 it has a live cross-parse memo consumer -- unread is stale). The `conformance-identity` domain asks derivation questions of every change (M0); cache purity is `conformance-realization`.
...
- **Cross-run hit protocol.** Two routes are admissible. Manifest-validation: DemandIdentity looks up a prior evaluation record {read-set manifest, EvaluationIdentity, ResultContentIdentity}, validates that current member identities match the sealed read set, then exact-hits or re-evaluates. Discovery-first: re-run pure dependency discovery, form the exact read set, derive EvaluationIdentity, then immutable lookup. **Ruled for the first consumer: manifest-validation.** Discovery-first re-executes the dependency pass the demand engine already performs. Discovery-first remains an admissible later handler, not the first production route.
...
| `conformance-identity` / `conformance-realization` | **Partition.** `conformance-identity` owns key DERIVATION correctness (relation, encoding, family, scope); `conformance-realization` owns cache PURITY (whether a materialized value denotes the same fact). |
...
- **Conformance.** Every change is read by `conformance-identity`, `conformance-realization`, `conformance-compute` and `conformance-decision`; departures state reasons. `conformance-identity` owns key derivation; `conformance-realization` owns cache purity.
