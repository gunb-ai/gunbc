...
- **FLAG E — first scenario. SIGNED: single-process-multi-thread first**, multi-process explicitly deferred. First concrete win: the floor's own dependency-independent gate-series → `compile-clean + MAX(gate)` instead of `SUM(gates)` (§6-1).
