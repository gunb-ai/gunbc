...
This is not merely `state_space_conflation`. A state-space conflation makes a carrier unable to represent distinct states. A meaning fork can exist even when the underlying states are modeled perfectly: the defect is that one **scoped name** continues to denote more than one **material semantic referent**. Hold `(naming surface, visible name, declared effective version or epoch)` constant and vary the hidden state. If the material semantic referent changes, the name has forked. For a contract-bearing name, a change in obligations, floor, refusal behavior, billing consequence, or remedy is sufficient evidence of that change.

The private premium-name/full-strength-versus-quantized case is the direct specimen. `ProviderInterfaceBinding` is an adjacent homonym across distinct declared naming surfaces, not a direct specimen; whether a shared unqualified surface collapses those senses belongs to the operator census.
