...
**What the private repo already carries as executing evidence.** `strategy.pricing_decomposition` holds the typed constructions — `quality_dimensions` (`GuaranteedFloor`/`OpaqueOurs`/`DisclosedPolicy`, one witness per kind), `floor_met` (both sides), the closed invoice-constructor sum with seven refused constructors, `risk_interfaces`, `moral_hazard_walls` — exercised by `pricing_decomposition_witness_test`. `accuracy_of_meaning_ruling` itself is a prose `NonEmptyStr` row the witness neither imports nor evaluates, so the executing evidence covers the service-floor shape and selected risk/pricing declarations, not yet a generalized meaning-fork or externalization wall — one more reason the two ledger rows land as review-diligence vocabulary rather than citing a gate. The five-gap externalization audit (resale condition representation, durability remedy, isolation floor, home-hosting obligation gate, KnownRed staleness) is the specimen backlog the `externalized_degradation` row will accrete receipts from as those lanes land. Honesty boundary, stated so the rows do not inflate: private witnesses are not enrollable evidence in the public repo — no public gate executes them — so both rows land at *review-diligence vocabulary*, exactly like `v1_maintenance_standing`, and say so.
