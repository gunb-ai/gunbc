...
`native_witness_bundle_execution_admit` takes the `SelectedWitnessPlan` and current compiler identity at the acceptance boundary. Before inspecting verdicts it refuses a stale plan or receipt compiler identity, wrong bundle identity, an empty native plan, an incorrect counted interpreter frontier, a non-warm artifact receipt, or any receipt whose ordered `(witness identity, realization family)` roster differs from the plan's complete native roster. It then refuses the first divergence with the member and both typed verdicts. The wrong-body control deliberately preserves the interpreter's canonical `1,1,0` answer while the swapped-arm native bundle returns `0,0,1`; admission must therefore produce `NativeBundleExecutionRefusedDivergence`, never relabel the two answers as an equivalent failure. Cutover evidence is the complete planted plan plus its native execution receipt, not a detached claim row. Admission validates that receipt's compiler, bundle identity, process count, counted frontier, and complete ordered roster, then derives “discriminating” from an interpreter-pass/native-fail row whose entry, function, and family match a current-plan member while its planted subject digest differs. The production-positive witness feeds admission the live wrong-body execution receipt. It also refuses a family with no live discriminating RED and any receipt claiming other than one bundle process. Nothing auto-picks native or interpreted after a divergence.

Bundle admission is a separate check: a plan or bundle compiled under a different compiler identity is refused by its corresponding typed stale-compiler arm; a bundle whose identity does not equal the current plan is `NativeBundleRefusedPlanIdentity`; and a bundle whose independently writable member list differs from the plan-derived list is `NativeBundleRefusedMemberCoverage`, even if it reuses the expected identity. All refuse before execution.
...
- stale-plan and stale-receipt compiler, identity-drift, empty/incomplete member coverage, counted-frontier drift, warm-artifact miss, no-native-realization count, missing-RED, process-count, and native/interpreter-divergence refusals.
