...
- **`LiveTreeDisposition` stamp-retire** — the ~697/780 hand-stamped witness `live_tree_disposition` rows retire to `derive_live_tree_disposition(envelope)`. **BLOCKED on an operator decision**: path A (envelope-reach, i.e. build per-witness envelopes and derive) vs path B (the `v2.std.live_tree` G2 runtime-read-closure derivation its own dissolve trigger names — G2 lens landed, only G1 wired). Both are viable; the two design docs disagree on which is the retire target.
- **Item 4 — delete `Hermetic | Wet | Record`** — the enum retires once every reader migrates to envelopes. Gated on slice 2 landing + the stamp-retire; the presets (`v2.std.execution_envelope`) and the seed `std.execution_mode` twin delete together.
...
**Sibling thread disposition (origin 2026-07-21; superseded by operator ruling 2026-08-01):** node/subtree visibility was prototyped as `Reference` and `Publish` verbs riding this grant algebra, with publication audiences encoded as `AudienceScopeTree`. That carrier experiment is deleted: neither verb nor the audience tree remains in `std.effect_grant`, and its only consumer `std.visibility_grant` dissolved with the Stage 0 placement gate. Publication returns only at P-C as `PublicationPolicy` over a derived `PublicProjectionPlan` consumed by the sole modeled publisher. The distinct reference-edge intent has no current consumer and returns only if one appears, through a typed request profile projected into `std.access`; it does not alter this doc's landed Read/Write/Execute grant model. → [node/subtree visibility policy history and P-C direction](docs/plans/node-subtree-visibility-grants.md)
