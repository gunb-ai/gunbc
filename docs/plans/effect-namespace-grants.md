...
**Sibling thread disposition (origin 2026-07-21; superseded by operator ruling 2026-08-01):** node/subtree visibility was prototyped as `Reference` and `Publish` verbs riding this grant algebra, with publication audiences encoded as `AudienceScopeTree`. That carrier experiment is deleted: neither verb nor the audience tree remains in `std.effect_grant`, and its only consumer `std.visibility_grant` dissolved with the Stage 0 placement gate. This retires the carrier, not publication as a required capability; no replacement publication ontology is selected pending operator design review. The distinct reference-edge question is a separate deferred capability awaiting a concrete consumer, not a consequence of the publication rollback; neither capability alters this doc's landed Read/Write/Execute grant model. → [retired node/subtree carrier experiment](docs/plans/node-subtree-visibility-grants.md)
