...
Status: DRAFT for operator review (2026-07-16, session lively-heron-615). Origin: operator observation that "hermetic" conflates several concepts and that effect scoping should reuse **namespaces directly** rather than mint a parallel "universe" vocabulary (anti-fork ruling; *universe* appears here only as the industry name for the idea being dissolved). No code lands from this doc; the FLAG-A interim (§6) is the only near-term consumer.

## 0. Displaced cost (§6 — the pain this removes)

- **`Hermetic | Wet` is a 2-valued enum carrying four concepts** (DESIGN's recurring state-space conflation). Casualties measured this week: a witness reading `/proc/self/status` had to declare `ReadsLiveTree` (never-skip) though it reads no tree — wrong label, only label available; the artifact-store witnesses write `/tmp/gunbc_*` scratch and the model cannot distinguish that from writing the repo tree or POSTing to a BMC; FLAG A (may a build run hermetically?) is undecidable *as phrased* because "inputs pinned AND outputs scoped" is inexpressible in one bit.
- **Proto-envelopes already forked**: the hand-rolled `workspace_root` containment gate (`cli_run.rs` — refuses paths outside the workspace root: a grant check, in Rust, unmodeled), `std.resources.ResourceHandle` (capability-ish carrier), `AuthScope` (cache_interface), `LiveTreeDisposition` (input-axis only, selection-eligibility only). Four partial representations of one concept (§3).
...
- **Dispatch checks the envelope fail-closed**: an effect whose target position is under no grant root for its verb → typed, located, counted refusal (`EffectOutsideGrant { verb, target, frame }`). Never a silent widen; diagnostic modes read, no mode writes "keep going" (DESIGN §5).
...
**The containment law (operator refinement, 2026-07-16 — the basic cases that keep the model honest):** a write is **frame-contained** iff (target ⊑ a namespace the frame controls) ∧ (**target lifecycle ⊑ frame lifecycle**). The second conjunct is what the old "hermetic" intuition was about, graded on the §5 construction/validation axis:

- `LifecycleByConstruction` — the target's persistence is *unwritable past the frame*: an ephemeral container's filesystem (teardown erases it, kernel-guaranteed), a network-namespaced loopback receiver that dies with the frame. The packet leaves the process, the file persists — and none of it can outlive the frame.
...
### P-C PR ledger (silent-ibex-417, updated 2026-07-22 — winding-down snapshot)

The two P-C clauses were split into two stacked PRs (the operator's "single trackable PR" steer — one reviewable slice each; slice 2 stacked on slice 1 until it merged).

- **P-C slice 1 — derive the old axes. MERGED (#6987).** `std.effect_axes`: `EffectRegion = SubstrateClosure | RepoTree | FrameScratch | UnclassifiedRegion` derived from a grant root via canonical region roots + `position_under` (never a stamped bit); `envelope_replayable` / `envelope_isolated` / `envelope_live_tree_intersects` as pure folds over the P-A `Envelope`. `v2.std.execution_envelope`: the `Hermetic`/`Wet`/`Record` presets + `derive_live_tree_disposition` bridge to `v2.std.live_tree.LiveTreeDisposition`. Two fail-closed choices pinned by REDs: an `UnclassifiedRegion` read is assumed live; a `Replay`-bound repo read is replayable-and-not-live (keeps `replayable ⟹ SubstrateInputsOnly` honest). `derived_projections_deferred_note` in `effect_grant.dag` retired.
- **P-C slice 2 — per-grant `HandlerBinding` selection. OPEN, in review (#7071).** `std.effect_grant`: `covering_grant` (returns the *covering* `Grant` so a dispatch site reads its binding) + `CoveringGrant = CoveredBy | NoCoveringGrant`; `admit_effect` re-derived as its reach projection (one walk authority). `v2.extdeps.runtimes.v2_effect_io_host`: read dispatch selects `RealTransport` vs `Replay` off the covering Read grant's binding, not `execution_mode_is_wet_dispatch` — mode is consumed once, at envelope construction. Wet envelopes carry `RealTransport` Read+Write; hermetic carries a `Replay`-bound Read grant and zero Write grants (uncovered read → located `effect_io_host_read_outside_grant`, fail-closed). Green by execution: 4 `effect_io_host_mode` dispatch witnesses + 5 `covering_grant` witnesses + 5 `effect_io_host_grant` write-admission witnesses. **Status:** 2 distinct dashboard approvals (cursor review 41448, claude review 41457), `mergeable=CLEAN`, retargeted to `main` after #6987 merged, CI running. Operator merges manually (temporary policy).
  - *Declared frontiers (operator-approved 2026-07-22):* (1) the `Replay` grant carries a `fixture_store` String but the replay handler still resolves the concrete store from the passed `EffectIoFixtureStore` object — per-grant store-string resolution is the follow-on; (2) the seed `eval_service_op` transport selection stays on per-process `ExecutionMode` on its own dissolution-triggered frontier (`v1_interpreter.rs`).

**Remaining after slice 2 (not yet started, no PR):**
- **`LiveTreeDisposition` stamp-retire** — the ~697/780 hand-stamped witness `live_tree_disposition` rows retire to `derive_live_tree_disposition(envelope)`. **BLOCKED on an operator decision**: path A (envelope-reach — build per-witness envelopes and derive) vs path B (the `v2.std.live_tree` G2 runtime-read-closure derivation its own dissolve trigger names — G2 lens landed, only G1 wired). Both viable; the two design docs disagree on the retire target.
- **Item 4 — delete `Hermetic | Wet | Record`** — retires once every reader migrates to envelopes. Gated on slice 2 landing + the stamp-retire; the presets (`v2.std.execution_envelope`) and the seed `std.execution_mode` twin delete together.
- **hermetic-native run + network isolation** — the seed intrinsic still blanket-refuses host execution in hermetic mode; relaxing it requires the network-isolation realization (netns / `--offline` / sandbox — an absent grant is not enforcement, §5). Operator chose Option 1 (fail-closed: native hermetic runs require `LifecycleByConstruction`, refuse in-container). Deferred behind a target-env trigger.
...
**The `std.effects` ↔ `v2.std.effects` de-fork is a PREREQUISITE, not a rival question.** §3 rules "grants reference shapes; verbs do NOT fork `EffectShape`" — grants stand on a *single* `EffectShape`, two exist, so unifying them runs first. It splits by axis:
...
This also dissolves the **String model↔realization fork** that blocked a naive merge (`std.effects` = kernel `String`/`Value::Str`; the intern bridge needs `v2.std.text.String = FreeMonoid<Char>`): grounding key/target on the containment tree, staged, is where it resolves — never a crossing minted at the effects seam.
...
**Sibling thread disposition (origin 2026-07-21; superseded by operator ruling 2026-08-01):** node/subtree visibility was prototyped as `Reference` and `Publish` verbs riding this grant algebra, with publication audiences encoded as `AudienceScopeTree`. That carrier experiment is deleted: neither verb nor the audience tree remains in `std.effect_grant`, and its only consumer `std.visibility_grant` dissolved with the Stage 0 placement gate. This retires the carrier, not publication as a required capability; no replacement publication ontology is selected pending operator design review. The reference-edge question is a separate deferred capability awaiting a concrete consumer, not a consequence of the publication rollback; neither capability alters this doc's landed Read/Write/Execute grant model. → [retired node/subtree carrier experiment](node-subtree-visibility-grants.md)
