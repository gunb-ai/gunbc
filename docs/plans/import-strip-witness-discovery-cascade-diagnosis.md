...
This documents the mechanism the closure misses and what it must additionally cover
before the strip can re-run.
...
without being called or annotated. Every #6985 restore whose stated cause names a
bare symbol (not a compile *error* on the file itself) reduces to this one gap.
...
- **`cost.dag` / `enforcement/lens_module_gate/compile_gate.dag` / `machine_shape.dag`**
  ...
  NoConnective` branch; these restores suggest one more sub-case where the
  referenced name's `global_bare` binding is not the type's own Disj/Conj node but
  an intermediate re-export/alias binding whose `connective` reads `NoConnective`
  though the underlying reference is genuinely a type edge — needs confirming
  against the actual `TypeBinding.resolved` shape for an aliased type before
  generalizing (flagged open below).
...
local/irrelevant" assumption for the shapes proven above (nullary variant tag,
zero-param fn/data referenced by value). Two closures, in order of preference per
DESIGN §5 (construction over validation):
...
1. **Positive signal, not absence-of-signal.** Tag the census binding itself at
   ...
   `global_bare` today only holds declaration-grain entries (per
   `global_bare_fallback_invariant`), so absence of params/type_annotation/
   connective is not evidence of "don't pull" — it is evidence that "this decl
   shape doesn't need those fields," which `pullable()` misreads as "not a real
   reference." The construction fix: `pullable()` defaults to **true** for anything
   `global_bare`/`services` resolves to (a declaration, by the census's own
   invariant), carving out the *rare* named exception (a name body-bound in the
   *same* scope this file already special-cases via `candidates.bound`) rather
   than allowlisting shapes.
...
reference — the files that did were exactly the ones repeatedly restored to
import-bearing. The closure's own regression tests
(`bare_identifier_candidates_tests`, `cli_run.rs:4380-4425`) cover only the
bound-vs-referenced boundary (arrow-lambda params, pattern-value leaves), not the
`pullable()` arity gate targeted here — so the gap has no discriminating RED today.
...
execution"), §1's headline claim was tested by execution. Result: the claim as
stated is **wrong**.
...
`pullable()` DID pull it. `merge_global_bare_variant_locals`
...
checked at `:15879`), not to a connective-less binding for the tag in isolation.
So `pullable()` reading `binding.resolved.connective` reads the parent Disj's
`connective`, `Disj` — satisfying the fourth branch (`connective != NoConnective`)
directly. §1's claim that a nullary variant tag's resolved binding carries
`connective == NoConnective` is **false**: the census never binds a variant name to
a connective-less standalone binding; it binds it to its coproduct owner.
...
- The `DataDependent`/`EffectCoupled` evidence in §2 (the `parallelism.dag`
  ...
  already pullable; #6848's closure covers this shape correctly.
- The zero-param-fn/data-referenced-by-value evidence in §2 (`table_fixture_*`,
  ...
  rescue as a variant tag does (a fn has no "owning coproduct"), so this class
  needs its own discriminating repro before it can be trusted either way.
- The actual cause of the `parallelism.dag` restore is more likely the shape
  ...
  failure of a different shape: `name 'DataDependent' not found in module
  'v2.lens.parallelism'` — a broken qualified-import/re-export reference, not a
  bare-closure miss. A distinct bug class from §1's `pullable()` claim, likely
  an artifact of the batch strip running file-by-file with some files landing
  before their sibling consumers, not a closure predicate gap.

**Verdict:** §1–§4 are **not confirmed** as the mechanism behind the
`parallelism.dag` restore and must not scope a fix. The fn/data zero-arity claim
remains open pending its own execution proof. No fix should be attempted from this
document as written; the next pass needs a fresh, execution-first investigation
starting from the qualified-import/re-export shape above.

## 6. Controlled-pair + retrodiction (namespace-resolution-design.md §8 PR-4 lead)

Per nimble-owl-658's follow-up, §5's re-export lead has a name in the design: §8
"import-from-definer migration" (`namespace-resolution-design.md`, PR-4) — the
corpus deliberately relies on re-export transitivity today, and PR-4 is the
staged step migrating every import to name its true definer. Two discriminating
predictions were run by execution, plus one retrodiction against a real #6985
restore commit:
...
```
Then also stripping the two importers' `import v2.lens.parallelism { DataDependent,
EffectCoupled, ... }` lines (making the references bare, matching the real
batch-strip's simultaneous-file shape) returns to green. This confirms
...
does **not** fit (1)'s re-export-mismatch shape as-is; these files' *own* bare
references were failing, on their own imports. Repro:
...
they resolve some other way (or fail silently upstream and surface only as a
downstream type cascade). A **second, distinct failure class**, not yet
root-caused, explained by neither the refuted §1 `pullable()` claim nor the (1)
re-export-mismatch shape.
...
- **Class A — partial strip through a re-export chain** (parallelism.dag):
  ...
  (import-from-definer migration) closes it structurally — once every import
  names its true definer, no re-export chain remains to break when a file is
  stripped independently of its "hub."
- **Class B — hub file's own bare references, non-"not found" failure shape**
  ...
  nor explicitly refused; they cascade into a generic type-error. Needs its own
  investigation (start from where `Determinism`/`Deterministic` resolve today
  with the import present, and what differs with it absent) before a wave rule
...
covered by any rule** — its mechanism is open, so a Class-A-only wave rule is
still un-derived-from-execution risk for any wave touching
`determinism.dag`-shaped hub files. The zero-arity fn/data-by-value claim from
the original (refuted) diagnosis also remains untested.
...
`src/v2/lens/determinism.dag` declares no local copy, and every use of these four
names in that file is fully **qualified** (`std.determinism.X`), never bare — so
no bare-name lookup on them occurs, and the LCP-homonym mechanism has nothing to
fire on.
...
exactly one declaration site (no homonym), yet never appears in
`GUNBC_BARE_PULL_TRACE`'s output — not attempted, not refused, silently absent.
Consistent with type-annotation/field-type references resolving through the
typecheck/infer env path rather than `extend_with_bare_reference_closure`'s
loader-side pull loop, so that path never triggers a module pull for
`DeclarationRef` and the ungrounded field type cascades into the
`NonDeterministic { source: ... }` construction site as a generic type-error
rather than a loud refusal — not yet traced to a specific infer/typecheck line,
so a lead, not a confirmed mechanism.
...
by a finer-grained trace. Two temporary `eprintln!` probes in a throwaway scratch
build (never committed) at the two decision points inside
...
`target_module` value immediately after `resolve_in`, before the `known_paths`
short-circuit (`[bare-resolve]`). Running
...
`bare_identifier_candidates` (the KEY-POSITION, binder-keyword, fn-param, and
pattern-depth BOUND rules do not apply to a bare type in field-type position),
IS in `all_names`, and `resolve_in` DOES succeed on it (`pullable()` correctly
fires on the `Conj` connective of the `type DeclarationRef { ... }` record
declaration, confirmed by `Connective`'s definition at `v1_std_core.rs:110-115`
and its Conj/Disj assignment sites in `v1_compiler_infer.rs`). No `[bare-pull]`
line printed in §7.5's trace because the `known_paths.contains(&dep_rel)` check
(`cli_run.rs:4668-4670`) fires *before* the trace print — `dag/std/decl_ref.dag`
was **already** in `known_paths` every time, so the pull is a silent no-op, not a
silent failure.

Why it is already known: `dag/std/determinism.dag` (never stripped — only the
*consumer* `src/v2/lens/determinism.dag` and its test were) itself carries
`import std.decl_ref { DeclarationRef }` as a genuine, unstripped import edge,
and the entry test file directly imports `std.determinism` (also unstripped). So
`dag/std/decl_ref.dag` reaches the LOADER-tier file set through an ordinary,
wholly unrelated transitive import chain — the bare-reference closure never does
any work for this name, and its apparent "success" (`resolve_in` returning
`Some`) is coincidental, not load-bearing.

**This is the actual defect, and a different mechanism from both of
nimble-owl-658's B2 sub-hypotheses**: `extend_with_bare_reference_closure`
governs only the **LOADER tier** — which files get parsed and compiled into the
corpus (`cli_run.rs:3402-3419`'s own comment: "for the LOADER an over-connected
edge is harmless... The loader (`extend_with_bare_reference_closure`) is
deliberately left alone"). It never touches the **per-file typecheck/infer
environment** that resolves a bare name used *inside* `src/v2/lens/
determinism.dag`'s own body — built from *that file's own `import` statements*,
which the strip deleted. Loading `dag/std/decl_ref.dag` into the corpus (so it
compiles standalone and any file legitimately importing it can resolve names
against it) is orthogonal to whether `src/v2/lens/determinism.dag`, imports
gone, has a local binding for the bare name `DeclarationRef` in its own scope.
It does not, so the field type `source: DeclarationRef` on `DeterminismRoot` is
ungrounded at typecheck/infer time regardless of `dag/std/decl_ref.dag`'s
presence elsewhere in the loaded corpus — producing the observed generic "error
type cascade" at construction time (`determinism_root_map_keys`'s use of
...
original diagnosis and B2a fix location both assumed (patch
`bare_identifier_candidates`/`pullable()`, i.e. the LOADER) would do
**nothing** for this failure — the loader already succeeds. The actual gap:
`extend_with_bare_reference_closure` has no counterpart extending the
referencing file's own typecheck-time import/name-binding scope to the bare
references it resolved — resolving a name to a module at load time and admitting
that name into the referencing file's local env are two disconnected mechanisms,
and only the first is patched. Any fix must add or extend the second (env-side)
mechanism, not the loader; this is the `src/v2/lens/determinism.dag` shape's
manifestation of the broader import-from-definer gap PR-4 targets, but PR-4 as
scoped (a loader/edge-naming fix) does not by itself cover it — env construction
is a distinct consumer that must also be re-pointed.
...
141-line source file, so the number is an offset into a merged/compiled
...
`src/v2/lens/determinism.dag`'s own typecheck env, the field-type error is not
raised as a located, typed refusal on `DeterminismRoot.source` (DESIGN §5 —
"every path succeeds fully or fails with a typed, located diagnostic") but
propagates as an opaque `ExprError` compounding through every downstream
construction site touching the malformed type, arriving at the runtime boundary
as a generic, unlocatable "error type cascade." This absorption is *why* Class B
stayed invisible in the original small-residual landing (§4) and why B1/B2's
early framing ("never appears in the trace... silently absent," §7.5) was
plausible but wrong — the loudness gap hides the true failure site well enough
that even code-reading the closure logic pointed at the wrong mechanism. A
located fix (a typed "unresolved type: `DeclarationRef` in
`src/v2/lens/determinism.dag`, field `DeterminismRoot.source`" refusal at the
point the env lookup fails, instead of a generic downstream `ExprError`) is a
prerequisite for diagnosing any *future* instance of this env-side gap by
inspection rather than by scratch `eprintln!` probes, as was needed here.
...
*already* import-stripped on `main` and typecheck green, despite bare
cross-module references in field-type position exactly like
`DeterminismRoot.source: DeclarationRef`. Under §9's "the referencing file's
own typecheck env is built strictly from its own imports, and the loader
closure never feeds it" theory, they should fail the same way. They don't. A
second execution-tested probe was required before the diagnosis could close.
...
position). Fresh scratch worktree at `main` tip with the same two
`[bare-attempt]`/`[bare-resolve]` trace probes used for §9, and a new,
narrowly-scoped entry file `test.claim.scratch_bmc_types_probe` importing only
`RedfishProcessorSummary` from `extdeps.bmc.types` — structured to mirror
`DeterminismRoot`'s field-type bare-reference shape as closely as possible, run
via the same scoped `gunbc run --entry ... --claim-run --dry-run` path (not the
confounded whole-tree `gunbc compile`).
...
discriminating data nimble-owl-658 asked for, and it answers both questions at
once — **the gap is neither positional nor shape-dependent; it is a third factor,
pool-membership coincidence, the same mechanism §9 found for `std.decl_ref`,
narrower here because the probe's closure is smaller than the full repo:**
...
disconnected mechanisms" framing.** There is one mechanism: a module's own bare
cross-references resolve exactly when the *target* module has, by that point in
the fixpoint, been loaded into the shared compilation pool via *some* import
edge — stripped or not, related or not, anywhere in the currently assembled
transitive closure. `std.decl_ref` in the §9 determinism.dag repro and
`extdeps.uri`/`std.types` here succeed for the identical reason: an unrelated
unstripped import happens to drag the target module in first. `std.measure`
fails here for the identical *absence* of that coincidence. **Batch-1's ~74
files typecheck green on `main` not because of a different, working mechanism —
they get lucky at whole-repo scale**: enough files elsewhere still carry
unstripped imports of `std.measure`/`std.types`/etc. that those modules are
reliably in the pool by the time any batch-1 file's own bare references are
resolved. A probe closure this narrow lacks that ambient coverage and exposes
the gap directly. This is the "absorbing fallback [that] destroys the only
signal" pattern DESIGN §5 names, realized structurally rather than as a designed
fallback: the corpus's redundant import density is silently doing load-bearing
work, and nothing tracks that it is happening. **The wave rule is therefore
corpus-wide and load-bearing, not per-file:** any further import-strip batch's
safety depends on whether the *specific modules* its bare references target
remain reachable via *some other* unstripped import somewhere in the compiled
closure at strip time — a property that can silently break as *later,
unrelated* strips remove the last such incidental import, with no local signal
at the newly-broken file.
...
--claim-run` path used for the original determinism.dag repro, with exit code 0
(not the opaque `error type cascade at ...:1983-1984` seen there). So the
opaque cascade is not a general property of the `--claim-run` path, as §10
implied — it is specific to whatever `determinism.dag`'s failing construction
shape triggers (most likely still §10's inert `ExprError` arm, but only for
some field-type failure shapes). §10's fix is still worth landing, but its scope
must be re-verified against both failure shapes before being called complete,
not assumed general from the single determinism.dag repro.
...
- **LOUDNESS defect** — named side finding, narrower than first described
  ...
  failure shape); still worth a located-diagnostic fix for whatever triggers
  it in `determinism.dag`, but re-scope before building it.
...
mechanism, the LOUDNESS diagnostic, PR-4, the zero-arity test) go to fresh work
items for the operator to sequence — none is authorized to land from this
investigation.
...
Class B was re-run against the flipped tree. Two things changed; the second is
worse than anything §13 records.
...
signature. Second control: adding one *irrelevant* import (a sibling module not
reaching `std.types`) does not help — not a "module has zero imports"
degeneracy but import-chain reachability of the name itself.

### 14.2 Finding 1 — the qualified spelling buys no binding

Row 5 is load-bearing. `std.types.ContentHash` written in full, in type
position, resolves no better than the bare name. **There is no
containment-based binding path for cross-module type references today.** The
flip gave containment binding to values/functions (row-4 control), not to types.

This falsifies the premise Dispatch 2 was cleared on — that with binding by
containment, deletion no longer risks losing a resolution. True for values, not
for types. `namespace-resolution-design.md`'s Rule-1 end-state (delete the
`import` grammar, derive deps from `container.member` references) is therefore
**not reachable for type references** on the current resolver, in either
spelling.

### 14.3 Finding 2 — the failure arm widens instead of refusing (§5 fail-open)

Compare rows 3 and 4. Name **absent** from the pool: the resolver refuses
correctly, a typed, located `unresolved type 'ContentHash'`. Name **present in
the pool but not import-reachable**: it does not refuse — it fabricates an
anonymous product, `Product(<anon>)`, which then unifies with everything
downstream.
...
**Argument-position checking does not catch it.** What caught it in `node.dag`
was the accident that six `if`-expressions juxtaposed a fabricated branch
against a correctly-typed one. The two `if`s in the same file whose branches
are *both* fabricated
...
§13's "never assumed safe from 'it typechecks today'" understates the risk for
types: for types, typechecking today is not merely an accidental property of
the rest of the corpus — it is not even a signal.
...
Ordering follows §5 (construction before validation; a failure arm must refuse,
not widen): the **fail-open is the first fix**, independent of whether the strip
ever resumes — pool-present-but-not-import-reachable must become a typed,
located, counted refusal. It is also what makes a binding fix *verifiable*:
until the arm refuses, no strip's green is evidence. A containment binding path
for type references is second; only then is Dispatch 2 mechanical again.
...
establish, E and F remain unavailable." **That is withdrawn.** It came from
roadmap prose ahead of its own executable closing authority. The authority:
`gunbc.namespace_reference_derived_closure_contract`
...
arms: the ordinary front-end must emit the occurrence/declaration identities it
currently produces only from hand-authored fixture strings, the production
type-reference changeover must thread the binding context into the real resolve
path, the loader must consume the dependency projection instead of scanning
imports and bare-name census guesses, and pool independence must be proven
against the loader's own output rather than independently assembled fixture
closures.

### 15.1 What was run

Release `gunbc` built from `1eadad4af25`; no `.rs` changed between that commit
and the measured base, so the same binary is current for the measured tree.
(Rebuilt for this lane because #7924 changed explicit-import resolution after
the previous strip attempt — a binary older than the substrate it judges
reports a compiler that no longer exists.) Two scratch copies of `dag` + `src`;
one intact as the known-positive control, one stripped with a
**brace-depth-aware** pass (`sed '/^import /d'` orphans the multi-line
`import x { … }` blocks and inflates the reading roughly tenfold — the trap
recorded in §5's repro notes).
...
**It is a record, not a reproducer; the numbers below cannot be regenerated from
a clean checkout.** The Python harness that produced them was removed by
operator ruling — EARLY, before its declared dissolution trigger (B2 landing
`OrdinaryLoadedCompilationClosure` production, at which point the ledger becomes
a projection of the loader's accepted-binding output) had fired. That trigger
has NOT fired; the scripts went anyway, trading re-runnability for not carrying
a parallel text-scanning classifier in the tree.

Without the harness a reader can still check the corpus-hash anchor, which
decides whether these receipts describe the tree in front of them; re-deriving
them requires rebuilding the harness from the recorded command sequence.
...
positive" is this document's term, not DESIGN's; DESIGN §5 requires a real
consumer **green by execution** plus a discriminating input that goes *red*
when the behavior is wrong, and the stripped/unstripped pair supplies both
halves here.)
...
which measured 5,815 attributable at `1eadad4af25`, before #8056 landed the
`cell`/`row` disambiguation; the current base gives 3,050. Reading the two as a
trend would be a change detector — strip set, corpus and base all differ. For
the same reason the earlier 1,096 figure from the pool-pull receipt (1,447 files
stripped) is comparable to neither.

### 15.3 The residual, by disposition

Every failing name is joined against a corpus declaration index, so the class
derives from declaration multiplicity and first-failure shape, not diagnostic
prose. **No disposition claims a cause that was not observed**; the ones that
would carry `_unobserved` in the name:
...
`provider_in_loaded_closure = unobserved`; both cannot stand. What is
established: one indexed declaration, plus a stripped compile reporting an
unresolved reference. That is consistent with the provider never entering the
closure; the provider entering but the occurrence not being admitted into the
referencing environment; a wrong occurrence category taking a legacy path; a
fail-open/fabrication arm rewriting the downstream error; and the index matching
a plausible declaration that is not the intended binding. Promotion to clause E
requires loader instrumentation observing the accepted declaration and whether
its provider entered the closure. Until then the disposition names its own
ignorance.
...
load-bearing — a lead of exactly that strength: strong, not yet a measured seam.

**`variant_mechanism_unobserved` carries a re-proof obligation, not a feature.**
An earlier revision called these 599 rows "the arity-zero/variant-tag
...
available. Likely: most collapse into the provider-closure problem, some are
missing owner-type bindings, some genuine zero-arg by-value gaps, some wrong
candidate assignments from same-leaf matching.
...
previous revision put 505 rows in one `no_declaration_found` bucket, all
"the index needs widening." Some are (`ReadsLiveTree`, `Do`, `FailFast`,
`Empty` are inline variant tags; `trim` is a primitive — the same name #8062 is
making import-explicit). Many are not: `count`, `first`, `split` and `ends_with`
are method resolutions on receivers that never resolved, and `steps`,
`argv_input_refs`, `content_identity`, `subject_identity`, `kind` and
`qualified_name` are field failures downstream of a missing parent type —
symptoms of a missing provider, not separate index gaps, and the six
...
diagnostic mentions `Product(<anon>)`, so those specimens are greppable, not
buried in the cascade bucket.
...
The mixed buckets above prove it: one missing `EffectPlan`,
`PublicationSubject` or `DeclFact` surfaces as many independent method, field
and record-shape failures, each its own row. The ledger carries
`downstream_diagnostic_count` per (consumer, name) pair — a partial grouping
only; it does not link a field failure to the *provider* whose absence caused
it. Causal grouping (root occurrence → root provider/binding result → dependent
diagnostics) needs the same loader instrumentation clause E/F needs, so it lands
with that vertical, not before it.
...
repair and re-measures on the same head, so ledger and fix no longer disagree
inside one PR (an earlier revision's receipts pinned a pre-repair commit while
the diff consolidated the very forks the ledger listed as open — caught in
review 50740).
...
the ledger. Four named kinds of row replace it:
...
thresholds — and an earlier revision of this PR got that wrong where it matters
most.** It classified a convention by population size (ten or more
declarations) and an intentional collision by path shape (every candidate module
containing `.fixture.`), reasoning that a generalizing rule beats a maintained
list. Both rules fail OPEN, *into the number this section asserts at zero*: a
genuine accidental fork reaching ten sites would be reclassified as a convention
and vanish from the hygiene count, and an unrelated duplicate between two
fixture modules would be excused by its path. A rule whose failure mode is
"quietly removes rows from the figure under test" is the absorbing fallback
DESIGN §5 forbids, aimed at this measurement's one load-bearing claim (review
50775).

Each subject is now named, and its defining property checked against the tree:
a convention must be one declaration per module under a declared module prefix,
and an intentional ambiguity must match an exact pinned module set. A subject
that stops satisfying its property loses its disposition and lands in
`duplicate_unclassified` — loud, counted, not zero. Six controls held this in
both directions when the classification was produced (the harness is
deliberately untracked — see `import-strip-measurement/README.md`); the two
that matter are the ones the retired rules would have failed: a planted
ten-declaration accidental fork stays hygiene, and an unknown duplicate across
two fixture modules stays hygiene.
...
**Two families were renamed on BOTH sides because their bodies diverge** — a
finding, not a cleanup: `runner_slot_unit_name` and `RunnerReplacementCause`
exist twice with different behaviour — two systemd unit-name constructions and
two cause vocabularies — and `current_walk_attempt_id` differs in its
absent-variable arm (`none` vs the empty string). `gunbc.runner_lifecycle`
carries `Disposition = SingleAuthority`, so it keeps the names and
`gunbc.runner_connectivity_recovery`'s copies are prefixed. **Renaming makes the
fork visible; it does not fix it.** Two functions still compute a unit name
differently — a latent defect this section records, not resolves.

**Per-spec tokens stay with their spec.** `extdeps.w3c.accelerometer` keeps
`accelerometer_permission_name`, its own spec's token; the DeviceMotion copy
becomes `device_motion_accelerometer_permission_name`. Each upstream spec module
owns its own facts, so a shared literal is not consolidated across specs.
...
type `Repository`" errors — **pool coincidence reproduced on the unstripped
tree**, the same mechanism as the residual's largest class, visible in ordinary
compilation. A second came from the measurement itself: the first regeneration
reported `corpus_hygiene: 2`, a rename of mine colliding with an existing
`import_resolution_facts_live` in `v2.lens.module_graph`. It was named for its
return type when the distinguishing fact is its source, and is now
`reference_derived_import_resolution_facts_live`. The ledger caught a defect its
own author introduced — the argument for running it rather than predicting it.
...
`import_resolution_facts_live` — the producer already feeding `import_edges:`.
Bare-reference dependency edges were silently dropped from the module graph,
which feeds affected-set selection, import closure and compile-clean scope. Two
prose rows were rewritten to claim two import sources as well.

Repaired (`reference_edges:` now calls
`reference_derived_import_resolution_facts_live`, and both prose rows differ from
main by exactly the rename); the interesting part is why nothing caught it.
...
is type-correct; the whole-tree compile stayed green through the regression.
And the corpus has **no witness referencing `dependency_resolution_facts_live`
or `union_import_resolution_fact_lists` at all** — the two-source union has no
discriminating control, so a wrong producer at that seam is invisible to the
executing corpus. External review caught what no check does.

The gap is recorded, not closed: a real control needs a fixture where a module
is reachable by bare reference but NOT by import, so the two arms provably
disagree — the same fixture shape the ordinary-loader vertical needs, so it
belongs there rather than bolted onto a naming PR. Filed as an obligation, not a
plan.
...
`corpus_hygiene` is **absent from this table, at zero** — the close this section
reports. So is `duplicate_unclassified` — where a named convention or fixture
subject falls when its naming property stops holding — so the two remaining
excusing dispositions above absorbed nothing: each was checked against the tree,
and both held.

Three numbers moved for reasons other than this repair; saying so is the
difference between a measurement and a scoreboard. The unstripped control
FELL 22 → 10 because main landed #8072, healing part of the annotation-grain
class; all remaining 10 sit in
`dag/test/claim/host_phase_status_witness_test.dag`, a file this branch does not
touch, so the hygiene batch still adds **zero** diagnostics — this time evidenced
by the located diagnostics themselves, not a separate run. The corpus grew again
(16,388 imports across 2,583 files) because main advanced during the work. And
`unique_decl_unresolved_mechanism_unobserved` moved with that advance, not the
renames: 1,951 here against 1,656 before main moved.

The re-measurement is not free of the effect it measures: these figures come
from a tree including this branch's own four fixture modules and one new witness.
Their contribution is bounded by the control above — zero diagnostics unstripped
— not assumed.
...
of its two-source union (review 50749). The producers share a signature, so the
compile stayed green and the function kept returning well-formed edges — it
simply stopped carrying every dependency only a bare reference establishes,
precisely this lane's edge class. Nothing executed that seam. It does now:
`test.claim.module_graph_edge_source_witness` points the production function at
a four-module fixture and asserts the union at identity grain in both directions;
restoring the doubled arm reds two of its five witnesses. The fixture's shape is
derived from the seam, not invented — `reference_resolution_facts` emits nothing
for a file still carrying imports, so the two producers are disjoint by file, the
reference-only dependency must live in an import-less consumer, and a same-path
duplicate is unconstructible through this seam; the witness says so rather than
asserting a dedup that cannot arise.

**This does not close import deletion, and nothing here is progress toward it.**
The remaining residual is not a naming tail; it is one integration vertical:
...
What the close buys: **no one can point at a miscellaneous naming tail to
justify a workaround**. The corpus excuse is gone; §15.0's maturity matrix
remains, with the permanent closing contract still reporting all six
capabilities unavailable.
### 15.8 One ordering property of the fork half

Consolidating a fork today requires **adding** an import to the module that
loses its local copy — the very edge this lane deletes. An ordering artifact
(a corpus-unique name resolves bare once the ordinary-path changeover lands),
but it means fork consolidation must not be scored as import growth. The same
caveat applies to #8058 and #8062, which correctly add explicit imports to close
fail-open pool reliance under *today's* compiler: interim containment, needing a
dissolution trigger tied to the production changeover, or the temporary
mechanism hardens into the concept this lane exists to delete.
