...
| 13 | translate double walk | 06_translate.dag:3833–3860,3724 | `fold_node` raw pre-pass, then full `translate_node_projection_absent_fold` | 2× full O(M) walks (+#12 per node) | raw result thrown away if relation-row miss | **SUSPICIOUS** | fuse the relation-row check into one pass |
