...
**Instrument:** `gunbc test`-reachable claims in `v2.test.claim.coercion.two_algebra_fork` — `name_keyed_coercion_matches_the_declared_cast_rosters_holds`, `homomorphism_coercion_matches_the_structural_preservation_contract_holds`, `homomorphism_coercion_admits_only_identity_quality_holds`, `the_two_coercion_algebras_agree_exactly_where_declared_holds`. The last pins the agreement relation pair by pair and reds in EITHER direction, so consolidating the fork reds it too and the consolidator must restate the row. Two mutation arms establish it discriminates rather than detects change: adding a `Bool→Float` row to `dag_cast_rules` reds the roster claim and the join while leaving the homomorphism claims green, and weakening the structural-preservation predicate reds the homomorphism claims and the join while leaving the roster claim green.
