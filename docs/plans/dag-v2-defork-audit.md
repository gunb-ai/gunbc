...
**§2D, the shared type-name census, is derived from the live tree at generation and is projected at the end of this document** (`gunbc.defork_type_name_census`). It supersedes the per-basename `shared type-names` column of §2A; no second list exists.
...
**This audit classified `coercion` as a genuine not-a-fork on a shared *symbol* count of 0. Measured by execution, that count answers the wrong question.** The two modules share no symbol and denote no different concept: they answer ONE question — may this value cross from one model to another — and they answer it with **two admission authorities keyed on different identity relations**. `std.coercion` `dag_can_cast` folds over `dag_cast_rules` + `grounded_primitive_coproduct_identities`, keyed on type-name **STRINGS**, and it EXECUTES (`v1.compiler.infer` consults it at three decision points -- `kernel_value_declared_type_mismatch_bounded`, `application_type_names_compatible` and `validate_cast` (explicit `as`)). `v2.std.coercion` `coercion_fold` asks `v2.std.find_witness` `find_witness` for a structure-preserving witness, keyed on node **IDENTITY**, and has **no established executing consumer**. A shared-symbol count cannot see this, because the two sides cannot share a symbol: they cannot share a KEY.
...
**EXECUTED RESULT over seven supplied source/target pairs — the admission sets differ in BOTH directions, so neither side is merely the stricter one.** `coercion_fold`'s only realized preservation rule is `preservation_rule_exact_structural_equality_zip_fold`, whose predicate holds exactly when the candidate is structurally equal to the source, so the homomorphism side cannot SPELL a non-identity crossing at all. **Admitted name-only:** `Int→Float`, `Int→Nat` — and `Bool→Int` at #12055, until #12227 withdrew its row (a `Bool` is not an `Int`), after which both sides refuse it and the fixture keeps it as the control that the withdrawal holds. **Admitted homomorphism-only:** `String→String` — an IDENTITY crossing, which `dag_cast_rules` carries no row for. That cell is a property of the RAW HELPER roster, not a production refusal: every live composition in `v1.compiler.infer` admits name identity before it consults `dag_cast_rules`. Both sides matched their OWN declared contract on all seven pairs (the declared rosters for one, the structural-preservation predicate for the other), so this is a fork between two correct-by-their-own-lights authorities and not a defect in either.
...
| `Float` | `dag/std/float.dag`, `src/v2/std/float.dag` | `= Float64` → `std.float` `Float64` | `= Binary64` → `v2.std.float` `Binary64` (was `= Float64`, byte-identical text, different referent) | resolution-divergent (text was identical) | AWAITING OPERATOR RULING — seed kernel name — out of scope for the wave |
| `String` | `dag/std/string_type.dag`, `src/v2/std/text.dag` | `= FreeMonoid<Char>` → `std.algebra` `FreeMonoid`, `std.types` `Char` | same referents | one concept, two declarations (duplicate) | AWAITING OPERATOR RULING — seed kernel name — out of scope for the wave |
...
| `Nat` | `dag/std/nat.dag`, `src/v2/std/nat.dag` | `= CommutativeSemiring<Magnitude>` | `= Zero \| Succ` (Peano coproduct) | same concept, divergent model | NEEDS A DECISION RECORD (integer family root) |
...
| `AlgebraCarrier` | v2 side deleted — `v2.std.rust_leaf_model_claim` type and datum were never read; `std.algebra` `AlgebraProfile` `OrderedRingProfile` already names that structure | #12252 |
| `ApproximateField` | v2 side deleted — `v2.std.algebra` ops record had zero consumers; ops come from inhabitance (DESIGN §4) | #12252 |
| `EffectRequest` | v2 side renamed `OperationCall` — Plotkin–Pretnar, *Handling Algebraic Effects* (LMCS 2013): `op(v; y.c)` is an operation call | #12252 |
| `Float32`, `Float64` | v2 side renamed `Binary32 / Binary64` — IEEE 754-2019 §3.6 interchange-format names | #12252 |
| `RankingDimension` | v2 side renamed `RankingComponent` — Ben-Amram & Genaim, *Ranking Functions for Linear-Constraint Loops* (JACM 2014): a lexicographic ranking function's components | #12252 |
| `TestClaim` | dag side renamed `AssertionClaim` — a name collision resolved by renaming apart, not a grounding merge | #5640 |
