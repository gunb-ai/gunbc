...
**Status:** operator-directed design seed, revised 2026-07-27, superseding the 2026-07-25 premise
that “merge = keyed diff over node identity.” It keeps the signed visibility-first sequence and the
landed economics corpus, but treats nodes, files, branches, and three-way comparison as possible
evidence or realizations, not the foundation.

“Source-intent integration” is a descriptive name, not a proposed substrate type. No carrier lands
from this document alone; the first step is concept-DFS in `std/` plus a discriminating scenario
corpus, and the plan must not mint an SCM-local nickname for a concept the substrate already owns.
...
product-strategy record, not published in this repo (moved to the private overlay root,
2026-08-01). That strategy cannot weaken this document's semantic, evidence, admission, or
compatibility-fidelity contract; this document cannot promote a product stage merely because its
technical carrier exists. Git is the first adoption realization, not the interface the native model
derives from.

## 0. User contract — SCM-compatible, not SCM-shaped

The proof machinery below is an implementation contract, not product vocabulary: a developer need
not understand candidate closure, evidence relations, patch transport, linearizability, three-way
merge, or which compatibility realization is active. The native workflow is:

> **edit → submit → Landed**

Synchronization, replay against the current accepted target, integration, validation, receipt
reuse, atomic advance, and compatibility projection are system responsibilities. A normal user never
chooses merge vs rebase, maintains branch/bookmark/channel topology, edits conflict markers, or
interprets proof/evidence terminology.

The semantic integration boundary has only two semantic handoffs:
...
`Choice required` means more than one materially different requested meaning remains, or two
explicit requested outcomes cannot both be preserved. It asks exactly one localized question in
domain language, with concrete alternatives and result previews, and keeps enough continuation
state that one answer resumes the operation automatically. It never asks the user to perform
integration mechanics.

Missing observations, stale parents, incomplete models, retryable failures, and projection repair
are internal work: the system exhausts its declared mechanical observations and bounded proof
procedures before interrupting. A machine-answerable issue stays in the machine; missing
information becomes a user question only when it is genuinely a preference no observation can
answer. The precise receipt and mechanics stay available through an explicit inspect/debug surface.

A third **non-semantic terminal outcome** is required for fail-closed honesty:
...
only after every permitted retry, observation, repair, approval route, or other machine-owned
continuation is exhausted or ruled out. Internally it keeps a typed, located, counted refusal and an
audience-authorized receipt reference; the primary message gives the plain domain reason, who or
what owns any real next action, and confirms no transition committed. Permanent authorization
denial, policy refusal, unsupported proof bounds, and non-retryable effect failure therefore cannot
vanish into an inspect-only state. Non-terminal progress may show as status; it is not a semantic
conclusion or a preference request.

The first compatibility surface may look like an ordinary Git worktree accepting ordinary commits
or PRs, but Git is one import/export and target realization, not the native interaction model.
Submit imports the pending delta/proposal, replays it against the current accepted target, and
emits the target medium's ordinary representation on landing. The initial product emits an ordinary
one-parent/squash Git commit; Mercurial and Pijul are modeled up front as design-stress
realizations, not silently promised as product-grade adapters. Native SCM clients and expert
plumbing remain available; none is the conceptual front door.
...
presentation projection. Internal state is retained and inspectable; its terminology does not leak
into the default UI merely because it exists.
...
Humans and LLMs are expensive, lossy judgment providers and should receive only the irreducible
normative choices. Everything else—alignment, replay, independence, invariant checking,
affected-set calculation, receipt reuse, projection—is mechanical.

The objective is **lexicographic**, not a scalar score a sufficiently cheap unsafe answer can win:
...
developer, 10 proposals touching the same modeled region, a 30-minute CI path. Each proposal
invalidated after going green spends at least another affected 30-minute validation interval;
serial invalidations add wall-clock queue delay. The first model carries these as workload inputs
so the benefit is measured, not asserted.

A lower textual-conflict count is not success: an aggressive integrator improves it by silently
choosing wrong results. The product metric is **judgment displaced at a fixed safety contract**;
safety is a prerequisite, not a finite cost. Even when a batch cannot commit, the system returns
every independently grounded assessment; a top-level unknown must not erase safe partial deductions.

## 2. Separate the layers

Every SCM has useful native concepts: Git commits/trees/refs, Mercurial
changesets/manifests/bookmarks/phases, Pijul changes/dependencies/channels/pristine state. These are
upstream storage, history, publication, and compatibility concepts; none is the program or supplies
the native integration interface's vocabulary.
...
A path move, import deletion, formatting pass, or source reorder may thus be a semantic no-op
**under a declared language model and equivalence witness** though its Git tree changes; conversely,
two disjoint textual hunks may jointly violate one modeled invariant.
...
relation. That is the content behind the working shorthand `Group<U>`; this plan must not pretend
the shorthand has landed or mint a competing generic group type for convenience.

Module membership is containment-derived; `GroupMembership.ByContainment` classifies that
membership's provenance. The namespace/containment authority still supplies the actual member
relation and distinguishes immediate children from descendant closure. An affected closure is
derivation-backed membership; a temporarily hand-curated migration set is frontier membership.
...
Identity is involved only as detective evidence: a key aligns two facts through time; it does not
decide their joint result. No universal durable `ScmEntityId` is assumed. Containment position,
declaration reference, content identity, normalization, bounded extensional identity, provenance,
and an authored transformation each supply different grades of evidence. Unknown alignment stays
unknown.
...
intent in the orchestration lane, `ChangeSet`, temporal snapshots, and neighboring concepts must be
DFS'd before any new carrier is named.

**Source intent** here means only an authenticated authored transformation plus its explicit
...
An endpoint diff answers only “what observations differ?”; a native proposal answers “what
transformation was requested, against which accepted state, under which obligations?” Capturing the
operation before flattening it to two snapshots removes much of the detective work a later
three-way merge must redo. A frame condition makes “no extra change” checkable: every result delta
must be required by an admitted proposal or be an expressly permitted canonicalization, never an
unrelated solver repair.

### P−1 shared prerequisite — claim-indexed evidence

The dashboard/Codex incident and source integration need the same missing relation. A recorded
...
E.g. `codex login status` may support `CredentialRecorded` for one `CODEX_HOME`, never
`ProviderReady`. A successful live request can support a provider/request-capability claim for its
provider, scope, and freshness bound. A worker PID supports lifecycle facts, not provider readiness.
In SCM, a parse edge, typecheck, behavioral receipt, Git rename score, and native authored operation
are likewise facts with different claim relations and maximum conclusions.

The information-state floor for one claim keeps two independent bits—support and challenge: support
only, challenge only, both/conflicted, neither/insufficient. This is the
[Belnap–Dunn](https://doi.org/10.1007/978-94-010-1161-7_2) shape for incomplete and inconsistent
information, not permission to reuse that logic's type names before DFS. Conflicting evidence is
not semantic contradiction; absence of evidence is not refutation.
...
entity/activity/agent provenance, which helps assess an evidence link but does not by itself
establish the target claim.
...
P−1 for SCM P0. This plan names roles, not final carrier names, so the first DFS converges on
existing authorities instead of extracting three local versions later.

### Native history

Authority is **target-scoped**, not a branch-name property. A project may have a primary target,
supported-release targets, or another explicitly admitted target; each target has one current
accepted state and each proposal names exactly one target. A Git ref, Mercurial bookmark or branch
head, and Pijul channel are different upstream projections of target-like roles; checking out,
updating, switching, pulling, or receiving one does not make it the native authority.
...
Workspaces may be concurrent; branches and merge commits are not fundamental. A workspace is a
sandbox of pending proposals whose construction history is evidence, not accepted history.
Acceptance records the target, exact accepted predecessor, each proposal's authored parent, the
applied transformation set, the resulting state hash, and the grounding evidence/receipts. Git
commits/trees, Mercurial changesets/manifests, and Pijul channel/change state are a third thing:
compatibility projections or capture evidence for those facts.

This is not “last writer wins with a nicer log.” A stale-parent proposal is re-evaluated against
current modeled state before it can advance it; arrival order is not intent. If two compatible
proposals admit witnessed transports forming a commuting square, validation may run in parallel and
either serialization must yield the same declared-equivalent result. If order changes meaning and
no policy declares one, the system asks rather than letting queue timing choose.

Native transactions thus need no three-snapshot primitive: the authored parent stays as necessary
causal evidence, and the operation is replayed and verified on the new state. Three-way comparison
remains necessary at a **lower-fidelity import/capture boundary** where only base, left endpoint,
and right endpoint survived.

No wall-clock fact supplies semantic order. Commit, authoring, upload, and observation time, queue
position, and “objectively arrived first” may be kept as audit or performance evidence but cannot
rank proposals or recover intent. A transition precedes another because an authorized receipt names
its accepted predecessor; a proposal depends on another only through an explicit parent/dependency
edge. A declared domain-time contract may decide whether evidence is fresh enough, never turn
chronology into priority.
...
dispatch table for “rename versus call,” “move versus edit,” or every future feature pair.
Conceptually:
...
kernel, preferably with a small trusted checker. A domain contributes ordinary facts, claims, proof
rules/procedures, and checkable certificates through shared interfaces, never a feature-pair arm in
an SCM switch. A solver or LLM may propose a candidate or certificate; unverified, it has no
authority.

Safety, explicit obligations, grounding, and the required evidence profile are admission
conditions, not weighted preferences; a cheaper result cannot buy permission to violate one. Among
candidates meeting them, preserve every fact whose change is not grounded. By default cost selects
only among realizations of the **same semantically determined result**; it cannot decide which
materially different result the user intended. A contract may explicitly make a cost axis part of
intent; absent that authority, several result equivalence classes are `Ambiguous` even when one
looks cheaper. Within one class, the declared cost order eliminates only strictly dominated
realizations and preserves incomparable alternatives rather than inventing scalar weights.
...
examples below are discriminating inputs to the same operation, not implementation branches. If a
new language concept needs a new pairwise integrator case rather than a shared semantic interface,
the missing work is a model/algebra seam to ground—not a twentieth-year special case to preserve.
...
integration results nor proves the list exhaustive. Today `solve_constraints` builds a singleton
candidate set holding the constraint root and derives “closedness” from that root being well
formed—a useful scaffold, not authority that every relevant integration result was considered.
...
This follows abstract interpretation's vocabulary of
...
under a declared abstraction, and claim completeness only for the query and fragment whose
certificates establish it.
...
resource bounds, dependency closure, and execution receipts are premises Git lacks. That supports
more deductions; it does not license inferring an unexpressed premise.

This is not a new “humility” principle but DESIGN §5's rule—never fabricate a plausible output; a
failure arm refuses, never widens—applied to reconciliation. Returning the one candidate the current
model happened to expose while silently treating every unmodeled fact as irrelevant conflates
ignorance with an answer in exactly the absorbing-fallback shape §5 forbids.

> A modeled fact `X` licenses consequences of `X`. It does not prove that no relevant, unmodeled
> fact `Y` exists.

The change set is closed in one sense: every accepted result change must be grounded in an admitted
proposal or declared canonicalization. The world of relevant meaning is not closed merely because
the program typechecks. A closed coproduct closes its declared axis; a derived affected/dependency
closure closes only the relation and bound its evidence names; a lossless surface closes only its
declared construct set. A unique semantic result therefore requires the specific certificates
above, not a claim that all intent or all behavior is modeled. If a potentially relevant axis,
dependency, effect, capture fragment, or proof bound is missing, the result space is unclosed at
that evidence profile.

This is also how the model evolves without making old confidence implicit: an admission receipt
names the model/contract inputs and closedness evidence it relied on. Adding a relevant model fact
invalidates the affected receipt and forces re-evaluation; absence in the older model never becomes
...
repair stay internal work or non-semantic operational status while a permitted machine-owned
continuation exists—not questions disguised as delegation. One answer adds the authenticated
...
vanishing into inspect state: a permanently unclosed fragment reports the change cannot yet be
checked safely; denied authority reports it is not permitted; a non-retryable effect failure
reports the failed domain action. The wording never exposes the internal arm name, asks the user to
reconcile, or implies a preference could fix a non-preference failure. The receipt keeps the exact
typed location and cause for inspection, counting, and repair.

Receipts and inspect APIs preserve every underlying state. Every group keeps its partial
assessments even if the batch cannot partially commit: e.g. group A unique and authorized, B
closed-contradictory, C unclosed for missing live evidence, D ambiguous. Partial commit additionally
requires a declared atomicity split and proven independence.

The admission policy states the required evidence profile and authority. A structurally universal
result is never presented as behaviorally safe; a behaviorally bounded result names its bound.
Humans or LLMs answer a genuine normative choice by submitting another authenticated
...
A diagnostic promises only what it can compute. This plan chooses **subset-minimal within a
declared diagnostic bound** for an incompatible core; “minimal” never silently means globally
minimum-cardinality. “Localized blocking question” is the default phrase. A future diagnostic
optimizer may pick a lowest-cost discriminating observation/question only under an explicit cost
policy.
...
against `S` applies only to `S` or through a witnessed transport into the other proposal's context.
Compatibility means a grounded commuting square—or joinability/confluence modulo the declared
equivalence—not literal equality of unchanged `P ∘ Q` and `Q ∘ P`: the
...
[CRDTs](https://inria.hal.science/inria-00609399) are a useful convergence comparison;
...
A text merge sees two edits to one line and ordinarily refuses; a node-key rule might see the call
node overlap and also refuse. Neither uses all the available model.

The program graph instead observes two distinct argument-binding edges. If it also establishes
that:
...
ambiguous; if the relevant relationship is not modeled, unknown. “Different argument keys” starts
the check, it does not skip it.

Conversely, two transformations touching the **same** coarse key are not automatically
contradictory: they may be identical, act on independent substructure, or normalize to one
transformation. Key intersection marks where deeper reconciliation is required.

### 6.2 Rename plus a new reference — textual clean merge, semantic question

P renames declaration `foo` to `bar`. Q, authored concurrently, adds a call spelled `foo()` on a
different line or file. Git merges this cleanly and leaves a broken reference.
...
witness**. The transformation is a no-op only under that observation model; storage binding or
projection provenance changes separately.

## 7. The safety quadrants and the real bar

“Compatible” below means relative to the explicitly required model, claims, fidelity, and
bounds—not unknowable private thoughts:
...
No SCM can guarantee an unexpressed mental preference; claiming so converts missing information
...
recovery is neither globally above nor below an exact structural parse with no behavioral evidence.
An admission policy sets minimum requirements **per axis**:
...
`DecodeFidelity = Lossless` establishes exact endpoint decoding for its construct set; it cannot
reconstruct an authored operation that was never retained. Likewise a passing typecheck cannot stand
in for behavioral or authorization evidence. Each receipt carries the profile and policy
...
means the configured merge machinery found no conflict, not a language-semantic proof
...
- The ASE 2024
  ...
  5.4% of Git's handled outputs were incorrect. Its structural Java baseline handled more cases but
  produced more incorrect results—evidence that “fewer conflicts” alone is the wrong objective.
- The 2026 preprint
  [On Correctness of Software Merge](https://arxiv.org/abs/2607.07987) gives a structural
  floor—parsability plus universal edit preservation—checked over 43,774 Java file scenarios. Its
  guarantee is deliberately syntactic/structural and depends on AST differencing; it does not
  establish behavioral or normative intent.
...
No Mercurial or Pijul extdeps authority exists in-tree today. That is a substrate gap, not
permission to infer their shapes from Git or call a Git-shaped interface generic.

The existing keyed diff stays useful beneath the new model, re-read as “one observation/capture
engine” rather than “the merge algorithm.” The storage-binding plan's current same-key refusal is
the safe adapter available **before** semantic integration lands, not proof that same-key proposals
are contradictory.
...
compose, and must not land a generic `ScmSolver`, `ReconciliationConstraint`, or interaction-rule
...
The product wedge is Git-first to minimize adoption cost; the technical boundary is SCM-plural
because adoption order is not design authority. Three upstreams are modeled before a common
compatibility carrier may land:
...
(DESIGN §3), so a fourth SCM is a new extdeps model plus bound handlers and witnesses—not a kernel
edit or a new coproduct arm.
...
capability refusal for that target policy, not a fake implementation built from fetch timing, local
locks, or “most recent” state. The semantic kernel consumes native proposals, accepted states,
claims, and receipts; never a Git commit, Mercurial changeset, or Pijul change as the semantic
contract.
...
capabilities are introduced only by the later realization that actually produces those receipts.

The landed C0 interaction carrier (`gunbc.native_scm_interaction_contract`) remains only a
presentation fixture. R1 dissolves its former optional `git` slot and Git-named scenario surface
into realization-indexed receipts plus realization-neutral projection/import journeys.
`gunbc.scm_compatibility_shape` owns the extracted presentation/capability roles; the Git,
Mercurial, and Pijul handlers keep their native receipts beside it. Neither carrier may be consumed
as a semantic/admission or Applied receipt, and C0's dead-simple primary outcome behavior is
unchanged.
...
claims evaluate in parallel; the acceptance log serializes receipts only once their
commuting-square and current-parent conditions are established.

The frontier solves a finite coordination question without pretending to know the future. An
unobserved proposal cannot block progress; but if two known proposals are order-dependent,
accepting whichever the scheduler visited first would itself be a normative choice. Once an
authorized acceptance advances `T`, its receipt—not its timestamp—makes the result the
authoritative prior for later proposals.

This directly answers green-then-main-advanced CI waste. A receipt is keyed by the claim's actual
modeled inputs, including closedness evidence, not by “the whole branch is still at this SHA.” If
the target advances outside those inputs the green receipt stays grounded; inside them, the system
revalidates the affected claim before acceptance. A blanket rerun is an `Unknown`/modeling deficit,
not a silent “fail-closed” success.
...
[`git update-ref`](https://git-scm.com/docs/git-update-ref) supplies the exact-old-object check the
initial target policy needs; other SCMs must ground their guarantee independently or expose the
capability as unavailable. When a later native target becomes authoritative and external SCMs are
asynchronous projections, an idempotent outbox binds the committed native transition to each
...
and policy/evidence hashes prevent an ABA-shaped acceptance where the visible state hash returns to
an old value while the validating authority has changed.

## 10. Compatibility realizations are plural; Git is first, not authority

Compatibility is non-negotiable for adoption but downstream. Product priority and design coverage
...
- Each accepted native transition can emit an ordinary one-parent/squash Git commit and source
  tree; a squash-to-main team sees the history shape it expects.
- Submit treats a branch/PR as a proposal inbox, not a peer authority. Users need not merge or
  rebase it onto the target; the admission service transports and re-evaluates the proposal
  against the current accepted target.
- Native proposal/evidence receipts may travel as optional metadata a normal Git client can ignore
  while still cloning/building the projected tree.
...
the operation or equivalent authenticated metadata survived. A lossless endpoint decode proves
`ExactEndpointDelta`; it cannot recover the operation that produced the endpoint. Rename
similarity, matching content, and three-way ancestry are claim-indexed evidence, never invented
identity. A textual merge driver may remain as a compatibility fallback, but its clean result
carries only textual evidence and cannot claim the native safety profile.

`extdeps.git.object_store` is intentionally not treated as complete: `TreeEntry` omits Git modes,
and `StoreObject` omits tag objects though `extdeps.git.ObjectType` has `TagObj`. P4's
import/export DFS must cover:
...
clone/build compatibility is established only once these fidelity axes round-trip or refuse
honestly.

### Mercurial stress boundary

Mercurial modeling preserves its own distinctions rather than translating them into Git names:
portable changeset identity vs clone-local revision number; file revisions and manifests;
...
bounded repository. Managed target advance stays capability-unavailable until the upstream model
and an executing transport witness establish the exact concurrency/transaction guarantee the target
policy requires.

### Pijul stress boundary

Pijul modeling preserves change identity and dependency/context facts, the recorded
...
a commit, nor a channel forced into one-head semantics to inhabit a Git-derived adapter.

The first Pijul fixture imports and exports a bounded change/dependency/channel state and proves
that reordering independent change delivery does not change the native semantic receipt. This does
not license inheriting Pijul's textual conflict semantics as Daglang semantic safety. Managed target
advance stays capability-unavailable until its upstream operation and read-back contract meet the
...
receipts, never the native semantic result, admission decision, or user question. Removing an
upstream capability may turn projection or target admission into a typed refusal, not select a
different semantic result. A fourth SCM must require no edit to the proof kernel or primary
interaction carrier.

The first product can therefore be a Git-compatible semantic admission tool rather than a new
hosted object store, proving the integration advantage while keeping clone, editor, CI, and forge
...
The only integration consequences carried here:
...
No proposal or successful transition can widen a publication grant because its program is
structurally or behaviorally valid, and no secrecy/publication claim can be inferred from semantic
compatibility. Grant/policy changes require separate authority and the §4 parent-policy transition
rule. The concerns compose only through explicit grants and realization bindings.

Diagnostics are publication projections. A full internal proof may mention a locked node,
counterexample, affected closure, or alternate result an audience is not authorized to know exists,
so the default response is an audience-authorized projection—possibly a redaction or commitment to
hidden evidence—with no error, timing, size, or alternative-count distinction bypassing the
visibility model. The same
[information-flow/declassification](https://doi.org/10.3233/JCS-2009-0352) rule governs remote
execution: metering prices an oracle; it does not prove outputs, failures, effects, timing, or
resource use reveal only what policy authorizes.

Vertical integration brings the product closer to real compiler/use-case failures; it must not turn
“visibility into customer code” into an undeclared surveillance business model. Raw source stays
governed by `Publish`/effect grants; telemetry is derived, consented, and minimal—evidence profile,
layered outcome summary, cost, and anonymized mechanism gap where possible. Trust is part of the
safety product, not a growth shortcut.

## 12. Product thesis and falsification

Agent concurrency changes the economics of source control: a 30-minute validation path and many
simultaneous proposals make integration quality a direct, compounding compute/latency/token cost.
Daglang has unusually rich evidence because the program, claims, compiler, affected set, and
execution model share one substrate.

The product advantage is not “the language knows what the user meant.” It is:
...
The user-facing operation stays the §0 loop: edit, submit, then **Landed** or one localized
plain-language **Choice required** question with concrete previews and a stored continuation. The
system retries stale targets and seeks missing mechanical evidence itself; once every permitted
...
decades without changing that public operation or growing a catalog of feature interactions.
...
The GitLab corpus supports “cheap to serve,” not “distribution is free”: GitLab's serving cost is
a minority of revenue while selling and R&D dominate. Word of mouth is plausible only if savings
are obvious in the existing workflow, so compatibility and receipts are part of the product, not
...
reddens the appropriate semantic/admission/transition layer. Every refusal needs a nearby compatible
control, so “always refuse” cannot satisfy the suite. The timestamp case permutes and erases
incidental time metadata. The model-extension case adds its invariant as data through an authorized
earlier transition with the kernel fixed. The presentation witnesses reject internal
type/logic/proof vocabulary on the default surface and more than one simultaneous normative
question.

Git's default merge, the current keyed-diff adapter, and at least one structural merge baseline run
on the same corpus. Mercurial and Pijul fixtures additionally exercise the compatibility boundary,
not their merge engines as semantic oracles. The comparison is evidence, not design authority.

## 14. Sequencing and acceptance

The signed visibility sequence stays intact. Integration work starts model-first and does not wait
for a native store:
...
2. **R0 — three independent upstream-model PRs:** complete the Git extdeps boundary, then model
   ...
   distinctions and declares no shared SCM interface.
   **Accept T1 per upstream:** a bounded fixture decodes, projects, and independently reads back its
   declared fragment; a nearby RED proves one upstream-specific distinction cannot be represented
   as the tempting Git analogue. Git: modes, symlinks, gitlinks, tags, paths, object format,
   exact-old-OID ref advance. Mercurial: portable changeset identity vs clone-local revision,
   manifests/file revisions, bookmarks/branches, phases, optional evolution metadata. Pijul: change
   dependency/context, pristine/tree state, conflict facts, channels as sets of channel-independent
   changes. Write capability may remain honestly unavailable.
3. **P−1 — shared claim-indexed evidence carrier, separate PR:** after DFS, model recorded facts,
   ...
   **Accept T1:** login-status/live-request and self-establishing-probe scenarios preserve
   credential-vs-provider and support/challenge/neither distinctions; the three consumers share the
   carrier rather than local enums.
4. **R1 — extract the compatibility realization shape:** only after all three R0 models land, DFS
   their executing fixtures and extract the smallest role interface actually shared. Upstream
   dispatch stays peripheral; capability support is data/receipts, never a central vendor enum or
   switch.
   **Accept T1/T2:** one fixed accepted-state/proposal fixture exercises all three and preserves
   ...
   stay realization specific; R1 does not invent the later semantic/admission receipt. Removing a
   ...
   R0 and P−1 may proceed in parallel since neither may infer the other. R1 waits for all three R0
   models; the proof kernel waits for P−1.
   **Status 2026-08-01:** the three R0 receipts, P−1 receipt, R1 receipt, and P0 receipt are accepted
   ...
   evidence and compatibility shapes but does not claim the P1 proof kernel or P3 write transport.
   The operator-authored P1 roadmap node is the one active SCM slice; it stays fail-closed at
   `ExecutionContractUnspecified` until its first modeled carrier supplies a discriminating
   acceptance witness. P2 and P3 remain parked.
5. **P0 — user contract, cost, and layered scenario model:** carry the §0
   ...
   evidence profiles, semantic zero/one/many/unclosed space, admission, transition, projection,
   closure certificates, timestamp non-authority, and scenario corpus as `.dag` facts/witnesses.
   ...
   realization-indexed projection receipts while preserving the C0-owned identity, transition, and
   primary outcome; that projection list stays presentation/compatibility state, not an admission
   or Applied receipt.
...
8. **P3 — target-scoped atomic admission + receipt reuse:** consume the R1 bound
   target-transition capability, not an SCM name. The first writable realization uses Git
   `update-ref <ref> <new> <old>`; pending proposals re-evaluate against the accepted target, and
   affected receipts survive unrelated advances. Mercurial and Pijul fixtures stay read-only or
...
semantic-integration prerequisite. R0 (upstream grounding) and P−1 (shared deduction substrate) may
run concurrently, neither imports the other, and both precede the downstream consumer needing them.
No phase is complete at T0 algebra alone: each names an executing consumer and a discriminating
red.
