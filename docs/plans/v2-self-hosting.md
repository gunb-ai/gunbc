...
1. **General body producer emits REAL ingested fn bodies** — forward reader over grammar rows ([general-body-producer-design.md](general-body-producer-design.md)), not fixture-bound MVP productions. Stage A landed (#6443); Stages B/C/D in flight (#6459). Builds on [body-lowering-design.md](body-lowering-design.md).
2. **Namespace SymbolIndex / tree-resolution lands** — B1 merged + fill ([namespace-resolution-design.md](namespace-resolution-design.md)); unblocks roster rows whose `SeedRetained.migration_trigger` is `^migrate_when_namespace_only_resolution_lands` on `frontier.dag` (e.g. `03_name_resolve`, `03_resolve`). Resolution works while `import` grammar still exists.
3. **FLAG D binder identity grounded** — DerivedBy producer-minting; position-qualified canonical binders (no parallel binder-identity scheme in the body producer).
4. **Weak self-host behavioral receipt green** — `std/logic` (or equivalent): emit -> compile -> run -> equals seed on discriminating corpus ([s2-v2-self-emit-direction.md](s2-v2-self-emit-direction.md) §11).
...
- **Carrier-held (do not transcribe counts in this doc):** per-module gap classification lives on `src/v2/compiler/self_host/frontier.dag` today as the 27-row roster's `SeedRetained { reason, migration_trigger }` dispositions plus `compiler_frontier_census_attribution: KnowledgeAttributed` — **the carrier holds the numbers; this doc points at the carrier** (DESIGN §2/§6: no parallel-ledger doc). #6464 IN FLIGHT will add execution-measured `blocker_class_distribution` and `frontier_probe.dag`; until then, do not cite those symbols as present-tense authority.
- **IN FLIGHT mechanism (#6464):** execution-measured per-module emit probe + `frontier_probe.dag` will dissolve hand-refreshed gap clustering when the ingest-manifest fold binds live probe receipts — not landed in tree yet.
