...
**Anti-pattern (explicitly out of scope):** (1) scanning emitted function bodies for method names; (2) encoding all fn-bound facts as `TargetCollectionWitness*` inside `RequiredTraitWitness` — that coproduct is **collection-representation constraints only** (`TargetRepresentationChoice.constraints`) and cannot model bare generics, `Outcome`, or `Option` (R1–R3 scope); (3) **witnessing Clone from return-type or field-type occurrence alone** — e.g. `fn f<T>() -> Outcome<T>` or a parameter typed `Option<T>` does **not** require `T: Clone` unless a modeled operation at that site (typically `.clone()` on the carrier) grounds the requirement.
...
| **RED (refusal direction)** | Fixture: modeled fn declares generic `T` with **no** grounded operation requiring `Clone` (e.g. `fn f<T>() -> Outcome<T>` with no `.clone()` invoke) → emit refuses to fabricate `T: Clone` on the signature |
