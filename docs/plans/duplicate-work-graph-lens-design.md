...
**Sequence (updated):** **C0** first (kills the dual description the F commit introduced; small, mechanical); then §10a's C2→C5 unchanged, with C2 rows authored as C0 projections. The forward walls land with the row that makes each true (wall 2 with C2's roster; wall 3 with the first C2 receipt; wall 1 rides Half A).
