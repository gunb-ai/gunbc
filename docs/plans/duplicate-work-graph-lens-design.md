...
## 11. Materialization kernel — closing contract & delivered-vs-not audit (2026-08-03)

This lane's §8 named an artifact-grain kernel ("mandate 6B") as a downstream consumer of the identity lattice, without tracking its progress here. Since §8/§10b were last edited, that kernel landed and hardened independently — this section closes the gap: what the kernel now guarantees (the contract), and an execution-verified audit of who actually calls it, separating the *kernel* claim from the *adoption* claim (the §4b rung-honesty distinction — a landed door is not landed traffic).

**The contract — DELIVERED, `dag/std/materialization_provider.dag`.** Sequence: `#6375` (Duplicate Computation lens seed) → `#6422` (ParseTable/`cached_stage` inhabit the Realization carrier — §10b correction above) → `#7385` (typed contract: `ArtifactRequest`/`MaterializedArtifact` coproducts, model-only) → `#7411` (resolved-graph consumer seam reauthored on the merged contract) → `#7534` (first real cross-process consumer, opt-in). The API is one door, not seven maps (§2/§3): `ArtifactRequest` is a coproduct fixing each family's declared-input shape and required-output roster by construction (a caller cannot under-specify an obligation roster — it is unrepresentable, not merely unchecked); `MaterializedArtifact` carries its required per-kind parts as required fields (an incomplete artifact cannot be constructed, only decoded as the explicit `LegacyIncompleteArtifact` variant for pre-contract disk data); `request_artifact_binding` is the one obligation authority consumed by both `provider_serve` and `provider_admit`; identity is *derived* from that binding, never caller-supplied, and is pinned to `StructurallyIdentical` only — `NormalizedIdentical`/`ExtensionallyIdentical` are honestly unreachable here because advertising them while checking structural equality would itself be the fabricated-capability failure mode (§5). Four codex reviews (44175, 44265, 44278, 44300) each closed a real forgery/omission path in the door — a caller-supplied required-outputs list, a caller-supplied identity grade, an unauthenticated payload label, an unauthenticated size — not cosmetic churn; `dag/test/claim/materialization_provider_witness_test.dag` carries 35 green claims over the result. On the contract-shape axis this kernel is at the **structurally guaranteed** rung (§4b): the omission/forgery classes named above have no constructor, not just a checked one.

**The adoption — 1 of 7 targets wired, opt-in only.** `dag/gunbc/materialization_provider_targets.dag` is the mechanically tracked `DeclaredFrontier` (registered in `gunbc.roster_registry`) of the seven consumer flips the contract makes possible. Verified this session by grepping every target's `CacheInterfaceId` for a live call into `provider_serve`/`provider_admit`:

| target (`ProviderContractTarget.cache`) | consumer_seam | status, verified |
|---|---|---|
| `resolved_graph_cache_id` | `v1_compiler.cli_run.materialization_provider_consumer` | **wired, real** — `src/v1/stage0/src/cli_run/materialization_provider_consumer.rs` builds an `InterpContext` over `dag/std/materialization_provider.dag` and evaluates the real contract (not a hand-duplicated mirror), but gated behind `GUNBC_RESOLVED_GRAPH_CACHE_DIR` (`#7534` — "no default CI activation"), and the Rust bridge itself is a declared `Scaffold{dissolves_to: SingleAuthority}`, not yet the terminal shape. Roster row still open (dissolve_on unmet); this is honest, not stale. |
| `parse_cache_id` | `gunbc.plans.cli_run_hollowing_plan` | **not wired** — `extdeps.realization.reconcile_in_process` still only *describes* the cache (catalog facts); no call site into `provider_serve`/`provider_admit` exists anywhere in `dag/` or `src/` outside the contract module itself. |
| `typed_module_cache_id` | `gunbc.plans.cli_run_hollowing_plan` | **not wired** — same as above. |
| `compile_stage_memo_id` | `v2.std.staging` `cached_stage` | **not wired** — `cached_stage` answers through its own lookup shape (grounded on the Realization carrier per `#6422`, not yet on `provider_serve`'s typed outcomes); `compiler_materialization_witness_test.dag` proves the carrier, not the door. |
| `parse_table_memo_id` | `v2.compiler.parse` `ParseTable` | **not wired** — same shape as `compile_stage_memo_id`: carrier grounded (`#6422`), door not consumed. |
| `eval_memo_id` | `gunbc.plans.cli_run_hollowing_plan` (interpreter builtin bridge, area 2.13) | **not wired** — the interpreter eval-frame memo still answers with its own hand-rolled verify-on-serve arm. |
| `native_artifact_cache_id` | `std.emit_on_demand` `emit_on_demand_lookup` | **not wired** — the warm tier still trusts its `.native_ready` marker without the contract's read-back digest verification. |

**What this means for the sequence above.** The kernel's own shape is closed; §8's "corpus form once Half A lands" is unaffected by this audit — the contract covers the artifact-grain `RealizedStep`-shaped requests (parse/typecheck/resolve-closure), never general computation, so it is a scoped instance living *inside* Half B's boundary (§8), not a substitute for Half A. `five-minute-ci-gate-design.md`'s row 4 ("no default CI savings credited until ordinary invocations consume it") is confirmed accurate by this audit, not merely asserted — 6 of 7 targets have not started consuming, and the one that has is env-gated off by default. **No claim in this document or the roster overstates delivery**; the correction this section makes is narrow — §10b's stale "Next: ParseTable/`cached_stage` grounding" pointer, fixed above, is the one place a completed step had not been marked complete.

**Closing-contract statement.** Treat `dag/std/materialization_provider.dag`'s public surface (`ArtifactRequest`, `MaterializedArtifact`, `provider_serve`, `provider_admit`, `request_artifact_binding`) as **stable** going forward — four independent review passes each found and closed a real correctness gap, and no known omission/forgery path remains open on the door itself. Future work on this kernel is consumer flips, tracked one row at a time on the existing roster (`gunbc.materialization_provider_targets` — countable, closes when empty); it is not further contract-shape churn unless a new correctness finding requires it.
