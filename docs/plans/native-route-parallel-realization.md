...
- `std.realization_width` — envelope is `process_memory_aware_spawn_width` (cores ∩ memory ∩ pids). This is the bound fold, not a second knob. Do not mint `NATIVE_SHARD_COUNT` / `--width`. `NATIVE_SHARD_INDEX` is an observation label only.
