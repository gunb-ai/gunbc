...
Measured 2026-06-21 over `dag/**` + `src/v2/**`, reachability cross-checked against run-roots (`ci_floor_plan`, `scheduler`, the v1 run path). This is what the lens must independently re-derive:
...
| need | reuse | file |
| --- | --- | --- |
| transitive reachability BFS | `v2.lens.module_graph` selection-tier edges (was `inert_lens_modules`, DELETED gunbc#8141) | `module_graph.dag` + `cli_run.rs` `build_module_graph_facts_live` |
