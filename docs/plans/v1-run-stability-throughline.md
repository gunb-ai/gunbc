...
> **Status:** proposed roadmap, 2026-07-13, session sleek-deer-172 ("corpus OOM"). Operator-requested alignment doc: the decision tree + milestone list for making whole-corpus floor runs complete reliably inside the 16 GiB slot envelope, for the months the self-host waves need. **This doc is a bridge plan, not a destination:** its terminal dissolution trigger is the witness corpus running as emitted native artifacts (ROADMAP §④ / Wave 3+), at which point the interpreted cost class this doc stabilizes stops being the live path.
>
...
M0 denominate (≤1 day) ──► M1 construction fixes (short-term; "step 1")
                                  │ probe + floor receipts
                                  ▼
                     floor completes < 15 GiB uncensored?
                        │ yes                    │ no
                        ▼                        ▼
...
|---|---|
| M0 | probe line: modules, peak RSS, Σ ancestry-map entries — committed to the measurement carrier |
| M1 | probe before/after; per-module constant flat across 228-vs-full closure scales; one whole-corpus floor run `floor_peak_post` exact < 15 GiB; every-order + assembled-view oracles green; `regen --verify` green |
| M2 | teeth pair (envs-strip GREEN / items-strip RED); censored→exact flip on the `(closure_nodes, peak, outcome)` triple; counted post-strip-demand refusals = 0 on a green run |
| exit | two consecutive whole-corpus main runs + one falsifier cold run completing in-envelope with exact (uncensored) peaks |

## 6. Open decisions for the operator

1. **Sign-off on this throughline** (gates M1 — load-bearing `infer` files).
2. **M2 go/no-go criterion:** proposed = "M1 receipts show floor still censored at 15 GiB"; alternative = schedule M2 regardless (defense in depth).
