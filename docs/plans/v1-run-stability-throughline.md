...
- *Compute axis / CI wall:* the compute-side complement of this memory lane — measured kernel decomposition (resolve 12.5%, inference-proper 87.7%), the fleet clock-starvation finding, and the W1–W4 endgame (clock determinism → kernel root fix → cross-run store → sharding) — lives in [compile-wall-endgame](compile-wall-endgame.md) (clever-seal-476, 2026-07-17).
