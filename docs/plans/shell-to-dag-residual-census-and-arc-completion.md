...
- **Separate retry-fidelity residue is in flight on #7303, not credited to current main:** replace the `Optional<String>` conflation with `Outcome<Run>`, refuse multi-step bodies and distinct redirect/capture cases, validate the template-hardcoded log source, and refuse the 3+ escalation shape whose old template dropped `level1.env`. That work closes additional Phase-1 silent narrowing; it is not needed to call the already-merged `TeeTo` dual-representation fix discharged.
...
Every §5.A/§5.B row **could be** faked by joining argv back into a string and feeding `shell.Exec.Run(script)` / `ShellOnHost{script}` — sleek-crab #7064 did exactly this (`argv_join(...) + " 2>/dev/null || true"`). While those sinks were reachable from intent, relocation was the path of least resistance and a brief alone could not stop it. So a construction wall was **not cleanup-after** — it was the enabler. The `ShellOnHost` record wall is now built; direct `shell.Exec.Run` remains writable through the transparent cast and counted string-taking bridges, so its construction/meta-exec confinement is still open. The original plan follows, with each bullet's actual fate marked:
