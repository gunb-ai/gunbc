...
**Disposition — OPEN on the current tree; a fix is in flight but NOT merged (`review 43544`).** Verified on `origin/main` as of 2026-07-26: `ci_spec.dag:226` **still constructs** `capture: TeeTo { log: "BUILD_LOG" }`, and `05_emit_orchestration.dag:628` **still returns only `r.command`**, silently dropping redirect/capture. **This defect is live.** An earlier version of this row said "mostly DISCHARGED by #7293" — that was wrong in kind, not just in degree: **an unmerged branch is not current state**, and a census that reports a branch's intended result as done is the fabricated-green failure §5 names. If #7293 were abandoned, this row would have silently retired a real violation.

- **In flight (PR #7293, open):** deletes the inert declaration (`ci_retry_body_run` → `Absent`/`Absent`) and converts the silent drop into a typed refusal, with a discriminating RED. Verified on its branch, *not* on main.
...
- **Residue that will remain after #7293 merges** (do not treat as current): the intent layer will not be able to express "tee this run to a log" at all, leaving the tee solely as fixed tokens in the grammar row — an honest modelling gap rather than a dual representation. **Trigger:** dissolves when the bash rows grow a modelled tee/redirect construct that `Retry` can carry. `CmdSubst`'s deletion rides with that work.

Owner stays `snappy-moth-330`. **This row flips to discharged only when #7293 lands on main**, not before.
