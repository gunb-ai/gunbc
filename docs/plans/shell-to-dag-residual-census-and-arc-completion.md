...
> | --- | --- | --- |
> | `srv3_chown_directory_to_current_user` | **discharged** — the six surviving shell-string helpers took typed argv over SSH/FleetSsh | `61bf47bb876` (#8590) |
...
>
> Read that table before touching the rest of the list. Three absences, three different causes — dissolution, file deletion upstream of the name, and rename — and no textual property of the name distinguishes them. The `host_hygiene` row is the sharpest: the correcting commit does not mention the absent symbols, so the evidence that discharges them is not reachable from the symbol at all. The SS1.C rows for these subjects were corrected separately (#10529); this table exists so the per-row lane inherits the worked examples instead of rediscovering them, since a finding that must be rediscovered is a finding that will be.
>
...
> Population for that recurrence, stated as a denominator **measured at `70925ee89e` and valid only there** — every later edit that strikes a row through changes what the closed-marker grep counts, so this split moves under you and must be re-derived, never quoted: the census carries **180 data rows** across its tables, of which **41** already carry a closed marker (LANDED / DELETED / RETIRED / dissolved / MIGRATED / EXTINCT), leaving **~139 rows asserting live state**, none of them re-verified here.
>
