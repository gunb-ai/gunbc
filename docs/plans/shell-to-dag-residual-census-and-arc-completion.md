...
| ~~`ci_native_cache_root_toolchain_segment_command`~~ **DELETED #7436** | `rustc -V 2>/dev/null … ; if empty: toolchain-unresolved` slug | cache-poison fallback — **RESOLVED #7436**: toolchain segment computation reordered after setup-rust-toolchain; refusal instead of fabricated segment. |
