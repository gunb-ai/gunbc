...
| `extdeps.os.id` · ~~`Read`~~ **LANDED** (uid/gid/user) | `dag/extdeps/tools/id.dag` (created; landed #7194; post-#7231 home) | `id -u`, `id -g`, `id <user>` — service `os.Id` ops `Uid`/`Gid`/`Lookup` | **Done** — hermetic witness `test.claim.shell_dag_census_5a_typed_ops_witness` (`witness_os_id_uid_realizes_hermetically`, `witness_os_id_gid_realizes_hermetically`, `witness_os_id_lookup_realizes_hermetically`); `host_effect_realize` ssh probes already consume the typed argv rows `extdeps.tools.id.id_uid_argv`/`id_gid_argv` (absolute-path variants). `fleet_posix_accounts` `probe_command` is NOT-a-shell-site (census §4, `probe_command` row). **Ruling request below**: `id_lookup_argv` zero production references. |
...
**RULING REQUEST — `extdeps.tools.id.id_lookup_argv` (`id <user>`) has zero production references (filed 2026-09-03, keen-moth-880).** Measured denominator: grep of `dag/` + `src/v2/test/` at `739f1a548f` finds only (1) the definition in `extdeps.tools.id` (`id_lookup_argv`), (2) the hermetic-witness call in `test.claim.shell_dag_census_5a_typed_ops_witness` (`witness_os_id_lookup_realizes_hermetically`), and (3) service-name rows in the effect-plan tests — no production caller. Zero references is NOT filed as deadness: env-gated and emitted-path consumers exist that grep does not see, so this row is a ruling request with a measured denominator, not a deletion. The §5.A row's named consumers for the `id <user>` verb are reclassified (`fleet_posix_accounts` `probe_command` — NOT-a-shell-site, §4) or deferred (`spark/managed_access_apply` create-script bodies — dissolve-on marker, no operator verdict). Ruling requested: retain `id_lookup_argv` as reserved-for-deferred-consumer, or retire it. Do not delete on this finding alone.
