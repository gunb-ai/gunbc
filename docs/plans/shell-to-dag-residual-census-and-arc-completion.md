...
| `host_effect_realize.dag` srv3 transport helpers | `srv3_transport_witness_bin_success`, `srv3_transport_test_executable`, `srv3_apt_tool_present`, `srv3_tool_bin_path`, `srv3_chown_directory_to_current_user` still build/pass command strings to SSH; the old `srv3_host_effect_script` / diagnostic-checklist builders are deleted | typed local/SSH argv operations; operator-deferred with the surviving srv\* actuator graph (§4.D), never polish a deletion-bound subgraph in isolation |
| ~~`host_build_cache_provision_script.dag`~~ **FILE DELETED — A5 un-deferred, bounded to the effect-grain swap only** | `HostEffect.ProvisionBuildCache` is gone; the constructible arm is `EnsureBuildCacheInstance { desired: BuildCacheInstance }`, matched in `host_effect_realize.realize_provision_build_cache`. The six `build_cache_*_body` builders **relocated unchanged in spirit** into `host_effect_realize.dag` beside it — a RELOCATION of the concat-built shell, not the typed-argv conversion `#5828` names; render-is-not-admit (see `gunbc.build_cache_ensure`'s `ensure_effect_grain_migration_note`), so `admit_ensure_build_cache_instance`'s placement/path admission is still not wired into the realize path | `host_effect_apply_gated` retained-script transport, now built from `host_effect_realize.dag` | typed observe/effect operations over the new typed effect grain; `#5828` remains open, un-deferred by nothing this row records |
| `host_hygiene_{reaper,liveness}_script.dag` | concat-built runtime host mutation/observation bodies | `host_effect_apply_gated` retained-script transport | typed observe/effect operations; operator-deferred A5 |
