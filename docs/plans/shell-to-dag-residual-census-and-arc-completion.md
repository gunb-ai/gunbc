...
| `dispatch_pipe_pane_emit` · `dispatch_event_pipe_command` | tmux `pipe-pane` (its input contract IS a shell command) | **RESOLVED** (`review 44063`). The payload is now built as a bash AST (`bash_build_command_from_lits`) and rendered through `v2.workflow.bash_command_fold_serialize` — quoting is the lit-word production's job, so `shell_quote` no longer appears at this site. Reaching the backend required moving the construction onto a realization edge: the new `dag/gunbc/dispatch_pipe_pane_emit.dag` is enrolled in `realization_edge_path_prefixes` (precedent: `dag/gunbc/live_deploy/emit`), and `roadmap_dispatch_actuator` imports the finished command. The *seam* stays permanent, as the pre-push hook row above; the hand-joined *construction* is gone. A serializer refusal renders a non-executable poison marker, held dead in corpus by `witness_event_pipe_emits_grammar_quoted_command` |
