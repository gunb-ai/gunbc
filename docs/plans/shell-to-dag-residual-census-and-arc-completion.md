...
| `host_effect_realize.dag` srv3 transport helpers | `srv3_join_shell_words`, `srv3_transport_witness_bin_success`, `srv3_transport_test_executable`, `srv3_apt_tool_present`, `srv3_tool_bin_path`, `srv3_chown_directory_to_current_user` still build/pass command strings to SSH; the old `srv3_host_effect_script` / diagnostic-checklist builders are deleted | typed local/SSH argv operations; operator-deferred with the surviving srv\* actuator graph (§4.D), never polish a deletion-bound subgraph in isolation |
