...
The control widens the finding past the one file. `EmitArtifactThenThinRun` is **realized nowhere in the tree**: `gunbc.systemd_run_transient`, `gunbc.systemctl_stop_run`, `gunbc.systemctl_status_read`, `gunbc.systemctl_show_read`, `gunbc.systemctl_is_active_read`, `gunbc.hostname_set`, `gunbc.hostname_read`, `gunbc.spark_serving_converge_apply`, and `gunbc.live_deploy.readiness` each carry a refusing arm too. The sites that appear to accept it — `gunbc.ci_deploy_access`, `gunbc.ci_deploy_target_host`, `gunbc.host_identity_adopt` — are classifiers whose arms return a `Bool` or the variant's own name for totality; not one of them executes the transport. So the variant is **declared but unrealized**: nothing is pending cutover anywhere, and no srv3 call site is waiting on a Phase D that exists.
