...
SHELL-DAG is this document's; CLI-AUTHORITY is `cli-invocation-emission-design.md`'s. A site can be complete against the first and still owe the second, and that is not a defect in either — it is the boundary in §0c working. The corollary for dispatch: **a SHELL-DAG row is never blocked on DECIDING its semantic destination.** Its production cutover may share a carrier or target increment with CLI-AUTHORITY — `command_runner`'s is the live case: deleting that argv → quoted text → heredoc route needs the structured process-argv carrier, and its SSH arm needs the nested shell-command target — and those shared increments are implemented once, never bypassed with another shell string. *(Corrected 2026-08-19, review on #8535. This read "never blocked on CLI-AUTHORITY" flat, which is true of the homing decision and false as a sequencing rule, and the counter-example was named in the same document.)*
...
- **`DESIGN.md` needs no edit, and the reason CHANGED under this PR.** The first draft of this clause said its shell → intent open-thread row already routed runtime-present sites to typed effects, so the boundary falsified nothing in it. That row **no longer exists**: #8476 cut DESIGN to ~9.5k words and removed the open-threads section wholesale. What survives, and what the boundary now instantiates, is §3's general paragraph — *interface, realization and policy are three facts, not one row*, with the transport a Realization handler one of N and the dispatch that selects a realization sitting peripheral rather than in the interface. A CLI-backed handler is that paragraph's shape at the tool seam, so the boundary is a consequence of DESIGN rather than an addition to it, and still needs no edit there. *(Rewritten 2026-08-19 while correcting this document's other stale claims — the original clause was accurate when written and was falsified by a merge to main two commits later. Recorded rather than silently swapped, because the failure mode is the subject of this very commit.)*
...
The zero-caller measurement is right and reproduces. What it does not establish is that the fold is *unwanted*, and re-reading the module's own receipt rather than the grep output shows it is the opposite. `lane_e_srv1_ci_noncompletion_root_cause_receipt`, in this same file, names `placement_verdict` **as the fix authority for a live incident class** — in its words, "FIX AUTHORITY: ServerManagedUnitOwned on the managed compile-pool slice — ServerRunnerOwned on ANY lifecycle (active or retired) must refuse ProvisionConverged (`placement_verdict`, `build_cache_placement_observation_test` `witness_both_runner_lifecycles_refuse_at_the_verdict`)" — for the stranded-sccache-server defect that killed CI builds at the 15m compile ceiling. That receipt's own status is "INCIDENT CLEARED, NOT REPAIRED ... the class can strand again the next time a slot retires while holding the server."

Nor is the module dead: nine production modules import `gunbc.host_build_cache_provision` (`extdeps.cache.sccache`, `build_cache_ensure`, `build_cache_instance`, `build_cache_unit`, `fleet_host_budget`, `host_effect_realize`, `host_layout`, `host_standup`, `runner_capacity_realize`). They consume other declarations in it. A symbol-level zero sitting inside a heavily-imported module is exactly the shape that reads as dead from a whole-file glance and is not.

So the finding is **strictly worse than dead code, and it is a §4b rung problem rather than a §2 redundancy problem**: the verdict fold exists, the gate exists, the witnesses exist and pass — and nothing on the provisioning path asks any of them. The refusal named as the remedy is computed by a function no production caller calls. Green witnesses establish that the fold is *correct*, not that the class is *guarded*; reported as though they established the latter, the class reads a rung above where it sits, which is the inflation §4b calls worse than sitting low, because an inflated class never ranks for climbing.

**What the deletion would have cost, stated because it very nearly happened:** it would have applied cleanly, taken the witnesses with it, broken nothing, and removed the named remedy for a recurrable CI-capacity incident — leaving a prose receipt pointing at a symbol that no longer exists. The witnesses' greenness was the property that made removal look safe.

**Next step, replacing the one this paragraph supersedes: wire it up, do not delete it.** Someone must find the provisioning path that should be gated and establish why it is not — a different job, a different size, and a different owner than a deletion. The first thing to test is the one way this could still be benign: that the path is genuinely ungated, rather than gated by an equivalent check under another name. The `refuses()` rehoming note above becomes moot if the fold stays, and is retained only in case a future measurement re-establishes the deletion case.
...
`realize_provision_build_cache` has exactly one gate before it dispatches to the body — `provision_build_cache_catalog_gate_verdict(catalog_id: instance.implementation)` — and that function, in a third module, is pure id validation: unknown id refuses, an id other than `sccache_local` refuses, otherwise realizable. It never observes placement, principal, unit ownership, or lifecycle. So there *is* a check under a different name, and it is **not an equivalent check**: passing it says nothing about whether the daemon is stranded in a retired slot's cgroup, which is the class the incident receipt names `placement_verdict` as the fix for. `host_effect_realize.dag` matches none of `placement_verdict`, `provision_build_cache_verdict`, `build_cache_provision_gate_accepts`, `admit_ensure_build_cache_instance`, or `EnsureBuildCacheAdmission`.

**Two unconsumed authorities, not one — and therefore one wiring job, not two.** `build_cache_ensure.dag` carries a second admission authority, `admit_ensure_build_cache_instance`, with its own `EnsureBuildCacheAdmission` / `EnsureRefusal` / `EndpointPreflight` vocabulary and also no production caller. Both name the *same* single next consumer: `realize_provision_build_cache_body`. That materially reduces the sizing this note originally implied.

**The tree already recorded this, and the record has not gone stale.** `ensure_effect_grain_migration_note` in `build_cache_ensure.dag` states it in terms — "RENDER IS NOT ADMIT ... NO PRODUCTION CALLER IS NAMEABLE TODAY ... The nameable next consumer is `realize_provision_build_cache_body` ... once it is rewritten to observe `BuildCacheServerPlacement` and `EndpointPathState` for the target host and route through this admission instead of unconditionally rendering and restarting the unit." That claim was verified against the code rather than quoted, because a correct note goes stale silently; it has not. The scope boundary that produced the gap was a deliberate ruling on #8598 (arm-plus-render only, explicitly refusing to wire a consumer that did not then exist) — correct for that PR, and now the open obligation.
...
**LANDED 2026-08-21 — both authorities are consumed, and what it cost is recorded rather than left as the open question above.** The missing piece was a producer, and there was no cheap version of it: `build_cache_endpoint_observation.build_cache_ci_endpoint_observed_producer` returned `ProcTableUnreadable` in every arm because it takes a `HostIdentity` and therefore has no transport, so nothing could be threaded through it. The producer now lives in `gunbc.build_cache_endpoint_observe` as a PURE DECODE of read text into `BuildCacheEndpointObservation` / `EndpointPathState` / `ObservationCurrency`, and `gunbc.host_effect_realize` performs the reads through the transport it already holds — five to nine single-argv, non-mutating reads per pass (`cat /proc/sys/kernel/random/boot_id`, `stat -c %F` on the endpoint path, `cat /proc/net/unix`, the `find` fd-holder walk, then the owner's `cgroup`, `stat`, principal and `exe` only when a single owner resolved). Locally they go through `gunbc.command_runner.run_shell_command_capture`; over SSH through `run_typed_argv_transport`. No shell body was added: the reads land on the typed-argv side of `#5828`, and the ensure step's concat-built script is untouched, so the A5 row's dissolve-on is unchanged.

Both authorities are consumed, on opposite sides of the act and without duplicating each other. `build_cache_ensure.admit_ensure_build_cache_instance` is the PRECONDITION and runs before the first mutation — `EnsureAlreadyConverged` returns converged with no unit written and no restart issued, which removes a per-pass reinstall that re-colded every compile on the host; a runner-owned endpoint refuses with the owner located, which is the incident class. `host_build_cache_provision.provision_build_cache_verdict` is the POSTCONDITION over a re-observed placement after the ensure ran, which is what catches an ensure that reports success while the endpoint ends up owned by something else.

**A CAPABILITY BOUNDARY WAS CLAIMED HERE AND IS REFUTED BY MEASUREMENT — read this before concluding anything in this lane is impossible.** An earlier revision of this row stated that the fourth clause of the stale-socket observer contract (that no `/proc/*/fd` entry still resolves to the socket's inode) needs a glob over `/proc`, therefore a shell, therefore emitted-shell growth this A5 row may not do — and concluded that `EndpointStaleSocket` could never be produced and srv4's stranded socket could only ever refuse. **That is false.** The walk is one argv with no shell:

```
find /proc -maxdepth 3 -path "*/fd/*" -lname "socket:?INODE?"   ->  /proc/<pid>/fd/<n>
```

Executed 2026-08-21 against a live listening socket, reproduced independently by a second session, with two controls that are the actual content of the measurement: the bracketed form `socket:[INODE]` matches **nothing** (find reads `[...]` as a glob character class), and a non-existent inode returns **empty**. The original error was produced by exactly that trap — the bracketed probe returned nothing, and "found nothing" was accepted as confirmation that the walk was impossible. **A zero with no nonzero beside it in the same run is not a measurement**, which is the durable lesson and the reason this paragraph states the refutation rather than quietly dropping the claim: a silently removed impossibility leaves the next reader with no reason to try either.

So the shipped producer walks the receipt's chain — `/proc/net/unix` (exact path, `St==01`) → listening inode → the `find` walk → owner pid → `/proc/<pid>/{cgroup,stat,exe}` — and `ss` is not used, matching both `gunbc.build_cache_instance.endpoint_ownership_note` and the step-1 placement receipt, **which were right as written and are unamended.** All four stale-socket clauses are established, so `EndpointStaleSocket` is produced and the admitted unlink is reachable: srv4's `/var/lib/ctrl/sccache-ci/server.sock` becomes an admitted unlink rather than a permanent refusal. What remains genuinely unmodeled is TCP-endpoint ownership (`/proc/net/tcp` with its own hex address decode); no `BuildCacheInstance` in the tree carries a TCP endpoint, and that arm is a positive typed refusal rather than an untested path that answers.
...
The control widens the finding past the one file, but **not** to "the transport is unrealized" — that stronger claim was asserted here on 2026-08-20 and falsified the same day by reading every arm body rather than pattern-matching for refusal shapes. `EmitArtifactThenThinRun` has **exactly one realizer**: `gunbc.host_effect_realize` `realize_converge_on_host` splits its arm on `gunbc_apply_mode_for_host` — `FreshStandup` refuses as fail-closed Unimplemented, and `ExistingHostQuiescentReload` calls `realize_converge_in_process`, *the identical call the `LocalShell` arm makes*. For an existing quiescent host the transport is not merely accepted; it takes the same path as the shell transport. The neighbouring `SshShell`/`FleetSsh` refusals confirm the direction, since their text directs the caller to "use EmitArtifactThenThinRun."

Two disguises defeated the pattern-based reads that produced the wrong count, and both are worth naming because each survives a scan written for the other: a **refusal shaped as a success-valued record** (`gunbc.hostname_set` answers `HostnameSetCasApplyFailed { stderr: "EmitArtifactThenThinRun unsupported..." }`, which no `NotConverged` scan sees), and a **realizer behind a refusing first sub-arm** (the arm above, whose opening lines read as a refusal). Reading arm bodies rather than matching shapes was the only method that survived.

**The denominator both audits used was itself wrong, and the correction belongs here rather than in a footnote.** An earlier revision of this paragraph said "all 24 arm bodies," inherited from the manual audits. Measured at `4cec10f66a3d84cbde631e20ca9feaa31457d88c`, the tree carries **41** `EmitArtifactThenThinRun` occurrences: 1 variant declaration (`gunbc.host_effect`), **36 production match arms** across 14 modules, and 4 in `dag/test/claim/`. Neither manual audit reached 36, and neither mentioned `gunbc.host_effect_codex_supervised_turn`, whose arm answers `CodexSupervisedSessionApplyRefused` — another refusal wearing a record name rather than a refusal type. The **conclusion is unchanged**: exactly one arm realizes, and it is `realize_converge_on_host` under `ExistingHostQuiescentReload`. What was wrong was the count of what had been examined, in a paragraph about examining carefully.

That is the point, not an aside. Two readers who had just corrected each other on method still could not state their own denominator, because a hand-assembled roster cannot prove its own completeness — it can only report what someone remembered to look at. A derived census is not a stylistic preference over a careful manual one; it is the only kind that can be wrong *loudly*. The 36/1 partition above is therefore recorded as a **dated measurement at a named SHA, to be reproduced by a derived detector and reconciled against — never as a fixed law**; migrations are expected to change it.
...
**The srv3 finding is unaffected by the correction.** The one realizing path is for `HostConverge`, not for any srv3 effect: every `Srv3*` arm still refuses, including `Srv3BootOnceReadBackSleep`, `Srv3SeededInstallMediaToolchainEnsure`, and the four `srv3_actuator_toolchain_*` helper refusals. What changes is the supporting sentence, not the conclusion — it is no longer "the transport is realized nowhere, so nothing is pending cutover" but **"the transport has exactly one realizer and srv3 is not on it."**

What this settles and what it does not. **Settled by measurement:** the srv3 subgraph is not subsumed today and is not scheduled for subsumption by any live mechanism, so the deferral has no standing premise. **Not settled, and it is a decision rather than a measurement:** whether the `srv*` actuator graph survives at all. The tree cannot answer that — migrating 22 call sites (`srv3_transport_witness_bin_success` 10, `srv3_tool_bin_path` 6, `srv3_apt_tool_present` 4, `srv3_chown_directory_to_current_user` 2) in a subgraph the operator intends to delete by another route would be wasted work under a different premise than the original one. The deferral is therefore **due for re-decision, not automatically expired**, and that re-decision is the last blocking input on the largest remaining block in this census.

**Prior art the derived census must supersede rather than extend — `v2.lens.effect_reach` (measured 2026-08-20 at `4cec10f6`).** A lens answering approximately this question already exists, and a reader who finds it may mistake it for coverage. It classifies host-effect sinks and carries a `ShellExecRunSink` variant, so its subject overlaps the shell census directly.

Its selection is the forbidden seed form. `sink_kind_for_callee` takes a `callee_text: String` and compares it by name equality against a remembered list — `"Run"`, `"shell.Exec.Run"`, `"Exec.Run"`, `"WitnessBin.Run"`, `"Read"` — with everything else falling to `UnknownHostEffectSink`. That is *where we currently remember doing so*, not *what interprets bytes as shell*, and the consequence is mechanical: it cannot see `sh -c`, cannot see `sh -s` on stdin, and cannot see a wrapper stack such as `sudo -S sh -s`. It therefore carries **the same blind spot this census carried before Spark was found** — `gunbc.spark.managed_access_apply`'s four script builders reach a host through `stdin_payload` behind a wrapper, and no callee-name comparison can reach them.

Two further facts bound how much weight it can hold. Its classification is derived from callee *text* rather than from resolved callee identity, which is the text-scanning substitution the census brief's stop condition forbids. And `v2.test.claim.enforcement_live_witness_test` asserts `lens_contract_is_unbound(c: lens_contract_effect_reach)` — the corpus's own record that the contract has no enrolled consumer. Whatever else it is, it is not gating.

The disposition is therefore **supersede, not extend**: a census seeded from interpretation semantics subsumes this lens's entire sink vocabulary, while extending the lens would inherit its selection principle. Recorded here so the derived cut treats it as prior art with a known-narrow frontier rather than as an available foundation — and so its `ShellExecRunSink` rows are never read as an existing shell-reach population.
...
**End state:** no user-space `.dag` constructs a shell string. Every "this `.dag` wants to call a bash script" instance resolves to exactly one of four paths below. The `realization_vocabulary_containment` lens (#6854, LIVE) already forbids bash-AST vocab (`ShellProgram`/`ShellStmt`/`serialize_bash`) in user space — the `shell.Exec.Run(script: TransportScript)` sink **closed at construction with #7962** (§5.E). #7184 retyped the **`ShellOnHost` realization edge** to the `RetainedShellScript` record (`host_effect_realize.dag:167`, `retained_shell_script.dag:9-10`) and deleted the free minter `transport_script_from_body`, so a hand-assembled `String` no longer typechecks *there* — but it did not retype `shell.Exec.Run`; #7962 then sealed `TransportScript` itself, so that edge is closed at the type rather than by the lens. On the walled `ShellOnHost` edge what is still possible is **conspicuous**, not silent: authoring a counted `RetainedShellScript` row with a hand-built body is the *reviewed* escape, carrying a `reason` and a `dissolves_to` — that visibility is the mechanism, not a leak in it. The `shell.Exec.Run` sink now has a construction gate of its own — `TransportScript` is a **`sole_constructor` record** whose single mint `transport_script_seal` is `admit_callers`-sealed to exactly two production declarations (`gunbc.retained_shell_script` `retained_shell_script_to_transport`, `gunbc.bash_materialized_transport` `bash_materialized_transport`), and the cast form closed with it — `04_infer` `sole_constructor_construction_diags` judges a cast into a sealed type (#7962) — so the meta-exec confinement milestone is met at construction; what survives is the bridges' bare-`String` body, dissolved by migration rather than by a further wall. *(Rewritten 2026-07-26, `review 43544`. This sentence previously called that sink "the remaining hole", and my first correction **appended** a supersession notice below it instead of rewriting — leaving two mutually exclusive current-state claims in the same paragraph. That misapplied this document's own rule: strike-in-place is for corrections that are **illegible without the original**, and this one is not — the four-path end state survives the rewrite intact, so only the blocker claim needed replacing.)*
...
Every §5.A/§5.B row **could be** faked at the former `ShellOnHost{script}` edge and at direct `shell.Exec.Run(script)` by joining argv back into a string — sleek-crab #7064 did exactly this (`argv_join(...) + " 2>/dev/null || true"`). While those sinks were reachable from intent, relocation was the path of least resistance and a brief alone could not stop it. So a construction wall was **not cleanup-after** — it was the enabler. **Both sinks are now walled**: #7184 built the `ShellOnHost` record edge, #7962 sealed `TransportScript`. What survives is the counted string-taking bridges, which are conspicuous by design. The original plan follows, with each bullet's actual fate marked:
