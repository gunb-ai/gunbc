...
| --- | --- | --- |
| `RawLine` (bash-AST node) | **zero** — tree-wide | **six** occurrences, none a construction site: one prose string at `ubuntu_seeded_install_media_remaster.dag:43` (a dissolution `reason:` attesting to its own removal), and **five in two frozen `.diff` fixtures** — `src/v1/stage0/testdata/module_grain_affected_{dag_only_6edafbb,v2_only_bb6e656}.diff`. Two of those five are `-` **deletion** lines (`-  RawLine,`, `-      RawLine { text: … }`) — i.e. the recorded removal itself |
...
Error history: the original row overstated the surface at "11 files"; the first correction pass listed seven files as live sites when the true count is **zero and the declaring module does not exist**; the second pass fixed that but scoped the occurrence census to `dag/**` and `src/v2/**`, missing class (d) entirely (caught by `review 43455`). **Scope note, so the claim is checkable:** the occurrence counts above are over the *whole repository, all file types* — `git grep -l <Name> -- .` with no pathspec — not a `.dag`-only sweep. Before editing this section again: confirm the *declaring* module still exists, search unscoped, and read each hit rather than counting filenames.
...
- **`CaptureSpec.TeeTo`** — **was live but inert; being fixed in flight by #7293, so read the state below carefully.**
  - *Before #7293 (state on `main` as this row was written):* `ci_retry_body_run` (`dag/gunbc/ci_spec.dag:226`) constructed `capture: TeeTo { log: "BUILD_LOG" }`, but `orch_retry_step_command` returned only `r.command` and `orch_emit_retry_run` rebuilt the `Run` with `redirect`/`capture` `Absent`. The identical tee semantics were **hardcoded as fixed tokens** at `bash.dag:1093-1133`, so the emitted bytes were right and the *model* was the copy that did nothing — editing `log: "BUILD_LOG"` changed no output. Exactly the tell §5 names: the spec can be edited while the realizer keeps doing its own thing.
  - *After #7293 (verified on its branch):* `ci_retry_body_run` is `redirect: Absent, capture: Absent` — the lying declaration is **deleted** — and the retry path now **refuses** any `Present` redirect/capture instead of silently dropping it, with a discriminating RED (`ci_merge_admission_emit_test.dag:376`) asserting a retry-body `TeeTo` is `Rejected`. `TeeTo` then has **zero production constructors**; the only one left is that refusal control.
...
**Disposition — mostly DISCHARGED by #7293; the residue is smaller and different.** The §3 dual representation is resolved the honest way: by deleting the inert half rather than by making it load-bearing. Note this is *not* the fix direction I originally filed (thread `redirect`/`capture` through the retry path), and the change of direction is correct — an inert declaration is worse than an absent one, because it lies. What remains is **not** a dual representation but an **honest modeling gap**: the intent layer now cannot express "tee this run to a log" *at all*, and the tee lives solely as fixed tokens in the grammar row. **Trigger for that residue:** dissolves when the bash rows grow a modelled tee/redirect construct that `Retry` can carry, at which point `ci_retry_body_run` can declare it and have it mean something. `CmdSubst`'s deletion still rides with that work. Owner stays `snappy-moth-330`.

**Disposition (owner + trigger, per §6 — this is not an unowned deferral).** Owner: **`snappy-moth-330`** (this lane). **Dissolution trigger:** *dissolves when `orch_retry_step_command` threads `redirect`/`capture` through the retry path and the hardcoded tee tokens at `bash.dag:1093-1133` are deleted, so the declaration is load-bearing.* **Acceptance bar (discriminating, cheap):** rename the log in `ci_retry_body_run`, regen, and show `ci.yml` **changes** — today it is byte-identical, which is the defect. **Sequenced, deliberately not yet dispatched:** the fix direction is a real design choice (thread capture through retry vs. delete the hardcode and let the row own it) that lands inside `bash.dag`, a load-bearing grammar file, and it is the same *model-declares / realization-ignores* shape as the `Run.command` → `Do{effect}` arc-close awaiting operator decision. Spawning a worker onto an undecided design in a load-bearing file is how a bad PR gets made; this item moves with that decision. `CmdSubst`'s deletion rides along with it rather than landing as a separate one-line PR.
...
It has not, and the honest reason is that the trigger was **mis-specified rather than unmet**. It names two *mechanism* events, but what actually keeps this document alive is the **per-site punch-list** (§4.A–§4.J): the wall stops *new* concats from being written, it does not dissolve the ones already there, and those rows still have owners and open PRs. A correct trigger reads: *dissolves when the §4 punch-list empties* — mechanism landing was necessary, not sufficient.
