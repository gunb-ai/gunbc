...
**One bound is declared rather than closed, and it is the reason `EndpointStaleSocket` is still never produced.** The stale-socket observer contract has four clauses; `ss` establishes three. The fourth — that no `/proc/*/fd` entry still resolves to the socket's inode — needs a glob over `/proc`, i.e. a shell, i.e. the surface this row may not grow. So srv4's stranded socket refuses with `EnsureEndpointUnobservable` naming the unread clause instead of being unlinked on three quarters of the evidence, and the unlink path stays modeled-but-unreachable. It becomes reachable with a typed fd-inode read, the same `#5828` work this row's shell dissolution waits on. `gunbc.build_cache_instance.endpoint_ownership_note` — which specified the `/proc/net/unix` plus `/proc/*/fd` chain and named `ss` among the tools it does NOT depend on — is amended in place with what shipped and why, including this cost, rather than left to contradict the realization.
