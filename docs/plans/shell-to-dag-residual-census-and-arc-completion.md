...
- **`CaptureSpec.TeeTo`** — **live but inert.** `ci_retry_body_run` (`dag/gunbc/ci_spec.dag:226`) constructs `capture: TeeTo { log: "BUILD_LOG" }`, but `orch_retry_step_command` (`05_emit_orchestration.dag:626`) returns only `r.command`, and `orch_emit_retry_run` rebuilds the `Run` with `redirect`/`capture` `Absent`. The identical tee semantics are **hardcoded as fixed tokens** at `bash.dag:1093-1133`. So the emitted bytes are correct and the *model* is the copy that does nothing — editing `log: "BUILD_LOG"` changes no output. The tell §5 names exactly: the spec can be edited while the realizer keeps doing its own thing.
- **It is a divergence, not one bug:** the ordinary `Run` emit path refuses the same field fail-closed and typed (`^orch_emit_run_capture_unsupported`, `:230`/`:249`). Same field, two paths, opposite dispositions — the fix direction is one shared refusal authority.
