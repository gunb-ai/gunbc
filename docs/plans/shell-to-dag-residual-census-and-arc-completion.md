...
| `host_codex_runtime_provision_script.dag` | concat-built observe/ensure/read-back for `ProvisionCodexRuntime` | `host_effect_apply` retained-script transport (`codex_runtime_provision_shell_script`) | typed observe/effect operations; operator-deferred A5 (#8225) |
...
| `host_effect_realize.dag` · `ProvisionCodexRuntime` | `host_codex_runtime_provision_script.dag` (3 `codex_runtime_*_body` + `codex_runtime_provision_script_body`) | decompose | **A5** srv* deprioritized (PR #8225 — fleet codex runtime ensure; dissolve-on #5828) |
