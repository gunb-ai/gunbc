...
A distinct axis from §1's *emission* census: ops whose **interface shape is right** but whose **single hardwired transport is wrong** — the verbatim §3(b) "single hardwired transport is the N×M-adapter trap" tell. The shape belongs to the dependency; the transport is a Realization *handler, one of N* (§2). Each has exactly one handler — `shell` — where a **native in-process handler** is correct when locality is `OnTarget`, and `shell`/`ssh` belongs only to a target that is another process on another host.
...
**Two consequences are load-bearing.** Local process execution has **no shell and no quoting** — the v1 seed realizes `transport shell` as `Command::new(&argv[0]).args(&argv[1..])`, with no shell, word splitting or quoting layer, so the name is a historical DSL keyword, not a description (hence the host-edge carrier is `ProcessArgvExpansion`, not `ShellArgvExpansion` — the §3 nickname class, caught before it spread). And **SSH is a different target**: RFC 4254 carries one command string, and far-side shell interpretation is a deployment capability to be modeled or observed, never assumed.

### What this corrects in this document

§5's framing — "the arc needs only ~4 new typed ops", §5.A's new-op list and §5.B's call-an-existing-op table — is **not wrong about the destination** but uses the vocabulary of the layer #8467 replaces. Typed argv is the terminus for a **CLI-backed** handler only because argv is what the process boundary consumes; it is not the authority a domain module authors against. The §5.B rows stand as written: calling `systemd.Systemctl.ShowProperty` instead of concatenating `systemctl show --property=…` remains exactly right. What changes is one layer in — the op's *own* realization must not be a hand-authored flag list, the defect #8467 measured (a dropped `--fail` across eleven curl sites made a 401 read as success). A §5.B migration is complete at this lane's grain when the concat is gone and the caller names semantics; at #8467's grain when the op's flags derive from cited rows. **Do not treat a §5.B row as blocked on #8467** — the grains land independently, and this lane's row is the prerequisite, not the dependent.

### The measured shape of the remainder (2026-08-19)

Measured on `dag/**` and `src/v2/**`, excluding `/test/`, `*_test.dag` and fixtures — production populations, not witness counts:
...
operations; it stays struck-through rather than dropped, because a census row that vanishes unexplained cannot be
told from one never measured:
...
**Three are absent from the §4 punch-list**, which still calls itself the complete per-instance list at `78f43c38`:
`package_delivery`, `codex_app_server_press` and `bmc_netboot_serve` are post-July production work. That heading's
"complete, current" claim is true at its stated commit and false at this one; it is a snapshot, and this row is
its correction, not a second census.
...
interleaves a typed operation with a retained foreign script is runtime-present because its first half ran.
So the remaining six calls dissolve to typed effects (`retained_runtime`), not the Bash emitter, and the
`dissolves_to` on those sites is wrong. The question is *decidable* without its owners: a typed operation
in the same scope answers it.

**`provider_wire_evidence` is no longer a bridge file at all** and is struck from the 13 above, leaving 12. Its
effects became typed `extdeps.shell` operations under review 50540; what survives is an unused
`import gunbc.retained_shell_script { retained_foreign }`, dead and to be deleted. A module counted as a bridge on the
strength of an unused import is the census direction over-reporting, as §0c records it doing elsewhere.
...
root containing an apostrophe and forked an authority `extdeps.shell` already owned. That landed at one site.
Another `find '…' -type f -name '*.json' | sort` remains in the same module, built by the same concat-into-a-quoted-string
shape, with the same escape exposure and locale-dependent collation. The note is true of its site and reads as
true of the module. **A carrier that records a fix should name the population it fixed**, or the next reader
takes the note as coverage — as happened here, until the count disagreed with the prose.

**The load-bearing reading:** `transport shell` is overwhelmingly an `extdeps` population — 230 of 236 production lines — i.e. it sits *beneath already-typed operations*, in the interface layer, not the intent. This lane's original job, getting shell out of the intent, is substantially done; most of what remains is transport depth inside a CLI-backed cell — #8467's subject, not this document's.

Two cautions against over-reading it. The `dag/gunbc` argv residue is **172 lines across 41 files** — real intent-adjacent work, not a rounding error (an earlier scoping pass quoted the 41 *files* as though it were the line count; recorded so the smaller figure is not re-cited). And a `transport shell` block is not a residual shell program: the v1 seed executes it as `Command::new(&argv[0]).args(&argv[1..])`, so 236 blocks are 236 *process invocations*, of which the shell-shaped ones are the SSH-composed subset.

**Do not turn these counts into a work plan.** Under the replacement-migration rule the unit is a root consumer × subject population, and the census direction reliably overestimates: sizing by argv occurrence, by executable head, then by tool each inflated, because a tool vertical, its site cutovers and its `host_effect_apply` caller are commonly **one** vertical whose acceptance condition is the old site's deletion — not three workstreams. The §5.D domain clusters, each with its own un-defer trigger, are the better unit.
...
SHELL-DAG is this document's; CLI-AUTHORITY is `cli-invocation-emission-design.md`'s. A site can be complete against the first and still owe the second — not a defect in either, but the §0c boundary working. Corollary for dispatch: **a SHELL-DAG row is never blocked on DECIDING its semantic destination.** Its production cutover may share a carrier or target increment with CLI-AUTHORITY — `command_runner`'s is the live case: deleting that argv → quoted text → heredoc route needs the structured process-argv carrier, and its SSH arm needs the nested shell-command target — and those shared increments are implemented once, never bypassed with another shell string. *(Corrected 2026-08-19, review on #8535. This read "never blocked on CLI-AUTHORITY" flat — true of the homing decision, false as a sequencing rule, with the counter-example named in the same document.)*

### A live instance of the forbidden SSH shape

`extdeps.exec.command` `command_over_transport` constructs `ArgvCommand { argv: append(ssh_exec_prefix(target: t), items: command.argv) }` — exactly the `append(ssh_prefix, inner.argv)` composition §0c forbids: SSH is a nested target whose carrier is one RFC 4254 command string, not a prefix on an argument vector. It is production code beneath the generic runner, so every SSH-transported `ArgvCommand` in the tree flows through it; it is the concrete mechanism by which argument boundaries are lost remotely.

It sits in the same file as `gunbc.command_runner`. *(Corrected 2026-08-20: this paragraph previously described `run_shell_command` as rendering the outer `ArgvCommand` to quoted text via `shell_command_render` and posting it through `shell.Exec.Run`'s heredoc transport. That round trip is deleted — `run_shell_command` now asks `command_over_transport` once, per transport, and hands the outer argv directly to `shell.Exec.RunArgv`, with no shell and no per-transport match at the run site.)* `command_runner_dissolution_trigger` still declares the repair its first clause owns: *dissolves when `host_effect_apply` binds `ArgvCommand` execution as a typed transport handler*. So the runner cut and the SSH target were one vertical, not two, and the debt was declared before it was scoped — the SSH target landed in #8596, the runner's own heredoc hop is gone for both arms; what remains open is `host_effect_apply` itself.

### What is owed, and by whom

- The **negative falsifier** — one witness proving a Redfish/REST path reaches no `JqInvocation`, no `CliSurface`, no `ProcessArgvExpansion` and no shell target — is owed by the CLI lane (`sharp-ant-396`), not this one. It guards this boundary better than any further argv census, and per the replacement-migration rule a census over a structure being deleted is the attractor at work.
- The **scoped supersession sentence** in `cli-invocation-emission-design.md` is that document's to place. It supersedes typed argv as the terminal representation *for CLI-backed handlers*; not typed domain effects, typed extdeps operations, host-effect routing, or native realizations.
- **`DESIGN.md` needs no edit, and the reason CHANGED under this PR.** The first draft said its shell → intent open-thread row already routed runtime-present sites to typed effects, so the boundary falsified nothing in it. That row **no longer exists**: #8476 cut DESIGN to ~9.5k words and removed the open-threads section wholesale. What survives, and what the boundary instantiates, is §3's general paragraph — *interface, realization and policy are three facts, not one row*, the transport a Realization handler one of N, the realization-selecting dispatch peripheral rather than in the interface. A CLI-backed handler is that paragraph's shape at the tool seam: a consequence of DESIGN, not an addition, still needing no edit there. *(Rewritten 2026-08-19 while correcting this document's other stale claims — the original clause was accurate when written and falsified by a merge to main two commits later. Recorded rather than silently swapped, because that failure mode is the subject of this very commit.)*

---

## 1. Residual-shell census (current tree)

Five categories. "Genuine emitter" = emits bash that actually runs; "oracle/scaffold" = shell text retained only for a test. *(Corrected 2026-07-26: the oracle/scaffold definition read "`serialize_bash` retained only for a test" — that sidecar is deleted and **no** test retains it, so the definition described an empty class by mechanism instead of role. §4.H is the live statement of this category.)*

### A. Foreign-executor shell — PERMANENT framing (category (a), stays shell)

| site | what | invoked by |
| --- | --- | --- |
| `ci_workflow.dag` | 8 GHA `RunStep.run:` bodies + the floor-peak/cgroup runners (`ci_cgroup_peak_locate_shell`, `ci_floor_peak_{pre,post}_script`). *Corrected 2026-09-05 against `origin/main` @ `70925ee89e`: **"concat-built" is stale for the cgroup-ascent half.** The ascent loop's cond and body (`ci_floor_peak_while_cond_command`, `ci_floor_peak_while_body_command`) and the membership assign (`ci_floor_peak_assign_cgroup_dir_command`) are derived from a typed `NearestAncestorContaining` (`std.path_intent`) plus `extdeps.linux.proc_self_cgroup` membership, lowered by `v2.extdeps.languages.bash_orch_if` / `bash_proc_self_cgroup`, with a located refusal arm rather than a widened default — introduced by `ecbd0863c2a` (#7978, 2026-08-08, ancestor of `origin/main`). What is still raw String on that pipeline is the peak calibration leaves (`ci_floor_peak_test_f_memory_peak_command`, `ci_floor_peak_echo_reset_redirect_command`, and the five `floor_peak_{pre,post}` echo rows) and the runtime-scan transport in `bash_membership_assign_from_source`, which the module itself declares as Phase 2 typed filesystem observation. This row dispatched at least one lane onto already-discharged work.* | GitHub Actions (byte-consumes `run:`) |
...
These run before a gunbc runtime exists on the target, so shell is the honest lowest-dependency medium. **Of the three rows above, only the fresh-standup arm still emits** — the two ubuntu-media files dissolved to in-process typed reconcile and emit no shell (see their rows and the *Landed* note below; corrected 2026-07-26, since "they emit through the v2 bash rows" read as covering all three). The surviving emitter goes through the v2 bash rows (the If-band + the tier-2 Procedure/Let band, #6566; the arith arm had no live site and is being pruned by the consumer slice). **✅ FLAG B1 SIGNED (operator, 2026-07-14, relayed via calm-ferret-849):** the ISO remaster/fetch dissolve to a **typed target**, not a permanent `ShellPayloadRequired` bootstrap roster entry — these scripts execute on the provisioning host where the gunbc runtime IS present, so under the bash-minimization rule they are runtime-present, not bootstrap; `curl`/`xorriso`/`sed`/`sha256sum` become typed argv ops (`sha256sum` behind the `Sha256Verify` interface shape, `dag/extdeps/tools/sha256sum.dag`) and the NoCloud files become typed `Filesystem.Write`. Consequence: the for/heredoc emit band is never needed and the tier-2 band stays Procedure/Let-only. **Landed:** `ubuntu_install_media_fetch.dag` and `ubuntu_seeded_install_media_remaster.dag` no longer import `extdeps.languages.bash.program` / build `ShellProgram`/`RawLine` — both rewritten to in-process typed reconcile (`observe(path) → Observation`), green-by-execution.

### C. Runtime-present shell — MUST become typed via `host_effect_apply` (the real work)

| site | what | current path | dissolves to |
| --- | --- | --- | --- |
| ~~`fleet_converge_emit.dag`~~ **LANDED #7121 (`ac89afbe615`)** | The earlier slice-2 thin emitter and its `.github/fleet-converge.sh` artifact were subsequently deleted together | modeled convergence through `gunbc.fleet_converge_apply.converge_apply_for_host` and `gunbc.host_effect_realize.host_effect_apply` | done — no surviving thin-run golden or bootstrap emitter at this site |
| ~~`host_effect_realize.dag` srv3 shell-string transport helpers~~ **LANDED #8590 (`61bf47bb876`)** | `srv3_transport_witness_bin_success` takes `bin_path` + `List<String>` args; executable and tool-path probes use operation-derived argv. Directory ownership now follows `srv3_ensure_directory_owned_by_current_user` → ownership decision → `srv3_chown_then_read_back` | typed local operations and `typed_argv_exec_over_ssh` / `typed_argv_exec_over_fleet_ssh`; unsupported transports and non-root remote workdirs refuse | done for these helpers — this does not discharge the surviving srv* actuator graph (§4.D) |
| ~~`host_build_cache_provision_script.dag`~~ **FILE DELETED — A5 un-deferred, bounded to the effect-grain swap only** | `EnsureBuildCacheInstance { desired: BuildCacheInstance }` reaches `realize_provision_build_cache_body`, which surveys the endpoint and calls `admit_ensure_build_cache_instance` before mutation (**admission wiring LANDED #8825, `40f2fb60d39`**). The concat-built `build_cache_*_body` functions remain in `host_effect_realize.dag` | admitted changes reach `build_cache_ensure_step` → `run_shell_transport`; a second endpoint survey feeds `provision_build_cache_verdict` | typed observe/effect operations replacing the remaining shell construction; admission wiring does not discharge that conversion |
| ~~`host_hygiene_{reaper,liveness}_script.dag`~~ **LANDED #8583 (`ffa16a55bc9`)** | Both script files deleted; reaper observation reads typed systemd properties and cgroupfs/procfs, while liveness folds typed `systemd.Journalctl.UnitLog` output against a clock observation | `host_hygiene_reaper_observe` / `host_hygiene_liveness_observe` enumerate units through `systemctl_list_units_all_states`; separate reaper remediation uses `posix.Signal.TerminateProcess` and `shell.Remove.RecursiveForce` | done for the shell migration — LocalShell observation; reaper occupant-classification scaffold and wet/live remediation coverage remain separate obligations |
| ~~`host_codex_runtime_provision_script.dag`~~ **LANDED** | concat-built observe/ensure/read-back for `ProvisionCodexRuntime` | typed observe/materialize/read-back argv steps in `host_effect_realize.dag` (`gunbc.WitnessBin.Run` / `gunbc.Cli.Run` locally, `run_typed_argv_transport` over SSH/FleetSsh), verdict folded in `host_codex_runtime_provision_verdict.dag` | done — file deleted in the same cut |
| `live_deploy/emit.dag` | mutation operations remain raw `Run.command` leaves; #7315 deleted #7298's concat-built preflight and routes the roster-derived principal/sudo probes through typed effects before mutation | generated deploy scripts invoked from GHA, with gunbc already present | interpret the remaining mutation operations as typed host effects; the outer GHA thin invocation may emit Bash, the runtime operations may not (§4.J) |
...
| ~~`host_effect_nbd_proxy_serve.dag` systemd-run `WitnessBin` scaffold~~ **LANDED #8582 (`74b9a6e6a51`)** | `host_effect_nbd_proxy_serve_disposition` is `Terminal`; the former scaffold and dissolution trigger are retired. The 2026-07-14 operator ruling still excludes `trap`/`&`/`$!` `ShellStmt` vocabulary | start through `gunbc.systemd_run_transient.systemd_run_transient_read` (`RunTransient`), stop through `gunbc.systemctl_stop_run.systemctl_stop_run` (`Stop`), observe the transient unit through `gunbc.systemctl_is_active_read.systemctl_is_active_read` (`IsActive`), grounded on the cited `extdeps.systemd` operations | done — typed argv dispatch and unit observation replace the scaffold and unconditional inaccessible-port stub; captured active/inactive/refused results map to `PortObserved` / `PortAbsent` / `PortObservationRefused` |

The surviving srv\* dissolution triggers are unanimous: runtime-present raw transport dissolves into typed operations on `host_effect_apply`; the common runner `gunbc.shell_bash_runner.shell_exec_via_bash` (the `bash <<'GUNBC_BASH_EOF'` heredoc framing) dissolves when no caller passes a raw script. The former `srv3_install_diagnostic_checklist_shell_runner_dissolution_trigger` carrier was deleted with that dead reconcile/checklist subgraph; do not cite it as live.

### C-note — a dead verdict fold discovered adjacent to A5, filed not fixed (2026-08-20)

**Historical investigation; admission wiring subsequently LANDED #8825 (`40f2fb60d39`).** The current `gunbc.host_effect_realize.realize_provision_build_cache_body` surveys and admits before mutation, and `build_cache_ensure_step` surveys again for `provision_build_cache_verdict`. The zero-caller and unwired-path findings below describe the earlier tree, not the current dispatch obligation; the remaining shell construction is recorded in the table above.

**Not a shell-residual row** — a dead-code finding surfaced while deleting `host_build_cache_provision_script.dag` (row above): the module names differ by one suffix (`host_build_cache_provision.dag` vs `host_build_cache_provision_script.dag`), hold unrelated concepts, and one was edited while the other was deleted in the same PR — a naming hazard that produced a real false-negative dependents-census answer in that PR's review. Recorded so the next person near either file is warned where they stand, and so the fold is sequenced deliberately rather than appended to an unrelated vertical.

`dag/gunbc/host/host_build_cache_provision.dag` carries a full catalog-gate/verdict fold — `BuildCacheDaemonObservation`, `provision_build_cache_verdict`, `placement_verdict`, `build_cache_provision_gate_accepts` — with **zero non-test, non-self-file callers tree-wide**, confirmed by grep on both `main` and `session/proud-swift-703` (2026-08-20). The zero-caller count reproduces. **The characterization that followed it did not survive re-measurement, and is corrected below** — this is an *unconsumed authority*, not dead code. PR #8598's arm-swap touched only the two effect-constructor functions at the bottom of the file (mechanically required by the `HostEffect` rename) and did not create, fix, or reach this fold.
...
- `dag/test/claim/build_cache_placement_observation_test.dag` — its real subject, `placement_of_endpoint_observation` (in `build_cache_endpoint_observation.dag`), is **live and unrelated to this fold**. Only its `refuses()` helper (used by 3 of 8 witnesses) reaches into the dead fold's `placement_verdict` as a boolean-collapse over a `BuildCacheServerPlacement`. Deleting the fold without rehoming `refuses()` onto a live equivalent would silently drop those 3 witnesses' pass/fail assertion, not just their subject — the "reaches its subject through the fold while asserting something else" case, not deletion population.

**CORRECTION (2026-08-20, same day, before any deletion was made): the disposition above is wrong and the fold must NOT be deleted.**

The zero-caller measurement reproduces, but does not establish that the fold is *unwanted*; the module's own receipt, read rather than the grep output, shows the opposite. `lane_e_srv1_ci_noncompletion_root_cause_receipt`, in the same file, names `placement_verdict` **as the fix authority for a live incident class** — "FIX AUTHORITY: ServerManagedUnitOwned on the managed compile-pool slice — ServerRunnerOwned on ANY lifecycle (active or retired) must refuse ProvisionConverged (`placement_verdict`, `build_cache_placement_observation_test` `witness_both_runner_lifecycles_refuse_at_the_verdict`)" — for the stranded-sccache-server defect that killed CI builds at the 15m compile ceiling. Its own status: "INCIDENT CLEARED, NOT REPAIRED ... the class can strand again the next time a slot retires while holding the server."

Nor is the module dead: nine production modules import `gunbc.host_build_cache_provision` (`extdeps.cache.sccache`, `build_cache_ensure`, `build_cache_instance`, `build_cache_unit`, `fleet_host_budget`, `host_effect_realize`, `host_layout`, `host_standup`, `runner_capacity_realize`), consuming other declarations in it. A symbol-level zero inside a heavily-imported module is exactly the shape that reads as dead from a whole-file glance and is not.

So the finding is **strictly worse than dead code, and a §4b rung problem rather than a §2 redundancy problem**: fold, gate and witnesses exist and pass, and nothing on the provisioning path asks any of them; the named remedy is computed by a function no production caller calls. Green witnesses prove the fold *correct*, not the class *guarded*; reported as the latter, the class reads a rung above where it sits — the inflation §4b calls worse than sitting low, because an inflated class never ranks for climbing.

**What the deletion would have cost, stated because it very nearly happened:** it would have applied cleanly, taken the witnesses with it, broken nothing, and removed the named remedy for a recurrable CI-capacity incident — leaving a prose receipt pointing at a symbol that no longer exists. The witnesses' greenness made removal look safe.

**Next step, replacing the one this paragraph supersedes: wire it up, do not delete it.** Find the provisioning path that should be gated and establish why it is not — a different job, size and owner than a deletion. First test the one benign possibility: that the path is gated by an equivalent check under another name rather than genuinely ungated. The `refuses()` rehoming note above is moot if the fold stays; retained in case a future measurement re-establishes the deletion case.

**That first test is now answered, on `origin/main` at 2026-08-20, by reading `host_effect_realize.dag`, `build_cache_ensure.dag` and `host_build_cache_catalog_gate.dag` directly. The benign explanation is refuted: the path is genuinely ungated for placement.**

`realize_provision_build_cache` has exactly one gate before dispatching to the body — `provision_build_cache_catalog_gate_verdict(catalog_id: instance.implementation)` — and that function, in a third module, is pure id validation: unknown id refuses, an id other than `sccache_local` refuses, otherwise realizable. It never observes placement, principal, unit ownership, or lifecycle. So there *is* a check under a different name, and it is **not equivalent**: passing it says nothing about whether the daemon is stranded in a retired slot's cgroup, the class the incident receipt names `placement_verdict` as the fix for. `host_effect_realize.dag` matches none of `placement_verdict`, `provision_build_cache_verdict`, `build_cache_provision_gate_accepts`, `admit_ensure_build_cache_instance`, or `EnsureBuildCacheAdmission`.

**Two unconsumed authorities, not one — and therefore one wiring job, not two.** `build_cache_ensure.dag` carries a second admission authority, `admit_ensure_build_cache_instance`, with its own `EnsureBuildCacheAdmission` / `EnsureRefusal` / `EndpointPreflight` vocabulary and no production caller. Both name the *same* next consumer: `realize_provision_build_cache_body`. That materially reduces the sizing this note originally implied.

**The tree already recorded this, and the record has not gone stale.** `ensure_effect_grain_migration_note` in `build_cache_ensure.dag` states it in terms — "RENDER IS NOT ADMIT ... NO PRODUCTION CALLER IS NAMEABLE TODAY ... The nameable next consumer is `realize_provision_build_cache_body` ... once it is rewritten to observe `BuildCacheServerPlacement` and `EndpointPathState` for the target host and route through this admission instead of unconditionally rendering and restarting the unit." Verified against the code rather than quoted, because a correct note goes stale silently; it has not. The scope boundary that produced the gap was a deliberate ruling on #8598 (arm-plus-render only, explicitly refusing to wire a consumer that did not then exist) — correct for that PR, and now the open obligation.

**Not established here:** what it costs to observe `BuildCacheServerPlacement` and `EndpointPathState` at realize time. That is the actual content of the wiring job and the reason it was deferred rather than done; this note does not size it.

**LANDED 2026-08-21 — both authorities are consumed, and what it cost is recorded rather than left as the open question above.** The missing piece was a producer, with no cheap version: `build_cache_endpoint_observation.build_cache_ci_endpoint_observed_producer` returned `ProcTableUnreadable` in every arm because it takes a `HostIdentity` and so has no transport to thread through. The producer now lives in `gunbc.build_cache_endpoint_observe` as a PURE DECODE of read text into `BuildCacheEndpointObservation` / `EndpointPathState` / `ObservationCurrency`, and `gunbc.host_effect_realize` performs the reads through the transport it already holds — five to nine single-argv, non-mutating reads per pass (`cat /proc/sys/kernel/random/boot_id`, `stat -c %F` on the endpoint path, `cat /proc/net/unix`, the `find` fd-holder walk, then the owner's `cgroup`, `stat`, principal and `exe` only when a single owner resolved). Locally via `gunbc.command_runner.run_shell_command_capture`; over SSH via `run_typed_argv_transport`. No shell body was added: the reads land on the typed-argv side of `#5828`, and the ensure step's concat-built script is untouched, so the A5 row's dissolve-on is unchanged.

Both authorities are consumed, on opposite sides of the act, without duplicating each other. `build_cache_ensure.admit_ensure_build_cache_instance` is the PRECONDITION and runs before the first mutation — `EnsureAlreadyConverged` returns converged with no unit written and no restart issued, removing a per-pass reinstall that re-colded every compile on the host; a runner-owned endpoint refuses with the owner located, which is the incident class. `host_build_cache_provision.provision_build_cache_verdict` is the POSTCONDITION over a re-observed placement after the ensure ran, catching an ensure that reports success while the endpoint ends up owned by something else.

**A CAPABILITY BOUNDARY WAS CLAIMED HERE AND IS REFUTED BY MEASUREMENT — read this before concluding anything in this lane is impossible.** An earlier revision of this row stated that the fourth stale-socket observer clause (no `/proc/*/fd` entry still resolves to the socket's inode) needs a glob over `/proc`, therefore a shell, therefore emitted-shell growth this A5 row may not do — so `EndpointStaleSocket` could never be produced and srv4's stranded socket could only ever refuse. **That is false.** The walk is one argv with no shell:

```
find /proc -maxdepth 3 -path "*/fd/*" -lname "socket:?INODE?"   ->  /proc/<pid>/fd/<n>
```

Executed 2026-08-21 against a live listening socket, reproduced independently by a second session, with the two controls that are the measurement's content: the bracketed form `socket:[INODE]` matches **nothing** (find reads `[...]` as a glob character class), and a non-existent inode returns **empty**. The original error was exactly that trap — the bracketed probe returned nothing and "found nothing" was taken as proof the walk was impossible. **A zero with no nonzero beside it in the same run is not a measurement**; the refutation is stated rather than the claim dropped, because a silently removed impossibility leaves the next reader no reason to try either.

So the shipped producer walks the receipt's chain — `/proc/net/unix` (exact path, `St==01`) → listening inode → the `find` walk → owner pid → `/proc/<pid>/{cgroup,stat,exe}` — and `ss` is not used, matching both `gunbc.build_cache_instance.endpoint_ownership_note` and the step-1 placement receipt, **which were right as written and are unamended.** All four stale-socket clauses are established, so `EndpointStaleSocket` is produced and the admitted unlink is reachable: srv4's `/var/lib/ctrl/sccache-ci/server.sock` becomes an admitted unlink rather than a permanent refusal. Genuinely unmodeled: TCP-endpoint ownership (`/proc/net/tcp` with its own hex address decode); no `BuildCacheInstance` in the tree carries a TCP endpoint, and that arm is a positive typed refusal, not an untested path that answers.
...
- The ratchet was **pruned with it** — `plans/shell_emission_model.dag:50` records it as *"the vacuous bash-program importer-count ratchet is pruned with it (resolve fails before the ratchet could fire)"*. Verified: `bash_program_importer_count` now appears **nowhere in the repository except this document**, so the "baseline = 19" figure measured a mechanism that no longer exists.
...
> **Retitled and re-anchored 2026-07-26 (`review 43477`).** This section was framed *"to `program.dag` deletion"*, with deletion as the join the two tracks converge on. **That deletion already happened** — #6831, Phase 0 — so the goal is history, not pending. The tracks below are still the live work; only the terminus was stale. Do not dispatch anyone to "delete `program.dag`".
...
- **P3:** `local_tidy_spec` pre-push hook + cron lines stay **shell** permanently (foreign-executor roster entries, category (a)). These never dissolve — the honest residue. *(Corrected 2026-07-26: this row said they stay `serialize_bash`. They cannot — `serialize_bash` no longer exists. What is permanent is that these sites **stay shell**, emitted through the v2 bash rows like every other foreign-executor site. "Permanently shell" ≠ "permanently on the deleted sidecar".)*
...
**Superseded 2026-07-26 (`review 43477`).** This section described the arc's terminal step as: importers drain → the `bash_program_importer_count` ratchet hits its floor → `program.dag` + `serialize_bash` delete. **None of that is pending.** The sidecar was deleted outright in #6831 Phase 0 and the ratchet pruned with it as vacuous (resolve now fails before it could fire); the identifier survives nowhere but this document. The join is not a deletion the tracks build toward — it already happened, ahead of them.
...
The construction half of the global transport wall has **since closed too**: `TransportScript` is a **`sole_constructor` record** whose single mint `transport_script_seal` is `admit_callers`-sealed to exactly two production declarations (`gunbc.retained_shell_script` `retained_shell_script_to_transport`, `gunbc.bash_materialized_transport` `bash_materialized_transport`), and the cast form closed with it — `04_infer` `sole_constructor_construction_diags` judges a cast into a sealed type (#7962) — so neither a record literal nor a cast can produce one outside those bridges. What remains is not a construction hole but a **String residue** — both admitted bridges take a bare body `String`, so arbitrary text still reaches a transport through a counted, reason-bearing, `dissolves_to`-carrying call. That is conspicuous rather than impossible, and dissolves with the per-site migration, not more wall work. So Track 1's **deletion** terminal and Track 2's **construction** terminal are both behind us; what remains is the per-site migration itself.

*(Corrected 2026-07-27, `review 43608`. This read "has also **already landed** … is the sole mint. So Tracks 1 and 2 feed neither a deletion nor a wall; both terminals are behind us." That declared the narrower `ShellOnHost` wall to be the completed **global** transport wall — a §3 violation, the planning authority overstating a landed scope, with concrete misdispatch risk: a worker reading this routes away from the still-open meta-exec confinement work, exactly the harm this document exists to prevent. This is the **fifth** site of this claim corrected in this PR; the first four were fixed while this one survived because each pass swept for sentences already seen rather than enumerating every sentence asserting the fact. Standing rule, stated so it outlives the individual corrections: **enumerate by claim across the whole authority set, then verify each; a correction that fixes only the site under review is not finished.**)*

*(Corrected 2026-07-26, `review 43486`. The previous sentence here — written by me one round earlier while fixing the `program.dag` staleness — said the terminus was to "brand `TransportScript` … and activate the inert lens". Both halves were already dead, for *different* reasons — the brand **refuted by execution** (transparent brand, §5.E's ruling block), the lens directive **overtaken by completion** (#7184 had already activated it). A refuted plan and a completed one both read as "stale directive", but only the first means the idea was wrong. Fixing a stale directive with a differently-stale directive is the failure mode this document keeps reproducing; see §4.G's method note.)*
...
§1–§3 record *direction* at #6507; this section is the **complete, current, per-instance list** so every shell-string-construction site is tracked to closure. Grounded against `origin/main` @ `78f43c38`.
...
> **Anchoring (addresses review 41399 / DESIGN §6):** rows are keyed on **file + symbol name** (variant, fn), NOT line numbers — a line-numbered prose ledger drifts from the tree within a commit (it did: the first draft was mis-grounded on a divergent worktree that still had the pre-#7006 `ShellCommand{script}` sites; on `origin/main` those are already the nickname variants). Symbol anchors are stable across line moves. Grounded at `origin/main` @ `78f43c38`; an interim tracker that dissolves into the enforcement lens (§4.F), the drift-proof authority.
...
| ~~`host_effect.dag` · `ReadEffectivePosixPrincipal`, `SudoNopasswdExecuteProbe`, `SudoNopasswdGrantListProbe`~~ **LANDED (#6946/#7315)** | ~~`host_effect_deploy_access_probe_script.dag`~~ and ~~`ci_deploy_access_emit.dag`~~ deleted | `access.PosixEffectivePrincipal`, `sudo.NopasswdExecuteProbe`, `sudo.NopasswdGrantListProbe` | **A3 DONE.** #7315 corrected #7298's later raw-string regression: roster-derived typed effects execute at the runtime-present access gate; the generated deploy artifacts contain no embedded authorization probes. |
...
| ~~`host_effect_realize.dag` · `ProvisionBuildCache`~~ **ARM SWAPPED, builders relocated** | ~~`host_build_cache_provision_script.dag`~~ **file deleted**; the 6 `build_cache_*_body` fns live in `host_effect_realize.dag` now, matched on `EnsureBuildCacheInstance { desired: BuildCacheInstance }` | decompose | **A5 partially un-deferred** — the effect grain swapped (bounded operator clearance), the shell body did not convert to typed argv; `#5828` still open, `admit_ensure_build_cache_instance` still not wired into realize (render-is-not-admit) |
| ~~`host_effect_realize.dag` · `ProvisionCodexRuntime`~~ **LANDED** | ~~`host_codex_runtime_provision_script.dag`~~ **file deleted** — no such path in tree | typed observe/materialize/read-back argv steps (`gunbc.WitnessBin.Run` / `gunbc.Cli.Run` locally, `run_typed_argv_transport` over SSH/FleetSsh); verdict folded in `host_codex_runtime_provision_verdict.dag` | **A5 DONE** for this row — the other srv* A5 rows above remain deprioritized |
| ~~hygiene reaper/liveness~~ **DELETED #8583** | ~~`host_hygiene_reaper_script.dag` (4 `…_body`), `host_hygiene_liveness_script.dag`~~ **file deleted** — migrated to typed `host_hygiene_reaper_observe.dag` / `host_hygiene_reaper_remediate.dag` / `host_hygiene_liveness_observe.dag` | typed observe/remediate modules | **A5 DISSOLVED #8583** — CASE 2 (file deletion upstream): script file deleted, construction migrated to typed sub-modules. No direct successor body names. |
...
Both are deleted in this true-up (deletion is the receipt; a "done" row beside a live concat is the relocation pattern this census exists to catch). Two things make this worth recording rather than quietly fixing:
...
| `dag/gunbc/instruments/{host_prelude,gunbc_ci,~~emit_host_gate~~,merge_admission_stamp}`, `gunbc/tools/review.dag` | witness/CI transports invoked from `claim_executor` | typed `WitnessBin.Run`/argv (host_prelude precedent) | A4 — `emit_host_gate` **DISSOLVED 2026-09-13 (gentle-gull-238), CASE 1** — its narration shell-out (`shell.Exec.Run` of `shell_printf_line`, the module's only `retained_foreign` site) is deleted outright, no successor transport: the per-smoke verdict line it printed is carried typed by the `ExitFailure.reason` field end to end (`cli_run.rs` `ClaimOutcome::ExitFailure` → claim_batch FAIL row → terminal-ledger detail under the `exit-failure` tag), which is the first named arm of its own dissolution trigger — the scaffold and trigger are deleted with the site |

### 4.D — `ssh.Session.Exec` command-string (vs typed `ExecArgv`)

**RECLASSIFIED by the 2026-07-26 true-up — read before picking this up.** Every `ssh_session_exec(command:)` site below sits inside an **`srv3_*` function**, i.e. inside the srv\* cluster the operator wound down 2026-07-22 and authorized for retirement 2026-07-24 (§5.D roadmap item). So 4.D is **not** an independently-dispatchable A1 bucket: dissolving these to `ExecArgv` would be typed-argv work on a subgraph slated for deletion — motion, not progress. **4.D inherits A5's deferred status** and is picked up only as part of the srv3 retirement, or if that retirement is abandoned.
...
### 4.F — bottom transport (Phase-3 WALL — **LANDED at construction on both edges**: `ShellOnHost` record wall #7184, `TransportScript` seal #7962)

> **Trued up 2026-07-26 (`review 43486`).** This table described the wall as *pending* and directed Phase 3 to "brand `TransportScript`" across "26 sites". **The `ShellOnHost` half of the wall landed on 2026-07-24 and branding was proven ineffective by execution** (the `shell.Exec.Run` sink stayed open until #7962 sealed it — see the row below) — both facts were already recorded in §5.E's ruling block ~275 lines below, which superseded this table in place without updating it. A worker reading the action column would have executed superseded work. Corrected against the tree.

| surface | status | action |
| --- | --- | --- |
| `transport_script_from_body(body: String)` | **DELETED** from `extdeps.shell.exec`. Not 26 live sites — **zero**. Surviving occurrences are a note recording its replacement (`retained_shell_script.dag:15`), two compile-**RED** controls asserting the deleted-minter fake *fails to compile* (`transport_script_wall_compile_red_test.dag`), and v1-seed Rust detector code | **none — done for the `ShellOnHost` edge.** Its realization takes a counted `RetainedShellScript` record. Direct `shell.Exec.Run` closed at construction with #7962: `TransportScript` is a **`sole_constructor` record** whose single mint `transport_script_seal` is `admit_callers`-sealed to exactly two production declarations (`gunbc.retained_shell_script` `retained_shell_script_to_transport`, `gunbc.bash_materialized_transport` `bash_materialized_transport`), and the cast form closed with it — `04_infer` `sole_constructor_construction_diags` judges a cast into a sealed type (#7962). The residue is the bridges' bare-`String` body parameter, counted at every call, not a construction route |
| `TransportScript` branding | **abandoned deliberately, and now historical.** It *was* `= String where brand(…)`, a transparent brand `peel_nominal_alias_identity` peeled to its base, so branding alone could never make a hand-join a type error | **superseded twice.** The record wall closed `ShellOnHost`; #7962 then made `TransportScript` itself a `sole_constructor` record with an `admit_callers`-sealed mint, closing direct `shell.Exec.Run` and the cast form. Nothing here is pending |
...
**Corrected @ 2026-07-26 (snappy-moth-330), in two passes — read the method note below before trusting or editing this section.** The original row read *"in 11 files … + the two ubuntu-media files + `nbd_proxy_serve`"*, overstating the live surface and costing an audit a wrong prerequisite (see the trap note). The first correction pass still listed seven files as live `ShellProgram`/`serialize_bash` construction sites — **also wrong** (`review 43441`), the same way: every one of those hits is prose. The second pass scoped the occurrence count to `.dag` files, missing the frozen `.diff` fixtures (`review 43455`). Re-censused hit-by-hit, unscoped, over `origin/main`:
...
**Method note, because this row has been wrong three times with one cause.** `git grep -l` answers *"which files contain this string"*, **not** *"which files construct this thing"*. **Four** distinct classes match the string and none is a construction site:
...
- **(d) frozen historical artifacts** — recorded `.diff` fixtures under `src/v1/stage0/testdata/`, capturing the tree as it *was*. These match forever and must **never** be "cleaned up": they are test data, and editing them corrupts the fixture. The sharpest case is a `-` **deletion** line (`-  RawLine,`): it matches a search for the symbol *because it records the symbol's removal* — the strongest available evidence of absence, and the easiest to miscount as presence.

Error history: "11 files" originally; seven files as live sites in the first pass, when the true count is **zero and the declaring module does not exist**; the second pass scoped the occurrence census to `dag/**` and `src/v2/**`, missing class (d) entirely (caught by `review 43455`). **Scope note, so the claim is checkable — and it must exclude this file (`review 43530`).** The counts above are over the *whole repository, all file types*, **minus this document**, which is a census *of* these names and therefore full of them: an unscoped `git grep -- .` returns a larger number, because this file's own corrections contribute 11 `RawLine` lines of pure self-reference. The reproducible command is:

```
git grep -o RawLine -- . ":!docs/plans/shell-to-dag-residual-census-and-arc-completion.md" | wc -l   # → 6
```

Two further reproducibility traps, since a census's whole value is that its numbers can be re-derived: `git grep -c` counts **matching lines** while `git grep -o | wc -l` counts **occurrences** — they differ wherever a line mentions a name twice — and a count taken before a later commit will not match one taken after. The figure above is occurrences; the per-file split is `ubuntu_seeded_install_media_remaster.dag` 1 + `module_grain_affected_dag_only_6edafbb.diff` 4 + `module_grain_affected_v2_only_bb6e656.diff` 1. Before editing this section again: confirm the *declaring* module still exists, search unscoped, and read each hit rather than counting filenames.

**The second rule, which cost more rounds than the grep rule and generalises past greps: a correction is not done until the text it supersedes is GONE.** Across eight review rounds on this document the dominant failure was not a wrong fact but the corrected fact written *beside* the stale one, leaving both — two mutually exclusive instructions with no way to tell which is live, strictly worse than the original error because the stale half now looks reviewed. Instances: §4.F's table kept "brand `TransportScript`" while §5.E already recorded it refuted; §1.A/§1.D kept sibling rows citing deleted vocabulary; the `TeeTo` finding carried two contradictory **dispositions** in adjacent paragraphs, one directing the very fix the other says was rejected (`review 43513`). So, when correcting:
...
**Third rule, and the one that survived longest undetected: say HOW a directive died, because the three ways are not interchangeable.** *Refuted* (the idea does not work — never retry it), *completed* (it already happened — do not re-do it), or *partially superseded* (part landed, part is live residue — the dangerous one, because striking it whole retires open work). All three read identically as "out of date", so a bare strike-through loses exactly what a reader needs. `review 43522` caught two at once in §5.E: the lens directive labelled **refuted** when #7184 had **completed** it, and the typed-argv retype labelled **landed** when what landed was a *different sink* (`RetainedShellScript.body` is a `String`, not `List<String>`) with the typed-argv path still open. The first sends someone to re-argue a settled design; the second deletes live §5.A/§5.B work from the plan.

Confined by `realization_vocabulary_containment` (LANDED #6854). **No dissolution action, but the two halves are no longer the same kind of thing:** the bash-AST *sidecar* vocabulary (`ShellProgram`/`serialize_bash`/`ShellStmt`/`RawLine`) is **extinct** — nothing to dissolve or confine, because the declaring module is gone; only `bash_command_fold`/`bash_build` remains, and that is the replacement machinery proper. This section is now a *negative* result — kept so the extinct names aren't mistaken for construction sites, and so the next reader doesn't re-derive the same false positives from a filename grep.

**⚠ Name-collision trap — do not read this section as covering the bare-line path.** The live "emit one raw line" path is `realize_run_rawline` (`src/v2/std/orchestration_emit.dag:35`), a **method on the `OrchestrationEmitMedium` interface**, realized by `bash_fold_raw_line_target_model` (`bash_command_fold.dag:922`) and reached from `05_emit_orchestration.dag:192` for env-free `Run`s. It merely *echoes the name* of the retired bash-AST `RawLine` node; it is a different thing at a different layer, and very much alive. Consequence, recorded because an audit hit it: a non-command `PipelineStep` (e.g. dissolving `orch_bash_comment_line`) needs **only** a new variant wired to this existing medium method — **not** a new grammar row, **not** a restored `ShellProgram`. Reading §4.G's old "RawLine is confined emit-internal vocab" line as if it described this path is what produced the wrong prerequisite.
...
1. **§4.D is not a dispatchable bucket — it is A5-deferred.** Every `ssh_session_exec(command:)` site is inside an **`srv3_*`** fn (the wound-down srv\* cluster), except `:1169`, the realization core's own `RetainedShellScript` transport. The old table listed verbs without enclosing fns, so it read like an independent typed-argv bucket. It is not; picking it up would be typed-argv work on a subgraph slated for retirement.
...
**Net at the 2026-07-26 snapshot:** with bucket D (§4.E/§4.I) in flight, the non-deferred remainder of this arc was meta-exec confinement. Subsequent truth: merged #7298 added a runtime-present raw-`Run.command` preflight, #7315 corrected it onto roster-derived typed effects, and #7303 initially left source `Run.env` as a Retry silent-drop before #7318 discharged it by explicit nonempty-env refusal with an executing empty-env control. The current owner/trigger roster in §4.J supersedes this snapshot; the srv\* cluster remains operator-deferred.
...
§4 was grounded at `78f43c38`; a three-facet CI audit (2026-07-24) found **five CI `run:` shell sites that landed after that snapshot and are not in the tables above.** Each carries a rich in-code note — tracked *at the site*, never rolled up here. Keyed on file + symbol (the §4 convention). All are foreign-executor (GitHub Actions `run:`), so category **E-emit** (route through the v2 bash rows, a roster not a growth surface) unless a typed-op dissolution is named.
...
| ~~`ci_workflow_run_emit.dag:31` · `ci_native_cache_root_toolchain_segment_command`~~ **DELETED #7436** | `rustc -V 2>/dev/null … ; if empty: toolchain-unresolved` slug | cache-poison fallback — **RESOLVED #7436**: toolchain segment computation reordered after setup-rust-toolchain; refusal instead of fabricated segment. |
...
**Adjacent (NOT shell-string) — cross-ref, tracked elsewhere.** The same audit found a second class the CI shares that is *not* a `ShellProgram`/string sink and so does **not** belong in this census: hand-spelled **config** that should DERIVE from an existing model. The exemplar is toolchain isolation — `ToolchainEnvIsolation = SharedHomeAcrossJobs \| PerJobCargoHome{…} \| HermeticContainer` (`extdeps/toolchain/types.dag`) is modeled and consumed by `sccache`/`fleet_intent`, but the workflow emit never reads `env_isolation`; it hand-spells per-job isolation (build isolates, the ci/gate job shares → the rustup `ETXTBSY` race, documented in PROSE in `ci_floor_gate_toolchain_note` on `ci_workflow.dag` rather than modeled/gated). Same shape for the GHA **cache key** (modeled `gha_actions_cache_facts` key-derivation vs the hand-spelled `ci_cache_key_template`), **artifact** upload/download (un-modeled; the dissolve-on artifact-CacheProvider named in `ci_materialization.dag` is undone), and **permissions** (`std.effect_grant` vs per-job `WorkflowPermissions`). These are the model↔realization-fork open thread + roadmap `2-emit-partition`, not the ShellProgram arc; noted so the audit's findings aren't lost across the seam.

### 4.J — Bucket D foreign-executor emit punch-list @ post-#7216 (wise-crane-222, 2026-07-26)

> **STALENESS BANNER — the CI half of this section is grounded on a workflow that no longer exists (filed 2026-09-05, jolly-deer-148, against `origin/main` @ `70925ee89e`).**
> `.github/workflows/ci.yml` was **deleted** by `611fd027708` (#8283, 'FLOOR-Y cutover: delete the CI floor, rebuild from the `run_required_floor` seed'), confirmed an ancestor of `origin/main`. CI today is one emission, `gunbc.witness_floor_workflow` → `.github/workflows/witnesses.yml`. This document does not mention `witness_floor_workflow`, `witnesses.yml`, or #8283 anywhere, and still names `ci.yml` drift+parse as the byte-oracle in four places, so **every CI row below states its dissolve trigger in terms of a gate that cannot fire.**
> A symbol-existence check over the exact production symbols this section's dispatch roster names found the following are absent from the whole `.dag` corpus (`dag/**`, `src/v2/**`) at that head: `gunbc_ci_floor_only_script`, `gunbc_ci_regen_floor_only_script`, `gunbc_ci_run_script`, `ci_selection_control_script`, `ci_regen_floor_skip_shortcut_script`, `git_fetch_script`, `scheduler_invoke`, `scheduler_invoke_with`, `ci_floor_materialization_receipt_gate_script`, `ci_floor_resolve_receipt_gate_script`, `ci_floor_stamp_merge_admission_script`, `ci_floor_stamp_ambient_exit_command`, `ci_floor_stamp_root_command`, `ci_regen_ensure_rustfmt_path_script` and its eleven named raw leaves, `host_effect_plan_placeholder_effect`, the four `host_hygiene_reap_*_body` builders, and `srv3_chown_directory_to_current_user`. `ci_fmt_gate_line` and `ci_fmt_gate_note` DO still exist, so this is not a blanket sweep of the section.
> **WORKED EXAMPLES — the absent list resolves to a MIX, and that is the point.** Six of the names above already have verdicts, and they do not all point the same way. This is written out rather than left to be re-derived, because the per-row lane's whole difficulty is that these three outcomes are indistinguishable by grep:
>
> | absent name | why it is absent | introducing commit (on `main`) |
> | --- | --- | --- |
> | `srv3_chown_directory_to_current_user` | **renamed AND climbed — both at once, which is the case a two-way discharge/rename split cannot express.** The six surviving shell-string helpers took typed argv over SSH/FleetSsh, and directory ownership now follows a live successor under a different name, `srv3_ensure_directory_owned_by_current_user` (`gunbc.host_effect_realize`). Classifying this as plain "discharged" — as an earlier draft of this table did — loses the successor a reader needs to find | `61bf47bb876` (#8590) |
> | `host_build_cache_provision_script` | **discharged** — the realize path is wired through both admission authorities with the observation producer they were waiting on | `40f2fb60d39` (#8825) |
> | `host_hygiene_reap_install_units_body`, `host_hygiene_reap_classify_and_act_body`, `host_hygiene_reap_stale_override_body`, `host_hygiene_reap_script_body` | **absent because their FILES were deleted** — reaper and liveness migrated onto typed observation and `host_hygiene_*_script` went with them. The four body names never appear in the correcting diff at all, so a name-level check sees only absence and cannot see the cause | `ffa16a55bc9` (#8583) |
> | `git_fetch_script` | **rename, NOT discharge** — `git_fetch_no_tags_shell` and `git_fetch_prune_shell` exist. Identical grep signature to the rows above, opposite verdict | (unadjudicated) |
>
> Read that table before touching the rest of the list. Four different causes — dissolution, file deletion upstream of the name, bare rename, and rename-plus-climb — and no textual property of the name distinguishes any of them. The `host_hygiene` row is the sharpest: the correcting commit does not mention the absent symbols, so the evidence that discharges them is not reachable from the symbol at all. The SS1.C rows for these subjects were corrected separately (#10529); this table exists so the per-row lane inherits the worked examples instead of rediscovering them, since a finding that must be rediscovered is a finding that will be.
>
> **WHAT THIS BANNER ESTABLISHES AND WHAT IT DOES NOT.** It establishes that those names have no declaration in the corpus, which is a fact about spelling, and that the CI workflow the section's triggers cite is gone. It does **not** establish that each of those rows is discharged: a name can vanish by rename as easily as by dissolution — `git_fetch_script` is absent while `git_fetch_no_tags_shell` and `git_fetch_prune_shell` exist, which is exactly the ambiguity a per-row adjudication has to resolve. Nothing below has been re-verified at construction grain except the `ci_floor_peak_*` rows corrected in this same change. **Until a lane adjudicates them row by row with an introducing commit each, no row in §4.E/§4.J may be used to dispatch work without first re-deriving its claim against current `main`** — this document has now dispatched at least four lanes onto already-discharged rows (#10529 corrected five; this change corrects the `ci_floor_peak` set), and that recurrence is the reason for the banner rather than a silent per-row edit.
> Population for that recurrence, stated as a denominator **measured on the merge of `70925ee89e` with `origin/main` `16a702eac3d`, and valid only there** — every later edit that strikes a row through changes what the closed-marker grep counts, so this split moves under you and must be re-derived, never quoted: the census carries **180 data rows** across its tables, of which **43** already carry a closed marker (LANDED / DELETED / RETIRED / dissolved / MIGRATED / EXTINCT), leaving **~137 rows asserting live state**, none of them re-verified here.
> That this split MOVED is itself the demonstration, not a bookkeeping detail: measured one merge earlier the same recipe returned 41 closed / ~139 live, and #10529 (`16a702eac3d`) striking two more SS1.C rows through is the entire difference. A number transcribed into this banner would have been wrong within one merge of being written.
>
> **RE-DERIVE THESE NUMBERS; DO NOT TRUST THEM.** They are transcribed counts, which is the same defect this banner is about, one layer up — so the checks that produce them are named here and the counts are valid only for the head they cite (`70925ee89e`). Run them, do not read them:
>
> ```sh
> # denominator: data rows, and the subset already marked closed
> awk '/^\| /{ if ($0 !~ /^\| ---/) print }' docs/plans/shell-to-dag-residual-census-and-arc-completion.md \
>   | grep -vE '\| (op \(extdeps\)|site|symbol|module|row|#|population|construction site|state|what)' > /tmp/rows
> wc -l < /tmp/rows                                                              # 180 at 70925ee89e
> grep -cE 'LANDED|DELETED|RETIRED|dissolved|DONE|MIGRATED|fully migrated|extinct|EXTINCT' /tmp/rows   # 41
>
> # absent-symbol list: every backticked identifier in SS4.J with no declaration in the corpus
> sed -n '/^### 4\.J /,/^### Dissolution trigger/p' docs/plans/shell-to-dag-residual-census-and-arc-completion.md \
>   | grep '^|' | grep -v '^| ---' | grep -o '`[a-z_][a-z0-9_]\{6,\}`' | tr -d '`' | sort -u \
>   | while read -r sym; do grep -rqE "^ *(fn|data|type) $sym\b" --include='*.dag' dag src/v2 || echo "$sym"; done
> ```
>
> The second check over-reports by construction and its raw output must be read, not counted: at `70925ee89e` it emits **50** candidates, and the ~20 production symbols listed above are what survives discarding the module names (`ci_spec`, `extdeps`, `host_effect_realize`, `bash_orch_if`, `orchestration_bash_emit_support`, `realization_vocabulary_containment`, `ci_materialization_emit`, `host_hygiene_reaper_script`, `host_build_cache_provision_script`), the fn-fragment and commit-sha spellings this banner itself introduced, and rows already struck through as retired. A symbol declared under a spelling the anchor does not match also reads as absent. That is why its output is evidence for a per-row adjudication and never a verdict. The `ci.yml` deletion is the one claim here that is not count-shaped and needs no re-derivation beyond `git log --diff-filter=D -- .github/workflows/ci.yml`.

The per-symbol remaining work and batching plan for §4.E CI sites. **Load-bearing at the time of writing:** every `ci_spec.dag` composer change regenerated `.github/workflows/ci.yml`; drift+parse gate was the byte-oracle. **That oracle is gone with `ci.yml` (#8283); the current byte-oracle is the `witnesses.yml` emission from `gunbc.witness_floor_workflow`, and no row below has been restated against it.**
...
#### A — `merge_admission_produce.dag` (single live concat builder plus the emitted floor tail)
...
| ~~`ci_merge_admission_gate_script`~~ (concat builder) | merge-admission gate step | **MIGRATED 2026-07-28 (#7363, Wave B)** — the builder is deleted; the step's `run:` is materialized from a typed `EffectPlan<BoundOperationInvocation>` (`v2.workflow.ci_merge_admission_gate_emit`) whose argv derives from `git.Core.FetchNoTags` and `gunbc.WitnessBin.Run`. The `ROOT=$(git rev-parse …)` line went with it; `FailFast` is now the carrier's policy (`Continue` refuses) rather than the runner's `-e` flag. Gate policy (merge base, tool entry/function, source roots) stays in `merge_admission_produce.dag` as language-blind rows |
| ~~`ci_floor_stamp_merge_admission_script`~~ | ~~floor tail~~ | **STRIKE — all three raw leaves dissolved #7522 (87a4af3ad0).** `ci_floor_stamp_ambient_exit_command`, `ci_floor_stamp_root_command`, `merge_admission_stamp_command()` all deleted. CASE 1 dissolution. The "PARTIAL #7293" status is stale. |
...
| ~~`ci_floor_materialization_receipt_gate_script`~~ | ~~ci job gate~~ | **DELETED #7470 (b01cdf4d89).** CASE 1 dissolution — receipt gate dissolved with WalkPlan success stages finalization. No direct successor. |
| ~~`ci_floor_resolve_receipt_gate_script`~~ | ~~ci job gate~~ | **DELETED #7470 (b01cdf4d89).** CASE 1 dissolution — receipt gate dissolved with WalkPlan success stages finalization. No direct successor. |
...
| ~~`gunbc_ci_floor_only_script`~~ | ~~ci floor `run:`~~ | **DELETED #9252 (489346fff7).** CASE 1 dissolution — plan/walk CLI surface deleted, including its floor-only composer. |
| ~~`ci_regen_floor_skip_shortcut_script`~~ | ~~regen skip~~ | **DELETED #8406 (3b431f34a9).** CASE 1 dissolution — regen_stage0 root cut; regen skip shortcut dissolved with it. |
| ~~`gunbc_ci_regen_floor_only_script`~~ | ~~regen job~~ | **DELETED #8406 (3b431f34a9).** CASE 1 dissolution — regen_stage0 root cut; regen composer dissolved. |
...
| ~~`scheduler_invoke` / `scheduler_invoke_with`~~ | ~~floor/regen~~ | **DELETED #9252 (489346fff7).** CASE 1 dissolution — plan/walk CLI surface deleted; scheduler invoke removed. |
| ~~`git_fetch_script`~~ | ~~floor/regen/deploy~~ | **RENAMED #6833 (a5477cf5b4).** CASE 3 bare rename — successor is `git_fetch_no_tags_shell` / `git_fetch_prune_shell` in `dag/extdeps/git/git.dag`. Old name cleaned up by #9252. |
...
**Current-main ownership and dissolve triggers after #7293/#7298/#7303, including the later #7290 merge (verified at `c629a4ff0d`; this is the dispatch roster):**

| row | exact production symbols | owner | dissolve trigger |
| --- | --- | --- | --- |
| Merge-admission foreign executor | ~~concat builder `ci_merge_admission_gate_script`~~ (deleted #7363); ~~raw leaves remaining inside the already-emitted floor tail: `ci_floor_stamp_ambient_exit_command`, `ci_floor_stamp_root_command`, `merge_admission_stamp_command()`~~ **STRIKE — all three raw leaves DELETED #7522 (87a4af3ad0). CASE 1 dissolution.** | roadmap `2-emit-partition`; ~~gate half **CLOSED #7363 (Wave B)**; floor-tail half unassigned~~ **CLOSED — entire merge-admission raw-leaf surface dissolved #7522.** | ~~Gate half done: the live gate composer is a typed `EffectPlan<BoundOperationInvocation>` materialized through the canonical Bash medium (`effect_plan_bash_materialize`, Wave A #7356) and its concat builder is deleted; argv derives from the referenced `extdeps` declarations, and a materialization refusal renders a located marker rather than substitute shell. The floor-tail leaves remain separate raw-`Run.command` work and were explicitly out of Wave B's scope. The zero-consumer stamp-script orphan and the zero-writer documentation-only disposition reader/marker/policy family retired rather than gaining invented consumers or dead FileExists/typed-read models.~~ **All raw leaves deleted; the "floor-tail half" is now closed.** |
| CI materialization foreign executor | ~~`ci_floor_materialization_receipt_gate_script`, `ci_floor_resolve_receipt_gate_script`~~ **DELETED #7470 (b01cdf4d89). CASE 1 dissolution — WalkPlan success stages finalization dissolved both receipt gates.** | ~~roadmap `2-emit-partition`; unassigned~~ **CLOSED.** | ~~same, with receipt parsing/refusal represented by typed predicates rather than `sed`/test strings~~ **Rendered moot — both symbols gone; receipt gates no longer exist.** |
| CI-spec foreign executor | `ci_release_build_line`, `ci_fmt_gate_line`, `ci_floor_build_verify_script`, `ci_release_bins_pack_script`, `ci_release_bins_unpack_verify_script`, ~~`gunbc_ci_floor_only_script`~~ **DELETED #9252**, ~~`ci_regen_floor_skip_shortcut_script`~~ **DELETED #8406**, ~~`gunbc_ci_regen_floor_only_script`~~ **DELETED #8406**, `gunbc_ci_deploy_invoke`, `gunbc_ci_heal_regen_invoke`, `gunbc_ci_heal_commit_push_script`, ~~`scheduler_invoke`, `scheduler_invoke_with`~~ **DELETED #9252**, ~~`git_fetch_script`~~ **RENAMED #6833** | roadmap `2-emit-partition`; unassigned; `ci_spec.dag` is load-bearing | `ci_fmt_gate_line` calls `cargo.Build.Fmt`; the remaining rows become typed intent → canonical Bash emission; old composers delete and `ci.yml` drift+parse stays green |
| Phase-1 orchestration fidelity | ~~`ci_floor_peak_while_cond_command`, `ci_floor_peak_while_body_command`~~ **DISCHARGED #7978 (`ecbd0863c2a`, 2026-08-08)** — both leaves, and `ci_floor_peak_assign_cgroup_dir_command` with them, are derived from typed `std.path_intent` / `extdeps.linux.proc_self_cgroup` models via `bash_orch_if`, refusing with a located marker rather than widening. **Still open on the same module, and NOT discharged by that commit:** the peak calibration leaves (`ci_floor_peak_test_f_memory_peak_command`, `ci_floor_peak_echo_reset_redirect_command`, `ci_floor_peak_pre_reset_ok_command`, `ci_floor_peak_pre_reset_unsupported_command`, `ci_floor_peak_pre_unavailable_command`, `ci_floor_peak_post_ok_command`, `ci_floor_peak_post_unavailable_command`) and `bash_membership_assign_from_source`, which are Phase 2 typed-observation work | roadmap `6-shell-intent-phase1`; While-fidelity half CLOSED #7978, Phase 2 calibration half unassigned | ~~model the two live While leaves and move their discriminating witness onto normal cadence~~ (done); the residual trigger is Phase 2 typed filesystem observation through `host_effect_apply` for the calibration leaves; #7318 explicitly refuses nonempty Retry body `Run.env`, BoundedPoll was deleted 2026-08-07 (zero-production-constructor census), and three-or-more (N-level) Retry is refused, so the remaining scope/adoption claim either gains a need-driven consumer with faithful lowering or narrows explicitly |
| Already-routed foreign emit, terminal raw leaves | the §4.E exact output roster: `ci_isolate_toolchain_script`, `ci_pin_rustup_default_script`, ~~`ci_selection_control_script`~~ **DELETED #8283**, `ci_cgroup_peak_locate_shell`, `ci_floor_peak_pre_script`, `ci_floor_peak_post_script`, `ci_cargo_eagain_retry_script`, `ci_release_build_script`, ~~`gunbc_ci_run_script`~~ **DELETED #9252**, `ci_sccache_provider_shell_injection`, ~~`ci_regen_ensure_rustfmt_path_script`~~ **DELETED #8406** (and all 11 named raw leaves similarly deleted: `ci_regen_rustfmt_test_x_cargo_home`, `ci_regen_rustfmt_test_x_opt_cargo`, `ci_regen_rustfmt_test_x_home_cargo`, `ci_regen_rustfmt_test_x_rf`, `ci_regen_rustfmt_command_v`, `ci_regen_rustfmt_export_cargo_home_path`, `ci_regen_rustfmt_export_opt_cargo_path`, `ci_regen_rustfmt_export_home_cargo_path`, `ci_regen_rustfmt_export_rf_dir_path`, `ci_regen_rustfmt_error_echo`, `ci_regen_rustfmt_rustup_which_assign`), `gcp_token_smoke_script`, and `fresh_standup_bootstrap_intent`. | roadmap `2-emit-partition`; Phase-1 owns the While fidelity and scope/adoption subset | retain the correct foreign/bootstrap Bash target, but replace ambient `Run.command` strings with modeled `Do{effect}` leaves and delete each raw producer; for #7290 this is the cited `FileExists` + session-`EnvExport` carrier trigger; do not re-home them in another String carrier |
...
| Runtime `Run.command` / `ShellCommand` terminal | `live_deploy.emit.deploy_raw` fed by `ensure_apt_package_step`, `apply_systemd_unit_write_step`, `apply_tree_sync_unit_write_step`, `tree_sync_restart_step_with_diagnosis`, `tailscale_serve_apply_step`, `install_d_owned_command`, `deploy_apply_preamble_steps`, `emit_artifact_upsert`, `emit_artifact_teardown`, `emit_deploy_teardown_dependency_refusal`, `apply_intent_from_effects`, `retract_intent_from_effects`; ~~`host_effect_plan_placeholder_effect`~~ **DELETED #10509**; the `gunbc.host_effect.ShellCommand` carrier and its total match consumers `host_effect_realize.resolve_host_effect_cell` and `fleet_converge_cli.converge_cli_applied_knob_count` | roadmap `2-emit-partition`, coordinated with the host-effect lane | replace `Run.command` / `ShellCommand` with `Do{effect}` typed operations, migrate every total match, then delete the carriers; never count Pipeline wrapping as completion |
| Deferred runtime srv\* | `host_effect_realize` · `srv3_transport_witness_bin_success`, `srv3_transport_test_executable`, `srv3_apt_tool_present`, `srv3_tool_bin_path`, ~~`srv3_chown_directory_to_current_user`~~ **DELETED #8796 (see §4.D)**; `host_effect_realize` (relocated from the now-deleted `host_build_cache_provision_script`, unit body builder dropped in favor of `gunbc.build_cache_unit.render_build_cache_unit`) · `build_cache_observe_stats_body`, `build_cache_classify_failure_body`, `build_cache_ensure_daemon_body`, `build_cache_read_back_body`, `build_cache_provision_script_body`, `build_cache_provision_shell_script`; ~~`host_hygiene_reaper_script` · `host_hygiene_reap_install_units_body`, `host_hygiene_reap_classify_and_act_body`, `host_hygiene_reap_stale_override_body`, `host_hygiene_reap_script_body`~~ **ALL DELETED #8583 (see §4.A)**; ~~`host_hygiene_liveness_script.host_hygiene_liveness_read_body`~~ **DELETED #8583** | operator-deferred surviving srv\* actuator/runtime graph | un-defer a typed-effect migration with its live consumer; the separate dead install/reconcile subgraph is already deleted (#7233/#7237), so do not polish or resurrect it; build-cache's own trigger is `#5828` — the relocation into `host_effect_realize` is not that trigger firing |
| Direct meta-exec construction | `tools.gunbc_ci.main`, `tools.merge_admission_stamp.pr_number_from_event`, ~~`tools.emit_host_gate.gate_failure_exit`~~ **DELETED 2026-09-13 (gentle-gull-238) — the function's only direct `shell.Exec.Run` construction was the verdict-narration printf (§4.C row above); its body now builds a typed `exit_failure(reason)` whose reason carries the per-smoke verdict line, so the site is gone rather than migrated**, `tools.host_prelude.toolchain_provision_shell_exec`, `gunbc.bmc_netboot_serve.bmc_netboot_run_bmc`, `gunbc.bmc_netboot_serve.bmc_netboot_put_path`, `gunbc.command_runner.run_shell_command`, `gunbc.host_effect_realize.run_shell_transport` — direct `shell.Exec.Run` sites through the two counted bridges | per-site runtime/foreign owners delete their bodies | **construction is closed at both edges** (#7184 record wall, #7962 seal); these rows are migration work, not wall work, and the `Reference`-verb lane is no longer their blocker |

**srv3 deferral — premise measured false, disposition still owed (2026-08-20, measured at `origin/main` `4cec10f6`).** The deferral of the `srv3_*` runtime helpers rested on the premise that the subgraph was pending subsumption by the `EmitArtifactThenThinRun` transport, so migrating its call sites would be wasted work. That premise is false, and the refutation is a typed refusal rather than an absence: all 22 `EmitArtifactThenThinRun` match arms in `gunbc.host_effect_realize` refuse, none accept, and the refusal text names the boundary — *"reserved for ConvergePlan (host-effect Phase D)."*

The control widens the finding past the one file, but **not** to "the transport is unrealized" — that stronger claim was asserted here on 2026-08-20 and falsified the same day by reading every arm body rather than pattern-matching refusal shapes. `EmitArtifactThenThinRun` has **exactly one realizer**: `gunbc.host_effect_realize` `realize_converge_on_host` splits on `gunbc_apply_mode_for_host` — `FreshStandup` refuses as fail-closed Unimplemented; `ExistingHostQuiescentReload` calls `realize_converge_in_process`, *the identical call the `LocalShell` arm makes*, so for an existing quiescent host the transport takes the shell transport's own path. The neighbouring `SshShell`/`FleetSsh` refusals confirm the direction: their text directs the caller to "use EmitArtifactThenThinRun."

Two disguises defeated the pattern-based reads that produced the wrong count, each surviving a scan written for the other: a **refusal shaped as a success-valued record** (`gunbc.hostname_set` answers `HostnameSetCasApplyFailed { stderr: "EmitArtifactThenThinRun unsupported..." }`, which no `NotConverged` scan sees), and a **realizer behind a refusing first sub-arm** (the arm above, whose opening lines read as a refusal). Only reading arm bodies rather than matching shapes survived.

**The denominator both audits used was itself wrong, and the correction belongs here rather than in a footnote.** An earlier revision said "all 24 arm bodies," inherited from the manual audits. Measured at `4cec10f66a3d84cbde631e20ca9feaa31457d88c`, the tree carries **41** `EmitArtifactThenThinRun` occurrences: 1 variant declaration (`gunbc.host_effect`), **36 production match arms** across 14 modules, and 4 in `dag/test/claim/`. Neither manual audit reached 36, and neither mentioned `gunbc.host_effect_codex_supervised_turn`, whose arm answers `CodexSupervisedSessionApplyRefused` — another refusal wearing a record name rather than a refusal type. The **conclusion is unchanged**: exactly one arm realizes, `realize_converge_on_host` under `ExistingHostQuiescentReload`. What was wrong was the count of what had been examined, in a paragraph about examining carefully.

That is the point, not an aside. Two readers who had just corrected each other on method still could not state their own denominator: a hand-assembled roster cannot prove its own completeness, only report what someone remembered to look at. A derived census is not a stylistic preference; it is the only kind that can be wrong *loudly*. The 36/1 partition above is therefore a **dated measurement at a named SHA, to be reproduced by a derived detector and reconciled against — never a fixed law**; migrations are expected to change it.

Everything else does refuse: `gunbc.systemd_run_transient`, `gunbc.systemctl_stop_run`, `gunbc.systemctl_status_read`, `gunbc.systemctl_show_read`, `gunbc.systemctl_is_active_read`, `gunbc.hostname_set`, `gunbc.hostname_read`, `gunbc.spark.serving_converge_apply`, `gunbc.live_deploy.readiness`. The sites that appear to accept — `gunbc.ci_deploy_access`, `gunbc.ci_deploy_target_host`, `gunbc.host_identity_adopt` — are classifiers whose arms return a `Bool` or the variant's own name for totality; none executes the transport.

**The srv3 finding is unaffected by the correction.** The one realizing path is for `HostConverge`, not any srv3 effect: every `Srv3*` arm still refuses, including `Srv3BootOnceReadBackSleep`, `Srv3SeededInstallMediaToolchainEnsure`, and the four `srv3_actuator_toolchain_*` helper refusals. What changes is the supporting sentence, not the conclusion — no longer "the transport is realized nowhere, so nothing is pending cutover" but **"the transport has exactly one realizer and srv3 is not on it."**

What this settles and what it does not. **Settled by measurement:** the srv3 subgraph is not subsumed today and not scheduled for subsumption by any live mechanism, so the deferral has no standing premise. **Not settled — a decision, not a measurement:** whether the `srv*` actuator graph survives at all. The tree cannot answer that; migrating 22 call sites (`srv3_transport_witness_bin_success` 10, `srv3_tool_bin_path` 6, `srv3_apt_tool_present` 4, `srv3_chown_directory_to_current_user` 2) in a subgraph the operator intends to delete by another route would be wasted work under a different premise than the original. The deferral is **due for re-decision, not automatically expired**, and that re-decision is the last blocking input on the largest remaining block in this census.

**Prior art the derived census must supersede rather than extend — `v2.lens.effect_reach` (measured 2026-08-20 at `4cec10f6`).** A lens answering approximately this question exists and may be mistaken for coverage: it classifies host-effect sinks and carries a `ShellExecRunSink` variant, so its subject overlaps the shell census directly.

Its selection is the forbidden seed form. `sink_kind_for_callee` takes a `callee_text: String` and compares it by name equality against a remembered list — `"Run"`, `"shell.Exec.Run"`, `"Exec.Run"`, `"WitnessBin.Run"`, `"Read"` — everything else falling to `UnknownHostEffectSink`. That is *where we currently remember doing so*, not *what interprets bytes as shell*: it cannot see `sh -c`, `sh -s` on stdin, or a wrapper stack such as `sudo -S sh -s` — **the same blind spot this census carried before Spark was found**, where `gunbc.spark.managed_access_apply`'s four script builders reach a host through `stdin_payload` behind a wrapper, unreachable by any callee-name comparison.

Two further facts bound how much weight it can hold. Its classification derives from callee *text* rather than resolved callee identity — the text-scanning substitution the census brief's stop condition forbids. And `v2.test.claim.enforcement_live_witness_test` asserts `lens_contract_is_unbound(c: lens_contract_effect_reach)` — the corpus's own record that the contract has no enrolled consumer. Whatever else it is, it is not gating.

The disposition is therefore **supersede, not extend**: a census seeded from interpretation semantics subsumes this lens's entire sink vocabulary, while extending it would inherit its selection principle. Recorded so the derived cut treats it as prior art with a known-narrow frontier, not an available foundation — and so its `ShellExecRunSink` rows are never read as an existing shell-reach population.
...
| **PR 2** | merge-admission cluster completion + receipt gates (A/B remaining rows); the dead docs-only disposition family retired separately without new read models. |
...
**Adjacent finding — `CaptureSpec` capture is a §3 dual representation (raised wise-crane-222, corrected and verified snappy-moth-330, 2026-07-26).** Not part of either dissolution above. First reported as *"TeeTo and CmdSubst are declared but unreachable"*; that is **wrong for `TeeTo`** and the corrected form is what matters:
...
- **Fix direction changed, and the change is right:** originally filed as "thread `redirect`/`capture` through the retry path". Deleting the lying declaration is better — an inert declaration is worse than an absent one, because it lies.
...
- **#7303 is merged on current main (`5b229afd65`):** it replaces the `Optional<String>` conflation with `Outcome<Run>`, refuses multi-step bodies and distinct redirect/capture cases, validates the template-hardcoded log source, and refuses the 3+ escalation shape whose old template dropped `level1.env`.
- **#7318 is merged on current main (`e8b7117f4d`):** it refuses every nonempty Retry body `Run.env` before dispatch can discard it. The executing `orch_retry_body_env_present_refused_holds` RED proves the nonempty shape rejects; `orch_retry_body_env_empty_accepted_holds` plus the unchanged ordinary Retry goldens prove the empty shape keeps its existing bytes. No accepted Retry body field is silently discarded at this boundary.

### Dissolution trigger for §4

**The construction wall this trigger waited on has PARTIALLY landed (#7184, 2026-07-24)** — it record-walls the **`ShellOnHost` realization edge** (`ResolvedHostEffectCell.ShellOnHost { script: RetainedShellScript }`, `dag/gunbc/host/host_effect_realize.dag:167`) and deletes the free minter `extdeps.shell.exec.transport_script_from_body(body: String)`, so a hand-assembled `String` no longer typechecks *on that edge*, with compile-fail REDs (§4.F, §5.E).

It did not close the `shell.Exec.Run` sink; **#7962 did.** `TransportScript` is a **`sole_constructor` record** whose single mint `transport_script_seal` is `admit_callers`-sealed to exactly two production declarations (`gunbc.retained_shell_script` `retained_shell_script_to_transport`, `gunbc.bash_materialized_transport` `bash_materialized_transport`), and the cast form closed with it — `04_infer` `sole_constructor_construction_diags` judges a cast into a sealed type (#7962). The `meta_exec_confinement` scan and `host_language_transport_script` remain as validation backstops, but construction is no longer what they guard. The leak fixture at `src/v2/test/fixture/meta_exec_confinement_scan/leak/plant.dag` is now **scan input and historical record only** — no longer citable as evidence that the cast compiles.

So *new* instances of the `ShellOnHost` class can no longer be authored; the punch-list is **not** closed at the top.

*(Corrected 2026-07-27, `review 43600`. This read "a raw-string `shell.Exec.Run` / hand-built transport is already unwritable … so *new* instances of this class can no longer be authored, and to that extent the punch-list is closed at the top." **Overstated on both counts** — the wall is real but scoped to `ShellOnHost`, and new instances can still be authored through the brand cast. Declaring the top closed while a writable sink remains is precisely the misdispatch this document exists to prevent, and the third "wall landed" claim in this lane to overstate its scope: the recurring error is checking that a carrier exists rather than that every route into it is closed.)*

What the trigger does **not** discharge is the **existing** rows: the wall stops new concats **on the `ShellOnHost` edge**; it does not dissolve the ones already written, and does not stop new ones at the still-open `shell.Exec.Run` sink. Each remaining row is still discharged the same way — *deletion of the concat*, verified green-by-execution + an injection-RED, never by relocation. This section retires when that list empties, not on further wall work.

*(Corrected 2026-07-26, `review 43494` then `43501`: this trigger read "folds into the `host_language_transport_script` lens **going live**". Superseded on both counts — the lens **was already activated** by #7184 (§4.F), and it was never what closed this class. It is **live but deliberately green** on `ComputedApplication`, redding only a raw literal at an enrolled `Run` position: a backstop for literal blobs while **construction** closes the computed-join class. My first correction here said the lens "is inert by design" — itself wrong, contradicting §4.F in the same edit pass. A lens is *validation*, the `RetainedShellScript` record edge is *construction*, and §5 prefers the second because it makes the state unwritable rather than merely flagged.)*

---

## 5. Method of Action — the bounded path to bash-free user space (calm-ferret-849, 2026-07-22)

**End state:** no user-space `.dag` constructs a shell string. Every "this `.dag` wants to call a bash script" instance resolves to exactly one of the four paths below. The `realization_vocabulary_containment` lens (#6854, LIVE) already forbids bash-AST vocab (`ShellProgram`/`ShellStmt`/`serialize_bash`) in user space. #7184 retyped the **`ShellOnHost` realization edge** to the `RetainedShellScript` record (`host_effect_realize.dag:167`, `retained_shell_script.dag:9-10`) and deleted the free minter `transport_script_from_body`, so a hand-assembled `String` no longer typechecks *there*; what remains possible on that edge is **conspicuous**, not silent — authoring a counted `RetainedShellScript` row with a hand-built body is the *reviewed* escape, carrying a `reason` and a `dissolves_to`, and that visibility is the mechanism, not a leak in it. #7184 did not retype `shell.Exec.Run`; #7962 **closed the `shell.Exec.Run(script: TransportScript)` sink at construction** (§5.E), at the type rather than by the lens: `TransportScript` is a **`sole_constructor` record** whose single mint `transport_script_seal` is `admit_callers`-sealed to exactly two production declarations (`gunbc.retained_shell_script` `retained_shell_script_to_transport`, `gunbc.bash_materialized_transport` `bash_materialized_transport`), and the cast form closed with it — `04_infer` `sole_constructor_construction_diags` judges a cast into a sealed type — so the meta-exec confinement milestone is met at construction; what survives is the bridges' bare-`String` body, dissolved by migration rather than a further wall. *(Rewritten 2026-07-26, `review 43544`. This sentence previously called that sink "the remaining hole", and my first correction **appended** a supersession notice below it instead of rewriting — two mutually exclusive current-state claims in one paragraph, misapplying this document's own rule: strike-in-place is for corrections **illegible without the original**, and this one is not — the four-path end state survives the rewrite intact, so only the blocker claim needed replacing.)*
...
| --- | --- | --- | --- |
| `extdeps.tools.hostname` · ~~`Read`~~ **LANDED** + ~~`Set`~~ **LANDED, then migrated** | `dag/extdeps/tools/hostname.dag` (moved from `dag/extdeps/os/hostname.dag`) | ~~`hostname -s`~~ **done**; ~~`hostnamectl set-hostname`~~ **done** | **Read done** — `os.Hostname.ReadShort` realized via typed cell `HostnameReadOnHost` + `gunbc.hostname_read` (LocalShell → op, SshShell → typed argv); both read consumers dissolved: `host_identity_observation` (`HostIdentityShortHostnameRead`, the `host_identity_short_hostname_script` concat DELETED) and `ci_deploy_target_host` (runtime-present read; the GHA preflight *block* stays E-emit). **Set done at the §5.B grain, then migrated at the #8467 grain** (CLI-invocation lane): `os.Hostname.Set`'s hand-authored `transport shell { argv: ["hostnamectl", "set-hostname", "{desired}"] }` row is **deleted**; the local leg realizes through `extdeps.tools.hostname`'s `HostnameSetInvocation` → `hostname_set_invocation_process_plan` → `hostnamectl.Process.Run` (argv derived from the cited `hostnamectl_set_hostname_verb` row), and the SSH legs' privileged argv reads the same cited verb. |
...
| `extdeps.os.id` · `Read` (uid/gid/user) — **ops landed; row NOT done** | `dag/extdeps/tools/id.dag` (created; landed #7194; post-#7231 home) | `id -u`, `id -g`, `id <user>` — service `os.Id` ops `Uid`/`Gid`/`Lookup` | **Operation model + hermetic operation witnesses have landed** (`test.claim.shell_dag_census_5a_typed_ops_witness`, `witness_os_id_uid_realizes_hermetically`, `witness_os_id_gid_realizes_hermetically`, `witness_os_id_lookup_realizes_hermetically`; declared scope is model-before-implement, call-site migration §5.B a later lane). **NOT "Done"** — that word requires the production consumers to have moved AND the superseded string/argv path to be deleted. Remaining argv/SSH callers are dispositioned independently, not by this row: `host_effect_realize` srv3 probes (raw argv rows `extdeps.tools.id.id_uid_argv`/`id_gid_argv`, absolute-path variants), `fleet_posix_accounts` `probe_command` (NOT-a-shell-site, §4), `spark/managed_access_apply` (deferred, dissolve-on). `id_lookup_argv` RETIRED by ruling (below). |
...
| `dispatch_pipe_pane_emit` · `dispatch_event_pipe_emission` | tmux `pipe-pane` (its input contract IS a shell command) | **RESOLVED** (`review 44063`, refusal shape corrected per `review 44113`). The payload is built as a bash AST (`bash_build_command_from_lits`) and rendered through `v2.workflow.bash_command_fold_serialize` — quoting is the lit-word production's job, so `shell_quote` no longer appears at this site. Reaching the backend required moving the construction onto a realization edge: `dag/gunbc/dispatch_pipe_pane_emit.dag` is enrolled in `realization_edge_path_prefixes` (precedent: `dag/gunbc/live_deploy/emit`). The *seam* stays permanent, as the pre-push hook row above; the hand-joined *construction* is gone. A serializer refusal is a **typed refusal, not a command**: `PipePaneEmission = PipeEmitted { command } \| PipeEmitRefused { detail }` has no arm that yields a command on refusal, so it propagates as `DispatchSpawnRefused { step: "tmux-event-pipe-emit", detail }` and surfaces through the belt's existing `SpawnFailed` carrier — dispatching an un-emittable intent is unwritable, not merely unlikely. Witness: `witness_event_pipe_emits_grammar_quoted_command` |
| `roadmap_provider_events` · `codex_provider_event_projection_filter` | jq (a foreign DSL, not shell) | **declared scaffold** (`review 44058`), trigger in-file at `codex_projection_filter_scaffold_note`: the filter exists only to bound a JSONL line before the substrate reads it, so the dissolution is the substrate-side bounded read (witness-realization lane) deleting BOTH the filter and its `ProviderEventProjectionCapability` jq dependency — not a jq grammar. Interpolated values are two `Int` bound rows, never caller input |
...
| --- | --- | --- |
| C5 access probes | ~~`host_effect_deploy_access_probe_script`~~ and ~~`ci_deploy_access_emit`~~ **deleted** | **DISCHARGED by C5 #6946 + corrective #7315.** The runtime access gate uses typed effective-principal, execute-probe, and grant-list-probe operations; no embedded deploy-script preflight remains. |
| surviving srv* actuator/runtime cluster | `host_effect_realize` srv3 command-string helpers, `host_effect_realize` build-cache shell builders (relocated from the deleted `host_build_cache_provision_script`), `host_hygiene_*` | operator un-defers the surviving actuator graph; the dead install/reconcile subgraph named below is already retired (#7233/#7237) |
...
### 5.E — the enabler that had to come first — **LANDED in two steps**: #7184 closed the `ShellOnHost` string sink, #7962 sealed `TransportScript` and closed direct `shell.Exec.Run`

*(Trued up again 2026-08-19: the heading and premise below were written while the `shell.Exec.Run` sink was open; #7962 closed it, so the present-tense claims are now past tense for BOTH edges. Original correction note follows. (Heading and premise trued up 2026-07-26, `review 43537`. This read "THE ENABLER THAT **MUST COME** FIRST" in the imperative, and the paragraph below was written in the present tense — "can be faked" — as though the sink were still open. The `ShellOnHost` sink is closed, so both are past tense **for that edge** — but the `shell.Exec.Run` sink is **not** closed (transparent `TransportScript` brand, §4.F/§5.E), so the present tense still applies there. (Re-corrected 2026-07-27, `review 43614`.) The §5.A/§5.B rows it gates are the work that remains, not this.))*

Every §5.A/§5.B row **could be** faked at the former `ShellOnHost{script}` edge and at direct `shell.Exec.Run(script)` by joining argv back into a string — sleek-crab #7064 did exactly this (`argv_join(...) + " 2>/dev/null || true"`). While those sinks were reachable from intent, relocation was the path of least resistance and a brief alone could not stop it, so the construction wall was the enabler, **not cleanup-after**. **Both sinks are now walled**: #7184 built the `ShellOnHost` record edge, #7962 sealed `TransportScript`; what survives is the counted string-taking bridges, conspicuous by design. The original plan follows, each bullet's actual fate marked:

> ⚠ **The three bullets immediately below are the ORIGINAL plan, superseded by the ruling + design-correction block that follows.** Kept because the correction is only legible against them. **Each bullet failed differently, and the difference decides what a reader should do with it** — a *refuted* plan must never be retried; a *completed* one already happened; a *partially superseded* one still has live residue. Do not action them as written; read on. *(Flagged `review 43486`; classifications corrected `review 43522` — the earlier version lumped bullets 1 and 3 together as "refuted by execution", wrong for bullet 3 and exactly the conflation §4.G's method note warns about.)*
...
- **Make `ShellOnHost{script}` take typed argv (`List<String>`), not `String`** — **PARTIALLY SUPERSEDED, not landed.** What landed is a *different* sink: `ShellOnHost.script` is now `RetainedShellScript`, a **record whose `body` is still a `String`** (`host_effect_realize.dag:167`, `retained_shell_script.dag:9-10`). That closes the hand-assembled-`String`-at-the-edge hole by construction, but is **not** the typed-argv retype this bullet describes — the ruling block below says so explicitly: *"the typed-argv `List<String>` path of bullet 2 stays §5.A/§5.B's job — the two are distinct sinks, not one."* **Live residue.** *(Corrected `review 43522`: this previously read "held … **landed**", marking open §5.A/§5.B work as finished.)*
- ~~Activate the `host_language_transport_script` lens~~ — **COMPLETED (#7184), not refuted.** The lens *was* activated and is live, redding a raw literal at an enrolled `Run` position. Activation alone was never **sufficient**: it stays deliberately green on `ComputedApplication`, so it could not catch the #7064 computed-join fake — construction closed that class. Do not re-do it; do not read "insufficient" as "wrong".
...
> - **Surviving residue (filed here, CLOSED LATER by #7962):** at the time of this ruling `x as TransportScript`
>   remained writable anywhere, and the block below judged it not closable by a record, needing the
>   **`Reference` verb**. **That prediction was wrong in a useful way.** #7962 closed it without any
>   visibility feature, by making `TransportScript` a `sole_constructor` record AND extending
>   `04_infer` `sole_constructor_construction_diags` to judge the cast form — the language grew the
>   check rather than the carrier growing a guard. Kept because the reasoning is only legible against
>   the original: the error was assuming the cast form was outside what construction could reach.

### Sequence

1. **§5.A** — add the ~4 ops (each cited, typed `exit`, typed-argv transport). Small, finite.
2. **§5.E** — **the construction wall LANDED #7184**, but this step is *not wholly* discharged and the original wording overstated it. What landed: the `RetainedShellScript` **record** edge + free-minter deletion + counted bridges + compile-fail REDs. The branding half was **refuted** (a transparent brand), and the sink it left open was closed separately by #7962's seal. The **typed-argv realization edge is still open** — `RetainedShellScript.body` is a `String`, not `List<String>`, so that path remains §5.A/§5.B work (see §5.E bullet 2). §5.B is already the only writable path for the *retained-script* sink. *(Corrected `review 43522`: previously struck through entirely and marked "LANDED", which retired open work.)*
...
**⚠ Both remaining criteria are ALSO now met — this trigger has fired and needs an operator decision (flagged 2026-07-26, `review 43494`).** The stated condition: dissolve when the `host_language_transport_script` lens **goes live** *and* `program.dag` **deletes**. Verified: the lens was activated by **#7184** (`wall_residue_live_test.dag:4`) and `program.dag` deleted by **#6831**. By its own terms this document should already have dissolved.

It has not, and the honest reason is that the trigger was **mis-specified rather than unmet**. It names two *mechanism* events, but what keeps this document alive is the **per-site punch-list** (§4.A–§4.J): the wall stops *new* concats **on the `ShellOnHost` edge** — not at the still-open `shell.Exec.Run` sink — and does not dissolve the ones already there, whose rows still have owners and open PRs. A correct trigger reads: *dissolves when the §4 punch-list empties* — mechanism landing was necessary, not sufficient.
