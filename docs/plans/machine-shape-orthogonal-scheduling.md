...
**Declared supersession, requiring operator sign-off before this phase lands (review finding 2026-07-15; recorded §9 open-6).** Machine-fact-dependent batch composition conflicts with the signed scheduling authority as written (`bounded-input-cost-envelope-scheduling.md:10-18`: schedule topology stays central and hardware-free; `schedule_eq` preserved across host realizations — the earlier draft claimed compatibility it did not have). This plan does not quietly narrow that invariant; it proposes an explicit extension: **topology becomes a pure function of (computation graph × *declared* MachineShape)**, where "declared" means `Cited`/`DerivedFrom` evidence only — **`MeasuredBy` quantities remain banned from topology** (they stay peripheral: width, admission, the AIMD governor). `schedule_eq` generalizes from "equal across hosts" to "equal given equal declared shape inputs" — determinism as a function of content-addressed inputs, which preserves the invariant's actual §5 content: no runtime-measured topology flapping. If the operator declines, the confined fallback: affinity degrades to a peripheral within-batch *ordering* preference that never changes batch membership. (Composes cleanly with witness-realization P4, #6663: this plan prices *topology* from declared shape; P4 packs *width* from measured peaks — the same MeasuredBy-stays-peripheral split, verified compatible by that review.)
