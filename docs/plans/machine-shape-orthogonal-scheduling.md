...
Acceptance (green-by-execution): witness that one `MachineShape` value with all-`UnknownQuantity` magnitudes is constructible and consumable (the abstract shape is a citizen); witness that the concrete RTX 5090 fill derives a roofline bound; REDs: a `Known` whose evidence ref does not resolve — or whose `MeasuredBy` receipt does not exist — refuses at construction (enforceable now that evidence is carried by construction, revised per review); a `MachineShape` with a dangling `DomainId`/`LevelId` reference refuses.
