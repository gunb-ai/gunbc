...
- Rule (under the supersession): consumers of one `Share`d operand whose footprint is `Substantial` relative to a level's capacity prefer co-batch placement within that operand's residency window; where the readiness frontier permits multiple legal compositions, affinity selects deterministically among them.
