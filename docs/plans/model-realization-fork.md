...
**Closed by re-measurement (2026-09-07, royal-wren-665).** The class registered 2026-08-09 (#8062 trim + bold-eagle-107) claimed extdeps service-operation outputs declared `String from "stdout"` inferred as **FaithfulFreeMonoid / Coproduct** in pure-v2 closures via `rust_corpus_repr`, so ReceiverSelf `trim` refused on the wire-minted receiver. That trigger is deleted (eager-deer-389). Re-running `fixtures/class_b_trim/wire_projection_specimen.dag` through `v1-compiler-tests::wire_projection_method_form_compiles_on_service_op_string_field` on the post-cutover tree: method form compiles on Filesystem.Read `content: String from "content"` (`trim_not_found = 0`, compile not refused) — the same `T from "wire_key"` mint the class named. `shell.Exec.Run` `stdout` remains unmintable in a fixture because `TransportScript` is `sole_constructor`-sealed. The neighbouring control `trim_method_form_fails_on_freemonoid_receiver` still refuses on a hand-declared `FreeMonoid<String>` — a different subject. Do not read the historical registration as a live class.
