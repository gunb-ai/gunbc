...
- **The bootstrap arm is typed and cannot silently persist.** No slot carries a receipt before this lands, so `Unrecorded` advances from `AdvanceFromUnrecordedSlot` — monotonicity is vacuous when there is no prior revision. `Unreadable` has no constructor into `AdvancePredecessor` at all, so a corrupted or unreadable receipt cannot be laundered into a deploy. And because the receipt write is inside the converged path, a slot that fails to record its revision makes the deploy red, so "unrecorded forever" cannot happen quietly.

**Still open, and named rather than papered over:** flock(2) is advisory, so the lease excludes only callers that take the same lock. Both *named public* entries do, and there is no third — but the inner `_leased_wet` entries remain callable in principle by anyone who invokes them directly. Closing that by construction needs the reference-level visibility the meta-exec confinement milestone tracks, not another runtime check; a check here would concede the bad state is writable, which is the shape §5 rejects. The residual risk is therefore one an operator has to *choose* to take, not one an ordinary deploy can stumble into.
