...
**P0 — the first safe deployment was impossible.** Making an unrecorded routed slot refuse closed the rollback hole and closed liveness with it: no slot carries a receipt, receipts are written only after an admitted advance, and admission required a grounded receipt, so the first deploy after merge would refuse and so would every deploy after it. The repair is a grounding producer inside the leased apply path (`ground_unrecorded_slot_revision`): read the slot's own synced git `HEAD` — the tree sync whitelists `HEAD`, `objects`, `packed-refs` and `refs` into the slot's repo root, so this is an observation of the slot rather than a claim by the run observing it — write it as the slot's first receipt, and re-read through the ordinary path. Every failure arm still refuses; grounding can turn *unknown* into *known* and never into *assume*. A one-time manual seed was rejected because it would make the permanent apply path depend on undocumented external preparation, so any future slot silently returns to the deadlock.
