...
- **Cross-program producer:** Evaluator produces the runtime consumed by the R3 consequence-cycle lanes. **The 7 Evaluator-gated R3 lanes** (T-Tier3-Dissolution, T-LensProducer-Retirement, T-Verification-L4-L7-Direct, T-Verification-L5-Corpus, T-FixedPoint, T-Omni-Shape-B, T-CostLens-Composition) **cannot meaningfully start dispatch until Evaluator lands.** The 3 non-Evaluator-gated R3 lanes (T-Int128, T-Anthropic-Wire, T-Bridge-Retirement) consume R2 substrate carriers but not the Evaluator itself, so they may dispatch in parallel with R2-Evaluator work per scheduling preference (see [`docs/r3-structure.md`](r3-structure.md) §"R3 worker dispatch precondition" for the precise carve-out).
...
<<<<<<< HEAD
| Q6 | `Witness<C>` generality for non-trivial monoids + `Diagnostic.kind` extensibility protocol | (c)/(d) hybrid — Witness<C> stays as-is; rich structural validation failures encode into lens-owned, namespaced `Diagnostic.kind` declarations; no per-lens expansion of the global compiler diagnostic enum; compiler-authored and user-authored lenses use the same namespace extension protocol per `feedback_groundedness_gates_lenses`; arity-over-error-space pattern recognized observationally, no consolidation |
=======
...
- Q6–Q8 (lens-framework spec semantics, including Q6's `Diagnostic.kind` extensibility protocol): [`docs/design-lens-framework.md`](design-lens-framework.md) §"Design questions to lock before substrate dispatch"
