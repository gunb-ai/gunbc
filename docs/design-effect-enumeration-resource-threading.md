...
| `src/v3/std/effects.dag:550` (`WorkflowEffect.LinearEffect.ops`) | `List<OperationEffect>` | Reshape to `List<Operation>` |
...
- **The lens body does not change.** `src/v3/lenses/effect_enumeration.dag::callable_arrow_effect` already implements §2.4's rule. The migration is upstream of the lens; once the substrate threading lands, the lens fold becomes COMPLETE on the same fold body.
...
**Resolved:** deleted. Per [`feedback_closed_system_effects`](../README.md#): *"effects are not annotations."* `readonly` is exactly the annotation pattern P1 names. After migration, `readonly` is structurally derivable: an operation is read-shaped iff every threaded resource appears unchanged in input and output. The keyword's only role today is to short-circuit the algebra; once the algebra reads the signature directly, the keyword is a redundant annotation that can drift from the structural fact (the [`feedback_state_space_vs_behavioral_invariants`](../README.md#) failure mode: a behavioral invariant maintained by convention, not by the type).

**Implementation note:** existing `is_idempotent_effect` / `operation_is_breaking` continue to work — they read the partition off the derived `EffectShape`, not off a keyword.
...
- The lens fold body (`callable_arrow_effect` already implements the §2.4 rule; per §6.2 first reason).
...
5. **Cementing test** (Verification Manager). Author `effect_enumeration_lens_behaviorally_complete` cementing test under `src/v3/compiler/tests/integration/cementing/` per [`TESTING.md`](../TESTING.md) Band-C discipline. Tests minimal-`Dag` shapes asserting (a) read-shaped operations report `ReadShaped`, (b) write-shaped operations report `WriteShaped`, (c) operations with no resource thread report `NoEffect`, (d) coverage gaps surface explicitly. Same PR.
6. **Capability-register row update** (Verification Manager). Flip `docs/v3-lens-capability-register.md` row for `effect_enumeration.dag` from `BEHAVIORALLY PARTIAL` to `BEHAVIORALLY COMPLETE`; clear the "What v2 has that v3 drops" cell per the discipline at register §"Discipline" item 2. Same PR.

Steps 1–6 land as **one PR** per §6 atomic discipline. Internal sequencing within the PR is mechanical — substrate authoring (1) → consumer migration (2, 3) → algebra reshape (4) → tests (5) → docs (6) — but the PR is a single atomic unit per [`../INVARIANTS.md`](../INVARIANTS.md) §P5.

Total estimate (per L-XL sizing in lane row 146): substrate + extdeps consumer migration is the L-XL surface; lens-fold itself is unchanged; cementing test is M; doc is S. End-to-end: 3-5 weeks worker time at standard R3 cadence given coordinated cross-manager dispatch.
