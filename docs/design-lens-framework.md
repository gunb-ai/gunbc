...
- (d): keeps Witness<C> simple; pushes structural failure data into namespaced `Diagnostic.kind` declarations owned by each lens instance (the current bootstrap `CompilerDiagnosticKind` sum remains compiler-native vocabulary, not the per-lens extension point).
