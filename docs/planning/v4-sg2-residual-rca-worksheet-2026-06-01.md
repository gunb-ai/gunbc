# v4 SG-2 Residual RCA Worksheet — 2026-06-01

> **Status:** DRAFT RCA WORKSHEET — residual routing on top of approved SG-2 worksheet.
> **Authority:** PR #4140 (`E0107` = 1,654, `E0282` = 1,007); local regenerated M1 probe matched those counts.
> **Existing worksheet:** `docs/planning/v4-sg2-type-expression-projection-worksheet-2026-05-30.md`.
...
`E0282` is concentrated in constructor/cache/closure sites after those instantiations are erased. Top files include `v4_compiler_translate` (183), `v4_workflow_ci` (58), `v4_lens_coverage` (45), `v4_lens_testgen` (38), and `v4_std_target_model` (36).
...
Why forbidden:
  This duplicates generic-arity facts already present in the substrate instantiation edges and would require an emitter edit for every new generic carrier.
...
DFS path:
  ...
  consumer:
    - type emission for aliases, function signatures, cached statics, closure annotations, and constructor result types
...
Falsification probe:
  Add a new generic carrier `ProbePair<T, U>` and use it in:
    (a) type alias,
    (b) cached static,
