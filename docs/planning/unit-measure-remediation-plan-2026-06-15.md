...
**The carrier is value-carrying today for non-negative integers.** Per snappy-stag-903
(2026-06-15), `count: Nat` (`measure.dag:178`) round-trips for every integer-at-scale alias —
`ByteSize`, `Hertz`, `HardwareThreadCount`, `MoneyMicros`. The word "phantom" in the
`measure.dag` header refers **only** to the `<Q>`/`<S>` dimensional tags, not the magnitude.
What is *not* yet built is the **signed/continuous** generalization (`Measure<Q, S, M>` over
the numeric tower) needed for signal samples, deltas, and fractional readings.

So the census below is real, but the **sequencing is gated**: a migration only *lands* when
its magnitude domain is supported. Most of the census sits behind value-Measure follow-ons.

**Boundary-projection rule (the unblocked pattern).** extdeps stay spec-faithful — raw API
ints keep their declared `Int` type *inside* extdeps. The wrap onto a `Measure` alias happens
at the **std consumer boundary** via the constructor fn, exactly as
`std.compute_fabric` does for money (`money_micros(cax41_catalog.hourly_eur_micros)` at
`compute_fabric.dag:597-603`). Do not lift raw ints into std; do not change extdeps field types.
...
|------|------------------|-------|-----------------|
| **GATE 0** | **nothing — value-carrying today** | this lane | non-negative integer-at-scale leaves: boundary projection onto `ByteSize`/`Hertz`/`HardwareThreadCount`/`MoneyMicros` via the constructor fns (money_micros precedent). |
...
| Site | Change | Status |
|------|--------|--------|
| `dsl/std/placement_supply.dag:33` `cpu_capacity_hz_row` | returned flat `Int` (thread-count × per-thread-Hz, dimension dropped at the std boundary) → returns `Hertz` via `hertz()`. Zero consumers; safe. | **done (this PR)** |
...
Much of the std-internal migration already landed before this lane: `byte_size()` / `hertz()`
/ `hardware_thread_count()` / `money_micros()` are already in use across `std.memory`,
`std.cpu/ampere`, and `std.compute_fabric`. GATE 0 here closes the remaining std-boundary
dimension-drop.
