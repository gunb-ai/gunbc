# v4 Stable Rustc Bands RCA Worksheet — 2026-06-01

> **Status:** DRAFT RCA WORKSHEET — routes stable bands without creating a broad SG-3 lane.
> **Authority:** PR #4140 and local regenerated M1 probe; stable-band counts matched exactly.
...
| --- | ---:| --- | --- |
| `E0277` | 330 | trait bound / Ord eligibility on realized operands | Target trait eligibility + collection realization |
...
Existing worksheet:
  SG-5 covers Set/Map collection realization and BoundedLattice completeness:
...
Dispatch:
  Attach to SG-8 implementation after §8 ratification. Do not create a separate SG-3 worker.
```
...
Required worksheet before implementation:
  A small constructor-field-admission worksheet if E0560 remains after SG-8 and SG-2. Current +4 delta suggests it is not the critical path.
```
...
Dispatch:
  These are fail-closed gates, not Rust syntax patches. If a type expression cannot be projected, emit a typed diagnostic/probe failure rather than `_` or a self-alias.
```
...
Stable bands are real residuals, but they are not the growth driver. Route `E0573` to SG-8, collection/Ord `E0277` to SG-5, `E0369` to SG-RC, and hold `E0560`/`E0121`/`E0391` as small fail-closed worksheets after the P0 fanout lands or a worker proves an independent single-authority fact.
