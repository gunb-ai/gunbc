...
| 7   | **L4 emit-matches-`.dag`-eval**         | NO execution loop           | NO                       | T-38-PR2: `scripts/v4-testclaim-corpus-eval.sh` surfaces the per-row roster + authoring-time corpus tally witness (`execution_status=authoring_time_verdict_surface`); per-row RUNTIME execution is the escalated residual gate (was `blocked_m1_subset`) |
