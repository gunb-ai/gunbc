...
Systemic fix:
  ...
  `by_key` duplicate rejection and ordered `entries` preserved for subject×target aggregation,
...
|-----|-------------|-----------|
| G0.1 | `PerLanguageFactBundleKey` (`subject_carrier` + `target` + `ModelCorePrimitiveFactAxis`) + entry `fact_value`; registry `by_key: Map<…, Node>` rejects duplicate axis rows and `entries: List<…>` preserves the complete row set for subject aggregation | `v4.std.grounding` → `primitive_fact_bundle_for_subject` → `model_core` |
...
- **Branch G.1:** populate `PerLanguageFactBundleRegistry` via `insert_per_language_fact_bundle_entry` per RCA manager (one `fact_value` per keyed `fact_axis`; `by_key` rejects duplicates and `entries` feeds `primitive_fact_bundle_for_subject` aggregation).
