...
| P2 | v4 compiles `src/v4/compiler/*.dag` end-to-end | **TECHNICAL-PROVEN / AUTHORITY-BLOCKED**. P2-A probe (44-source closure 0 diagnostics) verified scope (a). M2 probe (#4097) is **day 1 of 14-day window** per `scripts/v4-bootstrap-resolve-posture-gate.sh` own dissolution policy. P2 full PROVEN gates on operator authorization OR 14-day window completion. |
...
- **P5 strict reading vs dep-graph reading**: per dep-graph framing, P5 is GREEN (both layers closed). Per TASKS.md strict reading, runtime-execution gate is the third gate that's been there all along — surfaced explicitly by T-38-PR2 worker's honest scope analysis (cool-boar-841 identified 3 readings, chose to do surface migration + escalate runtime gate as upward debt; correct discipline per project_spirit "stop and escalate").
