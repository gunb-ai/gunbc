...
|-------|---------|-------------|
| **Blocking** | M6 identical-payload coproduct arms (#3962) | Must collapse or dissolve before downstream carriers copy the pattern — **SG-2** (`TargetTypeExpressionProjection`, PR #3962); **SG-1** is downstream consumer of `type_form` (dispatch SG-2 before SG-1 per `docs/planning/v4-sg2-type-expression-projection-worksheet-2026-05-30.md`). SG-5/SG-6 are unrelated collection/`Ord` lanes. |
