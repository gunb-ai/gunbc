...
- Lens-local workaround-shaped coproducts with named dissolution triggers (today: `TemplateArgumentsMatch`, `TemplateArgumentCursor`, `NormalizedInstantiationArgs` in `infer_helpers.dag`, plus `NonCommutingPairLookup` in `parallelism.dag`) also count against the ratchet; these are implementation scaffolds, not genuine lens-API carriers.
...
| `src/v3/lenses/parallelism.dag` | 2 | 1 positive-def (`LinearBranchesLookup` — terminal lens-private branch extraction control-flow helper, not a dissolving scaffold) + 1 in-ratchet (`NonCommutingPairLookup` — scaffolded pair-evidence carrier until `WorkflowParallelismReport` grows typed non-commute evidence) |
...
**Primary ratchet count today: 4** (0 strict 2-variant Lookup-pattern carriers on lens surfaces — `complexity`, `cost`, and `infer_helpers` import `v3.std.lookup::Lookup` + 3 workaround-shaped infer-helper coproducts: `TemplateArgumentsMatch`, `TemplateArgumentCursor`, `NormalizedInstantiationArgs` + 1 parallelism scaffold coproduct: `NonCommutingPairLookup`). The tokenizer migration removed 6 compiler-local type declarations by moving them to `src/v3/std/tokenize.dag` (25 → 19). Consolidation tranche 2 removed 14 by moving the parse-surface family to `src/v3/std/parse_surface.dag` (19 → 5). The SG-4b callable-instantiation normalization cluster adds `NormalizedInstantiationArgs` as a tracked workaround carrier (Option-as-enum, paired to dissolve alongside `TemplateArgumentsMatch` when the lens→Rust emitter lands verified `T?` / `Bool` return mappings). The parallelism walker port adds `LinearBranchesLookup` as a terminal private branch-extraction control-flow helper and `NonCommutingPairLookup` as a scaffold that dissolves when typed pairwise non-commute evidence lands on `WorkflowParallelismReport`. The complexity lens completion slice widens the positive-def API from the old `CostEntry` row to `Certainty`, `ComplexitySummary`, `ComplexityEntry`, `DominanceOutcome`, and `ComplexityEnforcedApplication` while keeping the generic presence carrier imported from `v3.std.lookup`. All lens-local types now classified — `structural_resolution.dag`'s two record types (`UnresolvedArrowBody`, `NameKeyedReference`) are positive-def lens-API carrying the lens's findings.
