...
| `src/v3/lenses/parallelism.dag` | 2 | 2 positive-def (`LinearBranchesLookup` — terminal lens-private branch extraction control-flow helper; `NonCommutingPairLookup` — terminal private traversal result for the first found non-commuting pair vs all pairs commute, now routed into public `PairwiseNonCommute { left, right }` evidence) |
