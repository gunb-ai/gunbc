...
- Lens-local helper/workaround-shaped coproducts with named dissolution triggers (today: `TemplateArgumentsMatch`, `TemplateArgumentCursor`, `NormalizedInstantiationArgs` in `infer_helpers.dag`, plus `LinearBranchesLookup` / `NonCommutingPairLookup` in `parallelism.dag`) also count against the ratchet; these are implementation scaffolds, not genuine lens-API carriers.
...
| `src/v3/lenses/parallelism.dag` | 2 | 2 in-ratchet (`LinearBranchesLookup` — lens-local branch extraction control-flow helper; `NonCommutingPairLookup` — scaffolded pair-evidence carrier until `WorkflowParallelismReport` grows typed non-commute evidence) |
