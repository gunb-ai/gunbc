...
> Operational: [ROADMAP.md](../../ROADMAP.md), [INVARIANTS.md](../../INVARIANTS.md)
...
- **Bootstrap shim** — `src/v3/compiler/src/` at the **0-floor target per the Pure Bootstrap to Zero plan** (LIVE 2026-04-25; supersedes the prior ≤5-floor framing). The entry point dissolves under cascade promotion via the PB-Bootstrap-Process lane (bootstrap workflow declared as `.dag` data; `bootstrap.rs` becomes a generated trampoline or vanishes).
...
- Lens-local workaround-shaped coproducts with named dissolution triggers (today: `TemplateArgumentsMatch`, `TemplateArgumentCursor`, `NormalizedInstantiationArgs` in `infer_helpers.dag`) also count against the ratchet; these are implementation scaffolds, not genuine lens-API carriers.
...
| `src/v3/lenses/parallelism.dag` | 3 | 2 positive-def (`LinearBranchesLookup` — terminal lens-private branch extraction control-flow helper; `ParallelismMode` — T-LAS gate #95 §4.4 iteration-opt-in parallelism budget carrier for `parallelism_enforceable`) + 1 in-ratchet (`NonCommutingPairLookup` — workaround-shaped pairwise commute probe with named dissolution paired to `WorkflowParallelismReport` evidence) |
...
**Primary ratchet count today: 4** (authoritative inventoried count: `TRACKED_ROWS` in `scripts/check-compiler-std-ratchet.sh` — **`BASELINE_TRACKED_TOTAL=4`**, mechanically `tracked debt` in CI output — **matches this prose**.) Semantics: **0** strict **2-variant** `Missing | Found(T)` **Lookup-pattern** duplicates on **`complexity.dag`, `cost.dag`, `infer_helpers.dag`** lenses (they import **`v3.std.lookup::Lookup`**) **plus four** workaround-shaped scaffold coproducts with named dissolution triggers: **`TemplateArgumentsMatch`**, **`TemplateArgumentCursor`**, **`NormalizedInstantiationArgs`** (`infer_helpers.dag`) and **`NonCommutingPairLookup`** (`parallelism.dag`). The tokenizer migration removed 6 compiler-local type declarations by moving them to `src/v3/std/tokenize.dag` (25 → 19). Consolidation tranche 2 removed 14 by moving the parse-surface family to `src/v3/std/parse_surface.dag` (19 → 5). The SG-4b callable-instantiation normalization cluster adds `NormalizedInstantiationArgs` as a tracked workaround carrier (Option-as-enum, paired to dissolve alongside `TemplateArgumentsMatch` when the lens→Rust emitter lands verified `T?` / `Bool` return mappings). The parallelism walker port's `LinearBranchesLookup` is a terminal lens-private helper; iteration-opt-in LAS budget labeling adds positive-def `ParallelismMode` (gate #95); `NonCommutingPairLookup` remains in-ratchet; typed pairwise non-commute evidence now lives on `WorkflowParallelismReport` directly in the `PairwiseNonCommute { left, right }` variant. The complexity lens completion slice widens the positive-def API from the old `CostEntry` row to `Certainty`, `ComplexitySummary`, `ComplexityEntry`, `DominanceOutcome`, and `ComplexityEnforcedApplication` while keeping the generic presence carrier imported from `v3.std.lookup`. All lens-local types now classified — `structural_resolution.dag`'s two record types (`UnresolvedArrowBody`, `NameKeyedReference`) are positive-def lens-API carrying the lens's findings.

**End state:** 0.

Each migration lane reduces the primary ratchet count; positive-definition types track growth separately and are not bounded downward by this ratchet.
...
- **Lens capability tracking** — tracks whether each lens is behaviorally complete; a PROXY/STUB lens can't yet substitute for its v2 counterpart. Independent of this claim but complements it: both are "honesty about the current gap" tools.
...
- [ROADMAP.md](../../ROADMAP.md) — live planning and status authority
