# v3 substrate consumer enumeration — M1(2.7) through M1(2.8) historical record

**Status:** historical. Preserved verbatim from `src/v3/DOWNSTREAM_REQUIREMENTS.md` on 2026-04-17 as part of the docs-pruning pass (PR #506). The original enumeration was diagnostic during PR #445 review and became the spec for the M1(2.7) fix PR. All 14 original gaps resolved at M1(2.7); R9 and M1(2.8) addendums documented follow-up fixes that also landed.

The content below is the full enumeration — Consumer 1 through Consumer 4, Methodology, Resolution summary, Round 9 fixes, M1(2.8) fixes, R16 scaffold-boundary audit, write-pipeline gap summary, and substrate cross-reference — exactly as it stood pre-prune. Retained for audit traceability; not for active consumption.

Active substrate-consumer gaps (Class 5 items, class-6 tokenizer escapes, class-7 realization narrowing, PR-B validation summary) live in the current `src/v3/DOWNSTREAM_REQUIREMENTS.md`.
