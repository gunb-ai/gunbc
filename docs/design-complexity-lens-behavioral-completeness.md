...
**Why two `AnalysisDimension` instances rather than one with both fields:** per DB-3 §"Algebraic constraints", `compose` is a `Monoid<Carrier>` (carrying the binary op + identity together — F2 dispatch / PR #1607). Work and span happen to share the *same* sequential `Monoid<SymbolicCost>` (`sequential` op + `ConstantCost(0)` identity) for the across-workflow fold; the algebraic distinction lives in **`witness_of`**, which differs on `BranchNode` — work treats Branch as additive (both arms' work is performed) while span treats Branch as max-of-arms (only the dominating critical path counts). Keeping them as two `AnalysisDimension<SymbolicCost>` instances honors that per-Behavior witness distinction; collapsing into one would conflate the two compositional shapes at the witness layer even though their fold-monoid is the same.
