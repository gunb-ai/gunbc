...
2. **Every property a Rust test asserts is expressible as a `TestPredicate`.** Today's `TestPredicate` coproduct (per `src/v3/std/verification.dag:109-278`) covers 17 variants: `Compiles`, `FailsWithDiagnostic`, `OutputEquals`, `PortHasState`, `DeclarationHasRefinement`, `CostBounded`, `BehavioralObservation`, `MockBackedInvariant`, `ExecuteCommand`, `ForAllTargets`, `LensOutputEquals`, `DifferentialEquals`, `BinaryDimensionReportEquals`, `AlgebraicLaw`, `CensusBoundCheck`, `CensusSubsetCount`, `FixedPointConverges`, `RatchetZero`, `GeneratedFromDag`, `ReleaseDeferredClaim`, `SubstrateResearchDeferredClaim`, `BridgeLedgerZero`. The migration audit in §3 catalogs each Rust-test class against these variants and identifies the (small) residual that needs new carriers.
...
- The existing `TestPredicate` coproduct (the 17 variants are unchanged; quantification is at the claim level per §2.5).
