...
`bright-moth-390` recommends Director author the bypass implementation brief described in §Q4 and dispatch to the next idle worker. No upstream blockers identified. The cardinality-substrate gate named by `docs/architecture.md` §"Where We Are (current compiler primitives)" is closed (in v3) by virtue of `TypeConnective::Cardinality` being a first-class connective, not a bridge enum.
