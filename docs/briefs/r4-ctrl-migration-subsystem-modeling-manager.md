...
- **Trio-anchor Phase-3-partner placement correction** (per Emission-Targets Mgr `deep-ibex-326` 2026-05-12 `msg_c83099ac`): the catalog #8 trio's Phase-3 partner is a **gunbc-owned render surface** (proposed `dsl/gunbc/digest_render.dag`), **NOT** an extdep field. `dsl/extdeps/github/` owns GitHub-API source-of-record facts only — rendering/projection is gunbc-owned per INVARIANTS P1 + `feedback_extdeps_header_discriminator_before_field_placement.md`. Implication: trio anchor does **not** gate on Emission Mgr's HTTP/SQL extdeps PR #2778 — converges via a parallel gunbc-owned render landing. Worker brief + dsl/ctrl/README.md updated in PR #2777 to reflect this discipline; **future worker briefs from this Mgr MUST name correct placement (extdeps vs gunbc vs std) before dispatch** — this is a standing item on the per-worker-brief checklist.
