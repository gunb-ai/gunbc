...
**Before any Wave-2 dispatch fires, this Mgr requires at least one full trio convergence on a Wave-1 subsystem**: `algebra_landed` ✓ (per the ledger's `N/A` semantics: `algebra_landed ∈ {true, —}` is satisfied — so a non-consumer Wave-1 anchor like catalog #8 satisfies this leg with `—` and does NOT require Phase 1 substrate to land first; see [`docs/audit/r4-ctrl-phase15-subsystem-receipt-trail.md`](../audit/r4-ctrl-phase15-subsystem-receipt-trail.md) §"N/A semantics" / §"Wave-1-trio gate" for the SoT) **+** `phase15_pr_merged` ✓ (this lane) **+** `phase3_emission_landed` ✓ (Emission-Targets Mgr lane) **+** `parity_passed` ✓. Trio convergence is a **gating precondition for Phase 4 cut-over dispatch**, NOT itself the `STAGED → AUTHORITY` flip — the authority flip is the single ctrl-side cut-over PR that deletes the TS files (per INVARIANTS P2/P5 single-trigger discipline; see [`dsl/ctrl/README.md`](../../dsl/ctrl/README.md) §"Authority").
