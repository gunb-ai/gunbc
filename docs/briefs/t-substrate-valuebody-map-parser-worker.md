...
- **[`src/v3/compiler/src/lower.rs`](../../src/v3/compiler/src/lower.rs)** — exhaustive `match expr` sites (~20+ per worker survey). Adding a new `SurfaceExpr` variant means every exhaustive-match site must be updated to handle it (with a fall-through for non-data-body contexts where map literals aren't expected).
