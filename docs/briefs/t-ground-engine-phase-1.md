...
> **Status update (2026-04-25, second revision)** — Superseded for live dispatch by [`t-ground-engine-phase-1-typestructure.md`](t-ground-engine-phase-1-typestructure.md). This brief stays as a decision-history anchor for the structural contracts (no mirroring of validated types, state-space discipline, fail-closed by construction, SG-0 untouched) — those carry forward verbatim into the rescoped Phase 1 + the forward-planned Phase 2. **Workers: do not dispatch against this brief.** The rescoped split: Phase 1 = type-structure validation against the loader-close accessor (dispatchable post-[PR #776](https://github.com/gunb-ai/gunbc/pull/776) merge); Phase 2 = full pilot-list enumeration + mirror retirement (blocks on R2 T-Substrate 4th sub-lane closure). Director's signal: [PR #768 comment 2026-04-25T03:27:15Z](https://github.com/gunb-ai/gunbc/pull/768).
>
> Original status banner (preserved): Phase 0 substrate audit complete (PR #768 merged); Director routed Route 1 (small loader-close, ad-hoc Director dispatch); Phase 0 instructions below are decision history; do not re-run Phase 0.
...
7. **Pilot receipt** — [`docs/briefs/grounding-pilot-receipt.md`](grounding-pilot-receipt.md). Manager-authored synthesis of the five lessons Engine inherits. The receipt is the synthesis; [PR #765](https://github.com/gunb-ai/gunbc/pull/765) is the empirical record (read it for per-decision context the receipt elides).
