...
Per Mgr brief §"Wave-1-trio checkpoint" rationale: PR-digest helpers are pure functions over GitHub PR JSON shapes. The corresponding Phase 3 emission target is the smallest possible extdep — render-helpers, no new external authority beyond what `dsl/extdeps/github/pulls.dag` already encodes. This makes the trio convergence pattern testable at minimum surface area before fanning out to subsystems with real SQL/HTTP/audit-event emission requirements.
...
1. **Module header** with consumer receipt:
   ...
   // STAGED → AUTHORITY trigger: dsl/extdeps/github/digest_render.dag (Phase 3) lands +
   // ctrl PR cut-over deletes the 5 .mjs files + parity test passes.
   ```
...
3. Consumer-receipt header cites all 5 ctrl `.mjs` files + names `dsl/extdeps/github/digest_render.dag` as the Phase 3 trigger.
...
- **STOP** if `dsl/extdeps/github/pulls.dag` lacks `GithubPr` / `CiState` / `ConflictState` carriers — that's an extdep-side substrate gap; surface to Mgr who routes to Substrate Mgr or Emission-Targets Mgr.
