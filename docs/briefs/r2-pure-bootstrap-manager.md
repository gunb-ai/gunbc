...
- `bridge_canonical_lens_name_dispatch_retired` — distributed bridge #3 (**partial retirement; full close blocked.** §0.1 `PROGRAM_INPUT_SENTINEL` + §0.2 `cost_bind_for_claim_file` already retired structurally on main. R3 gate #33 retired the 2 `lens_decl.name == Some("…")` arms + 2 generic name-keyed lookups in `test_runner.rs`; remaining surface is 2 `include_str!` canonical lens byte bridges. Pinned by `tests/integration/canonical_lens_bridge_ratchet_test.rs`. Full close gated on PB-Runtime interpreter-as-data **or** typed lens-registry carrier substrate-introduction; see [`docs/briefs/r2-pb-canonical-lens-bridge-disposition.md`](r2-pb-canonical-lens-bridge-disposition.md))
