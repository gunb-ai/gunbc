...
- `bridge_canonical_lens_name_dispatch_retired` — distributed bridge #3 (**retired in R3 gate #33.**) `PROGRAM_INPUT_SENTINEL`, `cost_bind_for_claim_file`, `R1_CANONICAL_*_LENS` byte constants, `lens_decl.name == Some("…")` dispatch arms, and generic name-keyed lens lookups are gone from `test_runner.rs`. `tests/integration/canonical_lens_bridge_ratchet_test.rs` pins all three residual categories at zero; `src/v3/std/bridge_ledger.dag` marks `bridge_canonical_lens_name_patching_residual` as `Retired`.
...
- [`r3-pb-bridge-canonical-lens-name-dispatch-closure.md`](r3-pb-bridge-canonical-lens-name-dispatch-closure.md) — T-Bridge-Retirement bridge #3 (`bridge_canonical_lens_name_dispatch_retired`) closure brief consumed by R3 gate #33; zero-residual ratchet and ledger row are now live.
