...
| `kernel_algebra_profile` mirror dissolution | M | GATED (waiting on Substrate Manager `ValueBody::Map` carrier read-path/API + arrow-body evaluation; landed in R2 post-#1017 but consumer plumbing pending) | Map-shaped (not list/sum); requires `ValueBody::Map` substrate work. |
