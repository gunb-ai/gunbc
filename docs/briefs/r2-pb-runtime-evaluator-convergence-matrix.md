...
5. R3 PB Manager dispatches PB-Runtime authoring. PB-Runtime imports the PR-A.1 `Value` carrier from `src/v3/std/runtime/value.dag` (single authority — no parallel `Value` declaration) and declares `evaluate(...)` plus the per-`Behavior` evaluation rules per §3.2 in the runtime-behavior module (e.g. `dsl/std/runtime/runtime.dag` — path is PB Manager dispatch detail, but the file declares only runtime behavior, not a second `Value` type).
