...
| PR-A.1 (`Value` coproduct carrier; path is PR-A.1's call — PR-A.2 audit's working assumption: `src/v3/std/runtime.dag`) | [`r2-pr-a-runtime-value-model.md`](r2-pr-a-runtime-value-model.md) | **BLOCKED** on #1231 per `r2-pr-a1-runtime-value-dependency-audit.md`. |
...
| `pb_runtime_equivalent_to_evaluator_on_corpus` TestClaim declaration | Row 4; design lock §7.1 | **NOT YET AUTHORED** on main. The locked predicate variant (`DifferentialEquals`) and staging variant (`ReleaseDeferredClaim`) are live; the `TestClaim` consuming them and the forward `DeclarationRef`s (`pb_runtime_evaluate`, `r2_evaluator_evaluate`, `corpus`) are R3-worker dispatch outputs. Until that worker dispatches, Row 4's enforcement is paper-only. |
...
7. Worker dispatches Row 4's `pb_runtime_equivalent_to_evaluator_on_corpus` TestClaim authoring (it does NOT exist on main at audit time); declares as `ReleaseDeferredClaim` until forward refs resolve, then promotes to strict `DifferentialEquals`. Once the fixture goes green → anti-bridge invariants #1 + #6 transition from "held by reviewer convention" to "structurally enforced."
