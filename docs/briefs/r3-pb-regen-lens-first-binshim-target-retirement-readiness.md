...
**Non-authority.** Actual retirement waits for **PB-Runtime Item 4** (transitive emitter discipline), **Substrate-owned `BinShim` carrier**, **declaration framework + instance authoring** under `dsl/std/runtime/bin_shims/` (separate worker scope), **BinShim emitter**, then a **retirement PR** that lands equivalence + census deltas together. Until then, treat every row below as “not green for retirement.”
