...
|---|---|---|
| `tier3_termination_mirror_perf_within_budget` | `DescentEvidence`, `PositiveDescentAmount`, `ProportionalDivisor`, `ShrinkFactor`, `evidence_rank`, `merge_evidence` (was `dag.rs:628-790` per `r2-pure-bootstrap-manager.md` §"Owns (post-R1 R2 program work)" — "Tier 3 mirror dissolutions: termination" row) | Representative descent-evidence merge over fixture corpus, evaluated through `.dag` body |
| `tier3_computation_mirror_perf_within_budget` | `SizeBound`, `RecursionShape`-related (was `dag.rs:839-915` per `r2-pure-bootstrap-manager.md` §"Owns (post-R1 R2 program work)" — "Tier 3 mirror dissolutions: computation" row) | Representative recursion-shape derivation, evaluated through `.dag` body |
| `tier3_induction_mirror_perf_within_budget` | `RecursionShape`, `InductiveField`, `SubValueRelation` (was `dag.rs:916-980` per `r2-pure-bootstrap-manager.md` §"Owns (post-R1 R2 program work)" — "Tier 3 mirror dissolutions: induction" row) | Representative sub-value-relation walk, evaluated through `.dag` body |
| `tier3_effect_carrier_mirror_perf_within_budget` | `dag/effects.rs` (216 LOC) + `compose_operation_effects` (105 LOC) per `r2-pure-bootstrap-manager.md` §"Owns (post-R1 R2 program work)" — "Tier 3 mirror dissolutions: effect-carrier" row | Representative effect-composition over workflow corpus, evaluated through `.dag` body |
