...
- For `/` specifically, the closure cost has **dropped since the
  ...
  (1) change `OrderedRing.div`'s return at `algebra.dag:181` to
  ...
  `go_int_div`, `python.dag:486` `python_int_div` — all currently
  render `carrier: "({lhs} / {rhs})"`) to construct the new return
