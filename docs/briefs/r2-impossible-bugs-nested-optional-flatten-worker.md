...
- **[`src/v3/compiler/src/lower.rs:2129, :2226`](../../src/v3/compiler/src/lower.rs)** — `SurfaceType::Optional` lowering arms (lowering path + TypeConnective construction). Verify by grep `SurfaceType::Optional` at HEAD before editing.
- **[`src/v3/compiler/src/infer.rs:3129`](../../src/v3/compiler/src/infer.rs)** — `concretize_decl_with_subst` definition (the **killer construction site** per design doc — substitution path that bypasses `lower.rs`); call sites at `:2643, :2645, :3104, :3152, :3162, :3190` (grep `concretize_decl_with_subst` at HEAD for the live set).
