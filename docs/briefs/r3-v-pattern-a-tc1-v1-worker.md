...
**Status:** **HELD on Branch B η-non-vacuity** for **native `DimensionReport<Tc1EtaLensObservation>` report equality** — Director η non-vacuity (Branch B) 2026-05-06 ([gunbc#828](https://github.com/gunb-ai/gunbc/issues/828)). **Scaffold landed at [PR #2184](https://github.com/gunb-ai/gunbc/pull/2184)** with consumer wiring (η-pair callables + `BinaryDimensionReportEquals` predicate + integration tests) per Director (C-modified) ratification at [gunbc#828](https://github.com/gunb-ai/gunbc/issues/828) on 2026-05-07. **Path A SymbolicCost proxy (Q-PAFS / E6-G1.a):** `TestRunner::try_eval_tc1_eta_equivalence_executable` compares workflow-root **`composed` `SymbolicCost`** on the two η-pair entry binds in `TestClaim.source` (bounded proxy until **gunbc#1972** lands native lens-fold receipts). Strict-fire integration `tc1_substrate_lens_eta_equivalence_strict_fire_test.rs` may **Pass** on that proxy **without** changing §1.8 **Status** — canonical authority: `docs/r3-program-plan.md` §1.8 row #11 **Status vs Notes** (slice receipt ≠ ledger PASSING; INVARIANTS P5). **2026-05-09 Director (a)-disposition:** Evaluator E3.c ([#1970](https://github.com/gunb-ai/gunbc/issues/1970)) closed as superseded-by-deferral; the remaining **carrier-aligned** path is blocked on E4/G1.b ([#1972](https://github.com/gunb-ai/gunbc/issues/1972)), HELD-CANVAS-DEFERRED past R3. R3-thesis load-bearing arithmetic still subtracts #11 (105 load-bearing gates). **Q-Reification CLEARED 2026-05-07** ([Option A ratified](https://github.com/gunb-ai/gunbc/pull/2096), `Dag` IS the reflected program; no separate carrier). **Q-PAFS Path A** remains **ACCEPTED** (PR [#1824](https://github.com/gunb-ai/gunbc/pull/1824) merge record on `main`). **V1 (`tc1_eta_equivalence_executable`) unpairs** from Evaluator **E3 Option 3** narrow **argument-opaque** representative slice for **TC1 acceptance** — that shape yields **vacuous** `BinaryDimensionReportEquals` (constant `DimensionReport<C>`); it cannot honestly close the **ledger** gate against [`r3-v-tc1-eta-equivalence-deeper-analysis.md`](r3-v-tc1-eta-equivalence-deeper-analysis.md) §What TC1 Asserts + §Strict-Fire Extension Surface. **Resume dispatch** for **ledger PASSING** only after lens fold consumes `Dag` via `.dag` body authority through Evaluator (real lens-over-Dag fold, non-vacuous η obligation) **or** Director-visible §1.8 semantics revision for full carrier parity (**not** ratified 2026-05-06). **Q-Reification gate is cleared**; remaining hold for **native report equality** is Branch B + #1972.
...
**Program plan (single operational authority):** [`docs/r3-program-plan.md`](../r3-program-plan.md) §10.3 — rows **Q-PAFS** + **Q-EVAL-Lens-Fold-First-Slice** **Status** columns hold **policy ACCEPTED (Path A / G1.a)** **and** **implementation supersession**: TC1 V1 **HELD** for **ledger** **PASSING** (native `Tc1EtaLensObservation` report parity) until non-vacuous η via lens fold + **#1972** (or explicit §1.8 plumbing-only revision — **not** ratified). **Path A SymbolicCost proxy** CI receipts are **Notes-only** per §1.8 row #11 — **not** a Status promotion. **This brief elaborates worker mechanics only** — it **does not** override or restate dispatch authority independently of that table (**INVARIANTS** §P2 / modeling-discipline Practice 5).
...
| --- | --- | --- |
| **#11** | `tc1_eta_equivalence_executable` | §1.8 **Status** stays **DECLARED** for **ledger** closure (native `Tc1EtaLensObservation` report parity post-**#1972**). **Slice receipt:** strict-fire integration may **Pass** on **Path A SymbolicCost proxy** (`try_eval_tc1_eta_equivalence_executable`); see `r3-program-plan.md` §1.8 row #11 Notes vs Status — **not** a Status promotion. |
...
2. **Slice 2 — executable `TestClaim` + proxy CI:** `tc1_eta_equivalence_executable` + suite row; strict-fire integration asserts `Pass` on **Path A SymbolicCost proxy** per `try_eval_tc1_eta_equivalence_executable` rustdoc. **Ledger Status** remains **DECLARED** until #1972 supplies native report equality (see §1.8 row #11 Notes).
