...
**Authority:** `src/v4/TASKS.md` T-4 / parallel-fill block, **`docs/briefs/r4-program-dispatch-plan.md` §2**, per-file headers (not this receipt). **Note:** `src/v4/DECISIONS.md` is **retired** (operator 2026-05-19); cite **this lane's merge PR** for archived ledger text.
...
| `cpp.dag` | **🟡** `src/v4/extdeps/languages/cpp.dag` top `// Status:` header (opens immediately under the file path banner): D2-REV resolver slice; non-bool scalar ladder carries the verbatim tag **`feature:t4-cpp-scalar-ladder`** with **P4-3208 + T29-ABI** pointers on the same line (merge-base / header; retired `DECISIONS.md` — cite merge PR for prose) — not session shorthand; not parity with rust/go/python terminal posture |
...
Both headers claim **T-4.5 modeled 2026-05-16** with POSIX anchors; `file_system.dag` retains **🟡** coproduct markers for **OS-1** disposition (one-line tags; rationale in PR — retired `DECISIONS.md`).
...
- `coordination.dag` — **Modeled with tracked scaffolds** (B3 disposition; rationale in PR / brief — retired `DECISIONS.md`).
...
- **T-4.14** `ptx.dag` — **PASS (IN-B)** dispatch posture (**dispatch §2 + merge PR**, historical *L-3* stake; `DECISIONS.md` retired); file header intentionally domain-neutral (no process-axis status tag in the carrier).
