...
- **Receipt**: scaffold surfaces (Phase-1 bench functions) get a `ROADMAP.md` row or inline PR-description entry naming the deletion path + trigger. Steady-state surfaces (Phase-2 benches, extraction helper, helper module) are documented in the slice PR description with rationale to prevent future mis-classification.
