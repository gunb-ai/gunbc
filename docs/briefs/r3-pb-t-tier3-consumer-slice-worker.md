...
- Capture follows `docs/audit/c1-tier3-baseline-capture-procedure.md` §2 verbatim — **no new Rust CLI; no new bench-file flags**. Procedure §6 line 161 explicitly forbids altering `tier3_mirror_perf.rs` or extending the bench surface beyond the 5 budgeted bench-name targets.
- Phase-2 measurement reuses the same `cargo bench` invocation against a sibling Phase-2 bench group within the existing `tier3_mirror_perf.rs` file (the procedure permits adding additional `bench_function` registrations covering the dissolved-mirror Evaluator path; the bench file is the single authority for measurement targets). Phase-2 emits to a parallel `tier3_phase2_measurement.json` with identical schema to `tier3_baseline.json`.
...
**Acceptance:** (a) `tier3_baseline.json` lands per procedure §4 schema; (b) §3 `.dag data` values land coherent with JSON; (c) CI drift-detection test asserts JSON ↔ `.dag data` equality with explicit error if mismatch; (d) extraction helper from procedure §2.1 path (a) lands committed.
...
- No new `bin/` Rust CLI is added (procedure §2 + §6 forbid extending the bench surface or duplicating its capture path with new tooling). Capture is the bare `cargo bench --bench tier3_mirror_perf -p v3-compiler` invocation per procedure §2.
...
- **Receipt**: paired-dispatch debt row for the Phase-1 bench scaffold landed in `ROADMAP.md` §"P2 — structural compression" alongside this brief (search: "Tier-3 perf-budget Phase-1 hand-Rust mirror benches"). Row names the deletion path (per-`bench_function` retirement keyed to per-mirror retirement PR landing) + the trigger (mirror-function compile breakage as the per-mirror retirement workers land) + the owner (this consumer slice). Steady-state surfaces (Phase-2 benches, Python extraction helper, `PerfMeasurement` / `PerfMeasurementResolveError` helper module) are documented in the slice PR description with rationale, NOT added to ROADMAP — that prevents future maintainers from mis-classifying them as undeleted scaffolds.
