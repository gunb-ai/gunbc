...
|---|---|---|---|---|
| **R1C-A** | M | T-TestGen schema extensions (two coupled deliverables): (a) M1(2.8) `List<ResourceReference>` data body so `MockBackedInvariant.requires` can carry real refs (closes `testgen_mock_backed_integration_safe`); (b) scope predicate shapes for the 6 PB-census gates (T-PB-A `pb_hand_rust_at_shim_floor`, `lens_producer_files_remaining`, `pb_self_compile_fixed_point`, `pb_compiler_std_ratchet_zero` + T-PB-B `pb_test_file_generated_from_dag`, `pb_rust_tests_outside_residual_zero`) per ROADMAP authority that T-TestGen owns predicate shape. | none | NOT YET AUTHORED |
| **R1C-B** | S | T-P0 fixture authoring: 3 TestClaim fixtures (`p0_repeat_string_correct` Day-1; `p0_no_fabrication_sentinel` ext; `p0_rest_ops_aligned` ext). Features already work (oracle test for `repeat_string` is green; sentinel + ops require schema extensions from R1C-A IF they require new predicates beyond DB-15's existing surface — verify at brief authoring). | possibly R1C-A (if `[ext]` predicates need new shapes) | NOT YET AUTHORED |
| **R1C-C** | XS | T-Sub fixture authoring: 1 TestClaim fixture for `sub_type_alias_where_lowers` (PR #703 already landed the feature). | none | NOT YET AUTHORED |
| **R1C-D** | M-L | T-PB census-as-`.dag` wiring: 6 PB census gates as `.dag` TestClaims. Merges what was previously T-PB-A and T-PB-B into one lane because both share the predicate-shape work R1C-A scopes; splitting would duplicate the schema-consumer pattern. Authoritative ratchets remain `EXPECTED_HAND_AUTHORED_NON_TEST` + `EXPECTED_HAND_AUTHORED_FRAGMENTS` (T-PB-A subset) and `EXPECTED_HAND_AUTHORED_TEST` (T-PB-B subset) in `src/v3/compiler/tests/integration/sg0_census_test.rs`; the `.dag` TestClaims read those census values via the predicate shape R1C-A scopes. | R1C-A | NOT YET AUTHORED |
| **R1C-E** | S | T-Emit `.dag` TestClaim wrappers: 3 ExecuteCommand-based wrappers around the existing host harness (`emit_rust_fixtures_rustc_green` `[ext: ExecuteCommand]`; `emit_generic_bounds_survive` `[ext]`; `emit_omni_demo_fixtures_green` `[ext: ForAllTargets + ExecuteCommand]`). PB-Runtime `ExecuteCommand` runner landed PR #792, so this lane is dispatchable Day-1. Existing host harness in `tests/boundary/m1_3_emit_rust_test.rs` and `tests/boundary/m1_5_emit_omni_demo_test.rs` becomes the input to the `.dag` wrapper, not the gate authority. | none | NOT YET AUTHORED |
| **R1C-F** | S | T-Demo user-authored-lens fixture: 1 TestClaim fixture for `demo_user_authored_lens_rejects_violating_program` (consumes `user_authored_lens_compiles` from T-LensAPI which is GREEN). | none | NOT YET AUTHORED |
...
Authored: none (this is a new manager; lane sub-briefs dispatch from this document).

Pending — manager-authored on dispatch:
- R1C-A T-TestGen schema extensions worker brief (M1(2.8) data body + PB-census predicate shapes)
- R1C-B T-P0 fixture authoring worker brief (3 fixtures; ensure dispatch order respects R1C-A predicate-shape outputs if needed)
- R1C-C T-Sub `sub_type_alias_where_lowers` fixture worker brief (XS; one fixture)
- R1C-D T-PB census-as-`.dag` worker brief (6 fixtures + ratchet-bridge query mechanism per R1C-A predicate shape)
- R1C-E T-Emit ExecuteCommand-wrapper worker brief (3 fixtures wrapping existing host harness)
- R1C-F T-Demo `demo_user_authored_lens` fixture worker brief (1 fixture)

## Working state (fill on dispatch)

Lane status table refreshes here as work lands. Initial state: all 6 lanes NOT YET AUTHORED.
