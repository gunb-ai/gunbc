...
| **R1C-C** | XS | T-Sub fixture authoring: 1 TestClaim fixture for `sub_type_alias_where_lowers` (PR #703 already landed the feature). | none | **R1C-C: 1/1 gates green** — `sub_type_alias_where_lowers` in `r1_gates` (`Compiles` on `r1_sub_type_alias_where.v3`); `test_runner_runs_sub_type_alias_where_lowers_gate`. |
