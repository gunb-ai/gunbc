...
## Instance 16 — three independent measurements agreeing, all of them wrong

Recorded because it defeats the defence every other entry in this document relies on. Elsewhere
the repair is *re-derive through a second index*, *run a positive control*, *get a second
opinion*. This instance is three parties measuring independently, agreeing exactly, and all being
wrong together — and the agreement is what made the number feel safe.

The question was how many files in the v1 seed mirror the merge driver fails to cover. Three
sessions measured it separately, with different roots and different predicates, and got **135**
each time. Two of them had already corrected each other on ref-staleness that night, so the match
read as hard-won corroboration.

All three keyed on the same thing: the file header `// Generated by v1 compiler -- do not edit.`

A fourth lane tested that header **as a population basis** and rejected it. Verified against
`origin/main`:

| file | carries the header | actually |
|---|---|---|
| `main.rs` | yes | hand-maintained |
| `std_lens_verdict.rs` | yes | hand-maintained |
| `compiler_tests.rs` | no | in the emitted population |
| `emitted_population.rs` | no | in the emitted population |
| `v1_rt.rs` | no | in the emitted population |

The header disagrees with the model **in both directions** — it over-claims and under-claims. It
is not the population; it is a record of what an author remembered to type, which is this
document's own subject arriving as the denominator rather than as the reading.

### Why the agreement carried no information

Three instruments agreeing constrains the answer only if their errors are independent. These
three shared a **common proxy**, so they were not three measurements of the population — they
were one measurement of the header, performed three times. Independent *execution* is not
independent *derivation*, and only the second is corroboration.

> **Agreement between measurements that share a proxy is not evidence. It is the same measurement,
> repeated.**

The tell is available before the result: ask what each instrument would have to be *wrong about*
for all of them to be wrong together. Here the answer was one sentence — *the header is the
population* — and no one asked it, because the numbers matched.

### What replaced it, and why that basis is sound

Not a better proxy: **two independently derived authorities that agree**. The crate layout's own
claim (direct children minus the hand-maintained set) yields 136. The emitter's own declaration
in `emitted_population.rs` names 137. They agree on 136 of 137, and the single disagreement is
explained rather than absorbed — `main.rs` is the one row where the emitter emits a file whose
committed bytes deliberately diverge, so regeneration is not its recovery.

That is the shape a population claim needs: two derivations that could have disagreed, agreeing,
with every residual named. One authority asserting is a claim; two agreeing is evidence; three
instruments reading one proxy is neither.

**What survived the correction, which is the reason to record this precisely:** the conclusion
did not move. Coverage was zero before and after; the mechanism, and the two stale authority
claims, were unaffected. Only the population was wrong — and it was wrong in *both* directions,
so no amount of caution about the number being "at least" 135 would have helped. A conclusion can
be right while every number supporting it is measuring something else.

## The decision rule for gate-produced diffs — the one entry here that is someone getting it right
...
wrong and are wrong in *opposite* directions. (Not to be confused with the *positive control* in
instance 8, which is a method for testing an instrument. This is an example of correct judgement,
not a control experiment — the same word, two unrelated senses.)
...
Instances 14–16 move the class somewhere the earlier entries could not reach, and the move is the
same in all three: **the defect stops being in the reading and starts being in the artifact.**
Instances 1–13 are all readers failing to establish a subject that existed, and every one of them
is repairable by a more careful reader. Instance 14's fold has already discarded the diagnostic
before any reader arrives; instance 15's conjunct never held one; instance 16's three measurements
were each performed correctly against the wrong subject. No amount of care recovers information
the artifact does not carry — which is why the repairs at the end of this document are carrier
changes and instrument checks rather than advice.

That is also why the one entry recording somebody getting it right is a *rule* rather than a
resolution to be careful. Every entry here was written by someone who already knew the class and
made an instance of it anyway. The commands in the table are not a reminder to be careful; they
exist precisely because care does not survive contact with a plausible answer.
