...
- **Phase 4 verification + budget-tighten** (cleanup PR — NO ratchet decrement here per openai-pro BLOCKING review on PR #2725 absorption 2026-05-12): once all 20 cuts re-enabled, single PR (a) **VERIFIES** cumulative ratchet matches expected `(value at activation) − 20` (per-PR lockstep ratchet-down per §2 #2 + §3 means the value should ALREADY be at target if per-PR discipline was followed; this PR's job is verification + audit, NOT decrementing); (b) drops cold-CI `--timeout` budget from 4000s to ≤2000s (per PM ratchet-down candidate at msg_07f73de0). **CRITICAL**: Phase 4 must NOT carry the ratchet decrement itself — that would create a fail-open execution path where workers could defer per-PR ratchet-down to Phase 4 cleanup. The brief's per-PR lockstep rule (§2 #2 + §3) is load-bearing. If Phase 4 verification finds the cumulative ratchet ≠ `(value at activation) − 20`, that's a per-PR-discipline violation in the earlier waves — escalate per §5, do NOT silently patch in Phase 4. **Per `feedback_pb_zero_is_r3_close_target`**: long-run target is 0 exemptions; the hot-fix-tagged subset's contribution should already be delivered by the per-PR sequence, not a frozen historical baseline.
