...
4. **No new hand-Rust beyond shared-fixture helpers**: helpers can live in `src/v3/compiler/tests/integration/common/*` (shared test infra; covered by Layer 2 brief's harness-class regex). SG-0 delta ≤ 0 per PR (helpers may add lines but not files unless absolutely necessary).
