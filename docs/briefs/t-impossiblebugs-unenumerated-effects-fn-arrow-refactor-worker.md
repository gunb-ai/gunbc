...
1. **New input-element shapes in `parse_surface.dag`.** Two carriers, distinguishing declaration position (always named) from type position (mixed) — both illegal-state-unrepresentable per `feedback_state_space_vs_behavioral_invariants`:
   ...
   `NamedArrowInput` is the load-bearing atom for declaration position (function-decl params always have a name, so the binder is structurally mandatory; refinement remains optional because not every param is refined). `ArrowInput` is the coproduct used in type position only — a higher-order type `fn(A, B) -> C` may carry anonymous entries (no binder) or named entries (e.g., `fn(x: Int) -> ...`). Refinement-without-binder is structurally unrepresentable in either carrier. **Coproduct dissolution receipt** mandatory for `ArrowInput` per `feedback_coproduct_dissolution` and the `LoopBound` precedent at `docs/design-mutual-recursion-lowering.md:117-134` — worker authors the receipt in `parse_surface.dag` adjacent to the type. Receipt should also address why this isn't `feedback_parallel_representation_debt` against `SurfaceParam`: `SurfaceParam` is the *data of a param at a binding site* (lowerer-binding view, today); `NamedArrowInput` is the *type-signature view at declaration position*; `ArrowInput` is the *type-signature view at type-annotation position*. Once the refactor lands, `SurfaceParam` dissolves into `NamedArrowInput` (req 3).
2. **`SurfaceType.Arrow` refactored to wrap a typed `SurfaceArrow` sub-carrier.** Introduce a new top-level struct `SurfaceArrow { inputs: List<ArrowInput>, output: SurfaceType, span: SourceSpan }` (mixed inputs, type position), then change `SurfaceType.Arrow(SurfaceArrow)` (single positional payload, not record fields). This makes the type-position arrow-shape a typed entity that consumers can hold directly without destructuring through `SurfaceType` first. **No silent compatibility shim**; existing callers updated per req 6.
3. **`SurfaceItem.Fn` + `SurfaceItem.FnExternalBody` refactored to carry a typed `FnSignature` sub-carrier with restricted inputs.** Drop `params: List<SurfaceParam>` + `return_type: SurfaceType` fields; replace with `signature: FnSignature` where:
   ...
   `inputs: List<NamedArrowInput>` (not `List<ArrowInput>`) makes binderless params **structurally unrepresentable in declaration position** per `feedback_state_space_vs_behavioral_invariants` — rejecting an earlier draft's behavioral fall-back where the lowerer would have surfaced a Diagnostic for `ArrowInput::Anonymous` inside `Fn.signature`. The parser cannot construct `Fn { signature: FnSignature { inputs: [<anonymous-shape>] } }` because no `Anonymous` variant exists in `NamedArrowInput`. The bound-name + refinement information that today lives on `SurfaceParam` flows through `signature.inputs[i]: NamedArrowInput`. **`SurfaceParam` is fully retired** by this PR — no consumers reference it post-refactor. Worker should grep-survey + cite count in PR description.

   **Carrier-distinction rationale (mandatory in PR description).** `FnSignature` and `SurfaceArrow` are both arrow-shape sub-carriers but encode different invariants: `FnSignature.inputs: List<NamedArrowInput>` (declaration must bind names so the body can reference them); `SurfaceArrow.inputs: List<ArrowInput>` (type position; binders optional). This is concept distinction, not parallel-representation debt — same justification as `SurfaceParam` vs `NamedArrowInput` carrying overlapping fields but different concepts. The downstream parser-effects sub-lane (#799 brief) lands `declared_effects` on **both** `FnSignature` and `SurfaceArrow` — both are type-signature shapes per `feedback_no_annotations`, so the discipline anchor is satisfied per-carrier; this is co-invariant duplication, not bookkeeping duplication.
4. **`parse_parser_body.txt` updates.** The function-item parse path (where `SurfaceItem::Fn` is constructed) routes its parsed params directly through `NamedArrowInput` construction → `FnSignature` → `SurfaceItem::Fn { signature: FnSignature, ... }`. The higher-order type parse path (where `SurfaceType::Arrow(SurfaceArrow)` is constructed for type annotations) constructs `ArrowInput::Anonymous { ty }` or `ArrowInput::Named(NamedArrowInput { ... })` per the syntactic form. **No new lookahead** — this is a pure construction-site refactor; the surface syntax doesn't change.
5. **Lowerer extension.** Every consumer that today reads `Fn.params` + `Fn.return_type` is updated to read `Fn.signature.inputs: List<NamedArrowInput>` + `Fn.signature.output`. Per-input destructuring is uniform — every entry is a `NamedArrowInput` carrying `name` + `ty` + `refinement?`; **no `ArrowInput::Anonymous` arm to handle in declaration position** (structurally absent per req 3). Type-position consumers (those reading `SurfaceArrow.inputs: List<ArrowInput>`) branch on `Anonymous` vs `Named`. This is surface-side reshaping; substrate-side `Declaration` + `Arrow` untouched. **No wildcard arms** anywhere.
6. **Exhaustive-match audit + updates.** Every `match` site over `SurfaceType::Arrow` (snapshot grep: ~5 hits) and `SurfaceItem::Fn` / `FnExternalBody` (snapshot grep: ~20 hits) updated for the new shapes. **No wildcard `_` swallowing** per `feedback_missing_checks_review_heuristic`. Where a consumer today destructures `Fn { params, return_type, ... }`, it now destructures `Fn { signature: FnSignature { inputs, output, .. }, ... }` and reads each input as a `NamedArrowInput { name, ty, refinement }`. Where a consumer today destructures `SurfaceType::Arrow { inputs, .. }` over `List<SurfaceType>`, it now destructures `SurfaceType::Arrow(arrow)` and reads `arrow.inputs: List<ArrowInput>`, branching per-entry on `Anonymous { ty }` vs `Named(named)`.
