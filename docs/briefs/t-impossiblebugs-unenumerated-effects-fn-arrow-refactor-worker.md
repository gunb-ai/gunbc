...
4. **`parse_parser_body.txt` updates.** The function-item parse path (where `SurfaceItem::Fn` is constructed) now routes its parsed params + return-type through `ArrowInput::Named` construction → `SurfaceArrow` → `SurfaceItem::Fn { signature: SurfaceArrow, ... }` (signature is the typed sub-carrier per req 3 — **not** wrapped through `SurfaceType::Arrow` first). The higher-order type parse path (where `SurfaceType::Arrow(SurfaceArrow)` is constructed for type annotations) constructs `ArrowInput::Anonymous { ty }` entries — never the rejected `{ name: None, refinement: None, ty }` shape, which is structurally absent under the req 1 coproduct. **No new lookahead** — this is a pure construction-site refactor; the surface syntax doesn't change.
