...
|---|---|---|---|
| **`type BinShim` carrier landing** | Substrate Manager (per design-doc §5.4 + #1347 STOP+PING) | Field requirements: `name: String` (line 26 of bin), `entry: () -> std.process.ProcessExit` (line 33's `fn main() -> ExitCode` is the structural model), and whatever pipeline-composition fields the §4.2 sketch settles on. | `type BinShim { ... }` declaration in `dsl/std/runtime/bin_shim.dag` (or wherever Substrate picks). |
...
| **`REGEN_OUTPUTS` extension** | PB Manager (same retirement PR) | The 24-entry list at `build.rs:479-516`. | Add `"src/v3/compiler/src/bin/regen_lens.rs"` (the now-emitted path). |
