...
| **Cross-target equivalence harness primitives** | S | **PR-D slice 0 landed** — named `.dag` `TestClaim` + suite ([`docs/briefs/r2-pr-d-cross-target-equivalence-harness-primitives.md`](r2-pr-d-cross-target-equivalence-harness-primitives.md)); L5-shaped multi-target receipts deferred on LanguageSpec + Shape A grounding (brief §Dependencies). | For L5 verification in R3 (algebraic equivalence over a curated corpus, per #1078 design challenge #3 locked decision). Primitives only — corpus authoring is post-R2 (R3 lane T-Verification-L5-Corpus). |
...
- ~~PR-D worker brief — L5 corpus type spec~~ — **slice 0 landed** at [`docs/briefs/r2-pr-d-cross-target-equivalence-harness-primitives.md`](r2-pr-d-cross-target-equivalence-harness-primitives.md) (strict L5 multi-target harness: follow §Next implementation slices in that brief)
