...
- **Q6**: `Witness<C>` stays as-is; rich structural validation failures encode into lens-local namespaced `Diagnostic.kind` declarations owned by the lens instance, with no Substrate-owned closed-sum extension or cross-manager handoff.
