...
> [`docs/briefs/t-substrate-valuebody-list-worker.md`](t-substrate-valuebody-list-worker.md)
...
- **[`docs/briefs/t-substrate-valuebody-list-worker.md`](t-substrate-valuebody-list-worker.md)** — sibling sub-lane brief. Same overall pattern (extend `ValueBody` enum; new lowerer arm; R14 path narrows; consumer end-to-end visible). Read for the discipline; this brief inherits the structure and only differs in the variant shape + consumer set.
...
- **Parser dependency check (load-bearing).** Surface `SurfaceExpr` enum at `src/v3/compiler/src/parse_surface_generated.rs` has Record + List variants but **no `Map` variant**. The parser does not yet emit `SurfaceExpr::Map`. **Substrate work has a parser dependency**; brief must surface this explicitly.
...
**Critical caveat: parser dependency.** Unlike the list sub-lane (parser already emits `SurfaceExpr::List` for top-level scopes), this sub-lane requires extending the parser to emit `SurfaceExpr::Map` for top-level data bodies. The substrate work is bounded but the parser extension is not absorbable without explicit scope. Worker must coordinate or STOP-AND-ESCALATE if parser work is too large to bundle.
...
3. **Parser extension** (or STOP). New `SurfaceExpr::Map` variant in `parse_surface_generated.rs` + parser handling of map-literal syntax (e.g., `{"key": value, ...}` or whatever the surface syntax is). If parser scope balloons beyond a small extension, STOP-AND-ESCALATE — may need a coupled parser PR.
...
- [ ] Parser emits `SurfaceExpr::Map` for top-level data bodies.
...
- **Parser extension scope balloons** — if adding `SurfaceExpr::Map` requires extensive grammar rework or invasive `parse_parser_body.txt` changes, STOP. May need a coupled parser PR or re-scoping.
