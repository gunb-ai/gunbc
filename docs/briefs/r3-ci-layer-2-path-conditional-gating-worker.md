...
- Self-test (6 cases — expanded per Brian inline + codex BLOCKING convergent absorption on post-#2719 fail-open hole 2026-05-12): (a) docs-only PR triggers Layer 1 (entire `v3` skip — `code=false`); (b) a code PR touching only `src/v3/lenses/cost.dag` triggers ONLY the cost-related test groups (cost-dimension groups run; other-dimension groups skip); (c) a code PR touching only `.github/workflows/ci.yml` triggers ALL test groups via `force_full_run` (P3 fail-closed for shared infra); (d) a `push` to main runs everything; **(e) NEW — test-source-edit case**: a PR touching ONLY the cost-lens group's `*_test.rs` file (e.g., `src/v3/compiler/tests/integration/cost_lens_symbolic_consumer_test.rs` — real path verified on main) MUST trigger the cost-lens group regex (force_full_run = false BUT `skip_cost_lens` = false). If the group regex misses the test source itself, this case fail-opens — silently skipping a PR that explicitly edits the test; **(f) NEW — fixture-edit case**: a PR touching ONLY the cost-lens group's `tests/dag/*.dag` TestClaim fixture (e.g., `src/v3/compiler/tests/dag/t_r3_gate_87_cementing_regen_cost.dag` — real path verified on main) MUST trigger the cost-lens group regex. If the fixture arm is missing, this case fail-opens — silently skipping a PR that edits the canonical authority for what the test asserts. Bridge-tier proxy for the NodeRef-intersection step; dissolves with regex column when affected-set lens lands.
