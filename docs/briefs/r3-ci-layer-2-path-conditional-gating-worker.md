...
(c) **NEW per-group required-paths mapping** — for each group, hand-author the required-paths regex. The regex MUST include THREE arms (per Brian inline + codex BLOCKING absorption 2026-05-12 — convergent finding on post-#2719 fail-open hole):

  1. **Transitive `src/v3/*` source dependencies** — files the group's tests transitively depend on (existing arm).
  2. **The group's OWN test source files** (`*_test.rs` paths under `src/v3/compiler/tests/integration/`) — without this arm, a PR that edits a slow test would be classified as "unaffected" by the group's own regex and silently skipped. This is the fail-open boundary class P3 forbids: the test edit is exactly the change-class the test MUST run on.
  3. **The group's `tests/dag/*` TestClaim fixture paths** (`*.dag` files under `src/v3/compiler/tests/dag/`) — without this arm, a PR that edits a TestClaim fixture (the canonical authority for what the test asserts) would be skipped. Bridge-tier proxy for the NodeRef-intersection step at post-dissolution; preserves fail-closed direction at bridge.

  **Per-group regex completeness invariant**: a per-group `required_paths_regex` is REJECTED at Mgr-fill review if it fails ANY of:
  - Test-source arm: at least one alternate matches a `*_test.rs` path under `src/v3/compiler/tests/integration/` for the group's tests.
  - Fixture arm: at least one alternate matches a `*.dag` path under `src/v3/compiler/tests/dag/` for the group's TestClaim fixtures.
  - Dependency arm: alternates cover the transitive `src/v3/*` deps.

  **Self-test obligation** (per codex BLOCKING absorption 2026-05-12): the CI-level self-test suite at §5 MUST include a test-source-edit-only case for each group — a representative diff that touches ONLY the group's test source (no deps) verifies the regex correctly classifies the group as affected (force_full_run = false but `skip_<group>` = false because group regex matches). And a fixture-edit-only case (touches only `tests/dag/*.dag` TestClaim) verifies the fixture arm. This is bridge-tier — dissolves alongside the regex column when affected-set lens lands.

  This is the bridge-debt artifact; dissolves when the affected-set lens lands.
...
- Self-test (`4 baseline + 2 × N_groups` cases — expanded per Brian inline + codex BLOCKING convergent absorption on post-#2719 fail-open hole 2026-05-12 + openai-pro BLOCKING per-group matrix absorption 2026-05-12): the 4 baseline cases apply once for the whole CI flow; the 2 per-group cases MUST be replicated for EVERY Layer 2 group in the inventory. **The fail-open class is per-group; under-testing one group leaves the same hole open for that group's tests.** **Baseline (4 cases — apply once)**: (a) docs-only PR triggers Layer 1 (entire `v3` skip — `code=false`); (b) a code PR touching only `src/v3/lenses/cost.dag` triggers ONLY the cost-related test groups (cost-dimension groups run; other-dimension groups skip); (c) a code PR touching only `.github/workflows/ci.yml` triggers ALL test groups via `force_full_run` (P3 fail-closed for shared infra); (d) a `push` to main runs everything. **Per-group matrix (2 cases × N_groups)** — for each group `G` in the inventory: **(e) test-source-edit case for G**: a PR touching ONLY group `G`'s `*_test.rs` file MUST trigger `skip_G` = false via group regex match (force_full_run = false BUT group regex matches). If the group regex misses the test source itself, this case fail-opens for group `G` — silently skipping a PR that explicitly edits its test. Cost-lens worked example: PR touching ONLY `src/v3/compiler/tests/integration/cost_lens_symbolic_consumer_test.rs` (real path verified on main) MUST trigger `skip_cost_lens` = false; **(f) fixture-edit case for G**: a PR touching ONLY group `G`'s `tests/dag/*.dag` TestClaim fixture MUST trigger `skip_G` = false via group regex match. If the fixture arm is missing, this case fail-opens for group `G`. Cost-lens worked example: PR touching ONLY `src/v3/compiler/tests/dag/t_r3_gate_87_cementing_regen_cost.dag` (real path verified on main) MUST trigger `skip_cost_lens` = false. **Acceptance gating**: Mgr-fill REJECTS the brief if the per-group matrix is incomplete — every Layer 2 group MUST have its own (e) + (f) cases listed AND verified (worker provides representative diffs per group, or scripted enumeration via per-group rows in the inventory table). Bridge-tier proxy for the NodeRef-intersection step; dissolves with regex column when affected-set lens lands.
