...
| `provenance` | `src/v3/lenses/provenance.dag` | v3-native | Green: `LensOutputEquals` harness `src/v3/compiler/tests/dag/t_r3_gate_87_cementing_regen_provenance.dag` now carries the former Rust compile-to-DAG literal `Source` and sum `Computed` origin receipts. |
