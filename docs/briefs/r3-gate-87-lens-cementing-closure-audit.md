...
| `cost_symbolic` | `src/v3/lenses/cost.dag` | Real v2 counterpart (v2 `CostExpr` embedded in complexity) | Green: `.dag` symbolic-cost receipts `src/v3/compiler/tests/dag/t_r3_gate_87_cementing_regen_cost_symbolic.dag` cover the COMPLETE-row authority via `SymbolicCostExprEquals` / `SymbolicCostExprEqualsForBindParam`; residual `src/v3/compiler/tests/integration/cementing/cost_lens_symbolic_consumer_test.rs` is a host-wrapper pin, not the COMPLETE-row cementing authority. |
