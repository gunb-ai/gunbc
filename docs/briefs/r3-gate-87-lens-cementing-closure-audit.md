...
| `provenance` | `LensOutputEquals` over an `Origin` projection in `t_r3_gate_87_cementing_regen_provenance.dag` | `r3_gate_87_provenance_origin_rust_receipt_on_literal_bind` | The `.dag` receipt covers the literal-origin projection; full per-`Behavior` `Origin` mirror cases are still host-side until sum-typed expected values are complete. | T-Tests-As-Data carrier completeness for sum-typed lens outputs. |
