...
- **Status**: **(c) folded-carrier RATIFIED by Director** at #828 c#4412301889 + (A)-modified disposition at c#4413322671. **Q3 sub-disposition (a)(ii) RATIFIED** at c#4413284764 (`violates: fn(Output, Budget, Budget) -> Bool` full-signature extension). Per (A)-modified sequencing: PR #2360 ships folded-carrier as-is closing #55a; Substrate Mgr authors (a)(ii) extension PR co-closing #55a + #55b. See top-of-canvas authoritative table for binding shape.
...
- **Gate #55b** (fail-closed enforcement) — **PENDING**: `LensEnforcement` substrate-extension to land per Q-WAD-S2-Output **(a)(iii)-β-simplified** Director ratification at #828 c#4413567822 + c#4413581627 (NOTE: prior (a)(ii) ratification at c#4413284764 was reversed; (a)(iii)-β-simplified is the locked disposition). Shape: `project: fn(Output) -> Result<Budget, ProjectionFailure>` (signature change on `project`, NOT on `violates`); existing complexity_enforcement_project body wraps in `Ok(...)`; observation-driven lenses return `Err(ProjectionFailure::*)` on non-Observed report-state variants. Framework apply: `match project(output) { Err(_) => fail-closed; Ok(b) => violates(declared, b) }` — preserves typed report-state evidence per P3/C-8 without Budget fabrication. Substrate-extension lives in Mgr-scope PR (currently PR #2400 in flight; CI-blocked on DSL Result-emission mismatch escalated at c#4413661418).

Per Director (A)-modified disposition at #828 c#4413322671: gates #55a + #55b co-close in the LensEnforcement (a)(iii)-β-simplified extension PR (Substrate Mgr scope). Worker scope = #55a anchor + carrier landing (PR #2360 MERGED 2026-05-09); Mgr-tier extension PR adds project Result-typed signature + ProjectionFailure substrate type. **#55b state**: PENDING extension PR merge to main.
