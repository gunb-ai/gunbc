...
- **Phase 2 — full pilot enumeration + mirror retirement**: blocked on R2 T-Substrate 4th sub-lane (top-level `ValueBody::List`/aggregate extension). Brief forward-planned at [`t-ground-engine-phase-2-enumeration.md`](t-ground-engine-phase-2-enumeration.md) (not yet authored). Three substrate consumers converge on this sub-lane: tokenizer charclass phase-2, Engine Phase 2, `kernel_algebra_profile` mirror dissolution.
