...
| AC-2-COMPLEXITY-RED | `cargo test -p v3-compiler complexity_violation_compile_error_demonstrated` (or its v4 `TestClaim` runner equivalent per **DB-15**) | On the red snippet: `EnforcedApplication` walk runs `complexity_lens` on the section, obtains a composed `ComplexityBound`, and `LensEnforcement.violates(composed, budget)` returns **true** ⇒ fold-pass emits a typed **`Diagnostic`** (severity per declaration) into `DimensionFail.violations` (per **`design-lens-framework.md` §2** `DimensionReport` shape). Per-Behavior `Witness::Inhabits(...)` reads succeed (no missing facts); **test passes** asserting the `Diagnostic` is present and its `reason`/span name both the application site and the offending node | Cites **`design-lens-application-surface.md` §4.1** closure gate `complexity_violation_compile_error_demonstrated`. Carrier: **enforcement `Diagnostic`**, NOT `Witness::Violates`. ≥1 red row in §Witnesses. |
| AC-2-COMPLEXITY-GREEN | Same harness on **green** snippet | Per-Behavior reads return `Witness::Inhabits(...)`; fold composes to `ComplexityBound::O_log_n`; `LensEnforcement.violates(composed, budget)` returns **false** ⇒ **zero** enforcement `Diagnostic`s; aggregate is **`DimensionOk { dimension_name: "complexity", composed: O_log_n, witnesses: [...] }`**; **test passes** | ≥1 green row in §Witnesses. |
