...
| AC-2-COMPLEXITY-RED | `cargo test -p v3-compiler complexity_violation_compile_error_demonstrated` (or its v4 `TestClaim` runner equivalent per **DB-15**) | On the red snippet: the `EnforcedApplication<ComplexitySummary, AsymptoticClass, AsymptoticClass>` walk (per **`design-lens-application-surface.md` §4.1** parametric shape) runs `complexity_lens` on the section; per-Behavior reads return `Witness::Inhabits(<step ComplexityBound>)` per **`src/v4/lens/complexity.dag`** header `Node -> Witness<ComplexityBound>`; the fold composes these per-step bounds into a **`ComplexitySummary`** lens-output value (the §4.1 `Output` parameter). The enforcement step then evaluates `complexity_enforcement.violates(summary: ComplexitySummary, budget: AsymptoticClass) == true` (matching `LensEnforcement.violates(output, budget) -> Bool` from `design-lens-application-surface.md` §5; `enforcement.project(summary) -> AsymptoticClass` is the projected coordinate, used for diagnostic carriage but **not** swapped in for the lens `Output` carrier). The fold-pass emits a typed **`Diagnostic`** (severity per declaration) accumulated into `DimensionReport<ComplexitySummary>::DimensionFail.violations: List<Diagnostic>` (per **`design-lens-framework.md` §2**); **test passes** asserting the `Diagnostic` is present and its `reason`/span name both the application site and the offending node | Cites **`design-lens-application-surface.md` §4.1** closure gate `complexity_violation_compile_error_demonstrated`. Carrier: lens output is **`ComplexitySummary`**; budget/projection coordinate is **`AsymptoticClass`**; failure carrier is **enforcement `Diagnostic`**, NOT `Witness::Violates`. ≥1 red row in §Witnesses. |
| AC-2-COMPLEXITY-GREEN | Same harness on **green** snippet | Per-Behavior reads return `Witness::Inhabits(<step ComplexityBound>)`; fold composes a **`ComplexitySummary`** whose projected asymptotic class is `AsymptoticClass::O_log_n`; `complexity_enforcement.violates(summary, budget: AsymptoticClass::O_log_n) == false` ⇒ **zero** enforcement `Diagnostic`s; aggregate is **`DimensionReport<ComplexitySummary>::DimensionOk { dimension_name: "complexity", composed: <ComplexitySummary>, witnesses: [...] }`**; **test passes** | ≥1 green row in §Witnesses. |
...
- **Asserted outcome (§2):** per-Behavior `read` returns `Witness::Inhabits(<step ComplexityBound>)` at every leaf (no missing facts; per `src/v4/lens/complexity.dag` header `Node -> Witness<ComplexityBound>`); fold composes a **`ComplexitySummary`** (the §4.1 `EnforcedApplication<ComplexitySummary, AsymptoticClass, AsymptoticClass>` `Output` parameter); the enforcement step evaluates `complexity_enforcement.violates(summary, AsymptoticClass::O_log_n) == true` (signature `violates(output: ComplexitySummary, budget: AsymptoticClass) -> Bool` per `design-lens-application-surface.md` §5; `enforcement.project(summary) -> AsymptoticClass::O_n_squared` is the projected coordinate for diagnostic carriage). The fold-pass emits a typed **`Diagnostic`** with `reason: "complexity class O(n²) exceeds declared budget O(log n)"` and `span` covering both the application site and the offending nested-fold node; the `Diagnostic` is accumulated into **`DimensionReport<ComplexitySummary>::DimensionFail.violations: List<Diagnostic>`** per `design-lens-framework.md` §2. **Output carrier = `ComplexitySummary`; budget/projection coordinate = `AsymptoticClass`; failure carrier = enforcement `Diagnostic` (NOT `Witness::Violates`).**
