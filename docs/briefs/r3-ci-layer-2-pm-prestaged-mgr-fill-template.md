...
| E | `dimension::analyze_complexity_tests::.*` | `[Complexity]` | `^(src/v3/lenses/(complexity|cost)\.dag|src/v3/compiler/src/(analyze_complexity|dimension).*\.rs|src/v3/compiler/tests/integration\.rs)$` | medium | gate lands → Complexity. **Lens-path correction**: live authority at `src/v3/lenses/complexity.dag` + `src/v3/lenses/cost.dag`. **Test-source arm correction** (codex BLOCKING on PR #2721): tests live inline as a module in `src/v3/compiler/tests/integration.rs` (not as a standalone file under `tests/integration/dimension*.rs`); prior regex's `dimension.*\.rs` arm matched nothing → fail-open if integration.rs edited. Replaced with `src/v3/compiler/tests/integration\.rs` (broad-but-correct per fail-closed default) |
| E | `dimension::fail_closed_tests::.*` | `[Complexity]` | `^(src/v3/lenses/(complexity|cost)\.dag|src/v3/compiler/src/(analyze_complexity|dimension).*\.rs|src/v3/compiler/tests/integration\.rs)$` | medium | same. **Test-source arm correction** (codex BLOCKING): tests live inline in `tests/integration.rs`; added that path to regex |
| E | `e7_analyze_complexity_integration::.*` | `[Complexity]` | `^(src/v3/lenses/(complexity|cost)\.dag|src/v3/compiler/src/(analyze_complexity|dimension|public_api).*\.rs|src/v3/compiler/tests/integration\.rs)$` | medium | same. **Test-source arm correction** (codex BLOCKING): tests live inline in `tests/integration.rs`; added that path to regex |
| E | `lane2_stage_2f_dimension_test::.*` | `[Complexity, Cost]` | `^(src/v3/lenses/(complexity|cost)\.dag|src/v3/compiler/src/(lens_complexity|dimension).*\.rs|src/v3/compiler/tests/integration\.rs)$` | medium | "composed matches lens" reads BOTH analyze_symbolic_cost (Cost) and Complexity composition. **Test-source arm correction**: tests live inline in `tests/integration.rs`; added that path |
...
| H | `sg2c1_parse_tables_authority_test::.*` | `[Value]` | `^(src/v3/compiler/parse_tables\.dag|src/v3/(compiler|std)/tokenize\.dag|src/v3/compiler/src/parse.*\.rs|src/v3/compiler/tests/integration/sg2c1_parse_tables_authority_test\.rs)$` | medium | SG-2c-1 close. **Path correction**: live authorities at `src/v3/compiler/parse_tables.dag` + `src/v3/(compiler|std)/tokenize.dag`, not stale `dsl/std/parse_tables.*`. **Regex correction** (openai-pro APPROVE_WITH_COMMENTS on 45fc195a): unescaped pipe for alternation; prior `\|` was a markdown-cell escape that would have been interpreted as literal `compiler|std` by a regex engine, not alternation between `compiler` and `std`. **Test-source arm correction** (codex BLOCKING on PR #2721): test lives in standalone file `src/v3/compiler/tests/integration/sg2c1_parse_tables_authority_test.rs`; added that path to satisfy 3-arm completeness invariant |
...
| H | `t_ci_workflow_as_data_demo_test::.*` | `[Value, Cost, Timing]` | `^(src/v3/std/workflows\.dag|src/v3/compiler/src/(workflow|evaluator).*\.rs|src/v3/compiler/tests/integration/t_ci_workflow.*\.rs)$` | medium | workflow-as-data demo reads Value (workflow shape) + Cost (general cost lens) + **Timing** (user-defined dim per `DimensionReport<TimingMeasurement>` / `ci_modeled_timing` in the test). **Path correction**: live authority at `src/v3/std/workflows.dag`. **Dimension correction** (codex BLOCKING on PR #2721): prior revision normalized Timing into Cost; Timing is a user-defined dim distinct from generic Cost (per THESIS user-defined dimensions + design doc §2 open enum); narrowing would silently skip the test when only timing-shape changes (not generic Cost) — carrying Timing explicitly preserves fail-closed coverage. Demonstrates this template's open-enum support per §1 |
