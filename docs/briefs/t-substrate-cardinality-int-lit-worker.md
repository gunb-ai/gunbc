...
1. **`IntLit` carries a single canonical unbounded-magnitude payload; i64 (or other target-width) compatibility views derive AFTER reconciliation, not before.** **Specified shape**: replace `LiteralBits::Int(i64)` with `LiteralBits::Int(<canonical-unbounded>)` (e.g., `i128` as M2-bridge, or a typed unbounded magnitude when `std.natural` lands — surface choice in PR description). Do **NOT** add a parallel field or coexisting variant: that would create dual representations of the same fact at the same level and violate single-authority discipline. Pre-reconciliation, every int-literal is the single canonical unbounded form. Post-reconciliation, downstream consumers (emit, lens) project to target-width compat views from the canonical form via the reconciliation-decided target algebra. STOP-AND-ESCALATE if execution surfaces a consumer that requires the i64-pre-narrowed shape pre-reconciliation (would indicate a hidden coupling that needs Director scoping).
