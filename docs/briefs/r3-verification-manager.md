...
| Gap 9 — show-the-correct-code / gate #106 | Implement the Verification portion of the Diagnostic-with-correction coverage program after PM authors the §1.8 row and Substrate ratifies the `Correction` carrier shape. This is IN-R3 at 100% absolute coverage; no threshold relaxation or R4 carve is available. | Every `Diagnostic` value carries mandatory `correction: Correction` (`LiveCorrection` or `DeferredCorrection`), no `Option`/`None` form is representable, and the full test corpus fires zero `DeferredCorrection` diagnostics. |
...
| **Actual-close workstream: Gap 9 show-correct-code implementation** | M-L | **Verification implementation owner after PM/Substrate prerequisites** — implement the Diagnostic-with-correction retrofit once PM authors §1.8 row #106 and Substrate lands the ratified mandatory `Correction` sum-carrier shape. The close target is absolute: all fired diagnostics in the corpus use `LiveCorrection`, with zero `DeferredCorrection` survivors. | PM-authored §1.8 row #106 + Substrate-ratified `Correction` carrier; diagnostic class audit. |
...
- **Gap 9 / gate #106** — after PM/Substrate prerequisites, dispatch the Diagnostic-with-correction implementation work to achieve mandatory correction coverage and zero `DeferredCorrection` corpus survivors.
