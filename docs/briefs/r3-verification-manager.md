...
| 3 | Canonical lens-name dispatch | **PB** | **retired** | R3 gate #33 — `test_runner.rs` no longer dispatches `LensOutputEquals` through canonical lens bytes or lens declaration spelling; `canonical_lens_bridge_ratchet_test.rs` pins the residual categories at zero and `bridge_ledger.dag` marks the residual row `Retired`. |
...
**Net position:** 2 retired (#1272 secret opacity + R3 gate #33 canonical lens-name), 1 narrow slice landed (#1014 + #1192 lower-helper), 1 outstanding (#1171 suspended `reconcile_with_compile_body` only — `bridge_include_str_side_channels_retired` still open; tracked separately by closure-ledger update #1283), 1 R3-deferred (#1273 SourceSpan.file participation). **Unified `bridge_retirement_ledger_zero` gate remains open** until the remaining open rows fire structurally — row stays in-flight per closure ledger discipline.
