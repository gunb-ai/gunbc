...
- **R3 structure authority:** [`docs/r3-structure.md`](../r3-structure.md), amended by the 2026-05-13 actual-close plan in [`docs/r3-actual-close-plan.md`](../r3-actual-close-plan.md). Names this manager owner of 6 lanes: T-Verification-L4-L7-Direct + T-Verification-L5-Corpus + T-Free-Consequences-Demonstration + T-Tests-As-Data-Completeness + T-Lens-Self-Application, plus the TC bundle as absorbed responsibility; 2 cross-program partners: T-Lens-Behavioral-Parity + T-Lens-Application-Surface; and the `bridge_retirement_ledger_zero` audit gate of T-Bridge-Retirement.
...
## Owned program scope (6 lanes + 2 partners + actual-close workstreams + 1 ledger gate)
...
| **Lane 6: TC1/TC2/TC3 formal-grounding bundle** | S-M | **Absorbed lane responsibility** — strict-fire activation tracking for the formal-grounding deferred claims via the unified `BinaryDimensionReportEquals` path. This preserves the current `r3-structure.md` manager-row total while keeping the implementation cadence as audit/activation rather than a separate standalone dispatch program. | Substrate-owned predicate/modifier support; R2-Evaluator strategy/report producers; T-FixedPoint for TC3 |
...
- **Formal-grounding activation surface** — Lane 6 tracks TC1/TC2/TC3 strict-fire activation as an absorbed formal-grounding responsibility.
...
- **Lane 6**: TC1/TC2/TC3 strict-fire activation via unified `BinaryDimensionReportEquals` once Substrate lands the predicate/modifier support and each producer prerequisite is covered.
...
The brief no longer treats the original 3-lane standby surface as the authority. Current scope is the full `r3-structure.md` Verification Manager row: 6 lane surfaces + 2 cross-program partners + the bridge ledger gate.
...
- **TC1/TC2/TC3 formal-grounding bundle** — track as Lane 6 / absorbed responsibility; dispatch only concrete strict-fire activation slices.
