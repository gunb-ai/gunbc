...
- `FunctionPointerPrimitive { signature: FnSignature, qualifiers: FnQualifiers, .. }` — signature-only carrier; pointer-sized; multiple function items collapse to the same row when signatures match.
- `ClosurePrimitive { signature: FnSignature, captures: CaptureSet, fn_trait: FnTrait, .. }` — anonymous compiler-generated; `fn_trait` (`Fn`/`FnMut`/`FnOnce`) is derived from `captures`, not declared.
