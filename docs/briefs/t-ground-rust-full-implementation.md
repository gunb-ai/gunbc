...
- **Function item.** Each named `fn` (or `impl` method) has a unique anonymous **function-item type** carrying both *identity* (which named function) and signature (param/return types). Zero-sized. Coerces to a function pointer with the same signature, but the item identity is lost on coercion. Authority: Rust Reference §Function item types.
...
- `FunctionItemPrimitive { fn_decl: DeclarationId, signature: FnSignature, .. }` — one row per named function (carries identity); zero-sized.
...
- `ClosurePrimitive { signature: FnSignature, captures: CaptureSet, .. }` — anonymous compiler-generated. The `Fn` / `FnMut` / `FnOnce` capability set is **derived from how the closure body uses its captures**, not from capture mode. The cumulative tower per Rust Reference §Closure types: every closure inhabits `FnOnce`; it additionally inhabits `FnMut` if the body does not move out of any captured value; it additionally inhabits `Fn` if the body also does not mutate any captured value. The `move` keyword controls capture *mode* (by-value vs by-reference) and on its own neither precludes `Fn` nor `FnMut` — a `move` closure that only reads its captures still inhabits `Fn`. So `captures` carries (a) per-capture mode (by-ref / by-mut-ref / by-value/move) AND (b) per-capture body-use (read / mutate / consume); the capability set is the *cumulative* derivation over (b). The capability set is **NOT a stored field** on this primitive — a stored `fn_trait` field would author a parallel representation of a fact already determined by `captures` body-use coordinates (P2 violation).
