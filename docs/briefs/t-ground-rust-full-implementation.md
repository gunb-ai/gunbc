...
- **Numeric — floating-point family.** `f32`, `f64`. New `RustPrimitive` variant per the variant-aware partition lock (`grounding-pilot-receipt.md`). **M9 DFS receipt — partial parent at HEAD; TWO active gates:**
  - **(a) `ApproximateField<F>` is the *intended* parent** per `src/v3/std/approximate_field.dag:75` (`type ApproximateField<F> { base: Field<F>, rounding, precision, special_values, subnormal_policy }`). The "Float inhabits ApproximateField" line at `dsl/std/algebra.dag:66` is **prose in a comment block**, NOT a live `inhabits` declaration — flagged by reviewer; corrected here. The migration of float types to `ApproximateField` has not landed.
  - **(b) Live float substrate at HEAD declares the wrong parent**: `dsl/std/float.dag:14-15` reads `type Float32 = Field<Word32>` / `type Float64 = Field<Word64>` — i.e., exact-`Field` over a fixed-width word. This is structurally inadequate for IEEE-754 (rounding, NaN, signed-zero, subnormal policy are all elided) and is the gap T-NumericConstruction-ApproximateField is dissolving.
  - **(c) Real / base-carrier gate** still active (`src/v3/std/approximate_field.dag:11-14`; audit doc: `docs/audit/t-numeric-construction-approximate-field-real-parameter-stop.md`): even once Float migrates to `ApproximateField`, the `F` parameter has no honest substrate inhabitance target — `Rational` (#1508) names `Field<FieldOfFractions<Int>>` and cannot honestly be spelled as `F`; no `Real` alias exists.
  - **Therefore `f32`/`f64` row authoring is GATED on TWO substrate moves**: (1) Float migration from `Field<Word*>` to `ApproximateField<F>` (replacing the live `dsl/std/float.dag:14-18` declarations); (2) Real / base-carrier convention decision. Worker STOP-and-escalates at the §A float row until both clear. The `(precision, rounding, special_values, subnormal_policy)` coordinate-distinguishability discipline still applies post-clearance; binary32 vs binary64 differ on coordinates, not parent identity. Authority cited: Rust Reference §Floating-point types + IEEE-754 §3 + Real-parameter STOP audit doc above.
