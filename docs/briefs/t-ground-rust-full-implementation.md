...
- **Numeric — floating-point family.** `f32`, `f64`. New `RustPrimitive` variant per the variant-aware partition lock (`grounding-pilot-receipt.md`). **M9 DFS receipt — substrate parent FOUND, but base-carrier gate ACTIVE:** `src/v3/std/approximate_field.dag:75` declares `type ApproximateField<F> { base: Field<F>, rounding, precision, special_values, subnormal_policy }`, and `dsl/std/algebra.dag:66` records `Float inhabits ApproximateField`. **However, the `F` parameter has no honest substrate inhabitance target at HEAD** — `src/v3/std/approximate_field.dag:11-14` documents the **Real alias gate** (audit doc: `docs/audit/t-numeric-construction-approximate-field-real-parameter-stop.md`): `Rational` (#1508) names `Field<FieldOfFractions<Int>>` and cannot honestly be spelled as `F` for `ApproximateField<F>` without a design convention fix on carrier-vs-witness naming. No `Real` alias exists. **Therefore `f32`/`f64` row authoring is GATED** on the Real / base-carrier decision under T-NumericConstruction-ApproximateField slice. Worker STOP-and-escalates at the float row until the carrier convention is named. The `(precision, rounding, special_values, subnormal_policy)` coordinate-distinguishability discipline still applies once the gate clears; binary32 vs binary64 differ on coordinates, not parent identity. Authority cited: Rust Reference §Floating-point types + IEEE-754 §3 + the Real-parameter STOP audit doc above.
...
- `FloatPrimitive { approximate_field: ApproximateField<F>, .. }` — inhabits the existing `ApproximateField<F>` parent at `src/v3/std/approximate_field.dag:75`. **Authoring blocked on the Real / base-carrier decision** (the `F` parameter has no honest substrate target at HEAD per the Real-alias STOP audit) — see §A floating-point bullet. Once the gate clears, binary32 (`f32`) and binary64 (`f64`) differ on `(precision, special_values, subnormal_policy)` coordinate values, not parent identity. Distinct from `NonIntegerPrimitive` because pilot's `NonIntegerAlgebra` was Bool/Unit-only.
...
- `ImplTraitReturnPrimitive { trait_bounds: TraitBoundSet, opaque_captures: OpaqueCaptureSet, edition_capture_policy: EditionCapturePolicy, .. }` — return-position `impl Trait` (abstract return type, callee-chooses; existential/opaque). The `opaque_captures` record carries the captured generic parameters that flow into the opaque type: `{ type_params: List<TypeParamRef>, const_params: List<ConstParamRef>, lifetime_params: List<LifetimeParamRef>, precise_capture_restriction: Option<UseList> }`. The `edition_capture_policy` record carries the **edition-sensitive default-capture rule** that determines what's captured when `precise_capture_restriction` is `None`: `{ edition: RustEdition, default_capture: DefaultCaptureRule }`. Rust 2015/2018/2021: lifetimes are NOT captured by default (must be named in bounds); type/const params ARE captured. Rust 2024+ (per RFC 3498): ALL generic params in scope are captured by default. The substrate row MUST carry the edition because the *same source text* expands to different `opaque_captures` sets across editions. **`use<>` legality constraints**: `precise_capture_restriction` is legal only on `fn` items and inherent methods, not on trait method definitions (per RFC 3617); the row MUST validate that the precise-capture restriction site supports it before declaring `Some(_)`. Authority: Rust Reference §Impl Trait → Abstract return types + RFC 3498 (Rust 2024 default-capture rules) + RFC 3617 (precise capturing).
...
7. **Float row reached without Real / base-carrier decision** — `ApproximateField<F>`'s `F` parameter has no honest substrate target at HEAD per the Real-alias STOP audit (`docs/audit/t-numeric-construction-approximate-field-real-parameter-stop.md`). Worker STOP-and-escalates at the §A `f32`/`f64` row until T-NumericConstruction-ApproximateField names the carrier convention; do NOT pick a placeholder for `F`. Separate sub-condition: even after the carrier lands, if a Rust-required precision/rounding/special-values/subnormal-policy combination doesn't fit the existing `(precision, rounding, special_values, subnormal_policy)` coordinate space, escalate again — that's a substrate-shape change on `ApproximateField<F>` itself.
