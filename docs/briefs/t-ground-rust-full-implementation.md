...
- **Numeric — floating-point family.** `f32`, `f64`. New `RustPrimitive` variant per the variant-aware partition lock (`grounding-pilot-receipt.md`). **M9 DFS receipt — substrate parent FOUND:** `src/v3/std/approximate_field.dag:75` declares `type ApproximateField<F> { base: Field<F>, rounding, precision, special_values, subnormal_policy }`, and `dsl/std/algebra.dag:66` already records `Float inhabits ApproximateField`. Rust `f32`/`f64` therefore inhabit `ApproximateField<F>` directly — NOT a new sibling `FloatAlgebra`. Algebra inhabitance is **not** `OrderedRing` / `Semiring` (IEEE-754 fails ring axioms — NaN, signed zero, non-associative addition), but the existing `ApproximateField<F>` substrate already partitions IEEE-754's observable approximation policies into structural axes, so the rejection routes correctly. The structural difference between binary32 and binary64 is then a row-level difference on `(precision, rounding, special_values, subnormal_policy)` coordinates — both inhabit the same `ApproximateField<F>` parent with distinct coordinate values. Authority cited: Rust Reference §Floating-point types + IEEE-754 §3 (binary32 / binary64 formats) for the per-precision axis values.
...
- **`impl Trait` — split into two rows per position** (Rust Reference §Impl Trait distinguishes them as separate spec facts):
  ...
  - **Return-position `impl Trait`** — abstract/opaque return type; *callee* chooses the concrete type and the caller sees only the trait-bound interface. Carries trait-bound set + an `opaque_captures` record (which type/const/lifetime parameters from the enclosing scope flow into the opaque type, plus the `use<>` precise-capture restriction per RFC 3617 / Rust 2024). Without `opaque_captures` the row drops the auto-capture-vs-precise-capture distinction the borrow checker depends on. Existential, not universal. Authority: Rust Reference §Impl Trait → Abstract return types + RFC 3617.
...
- **`Vec<T>`** — owned, growable, contiguous heap buffer. Cardinality: `CardinalityBound::Unbounded` at HEAD (same `Cardinal`-substrate gap as Array/Slice in §A; retrofits to `Interval<Cardinal>::Unbounded` when `Cardinal` lands). Ownership: `Owned`; growability: `Growable`.
...
- **`HashMap<K, V>`** — hash-table-backed associative array. Inhabits `PartialFunction<K, V>` (`dsl/std/algebra.dag:428`). Refinement axes: `ordering: None`; **key-admissibility constraints** carried explicitly — `K: Hash + Eq` (Rust trait bounds the realization requires; without them the carrier is unconstructible). Trait-bound axes are NOT optional — \"hash-backed admissibility\" is the structural fact that distinguishes `HashMap` from `BTreeMap` at the realization step, not just ordering.
- **`BTreeMap<K, V>`** — B-tree-backed ordered associative array. Inhabits `PartialFunction<K, V>`; refinement axes: `ordering: Sorted`; **key-admissibility constraints** `K: Ord`. The axis discipline mirrors `HashMap`'s; `Hash + Eq` and `Ord` are *distinct* admissibility shapes (a key can be `Ord` without `Hash` — `f64` is `PartialOrd`-only and inhabits neither cleanly, which is itself a structural fact this row exposes).
- **`HashSet<T>`**, **`BTreeSet<T>`** — inhabit the existing `Set<T> = BooleanAlgebra<T>` authority declared at `dsl/std/types.dag:212` (per M9 DFS — `Set` is already in `dsl/std/`). Refinement axes: `BTreeSet<T>` carries `ordering: Sorted` + element-admissibility `T: Ord`; `HashSet<T>` carries `ordering: None` + element-admissibility `T: Hash + Eq`. The element-admissibility axis is required — `HashSet<T>` and `BTreeSet<T>` differ structurally on more than ordering (the realization-trait set distinguishes them at construction, not just at iteration). **Do NOT route through `PartialFunction<T, Unit>`** — P2 violation (parallel authority for `Set<T> = BooleanAlgebra<T>`).
...
- `FloatPrimitive { approximate_field: ApproximateField<F>, .. }` — inhabits the existing `ApproximateField<F>` parent at `src/v3/std/approximate_field.dag:75`; binary32 (`f32`) and binary64 (`f64`) differ on `(precision, special_values, subnormal_policy)` coordinate values, NOT on parent identity. Distinct from `NonIntegerPrimitive` because pilot's `NonIntegerAlgebra` was Bool/Unit-only.
...
- `ImplTraitReturnPrimitive { trait_bounds: TraitBoundSet, opaque_captures: OpaqueCaptureSet, .. }` — return-position `impl Trait` (abstract return type, callee-chooses; existential/opaque). The `opaque_captures` record carries the captured generic parameters that flow into the opaque type (per Rust Reference §Impl Trait → opaque types capture the generics in scope at the definition site): `{ type_params: List<TypeParamRef>, const_params: List<ConstParamRef>, lifetime_params: List<LifetimeParamRef>, precise_capture_restriction: Option<UseList> }`. The `precise_capture_restriction` records the Rust 2024 / RFC 3617 `use<>` syntax that explicitly narrows which generics are captured (e.g., `impl Trait + use<'a, T>` captures only `'a` and `T`, even if other generics are in scope); `None` means default-capture-all-in-scope. **Dropping `opaque_captures` loses the auto-capture-vs-precise-capture distinction**, which the borrow checker and trait-resolution depend on. Authority: Rust Reference §Impl Trait → Abstract return types + RFC 3617 (precise capturing).
...
7. **Float coordinate values can't be expressed against `ApproximateField<F>`'s axes** — e.g., a Rust-required precision/rounding/special-values/subnormal-policy combination doesn't fit the existing `(precision, rounding, special_values, subnormal_policy)` coordinate space. Signals a substrate-shape escalation on `ApproximateField<F>` itself, not a hierarchy gap.
...
11. **Repo / git prerequisites unsatisfied** at dispatch time (no clean working tree on the dispatching worker's environment). Implementation cannot start.
