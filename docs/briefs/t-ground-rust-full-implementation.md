...
- **Closure.** Anonymous compiler-generated type carrying captures. Trait inhabitance (`Fn` / `FnMut` / `FnOnce`) is a structural axis derived from capture-use, NOT a separate primitive. Authority: Rust Reference §Closure types.
...
- `ClosurePrimitive { signature: FnSignature, captures: CaptureSet, .. }` — anonymous compiler-generated. The `Fn` / `FnMut` / `FnOnce` capability set is **derived from `captures`** by structural rule (no by-move captures → `Fn`; mutable borrows → `FnMut`; consumed/moved values → `FnOnce`), so it is **NOT a stored field** on this primitive — `captures` is the sole authority. A `fn_trait` field would author a parallel representation of a fact already determined by `captures` (P2 violation).
