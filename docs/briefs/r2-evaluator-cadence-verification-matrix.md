...
| **PR-B — body evaluator** | Evaluator (worker brief pending) | **Not authored / not started** — [`r2-evaluator-manager.md`](r2-evaluator-manager.md) table. | **PR-A.1 through PR-A.3** implementation gates in PR-A brief. | Author PR-B **design** brief only after PR-A carriers exist; implementation still serializes after PR-E per manager cadence. | No structural body execution before frames + strategy boundary exist. |
