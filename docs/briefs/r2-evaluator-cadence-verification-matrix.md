...
| **PR-A.2 — `EvalFrame` / `EvalStateStack`** | **Worker C / merry-heron-351** | **Active** after PR-A.1 (Evaluator Manager coordinates). | PR-A.1 carriers; **unique-binding** `Map<PortId, Value>` disposition — merged dependency audit [PR #1222](https://github.com/gunb-ai/gunbc/pull/1222); PR-A brief: fail-closed map, not duplicate-admitting lists. | Declare evaluator-internal frames/stacks; mirror in Rust **without** new observable `Value` variants. | No `Value` inhabitants for closures; no skipping unique-binding carrier discipline. |
