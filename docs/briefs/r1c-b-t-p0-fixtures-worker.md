...
- **[`ROADMAP.md §"Lane acceptance — .dag gates"`](../../ROADMAP.md)** — T-P0 row: `p0_repeat_string_correct` [Day 1] · `p0_no_fabrication_sentinel` [ext] · `p0_rest_ops_aligned` [ext]. The Day-1 gate uses today's DB-15 schema; the two `[ext]` gates require T-TestGen schema extensions per ROADMAP authority.
...
The Day-1 fixture (`p0_repeat_string_correct`) is unblocked Day-1 — uses existing DB-15 schema (`Compiles` or `OutputEquals` predicate). The two `[ext]` fixtures depend on R1C-A's predicate-shape work IF they need new predicates beyond the DB-15 surface; **verify at brief authoring** by reading the integration test for each feature to determine the predicate shape needed.

## Three consumer-side requirements

1. **Author `p0_repeat_string_correct` TestClaim fixture** in `r1_gates.dag` (or sibling fixture file). Predicate: `Compiles` (proves repeat_string compiles correctly) OR `OutputEquals` (proves repeat_string output matches v2 oracle). Wire runner dispatch in `test_runner.rs`. Existing integration test `tests/integration/p0_std_render_repeat_string_test.rs` is the reference for input + expected output. **Day-1 unblocked** (uses DB-15 schema).
...
1. PR-1: `p0_repeat_string_correct` Day-1 fixture + runner dispatch (smallest deliverable; closure receipt for the unambiguous Day-1 gate).
...
- [ ] `p0_repeat_string_correct` `.dag` TestClaim authored + runner dispatch + gate evaluates `Pass`.
