...
| Rust module | G87-D5 handoff class | Predicate class (§2) | Owning lane / blocker | Expected SG-0 census delta |
|---|---|---|---|---|
| `tests/integration/cementing/cementing_provenance_origin_integration_test.rs` | Gate-87 residual. The gate-87 `.dag` provenance receipt covers the registry seam; this Rust row keeps the per-`Behavior` `Origin` mirror pinned until the exact carrier is data-authorable. | C-LensOutEq once unblocked; C-HandRustBlocker today. | Tests-as-data carrier completeness for sum-typed lens outputs (`docs/design-tests-as-data-completeness.md` §C5): author `Origin::{NoProducer, MissingPort, MissingBehavior, Source(NodeId), Computed(NodeId), Selected(NodeId), Accumulated(NodeId)}` expected values in `.dag` `TestPredicate` data. Target `.dag`: extend `tests/dag/t_r3_gate_87_cementing_regen_provenance.dag`. | -1 in the gate-87 residual bucket when the per-`Behavior` mirror moves into the existing `.dag` receipt and this Rust row leaves `EXPECTED_HAND_AUTHORED_TEST`. |
| `tests/integration/cementing/complexity_lens_behavioral_completion.rs` | Band-C bulk-port candidate, blocked. This is still a behavioral Band-C receipt for the complexity COMPLETE row, but it cannot port until report predicates can consume the full carrier. | C-HandRustBlocker today; C-LensOutEq on unblock. | T-LBP / gate #73 `Gate73_ReportPredicateCarriers`: `.dag` `TestClaim` predicates cannot yet consume the published `ComplexitySummary` report carrier. No `.dag` target yet; create one when the carrier lands. | -1 in the #84 cementing family on Gate-73 unblock plus same-PR move into a new `tests/dag/t_r3_*_complexity_*.dag` receipt. |
| `tests/integration/cementing/cost_lens_symbolic_consumer_test.rs` | Host-wrapper retirement, not Band-C. The `cost_symbolic` COMPLETE-row cementing already landed as data at `tests/dag/t_r3_gate_87_cementing_regen_cost_symbolic.dag`; this row now pins the host `symbolic_cost_of` wrapper path. | Outside §2 cementing selection for #84. | Gate #78 `per_call_pattern_at` / `symbolic_cost_of` host-wrapper retirement. Do not select this row for #84 cementing-class bulk-port. | 0 for #84 cementing. The eventual -1 belongs to the gate #78 host-wrapper retirement PR. |
| `tests/integration/cementing/memory_peak_cost_basis_demo.rs` | T-LAS demonstration scope. It is cementing-looking because it lives under `cementing/`, but the load-bearing claim is the lens-application surface memory-budget demonstration, not a gate-87 registry receipt. | C-HandRustBlocker today; C-LensOutEq or C-CompilesHelper on unblock depending on demonstration shape. | T-LAS Slice B lens-fold consumer / gate #91 parser-level `apply_lens(cost, DeclarationScope, Enforce { budget: SymbolicCost { dimension: Memory, ... } })`; gate #94 (`memory_peak_cost_basis_demonstrated`) is the consumer-side gate evidenced here. | -1 under T-LAS gate #91/#94 when a same-PR `tests/dag/t_r3_*_memory_peak_*.dag` replacement preserves max-dominance composition and `LensEnforcement` orientation semantics. |
