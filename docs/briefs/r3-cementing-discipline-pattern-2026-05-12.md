...
Census coverage check (G87-D5, refresh 2026-05-13): the four rows below are the complete set of paths under `src/v3/compiler/tests/integration/cementing/` that appear in `EXPECTED_HAND_AUTHORED_TEST` at HEAD. Gate-#87-adjacent SG-0 rows **outside** that directory (runner wiring, register correspondence, wiring scanner, consolidated regen Rust pin) are classified in §3.1 so #84 bulk-port workers do not treat them as `t_r3_gate_87_cementing_regen_<lens>.dag` port targets. No new `cementing/*.rs` residual rows have landed since the prior refresh; no §3 row has been ported away from Rust since the prior refresh, so the §3 dispositions stand. This brief makes no census-delta claim for §3 — it is docs-only aside from optional cross-refs from census comments.
...
| `tests/integration/lens_behavioral_parity_demonstration_test.rs` | Gate #73 (`lens_behavioral_parity_demonstration`) four-lens parity snapshot host receipt — T-LBP demonstration scope, not a `regen.dag` `LensRegistryEntry` harness row. Countdown symbolic-cost assertions already moved to gate-#87 `.dag` receipts; this file is the remaining multi-lens parity shell. | `docs/r3-program-plan.md` §1.8 row #73 + T-LBP; census −1 when the parity obligation is expressible as `.dag` `TestClaim` data without this host harness. |
| `tests/integration/lens_register_correspondence_test.rs` | Band-C **register correspondence** only: structural alignment between `docs/v3-lens-capability-register.md` (Band-C v2-cementing slice) and `cementing_dispatch` projection APIs — not a per-lens `t_r3_gate_87_cementing_regen_<lens>.dag` receipt and not a bulk-port pilot for a single registry lens. | Same evolution as `cementing_dispatch` / register-as-data (§1); dissolves when capability rows + wiring are asserted structurally without this Rust walk. |
| `tests/integration/t_pb_b_1_dag_runner_test.rs` | PB-B-1 `tests/dag` runner table for **multiple** suites (gate #74, gate-#87 regen inventory via `R3_GATE_87_CEMENTING_REGEN_SUITES`, R1C-D/E receipts). Runner-only wiring — not one isolatable per-lens cementing hand-path for #84 cementing-class selection. | T-PB-B / `pb_rust_tests_outside_residual_zero` (see census comment on the row — not gate #84 single-row closure); census −1 only when the runner obligations move behind structural `.dag` execution without this harness. |
