...
### §3.2 Cementing-looking census rows outside `integration/cementing/`

Workers grepping for “cementing” or “Band-C” still hit paths that are **not** `regen.dag` gate-#87 lens receipts. The table below is the post-#87 handoff so #84 **cementing-class** pilots do not treat these as `t_r3_gate_87_cementing_regen_<lens>.dag` work.

| Census row | Looks like Band-C cementing because… | Correct bucket | Owning lane / gate | SG-0 delta when resolved |
|---|---|---|---|---|
| `tests/integration/emission_provenance_lens_test.rs` | Module doc: “Cementing test” for `Lens<List<EmissionProvenance>>`. | **T-Substrate-Lens-Primitive structural slice** — substrate carrier + fixture-bound lens instance (Director Q1), not a `LensRegistryEntry` regen row. | T-Substrate / Class 5 Gap 3 cascade per module doc; brief `r3-substrate-emission-provenance-lens-worker.md`. | −1 when structural claims port to `.dag` / testgen over reflected `Dag`; do **not** add a gate-#87 regen file for this lens unless `regen.dag` gains a matching row under §1. |
| `tests/integration/lens_register_correspondence_test.rs` | Doc: “Band-C v2 cementing” — markdown slice ↔ `cementing_dispatch` structural projection. | **Register honesty / dispatch projection** — shares `cementing_dispatch` reader with §1 but asserts **capability-register** rows vs docs, not per-lens behavioral regen. | T-LensAPI honesty pass + `cementing_dispatch.dag` / `docs/v3-lens-capability-register.md` editors; coordinate with §1 single-authority edits. | −1 on `.dag` TestClaim (or generated consumer) that replaces the Rust diff; not a new `t_r3_gate_87_cementing_regen_*.dag` unless a **new registry lens row** is added under §1. |
| `tests/integration/lens_behavioral_parity_demonstration_test.rs` | `sg0_census_test` migration class `CementingV2Oracle`; module ties gate #73 to rows 84/87. | **Gate #73 host demonstration receipt** — four-lens frozen snapshot; countdown cost parity already moved to gate-#87 `cost_symbolic` `.dag` per module doc. | `lens_behavioral_parity_demonstration` / `Gate73_ReportPredicateCarriers` (ROADMAP #2610). | −1 when report-shaped slices become `.dag` predicates; **not** a bulk-port into gate-#87 regen harness. |
| `tests/integration/m2_substrate_inhabitance_test.rs` (cementing-named subtests only) | Comments: “cementing-only”, “per-arg cementing”, fixture `e_p_indirect_call_cementing.v3`. | **Substrate inhabittance / E-P** — indirect-call `SubValueRelation` pins, not lens-registry Band-C. | Substrate / lane-E-P scope per `m2_substrate_inhabitance_test` module context. | −1 only for the ported subtest harness; do not fold into `t_r3_gate_87_cementing_regen_*.dag` without a new `regen.dag` row. |
| `tests/integration/sg6_hand_authored_census_test.rs` (`sg6_cementing_dispatch_dag_is_wired_in_runner_receipt`) | “Band-C cementing” in comment — asserts runner text references `cementing_dispatch.dag`. | **SG-6 runner wiring ratchet** — merge-visible receipt that PB-B-1 harness stays wired. | SG-6 / `t_pb_b_1_dag_runner_test` maintenance; same structural class as gate #74 runner rows (see census P5(b) note on `t_pb_b_1_dag_runner_test.rs`). | −1 only if the assertion migrates to a stricter generated or `.dag` receipt; **not** #84 cementing-class lens port work. |

**Rule of thumb for #84 cementing-class selection:** eligible bulk-port rows are those whose dissolution is **replacing a behavioral Rust assertion with a `.dag` `TestClaim`** under `tests/dag/` while preserving the lens-subsumption story. If the row’s authoritative doc points at a **non-registry** gate (73, 78, 91, 94, substrate Q1, …), route it through that gate’s lane — do not expand the gate-#87 `R3_GATE_87_CEMENTING_REGEN_SUITES` inventory “by analogy.”
