...
Census coverage check (G87-D5, refresh 2026-05-13): the four rows below are the complete set of paths under `src/v3/compiler/tests/integration/cementing/` that appear in `EXPECTED_HAND_AUTHORED_TEST` at HEAD (authoritative list: `sg0_census_test.rs`). **Provenance dual receipt:** `t_r3_gate_87_cementing_regen_provenance.dag` carries PB-B-1 `LensOutputEquals` projections for variant shape; `cementing_provenance_origin_integration_test.rs` retains the full `Origin` + `NodeId` producer mirror (Codex PR #2894 review — no semantic dilution).

| Rust module | Predicate class (§2) | Blocker / owning lane | SG-0 census delta on port |
|---|---|---|---|
| `tests/integration/cementing/cementing_provenance_origin_integration_test.rs` | C-HandRustBlocker for full `Origin` literal in `.dag`; C-LensOutEq variant slice in harness today. | `Origin` + carried `NodeId` expected values in `TestPredicate` / tests-as-data carrier completeness (`docs/design-tests-as-data-completeness.md` §C5). `.dag` harness `t_r3_gate_87_cementing_regen_provenance.dag` covers PB-B-1 enumeratable variant checks only; Rust module is load-bearing for payload equality until carriers land. | -1 only when a single `.dag` artifact can express both runner visibility and full payload mirror; until then keep dual receipt (no census-only delete). |
