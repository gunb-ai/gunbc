...
| `parallelism` | `tests/dag/t_r3_gate_87_cementing_regen_parallelism.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_parallelism_lens_source_compiles` | `WorkflowParallelismReport` / `ParallelismUnsupportedDetail` carries typed left/right noncommute evidence and expected report literals become authorable as `.dag` data; replace with `LensOutputEquals(analyze_parallelism, ..., expected)`. Owning lane: DB-20 / Lane 2 Stage 2e parallelism behavioral completion. |
