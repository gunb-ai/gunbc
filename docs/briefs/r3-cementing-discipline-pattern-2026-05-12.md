...
Census coverage check (G87-D5, refresh 2026-05-13): the three rows below are the complete set of paths under `src/v3/compiler/tests/integration/cementing/` that appear in `EXPECTED_HAND_AUTHORED_TEST` at HEAD (authoritative list: `sg0_census_test.rs`). **G87-D5 landed:** `cementing_provenance_origin_integration_test.rs` retired — literal-bind `Source` + unary `Computed` `origin_of` seam witnesses now ride as second `TestClaim` in `tests/dag/t_r3_gate_87_cementing_regen_provenance.dag` via the existing gate-#87 Int projection (`gate87_provenance_sum_origin_computed` in `test_runner.rs`); SG-0 test census −1 same PR.
