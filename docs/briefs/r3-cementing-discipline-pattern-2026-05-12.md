...
### §2.2 Gate-#87 host-side Rust pins

The rows below are the Gate #87 receipts that are already stronger than bare `Compiles` in `.dag` form, but still need a Rust pin because the exact public carrier, strict user-module fixture, or generated-lens projection is not fully authorable in `.dag` test data. These pins are temporary Band-C receipts, not silent exceptions. When the missing carrier or runner capability lands, the same PR must broaden the `.dag` predicate and delete the paired host-side pin.

| Lens row | `.dag` receipt shape | Paired Rust pin | Missing carrier / runner capability and owning lane |
|---|---|---|---|
| `cost` | `LensOutputEquals(cost_of, ...)` Int projection plus `DifferentialEquals(v3_program_cost, v2_oracle_cost, ...)` in `tests/dag/t_r3_gate_87_cementing_regen_cost.dag` | `tests/integration/cementing/complexity_lens_behavioral_completion.rs` | Published `ComplexitySummary` expected literals, including nested `SymbolicCost`, become authorable in `.dag` predicates. Owning lane: T-LBP gate #73 report-predicate carrier authoring. |
| `cost_symbolic` | `SymbolicCostExprEquals` / `SymbolicCostExprEqualsForBindParam` in `tests/dag/t_r3_gate_87_cementing_regen_cost_symbolic.dag` | `tests/integration/cementing/cost_lens_symbolic_consumer_test.rs` | Residual host wrapper around `per_call_pattern_at` / `symbolic_cost_of` retires after the gate #78 generated-wrapper path can express the same call-site consumer without hand Rust. This is not a remaining Band-C gap for the `cost_symbolic` COMPLETE row. |
| `cost_target_realization` | Narrow `LensOutputEquals` Int witness in `tests/dag/t_r3_gate_87_cementing_regen_cost_target_realization.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_cost_target_realization_rust_receipt_resolves_type_realization_row` | Exact `Declaration?` / `TypeRealization` meta-row expected value becomes authorable as `.dag` data. Owning lane: T-CostLens-Composition epsilon target-realization carrier work. |
| `effect_enumeration` | Narrow `LensOutputEquals` Int witness in `tests/dag/t_r3_gate_87_cementing_regen_effect_enumeration.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_effect_enumeration_rust_receipt_on_minimal_program` | `EffectEnumerationReport` expected literals become stable `.dag` values after ambient resource / transport metadata and effect-set threading land. Owning lane: effect-enumeration / resources substrate follow-up. |
| `provenance` | `LensOutputEquals` projection in `tests/dag/t_r3_gate_87_cementing_regen_provenance.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_provenance_origin_rust_receipt_on_literal_bind`; `tests/integration/cementing/cementing_provenance_origin_integration_test.rs` | Full `Origin` sum expected values and per-`Behavior` mirror cases become authorable in `.dag` predicates. Owning lane: tests-as-data carrier completeness for sum-typed lens outputs (`docs/design-tests-as-data-completeness.md` §C5). |
| `structural_resolution` | `LensOutputEquals` Int projection in `tests/dag/t_r3_gate_87_cementing_regen_structural_resolution.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_structural_resolution_rust_receipt_on_literal_program` | Strict user-module fixtures can freeze `List<UnresolvedArrowBody>` without M1(2.8) opaque-body diagnostics. Owning lane: M1(2.8) strict-module carrier authoring. |
| `unused_parameters` | `LensOutputEquals` Int projection in `tests/dag/t_r3_gate_87_cementing_regen_unused_parameters.dag` | `r3_gate_87_lens_cementing_regen_receipts_test::r3_gate_87_unused_parameters_rust_receipt_on_literal_program` | Strict user-module fixtures can freeze `List<UnusedParameter>` expected values without M1(2.8) opaque-body diagnostics. Owning lane: M1(2.8) strict-module carrier authoring. |
