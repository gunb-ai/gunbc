...
1. **Per-target emission semantics** (the substantive canvas surface):
   - **YamlStatic** (Slice 4 deliverable): body of `project_github_actions(ci_workflow_dag, YamlStatic)` walks `CIWorkflowDag` (PR #2736) and emits `Workflow` value byte-equivalent to current `.github/workflows/ci.yml` content; deterministic; regenerated on workflow edit; consumer is GH Actions runner directly.
