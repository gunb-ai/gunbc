...
| T-4.8 | coordination | SCAFFOLD — deps now satisfied after T-4.7; ready for coordination-fill against existing `coordination.dag` interface | T-4, T-4.7 | IMPL | T-4 mgr |
...
- **T-4.7/T-4.8 coordination update:** T-4.7 is satisfied on this tree
  (`src/v4/extdeps/frameworks/react.dag` plus its v3 smoke ratchet). T-4.8
  should now fill `src/v4/extdeps/coordination.dag` under the existing
  Bind-composition / effect-typing discipline. T-4.6 does not gate starting
  T-4.8, but T-4.8 should coordinate field names and contract shape with the
  in-flight T-4.6 OpenAPI/SQL fill because T-16 consumes both
  `WireContract`/`DeploymentUnit` and format-backed artifacts.
