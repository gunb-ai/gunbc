...
**Runtime authority is structural (PR #637).** Pipeline stage order is read from the declaration order of `PipelineStageBinding` records in the Dag. `ordered_pipeline_stages` walks `dag.declarations()`, filters by `meta_tag == PipelineStageBinding`, and returns them in declaration order. No text slicing on the runtime path.

**`compile` body role: second authored surface; cross-check suspended (PR #1171, 2026-04-29).** The `fn compile { parse \n lower \n ... }` body remains the **human-readable pipeline contract**, but the lowered Dag still carries only `ArrowBody::Unparsed` for that arrow — there is no structural ordered stage list to compare against bindings without a source-text side channel (`include_str!` or `read_to_string` + slice). R3 `bridge_include_str_side_channels_retired` rejects those channels for this lane, so **`ordered_pipeline_stages` does not fail-closed on compile-body drift** until derivation provides a single carrier or a lowered structural witness. Bindings remain the sole **runtime** ordering authority read from the substrate; keeping `compile` text aligned with them is review/regen discipline until then.

**Receipt — bridge shape (scheduled, not terminal):** 2c is **not** an interim bridge to `ExternalRealization` (nothing to "realize" for `compile` itself). PR #637 narrowed the prior "body-span is the ordering authority" framing to "bindings are the runtime authority." The two authored carriers (`compile` body and `PipelineStageBinding` declaration order) still exist; the former reconcile (`reconcile_with_compile_body`) is **removed** pending substrate, not replaced with another string channel. **Dissolution trigger:** derivation collapses the two carriers to a single authored source — e.g., a regen/emit step writes the `compile` body from binding declaration order at regen time (or an inverse substrate makes `compile` the sole authored carrier with bindings derived from it).
...
>
> **Case 2c — `compile` orchestrator.** `fn compile(...) { ... }` in `pipeline.dag`: **`Unparsed` persists**. Runtime ordering authority is the declaration order of `PipelineStageBinding` records (`ordered_pipeline_stages`); compile-body vs binding drift check is **suspended** until a structural compile-body witness exists (PR #1171 disposition — no `include_str!` / no runtime source read for that reconcile). Not dissolved by `PipelineStageBinding`: 2c is **bridge shape** (two authored carriers), scheduled until derivation collapses them to one authored source.
>
...
The proper disambiguator is **downstream bootstrap / authority role**, not a single boolean: `PipelineStageBinding` for per-stage pipeline fns; persisted `Unparsed` for `compile` with binding-only runtime order until derivation (compile-body reconcile suspended per PR #1171); absence of those *and* no special pipeline role implies parse lag (case 1).
