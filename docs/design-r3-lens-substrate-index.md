...
|---|---|---|
| `SectionRef` / `SectionedLensApplication` (sum) / `EnforcedApplication<Output, Budget>` / `IntrospectApplication<Output>` / `LensEnforcement<Output, Budget>` | lens-application-surface §2 | (used by future user-authored lens applications; each variant carries exactly its required parameters — Introspect has no Budget axis; per-lens `LensEnforcement` projections co-located with each lens — complexity → AsymptoticClass projection, cost / parallelism → identity) |
