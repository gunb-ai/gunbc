...
2. **Plus four substrate operations** that aren't `fold_node` but are needed:
   ...
   - **`find_witness`** — the unified **candidate-enumeration-and-check** primitive (rescoped 2026-05-20 — Pass B unification). Given `(source_facts, closed_candidate_set, preservation_predicate)`, mechanically enumerates the candidates and checks the predicate against each; first match wins (or fail-closed when zero, ambiguity diagnostic when multiple). Decidable by construction over the closed candidate set — **NOT a search**, per `src/v4/TASKS.md` T-9 ratification ("not a search, not research"). The word "find" denotes the operation's *output* (the canonical candidate or a diagnostic), not a search algorithm. **Unifies what were two distinct primitives** (`solve_constraints` and the coercion fold): both are enumeration-and-check over closed candidate sets with different predicates. `solve_constraints` enumerates canonical-form candidates with a constraint-satisfaction predicate; the coercion fold enumerates target-language declared inhabitants with an exact-structural-equality-zip-fold predicate. The mechanic is the same — closed candidate enumeration + mechanical predicate check — only the predicate (and the candidate-source) differ. See "Primitive unification" below.
...
- **`traverse_node` / `sequence_node` / `bind_outcome`** are `fold_node` invocations with an Outcome-threading algebra; the combinator (not the algebra) interprets `StageDiagnosticPolicy` to thread failures. The algebra is pure-success-accumulator: `fn(R, Edge, Node) -> Outcome<R>`. Single authority for failure handling = combinator. See "Derived combinators" below.
- **`apply_diff`** is `fold_node(root, substitute_at_paths(diff))` — a specific algebra parameterizing the catamorphism with the substitution operation. The agent-side mutation surface (read/edit pipeline doc); composes with `fold_node`, not parallel to it.
...
| **`find_witness`** *(unified primitive — Pass B unification 2026-05-20)* | THE unified **candidate-enumeration-and-check** primitive. Given `(source_facts, closed_candidate_set, preservation_predicate)`, mechanically enumerates the candidate set and checks the preservation predicate against each candidate; first match returns `Outcome::Accepted` with witness; zero matches return `Outcome::Rejected` with a `NoTargetCandidate` / similar diagnostic; multiple matches return `Outcome::Rejected` with `AmbiguousTargetCandidate` diagnostic (unless a deterministic `TargetSelectionPolicy` resolves per Ratified Q14). **NOT a search — per T-9 ratification, the candidate set is closed and the predicate check is mechanical/decidable.** The word "find" denotes the operation's output, not a search algorithm. **Unifies what were previously two distinct primitives:** `solve_constraints` (enumerates canonical-form candidates with constraint-satisfaction predicate; per T-9, decidable-by-construction over the closed Find set) and the coercion fold (enumerates target-language declared inhabitants with exact-structural-equality-zip-fold predicate; per T-9, "mechanical zip-fold, decidable by construction, never a search"). Both are mechanical enumeration over closed sets — the predicate (and candidate source) differs. `LawfulRewriteWitness` (P7, when implemented) is a third instance: same primitive with a lawful-rewrite-precondition predicate over rewrites declared by the target/runtime model. | **Not yet declared.** Substrate likely lands in `std/find_witness.dag` with the predicate algebras living in their consuming files (`std/constraints.dag` for the constraint-satisfaction predicate; `std/coercion.dag` for the exact-zip-fold predicate). |
...
- **`apply_diff`** = `fold_node(root, substitute_at_paths(diff))` — a specific algebra parameterizing the catamorphism. The agent-side mutation surface (read/edit pipeline doc); composes with `fold_node`, doesn't extend the primitive set.
- **The coercion fold** = `find_witness(canonical_source_grounding, target_lang.declared_inhabitants, exact_structural_equality_zip_fold)`. The "fold" name is preserved in TASKS.md T-9 ratification; conceptually it's an instance of `find_witness`.
- **`solve_constraints`** = `find_witness(constraint_graph, canonical-form-candidates, constraint_satisfaction)`. Same primitive, different algebra.
- **`LawfulRewriteWitness`** (P7) = `find_witness(source_grounding, candidate_rewrites_declared_by_target, lawful_rewrite_precondition_predicate)`. Same primitive again.
...
**Resolved Q4a, refined under Pass B unification.** `traverse_node` is **derived** (not a separate primitive) — it is `fold_node` invoked with an Outcome-threading algebra, where the *combinator* (not the algebra) interprets `StageDiagnosticPolicy` to thread failures. Per Worker C's escalated `NodeOutcomeFold.step` signature collapse: the algebra is pure-success-accumulator `fn(R, Edge, Node) -> Outcome<R>`; failure policy lives at the combinator, not the algebra. Single authority for failure handling.
...
| **`find_witness`** (unified primitive, Pass B 2026-05-20) | `find_witness(source_facts, closed_candidate_set, preservation_predicate) -> Outcome<(Candidate, Witness)>`. THE unified **candidate-enumeration-and-check** primitive (NOT a search per T-9 ratification — closed candidate set + mechanical predicate check). Unifies what were previously `solve_constraints` + coercion fold + `LawfulRewriteWitness` — all three are `find_witness` invocations with different preservation predicates. |
| **`solve_constraints`** (DERIVED, not primitive) | `find_witness(constraint_graph, canonical-form-candidates, constraint_satisfaction_predicate)`. Produces canonical source grounding via the unified primitive. |
