...
Extending the audit to the other nine lenses under `src/v3/lenses/` surfaced the same pattern in `cost.dag` and (historically) the Lane-2 workflow lenses. The common root cause is substrate gaps — the v3 substrate does not yet carry `DescentEvidence`, `CallPattern`, `SubValueRelation`, or `MethodSemantics` — plus (for some lanes) emit or analysis-porting gaps. Lenses that depend on those facts either compute a structural proxy or forward to a Rust oracle.
...
| `parallelism.dag` | PARTIAL | **STUB** | None (v3-native) | Unconditional `report_parallelism_unsupported(LensSurfacePending, …)` | — (real analysis in `src/v3/compiler/src/workflow_parallelism.rs`; the `.dag` is still a fail-closed placeholder — **emit can now lower `match` on imported user sums like `WorkflowEffect` in lens modules**, but this lens has not yet been rewired to `lane2_workflow_at` + a `.dag`-authored Stage 2e walk) |
...
The remaining “not done” rows (**complexity**, **cost**, **parallelism**) still share substrate / analysis-porting gaps; **idempotency** is now **COMPLETE** (see table). Fixing the shared blockers unblocks multiple lenses at once; the register exists in part so that is visible.
...
The v3 `emit_rust_module` path historically failed `match` on some user-defined sums when lowering introduced **anonymous** specialized `Disj` copies (`name: None` so `Dag::declaration_by_name` stays single-authority), so Rust emission could not recover the template enum label. **Receipt:** `specialize_decl_for_lowering` threads **`meta_tag: Some(template_disj_id)`** on those anonymous sums (`src/v3/compiler/src/lower.rs`); Rust emit walks that lowering-only edge via **`named_disj_root_for_rust_match_emit`** (`src/v3/compiler/src/emit/rust_target.rs`) — not a second `Declaration::name`, and not emit-time label guessing against unrelated declarations. **Proof:** `m1_3_emit_rust_test::emit_rust_module_match_on_imported_workflow_effect_sum` (end-to-end `match` on an imported user sum) plus the unit test `named_disj_root_for_rust_match_emit_follows_meta_tag_to_named_template` in `src/v3/compiler/src/emit/rust_target.rs` (synthetic anonymous specialized `Disj` → `meta_tag` → named bootstrap template).
