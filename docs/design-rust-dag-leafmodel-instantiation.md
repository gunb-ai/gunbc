...
For M = rust.dag, the fact_id space at main HEAD comprises **all** facts the model declares, whether as top-level `data Symbol` lines OR as derived facts computed by model-owned functions (e.g., `rust_integer_algebra_inhabitance(facts)` at rust.dag:1788 derives an `AlgebraInhabitanceDecl` from a `RustIntegerPrimitiveFacts` record). The fact_id discipline accepts both shapes — but per the Node-typed `fact_id` authority ratified by Modeling DFS msg_d5181972 and the landed projection fns at `rust.dag:721-731` (see §3.1), the canonical form the runner consumes is ALWAYS a `Node` identity produced by a `rust_leaf_model_fact_id_node_*` projection function. The Symbol or derived-value key is the model-side declaration the projection-fn wraps; the runner-side fact_id is uniformly Node. No parallel Symbol-keyed or derived-value-keyed fact_id surface exists.
