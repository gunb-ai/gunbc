...
**Historical manager brief:** [`r2-grounding-manager.md`](briefs/r2-grounding-manager.md) (R2 program + 11-lane decomposition).

**R3 Grounding Manager brief:** [`r3-grounding-manager.md`](briefs/r3-grounding-manager.md) — Gap 13 / T-Ground 11-sub-lane **IN-R3 continuation** per operator §4 sub-item 6 (2026-05-13) + scope canvas [`r3-grounding-mgr-scope-discrimination-canvas-2026-05-14.md`](audit/r3-grounding-mgr-scope-discrimination-canvas-2026-05-14.md).

**Ledger protocol:** [`r2-release-manager.md`](briefs/r2-release-manager.md) + this doc’s header — **R2 Release Manager** remains the single **row owner**. R3 Grounding Mgr proposes `Last signal` / `Notes` / (rare) `Status` updates via lane-close **signals**, cross-manager **queue + ack**, or **Director-ratified** batch refresh (T-Ground table touched 2026-05-14 hold-pattern PR).
...
| T-Ground-Rust | lane | Rust target primitives | `rust_target_primitives_structural` | in-flight | #1005, #2272 | XL; **#2272** — `u128` row / target-primitive substrate slice at HEAD (R3-tier; Substrate vs Grounding handshake per scope canvas). |
...
| T-Ground-LanguageSpec | lane | LanguageSpec language-agnostic | `language_spec_language_agnostic_structural` | in-flight | #1168–#1174, #1195–#1196, #1210–#1213, **#2229** | Brief landed (#1168 + nits #1172/#1174); **Phase 1** registry rows (#1195 + hot-fix #1196); **Phase 2 partial** dead `MethodTranslation` retirement (#1210 + ROADMAP #1213). **#2229** — `cost_target_realization` registry-class slice at HEAD (coordinate §P1 ownership vs population per scope canvas). Structural gate **open** until `language_spec_language_agnostic_structural` fires — per [`r2-grounding-manager.md`](briefs/r2-grounding-manager.md) owned table. |
| T-Ground-Coercion-Fold | lane | coercion fold | `coercion_fold_structural` | in-flight | #989, `c0cc8b260`, #1241, **#1980**, **#2279** | Prior pilot-list / mirror-consistency footprint (#989 + `c0cc8b260` per manager brief). **#1241** — `grounding_coercion_fold` crate scaffold + algorithm-shape stub. **#1980** — Coercion-Fold ScratchIntExamples retirement (R3-tier). **#2279** — SelectedTargetInhabitance-class work (R3-tier). Row stays **`in-flight`** until `coercion_fold_structural` fires — likely closer to GREEN at HEAD than pre-refresh ledger era per [`r3-actual-close-plan.md`](r3-actual-close-plan.md) §Gap 13. |
...
| T-Ground-CrossTarget-Meta | lane | cross-target meta / L6 fold | `cross_target_meta_structural` | in-flight | #1224 brief + #1414 L6 coverage fold + **#2103** + closure-ledger gap ratchet | **#2103** — L6 `EmissionPathProjection` slice **CLOSED** at HEAD per Gap 13 audit; row stays **`in-flight`** until `cross_target_meta_structural` fires. Walker + [`r2-closure-ledger.md`](r2-closure-ledger.md) L6 key block below must remain consistent (no ledger-only key drops). |
...
| Identifier | Granularity | Scope | Gate (demo = gate) | Status | Last signal | Notes |
|---|---|---|---|---|---|---|
| Runtime value model | sub-lane | typed runtime values for 6 connectives + 5 L1 behaviors | `runtime_value_model_structural` | green | #1197, #1228, #1231, #1387, #1467, #1476, #1496, #1715, #1813, #2190 | **HEAD refresh 2026-05-14 UTC:** `src/v3/compiler/src/lib.rs::evaluator::Value` is the terminal Rust evaluator consumer mirror of `src/v3/std/runtime.dag` with literal, record, variant, node-ref, and cardinality inhabitants; Bool variant reification, cardinality/descent loop values, Bind callable entry, FieldProject/Callable execution, and constructor callable execution have landed. This closes the old lazy/eager Open call 3 for the runtime carrier surface; future expansion must add substrate authority, not a parallel mirror. |
| Body evaluator | sub-lane | execute `.dag` function bodies structurally | `body_evaluator_structural` | green | #1387, #1407, #1426, #1476, #1496, #1715, #1813, #2190; `src/v3/compiler/src/lib.rs`; `src/v3/compiler/tests/integration/t_ci_workflow_as_data_demo_test.rs`; `src/v3/compiler/tests/integration/r3_verification_l4_l7_l5_skeleton_test.rs` | **HEAD refresh 2026-05-14 UTC:** `evaluate_body` dispatches through `eval_node` for all five `Behavior` variants (`Value`, `Transform`, `Branch`, `Loop`, `Bind`). Tests cover literal/transform/branch/cardinality-loop/bind execution, injected `LoopBound::Descent` proof consumption and residual fail-closed paths, timing `DimensionReport` evaluation, and W1 `DifferentialEquals(rust_emit_output, dag_eval_output)` certification seeds. |
| Lens application | sub-lane | extend `reflect_program_dag_nodes_in_file` to complete reflection | `lens_application_complete_reflection` | in-flight | #1191, #1857, #2079, #2658, #2812; `docs/audit/r3-evaluator-phase5-post-e3-closure-handoff.md` | **HEAD refresh 2026-05-14 UTC:** static E6-G1.a `Lens<Int>` report production is executable through declared constructors and evaluator values, and gate #5 lens-apply retirement slices have landed. This remains **in-flight**, not green: Q-Reification Option A locks `Dag` as the reflected-program carrier; remaining work is real lens-over-`Dag` body authority plus generic `fold_lens<C>` runtime-callee execution, still held on X1.b S1/S3 per the Phase 5 handoff. |
| Witness construction | sub-lane | runtime materialization of `Witness::Inhabits` / `Witness::Violates` + algebraic-law witnesses | `witness_construction_structural` | in-flight | #1857, #2190, #2825, #2826, #2827, #2941; `src/v3/compiler/tests/integration/e6_g1a_option3_static_lens_test.rs`; `src/v3/compiler/tests/integration/r3_verification_l4_l7_l5_skeleton_test.rs` | **HEAD refresh 2026-05-14 UTC:** `Witness::Inhabits` materializes through the static lens report path, descent proof witnesses are consumable by the evaluator, and L7 algebraic-law witness receipts execute for bounded Int laws. This remains **in-flight**, not green: the E6-G1.a read-channel `Violates` path is still fail-closed with empty report lists, and global witness construction over the locked `Dag` reflected-program carrier depends on the lens-application closure above. |
| Cross-target equivalence harness primitives | sub-lane | for L5 verification in R3 | `cross_target_equivalence_harness_structural` | green | #1917, `src/v3/compiler/tests/boundary/l5_cross_target_consistency.rs`, `src/v3/compiler/tests/fixtures/r3_verification_l5_corpus.dag` | **HEAD refresh 2026-05-14 UTC:** L5 now has a runner-visible `ForAllTargets` harness over a non-empty curated corpus with Rust / Python / Go toolchain requirements, source-authority equality checks, and suite-level pass assertions. This closes the primitive harness row; broader L5 corpus completeness remains governed by the R3 §1.8 gate #15 close sweep, not this R2-Evaluator sub-lane. |
