...
| T-Ground-Diagnostic | lane | diagnostic surface | `target_primitives_diagnostic_structural` | in-flight | #1216 | Lane brief landed (#1216); Q6.5 Layer-1 `CompilerDiagnosticKind` authority consumed per brief — **implementation pending**; gate open. |
| T-Ground-CrossTarget-Meta | lane | cross-target meta | `cross_target_meta_structural` | in-flight | #1224, #1229 | Lane brief #1224 + L6 form-axis anchor to substrate `TypeConnective` (#1229); implementation **gated PR-J** (likely no-op) — gate open. |
| T-Ground-Tests | lane | grounding tests | `grounding_tests_structural` | in-flight | #1223 | Lane brief landed (#1223); L4 implementation **gated** Q4 (PR-I) + Coercion-Fold body — gate open. |
| T-Ground-Dissolve | lane | dissolution sweep | `grounding_dissolve_structural` | in-flight | #1234, #1240 | Lane brief #1234; **#1240** — R2 Grounding **brief-authoring program closure** receipt in manager brief (docs-only; **not** structural lane `green` — implementation still after Coercion-Fold carries load). |
...
| **R3-continuation: 3 distributed bridge retirements** | sub-gate progress | 3 PB-side bridges from T-Bridge-Retirement distribution map | tracked in R3 ledger at standup | in-flight | #1171, #1183, #1192 | **PB-side slices landed:** pipeline `include_str` side channel (#1171); canonical lens-name dispatch (#1183); lower-helper `patch_lower_helpers_*` residual ratchet (#1192). Unified Verification gate **`bridge_retirement_ledger_zero`** remains **R3 / Verification Manager** — row stays **in-flight** until that ledger fires, not **green** here. |
