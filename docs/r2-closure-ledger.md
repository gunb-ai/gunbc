...
**Manager brief (R2-era authority):** [`r2-evaluator-manager.md`](briefs/r2-evaluator-manager.md). **R3 close / Gap 3 framing + dispatch:** [`r3-evaluator-manager.md`](briefs/r3-evaluator-manager.md). **R3 disposition:** R2 closed with Evaluator **r3-continuation** (ROADMAP.md:512); five Gap-3 sub-lanes remain tracked here until each ledger gate is `green` at HEAD.

**Ledger row refresh (2026-05-13):** factual HEAD read against `main` by **R3 Evaluator Mgr** (neat-heron-793) per Director orientation (PR #3013 §4 option (a) re-spawn). Does **not** substitute the **Signal-receiver protocol** below for formal Release Manager queue acks; surfaces planning truth for Gap 3.

5 sub-lanes + PR-A through PR-E design-lock cadence.

| Identifier | Granularity | Scope | Gate (demo = gate) | Status | Last signal | Notes |
|---|---|---|---|---|---|---|
| Runtime value model | sub-lane | typed runtime values for 6 connectives + 5 L1 behaviors | `runtime_value_model_structural` | green | #1197–#1286 + `m1_5_verification_test.rs` | **Structural receipt at HEAD:** named claim `evaluator_runtime_value_model_landed` in `r2_evaluator_runtime_value_model.dag` **Pass**es under `TestRunner` (`r2_pr_a_runtime_value_model_suite_passes`). Substrate: `src/v3/std/runtime.dag` — `Value` / `NamedField`, `EvalFrame` / `EvalStateStack` (#1255), PR-A.3 strategy + memo-key carriers (#1286 + follow-ons). Residual **lazy / thunk** expansion remains a **cadence-matrix** widening surface ([`r2-evaluator-cadence-verification-matrix.md`](briefs/r2-evaluator-cadence-verification-matrix.md) PR-A.3 row), not a reopening of the landed runtime-value **structural** hook. |
| Body evaluator | sub-lane | execute `.dag` function bodies structurally | `body_evaluator_structural` | in-flight | #1813, #2190, #2152 + `evaluate_body` corpus | **Implementation substantially landed** post R2-close: eager body evaluator (`evaluate_body` + `Behavior` dispatch), cardinality loops, **#1813** E6-G0d constructor runtime, **#2190** `LoopBound::Descent` consumer post-`descent_execution_proof`, integration receipts (`e6_g1a_option3_static_lens_test`, `t_ci_workflow_as_data_demo_test`, `r3_class_2_function_valued_data_test`, …). **Still open for ledger `green`:** manager-brief structural claim `evaluator_body_evaluator_correctly_executes_std_termination` (representative `dsl/std/termination.dag` body) is **not** wired as a named `.dag` `TestClaim` + suite Pass row at HEAD — dispatch worker to land that hook (or Director-amended equivalent) per [`r3-evaluator-dispatch.md`](briefs/r3-evaluator-dispatch.md) + [`r2-evaluator-manager.md`](briefs/r2-evaluator-manager.md) Acceptance. |
| Lens application | sub-lane | extend `reflect_program_dag_nodes_in_file` to complete reflection | `lens_application_complete_reflection` | in-flight | #1191, #1857, #2658/#2812, #2825–#2941 | **Landings:** PR-E reflect→apply (#1191); **#1857** E6-G1.a Option 3 static `Lens<Int>` mechanism + `Witness` / `DimensionReport` construction path; **#2658/#2812** retire legacy `lens_apply.rs` lens fold → `lens_declaration_apply.rs`; self-application gates **#57–#59** chain (#2825/#2827/#2826/#2941). **Still open:** structural claim name `evaluator_lens_application_complete_reflection` **absent** from compiler fixtures at HEAD (grep); full lens-over-`Dag` fold + PR-C dissolution consumption per [`r2-pr-c-reflection-dissolution-gates.md`](briefs/r2-pr-c-reflection-dissolution-gates.md) + [`r3-evaluator-phase5-post-e3-closure-handoff.md`](audit/r3-evaluator-phase5-post-e3-closure-handoff.md) residuals. |
| Witness construction | sub-lane | runtime materialization of `Witness::Inhabits` / `Witness::Violates` + algebraic-law witnesses | `witness_construction_structural` | in-flight | #1857, #2257 | **Partial:** #1857 demonstrates declared-constructor `Witness` / `DimensionReport` materialization on static representative path; #2257 Tier-3 perf predicate consumer. **Still open:** `evaluator_witness_construction_per_lens_correct` (≥3 lens instances per `r2-evaluator-manager.md` Acceptance) **not** structurally closed. |
| Cross-target equivalence harness primitives | sub-lane | for L5 verification in R3 | `cross_target_equivalence_harness_structural` | in-flight | PR-D slices 0–1 + matrix | **Slice 0 + slice 1 closed at HEAD:** `evaluator_cross_target_equivalence_harness_primitives_landed` + `…_differential_scaffold` both **Pass** (`r2_pr_d_cross_target_equivalence_harness_primitives_suite_passes`). **Slice 2+ / strict `ForAllTargets` L5 receipts** remain explicitly deferred per [`r2-pr-d-cross-target-equivalence-harness-primitives.md`](briefs/r2-pr-d-cross-target-equivalence-harness-primitives.md) Dependencies + [`r2-evaluator-closure-residuals.md`](briefs/r2-evaluator-closure-residuals.md). |
| **PR-A through PR-E design-lock cadence** | progress track | pre-dispatch design lock cadence | per-PR design-lock landing | in-flight | #1129–#1286 + [`r3-evaluator-dispatch.md`](briefs/r3-evaluator-dispatch.md) | **Design locks landed** PR-A through PR-D + **PR-E dispatch** brief. **Implementation cadence** post R2-close: E1–E6-class slices merged (#1813, #1857, #2152, #2190, #2257, #2658, #2681, #2812, #2825–#2941, …) — track slice completion vs remaining STOP surfaces in phase-4/5 audit handoffs, not as a single historical PR-A..E PR cluster. |
