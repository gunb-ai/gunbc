...
- **Owner**: warm-wolf-698 (R3 Substrate Mgr, expanded scope absorbs all 11 PB-X lanes as sub-programs per operator §4 Item 5 α-ratification 2026-05-14) for substrate + per-stage L2.5 model + 4-step migration work; zesty-boar-261 (R3 Debt-Paydown Mgr) for non-pipeline-stage census-line discipline + ratchet-direction enforcement
- **Routing authority** — primary: `docs/design-pure-bootstrap-zero.md` (LIVE since 2026-04-25 cascade PR #782; supersedes predecessor `docs/design-pure-bootstrap.md` which retains numbered PB-X lane history) names the 10 PB-X lanes load-bearing under the 0-floor target (`PB-Substrate` / `PB-1` / `PB-4` / `PB-5` / `PB-6` / `PB-Bootstrap-Process` / `PB-Runtime` / `PB-Lib+PB-Build` / `PB-Workflow` / `PB-Tier1-Sweep`). The 11th routing lane — `PB-3` (parse retire) — is attributed to predecessor `docs/design-pure-bootstrap.md:145` (`### PB-3 — parse retire`); design-pure-bootstrap-zero.md discusses parse work under PB-1's inverted non-goals but does NOT introduce a PB-3 label directly. Listed here for completeness because the migration order (SELF_HOSTING.md §2 emit→lower→infer→parse) requires PB-3 routing semantically equivalent to PB-1/4/5/6's pipeline-stage retirement shape:
