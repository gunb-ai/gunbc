...
- **Substrate-gap-class closure** (NEW 2026-05-06; per Brian directive at [gunbc#846](https://github.com/gunb-ai/gunbc/issues/846); closure-criteria over existing lane work, NOT new separate lanes — added without expanding R3 lane count). Five class-level closure gates ratifying that R3 close requires a representative gap-test executing through v3 cleanly per substrate-gap class. Per [`docs/r3-program-plan.md`](r3-program-plan.md) §1.4 + §4.
  - `substrate_gap_parser_grammar_closed` — v3 parser handles **generic refinement syntax `Type<N>` over any algebraic type T** (per Brian directive 2026-05-06 at gunbc#846 — refinements are projections of abstract algebraic concepts onto concrete register/memory constraints; NOT Int-specific). Pass condition: programs using `Int<N>` (north-star integer projection) + `Real<N>` (real-number / `ApproximateField<F>`) + `Nat<N>` (natural-number) refinement shapes lower through stage0 emission without v2-fallback; gate Pass = all 3 refinement shapes produce 0 diagnostics + emitted artifact compiles. Generic-shape requirement prevents Int-specific hard-coding. Closes via T-V2-Retirement parser path + T-Numeric-Construction (Int/Real/Nat refinement chain) + Substrate continuation
