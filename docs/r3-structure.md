...
- **T-Verification-L4-L7-Direct** (Evaluator-direct).
  ...
  - `tc1_eta_equivalence_executable` (NEW 2026-05-06; per Brian closure-criteria framing — Pattern A NYI predicate executable; `BinaryDimensionReportEquals` shape) — TC1 η-equivalence predicate executes via Evaluator-constructed `DimensionReport<C>` comparison; ratchets from `NotYetImplemented` shell to passing TestClaim against representative input
  - `tc2_church_rosser_executable` (NEW 2026-05-06; same family as tc1) — TC2 Church-Rosser predicate executes via Evaluator-constructed `DimensionReport<C>` comparison
  - `tc3_pattern_a_second_mover_executable` (NEW 2026-05-06; same family as tc1) — TC3 Pattern-A second-mover predicate executes via Evaluator-constructed `DimensionReport<C>` comparison
  - `rust_dag_isomorphism_executable` (NEW 2026-05-06; structural-Dag-equivalence shape, NOT DimensionReport) — `RustDagIsomorphism` predicate executes via Dag-isomorphism check between Rust-emitted Dag and `.dag` source Dag; ratchets from `NotYetImplemented` shell to passing TestClaim
...
- **T-Numeric-Construction (REFRAMED 2026-05-01 from T-Int128).** Acceptance gates per [`docs/design-numeric-construction.md`](design-numeric-construction.md):
  - `numeric_abstract_carriers_landed` — Magnitude (terminal), Nat (Semiring<Magnitude>), Int (canonical AbelianGroup primitive **OR** `GroupCompletion<CommutativeMonoid<Nat>>` — **design selection pending** per P1 axiom-violation paydown 2026-05-06; prior `Int = AbelianGroup<Nat>` shorthand is structurally false — Nat lacks additive inverses; see [gunbc#1739 issuecomment-4385432940](https://github.com/gunb-ai/gunbc/issues/1739#issuecomment-4385432940) + [`docs/design-numeric-construction.md`](design-numeric-construction.md) §3), Rational (Field<Int>), Real (ApproximateField<Rational>) declared in `dsl/std/integer.dag` + `dsl/std/float.dag`
...
- **T-CostLens-Composition.**
  ...
  - `symbolic_cost_expr_equals_executable` (NEW 2026-05-06; per Brian closure-criteria framing — Pattern A NYI predicate executable; `SymbolicCost`-typed family) — `SymbolicCostExprEquals` predicate executes against representative input and produces `SymbolicCost` comparison result; ratchets from `NotYetImplemented` shell (per `src/v3/compiler/src/test_runner.rs` `SymbolicCostExprEquals: structural shape is valid` receipt) to passing TestClaim
- **T-Bridge-Retirement.**
...
- **Substrate-gap-class closure** (NEW 2026-05-06; per Brian directive at [gunbc#846](https://github.com/gunb-ai/gunbc/issues/846); closure-criteria over existing lane work, NOT new separate lanes — added without expanding R3 lane count). Five class-level closure gates each closing when **both** (per [`docs/r3-program-plan.md`](r3-program-plan.md) §1.4 conjunctive predicate; Director poke-hole 2026-05-06 finding 1.3 + openai-pro 2026-05-06 finding 2.2): **(a)** representative gap-test executes through v3 cleanly per Pass condition AND **(b)** systematic enumeration of class-bridges shows count = 0 (or any survivors carry explicit Director allocation per `docs/audit/r3-debt-sweep-2026-05-06.md` §3.A STRUCTURAL exception). Single-test-pass alone is sample-of-class, NOT closure-of-class; conjunctive predicate prevents class reading "GREEN" while real bridges in that class persist unexercised by the chosen representative. Per [`docs/r3-program-plan.md`](r3-program-plan.md) §1.4 + §4.
  - `substrate_gap_parser_grammar_closed` — v3 substrate models concrete types as the **interaction of independent constraint models** — abstract algebra × machine constraints — with concrete types emerging as consequences (per Brian directive 2026-05-06 at [gunbc#846 inline](https://github.com/gunb-ai/gunbc/pull/1808): *"these concepts should be the interaction of several constraint models i.e. abstract algebra × machine constraints — i64 would naturally emerge as a consequence"*). Refinement syntax `Type<N>` expresses the interaction, NOT a primary entity. Substrate declares abstract algebra (e.g., `Int` as canonical AbelianGroup carrier — design selection pending Option A canonical-primitive vs Option B `GroupCompletion<CommutativeMonoid<Nat>>`; prior `Int = AbelianGroup<Nat>` shorthand was structurally false per P1 axiom paydown 2026-05-06, see [gunbc#1739 issuecomment-4385432940](https://github.com/gunb-ai/gunbc/issues/1739#issuecomment-4385432940)) and machine constraints (e.g., `MachineWidth<bits>`) as **independent** models; concrete types emerge via interaction (e.g., `Int<64>` = `Int × MachineWidth<64>`; target-language `i64` is derived artifact, not primary substrate entity). Pass condition: ≥3 algebra×constraint pairs (`Int<64>` / `Real<64>` / `Nat<8>`) lower through stage0 emission without v2-fallback; emitted artifact compiles to expected target primitive (`i64` / `f64` / `u8`) where target primitive is consequence-of-interaction. Closes via T-V2-Retirement parser path + T-Numeric-Construction (algebra side) + Substrate continuation (machine-constraint side; may require new `MachineConstraint<C>` substrate carrier)
...
- **Demonstration principle** (NEW 2026-05-06; per Brian directive at [gunbc#846](https://github.com/gunb-ai/gunbc/issues/846): *"any feature we make should come with a demonstration"*; generalizes T-Free-Consequences-Demonstration framing to all R3 lanes; per [`docs/r3-program-plan.md`](r3-program-plan.md) §1.6). Each R3 lane MUST include at least one runtime-executable demonstration gate (a claim that exercises the lane's deliverable end-to-end, not just static-fact assertions about substrate shape). Existing demonstration-shaped gates (e.g., `pb_self_compile_fixed_point`, `omni_openapi_backend_emission_demo`, T-Free-Consequences-Demonstration's 10 gates, `lens_self_application_demonstrated`) satisfy this where present. New demonstration gates added per per-lane audit:
  ...
  - `lens_producer_retirement_executable_witness` (per PB Mgr poke-hole 2026-05-06 F3 + openai-pro 2026-05-06 finding 1 — gate name aligned across r3-structure.md + plan §1.6) — `lens_apply` reflection routes via PB-Runtime on representative lens program; **execution-ready witness DEFERRED** to Row-4 equivalence receipt landing per `docs/design-pb-runtime-interpreter.md` §5.1 + convergence matrix; near-term demo = retirement state-check + doc receipts; "demonstration" status re-promotes when Row-4 + Item 4 receipts exist
...
| **T-Bridge-Retirement** | M | **Verification (ledger only); retirement work distributed per bridge map** (Director-locked 2026-04-28: distribute-work-centralize-ledger discipline match — bridges retire in PB/Substrate territory; Verification owns the unified `bridge_retirement_ledger_zero` audit gate) | **Bridge distribution map** (5 named bridges): (1) `SourceSpan.file` participation checks → **Substrate** (typed identity surface); (2) `mark_bootstrap_secret_nominal_opacity()` → **Substrate** (Secret PR A continuation lineage); (3) canonical lens-name dispatch → **PB Manager** (lens-producer-retirement adjacent); (4) `include_str!` side channels (e.g., pipeline_authority.rs) → **PB Manager** (compiler-internal bootstrap); (5) `patch_lower_helpers_*` residual → **PB Manager** (Tier 2 retirement lineage; #1014 was first slice). **Net: 2 Substrate-owned bridges + 3 PB-owned bridges + 1 Verification-owned ledger gate (not a bridge — coordinates the 5 bridges above).** Verification's `bridge_retirement_ledger_zero` audit gate verifies cross-program coordination/reporting cadence; the actual retirement work absorbs into existing Substrate / PB scopes without spawning a parallel manager | R2 substrate carriers (typed identity surfaces); per-bridge gates depend on the natural-owner program's prerequisites |
...
**Hybrid mechanism**:
- **(a) Per-PR discipline rule** — every R3 PR includes debt receipt; vague deferrals rejected. Same shape as the per-PR gate from `INVARIANTS.md#p5-progress-is-dissolution` Dispatch-Discipline Mechanisms (b).
