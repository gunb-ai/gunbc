...
| **T-Lens-Application-Surface** (NEW 2026-05-02; design doc landed 2026-05-02 [`docs/design-lens-application-surface.md`](design-lens-application-surface.md)) | L-XL | **Substrate Manager + Verification Manager (cross-program)** | First-class authoring surface for applying lenses to arbitrary `.dag` sections (function / module / expression / declaration scope). Per user reframe 2026-05-02: lens application is a `.dag` declaration with configurable behavior — `apply_lens(lens, section, config)`. **Subsumes** prior T-Complexity-Contract-Compile-Error + T-User-Authored-Cost-Basis-Discipline as configurations of one mechanism. **Substrate carriers** (per design doc §2): `SectionedLensApplication` + `SectionRef` (DeclarationScope/NodeScope disjoint sum) + `ApplicationConfig` + `ViolationPolicy` (Enforce/Introspect — `CompileError | Warning | Silent` original framing resolved to fail-closed-compatible binary per design doc §3 + INVARIANTS C-8). **Default policy for complexity contract**: opt-out (default check fires; explicit waiver via structural `ComplexityBudgetWaiver` declaration carrier with `justification` field — NOT an annotation, per `feedback_no_annotations` and design doc §3.2). Demonstrations: complexity-contract-compile-error + CRDT cost basis + memory-peak cost basis + opt-in cross-iteration parallelism (4 worked examples; orthogonal axes per Director ratification; design doc §4). **Gates:** `lens_application_carrier_landed`, `section_ref_substrate_landed`, `application_config_violation_policy_routing`, `complexity_violation_compile_error_demonstrated`, `crdt_cost_basis_demonstrated`, `memory_peak_cost_basis_demonstrated`, `opt_in_iteration_parallelism_via_lens_application_demonstrated`. **Design doc §8 open questions** (5 items) require Director ratification before substrate authoring begins. | T-Lens-Behavioral-Parity (lenses must be COMPLETE first); R2-Evaluator |
