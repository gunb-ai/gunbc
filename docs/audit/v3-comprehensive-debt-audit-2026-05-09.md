...
| 6 | `derive_op_effect(method_str, path_str)` string parser at structural boundary | Class C — string-keyed dispatch over typed carriers; receipt landed in tidy-ram-461 by moving `derive_op_effect` to typed `HttpMethod` + `PathTemplate` and keeping parsing at the v2 surface wrapper only |
