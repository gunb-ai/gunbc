...
| 2 | `ShapeATarget = Rust \| Python \| Go` closed enum vs `LanguageSpec` data extensibility | Class F — ontology duplication — resolved by issue #2466 / crisp-raven-202: `ShapeATarget { spec: DeclarationRef }` carrier, fail-closed refined by consumers to `LanguageSpec` |
