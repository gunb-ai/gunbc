...
| 99 | `workflow_runtime_open_enum_landed` | substrate-shape | **PASSING (substrate landed)** | `git grep -nE '^type WorkflowRuntime = YamlStatic \| BinaryShim \| PythonShim' dsl/gunbc/ci_emission.dag` → 1 hit; `cargo test -p v3-compiler gunbc_ci_github_actions_workflow_dag_matches_yaml_generator_output` green. | PR #2774 (squash 2026-05-12T23:39:42Z); dsl/gunbc/ci_emission.dag:27 |
| 100 | `project_github_actions_landed` | substrate-shape | **PASSING (substrate landed)** | `git grep -nE 'fn project_github_actions\(dag: CIWorkflowDag, runtime: WorkflowRuntime\) -> Workflow' dsl/gunbc/ci_emission.dag` → 1 hit; pinned binding `gunbc_ci_yml_workflow` at line 95; drift-guard `gunbc_ci_github_actions_workflow_dag_matches_yaml_generator_output` green. | PR #2774; dsl/gunbc/ci_emission.dag:87,95 |
| 101 | `test_cost_dimension_landed` | substrate-shape | **PASSING (substrate landed)** | `git grep -nE '^type TestNodeCostDimension' src/v3/std/verification.dag dsl/std/verification.dag` → 2 hits; `cargo test -p v3-compiler test_cost_dimension_landed_on_test_node` green. | PR #2761 (squash 2026-05-12T18:07:31Z); src/v3/std/verification.dag:578 |
