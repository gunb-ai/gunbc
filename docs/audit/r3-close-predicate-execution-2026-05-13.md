...
| 101 | `test_cost_dimension_landed` | substrate-shape | **NOT_EXECUTED** | `git grep -nE '^type TestNodeCostDimension' src/v3/std/verification.dag dsl/std/verification.dag` (expect 2 hits); `cargo test -p v3-compiler test_cost_dimension_landed_on_test_node` (expect green). | Ledger status tracked separately in §1.8 row #101; this row pending close-ceremony sweep. |
