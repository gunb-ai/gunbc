...
- **C3 substrate prerequisites** (per §3.1 live-ledger table): (a) per-lens parallelism `LensEnforcement` instance per #90 Pass condition (Slice B specifically; #90 itself CONSUMER_LANDED on main as parametric carrier); (b) fold-pass consumer for #91 violation routing per design `docs/design-lens-application-surface.md` §10 step 2 (substrate routing surface landed Slice A; consumer pending Slice B); (c) parallelism lens BEHAVIORALLY COMPLETE (C1 #81). All tracked in their respective lanes; not blockers for the carve-promotion ratification, just sequencing prerequisites for the gate firing.
