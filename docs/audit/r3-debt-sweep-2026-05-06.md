...
| `ShapeATarget = Rust \| Python \| Go` closed enum vs `LanguageSpec` data extensibility | F | gpt-5-5-pro Finding 2 | `ShapeATarget { spec: LanguageSpec }` typed-ref carrier; cross-target consumers enumerate loaded `LanguageSpec` declarations | GREEN | issue #2466 / crisp-raven-202 | Grounding |
