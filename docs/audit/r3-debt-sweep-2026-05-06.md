...
| `MissingEmissionPath` stringifies typed axes (connective/behavior/target as String) | C | gpt-5-5-pro Finding 1 | typed-carrier diagnostic boundary | RETIRED | PR #2358; issue #2465 receipt: `EmissionDiagnostic::MissingEmissionPath` now carries `FormAxis` / `BehaviorAxis` / `ShapeATarget`, ratcheted by `t_r3_missing_emission_path_typed_axes.dag` | Substrate |
