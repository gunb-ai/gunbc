...
| `MissingEmissionPath` stringifies typed axes (connective/behavior/target as String) | C | gpt-5-5-pro Finding 1 | typed-carrier diagnostic boundary | RETIRED | PR #2358; issue #2465 receipt: `EmissionDiagnostic::MissingEmissionPath` now carries `FormAxis` / `BehaviorAxis` / `ShapeATarget`, ratcheted by `t_r3_missing_emission_path_typed_axes.dag`. P5 hand-Rust receipt: the same PR adds exactly one `src/v3/compiler/tests/integration/t_pb_b_1_dag_runner_test.rs` shim that runs this `.dag` `TestClaim`; bridge class C, dissolution trigger is T-PB-B generated test execution replacing per-fixture Rust runner registration (`ROADMAP.md` §Release R1 Program / `T-PB-B` `pb_test_file_generated_from_dag` and `pb_rust_tests_outside_residual_zero` gates), R3-close eligibility YELLOW. Net SG-0 delta: `+0` files; existing runner file expanded only to execute the structural `.dag` receipt. | Substrate |
...
| `ShapeATarget = Rust \| Python \| Go` closed enum vs `LanguageSpec` data extensibility | F | gpt-5-5-pro Finding 2 | `ShapeATarget` aliases `LanguageSpec`, with named Shape A target data bindings consumed by projection rows; cross-target consumers discover those target bindings, with `EmissionPathProjection` rows as the coverage authority instead of a parallel `MethodTemplateContract` join | GREEN | issue #2466 / crisp-raven-202 | Grounding |
