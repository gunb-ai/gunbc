...
| `ShapeATarget = Rust \| Python \| Go` closed enum vs `LanguageSpec` data extensibility | F | gpt-5-5-pro Finding 2 | `ShapeATarget { spec: DeclarationRef }` carrier, fail-closed refined by consumers to `LanguageSpec`; cross-target consumers enumerate loaded `LanguageSpec` declarations, with `EmissionPathProjection` rows as the coverage authority instead of a parallel `MethodTemplateContract` join | GREEN | issue #2466 / crisp-raven-202 | Grounding |
