...
| `derive_op_effect(method_str, path_str)` string parser at structural boundary | C | gpt-5-5-pro Finding 6 | typed dispatch over Method × Path carriers | GREEN (tidy-ram-461 receipt: typed `derive_op_effect(HttpMethod, PathTemplate)`; parser retained only at v2 surface re-export) | PR #2358 | Substrate |
