...
| 6 | #897 | Annotated-let literal narrowing skips diagnostic-producing range/refinement check — original BLOCKING (sha 69e9a3f1) | `src/v3/compiler/src/infer.rs` annotated-let literal seed path | R2-cleanup | Route annotated-let literal narrowing through the same diagnostic path as call-site narrowing | Test: out-of-range annotated literal raises Diagnostic; not silently accepted | control-table-only; defer to bright-wolf-465 archaeology before opening worker | tracked (bright-wolf-465) |
| 7 | #824 | Latest scheduled review environmental-fail; substantive review (sha 8a7aa473) not re-confirmed against merge SHA | n/a (review-coverage gap) | R1-process | Re-run codex review on merge SHA; if BLOCKING surfaces, promote to a row | Non-environmental codex review on merge SHA | control-table-only; defer to bright-wolf-465 | tracked (bright-wolf-465) |
| 8 | #825 | B4.4 carrier wired into fresh-regen path but parity loop / committed bootstrap snapshot not regenerated — original BLOCKING (sha 59160671); latest review reports "No findings" only because env-failed reviews preceded; needs bootstrap-parity confirmation | `src/v3/compiler/src/bootstrap.rs`; `bootstrap_generated.rs` and `bootstrap_generated_without_parse_surface.rs`; missing parity test on `EXTDEPS_BOOTSTRAP_PATH_KEYS` vs `Dag::extdeps_bootstrap_fixture_virtual_paths()` | R2-cleanup | Regenerate both snapshots from new `.dag`; add parity test that fails closed on key divergence | Snapshots reflect new fixture-set carrier; parity test exists and is green; CX gate stable | control-table-only; defer to bright-wolf-465 archaeology | tracked (bright-wolf-465) |
