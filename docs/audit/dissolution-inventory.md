...
|---|---|---|---|---|
| **P1** ✅ **CLOSED OUT** (substrate landed #3259; burn-down PR closeout) | `std/cardinality.dag` bounded-natural / refinement substrate | std / T-3 Wave-A2 | **~22** → see disposition | DECISIONS.md outcomes — **🟢 RESOLVED in-PR:** `SL-3229-LLVM-WIDTH` (`LlvmType` widths→`NonZeroNat`/`Nat`), `SL-3229-PTX-COST` (→`Nat`), `SL-3229-VERILOG-COST` (→`Nat`). **🟡 PARTIAL + re-gated:** `SL-3229-PTX-DIM3` (positivity dissolved→`NonZeroNat`; residual = type-level per-axis-max bound, substrate not landed). **🟡 re-gated (gate-attribution correction):** `SL-3229-LLVM-OPS` (operand-*relation* refinement, not bounded-natural — needs dependent/relational substrate). **`SL-3229-FLOAT-NOMINAL`** untouched (its trigger is `std/machine.dag` bounded refinement, not cardinality — not a P1 dissolution). In-file: `llvm_ir.dag` `LlvmType`→🟢; the `json.dag` / `yaml.dag` / `toml.dag` numeric cite-sites **re-gated** off the now-resolved `SL-3229-LLVM-WIDTH` to new row **`SL-3229-FORMAT-NUMERIC`** (residual = `std/float.dag` / `std/integer.dag` typed numeric carriers, not landed — numeric *value* never dissolved with cardinality). |
