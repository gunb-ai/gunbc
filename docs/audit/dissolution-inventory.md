...
canonical `Outcome<Node>` / `Outcome<List<Edge>>` carrier (M6), and the
recursive two-arm `Scope` coproduct dissolved to the product
`Scope { module: Namespace, frames: List<Map<Symbol, Symbol>> }` (the
`ScopeFrame` cons-list recursion is `List`; P1/M9). The `ResolvedTree`,
`Namespace`, `Scope`, `EdgeResolveAcc`, and `BindEdgeAcc` declarations
remain.
