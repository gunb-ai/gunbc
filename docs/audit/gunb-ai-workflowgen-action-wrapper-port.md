...
| --- | --- | --- |
| `schema.Workflow` / `schema.Job` / `schema.Step` | `dsl/extdeps/github/actions.dag` (`Workflow`, `Job`, `Step`, `UsesStep`, …) | gunbc already has a **platform-shaped** Actions model. Gap: marketplace `uses` strings today are Go `ActionRef` / free strings — need a **typed catalog** or extdeps sub-module per vendor if we want parity with “pinned action surface”. |
