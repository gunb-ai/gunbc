...
Substrate-owned **snapshot format**, **carrier shape**, or **first-hook** placement for facts that must not fork between Grounding, emit retirement, and PB bootstrap work is **not** something PB documentation may invent here. Route those decisions through **[`INVARIANTS.md`](../../INVARIANTS.md)** using **grep-stable anchors** on the home-of-record file: **## P1: Modeling Faithfulness** (substrate-fact introduction — subsections *Problem shape: Unnamed substrate target* and *Procedure: substrate-fact introduction*; see file ~L23+) and, when single-consumer / no-duplicate-authority is at issue, **## P2: Boundary Discipline** (*Problem shape: Parallel authority*; see file ~L144+). **[`docs/modeling-discipline.md`](../modeling-discipline.md)** maps modeling practices to those **P1**/**P2** headings for checklist-style review. **R2 Substrate Manager** / Grounding briefs are the owning intake; see **[`docs/briefs/r2-substrate-manager.md`](../briefs/r2-substrate-manager.md)**.
