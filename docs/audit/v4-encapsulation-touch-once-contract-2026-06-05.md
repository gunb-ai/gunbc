# v4 Encapsulation Audit — the "touch-once contract"

**Date:** 2026-06-05
**Lane:** v4 encapsulation audit (read-only feed lane)
...
beneath it. This dossier measures how close v4 is to the contract, concept by concept.
...
`v2_rt::substring` (char-offset, O(file_len) on non-ASCII) was reached by ~27 `.dag` callers
...
this audit. The v4 mirror (`Token { start, end: Int }`, `src/v4/std/lexing.dag:62-68`) replicates
the structural cause; the symptom is not yet triggered (v4 has no span→text consumer yet).
...
## 1. HEADLINE — alias-vs-atom is the single best predictor of tier in v4
...
`Hash` is the proof that v4 **can** reach tier 3 today: a consumer cannot forge or destructure
...
## 2. CAPABILITY VERDICT (highest priority) — v4 has THREE opacity mechanisms, not one

The pivotal question: *can v4 enforce correct-by-construction / hide a representation at all, and
...
### 2.1 `where` / `brand` refinement — **BLOCKED for v4's own concepts**

- The **v3 bootstrap compiler that ingests v4 today** *does* statically parse and fail-closed
...
- **But this does not help v4's target concepts**, for three compounding reasons:
  ...
  3. v4's own substrate uses **zero** `where`-refinements (grep: 0 hits in `src/v4/`), and v4's
     self-hosted infer (`src/v4/compiler/04_infer.dag`) evaluates **no** `where`/`brand` predicate
     at all — it does grounding / constraint / algebra-inhabitance facts. So when v4 stops being
     compiled by v3, this capability is **not modeled** in v4 infer.

### 2.2 Representation-LESS atom (`Symbol`, `Hash`) — **a PRIVILEGE of the closed primitive registry**

- `type Symbol` / `type Hash` are bare opaque abstract types (`src/v4/std/node.dag:10-11`): no
...
- Their representation + sole producers live in the **v2 runtime**: `pub type Hash = String` with
  ...
  (`src/v2/stage0/src/v2_compiler_runtime_rust.rs:101`; mirrored `src/v2/runtime_rust.dag:312-315`).
  Binding is via a closed checkpoint registry, `coerce_primitive_type` / `lookup_checkpoint`
  (`src/v2/coercion.dag:105`), exercised for `"Symbol"` / `"Hash"`
  (`src/v2/stage0/src/compiler_tests.rs:670,694`).
...
### 2.3 `nominal_opaque` wrapper — **a `.dag`-EXPRESSIBLE modeling choice, compiler-enforced, UNUSED by v4**

This is the mechanism that changes the recommendation.

- `nominal_opaque` is a **type modifier in the surface language** the same compiler parses for v4:
...
- **v4 uses it zero times** (grep: 0 hits for `nominal_opaque` in `src/v4/`).
...
| Route | Reaches tier 3? | For v4 today |
|---|---|---|
...
**Verdict:** tier-3 **opacity** for source-position / `Span` is achievable in v4 **today** via
...
- **Derived coercion:** ✓✓ (**what v4 gets RIGHT**) — `content_hash` (`node.dag:1267`), `fold_node`
...
- **CAVEAT — opacity ≠ uniqueness:** the symbol/variant **name-resolution** layer is multi-authority /
  ...
  (cross-ref `v4_variant_name_resolution_multi_authority`; ctrl#1449; namespacing PR #4418). So the
...
- **Opacity / closure:** ✓✓ — `type Hash` (`node.dag:11`); only `atom_identity_hash` /
  ...
  `v2_compiler_runtime_rust.rs:101`). A consumer cannot forge or destructure a `Hash` from `.dag`.
...
- **MAP+LOCK:** *tripwire* — static check that no `.dag` destructures `Hash`; *metamorphic* — swap the
  ...
  (existing stability test `src/v2/tests/src/b1_hash_primitive_test.rs:11`).
...
- Types **are** `Node`s: `NodeKind = TypeNode { connective }` (`node.dag:73-74`). There is no separate
  ...
  types and computations uniformly). **What v4 gets right.**
...
**What v4 gets RIGHT (templates to imitate):**
...
**Ratio read.** v4 gets **derived-coercion** and **closed-coproduct** right *almost everywhere*; it
...
1. v4 **can** make units / source-position **impossible-to-misread today** — via `nominal_opaque`
...
2. The single highest-leverage move is **stop aliasing structural defaults**: model `Span` / `Source`,
   ...
   moot at the interface and pre-empts the v2 `substring` 27-caller leak from ever re-forming in v4.
3. **Correct-by-construction** (no raw constructor) remains the genuinely harder axis: `nominal_opaque`
   ...
   boundary (or v4-infer `where`/`brand` enforcement, which today does not exist).
