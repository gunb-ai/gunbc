...
**Summary:** Historical Phase 1 notes still reference erstwhile R‑1/`criterion`/R‑3 ambiguity at older HEADs—the live repo now carries **`criterion`**, ratified **`ubicloud-standard-2`** (see `docs/audit/c1-r3-canonical-bench-host-decision-matrix.md`), and **R‑7** decomposes into **`presence` vs canonical `host_id` coherence** per PR #2702 procedure addendum (`c1-tier3-baseline-capture-procedure.md` §1). Phase 2 dispatch remains blocked additionally on **R‑4 + R‑5 + R‑6 + coherent R‑7** (presence alone does not unblock dissolution-linked perf parity).
