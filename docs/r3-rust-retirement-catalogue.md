...
5. Should the catalogue have a section listing files whose retirement target is "move out-of-tree" (per `docs/design-pure-bootstrap-zero.md` N=0 resolution paths — install script / gunbc-runtime crate / rustc macro) rather than "generated from `.dag`" or "dissolved entirely"? Useful for clarifying which Phase 5 trajectory each file is on, but NOT a permanent in-tree residual classification (per codex + briansrls BLOCKING PR #3139 2026-05-15: planning artifact cannot introduce permanent in-tree carve-out against the 0-floor target).
