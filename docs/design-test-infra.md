...
1. **Exact syntax for `requires: List<ResourceReference>` on `TestClaim`.** Structural: should ResourceReference be a typed declaration reference (`DeclarationRef`) or a typed resource type (`ResourceHandle` in resources.dag terminology)? Probably the former — handles are runtime artifacts, not compile-time declarations. Verify at implementation time.
   ...
   **Answered (R2.1, DB-15 impl PR):** ResourceReference is a typed compile-time declaration sentinel — "the former." Authored in `src/v3/std/verification.dag` as a 🟡 scaffold record `type ResourceReference { identifier: String }` pending the `dsl/std/resources.dag` → v3 port. The `identifier: String` carrier is bootstrap-local; post-port, the shape narrows to a `DeclarationRef` mirror that structurally rejects any non-`resource` declaration. `ResourceHandle` (runtime-minted opaque proof of acquisition) is explicitly NOT the referent type — handles are produced by the compiler's acquire nodes at runtime, which is the wrong layer for a compile-time test declaration. Kept distinct from the generic `DeclarationRef` sentinel so the narrowed accept set dissolves into the type itself once the `resource` keyword lands in v3.
...
2. **Which existing `TestPredicate` variants need the `requires` declaration, and which are self-contained?** `PortStateExpectation` and `CostBounded` are compile-time assertions with no runtime resource needs. `BehavioralObservation` needs a test-runner resource. `MockBackedInvariant` needs both a test-runner AND a mock-transport resource. Open: is `requires` per-claim or per-predicate-variant?
   ...
   **Answered (R2.1, DB-15 impl PR):** `requires` is per-claim. A single subject-under-test exercised by multiple predicate variants (compile check + behavioral observation + cost check) shares one resource declaration set because those needs are properties of the claim's execution context, not the predicate shape. Per-predicate-variant `requires` would duplicate the data across arms and admit an illegal-state combination where variants disagree about the required resources for the same subject. Compile-time predicates (`Compiles`, `FailsWithDiagnostic`, `PortHasState`, `CostBounded`) carry a single-element `requires: [{ identifier: "compile_time" }]`; runtime-backed predicates (`BehavioralObservation`, `MockBackedInvariant`) carry non-empty lists naming the test-runner plus any mock transports. The empty-list case is deferred to the post-resources-port window when v3 user-code empty-list-literal template inference also stabilizes.
...
3. **Tautology-avoidance enforcement.** The rule "predicate cannot rerun the producing lens" is currently prose. Can it be enforced structurally — e.g., the predicate variants are explicitly behavioral/observational by type, and "rerun lens X" is not even expressible? Needs a pass to confirm no variant sneaks in that permits the pattern.
   ...
   **Answered (R2.1, DB-15 impl PR):** Structurally enforced by the `TestPredicate` coproduct. No variant takes the lens's own output as its verification oracle. `BehavioralObservation` takes a `(subject, input_sample, expected_output)` triple where `expected_output` is a separately-declared `DeclarationRef` — the oracle is an independent declared value, not a lens rerun. `MockBackedInvariant` takes a `(subject, mock_transport, invariant)` triple where `invariant` is a separately-declared `DeclarationRef` asserting a property under the mocked observation. A hypothetical `LensSays { lens, subject, expected }` variant is NOT in the coproduct, and — per the `TestPredicate` docblock added in the impl PR — cannot be added without re-opening tautology avoidance in DB-15. `src/v3/compiler/tests/lane2_stage_2c_verification_smoke.rs::test_predicate_coproduct_blocks_lens_rerun_tautology` is the structural regression guard: it enumerates a set of forbidden variant names (`LensSays`, `LensEquals`, `LensOutput`, `RunLens`) and asserts none are in the coproduct. Lens outputs can still supply the *value* that seeds an `expected_output` or a `bound` (pre-computed at declaration time); the test runner does not rerun the lens at claim-check time.
