...
The managed-credential checks `srv3_managed_credential_probe_check` and `srv4_managed_credential_probe_check` both completed live on 2026-07-27. On each host, the factory credential was rejected with HTTP 401, the active gcloud session satisfied the access ensure without human intervention, the Secret Manager `payload.data` value was decoded from standard base64, and the stored credential authenticated to the BMC. Credential probing now sends netrc content on curl's stdin, and password SSH supplies the same `Secret` to `sshpass -d 0`; neither path puts the password in argv, the environment, or remote-command text. No generic interpreter substring-redaction mechanism is part of this work.
...
Durability qualification is exact rather than name-based. The admission binds srv4's host identity, desired config SHA-256, realization content hash, capability-definition hash, perturbation generation, and readback path. The restart evidence carries the matching perturbation receipt and before/after readback digests. Both readbacks must equal the desired digest; equality with each other is insufficient. Zero matching receipts refuses, one may qualify, and two or more matching receipts refuse as ambiguous regardless of list order.
...
- Exact, time-stamped firmware, Redfish, filesystem, convergence, and restart observations: `gunbc.fleet_bmc_observation`
- Board/firmware curve and controller observations: `extdeps.boards.asrock_rack`
- Exact board/firmware projection rule and desired-state qualification: `gunbc.bmc_fan_projection` and `gunbc.fleet_bmc_state`
