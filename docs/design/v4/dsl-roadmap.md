...
- [x] New command path handling is hardened with regression coverage (relative/absolute targets, normalized absolute path spellings, case-variant `.dag` files, and `.dag`-suffixed directory/symlink aliases including absolute alias paths)
