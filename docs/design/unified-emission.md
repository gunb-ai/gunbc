...
### 2. Makegen
**Location**: `gunbc-dag/src/makegen/`
**Pattern**: Renderable trait (header only) + direct string emission

```
ToolRegistry + BuildConfig → render_makefile_content() → String → Renderable::render() adds header
```

- `Renderable` trait (`core/ir/src/render.rs`) — standardizes "Generated by / DO NOT EDIT" headers
- `MakefileRenderer` implements `Renderable` but `render_content()` builds the entire Makefile via string concatenation
- No intermediate representation for targets/rules/dependencies
- `IgnoreCategory` struct provides minimal structure for .gitignore sections

**What's good**: Consistent headers via `Renderable`. Self-documenting
.gitignore with provenance.

**What's wrong**: No IR for Makefile targets. Can't validate target
dependencies, detect duplicates, or render to alternative formats (e.g.,
Justfile, Taskfile). `Renderable` only handles the header — the body is a
black box.
...
### 4. CI Workflow Rendering
**Location**: `core/ir/src/transport/ci/`
**Pattern**: SharedStep IR → provider trait → YAML string
...
**What's good**: Clean two-layer architecture. Adding a provider only requires
implementing `CiRenderer`. SharedStep IR prevents duplication.

**What's wrong**: `CiRenderer` is unrelated to `TestRenderer` or `Renderable`.
SharedStep has no connection to the test IR or any other IR. Comments say
"following the Renderable pattern" but doesn't actually use it.
...
**Location**: `gunbc-dag/src/policy/pragma.rs`
...
**Location**: `gunbc-dag/src/ci/ops.rs`
...
### 13. WorkflowConfig YAML (legacy)
**Location**: `core/ir/src/transport/github_actions.rs`
**Pattern**: Renderable trait + string building

- `WorkflowConfig` implements `Renderable` (line 580)
- Generates GitHub Actions YAML via string building
- Exists alongside the newer `CiRenderer` trait system
- **Possibly redundant** with `GitHubActionsProvider`
...
|----------|-------|---------|
| Has a trait | 5 | Testgen (TestRenderer), Makegen/gitignore/clippy/WorkflowConfig (Renderable), CI YAML (CiRenderer), Terminal (FrameLoop), CI commands (CiProvider) |
| No trait, has IR | 0 | — |
| No trait, no IR | 8 | CLI gen, pragma text files, DAG code gen, CI report, markdown, LLM prompts, dag_gen, WorkflowConfig (redundant) |
...
|-------|----------|-------|---------------|
| `Renderable` | `core/ir/src/render.rs` | `MakefileRenderer`, `ClippyConfigRenderer`, `WorkflowConfig`, `IgnoreCategorySet` | 3 |
| `TestRenderer` | `core/codegen/src/testgen/render.rs` | `RustRenderer`, `PythonRenderer`, `TypeScriptRenderer` | 2 |
| `CiRenderer` | `core/ir/src/transport/ci/render.rs` | `GitHubActionsProvider`, `GitLabCiProvider` | 5 |
...
|----|----------|-----------|-------------|
| `SharedStep` | `core/ir/src/transport/ci/render.rs` | 12 files (both CI providers, layout, codegen) | `StructuredBlock::Target` + `YamlRenderer` |
| `IgnoreCategory` | `gunbc-dag/src/makegen/gitignore.rs` | gitignore generation | `Category` IR |
...
|------|----------|-----------|---------------|
| Pragma allowlist | `gunbc-dag/src/policy/pragma.rs` | `disallowed-methods-allowlist.txt` | 3 |
| Pragma lint policy | `gunbc-dag/src/policy/pragma.rs` | `pragma-lint-policy.txt` | 3 |
| DAG code gen | `core/codegen/src/dag_gen.rs` | `graph.rs` Rust source | 2 |
| CI report | `gunbc-dag/src/ci/ops.rs` | Plain-text CI report | 3 |
...
`gunbc-dag/src/makegen/render.rs`, `gunbc-dag/src/makegen/gitignore.rs`,
`lib/tools/clippy/src/config.rs`, `gunbc-dag/src/policy/pragma.rs`,
...
| CI YAML (GitHub) | partial (SharedStep) | partial (CiRenderer) | no | no |
| CI YAML (GitLab) | partial (SharedStep) | partial (CiRenderer) | no | no |
...
- `grep -r "Raw(" core/ lib/ gunbc-dag/ --include='*.rs'` in emission-related code → 0 matches
