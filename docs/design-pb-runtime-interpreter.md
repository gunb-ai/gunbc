...
- **R2 PB Manager** (`cool-stag-230` / `r2-pure-bootstrap-manager.md`): owns T-LensProducer-Retirement R3 lane (with sub-gates per §5.1); owns concrete `BinShim` declarations for PB-owned shims (instance-row authoring under `dsl/std/runtime/bin_shims/`) + bin-shim retirement dispatch; dispatches workers on bin-shim retirement once Item 5 lands. **Boundary**: PB owns the retirement lane and the per-shim instance declarations; any generalized substrate-shape change to the `BinShim` carrier itself (additional fields, refining `entry`'s type signature beyond `() -> std.process.ProcessExit`, etc.) follows the §P1 substrate-fact-introduction procedure with escalation to Substrate Manager — same shape as anti-bridge invariant #2 below for `Value`. This split keeps PB's lane scope crisp while preserving Substrate's authority over carrier-type evolution.
