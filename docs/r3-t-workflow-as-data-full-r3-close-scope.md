...
2. **Hand-authored `ci.yml` AUTHORITY DISSOLVED** — `.github/workflows/ci.yml` content authored by gunbc emission (not hand-edited); file present as full-emit artifact (YamlStatic) OR thin-shim entry-point (BinaryShim); regression-guard test prevents hand-authored re-introduction. **Per briansrls BLOCKING #PR2744 2026-05-12 + openai-pro REQUEST_CHANGES on PR #2744 2026-05-12T10:51:06Z**: NOT file deletion — P5 / Pure Bootstrap dissolves AUTHORITY, not file presence; both initial-enum WorkflowRuntime arms (YamlStatic + BinaryShim) REQUIRE the artifact for GH Actions trigger discovery. The "absent" option is structurally impossible for the initial enum (only conceivable for future DESIGN-ONLY runtimes like InlineGunbc that don't target GH Actions; such runtimes land via separate substrate-prereq PR and would extend this gate description at that time).
...
| **NEW: `ci_yml_hand_authority_dissolved`** | state-check | T-WAD | `.github/workflows/ci.yml` is no longer hand-authored CI logic — file is either (a) committed-emission-artifact byte-identical to `project_github_actions(ci_workflow_dag, target)` output with regression-guard test (YamlStatic), or (b) thin-shim entry-point invoking the emitted binary (BinaryShim); in NO case is it hand-edited CI logic AND in NO case is the artifact absent (both initial-enum WorkflowRuntime arms require `.github/workflows/ci.yml` for GH Actions trigger discovery). Regression guard test prevents hand-authored re-introduction. **Per briansrls BLOCKING #PR2744 inline review 2026-05-12T06:58:55Z + openai-pro REQUEST_CHANGES on PR #2744 2026-05-12T10:51:06Z**: deleting hand-maintenance ≠ deleting executable artifact; P5 / Pure Bootstrap dissolves AUTHORITY, not file presence; "absent" pass condition is structurally impossible for initial enum (only conceivable for future DESIGN-ONLY runtimes that don't target GH Actions, e.g., InlineGunbc — would extend this gate at land-time via substrate-prereq PR). |
...
- **Slice 8**: ci.yml hand-authority dissolution (hand-authored content replaced by emission artifact or thin-shim per chosen `WorkflowRuntime`) + workflow-runtime toggle finalized + `ci_yml_hand_authority_dissolved` gate PASSING → **Substrate Mgr (warm-wolf-698) lane**. Gated on Slices 4-7 + acceptance verification. **NOTE** (per briansrls BLOCKING #PR2744 2026-05-12 + openai-pro REQUEST_CHANGES on PR #2744 2026-05-12T10:51:06Z): NOT file-deletion — P5 / Pure Bootstrap dissolves AUTHORITY, not file presence. Both initial-enum WorkflowRuntime arms (YamlStatic + BinaryShim) require `.github/workflows/ci.yml` (full-emit or thin-shim) for GH Actions trigger discovery; "absent" is NOT a valid pass condition for the initial enum.
