...
The fix is one reorder and one re-attachment, both grounded in mechanisms DB-11 already built.

### D1 — Reorder: substitution runs before the refinement identity-terminator
...
fn signature_type_shape(
    ...
    // DB-16 (3a.3 closure): when the declaration carries a refinement
    // AND its base connective (followed through ResolvedIdentifier
    // hops) is a TypeParam with an active substitution binding OR an
    // Instantiation reachable through substitution, substitute the
    // base first and re-attach the refinement to the substituted
    // carrier. Otherwise fall through to the DB-11 identity-terminator.
    if decl.refinement.is_some() {
        if refinement_base_requires_substitution(dag, current, subst) {
            return reattach_refinement_to_substituted_base(
                dag, current, subst, depth,
            )
            .map(TypeShape::new);
        }
        return Some(TypeShape::new(current));
...
The `refinement_base_requires_substitution` helper walks from the refined carrier through `Atom(ResolvedIdentifier(_))` hops (depth-bounded) and returns `true` iff the walk lands on either:

- a `TypeParam` declaration whose id is bound in `subst`, or
- an `Instantiation` declaration whose arguments contain values bound in `subst` — i.e., substitution would materially change the resolved base.

For a concrete refined carrier (`Int where pred(x)`) the walk lands on the named `Int` declaration and short-circuits to `false`; DB-16's reorder becomes a no-op and the identity-terminator fires as before. For a refined generic carrier (`T where pred(x)` with T bound in `subst`) the walk lands on the TypeParam, the predicate returns `true`, and the re-attachment path runs. For a refined generic with T *unbound* (earlier iteration retry case) the predicate returns `false` and the identity-terminator returns the template carrier; the downstream `is_retryable_generic_decl` check at `infer.rs:986` handles the retry signal, matching pre-DB-16 behavior.
...
2. **Concrete refined declarations** see the base-walk short-circuit to `false` and fall through to the DB-11 identity-terminator unchanged. All 14 `test_3a3_*` tests exercise concrete refined types; none see a behavior change.
3. **The new code path fires only when substitution actually has work to do.** Refined TypeParams with no active binding fall through to the identity-terminator and the retry machinery.

### D2 — Re-attachment: fresh substituted-refined `Declaration`

`reattach_refinement_to_substituted_base` constructs a new anonymous `Declaration` that is structurally identical to what `lower_parameter_refinement` (`lower.rs:269`) would produce if the user had written `fn f(x: Int where pred(x))` directly. Walk:

1. **Resolve the substituted base.** Reuse `resolve_decl_with_subst` (`infer.rs:3432`) — it already handles the `TypeParam` + `Instantiation` + `ResolvedIdentifier` chain. Called with the refined carrier's own `DeclarationId`, it traverses the `Atom(ResolvedIdentifier(base))` hop, then the base's own connective, and returns the concrete substituted base (e.g., `Int_decl` when `T := Int` is in `subst`). If resolution fails (e.g., unbound TypeParam — but D1's gate already filtered this case out), return `None` and let the caller fall through to the retry path.
2. **Extract the refinement's predicate slots.** Read the refinement pointer `decl.refinement` (guaranteed `Some` here by D1's gate) and decompose the predicate declaration into `(original_param_port, original_body_port)` using the same read pattern as `predicate_info` (`infer.rs:1168`) and `outer_predicate_slots` (`lower.rs:736`). If the shape is not a well-formed `Arrow { body: UserDefined(bind) }`, return `None` (defensive; should not occur because DB-11's lowering constructs this shape exclusively).
...
4. **Clone the refinement body into the fresh slot.** Invoke `clone_predicate_body` (`lower.rs:762`) with `source_port = original_body_port`, `substitute_from = original_param_port`, `substitute_to = fresh_param_port`. Depth-bounded by `DEPTH_LIMIT = 64`. Produces a fresh sub-DAG that references the fresh param port everywhere the original referenced the template param slot. Walks `Value` and `Transform` nodes only; `Branch` / `Loop` / `Bind` encounters return `None` and DB-16 falls through to the identity-terminator — matching DB-11's `refinement_predicate_out_of_fragment` gate, which rejects the same shapes at lowering.
...
DB-11's `signature_type_shape` takes `dag: &Dag`. DB-16's re-attachment needs `&mut Dag` to allocate ports, nodes, and the fresh declarations. Two options:

- **Option (i): widen the walk function to `&mut Dag`.** Simple but pollutes a read-only signature walk with mutation. Ripples into `resolve_arrow_walk` and callers.
- **Option (ii): cache substituted-refined carriers in an inference-scoped side table.** Call-site-keyed map `(template_refined_decl, SubstStack) → cached_substituted_refined_decl`; pass the cache through `signature_type_shape` alongside `subst`. First call to a given `(template, subst)` materializes the declaration (still needs `&mut Dag` exposure, but only at the cache-miss boundary); subsequent calls reuse. Keeps the common path read-only.

**Proposed: option (ii),** with the cache living on the inference driver (the same scope that already owns `SubstStack`). Prevents redundant declaration allocation when the same call site is re-inferred in a fixpoint iteration. Final shape locked in Part 2 implementation after reading the driver-level plumbing; document here as a constraint, not a decision.
...
4. **Same body sub-DAG.** `clone_predicate_body` reproduces the original predicate's `Value` and `Transform` node topology exactly, re-pointing only the parameter slot; this is the same cloning technique DB-11 uses in `build_narrowed_refinement` to produce a composite that the DB-11 acceptance suite already confirms is structurally identical to a user-authored composite (`test_3a3_narrowed_already_refined_param_preserves_outer_refinement`, `test_3a3_conjunction_discharge_ignores_grouping`).
...
- D2 constructs the substituted carrier by **cloning the original predicate body**, not by transforming or simplifying it. The sub-DAG after substitution is the same sub-DAG before substitution, with one slot re-pointed.
...
### D7 — Substitution caching (detail)

Because `signature_type_shape` may be called multiple times per call site during fixpoint iteration, allocating a fresh carrier on each call is wasteful and — worse — produces a DAG with many structurally-equivalent-but-not-id-equal refined declarations. Cache keyed by `(original_refined_decl_id, substitution_snapshot)`. Cache is inference-scoped (re-built per inference run); does not need to survive across module boundaries since refined generics are confined to single-module usage per §Scope.

The cache is not load-bearing for correctness (DB-11's identity-via-structural-comparison makes duplicate declarations discharge each other fine); it is a substrate-hygiene detail.
...
- **Touches.** `signature_type_shape` arm ordering (`src/v3/compiler/src/infer.rs:3381–3430`). New helper `reattach_refinement_to_substituted_base` in the same module. Reuses `resolve_decl_with_subst` (`infer.rs:3432`) and `clone_predicate_body` (`lower.rs:762`); the latter becomes cross-module-visible (move to a shared internal module or re-export; decision locked in Part 2). Optional inference-driver cache for substituted carriers (D3/D7).
...
5. **`test_3a4_refined_generic_retry_on_unbound_type_param`** — fixpoint interaction. A call site where `T` is not yet bound (an earlier-iteration retry case) must still retry rather than fail hard; D1's fallback `_ => return Some(TypeShape::new(current))` arm + `is_retryable_generic_decl` wiring keeps the existing retry semantics.
...
The re-attachment has to happen somewhere. Doing it at comparison time scatters construction logic across the discharge walker and couples `check_refinement_discharge` to `SubstStack`. Doing it at the single `signature_type_shape` boundary keeps construction logic at the substrate-production site (`signature_type_shape` is the authoritative walk from declaration to `TypeShape`), which matches DB-11's "composite-canonical refinement form" principle — one construction authority per substrate shape. Rejected.
...
- **`src/v3/compiler/src/infer.rs`** — `signature_type_shape` (reorder site), `resolve_decl_with_subst` (substitution helper reused), `SubstStack` (substitution stack passed through), `check_refinement_discharge` / `predicate_discharges` / `refinement_ports_equal` / `refinement_targets_equal` / `declaration_shapes_equivalent` (consumers, unchanged).
- **`src/v3/compiler/src/lower.rs`** — `clone_predicate_body` (re-attachment helper reused; may require cross-module visibility widening). `lower_parameter_refinement`, `narrow_scope_for_predicate`, `build_narrowed_refinement`, `refinement_predicate_out_of_fragment` (DB-11 authorities, unchanged).
...
1. **`signature_type_shape` `&Dag` vs `&mut Dag`** (D3). Final choice between option (i) (widen the walk) and option (ii) (inference-scoped cache) lands in Part 2 after reading the driver-level plumbing. Option (ii) is the stated preference; confirm once the cache shape is verified against the actual inference loop.

2. **Cross-module visibility of `clone_predicate_body`.** Currently private to `lower.rs`. DB-16's re-attachment in `infer.rs` needs access. Lightweight options: move it to a shared internal module, re-export from `lower.rs`, or inline a second copy (rejected — parallel authority). Decision locked in Part 2.
