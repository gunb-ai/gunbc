...
4. **Clone the refinement body into the fresh slot, substituting through Transform targets.** Invoke `clone_predicate_body` (`lower.rs:762`) — **extended with a new `subst: &SubstStack` parameter** — with `source_port = original_body_port`, `substitute_from = original_param_port`, `substitute_to = fresh_param_port`, and the active substitution stack. Depth-bounded by `DEPTH_LIMIT = 64`. The walk produces a fresh sub-DAG that references the fresh param port everywhere the original referenced the template param slot, **and, crucially, concretizes any declaration-level references embedded in `Transform` targets by routing them through `concretize_decl_with_subst`**:
   ...
   Walks `Value` and `Transform` nodes only. `Branch` / `Loop` / `Bind` encounters at this stage are a substrate-integrity violation: `refinement_predicate_out_of_fragment` (`lower.rs:489`) rejects these shapes at lowering time, so any predicate body reaching the materialize phase is in-fragment by construction. **Fail closed per C-8:** register `Diagnostic::ResolveError { name: "refined-generic substitution: out-of-fragment predicate body reached materialization" }` on the refined decl's span and return `None`. A silent None here would mask disagreement between the lowering gate and the materialization walk — a design-level coherence bug worth surfacing.
...
**Single authority, not parallel side table.** Every substituted-refined carrier is a canonical `Declaration` in the Dag — same visibility, same consumer surface as any user-authored refinement. There is no infer-local "pseudo-shape" that only some consumers see. The lookup is `find_equivalent_substituted_refined_decl(dag, substituted_base, original_pred_decl)`: a linear scan over `dag.declarations()` matching on (a) `connective == Atom(ResolvedIdentifier(substituted_base))`, (b) `refinement: Some(pred)` whose body walks structurally equal to the cloned-and-substituted predicate body (using `refinement_ports_equal` / `predicate_discharges` as the equivalence relation — same relation already used for DB-11 discharge, so no new comparator is introduced). Dedup mirrors `find_equivalent_anonymous_instantiation` (`infer.rs:3484`). Two distinct call sites that require the same (template, substitution) combination end up sharing one `DeclarationId`, not producing two structurally-equivalent copies.

This closes the chatgpt-review concern on D3/D7 being a "parallel semantic side table": the cache is the Dag itself, keyed by structural match, consumed through the same boundary every Declaration is consumed through. Any consumer of DB-16's output — `check_refinement_discharge`, emission, analysis lenses — sees a canonical `DeclarationId`.
