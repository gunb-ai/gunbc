...
- **R1** (current, proposed): Workflow control-flow shape is lifted into the substrate as a first-class carrier `WorkflowEffect`. Stage 2b's lens reads the carrier structurally and dispatches per variant. Four variants (`LinearEffect | BranchEffect | LoopEffect | ParallelEffect`) cover the four workflow-level control-flow topologies; recursion through `WorkflowEffect` fields composes them. `LinearEffect` is the only variant Stage 2b consumes; the other three emit diagnostic + name the downstream stage.
