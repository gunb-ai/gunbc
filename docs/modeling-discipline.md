...
from top-level declarations and `// Consumes:` is script-authored input
for that gate. Those allowlist headers are generated artifacts, not
...
2. **A terse header** — ordinary files may keep `Scope:` and `Status:`
   ...
   `Owns:`/`Consumes:` lines. Nothing else: no `Brief:`, no `Seams:`, no
