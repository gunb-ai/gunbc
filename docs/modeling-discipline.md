...
things — nothing else survives. `// Owns:` and `// Consumes:` are
parallel ledgers and are removed from ordinary `.dag` files; ownership
is the module body, and consumption is the import graph. The only
exception is the narrow strict-deprose allowlist regenerated by
`scripts/strict_deprose_dag.py`, where `// Owns:` is machine-derived
from top-level declarations, `// Consumes:` is script-authored input for
that gate, `// Ledger:` is script-generated coproduct inventory, and the
emoji-only RULING-1 slice line (`// 🟢` or `// 🟡`) is the operator-
ratified groundedness marker. Those allowlist headers are generated
artifacts, not hand-maintained prose ledgers.

1. **Line 1** — the file-path line.
2. **A terse header** — ordinary files may keep `Scope:` and `Status:`
   one-liners when they are useful boundary signals. The eight
   strict-deprose allowlist files additionally carry generated
   `Owns:`/`Consumes:`, `Ledger:`, and emoji-only RULING-1 slice lines.
   Nothing else: no `Brief:`, no `Seams:`, no `HEADER RECONCILE`, no
   `Deferred (N)` rationale, no multi-line block.
