...
- **🟢 terminal** — the variants are irreducible distinctions at the
  ...
  structure can be named. Requires that **patterns-tried / terminal rationale**
  be **reviewer-visible** (PR thread or design brief), not a second in-file prose authority.
...
or a `.dag` `type X = A | B | …`) must be classified (🟢/🟡/🔴), with
...
classification write-up — the dissolution-patterns-tried
...
disposition above) — it is recorded in the **PR thread** (or linked brief) even when the
fuller patterns-tried analysis defers. The gate names the **concrete
missing primitive**, e.g. `// 🟡 feature:<missing-primitive> —
<PR-or-brief ref>` — a bare `<sunset-milestone>` is **not** a valid
...
have **proven** the two coincide — and *coincide* means: both groundings,
reduced to canonical `Node`s, are structurally equal, expressed in shared
`std/` vocabulary. Identity is
...
2. its subject is an **external spec primitive** — something a
   ...
   about; **and**
3. it carries **no coincidence evidence** — no review-visible proof that
   `X` and `Y` coincide (PR thread, brief, or the Practice-9 one-line
   `// coincides:` cite on the carrier).
...
never a record of the work done. Rationale lives in the **PR / review
thread** (single authority for prose decisions); process notes live in the commit message. A comment
...
blocks. Architectural decisions move to **PR-visible rationale** (or linked briefs); process
...
structure it narrates and from **review-visible** decision records; it is the
...
| coproduct dissolution | already enforced — per-coproduct 🟢/🟡/🔴 tag (Practices 4 / 9); rationale in PR | already enforced |
