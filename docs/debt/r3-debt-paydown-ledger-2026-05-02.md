...
| `openai_chat_message_full_coproduct` | 2026-04-21 | Grounding services | Retired | Issue #2605 / PR #2627 widened `OpenAiChatMessage` into a role-tagged request coproduct with tool + `tool_call_id`, legacy function rows, and typed text/image content parts. Additive content/tool-call surfaces observed after the original closure tag are tracked separately by `structural_coverage_gap_openai_chat_message_additive_content_parts`. |
...
| `http_path.dag` `None => ""` | 2026-04 | Grounding / fail-closed | Retired | Parser returns the existing typed coproduct on every internal split `None` path: `dsl/std/http_path.dag` now maps those arms to `MalformedPathSegment`, with v2 seed mirror coverage and `effects.rs::parse_path_malformed_segments_propagate_typed_failure` pinning boundary propagation. |
...
| `DeclarationRef` unrefined-any-declaration handle on `CoproductProjection.declaration` | 2026-05-07 | R3 Substrate | Open | Carrier at `src/v3/std/coproduct_projection.dag` (PR #2164, closing #1947) imports the structural typed `DeclarationRef` from `v3.spec.v3_l1` rather than the `dsl/std/serialization.dag::DeclarationRef = String` alias. Selection rationale: codex BLOCKING (sha 0e185083) flagged that adding `dsl/std/serialization.dag` to bootstrap-authority would resolve the alias-import gap but doing so reproduces a 2MB test-thread stack overflow on lane2 symbolic-cost — the typed v3_l1 `DeclarationRef` is already in `V3SpecAuthority` and avoids both the silent-mis-resolution AND stack-budget regressions. The typed handle still admits any declaration at the type level (refinement-typing-on-`DeclarationRef` has not landed); same #1175 substrate gap classification as `MethodRef` (`src/v3/std/methods.dag:31`) + `CallableRef` (`src/v3/std/services.dag:86-100`). **Dissolution trigger**: the same #1175 substrate-level refinement-typing-on-`DeclarationRef` landing that retires those wrappers retires the untyped any-declaration admission here. The `VariantId = String` sibling alias on `coproduct_projection.dag` (Map key handle) dissolves together — promotes to `(coproduct: DeclarationRef, variant: VariantDeclaration)`-shaped structural identity when the same #1175 surface lands. |
