...
| Missing diagnostics-empty bootstrap gate | 2026-04-30 | R3 Substrate + Verification | Retired | `diagnostics_empty_after_bootstrap` ratchet consumes the `bootstrap_authority` carrier and fails with every authority row whose post-bootstrap diagnostics are non-empty. |
