...
|---|---|---|
| `src/v3/compiler/src/dag.rs` | Hybrid: `include!()`s several `*_generated.rs` modules produced from `substrate.dag` via `scripts/regen_runtime_mirrors.py` (scalar / branch / cluster / lookup / cost / scalar-operator-shim — see §"Substrate generation is already proven and shipping"). The remaining hand-authored content is the core Dag/Node/Port/Conj/Disj wiring not yet covered by `render_dag_*_module` renderers. PB-Substrate pilot v2 (#780) extended the scalar module with `ArithmeticOp` / `ComparisonOp` / `LogicalOp` / `OperatorKind`; further slices follow the same pattern. | Continue extending `regen_runtime_mirrors.py` renderers until the residual hand-authored slice in `dag.rs` is empty. |
