...
One single-module `.dag` program per (position, specimen) pair. Every arm shares one prelude
declaring a coproduct, one of its payload types, a generic carrier and a record, and differs
only in the marked expression. Each arm is compiled on its own source root.

The arms were generated by a throwaway shell loop over the specimen table below, run in a
scratch directory and not committed: raw shell in the repo is the scaffold tell DESIGN §6
names, and a one-off generator has no final consumer. What survives is what the arms
*measured* — the tables here — plus the arm shapes themselves, which are stated exactly
enough below to be rebuilt from this file. The prelude and the four specimen expressions are
the whole instrument; nothing about it is recoverable only from the script.

The prelude every arm shares, verbatim:

```
module probe.inhabit
...
`Rel` is the declared coproduct, `Wrapped`'s payload `Inner` is the type used for the
arm-payload-at-parent specimen, `Boxed<T>` carries the generic-argument position and `Holder`
the record-field one. Each arm appends one line to this prelude and nothing else.
