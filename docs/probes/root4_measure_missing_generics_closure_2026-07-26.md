...
1. A locally closed `List<Measure<Time, S, Nat>>` positive fixture, isolated from the independent `std.measure` import-advisory wall.
2. A closed zero-arity `ByteSize = Measure<Memory, One, Nat>` alias control.
