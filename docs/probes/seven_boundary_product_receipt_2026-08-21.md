...
**AND IT HAS MOVED AGAIN, OFF RETENTION ENTIRELY (2026-08-22).** With the declaration-interior classification corrected, wrapper retention on that module is not reduced but ABSENT -- the retention census reports `families 0, observations 0` -- and boundary 4 refuses at `8` diagnostics: the invariant `7 parse_grammar_choice_overlap_residue` plus `1 parse_g0_tokens_remain`, a PARSE diagnostic and a different stage from every door before it. B5 remains NotExercised. -> [b4 door-holder: the declaration interior](b4_door_holder_declaration_interior_2026-08-22.md).
