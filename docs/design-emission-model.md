...
- **`include_str!` retirement** (gpt-5-5-pro Finding #12): `src/v3/compiler/src/pipeline_authority.rs` cross-checks `fn compile` stage order against `PipelineStageBinding` declaration order. The compile-body witness is sliced using the `ArrowBody::Unparsed` span already on the lowered `compile` declaration, then read from on-disk `pipeline.dag` (no compile-time `include_str!` of that file). Same fact still lives twice (bindings + orchestrator body) until case 2c derivation collapses the carriers. **R3 T-Bridge-Retirement sub-lane:** unified ledger of `include_str!` side-channels across the codebase; each instance retires when its consumer can read the structured authority directly.
