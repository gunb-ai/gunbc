...
- **`include_str!` retirement** (gpt-5-5-pro Finding #12): `src/v3/compiler/src/pipeline_authority.rs:135-178` does `include_str!("../pipeline.dag")` then line-parses the source text to extract stage names — same fact lives twice (as ordered `PipelineStageBinding` data + as compile-body source-text lines). Pipeline-stage-order is a concrete instance of the broader `include_str!` side-channel pattern. **R3 T-Bridge-Retirement sub-lane:** unified ledger of `include_str!` side-channels across the codebase; each instance retires when its consumer can read the structured authority directly.
