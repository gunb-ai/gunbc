...
- **R3 Debt-Paydown PR-anticipation gate** (added 2026-05-06): `pr_anticipation_discipline_ci_active` — CI verifiably enforces §7 PR-authoring contract via `scripts/check-pr-sg0-net-shrink-discipline.sh` (**CONSUMER_LANDED** — `.github/workflows/ci.yml` `ci` job; §1.8 row #75). Closes the predicate→consumer gap for going-forward debt-discipline gates.
...
| 75 | `pr_anticipation_discipline_ci_active` | CI-discipline | R3 Debt-Paydown (standing) | **CONSUMER_LANDED** — `.github/workflows/ci.yml` `ci` job runs `scripts/check-pr-sg0-net-shrink-discipline.sh --self-test` on every non-draft `push`/`pull_request` and runs the script with `PR_BODY` on `pull_request` when the SG-0 census differs from `origin/main` | `scripts/check-pr-sg0-net-shrink-discipline.sh` in CI |
...
| T-Debt-Paydown (standing) | YELLOW | **gentle-newt-665 archived 2026-05-10; silent-ram-834 re-spawned per Director Option A at gunbc#846 c#4414372073 (sub-issue #2566 charter, identity inbox #2567)**; Phase 1+2 deliverables MERGED in PR #2437 (2026-05-09; Class A-G + Class P + 6 novel-finding briefs); Phase 3 dispatch tracker MERGED in PR #2454/#2456; Phase 3 fleet at 8/10 work items closed/absorbed per silent-ram-834 status report (gunbc#828 c#4414611117); orphan PR #2503 closed 2026-05-10 06:55Z per Director default disposition; PR #2399 §1.8 ledger Status drift sweep MERGED 2026-05-10 | §1.8 gate **#75** `pr_anticipation_discipline_ci_active` **CONSUMER_LANDED** (`.github/workflows/ci.yml` `ci` job — see §1.8 row #75); SG-0 trajectory tracker (`docs/audit/r3-sg0-trajectory-tracker.md`) at 164 entries (b71148d06 per silent-ram-834 04:54Z reading); +2 vs corrected 2026-05-09 EOD baseline 162; tripwire status pending/uncomputed | continuous; F1 (#2465) + F2 (#2466/#2473) remaining as last 2/10 Phase 3 items |
