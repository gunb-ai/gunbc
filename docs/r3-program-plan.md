...
| 9 | `l4_emit_eval_match` | structural-fold | T-V-L4-L7-Direct | **CONSUMER_LANDED** (`r3_verification_l4_emit_eval_match.dag` + `r3_verification_l4_l7_l5_skeleton_test.rs`; canonical `TestClaim.name` = `l4_emit_eval_match`; suite `r3_verification_l4_l7_direct_suite` wires three Rust/Int W1 `DifferentialEquals` certification seeds; each seed **Pass** in CI). **Not PASSING:** `r3-structure.md` §Acceptance quantifies over **every** certification-corpus program — exhaustive wiring remains lane work; **PASSING** attaches only under §1.7 corpus-quantified rule | per-program emit ↔ eval algebraic equality |
