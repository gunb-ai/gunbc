...
| 19 | `numeric_aliases_align_to_refinements` | substrate-shape | T-Numeric-Construction | **CONSUMER_LANDED + PASSING for Int/UInt alias refinements** (2026-05-11). Producer substrate: `dsl/std/integer.dag` declares `Int8`/`Int16`/`Int32`/`Int64`/`Int128` as `Compose<Int, MachineWidth<Byte\|Word16\|Word32\|Word64\|Word128>>` and `UInt8`/`UInt16`/`UInt32`/`UInt64`/`UInt128` as `Compose<UInt, MachineWidth<Byte\|Word16\|Word32\|Word64\|Word128>>`; `Int = AbelianGroup<GroupCompletion<Nat>>`, `UInt = Nat`. Consumer evidence: `bootstrap_integer_aliases_align_to_refinements_per_gate_19` / `assert_bootstrap_integer_aliases_align_to_refinements` walks all ten aliases and asserts the `Compose<abstract, MachineWidth<...>>` shape. Receipt: [`docs/audit/t-numeric-construction-gate-19-alias-refinement-receipt.md`](audit/t-numeric-construction-gate-19-alias-refinement-receipt.md). | Fixed-width Int/UInt public names are refinement-shaped aliases, not parallel `OrderedRing<Word*>` / `Semiring<Word*>` substrate. Float alias receipts route through Real32/Real64 and remain bounded by the existing Real/Float follow-on scope; this row does not claim default-width alias deletion. |
