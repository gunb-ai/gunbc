...
|---|---|---|---|
| 1 — parser/grammar surface | `substrate_gap_parser_grammar_closed` | v3 parser handles `<TBD-input-from-Substrate-canvas>` without v2-fallback | T-V2-Retirement (parser path) + Substrate continuation |
...
**Open** (Q2, §10): ratify framing as "closure-criteria-not-lanes". Ratify representative gap-test selection per class (TBD for Class 1).

### §1.5 Closure-ledger total

**Total R3 closure gates** (post-Q1 + Q2 ratification): existing 60+ gates from `r3-structure.md` §"Acceptance" + 5 new substrate-gap-class gates = **~65 gates** across 18 lanes + 1 standing program.
...
**Source-of-truth**: [`docs/audit/r3-debt-sweep-2026-05-06.md`](audit/r3-debt-sweep-2026-05-06.md) (PR #1804 lane). Phase 2 sweep deliverables landed §1.1–§2.2 covering ~~30-40~~ **66 hand-Rust files in `src/v3/compiler/src/`** (excluding `emit/`) + 35 v3 std mirror entries + 112 R2/R3-era PRs.
...
| # | Question | PM recommendation | Ratification-needed-by | Status |
|---|---|---|---|---|
| Q1 | Closure-criteria predicate set (§1.1–§1.4) | Predicates per §1 tables; 7 Pattern-A + 2 v2 + 1 BridgeLedgerZero + 5 substrate-gap = 15 new/named gates | Brian | OPEN |
| Q2 | 5 substrate-gap classes — separate lanes vs closure-criteria-over-existing-lanes | Closure-criteria framing (§1.4 + §4) | Brian | OPEN |
| Q3 | v2 retirement Mgr ownership | PB Mgr owns all (existing T-V2-Retirement scope) | Brian (informational; PM proceeds unless redirected) | OPEN |
| Q4 | Mgr structure (keep 9 vs reorganize) | Keep 9 standing Mgrs with re-anchored scope | Brian | OPEN |
| Q5 | Free-consequences-demonstration as closure gate | Required as gate; 10 sub-gates per `r3-structure.md` line 119–129; performance metric informational | Brian | OPEN |
| Q6 | Plan-as-doc shape (this doc vs r3-structure.md merge) | New doc (this); cross-link both directions | PM proceeds unless redirected | OPEN |
| Q7 | Drift items reconciliation routing (#1638, #1499, declaration_by_name) | Single Debt-Paydown worker reconciles all three → one PR | PM proceeds unless redirected | OPEN |
...
| ID | Question | Decision needed | Default-if-no-answer |
|---|---|---|---|
| Q-PAFS | Which Pattern-A first executable slice (TC1 vs RustDagIsomorphism vs other) | Director + Brian | TC1 η-equivalence (Verification has deeper analysis docs in flight) |
| Q-NYI-Accounting | NYI claim coverage/completeness accounting | Verification + Director | Mark NYI incomplete in aggregation; ratchet-on-evaluator-slice |
| Q-Std-Carrier-Phasing | Rust std-carrier phasing (Option/Cardinal/HigherOrderMethodSpec/allocator-hasher) | Substrate + Director | Phase 1 = default-only/default-instance; later phases as separate Substrate slices |
| Q-Drift-Reconcile | Drift items routing (#1638, #1499, declaration_by_name) | PM (proceed unless redirected) | Single Debt-Paydown worker; one PR |
| Q-Pattern-Class-Naming | Substrate-gap-class TestClaim names | Brian | Names per §1.4 table |
| Q-PR-F | PR-F priority + sequencing (BoundDeclaration consumer + Rust ReferenceModel<T>) | Substrate + Director | Schedule into critical path post-T-E-P-Producer-Broadening |
| Q-Anthropic-Variant-Aware | Variant-aware typed REST response projection carrier shape | Substrate + Director | Defer #1702 re-dispatch until Substrate authors variant-aware projection metadata carrier |
| Q-Emit-Shim-Handoff | Grounding direct execution vs PB/Substrate/v2-retirement consumption for `emit/rust_target.rs` family | Director | Grounding consumes PB/v2-retirement milestones; no Grounding direct execution on these files |
| Q-Coercion-Fold-Scratch | Lane placement for `ScratchIntExamples` retirement | Director | Grounding owns close after LanguageSpec projection executable |
