...
| 59 | `recursive_flex_demonstration_landed` | demonstration | T-Lens-Self-Application | **CONSUMER_LANDED** (PR #2826) — emit-back per `docs/r3-structure.md` T-Lens-Self-Application: `t_ci_workflow_as_data_demo_test::gunbc_ci_github_actions_workflow_dag_matches_yaml_generator_output` locks `.github/workflows/ci.yml` ↔ `dsl/gunbc/ci_github_actions_workflow.dag` via `gen_gunbc_ci_workflow_dag`. **Bootstrap:** `bootstrap_authority.dag` lists `t_ci_workflow_as_data_demo.dag` (CI `Workflow` + timing-lens shell). **Follow-on:** restore companion `compile_to_dag` receipts in that module when generator-sized `Workflow` literals clear M1(2.8) opaque-body checks. **Deferred:** `evaluate_body` on `demo_ci_modeled_timing_dimension_report` remains `#[ignore]` (ROADMAP T-WAD). | narrative thesis cashes (emit-back branch) |
