...
| 59 | `recursive_flex_demonstration_landed` | demonstration | T-Lens-Self-Application | DECLARED — **not** CONSUMER_LANDED on PR #2826 (codex + inline BLOCKING 2026-05-13): per `docs/r3-structure.md` §"Acceptance" T-Lens-Self-Application / INVARIANTS P2, the emit-back narrative requires a **`.dag`-authoritative** receipt — **`project_github_actions(ci_workflow_dag, YamlStatic)`** (pinned as `data gunbc_ci_yml_workflow` in `dsl/gunbc/ci_emission.dag`) **or** a **direct-execution** witness — **not** substitutable by a YAML-ingestion drift lock alone. **Supporting ratchet (orthogonal):** `t_ci_workflow_as_data_demo_test::gunbc_ci_github_actions_workflow_dag_matches_yaml_generator_output` keeps committed `dsl/gunbc/ci_github_actions_workflow.dag` **byte-identical to** `gen_gunbc_ci_workflow_dag` output when fed **YAML authority** `.github/workflows/ci.yml` (direction: YAML upstream → generator → derived `.dag` bytes on disk). **Bootstrap / follow-on / deferred:** `bootstrap_authority.dag` lists `t_ci_workflow_as_data_demo.dag`; companion `compile_to_dag` receipts blocked on M1(2.8) for generator-sized `Workflow` literals; `evaluate_body` on `demo_ci_modeled_timing_dimension_report` remains `#[ignore]` (ROADMAP T-WAD). | narrative thesis cashes (pending projection or direct-execution receipt) |
