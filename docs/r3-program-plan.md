...
| 51 | `cross_target_optimization_constant_fold_consistent` | structural-fold | T-Free-Consequences-Demonstration | **CONSUMER_LANDED (partial)** — PR #2582: representative `fold_pre` / `fold_post` witness + `TestRunner` executable `BinaryDimensionReportEquals` (structural fold via Int-add `OperatorRealization` + cross-target `Type(Int)` overlay). **Not** `r3-structure.md` certification-corpus-wide closure; **PASSING** stays deferred until that bar is evidenced. | structural cost-shrink across targets |
