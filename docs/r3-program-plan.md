...
| 80 | `cost_lens_behaviorally_complete` | structural-fold | T-Lens-Behavioral-Parity | **PASSING** — gate #80 abstract-cost scope now has executable `DimensionReport<SymbolicCost>` wiring via `analyze_symbolic_cost_dimension`, `SizeVariable` value identity by `source_port`, recursive `CallPattern` consumption, and Band-C frozen-projection cementing in `tests/dag/t_r3_gate_87_cementing_regen_cost_symbolic.dag` through the PB-B-1 runner table; target-realization composition remains tracked separately by T-CostLens gates #37/#40/#70 | Dimension<SymbolicCost> wiring + SizeVar value semantics + cementing test |
