...
| 18 | `numeric_width_refinements_landed` | substrate-shape | T-Numeric-Construction | **CONSUMER_LANDED for Grounding G2 primitive rows** (2026-05-10). Producer-side substrate: Int<N> arm via `IntW32/64/128 = Compose<Int, MachineWidth<WordN>>` at `dsl/std/integer.dag:93-95` (PR #2161); Nat<N> arm via `UIntW32/64/128` at `dsl/std/integer.dag:96-98` + `UInt = Nat` axiom at `dsl/std/integer.dag:148`; Float/Real arm unblocked by S8 shape in `dsl/std/float.dag` (`Float32/Float64 = Compose<Ieee754Float, MachineWidth<Word32|Word64>>`, `Real = ApproximateField<FieldOfFractions<Int>>`). Consumer evidence: Rust `f32`/`f64` rows in `dsl/extdeps/languages/rust/primitives.dag`, pilot mirror routing in `src/v3/grounding_pilot`, and `src/v3/grounding_engine` full-list validation over `rust_pilot_primitives` rather than first-row-only sampling. | `Int<N>` / `Nat<N>` / `Real<N>` width-refinement surface has Grounding G2 primitive-row consumer evidence; broader Real<N> emission demonstrations remain under S9/Shape-A follow-ons. |
...
| 24 | `numeric_reframe_no_parallel_authority` | state-check | T-Numeric-Construction | CONSUMER_LANDED | old integer aliases removed, and Float rows no longer consume the legacy `Field<Word*>` parent. **Per-arm HEAD evidence:** **Int arm** — `type Int = AbelianGroup<GroupCompletion<Nat>>` at `dsl/std/integer.dag:147`; no `type Int = Int64` residue in `dsl/std/` or `src/v3/std/`. **UInt arm** — `type UInt = Nat` at `dsl/std/integer.dag:148`; no `type UInt = UInt64` residue at HEAD. **Float arm** — `dsl/std/float.dag` now imports `ApproximateField`, defines `Float32/Float64` as `Compose<Ieee754Float, MachineWidth<Word32|Word64>>`, and defines `Real = ApproximateField<FieldOfFractions<Int>>`; Rust Grounding rows consume `ApproximateFieldAlgebra` for `f32`/`f64` and the engine validates those rows through the loaded pilot list. The default `type Float = Float64` remains a default-width alias, not the former exact-field parent authority. |
...
PR-F (`BoundDeclaration` consumer + Rust `ReferenceModel<T>` axes) and the now-landed Float migration (T-NumericConstruction-ApproximateField) were blockers for **full Rust primitive grounding** (T-Ground-Rust complete-coverage) + downstream **L5 cross-target consistency**, NOT for `v2_directory_deleted` gate.
...
2. Float carrier migration landed (`Float32/Float64 = Compose<Ieee754Float, MachineWidth<Word32|Word64>>`; `Real = ApproximateField<FieldOfFractions<Int>>`)
3. T-Ground-Rust full-coverage primitive rows land (post-PR-F + post-Float)
...
| T-Numeric-Construction | YELLOW | T-NumericConstruction-ApproximateField; #1523 landed; Float migration/base-carrier consumed by Grounding G2 primitive rows | Remaining numeric construction demonstrations and downstream emission consumers | (TBD) |
...
- **Plan integration**: Rust primitive rows now cover `f32`/`f64` through `ApproximateFieldAlgebra`; broader Real<N> emission demonstrations remain in S9/Shape-A follow-ons.
