...
| T-V-L4-L7-Direct | `l4_emit_eval_match` (**§1.8 #9** slice-1 consumer landed — three Rust/Int W1 seeds + canonical claim name); `l7_algebraic_laws_witnessed` still staged (closure bar = exhaustive L4 corpus + per-(algebra, inhabitant, law) L7 coverage per `r3-structure.md` §"Lane structure") | `l4_emit_eval_match` executable receipt lands demonstration minimum for gate #9 only; full lane demonstration remains gated on L7 + corpus exhaustion |
...
| 9 | `l4_emit_eval_match` | structural-fold | T-V-L4-L7-Direct | **CONSUMER_LANDED + PASSING** (`r3_verification_l4_emit_eval_match.dag` + `r3_verification_l4_l7_l5_skeleton_test.rs`; canonical `TestClaim.name` = `l4_emit_eval_match`; suite `r3_verification_l4_l7_direct_suite` wires three Rust/Int W1 `DifferentialEquals` certification seeds) — **slice-1 only**; exhaustive certification corpus per `r3-structure.md` §Acceptance remains lane closure work alongside `l7_algebraic_laws_witnessed` | per-program emit ↔ eval algebraic equality |
