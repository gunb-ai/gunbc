...
| 18 | `numeric_width_refinements_landed` | substrate-shape | T-Numeric-Construction | **PARTIAL CONSUMER_LANDED 2026-05-09** (Int<N> + Nat<N> arms closed; Real<N> arm HELD on S8 `ApproximateField<F>` cascade per S9 brief Phase-2 STOP) — Int<N> arm via `IntW32/64/128 = Compose<Int, MachineWidth<WordN>>` at `dsl/std/integer.dag:93-95` (PR #2161); Nat<N> arm structurally satisfied by `UIntW32/64/128 = Compose<UInt, MachineWidth<WordN>>` at `dsl/std/integer.dag:96-98` via `UInt = Nat` axiom at `dsl/std/integer.dag:148` per Director **(c) structural-coverage** ratification at gunbc#828 c#4413633535 (compiler sees through alias names; no parallel `NatW*` representation needed — would be P2 single-authority violation). Advances to full CONSUMER_LANDED upon S8 cascade. | `Int<N>` / `Nat<N>` / `Real<N>` (post-#2156 spelling target; pre-#2156 Word-carrier spelling structurally equivalent) |
