...
| 72 | `e_p_producer_demonstration` | demonstration | T-E-P-Producer-Broadening | **CONSUMER_LANDED** — integration `e_p_producer_demonstration` (`m2_substrate_inhabitance_test.rs`) | representative two-arg self-call: per-arg `SubValueRelation` + `per_call_pattern_at` + `LoweringTarget` `DescentEvidence::Strict` |
