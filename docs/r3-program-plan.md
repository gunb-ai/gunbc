...
| 59 | `recursive_flex_demonstration_landed` | demonstration | T-Lens-Self-Application | **CONSUMER_LANDED** (PR #2826) — R3 gate `recursive_flex_demonstration_landed` narrative (*compiler stack for gunbc programs validates the workflow artifact that produces gunbc*) lands on the **emit-back** branch per `docs/r3-structure.md` §"Acceptance" T-Lens-Self-Application ("Either emit-back-to-CI-YAML … OR direct execution"): `t_ci_workflow_as_data_demo_test.rs::gunbc_ci_github_actions_workflow_dag_matches_yaml_generator_output` byte-locks `.github/workflows/ci.yml` ↔ `dsl/gunbc/ci_github_actions_workflow.dag` via workspace `gen_gunbc_ci_workflow_dag` (same implementation as the `gen_gunbc_ci_workflow_dag` binary), mechanically coupling the CI workflow bytes GitHub executes to the gunbc-authored `.dag` module. **Bootstrap surface:** `src/v3/std/bootstrap_authority.dag` lists `src/v3/std/t_ci_workflow_as_data_demo.dag` (CI-shaped `Workflow` + `Lens<TimingMeasurement>` reporting shell). **Follow-on:** restore ledger citations to companion `compile_to_dag` receipts in the same integration module (`gunbc_ci_*_compiles`, `ci_workflow_as_data_demo_*`) once isolated compilation of generator-sized `Workflow` literals clears M1(2.8) opaque-body validation (current failures are upstream of this gate's emit-back cash-out). **Deferred:** full-bootstrap `evaluate_body` on `demo_ci_modeled_timing_dimension_report` remains `#[ignore]` per ROADMAP T-WAD (direct-execution branch; informational SLO only). | narrative thesis cashes (emit-back branch) |
