...
| 17 | `numeric_abstract_carriers_landed` | substrate-shape | T-Numeric-Construction | **CONSUMER_LANDED** | Magnitude/Nat/Int/Rational abstract chain is declared and ratcheted; `Real` alias is held by the approximate-field parameter STOP, while `ApproximateField<F>` carrier axes are landed. |
| 18 | `numeric_width_refinements_landed` | substrate-shape | T-Numeric-Construction | **DECLARED — PRODUCER-SUBSTRATE LANDED 2026-05-09** (PARTIAL CONSUMER_LANDED claim **RETRACTED 2026-05-09** per codex BLOCKING review on PR #2425 sha 7d455658 — producer-side substrate carriers ≠ fail-closed consumer per §1.7 status taxonomy). **Producer-side substrate landed**: Int<N> arm via `IntW32/64/128 = Compose<Int, MachineWidth<WordN>>` at `dsl/std/integer.dag:93-95` (PR #2161); Nat<N> arm via `UIntW32/64/128` at `dsl/std/integer.dag:96-98` + `UInt = Nat` axiom at `dsl/std/integer.dag:148` per Director **(c) structural-coverage** ratification at gunbc#828 c#4413633535. **No fail-closed consumer at HEAD** — only generated bootstrap snapshots cite IntW*/UIntW* (auto-emit, not assertion). Per S9 brief: *"Phase-1 lands the substrate; emission consumer landing is Grounding G2 (T-Ground-Rust)"* — gate #18 advances to CONSUMER_LANDED upon Grounding G2 fail-closed consumer landing (cross-program with Grounding Mgr #1745). Real<N> arm separately HELD on S8 `ApproximateField<F>` cascade. | `Int<N>` / `Nat<N>` / `Real<N>` (post-#2156 spelling target; pre-#2156 Word-carrier spelling structurally equivalent); CONSUMER prereq: Grounding G2 |
...
| 24 | `numeric_reframe_no_parallel_authority` | state-check | T-Numeric-Construction | DECLARED | old aliases removed. **Per-arm HEAD evidence (non-closure receipt; §1.7 status remains DECLARED until executable consumer lands per §1.7 taxonomy — slice receipts are evidence, not closure)** per fierce-fox-86 audit at gunbc#2068 c#4413645211 (Mgr-ratified scope-narrow 2026-05-09 c#4413651570; Director (c) structural-coverage precedent at gunbc#828 c#4413633535): **Int arm** — `type Int = AbelianGroup<GroupCompletion<Nat>>` at `dsl/std/integer.dag:147` (Slice 3 PR #1466 Q6 single-authority form); no `type Int = Int64` residue in `dsl/std/` or `src/v3/std/` at HEAD. **UInt arm** — `type UInt = Nat` at `dsl/std/integer.dag:148` (PR #1818 canonical-instance form); no `type UInt = UInt64` residue at HEAD. **Float arm** — `dsl/std/float.dag:18` `type Float = Float64` parallel-authority alias persists; deferred to S8 `ApproximateField<F>` import (currently lives in `src/v3/std/approximate_field.dag:75` only). Editing `dsl/std/float.dag` ahead of S8 precluded by `feedback_import_not_redeclare_carriers`. Status → CONSUMER_LANDED when an executable HEAD-grep consumer covers all three arms (Float arm unblocks on S8 cascade) |
...
| T-CostLens-Composition | YELLOW | α-narrow ratified at gunb-ai/gunbc#828 #issuecomment-4400772335 — gates **#38** + **#39** structurally satisfied at HEAD by construction (`SymbolicCost` sole cost dimension; `Semiring<SymbolicCost>` sole composition authority); receipts landed via PR #2171. **ε path RATIFIED 2026-05-07** (Q-Cost-Composition-Layering canvas at PR #2181, canonical-not-transitional) — gates **#37** + **#40** + **#70** close via Rust-side composition reading abstract `SymbolicCost` × per-primitive realization-cost in T-CostLens follow-on slice. **Q-Lens-Target-Context β-extended path DEFERRED** to N=2 trigger event (when second lens surfaces target-context need; emission_provenance most likely candidate per cross-cutting analysis); not active R3 ratification gate. | R2-Evaluator + R2-T-Substrate-Lens-Primitive + T-CostLens follow-on slice (Rust-side composition per ε) | partial close per α; #37/#40/#70 close post-ε-slice |
