...
| 24 | `numeric_reframe_no_parallel_authority` | state-check | T-Numeric-Construction | **PARTIAL CONSUMER_LANDED 2026-05-09** (Int + UInt arms closed; Float arm HELD on S8 `ApproximateField<F>` cascade per S9 brief Phase-2 STOP) — Int arm satisfied at HEAD by `type Int = AbelianGroup<GroupCompletion<Nat>>` at `dsl/std/integer.dag:147` (Slice 3 PR #1466 single-authority Q6 form); UInt arm satisfied at HEAD by `type UInt = Nat` at `dsl/std/integer.dag:148` (PR #1818 canonical-instance form). No `type Int = Int64` / `type UInt = UInt64` parallel-authority residue remains in `dsl/std/` or `src/v3/std/` per fierce-fox-86 grep audit at gunbc#2068 c#4413645211. Float arm: `dsl/std/float.dag:18` `type Float = Float64` parallel-authority alias persists pending S8 `ApproximateField<F>` import (currently lives in `src/v3/std/approximate_field.dag:75` only); editing `dsl/std/float.dag` ahead of S8 would parallel-declare per `feedback_import_not_redeclare_carriers`. Mgr ratification of partial-arm-closure precedent (per gate #18 #2068 c#4413651570) follows Director (c) structural-coverage at gunbc#828 c#4413633535. Advances to full CONSUMER_LANDED upon S8 cascade landing abstract Float in `dsl/std/float.dag`. | old aliases removed |
