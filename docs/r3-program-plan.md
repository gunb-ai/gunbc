...
| 16 | `pb_self_compile_fixed_point` | fixed-point | T-FixedPoint | CONSUMER_LANDED (R1 horizon; R3 stronger interpretation pending) — **Wave-1 P0 readiness pins landed via PR #2783 squash 2026-05-12 23:22Z** (`self_host_fixed_point.rs` + `db-8.md` cross-links + CI comments + `verification.dag` not-yet-dispatch-eligible banners; no R1 Pass/Fail change); P3 strong fixed-point claim deferred per parent brief joint-precondition rule (R2-Evaluator + R2-Grounding-Rust+Python + T-LP/SG-0 + Row-B materialization) | bit-identical stage0 + emitted artifact |
