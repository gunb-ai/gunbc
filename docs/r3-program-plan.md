...
| 11 | `tc1_eta_equivalence_executable` | DimensionReport-typed | T-V-L4-L7-Direct | DECLARED (NEW 2026-05-06; scaffold authored PR #2184 with NotYetImplemented sentinel per Director (C-modified) ratification at #828; assertion upgrades on Evaluator E3.c (#1970) merge → flips DECLARED → CONSUMER_LANDED → PASSING in one move) | runtime prereq: G1.a static-rep OR G1.b generic + eta relation |
...
| 79 | `complexity_lens_behaviorally_complete` | structural-fold | T-Lens-Behavioral-Parity | DECLARED — **IN R3 (option b RATIFIED 2026-05-06)** | symbolic CostExpr + work/span split + asymptotic classification + cementing test |
| 80 | `cost_lens_behaviorally_complete` | structural-fold | T-Lens-Behavioral-Parity | DECLARED — **IN R3 (option b RATIFIED 2026-05-06)** | Dimension<SymbolicCost> wiring + SizeVar value semantics + cementing test |
| 81 | `parallelism_lens_behaviorally_complete` | structural-fold | T-Lens-Behavioral-Parity → **R4-CARVED (C1)** | **CARVED to R4** per `docs/r4-carve-out-routing.md` C1 (option b RATIFIED 2026-05-06) — Stage 2e walker port substrate exists; port + rewire dispatched in R4 Substrate continuation |
| 82 | `effect_enumeration_lens_behaviorally_complete` | structural-fold | T-Lens-Behavioral-Parity → **R4-CARVED (C2)** | **CARVED to R4** per `docs/r4-carve-out-routing.md` C2 (option b RATIFIED 2026-05-06) — 4c caller-side effect-set pinning carrier requires P1 substrate-fact-introduction not landed in R3 horizon |
| 83 | `lens_capability_register_zero_proxy_zero_stub` | state-check | T-Lens-Behavioral-Parity | DECLARED — **NARROWED scope (C3) IN R3** | register status ZERO PROXY / ZERO STUB **for in-scope lenses (complexity + cost) only** at R3 close per `docs/r4-carve-out-routing.md` C3; parallelism + effect_enum entries documented as R4-carved |
...
| 96 | `value_body_substrate_mirror_isomorphism_executable` | substrate-shape | T-V-L4-L7-Direct (Verification cross-program with Substrate) | DECLARED (NEW 2026-05-08; Q-ValueBody-Isomorphism RATIFIED at gunbc#828 #issuecomment-4403972737 + Brian sanction at gunbc#846; substantive content per `docs/briefs/r3-v-valuebody-substrate-mirror-isomorphism-v1-worker.md`) | Rust `ValueBody` ↔ substrate `.dag` mirror isomorphism gate; **gate-satisfaction discipline**: `feedback_isomorphism_or_generation_for_mirrors` (**generation-or-isomorphism only** — **INVARIANTS §P1 Modeling Faithfulness**: mirror grounds in canonical source via generation or structural-derivation), **NOT hand-maintained dual taxonomies** (**INVARIANTS §P2 Boundary Discipline**: every fact in exactly one authoritative place; hand-maintained TestClaim that enumerates variants is structurally inadmissible — would itself constitute parallel authority). CI-visible conformance must derive from a single canonical source (generation from single authority OR structural-isomorphism check via existing predicates such as `Lens<DagShapeReport>` + `BinaryDimensionReportEquals` post-shape-report producers per gate #14); bounded mechanism set per brief: build-time enum walk over canonical type definition / boot-time structural check / `.dag` `TestClaim` over canonical shape projection / `build.rs` check reading canonical generation source — **all four are structural-derivation-from-single-authority**, not hand-maintenance. Specific mechanism ratified Slice 1 with Substrate Mgr per brief §Implementation slices. Fail-closed on variant/shape skew; co-ownership Substrate (carrier/mirror authority) + Verification (harness/conformance test spec) per brief §Worker pin. |
