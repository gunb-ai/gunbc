...
| 17 | `numeric_abstract_carriers_landed` | substrate-shape | T-Numeric-Construction | LANDED | Magnitude/Nat/Int/Rational abstract chain is declared and ratcheted; `Real` alias is held by the approximate-field parameter STOP, while `ApproximateField<F>` carrier axes are landed. |
