...
| 66 | `lens_producer_retirement_executable_witness` | demonstration | T-LensProducer-Retirement | **CONSUMER_LANDED** (PR #2595 squash `a085f6e9` 2026-05-10 15:52Z — `src/v3/compiler/tests/integration/r3_lens_producer_retirement_executable_witness_test.rs` runs `.dag` PB census claim through `TestRunner` over fixture `r1_pb_census_gates.dag`; observes live lens-producer residual `CURRENT_RESIDUAL_COUNT = 3` instead of `NotYetImplemented`); **PASSING gated on residual = 0** (cascades from T-LensProducer-Retirement: `lens_apply.rs` + `lens_testgen.rs` + `regen_lens.rs` all retired ⇒ `lens_producer_files_remaining` claim returns `Pass`) | NEW 2026-05-06; per §1.7 taxonomy executable consumer ⇒ CONSUMER_LANDED; PASSING is the deferred state pending gates #5 + #6 + #7 producer-surface retirements |
