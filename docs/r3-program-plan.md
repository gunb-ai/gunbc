...
| 52 | `cross_target_optimization_cost_structurally_derived` | structural-fold | T-Free-Consequences-Demonstration | **CONSUMER_LANDED + PASSING** — `src/v3/compiler/tests/integration/r3_free_consequences_second_batch_test.rs` executes the `BinaryDimensionReportEquals` consumer for this gate. Observed reads the emitted Rust target bind structurally and folds Rust `LanguageSpec` realization rows; expected reads the source-DAG `symbolic_cost_of` cost lens and folds the corresponding LanguageSpec rows via `SymbolicCost::sequential`. | cost-lens reading structurally derived |
