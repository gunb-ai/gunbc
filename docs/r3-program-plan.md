...
| 64 | `substrate_gap_reflection_closure_closed` | substrate-gap-class | T-LensProducer-Retirement | **EXECUTABLE RECEIPT** (PR TBD — `src/v3/compiler/tests/fixtures/r3_substrate_gap_reflection_closure.dag` binds the canonical TestClaim to the live `CensusSubsetCount` lens-producer residual predicate; `r3_substrate_gap_reflection_closure_test.rs` proves the claim runs and reports the current residual count of 3. The gate turns PASSING when the residual set reaches 0.) | `lens_apply.rs` reflection via PB-Runtime |
