...
| 62 | `substrate_gap_file_ingestion_closed` | substrate-gap-class | T-Workflow-As-Data | DECLARED-with-ratified-shape-and-sub-canvas-pending (2026-05-13 per Director ratification msg_09df44c6 on canvas PR #2820 + §4.3 line 505 verbatim "T-Workflow-As-Data file-ingestion substrate (`workflow_substrate_carriers_landed` extended to file-attachment)") | `.dag` program ingests external file w/o include_str!. **Ratified shape**: workflow-substrate `FileAttachment` carrier extending gate #53 (sibling-carrier pattern with WorkflowSecret + CronExpression β-ratified via PR #2160). **Disqualified candidates** (per Director rationale): (a) compile-time `read_utf8_file` — P5-style intrinsic, doesn't match §4.3 framing, re-introduces `include_str!` semantics under new syntax (PR #2819 STAND DOWN); (c) precedent-mirror — no precedent enumerated, §4.3 already names answer; (d) A+B bundle — creates parallel-authority violating §P1. **Anti-patterns post-ratification**: compile-time `read_utf8_file`-equivalent additions, FileAttachment landed without #53 sibling-carrier alignment, legacy `include_str!` bridges preserved alongside FileAttachment (§P5 atomic-migration). **Sub-canvas pending**: Substrate Mgr authors FileAttachment carrier-shape canvas (type def + fields + workflow-context coupling) per recursive `feedback_substrate_shape_belongs_in_mgr_canvas`; worker dispatch follows sub-shape ratification. **`include_str!` audit at HEAD** (Director-verified `grep -rn "include_str!" dsl/`): no matches — gate is forward-looking (model the carrier so future file-ingestion sites have first-class workflow-substrate form), not retire-existing-side-channel. |
