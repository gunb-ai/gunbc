...
| T-LensProducer-Retirement | YELLOW | valiant-otter-715 active on **gate #5** (`lens_apply.rs` retired); eager-koi-853 R3 lane tracking under warm-dove-618 PB Mgr; PR #2471 PB Item 5 follow-on brief 8-shim batch retirement merged 2026-05-10 | T-FixedPoint + R2-Evaluator (gate #6 `lens_testgen_dot_rs_retired` same gate-cascade as #5; gate (iii) `regen_lens.rs` retired distinct on PB-1 bin-shim emit pattern) | (TBD; gate #5 in flight) |
