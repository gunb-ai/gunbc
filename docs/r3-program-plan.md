...
| 87 | `lens_cementing_test_discipline_complete` | state-check | T-Tests-As-Data-Completeness | **CONSUMER_LANDED** (PR #2639 — per-`regen.dag` harness inventory + `t_pb_b_1_dag_runner_test::r3_gate_87_cementing_regen_lens_suites_pass_through_runner`; Lane-E merge-sort `DifferentialEquals` + `SymbolicCostExprEquals` smoke; PR #2757 replaces behavior-bearing regen `Compiles` placeholders with named `LensOutputEquals` Int-projection receipts where strict modules cannot freeze full lens carriers; helper-only rows remain explicit temporary `Compiles` placeholders with per-file dissolution triggers and paired Rust receipts) | **§Acceptance (canonical):** every `.dag` lens has cementing test **against frozen v2-oracle on same source** (`r3-structure.md` §Acceptance). **Ledger:** not yet promoted to PASSING until review/CI accepts the full per-lens frozen-oracle / `LensOutputEquals` parity receipt set per that acceptance (§1.7 slice-vs-corpus rule). |
...
| T-Tests-As-Data-Completeness | YELLOW | `r3-v-tests-as-data-*` briefs; **PR #2287 Verification V1 (TC1 first slice) MERGED 2026-05-10**; **§1.8 gate #87 CONSUMER_LANDED (PR #2639 partial harness + runner — not §Acceptance PASSING)** | TestClaim infra (R1 DB-15 schema landed); T-LBP COMPLETE for executable lens cementing | (TBD) |
