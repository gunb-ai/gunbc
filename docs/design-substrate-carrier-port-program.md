...
**Status:** Execution started. Lane **E-T** landed 2026-04-24 (PR #682). Lanes **E-C** and **E-I** have landed their **carrier surfaces** in `src/v3/std/{computation,induction}.dag` (and E-T in `termination.dag`), bootstrap mirrors of `dsl/std/*`. **E-C** integration also tracks `e-c-branch` merges of `main`. **E-M is closed via M-b structural subsumption**: v3 does not need a ported `MethodSemantics` carrier. **E-P has landed its first P-c producer slice** (`v3_compiler::dag::per_call_descent_evidence`, implemented in `src/v3/compiler/src/dag.rs`, PR #742) without widening `TransformNode`; remaining work is cost/complexity consumer wiring, same-source v2/v3 cementing, and the non-carrier grammar/emit items the lens register lists for full `BEHAVIORALLY COMPLETE` — not staging the E-I types themselves.
**Scope:** Program-of-work scoping for porting v2's `DescentEvidence` / `CallPattern` / `SubValueRelation` carrier families into the v3 substrate, and for deciding whether `MethodSemantics` ports or dissolves structurally, so that v3 lenses over termination, cost, and method dispatch can reach `BEHAVIORALLY COMPLETE` per `docs/v3-lens-capability-register.md`.
**Remaining scope:** no lens migrations in E-T; remaining carrier/producer work stays in separate lane placeholders below.

## 1. Why this document exists

The 2026-04-21 v3 lens capability audit (`docs/v3-lens-capability-register.md`) identified a common root cause for lenses sitting at `PROXY` / `STUB`: v2's analyses consume termination, computation, and induction carriers from `dsl/std/termination.dag`, `dsl/std/computation.dag`, `dsl/std/induction.dag`, plus `MethodSemantics` from `src/v2/00_core.dag`. v3 now mirrors the first three families on `src/v3/std/termination.dag`, `src/v3/std/computation.dag`, and `src/v3/std/induction.dag` (E-T / E-C / E-I). **`src/v3/std/substrate.dag` still does not attach per-call evidence** the way v2's `ExprCall` does; E-P's first slice derives that evidence as a side table instead. **E-M resolved that `MethodSemantics` is not a v3 carrier gap**: method-like dispatch facts are already structural in `TransformTarget` and typed declaration/effect metadata. Cost/complexity lenses still cannot match v2's symbolic bounds **behaviorally** until they consume E-P evidence and the non-carrier register blockers land, even though the E-I **vocabulary** is present.
...
- `SizeBound = CollectionSize | TreeSize | ArithmeticParam | ExplicitCountZero | ExplicitCountPositive | Forever`.
...
- **Shape.** `CallPattern` is a flat coproduct; amount-carrying variants reuse `std.termination::PositiveDescentAmount` and `ProportionalDivisor` (Peano-style witnesses), mirroring `DescentSource::ListShrink` / `ArithmeticSubtractDescent` / `ArithmeticDivideDescent`, so non-positive steps and divide-by-one are unrepresentable. `SizeBound` is a 6-variant flat coproduct where explicit counts are split into `ExplicitCountZero` and `ExplicitCountPositive { steps: PositiveDescentAmount }`, so negative repeat counts are uninhabited. `IterationPrimitive` 3-variant pure enum. `LoweringTarget` bundles `DescentEvidence` + optional `ShrinkFactor` (`ConstantShrink` / `ProportionalShrink` use the same Peano carriers). `IterationDimension` 3-variant pure enum. `lower_call_pattern` is a pure total function — one match, no recursion.
...
- **Decision.** **M-b structural subsumption.** v3's structural-resolution model already replaces the v2 side carrier. `PlainMethodSemantics` maps to ordinary `TransformTarget::Callable(DeclarationId)` or `TransformTarget::FieldProject { field_label, field_child }`; `AlgebraMethodSemantics.method_def` maps to the resolved callable/algebra declaration (or the temporary `TransformTarget::Operator(OperatorKind)` scaffold plus infer-time algebra walk); `AlgebraMethodSemantics.fold_accumulator_type` maps to resolved callable/lambda `Arrow { inputs, output, body }` signatures plus `BindNode.params` / port states and callable template substitution; `size_effect` / `cost_shape` / `algebra_template` remain facts on algebra declarations; `ServiceMethodSemantics` maps to typed service/effect declarations and operation metadata, not an expression-side method tag.
- **Receipt.** No `src/v3/std/method_semantics.dag`, no Rust mirror, and no `ExprMethodCall` attachment are added. A port would duplicate declaration-graph facts and create a second authority. The register now records the M-b receipt and keeps cost/complexity at **PROXY** for E-P producer wiring plus the separate non-carrier blockers.
- **Lane size.** Closed by design receipt.
...
5. **M** (`MethodSemantics`) — **closed via M-b structural subsumption.** v3 structural resolution carries method-dispatch facts through `TransformTarget` plus typed declaration/effect metadata, so no carrier crosses over. This clears the **method carrier-parity** portion of `cost.dag` / `complexity.dag`'s drop-lists. Full `BEHAVIORALLY COMPLETE` for `cost.dag` also requires E-P and the non-carrier blockers the register records (`Dimension<SymbolicCost>` wiring on grammar/data-body gaps; named `SizeVariable` value semantics) — those are grammar/emit work, outside this program.
...
| I | types ported; no lens row moves yet — producer/consumer path still missing at that point | PROXY still (pending E-P) | **Landed:** `src/v3/std/induction.dag` bootstraps `SubValueRelation`, `InductiveField`, `RecursionShape`, `CostBound` (+ lattice + master-theorem surface); `m2_substrate_inhabitance_test` and `e_i_lane_induction_preflight_test` pin the carrier shape. Lens rows still wait on **E-P** side-table evidence plus cost/complexity consumer wiring. |
| P | `complexity.dag` / `cost.dag` advance on the per-call evidence slice; "What v2 has that v3 drops" column shrinks to grammar/emit and output-shape items | PROXY still (pending consumer wiring, same-source cementing, non-carrier blockers, and analysis-porting) | **Partial landed:** `v3_compiler::dag::per_call_descent_evidence` side table (P-c) emits fail-closed per-call `SubValueRelation` rows for the first recursive self-call / arithmetic-descent slice. **Pending:** v2-oracle-vs-v3 per-call descent-evidence golden for non-method-dispatch inputs |
| M | **Closed via M-b:** v3 structural subsumption replaces the v2 `MethodSemantics` carrier (`TransformTarget::{Callable, FieldProject, Operator}` plus typed declaration/effect facts). No carrier or mirror added. Full `BEHAVIORALLY COMPLETE` still requires `cost.dag`'s separate non-carrier blockers to clear (`Dimension<SymbolicCost>` wiring deferred on grammar/data-body gaps per the register; named `SizeVariable` with v2's value semantics). Those live outside this program. | method carrier-parity column goes to N/A; the remaining `cost.dag` drop-list lives on after E-M | design receipt in this doc + `docs/v3-lens-capability-register.md` |
...
- **Acceptance:** carrier types (`SubValueRelation`, `InductiveField`, `RecursionShape`, `CostBound` + supporting algebra) parse/lower/emit in v3 with round-trip tests — **met for the staged std module.** **No lens row moves on E-I alone** — until E-P's side-table producer and later cost/complexity consumer wiring, `cost.dag` / `complexity.dag` cannot read per-call `SubValueRelation` from v3 IR and their drop-lists do not shrink. The v2 complexity lens depends on `ExprCall.descent_evidence` (`src/v2/00_core.dag:199`), which is an E-P deliverable, not an E-I deliverable. Drop-list shrinkage, lens advance on the non-method-dispatch slice, and **behavioral** carrier parity start at E-P and finish when the lenses consume those facts. Full `BEHAVIORALLY COMPLETE` also depends on E-M (method-dispatch) and the separate grammar/emit non-carrier blockers.
...
- **Context:** v2 stores per-call witness facts as `descent_evidence: List<SubValueRelation>?` directly on `ExprCall` (`src/v2/00_core.dag:199`), populated by the v2 complexity/lookup passes. v3's call-site node (`TransformNode`, `src/v3/std/substrate.dag:264-270`) has no analogous attachment. **E-I has landed the carrier types** in `std.induction`; **E-P's first slice now makes those types live at call sites through a derived side table**, not a substrate attachment.
- **Status:** **Partial P-c receipt landed (PR #742).** The first E-P patch keeps `TransformNode` unchanged and adds a side-table producer (`v3_compiler::dag::per_call_descent_evidence`, implemented in `src/v3/compiler/src/dag.rs`) over the native v3 DAG. The side table emits `SubValueRelation` mirrors for provable recursive self-call arguments and emits `SubValueUnknown` for unproven callable edges, including resolved non-self calls, so absence of an entry does not masquerade as "no recursive edge." This is a first narrow slice, not full v2 `ExprCall.descent_evidence` parity. Parameter names are currently ordinal scaffolds (`param_0`, …) because `BindNode` reflects parameter ports but not parameter names; dissolve that when parameter-name refs or structural `ParamRef` evidence is available. The first arithmetic slice intentionally recognizes only the existing left-operand descent convention (`param - k`, `param / k`); direct field projection currently fails closed until `RecursionShape` is reflectable at the producer.
...
- **Work:** **Complete.** E-M chose M-b: v3's structural-resolution model (`TransformTarget::Callable` + `FieldProject` + typed transforms, with `Operator` as the temporary algebra-operator scaffold) already carries the facts `MethodSemantics` carried in v2. No `MethodSemantics` carrier, transitive v3 std surface, Rust mirror, or `ExprMethodCall` attachment point was added.
- **Acceptance:** **Met.** The register and ROADMAP record the structural subsumption receipt. The **method carrier-parity** portion of the "What v2 has that v3 drops" column clears for `cost.dag` and `complexity.dag`. Full `BEHAVIORALLY COMPLETE` for `cost.dag` also requires the separate non-carrier blockers the register already records (`Dimension<SymbolicCost>` wiring deferred on grammar/data-body gaps; named `SizeVariable` with v2's value semantics) and E-P producer wiring — those are tracked elsewhere and are **not** in scope for E-M.
- **Sanity predicate passed:** v3 structural resolution carries all v2 `MethodSemantics` facts needed for carrier parity. If future evidence shows only partial coverage, reopen E-M as a hybrid-shape modeling discovery instead of adding an unreviewed side carrier.
...
3. **Unified `MethodContract` carrier.** One generic carrier indexed by `(algebra_id: DeclarationId, method_id: DeclarationId)` that lenses query by structural declaration identity. No substrate change; minimal std/ surface growth (one carrier total). Closest in shape to a `TemplateArgumentBinding`-style lookup.
...
**Decision:** pick **Option 3, unified `MethodContract` carrier**. The E-I/E-P evidence did not show `size_effect`, `cost_shape`, and `callback_element_position` are cleanly separable from the cost/descent path. Instead, `cost.dag` and `complexity.dag` both still name live call-site consumption of the E-C/E-I/E-P facts as their remaining behavioral gap, while the existing `AlgebraFieldTemplate` rows carry `size_effect`, `cost_shape`, and callback element position together for the same method. Splitting those facts back into lens-local tables (Option 0) would preserve the current duplicate lookup story at exactly the point the lenses need a shared method contract. Type-declaration annotations (Option 1) require substrate field-level refinement work, which is out of scope for this design-call lane. Per-algebra carriers (Option 2) avoid substrate work but multiply the same contract shape across `OrderedRingMetadata`, `PartialFunctionMetadata`, and siblings. A single `MethodContract { algebra_id: DeclarationId, method_id: DeclarationId, size_effect, cost_shape, callback_element_position }` keeps one std-level authority keyed by structural declaration identity and matches the existing template-argument lookup style without widening substrate.
