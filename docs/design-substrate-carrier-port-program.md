...
| P | `complexity.dag` / `cost.dag` advance on the per-call evidence slice; "What v2 has that v3 drops" column shrinks to grammar/emit and output-shape items | PROXY still (pending non-carrier blockers and analysis-porting) | v2-oracle-vs-v3 per-call descent-evidence golden for non-method-dispatch inputs |
