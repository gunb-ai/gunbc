...
| P | `complexity.dag` / `cost.dag` advance on the per-call evidence slice; "What v2 has that v3 drops" column shrinks to grammar/emit and output-shape items | PROXY still (pending consumer wiring, same-source cementing, non-carrier blockers, and analysis-porting) | **Partial landed:** `per_call_descent_evidence` side table (P-c) emits fail-closed per-call `SubValueRelation` rows for the first self-call slice. **Pending:** v2-oracle-vs-v3 per-call descent-evidence golden for non-method-dispatch inputs |
...
- **Status:** **Partial P-c receipt landed (PR #742).** The first E-P patch keeps `TransformNode` unchanged and adds a side-table producer (`per_call_descent_evidence`) over the native v3 DAG. The side table emits `SubValueRelation` mirrors for provable recursive self-call arguments and emits `SubValueUnknown` for unproven callable edges, including resolved non-self calls, so absence of an entry does not masquerade as "no recursive edge." Parameter names are currently ordinal scaffolds (`param_0`, …) because `BindNode` reflects parameter ports but not parameter names; dissolve that when parameter-name refs or structural `ParamRef` evidence is available. The first arithmetic slice intentionally recognizes only the existing left-operand descent convention (`param - k`, `param / k`); direct field projection currently fails closed until `RecursionShape` is reflectable at the producer.
