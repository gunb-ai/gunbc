...
**`timing_measurement_iterate` (P3 / openai-pro):** Until loop-bound measure lowering lands, **`Observed` combined with any `LoopBound` maps to `Unobserved`** — the substrate must not return an unchanged `Observed` duration while dropping the bound (that would be a plausible fabricated success). **`Unobserved` / `Ambiguous` / `Stale`** pass through unchanged at this scaffold.
...
2. **Observed-artifact identity** — the external payload is bound via **`artifact_digest: ContentHash`** (`std.types` branded digest), not a bare `String`, so “what was read” is content-addressed by construction (M9 / P2); stale checks target the same nominal digest authority.
3. **Producer / observer / prover separation** — three roles are recorded as **`WorkflowProducerId`**, **`WorkflowObserverId`**, and **`WorkflowProverId`** (distinct `std.types` branded nominals), not three undifferentiated `String`s, so provenance cannot collapse silently at the type level.
4. **Attachment time and run context** — `attached_at_ns: Nanoseconds` plus **`workflow_run_id: WorkflowRunId`** tie the fact to a concrete run; `Nanoseconds` is the same nominal coordinate used for durations (SI-nanosecond magnitude as `Nat`; signed-epoch / range refinements deferred per §1.1).
...
**Gate #55 closure (worker, typed substrate):** Reviews pinned to pre-anchor refactors (e.g. codex **`a1cce3d6`**) predated the nominal **`WorkflowObservationAnchor`** row. At this branch, **§2 invariants 1–6** are carried as **typed fields** in `src/v3/std/timing_lens.dag` (`NodeId`, `ContentHash`, distinct workflow provenance brands, `Nanoseconds`, `WorkflowRunId`) together with **`TimingObservationEntry { anchor, measurement }`** and the **`timing_lens_validate`** scaffold in §2.6; `timing_lens_substrate_carrier_test.rs` ratchets those field types. **Director / #828** still owns **Q-WAD-S2-Anchor (b)** parametric promotion (`ExternalDataAnchor<Subject, Source>`) for a second consumer — that is **not** the same debt class as “anchor shape undocumented / stringly”.
