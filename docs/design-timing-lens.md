...
`TimingMeasurement` is a closed sum: `Observed { nanoseconds } | Missing | Ambiguous | Stale`. It is intentionally **not** a bare scalar: missing or ambiguous runner evidence stays explicit in the type (fail-closed discipline; no silent defaults).
...
5. **Report states, not silent scalars** — timing outcomes use `TimingMeasurement`’s `Observed | Missing | Ambiguous | Stale` sum; consumers must branch on the state instead of coercing absent data to zero.
6. **Fail-closed on non-observed / non-valid states** — enforcement and lens application surfaces treat `Missing`, `Ambiguous`, and `Stale` as non-evidence unless a named consumer explicitly documents widening; the substrate vocabulary does not fabricate `Observed` from thin air.
