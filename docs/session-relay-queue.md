...
**Duplicate `chatgpt-review` comments:** [PR #556](https://github.com/gunb-ai/gunbc/pull/556) may show **more than one** `status:complete` issue comment with the **same** parent SHA and the same body — e.g. `fd6ef9e…` @ 2026-04-19T07:25:09Z / **08:26:30Z**; **`06b14483…`** @ **07:55:14Z** / **09:25:06Z** — still **one** ChatGPT verdict per SHA; do not double-count.
...
**Indexed automated rounds:** **ChatGPT** (per-diff) — **five** parent SHAs indexed **complete** (no pending stubs). Verdicts **differ by parent SHA** (**`fd6ef9…`** / **`e529…`**: **REQUEST_CHANGES** on global syntax authority; **`527d7fd3…`** / several earlier rounds: **APPROVE_WITH_COMMENTS**; **meta** `3fc44f…`: **SHIP_WITH_DEBT**) — reconcile on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Codex** — **two** reviews (`7c53b24f…`, `37c3d25f…`), **0** blocking each. A “+2 queued” dashboard count after bots are indexed is often **human** brief + **Checks** (or threads), or can include **duplicate** `status:complete` reposts — **dedupe** first. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).
