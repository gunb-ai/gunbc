...
**Indexed automated rounds:** **ChatGPT** (per-diff) — **six** parent SHAs **complete** + **`94a0950…` pending** (stub @ 2026-04-19T10:01:36Z — refresh [PR #556](https://github.com/gunb-ai/gunbc/pull/556) for `status:complete`). **REQUEST_CHANGES** rounds: **`fd6ef9…`**, **`e529…`**, **`1cbda4f9…`** (dual `tokenize.dag` / `syntax.dag` authority); **APPROVE_WITH_COMMENTS**: e.g. **`527d7fd3…`**, **`06b…`**, **`37c3d25…`**; **meta** `3fc44f…`: **SHIP_WITH_DEBT** — reconcile on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Human inline** + **Codex `da52ee9a…`** align with **REQUEST_CHANGES** on authority more than **meta** — resolve on GitHub. **Codex** — **`7c53b24f…`** / **`37c3d25f…`** (**0** blocking), **`da52ee9a…`** (**1** blocking). Pending queue often **`94a0950…`** + **Checks** / **inline** / dupes — **dedupe** first. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).
