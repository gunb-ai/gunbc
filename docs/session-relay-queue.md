...
**Indexed automated rounds:** **ChatGPT** (per-diff) — **five** parent SHAs **complete** + **two** distinct **`status:pending`** stubs — **`1cbda4f9…`** (@ 09:31:36Z) and **`94a0950…`** (@ **10:01:36Z**) — refresh [PR #556](https://github.com/gunb-ai/gunbc/pull/556) for each `status:complete`. Verdicts **differ by parent SHA** (**`fd6ef9…`** / **`e529…`**: **REQUEST_CHANGES** on global syntax authority; **`527d7fd3…`** / several earlier rounds: **APPROVE_WITH_COMMENTS**; **meta** `3fc44f…`: **SHIP_WITH_DEBT**) — reconcile on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Human inline** @ `tokenize.dag:~L6` (**BLOCKING** on dual authority vs `syntax.dag`) aligns with **`fd6ef9…` / `e529…` REQUEST_CHANGES** and Codex **`da52ee9a…` (1 blocking)** more than with **meta SHIP_WITH_DEBT** — resolve on GitHub. **Codex** — **three** parent SHAs: **`7c53b24f…`** / **`37c3d25f…`** (**0** blocking each), **`da52ee9a…`** (**1** blocking). A “+1 queued” / “+2 queued” count may count **one or both** pending ChatGPT stubs, **Checks**, **inline**, or duplicate bot posts — **dedupe** (**ChatGPT** reposts @ ~09:25Z; **Codex** e.g. **`da52ee9a…`** @ 09:30:30Z / **09:30:31Z**; **`7c53b24f…`** @ **09:25:23Z**) first. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).
