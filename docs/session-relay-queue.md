...
| `94a0950315bda71fcedd97ef7b412990203bbb8c` | pending | Stub @ 2026-04-19T10:01:36Z — `status:pending` — await `status:complete` on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). | [ChatGPT](https://chatgpt.com/g/g-p-69e3c70def688191bf8fa7c2cb3292ba-gunbc-review/c/69e4a7e5-528c-83ea-9163-1ffb3ee2cf84) |
...
**Same SHA, different tools:** A `codex-review` parent SHA may **equal** a `chatgpt-review` parent SHA (e.g. `37c3d25…`, or **`94a0950…`** where ChatGPT may still be `pending` while Codex has posted) because both key off the same PR head — **independent** verdicts; index **Codex** and **ChatGPT** rows separately.

**Duplicate `codex-review` comments:** [PR #556](https://github.com/gunb-ai/gunbc/pull/556) may show **multiple** issue comments with the **same** parent SHA (seconds apart or later reposts) and duplicated `<!-- [codex-review] -->` lines — e.g. `7c53b24f…` @ 08:25:10Z / 08:25:11Z / **09:25:23Z**; `37c3d25f…` @ 08:27:11Z / **08:27:12Z** / **10:24:59Z**; **`da52ee9a…`** @ **09:30:30Z** / **09:30:31Z**; **`94a0950…`** @ **10:21:33Z** / **10:21:35Z** — still **one** Codex verdict per SHA; do not double-count.
...
**`94a0950…` Codex** (first @ 2026-04-19T10:21:33Z; duplicate marker @ **10:21:35Z** — **1 blocking**; same parent SHA as **`chatgpt-review` `94a0950…`** which may still be **pending**): authority split as in table; **docs**: Codex asks to **slim `session-relay-queue.md`** to stable PR/SHA/conversation pointers — reconcile with this file’s preamble in a **docs hygiene** pass. Full text on [PR #556](https://github.com/gunb-ai/gunbc/pull/556).

**Indexed automated rounds:** **ChatGPT** (per-diff) — **six** parent SHAs **complete** + **`94a0950…` pending** (stub @ 2026-04-19T10:01:36Z — refresh [PR #556](https://github.com/gunb-ai/gunbc/pull/556) for `status:complete`). **REQUEST_CHANGES** rounds: **`fd6ef9…`**, **`e529…`**, **`1cbda4f9…`** (dual `tokenize.dag` / `syntax.dag` authority); **APPROVE_WITH_COMMENTS**: e.g. **`527d7fd3…`**, **`06b…`**, **`37c3d25…`**; **meta** `3fc44f…`: **SHIP_WITH_DEBT** — reconcile on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Human inline** (two on `tokenize.dag:~L6`, **09:30:30Z** & **10:21:34Z**) + **Codex `da52ee9a…` / `94a0950…`** align with **REQUEST_CHANGES** on authority more than **meta** — resolve on GitHub. **Codex** — **`7c53b24f…`** / **`37c3d25f…`** (**0** blocking), **`da52ee9a…`** / **`94a0950…`** (**1** blocking each). **ChatGPT `94a0950…`** may still be **pending** while Codex on the **same SHA** is **complete** — both indexed. A “+1 queued” count after indexing **human** + bots is often **only** **Checks** (or one open thread), or inflated by duplicate **Codex** posts (e.g. **`94a0950…`** @ 10:21:33Z / **10:21:35Z**) — **dedupe** dupes first. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).

**Superseded bot stubs:** For a given parent SHA, ChatGPT often posts `status:pending` first, then `status:complete` for the **same** SHA (e.g. `06b14483…`: stub @ 2026-04-19T07:31:40Z → complete @ 07:55:14Z; `37c3d25…` (per-diff): stub @ 08:01:39Z → complete @ 08:24:54Z; **`e529d136…`**: stub @ 08:31:37Z → complete @ **09:06:59Z**; **`527d7fd3…`**: stub @ 09:01:37Z → complete @ **09:25:01Z**; **`1cbda4f9…`**: stub @ 09:31:36Z → complete @ **10:06:50Z**; **`chatgpt-meta-review` `3fc44f…`**: stub @ 08:26:49Z → complete @ **08:31:06Z** on [PR #556](https://github.com/gunb-ai/gunbc/pull/556)). The table rows are the **final** outcome per SHA, not the queue at a single timestamp.
