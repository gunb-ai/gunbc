...
**Indexed automated rounds:** **ChatGPT** (per-diff) — **five** parent SHAs indexed **complete** (no pending stubs). Verdicts **differ by parent SHA** (**`fd6ef9…`** / **`e529…`**: **REQUEST_CHANGES** on global syntax authority; **`527d7fd3…`** / several earlier rounds: **APPROVE_WITH_COMMENTS**; **meta** `3fc44f…`: **SHIP_WITH_DEBT**) — reconcile on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Codex** — **two** reviews (`7c53b24f…`, `37c3d25f…`), **0** blocking each. A “+1 queued” / “+2 queued” dashboard count after bots are indexed is often **human** brief + **Checks** (or threads), or inflated by **duplicate** bot comments (**ChatGPT** `status:complete` reposts, e.g. **`06b14483…`** / **`37c3d25…`** @ ~09:25Z; **Codex** reposts, e.g. **`7c53b24f…`** @ **09:25:23Z**) — **dedupe** first. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).
