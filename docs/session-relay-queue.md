...
**Indexed automated rounds:** **ChatGPT** (per-diff) — **four** parent SHAs **complete** (incl. **`e529d136…`** @ **09:06:59Z**) + **`527d7fd3…` pending** (stub @ 09:01:37Z). **ChatGPT meta-review** (`3fc44f…`, **SHIP_WITH_DEBT** @ 08:31:06Z) and **`e529…` REQUEST_CHANGES** are different **parent SHAs** — reconcile outcomes on [PR #556](https://github.com/gunb-ai/gunbc/pull/556). **Codex** — **two** reviews (`7c53b24f…`, `37c3d25f…`), **0** blocking each. **Dedupe by parent SHA and marker kind** (`chatgpt-review` vs `chatgpt-meta-review` vs `codex-review`; same SHA may appear under **both** ChatGPT and Codex).

**Superseded bot stubs:** For a given parent SHA, ChatGPT often posts `status:pending` first, then `status:complete` for the **same** SHA (e.g. `06b14483…`: stub @ 2026-04-19T07:31:40Z → complete @ 07:55:14Z; `37c3d25…` (per-diff): stub @ 08:01:39Z → complete @ 08:24:54Z; **`e529d136…`**: stub @ 08:31:37Z → complete @ **09:06:59Z**; **`chatgpt-meta-review` `3fc44f…`**: stub @ 08:26:49Z → complete @ **08:31:06Z** on [PR #556](https://github.com/gunb-ai/gunbc/pull/556)). The table rows are the **final** outcome per SHA, not the queue at a single timestamp.
