...
use gunbc_gitignoregen::{build_gitignoregen_dag, GitignoreConfig};

#[derive(Parser, Debug)]
#[command(name = "gunbc-gitignoregen", about = "Generate a .gitignore file")]
struct Cli {
...
fn main() {
    ...
    let dag = build_gitignoregen_dag(&config, cli.dry_run);
    ...
    let mut observer = TerminalObserver::new("gitignoregen");
